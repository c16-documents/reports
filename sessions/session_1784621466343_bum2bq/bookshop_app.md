...
## 📚 Bookshop Catalog — Standalone SAPUI5 App

Save this file and open it in your browser, **OR** copy the code block below into a new `.html` file.

**How to use:** Click "View Full App Code" below → Copy all the code → Save as `bookshop.html` → Open in Chrome/Edge/Firefox

### Features

*   ✅ 12 programming books with realistic data
*   ✅ KPI cards showing inventory status
...
<head>
  ...
  <script src="https://sapui5.hana.ondemand.com/1.120.0/resources/sap-ui-core.js"
  id="sap-ui-bootstrap"
  ...
  <style>
  .kpiCard { padding: 1rem; border-radius: 8px; text-align: center; min-width: 140px; }
  .kpiCard .value { font-size: 2rem; font-weight: bold; }
  .kpiCard .label { font-size: 0.85rem; color: #666; }
  .kpiSuccess { background: #e8f5e9; border-left: 4px solid #4caf50; }
  .kpiWarning { background: #fff3e0; border-left: 4px solid #ff9800; }
  .kpiError { background: #ffebee; border-left: 4px solid #f44336; }
  .kpiNeutral { background: #e3f2fd; border-left: 4px solid #2196f3; }
  </style>
  <script>
  ...
  var oData = {
  Books: [
  {ID: "1", title: "Clean Code", author: "Robert C. Martin", genre: "Programming", price: 39.99, currency: "USD", stock: 120, rating: 4.8, status: "Available"},
  {ID: "2", title: "The Pragmatic Programmer", author: "David Thomas", genre: "Programming", price: 49.95, currency: "USD", stock: 85, rating: 4.7, status: "Available"},
  {ID: "3", title: "Design Patterns", author: "Gang of Four", genre: "Programming", price: 54.99, currency: "USD", stock: 45, rating: 4.5, status: "Low Stock"},
  {ID: "4", title: "Domain-Driven Design", author: "Eric Evans", genre: "Architecture", price: 62.00, currency: "USD", stock: 30, rating: 4.6, status: "Low Stock"},
  {ID: "5", title: "Refactoring", author: "Martin Fowler", genre: "Programming", price: 47.99, currency: "USD", stock: 95, rating: 4.7, status: "Available"},
  {ID: "6", title: "The Mythical Man-Month", author: "Frederick Brooks", genre: "Management", price: 34.99, currency: "USD", stock: 150, rating: 4.4, status: "Available"},
  {ID: "7", title: "Code Complete", author: "Steve McConnell", genre: "Programming", price: 44.99, currency: "USD", stock: 70, rating: 4.5, status: "Available"},
  {ID: "8", title: "Working Effectively with Legacy Code", author: "Michael Feathers", genre: "Programming", price: 52.00, currency: "USD", stock: 25, rating: 4.3, status: "Low Stock"},
  {ID: "9", title: "Continuous Delivery", author: "Jez Humble", genre: "DevOps", price: 49.99, currency: "USD", stock: 60, rating: 4.6, status: "Available"},
  {ID: "10", title: "Site Reliability Engineering", author: "Google SRE Team", genre: "DevOps", price: 55.00, currency: "USD", stock: 40, rating: 4.5, status: "Available"},
  {ID: "11", title: "Building Microservices", author: "Sam Newman", genre: "Architecture", price: 45.99, currency: "USD", stock: 55, rating: 4.4, status: "Available"},
  {ID: "12", title: "Release It!", author: "Michael Nygard", genre: "DevOps", price: 47.95, currency: "USD", stock: 0, rating: 4.5, status: "Out of Stock"}
  ]
  };
  ...
  // Calculate KPIs
  var totalBooks = oData.Books.length;
  var inStock = oData.Books.filter(function(b) { return b.stock > 0; }).length;
  var lowStock = oData.Books.filter(function(b) { return b.status === "Low Stock"; }).length;
  var outOfStock = oData.Books.filter(function(b) { return b.stock === 0; }).length;

  // KPI Cards
  var oKPIPanel = new sap.m.HBox({
  justifyContent: "Start",
  wrap: "Wrap",
  items: [
  new sap.ui.core.HTML({content: '<div class="kpiCard kpiNeutral"><div class="value">' + totalBooks + '</div><div class="label">Total Books</div></div>'}),
  new sap.ui.core.HTML({content: '<div class="kpiCard kpiSuccess"><div class="value">' + inStock + '</div><div class="label">In Stock</div></div>'}),
  new sap.ui.core.HTML({content: '<div class="kpiCard kpiWarning"><div class="value">' + lowStock + '</div><div class="label">Low Stock</div></div>'}),
  new sap.ui.core.HTML({content: '<div class="kpiCard kpiError"><div class="value">' + outOfStock + '</div><div class="label">Out of Stock</div></div>'})
  ]
  ...
  oTable.getBinding("items").filter(sQuery ? [oFilter] : []);
  }
  });

  // Status formatter
  var formatStatus = function(sStatus) {
  switch(sStatus) {
  ...
  content: [
  new sap.m.Title({text: "Books ({/Books/length})"}),
  new sap.m.ToolbarSpacer(),
  oSearchField
  ]
  }),
  columns: [
  new sap.m.Column({header: new sap.m.Label({text: "Title"}), width: "25%"}),
  new sap.m.Column({header: new sap.m.Label({text: "Author"}), width: "20%"}),
  new sap.m.Column({header: new sap.m.Label({text: "Genre"}), width: "12%"}),
  new sap.m.Column({header: new sap.m.Label({text: "Price"}), width: "10%", hAlign: "End"}),
  new sap.m.Column({header: new sap.m.Label({text: "Stock"}), width: "8%", hAlign: "End"}),
  new sap.m.Column({header: new sap.m.Label({text: "Rating"}), width: "10%", hAlign: "Center"}),
  new sap.m.Column({header: new sap.m.Label({text: "Status"}), width: "15%"})
  ],
  items: {
  path: "/Books",
  ...
  new sap.m.Text({text: "{genre}"}),
  new sap.m.ObjectNumber({number: "{price}", unit: "{currency}"}),
  new sap.m.Text({text: "{stock}"}),
  new sap.m.RatingIndicator({value: "{rating}", maxValue: 5, editable: false, iconSize: "0.8rem"}),
...
1.  **Download** the document from the Documents panel (right side)
2.  **Copy** the HTML code from the code block above
3.  **Save** as `bookshop.html`
4.  **Open** in your browser — the app runs instantly!

**Want the full CAP project instead?** Just say "give me the CAP files" and I'll generate the complete `db/schema.cds`, `srv/service.cds`, and Fiori Elements UI — ready to run with `cds watch`.

**C16 — AI for SAP operational efficiency**  |  Document version 1.0  |  21 July 2026
