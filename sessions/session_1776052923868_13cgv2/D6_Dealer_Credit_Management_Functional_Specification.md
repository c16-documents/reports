  D6 — Functional Specification: Dealer Credit Management Optimization @page { size: A4; margin: 20mm 15mm; } @media print { body { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; } .page-break { page-break-before: always; } .no-print { display: none; } .header-banner { -webkit-print-color-adjust: exact !important; } } \* { box-sizing: border-box; margin: 0; padding: 0; } body { font-family: 'Segoe UI', Arial, Helvetica, sans-serif; color: #2c3e50; background: #ffffff; line-height: 1.6; font-size: 11pt; } .doc-wrapper { max-width: 210mm; margin: 0 auto; padding: 0; background: #fff; } /\* Header \*/ .header-banner { background: linear-gradient(135deg, #C97B3A, #B5567A, #6B3FA0); color: #fff; padding: 30px 35px; } .header-banner h1 { font-size: 22pt; font-weight: 700; margin-bottom: 4px; } .header-banner h2 { font-size: 13pt; font-weight: 400; opacity: 0.92; margin-bottom: 8px; } .header-banner .doc-id { font-size: 9pt; opacity: 0.8; } .logo-row { display: flex; align-items: center; gap: 12px; margin-bottom: 10px; } .logo-row img { height: 40px; border-radius: 6px; } /\* Meta bar \*/ .meta-bar { background: #f8f9fa; border-bottom: 2px solid #6B3FA0; padding: 12px 35px; display: flex; flex-wrap: wrap; gap: 25px; font-size: 9pt; color: #555; } .meta-item { display: flex; flex-direction: column; } .meta-label { font-weight: 700; color: #6B3FA0; text-transform: uppercase; font-size: 7pt; letter-spacing: 0.5px; } /\* Content \*/ .content { padding: 25px 35px; } /\* Section headers \*/ .section-num { display: inline-block; background: #6B3FA0; color: #fff; width: 28px; height: 28px; border-radius: 50%; text-align: center; line-height: 28px; font-size: 13pt; font-weight: 700; margin-right: 10px; vertical-align: middle; } h2.section-title { color: #6B3FA0; font-size: 16pt; border-bottom: 3px solid #B5567A; padding-bottom: 8px; margin: 30px 0 15px 0; } h3 { color: #4a2873; font-size: 12pt; margin: 18px 0 8px 0; border-left: 4px solid #B5567A; padding-left: 10px; } h4 { color: #6B3FA0; font-size: 10.5pt; margin: 12px 0 6px 0; } /\* Tables \*/ table { width: 100%; border-collapse: collapse; margin: 10px 0 18px 0; font-size: 9.5pt; } thead th { background: #6B3FA0; color: #fff; padding: 8px 10px; text-align: left; font-weight: 600; font-size: 9pt; } tbody td { padding: 7px 10px; border-bottom: 1px solid #e2e8f0; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } /\* Status badges \*/ .badge-critical { background: #e74c3c; color: #fff; padding: 2px 8px; border-radius: 10px; font-size: 8pt; font-weight: 600; } .badge-warning { background: #e67e22; color: #fff; padding: 2px 8px; border-radius: 10px; font-size: 8pt; font-weight: 600; } .badge-ok { background: #27ae60; color: #fff; padding: 2px 8px; border-radius: 10px; font-size: 8pt; font-weight: 600; } .badge-info { background: #3498db; color: #fff; padding: 2px 8px; border-radius: 10px; font-size: 8pt; font-weight: 600; } .badge-new { background: #8e44ad; color: #fff; padding: 2px 8px; border-radius: 10px; font-size: 8pt; font-weight: 600; } /\* Callout boxes \*/ .callout { border-left: 5px solid #6B3FA0; background: #f5f0fa; padding: 12px 15px; margin: 12px 0; border-radius: 0 6px 6px 0; font-size: 9.5pt; } .callout-green { border-left-color: #27ae60; background: #f0faf4; } .callout-red { border-left-color: #e74c3c; background: #fdf2f2; } .callout-blue { border-left-color: #3498db; background: #f0f7fd; } /\* Process flow boxes \*/ .process-step { display: inline-block; background: #6B3FA0; color: #fff; padding: 6px 14px; border-radius: 20px; margin: 3px 4px; font-size: 9pt; } .process-arrow { display: inline-block; color: #B5567A; font-size: 14pt; margin: 0 2px; vertical-align: middle; } /\* Code blocks \*/ pre { background: #f5f5f5; border-left: 4px solid #B5567A; padding: 12px 15px; margin: 10px 0; font-size: 9pt; overflow-x: auto; border-radius: 0 4px 4px 0; font-family: 'Consolas', 'Courier New', monospace; } code { font-family: 'Consolas', 'Courier New', monospace; background: #f0f0f0; padding: 1px 4px; border-radius: 3px; font-size: 9pt; } /\* TOC \*/ .toc { background: #f8f9fa; border: 1px solid #e2e8f0; border-radius: 6px; padding: 15px 20px; margin: 15px 0; } .toc-title { font-weight: 700; color: #6B3FA0; font-size: 11pt; margin-bottom: 8px; } .toc a { color: #4a2873; text-decoration: none; font-size: 9.5pt; } .toc a:hover { text-decoration: underline; } .toc ol { padding-left: 20px; } .toc li { margin: 3px 0; } /\* Decision matrix \*/ .decision-yes { color: #27ae60; font-weight: 700; } .decision-no { color: #e74c3c; font-weight: 700; } .decision-manual { color: #e67e22; font-weight: 700; } /\* Footer \*/ .doc-footer { background: #f8f9fa; border-top: 2px solid #6B3FA0; padding: 15px 35px; font-size: 8pt; color: #888; margin-top: 30px; } /\* Field mapping table \*/ .field-map td:first-child { font-family: 'Consolas', monospace; font-size: 8.5pt; color: #6B3FA0; } /\* Mermaid \*/ .mermaid { margin: 15px 0; } /\* Watermark \*/ .watermark { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%) rotate(-45deg); font-size: 80pt; color: rgba(107, 63, 160, 0.04); font-weight: 900; z-index: -1; pointer-events: none; } mermaid.initialize({startOnLoad:true, theme:'base', themeVariables:{primaryColor:'#6B3FA0', primaryTextColor:'#fff', lineColor:'#B5567A', secondaryColor:'#f5f0fa'}});

CONFIDENTIAL

![C16](data:image/jpeg;base64,) C16 Process Intelligence

# Functional Specification

## Dealer Credit Management — Approval Workflow Optimization

Document ID: C16-FS-DCM-001  |  Version: 1.0  |  Classification: Confidential  |  Date: 25 April 2026

ClientMahindra & Mahindra Ltd.

ModuleSD / FI-AR / Credit Management

SystemS/4HANA 2023 (Target: 2025)

Use CaseDealer Credit Approval TAT Optimization

ComplexityComplex (Cross-Module)

Prepared ByC16 Process Intelligence Engine

📑 Table of Contents

1.  [Document Control](#sec1)
2.  [Business Context & Problem Statement](#sec2)
3.  [Scope & Boundaries](#sec3)
4.  [AS-IS Process Flow](#sec4)
5.  [AS-IS Data Model](#sec5)
6.  [Gap Analysis Summary](#sec6)
7.  [TO-BE Process Flow](#sec7)
8.  [TO-BE Data Model](#sec8)
9.  [Credit Scoring Algorithm](#sec9)
10.  [Auto-Release Decision Matrix](#sec10)
11.  [Configuration Specification](#sec11)
12.  [Enhancement / BAdI Specification](#sec12)
13.  [Data Migration & Enrichment Plan](#sec13)
14.  [Dashboard & Reporting Wireframes](#sec14)
15.  [Integration Points](#sec15)
16.  [Authorization Concept](#sec16)
17.  [Test Scenarios](#sec17)
18.  [Cutover & Go-Live Checklist](#sec18)

## 1 Document Control

Attribute

Value

**Document Title**

Functional Specification — Dealer Credit Management Optimization

**Document ID**

C16-FS-DCM-001

**Version**

1.0

**Status**

Draft — Pending M&M Review

**Prepared By**

C16 Process Intelligence Engine

**Prepared For**

Sanjay Chavan, CTO — Mahindra & Mahindra Ltd.

**Date**

25 April 2026

**Validity Period**

6 months from date of issue

### Change History

Version

Date

Author

Changes

1.0

25-Apr-2026

C16 Engine

Initial draft — generated from live S/4HANA data analysis

### Review & Approval

Role

Name

Date

Signature

CTO / Business Sponsor

_\[Sanjay Chavan\]_

Credit Management Head

_\[To be assigned\]_

IT / SAP Basis Lead

_\[To be assigned\]_

Solution Architect

_\[To be assigned\]_

## 2 Business Context & Problem Statement

### 2.1 Background

Mahindra & Mahindra operates a large dealer network across multiple vehicle segments (Auto, Farm, Commercial). Each dealer requires a credit limit in SAP to place orders. The credit management process involves credit analysts reviewing dealer financial health and credit managers approving or adjusting credit limits.

### 2.2 Problem Statement

**Core Issue:** Credit managers are taking excessive time to approve dealer credit limits. This results in sales orders being blocked, delayed deliveries, and dealer dissatisfaction. Root cause analysis (from D2/D3 deliverables) reveals that **insufficient data availability at the time of credit decision** is the primary bottleneck.

### 2.3 Current State Evidence (from SAP)

Metric

Current Value

Evidence Source

Customers with credit limits (KNKK)

50+ records across 3 credit control areas

KNKK table — KKBER: 1000, 2222, 3000

Credit control areas active

3 (1000, 2222, 3000)

KNKK.KKBER distinct values

Credit limits range

511.29 to 511,291,880.68

KNKK.KLIMK min/max

Customers with credit block (CRBLB = X)

5 customers (1330, 1340, 1350, 1380, 1390)

KNKK.CRBLB = 'X'

Customers with zero credit limit

2 customers (1380, 1390) — blocked with ₹0 limit

KNKK.KLIMK = 0.00

Risk category (CTLPC) populated

Only 8 of 50 records (16%)

KNKK.CTLPC — 84% blank

Credit review date (DTREV) populated

Only 3 of 50 records (6%)

KNKK.DTREV — 94% are 00000000

Next review date (NXTRV) populated

Only 3 of 50 records (6%)

KNKK.NXTRV — 94% blank

Credit representative group (GRUPP)

28 of 50 populated (56%)

KNKK.GRUPP — groups 1100, 1200, 1500, 1600

Sales orders blocked for delivery

27 orders with blocks (LIFSK ≠ '')

VBAK.LIFSK — codes 01, 02, 05, 07, 08, 42, 91, 97, 98

Highest blocked order value

USD 75,000,000.00

VBAK SO 5003063, customer 3271

Tax number (STCEG) completeness

Only 1 of 50 customers has STCEG populated (2%)

KNA1.STCEG — 98% blank

Open receivables (BSID)

100+ line items totaling millions (USD, EUR)

BSID — multiple company codes

Payment terms (ZBD1T) on open items

88% have ZBD1T = 0 (immediate / not configured)

BSID.ZBD1T

### 2.4 Business Impact

Impact Area

Description

Estimated Annual Cost

**Revenue Delay**

27 blocked sales orders — revenue cannot be recognized until block is released

Opportunity cost of delayed billing

**Dealer Dissatisfaction**

Dealers wait days for credit limit approvals, escalate to management

Relationship risk

**Manual Effort**

Credit managers manually chase financials via phone/email for each request

FTE hours wasted

**Bad Debt Risk**

Without risk categories (84% missing), no systematic risk assessment

Potential bad debt exposure

## 3 Scope & Boundaries

### 3.1 In Scope

#

Area

Description

S1

Credit limit approval workflow

End-to-end process from credit limit request to approval/rejection

S2

Credit scoring model

Automated scoring based on payment history, utilization, order frequency

S3

Auto-release of credit blocks

Rule-based auto-release when limit change resolves the block

S4

Data completeness enforcement

Mandatory fields before credit limit request can be submitted

S5

Credit monitoring dashboard

Real-time KPIs for credit exposure, utilization, blocked orders

S6

Dynamic credit limits

Seasonal adjustment based on historical ordering patterns

S7

Configuration optimization

OVA8, credit groups, risk categories, representative groups

### 3.2 Out of Scope

#

Area

Reason

O1

Dunning / Collections

Separate process — covered by FI-AR dunning module

O2

Legal recovery

Post-write-off process — outside credit management

O3

External credit rating integration

Phase 2 — requires third-party API integration

O4

Insurance / hedging

Treasury function — separate project

## 4 AS-IS Process Flow

### 4.1 Current Credit Approval Workflow

flowchart TD A\[Dealer requests  
credit limit\] --> B\[Sales team creates  
request in SAP\] B --> C{Credit Analyst  
reviews} C -->|Data complete| D\[Forward to  
Credit Manager\] C -->|Data missing| E\[Request dealer  
financials offline\] E -->|Days pass...| F\[Dealer sends  
docs via email\] F --> C D --> G{Credit Manager  
approves?} G -->|Approved| H\[Update KNKK  
credit limit\] G -->|Rejected| I\[Send back with  
comments\] I --> B H --> J\[Release blocked  
orders manually\] J --> K\[End\] style A fill:#e74c3c,color:#fff style E fill:#e67e22,color:#fff style F fill:#e67e22,color:#fff style H fill:#27ae60,color:#fff style K fill:#6B3FA0,color:#fff

### 4.2 Current Process Pain Points

#

Pain Point

Root Cause

Impact

P1

Credit analyst lacks dealer financial data

KNKK has 84% blank risk category, 94% blank review dates

Analyst must chase data offline — adds 3-5 days

P2

No automated credit scoring

Every request is a manual judgment call

Inconsistent decisions, manager bottleneck

P3

Credit blocks not auto-released

After limit increase, blocked orders need manual VKM1 release

4+ hours average release delay

P4

No proactive review cadence

94% of records have no next review date (NXTRV = 00000000)

Limits go stale — sudden blocks during peak season

P5

Tax compliance data missing

98% of KNA1 records have blank STCEG (tax number)

GST compliance risk for Indian operations

P6

Payment terms not configured

88% of BSID items have ZBD1T = 0 (immediate payment)

Cannot calculate true overdue aging

## 5 AS-IS Data Model

### 5.1 Entity Relationship

erDiagram KNA1 ||--o{ KNB1 : "has company code" KNA1 ||--o{ KNVV : "has sales area" KNA1 ||--o{ KNKK : "has credit segment" KNA1 ||--o{ BSID : "has open items" KNA1 ||--o{ BSAD : "has cleared items" KNA1 ||--o{ VBAK : "has sales orders" KNA1 { string KUNNR PK "Customer Number" string NAME1 "Name" string LAND1 "Country" string STCEG "Tax Number" string KTOKD "Account Group" string SPERR "Central Block" } KNKK { string KUNNR PK "Customer" string KKBER PK "Credit Control Area" decimal KLIMK "Credit Limit" string CTLPC "Risk Category" date DTREV "Last Review" string CRBLB "Credit Block" string GRUPP "Credit Group" } KNB1 { string KUNNR PK "Customer" string BUKRS PK "Company Code" string AKONT "Recon Account" string ZTERM "Payment Terms" string ZWELS "Payment Methods" } BSID { string KUNNR "Customer" string BELNR "Document" date BUDAT "Posting Date" decimal DMBTR "Amount" string ZBD1T "Payment Terms Days" } VBAK { string VBELN PK "Sales Order" string KUNNR "Customer" string LIFSK "Delivery Block" decimal NETWR "Net Value" }

### 5.2 Key Fields — Current Population Status

Table

Field

Description

Data Element

Population %

Status

KNKK

KLIMK

Credit Limit

KLIMK

96%

OK

KNKK

CTLPC

Risk Category

CTLPC\_CM

16%

CRITICAL

KNKK

DTREV

Last Review Date

DTREV\_CM

6%

CRITICAL

KNKK

NXTRV

Next Review Date

NXTRV\_CM

6%

CRITICAL

KNKK

GRUPP

Credit Rep. Group

GRUPP\_CM

56%

WARNING

KNKK

CRBLB

Credit Block

CRBLB\_CM

10% have X

INFO

KNKK

SBGRP

Credit Sub-Group

SBGRP\_CM

56%

WARNING

KNA1

STCEG

Tax Number (GST)

STCEG

2%

CRITICAL

KNA1

NAME1

Customer Name

NAME1\_GP

100%

OK

KNA1

LAND1

Country

LAND1\_GP

100%

OK

KNB1

ZTERM

Payment Terms

DZTERM

~40%

WARNING

KNB1

ZWELS

Payment Methods

DZWELS

~30%

CRITICAL

BSID

ZBD1T

Payment Terms Days

—

12% non-zero

CRITICAL

## 6 Gap Analysis Summary

#

Gap

Current State

Required State

Severity

G1

**No credit scoring model**

100% manual assessment

Automated score (0-100) with band classification

CRITICAL

G2

**Risk category not maintained**

84% blank CTLPC

100% populated based on auto-scoring

CRITICAL

G3

**No review cadence**

94% blank DTREV/NXTRV

Quarterly auto-review for all dealers

CRITICAL

G4

**Tax data missing**

98% blank STCEG

Mandatory GST number for Indian dealers

CRITICAL

G5

**No auto-release**

Manual VKM1 after every limit change

Rule-based auto-release within minutes

HIGH

G6

**Payment terms inconsistent**

88% ZBD1T = 0

Proper payment terms for accurate aging

HIGH

G7

**No credit dashboard**

Manual Excel reports

Real-time SAP dashboard with alerts

MEDIUM

G8

**Credit groups incomplete**

44% blank GRUPP

All dealers assigned to credit representative group

MEDIUM

## 7 TO-BE Process Flow

### 7.1 Redesigned Credit Approval Workflow

flowchart TD A\[Dealer requests  
credit limit\] --> B{Data  
completeness  
check} B -->|Incomplete| C\[Block request —  
notify dealer of  
missing fields\] C --> A B -->|Complete| D\[Auto Credit  
Scoring Engine\] D --> E{Score  
Band?} E -->|A: 80-100| F\[Auto-Approve ✅  
Update KNKK\] E -->|B: 60-79| G\[Credit Analyst  
Review\] E -->|C: 40-59| H\[Credit Manager  
Review\] E -->|D: 0-39| I\[Credit Head  
Review + Docs\] G -->|Approve| F G -->|Escalate| H H -->|Approve| F H -->|Reject| J\[Reject with  
documented reason\] I -->|Approve| F I -->|Reject| J F --> K{Blocked  
orders  
exist?} K -->|Yes| L\[Auto-Release  
Engine\] K -->|No| M\[End\] L --> M style F fill:#27ae60,color:#fff style J fill:#e74c3c,color:#fff style D fill:#6B3FA0,color:#fff style L fill:#3498db,color:#fff

### 7.2 Key Design Decisions

Decision

Choice

Rationale

Scoring frequency

Real-time on request + quarterly batch

Real-time for new requests; batch for proactive review

Auto-approval threshold

Score ≥ 80 AND renewal (not new dealer)

New dealers always need human review

Auto-release scope

Only if new exposure ≤ 90% of new limit

Safety margin — don't release if already near limit

Exception routing

Based on score band + limit amount

Higher amounts need senior approval regardless of score

## 8 TO-BE Data Model

### 8.1 New / Enhanced Fields

Table

Field

Type

Length

Description

Status

KNKK

CTLPC

CHAR

3

Risk Category — populate from scoring engine

EXISTING — Enrich

KNKK

DTREV

DATS

8

Last Review Date — auto-update on scoring run

EXISTING — Enrich

KNKK

NXTRV

DATS

8

Next Review Date — auto-set to DTREV + 90 days

EXISTING — Enrich

KNKK

GRUPP

CHAR

4

Credit Representative Group — assign by region

EXISTING — Enrich

ZCREDIT\_SCORE

KUNNR

CHAR

10

Customer Number

NEW TABLE

ZCREDIT\_SCORE

KKBER

CHAR

4

Credit Control Area

NEW

ZCREDIT\_SCORE

SCORE

DEC

5,2

Credit Score (0.00 — 100.00)

NEW

ZCREDIT\_SCORE

BAND

CHAR

1

Score Band (A/B/C/D)

NEW

ZCREDIT\_SCORE

CALC\_DATE

DATS

8

Last Calculation Date

NEW

ZCREDIT\_SCORE

PAY\_SCORE

DEC

5,2

Payment History Sub-Score

NEW

ZCREDIT\_SCORE

UTIL\_SCORE

DEC

5,2

Utilization Sub-Score

NEW

ZCREDIT\_SCORE

FREQ\_SCORE

DEC

5,2

Order Frequency Sub-Score

NEW

ZCREDIT\_SCORE

ODUE\_SCORE

DEC

5,2

Overdue Percentage Sub-Score

NEW

**Design Note:** A custom table ZCREDIT\_SCORE is recommended rather than appending standard tables, following Clean Core principles. This allows independent lifecycle management and avoids upgrade conflicts.

## 9 Credit Scoring Algorithm

### 9.1 Scoring Formula

**Credit Score = (Payment History × 0.40) + (Utilization × 0.25) + (Order Frequency × 0.20) + (Overdue Inverse × 0.15)**  
Range: 0 — 100 | Bands: A (80-100), B (60-79), C (40-59), D (0-39)

### 9.2 Sub-Score Calculations

#### 9.2.1 Payment History Score (Weight: 40%)

Data Source

Calculation

Score Range

BSAD (cleared items)

Average days to pay = AVG(AUGDT - ZFBDT) for last 12 months

0-100

Mapping

≤ terms: 100 | 1-15 days late: 80 | 16-30 late: 60 | 31-60 late: 40 | 60+ late: 20 | No history: 50

**Sample from system:** Customer 1000 (BSAD) shows BUDAT 19940323 → AUGDT 19940623 = 92 days. Customer 1050 shows BUDAT 20060504 → AUGDT 20060509 = 5 days. The engine calculates per-customer averages across all cleared items.

#### 9.2.2 Credit Utilization Score (Weight: 25%)

Data Source

Calculation

Score Range

KNKK.KLIMK (limit) vs KNKK.SKFOR (exposure)

Utilization % = SKFOR / KLIMK × 100

0-100

Mapping

≤50%: 100 | 51-70%: 80 | 71-85%: 60 | 86-95%: 40 | 96-100%: 20 | >100%: 0

#### 9.2.3 Order Frequency Score (Weight: 20%)

Data Source

Calculation

Score Range

VBAK (sales orders last 12 months)

Count of orders per customer

0-100

Mapping

≥12 orders/year: 100 | 6-11: 80 | 3-5: 60 | 1-2: 40 | 0: 20

#### 9.2.4 Overdue Inverse Score (Weight: 15%)

Data Source

Calculation

Score Range

BSID (open items)

Overdue % = COUNT(overdue items) / COUNT(all open items) × 100

0-100

Mapping

0% overdue: 100 | 1-10%: 80 | 11-25%: 60 | 26-50%: 40 | >50%: 20

### 9.3 Band Classification & Action Matrix

Band

Score

Risk Level

CTLPC Value

Approval Action

A

80 — 100

Low Risk

001

Auto-approve renewals. New limits → Analyst review only.

B

60 — 79

Moderate Risk

002

Credit Analyst reviews. Approve up to 120% of current limit.

C

40 — 59

Elevated Risk

003

Credit Manager reviews. Limit increase capped at 10%.

D

0 — 39

High Risk

100

Credit Head reviews. Mandatory documentation required.

## 10 Auto-Release Decision Matrix

### 10.1 Decision Rules

Condition

Value

Auto-Release?

Credit limit was INCREASED

AND

✅ YES — Auto-Release

New exposure ≤ 90% of new limit

AND

No payment block on customer (KNB1.ZAHLS = blank)

Credit limit was DECREASED

❌ NO — Manual Review

New exposure > 90% of new limit

⚠️ Manual — still near limit

Customer has payment block (KNB1.ZAHLS ≠ blank)

❌ NO — blocked for payment

Delivery block code = 97 or 98 (not credit-related)

❌ NO — block is not credit-related

### 10.2 Auto-Release Process Flow

flowchart LR A\[KNKK.KLIMK  
changed\] --> B{Limit  
increased?} B -->|No| C\[Exit — Manual\] B -->|Yes| D{Exposure  
≤ 90% of  
new limit?} D -->|No| C D -->|Yes| E{Payment  
block?} E -->|Yes| C E -->|No| F\[Read VBAK  
where LIFSK  
IN 01,07,08\] F --> G\[Release each  
blocked order\] G --> H\[Log release  
in ZCREDIT\_LOG\] style F fill:#3498db,color:#fff style G fill:#27ae60,color:#fff style C fill:#e74c3c,color:#fff

### 10.3 Simulation Against Current Data

Based on the 27 currently blocked orders in VBAK:

Block Code

Count

Auto-Release Eligible?

Notes

01 (General delivery block)

9

✅ If credit-related

Check if block set by credit check

07 (Credit block)

4

✅ Yes

Primary target for auto-release

08 (Confirm. of Order Required)

2

❌ No

Not credit-related

91 (Special block)

4

⚠️ Review

Custom block — check business rules

97 (WM block)

5

❌ No

Warehouse management block

98 (PP block)

1

❌ No

Production planning block

Others (02, 05, 42)

2

❌ No

Not credit-related

**Result:** Of 27 blocked orders, approximately **13 (48%)** are potentially auto-releasable (codes 01 and 07). This would immediately unlock revenue from orders worth millions in blocked value.

## 11 Configuration Specification

### 11.1 Credit Control Area (OB45)

SPRO Path

Current

Recommended

Enterprise Structure → Financial Accounting → Define Credit Control Area

3 areas: 1000, 2222, 3000

Consolidate to 2 (domestic + export) if possible

### 11.2 Automatic Credit Control (OVA8)

Setting

Recommended Value

Rationale

Credit check on order type TA

Static + Dynamic check

Check limit + open items + open deliveries

Reaction code

A (warning) for Band A/B; B (error/block) for Band C/D

Low risk gets through; high risk gets blocked

Status group

Credit group 001 = Block delivery; 002 = Block order

Tiered response based on risk

### 11.3 Risk Categories (T691E / OB01)

Risk Cat.

Description

Scoring Band

Dynamic Check Factor

001

Low Risk

A (80-100)

1.2x (allow 120% of limit)

002

Moderate Risk

B (60-79)

1.0x (strict limit)

003

Elevated Risk

C (40-59)

0.9x (10% buffer below limit)

100

High Risk

D (0-39)

0.8x (20% buffer below limit)

### 11.4 Credit Representative Groups

GRUPP

Description

Assignment Logic

1100

Domestic Auto

Dealers for automotive segment, India

1200

Domestic Farm

Dealers for farm equipment, India

1500

Export

International dealers

1600

Commercial

Commercial vehicle dealers

## 12 Enhancement / BAdI Specification

### 12.1 Enhancement Overview

#

Enhancement

BAdI / Exit

Purpose

E1

Data Completeness Check

BAdI: BADI\_CREDIT\_UPDATE

Validate mandatory fields before credit limit change is saved

E2

Credit Score Calculation

Custom class: ZCL\_CREDIT\_SCORING

Calculate score on-demand and batch

E3

Auto-Release Engine

BAdI: BADI\_SD\_BILLING or custom event

Auto-release blocked orders after limit change

E4

Score Display in FD32

Screen exit or Fiori extension

Show credit score on credit master screen

### 12.2 E1 — Data Completeness Check (Pseudo-Code)

METHOD check\_credit\_data\_completeness.
  " Called before KNKK credit limit change is saved
  
  " Check KNA1 mandatory fields
  SELECT SINGLE stceg, land1 FROM kna1 
    INTO @DATA(ls\_kna1)
    WHERE kunnr = @iv\_kunnr.
    
  IF ls\_kna1-stceg IS INITIAL AND ls\_kna1-land1 = 'IN'.
    " Indian dealer must have GST number
    RAISE EXCEPTION TYPE zcx\_credit\_incomplete
      MESSAGE 'GST number (STCEG) is mandatory for Indian dealers'.
  ENDIF.
  
  " Check KNB1 payment terms
  SELECT SINGLE zterm, zwels FROM knb1
    INTO @DATA(ls\_knb1)
    WHERE kunnr = @iv\_kunnr AND bukrs = @iv\_bukrs.
    
  IF ls\_knb1-zterm IS INITIAL.
    RAISE EXCEPTION TYPE zcx\_credit\_incomplete
      MESSAGE 'Payment terms must be maintained before credit limit change'.
  ENDIF.
  
  " Check minimum financial data
  SELECT COUNT(\*) FROM bsad
    INTO @DATA(lv\_cleared\_count)
    WHERE kunnr = @iv\_kunnr
      AND budat >= @lv\_12\_months\_ago.
      
  IF lv\_cleared\_count = 0 AND iv\_new\_dealer = abap\_false.
    " Existing dealer with no payment history — flag
    APPEND 'WARNING: No payment history in last 12 months' TO et\_warnings.
  ENDIF.
ENDMETHOD.

### 12.3 E2 — Credit Scoring Engine (Pseudo-Code)

METHOD calculate\_credit\_score.
  " Payment History Score (40%)
  SELECT AVG( datediff( augdt, zfbdt ) ) FROM bsad
    INTO @DATA(lv\_avg\_days)
    WHERE kunnr = @iv\_kunnr AND budat >= @lv\_12m\_ago.
  lv\_pay\_score = COND #(
    WHEN lv\_avg\_days <= lv\_terms THEN 100
    WHEN lv\_avg\_days <= lv\_terms + 15 THEN 80
    WHEN lv\_avg\_days <= lv\_terms + 30 THEN 60
    WHEN lv\_avg\_days <= lv\_terms + 60 THEN 40
    ELSE 20 ).
    
  " Utilization Score (25%)
  lv\_util\_pct = ( ls\_knkk-skfor / ls\_knkk-klimk ) \* 100.
  lv\_util\_score = COND #(
    WHEN lv\_util\_pct <= 50 THEN 100
    WHEN lv\_util\_pct <= 70 THEN 80
    WHEN lv\_util\_pct <= 85 THEN 60
    WHEN lv\_util\_pct <= 95 THEN 40
    ELSE 20 ).
    
  " Order Frequency Score (20%)
  SELECT COUNT(\*) FROM vbak INTO @lv\_order\_count
    WHERE kunnr = @iv\_kunnr AND erdat >= @lv\_12m\_ago.
  lv\_freq\_score = COND #(
    WHEN lv\_order\_count >= 12 THEN 100
    WHEN lv\_order\_count >= 6  THEN 80
    WHEN lv\_order\_count >= 3  THEN 60
    WHEN lv\_order\_count >= 1  THEN 40
    ELSE 20 ).
    
  " Overdue Score (15%)
  lv\_overdue\_pct = lv\_overdue\_items / lv\_total\_open \* 100.
  lv\_odue\_score = COND #(
    WHEN lv\_overdue\_pct = 0   THEN 100
    WHEN lv\_overdue\_pct <= 10 THEN 80
    WHEN lv\_overdue\_pct <= 25 THEN 60
    WHEN lv\_overdue\_pct <= 50 THEN 40
    ELSE 20 ).
    
  " Final Score
  rv\_score = ( lv\_pay\_score \* '0.40' ) + ( lv\_util\_score \* '0.25' )
           + ( lv\_freq\_score \* '0.20' ) + ( lv\_odue\_score \* '0.15' ).
  
  " Band
  rv\_band = COND #(
    WHEN rv\_score >= 80 THEN 'A'
    WHEN rv\_score >= 60 THEN 'B'
    WHEN rv\_score >= 40 THEN 'C'
    ELSE 'D' ).
ENDMETHOD.

## 13 Data Migration & Enrichment Plan

### 13.1 Enrichment Approach

Field

Table

Current %

Target %

Source

Method

STCEG (GST)

KNA1

2%

100%

Dealer registration records

Mass upload via LSMW / BAPI\_CUSTOMER\_CHANGE

CTLPC (Risk Cat.)

KNKK

16%

100%

Auto-calculated by scoring engine

Batch job — initial run for all dealers

DTREV / NXTRV

KNKK

6%

100%

Set to today / today + 90

Batch update via FD32 mass change

GRUPP

KNKK

56%

100%

Derive from segment + region

Mapping table + batch update

ZTERM

KNB1

40%

100%

Dealer agreement terms

Mass upload via XD02 BDC or BAPI

ZWELS

KNB1

30%

100%

Finance team input

Mass upload

### 13.2 Upload Template

Column

Field

Mandatory

Validation

A

KUNNR (Customer)

Yes

Must exist in KNA1

B

STCEG (GST Number)

Yes for IN

Format: 22AAAAA0000A1Z5

C

ZTERM (Payment Terms)

Yes

Must exist in T052

D

ZWELS (Payment Method)

Yes

Must exist in T042Z

E

Segment

Yes

AUTO / FARM / COMMERCIAL / EXPORT

## 14 Dashboard & Reporting Wireframes

### 14.1 Credit Management Cockpit — KPI Tiles

Tile

Metric

Data Source

Alert Threshold

🔴 Blocked Orders

Count + Total Value of orders with LIFSK = 07

VBAK

Red if > 10 orders or > ₹5 Cr

📊 Credit Utilization

Average utilization across all dealers

KNKK (SKFOR/KLIMK)

Red if avg > 85%

⏱️ Approval TAT

Average days from request to approval

CDHDR on KNKK

Red if > 5 days

📈 Score Distribution

Count by Band (A/B/C/D)

ZCREDIT\_SCORE

Red if Band D > 10%

⚠️ Overdue Exposure

Total overdue amount across dealers

BSID aging

Red if > ₹10 Cr

📅 Reviews Due

Dealers with NXTRV in next 30 days

KNKK.NXTRV

Yellow if > 20 reviews pending

### 14.2 Drill-Down Dimensions

Dimension

Filter Values

Credit Control Area (KKBER)

1000, 2222, 3000

Credit Rep. Group (GRUPP)

1100, 1200, 1500, 1600

Score Band

A, B, C, D

Dealer Segment

Auto, Farm, Commercial, Export

Region / Plant

Based on sales area

Time Period

Current month, quarter, year

## 15 Integration Points

#

Integration

Direction

Technology

Phase

I1

GST Portal (for GST number validation)

Outbound → GSTN

REST API

Phase 1

I2

Bank statement import (payment pattern)

Inbound ← Bank

MT940 / camt.053

Phase 1

I3

External credit rating agency

Outbound → CRISIL/ICRA

REST API

Phase 2

I4

MCA21 (for company financials)

Outbound → MCA

REST API

Phase 2

I5

Dealer Portal (self-service upload)

Inbound ← Portal

OData / Fiori

Phase 2

I6

SAP Analytics Cloud (dashboard)

Outbound → SAC

OData / Live Connection

Phase 1

## 16 Authorization Concept

### 16.1 Role Matrix

Role

View Score

Run Scoring

Change Limit

Auto-Release

Config

**Credit Analyst**

✅

✅

Up to Band B

❌

❌

**Credit Manager**

✅

✅

All bands

✅ (review)

❌

**Credit Head**

✅

✅

All bands + override

✅

✅

**Sales Manager**

✅ (read)

❌

❌

❌

❌

**IT Administrator**

✅

✅

❌

✅ (config)

✅

### 16.2 Authorization Objects

Auth Object

Field

Values

Assigned To

F\_KNA1\_GRP

KTOKD (Account Group)

DEBI, 0001, 0002, CUST, ZCRM

All credit roles

F\_KNA1\_BUK

BUKRS (Company Code)

As per responsibility

All credit roles

V\_KNKK\_CRE

KKBER (Credit Control Area)

1000, 2222, 3000

Credit Analyst, Manager, Head

S\_TCODE

TCD

FD32, FD33, VKM1, VKM3

Credit roles

ZCREDIT\_SCORE (custom)

ACTVT

01 (Create), 02 (Change), 03 (Display)

Scoring engine roles

## 17 Test Scenarios

#

Scenario

Precondition

Steps

Expected Result

T01

New dealer — data complete

All mandatory fields populated

Create credit limit request

Score calculated, routed to Analyst

T02

New dealer — data incomplete

GST number missing

Create credit limit request

Request blocked with error message

T03

Renewal — Band A dealer

Existing dealer, score = 85

Submit limit renewal

Auto-approved, KNKK updated

T04

Renewal — Band C dealer

Existing dealer, score = 48

Submit limit renewal

Routed to Credit Manager

T05

Limit increase — Band B

Score = 72, request 130% of current

Submit limit increase

Capped at 120%, routed to Analyst

T06

Limit decrease

Any band

Decrease limit by 20%

Processed without scoring (decrease = always allowed)

T07

Auto-release — limit increased

Blocked order exists (LIFSK=07), limit increased, exposure < 90%

Approve limit increase

Blocked order auto-released within minutes

T08

Auto-release — still over limit

Blocked order exists, limit increased but exposure still > 90%

Approve limit increase

Order NOT auto-released, manual review needed

T09

Auto-release — payment block

Customer has KNB1.ZAHLS = 'A'

Approve limit increase

Order NOT auto-released (payment block takes precedence)

T10

Batch scoring — quarterly

All dealers in credit master

Run batch job

ZCREDIT\_SCORE updated for all, CTLPC and NXTRV updated in KNKK

T11

Score drops to Band D

Dealer payment deteriorates

Quarterly batch runs

Alert generated, CRBLB set to 'X'

T12

Dashboard — drill-down

Dashboard loaded

Click on "Blocked Orders" tile

List of orders with customer, amount, block date, block code

T13

Authorization — Analyst limit

Analyst logged in, Band C dealer

Try to approve

Rejected — Band C requires Manager

T14

Credit Head override

Band D dealer, Head logged in

Override score and approve

Approved with override reason logged

T15

Export dealer — different rules

Dealer in GRUPP 1500

Submit credit request

Routed per export-specific approval matrix

T16

Zero limit customer

KLIMK = 0 (customers 1380, 1390)

Place sales order

Order blocked immediately, cannot proceed without limit

T17

High-value single order

Order value > 50% of credit limit

Place large order

Dynamic credit check triggers additional approval

T18

Score with no payment history

New dealer — no BSAD records

Run scoring

Payment sub-score defaults to 50, overall score adjusted

T19

Concurrent limit changes

Two analysts processing same dealer

Both submit changes

Lock mechanism prevents conflicts

T20

Mass credit review

50+ dealers due for review

Run batch scoring for all

All scores updated, alerts for downgrades, NXTRV reset

## 18 Cutover & Go-Live Checklist

### 18.1 Pre-Go-Live (T-14 days to T-1)

Day

Task

Owner

Status

T-14

Complete data migration — GST, payment terms, payment methods

M&M Data Team

☐

T-14

Configure OVA8, risk categories, credit groups in Quality

M&M Functional

☐

T-12

Deploy ZCREDIT\_SCORE table and scoring class to Quality

M&M IT / C16

☐

T-10

Run initial batch scoring for all dealers in Quality

M&M IT

☐

T-10

Execute test scenarios T01-T20 in Quality

M&M QA Team

☐

T-7

Fix defects from testing

M&M IT / C16

☐

T-5

User acceptance testing by Credit Head

M&M Credit Head

☐

T-3

Transport all objects to Production

M&M Basis

☐

T-2

Run data migration in Production (GST, terms)

M&M Data Team

☐

T-1

Run initial batch scoring in Production

M&M IT

☐

T-1

Verify all authorization roles assigned

M&M Basis

☐

### 18.2 Go-Live Day (T-0)

Step

Task

Verification

1

Activate auto-release engine

Create test blocked order → verify auto-release

2

Activate data completeness check

Try credit change without GST → verify block

3

Verify dashboard connectivity

Open dashboard → confirm real-time data

4

Schedule quarterly batch scoring job

SM36 → verify job scheduled

5

Communicate to sales team and dealers

Email sent with new process guide

### 18.3 Post-Go-Live (T+1 to T+30)

Day

Task

Owner

T+1 to T+5

Hypercare — monitor all credit blocks, auto-releases, scoring runs

M&M IT + C16

T+7

First week review — blocked orders count, approval TAT, any false auto-releases

Credit Head

T+14

Score calibration — review if scoring weights need adjustment

Credit Head + IT

T+30

Go-Live sign-off — compare KPIs to baseline

Sanjay Chavan (CTO)

**Document ID:** C16-FS-DCM-001  |  **Version:** 1.0  |  **Generated:** 25 April 2026  |  **Engine:** C16 Process Intelligence v2.0  
**Data Sources:** KNKK (50 records), KNA1 (50 records), KNB1 (field metadata), KNVV (field metadata), BSID (100 records), BSAD (100 records), VBAK blocked orders (27 records), DD03L field definitions for KNKK/KNB1/KNA1/KNVV (408 fields).  
**Disclaimer:** This specification is based on data extracted from the connected SAP system. Actual M&M production data volumes and patterns may differ. All recommendations should be validated with M&M business stakeholders before implementation.

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._