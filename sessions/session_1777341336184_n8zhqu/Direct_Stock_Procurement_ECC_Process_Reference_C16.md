  Direct / Stock Procurement — ECC Process Reference & C16 Value Map :root { --c16-purple: #6B3FA0; --c16-pink: #B5567A; --c16-copper: #C97B3A; --c16-dark: #2c3e50; --c16-light: #f8f9fa; --c16-green: #27ae60; --c16-blue: #3498db; --c16-orange: #e67e22; --c16-red: #e74c3c; --c16-teal: #1abc9c; --c16-navy: #2c3e50; } \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, Arial, sans-serif; color: var(--c16-dark); background: #e9ecef; line-height: 1.7; font-size: 15px; } .doc-wrapper { max-width: 1200px; margin: 30px auto; background: #fff; box-shadow: 0 4px 24px rgba(0,0,0,0.12); border-radius: 12px; overflow: hidden; } /\* ── Header Banner ── \*/ .header-banner { background: linear-gradient(135deg, var(--c16-copper), var(--c16-pink), var(--c16-purple)); color: #fff; padding: 50px 60px; position: relative; overflow: hidden; } .header-banner::before { content: ''; position: absolute; top: -50%; right: -20%; width: 500px; height: 500px; background: rgba(255,255,255,0.04); border-radius: 50%; } .header-banner::after { content: ''; position: absolute; bottom: -40%; left: -10%; width: 400px; height: 400px; background: rgba(255,255,255,0.03); border-radius: 50%; } .header-banner img { height: 55px; margin-bottom: 16px; border-radius: 8px; position: relative; z-index: 2; } .header-banner h1 { font-size: 2.2em; font-weight: 700; margin-bottom: 8px; position: relative; z-index: 2; letter-spacing: -0.5px; } .header-banner .subtitle { font-size: 1.15em; opacity: 0.92; position: relative; z-index: 2; font-weight: 300; } .header-banner .badges { margin-top: 18px; position: relative; z-index: 2; display: flex; gap: 10px; flex-wrap: wrap; } .header-banner .badge { display: inline-block; padding: 5px 16px; border-radius: 20px; font-size: 0.82em; font-weight: 600; letter-spacing: 0.3px; } .badge-ecc { background: rgba(255,255,255,0.22); color: #fff; border: 1px solid rgba(255,255,255,0.4); } .badge-mm { background: rgba(39,174,96,0.85); color: #fff; } .badge-version { background: rgba(255,255,255,0.15); color: #fff; border: 1px solid rgba(255,255,255,0.3); } /\* ── Classification Banner ── \*/ .classification-bar { background: #1a1a2e; color: #ff6b6b; text-align: center; padding: 8px 20px; font-size: 0.8em; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; } /\* ── Content Area ── \*/ .content { padding: 40px 60px 50px; } /\* ── Section Titles ── \*/ .section-title { color: var(--c16-purple); font-size: 1.55em; font-weight: 700; margin: 45px 0 20px; padding-bottom: 12px; border-bottom: 3px solid var(--c16-pink); display: flex; align-items: center; gap: 14px; } .section-title .num { background: var(--c16-pink); color: #fff; width: 36px; height: 36px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; font-size: 0.65em; font-weight: 700; flex-shrink: 0; } .subsection-title { color: var(--c16-purple); font-size: 1.2em; font-weight: 700; margin: 30px 0 14px; padding-left: 16px; border-left: 4px solid var(--c16-copper); } /\* ── Tables ── \*/ table { width: 100%; border-collapse: collapse; margin: 16px 0 24px; font-size: 0.92em; } thead th { background: var(--c16-purple); color: #fff; padding: 12px 16px; text-align: left; font-weight: 600; font-size: 0.88em; text-transform: uppercase; letter-spacing: 0.3px; } tbody td { padding: 11px 16px; border-bottom: 1px solid #edf2f7; vertical-align: top; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; transition: background 0.2s; } /\* ── KPI Cards ── \*/ .kpi-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 18px; margin: 24px 0; } .kpi-card { background: linear-gradient(135deg, #f8f9fa, #fff); border: 1px solid #e2e8f0; border-radius: 12px; padding: 22px 20px; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.04); transition: transform 0.2s, box-shadow 0.2s; } .kpi-card:hover { transform: translateY(-2px); box-shadow: 0 6px 16px rgba(0,0,0,0.08); } .kpi-card .kpi-value { font-size: 2em; font-weight: 800; color: var(--c16-purple); } .kpi-card .kpi-label { font-size: 0.82em; color: #718096; margin-top: 4px; font-weight: 500; } .kpi-card.green .kpi-value { color: var(--c16-green); } .kpi-card.blue .kpi-value { color: var(--c16-blue); } .kpi-card.orange .kpi-value { color: var(--c16-orange); } .kpi-card.copper .kpi-value { color: var(--c16-copper); } .kpi-card.teal .kpi-value { color: var(--c16-teal); } /\* ── Phase Boxes ── \*/ .phase-box { border-radius: 12px; padding: 28px 28px 20px; margin: 20px 0; border-left: 5px solid; } .phase-planning { background: #eef6ff; border-color: var(--c16-blue); } .phase-sourcing { background: #fef9ef; border-color: var(--c16-copper); } .phase-receiving { background: #eefaf3; border-color: var(--c16-green); } .phase-invoice { background: #fdf0f5; border-color: var(--c16-pink); } .phase-box h4 { font-size: 1.15em; margin-bottom: 10px; } .phase-box .phase-icon { font-size: 1.3em; margin-right: 8px; } /\* ── C16 Callout ── \*/ .c16-callout { background: linear-gradient(135deg, #f5f0ff, #fdf0f5); border: 1px solid #d6bcfa; border-left: 5px solid var(--c16-purple); border-radius: 10px; padding: 20px 24px; margin: 18px 0; position: relative; } .c16-callout::before { content: '🔍 C16'; position: absolute; top: -10px; left: 20px; background: var(--c16-purple); color: #fff; padding: 2px 14px; border-radius: 12px; font-size: 0.75em; font-weight: 700; letter-spacing: 0.5px; } .c16-callout h5 { color: var(--c16-purple); margin: 6px 0 8px; font-size: 1em; } .c16-callout ul { margin: 0; padding-left: 20px; } .c16-callout li { margin-bottom: 4px; font-size: 0.93em; } /\* ── RACI Badges ── \*/ .raci-r { background: #e74c3c; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 700; } .raci-a { background: #e67e22; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 700; } .raci-c { background: #3498db; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 700; } .raci-i { background: #95a5a6; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 700; } /\* ── Risk Badges ── \*/ .risk-high { background: #e74c3c; color: #fff; padding: 3px 12px; border-radius: 12px; font-size: 0.82em; font-weight: 600; } .risk-medium { background: #e67e22; color: #fff; padding: 3px 12px; border-radius: 12px; font-size: 0.82em; font-weight: 600; } .risk-low { background: #27ae60; color: #fff; padding: 3px 12px; border-radius: 12px; font-size: 0.82em; font-weight: 600; } /\* ── Control Type Badges ── \*/ .ctrl-preventive { background: #27ae60; color: #fff; padding: 2px 10px; border-radius: 10px; font-size: 0.78em; } .ctrl-detective { background: #3498db; color: #fff; padding: 2px 10px; border-radius: 10px; font-size: 0.78em; } .ctrl-corrective { background: #e67e22; color: #fff; padding: 2px 10px; border-radius: 10px; font-size: 0.78em; } /\* ── Mermaid ── \*/ .mermaid { margin: 20px 0; text-align: center; } .mermaid svg { max-width: 100%; } /\* ── Diagram Container ── \*/ .diagram-container { background: #fafbfc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; margin: 20px 0; overflow-x: auto; } .diagram-container .diagram-title { text-align: center; font-weight: 700; color: var(--c16-purple); margin-bottom: 16px; font-size: 1.05em; } /\* ── Callout Boxes ── \*/ .info-box { background: #eef6ff; border: 1px solid #b3d4fc; border-radius: 10px; padding: 18px 22px; margin: 16px 0; } .info-box.warning { background: #fff8e1; border-color: #ffe082; } .info-box.success { background: #e8f5e9; border-color: #a5d6a7; } /\* ── TOC ── \*/ .toc { background: linear-gradient(135deg, #f8f9fa, #fff); border: 1px solid #e2e8f0; border-radius: 12px; padding: 28px 32px; margin: 28px 0; columns: 2; column-gap: 40px; } .toc h3 { column-span: all; color: var(--c16-purple); margin-bottom: 14px; font-size: 1.15em; } .toc a { color: var(--c16-purple); text-decoration: none; display: block; padding: 5px 0; font-size: 0.93em; transition: color 0.2s; } .toc a:hover { color: var(--c16-pink); text-decoration: underline; } .toc a .toc-num { font-weight: 700; margin-right: 8px; color: var(--c16-pink); } /\* ── Disclaimer ── \*/ .disclaimer-box { background: #f8f9fa; border: 1px solid #dee2e6; border-radius: 10px; padding: 22px 26px; margin: 24px 0; font-size: 0.88em; color: #555; line-height: 1.8; } .disclaimer-box h4 { color: var(--c16-dark); margin-bottom: 8px; } /\* ── Footer ── \*/ .doc-footer { background: #1a1a2e; color: #a0aec0; padding: 30px 60px; font-size: 0.82em; line-height: 1.8; } .doc-footer .footer-brand { color: #fff; font-weight: 700; font-size: 1.1em; margin-bottom: 6px; } /\* ── Collapsible Details ── \*/ details { margin: 12px 0; } details summary { background: #f0f4f8; padding: 12px 18px; border-radius: 8px; cursor: pointer; font-weight: 600; color: var(--c16-purple); transition: background 0.2s; } details summary:hover { background: #e2e8f0; } details\[open\] summary { border-radius: 8px 8px 0 0; } /\* ── Print ── \*/ @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; margin: 0; max-width: 100%; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .toc { columns: 1; } .kpi-row { grid-template-columns: repeat(3, 1fr); } .mermaid svg { max-width: 100% !important; } .diagram-container { page-break-inside: avoid; } }

CONFIDENTIAL — FOR INTENDED RECIPIENTS ONLY

![C16](data:image/jpeg;base64,)

# Direct / Stock Procurement

Enterprise Process Reference Model & C16 Intelligence Overlay — SAP ECC

SAP ECC 6.0 / EhP 7+ MM — Materials Management Document v1.0 — April 2026

### 📑 Table of Contents

[01 Executive Summary](#executive-summary) [02 Process Scope & Boundaries](#process-scope) [03 End-to-End BPMN Process Flow](#end-to-end-bpmn) [04 Phase 1 — Demand Planning & MRP](#phase1) [05 Phase 2 — Sourcing & Purchase Order](#phase2) [06 Phase 3 — Goods Receipt & Warehousing](#phase3) [07 Phase 4 — Invoice Verification & Payment](#phase4) [08 RACI Responsibility Matrix](#raci) [09 Internal Controls & Compliance Framework](#controls) [10 Risk Assessment Matrix](#risk-framework) [11 SAP Transaction Reference Matrix](#transaction-matrix) [12 C16 Intelligence Value Map](#c16-value-map) [13 C16 Detection Point Architecture](#c16-detection) [14 Disclaimers & Legal Notices](#disclaimers)

## 01 Executive Summary

This document provides an **enterprise-grade reference model** for the **Direct / Stock Procurement** process within SAP ECC. It serves as both a process blueprint for implementation teams and a value map demonstrating where **C16's AI-powered intelligence platform** delivers measurable impact across the procurement lifecycle.

Direct procurement — the acquisition of materials consumed directly in production or stocked for operational use — is the highest-volume, highest-value procurement sub-process in most manufacturing and distribution enterprises. It spans **four distinct phases** from demand recognition through payment execution, involving cross-functional handoffs between production planning, purchasing, warehouse operations, and accounts payable.

4

Major Process Phases

14

L4 Activities Mapped

18+

SAP Transactions

6

Key Decision Gateways

12

C16 Detection Points

5

Organizational Roles

The document is structured to support multiple audiences: **process consultants** receive a validated reference architecture; **test managers** receive a test-scope definition with decision gateways and exception paths; **compliance officers** receive an internal-controls mapping with SOX-relevant checkpoints; and **technology teams** receive a C16 integration blueprint showing real-time mining, gap detection, and test-data generation capabilities at each process stage.

## 02 Process Scope & Boundaries

### 2.1 Process Hierarchy

Level

Name

Description

**L1**

Procure to Pay (P2P)

Enterprise-wide procurement lifecycle from demand to payment

**L2**

Direct / Stock Procurement

Procurement of production and stock materials via MRP-driven or manual POs

**L3**

4 Process Phases

Planning → Sourcing → Receiving → Invoice & Payment

**L4**

14 Discrete Activities

Individual executable steps within each phase

### 2.2 In-Scope

Dimension

Included

**Material Types**

Raw Materials (ROH), Semi-Finished (HALB), Finished Goods (FERT), Spare Parts (ERSA), Operating Supplies (HIBE)

**PO Types**

Standard PO (NB), Scheduling Agreement Call-off (LP), Contract Release (MK)

**Procurement Trigger**

MRP-generated Purchase Requisitions, manual Purchase Requisitions

**GR Scenarios**

GR to warehouse (101), GR to production (101/261), Returns to vendor (122)

**Invoice Scenarios**

PO-based invoice, credit memo, subsequent debit/credit, planned delivery costs

**SAP Module**

MM (Materials Management), PP (limited — MRP only), FI-AP (invoice & payment)

### 2.3 Out of Scope

Excluded Process

Reason

Indirect / Non-PO Procurement

Separate L2 process — shopping cart, blanket PO model

Subcontracting

Separate L2 — component issue (541) and subcontract GR (101)

Consignment

Separate L2 — vendor-owned stock, settlement via MRKO

Intercompany STO

Separate L2 — stock transport order between plants/company codes

Services Procurement

Separate L2 — service entry sheets, ML81N workflow

Third-Party / Drop-Ship

Separate L2 — TAS item category, statistical GR

Vendor Master Management

Separate L2 — creation, extension, governance

Material Master Management

Separate L2 — creation, classification, MRP views

### 2.4 Organizational Entities

Org Element

ECC Object

Config Table

Role

Company Code

BUKRS

T001

Legal entity for financial posting

Plant

WERKS

T001W

Physical location for inventory management

Storage Location

LGORT

T001L

Sub-division of plant for stock management

Purchasing Organization

EKORG

T024E

Procurement entity responsible for vendor negotiations

Purchasing Group

EKGRP

T024

Buyer group responsible for material category

## 03 End-to-End BPMN Process Flow

The following diagram presents the **master end-to-end flow** for Direct/Stock Procurement using BPMN-style notation. Four swim lanes represent the major organizational roles involved. Decision gateways (◇) indicate branching logic where process variants diverge.

🗺️ Direct / Stock Procurement — Master BPMN (ECC)

flowchart TD subgraph PLAN\["🔵 PHASE 1 — DEMAND PLANNING & MRP"\] direction LR A1(\[🟢 Start: Demand Trigger\]) --> A2\[MRP Run  
MD01 / MD02\] A2 --> A3{MRP Exception  
Messages?} A3 -->|Yes| A4\[Review Exception  
Messages MD04\] A3 -->|No| A5\[Auto-generated  
Purchase Requisition\] A4 --> A6{Manual PR  
Needed?} A6 -->|Yes| A7\[Create Manual PR  
ME51N\] A6 -->|No| A5 A5 --> A8\[PR Created in System\] A7 --> A8 end subgraph SOURCE\["🟠 PHASE 2 — SOURCING & PURCHASE ORDER"\] direction LR B1\[Review & Assign  
PR to Source ME57\] --> B2{Source of  
Supply?} B2 -->|Contract| B3\[Release vs Contract  
ME31K / ME38\] B2 -->|Info Record| B4\[Select Info Record  
ME11 / ME12\] B2 -->|New Vendor| B5\[Quotation Process  
ME47 / ME49\] B3 --> B6\[Create Purchase Order  
ME21N\] B4 --> B6 B5 --> B6 B6 --> B7{Release Strategy  
Required?} B7 -->|Yes| B8\[PO Release  
ME29N / ME28\] B7 -->|No| B9\[PO Dispatched  
to Vendor\] B8 --> B9 B9 --> B10\[PO Confirmation  
& Monitoring ME2M\] end subgraph RECEIVE\["🟢 PHASE 3 — GOODS RECEIPT & WAREHOUSING"\] direction LR C1\[Goods Arrive  
at Dock\] --> C2{Quality  
Inspection?} C2 -->|Yes| C3\[Post GR to  
QI Stock MIGO 103\] C2 -->|No| C4\[Post GR to  
Unrestricted MIGO 101\] C3 --> C5\[Quality Decision  
QA11 / QA32\] C5 --> C6{Quality  
Passed?} C6 -->|Yes| C7\[Transfer to  
Unrestricted MB1B 321\] C6 -->|No| C8\[Return to Vendor  
MIGO 122 / Block\] C7 --> C9\[Stock Updated  
& Available\] C4 --> C9 end subgraph INVOICE\["🔴 PHASE 4 — INVOICE VERIFICATION & PAYMENT"\] direction LR D1\[Vendor Invoice  
Received\] --> D2\[Post Invoice  
MIRO\] D2 --> D3{3-Way Match  
PO / GR / IR?} D3 -->|Match| D4\[Invoice Posted  
Successfully\] D3 -->|Variance| D5{Within  
Tolerance?} D5 -->|Yes| D4 D5 -->|No| D6\[Invoice Blocked  
for Review\] D6 --> D7\[Resolve & Release  
MRBR\] D7 --> D4 D4 --> D8\[Payment Run  
F110\] D8 --> D9(\[🔴 End: Payment  
Executed\]) end PLAN --> SOURCE SOURCE --> RECEIVE RECEIVE --> INVOICE style PLAN fill:#eef6ff,stroke:#3498db,stroke-width:2px style SOURCE fill:#fef9ef,stroke:#e67e22,stroke-width:2px style RECEIVE fill:#eefaf3,stroke:#27ae60,stroke-width:2px style INVOICE fill:#fdf0f5,stroke:#e74c3c,stroke-width:2px

**📖 Reading the Diagram:** Rounded rectangles = start/end events. Rectangles = process tasks. Diamonds = decision gateways. Arrows = sequence flow. Color-coded swim lanes represent the four major phases. Each phase maps to a distinct organizational responsibility and SAP transaction cluster.

## 04 Phase 1 — Demand Planning & MRP

#### 🔵 Phase Overview

Demand planning initiates the procurement cycle by identifying material shortages through MRP (Material Requirements Planning). The MRP run evaluates current stock, safety stock levels, planned independent requirements (PIR), and production orders to generate Purchase Requisitions for materials that fall below the reorder point or planned demand.

Phase 1 — Detailed BPMN: Demand Planning & MRP

flowchart LR subgraph PP\["Production Planner"\] P1(\[Start\]) --> P2\[Review Planned  
Independent Requirements  
MD61 / MD62\] P2 --> P3\[Execute MRP Run  
MD01 — Plant Level  
MD02 — Single Material\] P3 --> P4\[Review MRP List  
MD05 / MD06 / MD07\] end subgraph EVAL\["MRP Controller"\] P4 --> E1{Exception  
Messages?} E1 -->|Rescheduling| E2\[Adjust PR Dates  
MD04\] E1 -->|New Demand| E3\[Review Auto PR  
ME5A\] E1 -->|Cancel| E4\[Delete PR Line  
ME52N\] E2 --> E5{Manual PR  
Required?} E3 --> E5 E4 --> E5 E5 -->|Yes| E6\[Create Manual PR  
ME51N\] E5 -->|No| E7\[PR Ready for  
Processing\] E6 --> E7 end subgraph APPROVE\["PR Approver"\] E7 --> AP1{PR Release  
Strategy?} AP1 -->|Yes| AP2\[Release PR  
ME54N / ME55\] AP1 -->|No| AP3\[Auto-Released\] AP2 --> AP4\[PR Released ✓\] AP3 --> AP4 end style PP fill:#eef6ff,stroke:#3498db,stroke-width:1px style EVAL fill:#f0f4ff,stroke:#5b7dbd,stroke-width:1px style APPROVE fill:#e8f4fd,stroke:#2980b9,stroke-width:1px

### 4.1 L4 Activity Breakdown

#

Activity

Transaction

Description

Output

**1.1**

Maintain Planned Independent Requirements

MD61 / MD62

Create or update demand forecasts that drive MRP calculations for stock materials

PIR entries in PBED/PBIM

**1.2**

Execute MRP Run

MD01 (plant) / MD02 (single)

System evaluates stock, requirements, and lead times to generate planned orders and purchase requisitions

Planned Orders, Purchase Requisitions in EBAN

**1.3**

Review MRP Results & Exceptions

MD04 / MD05 / MD07

MRP controller reviews exception messages (rescheduling in/out, new demand, cancellation) and resolves conflicts

Adjusted supply plan

**1.4**

Create / Modify Purchase Requisition

ME51N / ME52N

Create manual PRs for non-MRP items or modify MRP-generated PRs (change source, quantity, delivery date)

PR document in EBAN

**1.5**

Release Purchase Requisition

ME54N / ME55

Approve PRs per release strategy (value-based, material group-based). Unreleased PRs cannot be converted to POs.

Released PR (FRGST = approved)

##### C16 Intelligence — Demand Planning Phase

*   **MRP Exception Mining:** C16 reads MDKP/MDTB (MRP header/item) and EBAN (requisitions) to identify chronic exception patterns — materials perpetually rescheduled, PRs created and cancelled repeatedly, phantom demand from inactive PIRs
*   **PR Ageing Analysis:** Queries EBAN with FRGST (release status) and ERDAT (creation date) to flag requisitions stuck unreleased beyond SLA thresholds — typically indicating approval bottlenecks or missing release strategies
*   **MRP Parameter Audit:** Reads MARC (MRP views) to detect misconfigured reorder points, lot sizes, or safety stock levels causing excessive procurement or stock-outs
*   **Dead PR Detection:** Identifies PRs older than configurable thresholds (e.g., 90 days) with no conversion to PO — candidates for cleanup or process re-evaluation

## 05 Phase 2 — Sourcing & Purchase Order

#### 🟠 Phase Overview

This phase transforms approved Purchase Requisitions into Purchase Orders by identifying the optimal source of supply, negotiating terms, and creating formal purchase commitments. Source determination may leverage existing contracts (outline agreements), purchasing info records, or new vendor quotations. The PO then undergoes release (approval) per the configured release strategy before dispatch to the vendor.

Phase 2 — Detailed BPMN: Sourcing & Purchase Order Management

flowchart TD subgraph BUYER\["Buyer / Purchasing Agent"\] B1\[Assign PR to  
Source ME57\] --> B2{Source of  
Supply Exists?} B2 -->|Contract ME31K| B3\[Release Against  
Outline Agreement\] B2 -->|Info Record| B4\[Use Pricing from  
Info Record ME13\] B2 -->|None| B5\[Request for  
Quotation ME41\] B5 --> B6\[Receive &  
Compare Quotes ME49\] B6 --> B7\[Select Vendor &  
Create Info Record ME11\] B3 --> B8\[Create Purchase  
Order ME21N\] B4 --> B8 B7 --> B8 B8 --> B9\[Print / Transmit  
PO to Vendor ME9F\] end subgraph RELEASE\["PO Approver(s)"\] B8 --> R1{Release Strategy  
Configured?} R1 -->|Single Release| R2\[Individual Release  
ME29N\] R1 -->|Collective| R3\[Mass Release  
ME28\] R1 -->|No Strategy| R4\[Auto-Released\] R2 --> R5\[PO Released ✓\] R3 --> R5 R4 --> R5 end subgraph MONITOR\["Buyer — PO Monitoring"\] R5 --> M1\[Monitor PO Status  
ME2M / ME2L\] M1 --> M2{Vendor  
Confirmation?} M2 -->|Order Ack| M3\[Record Confirmation  
ME22N — Conf Tab\] M2 -->|ASN| M4\[Inbound Delivery  
Created VL31N\] M2 -->|Delay| M5\[Expedite /  
Reminder ME91\] end style BUYER fill:#fef9ef,stroke:#e67e22,stroke-width:1px style RELEASE fill:#fff5e6,stroke:#d4840a,stroke-width:1px style MONITOR fill:#fdf2e0,stroke:#c97b3a,stroke-width:1px

### 5.1 L4 Activity Breakdown

#

Activity

Transaction

Description

Output

**2.1**

Assign Source of Supply

ME57 / ME56

Match released PRs to source — contracts, info records, or manual assignment. Source list (ME01) drives automatic source determination.

PR assigned to vendor/contract

**2.2**

Request for Quotation (if needed)

ME41 / ME47 / ME49

For new materials or vendors: create RFQ, maintain vendor quotes, run price comparison to select optimal source

Quotation comparison, vendor selected

**2.3**

Create Purchase Order

ME21N

Convert PR to PO (automatic via ME59N or manual via ME21N). System copies pricing from info record or contract. Delivery schedule, incoterms, and payment terms applied.

PO document in EKKO/EKPO

**2.4**

PO Release / Approval

ME29N / ME28

Release strategy based on PO value, material group, or plant. Multi-level approval chains supported (T16FS/T16FE/T16FB). Unreleased POs cannot be printed or transmitted.

Released PO (FRGKE = approved)

**2.5**

Transmit PO to Vendor

ME9F / NACE

Output determination sends PO via print, EDI, email, or fax per output condition record. Output type NEU (new PO) is standard.

PO communicated to vendor

**2.6**

PO Monitoring & Confirmation

ME2M / ME2L / ME22N

Track PO status: delivery dates, vendor confirmations (order acknowledgment, ASN), overdue deliveries. Reminder/urging letters via ME91.

Updated PO confirmations

##### C16 Intelligence — Sourcing & PO Phase

*   **Maverick Spend Detection:** Cross-references EKKO (PO) with EBAN (PR) to identify POs created without a preceding PR — a SOX-relevant compliance finding indicating bypass of the requisition approval workflow
*   **Release Strategy Audit:** Reads T16FS/T16FE/T16FB to verify release strategy completeness — detects PO types or value ranges with no strategy configured, allowing auto-approval of high-value orders
*   **Duplicate PO Detection:** Mines EKKO/EKPO for same vendor + same material + same quantity + same delivery date combinations within configurable windows — flags potential duplicate ordering
*   **Price Variance Analysis:** Compares EKPO net price against purchasing info record (EINE/EINA) and last PO price to flag deviations exceeding configurable thresholds
*   **Vendor Concentration Risk:** Aggregates EKKO by LIFNR to identify single-source dependencies where >80% of a material category is sourced from one vendor
*   **PO Output Verification:** Queries NAST (output status) to find POs where output type NEU was never successfully processed — vendor may not have received the PO

## 06 Phase 3 — Goods Receipt & Warehousing

#### 🟢 Phase Overview

Upon physical arrival of materials, the warehouse team performs goods receipt (GR) against the Purchase Order. GR triggers inventory updates, accounting postings (stock account debit / GR/IR clearing credit), and quality inspection (if QM inspection type is active for the material/plant). Discrepancies in quantity or quality are managed through returns, rejection, or tolerance-based acceptance.

Phase 3 — Detailed BPMN: Goods Receipt & Quality

flowchart TD subgraph WH\["Warehouse Operator"\] W1(\[Goods Arrive\]) --> W2\[Verify Delivery  
vs PO Schedule\] W2 --> W3{Quantity  
Matches PO?} W3 -->|Yes| W4{QM Inspection  
Active?} W3 -->|Partial / Over| W5\[Adjust Quantity  
Tolerance Check T156C\] W5 --> W4 W4 -->|Yes| W6\[Post GR to  
QI Stock — MIGO 103\] W4 -->|No| W7\[Post GR to  
Unrestricted — MIGO 101\] end subgraph QM\["Quality Inspector"\] W6 --> Q1\[Perform Inspection  
QA01 / QE51N\] Q1 --> Q2\[Record Results  
QA02 / QE71\] Q2 --> Q3{Usage  
Decision} Q3 -->|Accept| Q4\[Transfer to  
Unrestricted MB1B 321\] Q3 -->|Conditional| Q5\[Accept with  
Restrictions\] Q3 -->|Reject| Q6\[Return to Vendor  
MIGO 122\] end subgraph STOCK\["Inventory Update"\] W7 --> S1\[Material Document  
Created MKPF/MSEG\] Q4 --> S1 Q5 --> S1 S1 --> S2\[Accounting Document  
Created BKPF/BSEG\] S2 --> S3\[Stock Updated  
MARD — Unrestricted\] S3 --> S4(\[GR Complete ✓\]) end Q6 --> RET\[Return / Credit  
Process Initiated\] style WH fill:#eefaf3,stroke:#27ae60,stroke-width:1px style QM fill:#e8f8ee,stroke:#1e8449,stroke-width:1px style STOCK fill:#def7e8,stroke:#16a34a,stroke-width:1px

### 6.1 L4 Activity Breakdown

#

Activity

Transaction

Mvt Type

Description

Accounting Impact

**3.1**

Post Goods Receipt

MIGO

101

Receive goods against PO, creating material document and accounting document. Stock value and quantity updated in real-time.

Stock A/c Dr ↔ GR/IR Clearing Cr

**3.2**

Post GR to Quality Inspection

MIGO

103

Goods received into QI stock pending inspection decision. Material not available for production until released.

QI Stock A/c Dr ↔ GR/IR Clearing Cr

**3.3**

Quality Inspection & Usage Decision

QA01 / QA11 / QA32

—

Execute inspection plan, record results, make usage decision (accept/reject/conditional). Links to QM notification for defects.

No direct FI posting

**3.4**

Transfer QI to Unrestricted

MB1B

321

Move accepted goods from QI stock to unrestricted stock, making them available for MRP and production.

Unrestricted Dr ↔ QI Stock Cr

**3.5**

Return to Vendor

MIGO

122

Return defective/rejected goods to vendor. Reverses GR quantity and creates delivery note for return shipment.

GR/IR Clearing Dr ↔ Stock A/c Cr

**3.6**

GR Reversal (if error)

MIGO

102

Cancel incorrect goods receipt. Requires original material document reference. Reverses all stock and accounting postings.

Full reversal of 101 posting

##### C16 Intelligence — Goods Receipt Phase

*   **GR/IR Imbalance Detection:** Compares EKBE entries where BEWTP='E' (GR) vs BEWTP='Q' (invoice) to identify PO line items with goods received but no invoice posted — GR/IR clearing account remains open, distorting balance sheet
*   **Over-Receipt Mining:** Reads EKPO (ordered qty) vs EKBE (received qty) to flag GR postings exceeding PO quantity beyond the configured over-delivery tolerance (T156C) — indicates control bypass
*   **QI Ageing Analysis:** Queries MARD (QI stock INSME) with aging against GR date to identify materials stuck in quality inspection beyond SLA — blocking production availability
*   **Reversal Pattern Detection:** Mines MKPF/MSEG for movement type 102 (GR reversal) frequency — high reversal rates indicate receiving errors, training gaps, or vendor issues
*   **Backdated GR Detection:** Compares MKPF.BUDAT (posting date) vs MKPF.CPUDT (entry date) to find goods receipts posted with historical dates — a SOX-relevant finding affecting period-end accuracy

## 07 Phase 4 — Invoice Verification & Payment

#### 🔴 Phase Overview

The final phase closes the procurement cycle by verifying vendor invoices against PO terms and GR quantities (3-way match), posting the invoice to create an accounts payable liability, and executing payment via the automatic payment program (F110). Variances beyond configured tolerances trigger invoice blocks requiring manual review and release.

Phase 4 — Detailed BPMN: Invoice Verification & Payment

flowchart TD subgraph AP\["Accounts Payable Specialist"\] I1(\[Vendor Invoice  
Received\]) --> I2\[Enter Invoice  
MIRO — PO Reference\] I2 --> I3{3-Way Match  
PO ↔ GR ↔ IR} I3 -->|✅ Match| I4\[Invoice Posted  
Successfully\] I3 -->|❌ Variance| I5{Within Tolerance  
T169G / OMR6?} I5 -->|Yes| I4 I5 -->|No| I6\[Invoice Blocked  
Reason: M / W / R\] end subgraph RESOLVE\["AP Supervisor"\] I6 --> R1\[Review Blocked  
Invoice MRBR\] R1 --> R2{Root Cause} R2 -->|Price Diff| R3\[Accept / Adjust  
Price in PO ME22N\] R2 -->|Qty Diff| R4\[Post Missing GR  
or Adjust Invoice\] R2 -->|Tax / Other| R5\[Manual Correction  
& Re-post\] R3 --> R6\[Release Invoice  
MRBR\] R4 --> R6 R5 --> R6 R6 --> I4 end subgraph PAY\["Treasury / AP Manager"\] I4 --> P1\[Due Date  
Monitoring FBL1N\] P1 --> P2\[Configure Payment  
Run F110\] P2 --> P3{Payment  
Method?} P3 -->|Bank Transfer| P4\[EFT / Wire  
Payment\] P3 -->|Check| P5\[Check Print  
& Dispatch\] P4 --> P6\[Payment Document  
Created BKPF\] P5 --> P6 P6 --> P7\[Vendor Cleared  
BSAK Updated\] P7 --> P8(\[🔴 End: Payment  
Complete\]) end style AP fill:#fdf0f5,stroke:#e74c3c,stroke-width:1px style RESOLVE fill:#fce4ec,stroke:#c0392b,stroke-width:1px style PAY fill:#f8d7e0,stroke:#b03060,stroke-width:1px

### 7.1 L4 Activity Breakdown

#

Activity

Transaction

Description

Accounting Impact

**4.1**

Enter Vendor Invoice

MIRO

Post invoice with PO reference. System performs 3-way match: PO price × GR quantity = invoice amount. Variances checked against tolerance limits (T169G).

GR/IR Clearing Dr ↔ Vendor (AP) Cr

**4.2**

Handle Invoice Blocks

MRBR

Review blocked invoices (stochastic block, price variance, quantity variance). Block reasons: M=manual, W=quality, R=stochastic, P=price. Release after resolution.

No additional posting until release

**4.3**

Credit Memo Processing

MIRO (credit)

Post credit memo for price adjustments, returns, or quality claims. Reduces vendor liability.

Vendor (AP) Dr ↔ GR/IR Clearing Cr

**4.4**

GR/IR Clearing Reconciliation

MR11 / F.19

Periodic clearing of GR/IR account. Identifies items with GR but no IR (accruals) or IR but no GR (pre-payments). Critical for period-end close.

GR/IR balance adjustments

**4.5**

Automatic Payment Run

F110

Configure payment run with run date, company code, vendor range, payment method. System selects due/overdue open items, applies cash discount, groups payments, generates payment medium.

Vendor (AP) Dr ↔ Bank A/c Cr

**4.6**

Payment Monitoring & Reconciliation

FBL1N / F.44

Monitor open/cleared items per vendor. Reconcile bank statement with payment documents. Handle partial payments, returned payments, and payment advice notes.

Bank reconciliation entries

##### C16 Intelligence — Invoice & Payment Phase

*   **Blocked Invoice Ageing:** Reads RBKP (invoice header) with RBSTAT (block status) and aging analysis to identify invoices blocked beyond configurable SLA — causing vendor relationship damage and potential supply disruption
*   **3-Way Match Exception Mining:** Correlates EKBE (GR history) with RSEG (invoice items) to quantify match failure rates per vendor, material, and plant — identifies systemic issues (wrong pricing, chronic over-delivery)
*   **Duplicate Invoice Detection:** Mines RBKP for same vendor + same amount + same reference number + same invoice date — flags potential duplicate payments before they clear
*   **Cash Discount Leakage:** Reads BSIK (open items) with ZBD1T/ZBD2T (payment terms) and BUDAT to calculate missed early-payment discounts — quantifies the financial impact of slow invoice processing
*   **Payment Run Gap Detection:** Queries REGUH (payment program data) to verify F110 execution frequency and coverage — detects if payment runs are skipped or exclude certain vendor groups
*   **GR/IR Clearing Balance:** Reads open items on GR/IR clearing accounts (BSS segment) to quantify exposure — high balances indicate either missing invoices or missing goods receipts requiring investigation

## 08 RACI Responsibility Matrix

The RACI matrix maps accountability across the five primary organizational roles involved in Direct/Stock Procurement. R = Responsible (performs the work), A = Accountable (owns the outcome), C = Consulted, I = Informed.

Activity

Production Planner

Buyer / Purch. Agent

PO Approver

Warehouse Operator

AP Specialist

**1.1** Maintain PIR

R A

I

**1.2** Execute MRP Run

R

C

**1.3** Review MRP Exceptions

R A

C

**1.4** Create / Modify PR

R

I

C

**1.5** Release PR

I

I

R A

**2.1** Assign Source of Supply

R A

**2.2** RFQ Process

C

R A

I

**2.3** Create Purchase Order

I

R

A

I

**2.4** PO Release / Approval

I

R A

**2.5** Transmit PO

R

I

**2.6** PO Monitoring

C

R A

I

**3.1** Post Goods Receipt

I

R A

I

**3.2** Quality Inspection

C

I

R A

**3.3** Return to Vendor

A

R

I

**4.1** Enter Vendor Invoice

C

R A

**4.2** Handle Blocked Invoices

C

A

R

**4.3** GR/IR Reconciliation

C

C

R A

**4.4** Payment Run

A

R

## 09 Internal Controls & Compliance Framework

The following control framework maps key internal controls across the Direct/Stock Procurement process. Controls are classified as Preventive (stops risk before occurrence), Detective (identifies risk after occurrence), or Corrective (remediates identified issues).

Control ID

Phase

Control Description

Type

SOX Relevant

ECC Mechanism

**IC-01**

Planning

PR release strategy requires approval for PRs above defined value thresholds

Preventive

✅ Yes

T16FS/T16FE — Release strategy config

**IC-02**

Sourcing

PO release strategy with multi-level approval based on value and material group

Preventive

✅ Yes

T16FS/T16FB — Release code, class, strategy

**IC-03**

Sourcing

Segregation of Duties: PR creator ≠ PO creator ≠ PO approver

Preventive

✅ Yes

Authorization objects: M\_BANF\_\*, M\_BEST\_\*

**IC-04**

Sourcing

Source list obligation ensures materials can only be procured from approved vendors

Preventive

✅ Yes

ME01 source list with MRP indicator 2

**IC-05**

Receiving

Over-delivery tolerance prevents GR quantity exceeding PO + tolerance %

Preventive

✅ Yes

EKPO-UEBER\_TOL / T156C config

**IC-06**

Receiving

Quality inspection gate prevents movement to unrestricted stock without QM decision

Preventive

⚠️ Optional

QM inspection type 01 in material master

**IC-07**

Invoice

3-way match verification: PO price × GR qty = Invoice amount within tolerance

Detective

✅ Yes

T169G tolerance limits, MIRO auto-check

**IC-08**

Invoice

Stochastic block: Random percentage of invoices blocked for manual review

Detective

⚠️ Optional

OMR4 — Stochastic block config

**IC-09**

Invoice

Duplicate invoice check on vendor + reference + amount + date

Detective

✅ Yes

MIRO built-in check (can be overridden — risk)

**IC-10**

Payment

Payment run requires dual authorization: proposal run → review → payment run

Preventive

✅ Yes

F110 — Proposal ≠ Payment execution

**IC-11**

Payment

Payment block codes on vendor master prevent automatic payment for flagged vendors

Preventive

✅ Yes

LFB1-ZAHLS (payment block indicator)

**IC-12**

All

Period-end GR/IR reconciliation clears aged open items on clearing account

Corrective

✅ Yes

MR11 / F.19 — GR/IR clearing program

## 10 Risk Assessment Matrix

Risk ID

Phase

Risk Description

Likelihood

Impact

Severity

Mitigating Control

**R-01**

Planning

MRP generates unnecessary PRs due to incorrect safety stock or lot size parameters

Medium

Medium

Medium

Periodic MARC parameter review

**R-02**

Sourcing

Maverick spend: POs created without approved PR, bypassing approval chain

Medium

High

High

IC-03 (SoD) + C16 maverick detection

**R-03**

Sourcing

Release strategy gap: High-value POs auto-approved due to missing strategy config

Low

Critical

High

IC-02 + C16 release strategy audit

**R-04**

Sourcing

Single-source dependency: Critical material sourced from only one vendor

Medium

High

High

C16 vendor concentration analysis

**R-05**

Receiving

Over-delivery beyond PO + tolerance: Goods accepted in excess, inflating inventory

Low

Medium

Medium

IC-05 (tolerance config)

**R-06**

Receiving

Backdated GR postings distort period-end inventory valuation and financial reports

Medium

High

High

C16 backdated GR detection

**R-07**

Invoice

Duplicate invoice payment: Same invoice posted twice with different references

Medium

High

High

IC-09 + C16 duplicate detection

**R-08**

Invoice

Blocked invoices aged beyond SLA: Vendor relationship damage, supply disruption

High

Medium

High

C16 blocked invoice ageing

**R-09**

Payment

Cash discount leakage: Invoices not paid within discount period due to process delays

High

Medium

Medium

C16 discount leakage calculation

**R-10**

All

GR/IR clearing account imbalance: Unresolved open items create balance sheet risk

High

High

High

IC-12 + C16 GR/IR balance monitor

## 11 SAP Transaction Reference Matrix

TCode

Description

Phase

Module

Primary Table(s)

Auth Object

**MD01**

MRP Run — Plant Level (Total Planning)

Planning

PP-MRP

MDKP, MDTB, RESB

C\_MDPS\_WRK

**MD02**

MRP Run — Single Material

Planning

PP-MRP

MDKP, MDTB

C\_MDPS\_WRK

**MD04**

Stock/Requirements List (MRP Evaluation)

Planning

PP-MRP

MDKP, MDTB, RESB

C\_MDPS\_WRK

**MD61**

Create Planned Independent Requirements

Planning

PP-MP-DEM

PBED, PBIM

C\_AFKO\_AWA

**ME51N**

Create Purchase Requisition

Planning

MM-PUR

EBAN

M\_BANF\_BSA

**ME54N**

Release Purchase Requisition

Planning

MM-PUR

EBAN

M\_BANF\_FRG

**ME57**

Assign Source to PR (Auto/Manual)

Sourcing

MM-PUR

EBAN, EORD

M\_BANF\_WRK

**ME21N**

Create Purchase Order

Sourcing

MM-PUR

EKKO, EKPO

M\_BEST\_BSA

**ME22N**

Change Purchase Order

Sourcing

MM-PUR

EKKO, EKPO

M\_BEST\_BSA

**ME29N**

Release Purchase Order

Sourcing

MM-PUR

EKKO

M\_BEST\_FRG

**ME28**

Collective PO Release

Sourcing

MM-PUR

EKKO

M\_BEST\_FRG

**ME9F**

PO Output / Message Processing

Sourcing

MM-PUR

NAST

M\_BEST\_BSA

**ME2M**

PO List by Material

Sourcing

MM-PUR

EKKO, EKPO

M\_BEST\_BSA

**MIGO**

Goods Movement (GR 101, Return 122, Reversal 102)

Receiving

MM-IM

MKPF, MSEG, EKBE

M\_MSEG\_BWA

**MB1B**

Transfer Posting (QI→Unrestricted 321)

Receiving

MM-IM

MKPF, MSEG, MARD

M\_MSEG\_BWA

**MIRO**

Logistics Invoice Verification

Invoice

MM-IV

RBKP, RSEG, EKBE

M\_RECH\_BUK

**MRBR**

Release Blocked Invoices

Invoice

MM-IV

RBKP

M\_RECH\_BUK

**MR11**

GR/IR Account Maintenance

Invoice

MM-IV

EKBE, BSEG

M\_RECH\_BUK

**F110**

Automatic Payment Program

Payment

FI-AP

REGUH, REGUP, BSIK

F\_REGU\_BUK

**FBL1N**

Vendor Line Items (Open/Cleared)

Payment

FI-AP

BSIK, BSAK

F\_BKPF\_BUK

**MMBE**

Stock Overview (Multi-Level)

All

MM-IM

MARD, MCHB, MKOL

M\_MATE\_WRK

## 12 C16 Intelligence Value Map

The following matrix maps C16's analytical capabilities to each process phase, identifying the **specific data mining, compliance detection, and test-scope optimization** contributions that C16 delivers. Each capability has been designed to integrate with downstream testing platforms (e.g., Tricentis Tosca) by generating structured findings that translate directly into test scenarios.

C16 Capability

Phase

What C16 Does

Business Value

Tricentis Test Feed

**MRP Parameter Audit**

Planning

Reads MARC (MRP views) to detect misconfigured reorder points, lot sizes, safety stock, planning strategies across all plant/material combinations

Prevents over-procurement and stock-outs caused by incorrect MRP settings

Test cases for MRP run with edge-case parameters

**PR Ageing & Dead PR Detection**

Planning

Queries EBAN with FRGST (status) + ERDAT (date) to identify unreleased PRs, PRs aged beyond threshold, and PRs never converted to POs

Highlights approval bottlenecks and process dead-ends

Regression tests for PR → PO conversion paths

**Maverick Spend Detection**

Sourcing

Cross-references EKKO with EBAN to find POs without preceding PRs — SOX-relevant compliance finding

Quantifies procurement bypass rate and compliance exposure

Negative test: attempt PO creation without PR (should fail)

**Release Strategy Completeness**

Sourcing

Reads T16FS/T16FE/T16FB to identify PO type/value ranges with no release strategy configured

Prevents unauthorized high-value POs from being auto-approved

Boundary tests for release thresholds

**Duplicate PO Detection**

Sourcing

Mines EKKO/EKPO for vendor + material + qty + date duplicates within configurable window

Prevents double-ordering and excess inventory

Test data: known duplicate scenarios for detection validation

**Price Variance Analysis**

Sourcing

Compares EKPO prices against EINE/EINA info records and historical POs to flag outlier pricing

Identifies over-payment and contract price compliance

Price comparison test scenarios with tolerance thresholds

**GR/IR Imbalance Monitor**

Receiving

Compares EKBE GR entries vs IR entries per PO line to quantify clearing account exposure

Prevents balance sheet misstatement at period-end

End-to-end tests: PO → GR → IR → clearing verification

**Over-Receipt & Backdated GR**

Receiving

Reads EKPO tolerance vs EKBE received qty, and MKPF BUDAT vs CPUDT for date discrepancies

Detects control bypass (over-delivery) and audit risk (backdating)

Tolerance boundary tests + date validation scenarios

**Blocked Invoice Ageing**

Invoice

Reads RBKP with block status and aging to identify invoices stuck beyond SLA

Reduces vendor payment delays and supply disruption risk

Workflow tests for block → review → release path

**Duplicate Invoice Detection**

Invoice

Mines RBKP for same vendor + amount + reference + date to flag potential double payments

Prevents duplicate payment — direct financial loss prevention

Negative test: post duplicate invoice (should trigger block)

**Cash Discount Leakage**

Payment

Reads BSIK with payment terms (ZBD1T/ZBD2T) and due dates to calculate missed early-payment discounts

Quantifies financial impact of slow invoice processing in EUR/USD

Payment timing tests with discount period validation

**Payment Run Coverage Audit**

Payment

Queries REGUH to verify F110 execution frequency, vendor coverage, and payment method distribution

Ensures all eligible vendors are included in payment runs

F110 configuration tests with various vendor/payment method combos

## 13 C16 Detection Point Architecture

The following diagram illustrates how C16's detection capabilities overlay onto the end-to-end procurement flow. Each **🔍** marker represents a C16 mining touchpoint where the platform extracts intelligence from live SAP data.

🎯 C16 Detection Points Across the Procurement Lifecycle

flowchart LR subgraph P1\["Phase 1: Planning"\] A1\[MRP Run\] --> A2\[PR Created\] A2 --> A3\[PR Released\] end subgraph P2\["Phase 2: Sourcing"\] B1\[Source  
Assigned\] --> B2\[PO Created\] B2 --> B3\[PO Released\] B3 --> B4\[PO Dispatched\] end subgraph P3\["Phase 3: Receiving"\] C1\[GR Posted\] --> C2\[QM Decision\] C2 --> C3\[Stock Updated\] end subgraph P4\["Phase 4: Invoice & Pay"\] D1\[Invoice  
Posted\] --> D2\[3-Way Match\] D2 --> D3\[Payment  
Executed\] end A3 --> B1 B4 --> C1 C3 --> D1 A1 -. "🔍 MRP Parameter  
Audit MARC" .-> A1 A2 -. "🔍 PR Ageing  
EBAN" .-> A2 B1 -. "🔍 Maverick Spend  
EKKO vs EBAN" .-> B1 B2 -. "🔍 Duplicate PO  
EKKO/EKPO" .-> B2 B2 -. "🔍 Price Variance  
EKPO vs EINE" .-> B2 B3 -. "🔍 Release Strategy  
T16FS Gaps" .-> B3 C1 -. "🔍 Over-Receipt  
EKBE vs EKPO" .-> C1 C1 -. "🔍 Backdated GR  
MKPF Dates" .-> C1 C3 -. "🔍 GR/IR Imbalance  
EKBE" .-> C3 D1 -. "🔍 Duplicate Invoice  
RBKP" .-> D1 D2 -. "🔍 Blocked Invoice  
Ageing RBKP" .-> D2 D3 -. "🔍 Cash Discount  
Leakage BSIK" .-> D3 style P1 fill:#eef6ff,stroke:#3498db,stroke-width:2px style P2 fill:#fef9ef,stroke:#e67e22,stroke-width:2px style P3 fill:#eefaf3,stroke:#27ae60,stroke-width:2px style P4 fill:#fdf0f5,stroke:#e74c3c,stroke-width:2px

### 13.1 C16 Engagement Model

C16 operates in three engagement modes across the procurement lifecycle:

🔬

**Discovery Sprint**  
2–4 hours · Connect to SAP · Full P2P scan · Findings report with evidence

📊

**Deep Analysis**  
1–3 days · Custom code audit · Fit-to-Standard · Test scope reduction · Data quality scorecard

🔄

**Continuous Monitor**  
Ongoing · Compliance drift detection · Regression trigger · Change impact feeds to Tricentis

### 13.2 C16 → Tricentis Integration Flow

C16 Intelligence → Tricentis Test Automation Pipeline

flowchart LR subgraph C16\["C16 Platform"\] direction TB C1\[Data Mining  
Engine\] --> C2\[Process  
Variant Discovery\] C2 --> C3\[Gap & Risk  
Identification\] C3 --> C4\[Test Scope  
Optimization\] C4 --> C5\[Test Data  
Profiling\] end subgraph OUTPUT\["C16 Outputs"\] direction TB O1\["📋 Findings Report  
(Risk + Evidence)"\] O2\["🗺️ Process Variants  
(Happy + Exception Paths)"\] O3\["📊 Test Data Profiles  
(Edge Cases + Boundaries)"\] O4\["🎯 Test Scope Matrix  
(Prioritized Scenarios)"\] end subgraph TOSCA\["Tricentis Tosca"\] direction TB T1\[Test Case  
Design\] --> T2\[Test Data  
Management\] T2 --> T3\[Test  
Execution\] T3 --> T4\[Results &  
Reporting\] end C5 --> O1 C5 --> O2 C5 --> O3 C5 --> O4 O1 --> T1 O2 --> T1 O3 --> T2 O4 --> T1 style C16 fill:#f5f0ff,stroke:#6B3FA0,stroke-width:2px style OUTPUT fill:#fff8f0,stroke:#C97B3A,stroke-width:2px style TOSCA fill:#eef6ff,stroke:#3498db,stroke-width:2px

### 13.3 Quantified Value Proposition

Metric

Without C16

With C16

Improvement

**Discovery Time**

4–8 weeks (manual workshops, interviews, document review)

2–4 hours (automated SAP data mining)

95% faster

**Test Scope Accuracy**

Based on documentation (often outdated)

Based on live data — actual process variants discovered

Data-driven

**Test Data Quality**

Synthetic / manually curated

Real SAP data patterns with edge cases identified

Real-world

**Test Object Count**

100% of custom objects tested

50–67% reduction via Fit-to-Standard (retire/simplify)

50–67% less

**Compliance Findings**

Found during testing (late, expensive to fix)

Found during discovery (early, cheap to fix)

Shift-left

**Post-Go-Live Defects**

Higher — untested process variants

Lower — all variants discovered and tested

Reduced risk

## 14 Disclaimers & Legal Notices

#### 📋 Process Reference Disclaimer

This document presents a **reference process model** based on standard SAP ECC configuration for Direct/Stock Procurement. Actual process flows, transaction usage, and organizational responsibilities **may vary** based on client-specific customizing, industry requirements, release strategies, organizational structure, and enhancement implementations. This model should be validated against the specific client environment before use as a basis for implementation, testing, or compliance activities.

#### 🔧 C16 Platform Disclaimer

C16 capabilities described in this document are based on current platform features as of April 2026. Specific outcomes — including detection accuracy, data mining depth, and integration quality — depend on **system connectivity, SAP system version, data volume, authorization scope, and configuration completeness** of the target SAP environment. C16 operates in a read-only capacity against SAP systems for data mining and analysis activities; no production data is modified during the discovery or monitoring process.

#### 🔒 Confidentiality Notice

This document is **CONFIDENTIAL** and intended solely for the use of the individual(s) or entity to whom it is addressed. It contains proprietary process models and platform capability descriptions. Unauthorized copying, distribution, or disclosure of this document or any part thereof is strictly prohibited. If you have received this document in error, please notify the sender immediately and destroy all copies.

#### ™ Trademark Notice

SAP, SAP ECC, SAP S/4HANA, SAP Fiori, Materials Management (MM), and all related product and service names are **trademarks or registered trademarks of SAP SE** in Germany and other countries. Tricentis and Tricentis Tosca are trademarks of Tricentis GmbH. This document is **not an official publication of SAP SE or Tricentis GmbH** and does not represent the views or endorsement of either company. All other trademarks are the property of their respective owners.

#### ⚠️ No Warranty

This document is provided **"as-is" without warranty of any kind**, express or implied, including but not limited to the warranties of merchantability, fitness for a particular purpose, or non-infringement. The authors and C16 shall not be liable for any damages arising from the use of or reliance upon information contained in this document.

C16 — AI-Powered SAP Intelligence Platform

Document generated: April 2026  |  Classification: Confidential  |  Version: 1.0

Process model scope: L1 Procure-to-Pay → L2 Direct/Stock Procurement → L3 (4 Phases) → L4 (14 Activities)

Platform: SAP ECC 6.0 / EhP 7+  |  Module: MM (Materials Management) + FI-AP

© 2026 C16. All rights reserved.

mermaid.initialize({ startOnLoad: true, theme: 'base', themeVariables: { primaryColor: '#f5f0ff', primaryTextColor: '#2c3e50', primaryBorderColor: '#6B3FA0', lineColor: '#6B3FA0', secondaryColor: '#fef9ef', tertiaryColor: '#eefaf3', fontFamily: 'Segoe UI, Arial, sans-serif', fontSize: '13px' }, flowchart: { htmlLabels: true, curve: 'basis', padding: 15, nodeSpacing: 30, rankSpacing: 40 } });

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._