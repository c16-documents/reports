  Direct / Stock Procurement — Process Reference & C16 Value Map :root { --primary: #6B3FA0; --primary-light: #8B5CF6; --accent: #B5567A; --accent-warm: #C97B3A; --dark: #2c3e50; --light-bg: #f8f9fc; --card-bg: #ffffff; --success: #27ae60; --warning: #e67e22; --danger: #e74c3c; --info: #3498db; } \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, Arial, sans-serif; color: var(--dark); background: #e9ecef; line-height: 1.7; } .doc-wrapper { max-width: 1140px; margin: 30px auto; background: var(--card-bg); border-radius: 16px; box-shadow: 0 8px 40px rgba(107,63,160,0.10); overflow: hidden; } /\* ─── Header Banner ─── \*/ .header-banner { background: linear-gradient(135deg, #C97B3A, #B5567A, #6B3FA0); color: #fff; padding: 48px 56px; position: relative; overflow: hidden; } .header-banner::after { content: ''; position: absolute; top: -40%; right: -10%; width: 380px; height: 380px; background: rgba(255,255,255,0.06); border-radius: 50%; } .header-banner img { height: 50px; margin-bottom: 14px; border-radius: 8px; } .header-banner h1 { font-size: 2rem; font-weight: 700; margin-bottom: 6px; letter-spacing: -0.5px; } .header-banner .subtitle { font-size: 1.1rem; opacity: 0.92; font-weight: 400; } .header-badges { display: flex; gap: 10px; margin-top: 18px; flex-wrap: wrap; } .header-badges span { background: rgba(255,255,255,0.18); border: 1px solid rgba(255,255,255,0.3); padding: 4px 14px; border-radius: 20px; font-size: 0.82rem; font-weight: 500; backdrop-filter: blur(4px); } /\* ─── Content Area ─── \*/ .content { padding: 48px 56px; } /\* ─── Section Titles ─── \*/ .section-title { font-size: 1.45rem; font-weight: 700; color: var(--primary); border-bottom: 3px solid var(--accent); padding-bottom: 10px; margin: 48px 0 24px 0; display: flex; align-items: center; gap: 12px; } .section-title:first-child { margin-top: 0; } .section-num { background: var(--accent); color: #fff; width: 32px; height: 32px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; font-size: 0.85rem; font-weight: 700; flex-shrink: 0; } /\* ─── TOC ─── \*/ .toc { background: var(--light-bg); border-radius: 12px; padding: 28px 32px; margin-bottom: 36px; border: 1px solid #e2e8f0; } .toc h3 { font-size: 1.05rem; color: var(--primary); margin-bottom: 14px; font-weight: 700; } .toc ol { columns: 2; column-gap: 40px; padding-left: 20px; } .toc li { margin-bottom: 6px; font-size: 0.92rem; } .toc a { color: var(--primary); text-decoration: none; font-weight: 500; } .toc a:hover { text-decoration: underline; } /\* ─── KPI Cards ─── \*/ .kpi-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin: 24px 0; } .kpi-card { background: linear-gradient(135deg, #f8f4ff 0%, #fff 100%); border: 1px solid #e8dff5; border-radius: 12px; padding: 20px; text-align: center; } .kpi-card .kpi-value { font-size: 1.8rem; font-weight: 800; color: var(--primary); } .kpi-card .kpi-label { font-size: 0.82rem; color: #64748b; margin-top: 4px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px; } /\* ─── Tables ─── \*/ table { width: 100%; border-collapse: collapse; margin: 20px 0; font-size: 0.9rem; } thead th { background: var(--primary); color: #fff; padding: 12px 16px; text-align: left; font-weight: 600; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.3px; } tbody td { padding: 10px 16px; border-bottom: 1px solid #e2e8f0; vertical-align: top; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } /\* ─── Badges ─── \*/ .badge { display: inline-block; padding: 2px 10px; border-radius: 12px; font-size: 0.78rem; font-weight: 600; color: #fff; } .badge-planner { background: #3498db; } .badge-buyer { background: var(--primary); } .badge-warehouse { background: var(--success); } .badge-ap { background: var(--warning); } .badge-manager { background: var(--accent); } .badge-c16 { background: linear-gradient(135deg, #C97B3A, #B5567A); } /\* ─── C16 Callout ─── \*/ .c16-callout { background: linear-gradient(135deg, #fef9f0 0%, #fdf4ff 100%); border-left: 5px solid var(--accent); border-radius: 0 10px 10px 0; padding: 18px 22px; margin: 16px 0; font-size: 0.92rem; } .c16-callout strong { color: var(--primary); } .c16-callout .c16-tag { background: linear-gradient(135deg, #C97B3A, #B5567A, #6B3FA0); color: #fff; padding: 2px 10px; border-radius: 10px; font-size: 0.75rem; font-weight: 700; margin-right: 8px; display: inline-block; vertical-align: middle; } /\* ─── Mermaid container ─── \*/ .mermaid-container { background: #fafbfe; border: 1px solid #e2e8f0; border-radius: 12px; padding: 28px 20px; margin: 24px 0; overflow-x: auto; text-align: center; } .mermaid-container .diagram-title { font-size: 0.85rem; color: #64748b; text-transform: uppercase; letter-spacing: 1px; font-weight: 600; margin-bottom: 16px; } /\* ─── RACI ─── \*/ .raci-r { background: #e74c3c; color: #fff; padding: 2px 8px; border-radius: 6px; font-size: 0.78rem; font-weight: 700; } .raci-a { background: #e67e22; color: #fff; padding: 2px 8px; border-radius: 6px; font-size: 0.78rem; font-weight: 700; } .raci-c { background: #3498db; color: #fff; padding: 2px 8px; border-radius: 6px; font-size: 0.78rem; font-weight: 700; } .raci-i { background: #95a5a6; color: #fff; padding: 2px 8px; border-radius: 6px; font-size: 0.78rem; font-weight: 700; } /\* ─── Process Stage Cards ─── \*/ .stage-card { border: 1px solid #e2e8f0; border-radius: 12px; margin: 24px 0; overflow: hidden; } .stage-header { padding: 16px 22px; font-weight: 700; font-size: 1rem; display: flex; align-items: center; gap: 10px; } .stage-header .stage-icon { font-size: 1.3rem; } .stage-body { padding: 0 22px 22px 22px; } .stage-mrp .stage-header { background: #ebf5fb; color: #2471a3; border-bottom: 2px solid #3498db; } .stage-pr .stage-header { background: #f4ecf7; color: #6c3483; border-bottom: 2px solid var(--primary); } .stage-po .stage-header { background: #fdf2e9; color: #af601a; border-bottom: 2px solid var(--warning); } .stage-gr .stage-header { background: #eafaf1; color: #1e8449; border-bottom: 2px solid var(--success); } .stage-iv .stage-header { background: #fdedec; color: #922b21; border-bottom: 2px solid var(--danger); } .stage-py .stage-header { background: #f0f3f4; color: #2c3e50; border-bottom: 2px solid #566573; } /\* ─── Value Overlay Grid ─── \*/ .value-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin: 20px 0; } .value-card { background: #fff; border: 1px solid #e2e8f0; border-radius: 10px; padding: 18px; transition: box-shadow 0.2s; } .value-card:hover { box-shadow: 0 4px 16px rgba(107,63,160,0.12); } .value-card h4 { color: var(--primary); font-size: 0.95rem; margin-bottom: 8px; display: flex; align-items: center; gap: 8px; } .value-card p { font-size: 0.88rem; color: #475569; } /\* ─── Disclaimer ─── \*/ .disclaimer-box { background: #fffbeb; border: 1px solid #fde68a; border-radius: 10px; padding: 22px 26px; margin: 32px 0; font-size: 0.85rem; color: #92400e; } .disclaimer-box h4 { color: #b45309; font-size: 0.95rem; margin-bottom: 10px; display: flex; align-items: center; gap: 8px; } .disclaimer-box ul { padding-left: 20px; margin-top: 8px; } .disclaimer-box li { margin-bottom: 4px; } /\* ─── Footer ─── \*/ .doc-footer { background: #f8f9fc; border-top: 1px solid #e2e8f0; padding: 28px 56px; font-size: 0.82rem; color: #94a3b8; text-align: center; } .doc-footer a { color: var(--primary); text-decoration: none; } /\* ─── Print ─── \*/ @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; margin: 0; max-width: 100%; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .mermaid-container { page-break-inside: avoid; } .stage-card { page-break-inside: avoid; } } @media (max-width: 768px) { .content { padding: 24px 20px; } .header-banner { padding: 32px 24px; } .toc ol { columns: 1; } .value-grid { grid-template-columns: 1fr; } .kpi-grid { grid-template-columns: repeat(2, 1fr); } } :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,)

# Direct / Stock Procurement

End-to-End Process Reference with BPMN Diagrams & C16 Value Overlay

📦 SAP S/4HANA — MM Module 🔄 MRP → PR → PO → GR → Invoice → Payment 📅 April 2026 🔒 Confidential

### 📑 Table of Contents

1.  [Executive Summary](#exec-summary)
2.  [Process Scope & KPIs](#scope)
3.  [BPMN Overview — End-to-End Flow](#bpmn-overview)
4.  [BPMN Detailed — Swim Lane Diagram](#bpmn-detailed)
5.  [Stage-by-Stage Breakdown](#stage-breakdown)
6.  [RACI Matrix](#raci)
7.  [SAP Transaction Matrix](#transaction-matrix)
8.  [C16 Value Overlay — Intelligence at Every Stage](#c16-overlay)
9.  [C16 + Tricentis Synergy Map](#c16-tricentis)
10.  [BPMN — C16 Integration Points](#bpmn-c16)
11.  [Key Controls & Compliance](#controls)
12.  [Disclaimers & Limitations](#disclaimers)

## 1 Executive Summary

Direct / Stock Procurement is the backbone of SAP Materials Management — covering the full lifecycle from **demand identification through MRP**, to **purchase requisition creation**, **purchase order processing**, **goods receipt**, **invoice verification**, and ultimately **payment execution**. This is the most transaction-intensive and data-rich P2P sub-process, touching every organizational level from plant to company code.

This document serves two purposes: **(A)** a comprehensive process reference with BPMN diagrams, RACI matrices, and SAP transaction mapping — suitable for consulting delivery and S/4HANA transformation projects; and **(B)** a value overlay showing how **C16's AI-powered platform** can mine live SAP data at every stage to detect anomalies, assess compliance, profile data quality, and generate actionable test scenarios for **Tricentis Tosca** test suites.

6

Process Stages

15

L4 Activities

12+

SAP Transactions

5

Key Roles

8

C16 Checkpoints

## 2 Process Scope & KPIs

This process covers the **procurement of direct materials (raw materials, components, MRO items) into warehouse stock** via standard purchase orders. It applies to all plant-level procurement where materials are received into unrestricted stock via goods receipt and settled via logistics invoice verification.

Dimension

Scope

**L1 Process**

Procure to Pay (P2P)

**L2 Sub-Process**

Direct / Stock Procurement

**SAP Module**

MM (Materials Management), FI (Financial Accounting)

**Org Levels**

Company Code → Purchasing Org → Plant → Storage Location

**Document Flow**

MRP Run → PR → PO → GR (Material Doc) → Invoice (Accounting Doc) → Payment

**Process Owner**

Head of Procurement / Supply Chain Manager

**Key Performance Indicators**

PO Cycle Time, GR-to-Invoice Lead Time, 3-Way Match Rate, On-Time Delivery %, Maverick Buying Rate

## 3 BPMN Overview — End-to-End Flow

The diagram below shows the high-level process flow with key decision gateways. Each rectangular task maps to an SAP transaction; diamond shapes represent decision/validation points.

Direct / Stock Procurement — End-to-End BPMN Overview

flowchart LR A(\[🟢 Start:  
Demand Identified\]) --> B\[📊 MRP Run  
MD01N / MD02\] B --> C{MRP  
Exception?} C -->|Exception Messages| D\[📋 Review MRP List  
MD04 / MD05\] C -->|Auto-Planned| E\[📝 Create PR  
ME51N / Auto\] D --> E E --> F{PR Approval  
Required?} F -->|Yes| G\[✅ Release PR  
ME54N\] F -->|No Release Strategy| H\[📦 Create PO  
ME21N\] G --> H H --> I{PO Release  
Required?} I -->|Yes| J\[✅ Release PO  
ME29N / ME28\] I -->|No| K\[🚚 Goods Receipt  
MIGO - 101\] J --> K K --> L{Quality  
Inspection?} L -->|Yes| M\[🔬 QI Decision  
QA32\] L -->|No| N\[📄 Invoice Receipt  
MIRO\] M --> N N --> O{3-Way  
Match OK?} O -->|Yes| P\[💰 Payment Run  
F110\] O -->|Tolerance Exceeded| Q\[🔒 Block Invoice  
MRBR\] Q --> N P --> R(\[🔴 End:  
Vendor Paid\]) style A fill:#27ae60,stroke:#1e8449,color:#fff style R fill:#e74c3c,stroke:#c0392b,color:#fff style C fill:#f9e79f,stroke:#f1c40f,color:#333 style F fill:#f9e79f,stroke:#f1c40f,color:#333 style I fill:#f9e79f,stroke:#f1c40f,color:#333 style L fill:#f9e79f,stroke:#f1c40f,color:#333 style O fill:#f9e79f,stroke:#f1c40f,color:#333

## 4 BPMN Detailed — Swim Lane Diagram

This diagram organizes activities by **responsible role**, showing handoffs between the Production Planner, Buyer, Approver, Warehouse Operator, and AP Specialist.

Direct / Stock Procurement — Role-Based Swim Lanes

flowchart TD subgraph PLANNER \["🏭 Production Planner / MRP Controller"\] P1\[Run MRP  
MD01N\] --> P2\[Review MRP List  
MD04\] P2 --> P3\[Convert Planned Order  
to PR - ME51N\] end subgraph BUYER \["🛒 Buyer / Purchasing Agent"\] B1\[Review & Assign  
Source of Supply\] --> B2\[Create Purchase Order  
ME21N\] B2 --> B3\[Print / Send PO  
to Vendor - ME9F\] B3 --> B4\[Monitor PO  
Confirmations - ME2M\] end subgraph APPROVER \["✅ Approving Manager"\] A1\[Release PR  
ME54N\] --> A2\[Release PO  
ME29N / ME28\] end subgraph WAREHOUSE \["📦 Warehouse / Goods Receipt"\] W1\[Post Goods Receipt  
MIGO - Mvt 101\] --> W2\[Put Away Stock  
& Update Inventory\] end subgraph AP \["💰 Accounts Payable"\] AP1\[Post Invoice  
MIRO\] --> AP2{3-Way Match  
PO / GR / Invoice} AP2 -->|Match OK| AP3\[Invoice Cleared  
for Payment\] AP2 -->|Mismatch| AP4\[Block & Investigate  
MRBR\] AP4 --> AP1 AP3 --> AP5\[Payment Run  
F110\] end P3 --> A1 A1 --> B1 A2 --> B3 B2 --> A2 B4 --> W1 W2 --> AP1 style PLANNER fill:#ebf5fb,stroke:#3498db style BUYER fill:#f4ecf7,stroke:#8B5CF6 style APPROVER fill:#fef9e7,stroke:#f1c40f style WAREHOUSE fill:#eafaf1,stroke:#27ae60 style AP fill:#fdedec,stroke:#e74c3c

## 5 Stage-by-Stage Breakdown

📊 Stage 1 — Material Requirements Planning (MRP)

L4 Activity

Transaction

Role

Description

Execute MRP Run

**MD01N** / MD02

Planner

Run total or single-item MRP to calculate net requirements based on demand, stock levels, and safety stock parameters.

Review MRP List / Stock Requirements

**MD04** / MD05

Planner

Analyze exception messages, review planned orders, confirm or adjust quantities and dates.

Convert Planned Order to PR

**ME51N** (manual) / MD04 (auto)

Planner

Convert MRP-generated planned orders into purchase requisitions. May be automatic via MRP settings.

C16 **MRP Exception Mining:** C16 connects to the live system to profile MRP exception messages, identify materials with chronic re-scheduling, detect phantom demand patterns, and highlight MRP parameter misconfigurations (lot size, safety stock, reorder point) — feeding Tricentis with edge-case test scenarios.

📋 Stage 2 — Purchase Requisition Processing

L4 Activity

Transaction

Role

Description

Create Purchase Requisition

**ME51N**

Planner / Buyer

Create PR with material, quantity, delivery date, plant, and account assignment. Can originate from MRP or be manual.

Release / Approve PR

**ME54N**

Manager

Approve PR per release strategy (value-based, cost center-based). Multi-level approval if configured.

C16 **PR Compliance Audit:** C16 mines PR data to detect PRs without release, overdue PRs not yet converted to POs, split PRs designed to circumvent approval thresholds, and PRs with missing account assignments — enabling proactive compliance testing.

📦 Stage 3 — Purchase Order Processing

L4 Activity

Transaction

Role

Description

Create Purchase Order

**ME21N**

Buyer

Create PO from PR (with reference) or manually. Assign vendor, pricing conditions, delivery schedule, incoterms.

Release Purchase Order

**ME29N** / ME28

Manager

Release PO per release strategy. Collective release possible via ME28.

Print / Transmit PO to Vendor

**ME9F**

Buyer

Output PO via print, email, EDI, or Ariba Network. Output determination via NACE configuration.

Monitor PO Status

**ME2M** / ME2N

Buyer

Track order confirmations, overdue deliveries, partial deliveries, and vendor performance.

C16 **PO Analytics & Anomaly Detection:** C16 mines PO data to identify maverick purchases (POs without PR reference), price variance against info records and contracts, unreleased POs aging beyond thresholds, duplicate POs to the same vendor/material, and PO split patterns — all of which become targeted Tricentis test cases.

🚚 Stage 4 — Goods Receipt

L4 Activity

Transaction

Role

Description

Post Goods Receipt (Mvt 101)

**MIGO**

Warehouse

Receive materials against PO. Movement type 101 posts to unrestricted stock. Generates material document + accounting document (GR/IR clearing).

Quality Inspection (if applicable)

**QA32**

Warehouse

If QM active, GR posts to inspection stock (Mvt 103). Usage decision releases to unrestricted or returns to vendor.

Handle Returns / Over-Delivery

**MIGO** (Mvt 122)

Warehouse

Return to vendor for quality issues. Over-delivery tolerance checked against PO settings.

C16 **GR Integrity & Compliance:** C16 detects GR-without-PO postings, over-deliveries exceeding tolerance, GR reversals (Mvt 102) patterns, and GR/IR clearing account imbalances — critical test scenarios for Tricentis to validate 3-way match integrity.

📄 Stage 5 — Invoice Verification

L4 Activity

Transaction

Role

Description

Post Vendor Invoice

**MIRO**

AP Specialist

Enter invoice with reference to PO. System performs automatic 3-way match (PO qty × price vs GR qty vs Invoice amount).

Handle Invoice Blocks

**MRBR**

AP Specialist

Review and release blocked invoices. Blocks triggered by price variance, quantity variance, or manual hold.

Post Credit Memo

**MIRO** (credit memo)

AP Specialist

Process credit memos for returns, price adjustments, or subsequent debits/credits.

C16 **Invoice Variance Intelligence:** C16 profiles invoice block reasons, calculates block-to-release cycle times, identifies chronic tolerance breaches by vendor or material group, and detects duplicate invoices — providing Tricentis with a prioritized list of high-risk invoice scenarios to automate.

💰 Stage 6 — Payment Execution

L4 Activity

Transaction

Role

Description

Execute Payment Run

**F110**

AP Specialist

Automatic payment program selects open items per payment terms, payment methods, and vendor bank details. Generates payment media (bank file, check).

Manual Payment / Clearing

**F-53** / F-58

AP Specialist

Manual outgoing payment or payment with print for individual vendor clearing.

C16 **Payment Compliance & Vendor Master Audit:** C16 mines vendor master data for missing bank details, blocked payment methods, incorrect payment terms, and duplicate vendors — all of which directly impact F110 payment run success. It also detects early payments that miss cash discount opportunities.

## 6 RACI Matrix

Activity

Production Planner

Buyer

Approving Manager

Warehouse

AP Specialist

MRP Run

R

I

Review MRP List

R

C

Create PR

R

I

A

Release PR

I

I

R

Create PO

I

R

A

Release PO

I

R

Transmit PO

R

Post Goods Receipt

I

I

R

Post Invoice

C

R

Release Blocked Invoice

C

A

R

Execute Payment

A

R

R = Responsible  |  A = Accountable  |  C = Consulted  |  I = Informed

## 7 SAP Transaction Matrix

Stage

Transaction

Description

SAP Module

Key Tables

MRP

**MD01N**

Total Planning Run (MRP)

PP-MRP

MDKP, MDTB, RESB

MRP

**MD04**

Stock/Requirements List

PP-MRP

MDKP, MDTB, MARD

PR

**ME51N**

Create Purchase Requisition

MM-PUR

EBAN, EBKN

PR

**ME54N**

Release Purchase Requisition

MM-PUR

EBAN (FRGST)

PO

**ME21N**

Create Purchase Order

MM-PUR

EKKO, EKPO, EKET

PO

**ME29N**

Release Purchase Order

MM-PUR

EKKO (FRGKE, FRGZU)

PO

**ME9F**

Message Output (Print PO)

MM-PUR

NAST

PO

**ME2M**

PO List by Material

MM-PUR

EKKO, EKPO, EKBE

GR

**MIGO**

Goods Movement (GR 101)

MM-IM

MKPF, MSEG, MARD

GR

**QA32**

Quality Inspection Decision

QM

QALS, QAVE

Invoice

**MIRO**

Enter Vendor Invoice

MM-IV

RBKP, RSEG, BSEG

Invoice

**MRBR**

Release Blocked Invoices

MM-IV

RBKP (RBSTAT)

Payment

**F110**

Automatic Payment Program

FI-AP

REGUH, REGUP, BSIK

Payment

**F-53**

Manual Outgoing Payment

FI-AP

BSEG, BKPF

## 8 C16 Value Overlay — Intelligence at Every Stage

C16 connects to the live SAP system and mines data **at each process stage**. The table below maps every C16 capability to the specific procurement stage where it delivers value.

Stage

C16 Capability

What It Detects

Tricentis Test Feed

**MRP**

MRP Exception Profiling

Materials with chronic rescheduling, phantom demand, misconfigured lot sizes & safety stock

Edge-case MRP scenarios per exception code

**PR**

PR Compliance Mining

Unreleased PRs, split PRs to avoid thresholds, aging PRs not converted to POs

Release strategy coverage tests, threshold boundary tests

**PO**

PO Anomaly Detection

Maverick buying, price variance vs info records, unreleased POs, duplicate POs, PO without contract reference

Price boundary tests, release strategy tests, duplicate detection regression

**GR**

GR Integrity Checks

Over-deliveries beyond tolerance, GR reversals, GR-without-PO, GR/IR clearing imbalances

Tolerance boundary tests, reversal workflow tests, 3-way match setup

**Invoice**

Invoice Variance Intelligence

Block reasons distribution, block-to-release cycle time, duplicate invoices, chronic tolerance breaches

Invoice block/release cycle tests, duplicate detection, variance threshold tests

**Payment**

Vendor Master & Payment Audit

Missing bank details, blocked vendors, incorrect payment terms, early/late payment patterns, cash discount leakage

F110 payment run tests per vendor profile, bank detail validation tests

**Cross-Stage**

Custom Code Discovery

Z-programs, user exits, BAdIs, enhancements touching MM tables — assessed for Clean Core compliance

Regression test scope for custom code, impact analysis for S/4HANA migration

**Cross-Stage**

Master Data Quality Scorecard

Vendor master completeness (name, address, bank, tax, payment terms), material master gaps (MRP views, purchasing views)

Master data validation test suite, negative test cases for incomplete records

## 9 C16 + Tricentis Synergy Map

The partnership between C16 and Tricentis creates a **closed-loop intelligence → testing pipeline**. C16 discovers _what to test_ and _with what data_; Tricentis Tosca executes the tests and validates at scale.

#### 🔍 Discovery → Test Scope

C16 scans the entire Z-namespace to build a complete inventory of custom objects. Each object is classified (Retire / Simplify / Modernize / Keep) with Fit-to-Standard scoring. Tricentis receives a **prioritized test backlog** — not a flat list, but a risk-ranked scope with coverage recommendations.

#### 📊 Data Mining → Test Data

C16 profiles live SAP tables to identify realistic data combinations, edge cases, boundary values, and error-prone patterns. Instead of synthetic test data, Tricentis tests with **production-representative data profiles** that reflect actual system behavior.

#### 🔄 Process Mining → Test Scenarios

C16's process mining engine extracts actual process variants from document flow tables. Each variant (happy path, exception path, rework loop) becomes a **Tosca test case template** with pre-populated transaction sequences and expected outcomes.

#### 🛡️ Compliance → Continuous Testing

After go-live, C16 monitors for configuration drift, new Z-objects, and data quality degradation. Each detection triggers a **Tricentis regression cycle** — ensuring the system stays clean post-migration.

C16 + Tricentis — Closed-Loop Intelligence Pipeline

flowchart LR subgraph C16 \["🧠 C16 Platform"\] C1\[Custom Code  
Discovery\] --> C2\[Data Mining  
& Profiling\] C2 --> C3\[Process Mining  
& Variants\] C3 --> C4\[Compliance  
Scoring\] end subgraph BRIDGE \["🔗 Integration Layer"\] BR1\[Test Scope  
Backlog\] BR2\[Test Data  
Profiles\] BR3\[Test Scenario  
Templates\] BR4\[Regression  
Triggers\] end subgraph TOSCA \["🧪 Tricentis Tosca"\] T1\[Test Case  
Design\] --> T2\[Test Execution  
& Automation\] T2 --> T3\[Results &  
Reporting\] end C1 --> BR1 C2 --> BR2 C3 --> BR3 C4 --> BR4 BR1 --> T1 BR2 --> T1 BR3 --> T1 BR4 --> T2 style C16 fill:#f4ecf7,stroke:#8B5CF6 style BRIDGE fill:#fef9e7,stroke:#f1c40f style TOSCA fill:#eafaf1,stroke:#27ae60

## 10 BPMN — C16 Integration Points

This diagram overlays C16 intelligence checkpoints on the process flow. The **pink nodes** represent C16 intervention points where the platform adds value through real-time data mining.

Direct / Stock Procurement — With C16 Intelligence Overlay

flowchart TD A(\[🟢 Demand  
Identified\]) --> B\[MRP Run  
MD01N\] B --> C16\_1\[/"🧠 C16: MRP Exception  
Profiling & Parameter Audit"/\] C16\_1 --> D\[Create PR  
ME51N\] D --> C16\_2\[/"🧠 C16: PR Split Detection  
& Compliance Scan"/\] C16\_2 --> E\[Release PR  
ME54N\] E --> F\[Create PO  
ME21N\] F --> C16\_3\[/"🧠 C16: Maverick Buy Detection  
Price Variance & Duplicate PO"/\] C16\_3 --> G\[Release PO  
ME29N\] G --> H\[Goods Receipt  
MIGO 101\] H --> C16\_4\[/"🧠 C16: GR Integrity Check  
Over-Delivery & GR-IR Balance"/\] C16\_4 --> I\[Invoice  
MIRO\] I --> C16\_5\[/"🧠 C16: Invoice Block Analysis  
Duplicate Detection"/\] C16\_5 --> J{3-Way  
Match?} J -->|OK| K\[Payment  
F110\] J -->|Blocked| L\[Investigate  
MRBR\] L --> I K --> C16\_6\[/"🧠 C16: Vendor Master Audit  
Payment Term Compliance"/\] C16\_6 --> M(\[🔴 Vendor Paid\]) style C16\_1 fill:#fce4ec,stroke:#B5567A,color:#880e4f style C16\_2 fill:#fce4ec,stroke:#B5567A,color:#880e4f style C16\_3 fill:#fce4ec,stroke:#B5567A,color:#880e4f style C16\_4 fill:#fce4ec,stroke:#B5567A,color:#880e4f style C16\_5 fill:#fce4ec,stroke:#B5567A,color:#880e4f style C16\_6 fill:#fce4ec,stroke:#B5567A,color:#880e4f style A fill:#27ae60,stroke:#1e8449,color:#fff style M fill:#e74c3c,stroke:#c0392b,color:#fff style J fill:#f9e79f,stroke:#f1c40f,color:#333

## 11 Key Controls & Compliance

Control

Stage

SAP Configuration

Risk if Missing

**PR Release Strategy**

PR

T16FS / T16FW / T16FD — Release groups, codes, strategies

Unauthorized purchases bypass approval

**PO Release Strategy**

PO

T16FS (purchasing group) — Value-based release

High-value POs executed without management approval

**Source of Supply Check**

PO

ME01 (Quota Arrangement), ME11 (Info Record)

Maverick buying, non-preferred vendor selection

**GR Tolerance Check**

GR

OMR6 — Over/under delivery tolerance %

Accepting deliveries significantly over/under PO quantity

**3-Way Match**

Invoice

OMRM — Invoice tolerance keys (BD, AN, AP, DW, VP, etc.)

Paying incorrect amounts, variance not caught

**Payment Block**

Payment

LFB1 — Vendor payment block (ZAHLS)

Paying blocked/disputed vendors

**Segregation of Duties**

All

PFCG roles — Separate PR creator / PO creator / GR poster / AP poster

Fraud risk: one person controls entire P2P cycle

C16 **Segregation of Duties Analysis:** C16 can mine user activity logs and role assignments to identify SoD conflicts — for example, the same user creating a PO and posting the GR, or a single user both creating and releasing a PO. These findings map directly to Tricentis access control test scenarios.

## 12 Disclaimers & Limitations

#### ⚠️ Important Disclaimers

*   **Reference Document Only:** This document is intended as a process reference and value proposition guide. It does not constitute an implementation specification, system design document, or binding statement of work.
*   **Illustrative Process Flow:** The BPMN diagrams represent a typical/standard Direct Stock Procurement flow based on SAP S/4HANA best practices. Actual process flows vary significantly based on organizational structure, industry, customizations, and configuration. These diagrams should be validated against the specific client's process landscape before use.
*   **SAP Transaction References:** Transaction codes, table names, and configuration references are based on standard SAP S/4HANA / ECC functionality as of April 2026. SAP may deprecate, rename, or restructure these objects in future releases. Always verify against the target system's specific version and patch level.
*   **C16 Capabilities:** The C16 value overlay represents the platform's current capabilities as of April 2026. Specific capabilities, detection accuracy, and data mining depth may vary depending on the client's SAP system version, data volume, authorization model, and network connectivity. C16 requires a configured service user with appropriate RFC and OData access — capabilities described herein assume sufficient system access is provisioned.
*   **Tricentis Integration:** The C16-to-Tricentis synergy described in this document represents a conceptual integration pathway. The actual integration mechanism (API feeds, file exports, manual handoff) depends on the implementation approach agreed upon with the customer. Tricentis Tosca is a trademark of Tricentis GmbH.
*   **Not a Substitute for Expertise:** This document supplements, but does not replace, domain expertise from certified SAP MM/FI consultants. Process decisions, configuration choices, and control design should be reviewed by qualified professionals with knowledge of the client's specific business requirements and regulatory environment.
*   **Data Privacy:** When C16 connects to a client's SAP system, it accesses live transactional and master data. All data mining activities must comply with the client's data governance policies, applicable data protection regulations (GDPR, DPDPA, etc.), and any contractual data handling agreements. No persistent data storage occurs outside the client's environment unless explicitly configured and authorized.
*   **No Warranty:** This document is provided "as-is" without warranty of any kind, express or implied. The authors assume no liability for decisions made based on this document's content.

#### ℹ️ Version & Distribution

*   **Version:** 1.0 — April 2026
*   **Classification:** Confidential — For internal use and authorized customer presentations only
*   **Distribution:** Restricted to C16 sales/pre-sales team and authorized partner representatives
*   **Review Cycle:** Quarterly — or upon major SAP S/4HANA release updates
*   **Contact:** For questions or corrections, contact the C16 Solutions team

**C16** — AI-Powered SAP Intelligence Platform

Direct / Stock Procurement — Process Reference & C16 Value Overlay  |  April 2026  |  v1.0

© 2026 C16. All rights reserved. SAP, S/4HANA, and related marks are trademarks of SAP SE. Tricentis and Tosca are trademarks of Tricentis GmbH.

mermaid.initialize({ startOnLoad: true, theme: 'base', themeVariables: { primaryColor: '#f4ecf7', primaryTextColor: '#2c3e50', primaryBorderColor: '#8B5CF6', lineColor: '#8B5CF6', secondaryColor: '#fef9e7', tertiaryColor: '#eafaf1', fontSize: '13px' }, flowchart: { htmlLabels: true, curve: 'basis', padding: 12, nodeSpacing: 30, rankSpacing: 40 } });

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._