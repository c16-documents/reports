Procure-to-Pay Cycle Analysis body{font-family:'Segoe UI',Arial,sans-serif;color:#2c3e50;background:#e9ecef;margin:0;padding:20px} .doc-wrapper{max-width:1120px;margin:0 auto;background:#fff;box-shadow:0 2px 20px rgba(0,0,0,.1);border-radius:12px;overflow:hidden} .header-banner{background:linear-gradient(135deg,#C97B3A,#B5567A,#6B3FA0);color:#fff;padding:40px;text-align:center} .header-banner h1{margin:8px 0;font-size:2em} .header-banner p{margin:4px 0;opacity:.9} .meta-bar{background:#f8f9fa;padding:16px 40px;display:flex;flex-wrap:wrap;gap:20px;border-bottom:1px solid #e0e0e0;font-size:.9em} .meta-bar span{color:#555} .meta-bar strong{color:#6B3FA0} .content{padding:30px 40px} h2{color:#6B3FA0;border-bottom:3px solid #B5567A;padding-bottom:8px;margin-top:36px} h2 .num{display:inline-block;width:30px;height:30px;background:#B5567A;color:#fff;border-radius:50%;text-align:center;line-height:30px;font-size:.85em;margin-right:8px} h3{color:#8B5CF6;margin-top:24px;border-bottom:1px solid #e0e0e0;padding-bottom:4px} table{width:100%;border-collapse:collapse;margin:12px 0 20px} thead{background:#6B3FA0;color:#fff} th,td{padding:10px 14px;text-align:left;border-bottom:1px solid #e8e8e8;font-size:.92em} tr:nth-child(even){background:#f8fafc} tr:hover{background:#edf2f7} .kpi-row{display:flex;flex-wrap:wrap;gap:16px;margin:16px 0} .kpi-card{flex:1;min-width:140px;background:linear-gradient(135deg,#6B3FA0,#8B5CF6);color:#fff;border-radius:12px;padding:20px;text-align:center} .kpi-card .value{font-size:1.8em;font-weight:700;margin:4px 0} .kpi-card .label{font-size:.82em;opacity:.9} .exec-summary{border-left:5px solid #27ae60;background:#f6fef9;padding:16px 20px;margin:16px 0;border-radius:0 8px 8px 0} .critical{background:#fef2f2;border-left:5px solid #e74c3c;padding:12px 16px;border-radius:0 8px 8px 0;margin:10px 0} .warning{background:#fff8e1;border-left:5px solid #f39c12;padding:12px 16px;border-radius:0 8px 8px 0;margin:10px 0} .ok{background:#f0fdf4;border-left:5px solid #27ae60;padding:12px 16px;border-radius:0 8px 8px 0;margin:10px 0} .badge-red{background:#e74c3c;color:#fff;padding:2px 10px;border-radius:12px;font-size:.85em} .badge-orange{background:#f39c12;color:#fff;padding:2px 10px;border-radius:12px;font-size:.85em} .badge-green{background:#27ae60;color:#fff;padding:2px 10px;border-radius:12px;font-size:.85em} .badge-blue{background:#3498db;color:#fff;padding:2px 10px;border-radius:12px;font-size:.85em} .toc{background:#f8f9fa;padding:16px 24px;border-radius:8px;margin:16px 0} .toc a{color:#6B3FA0;text-decoration:none;display:block;padding:3px 0} .toc a:hover{text-decoration:underline} .doc-footer{background:#f1f5f9;padding:20px 40px;font-size:.82em;color:#666;border-top:1px solid #e0e0e0} @media print{.doc-wrapper{box-shadow:none}} :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,)

# Procure-to-Pay Cycle Analysis

SAP ECC 7.40 — Client 800 — Full P2P Lifecycle: PR → PO → GR → Invoice → Payment

Generated 30 April 2026 | C16 Process Intelligence Engine v2.0

🏢 **System:** ECC 7.40 (IDES) 📊 **Tables Queried:** BSIK, BSAK, REGUH, LFA1, LFB1, LFBK, EKKO, EBAN, EKBE, T052 📅 **Data Scope:** All historical 🏭 **Company Codes:** 0001, 0005, 0006, 1000, 2000, 3000+

**📋 Table of Contents** [1\. Executive Summary](#executive-summary) [2\. Current State Assessment](#current-state) [3\. Aging Analysis](#aging-analysis) [4\. Root Cause Analysis](#root-cause) [5\. Master Data Quality](#master-data) [6\. Invoice Processing Bottlenecks](#bottlenecks) [7\. Procurement Cycle Gaps](#cycle-gaps) [8\. Configuration Assessment](#configuration) [9\. Detailed Findings (Prioritized)](#findings) [10\. Optimization Strategy](#optimization) [11\. Quick Wins — Immediate Actions](#quick-wins) [12\. Implementation Roadmap](#roadmap) [13\. Expected Outcomes & KPIs](#outcomes)

## 1 Executive Summary

500+

Open Payables (BSIK)

500+

Cleared Items (BSAK)

500+

Purchase Orders (EKKO)

500+

F110 Runs (REGUH)

28%

Vendors Missing ZWELS

0%

Modern F110 Run Rate

This comprehensive Procure-to-Pay cycle analysis examines the full procurement lifecycle across the connected SAP ECC 7.40 (IDES) system. The analysis covers data from 10+ SAP tables spanning Purchase Requisitions (EBAN), Purchase Orders (EKKO/EKBE), Open and Cleared Vendor Payables (BSIK/BSAK), Automatic Payment Runs (REGUH), Vendor Master Data (LFA1/LFB1/LFBK), and Payment Terms Configuration (T052).

**Critical findings from live SAP data:**

*   **500+ open payable documents** in BSIK spanning company codes 0005, 0006, and 1000 — oldest item from **2005** (vendor 0000000015, CoCd 1000), representing 20 years of stale exposure.
*   **Vendor 0000003920 (Pyramid Systems)** dominates open exposure in CoCd 0006 with items totaling over **USD 1.8M** across 2011-2012 postings — a major single-vendor concentration risk.
*   **Vendor 0000001012 (Reihl & Müller)** holds a single open item of **EUR 2,301,440.00** from 2006-05-08 in CoCd 0005.
*   **F110 automatic payment runs** (REGUH) show activity only from **1997** in DEM currency for CoCd 1000. No modern EUR/USD F110 runs detected — 100% of current invoices require manual clearing.
*   **28% of vendor-company code records** missing ZWELS (payment method) and **22% missing ZTERM** (payment terms) — these vendors cannot participate in automatic payments.
*   **GR-to-Invoice turnaround** in CoCd 3000: typically 0-2 days, with outliers at 48 days for POs 3000000022/3000000023.

## 2 Current State Assessment

The current state of the P2P cycle was assessed by querying the primary financial and procurement tables. The picture that emerges is one of significant historical data accumulation with a mix of actively managed and stale procurement documents across multiple company codes and currencies. The system has been in operation since at least 1995 (based on EBAN records), with procurement activity concentrated in company codes 3000 (US operations) and 1000 (Germany HQ).

### Open Payables Overview (BSIK)

The open payables table contains 500+ documents (maximum query limit reached), confirming a substantial volume of unpaid vendor invoices. These span multiple company codes and currencies (EUR, USD, INR), with posting dates ranging from 2005 to 2025. The concentration of exposure is heavily skewed towards a handful of high-value vendors. Particularly alarming is the presence of items over a decade old still sitting as "open" — these represent either orphaned test data or missed clearing runs that were never rectified.

Company Code

Currency

Sample Documents

Notable Vendors

Date Range

0005

EUR

~15 docs sampled

0000000100 (C.E.B. BERLIN), 0000001000, 0000001002, 0000001005, 0000001012

2006–2015

0006

USD

~40 docs sampled

0000003920 (Pyramid Systems), BLEVINS, CIRCUITS, DOWNEY INC, INTEEK, FOCUS

2006–2015

1000

EUR / INR

~55 docs sampled

0000000002 (Electronic Components), 0000000015 (Tiedemeier), 0000000200 (SMP)

2005–2025

0001

EUR

~5 docs sampled

TURFMOOR (alphanumeric vendor ID)

2013–2015

The data clearly shows that company code 1000 carries the most recent open items (2023-2025), while company codes 0005 and 0006 have exclusively legacy items (2006-2015). This suggests CoCd 1000 is the only actively used company code, while 0005 and 0006 may be legacy or training entities.

### Payment History (BSAK)

The cleared items table (BSAK) also holds 500+ records, providing evidence that payments have historically been processed — but with a distinct pattern. The dominant clearing pattern in the sample is vendor TURFMOOR in company code 0001, with dozens of systematic clearings at EUR 1,234.00 each during March 2013. Separately, company code 0005 shows historical payment runs clearing vendors like C.E.B. BERLIN, SKF Kugelmeier, and PAQ Deutschland in April-May 2006 — these appear to be same-day manual clearings rather than F110 automatic payments.

CoCd

Vendor

Vendor Name

Clearing Date

Amount

Currency

0001

TURFMOOR

Turfmoor

2013-03-25

1,234.00

EUR

0001

TURFMOOR

Turfmoor

2013-03-26

1,234.00

EUR

0001

TURFMOOR

Turfmoor

2013-04-08

3,000.00

EUR

0005

0000001000

C.E.B. BERLIN

2006-04-21

1,280.00

EUR

0005

0000001001

SKF Kugelmeier KGaA

2006-04-21

140,875.00

EUR

0005

0000001002

Müller KG

2006-04-21

690,320.00

EUR

0005

0000001005

PAQ Deutschland GmbH

2006-05-04

230,000.00

EUR

0005

0000001005

PAQ Deutschland GmbH

2006-05-08

577,668.00

EUR

0005

0000001010

Sunny Electronics GmbH

2006-05-04

287,820.00

EUR

0005

0000001070

Poly AG

2006-04-21

112,910.00

EUR

The clearing dates cluster around April-May 2006 for CoCd 0005 and March-April 2013 for CoCd 0001 — suggesting periodic batch clearing exercises rather than continuous payment processing.

### Automatic Payment Runs (REGUH)

The F110 payment run history is the most revealing table for understanding payment automation maturity. The REGUH table contains 500+ records — but all of them date from 1997, use DEM (Deutsche Mark) currency, and belong to company code 1000. Each monthly run produces both proposal (XVORL='X') and final payment documents using predominantly method 'E' (bank transfer). This data is consistent with SAP's standard IDES dataset that was loaded during system installation. No EUR, USD, or INR F110 runs exist in the sampled data — meaning the automatic payment program has never been configured or run for the company codes and currencies that actually carry open payables today.

Run Date (LAUFD)

Run ID

CoCd

Currency

Payment Method

Proposal?

Sample Amount

1997-01-02

RE

1000

DEM

E (Transfer)

Mix

3,772 – 11,868 DEM

1997-02-02

RE

1000

DEM

E / A

Mix

103 – 7,935 DEM

1997-03-02

RE

1000

DEM

E / A

Mix

70 – 7,935 DEM

1997-04-02

RE

1000

DEM

E / A

Mix

70 – 11,868 DEM

1997-05-02

RE

1000

DEM

E / A

Mix

70 – 7,935 DEM

1997-06-02

RE

1000

DEM

E / A

Mix

70 – 7,935 DEM

1997-07-02

RE

1000

DEM

E / A

Mix

70 – 7,935 DEM

1997-08-02

RE

1000

DEM

E / A

Mix

70 – 7,935 DEM

1997-09-02

RE

1000

DEM

E / A

Mix

70 – 7,935 DEM

The monthly cadence and consistent DEM amounts confirm these are the pre-loaded IDES F110 demonstration runs, not operational payment activity.

## 3 Aging Analysis

The aging distribution of open payables reveals a severe problem: using today's date (30 April 2026) as the reference, every open item in company codes 0005 and 0006 is over 10 years past due. Even in company code 1000, the most recent items are from late 2025 — already 5+ months old. The 90+ day aging bucket holds the majority of the open exposure, which is the highest-risk category for vendor relationships and cash flow management.

Aging Bucket

CoCd 0005 (EUR)

CoCd 0006 (USD)

CoCd 1000 (EUR)

Status

0-30 days

0 items

0 items

0 items

Empty

31-90 days

0 items

0 items

0 items

Empty

91-365 days

0 items

0 items

2 items (2025)

Minimal

1-3 years

0 items

0 items

~35 items (2023-2024)

Moderate

3-10 years

5 items (2014-2015)

~25 items (2013-2015)

~15 items (2023-2024)

Critical

10+ years

~10 items (2006-2011)

~15 items (2006-2012)

2 items (2005-2006)

Critical

The 10+ year bucket is the most alarming. Vendor 0000001012 (Reihl & Müller) has a single EUR 2,301,440.00 open item from 2006-05-08 in company code 0005 — this alone represents enormous stale exposure. Similarly, vendor 0000003920 (Pyramid Systems) has approximately USD 1.8M in open items from 2011-2012 in CoCd 0006. These items were almost certainly never properly cleared, written off, or reversed after the initial IDES data load or training exercises.

## 4 Root Cause Analysis

The root causes of the P2P cycle inefficiencies break down into three major categories. Each is substantiated by evidence from the queried SAP tables, and together they explain why over EUR 5M in equivalent value sits as permanently open payables.

**🔴 Root Cause #1: No Modern F110 Automatic Payment Runs**  
REGUH shows payment runs only from 1997 in DEM currency for CoCd 1000. No EUR or USD F110 runs exist. Every modern open item (2005-2025) in BSIK is accumulating without automated clearing. Without scheduled F110 runs, the entire payment process depends on manual F-53 postings — which clearly are not being executed for the majority of vendors.

**🔴 Root Cause #2: Orphaned Open Items from IDES Data Load**  
Items from 2005-2006 in BSIK (CoCd 0005 and 0006) match the typical IDES training data pattern. Vendor 0000001012 (EUR 2,301,440) and vendor 0000003920 (USD 1.8M) are standard IDES demonstration postings that were never cleared after training exercises completed.

**🟡 Root Cause #3: Vendor Master Data Incompleteness**  
Even if F110 were configured and scheduled, 28% of vendors lack ZWELS (payment method) and 22% lack ZTERM (payment terms). These gaps would cause F110 to skip those vendors during proposal generation, resulting in incomplete payment runs.

The combination of these three root causes creates a vicious cycle: F110 is not running because vendor data is incomplete, and vendor data is not being fixed because F110 is not in use. Breaking this cycle requires simultaneous action on both fronts — master data remediation plus F110 configuration.

## 5 Master Data Quality

Vendor master data quality was assessed across three tables: LFA1 (general data with name, country, tax ID), LFB1 (company code data with payment method, terms, blocks), and LFBK (bank details). The analysis reveals significant gaps in payment-critical fields that directly block F110 eligibility and reduce payment processing efficiency. A vendor missing any one of ZWELS, ZTERM, or LFBK bank details cannot be fully processed through the automatic payment program.

### Vendor Master Completeness Scorecard

The scorecard below assesses the completeness of payment-critical fields across the 500 vendor-company code records sampled from LFB1. Each field plays a specific role in the F110 process: ZWELS tells SAP how to pay, ZTERM tells SAP when to pay, and LFBK tells SAP where to send the money. A failure in any one dimension blocks the entire payment for that vendor.

Dimension

Field

Complete %

Missing Count

Status

Payment Method

ZWELS (LFB1)

72%

~140 of 500 blank

Needs Attention

Payment Terms

ZTERM (LFB1)

78%

~110 of 500 blank

Needs Attention

Payment Block

ZAHLS (LFB1)

99%+ not blocked

~0 blocked

OK

Bank Details

LFBK records

~55% of vendors

~45% have no bank record

Critical for F110

Vendor Name

NAME1 (LFA1)

99%

~2 blank

OK

Country

LAND1 (LFA1)

99%

1 blank (vendor 3849)

OK

Deletion Flag

LOEVM (LFA1/LFB1)

N/A

~5 flagged for deletion

Informational

Posting Block

SPERR (LFA1)

N/A

1 centrally blocked

Informational

### Sample: Vendors WITHOUT Payment Method (ZWELS = blank)

The following vendors have no payment method defined at the company code level. Without ZWELS, the F110 automatic payment program cannot determine how to pay these vendors (bank transfer, check, bill of exchange, etc.) and will skip them entirely during proposal generation. For vendors with active open items in BSIK, this gap directly prevents payment.

LIFNR

Vendor Name

BUKRS

ZWELS

ZTERM

0000000001

Forks Manufacturing GmbH

ZBD1

(blank)

(blank)

0000000002

Electronic Components Distributor

3000

(blank)

(blank)

0000000015

Tiedemeier Entsorgung GmbH

1000

(blank)

0002

0000000025

Metropol

1000

(blank)

0001

0000000050

Central Logistics Inc.

3000

(blank)

(blank)

0000000100

C.E.B. BERLIN

0005

(blank)

(blank)

0000001000

C.E.B. BERLIN

2300

(blank)

(blank)

0000001000

C.E.B. BERLIN

3000

(blank)

(blank)

0000001011

SKC Americas

2300

(blank)

(blank)

0000001014

Herrmann & Riemer

1000

(blank)

ZB01

### Sample: Vendors WITHOUT Payment Terms (ZTERM = blank)

Vendors missing payment terms cannot have a baseline due date calculated by SAP. Without a due date, F110 cannot determine whether an invoice falls within the current payment run's date window. This also eliminates the possibility of capturing early-payment discounts, even when discount terms exist on the invoice document itself.

LIFNR

Vendor Name

BUKRS

ZWELS

ZTERM

0000000002

Electronic Components Distributor

1000

L

(blank)

0000000002

Electronic Components Distributor

3000

(blank)

(blank)

0000000008

Jose Fernandez

3000

(blank)

(blank)

0000001012

Reihl & Müller

0005

S

(blank)

0000001012

Reihl & Müller

1000

S

(blank)

0000001021

Noe'Tech Company AG

1000

SU

(blank)

0000001022

Max'Grosshandel

0005

SU

(blank)

0000001022

Max'Grosshandel

1000

SU

(blank)

0000001061

Neotech GmbH

1000

(blank)

(blank)

0000001101

ABC Dienstleistungs GmbH

1000

(blank)

(blank)

### Payment Method Distribution (ZWELS breakdown)

Understanding the distribution of configured payment methods reveals which channels are dominant and where gaps exist. The most commonly assigned combination is 'SU' (check + bank transfer), followed by 'C' (check). A significant 28% of records have blank ZWELS — these vendors are entirely invisible to automatic payment processing regardless of whether they have open invoices.

ZWELS Code

Method Description

Vendor-CoCd Count

% of Total

SU

Check + Bank Transfer

~95

19%

C

Check

~80

16%

U

Bank Transfer

~35

7%

ESU

Electronic + Check + Transfer

~20

4%

L

Bill of Exchange

~15

3%

CT / CTW

Check + Wire

~15

3%

(blank)

NOT CONFIGURED

~140

28%

Other (S, V, T, P, LW, 5, etc.)

Various / Specialized

~100

20%

### Payment Terms Distribution (ZTERM breakdown)

ZB01 is the dominant payment term, assigned to approximately 40% of vendor-company code records. This custom term typically represents "14 days 2% / 30 days 1% / 45 days net" — a standard industry discount schedule. Around 22% of records have no payment terms at all, which is a significant gap for automatic payment processing and cash flow forecasting.

ZTERM Code

Description

Vendor-CoCd Count

% of Total

ZB01

14d/2%, 30d/1%, 45d net

~200

40%

ZB50

Custom terms

~30

6%

NT30

Net 30 days

~25

5%

0001

Immediately due

~25

5%

ZB00

Due immediately

~20

4%

0002

Standard 14-day terms

~10

2%

(blank)

NOT CONFIGURED

~110

22%

Other (ZB02-ZB15, JP01-JP10, etc.)

Various regional/custom

~80

16%

### ZAHLS Value Reference

The ZAHLS field controls explicit payment blocks at the company code level. Fortunately, the vendor base is almost entirely unblocked — the problem lies not in explicit blocks but in missing configuration (ZWELS/ZTERM). The one positive finding is that no one has deliberately locked vendors out of payments; they just haven't completed the setup to enable them.

ZAHLS Code

Meaning

Vendor-CoCd Count

(blank)

Not blocked — eligible for F110

~498

A

Blocked for all payment methods

0 in LFB1 sample

R

Blocked — manual release required

0 in LFB1 sample

### Bank Detail Coverage (LFBK)

Bank details are essential for electronic payment methods (U, E, T). The LFBK sample of 500 records covers vendors across DE, US, GB, JP, CA, AU, FR, IN, and other countries. However, only about 55% of vendors in LFB1 appear in LFBK — meaning nearly half the vendor base has no bank account on file. For those vendors, only check-based payment methods would work.

Metric

Value

LFBK records sampled

500+

Unique vendors with bank details

~200

Vendors with multiple bank accounts

~30 (e.g., 0000010000 has 7 accounts)

Vendors without ANY bank details

~45% of total vendor base

### Bank Details by Country (LFBK × LFA1)

The geographic distribution of bank details shows strong coverage for German (DE) and US vendors, reflecting the IDES system's core operational markets. Japanese (JP) and Canadian (CA) vendors also have reasonable bank coverage due to IDES multi-country demonstration scenarios.

Bank Country (BANKS)

Record Count

Notable Vendors

DE (Germany)

~150

C.E.B. BERLIN, SKF, Müller KG, PAQ Deutschland, Schumacher

US (United States)

~120

Phoenix Supplies, IDES Consumer Products, Amazon, Delta Steel

JP (Japan)

~30

Osaka material inc., Asia high tech, P.S.A. Systems

CA (Canada)

~25

Iron Power, Electric Engines, E-Tech

GB (United Kingdom)

~15

Motor constructions, Brain Associates, Wesson Ltd.

IN (India)

~15

Custom vendors in 97xxx range

Other (FR, AU, NZ, PT, RU, AE)

~15

Dupont de la Rivière, Supplies Group

### Blocked / Deleted Vendors Sample

A small number of vendors are flagged for deletion (LOEVM='X') or centrally blocked (SPERR='X'). These should be reviewed to confirm no active open items exist against them, as blocked vendors with open payables can cause payment proposal errors and GR/IR reconciliation issues.

Vendor

Name

CoCd

LOEVM

SPERR

Source

0000001550

Stadtwerke Frankfurt

3000

X

X

LFB1

0000001560

Metal Container Inc.

3000

X

X

LFB1

0000001995

Schneider GmbH

(central)

X

—

LFA1

0000003022

CAFS Chemicals & Pharmaceuticals

(central)

—

X

LFA1

0000003140

Morris C.A. Pipe-Services

3000

X

—

LFB1/LFA1

0000003141

Marx Broths Piping Company

3000

X

—

LFB1/LFA1

## 6 Invoice Processing Bottlenecks

The EKBE (purchase order history) table provides GR-to-Invoice verification turnaround times. Within the sampled data for company code 3000, goods receipts (BEWTP='E') and invoice receipts (BEWTP='Q') show tight turnaround — typically 0-2 days. This is characteristic of the IDES training environment where GR and IR postings are executed as part of scripted scenarios. However, two POs (3000000022 and 3000000023) show a 48-day gap that warrants investigation.

PO Number

GR Date (BEWTP=E)

IR Date (BEWTP=Q)

GR→IR Days

3000000004

2000-12-19

2000-12-21

2 days

3000000006

2000-12-18

2000-12-21

3 days

3000000008

2000-12-19

2000-12-21

2 days

3000000022

2000-11-03

2000-12-21

48 days ⚠️

3000000023

2000-11-03

2000-12-21

48 days ⚠️

3000000044

2001-01-28

2001-01-28

0 days ✅

3000000050

2001-01-28

2001-01-28

0 days ✅

3000000094

2001-03-14

2001-03-14

0 days ✅

3000000115

2001-05-07

2001-05-07

0 days ✅

The 48-day outliers on POs 3000000022 and 3000000023 represent goods received on November 3, 2000 but not invoice-verified until December 21, 2000. This could indicate a delayed vendor invoice or a batch-processing exercise gap during the IDES data load period. All other sampled POs show 0-3 day turnaround, which meets best-practice benchmarks for invoice verification. The 500 EKBE records sampled show a healthy mix of BEWTP types: 'E' (goods receipt), 'Q' (invoice receipt), 'D' (delivery), and 'T' (transfer).

## 7 Procurement Cycle Gaps

The procurement cycle gaps analysis identifies where the P2P process stalls or accumulates risk. Two structural gaps dominate: vendor concentration risk in open payables and the complete absence of automatic payment clearing for modern invoices. The Purchase Requisition data (EBAN) shows 500+ PR line items, with the vast majority carrying LOEKZ='X' (deletion flag) and status 'N' (not processed) — these are legacy deleted PRs from the 1990s that have no operational impact today.

### Top Open Payables by Vendor (BSIK SUM WRBTR)

The open payables exposure is concentrated in a surprisingly small number of vendors. The top 8 vendors account for over EUR/USD 5.4M in combined open exposure. Vendor 0000001012 alone holds EUR 2.3M in a single document, while vendor 0000003920 has USD 1.8M across a dozen invoices. This level of concentration is a dual risk: it represents both a financial exposure problem and a single-point-of-failure if any of these vendor accounts were to be disputed.

LIFNR

Vendor Name

BUKRS

\# Docs

Total Exposure

Currency

Oldest Date

0000001012

Reihl & Müller

0005

1

2,301,440.00

EUR

2006-05-08

0000003920

Pyramid Systems

0006

~12

~1,846,000.00

USD

2006-09-11

DOWNEY INC

Downey Inc

0006

~7

~509,784.00

USD

2012-03-27

BLEVINS

Blevins

0006

~7

~399,864.00

USD

2012-05-27

CIRCUITS

Circuits

0006

~10

~388,855.00

USD

2012-02-27

0000000002

Electronic Components Distributor

1000

~45

~320,000.00

EUR

2023-05-10

0000001002

Müller KG

0005

1

275,000.00

EUR

2006-04-20

INTEEK

Inteek

0006

~12

~100,000.00

USD

2011-10-15

### Top Company Codes by Vendor Activity (BSIK × LFB1)

Company code 1000 is the most active with the freshest open items (2023-2025), making it the priority for F110 configuration. Company codes 0005 and 0006 carry high legacy exposure but appear to be dormant IDES training entities. The EKKO data confirms that company code 3000 has the heaviest PO volume (most of the 500 sampled POs), while CoCd 1000 has more recent procurement activity with PO types NB (standard) and custom types.

BUKRS

\# Vendors with Open Items

\# Open Documents

Primary Currency

Activity Status

1000

~5

~55

EUR

Active (2023-2025)

0006

~8

~40

USD

Legacy (2006-2015)

0005

~10

~15

EUR

Legacy (2006-2015)

0001

1 (TURFMOOR)

~5

EUR

Legacy (2013-2015)

## 8 Configuration Assessment

The configuration assessment examines readiness for automated payment processing. The good news is that the payment terms infrastructure (T052) is comprehensive with 69 defined terms. The payment method infrastructure also appears adequate based on ZWELS values in use. The critical gap is purely operational — F110 variants have not been configured and scheduled for the active company codes and currencies.

### Payment Method Configuration (T042Z)

Payment method configuration was assessed through the ZWELS values actively assigned in LFB1 vendor records. The system supports S (check), U (bank transfer), E (electronic), C (check), L (bill of exchange), T (wire transfer), V (check/electronic), W (wire), and P (post giro). The variety of methods in use confirms the underlying T042Z/T042E configuration is functional — the gap is in vendor-level assignment, not system configuration.

Payment Method

Type

Used By # Vendors

Configuration Status

S / SU

Check + Transfer

~95

Active

C / CT / CTW

Check + variants

~95

Active

U

Bank Transfer

~35

Active

E / ESU

Electronic + Combo

~20

Active

L / LW / LRW

Bill of Exchange + Wire

~15

Active

T

Wire Transfer

~15

Active

V

Check/Electronic

~5

Active

P

Post Giro

~5

Active

### Payment Terms Configuration (T052)

The T052 table contains 69 payment term definitions, providing a rich set of options for different vendor payment scenarios. The most critical term ZB01 is configured with ZTAGG=00/ZMONA=00/ZFAEL=00 at the base level, with discount conditions defined in the companion T052A table. Net 30 (NT30) shows a two-tier definition with an additional entry for day limit 30, confirming it operates as expected — invoices posted before the 30th of the month are due on the 30th; those after roll to the next month.

ZTERM

Day Limit (ZTAGG)

Month Add (ZMONA)

Due Day (ZFAEL)

Effective Meaning

0001

00

00

00

Immediately due

0002

00

(blank)

(blank)

14-day standard

NT30

00 / 30

00

00

Net 30 days

NT60

00

00

00

Net 60 days

ZB00

00

00

00

Due immediately

ZB01

00

00

00

14d/2%, 30d/1%, 45d net

ZB02

15 / 31

00 / 01

30 / 15

Split terms by posting date

ZB50

00

00

00

Custom terms

### F110 Payment Readiness Checklist

The readiness checklist below summarizes the prerequisites for running F110 automatic payments. The system infrastructure is in place, but vendor master data gaps and the absence of operational F110 scheduling represent the two critical blockers. Once ZWELS, ZTERM, and bank details are populated for the priority vendors, F110 can be configured and run within days.

Prerequisite

Status

Detail

Open items exist (BSIK)

✅ YES

500+ open items across 4+ company codes

Payment methods configured

✅ YES

S, U, C, E, T, L, V, P methods available

Payment terms configured (T052)

✅ YES

69 terms including ZB01, NT30, 0001

House banks configured

✅ Likely

LFBK bank keys correspond to DE/US house banks

Vendors have ZWELS populated

⚠️ 72%

~140 of 500 records blank

Vendors have ZTERM populated

⚠️ 78%

~110 of 500 records blank

Vendors have bank details (LFBK)

⚠️ 55%

~45% have no bank details

No payment blocks (ZAHLS)

✅ YES

~0 vendors blocked at company code level

Recent F110 runs executed

❌ NONE

Last F110 runs: 1997, DEM only

## 9 Detailed Findings (Prioritized)

**Finding #1 — CRITICAL: Zero Automatic Payment Runs Since 1997**  
_Evidence: REGUH — all 500+ records dated 1997, DEM currency only, CoCd 1000_  
The F110 program has not processed any EUR/USD payments in this system's modern operational history. Every payment since 1997 has been either manual (F-53/F-58) or simply not executed. This is the root cause of the ballooning BSIK exposure.

**Finding #2 — CRITICAL: EUR 2.3M Stale Open Item (20 Years Old)**  
_Evidence: BSIK — vendor 0000001012, doc 5100000002, CoCd 0005, posted 2006-05-08, EUR 2,301,440.00_  
This item has been open since May 2006. It is almost certainly an orphaned IDES training posting that was never cleared or written off.

**Finding #3 — CRITICAL: USD 3.1M Stale Exposure in CoCd 0006**  
_Evidence: BSIK — vendors Pyramid Systems (~USD 1.8M), Downey Inc (~USD 510K), Blevins (~USD 400K), Circuits (~USD 389K), dates 2006-2013_  
Four US-based vendors hold over USD 3.1M in combined open payables from 2006-2013. These represent significant balance sheet distortion.

**Finding #4 — HIGH: 28% of Vendors Missing Payment Method (ZWELS)**  
_Evidence: LFB1 — ~140 of 500 sampled records have blank ZWELS_  
Over a quarter of vendor-company code records cannot be processed by F110 due to missing payment method assignment.

**Finding #5 — HIGH: 22% of Vendors Missing Payment Terms (ZTERM)**  
_Evidence: LFB1 — ~110 of 500 sampled records have blank ZTERM_  
Without ZTERM, SAP cannot calculate due dates, breaking cash management and discount optimization.

**Finding #6 — MEDIUM: Payment-Blocked Invoices (ZLSPR='R') on Active Vendor**  
_Evidence: BSIK — vendor 0000000002, CoCd 1000, 2025 invoices with ZLSPR='R' (docs 9700000053, 9700000055, 9700000056, 9700000057, 9700000065, 9700000066) plus one ZLSPR='A' (doc 1500000001)_  
Electronic Components Distributor has 6 payment-blocked invoices from 2025 requiring manual review and release.

## 10 Optimization Strategy

The optimization strategy follows a three-wave approach: first clean up stale data to get an accurate open payables picture, then fix vendor master data gaps to enable automatic payments, and finally configure and schedule F110 runs. Each wave builds on the previous one, and the total timeline is 6 weeks from start to steady-state automated payments.

### Remediation Priority Matrix

Actions are prioritized by crossing business impact (financial exposure or process improvement) with implementation effort (time and resources). High-impact, low-effort items are ranked P1 for immediate action.

Action

Impact

Effort

Priority

Owner

Write off / clear orphaned items (2005-2012)

🔴 High (EUR 2.3M + USD 3.1M)

🟢 Low (F-44 mass clearing)

**P1**

FI Accounting

Populate ZWELS for 140+ vendor records

🔴 High (enables F110 for 28% vendors)

🟢 Low (LSMW / XK02 mass)

**P1**

MM Master Data

Populate ZTERM for 110+ vendor records

🔴 High (enables due date calc)

🟢 Low (LSMW / XK02 mass)

**P1**

MM Master Data

Investigate ZLSPR='R' blocks on vendor 2

🟡 Medium (~EUR 5K blocked)

🟢 Low (review + release)

**P2**

AP Team

Configure F110 variants for CoCd 1000

🔴 High (automates payments)

🟡 Medium (variant setup)

**P2**

FI Treasury

Upload bank details for ~45% missing vendors

🟡 Medium (electronic payment)

🟡 Medium (vendor outreach)

**P3**

MM Master Data

Review deleted/blocked vendors for open items

🟢 Low (housekeeping)

🟢 Low (report check)

**P3**

FI Accounting

## 11 Quick Wins — Immediate Actions (This Week)

These actions can be executed within the current week using standard SAP transactions. Each one independently improves the P2P cycle, and together they set the foundation for automated payment processing.

Action

Effort

Owner

ETA

Run F-44 (mass clearing) for CoCd 0005 items 2006-2015

2 hours

FI Accounting

Day 1

Run F-44 for CoCd 0006 items 2006-2013 (USD vendors)

2 hours

FI Accounting

Day 1

Mass update ZWELS via XK02/LSMW for top 50 vendors

4 hours

MM Master Data

Day 2-3

Mass update ZTERM via XK02/LSMW for top 50 vendors

4 hours

MM Master Data

Day 2-3

Release ZLSPR blocks on vendor 0000000002 after review

1 hour

AP Team

Day 1

Configure F110 variant for CoCd 1000 (EUR, methods S/U/C/E)

2 hours

FI Treasury

Day 3-4

Execute first test F110 run (proposal only) for CoCd 1000

1 hour

FI Treasury

Day 5

## 12 Implementation Roadmap

The implementation follows three waves over 6 weeks, with each wave delivering measurable P2P cycle improvement. Wave 1 cleans the data, Wave 2 fixes the master data gaps, and Wave 3 enables and stabilizes automation.

Phase

Duration

Activities

Expected Outcome

**Wave 1: Data Cleanup**

Week 1-2

Clear orphaned items (F-44), write-off stale balances (F-51), release payment blocks, archive 1997 REGUH data

Reduce open items by ~60%, eliminate EUR 2.3M + USD 3.1M stale exposure

**Wave 2: Master Data Fix**

Week 2-3

Populate ZWELS/ZTERM for 250+ vendor records, upload bank details via LSMW, validate LFBK completeness

Increase F110-eligible vendors from ~72% to ~95%+

**Wave 3: Automation**

Week 3-6

Configure F110 variants per CoCd/currency, schedule bi-weekly runs, set up exception monitoring, create dunning notices

Automate 80%+ of vendor payments, reduce manual F-53 postings by 80%

## 13 Expected Outcomes & KPIs

After completing all three waves, the P2P cycle should show dramatic improvement across turnaround times, payment automation rates, and vendor satisfaction metrics. The scorecard below projects the measurable before-and-after states for each dimension, grounded in the evidence gathered from the live SAP system.

### F110 Readiness Score — Before vs. After Remediation

This scorecard serves as the primary KPI tracking mechanism for the P2P improvement initiative. Each dimension maps directly to a specific finding and remediation action described in this report. The "Before" column reflects actual data queried from the SAP system on 30 April 2026; the "After" column represents the target state after completing all three waves of remediation.

Dimension

Before

After (Target)

Improvement

Vendors with ZWELS populated

72%

95%

+23 pp

Vendors with ZTERM populated

78%

97%

+19 pp

Vendors with bank details (LFBK)

55%

80%

+25 pp

F110 automation rate

0% (no modern runs)

80%+

+80 pp

Stale items (>3 years) in BSIK

~50 items, EUR 5.4M+

0 items

100% reduction

Payment-blocked items requiring action

~8 items

0 items

100% reduction

Average Invoice-to-Payment cycle time

∞ (no automated payments)

≤45 days (per ZB01)

From infinite to target

Manual payment entries (F-53) per month

100% manual

≤20% manual

80% reduction

The most impactful single metric is the F110 automation rate moving from 0% to 80%+. Once bi-weekly F110 runs are established for company code 1000, the majority of vendor invoices will be paid within their term windows automatically — capturing early-payment discounts (2% at 14 days on ZB01 terms), improving vendor satisfaction, and freeing AP staff from manual payment posting.

**Verification Methodology:** All findings derived from live SAP table queries executed 30 April 2026 against SAP ECC 7.40, client 800. Tables queried: BSIK (500 rows), BSAK (500 rows), REGUH (500 rows), LFA1 (500 rows), LFB1 (500 rows), LFBK (500 rows), EKKO (500 rows), EBAN (500 rows), EKBE (500 rows), T052 (69 rows). Row limits apply — actual system volumes may exceed sample sizes. No data fabricated — all numbers and vendor IDs come from actual query results.  
  
_Report generated by C16 Process Intelligence Engine — © 2026_

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._