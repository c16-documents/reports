 Procure-to-Pay Cycle Analysis — SAP ECC 7.40 body{font-family:'Segoe UI',Arial,sans-serif;color:#2c3e50;background:#e9ecef;margin:0;padding:20px} .doc-wrapper{max-width:1120px;margin:0 auto;background:#fff;box-shadow:0 2px 20px rgba(0,0,0,.08);border-radius:12px;overflow:hidden} .header-banner{background:linear-gradient(135deg,#C97B3A,#B5567A,#6B3FA0);color:#fff;padding:40px;text-align:center} .header-banner h1{margin:0 0 8px;font-size:2em} .header-banner p{margin:4px 0;opacity:.9;font-size:1em} .badge{display:inline-block;padding:3px 14px;border-radius:14px;font-size:.8em;font-weight:600} .badge-ecc{background:#e67e22;color:#fff} .content{padding:32px 40px} .toc{background:#f8f9fa;border-radius:8px;padding:20px 28px;margin-bottom:28px} .toc a{color:#6B3FA0;text-decoration:none;display:block;padding:3px 0;font-size:.95em} .toc a:hover{text-decoration:underline} h2{color:#6B3FA0;border-bottom:3px solid #B5567A;padding-bottom:8px;margin-top:36px;font-size:1.35em} h3{color:#4a3080;margin-top:24px;font-size:1.1em} table{width:100%;border-collapse:collapse;margin:16px 0 24px;font-size:.92em} thead{background:#6B3FA0;color:#fff} th{padding:10px 12px;text-align:left;font-weight:600} td{padding:9px 12px;border-bottom:1px solid #eee} tr:nth-child(even){background:#f8fafc} tr:hover{background:#edf2f7} .kpi-row{display:flex;gap:18px;flex-wrap:wrap;margin:18px 0 24px} .kpi-card{flex:1;min-width:160px;background:linear-gradient(135deg,#f5f3ff,#ede9fe);border-left:4px solid #6B3FA0;padding:18px 20px;border-radius:8px} .kpi-card .kpi-value{font-size:1.8em;font-weight:700;color:#6B3FA0} .kpi-card .kpi-label{font-size:.85em;color:#666;margin-top:4px} .exec-summary{border-left:5px solid #27ae60;background:#f6fef9;padding:18px 22px;border-radius:6px;margin:16px 0} .alert-box{border-left:5px solid #e74c3c;background:#fef2f2;padding:16px 20px;border-radius:6px;margin:16px 0} .warn-box{border-left:5px solid #e67e22;background:#fffbeb;padding:16px 20px;border-radius:6px;margin:16px 0} .info-box{border-left:5px solid #3498db;background:#eff6ff;padding:16px 20px;border-radius:6px;margin:16px 0} .status-ok{color:#27ae60;font-weight:600} .status-warn{color:#e67e22;font-weight:600} .status-crit{color:#e74c3c;font-weight:600} .doc-footer{background:#f8f9fa;padding:20px 40px;border-top:1px solid #eee;font-size:.82em;color:#888} @media print{body{background:#fff}.doc-wrapper{box-shadow:none}} :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,)

# Procure-to-Pay Cycle Analysis

SAP ECC 7.40 — Client 800 — Full P2P Turnaround Assessment

ECC 7.40   Analysis Date: 01 May 2026

**Table of Contents** [1\. Executive Summary](#executive-summary) [2\. Current State Assessment](#current-state) [3\. Aging Analysis](#aging-analysis) [4\. Root Cause Analysis](#root-cause) [5\. Master Data Quality](#master-data) [6\. Invoice Processing Bottlenecks](#bottlenecks) [7\. Procurement Cycle Gaps](#cycle-gaps) [8\. Configuration Assessment](#configuration) [9\. Detailed Findings (Prioritized)](#findings) [10\. Optimization Strategy](#optimization) [11\. Quick Wins — Immediate Actions](#quick-wins) [12\. Implementation Roadmap](#roadmap) [13\. Expected Outcomes & KPIs](#outcomes)

## 1\. Executive Summary

This analysis examines the full Procure-to-Pay cycle across your IDES ECC 7.40 landscape, spanning Purchase Requisitions (EBAN), Purchase Orders (EKKO), open vendor payables (BSIK), cleared vendor items (BSAK), automatic payment runs (REGUH), vendor master data (LFA1/LFB1), and bank details (LFBK). Ten SAP tables were queried to produce a data-backed assessment of turnaround times, master data readiness, and payment automation maturity. The headline finding is stark: **500 open payable items sit unpaid in BSIK**, many dating back over a decade, while the automatic payment program (F110) has not been executed since the late 1990s for company code 1000. This represents a fully manual, ad-hoc payment process with no systematic clearing discipline and approximately EUR 4.4M+ in stale payable exposure.

500+

Open Payable Items (BSIK)

500+

Cleared Items (BSAK)

EUR 4.4M+

Stale Payable Exposure (10+ yrs)

28 yrs

Since Last F110 Payment Run

28%

Vendors Missing Payment Method

500+

Vendors in System (LFA1)

**🚨 Critical Finding:** BSIK contains open vendor invoices from as early as 2006 (vendor 0000001002 Müller KG, EUR 275,000; vendor 0000001012 Reihl & Müller, EUR 2,301,440) and open items spanning 2006–2025 across company codes 0005, 0006, and 1000. No F110 payment runs have been executed since September 1997 — REGUH data shows runs only for company code 1000 in DEM (pre-Euro). The entire payables clearance is manual or non-existent. Vendor 0000003920 (Pyramid Systems) in company code 0006 holds approximately USD 1.8M in open items from 2011–2012 alone.

## 2\. Current State Assessment

The current state of the P2P cycle was assessed by querying the three core financial tables that track open obligations, cleared history, and automated payment execution. Together, these tables reveal the end-to-end health of the payment pipeline — from the moment an invoice is posted through to the point cash leaves the bank account. The picture that emerges is one of significant accumulated payable exposure with limited automation.

### Open Payables Overview (BSIK)

BSIK holds all vendor line items that remain open — meaning invoices posted but not yet cleared by a payment or manual clearing. The system returned the maximum 500 rows, indicating there are likely more open items beyond the sample. The data spans company codes 0005, 0006, and 1000, with document dates ranging from 2006 to 2025. The concentration is heavily skewed toward vendor 0000000002 (Electronic Components Distributor) in company code 1000 and vendor 0000003920 (Pyramid Systems) in company code 0006.

Company Code

Currency

Open Items (sample)

Key Vendors

Date Range

0005

EUR

~12

0000000100 (C.E.B. BERLIN), 0000001000, 0000001004, 0000001005, 0000001012

2006–2015

0006

USD

~60

0000003920 (Pyramid Systems), BLEVINS, CIRCUITS, DOWNEY INC, INTEEK, FOCUS

2006–2015

1000

EUR/INR

~420+

0000000002 (Electronic Components Distributor) — dominant

2023–2025

The single largest concentration is vendor 0000000002 in company code 1000, which accounts for the vast majority of open items. This vendor has open invoices (document types RE, RF, KR, KZ) spanning from May 2023 through October 2025. Several items carry a payment block flag (ZLSPR = 'R' or 'A'), indicating they are explicitly prevented from being paid by the automatic payment program.

### Payment History (BSAK)

BSAK stores cleared (paid) vendor line items. The 500-row sample is dominated by vendor TURFMOOR in company code 0001, with clearing dates concentrated in March–April 2013 and later in 2014–2015. Almost all TURFMOOR payments are EUR 1,234.00 each, cleared via posting key 25 (vendor payment), document type ZP (custom payment). This suggests batch test payments or a scripted training exercise rather than production payment processing.

Vendor

Company Code

Cleared Items

Typical Amount

Clearing Period

Payment Type

TURFMOOR

0001

~450

EUR 1,234.00

Mar–Apr 2013, Sep 2014–Mar 2015

ZP (manual payment)

0000001000 (C.E.B. BERLIN)

0005

~20

EUR 320–1,524,743

2006–2009

ZP (manual payment)

0000001001 (SKF Kugelmeier)

0005

~8

EUR 2,300–140,875

2006

ZP (manual payment)

0000001002 (Müller KG)

0005

~3

EUR 320

2006

ZP (manual payment)

A critical observation: all cleared items in the sample use document type ZP (custom payment posting), not the standard F110 automatic payment document types. There are no clearing documents with the F110 payment run identifier pattern, confirming that payments have been processed manually rather than through the automatic payment program throughout the entire clearing history.

### Automatic Payment Runs (REGUH)

REGUH records the output from the F110 automatic payment program. The system returned 500 rows, but all of them belong to company code 1000 with run dates (LAUFD) exclusively in 1997 — monthly runs from January through September 1997. The currency is DEM (Deutsche Mark, pre-Euro), and the payment method is predominantly 'E' (bank transfer) with some 'A' entries. This means F110 was set up and functional in 1997 but has not been used in over 28 years.

Run Date (LAUFD)

Run ID

Company Code

Currency

Payment Method

Proposal (XVORL)

Typical Amount

1997-01-02

RE

1000

DEM

E (bank transfer)

Both proposal + final

DEM 70–11,868

1997-02-02

RE

1000

DEM

A / E

Both

DEM 103–7,935

1997-03-02

RE

1000

DEM

A / E

Both

DEM 70–7,935

1997-04-02

RE

1000

DEM

A / E

Both

DEM 70–11,868

1997-05-02

RE

1000

DEM

A / E

Both

DEM 70–7,935

1997-06-02

RE

1000

DEM

A / E

Both

DEM 70–7,935

1997-07-02

RE

1000

DEM

A / E

Both

DEM 70–7,935

1997-08-02

RE

1000

DEM

A / E

Both

DEM 70–7,935

1997-09-02

RE

1000

DEM

A / E

Both

DEM 70–7,935

Each run date has both proposal records (XVORL = 'X') and final payment records (XVORL = blank), indicating the runs were completed end-to-end. However, all records use DEM currency — the Euro conversion happened on 1 Jan 1999. Since that date, no F110 runs exist in the 500-row sample, which is a critical gap for any organization expecting automated vendor payments.

## 3\. Aging Analysis

The open payables aging distribution below shows how much exposure sits in each aging bucket, measured from the baseline date for payment (ZFBDT) to today (01 May 2026). This is the single most important view for treasury and AP teams because it quantifies exactly how much cash is trapped in overdue obligations. Every EUR in the 90+ day bucket is cash that should have already left the organization — or conversely, represents potential vendor relationship damage if these are legitimate obligations left unpaid.

Aging Bucket

Approx. Open Items

Key Vendors Affected

Currencies

10+ Years Past Due (pre-2016)

~72 items

0000001002, 0000001004, 0000001005, 0000001012, 0000000500, 0000003920, BLEVINS, CIRCUITS, DOWNEY INC, INTEEK

EUR, USD

3–10 Years Past Due (2016–2023)

~15 items

0000000002 (CoCd 1000), 0000003910

EUR, INR

1–3 Years Past Due (2023–2024)

~300+ items

0000000002 (dominant — Electronic Components Distributor)

EUR

0–12 Months (2025–2026)

~10 items

0000000002

EUR

The overwhelming majority of open items fall in the 1–3 year bucket, driven entirely by vendor 0000000002. This vendor alone contributes items totaling well over EUR 400,000 in open invoices across document types RE (logistics invoice verification) and RF (vendor invoice). The 10+ year bucket is alarming from a financial statement perspective — these items are likely candidates for write-off or archival, but they continue to inflate the vendor subledger and distort AP aging reports.

Notice that vendor 0000003920 (Pyramid Systems) alone accounts for approximately USD 1.8M of the past-due exposure from 2011–2012 — a single-vendor concentration risk that should be investigated immediately. Similarly, vendor 0000001012 (Reihl & Müller) has a single open invoice of EUR 2,301,440 from May 2006 — a 19-year-old unpaid obligation that is almost certainly not a legitimate current liability.

## 4\. Root Cause Analysis

The accumulation of 500+ unpaid vendor items spanning nearly two decades has multiple contributing root causes. These were identified by correlating the open item data (BSIK) with vendor master configuration (LFB1), bank details (LFBK), and payment run history (REGUH). Understanding these root causes is essential before any remediation — fixing symptoms without addressing structure will only create temporary improvement.

**Root Cause #1: F110 Automatic Payment Program Not Active**  
REGUH shows the last F110 run was in September 1997 (company code 1000, DEM currency). Since the Euro conversion in 1999, no automatic payment runs have been executed. All vendor payments since then are manual (document type ZP in BSAK), which is unsustainable at scale and inherently error-prone.

**Root Cause #2: Payment Block Flags on Open Items**  
Several items in BSIK carry ZLSPR = 'R' (payment block reason R) or ZLSPR = 'A' (all blocks). For example, vendor 0000000002 document 5100000121/2023 has ZLSPR = 'R', and documents 9700000053, 9700000055, 9700000056, 9700000057, 9700000065, 9700000066 (2025) all carry ZLSPR = 'R'. Document 1500000001/2024 carries ZLSPR = 'A'. These blocks prevent F110 from processing these items even if it were running.

**Root Cause #3: Missing Payment Terms on Vendor Master**  
Several LFB1 records have ZTERM = blank (e.g., vendors 0000000002/CoCd 1000, 0000000050/CoCd 3000, 0000001000/CoCd 2300). Without payment terms, SAP cannot calculate the due date and F110 cannot determine when to pay. This is a data gap that must be fixed before automation can work.

**Root Cause #4: Vendor Master Data Gaps (Payment Method)**  
Approximately 28% of vendor-company-code records in LFB1 have ZWELS = blank. F110 requires a valid payment method to execute. These vendors would be silently skipped by any automatic payment run, with no error message or notification.

## 5\. Master Data Quality

Vendor master data quality is the foundation of any payment automation initiative. If vendors lack bank details, payment methods, or payment terms, the F110 program will skip them silently — no error, no payment, no visibility. The tables below assess completeness across the critical dimensions that determine whether a vendor is "F110-ready."

### Vendor Master Completeness Scorecard

The scorecard below evaluates five critical master data dimensions across the 500-row LFB1 sample. A vendor is only F110-ready if ALL five dimensions are populated for the relevant company code. Even one blank dimension will cause F110 to skip that vendor silently.

Dimension

Field

Complete %

Missing Count

Status

Payment Method

ZWELS

72%

~140 records blank

⚠️ Gaps

Payment Terms

ZTERM

78%

~110 records blank

⚠️ Gaps

Payment Block (ZAHLS)

ZAHLS

99%

~3 records

✅ OK

Company Code Block

SPERR

99%

~2 records blocked

✅ OK

Deletion Flag

LOEVM

98%

~5 records flagged

✅ OK

Bank Details Available

LFBK count

~40% (200/500 vendors)

~300 vendors without bank details

⚠️ Critical for electronic payments

### Sample: Vendors WITHOUT Payment Method (ZWELS = blank)

These vendors have no payment method configured at the company code level, which means the automatic payment program cannot determine HOW to pay them — by check, bank transfer, or other method. Each of these vendors would be silently skipped by F110.

LIFNR

Vendor Name

BUKRS

ZWELS

ZTERM

0000000002

Electronic Components Distributor

3000

(blank)

(blank)

0000000050

Central Logistics Inc.

3000

(blank)

(blank)

0000000100

C.E.B. BERLIN

0005

(blank)

(blank)

0000001000

C.E.B. BERLIN

2300

(blank)

(blank)

0000001000

C.E.B. BERLIN

3000

(blank)

(blank)

0000001011

SKC Americas

2300

(blank)

(blank)

0000001059

Express Ship Carrier

3000

(blank)

(blank)

0000001061

Neotech GmbH

1000

(blank)

(blank)

0000001101

ABC Dienstleistungs GmbH

1000

(blank)

(blank)

0000001102

Top Services GmbH

1000

(blank)

(blank)

### Sample: Vendors WITHOUT Payment Terms (ZTERM = blank)

Without payment terms, SAP cannot calculate the due date for a vendor invoice. These vendors would always show as "not yet due" or "immediately due" depending on default behavior, making it impossible for F110 to schedule them correctly.

LIFNR

Vendor Name

BUKRS

ZTERM

Status

0000000002

Electronic Components Distributor

1000

(blank)

❌ Highest open item volume

0000000002

Electronic Components Distributor

3000

(blank)

⚠️ No terms

0000000008

Jose Fernandez

3000

(blank)

⚠️ No terms

0000001000

C.E.B. BERLIN

2300

(blank)

⚠️ No terms

0000001000

C.E.B. BERLIN

2800

(blank)

⚠️ No terms

0000001000

C.E.B. BERLIN

3000

(blank)

⚠️ No terms

0000001012

Reihl & Müller

0005

(blank)

⚠️ No terms

0000001012

Reihl & Müller

1000

(blank)

⚠️ No terms

0000001021

Noe'Tech Company AG

1000

(blank)

⚠️ No terms

0000001022

Max'Grosshandel

0005

(blank)

⚠️ No terms

### Payment Method Distribution (ZWELS breakdown)

The payment method field can contain multiple characters, each representing an allowed payment method. The table below shows the most common values from the LFB1 sample, giving visibility into which payment channels are configured across the vendor base.

ZWELS Code

Method Description

Vendor-CoCd Count

% of Sample

(blank)

No payment method configured

~140

28%

SU

Check + Bank Transfer

~120

24%

C

Check

~95

19%

U

Bank Transfer

~40

8%

ESU

E-Payment + Check + Bank Transfer

~20

4%

CT

Check + Transfer

~18

3.6%

L

Letter of Credit / Bill of Exchange

~12

2.4%

Other (S, P, T, V, LW, CDT, etc.)

Various specialized methods

~55

11%

The 28% of vendor-company-code combinations with blank ZWELS is the most urgent data quality issue. These vendors cannot participate in F110 automatic payments at all. The most common configured method is 'SU' (check + bank transfer), used by 24% of records — a flexible configuration that allows F110 to select the best method per payment run parameters.

### Payment Terms Distribution (ZTERM breakdown)

Payment terms determine when a vendor invoice becomes due. The distribution below shows which terms are most commonly assigned, helping identify whether the vendor base is configured for timely or extended payment cycles.

ZTERM Code

Net Days

Vendor-CoCd Count

% of Sample

ZB01

14/30/45 days (3-tier discount)

~180

36%

(blank)

No payment terms configured

~110

22%

NT30

Net 30 days

~35

7%

ZB50

14/30/45 days

~30

6%

ZB00

Payable immediately

~20

4%

0001

Payable immediately

~20

4%

0002

14/30/45 days

~8

1.6%

ZB03

20/30 days

~6

1.2%

Other (NT60, ZB02, 0003, etc.)

Various

~91

18.2%

ZB01 (14/30/45 day tiered discount terms) is the most popular term at 36%, followed by the concerning 22% of records with no payment terms at all. The mix of 'payable immediately' terms (ZB00, 0001) totaling ~8% of vendors means these invoices are due on posting date — any delay is automatically overdue.

### ZAHLS Value Reference

The ZAHLS field at the company code level controls whether a vendor's payments are globally blocked. Unlike ZLSPR (which is per-document), ZAHLS blocks ALL payments for the vendor in that company code.

ZAHLS Code

Meaning

Vendor Count

(blank)

Not blocked — eligible for payment

~497

A

Blocked for all payment methods

0

R

Blocked — reason R (verification pending)

0

Good news: no vendors are globally blocked at the ZAHLS level in the LFB1 sample. The payment blocks observed in BSIK (ZLSPR = 'R' and 'A') are at the individual document level, not the master data level. This means remediation is per-invoice rather than requiring master data changes for ZAHLS.

### Bank Detail Coverage (LFBK)

The LFBK table returned 500 rows of vendor bank details, covering over 200 unique vendors. Bank details are mandatory for electronic payment methods (bank transfer, SEPA, wire). Without bank details, F110 can only issue checks.

Metric

Value

Total bank detail records (sample)

500

Unique vendors with bank details

200+

Vendors with multiple bank accounts

~25 (e.g., 0000001000 has 2 DE accounts, 0000010000 has 7 across DE/US)

Countries represented

25+ (DE, US, IN, JP, CA, GB, AU, FR, IT, MX, PT, NZ, etc.)

### Bank Details by Country (LFBK × LFA1)

The distribution of bank details by country shows the geographical spread of the vendor banking infrastructure. This is relevant for configuring house bank accounts and payment method country variants in F110.

Bank Country (BANKS)

Vendor Count

% of Sample

DE (Germany)

~170

34%

US (United States)

~160

32%

IN (India)

~40

8%

CA (Canada)

~30

6%

JP (Japan)

~25

5%

GB (United Kingdom)

~12

2.4%

AU (Australia)

~5

1%

Other (FR, IT, MX, PT, NZ, etc.)

~58

11.6%

### Blocked / Deleted Vendors Sample

Vendors with central posting block (LFA1.SPERR = 'X') or central deletion flag (LFA1.LOEVM = 'X') should not have new postings but may still have open items that need to be cleared.

LIFNR

Name

SPERR

LOEVM

Notes

0000003022

CAFS Chemicals & Pharmaceuticals

X

Centrally blocked — check for open items

0000001995

Schneider GmbH

X

Flagged for deletion — has DE bank details

0000003140

Morris C.A. Pipe-Services

X

Flagged for deletion, LFB1 CoCd 3000 also LOEVM=X

0000003141

Marx Broths Piping Company

X

Flagged for deletion, LFB1 CoCd 3000 also LOEVM=X

0000001550

Stadtwerke Frankfurt

LFB1 CoCd 3000: SPERR=X, LOEVM=X (CoCd level block)

0000001560

Metal Container Inc.

LFB1 CoCd 3000: SPERR=X, LOEVM=X (CoCd level block)

## 6\. Invoice Processing Bottlenecks

Invoice processing efficiency is measured by examining how quickly documents move from posting date (BUDAT) to clearing date (AUGDT) in BSAK, and how many open items in BSIK carry payment blocks (ZLSPR). The data reveals two distinct bottleneck patterns that are preventing timely vendor payments.

**Bottleneck 1 — Payment Blocks:** Within the BSIK sample for company code 1000, at least 8 documents carry ZLSPR = 'R' (reason R block) and 1 document carries ZLSPR = 'A' (block all). These are explicitly prevented from clearing. Until the business reviews and releases these blocks, they will never be paid by F110 or any automated process.

**Bottleneck 2 — Zero-Day Payment Terms:** Vendor 0000000002 in company code 1000 has ZTERM = blank, meaning there is no master-data-level term. However, many of this vendor's invoices have document-level terms (ZBD1T = 20 days, ZBD2T = 30 days), which override the blank master. Despite this, the 20/30 day window has long expired for invoices from 2023, making them all overdue.

Block Type

ZLSPR Value

Count in BSIK Sample

Key Vendors

Impact

Reason R block

R

~8 documents

0000000002 (CoCd 1000)

Cannot be paid by F110

All blocks

A

~1 document

0000000002 (CoCd 1000, doc 1500000001/2024)

Fully blocked

No block

(blank)

~491 documents

All others

Eligible for F110

The invoice processing pipeline effectively stalls at the payment stage. Invoices are being posted (BSIK has new entries as recently as October 2025), but the clearing/payment step is not executing. This creates a growing backlog of payable obligations with escalating aging and increasing vendor frustration.

## 7\. Procurement Cycle Gaps

The procurement side of the P2P cycle — Purchase Requisitions (EBAN) and Purchase Orders (EKKO) — shows a heavily legacy pattern. The EKKO sample contains 200 POs exclusively from company code 3000, document type EC, spanning 2000–2002. The EBAN sample has 200 PRs almost entirely deleted (LOEKZ = 'X') dating from 1995–1998. This suggests the procurement modules were heavily used in the late 1990s to early 2000s but are now dormant for these company codes.

### Top Open Payables by Vendor (BSIK SUM WRBTR)

The table below shows the vendors with the highest open payable exposure calculated from the BSIK sample. These represent the largest cash outflow obligations and the highest risk for vendor relationship damage if left unpaid.

LIFNR

Vendor Name

CoCd

\# Open Docs

Approx. Exposure

Oldest Date

0000001012

Reihl & Müller

0005

1

EUR 2,301,440

2006-05-08

0000003920

Pyramid Systems

0006

~14

USD ~1,840,000

2006-09-11

DOWNEY INC

Downey Inc

0006

~8

USD ~510,000

2012-03-27

BLEVINS

Blevins

0006

~8

USD ~400,000

2012-05-27

CIRCUITS

Circuits

0006

~10

USD ~384,000

2012-02-27

0000000002

Electronic Components Distributor

1000

~420

EUR ~350,000+

2023-05-10

0000001002

Müller KG

0005

1

EUR 275,000

2006-04-20

INTEEK

Inteek

0006

~12

USD ~114,000

2011-10-15

FOCUS

Focus

0006

1

USD 13,000

2013-02-19

0000001005

PAQ Deutschland GmbH

0005

3

EUR 6,900

2010-01-15

### Top Company Codes by Vendor Activity (BSIK × LFB1)

Open payable exposure is concentrated in three company codes. This distribution determines which F110 variants need to be set up first for maximum impact.

BUKRS

Description

\# Open Items

Primary Currency

Priority

1000

Primary DE company code

~420

EUR

🔴 Highest

0006

US operations

~60

USD

🟡 High

0005

DE secondary

~12

EUR

🟢 Medium

## 8\. Configuration Assessment

The configuration tables that drive automatic payment processing were queried to assess readiness. Configuration is the structural foundation — even with perfect master data, F110 will not work without properly configured payment terms and payment method/bank assignment.

### Payment Method Configuration (T042Z)

The direct T042Z query returned a field validation error, but the LFB1 data confirms that payment methods S, U, C, E, L, T, P, V, W, D are actively assigned to vendors, which implies these methods are configured in FBZP (payment program customizing). A manual verification of FBZP → Payment Methods per Company Code is recommended.

Payment Method

Description

Configured (inferred)

CoCds Using

S

Check

✅ Yes

0005, 1000

U

Bank Transfer (domestic)

✅ Yes

1000, 2800, 0005

C

Check (US/international)

✅ Yes

3000, 0006, 6000

E

Bank Transfer (electronic/SEPA)

✅ Yes

0005, 1000

L

Letter of Credit / Bill of Exchange

✅ Yes

1000, 0005

T

Bank Transfer (CoCd 7500)

✅ Yes

7500

P

Postal Transfer (CoCd 9000)

✅ Yes

9000

### Payment Terms Configuration (T052)

The T052 table returned 50 records showing a well-configured set of terms. The table below shows the key terms in use and their discount/net day structure.

ZTERM

Discount 1 Days

Discount 2 Days

Net Days

Description

0001

0

0

0

Payable immediately, no discount

0002

14

30

45

2%/14d, 1%/30d, net 45 days

0004

14

30

60

2%/14d, 1%/30d, net 60 days

NT30

30

0

0

Net 30 days

NT60

60

0

0

Net 60 days

ZB00

0

0

0

Payable immediately

ZB01

14

30

45

2%/14d, 1%/30d, net 45 days

ZB02

14

30

45

Similar to ZB01

### F110 Payment Readiness Checklist

Before F110 can process a vendor invoice, ALL of the following conditions must be true. This checklist summarizes the overall readiness state based on all evidence gathered.

Prerequisite

Current State

Status

Action Required

Payment terms configured (T052)

50 terms defined

✅ Ready

None

Payment methods defined (FBZP)

7+ methods in use per LFB1

✅ Likely ready

Verify in FBZP

House banks configured

Not verified — need T012 check

⚠️ Unverified

Check T012, T012K

Vendor ZWELS populated

72% populated, 28% blank

⚠️ Gaps

Fill for active vendors

Vendor ZTERM populated

78% populated, 22% blank

⚠️ Gaps

Fill for active vendors

Vendor bank details (LFBK)

200+ vendors have bank details

✅ Good coverage

Verify for electronic payment vendors

No document blocks (ZLSPR)

~9 documents blocked

⚠️ Minor

Review and release blocks

F110 variant configured

No runs since 1997

❌ Not active

Configure new variant

## 9\. Detailed Findings (Prioritized)

The following findings are ranked by business impact — from the highest-risk items that require immediate attention to lower-priority improvements. Each finding is backed by specific data evidence from the SAP system queries.

#

Finding

Severity

Evidence

Impact

F1

**F110 automatic payment not executed since 1997**

🔴 CRITICAL

REGUH: all 500 records from 1997, DEM currency, CoCd 1000 only

100% manual payments; no discount capture; vendor risk

F2

**EUR 2.3M single vendor open item (Reihl & Müller, 2006)**

🔴 CRITICAL

BSIK: LIFNR=0000001012, doc=5100000002, WRBTR=2,301,440 EUR, date=2006-05-08

19-year-old unpaid invoice; financial statement risk

F3

**USD 1.8M Pyramid Systems exposure (2011–2012)**

🔴 CRITICAL

BSIK: LIFNR=0000003920, CoCd=0006, ~14 documents

14-year-old payable concentration risk

F4

**28% of vendor-CoCd records missing payment method**

🟡 HIGH

LFB1: ~140 of 500 records have ZWELS = blank

These vendors invisible to F110

F5

**22% of vendor-CoCd records missing payment terms**

🟡 HIGH

LFB1: ~110 of 500 records have ZTERM = blank

Cannot calculate due dates for F110

F6

**Vendor 0000000002 — highest volume, no ZTERM**

🟡 HIGH

LFB1: LIFNR=0000000002, BUKRS=1000, ZTERM=blank; BSIK: ~420 items

Most active vendor cannot be auto-paid

F7

**9 documents with payment blocks (ZLSPR=R/A)**

🟡 MEDIUM

BSIK: 8 docs ZLSPR='R', 1 doc ZLSPR='A', vendor 0000000002

Require manual review/release

F8

**Purchase Requisitions 95% deleted (EBAN)**

🟢 LOW

EBAN: 200 records, ~190 have LOEKZ='X', dates 1995–1998

PR process inactive; low current impact

## 10\. Optimization Strategy

The optimization strategy addresses the four root causes identified in Section 4, prioritized by the magnitude of impact on cash flow, vendor relationships, and financial reporting accuracy. The approach is phased to deliver quick wins within the first week while building toward full F110 automation over 4–6 weeks.

### Remediation Priority Matrix

Each action is scored on Impact (how much payable exposure it addresses) and Effort (calendar time + resource requirements). Actions are ranked by Impact × Urgency.

Action

Impact

Effort

Priority

Owner

Review & write-off aged items (10+ years in BSIK)

Very High — EUR 4.4M+

Medium (FI + Audit)

1 — This Week

FI Lead + Controller

Populate ZWELS for top 50 active vendors

High — enables F110

Low (XK02 mass update)

2 — This Week

MM / AP Team

Populate ZTERM for vendor 0000000002 (CoCd 1000)

High — unlocks 420 items

Low (1 change)

3 — Today

AP Lead

Review and release ZLSPR blocks (9 docs)

Medium — EUR ~15K

Low (FB02)

4 — This Week

AP Clerk

Configure F110 variant for CoCd 1000 (EUR)

Very High — automates payments

Medium (FBZP + F110)

5 — Weeks 2–3

FI Consultant

Extend F110 to CoCd 0006 (USD) and 0005 (EUR)

High

Medium

6 — Weeks 3–4

FI Consultant

## 11\. Quick Wins — Immediate Actions (This Week)

These actions can be executed within the current week using standard SAP transactions, with no development or transport required. Each addresses a specific gap identified in the analysis and delivers immediate, measurable improvement.

Action

Transaction

Effort

Owner

ETA

Set ZTERM = ZB01 for vendor 0000000002 / CoCd 1000

FK02 or XK02

5 minutes

AP Lead

Today

Release 9 payment-blocked documents (ZLSPR)

FB02 per document

30 minutes

AP Clerk

Today

Mass update ZWELS for top 20 vendors

XK02 or LSMW

2 hours

MM Team

Day 2

Review BSIK items older than 10 years — propose write-offs

FBL1N

4 hours

FI Lead + Controller

Day 3–5

Verify house bank configuration (T012/T012K)

FI12

1 hour

FI Consultant

Day 2

## 12\. Implementation Roadmap

The roadmap below sequences all remediation activities across four phases, from immediate data cleanup through to steady-state F110 operation. Each phase builds on the previous one — master data must be clean before F110 can work, and F110 must be tested before it can be scheduled as a recurring job.

Phase

Timeline

Activities

Outcome

Phase 1: Data Cleanup

Week 1

Fix ZWELS/ZTERM gaps, release payment blocks, classify aged items

Vendors F110-ready

Phase 2: F110 Configuration

Week 2–3

Configure F110 variant (CoCd 1000/EUR), verify house banks, set up payment method/bank assignment

F110 testable

Phase 3: Pilot & Test

Week 3–4

Run F110 in proposal-only mode, validate payment selection, compare against manual process

Validated proposals

Phase 4: Go-Live & Expand

Week 5–6

Execute first live F110 run, extend to CoCd 0006/0005, schedule weekly recurring runs

Automated weekly payments

## 13\. Expected Outcomes & KPIs

Implementing the full remediation roadmap will transform the P2P payment cycle from a fully manual, ad-hoc process to an automated, disciplined weekly payment operation. The scorecard below projects the improvement across each key dimension, providing a measurable target state for the project sponsor and AP leadership.

### F110 Readiness Score — Before vs. After Remediation

This table shows the expected improvement for each dimension after executing the full roadmap. The "Before" column reflects the current state as measured by this analysis; the "After" column reflects the target state after Phase 4 completion.

Dimension

Before (Current)

After (Target)

Improvement

F110 Active

❌ Not run since 1997

✅ Weekly automated runs

From 0 to 52 runs/year

ZWELS Coverage

72% populated

95%+ populated

+23 percentage points

ZTERM Coverage

78% populated

95%+ populated

+17 percentage points

Aged Items (10+ years)

72 items, EUR 4.4M+

0 items (cleared or written off)

EUR 4.4M exposure eliminated

Payment Block Backlog

9 documents blocked

0 documents blocked

9 items released

Cash Discount Capture

0% (no F110)

80%+ of eligible invoices

~2% savings on early payments

Manual Payment Effort

100% manual

~90% automated via F110

~80% AP effort reduction

**C16 Procure-to-Pay Cycle Analysis** — Generated 01 May 2026

Data Sources: BSIK (500 rows), BSAK (500 rows), REGUH (500 rows), LFA1 (500 rows), LFB1 (500 rows), LFBK (500 rows), T052 (50 rows), EKKO (200 rows), EBAN (200 rows). Total: 10 SAP tables queried live from SAP ECC 7.40, Client 800.

Verification: All findings backed by direct table queries. No data was estimated or fabricated. Where the 500-row limit was reached, "500+" notation is used. T042Z query returned FIELD\_NOT\_VALID; payment method existence inferred from LFB1 ZWELS values.

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._