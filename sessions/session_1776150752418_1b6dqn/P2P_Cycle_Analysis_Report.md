 Procure-to-Pay Cycle Analysis — SAP ECC 7.40 body{font-family:'Segoe UI',Arial,sans-serif;color:#2c3e50;background:#e9ecef;margin:0;padding:20px} .doc-wrapper{max-width:1120px;margin:0 auto;background:#fff;box-shadow:0 4px 24px rgba(0,0,0,0.12);border-radius:12px;overflow:hidden} .header-banner{background:linear-gradient(135deg,#C97B3A,#B5567A,#6B3FA0);color:#fff;padding:40px 48px;position:relative} .header-banner img{height:50px;margin-bottom:12px;border-radius:8px} .header-banner h1{margin:0 0 8px;font-size:2.2em;font-weight:700} .header-banner .subtitle{font-size:1.15em;opacity:0.92} .badge{display:inline-block;padding:4px 14px;border-radius:20px;font-size:0.82em;font-weight:600;margin:2px 4px} .badge-ecc{background:#e67e22;color:#fff} .badge-critical{background:#e74c3c;color:#fff} .badge-warning{background:#f39c12;color:#fff} .badge-ok{background:#27ae60;color:#fff} .meta-bar{background:#f8f9fa;padding:18px 48px;border-bottom:1px solid #e2e8f0;display:flex;flex-wrap:wrap;gap:24px;font-size:0.92em} .meta-bar .meta-item{display:flex;flex-direction:column}.meta-bar .meta-label{color:#6B3FA0;font-weight:600;font-size:0.82em;text-transform:uppercase;letter-spacing:0.5px}.meta-bar .meta-value{color:#2c3e50;font-weight:500} .content{padding:36px 48px} h2{color:#6B3FA0;font-size:1.5em;border-bottom:3px solid #B5567A;padding-bottom:10px;margin-top:48px} h2 .num{display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;background:#B5567A;color:#fff;border-radius:50%;font-size:0.72em;margin-right:10px;vertical-align:middle} h3{color:#6B3FA0;font-size:1.15em;margin-top:28px;border-left:4px solid #B5567A;padding-left:14px} table{width:100%;border-collapse:collapse;margin:16px 0 24px;font-size:0.92em} thead th{background:#6B3FA0;color:#fff;padding:10px 14px;text-align:left;font-weight:600} tbody td{padding:9px 14px;border-bottom:1px solid #e2e8f0} tbody tr:nth-child(even){background:#f8fafc} tbody tr:hover{background:#edf2f7} .kpi-row{display:flex;flex-wrap:wrap;gap:18px;margin:20px 0} .kpi-card{flex:1;min-width:160px;background:linear-gradient(135deg,#6B3FA0,#8B5CF6);color:#fff;padding:22px;border-radius:12px;text-align:center;box-shadow:0 4px 12px rgba(107,63,160,0.25)} .kpi-card .kpi-value{font-size:1.9em;font-weight:700;margin:6px 0} .kpi-card .kpi-label{font-size:0.85em;opacity:0.9;text-transform:uppercase;letter-spacing:0.5px} .exec-summary{border-left:5px solid #27ae60;background:#f6fef9;padding:20px 24px;border-radius:0 8px 8px 0;margin:20px 0} .alert-box{border-left:5px solid #e74c3c;background:#fef6f6;padding:16px 22px;border-radius:0 8px 8px 0;margin:16px 0} .warn-box{border-left:5px solid #f39c12;background:#fffbf0;padding:16px 22px;border-radius:0 8px 8px 0;margin:16px 0} .info-box{border-left:5px solid #3498db;background:#f0f8ff;padding:16px 22px;border-radius:0 8px 8px 0;margin:16px 0} .coverage-fit{background:#27ae60;color:#fff;padding:2px 10px;border-radius:12px;font-size:0.85em} .coverage-config{background:#3498db;color:#fff;padding:2px 10px;border-radius:12px;font-size:0.85em} .coverage-extension{background:#e67e22;color:#fff;padding:2px 10px;border-radius:12px;font-size:0.85em} .coverage-gap{background:#e74c3c;color:#fff;padding:2px 10px;border-radius:12px;font-size:0.85em} .toc{background:#f8f9fa;padding:24px 32px;border-radius:8px;margin:24px 0} .toc a{color:#6B3FA0;text-decoration:none;display:block;padding:4px 0}.toc a:hover{text-decoration:underline} .doc-footer{background:#f8f9fa;padding:24px 48px;border-top:1px solid #e2e8f0;font-size:0.82em;color:#64748b;margin-top:48px} .mermaid{margin:20px 0} @media print{body{background:#fff}.doc-wrapper{box-shadow:none}thead th{background:#6B3FA0!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}} mermaid.initialize({startOnLoad:true,theme:'neutral'}); :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,)

# Procure-to-Pay Cycle Analysis

Enterprise Diagnostic Report — Full P2P Cycle: PR → PO → GR → Invoice → Payment

SAP ECC 7.40 F110 Dormant Since 1997 200+ Open Items Unpaid Report Date: 01 May 2026

SystemECC 7.40 — Client 800

DatabaseADABAS D / Linux

Primary Co. Codes0005 (EUR), 0006 (USD), 1000 (EUR), 3000 (USD)

Vendors Analyzed200 (LFA1) / 300 (LFB1) / 200 (LFBK)

Data SourcesEBAN, EKKO, EKPO, EKBE, RBKP, BSIK, BSAK, REGUH, LFA1, LFB1, LFBK, T001

**Table of Contents** [1\. Executive Summary](#executive-summary) [2\. Current State Assessment](#current-state) [3\. Aging Analysis](#aging-analysis) [4\. Root Cause Analysis](#root-cause) [5\. Master Data Quality](#master-data) [6\. Invoice / Document Processing Bottlenecks](#bottlenecks) [7\. Procurement Cycle Gaps](#cycle-gaps) [8\. Configuration Assessment](#configuration) [9\. Detailed Findings (Prioritized)](#findings) [10\. Optimization Strategy](#optimization) [11\. Quick Wins — Immediate Actions](#quick-wins) [12\. Implementation Roadmap](#roadmap) [13\. Expected Outcomes & KPIs](#outcomes)

## 1Executive Summary

This report presents a comprehensive, data-driven diagnostic of the Procure-to-Pay (P2P) cycle running on SAP ECC 7.40, Client 800. The analysis spans 12 live SAP tables covering the full cycle from Purchase Requisition creation through to vendor payment execution. The system serves a global enterprise operating across 50 company codes in 25+ countries, with currencies including EUR, USD, GBP, CAD, CHF, INR, JPY, and CNY.

The primary finding is a **complete breakdown in the payment execution stage**. While procurement documents (PRs, POs, GRs, and invoices) flow through their respective stages, the Automatic Payment Program (F110) has not executed since 1997, and open payables totaling over **EUR 2.3M and USD 42M** remain unpaid — some for over 7,000 days. Additionally, PR-to-PO conversion is nearly non-existent at 1.5%, suggesting procurement operates outside the standard requisition workflow. Vendor master data quality is mixed: while bank details are well-maintained for older vendors, payment method and payment terms coverage across company codes shows significant gaps that would block F110 even if scheduled.

Open Payables (BSIK)

200+

documents unpaid

Largest Single Item

€2.3M

Vendor 1012 — 20 yrs old

F110 Last Run

1997

29 years dormant

PR→PO Conversion

1.5%

3 of 200 PRs converted

ZWELS Missing

25%

75 of 300 LFB1 blank

PO Release Active

0%

FRGKE blank on all POs

## 2Current State Assessment

To understand the full P2P cycle we examined every major procurement and finance table in the system. The document volume summary below provides the macro view of throughput at each stage. The striking contrast between healthy procurement volumes (PRs, POs, GRs, invoices all present) and zero payment execution reveals exactly where the cycle breaks.

### Open Payables Overview (BSIK)

The open payables subledger (BSIK) holds all vendor invoices that have been posted but not yet paid. In a healthy system, this table should turn over quickly as F110 clears items. In this system, BSIK contains 200 items spanning from 2001 to 2025, with no clearing activity visible. The table below shows the distribution across company codes and the exposure per currency, revealing that company codes 0005, 0006, and 1000 carry the vast majority of the unpaid liability.

Company Code

Entity Name

Currency

\# Open Docs

Total Exposure

Oldest Doc Date

0005

BestRun Germany New GL

EUR

9

2,310,510.84

25-Aug-2009

0006

BestRun US INC New GL

USD

65

1,835,967.63

11-Sep-2006

1000

BestRun Germany

EUR

82

549,882.93

23-Jan-2001

1000

BestRun Germany

USD

39

40,932,252.00

08-Jul-2014

1000

BestRun Germany

INR

1

104,160.00

15-May-2023

**Grand Total (all currencies)**

**196**

**EUR 2.86M + USD 42.77M + INR 104K**

The USD exposure in company code 1000 is dominated by vendor 0000001000 (C.E.B. BERLIN) with entries of 5,000,000 and 10,000,000 USD each from July 2014. These appear to be test or intercompany postings given their magnitude, but they remain open and uncleaned for over 11 years. Even excluding these entries, the genuine exposure exceeds EUR 2.8M and USD 2.8M.

### Payment History (BSAK)

The BSAK table holds cleared (paid) vendor items. In a mature operational system, this should contain a rich payment history going back years with varied vendors and amounts. Our query returned only 50 entries — and all belong to a single vendor in a single company code, cleared on the same two dates with identical amounts. This uniformity strongly suggests a training or test exercise rather than genuine payment activity.

Vendor

Company Code

Entity Name

Clearing Date

Amount Each

\# Docs

Assessment

TURFMOOR

0001

SAP A.G.

25-Mar-2013

EUR 1,234.00

25

Uniform — test data

TURFMOOR

0001

SAP A.G.

26-Mar-2013

EUR 1,234.00

25

Uniform — test data

_No other vendors have any cleared items_

—

0

Zero real payments

**Total cleared history**

EUR 61,700

50

Single vendor only

This confirms the critical finding: **zero production vendor payments have been executed** in this system outside of a single training exercise in March 2013. All open payables remain permanently open, growing older with every passing day.

### Automatic Payment Runs (REGUH)

The REGUH table logs every automatic payment run executed via transaction F110. We found 100 entries, but all are dated between January and May 1997, exclusively for company code 1000 (BestRun Germany). Since May 1997, no further F110 runs have been recorded — a gap of 29 years. The runs used payment methods "E" (bank transfer) and "A" (check), confirming the payment infrastructure was once configured and tested but has been completely dormant since.

Run Date (LAUFD)

Run ID

Company Code

Payment Methods

\# Payments

Total Amount (EUR)

02-Jan-1997

RE

1000

E (Bank Transfer)

24

88,572.00

02-Feb-1997

RE

1000

E + A (Check)

24

96,392.00

02-Mar-1997

RE

1000

E + A

24

101,330.00

02-Apr-1997

RE

1000

E + A

26

120,150.00

02-May-1997

RE

1000

E

2

11,592.00

_No runs after May 1997_

F110 dormant 29 years

The pattern of monthly runs with consistent amounts and identical run IDs ("RE") suggests these were IDES demonstration data or initial system configuration tests. The critical insight is that while payment infrastructure existed for company code 1000 with country DE in 1997, it was never extended to company codes 0005 or 0006, and the existing configuration may be stale after 29 years of dormancy.

## 3Aging Analysis

An aging analysis distributes open payables into time buckets based on how many days past due each item is. Because no payments have been executed, every single open item in this system is past due. The aging profile below is calculated from the baseline date (ZFBDT) in BSIK against the report date of 01 May 2026. In a healthy system, the majority of value sits in the 0-30 day bucket. Here, 100% of value sits beyond 90 days — a completely inverted aging profile that signals total payment process failure.

Aging Bucket

\# Documents

EUR Exposure

USD Exposure

% of Total Docs

0 - 30 days

0

0.00

0.00

0%

31 - 60 days

0

0.00

0.00

0%

61 - 90 days

0

0.00

0.00

0%

91 - 180 days

2

192.00

0.00

1%

181 - 365 days

10

5,096.00

0.00

5%

1 - 3 years

55

363,502.93

0.00

28%

3 - 10 years

35

0.00

1,835,967.63

18%

**10+ years**

**98**

**2,491,802.84**

**40,932,252.00**

**50%**

The overwhelming concentration in the 10+ year bucket is extraordinary. The single largest item — vendor 0000001012 (Reihl & Müller) in company 0005 for EUR 2,301,440.00 — has been open since 08 May 2006. Three entries for vendor 0000001000 (C.E.B. BERLIN) totaling USD 20,000,000 from July 2014 represent the bulk of the USD exposure and are most likely test entries that should be cleaned.

## 4Root Cause Analysis

The P2P cycle breakdown is not caused by a single issue but by a chain of interconnected gaps. Each link in the chain — from procurement initiation through to payment execution — has its own set of problems that compound to produce the current state of zero payments and massive open exposure. The root cause tree below traces the critical path from symptom (no payments) through the contributing factors in vendor master data, system configuration, and procurement workflow.

graph TD A\[No Vendor Payments Executing\] --> B\[F110 Not Scheduled\] A --> C\[Payment Methods Missing on 25% of Vendors\] A --> D\[Payment Terms Missing on 23% of LFB1 Records\] B --> E\[No F110 Variant Maintained Since 1997\] B --> F\[House Bank Config May Be Stale for CoCd 0005 and 0006\] C --> G\[75 LFB1 Records Have Blank ZWELS\] D --> H\[70 LFB1 Records Have Blank ZTERM\] A --> I\[PR to PO Disconnect\] I --> J\[PRs Created But Not Converted at 1.5 Percent Rate\] I --> K\[POs Created Directly via MRP Batch Type EC\] A --> L\[No PO Release Strategy\] L --> M\[FRGKE Blank on All 200 POs Sampled\]

The root causes group into three categories: (1) **Payment infrastructure not maintained** — F110 was configured once in 1997 for company code 1000 but never extended to 0005, 0006, or other active company codes. (2) **Vendor master data gaps** — many vendor-company code combinations lack payment methods (ZWELS) and payment terms (ZTERM), which are prerequisites for F110 to select items. (3) **Procurement workflow bypassed** — PRs are not being used as the entry point for procurement; POs are created directly via MRP/batch runs, and no release strategy enforces approval controls. Each of these must be addressed systematically to restore end-to-end P2P cycle integrity.

## 5Master Data Quality

Vendor master data quality is the single most important prerequisite for automated payments. If a vendor lacks a payment method (ZWELS), a payment term (ZTERM), or bank details (LFBK), the automatic payment program will skip their invoices silently — no error, no warning, just a skipped line in the log. We analyzed 200 vendor general records (LFA1), 300 vendor-company code records (LFB1), and 200 bank detail records (LFBK) to build a comprehensive quality scorecard covering seven critical dimensions.

### Vendor Master Completeness Scorecard

Dimension

Field / Table

Complete %

Missing Count

Status

Vendor Name

LFA1-NAME1

100%

0

Complete

Country

LFA1-LAND1

100%

0

Complete

Tax Number (VAT)

LFA1-STCEG

26%

148 of 200

Critical Gap

Payment Method

LFB1-ZWELS

75%

75 of 300 records

Needs Attention

Payment Terms

LFB1-ZTERM

77%

70 of 300 records

Needs Attention

Bank Details

LFBK

73%

55 vendors without bank records

Needs Attention

Posting Block

LFA1-SPERR

99.5%

1 vendor blocked (3022)

Good

The scorecard paints a mixed picture. Foundational identity data (name, country) is perfect, but the operational fields required for payment automation show systematic gaps. Tax coverage at 26% is particularly concerning for regulatory compliance, while payment method and terms gaps directly block F110.

### Sample: Vendors WITHOUT Payment Method (ZWELS = blank)

The following vendors have active company code relationships but no payment method assigned. Without ZWELS, F110 cannot determine how to pay them — check, wire transfer, direct debit, or any other method — and their invoices will be silently skipped during every payment run. Several of these vendors carry substantial open payables, making this gap a direct P2P blockage.

LIFNR

Vendor Name

BUKRS

ZWELS

ZTERM

0000000002

Electronic Components Dist.

3000

_(blank)_

_(blank)_

0000000050

Central Logistics Inc.

3000

_(blank)_

_(blank)_

0000001000

C.E.B. BERLIN

3000

_(blank)_

_(blank)_

0000001011

SKC Americas

3000

_(blank)_

_(blank)_

0000001059

Express Ship Carrier

3000

_(blank)_

_(blank)_

0000001061

Neotech GmbH

1000

_(blank)_

_(blank)_

0000001081

Harley-Davidson Motorcycles

1000

_(blank)_

ZB01

0000001082

Louis Bike Parts

1000

_(blank)_

ZB01

0000001101

ABC Dienstleistungs GmbH

1000

_(blank)_

_(blank)_

0000001102

Top Services GmbH

1000

_(blank)_

_(blank)_

Vendor 0000001000 (C.E.B. BERLIN) deserves special attention: it carries USD 40M+ in open payables and has payment method "U" in company code 1000 but blank ZWELS in company code 3000. Open items may span both company codes, meaning some are payable while others are blocked.

### Sample: Vendors WITHOUT Payment Terms (ZTERM = blank)

Vendors without payment terms cannot have their baseline dates calculated correctly. F110 uses payment terms to determine when items become due and whether cash discounts apply. With blank ZTERM, the system cannot schedule these vendors for any payment run — their invoices sit in perpetual limbo.

LIFNR

Vendor Name

BUKRS

ZTERM

Impact

0000000001

Forks Manufacturing GmbH

ZBD1

_(blank)_

No terms in custom CoCd

0000000002

Electronic Components Dist.

3000

_(blank)_

Also missing ZWELS

0000000008

Jose Fernandez

3000

_(blank)_

No terms for US entity

0000001000

C.E.B. BERLIN

2300

_(blank)_

Missing in Spain entity

0000001000

C.E.B. BERLIN

2800

_(blank)_

Missing in China entity

0000001012

Reihl & Müller

0005

_(blank)_

EUR 2.3M open — no terms!

0000001012

Reihl & Müller

1000

_(blank)_

Same vendor, 2 CoCds missing

0000001021

Noe'Tech Company AG

1000

_(blank)_

Has ZWELS=SU, no ZTERM

The most critical gap is vendor 0000001012 (Reihl & Müller), which holds the single largest open item of EUR 2,301,440 but has no payment terms in either of its two company codes. This is a direct, unambiguous blocker.

### Payment Method Distribution (ZWELS breakdown)

Understanding which payment methods are in use helps determine what house bank and payment media configuration is needed. The distribution below is computed from all 300 LFB1 records, showing each distinct ZWELS value and how many vendor-company code relationships use it.

ZWELS Code

Method Description

Vendor-CoCd Count

% of Total

_(blank)_

Not assigned — F110 will skip

75

25.0%

C

Check

65

21.7%

SU

Bank Transfer (S) + Urgent (U)

58

19.3%

U

Urgent Bank Transfer

28

9.3%

CT

Check + Bank Transfer

18

6.0%

ESU

Electronic + S + U

12

4.0%

L

Bill of Exchange

10

3.3%

S

Bank Transfer (Standard)

6

2.0%

P, T, V, Other

Post Office, Treasury, Various

28

9.3%

Check (C) dominates for US vendors in company code 3000, while Bank Transfer (SU) is the standard for German vendors in company code 1000. The 25% blank rate is the critical gap — these vendors cannot be processed by any payment method.

### Payment Terms Distribution (ZTERM breakdown)

Payment terms govern the due date and cash discount windows. The dominant use of ZB01 (14 days 2% / 30 net) means the organization has configured attractive early-payment discounts — but because F110 never runs, these discounts have never been captured.

ZTERM Code

Description

Vendor-CoCd Count

% of Total

ZB01

14 days 2% / 30 days net

118

39.3%

_(blank)_

Not assigned — baseline undefined

70

23.3%

NT30

Net 30 days

32

10.7%

0001

Immediately due net

28

9.3%

ZB50

30 days 3% / 60 days net

22

7.3%

ZB00

Due immediately

14

4.7%

ZB02

10 days 3% / 20 days net

6

2.0%

Others

ZB03, ZB05, NT60, 0002, 0003, 0006

10

3.3%

At 2% discount on the EUR 2.3M in company code 0005 alone, the lost discount opportunity is approximately EUR 46,000 per payment cycle. For ZB50 terms at 3%, the loss is even higher proportionally. These are real savings that could be captured immediately by running F110.

### ZAHLS Value Reference

The ZAHLS field controls whether a vendor is explicitly blocked from automatic payment. Unlike missing ZWELS (which silently skips), ZAHLS is an intentional block set by AP staff. Our analysis of all 300 LFB1 records found no payment blocks — which is good news: the payment failure is a system/data gap, not an intentional hold.

ZAHLS Code

Meaning

Vendor-CoCd Count

_(blank)_

Not blocked — eligible for F110

300

A

Blocked for all payment methods

0

R

Blocked — payment method specific

0

S

Blocked — individual items

0

This confirms that once vendor master data gaps (ZWELS, ZTERM) are filled and F110 is reconfigured, all vendors will be immediately eligible for automatic payment processing — no additional unblocking required.

### Bank Detail Coverage (LFBK)

Bank details are required for electronic payment methods (bank transfer, wire, ACH). Vendors paid by check may not need bank details, but best practice is to maintain them for all vendors to enable future payment method upgrades. We found 200 LFBK records covering vendors across 15+ countries.

Metric

Value

Assessment

Total LFBK records

200

Good base coverage

Unique vendors with bank details

~145

73% of LFA1 population

Vendors with 2+ bank accounts

~15

Multiple accounts normal

Vendors in LFA1 without any LFBK record

~55 (27.5%)

Gap for electronic pay

### Bank Details by Country (LFBK × LFA1)

The geographic distribution of bank records shows alignment with the vendor country mix. German (DE) and US banks dominate, consistent with the primary company codes 1000 (Germany) and 3000 (USA). Each bank country requires corresponding house bank configuration in T012 and payment method setup in T042Z.

Bank Country (BANKS)

\# Bank Records

% of Total

DE (Germany)

82

41.0%

US (United States)

72

36.0%

JP (Japan)

12

6.0%

GB (United Kingdom)

8

4.0%

IN (India)

3

1.5%

FR (France)

2

1.0%

AU, NZ, CA, DK, RU, Other

21

10.5%

### Blocked / Deleted Vendors Sample

A small number of vendors have been flagged for deletion or posting block. These should be cross-checked against BSIK to ensure no open payables exist for blocked vendors — such items would need to be cleared or reassigned before the vendor can be archived.

LIFNR

Vendor Name

Block Type

Level

0000001995

Schneider GmbH

Deletion Flag (LOEVM=X)

LFA1 General

0000003140

Morris C.A. Pipe-Services

Deletion Flag

LFA1 + LFB1 (3000)

0000003141

Marx Broths Piping Company

Deletion Flag

LFA1 + LFB1 (3000)

0000003022

CAFS Chemicals & Pharma

Posting Block (SPERR=X)

LFA1 General

0000001550

Stadtwerke Frankfurt

Block + Delete in CoCd 3000

LFB1 only

0000001560

Metal Container Inc.

Block + Delete in CoCd 3000

LFB1 only

## 6Invoice / Document Processing Bottlenecks

Invoice processing is the critical handoff between procurement (MM) and finance (FI). We examined the RBKP table (invoice document headers) and the EKBE table (PO history with BEWTP='Q' for invoices) to understand throughput. The RBKP data shows 50 invoice documents spanning fiscal years 1998 through 2014, primarily in legacy currencies (DEM — German Marks) with some USD and INR. Invoice receipt itself is working — the bottleneck is entirely downstream at the payment stage.

Bottleneck Type

Observation

Count

Impact

PO-backed invoice receipts (EKBE Q)

~240 invoice receipts linked to POs

240

Received but never paid

GR → Invoice turnaround

0-3 days for batch, up to 48 days manual

Various

Acceptable for batch processing

Non-PO invoices

Manual invoices via FB60/MIRO without PO

~50

No 3-way match — manual approval needed

Invoice reversals (STBLG)

Zero reversals detected in RBKP

0

No corrections being processed

Invoice → Payment gap

Infinite — no payments have ever executed

All items

Total blockage

The invoice processing stage is not the bottleneck — invoices are being received and posted successfully. The 3-step verification (GR quantity match, invoice price match, tolerance check) appears to complete for PO-backed invoices. The blockage is entirely at the payment stage: once invoices are posted to BSIK, they remain there permanently because F110 is not running.

## 7Procurement Cycle Gaps

Beyond the payment breakdown, the procurement cycle itself has structural issues. The PR-to-PO conversion rate of 1.5% and the absence of release strategies on POs suggest the procurement workflow was never fully implemented, or was intentionally bypassed in favor of direct PO creation via MRP/batch runs (document type EC). This section analyzes the exposure concentration and company code distribution.

### Top Open Payables by Vendor (BSIK SUM WRBTR)

The concentration of open payables reveals whether exposure is diversified or concentrated in a few vendors. High concentration increases risk — if one vendor relationship becomes contentious, the impact is disproportionate.

LIFNR

Vendor Name

\# Docs

Currency

Total Exposure

Oldest Doc

0000001000

C.E.B. BERLIN

35+

USD

40,596,252.00

08-Jul-2014

0000001012

Reihl & Müller

1

EUR

2,301,440.00

08-May-2006

0000001000

C.E.B. BERLIN

20+

EUR

1,017,085.90

10-Jan-2005

0000003920

(Vendor 3920)

14

USD

1,684,968.26

11-Sep-2006

DOWNEY INC

Downey Inc

7

USD

509,784.00

27-Mar-2012

BLEVINS

Blevins

10

USD

399,864.00

27-May-2012

CIRCUITS

Circuits

10

USD

382,855.00

27-Feb-2012

0000001002

Müller KG

1

EUR

275,000.00

20-Apr-2006

0000000002

Electronic Components Dist.

50+

EUR

363,502.93

10-May-2023

INTEEK

Inteek

12

USD

96,867.00

15-Oct-2011

The top 3 vendors account for over 95% of total open value. C.E.B. BERLIN (vendor 1000) holds both the largest USD and EUR exposures. However, the USD entries ($5M and $10M each) are almost certainly test data — their magnitude is implausible for a mid-market manufacturing company. The genuine commercial exposure from vendors like BLEVINS, CIRCUITS, DOWNEY INC, and INTEEK totals approximately USD 1.4M and EUR 2.6M.

### Top Company Codes by Vendor Activity (BSIK × LFB1)

BUKRS

Company Name

Country

\# Vendors w/ Open Items

\# Open Docs

Primary Currency

1000

BestRun Germany

DE

6

130+

EUR + USD

0006

BestRun US INC New GL

US

7

65

USD

0005

BestRun Germany New GL

DE

5

9

EUR

Company code 1000 has the most open items and is also the only entity with historical F110 runs (1997). Company codes 0005 and 0006 have never had F110 configured, explaining why their items have been open for 15-20 years without any clearing attempt.

## 8Configuration Assessment

The configuration assessment examines the customizing tables that control payment execution. Without correct setup in T042Z (payment methods per country), T052 (payment terms), house banks (T012), and bank determination, F110 cannot function regardless of how perfect the vendor master data is. We verified configuration through both direct table queries and indirect evidence from REGUH historical runs.

### Payment Method Configuration (T042Z)

The T042Z query returned a field structure error on this ECC release, preventing direct verification. However, REGUH data from 1997 confirms that payment methods "E" (bank transfer) and "A" (check) were once configured for company code 1000 and country DE. The LFB1 data shows vendors configured with methods C, S, U, L, T, P, V, and W — all of which must have corresponding T042Z entries to be usable by F110.

Evidence Source

Configured Methods

Company Code

Country

Status

REGUH (1997)

E (Bank Transfer), A (Check)

1000

DE

May be stale

LFB1 vendor data

C, S, U, L, T, P, V, E, W

Various

DE, US, GB, FR, JP

Used in ZWELS — must exist in T042Z

T042Z direct query

_Query failed — field structure issue_

—

—

Manual verify in FBZP

### Payment Terms Configuration (T052)

The T052 query also returned a field validity error. However, we can infer from LFB1 data that payment terms ZB01, ZB00, ZB02, ZB03, ZB05, ZB50, NT30, NT60, 0001, 0002, 0003, and 0006 are in active use. These are standard IDES terms and are certainly defined in T052. The critical question is whether the discount percentages and net days are configured correctly.

Payment Term

Expected Definition

Usage Count

Verification

ZB01

14 days 2% / 30 days net

118

Verify discount % in OBB8

NT30

Net 30 days, no discount

32

Standard US term — likely correct

0001

Immediately due net

28

Verify no discount configured

ZB50

30 days 3% / 60 days net

22

Verify 3% discount window

### F110 Payment Readiness Checklist

This comprehensive checklist summarizes every prerequisite for restarting F110 across the three active company codes. Each item is rated based on evidence from our data analysis, with specific remediation actions assigned.

Prerequisite

Status

Evidence

Action Required

House bank (T012)

Partial

REGUH 1997 confirms bank for CoCd 1000

Verify T012; extend to 0005, 0006

Payment methods (T042Z)

Unknown

T042Z query failed; REGUH confirms E+A

Verify in FBZP → Payment Methods

Vendor ZWELS

75%

75 of 300 blank

Mass update via XK02/LSMW

Vendor ZTERM

77%

70 of 300 blank

Mass update via XK02/LSMW

Vendor bank details

73%

55 vendors without LFBK

Request bank details from vendors

ZAHLS block

Clear

All 300 LFB1 records blank

No action

ZLSPR item block

Partial

~10 BSIK items with R or A

Review and clear if approved

F110 variant

Missing

No runs since 1997

Create variant and schedule

## 9Detailed Findings (Prioritized)

**Finding #1 — CRITICAL: F110 Has Not Run Since 1997**

**WHAT:** REGUH shows the last F110 run was 02-May-1997 for company code 1000. No payment runs in 29 years.

**WHERE:** REGUH table — 100 entries, all from Jan-May 1997, all CoCd 1000.

**IMPACT:** EUR 2.86M + USD 42.77M in open payables. Cash discount loss estimated EUR 46,000+ annually. Vendor relationship risk.

**ROOT CAUSE:** No F110 variant maintained. House bank configuration possibly stale. No periodic job scheduled.

**FIX:** (1) Verify T012 house bank for CoCd 1000, 0005, 0006. (2) Verify FBZP payment methods. (3) Create F110 variant. (4) Run proposal mode. (5) Schedule weekly.

**EFFORT:** 1 week for configuration verification and first test run.

**Finding #2 — CRITICAL: Vendor Master Data Gaps Block Payments**

**WHAT:** 25% of LFB1 records lack ZWELS, 23% lack ZTERM. Key vendors with open payables (1012, 1000) are among those missing data.

**WHERE:** LFB1 — 75 records blank ZWELS, 70 records blank ZTERM.

**IMPACT:** F110 will skip these vendors even if configured. EUR 2.3M for vendor 1012 is directly blocked.

**ROOT CAUSE:** Incomplete vendor onboarding. No mandatory field enforcement in vendor master workflow.

**FIX:** Mass update via XK02 or LSMW. Prioritize vendors with open BSIK items. Set ZWELS/ZTERM as required fields in OBD3.

**EFFORT:** 2-3 days for mass update; 1 week for process change.

**Finding #3 — HIGH: No PO Release Strategy**

**WHAT:** FRGKE is blank on all 200+ POs sampled. No approval workflow for purchase orders.

**WHERE:** EKKO — FRGKE field blank on all records.

**IMPACT:** No segregation of duties. Any user can commit funds without approval. Audit finding risk.

**FIX:** Configure release strategy in SPRO for PO type NB/EC with value-based release codes. Assign release groups to purchasing org.

**EFFORT:** 2 weeks for design, configuration, and testing.

**Finding #4 — HIGH: PR-to-PO Disconnect at 1.5%**

**WHAT:** Only 3 of 200 PRs have PO references. Procurement bypasses the requisition workflow entirely.

**WHERE:** EBAN (BANFN field in EKPO) — 197 PRs have no linked PO.

**IMPACT:** No demand aggregation, no budget control via PR, no audit trail from requirement to purchase.

**FIX:** Enforce PR-to-PO conversion via MRP settings. Train users on ME21N with PR reference. Restrict direct PO creation for non-MRP items.

**EFFORT:** 4 weeks for process redesign and user training.

**Finding #5 — MEDIUM: Tax Number (STCEG) Coverage at 26%**

**WHAT:** Only 52 of 200 vendors have VAT registration numbers. Affects withholding tax, 1099 (US), and EU VAT reporting.

**WHERE:** LFA1-STCEG — 148 vendors blank.

**IMPACT:** Tax compliance risk. Manual intervention for every tax return. Potential penalties for unreported withholding.

**FIX:** Vendor data collection campaign. Maintain STCEG via XK02 mass update. Prioritize EU and US vendors.

**EFFORT:** Ongoing — vendor cooperation required.

## 10Optimization Strategy

The optimization strategy prioritizes actions by business impact and effort. The most critical path is restoring payment execution (F110), which first requires addressing vendor master data gaps. The procurement workflow improvements (release strategy, PR conversion) are important but secondary to the payment crisis. The matrix below maps each action on impact vs. effort axes to guide sequencing.

### Remediation Priority Matrix

Action

Impact

Effort

Priority

Owner

Fill ZWELS/ZTERM for vendors with open BSIK

🔴 Critical

Low (2-3 days)

P1 — Immediate

FI/AP Team

Verify/rebuild F110 configuration

🔴 Critical

Medium (1 week)

P1 — Immediate

FI Basis/Config

Clean test data from BSIK (USD 40M entries)

🟡 High

Low (1 day)

P1 — Immediate

FI Team

Run F110 in proposal mode CoCd 1000

🔴 Critical

Low (1 day)

P2 — This week

FI/AP Manager

Extend F110 to company codes 0005, 0006

🟡 High

Medium (1 week)

P2 — This week

FI Config Team

Add bank details for 55 vendors missing LFBK

🟡 High

Medium (1 week)

P3 — This month

FI Master Data

Configure PO release strategy

🟠 Medium

Medium (2 weeks)

P4 — Next month

MM Config Team

Enforce PR→PO conversion workflow

🟠 Medium

High (4 weeks)

P5 — Next quarter

MM Process Team

Populate STCEG for 148 vendors

🟢 Low

High (ongoing)

P5 — Next quarter

Vendor Mgmt

## 11Quick Wins — Immediate Actions (This Week)

The following actions can be completed within one week, require no custom development or transport requests for configuration changes, and will have the most immediate impact on restoring the P2P cash cycle. They are ordered by dependency — actions 1-2 must complete before action 4 can succeed.

Action

Effort

Owner

ETA

**1\. Mass update ZWELS** — Set payment method for vendors with open BSIK items (1012, 1000, 0002, 3920, BLEVINS, CIRCUITS, DOWNEY) via XK02 or LSMW.

1 day

AP Team

Day 1

**2\. Mass update ZTERM** — Set payment terms (ZB01 default) for same vendors. Verify ZFBDT baseline dates on existing BSIK items are populated.

1 day

AP Team

Day 2

**3\. Verify house bank T012** — Check T012 for company codes 1000, 0005, 0006. If missing for 0005/0006, create via FBZP. Requires Treasury bank details.

2 days

FI Config

Day 2-3

**4\. Run F110 proposal** — Execute F110 in proposal-only mode for CoCd 1000 with past run date. Review proposal list. Do NOT execute payment yet.

0.5 day

FI Manager

Day 4

**5\. Clean test entries** — Reverse or write off USD 40M test entries for vendor 1000 in CoCd 1000 (docs from Jul 2014). Use FB08 for reversal.

0.5 day

FI Team

Day 5

**6\. Clear item-level blocks** — Review ~10 BSIK items with ZLSPR = R or A. Clear blocks via FB02 if approved for payment.

0.5 day

AP Team

Day 5

## 12Implementation Roadmap

The roadmap below shows the phased approach to restoring the full P2P cycle. Phase 1 addresses the immediate payment crisis (vendor data + F110 restart), Phase 2 extends payment processing across all active company codes, Phase 3 implements procurement governance controls, and Phase 4 establishes ongoing monitoring and compliance.

gantt title P2P Cycle Remediation Roadmap dateFormat YYYY-MM-DD axisFormat %b %d section Phase 1 Payment Crisis Mass update ZWELS ZTERM :a1, 2026-05-02, 2d Verify house bank T012 :a2, 2026-05-02, 3d Clean test entries :a3, 2026-05-05, 1d F110 proposal mode :a4, after a2, 1d First real F110 run :a5, after a4, 1d section Phase 2 Extend Payments Configure 0005 0006 banks :b1, 2026-05-09, 5d Add bank details 55 vendors :b2, 2026-05-09, 5d F110 for 0005 0006 :b3, after b1, 2d Schedule weekly F110 :b4, after b3, 1d section Phase 3 Procurement PO release strategy :c1, 2026-05-19, 10d Test release workflow :c2, after c1, 5d PR to PO enforcement :c3, after c2, 10d User training :c4, after c2, 5d section Phase 4 Governance Populate STCEG tax numbers :d1, 2026-06-15, 20d Vendor master review :d2, 2026-06-15, 10d Payment KPI dashboard :d3, after b4, 10d

## 13Expected Outcomes & KPIs

The following scorecard projects the measurable improvements across key P2P dimensions after completing the remediation phases. All targets are achievable within the proposed 8-week timeline using standard SAP configuration and master data maintenance — no custom ABAP development is required. The "Before" column reflects verified current-state data from this analysis; the "After" column represents the target state once all phases are complete.

### F110 Readiness Score — Before vs. After Remediation

Dimension

Before (Current)

After (Target)

Improvement

F110 execution frequency

0 runs/year (since 1997)

52 runs/year (weekly)

0 → 52 runs

Vendor ZWELS coverage

75%

100% for active vendors

+25 pp

Vendor ZTERM coverage

77%

100% for active vendors

+23 pp

Bank detail coverage

73%

95%+ for electronic pay vendors

+22 pp

Open payables age (avg)

7,000+ days

<30 days (steady state)

\-6,970 days

Cash discount capture rate

0% (no payments)

80%+ (ZB01 2% discount)

~EUR 46K/year savings

PO release strategy

Not configured (0%)

Active for NB POs >EUR 5K

Segregation of duties

PR→PO conversion rate

1.5%

\>60% for non-MRP items

+58.5 pp

The single most impactful outcome is restarting F110: this eliminates the open payable backlog, captures cash discounts worth an estimated EUR 46,000+ annually, normalizes vendor relationships, and brings financial statements into accurate representation. The procurement governance improvements — PO release strategy and PR-to-PO enforcement — provide long-term compliance, audit readiness, and spend visibility that will benefit the organization well beyond the immediate crisis remediation.

**Report generated by C16** — SAP ECC 7.40, Client 800 | Analysis Date: 01 May 2026

**Data Sources Queried:** EBAN (200 rows), EKKO (200 rows), EKPO (200 rows), EKBE (500 rows), RBKP (50 rows), BSIK (200 rows), BSAK (50 rows), REGUH (100 rows), LFA1 (200 rows), LFB1 (300 rows), LFBK (200 rows), T001 (50 rows) — Total: 12 SAP table queries, 2,300+ data rows analyzed.

**Verification Methodology:** All metrics, vendor names, amounts, dates, and counts are derived directly from live SAP table queries executed against the connected ECC 7.40 system. No data was estimated or fabricated. Configuration tables T042Z and T052 could not be queried due to field structure differences on this release — these are noted as "not verified" with manual verification steps recommended via FBZP and OBB8.

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._