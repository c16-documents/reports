  C16 Migration Decision Record — SAP API Policy Compliance \* { box-sizing: border-box; margin: 0; padding: 0; } body { font-family: 'Segoe UI', Arial, sans-serif; color: #2c3e50; background: #e9ecef; line-height: 1.7; } .doc-wrapper { max-width: 1120px; margin: 30px auto; background: #fff; box-shadow: 0 4px 24px rgba(0,0,0,0.10); border-radius: 12px; overflow: hidden; } /\* Header \*/ .header-banner { background: linear-gradient(135deg, #1a1a2e 0%, #16213e 40%, #0f3460 70%, #533483 100%); color: #fff; padding: 40px 48px; position: relative; } .header-banner::after { content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 4px; background: linear-gradient(90deg, #C97B3A, #B5567A, #6B3FA0); } .header-banner img { height: 50px; margin-bottom: 12px; border-radius: 8px; } .header-banner h1 { font-size: 1.9em; font-weight: 700; margin-bottom: 4px; letter-spacing: -0.5px; } .header-banner .subtitle { font-size: 1.15em; opacity: 0.9; margin-bottom: 16px; } .doc-type-badge { display: inline-block; background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.3); padding: 4px 16px; border-radius: 20px; font-size: 0.85em; letter-spacing: 1px; text-transform: uppercase; } /\* Meta bar \*/ .meta-bar { background: #f8f9fa; padding: 20px 48px; border-bottom: 1px solid #e0e0e0; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 12px; } .meta-item { font-size: 0.9em; } .meta-item .label { font-weight: 600; color: #6B3FA0; text-transform: uppercase; font-size: 0.75em; letter-spacing: 0.5px; } .meta-item .value { color: #2c3e50; } /\* Content \*/ .content { padding: 40px 48px; } /\* Section headers \*/ .section-title { font-size: 1.45em; font-weight: 700; color: #1a1a2e; margin: 40px 0 16px 0; padding-bottom: 8px; border-bottom: 3px solid #6B3FA0; display: flex; align-items: center; gap: 12px; } .section-title:first-of-type { margin-top: 0; } .section-num { background: #6B3FA0; color: #fff; width: 32px; height: 32px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; font-size: 0.85em; font-weight: 700; flex-shrink: 0; } h3 { font-size: 1.15em; color: #0f3460; margin: 28px 0 12px 0; font-weight: 600; } h4 { font-size: 1.0em; color: #533483; margin: 20px 0 8px 0; font-weight: 600; } p { margin-bottom: 14px; } /\* Tables \*/ table { width: 100%; border-collapse: collapse; margin: 16px 0 24px 0; font-size: 0.92em; } thead th { background: #1a1a2e; color: #fff; padding: 10px 14px; text-align: left; font-weight: 600; font-size: 0.85em; text-transform: uppercase; letter-spacing: 0.3px; } tbody td { padding: 10px 14px; border-bottom: 1px solid #e8e8e8; vertical-align: top; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } /\* Status badges \*/ .badge-red { background: #e74c3c; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; font-weight: 600; white-space: nowrap; } .badge-yellow { background: #f39c12; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; font-weight: 600; white-space: nowrap; } .badge-green { background: #27ae60; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; font-weight: 600; white-space: nowrap; } .badge-blue { background: #3498db; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; font-weight: 600; white-space: nowrap; } .badge-purple { background: #6B3FA0; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; font-weight: 600; white-space: nowrap; } /\* Decision box \*/ .decision-box { background: linear-gradient(135deg, #f0fdf4, #dcfce7); border: 2px solid #27ae60; border-radius: 10px; padding: 24px 28px; margin: 20px 0; } .decision-box h4 { color: #166534; margin-top: 0; font-size: 1.1em; } /\* Alert box \*/ .alert-box { background: #fef2f2; border: 2px solid #e74c3c; border-radius: 10px; padding: 20px 24px; margin: 20px 0; } .alert-box h4 { color: #991b1b; margin-top: 0; } /\* Info box \*/ .info-box { background: #f0f4ff; border-left: 5px solid #3498db; border-radius: 0 8px 8px 0; padding: 16px 20px; margin: 16px 0; } /\* Architecture diagram \*/ .arch-diagram { background: #1a1a2e; color: #e0e0e0; padding: 24px; border-radius: 10px; font-family: 'Consolas', 'Monaco', 'Courier New', monospace; font-size: 0.85em; line-height: 1.5; overflow-x: auto; margin: 16px 0 24px 0; white-space: pre; } .arch-highlight { color: #4ade80; font-weight: bold; } .arch-warn { color: #fbbf24; font-weight: bold; } .arch-danger { color: #f87171; font-weight: bold; } .arch-info { color: #60a5fa; } /\* KPI cards \*/ .kpi-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 16px; margin: 20px 0; } .kpi-card { background: #f8f9fa; border-radius: 10px; padding: 20px; text-align: center; border-top: 4px solid #6B3FA0; } .kpi-card.red { border-top-color: #e74c3c; } .kpi-card.yellow { border-top-color: #f39c12; } .kpi-card.green { border-top-color: #27ae60; } .kpi-card .kpi-value { font-size: 2em; font-weight: 700; color: #1a1a2e; } .kpi-card .kpi-label { font-size: 0.8em; color: #666; text-transform: uppercase; letter-spacing: 0.5px; margin-top: 4px; } /\* TOC \*/ .toc { background: #f8f9fa; border-radius: 10px; padding: 24px 28px; margin: 20px 0; border-left: 5px solid #6B3FA0; } .toc ul { list-style: none; padding: 0; } .toc li { margin: 6px 0; } .toc a { color: #6B3FA0; text-decoration: none; font-weight: 500; } .toc a:hover { text-decoration: underline; } /\* Comparison table special \*/ .compare-header-a { background: #dc2626 !important; } .compare-header-b { background: #16a34a !important; } /\* Footer \*/ .doc-footer { background: #f8f9fa; padding: 24px 48px; border-top: 1px solid #e0e0e0; font-size: 0.82em; color: #888; } /\* Print \*/ @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; margin: 0; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } thead th { -webkit-print-color-adjust: exact; print-color-adjust: exact; } } /\* Responsive \*/ @media (max-width: 768px) { .content, .meta-bar, .header-banner, .doc-footer { padding-left: 20px; padding-right: 20px; } } :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,)

# C16 Migration Decision Record

SAP API Policy Compliance — Architecture Decision & Migration Plan

Architecture Decision Record • ADR-2026-001

Document Type

Architecture Decision Record (ADR)

Status

Approved for Implementation

Date

02 May 2026

Classification

Internal — C16 Engineering

Decision Owner

C16 Platform Team

Policy Reference

SAP API Policy v4.2026a (April 2026)

**Contents**

*   [1\. Trigger — Why This Decision Is Needed](#trigger)
*   [2\. As-Is Architecture (Current State)](#as-is)
*   [3\. Gap Analysis — What the Policy Exposes](#gaps)
*   [4\. Options Considered](#options)
*   [5\. Decision](#decision)
*   [6\. To-Be Architecture (Target State)](#to-be)
*   [7\. Tool Migration Matrix (102 Tools)](#tool-migration)
*   [8\. C16 Connector — Specification Summary](#connector)
*   [9\. ICF Endpoint Migration — Summary](#icf)
*   [10\. Migration Sequence & Timeline](#roadmap)
*   [11\. Customer Deployment Tiers](#customer-tiers)
*   [12\. Residual Risks After Migration](#risks)
*   [13\. Immediate Actions](#actions)

## 1 Trigger — Why This Decision Is Needed

In April 2026, SAP issued a refreshed **SAP API Policy** that formalises a hard contractual boundary around how external systems — particularly AI agents — can interact with SAP environments. The policy is maintained at:

*   📄 **Policy PDF:** [https://help.sap.com/doc/sap-api-policy/latest/en-US/API\_Policy\_latest.pdf](https://help.sap.com/doc/sap-api-policy/latest/en-US/API_Policy_latest.pdf)
*   🌐 **Policy landing page:** [help.sap.com — SAP API Policy](https://help.sap.com/docs/business-accelerator-hub/sap-business-accelerator-hub/sap-api-policy)

Separately, the **SAP Business Accelerator Hub Terms of Use** (last updated October 8, 2025) governs the api.sap.com website itself. Section 3 of that document explicitly names _"Model Context Protocol (MCP), agent-frameworks"_ among prohibited automated access methods.

In parallel, **SAP Notes 3255746 and 3439624** ban ODP-RFC for non-SAP targets, with a technical blocker landing in the June 2026 patch day — demonstrating that SAP is enforcing the policy through code, not just contracts.

### The Four Policy Provisions That Matter for C16

Clause

Verbatim Excerpt

Why It Matters for C16

**§1.1**  
Published APIs

_"APIs published on the SAP Business Accelerator Hub or otherwise identified in product-specific Documentation ('Published APIs') are made available for the purposes described in that Documentation."_

Defines the permitted API surface. C16's ADT APIs are **not** listed on api.sap.com — they are not "Published APIs" under this definition.

**§1.2**  
Non-Published APIs

_"Customer and third-party applications must not access, invoke, or interact in any manner with APIs that are not Published APIs… except as permitted by the Documentation or otherwise authorized by SAP (e.g., customers may use custom-developed ABAP interfaces in private cloud and on-premise deployments)."_

C16 currently uses 29 ADT-backed tools that are non-published. **However**, §1.2 explicitly permits custom-developed ABAP interfaces on-premise — this is C16's primary compliance path.

**§3**  
Anti-Circumvention

_"Customers, partners, and third parties must not bypass, disable, or otherwise circumvent API Controls, including through intermediary services, custom code or developments, proxies, gateways, impersonation techniques, or similar mechanisms."_

C16 SaaS (Azure) could be characterised as an "intermediary service" or "gateway" between the user and SAP. The C16 Connector architecture directly addresses this — the customer's own infrastructure calls the APIs, not an external service.

**§3(m) of api.sap.com ToU**  
Agent-Frameworks

_"…automated data gathering or extraction methods… (such as bots, scrapers, spiders, crawlers, Model Context Protocol (MCP), agent-frameworks, or any other text and data mining technology), including for text and data mining purposes or artificial intelligence training…"_

C16's `sap_api_hub_search` tool makes automated calls to api.sap.com. Under strict reading, this is an agent-framework accessing the website. **Must be removed.**

**Important interpretive note:** The official SAP API Policy (v4.2026a) does **not** contain explicit, named anti-AI-agent language. Industry coverage (SAPinsider, analyst reports) citing "Section 2.2.2" references an interpretive reading of §1.2, §2, and §3 together, reinforced by SAP CEO Christian Klein's earnings call statements distinguishing "customer data" from "SAP's domain know-how / IP." This document treats the AI-agent implications as **interpretive, not verbatim**, and takes a precautionary compliance posture.

## 2 As-Is Architecture (Current State)

29

Tools via ADT  
(Non-Published)

17

Tools via RFC  
(Classic API)

6

Tools via OData  
(Published)

50

Internal Tools  
(No SAP Call)

### 2.1 Current Architecture Diagram

┌──────────────────────────────────────────────────────────────────┐ │ C16 SaaS Platform (Azure) │ │ │ │ ┌──────────────┐ ┌──────────────┐ ┌──────────────────────┐ │ │ │ Claude LLM │ │ Python │ │ Document Generator │ │ │ │ (AI Engine) │ │ Analysis │ │ (HTML / MD) │ │ │ │ │ │ Engines │ │ │ │ │ └──────┬───────┘ └──────┬───────┘ └──────────┬───────────┘ │ │ │ │ │ │ │ └────────┬────────┘ │ │ │ │ │ │ │ ┌────────▼────────────────────────────────┘ │ │ │ Tool Dispatcher (102 tools) │ │ │ ┌─────────────────────────────────────────────┐ │ │ │ │ 29 tools → ADT REST API (/sap/bc/adt/\*) │ 🔴 │ │ │ │ 17 tools → RFC via SOAP │ 🟡 │ │ │ │ 6 tools → OData Gateway │ 🟢 │ │ │ │ 1 tool → api.sap.com │ 🔴 │ │ │ │ 49 tools → Internal (no SAP call) │ 🟢 │ │ │ └─────────────────────────────────────────────┘ │ │ │ │ │ │ SAP Credentials: Azure Key Vault (C16 tenant) 🔴 │ │ └────────────────┬────────────────────────────────────────┘ └──────────────────────────┼───────────────────────────────────────┘ │ │ Site-to-Site VPN (IPSec / WireGuard) │ HTTPS (port 44300) │ Basic Auth: customer\_user / password │ ┌──────────────────────────▼───────────────────────────────────────┐ │ Customer SAP System (On-Premise) │ │ │ │ /sap/bc/adt/\* ← ADT REST APIs (non-published) │ │ /sap/bc/soap/rfc ← RFC via SOAP (Level B classic) │ │ /sap/opu/odata/\* ← OData Gateway (published) │ │ /sap/bc/zc16/\* ← C16 custom ICF (7 endpoints live) │ │ │ └──────────────────────────────────────────────────────────────────┘

### 2.2 Current Risk Summary

Risk Area

Current State

Policy Clause

Severity

AI directly calls SAP APIs

Claude LLM plans and the Tool Dispatcher executes sequences of API calls against customer SAP

§1.2, §3 (interpretive AI restriction)

HIGH

Non-published APIs used

29 tools use ADT REST APIs not listed on api.sap.com

§1.1, §1.2

HIGH

api.sap.com automated access

`sap_api_hub_search` makes automated search calls

api.sap.com ToU §3(m)

HIGH

SAP credentials outside customer env

Stored in Azure Key Vault (C16's tenant)

Data sovereignty, customer trust

MEDIUM

Source code leaves customer env

Full ABAP source code transferred to C16 cloud for analysis

§3 (anti-circumvention), IP protection

MEDIUM

No SAP-endorsed architecture in path

VPN is network infrastructure, not SAP-endorsed

§1.2, §3 (interpretive exception)

MEDIUM

## 3 Gap Analysis — What the Policy Exposes

#

Policy Requirement

Current C16 State

Gap?

Required Change

G1

Use only Published APIs (§1.1)

29 tools use ADT (non-published), 17 use RFC (Level B classic)

YES

Migrate all ADT-backed tools to customer-owned ICF endpoints (`/sap/bc/zc16/*`) wrapping released ABAP classes — qualifying under §1.2 exception

G2

Do not access api.sap.com via agent-frameworks (ToU §3(m))

`sap_api_hub_search` makes automated calls to api.sap.com

YES

Remove `sap_api_hub_search` entirely. Replace with C16 internal knowledge base.

G3

Do not bypass controls via intermediary services (§3)

C16 SaaS (Azure) acts as intermediary between user and SAP

PARTIAL

Deploy C16 Connector in customer's environment — SAP calls originate from customer's own infrastructure, not external cloud

G4

SAP credentials must remain under customer control

Stored in Azure Key Vault (C16's tenant)

PARTIAL

C16 Connector holds credentials locally in customer's environment — encrypted at rest, never transmitted to C16 SaaS

G5

Avoid systematic data extraction / replication (§2)

Code search index (Zoekt) extracts source code from SAP to C16 cloud. SOD analysis extracts 20,000+ rows.

PARTIAL

Move analysis engines into Connector — run inside customer environment, return results only, not raw data

G6

Customer-developed ABAP interfaces permitted (§1.2)

7 of 13 planned ICF endpoints deployed

ON TRACK

Complete remaining 6 ICF endpoints. Ensure all are in customer namespace, transported through CTS, wrapping released ABAP only.

G7

Audit trail and monitoring capability (§3)

ZC16\_API\_LOG exists for ICF endpoints. ADT calls not logged by C16.

PARTIAL

Once all traffic routes through ICF, every call logged. Connector adds local audit log. No gap.

## 4 Options Considered

#

Option

Description

Compliance Strength

Effort

Works For All Customers?

**A**

**Improve in Place**  
(ICF migration only, keep SaaS-direct)

Complete ICF migration, remove api.sap.com, add human approval gates, client-side throttling. Keep current SaaS → VPN → SAP architecture.

⭐⭐⭐ Defensible  
§1.2 defense only

Low  
Q2-Q3 2026

Yes

**B**

**C16 Connector**  
(Split control/data plane)

Deploy a non-AI Connector component in customer's environment. AI stays in C16 cloud. Connector makes all SAP calls. Credentials never leave customer.

⭐⭐⭐⭐ Strong  
AI never touches SAP

Medium  
Q3-Q4 2026

Yes  
(just needs a VM)

**C**

**Full C16 in Customer Network**

Entire C16 platform deployed inside customer's infrastructure. Only outbound call = LLM API.

⭐⭐⭐⭐⭐ Maximum  
Everything is customer's

High  
2027+

Large enterprises only

**D**

**Cloud Connector + BTP**

Route through SAP Cloud Connector and BTP Destination Service.

⭐⭐⭐⭐⭐ Maximum  
SAP-endorsed architecture

Medium

No  
BTP required

## 5 Decision

#### ✅ Decision: Pursue Option A (immediate) + Option B (strategic target)

**Option A** is the bridge — it reduces risk immediately with changes that are mostly already in progress (ICF migration, api.sap.com removal, human approval gates). This is achievable by Q3 2026.

**Option B** (C16 Connector) is the destination — it structurally eliminates the "AI touches SAP" concern by splitting the control plane (AI in cloud) from the data plane (Connector in customer environment). This is the target architecture for all new customers from 2027.

**Option D** (Cloud Connector) is offered as an optional enhancement on top of Option B for customers who have BTP.

**Option C** (Full C16 in customer) is reserved for regulated industries (banking, pharma, defense, government) where even the Connector model is insufficient and full air-gapped deployment is required.

### Decision Rationale

Criterion

Option A alone

Option A + B (chosen)

Why A+B wins

AI touches SAP?

🔴 Yes — AI still makes SAP calls from cloud

🟢 No — Connector (non-AI) makes calls from customer env

Structurally eliminates the core policy concern

Credentials in customer env?

🔴 No — still in Azure Key Vault

🟢 Yes — Connector holds creds locally

Data sovereignty compliance

Source code leaves customer?

🔴 Yes — transferred to C16 cloud

🟢 No — analysis runs in Connector

IP protection, SAP's "domain know-how" concern

Works for all customers?

🟢 Yes

🟢 Yes (Connector just needs a VM)

Universal — no BTP dependency

Time to implement

🟢 Q2-Q3 2026

🟡 Q3-Q4 2026

Option A runs in parallel, provides immediate risk reduction

## 6 To-Be Architecture (Target State)

┌──────────────────────────────────────────────────────────────────────┐ │ C16 SaaS Platform (Azure) │ │ │ │ ┌──────────────┐ ┌──────────────┐ ┌──────────────────────────┐ │ │ │ Claude LLM │ │ UI / Project│ │ Document Generator │ │ │ │ (AI Engine) │ │ Management │ │ (HTML / MD formatting) │ │ │ └──────┬───────┘ └──────────────┘ └──────────┬───────────────┘ │ │ │ │ │ │ └────────────────┬───────────────────────┘ │ │ │ │ │ ┌────────────────▼───────────────────────────────────────┐ │ │ │ Instruction Compiler │ │ │ │ • Translates AI tool calls → signed instruction JSON │ │ │ │ • JWT/HMAC signed │ │ │ │ • NEVER calls SAP directly │ │ │ └────────────────┬───────────────────────────────────────┘ │ │ │ │ │ No SAP credentials stored here │ │ No SAP API calls made from here │ │ No raw source code received here │ └──────────────────────────┼───────────────────────────────────────────┘ │ │ HTTPS (outbound only, port 443) │ Signed instructions ↓ │ Analysis results ↑ │ ┌──────────────────────────▼───────────────────────────────────────────┐ │ C16 Connector (Customer's Environment — VM / Docker / K8s) │ │ │ │ ┌──────────────────────────────────────────────────────────────┐ │ │ │ server.py — Instruction Executor (deterministic, NOT AI) │ │ │ │ • Validates JWT signature │ │ │ │ • Maps instruction → SAP API call │ │ │ │ • Executes against customer's SAP via ICF / OData │ │ │ │ • Returns results (scrubbed, no raw source to SaaS) │ │ │ └──────────────────────────────────────────────────────────────┘ │ │ │ │ ┌──────────────────────────────────────────────────────────────┐ │ │ │ Local Analysis Engines (Python) │ │ │ │ • Clean Core engine • SOD conflict engine │ │ │ │ • Code metrics engine • Performance analysis │ │ │ │ • Zoekt code search index (built and queried locally) │ │ │ └──────────────────────────────────────────────────────────────┘ │ │ │ │ ┌──────────────────────┐ ┌──────────────────────────────────────┐ │ │ │ Credential Vault │ │ Audit Log │ │ │ │ AES-256-GCM │ │ Every SAP call logged locally │ │ │ │ Never transmitted │ │ Forward to SIEM (optional) │ │ │ └──────────────────────┘ └──────────────────────────────────────┘ │ │ │ └──────────────────────────┬───────────────────────────────────────────┘ │ │ HTTPS (LAN or VPN, port 44300) │ Customer SAP user credentials │ ┌──────────────────────────▼───────────────────────────────────────────┐ │ Customer SAP System (On-Premise / Private Cloud) │ │ │ │ ┌──────────────────────────────────────────────────────────────┐ │ │ │ /sap/bc/zc16/\* — Customer-Owned ICF Endpoints (13 total) │ │ │ │ │ │ │ │ ZCL\_C16\_REST\_BASE (framework) │ │ │ │ ZCL\_C16\_AUTH\_CHECK (authorization) │ │ │ │ ZCL\_C16\_AUDIT → ZC16\_API\_LOG (audit trail) │ │ │ │ │ │ │ │ ↓ wraps ONLY released ABAP classes (verified via │ │ │ │ cloudification catalog — Level A / B classification) │ │ │ └──────────────────────────────────────────────────────────────┘ │ │ │ │ ┌──────────────────────────────────────────────────────────────┐ │ │ │ Published OData Services (V2/V4) — customer's own services │ │ │ └──────────────────────────────────────────────────────────────┘ │ │ │ └──────────────────────────────────────────────────────────────────────┘

### 6.1 How the Target Architecture Addresses Each Gap

Gap

As-Is

To-Be

Policy Clause Addressed

**G1:** Non-published APIs

29 tools call ADT directly

All tools call customer-owned ICF endpoints (`/sap/bc/zc16/*`)

§1.2 — custom-developed ABAP permitted

**G2:** api.sap.com access

`sap_api_hub_search` active

Removed. C16 internal knowledge base used instead.

api.sap.com ToU §3(m)

**G3:** AI as intermediary

AI (Claude) directly executes SAP API calls from Azure

AI generates signed instructions. Connector (non-AI, customer-owned) executes SAP calls.

§3 — no intermediary; customer's own infrastructure

**G4:** Credentials in cloud

Azure Key Vault (C16 tenant)

Encrypted vault inside Connector (customer environment)

Data sovereignty

**G5:** Data extraction

Source code and table data sent to C16 cloud

Analysis engines run inside Connector. Only results sent to SaaS.

§2 — bulk extraction minimised

**G6:** ICF migration

7 of 13 endpoints deployed

All 13 deployed, transported, customer-owned

§1.2 — custom ABAP exception

**G7:** Audit trail

ZC16\_API\_LOG for ICF only; ADT calls unlogged

100% of SAP calls logged — ZC16\_API\_LOG + Connector local log

§3 — monitoring compliance

## 7 Tool Migration Matrix (102 Tools)

### 7.1 ADT-Backed Tools — Must Migrate (29 tools)

#

Tool

What It Does

Target ICF Endpoint

Status

Priority

1

`sap_get_source`

Read ABAP source code

`/sap/bc/zc16/obj_source`

Partial

P0

2

`sap_get_class_source`

Read class source

`/sap/bc/zc16/obj_source`

Partial

P0

3

`sap_get_ddl_source`

Read CDS view source

`/sap/bc/zc16/obj_source`

Partial

P0

4

`sap_scan_sources`

Scan programs for patterns

`/sap/bc/zc16/obj_search`

Partial

P0

5

`sap_syntax_check`

Run syntax check

`/sap/bc/zc16/syntax_check`

Not started

P1

6

`sap_atc_check`

Run ATC quality check

`/sap/bc/zc16/atc_run`

Not started

P1

7

`sap_run_unit_tests`

Execute ABAP Unit tests

`/sap/bc/zc16/unit_test`

Done ✅

—

8

`sap_validate_cds`

Validate CDS DDL

`/sap/bc/zc16/cds_validate`

Not started

P2

9

`sap_check_cds_syntax`

CDS syntax validation

`/sap/bc/zc16/cds_validate`

Not started

P2

10

`sap_goto_definition`

Navigate to definition

`/sap/bc/zc16/goto_def`

Not started

P2

11

`sap_resolve_dependencies`

Resolve compiler deps

`/sap/bc/zc16/dependencies`

Not started

P1

12

`sap_clean_core_check`

ATC cloud readiness

`/sap/bc/zc16/atc_run`

Not started

P1

13

`sap_check_api_release_status`

Check release status

`/sap/bc/zc16/obj_search`

Partial

P1

14

`sap_data_preview`

Execute OpenSQL

`/sap/bc/zc16/data_preview`

Done ✅

—

15

`download_abap_object`

Download baseline

`/sap/bc/zc16/obj_source`

Partial

P0

16

`sync_abap_git_objects`

Deploy objects to SAP

`/sap/bc/zc16/abapgit`

Partial

P0

17

`sap_publish_service_binding`

Publish OData service

`/sap/bc/zc16/sb_publish`

Done ✅

—

18

`sap_batch_deploy_cds`

Batch deploy CDS

`/sap/bc/zc16/abapgit`

Partial

P1

19

`sap_deploy_ui5_app`

Deploy UI5 to BSP

`/sap/bc/zc16/bsp_deploy`

Not started

P2

20

`sap_deploy_optimized_code`

Deploy pushdown artifacts

`/sap/bc/zc16/abapgit`

Partial

P1

21

`sap_probe_dev_capabilities`

Check dev FMs

`/sap/bc/zc16/sys_info`

Done ✅

—

22

`sap_check_system_version`

Read system version

`/sap/bc/zc16/sys_info`

Done ✅

—

23

`sap_list_transports`

List open transports

`/sap/bc/zc16/tr_get`

Done ✅

—

24

`sap_create_transport`

Create transport

`/sap/bc/zc16/tr_create`

Done ✅

—

25

`sap_release_transport`

Release transport

`/sap/bc/zc16/tr_release`

Not started

P2

26

`sap_validate_deployed_objects`

Verify objects exist

`/sap/bc/zc16/obj_search`

Partial

P1

27

`sap_cleanup_orphans`

Find orphaned objects

`/sap/bc/zc16/obj_search`

Partial

P2

28

`get_runtime_dumps`

Read short dumps

`/sap/bc/zc16/runtime_dumps`

Done ✅

—

29

`export_abap_sources`

Bulk export for indexing

`/sap/bc/zc16/obj_search` (in-system search replaces bulk export)

Not started

P1

**Summary:** 8 Done ✅ | 10 Partial ⚠️ | 11 Not started ❌

### 7.2 api.sap.com Tool — Remove (1 tool)

#

Tool

Action

Timeline

30

`sap_api_hub_search`

**Remove entirely.** Replace with C16 internal knowledge base for Fiori app / API recommendations.

This week

### 7.3 RFC-Backed Tools — Migrate to ICF (Lower Priority) (17 tools)

#

Tool

What It Does

Target

Priority

31

`sap_read_table`

Query any SAP table

`/sap/bc/zc16/data_preview`

P1

32

`sap_call_rfc`

Call any RFC FM

`/sap/bc/zc16/rfc_call`

P2

33

`sap_test_rfc_table`

Test RFC with STRING\_TABLE

Deprecate — used only for diagnostics

P3

34

`sap_search_objects`

Search TADIR

`/sap/bc/zc16/obj_search`

P1

35

`sap_resolve_types`

Resolve DD04L/DD03L types

`/sap/bc/zc16/data_preview`

P2

36

`batch_field_lookup`

Batch lookup table fields

`/sap/bc/zc16/data_preview`

P2

37

`sap_read_function_module`

Read FM interface

`/sap/bc/zc16/obj_metadata`

P2

38

`sap_read_class_info`

Read class metadata

`/sap/bc/zc16/obj_metadata`

P2

39

`sap_read_webdynpro_component`

Read WebDynpro

`/sap/bc/zc16/obj_metadata`

P3

40

`sap_read_enhancement_info`

Read enhancement/BAdI

`/sap/bc/zc16/obj_metadata`

P3

41

`sap_find_user_exits`

Find user exits

`/sap/bc/zc16/obj_metadata`

P3

42

`sap_create_table`

Create DDIC table

`/sap/bc/zc16/abapgit`

P2

43

`sap_create_ddic_view`

Create DDIC view

`/sap/bc/zc16/abapgit`

P2

44

`sap_create_package`

Create dev package

`/sap/bc/zc16/pak_validate`

P2

45

`sap_list_packages`

List Z/Y packages

`/sap/bc/zc16/obj_search`

P2

46

`sap_download_smartform`

Download SmartForm

`/sap/bc/zc16/smartform`

P3

47

`sap_generate_odata_classes`

Generate MPC/DPC

`/sap/bc/zc16/abapgit`

P2

### 7.4 OData Gateway Tools — Already Compliant (6 tools)

#

Tool

Status

48-53

`sap_list_odata_services`, `sap_get_service_entities`, `sap_read_odata_data`, `sap_api_test`, `sap_check_gateway_access`, `sap_check_bsp_access`

Compliant ✅ — Published APIs, no change needed

### 7.5 Composite Business Tools (17 tools) — Inherit Parent Transport

#

Tools

Underlying Transport

Action

54-70

`sap_open_po_report`, `sap_procurement_cycle_analysis`, `sap_po_detail_report`, `sap_vendor_po_summary`, `sap_material_po_summary`, `sap_purchase_reference_data`, `sap_inventory_status`, `sap_o2c_process_report`, `sap_o2c_audit_report`, `sap_discover_programs`, `sap_list_programs`, `sap_test_connection`, `sap_refresh_cache`, `sap_analyze_for_migration`, `sap_generate_rap_migration`, `sap_convert_smartform_to_adobe`, `sap_convert_smartform_to_adobe_full`

RFC / OData (mixed)

Automatically compliant once underlying RFC tools are migrated to ICF

### 7.6 Internal C16 Tools — No SAP Connection (32 tools)

#

Tools

Status

71-102

`analyze_clean_core`, `analyze_performance`, `analyze_source_architecture`, `analyze_fit_to_standard`, `generate_pushdown_code`, `write_document`, `learn_skill`, `read_skill_file`, `submit_feedback`, `list_corrections`, `search_code_index`, `search_code_files`, `sandbox_explore`, `container_write_file`, `container_edit_file`, `container_read_file`, `container_list_files`, `start_preview`, `stop_preview`, `init_cap_project`, `cap_validate_tool`, `SAPServiceTool`, `CAPAPITestingTool`, `fiori_generate_tool`, `execute_browser_action`, `cf_deploy_tool`, `shell_tool_fn`, `cap_container`, `deep_research`, `clean_core_lookup`, `component_impact_check`, `sap_deprecated_fm_lookup`

No impact ✅ — No SAP API calls. Internal processing, local CLI, or C16 databases only.

## 8 C16 Connector — Specification Summary

### 8.1 What the Connector Is

The C16 Connector is a **lightweight, deterministic, non-AI execution component** deployed inside the customer's environment. It receives signed instructions from C16 SaaS, executes SAP API calls against the customer's system, runs analysis engines locally, and returns only processed results to C16 SaaS.

**Critical architectural property:** The Connector is NOT an AI system. It does not contain an LLM, does not plan or select API calls, and does not make autonomous decisions. It executes a fixed instruction set — architecturally equivalent to SAP Cloud Connector, GitHub self-hosted runners, Azure Arc agents, and Datadog agents.

### 8.2 Connector Components

Component

Purpose

Runs Where

**server.py**

Instruction receiver, JWT validator, instruction executor, result formatter

Customer VM / Docker

**SAP client libraries**

RFC via SOAP, ICF HTTPS, OData V2/V4 — makes actual SAP calls

Customer VM / Docker

**Analysis engines**

Clean Core, SOD conflict, code metrics, performance analysis, fit-to-standard

Customer VM / Docker

**Zoekt search engine**

Trigram-based code search index — built and queried entirely locally

Customer VM / Docker

**Credential vault**

AES-256-GCM encrypted SAP credentials — never transmitted

Customer VM / Docker

**Audit log**

Every SAP call logged with timestamp, endpoint, parameters, outcome

Customer VM / Docker

### 8.3 What Flows Between SaaS and Connector

Direction

What Flows

What Does NOT Flow

**C16 SaaS → Connector**

Signed instruction JSON: workflow type, parameters, JWT signature

❌ SAP credentials  
❌ Direct API call parameters  
❌ Raw SQL queries

**Connector → C16 SaaS**

Analysis results: scores, findings, summarised metrics, formatted data

❌ Raw source code  
❌ Bulk table data  
❌ SAP credentials  
❌ SAP internal metadata

### 8.4 Connector Infrastructure Requirements

Resource

Minimum

Recommended

Operating System

Ubuntu 22.04 LTS / RHEL 8

Any Linux with Docker support

CPU

2 vCPU

4 vCPU

RAM

4 GB

8 GB

Disk

20 GB

50 GB (for code search index)

Network → SAP

LAN or VPN to SAP (port 44300 or 8000)

LAN preferred

Network → C16 SaaS

Outbound HTTPS to C16 cloud (port 443)

Outbound only — no inbound ports required

Docker

Docker Engine 24+

Docker Compose or Kubernetes

## 9 ICF Endpoint Migration — Summary

#

ICF Endpoint

Replaces (ADT/RFC)

Released ABAP Classes Used

Status

1

`/sap/bc/zc16/data_preview`

ADT freestyle data preview

CL\_SQL\_STATEMENT (released)

Done ✅

2

`/sap/bc/zc16/runtime_dumps`

ADT runtime dump reader

CL\_ABAP\_RUNTIME\_INFO (released)

Done ✅

3

`/sap/bc/zc16/unit_test`

ADT unit test runner

CL\_AUNIT\_RUNNER (released)

Done ✅

4

`/sap/bc/zc16/activate`

ADT activation

Released activation FMs

Done ✅

5

`/sap/bc/zc16/sb_publish`

ADT service binding handler

Released SRVB APIs

Done ✅

6

`/sap/bc/zc16/tr_create`

ADT CTS adapter

Released CTS FMs

Done ✅

7

`/sap/bc/zc16/pak_validate`

ADT package adapter

Released package FMs

Done ✅

8

`/sap/bc/zc16/obj_source`

ADT source reader (programs, classes, CDS)

Released source read APIs + S\_DEVELOP auth

In design

9

`/sap/bc/zc16/obj_search`

ADT search + Zoekt bulk export

Released search APIs + TADIR read

In design

10

`/sap/bc/zc16/abapgit`

ADT abapGit sub-path

Released abapGit classes

Partial

11

`/sap/bc/zc16/goto_def`

ADT navigation API

Released class/method resolution APIs

Not started

12

`/sap/bc/zc16/atc_run`

ADT ATC API

Released ATC FMs

Not started

13

`/sap/bc/zc16/syntax_check`

ADT syntax check

Released syntax check APIs

Not started

**Migration score:** 7 Done ✅ | 3 In progress ⚠️ | 3 Not started ❌

## 10 Migration Sequence & Timeline

Phase

Name

Timeline

Key Deliverables

Dependency

Phase 0

**Immediate Risk Removal**

Week of 5 May 2026

• Remove `sap_api_hub_search` from codebase  
• Replace with C16 internal knowledge base  
• Enforce human approval gate for multi-step workflows

None

Phase 1

**Complete ICF Migration (P0 tools)**

May – Jun 2026

• Build `obj_source`, `obj_search`, `abapgit` ICF endpoints  
• Validate against ADT equivalents  
• Move ICF package out of $TMP into transportable package

ABAP developer availability

Phase 2

**Complete ICF Migration (P1/P2 tools)**

Jun – Aug 2026

• Build `atc_run`, `syntax_check`, `goto_def`, `dependencies`  
• Migrate RFC tools to ICF where value justifies  
• All 13 ICF endpoints active

Phase 1 complete

Phase 3

**Build C16 Connector**

Jul – Sep 2026

• Docker image with server.py + analysis engines  
• Signed instruction protocol (JWT/HMAC)  
• Credential vault implementation  
• Local audit logging  
• Customer installation script

Architect resource hired; Phase 1 endpoints available for Connector to call

Phase 4

**Deprecate ADT Direct Path**

Sep – Oct 2026

• Feature flag: disable ADT path for customers with full ICF set  
• SaaS Tool Dispatcher routes all SAP calls through Connector  
• Remove ADT client code from C16 SaaS

Phase 2 + Phase 3 complete

Phase 5

**Continuous Compliance**

Ongoing

• Monitor SAP Security Patch Day for new enforcement (analogous to June 2026 ODP-RFC blocker)  
• Track ICF endpoint underlying classes against cloudification catalog quarterly  
• Build compliance dashboard per customer  
• Offer Cloud Connector as optional Tier 3 enhancement

Phase 4 complete

## 11 Customer Deployment Tiers

Customer Type

Tier 1: Essential  
(Option A — ICF + VPN)

Tier 2: Standard  
(Option B — Connector)

Tier 3: Enterprise  
(Connector + Cloud Connector)

Tier 4: Sovereign  
(Full C16 in Customer)

**ECC On-Premise**

⭐⭐⭐ Defensible  
§1.2 defense

⭐⭐⭐⭐ Strong  
AI doesn't touch SAP

N/A — ECC has no BTP

⭐⭐⭐⭐⭐ Maximum

**S/4HANA On-Premise**

⭐⭐⭐ Defensible  
§1.2 defense

⭐⭐⭐⭐ Strong  
AI doesn't touch SAP

⭐⭐⭐⭐⭐ Maximum  
SAP-endorsed + split plane

⭐⭐⭐⭐⭐ Maximum

**RISE with SAP /  
Private Cloud**

⭐⭐ At risk  
SAP can enforce

⭐⭐⭐⭐ Strong  
AI doesn't touch SAP

⭐⭐⭐⭐⭐ Maximum  
SAP-endorsed + split plane

⭐⭐⭐⭐⭐ Maximum

**Regulated Industry**  
(banking, pharma, defense)

⭐⭐ Not recommended

⭐⭐⭐⭐ Strong

⭐⭐⭐⭐⭐ Recommended

⭐⭐⭐⭐⭐ Required  
(with local LLM for air-gap)

**Government /  
Air-Gapped**

❌ Not viable

⚠️ Needs internet for SaaS

⚠️ Needs internet for SaaS

✅ Only option  
(local LLM mandatory)

### 11.1 Customer Infrastructure Requirements per Tier

Requirement

Tier 1

Tier 2

Tier 3

Tier 4

VPN to C16 cloud

✅ Required

✅ Required

❌ Replaced by CC

❌ Not needed

Linux VM / Docker

❌ Not needed

✅ 1 VM (2 vCPU, 4GB)

✅ 1 VM (2 vCPU, 4GB)

✅ Multiple VMs / K8s cluster

SAP BTP subaccount

❌ Not needed

❌ Not needed

✅ Required

❌ Not needed

SAP Cloud Connector

❌ Not needed

❌ Not needed

✅ Required

❌ Not needed

GPU for local LLM

❌ Not needed

❌ Not needed

❌ Not needed

⚠️ Optional (for air-gap)

ICF endpoints installed

✅ Required

✅ Required

✅ Required

✅ Required

Customer setup effort

~2 hours

~4 hours

~1 day

~2-3 days

## 12 Residual Risks After Migration

Risk

After Option A Only

After Option A + B (Connector)

Mitigation

C16 is still a generative AI system

MEDIUM — can't change this

LOW — AI never touches SAP

Connector architecture structurally separates AI from SAP

SAP changes policy further

MEDIUM

MEDIUM

Monitor SAP policy updates. Phase 5 continuous compliance.

SAP restricts custom ABAP interfaces

MEDIUM — §1.2 is our foundation

MEDIUM — same foundation

Seek SAP partnership to get C16 formally endorsed. Reduces dependency on §1.2 interpretation.

Released ABAP classes reclassified

LOW

LOW

Quarterly cloudification catalog verification

LLM prompts contain SAP data

MEDIUM — prompts go to Anthropic

MEDIUM — Connector results included in prompts

Offer customer-owned LLM (Azure OpenAI in customer tenant) for Tier 3/4

## 13 Immediate Actions

#### 🔴 Actions Required This Week (Phase 0)

#

Action

Owner

Deadline

Verification

1

**Remove `sap_api_hub_search`** from C16 codebase entirely

C16 Engineering

9 May 2026

Verify tool is not callable. Search codebase for any reference to api.sap.com.

2

**Implement mandatory human approval gate** for multi-step SAP workflows

C16 Engineering

9 May 2026

Every workflow shows numbered plan and waits for user "proceed" before executing SAP calls.

3

**Do NOT share** the earlier "Compliance Brief v1" externally — it was based on incomplete policy analysis

C16 Leadership

Immediate

Confirm no external distribution has occurred.

4

**Engage legal counsel** specialising in SAP licensing to review C16's position under the API Policy

C16 Leadership

16 May 2026

Counsel engaged, initial opinion received.

5

**Begin hiring** SAP Platform Engineer / Integration Architect for Connector build

C16 Leadership

16 May 2026

Job posting published, screening criteria defined.

6

**Prioritise ICF endpoint P0 builds**: `obj_source`, `obj_search`, `abapgit` re-rooting

C16 ABAP Team

30 May 2026

Design documents for all three endpoints reviewed and approved.

#### Summary

**Option A is the bridge. Option B is the destination.**

Option A (ICF migration + api.sap.com removal + human approval gates) provides immediate risk reduction with changes that are mostly already in progress. Target: Q3 2026.

Option B (C16 Connector) structurally eliminates the core policy concern by ensuring the AI never touches SAP directly. Target: Q4 2026.

Together, they create a compliance posture where C16 is a **customer-owned development tool**, calling **customer-owned APIs**, through **customer-controlled infrastructure**, with **full audit trail** — exactly what the SAP API Policy permits under Section 1.2.

**C16 Migration Decision Record** — ADR-2026-001 — Version 1.0 — 02 May 2026

**Policy sources verified:** SAP API Policy PDF retrieved 2026-05-01 from help.sap.com. SAP Business Accelerator Hub Terms of Use (last updated October 8, 2025) reviewed in full. SAP Notes 3255746 and 3439624 referenced from SAP for Me. SAPinsider article (Adam Pitman, 2026-04-28) used for industry context.

**Honesty note:** The official SAP API Policy (v4.2026a) does not contain explicit, named anti-AI-agent language. The AI-agent implications referenced in this document are interpretive consequences of §1.2, §2, and §3 read together, reinforced by SAP CEO statements. This document takes a precautionary compliance posture and is explicit about which claims are verbatim policy quotes versus industry interpretation.

Classification: Internal — C16 Engineering. Not for external distribution without review.

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._