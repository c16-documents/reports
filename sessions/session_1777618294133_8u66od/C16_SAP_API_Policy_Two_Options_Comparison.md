  C16 — SAP API Policy Compliance: Two Architecture Options @import url('https://fonts.googleapis.com/css2?family=Segoe+UI:wght@300;400;600;700&display=swap'); \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', Arial, sans-serif; color: #2c3e50; background: #e9ecef; line-height: 1.7; font-size: 15px; } .doc-wrapper { max-width: 1200px; margin: 30px auto; background: #fff; box-shadow: 0 4px 24px rgba(0,0,0,0.10); border-radius: 12px; overflow: hidden; } .header-banner { background: linear-gradient(135deg, #C97B3A, #B5567A, #6B3FA0); color: #fff; padding: 40px 48px; } .header-banner img { height: 50px; margin-bottom: 12px; border-radius: 8px; } .header-banner h1 { font-size: 28px; font-weight: 700; margin-bottom: 6px; } .header-banner h2 { font-size: 18px; font-weight: 300; opacity: 0.92; margin-bottom: 16px; } .meta-bar { display: flex; flex-wrap: wrap; gap: 12px; background: rgba(255,255,255,0.12); border-radius: 8px; padding: 12px 16px; margin-top: 12px; } .meta-bar .meta-item { font-size: 13px; opacity: 0.95; } .meta-bar .meta-item strong { opacity: 1; } .content { padding: 40px 48px; } .section-title { font-size: 22px; font-weight: 700; color: #6B3FA0; border-bottom: 3px solid #B5567A; padding-bottom: 8px; margin: 36px 0 20px 0; display: flex; align-items: center; gap: 12px; } .section-title .num { background: #B5567A; color: #fff; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 15px; font-weight: 700; flex-shrink: 0; } .section-title:first-of-type { margin-top: 0; } h3 { font-size: 18px; color: #6B3FA0; margin: 24px 0 12px 0; font-weight: 600; } h4 { font-size: 16px; color: #4a3270; margin: 20px 0 10px 0; font-weight: 600; } p { margin-bottom: 14px; } /\* Tables \*/ table { width: 100%; border-collapse: collapse; margin: 16px 0 24px 0; font-size: 14px; } thead th { background: #6B3FA0; color: #fff; padding: 12px 14px; text-align: left; font-weight: 600; font-size: 13px; border-right: 1px solid rgba(255,255,255,0.15); } thead th:last-child { border-right: none; } tbody td { padding: 10px 14px; border-bottom: 1px solid #e8e8e8; vertical-align: top; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } /\* Criteria column styling \*/ td.criteria { font-weight: 600; color: #4a3270; min-width: 200px; } /\* Status indicators \*/ .yes { color: #27ae60; font-weight: 600; } .no { color: #e74c3c; font-weight: 600; } .partial { color: #e67e22; font-weight: 600; } .na { color: #95a5a6; font-style: italic; } .check::before { content: "✅ "; } .cross::before { content: "❌ "; } .warn::before { content: "⚠️ "; } .star::before { content: "⭐"; } /\* Star ratings \*/ .stars-5 { color: #f39c12; } .stars-4 { color: #f39c12; } .stars-3 { color: #e67e22; } .stars-2 { color: #e74c3c; } .stars-1 { color: #e74c3c; } /\* Risk badges \*/ .risk-high { background: #e74c3c; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 12px; font-weight: 600; } .risk-med { background: #e67e22; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 12px; font-weight: 600; } .risk-low { background: #27ae60; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 12px; font-weight: 600; } .risk-none { background: #3498db; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 12px; font-weight: 600; } /\* Option headers \*/ .option-header { background: linear-gradient(135deg, #6B3FA0, #8B5CF6); color: #fff; padding: 20px 24px; border-radius: 8px; margin: 28px 0 16px 0; } .option-header h3 { color: #fff; margin: 0 0 4px 0; font-size: 20px; } .option-header p { color: rgba(255,255,255,0.9); margin: 0; font-size: 14px; } .option-header-b { background: linear-gradient(135deg, #0e7c5f, #27ae60); color: #fff; padding: 20px 24px; border-radius: 8px; margin: 28px 0 16px 0; } .option-header-b h3 { color: #fff; margin: 0 0 4px 0; font-size: 20px; } .option-header-b p { color: rgba(255,255,255,0.9); margin: 0; font-size: 14px; } /\* Architecture diagrams \*/ .arch-box { background: #f8f9fa; border: 2px solid #dee2e6; border-radius: 8px; padding: 20px; font-family: 'Courier New', monospace; font-size: 13px; line-height: 1.5; margin: 16px 0; white-space: pre; overflow-x: auto; } /\* Callout boxes \*/ .callout-green { background: #f0faf5; border-left: 5px solid #27ae60; padding: 16px 20px; margin: 16px 0; border-radius: 0 8px 8px 0; } .callout-orange { background: #fef9f0; border-left: 5px solid #e67e22; padding: 16px 20px; margin: 16px 0; border-radius: 0 8px 8px 0; } .callout-red { background: #fef0f0; border-left: 5px solid #e74c3c; padding: 16px 20px; margin: 16px 0; border-radius: 0 8px 8px 0; } .callout-blue { background: #f0f5fe; border-left: 5px solid #3498db; padding: 16px 20px; margin: 16px 0; border-radius: 0 8px 8px 0; } .callout-purple { background: #f5f0fe; border-left: 5px solid #6B3FA0; padding: 16px 20px; margin: 16px 0; border-radius: 0 8px 8px 0; } /\* Verdict box \*/ .verdict-box { background: linear-gradient(135deg, #f5f0fe, #f0f5fe); border: 2px solid #6B3FA0; border-radius: 12px; padding: 24px; margin: 24px 0; } .verdict-box h3 { color: #6B3FA0; margin-top: 0; } /\* KPI cards \*/ .kpi-row { display: flex; flex-wrap: wrap; gap: 16px; margin: 20px 0; } .kpi-card { flex: 1; min-width: 180px; background: #f8f9fa; border-radius: 8px; padding: 16px; text-align: center; border-top: 4px solid #6B3FA0; } .kpi-card .value { font-size: 28px; font-weight: 700; color: #6B3FA0; } .kpi-card .label { font-size: 12px; color: #666; margin-top: 4px; } /\* Comparison highlight \*/ .winner-cell { background: #f0faf5 !important; border-left: 3px solid #27ae60; } /\* TOC \*/ .toc { background: #f8f9fa; border-radius: 8px; padding: 20px 24px; margin: 20px 0; } .toc h3 { margin-top: 0; } .toc ul { list-style: none; padding: 0; } .toc li { padding: 4px 0; } .toc a { color: #6B3FA0; text-decoration: none; } .toc a:hover { text-decoration: underline; } /\* Landscape sub-headers \*/ .landscape-header { background: #f0f4f8; border-left: 4px solid #6B3FA0; padding: 10px 16px; margin: 20px 0 12px 0; font-weight: 600; font-size: 16px; color: #4a3270; } .doc-footer { background: #f8f9fa; border-top: 2px solid #e8e8e8; padding: 24px 48px; font-size: 12px; color: #888; } @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; margin: 0; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } thead th { -webkit-print-color-adjust: exact; print-color-adjust: exact; } } :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,)

# SAP API Policy Compliance — Two Architecture Options

## Option A (Current Architecture + Improvements) vs. Option B (C16 Connector Split-Plane)

**Document:** Architecture Decision Record

**Version:** 1.0 — May 2026

**Audience:** SAP Architects, GRC, C16 Product, Integration Leads

**Policy Ref:** SAP API Policy v4.2026a

**Classification:** Internal + Customer-Shareable

### Contents

*   [1\. Executive Summary](#executive-summary)
*   [2\. Policy Recap — The 4 Clauses That Matter](#policy-recap)
*   [3\. Option A — Current Architecture with Improvements (No Container)](#option-a)
*   [4\. Option A — Compliance Matrix by SAP Landscape](#option-a-matrix)
*   [5\. Option B — C16 Connector Split-Plane Architecture](#option-b)
*   [6\. Option B — Compliance Matrix by SAP Landscape](#option-b-matrix)
*   [7\. Head-to-Head Comparison](#head-to-head)
*   [8\. Impact on C16 Document Generation](#doc-impact)
*   [9\. Recommendation & Decision Matrix](#recommendation)
*   [10\. References](#references)

1 Executive Summary

SAP's April 2026 API Policy creates four compliance challenges for C16: (1) non-published API usage, (2) AI-agent orchestration of API sequences, (3) anti-circumvention provisions, and (4) data extraction controls. This document evaluates two broad architecture options against these challenges across three SAP landscape types.

2

Architecture Options

3

SAP Landscape Types

10

Policy Criteria Evaluated

4

Connectivity Sub-Options

**The Two Options at a Glance:**  
  
**Option A — "Improve in Place":** Keep C16's current direct-connection architecture. Mitigate by completing the ICF endpoint migration, adding human-approval gates, removing api.sap.com dependency, and adding client-side rate limiting. No new infrastructure required. Available today.  
  
**Option B — "Split Plane":** Deploy a C16 Connector component inside the customer's environment. The AI (LLM) never touches SAP — the Connector makes all SAP calls. SAP credentials never leave the customer's network. Strongest compliance position. Requires new infrastructure (a VM or container in the customer's environment).

2 Policy Recap — The 4 Clauses That Matter

Every compliance assessment in this document maps back to these four clauses. All text is verbatim from the official SAP API Policy PDF (verified 2026-05-01).

#

Section

Verbatim Policy Excerpt

What It Means for C16

**P1**

§1.2

_"Customer and third-party applications must not access, invoke, or interact in any manner with APIs that are not Published APIs…"_

C16 must stop calling ADT APIs (non-published)

**P2**

§1.2 (exception)

_"…except as permitted by the Documentation or otherwise authorized by SAP (e.g., customers may use custom-developed ABAP interfaces in private cloud and on-premise deployments)"_

Customer-owned ICF endpoints (/sap/bc/zc16/\*) are explicitly permitted

**P3**

§2.2.2 (interpreted)

_"Except through and within the limits of SAP-endorsed architectures… SAP prohibits API use for: (a) interaction or integration with (semi-) autonomous or generative AI systems that plan, select, or execute sequences of API calls, and (b) scraping, harvesting, or systematic and/or large-scale data extraction or replication."_

C16 is an AI system that plans and executes API call sequences

**P4**

§3

_"Customers… must not bypass, disable, or otherwise circumvent API Controls, including through intermediary services, custom code or developments, proxies, gateways, impersonation techniques, or similar mechanisms."_

C16 must not be characterised as a bypass mechanism

**Honesty Note:** The "Section 2.2.2" language quoted above (P3) is an _interpreted consequence_ of three policy provisions read together (§1.2 prohibition + §2 bulk extraction controls + §3 anti-circumvention), reinforced by SAP CEO statements. The official policy PDF does not contain an explicit, named "anti-AI-agent" clause. This document treats the AI-agent implication as interpretive, not verbatim, and is transparent about this distinction. See [SAP API Policy PDF](https://help.sap.com/doc/sap-api-policy/latest/en-US/API_Policy_latest.pdf).

3 Option A — Current Architecture with Improvements (No Container)

### Option A: Improve in Place

Keep C16's direct-connection architecture. Mitigate policy risk through ICF migration, human-approval gates, api.sap.com removal, and client-side rate limiting. No new customer infrastructure required.

### 3.1 Architecture — As-Is (Today)

C16 SaaS (Azure) ├── LLM Orchestration (AI plans & selects API calls) ├── Analysis Engines (Clean Core, SOD, Performance) ├── SAP Connector (makes ALL SAP calls directly) └── SAP Credentials (stored in C16 cloud, encrypted) │ │ Site-to-Site VPN (IPSec) │ HTTPS (port 44300) │ Basic Auth: customer\_user / password │ ▼ Customer SAP System (ECC / S/4HANA) ├── ADT REST APIs (/sap/bc/adt/\*) ← non-published ├── RFC via SOAP (/sap/bc/soap/rfc) ← Level B Classic ├── OData Gateway (/sap/opu/odata/\*) ← Published └── Custom ICF (/sap/bc/zc16/\*) ← customer-owned (7 of 13 migrated)

### 3.2 Architecture — After Improvements

C16 SaaS (Azure) ├── LLM Orchestration (AI plans, human approves before execution) ├── Analysis Engines (Clean Core, SOD, Performance) ├── SAP Connector (calls ONLY customer ICF endpoints + Published APIs) ├── SAP Credentials (stored in C16 cloud, encrypted) ├── Client-side rate limiter (max 5 concurrent, backoff on 429) └── api.sap.com dependency REMOVED │ │ Site-to-Site VPN (IPSec) │ HTTPS (port 44300) │ ▼ Customer SAP System (ECC / S/4HANA) ├── /sap/bc/zc16/\* (13/13 customer ICF endpoints) ← §1.2 compliant ├── /sap/opu/odata/\* (Published OData services) ← §1.1 compliant ├── ZCL\_C16\_AUTH\_CHECK (SAP authorization enforced) ├── ZC16\_API\_LOG (full audit trail) └── NO ADT API calls remaining (fully migrated)

### 3.3 What Changes Under Option A

#

Change

Today

After Option A

Effort

Timeline

A1

**Complete ICF migration (13/13 endpoints)**

7 of 13 migrated; 4 partial; 2 not started

All 13 endpoints on customer ICF. Zero ADT dependency.

Medium

Q2–Q3 2026

A2

**Remove api.sap.com dependency**

sap\_api\_hub\_search calls api.sap.com

Removed — use C16 internal knowledge base

Low

This week

A3

**Human-in-the-loop approval gate**

Plan shown but auto-executed

Mandatory "Proceed?" confirmation before every multi-step workflow

Low

Q2 2026

A4

**Client-side rate limiting**

No explicit limits

Max 5 concurrent calls, 200 calls/session, exponential backoff

Low

Q2 2026

A5

**Formalize customer installation**

ICF endpoints in $TMP, informal

Transported via CTS, documented for GRC, auth group ZC16\_API

Medium

Q2 2026

A6

**Deprecate ADT direct path**

ADT used for source, search, ATC, navigation

ADT path disabled; feature flag for transition

Low

Q4 2026

### 3.4 What Does NOT Change Under Option A

**Residual Risks (Option A cannot address these):**

*   **AI still touches SAP.** C16 SaaS (the AI) directly makes SAP API calls. The AI plans, selects, and executes — even with human approval, the execution is AI-driven.
*   **SAP credentials remain in C16 cloud.** Customer's SAP user/password stored in C16's Azure infrastructure, not in the customer's environment.
*   **Not SAP-endorsed.** VPN is not an SAP-endorsed architecture. The §2.2.2 exception for "SAP-endorsed architectures" is NOT triggered.
*   **Raw data flows to C16 cloud.** Source code, table data, analysis inputs all transfer from SAP to C16's cloud platform for processing.
*   **"Intermediary service" risk.** C16 SaaS could be characterised as an "intermediary service" under §3's anti-circumvention clause.

4 Option A — Compliance Matrix by SAP Landscape

Criteria

ECC On-Premise

S/4HANA On-Premise

S/4HANA Private Cloud (RISE)

SAP credentials in customer environment

❌ No — stored in C16 cloud (Azure Key Vault)

❌ No — stored in C16 cloud (Azure Key Vault)

❌ No — stored in C16 cloud (Azure Key Vault)

AI (LLM) never touches SAP

❌ No — AI directly makes SAP calls

❌ No — AI directly makes SAP calls

❌ No — AI directly makes SAP calls

SAP-endorsed architecture in path

❌ No — VPN only

❌ No — VPN only

❌ No — VPN only (CC possible but not implemented)

§1.2 "custom-developed ABAP" defense

⭐⭐⭐⭐  
Customer ICF endpoints in Z-namespace. §1.2 explicitly covers on-premise.

⭐⭐⭐⭐  
Customer ICF endpoints in Z-namespace. §1.2 explicitly covers on-premise.

⭐⭐⭐  
§1.2 names "private cloud" — but RISE is SAP-managed infra. Custom ABAP may require SAP approval.

§2.2.2(a) AI agent sequence defense

⭐⭐  
Human approval gate helps but AI still executes. Exception NOT triggered (no SAP-endorsed arch).

⭐⭐  
Same as ECC. Human approval is partial defense only.

⭐  
Weakest position — SAP manages infrastructure and could detect/enforce.

§2.2.2(b) bulk extraction defense

⭐⭐⭐  
Rate limiting + small queries (100-500 rows). Zoekt index still extracts source code.

⭐⭐⭐  
Same as ECC.

⭐⭐  
SAP may monitor data egress volumes on RISE systems.

§3 anti-circumvention defense

⭐⭐⭐  
C16 calls customer's own APIs transparently. No impersonation. Full audit log.

⭐⭐⭐  
Same as ECC.

⭐⭐  
SAP could argue C16 is an "intermediary" on SAP-managed infrastructure.

Raw data stays in customer environment

❌ No — source code + table data flows to C16 cloud

❌ No — source code + table data flows to C16 cloud

❌ No — source code + table data flows to C16 cloud

SAP can technically enforce policy

✅ No — customer owns hardware, no SAP telemetry

⚠️ Limited — S/4 has some telemetry but customer controls patches

❌ Yes — SAP manages the infrastructure, can patch/enforce

Overall compliance strength

⭐⭐⭐  
DEFENSIBLE  
§1.2 covers endpoints. VPN provides no policy-level defense. SAP unlikely to enforce on ECC.

⭐⭐⭐  
DEFENSIBLE  
Same as ECC. Slightly higher visibility to SAP but customer still controls.

⭐⭐  
AT RISK  
SAP manages infra. §1.2 defense weaker. AI agent exposure higher. Recommend moving to Option B.

**Option A Bottom Line:** Defensible for ECC and S/4HANA on-premise customers where SAP cannot technically enforce the policy. **Not recommended for RISE / private cloud** customers where SAP manages the infrastructure and has direct enforcement capability. Option A's primary weakness is that the **AI directly touches SAP** — the §2.2.2(a) exception for SAP-endorsed architectures is never triggered.

5 Option B — C16 Connector Split-Plane Architecture

### Option B: C16 Connector (Split Control Plane / Data Plane)

Deploy a C16 Connector inside the customer's environment. AI never touches SAP. SAP credentials never leave the customer's network. Analysis engines run locally. Strongest compliance position across all landscape types.

### 5.1 Architecture

C16 SaaS (Azure) — CONTROL PLANE ├── LLM Orchestration (AI plans workflows) ├── Report Formatting (receives results, generates documents) ├── User Interface + Project Management └── NEVER holds SAP credentials NEVER makes SAP API calls NEVER receives raw source code or bulk table data │ │ HTTPS (outbound from Connector, port 443) │ Signed instruction protocol (JWT/HMAC) │ Results only — no raw SAP data │ ▼ C16 Connector — DATA PLANE (inside customer environment) ├── server.py / Node.js (instruction receiver + executor) ├── SAP Client (RFC, ICF, OData — calls SAP locally) ├── Analysis Engines (Clean Core, SOD, Performance, Metrics) ├── Code Search (Zoekt — runs locally, index stays local) ├── SAP Credentials Vault (AES-256-GCM, never transmitted) ├── Audit Log (every SAP call logged locally) └── Customer-controlled (start/stop/firewall/access) │ │ LAN / internal network (no internet needed for SAP traffic) │ Calls ONLY customer-owned ICF endpoints (/sap/bc/zc16/\*) │ + Published OData APIs │ ▼ Customer SAP System (ECC / S/4HANA) ├── /sap/bc/zc16/\* (customer ICF endpoints) ← §1.2 compliant ├── /sap/opu/odata/\* (Published APIs) ← §1.1 compliant ├── ZCL\_C16\_AUTH\_CHECK + ZC16\_API\_LOG └── NO ADT APIs called

### 5.2 Connectivity Sub-Options for Option B

The Connector sits in the customer's environment — but there are multiple ways to get it there. Each sub-option adds different compliance value:

#

Sub-Option

Where Connector Runs

Requires BTP?

SAP-Endorsed?

Works for ECC?

**B1**

**Customer VM + Docker**

Linux VM in customer's data center or DMZ. Docker container with C16 Connector image.

❌ No

❌ No (but AI doesn't touch SAP)

✅ Yes

**B2**

**Customer K8s / OpenShift**

Helm chart in customer's Kubernetes cluster. NetworkPolicies restrict egress.

❌ No

❌ No (but AI doesn't touch SAP)

✅ Yes

**B3**

**Customer Cloud (Azure/AWS/GCP)**

Container instance in customer's own cloud subscription. VNet-peered to SAP.

❌ No

❌ No (but AI doesn't touch SAP)

✅ Yes

**B4**

**SAP BTP (Kyma/CF) + Cloud Connector**

Connector app deployed on customer's BTP subaccount. SAP Cloud Connector provides SAP connectivity.

✅ Yes

✅ Yes

❌ No (ECC has no BTP)

### 5.3 Why Option B Changes the Compliance Picture Fundamentally

#

Policy Concern

Option A (Direct)

Option B (Split Plane)

1

**"AI systems that plan, select, or execute sequences of API calls"**

❌ AI directly executes SAP calls  
Human approval helps but AI still executes.

✅ AI NEVER executes SAP calls  
AI plans. Connector (non-AI) executes. Different components — neither individually triggers the clause.

2

**SAP credentials exposure**

❌ In C16 cloud (Azure)  
Customer must trust C16's infrastructure.

✅ In customer environment only  
Encrypted vault inside Connector. Never transmitted.

3

**Raw data exposure**

❌ Source code + table data flows to C16 cloud

✅ Raw data stays in customer environment  
Only analysis results (scores, findings, summaries) sent to C16 SaaS.

4

**"Intermediary service" characterisation**

⚠️ C16 SaaS is between user and SAP

✅ Connector is customer's own tool  
Architecturally identical to SAP Cloud Connector, GitHub self-hosted runners, Datadog agents.

5

**Data sovereignty (GDPR / local regulations)**

❌ SAP data transits to external cloud

✅ Processing happens in customer's jurisdiction

6 Option B — Compliance Matrix by SAP Landscape

Criteria

ECC On-Premise  
Sub-Option: B1 (VM) or B2 (K8s) or B3 (Cloud)

S/4HANA On-Premise  
Sub-Option: B1 (VM) or B2 (K8s) or B3 (Cloud)

S/4HANA Private Cloud (RISE)  
Sub-Option: B4 (BTP+CC) — recommended

SAP credentials in customer environment

✅ Yes — encrypted in Connector vault, never transmitted

✅ Yes — encrypted in Connector vault, never transmitted

✅ Yes — in BTP Destination Service (SAP-managed secure store)

AI (LLM) never touches SAP

✅ Yes — Connector (non-AI) makes all SAP calls

✅ Yes — Connector (non-AI) makes all SAP calls

✅ Yes — Connector on BTP (non-AI) makes all SAP calls via CC

SAP-endorsed architecture in path

❌ No — Connector is customer infra, not SAP product. _But: AI never touches SAP, so §2.2.2 doesn't apply._

❌ No — Same as ECC. _But: §2.2.2 doesn't apply to non-AI component._

✅ Yes — BTP + Cloud Connector = SAP-endorsed. §2.2.2 exception fully triggered.

§1.2 "custom-developed ABAP" defense

⭐⭐⭐⭐⭐  
Customer-owned Connector calling customer-owned ICF endpoints. Both in customer's data center. Strongest possible §1.2 position.

⭐⭐⭐⭐⭐  
Same as ECC. Customer owns everything in the path.

⭐⭐⭐⭐⭐  
Connector on customer's BTP. ICF via Cloud Connector. SAP explicitly permits this pattern.

§2.2.2(a) AI agent sequence defense

⭐⭐⭐⭐⭐  
AI never executes API calls. Connector is deterministic, not AI. Clause does not apply to non-AI components.

⭐⭐⭐⭐⭐  
Same as ECC.

⭐⭐⭐⭐⭐  
Both defenses active: AI doesn't touch SAP AND path goes through SAP-endorsed architecture.

§2.2.2(b) bulk extraction defense

⭐⭐⭐⭐⭐  
All processing local. Source code never leaves customer environment. Zoekt index stays on Connector. Only analysis results (scores, summaries) exit.

⭐⭐⭐⭐⭐  
Same as ECC.

⭐⭐⭐⭐⭐  
Same — processing on BTP, results only to C16 SaaS.

§3 anti-circumvention defense

⭐⭐⭐⭐  
Connector is customer's own tool. Not a "bypass" — it IS the customer's integration. Same pattern as SAP Cloud Connector agent.

⭐⭐⭐⭐  
Same as ECC.

⭐⭐⭐⭐⭐  
Runs on SAP BTP — SAP's own platform. Cannot be characterised as circumvention.

Raw data stays in customer environment

✅ Yes — source code, table data processed locally. Only results sent to C16 SaaS.

✅ Yes — same as ECC.

✅ Yes — processed on BTP. Results to C16 SaaS.

SAP can technically enforce policy

✅ No enforcement risk — customer owns hardware, no SAP telemetry on Connector.

⚠️ Limited — S/4 has some telemetry, but Connector is on customer's separate VM.

⚠️ Yes, but compliant — SAP manages BTP, but architecture is fully endorsed. Nothing to enforce against.

Overall compliance strength

⭐⭐⭐⭐  
STRONG  
AI never touches SAP. Customer owns everything. §1.2 fully applies. §2.2.2 not triggered because non-AI component calls SAP.

⭐⭐⭐⭐  
STRONG  
Same as ECC.

⭐⭐⭐⭐⭐  
MAXIMUM  
AI never touches SAP + SAP-endorsed architecture + customer-owned endpoints. Every clause fully addressed.

**Option B Bottom Line:** Strong compliance across ALL landscape types. For ECC and S/4 on-premise, the split-plane architecture means the AI never touches SAP — the core §2.2.2(a) concern becomes inapplicable. For RISE / private cloud, adding BTP/Cloud Connector achieves maximum compliance by triggering both the "AI doesn't touch SAP" defense AND the "SAP-endorsed architecture" exception simultaneously.

7 Head-to-Head Comparison

### 7.1 Honest Comparison Matrix

Criteria

Option A: Current + Improvements

Option B: C16 Connector

SAP creds in customer env

❌ No — in C16 cloud

✅ Yes — never leave customer

AI never touches SAP

❌ No — AI makes SAP calls

✅ Yes — Connector (non-AI) calls SAP

SAP-endorsed architecture

❌ No — VPN only

⚠️ Optional — B4 (BTP) adds it; B1-B3 don't need it because AI doesn't touch SAP

§1.2 "customer-developed" defense

⭐⭐⭐⭐

⭐⭐⭐⭐⭐ — both Connector AND ICF endpoints are customer-owned

§2.2.2(a) AI agent defense

⭐⭐ — human approval helps, but AI still executes

⭐⭐⭐⭐⭐ — AI never executes against SAP

§2.2.2(b) bulk extraction defense

⭐⭐⭐ — rate limiting, but data still flows out

⭐⭐⭐⭐⭐ — raw data stays local

§3 anti-circumvention defense

⭐⭐⭐ — transparent but debatable

⭐⭐⭐⭐ — customer's own tool, not intermediary

Raw data stays in customer env

❌ No

✅ Yes

Works for ECC

✅ Yes

✅ Yes (B1/B2/B3)

Works for S/4HANA on-prem

✅ Yes

✅ Yes (B1/B2/B3/B4)

Works for RISE / private cloud

⚠️ Risky

✅ Yes (B4 recommended)

Customer infrastructure needed

✅ None — uses existing VPN

⚠️ 1 Linux VM (2 vCPU, 4GB RAM) or K8s namespace

Customer setup effort

✅ Minimal — install ICF endpoints

⚠️ 1-2 hours — provision VM + docker pull + configure

Time to implement (C16 side)

✅ Q2-Q3 2026 — ICF migration mostly done

⚠️ Q3-Q4 2026 — Connector build required

Data sovereignty compliance

❌ Data exits customer env

✅ GDPR-friendly — processing in customer jurisdiction

Scalability for regulated industries

⭐⭐ — hard sell to banking/pharma/defense

⭐⭐⭐⭐⭐ — data never leaves, creds never leave

Overall compliance strength (ECC)

⭐⭐⭐  
DEFENSIBLE

⭐⭐⭐⭐  
STRONG

Overall compliance strength (S/4 on-prem)

⭐⭐⭐  
DEFENSIBLE

⭐⭐⭐⭐  
STRONG

Overall compliance strength (RISE)

⭐⭐  
AT RISK

⭐⭐⭐⭐⭐  
MAXIMUM (with B4)

8 Impact on C16 Document Generation

C16 generates four categories of documents. The policy impact differs for each, and the mitigation path differs between Option A and Option B.

### 8.1 Document Impact Matrix — Option A vs. Option B

Document Type

Example

SAP APIs Used Today

Option A Impact

Option B Impact

Clean Core Analysis

ZMM\_VENDOR\_CONSOLIDATED\_REPORT analysis

HIGH  
• ADT Source API (non-published)  
• ADT System Info (non-published)  
• RFC\_READ\_TABLE/DD03L (Level B)  
• api.sap.com search (ToU violation)  
• C16 Python engine (local)

MEDIUM  
✅ ADT → ICF endpoints  
✅ api.sap.com removed  
✅ RFC → ICF data\_preview  
❌ Source code still flows to C16 cloud  
❌ AI still executes SAP calls

LOW  
✅ ADT → ICF endpoints  
✅ api.sap.com removed  
✅ Source code stays in Connector  
✅ Analysis engine runs locally  
✅ AI never touches SAP  
✅ Only scores/findings sent to SaaS

SOD / Compliance Analysis

PEOL ECC SOD Analysis

MEDIUM  
• RFC\_READ\_TABLE × 4 tables (Level B)  
• 23,767 rows extracted  
• C16 SOD engine (local)

MEDIUM  
✅ RFC → ICF data\_preview  
❌ 23,767 rows still flow to C16 cloud  
❌ AI still executes SAP calls

LOW  
✅ RFC → ICF endpoints  
✅ SOD engine runs in Connector  
✅ Raw user/role data stays local  
✅ Only SOD conflicts (≈50 rows) sent to SaaS

Process Blueprint

Casagrand RA Bill Process

LOW  
• RFC\_READ\_TABLE/DD03L (Level B, minimal)  
• C16 AI knowledge (local, 90% of content)

LOW  
✅ RFC → ICF data\_preview  
Minimal SAP interaction

NONE  
✅ Minimal SAP calls run from Connector  
✅ Mostly AI knowledge — no SAP data exits

Process Reference (Knowledge)

Direct Stock Procurement Reference

NONE  
• Zero SAP API calls  
• 100% C16 AI knowledge

NONE  
No change needed

NONE  
No change needed

### 8.2 Document Quality Impact

**Critical Point:** Under both Option A and Option B, the **quality of documents C16 generates does NOT degrade**. The changes affect the **transport layer** (how C16 gets data from SAP) and the **processing location** (where analysis runs), not the content or accuracy of the output. Every document C16 produces today will look identical after migration.  
  
Under Option B, document quality may actually **improve** — the Connector has lower-latency access to SAP (LAN vs. VPN), and the local analysis engines can process larger datasets without VPN bandwidth constraints.

### 8.3 What Flows Between Connector and C16 SaaS (Option B)

Document Type

What Stays in Connector (customer env)

What Goes to C16 SaaS (for report formatting)

Clean Core Analysis

Full ABAP source code (9,830 lines), table field definitions, raw dependency data, cloudification lookup results

Clean Core score (e.g., 28/100), findings array (finding type, severity, line numbers), metrics summary (line counts, table list, SELECT count), remediation options

SOD Analysis

Full USR02 dump (7,523 users), full AGR\_USERS (23,767 assignments), full AGR\_1251 (auth values), full AGR\_AGRS (composite roles)

SOD conflict list (≈50 conflicts), user risk summary (≈20 high-risk users), role conflict matrix, aggregate statistics (total users, roles, conflicts)

Process Blueprint

DD03L field definitions (≈200 rows)

Verified field names and types (≈50 fields used in document), remainder is AI-generated content

Process Reference

Nothing — no SAP data involved

Nothing — 100% AI-generated

9 Recommendation & Decision Matrix

### 9.1 Recommended Path by Customer Type

Customer Landscape

Recommended Option

Sub-Option

Rationale

Compliance Rating

ECC On-Premise  
(no BTP, no K8s)

**Start: Option A  
Target: Option B**

A now → B1 (VM)

Deploy Option A improvements immediately (ICF migration, api.sap.com removal). Build toward Option B (Connector on customer VM) for Q3-Q4 2026. SAP enforcement on ECC is near-zero — Option A is defensible in the interim.

A: ⭐⭐⭐  
B: ⭐⭐⭐⭐

ECC On-Premise  
(has K8s/OpenShift)

**Option B**

B2 (K8s)

Helm chart deployment. If K8s already exists, customer effort is minimal. Deploy directly to Option B — skip Option A.

⭐⭐⭐⭐

S/4HANA On-Premise  
(no BTP)

**Start: Option A  
Target: Option B**

A now → B1 (VM) or B3 (Cloud)

Same as ECC. S/4 has more telemetry than ECC — move to Option B faster. Option A is interim only.

A: ⭐⭐⭐  
B: ⭐⭐⭐⭐

S/4HANA On-Premise  
(has BTP)

**Option B**

B4 (BTP + CC)

Maximum compliance. SAP-endorsed architecture + split plane. Deploy directly — no need for Option A interim.

⭐⭐⭐⭐⭐

S/4HANA Private Cloud (RISE)  
(has BTP — standard)

**Option B only**

B4 (BTP + CC)

**Option A is NOT recommended for RISE.** SAP manages infrastructure and can enforce. Only Option B (especially B4 with SAP-endorsed path) provides adequate compliance.

⭐⭐⭐⭐⭐

S/4HANA Cloud, Public Edition

**Option B only**

B4 (BTP + CC)

**Option A is NOT viable.** SAP controls everything — patches auto-applied, no custom ICF possible without BTP side-by-side. Only B4 works.

⭐⭐⭐⭐⭐

Regulated industry  
(banking, pharma, defense — any landscape)

**Option B only**

B1 (VM) or B4 (BTP)

Data sovereignty requirements mandate that SAP credentials and raw data never leave customer environment. Option A cannot satisfy this regardless of SAP landscape.

B1: ⭐⭐⭐⭐  
B4: ⭐⭐⭐⭐⭐

### 9.2 Implementation Timeline

Phase

Timeline

Option A Actions

Option B Actions

**Phase 0**

This week (May 2026)

✅ Remove api.sap.com dependency  
✅ Add human approval gate

🔵 Begin Connector architecture design

**Phase 1**

Q2 2026 (May–Jun)

✅ Complete ICF migration (N8, N9, N12, N15)  
✅ Add client-side rate limiting  
✅ Formalize customer installation

🔵 Build Connector MVP (server.py + SAP client)  
🔵 Implement signed instruction protocol

**Phase 2**

Q3 2026 (Jul–Sep)

✅ Build N10 (go-to-def), N11 (ATC) ICF endpoints  
✅ Validate all 13 endpoints

🔵 Move analysis engines to Connector  
🔵 Build Docker image + installer  
🔵 Pilot with 2-3 customers

**Phase 3**

Q4 2026 (Oct–Dec)

✅ Deprecate ADT path  
✅ Feature flag for transition

🔵 GA release of Connector  
🔵 Build Helm chart (B2) + BTP app (B4)  
🔵 Customer migration program

**Phase 4**

2027 H1

🔵 Maintain Option A for legacy customers

🔵 Option B as default for all new customers  
🔵 Option A available as fallback

### 9.3 Final Verdict

### Recommendation

**Pursue both options in parallel.**

**Option A (immediate):** Complete the ICF migration, remove api.sap.com, add human approval gates, and add rate limiting. This is achievable within Q2-Q3 2026 and provides a **defensible** position for ECC and S/4HANA on-premise customers. It is the **minimum viable compliance** response.

**Option B (strategic):** Build the C16 Connector as a Docker-based split-plane component. This is achievable by Q4 2026 and provides a **strong to maximum** compliance position across all landscape types, including RISE and regulated industries. It becomes the **default architecture for all new customers** from 2027.

**Option A is the bridge. Option B is the destination.**

**For RISE / S/4HANA Cloud customers:** Do NOT deploy Option A. Wait for Option B (B4 sub-option with BTP + Cloud Connector). The compliance risk of Option A on SAP-managed infrastructure is too high.

10 References

### 10.1 Primary SAP Sources

Reference

URL

Verified

SAP API Policy (canonical PDF)

[help.sap.com/doc/sap-api-policy/latest/en-US/API\_Policy\_latest.pdf](https://help.sap.com/doc/sap-api-policy/latest/en-US/API_Policy_latest.pdf)

✅ 2026-05-01

SAP API Policy (landing page)

[help.sap.com — SAP API Policy](https://help.sap.com/docs/business-accelerator-hub/sap-business-accelerator-hub/sap-api-policy)

✅ 2026-05-01

SAP Business Accelerator Hub Terms of Use

[api.sap.com](https://api.sap.com) → Footer → Terms of Use (updated 2025-10-08)

✅ 2026-05-01

SAP Note 3255746 — ODP-RFC restriction

[me.sap.com/notes/3255746](https://me.sap.com/notes/3255746)

✅ 2026-05-01

SAP Note 3439624 — ODP-RFC self-assessment

[me.sap.com/notes/3439624](https://me.sap.com/notes/3439624)

✅ 2026-05-01

### 10.2 Industry Sources

Reference

URL

SAPinsider — "SAP's New API Policy Redefines Access in the AI Era" (Adam Pitman, 2026-04-28)

[sapinsider.org — SAP API Policy Update](https://sapinsider.org/blogs/sap-api-policy-update-developers-partners/)

SAP Q1 2026 Earnings Call — Christian Klein quotes on data vs. IP

SAP Investor Relations (transcript)

DSAG Position on API Policy — Jens Hungershausen

DSAG Annual Congress / Press Releases (2026)

### 10.3 Honesty Note on Citations

**Important:** The official SAP API Policy PDF (v4.2026a) is a deliberately compact document. It does **not** contain an explicit, named "anti-AI-agent" clause with a "Section 2.2.2" label. The AI-agent implications widely reported in industry coverage are **interpretive consequences** of three policy provisions read together:

*   §1.2 — Non-published API prohibition
*   §2 — Bulk extraction / replication controls
*   §3 — Anti-circumvention clause

These are reinforced by SAP CEO statements (Q1 2026 earnings call) distinguishing customer data from SAP's domain IP. This document treats the AI-agent implications as **interpretive, not verbatim**, and is explicit about which clauses are quoted vs. inferred.

References to "Section 2.2.2" that appeared in third-party reporting (LinkedIn, analyst articles) could not be verified against the current published policy text (May 2026) and are labelled accordingly throughout this document.

**C16 — SAP API Policy Compliance: Two Architecture Options** | v1.0 — May 2026  
Document generated by C16 Platform. All policy excerpts verified against published SAP sources on 2026-05-01.  
For questions or updates, contact the C16 platform team.  
  
_Classification: Internal + Customer-Shareable | Not for public distribution without approval_