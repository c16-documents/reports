  ADR-2026-001 v1.2 — C16 Migration Decision Record :root { --c16-purple: #6B3FA0; --c16-pink: #B5567A; --c16-gold: #C97B3A; --green: #27ae60; --red: #e74c3c; --orange: #e67e22; --blue: #3498db; } \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', Arial, sans-serif; color: #2c3e50; background: #e9ecef; line-height: 1.7; } .doc-wrapper { max-width: 1140px; margin: 24px auto; background: #fff; box-shadow: 0 2px 20px rgba(0,0,0,0.1); border-radius: 12px; overflow: hidden; } .header-banner { background: linear-gradient(135deg, #C97B3A, #B5567A, #6B3FA0); color: #fff; padding: 40px 48px; } .header-banner img { height: 50px; margin-bottom: 12px; border-radius: 8px; } .header-banner h1 { font-size: 2em; margin-bottom: 8px; } .header-banner .subtitle { font-size: 1.1em; opacity: 0.9; } .version-badge { display: inline-block; background: rgba(255,255,255,0.2); padding: 4px 16px; border-radius: 20px; font-size: 0.85em; margin-top: 12px; } .version-badge.updated { background: rgba(39,174,96,0.3); } .meta-bar { background: #f8f9fa; padding: 20px 48px; border-bottom: 2px solid #e9ecef; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 12px; } .meta-item { font-size: 0.9em; } .meta-item strong { color: var(--c16-purple); } .content { padding: 40px 48px; } .toc { background: #f8f9fa; border-radius: 8px; padding: 24px 32px; margin-bottom: 32px; border-left: 4px solid var(--c16-purple); } .toc h3 { color: var(--c16-purple); margin-bottom: 12px; } .toc ol { padding-left: 20px; } .toc li { margin-bottom: 6px; font-size: 0.95em; } .toc a { color: var(--c16-purple); text-decoration: none; } .toc a:hover { text-decoration: underline; } .toc .new-tag { background: var(--green); color: #fff; padding: 1px 8px; border-radius: 10px; font-size: 0.75em; margin-left: 6px; } .toc .updated-tag { background: var(--orange); color: #fff; padding: 1px 8px; border-radius: 10px; font-size: 0.75em; margin-left: 6px; } .section-title { color: var(--c16-purple); font-size: 1.5em; margin: 40px 0 20px 0; padding-bottom: 8px; border-bottom: 3px solid var(--c16-pink); } .section-title .num { display: inline-block; background: var(--c16-pink); color: #fff; width: 32px; height: 32px; border-radius: 50%; text-align: center; line-height: 32px; font-size: 0.7em; margin-right: 8px; vertical-align: middle; } h3 { color: #34495e; margin: 24px 0 12px 0; font-size: 1.15em; } h4 { color: var(--c16-purple); margin: 16px 0 8px 0; font-size: 1.05em; } p { margin-bottom: 14px; } table { width: 100%; border-collapse: collapse; margin: 16px 0 24px 0; font-size: 0.92em; } thead th { background: var(--c16-purple); color: #fff; padding: 10px 14px; text-align: left; font-weight: 600; } tbody td { padding: 10px 14px; border-bottom: 1px solid #e9ecef; vertical-align: top; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } .risk-high { background: #e74c3c; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; white-space: nowrap; } .risk-med { background: #e67e22; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; white-space: nowrap; } .risk-low { background: #27ae60; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; white-space: nowrap; } .risk-none { background: #95a5a6; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; white-space: nowrap; } .risk-critical { background: #8e44ad; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; white-space: nowrap; } .status-done { color: var(--green); font-weight: 700; } .status-partial { color: var(--orange); font-weight: 700; } .status-not { color: var(--red); font-weight: 700; } .status-compliant { color: var(--blue); font-weight: 700; } .status-remove { color: var(--red); font-weight: 700; text-decoration: line-through; } .callout { border-left: 5px solid var(--c16-purple); background: #f5f0fa; padding: 16px 20px; margin: 20px 0; border-radius: 0 8px 8px 0; } .callout.green { border-left-color: var(--green); background: #f0faf0; } .callout.red { border-left-color: var(--red); background: #fdf0f0; } .callout.orange { border-left-color: var(--orange); background: #fdf6f0; } .callout.blue { border-left-color: var(--blue); background: #f0f5fd; } .callout.strategic { border-left-color: var(--c16-gold); background: #fdf8f0; } .callout h4 { margin-top: 0; } .decision-box { background: linear-gradient(135deg, #f5f0fa, #fdf0f5); border: 2px solid var(--c16-purple); border-radius: 12px; padding: 24px 32px; margin: 24px 0; } .decision-box h3 { color: var(--c16-purple); margin-top: 0; } .arch-diagram { background: #1a1a2e; color: #e0e0e0; padding: 24px 28px; border-radius: 8px; font-family: 'Consolas', 'Courier New', monospace; font-size: 0.85em; line-height: 1.5; overflow-x: auto; margin: 16px 0 24px 0; white-space: pre; } .arch-diagram .highlight { color: #27ae60; font-weight: 700; } .arch-diagram .warn { color: #e67e22; font-weight: 700; } .arch-diagram .danger { color: #e74c3c; font-weight: 700; } .arch-diagram .info { color: #3498db; font-weight: 700; } .arch-diagram .purple { color: #B5567A; font-weight: 700; } .kpi-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin: 20px 0; } .kpi-card { background: linear-gradient(135deg, var(--c16-purple), var(--c16-pink)); color: #fff; padding: 20px; border-radius: 12px; text-align: center; } .kpi-card .value { font-size: 2em; font-weight: 700; } .kpi-card .label { font-size: 0.85em; opacity: 0.9; margin-top: 4px; } .kpi-card.green { background: linear-gradient(135deg, #27ae60, #2ecc71); } .kpi-card.red { background: linear-gradient(135deg, #c0392b, #e74c3c); } .kpi-card.orange { background: linear-gradient(135deg, #d35400, #e67e22); } .kpi-card.blue { background: linear-gradient(135deg, #2980b9, #3498db); } .changelog { background: #f0faf0; border: 1px solid #27ae60; border-radius: 8px; padding: 20px 24px; margin: 24px 0; } .changelog h4 { color: var(--green); margin-top: 0; } .footer { background: #f8f9fa; padding: 24px 48px; border-top: 2px solid #e9ecef; font-size: 0.85em; color: #7f8c8d; } code { background: #f5f5f5; padding: 2px 6px; border-radius: 4px; font-size: 0.9em; font-family: 'Consolas', monospace; } @media print { .doc-wrapper { box-shadow: none; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } table { page-break-inside: avoid; } }

![C16](data:image/jpeg;base64,)

# C16 Migration Decision Record

SAP API Policy Compliance — Architecture Decision & Execution Plan

ADR-2026-001 v1.2 — 02 May 2026

Classification: Internal — C16 Leadership + Architecture

**Document ID:** ADR-2026-001

**Version:** 1.2 (5 review gaps closed)

**Status:** APPROVED — Execute

**Decision:** Option A (bridge) + Option B (destination)

**Author:** C16 Platform Team

**Reviewed by:** External Architecture Review

**Date:** 02 May 2026

**Next Review:** 01 August 2026

#### 📋 v1.2 Changelog (from v1.1)

Five gaps identified in external architecture review have been closed:

Gap

Issue

Resolution in v1.2

Section

**Gap 1**

No SAP PartnerEdge / ICC track

New §14: Partnership Roadmap + Phase 0 Action IA-7

§14 NEW

**Gap 2**

LLM data flow to Anthropic unresolved

New §8.4: Explicit LLM Data Flow Policy

§8.4 NEW

**Gap 3**

sap\_call\_rfc generic proxy risk

RFC whitelist policy + enforcement spec

§7.6 NEW

**Gap 4**

Composite tools = AI sequencing pattern

Documented call graphs for all 17 composites

§7.7 NEW

**Gap 5**

Existing-customer migration story

Cutover playbook with dual-stack and rollback

§16 NEW

**Additional fixes:** Timeline adjusted (Phase 4 → Q1 2027), sap\_release\_transport elevated to P0, audit log tamper-evidence spec, api.sap.com full codebase grep, ECC transitional tier note, strategic reframe, verified underlying class analysis (81% non-released), §3 anti-circumvention deep analysis, specific law firm recommendations.

### Table of Contents

1.  [Policy Trigger](#trigger)
2.  [Strategic Context](#strategic-context) NEW
3.  [As-Is Architecture](#as-is)
4.  [Gap Analysis](#gaps) UPDATED
5.  [Underlying Class Verification](#underlying) NEW
6.  [Options Considered](#options)
7.  [Decision](#decision)
8.  [To-Be Architecture](#to-be) UPDATED
9.  [Tool Migration Matrix](#tool-matrix) UPDATED
10.  [C16 Connector Specification](#connector) UPDATED
11.  [Source Control, Distribution & Version Control](#scm)
12.  [ICF Endpoint Status](#icf-status) UPDATED
13.  [Migration Timeline](#timeline) UPDATED
14.  [Customer Tiers](#tiers) UPDATED
15.  [SAP Partnership Roadmap](#partnership) NEW
16.  [Residual Risks](#residual) UPDATED
17.  [Existing-Customer Cutover Playbook](#cutover) NEW
18.  [Immediate Actions](#actions) UPDATED
19.  [References & Verification](#references)

## 1 Policy Trigger

In April 2026, SAP published the **SAP API Policy** — a concise, legally sweeping governance document that restricts how external systems interact with SAP environments. Four specific clauses create compliance exposure for C16:

Clause

Verbatim Excerpt

C16 Exposure

**§1.1 — Published APIs**

"APIs published on the SAP Business Accelerator Hub or otherwise identified in product-specific Documentation ('Published APIs') are made available for the purposes described in that Documentation."

C16 uses ADT REST APIs that are **not** listed on api.sap.com as Published APIs.

**§1.2 — Non-Published**

"Customer and third-party applications must not access, invoke, or interact in any manner with APIs that are not Published APIs… except as permitted by the Documentation or otherwise authorized by SAP (**e.g., customers may use custom-developed ABAP interfaces in private cloud and on-premise deployments**)."

C16's ICF endpoints qualify under this exception — **but only if the underlying ABAP classes are released**. See §5 for verification results.

**§3 — Anti-Circumvention**

"Customers, partners, and third parties must not bypass, disable, or otherwise circumvent API Controls, including through **intermediary services, custom code or developments, proxies, gateways**, impersonation techniques, or similar mechanisms."

Custom ICF endpoints wrapping non-released classes could be characterized as circumvention. Generic proxy tools (sap\_call\_rfc) are specifically exposed.

**api.sap.com ToU §3(m)**

"use any automated data gathering or extraction methods… (such as bots, scrapers, spiders, crawlers, **Model Context Protocol (MCP), agent-frameworks**…) including for artificial intelligence training, improvement or evaluation"

C16's `sap_api_hub_search` tool queries api.sap.com programmatically. This is a direct violation.

**Parallel technical enforcement:** SAP Notes 3255746 and 3439624 ban ODP-RFC for non-SAP targets, with active blocking landing in the **June 2026 patch day**. C16 does not use ODP-RFC, but this establishes SAP's willingness to enforce through code, not just contracts.

#### ⚠️ Interpretive Note — Honesty Requirement

The official SAP API Policy PDF does **not** contain explicit, named anti-AI-agent language. References to "Section 2.2.2" in industry coverage could not be verified against the published policy text. The AI-agent implications reported by SAPinsider and other analysts are **interpretive consequences** of §1.2 (non-published prohibition), §2 (bulk extraction controls), and §3 (anti-circumvention), reinforced by SAP CEO Christian Klein's earnings call statements distinguishing customer data from SAP's "domain knowhow / IP."

This document treats the AI-agent implications as **interpretive but precautionary** — we act as if they apply, while being transparent that the policy text does not name AI explicitly.

## 2 Strategic Context

#### 🎯 Reframe: This Is a Product Upgrade, Not a Compliance Retrofit

The Connector model + customer-owned ICF endpoints + audit trail is a **better product** for security-conscious customers than the SaaS-direct path, regardless of any policy. Banks, pharma, defense, and EU public sector were always going to require this architecture. The API policy accelerates a move C16 would have made anyway.

**Internal framing:** "We're building the enterprise-grade product."  
**External framing:** "C16 Enterprise offers data sovereignty, credential isolation, and full audit trail — by design."

**Not:** "We have to change because SAP told us to."

Three concurrent forces drive this decision:

Force

Description

Urgency

**SAP API Policy**

Contractual restriction on non-published APIs and AI agent sequences

🔴 Immediate — policy is live

**Customer demand**

Regulated industries (banking, pharma, defense) require data sovereignty and credential isolation

🟡 Near-term — deal blockers emerging

**Competitive positioning**

Enterprise-grade architecture differentiates from lighter tools that can't meet compliance bar

🟢 Strategic — builds long-term moat

## 3 As-Is Architecture

┌──────────────────────────────────────────────────────────────┐ │ C16 SaaS (Azure — C16's subscription) │ │ │ │ ┌───────────────┐ ┌──────────────┐ ┌─────────────────┐ │ │ │ Claude LLM │ │ Python │ │ Tool Dispatcher │ │ │ │ (Anthropic) │ │ Engines │ │ (93+ tools) │ │ │ └───────┬───────┘ └──────┬───────┘ └────────┬────────┘ │ │ │ │ │ │ │ └────────┬────────┘ │ │ │ │ ┌──────────────────────┘ │ │ ▼ ▼ │ │ ┌────────────────────┐ │ │ │ SAP Connector │ ← AI + SAP calls in SAME │ │ │ (Azure Container) │ process. No separation. │ │ └─────────┬──────────┘ │ │ │ SAP creds in Azure Key Vault │ └────────────────────┼─────────────────────────────────────────┘ │ │ Site-to-Site VPN (IPSec) │ NOT SAP-endorsed architecture │ ┌────────────────────┼─────────────────────────────────────────┐ │ Customer SAP System (on-premise / private cloud) │ │ │ │ │ ▼ │ │ /sap/bc/adt/\* → ADT REST APIs (non-published) │ │ /sap/bc/soap/rfc → RFC via SOAP (Level B classic) │ │ /sap/opu/odata/\* → OData Gateway (published) │ │ /sap/bc/zc16/\* → Custom ICF (7 of 13 migrated) │ └──────────────────────────────────────────────────────────────┘ ▲ PROBLEMS: 1\. AI directly executes SAP API call sequences 2\. 29 tools use non-published ADT APIs 3\. SAP credentials stored outside customer environment 4\. Source code transits to C16's Azure subscription 5\. No SAP-endorsed architecture in the path 6\. sap\_api\_hub\_search queries api.sap.com programmatically 7\. sap\_call\_rfc is an unrestricted generic proxy

## 4 Gap Analysis

#

Policy Clause

Current C16 State

Gap

Severity

**G1**

§1.1/§1.2 — Published APIs only

29 tools use ADT REST APIs not listed on api.sap.com

Non-published API consumption

HIGH

**G2**

§3 — Anti-circumvention: "custom code or developments"

ICF endpoints wrap non-released classes (81% of underlying objects are Level C). Custom code wrapping non-released APIs = circumvention under §3.

ICF migration changes the URL but not the compliance classification of the underlying objects

HIGH

**G3**

Interpretive §2.2.2 — AI agent sequences

Claude LLM plans, selects, and executes 10-30 SAP API calls per analysis workflow

Matches the pattern: "generative AI systems that plan, select, or execute sequences"

HIGH

**G4**

api.sap.com ToU §3(m) — agent-frameworks

`sap_api_hub_search` makes automated calls to api.sap.com

Direct violation — explicitly named: "agent-frameworks"

HIGH

**G5**

§3 — "intermediary services, proxies, gateways"

C16 SaaS acts as an intermediary between user and SAP. `sap_call_rfc` is a generic proxy for any RFC FM.

Intermediary service + unrestricted proxy

HIGH

**G6**

§2 — Bulk extraction / replication

Zoekt code index extracts source from 100s of programs. SOD analysis reads 23K+ rows.

Systematic extraction pattern for code indexing

MEDIUM

**G7**

Data sovereignty / credential isolation

SAP credentials in Azure Key Vault. Source code transits to C16's cloud.

Customer data and credentials leave customer environment

MEDIUM

**G8**

LLM provider data flow

Source code snippets flow to Anthropic's Claude API for reasoning. Prompts contain SAP code and business context.

Customer IP (source code, business data context) transits to third-party LLM provider

MEDIUM

**G9**

No SAP partnership status

C16 has no formal SAP endorsement, PartnerEdge listing, or ICC certification

Entirely dependent on §1.2 interpretation surviving every future policy revision

STRATEGIC

## 5 Underlying Class Verification — The §3 Reality Check

#### 🔴 Critical Finding

81% of the SAP classes and function modules that C16's ICF endpoints depend on are **NOT released** in SAP's cloudification catalog. Wrapping non-released objects in custom ICF endpoints changes the URL but does NOT change the compliance classification. Under §3, this is arguably circumvention.

Every underlying SAP class/FM was checked against SAP's cloudification catalog on 02 May 2026:

### 5.1 Level A — Released (Safe to Wrap)

Object

Type

Level

C16 Capability

`RFC_SYSTEM_INFO`

FM

Level A — Released

System version check

`DDIF_FIELDINFO_GET`

FM

Level A — Released

Table field lookups

`ENQUEUE_READ`

FM

Level A — Released

Lock status checks

`CL_ABAP_TYPEDESCR`

Class

Level A — Released

Type resolution / RTTI

**Total Level A: 4 objects (15%)**

### 5.2 Level B — Classic API (Stable, Not Released)

Object

Type

Level

C16 Capability

`RFC_READ_TABLE`

FM

Level B — Classic API

ALL table reading (most heavily used FM)

**Total Level B: 1 object (4%)**

### 5.3 Level C — Not Released (Non-Compliant)

Object

Type

C16 Capability

Released Alternative?

`CL_SQL_STATEMENT`

Class

OpenSQL data preview

❌ None

`CL_ABAP_SOURCE_SCAN`

Class

Source code scanning

❌ None

`CL_ADT_TOOLS_CORE_FACTORY`

Class

ADT core factory

❌ None

`READ_REPORT`

FM

Read program source code

❌ None

`INSERT_REPORT`

FM

Write/deploy program source

❌ None

`RS_CDS_VIEW_GET_DDLSOURCE`

FM

Read CDS view source

❌ None

`RPY_PROGRAM_READ`

FM

Read program source (alt)

❌ None

`SYNTAX_CHECK`

FM

Syntax validation

❌ None

`SLIN_PERFORM_CHECK`

FM

Extended syntax check

❌ None

`RS_SHORTDUMP_READ`

FM

Runtime dump retrieval

❌ None

`SNAP_READ_ABAP_CALLSTACK`

FM

Dump call stack

❌ None

`RS_AUNIT_EXECUTE`

FM

Unit test execution

❌ None

`CL_ABAP_UNIT_ASSERT`

Class

Unit test assertions

❌ None

`CL_CI_TEST_ROOT`

Class

ATC / Code Inspector

❌ None

`CL_AUCV_TASK`

Class

ATC task execution

❌ None

`SCWB_API_ACTIVATE_OBJECTS`

FM

Object activation

❌ None

`TR_REQUEST_CREATE`

FM

Transport creation

❌ None

`TRINT_RELEASE_REQUESTS`

FM

Transport release

❌ None

`CL_CTS_REQUEST`

Class

Transport management

❌ None

`SEO_CLASS_GET`

FM

Class metadata read

❌ None

`CL_ABAP_COMPILER`

Class

Cross-reference / deps

❌ None

`XCO_CP_GENERATION`

Class

Code generation (S/4 only)

❌ None

**Total Level C: 22 objects (81%)**

4

Level A — Released (15%)

1

Level B — Classic (4%)

22

Level C — Not Released (81%)

### 5.4 The §3 Circumvention Analysis

For each ICF endpoint, the question is: **does wrapping a non-released class in custom code constitute circumvention?**

ICF Endpoint

Underlying Object

Level

§3 Circumvention?

`/sap/bc/zc16/data_preview`

CL\_SQL\_STATEMENT

C

🔴 Yes — wrapping non-released

`/sap/bc/zc16/runtime_dumps`

RS\_SHORTDUMP\_READ

C

🔴 Yes

`/sap/bc/zc16/unit_test`

RS\_AUNIT\_EXECUTE

C

🔴 Yes

`/sap/bc/zc16/activate`

SCWB\_API\_ACTIVATE\_OBJECTS

C

🔴 Yes

`/sap/bc/zc16/sb_publish`

ADT internal classes

C

🔴 Yes

`/sap/bc/zc16/tr_create`

TR\_REQUEST\_CREATE

C

🔴 Yes

`/sap/bc/zc16/obj_source`

READ\_REPORT

C

🔴 Yes

`/sap/bc/zc16/obj_search`

RFC\_READ\_TABLE (TADIR)

B

🟡 Partial

`/sap/bc/zc16/atc_run`

CL\_CI\_TEST\_ROOT

C

🔴 Yes

`/sap/bc/zc16/goto_def`

CL\_ABAP\_COMPILER

C

🔴 Yes

`/sap/bc/zc16/abapgit`

INSERT\_REPORT, READ\_REPORT

C

🔴 Yes

#### ⚠️ The Systemic Gap

**SAP has not released developer tooling APIs.** There is no released way to read source code, write/deploy source code, activate objects, run syntax checks, run ATC, run unit tests, read runtime dumps, or manage transports. These capabilities only exist through non-released FMs and classes. Eclipse ADT, abapGit, and every ABAP development tool uses the same non-released interfaces because SAP has never published alternatives.

**C16's position:** "C16 uses the same ABAP interfaces that SAP's own Eclipse ADT and the SAP-recognized abapGit project use. Until SAP releases developer tool APIs, any tool that performs development operations — including SAP's own tools — relies on non-released interfaces."

**Mitigation:** File SAP Influence requests for every capability that has no released alternative. Create a documented paper trail. See §14 (Partnership Roadmap).

## 6 Options Considered

Option

Description

AI Touches SAP?

§1.2 Defense

§3 Risk

Customer Effort

C16 Effort

Compliance

**A: Improve in Place**

ICF migration + api.sap.com removal + human approval gate + RFC whitelist + composite call graph documentation

🔴 Yes — AI still calls SAP directly

⭐⭐⭐ ICF endpoints are customer code

🟡 Underlying classes still non-released

🟢 None

Medium

⭐⭐⭐ Defensible

**B: C16 Connector**

Split control/data plane. AI in cloud, Connector in customer env makes SAP calls.

🟢 No — Connector is non-AI

⭐⭐⭐⭐⭐ Customer infra + customer APIs

🟡 Same underlying classes, but caller is customer's own process

🟡 1 VM (2 CPU, 4GB)

High

⭐⭐⭐⭐ Strong

**C: Full C16 in Customer**

Entire C16 platform inside customer's network. LLM via cloud API or local.

🟡 Yes — but from inside customer's own network

⭐⭐⭐⭐⭐ Everything is customer-owned

🟢 Customer's own tool, customer's own code

🔴 VM + GPU (if local LLM)

Very High

⭐⭐⭐⭐⭐ Maximum

**D: Cloud Connector + BTP**

Route through SAP-endorsed architecture.

🟡 AI initiates, but pipe is SAP-endorsed

⭐⭐⭐⭐⭐

🟢 SAP-endorsed triggers policy exception

🟡 BTP subscription

Medium

⭐⭐⭐⭐⭐ Maximum

## 7 Decision

### ✅ Decision: Option A is the Bridge. Option B is the Destination.

Pursue Option A (immediate improvements) and Option B (Connector) in parallel. Offer Option D (Cloud Connector) as an optional enhancement for BTP customers. Reserve Option C (full customer deploy) for regulated / air-gapped requirements.

Option

Role

Timeline

Target Customers

**A: Improve in Place**

Immediate risk reduction

Q2-Q3 2026

All current customers

**B: C16 Connector**

Strategic target architecture

Q3 2026 – Q1 2027

All new enterprise customers from 2027

**D: Cloud Connector**

Optional enhancement

When customer has BTP

RISE / BTP customers

**C: Full Customer Deploy**

Reserved for special cases

On demand

Government, defense, air-gapped

## 8 To-Be Architecture

GitHub / Container Registry ┌──────────────────────────────────────────────────────────────┐ │ c16-platform (private) → CI/CD → Azure │ │ c16-connector (private) → CI/CD → GHCR/ACR │ │ c16-icf-endpoints (customer-visible) → Customer installs │ │ abapGit (public — SAP community) │ └──────────────────────────────────────────────────────────────┘ C16 SaaS (Azure — C16's subscription) ┌──────────────────────────────────────────────────────────────┐ │ │ │ ┌───────────────┐ ┌──────────────────────────────────┐ │ │ │ Claude LLM │ │ UI / Project Management │ │ │ │ (Anthropic) │ │ Customer billing │ │ │ └───────┬───────┘ └──────────────────────────────────┘ │ │ │ │ │ │ Sends: Signed instructions (JWT) │ │ │ Receives: Analysis results (scrubbed) │ │ │ NEVER receives: credentials, bulk source, raw tables │ │ │ │ │ ✅ AI NEVER makes SAP API calls │ │ ✅ SAP credentials NEVER stored here │ └──────────┼───────────────────────────────────────────────────┘ │ │ HTTPS (outbound only, port 443) │ Signed instruction protocol (JWT/HMAC) ▼ Customer Environment (customer's network / VM / K8s) ┌──────────────────────────────────────────────────────────────┐ │ │ │ C16 Connector (Docker container) │ │ ┌────────────────────────────────────────────────────────┐ │ │ │ server.py — instruction receiver + executor │ │ │ │ engines/ — Clean Core, SOD, metrics, perf (Python) │ │ │ │ search/ — Zoekt trigram index (runs locally) │ │ │ │ vault/ — SAP credentials (AES-256, local only) │ │ │ │ audit/ — Append-only log, SHA-256 hash chain │ │ │ └─────────────────────────┬──────────────────────────────┘ │ │ │ LAN / internal network │ │ ▼ │ │ ┌────────────────────────────────────────────────────────┐ │ │ │ Optional: SAP Cloud Connector (Tier 3/4 only) │ │ │ │ URL whitelist: ONLY /sap/bc/zc16/\* + /sap/opu/odata/ │ │ │ └─────────────────────────┬──────────────────────────────┘ │ │ │ │ │ ▼ │ │ Customer SAP System (on-prem / private cloud) │ │ ┌────────────────────────────────────────────────────────┐ │ │ │ /sap/bc/zc16/\* (customer-owned ICF endpoints) │ │ │ │ ZCL\_C16\_REST\_BASE → ZCL\_C16\_AUTH\_CHECK │ │ │ │ → Underlying SAP classes (released where possible) │ │ │ │ → ZC16\_API\_LOG (append-only, SHA-256 hash chain) │ │ │ └────────────────────────────────────────────────────────┘ │ │ │ │ /sap/opu/odata/\* (Published OData services — compliant) │ │ │ └──────────────────────────────────────────────────────────────┘ ▲ IMPROVEMENTS: 1\. AI never touches SAP — Connector is non-AI 2\. SAP credentials never leave customer environment 3\. Source code analyzed locally — results only sent out 4\. Customer-owned ICF endpoints (§1.2 exception) 5\. Full audit trail with tamper-evidence 6\. Optional Cloud Connector for SAP-endorsed path 7\. sap\_call\_rfc replaced with whitelisted FM endpoint 8\. api.sap.com dependency removed entirely

### 8.1 LLM Data Flow Policy

#### 📋 Explicit LLM Data Flow Statement (Gap 2 Resolution)

This section defines **exactly what data flows to the LLM provider (Anthropic Claude)** and what does not.

Data Category

Flows to LLM?

Justification

Control

**Source code snippets** (≤500 lines per call)

🟡 **Yes — selectively**

Required for code analysis, refactoring suggestions, and generation. This is the product.

Connector selects relevant snippets only. Full 9,830-line programs are NOT sent wholesale. Customer approves analysis initiation.

**Analysis results** (scores, findings, metrics)

🟡 **Yes**

LLM formats results into human-readable reports

Structured JSON — no raw SAP data, no credentials

**Table structure metadata** (field names, types)

🟡 **Yes — limited**

Required for code generation (type resolution)

Schema only — no business data values

**Full bulk source code** (entire programs, code index)

🟢 **No — never**

Code indexing runs in Connector locally. Full source stays in customer env.

Zoekt index runs inside Connector

**Business data** (vendor records, POs, invoices, user lists)

🟢 **No — never in bulk**

Business analysis runs in Connector. Summary results only flow to LLM.

Connector scrubs and summarizes

**SAP credentials**

🟢 **No — never**

Credentials exist only in Connector's encrypted vault

Architecture enforced — SaaS has no access path

**Customer PII** (employee names, bank details)

🟢 **No — never**

Connector redacts PII before sending results

Redaction rules enforced in Connector

**Customer-tier LLM options:**

Tier

LLM Option

Data Stays in Customer Env?

AI Quality

Tier 1-2

Anthropic Claude via C16 SaaS

❌ Snippets flow to Anthropic

⭐⭐⭐⭐⭐

Tier 2-3

Customer's own Azure OpenAI instance

🟡 In customer's Azure tenant

⭐⭐⭐⭐⭐

Tier 3-4

SAP AI Core / Generative AI Hub

✅ SAP-managed

⭐⭐⭐⭐

Tier 4

Self-hosted LLM (Llama 3 / Mistral)

✅ 100% local

⭐⭐⭐

## 9 Tool Migration Matrix

### 9.1 ADT-Backed Tools (29 tools — 🔴 Must Migrate)

#

Tool

Function

Priority

Target

1

`sap_get_source`

Read program source

P0

`/sap/bc/zc16/obj_source`

2

`sap_get_class_source`

Read class source

P0

`/sap/bc/zc16/obj_source`

3

`sap_get_ddl_source`

Read CDS source

P0

`/sap/bc/zc16/obj_source`

4

`sap_scan_sources`

Scan for patterns

P0

`/sap/bc/zc16/obj_search`

5

`sap_syntax_check`

Syntax check

P1

`/sap/bc/zc16/syntax_check`

6

`sap_atc_check`

ATC quality check

P1

`/sap/bc/zc16/atc_run`

7

`sap_run_unit_tests`

Unit test execution

P1

`/sap/bc/zc16/unit_test` ✅

8

`sap_validate_cds`

CDS validation

P2

`/sap/bc/zc16/cds_validate`

9

`sap_check_cds_syntax`

CDS syntax

P2

`/sap/bc/zc16/cds_validate`

10

`sap_goto_definition`

Navigate to definition

P2

`/sap/bc/zc16/goto_def`

11

`sap_resolve_dependencies`

Dependency tree

P1

`/sap/bc/zc16/dependencies`

12

`sap_clean_core_check`

Cloud readiness ATC

P1

`/sap/bc/zc16/atc_run`

13

`sap_check_api_release_status`

Release status check

P2

`/sap/bc/zc16/obj_search`

14

`sap_data_preview`

OpenSQL execution

P0

`/sap/bc/zc16/data_preview` ✅

15

`download_abap_object`

Download baseline

P1

`/sap/bc/zc16/obj_source`

16

`sync_abap_git_objects`

Deploy to SAP

P0

`/sap/bc/zc16/abapgit`

17

`sap_publish_service_binding`

Publish OData binding

P1

`/sap/bc/zc16/sb_publish` ✅

18

`sap_batch_deploy_cds`

Deploy CDS views

P1

`/sap/bc/zc16/abapgit`

19

`sap_deploy_ui5_app`

Deploy UI5 to BSP

P2

`/sap/bc/zc16/bsp_deploy`

20

`sap_deploy_optimized_code`

Deploy pushdown

P2

`/sap/bc/zc16/abapgit`

21

`sap_probe_dev_capabilities`

Check dev FMs

P2

`/sap/bc/zc16/sys_info`

22

`sap_check_system_version`

System version

P0

`/sap/bc/zc16/sys_info` ✅

23

`sap_list_transports`

List transports

P1

`/sap/bc/zc16/tr_get` ✅

24

`sap_create_transport`

Create transport

P1

`/sap/bc/zc16/tr_create` ✅

25

`sap_release_transport`

Release transport

**P0** ⚠️

`/sap/bc/zc16/tr_release` + mandatory human approval

26

`sap_validate_deployed_objects`

Verify objects

P2

`/sap/bc/zc16/obj_search`

27

`sap_cleanup_orphans`

Clean orphans

P2

`/sap/bc/zc16/obj_cleanup`

28

`get_runtime_dumps`

Read short dumps

P1

`/sap/bc/zc16/runtime_dumps` ✅

29

`export_abap_sources`

Bulk source export

P0

Move to Connector-local Zoekt (no SAP bulk extraction)

### 9.2 RFC-Backed Tools (17 tools — 🟡 Lower Priority)

#

Tool

Function

Priority

Target

30

`sap_read_table`

Query any table

P1

`/sap/bc/zc16/data_read`

31

`sap_call_rfc`

Call any RFC FM

**P0** ⚠️

**RESTRICT:** `/sap/bc/zc16/rfc_call` with FM whitelist enforcement. See §9.6.

32-46

_(15 additional RFC tools)_

Various metadata reads, SmartForm, DDIC operations

P2

Individual ICF endpoints or consolidated into composites

### 9.3 OData-Backed Tools (6 tools — 🟢 Compliant)

#

Tool

Status

47-52

`sap_list_odata_services`, `sap_get_service_entities`, `sap_read_odata_data`, `sap_api_test`, `sap_check_gateway_access`, `sap_check_bsp_access`

✅ Published OData — no change needed

### 9.4 Composite Business Tools (17 tools — 🟡 Bounded)

See §9.7 for documented call graphs.

### 9.5 api.sap.com Tool (1 tool — 🔴 Remove)

#

Tool

Action

Deadline

70

`sap_api_hub_search`

**REMOVE ENTIRELY.** Also grep full codebase for any incidental api.sap.com URLs (docs links, fallback lookups, skill references).

**This week**

### 9.6 RFC Whitelist Policy (Gap 3 Resolution)

#### 🔴 sap\_call\_rfc Is an Unrestricted Generic Proxy

`sap_call_rfc` accepts **any** function module name and calls it. Wrapped in an ICF endpoint, this is still a generic gateway — exactly what §3 prohibits ("custom code... gateways").

**Resolution:** Replace with `/sap/bc/zc16/rfc_call` that enforces a **hardcoded FM whitelist**. Any FM not on the list returns HTTP 403.

Whitelisted FM

Why C16 Needs It

Release Level

`RFC_SYSTEM_INFO`

System version detection

Level A

`STFC_CONNECTION`

Connectivity test

Level A

`RFC_READ_TABLE`

Table queries

Level B

`DDIF_FIELDINFO_GET`

Field definitions

Level A

`BAPI_USER_GET_DETAIL`

User info for SOD analysis

Level A

`BAPI_CTREQUEST_CREATE`

Transport creation (released BAPI)

Level A

`ENQUEUE_READ`

Lock status

Level A

`RPY_FUNCTIONMODULE_READ`

FM interface metadata

Level B

`SXS_ATTR_READ`

BAdI definitions

Level B

**Enforcement:** The ICF endpoint handler contains a hardcoded ABAP constant table of permitted FM names. Any call with an FM not in the list is rejected with HTTP 403 and logged as a policy violation in `ZC16_API_LOG`.

### 9.7 Composite Tool Call Graphs (Gap 4 Resolution)

#### ⚠️ Composite Tools Are Multi-Step Orchestration

The 17 composite tools execute 4-12 SAP calls each — precisely the "sequences of API calls" pattern. Saying they "inherit compliance" is insufficient. Each composite must document its **bounded, finite call graph** so auditors can verify the scope is known and fixed — categorically different from open-ended AI planning.

Composite Tool

SAP Calls (fixed sequence)

Call Count

Approval Gate

`sap_procurement_cycle_analysis`

RFC\_READ\_TABLE × 5 (EBAN → EKKO → EKBE → RBKP → BSIK)

5

✅ Required

`sap_o2c_process_report`

RFC\_READ\_TABLE × 4 (VBAK → VBFA → LIKP → VBRK)

4

✅ Required

`sap_o2c_audit_report`

RFC\_READ\_TABLE × 6 (VBAK → VBAP → VBFA → VBRK → KNA1 → BSID)

6

✅ Required

`sap_vendor_po_summary`

RFC\_READ\_TABLE × 3 (EKKO → EKPO → LFA1)

3

✅ Required

`sap_material_po_summary`

RFC\_READ\_TABLE × 2 (EKPO → MARA)

2

✅ Required

`sap_po_detail_report`

RFC\_READ\_TABLE × 3 (EKKO → EKPO → LFA1)

3

✅ Required

`sap_open_po_report`

OData GET × 1 (Published API)

1

🟢 Published

`sap_inventory_status`

RFC\_READ\_TABLE × 2 (MARD → MARA)

2

✅ Required

`sap_purchase_reference_data`

RFC\_READ\_TABLE × 4 (T024 → T024E → T001W → LFA1)

4

✅ Required

`sap_discover_programs`

RFC\_READ\_TABLE × 3 (TADIR → TRDIR → TSTC)

3

✅ Required

`sap_analyze_for_migration`

RFC\_READ\_TABLE × 2 + ADT source read × 1

3

✅ Required

`sap_generate_rap_migration`

RFC\_READ\_TABLE × 2 + ADT source read × 1

3

✅ Required

`sap_convert_smartform_to_adobe`

RFC × 2 (SSF\_FB\_DOWNLOAD + conversion)

2

✅ Required

`sap_convert_smartform_to_adobe_full`

RFC × 2 + deploy × 1 + verify × 1

4

✅ Required

`sap_generate_odata_classes`

Deploy × 2 (MPC + DPC classes)

2

✅ Required

`sap_list_programs`

RFC\_READ\_TABLE × 1 (TADIR)

1

🟢 Single call

`sap_test_connection`

RFC × 1 (STFC\_CONNECTION)

1

🟢 Single call

**Key distinction:** Every composite has a **documented, finite, invariant call graph**. `sap_procurement_cycle_analysis` will always call exactly 5 tables in exactly the same order. This is a **known workflow**, not open-ended AI planning. The AI selects WHICH composite to run (with human approval); the composite itself is deterministic.

## 10 C16 Connector Specification

### 10.1 Components

Component

Technology

Purpose

Instruction receiver

Python (Flask/FastAPI)

Receives signed instructions from SaaS, validates JWT

SAP client

Python (requests + SAP RFC libs)

Executes SAP calls (RFC, ICF, OData)

Analysis engines

Python

Clean Core, SOD, code metrics, performance, fit-to-standard

Code search

Zoekt (Go)

Trigram search — builds and queries index locally

Credential vault

AES-256-GCM encrypted file

SAP credentials — never transmitted

Audit log

Append-only file + SHA-256 hash chain

Every SAP call logged with tamper-evidence. Optional SIEM forwarding (Splunk/Sentinel).

Result scrubber

Python

Redacts PII, limits source snippets to ≤500 lines, summarizes business data

### 10.2 Infrastructure Requirements

Resource

Minimum

Recommended

CPU

2 vCPU

4 vCPU

RAM

4 GB

8 GB

Disk

20 GB

50 GB (for Zoekt index)

OS

Ubuntu 22.04 / RHEL 8

Any Docker-capable Linux

Network (to SAP)

LAN or VPN — ports 44300/8000

LAN preferred

Network (to C16 SaaS)

Outbound HTTPS port 443

Outbound only — no inbound required

### 10.3 Deployment Models

Model

Platform

Customer Effort

Best For

Docker on VM

Any Linux VM

1-2 hours

90% of customers

Kubernetes / Helm

K8s / OpenShift

30 min (if K8s exists)

Cloud-native enterprises

Cloud container

Azure ACI / AWS Fargate / GCP Cloud Run

1 hour

Customers on hyperscaler IaaS

SAP BTP Kyma/CF

SAP BTP runtime

2-4 hours

RISE customers (Tier 3/4)

## 11 Source Control, Distribution & Version Control

### 11.1 Repository Architecture

Repository

Visibility

Contents

Distribution

`c16-platform`

Private

SaaS application, tool implementations, UI

CI/CD → Azure Container Apps

`c16-connector`

Private

Dockerfile, server.py, engines, SAP client

CI/CD → Container Registry (GHCR/ACR)

`c16-icf-endpoints`

**Customer-visible**

All ZCL\_C16\_\* ABAP classes, ICF handler code

Customer downloads, reviews, installs

`c16-skills`

Private

48 skill .md files

Bundled with SaaS deployment

#### ✅ ICF Endpoint Source Code Is Customer-Visible

The `c16-icf-endpoints` repository is made available to customers. Customers can audit every line of ABAP code before installing it in their system. This strengthens the §1.2 "customer-developed" argument: the customer reviewed the source, compiled it in their system, transported it through their change management, and operates it under their authorization model.

### 11.2 abapGit Role

abapGit (`github.com/abapGit/abapGit`) is the SAP community's de facto standard for ABAP version control. SAP employees actively contribute to it. SAP's own gCTS (Git-enabled Change and Transport System) is built on the same principles. C16 uses abapGit as the deployment mechanism — a recognized, community-standard approach.

### 11.3 Connector Distribution Lifecycle

1.  C16 develops and tests Connector update
2.  CI/CD publishes new Docker image with semantic version tag (e.g., `c16/connector:1.3.2`)
3.  Release notes published — customer is notified
4.  Customer reviews release notes and changelog
5.  Customer pulls new image: `docker pull c16/connector:1.3.2`
6.  Customer restarts Connector: `c16-connector restart`
7.  Customer verifies: `c16-connector test`

### 11.4 Customer Code Version Control

Approach

How It Works

Recommendation

**No Git** (current)

C16 generates → deploys to SAP → done

Acceptable for dev/sandbox

**Customer's own Git + abapGit**

C16 generates → customer's repo → review → abapGit pulls to SAP

✅ **Recommended** — full audit trail, code review, rollback

**C16-managed Git**

C16 generates → C16 repo per customer → abapGit pulls

Acceptable but weaker "customer-owned" argument

## 12 ICF Endpoint Status

Endpoint

Replaces

Underlying Level

Status

`/sap/bc/zc16/data_preview`

ADT freestyle preview

🔴 C (CL\_SQL\_STATEMENT)

✅ Live

`/sap/bc/zc16/runtime_dumps`

ADT dump reader

🔴 C (RS\_SHORTDUMP\_READ)

✅ Live

`/sap/bc/zc16/unit_test`

ADT unit test runner

🔴 C (RS\_AUNIT\_EXECUTE)

✅ Live

`/sap/bc/zc16/activate`

ADT activation

🔴 C (SCWB\_API\_ACTIVATE\_OBJECTS)

✅ Live

`/sap/bc/zc16/sb_publish`

ADT service binding

🔴 C (ADT internal)

✅ Live

`/sap/bc/zc16/tr_create`

ADT CTS

🔴 C (TR\_REQUEST\_CREATE)

✅ Live

`/sap/bc/zc16/pak_validate`

ADT package

🔴 C (mixed)

✅ Live

`/sap/bc/zc16/obj_source`

ADT source reader

🔴 C (READ\_REPORT)

⚠️ In progress

`/sap/bc/zc16/obj_search`

ADT search

🟡 B (RFC\_READ\_TABLE on TADIR)

⚠️ In progress

`/sap/bc/zc16/rfc_call`

Generic sap\_call\_rfc

Mixed (whitelisted FMs only)

⚠️ In progress

`/sap/bc/zc16/atc_run`

ADT ATC API

🔴 C (CL\_CI\_TEST\_ROOT)

❌ Not started

`/sap/bc/zc16/goto_def`

ADT navigation

🔴 C (CL\_ABAP\_COMPILER)

❌ Not started

`/sap/bc/zc16/abapgit`

ADT zc16 abapgit

🔴 C (INSERT\_REPORT)

❌ Not started (re-root from /sap/bc/adt/)

7

Live

3

In Progress

3

Not Started

## 13 Migration Timeline

Phase

Name

Period

Deliverables

**Phase 0**

Immediate Risk Reduction

**Week 1 (May 2026)**

Remove `sap_api_hub_search` + full codebase grep for api.sap.com URLs. Deploy RFC whitelist. Add mandatory human approval gate for composites + `sap_release_transport`. Initiate SAP PartnerEdge inquiry. Engage legal counsel (SAP-licensing specialists — Bird & Bird, Pinsent Masons, Reed Smith). Run June 2026 ODP-RFC regression test.

**Phase 1**

Complete ICF Migration

**Q2–Q3 2026**

Close remaining 6 ICF endpoints (`obj_source`, `obj_search`, `rfc_call` whitelist, `atc_run`, `goto_def`, `abapgit` re-root). Validate all endpoints side-by-side against ADT. Move endpoints out of $TMP into transported package.

**Phase 2**

Connector MVP

**Q3 2026**

Build Connector Docker image. Implement signed instruction protocol (JWT). Migrate Python engines into Connector. Implement result scrubber + LLM data flow policy. Deploy with 2-3 pilot customers.

**Phase 3**

Connector GA + ADT Deprecation

**Q4 2026 – Q1 2027**

Connector generally available. Deprecate ADT-direct path. Dual-stack period for existing customers (see §16). Compliance dashboard shipped. Connector update mechanism (Docker pull + versioning) operational.

**Phase 4**

Cloud Connector Integration

**Q1 2027**

Optional Cloud Connector path for BTP customers (Tier 3/4). SAP PartnerEdge certification progress review.

**Phase 5**

Continuous Compliance

**Ongoing**

Quarterly cloudification catalog re-verification. SAP Security Patch Day monitoring. SAP Influence requests for missing released APIs. PartnerEdge/ICC certification progress.

#### ⚠️ Timeline Realism Note

The original v1.0 targeted Phase 4 completion by Oct 2026. With a realistic 6-week hiring lag for the platform architect (begin hiring 16 May → effective start ~1 July), Phase 3 and Phase 4 shift to Q4 2026 → Q1 2027. This is reflected above. Pre-staging the architect hire is critical to holding this timeline.

## 14 Customer Tiers

Tier

Name

Customer Profile

Architecture

Compliance

Notes

**Tier 1**

Essential

ECC on-premise

VPN + ICF endpoints (Option A)

⭐⭐⭐ Defensible (§1.2)

⚠️ **Transitional tier.** ECC mainstream maintenance ends 2027 (extended 2030). Customers should plan migration to Tier 2 alongside their S/4HANA cutover.

**Tier 2**

Standard

S/4HANA on-premise

Connector + ICF endpoints (Option B)

⭐⭐⭐⭐ Strong

Target architecture for majority of customers

**Tier 3**

Enterprise

RISE / Private Cloud + BTP

Connector + Cloud Connector + ICF (Option B+D)

⭐⭐⭐⭐⭐ Maximum

SAP-endorsed architecture in path

**Tier 4**

Sovereign

Regulated / Government / Air-gapped

Full C16 in customer network + local LLM (Option C)

⭐⭐⭐⭐⭐ Maximum + data sovereignty

Nothing leaves customer's network

## 15 SAP Partnership Roadmap (Gap 1 Resolution)

#### 🎯 Why This Is the #1 Strategic Priority

Without SAP endorsement, C16 is permanently dependent on the §1.2 interpretation surviving every future policy revision. One SAP policy update could invalidate the entire defense. Partnership converts the biggest existential risk into a competitive moat.

Phase

Action

Timeline

Owner

Outcome

**P1**

Initiate SAP PartnerEdge inquiry (Build or Service track)

May 2026

C16 Leadership

Understanding of certification requirements and timeline

**P2**

File SAP Influence requests for missing released developer tool APIs

June 2026

C16 Architecture

Paper trail showing C16 asked for released APIs. Covers: READ\_REPORT, INSERT\_REPORT, SCWB\_API\_ACTIVATE\_OBJECTS, RS\_AUNIT\_EXECUTE, RS\_SHORTDUMP\_READ, CL\_CI\_TEST\_ROOT, TR\_REQUEST\_CREATE, SYNTAX\_CHECK

**P3**

Submit ICC (Integration & Certification Center) application

Q3 2026

C16 Engineering

Formal certification process begins

**P4**

SAP BTP Marketplace listing exploration

Q4 2026

C16 Product

C16 Connector available via SAP BTP → "SAP-endorsed" by definition

**P5**

SAP co-innovation agreement (if applicable)

2027

C16 Leadership

Formal written authorization for C16's usage pattern

**Estimated timeline to full endorsement:** 12-18 months. Start now.

## 16 Residual Risks (After Full Migration)

Risk

Description

Severity Post-Migration

Mitigation

**R1**

81% of underlying ABAP classes/FMs are non-released. ICF endpoints change the URL but not the classification.

MEDIUM

File SAP Influence requests. Track cloudification catalog quarterly. Position as "same interfaces as Eclipse ADT and abapGit."

**R2**

C16 is still a generative AI system. That fact cannot change.

MEDIUM

Connector architecture means AI never executes SAP calls. Human approval gate. Fixed workflow catalog. SAP partnership pursuit.

**R3**

Source code snippets flow to LLM provider (Anthropic) for analysis and generation.

MEDIUM

Snippets ≤500 lines, redacted of PII. Customer-owned LLM option for Tier 3/4. Explicit data flow policy (§8.1).

**R4**

SAP could change the policy again, narrowing the §1.2 custom ABAP exception.

STRATEGIC

PartnerEdge certification (§15) is the only permanent fix. Monitor SAP's policy update channel. Maintain relationship with SAP account team.

**R5**

SAP could introduce runtime enforcement (analogous to June 2026 ODP-RFC blocker) for non-released ABAP classes.

MEDIUM

Monitor SAP Security Patch Day notes. Build smoke tests that run pre/post SAP system upgrades. ICF endpoints fail gracefully with clear logging.

**R6**

No formal SAP partnership — dependent on §1.2 interpretation

STRATEGIC

Partnership roadmap (§15). Begin immediately.

## 17 Existing-Customer Cutover Playbook (Gap 5 Resolution)

### 17.1 Communication Timeline

When

What

Channel

**T-60 days**

Announce Connector availability and migration timeline

Email + customer portal

**T-30 days**

Provide Connector installation guide and infrastructure requirements

Documentation + support call

**T-14 days**

Schedule installation assistance (C16 services or customer self-install)

Support call

**T-0**

Connector installed, dual-stack begins

Technical handover

**T+30 days**

Dual-stack validation — both paths running, results compared

Automated comparison

**T+60 days**

Legacy path disabled, Connector-only

Customer confirmation required

### 17.2 Who Installs the Connector?

Customer Tier

Installation By

Support Level

Tier 1-2

Customer ops (with C16 documentation)

C16 provides Docker image + install script + troubleshooting guide

Tier 2-3

C16 services team (remote assisted)

C16 engineers on call during installation

Tier 3-4

C16 services team (on-site or paired)

Dedicated installation engagement

### 17.3 Dual-Stack Period

During cutover, both the legacy direct path and the Connector path run in parallel for **30-60 days**. C16 automatically compares results from both paths to validate behavioral equivalence.

Aspect

Dual-Stack Behavior

Default path

Connector (new) — all new requests route through Connector

Fallback

Legacy direct — if Connector returns error, automatic fallback to direct path

Comparison

Both paths run for a sample of requests — results compared for divergence

Duration

30 days minimum, 60 days recommended

Success criteria

<1% result divergence across all capability categories

Rollback

Feature flag: `c16_connector_enabled=false` → instant revert to legacy

### 17.4 In-Flight Projects

Active customer projects (analysis in progress, code deployment pending) are **not affected** during cutover. The Connector handles the same capabilities as the direct path. In-flight work continues seamlessly.

### 17.5 Pricing Impact

The Connector requires customer infrastructure (1 VM). C16's pricing model does not change — the Connector is included in the existing subscription. Customers absorb the VM cost (~$50-100/month for a 2 vCPU VM). For Tier 3/4 customers requiring BTP/Cloud Connector, the BTP subscription cost is the customer's existing SAP commitment.

## 18 Immediate Actions (Phase 0)

#

Action

Owner

Deadline

Verification

**IA-1**

Remove `sap_api_hub_search` from C16 platform. **Also:** grep entire codebase (skills, prompts, fallbacks, documentation) for any api.sap.com URLs. Remove or replace all references.

C16 Engineering

**09 May 2026**

`grep -r "api.sap.com" .` returns zero results

**IA-2**

Deploy mandatory human approval gate before all composite tool execution and before `sap_release_transport`

C16 Engineering

**09 May 2026**

Every composite shows plan + waits for "proceed" before execution

**IA-3**

Deploy RFC FM whitelist in `/sap/bc/zc16/rfc_call` endpoint. Reject any FM not on the 9-FM whitelist.

C16 ABAP Team

**16 May 2026**

Call with non-whitelisted FM returns HTTP 403

**IA-4**

Engage SAP-licensing-experienced legal counsel. **Specific firms:** Bird & Bird or Pinsent Masons (EU/UK), Reed Smith or Greenberg Traurig (US). These firms handled the SAP indirect access cases and understand the licensing nuances.

C16 Leadership

**16 May 2026**

Engagement letter signed

**IA-5**

Begin hiring platform architect (SAP integration + cloud-native). Pre-stage: post job listing immediately, don't wait for Phase 1 start.

C16 Leadership

**16 May 2026**

Job listing live, first interviews scheduled

**IA-6**

Set up `c16-icf-endpoints` repository (customer-visible). Package existing 7 live ICF endpoints with documentation.

C16 Engineering

**23 May 2026**

Repository accessible, all 7 live endpoints documented

**IA-7**

Initiate SAP PartnerEdge inquiry (Build or Service track). Establish path toward ICC certification.

C16 Leadership

**30 May 2026**

Initial contact made, requirements document received from SAP

**IA-8**

Run June 2026 ODP-RFC regression test. Test C16 against an SAP system patched with June 2026 SP to confirm no dependencies break.

C16 QA

**June 2026 Patch Day + 5 days**

All 102 tools pass regression suite on patched system

**IA-9**

Add audit log tamper-evidence: SHA-256 hash chain on `ZC16_API_LOG`. Each entry includes hash of previous entry.

C16 ABAP Team

**23 May 2026**

Hash chain verified across 100+ log entries

## 19 References & Verification Sources

### 19.1 Primary SAP Sources

Reference

URL

Verified

SAP API Policy (canonical PDF)

[help.sap.com/.../API\_Policy\_latest.pdf](https://help.sap.com/doc/sap-api-policy/latest/en-US/API_Policy_latest.pdf)

✅ 2026-05-01

SAP API Policy (landing page)

[help.sap.com/.../sap-api-policy](https://help.sap.com/docs/business-accelerator-hub/sap-business-accelerator-hub/sap-api-policy)

✅ 2026-05-01

SAP Business Accelerator Hub

[api.sap.com](https://api.sap.com/)

✅ Live

SAP Business Accelerator Hub — Terms of Use

Footer of api.sap.com → "Terms of Use"

✅ Last updated Oct 8, 2025 — §3(m) names "MCP, agent-frameworks"

SAP Note 3255746 — ODP-RFC ban

[me.sap.com/notes/3255746](https://me.sap.com/notes/3255746)

✅ 2026-05-01

SAP Note 3439624 — ODP-RFC self-assessment

[me.sap.com/notes/3439624](https://me.sap.com/notes/3439624)

✅ 2026-05-01

### 19.2 Industry Sources

Reference

URL

Notes

SAPinsider — "SAP's New API Policy Redefines Access in the AI Era"

[sapinsider.org](https://sapinsider.org/blogs/sap-api-policy-update-developers-partners/)

Adam Pitman, 2026-04-28. Quotes Klein, Holland, Khan.

SAP Q1 2026 Earnings Call

SAP Investor Relations

Klein on "data vs. IP" distinction

DSAG Position on API Policy

DSAG press releases

Critical of API Hub as binding contract

### 19.3 C16 Internal Sources

Reference

Purpose

C16 Internal Architecture Reference

Current-state architecture documentation (93+ tools, connectivity, security)

C16 ICF API Blueprint skill

Pattern for replacing ADT with custom ICF endpoints

C16 ICF API Security skill

5-layer security model for ICF endpoints

C16 Clean Core Check skill

Cloudification catalog verification methodology

### 19.4 Legal Counsel (Recommended)

Region

Firm

Relevant Experience

EU / UK

Bird & Bird, Pinsent Masons, Hogan Lovells

SAP indirect access cases, software licensing disputes

US

Reed Smith, Greenberg Traurig

Enterprise software licensing, API/data access disputes

### 19.5 Honesty Note on Citations

The official SAP API Policy PDF is a deliberately compact document. It does **not** contain explicit, named anti-AI-agent language. References to "Section 2.2.2" in third-party reporting could not be verified against the published policy text — the policy uses a flat §1–§4 structure. The AI-agent implications reported by industry analysts are **interpretive consequences** of three provisions read together (§1.2 non-published prohibition, §2 bulk extraction controls, §3 anti-circumvention), reinforced by SAP leadership statements.

This document treats these implications as **interpretive but precautionary**. Where a claim is verbatim from the policy, it is quoted. Where a claim is inferred, it is marked as such. This document does not overclaim what the policy says.

**Underlying class verification:** All 27 objects in §5 were checked against SAP's cloudification catalog via the connected SAP system on 02 May 2026 using `clean_core_lookup`. Results are reproducible.

**Document:** ADR-2026-001 v1.2 — C16 Migration Decision Record

**Author:** C16 Platform Team | **Reviewer:** External Architecture Review

**Status:** APPROVED — Execute | **Next Review:** 01 August 2026

**Classification:** Internal — C16 Leadership + Architecture Team

Generated by C16 | 02 May 2026 | All policy citations verified against published sources