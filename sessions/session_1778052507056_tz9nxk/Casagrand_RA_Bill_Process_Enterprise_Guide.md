  Casagrand — Running Account Bill (RA Bill) Process | Enterprise Guide mermaid.initialize({startOnLoad:true, theme:'base', themeVariables:{primaryColor:'#6B3FA0',primaryTextColor:'#fff',primaryBorderColor:'#B5567A',lineColor:'#8B5CF6',secondaryColor:'#F3E8FF',tertiaryColor:'#FDF2F8'}}); @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800;900&family=JetBrains+Mono:wght@400;500;600&display=swap'); :root { --primary: #6B3FA0; --primary-light: #8B5CF6; --accent: #B5567A; --accent-gold: #C97B3A; --dark: #1a1a2e; --text: #2c3e50; --text-light: #64748b; --bg: #f0f2f5; --card-bg: #ffffff; --border: #e2e8f0; --success: #059669; --warning: #d97706; --danger: #dc2626; --info: #2563eb; } \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; color: var(--text); background: var(--bg); line-height: 1.7; font-size: 15px; -webkit-font-smoothing: antialiased; } .doc-wrapper { max-width: 1200px; margin: 0 auto; background: var(--card-bg); box-shadow: 0 4px 60px rgba(107,63,160,0.08); border-radius: 0; } /\* ═══════════ HEADER BANNER ═══════════ \*/ .header-banner { background: linear-gradient(135deg, #1a1a2e 0%, #16213e 20%, #6B3FA0 50%, #B5567A 80%, #C97B3A 100%); padding: 60px 60px 50px; color: #fff; position: relative; overflow: hidden; } .header-banner::before { content: ''; position: absolute; top: -50%; right: -20%; width: 600px; height: 600px; background: radial-gradient(circle, rgba(255,255,255,0.04) 0%, transparent 70%); border-radius: 50%; } .header-banner::after { content: ''; position: absolute; bottom: -30%; left: -10%; width: 400px; height: 400px; background: radial-gradient(circle, rgba(181,86,122,0.15) 0%, transparent 70%); border-radius: 50%; } .header-top { display: flex; align-items: center; gap: 16px; margin-bottom: 30px; position: relative; z-index: 2; } .header-top img { height: 50px; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.3); } .header-badge { background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.2); padding: 6px 16px; border-radius: 20px; font-size: 0.78em; font-weight: 600; letter-spacing: 0.5px; } .header-title { position: relative; z-index: 2; } .header-title h1 { font-size: 2.6em; font-weight: 900; letter-spacing: -1px; line-height: 1.15; margin-bottom: 12px; text-shadow: 0 2px 20px rgba(0,0,0,0.2); } .header-title .subtitle { font-size: 1.15em; font-weight: 400; opacity: 0.9; max-width: 750px; line-height: 1.6; } .header-meta { display: flex; gap: 28px; margin-top: 28px; position: relative; z-index: 2; flex-wrap: wrap; } .header-meta-item { display: flex; flex-direction: column; gap: 4px; } .header-meta-item .label { font-size: 0.7em; text-transform: uppercase; letter-spacing: 1.2px; opacity: 0.65; font-weight: 600; } .header-meta-item .value { font-size: 0.95em; font-weight: 600; } /\* ═══════════ CONTENT BODY ═══════════ \*/ .content-body { padding: 50px 60px 60px; } /\* ═══════════ TABLE OF CONTENTS ═══════════ \*/ .toc { background: linear-gradient(135deg, #f8f6ff 0%, #fdf2f8 100%); border: 1px solid #e9d5ff; border-radius: 16px; padding: 36px 40px; margin-bottom: 50px; } .toc h2 { font-size: 1.3em; color: var(--primary); margin-bottom: 20px; font-weight: 800; display: flex; align-items: center; gap: 10px; } .toc-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px 40px; } .toc a { display: flex; align-items: center; gap: 10px; text-decoration: none; color: var(--text); padding: 8px 12px; border-radius: 8px; font-size: 0.92em; font-weight: 500; transition: all 0.2s; } .toc a:hover { background: rgba(107,63,160,0.08); color: var(--primary); transform: translateX(4px); } .toc a .num { background: var(--primary); color: #fff; width: 26px; height: 26px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 0.75em; font-weight: 700; flex-shrink: 0; } /\* ═══════════ SECTIONS ═══════════ \*/ .section { margin-bottom: 55px; scroll-margin-top: 20px; } .section-header { display: flex; align-items: center; gap: 14px; margin-bottom: 24px; padding-bottom: 14px; border-bottom: 3px solid; border-image: linear-gradient(90deg, var(--primary), var(--accent), transparent) 1; } .section-num { background: linear-gradient(135deg, var(--primary), var(--accent)); color: #fff; width: 42px; height: 42px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 1.1em; font-weight: 800; flex-shrink: 0; box-shadow: 0 4px 12px rgba(107,63,160,0.25); } .section-header h2 { font-size: 1.55em; font-weight: 800; color: var(--dark); line-height: 1.3; } h3 { font-size: 1.2em; font-weight: 700; color: var(--primary); margin: 32px 0 16px; padding-left: 14px; border-left: 4px solid var(--accent); } h4 { font-size: 1.05em; font-weight: 700; color: var(--text); margin: 24px 0 12px; } p { margin-bottom: 14px; line-height: 1.8; color: var(--text); } /\* ═══════════ KPI CARDS ═══════════ \*/ .kpi-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 18px; margin: 24px 0 30px; } .kpi-card { background: linear-gradient(135deg, #f8f6ff 0%, #fff 100%); border: 1px solid #e9d5ff; border-radius: 14px; padding: 22px 24px; text-align: center; transition: transform 0.2s, box-shadow 0.2s; } .kpi-card:hover { transform: translateY(-3px); box-shadow: 0 8px 25px rgba(107,63,160,0.12); } .kpi-card .kpi-value { font-size: 2em; font-weight: 900; background: linear-gradient(135deg, var(--primary), var(--accent)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; line-height: 1.2; margin-bottom: 6px; } .kpi-card .kpi-label { font-size: 0.82em; color: var(--text-light); font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; } .kpi-card .kpi-sub { font-size: 0.78em; color: var(--text-light); margin-top: 4px; } .kpi-card.success { border-color: #bbf7d0; background: linear-gradient(135deg, #f0fdf4 0%, #fff 100%); } .kpi-card.success .kpi-value { background: linear-gradient(135deg, #059669, #10b981); -webkit-background-clip: text; background-clip: text; } .kpi-card.warning { border-color: #fde68a; background: linear-gradient(135deg, #fffbeb 0%, #fff 100%); } .kpi-card.warning .kpi-value { background: linear-gradient(135deg, #d97706, #f59e0b); -webkit-background-clip: text; background-clip: text; } .kpi-card.danger { border-color: #fecaca; background: linear-gradient(135deg, #fef2f2 0%, #fff 100%); } .kpi-card.danger .kpi-value { background: linear-gradient(135deg, #dc2626, #ef4444); -webkit-background-clip: text; background-clip: text; } /\* ═══════════ TABLES ═══════════ \*/ table { width: 100%; border-collapse: separate; border-spacing: 0; margin: 18px 0 24px; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 12px rgba(0,0,0,0.04); border: 1px solid var(--border); font-size: 0.9em; } thead th { background: linear-gradient(135deg, var(--primary), #7c4dba); color: #fff; padding: 13px 16px; font-weight: 700; text-align: left; font-size: 0.88em; letter-spacing: 0.3px; text-transform: uppercase; white-space: nowrap; } tbody td { padding: 11px 16px; border-bottom: 1px solid #f1f5f9; vertical-align: top; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } tbody tr:last-child td { border-bottom: none; } /\* ═══════════ CODE / PRE BLOCKS ═══════════ \*/ pre { background: #1e1e2e; color: #cdd6f4; padding: 22px 26px; border-radius: 12px; overflow-x: auto; font-family: 'JetBrains Mono', monospace; font-size: 0.85em; line-height: 1.65; margin: 16px 0 24px; border-left: 4px solid var(--primary); box-shadow: inset 0 2px 8px rgba(0,0,0,0.15); } code { font-family: 'JetBrains Mono', monospace; font-size: 0.9em; } p code, li code, td code { background: #f1f0fb; color: var(--primary); padding: 2px 7px; border-radius: 5px; font-size: 0.88em; font-weight: 500; } /\* ═══════════ CALLOUT BOXES ═══════════ \*/ .callout { border-radius: 12px; padding: 22px 26px; margin: 20px 0 24px; display: flex; gap: 14px; align-items: flex-start; } .callout .icon { font-size: 1.5em; flex-shrink: 0; margin-top: 2px; } .callout .content { flex: 1; } .callout .content strong { display: block; margin-bottom: 4px; } .callout.info { background: #eff6ff; border: 1px solid #bfdbfe; } .callout.success { background: #f0fdf4; border: 1px solid #bbf7d0; } .callout.warning { background: #fffbeb; border: 1px solid #fde68a; } .callout.danger { background: #fef2f2; border: 1px solid #fecaca; } .callout.purple { background: #f8f6ff; border: 1px solid #e9d5ff; } /\* ═══════════ EQUATION BOX ═══════════ \*/ .equation-box { background: linear-gradient(135deg, #1a1a2e, #16213e); color: #e2e8f0; border-radius: 14px; padding: 30px 36px; margin: 24px 0; font-family: 'JetBrains Mono', monospace; font-size: 0.92em; line-height: 1.8; box-shadow: 0 8px 30px rgba(26,26,46,0.3); } .equation-box .eq-title { color: var(--accent); font-weight: 700; font-size: 1.05em; margin-bottom: 14px; text-transform: uppercase; letter-spacing: 1px; } .equation-box .eq-line { padding: 2px 0; } .equation-box .eq-result { border-top: 2px dashed rgba(255,255,255,0.15); margin-top: 10px; padding-top: 10px; color: #a78bfa; font-weight: 700; } .equation-box .eq-highlight { color: #fbbf24; font-weight: 600; } /\* ═══════════ PROCESS STEP CARDS ═══════════ \*/ .process-steps { display: grid; gap: 18px; margin: 24px 0; } .step-card { display: flex; gap: 18px; align-items: flex-start; background: #fff; border: 1px solid var(--border); border-radius: 14px; padding: 22px 26px; transition: all 0.2s; } .step-card:hover { border-color: var(--primary-light); box-shadow: 0 4px 20px rgba(107,63,160,0.08); } .step-num { background: linear-gradient(135deg, var(--primary), var(--accent)); color: #fff; width: 44px; height: 44px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.15em; font-weight: 800; flex-shrink: 0; box-shadow: 0 4px 12px rgba(107,63,160,0.3); } .step-content h4 { font-size: 1.05em; margin: 0 0 6px; color: var(--dark); } .step-content p { font-size: 0.92em; color: var(--text-light); margin: 0; } /\* ═══════════ BADGE / TAG ═══════════ \*/ .badge { display: inline-block; padding: 3px 12px; border-radius: 20px; font-size: 0.78em; font-weight: 700; letter-spacing: 0.3px; } .badge-green { background: #dcfce7; color: #166534; } .badge-yellow { background: #fef9c3; color: #854d0e; } .badge-orange { background: #ffedd5; color: #9a3412; } .badge-red { background: #fee2e2; color: #991b1b; } .badge-purple { background: #f3e8ff; color: #6b21a8; } .badge-blue { background: #dbeafe; color: #1e40af; } /\* ═══════════ MERMAID ═══════════ \*/ .mermaid-wrapper { background: #fafafa; border: 1px solid var(--border); border-radius: 14px; padding: 30px 20px; margin: 20px 0 28px; overflow-x: auto; text-align: center; } /\* ═══════════ DETAILS / COLLAPSIBLE ═══════════ \*/ details { margin: 14px 0; border: 1px solid var(--border); border-radius: 10px; overflow: hidden; } details summary { padding: 14px 20px; background: #f8fafc; cursor: pointer; font-weight: 600; color: var(--primary); font-size: 0.95em; display: flex; align-items: center; gap: 8px; user-select: none; } details summary:hover { background: #f0f0fa; } details\[open\] summary { border-bottom: 1px solid var(--border); } details .detail-content { padding: 18px 22px; } /\* ═══════════ TWO COLUMN LAYOUT ═══════════ \*/ .two-col { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin: 20px 0; } .col-card { background: #f8f6ff; border: 1px solid #e9d5ff; border-radius: 12px; padding: 24px; } .col-card h4 { margin-top: 0; color: var(--primary); } /\* ═══════════ TRACKER TABLE ═══════════ \*/ .tracker-table { font-size: 0.85em; } .tracker-table th { font-size: 0.8em; padding: 10px 12px; } .tracker-table td { padding: 9px 12px; } .tracker-table .healthy { background: #f0fdf4; color: #059669; font-weight: 700; } .tracker-table .caution { background: #fffbeb; color: #d97706; font-weight: 700; } .tracker-table .alert { background: #fef2f2; color: #dc2626; font-weight: 700; } .progress-bar { height: 8px; background: #e2e8f0; border-radius: 4px; overflow: hidden; min-width: 100px; } .progress-bar .fill { height: 100%; border-radius: 4px; transition: width 0.3s; } .progress-bar .fill.purple { background: linear-gradient(90deg, var(--primary), var(--accent)); } .progress-bar .fill.green { background: linear-gradient(90deg, #059669, #34d399); } /\* ═══════════ TIMELINE ═══════════ \*/ .timeline { position: relative; padding-left: 40px; margin: 24px 0; } .timeline::before { content: ''; position: absolute; left: 18px; top: 0; bottom: 0; width: 3px; background: linear-gradient(180deg, var(--primary), var(--accent), #e2e8f0); border-radius: 2px; } .timeline-item { position: relative; margin-bottom: 24px; padding: 16px 20px; background: #fff; border: 1px solid var(--border); border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.03); } .timeline-item::before { content: ''; position: absolute; left: -30px; top: 20px; width: 14px; height: 14px; background: var(--primary); border-radius: 50%; border: 3px solid #fff; box-shadow: 0 0 0 2px var(--primary); } .timeline-item .tl-title { font-weight: 700; color: var(--dark); margin-bottom: 4px; } .timeline-item .tl-date { font-size: 0.8em; color: var(--text-light); font-weight: 600; } /\* ═══════════ FOOTER ═══════════ \*/ .doc-footer { background: linear-gradient(135deg, #1a1a2e, #16213e); color: rgba(255,255,255,0.7); padding: 40px 60px; font-size: 0.85em; } .doc-footer .footer-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 30px; margin-bottom: 24px; } .doc-footer h4 { color: #a78bfa; font-size: 0.9em; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 12px; } .doc-footer .divider { border-top: 1px solid rgba(255,255,255,0.1); padding-top: 20px; text-align: center; font-size: 0.82em; } /\* ═══════════ LISTS ═══════════ \*/ ul, ol { margin: 10px 0 18px 24px; line-height: 1.9; } li { margin-bottom: 4px; } li::marker { color: var(--primary); } /\* ═══════════ HIGHLIGHT BOX ═══════════ \*/ .highlight-box { background: linear-gradient(135deg, var(--primary), var(--accent)); color: #fff; border-radius: 14px; padding: 28px 32px; margin: 24px 0; box-shadow: 0 8px 30px rgba(107,63,160,0.25); } .highlight-box h4 { color: #fff; margin-top: 0; font-size: 1.15em; } .highlight-box p { color: rgba(255,255,255,0.9); } /\* ═══════════ PRINT ═══════════ \*/ @media print { .doc-wrapper { box-shadow: none; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .section { page-break-inside: avoid; } .mermaid-wrapper { page-break-inside: avoid; } table { page-break-inside: avoid; } } /\* ═══════════ RESPONSIVE ═══════════ \*/ @media (max-width: 768px) { .header-banner { padding: 36px 24px; } .header-title h1 { font-size: 1.8em; } .content-body { padding: 30px 24px; } .toc-grid { grid-template-columns: 1fr; } .kpi-row { grid-template-columns: 1fr 1fr; } .two-col { grid-template-columns: 1fr; } .doc-footer .footer-grid { grid-template-columns: 1fr; } .header-meta { gap: 16px; } } :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,) 📋 ENTERPRISE PROCESS GUIDE v2.0 — May 2026 S/4HANA 758 Reference

# Running Account Bill (RA Bill)  
Contractor Payment Process

End-to-end process documentation for contractor billing against work orders — covering services, materials, project execution, compliance, advance adjustment, retention management, escalation/de-escalation, and AP reporting.

Organization Casagrand Group

Domain Real Estate — Construction

SAP Modules MM-SRV · PS · FI-AP · RE-FX

Backend Reference S/4HANA 758 (HANA DB)

Classification Confidential — Internal

## 📑 Table of Contents

[1 Executive Summary & Process Overview](#s1) [2 Master Data Foundation](#s2) [3 Work Order & Bill of Quantities](#s3) [4 Physical Measurement & SES Creation](#s4) [5 Compliance & Supporting Documents](#s5) [6 Advance Management & Recovery](#s6) [7 Retention Money Management](#s7) [8 Escalation & De-escalation](#s8) [9 Approval Workflow & Invoice Verification](#s9) [10 AP Department — Payment Processing](#s10) [11 % Work Completion vs. % Amount Disbursed](#s11) [12 SAP Transaction Code Reference](#s12) [13 Risk, Controls & Audit Framework](#s13) [14 End-to-End Process Flow](#s14) [15 Implementation Roadmap](#s15) [16 Appendices & Data Dictionary](#s16)

1

## Executive Summary & Process Overview

A **Running Account Bill (RA Bill)** is the periodic billing mechanism through which contractors claim payment for work completed to date against a Work Order placed by Casagrand. Unlike a final settlement bill, an RA Bill is submitted at regular intervals — typically monthly or at milestone triggers — and represents the **cumulative value of work done**, adjusted for advances recovered, retention withheld, statutory deductions, and price escalation/de-escalation.

This document provides the definitive enterprise-grade process guide for Casagrand's Accounts Payable, Project Management, and Site Operations teams. It spans the complete lifecycle from work order issuance through physical measurement, compliance verification, invoice processing, and the critical management reporting of **% Work Completion vs. % Amount Disbursed**.

₹15 Cr

Illustrative Contract Value

Per Work Order (Civil Package)

12–15

RA Bills per Contract

Monthly billing cycle

5–10%

Retention Rate

Released after DLP

10%

Advance Recovery

Per RA Bill (proportional)

### The Core RA Bill Equation

Every RA Bill is governed by a single financial equation that determines the net amount payable to the contractor for the current billing period. This equation is the foundation of all subsequent reporting and controls.

RA Bill — Net Payable Calculation

Gross Value of Work Done (Current Measurement Period)

\+ Value of Materials at Site (if contractor-supplied & measured)

− Previous RA Bill Certified Amounts (Cumulative to date)

− Advance Recovery (proportional to gross bill value)

− Retention Money (% withheld per contract terms)

± Escalation / De-escalation Adjustment (index-linked)

− TDS Deduction (Section 194C — 1% individual / 2% company)

− Material Recovery (Casagrand-supplied materials consumed)

− Penalties / Liquidated Damages (if applicable)

\= NET AMOUNT PAYABLE THIS PERIOD

### Process Scope & Boundaries

Dimension

In Scope

Out of Scope

**Billing Type**

Interim RA Bills, Final Bills, Supplementary Bills

Tender/Pre-award estimates

**Contractor Type**

Civil, MEP, Finishing, Landscaping, External Development

Design consultants (fee-based billing)

**SAP Modules**

MM-SRV (Service Procurement), PS (Project System), FI-AP (Accounts Payable)

SD, PP, PM

**Documents**

PO, SES, Invoice, Payment, Advance, Retention, Escalation DN/CN

Non-SAP billing systems

**Compliance**

GST, TDS, PF, ESI, Labour License, Insurance, Safety

Environmental Impact Assessment (EIA)

**Reporting**

% Work vs. % Disbursed, Retention tracking, Advance balance

Financial consolidation/statutory audit

2

## Master Data Foundation

The integrity of the RA Bill process depends entirely on the completeness and accuracy of the underlying master data. Three master data domains form the foundation: **Project Structure** (where does the cost go), **Contractor Master** (who gets paid), and **Service Master** (what work is priced). Each domain is mapped to specific SAP tables within the S/4HANA HANA backend.

### 2.1 Project Structure (PS Module)

Casagrand's real estate projects are modeled using SAP Project System with a hierarchical Work Breakdown Structure (WBS). Each WBS element becomes a cost collector that receives actuals when RA Bill invoices are posted. The table `PROJ` holds project definitions, while `PRPS` contains WBS elements with their cost assignments.

SAP Element

Table

Key Fields

Casagrand Context

Project Definition

`PROJ`

`PSPID`, `POST1`, `WERKS`, `VBUKR`

Each township/project (e.g., "Casagrand Zenith Phase 2")

WBS Elements

`PRPS`

`POSID`, `POST1`, `PSPHI`, `STUFE`

Work breakdown: Foundation → Structure → MEP → Finishing

Networks

`AFKO`

`AUFNR`, `GSTRP`, `GLTRP`

Scheduled activity networks with dates

Activities

`AFVC`

`VORNR`, `LTXA1`, `ARBPL`

Individual activities (Concreting Floor 8, Plumbing Block B)

#### Illustrative WBS Hierarchy — Casagrand Township Project

graph TD A\["**CG-ZEN-P2**  
Casagrand Zenith Phase 2  
_₹150 Cr Budget_"\] --> B\["**CG-ZEN-P2.01**  
Civil Works  
_₹75 Cr_"\] A --> C\["**CG-ZEN-P2.02**  
MEP Works  
_₹35 Cr_"\] A --> D\["**CG-ZEN-P2.03**  
Finishing  
_₹25 Cr_"\] A --> E\["**CG-ZEN-P2.04**  
External Dev  
_₹15 Cr_"\] B --> B1\["**.01.01**  
Foundation"\] B --> B2\["**.01.02**  
Superstructure"\] B --> B3\["**.01.03**  
Waterproofing"\] C --> C1\["**.02.01**  
Plumbing"\] C --> C2\["**.02.02**  
Electrical"\] C --> C3\["**.02.03**  
HVAC & Fire"\] D --> D1\["**.03.01**  
Plastering"\] D --> D2\["**.03.02**  
Tiling"\] D --> D3\["**.03.03**  
Joinery"\] style A fill:#6B3FA0,stroke:#6B3FA0,color:#fff style B fill:#7c4dba,stroke:#7c4dba,color:#fff style C fill:#8B5CF6,stroke:#8B5CF6,color:#fff style D fill:#B5567A,stroke:#B5567A,color:#fff style E fill:#C97B3A,stroke:#C97B3A,color:#fff

### 2.2 Contractor (Vendor) Master

Each contractor is maintained as a vendor in SAP with general data (name, address, GST), company code data (payment terms, retention terms, payment methods), and bank details for electronic fund transfer. The critical fields for RA Bill processing span across `LFA1`, `LFB1`, and `LFBK`.

Data Category

Table

Key Field

Purpose in RA Bill

Validation Rule

Vendor ID

`LFA1`

`LIFNR`

Unique contractor identification

Mandatory — auto-generated

Vendor Name

`LFA1`

`NAME1`

Contractor legal name

Must match PAN/GST registration

GST Registration

`LFA1`

`STCEG`

Input tax credit verification

15-digit GSTIN format

PAN Number

`LFA1`

`J_1IPANNO`

TDS deduction (Sec 194C)

Mandatory for payments > ₹30,000

Payment Terms

`LFB1`

`ZTERM`

Due date calculation after certification

e.g., NET30 = 30 days from SES release

Payment Method

`LFB1`

`ZWELS`

NEFT/RTGS/Cheque routing

Mandatory for F110 payment runs

Payment Block

`LFB1`

`ZAHLS`

Hold payments if compliance pending

Set to 'A' if docs missing

Retention Terms

`EKPO`

`RETPC`

% retention per invoice

5% or 10% as per contract

Bank Details

`LFBK`

`BANKL`, `BANKN`

EFT payment disbursement

IFSC code mandatory

Vendor Block

`LFA1`

`SPERR`

Central posting/purchasing block

Must be blank for active contractors

⚠️

**Compliance Gate** Before any RA Bill is processed, the AP team must verify that `LFB1-ZAHLS` (payment block) is blank. If statutory compliance documents (PF challan, ESI, labour license) are missing, the payment block is set to **'A'**, preventing the F110 payment run from picking up the vendor's invoices.

### 2.3 Service Master — Activity Types

The Service Master in SAP (table `ASMD`) defines standard service activities with predefined units of measurement and base rates. Casagrand maintains a service catalog covering all construction activities:

Service #

Description

UOM

Base Rate (₹)

Category

5001001

Earthwork excavation in all types of soil

CUM

180.00

Civil — Foundation

5001002

PCC M10 grade (plain cement concrete)

CUM

4,500.00

Civil — Foundation

5001003

RCC M25 grade (reinforced concrete)

CUM

6,200.00

Civil — Structural

5001004

Reinforcement steel (TMT Fe500D)

MT

62,000.00

Civil — Structural

5001005

Formwork — all types (plywood/steel)

SQM

350.00

Civil — Structural

5002001

Brickwork 230mm (1st class bricks)

SQM

650.00

Masonry

5002002

Internal plastering 12mm (CM 1:4)

SQM

180.00

Finishing

5002003

External plastering 20mm (CM 1:4)

SQM

280.00

Finishing

5003001

Plumbing point — complete (supply & fix)

Point

2,800.00

MEP — Plumbing

5003002

Electrical point — complete (supply & fix)

Point

1,200.00

MEP — Electrical

5004001

Floor tiling (vitrified 600×600)

SQM

450.00

Finishing — Tiling

5004002

Wall tiling (ceramic 300×450)

SQM

380.00

Finishing — Tiling

5005001

Waterproofing — terrace (APP membrane)

SQM

520.00

Protection

3

## Work Order & Bill of Quantities (BOQ)

The Work Order is created as a **Service Purchase Order** in SAP (transaction `ME21N`). It binds the contractor to the project, defines the scope via the Bill of Quantities (BOQ), establishes retention terms, and creates the account assignment to the WBS element for cost flow. The BOQ resides as service line items within the PO's service package.

### 3.1 Work Order Structure in SAP

PO Element

SAP Table

Field

Value / Setting

Business Meaning

PO Number

`EKKO`

`EBELN`

4500000123

Work Order reference number

PO Type

`EKKO`

`BSART`

NB (or custom ZWO)

Standard PO / Work Order

Vendor

`EKKO`

`LIFNR`

0000001234

ABC Construction Pvt Ltd

Item Category

`EKPO`

`PSTYP`

D (Service)

Service procurement — enables SES

Acct Assignment

`EKPO`

`KNTTP`

P (Project)

Cost posted to WBS element

WBS Element

`EKKN`

`PS_PSP_PNR`

CG-ZEN-P2.01

Maps to Civil Works WBS

Retention %

`EKPO`

`RETPC`

5.00

5% retention per RA Bill

Retention Type

`EKKO`

`RETTP`

Per invoice

Retention on every invoice

Contract Value

`EKPO`

`NETWR`

₹15,00,00,000

Total contract amount

Service Package

`ESLH`

`PACKNO`

0000000001

Links to BOQ service lines

### 3.2 Bill of Quantities (BOQ) — Service Lines

The BOQ is the contractual price list. Each line represents a measurable activity with a unit rate. During RA Billing, the contractor claims the quantity of each BOQ item completed to date, multiplied by the agreed unit rate. The BOQ lines reside in `ESLL` (Service Line Items) linked via `ESLH` (Service Header) to the PO item.

Line

Service #

Description

BOQ Qty

UOM

Rate (₹)

BOQ Value (₹)

1

5001001

Earthwork excavation

5,000

CUM

180

9,00,000

2

5001002

PCC M10

2,000

CUM

4,500

90,00,000

3

5001003

RCC M25 (Foundation + Structure)

3,500

CUM

6,200

2,17,00,000

4

5001004

Reinforcement Steel (TMT Fe500D)

450

MT

62,000

2,79,00,000

5

5001005

Formwork (all types)

25,000

SQM

350

87,50,000

6

5002001

Brickwork 230mm

18,000

SQM

650

1,17,00,000

7

5002002

Internal Plastering 12mm

45,000

SQM

180

81,00,000

8

5002003

External Plastering 20mm

15,000

SQM

280

42,00,000

9

5003001

Plumbing Points

4,200

Point

2,800

1,17,60,000

10

5003002

Electrical Points

12,000

Point

1,200

1,44,00,000

11

5004001

Floor Tiling (vitrified)

35,000

SQM

450

1,57,50,000

12

5005001

Waterproofing (terrace + wet areas)

8,000

SQM

520

41,60,000

Total Contract Value

₹15,00,00,000\*

\*Illustrative — includes additional minor items, contingency, and rounding to reach ₹15 Cr.

📐

**Deviation / Extra Items** If work is required beyond the original BOQ (design changes, unforeseen site conditions), the contractor submits a **Deviation Order**. This creates additional service lines in the PO via transaction `ME22N` (Change PO), subject to approval by the Project Head. Extra items are tracked separately in RA Bill reporting as "Variation Work."

4

## Physical Measurement & Service Entry Sheet (SES)

The RA Bill begins on-site with physical measurement. Casagrand's site engineer and the contractor jointly measure the work completed during the billing period. These measurements are recorded in a **Measurement Book (MB)**, which then becomes the source document for creating the Service Entry Sheet in SAP.

### 4.1 Measurement Process

1

#### Joint Physical Measurement

Site Engineer (Casagrand) and Contractor Representative jointly visit completed work areas. Each BOQ item is measured using standard methods: length × breadth × height for volumetric work (CUM), area calculations for surface work (SQM), item counts for points/fixtures.

2

#### Measurement Book (MB) Recording

All measurements are recorded in the official Measurement Book with: item description, reference drawing number, floor/block identification, detailed dimensional calculations, and signatures of both parties. Each page is numbered and serially controlled.

3

#### Quality Certification

Casagrand's QA/QC team verifies that the measured work meets specified quality standards. Concrete cube test results, steel test certificates, soil compaction reports, and alignment/level checks are validated. Only quality-approved work is included in the RA Bill.

4

#### MB Abstract Preparation

The billing team prepares an abstract summarizing quantities per BOQ line item. This abstract shows: previously certified quantity, current period quantity, and cumulative quantity to date. This becomes the input for the Service Entry Sheet.

### 4.2 Service Entry Sheet (SES) in SAP

The SES is the SAP equivalent of the RA Bill measurement recording. It is created in transaction `ML81N` and links the physical measurement to the financial system. The SES header sits in table `ESSR`, while service line details reside in `ESLL`.

SES Field

Table

SAP Field

Example Value

Meaning

PO Reference

`ESSR`

`EBELN`

4500000123

Work Order number

PO Item

`ESSR`

`EBELP`

10

Service item line

Posting Date

`ESSR`

`BUDAT`

30.04.2026

Billing period end date

External Ref

`ESSR`

`XBLNR`

RA-BILL-007

RA Bill sequential number

Header Text

`ESSR`

`BKTXT`

"7th RA Bill – Block A Civil"

Description for audit trail

Net Value

`ESSR`

`NETWR`

₹1,25,00,000

This period's gross value

Cumulative Value

`ESSR`

`LWERT`

₹7,00,00,000

Total work done to date

Release Required

`ESSR`

`FRGRL`

X

Approval workflow enabled

Release Status

`ESSR`

`FRGZU`

02 (Released)

Current approval level

Final Entry

`ESSR`

`FINAL`

blank (not final)

Set 'X' only for last RA Bill

#### SES Line Items (ESLL) — Period Measurement Detail

Line

Service

Description

This Period Qty

UOM

Rate

Value (₹)

1

5001003

RCC M25 — Floors 8-9

280

CUM

6,200

17,36,000

2

5001004

Reinforcement Steel

38

MT

62,000

23,56,000

3

5001005

Formwork — Floors 8-9

2,100

SQM

350

7,35,000

4

5002001

Brickwork — 9th Floor

1,500

SQM

650

9,75,000

5

5003001

Plumbing Points — 8th Floor

120

Point

2,800

3,36,000

6

5002002

Internal Plastering — 7th Floor

3,200

SQM

180

5,76,000

7

5003002

Electrical Points — 7th-8th

480

Point

1,200

5,76,000

8

5004001

Floor Tiling — 6th Floor

2,800

SQM

450

12,60,000

9

5005001

Waterproofing — 5th Floor Terrace

600

SQM

520

3,12,000

Total This Period

₹88,62,000

\+ Previous SES Balance

₹36,38,000

RA Bill #7 Gross Value

₹1,25,00,000

5

## Compliance & Supporting Documents

Every RA Bill submission triggers a comprehensive compliance verification. The contractor must submit a bundle of construction documents, statutory compliance certificates, and contractual documents. The Accounts Payable team cannot release payment until all required documents are verified. Missing documents trigger a **payment block** (`LFB1-ZAHLS = 'A'`) until resolved.

### 5.1 Construction & Technical Documents

#

Document

Purpose

Frequency

Verified By

Criticality

1

Measurement Book (MB) Abstract

Quantity verification & audit trail

Every RA Bill

Site Engineer

Mandatory

2

Joint Measurement Sheet (signed)

Bilateral confirmation — dispute prevention

Every RA Bill

Site Engineer + Contractor

Mandatory

3

Quality Test Reports

Concrete cube test, steel test, compaction

Per activity

QA/QC Team

Mandatory

4

Approved Drawings (marked-up)

Work scope reference + as-built variations

As revised

Design Team

Conditional

5

Progress Photographs

Date-stamped visual evidence of completion

Monthly

Site Engineer

Mandatory

6

Daily Work Reports / Logbook

Labour deployment, equipment, weather log

Daily (monthly compilation)

Contractor → Site

Required

7

Work Completion Certificate

Engineer certification for milestone bills

At milestones

Project Manager

Conditional

### 5.2 Statutory Compliance Documents

Indian construction operations are governed by multiple labour and tax regulations. The contractor must demonstrate compliance with all applicable statutes before Casagrand can release payment. This is both a legal obligation and a risk mitigation measure for principal employer liability under the Contract Labour (Regulation & Abolition) Act, 1970.

#

Document

Governing Regulation

Frequency

Penalty for Non-Compliance

1

**GST Tax Invoice**

Goods & Services Tax Act, 2017

Every RA Bill

Input tax credit denial; ₹10K/invoice penalty

2

**PF Challan** (previous month)

Employees' PF & MP Act, 1952

Monthly

Principal employer liability + 12% interest + penalty

3

**ESI Challan** (previous month)

Employees' State Insurance Act, 1948

Monthly

Principal employer liability + interest

4

**Labour License** (valid copy)

Contract Labour Act, 1970

Annual / On renewal

Criminal prosecution; site shutdown risk

5

**Wage Register / Muster Roll**

BOCW Act, 1996; Payment of Wages Act

Monthly

Principal employer liability for unpaid wages

6

**Insurance — CAR Policy**

Contract conditions

Annual / Project duration

Uncovered loss liability on Casagrand

7

**Workmen Compensation Insurance**

Workmen's Compensation Act, 1923

Annual

Principal employer pays compensation directly

8

**Safety Compliance Certificate**

BOCW Act; IS Codes

Quarterly

Site stoppage by safety inspector

9

**RERA Registration** (project level)

Real Estate (Regulation & Development) Act

Project lifecycle

Project registration cancellation

### 5.3 Contractual Documents

#

Document

Purpose

When Required

1

**Performance Bank Guarantee**

5-10% of contract value; protects against default

Before first RA Bill

2

**Deviation / Extra Item Approval**

Authorized work beyond original BOQ

When extra work recorded

3

**Extension of Time (EOT) Approval**

Formal timeline extension

Before LD imposition

4

**No Claim Certificate**

Contractor settlement acceptance

Final Bill only

5

**Defect Liability Period (DLP) BG**

Guarantee for post-completion defect repairs

Final Bill / Retention release

🛑

**Payment Block Protocol** If ANY mandatory statutory document is missing or expired, the AP team must set `LFB1-ZAHLS = 'A'` (payment block = All) for the vendor at the company code level. The F110 payment run will skip all invoices for this vendor until the block is removed. The compliance team sends a formal notification to the contractor with a 7-day cure period.

6

## Advance Management & Recovery

Casagrand typically disburses a **Mobilization Advance** (5-15% of contract value) to the contractor at project commencement. This advance enables the contractor to procure materials, mobilize equipment, and set up site infrastructure. The advance is then **recovered proportionally** from each subsequent RA Bill.

### 6.1 Types of Advances

#### 🏗️ Mobilization Advance

**Purpose:** Initial setup — equipment mobilization, site office, temporary facilities, initial material procurement.

**Typical Rate:** 10% of contract value

**Security:** Bank Guarantee for 110% of advance amount

**Recovery:** 10% from each RA Bill until fully recovered

**SAP Posting:** F-47 (Down Payment Request) → F-48 (Down Payment)

#### 📦 Material Advance

**Purpose:** Procurement of long-lead materials (lifts, DG sets, transformers, fire fighting equipment).

**Typical Rate:** Up to 75% of material cost on proof of order

**Security:** Material delivery proof or BG

**Recovery:** On delivery and installation at site

**SAP Posting:** F-47 with Special GL indicator 'A'

### 6.2 Advance Recovery Schedule — Illustrative

The advance recovery table below shows how a ₹1.50 Cr mobilization advance (10% of ₹15 Cr contract) is systematically recovered across RA Bills. The recovery rate of 10% per bill means each RA Bill's gross value is reduced by 10% until the entire advance is recovered.

RA Bill #

Gross Value (₹)

Recovery Rate

Recovery This Bill (₹)

Cumulative Recovered (₹)

Balance Outstanding (₹)

RA-001

75,00,000

10%

7,50,000

7,50,000

1,42,50,000

RA-002

90,00,000

10%

9,00,000

16,50,000

1,33,50,000

RA-003

1,10,00,000

10%

11,00,000

27,50,000

1,22,50,000

RA-004

1,05,00,000

10%

10,50,000

38,00,000

1,12,00,000

RA-005

95,00,000

10%

9,50,000

47,50,000

1,02,50,000

RA-006

1,00,00,000

10%

10,00,000

57,50,000

92,50,000

RA-007

1,25,00,000

10%

12,50,000

70,00,000

80,00,000

RA-008

1,15,00,000

10%

11,50,000

81,50,000

68,50,000

...

...

10%

...

...

...

**RA-014**

1,20,00,000

—

**8,50,000** (balance)

**1,50,00,000**

**NIL ✅**

💡

**SAP Down Payment Clearing** In S/4HANA, the advance is posted as a Special GL transaction with indicator **'A'**. During invoice processing (MIRO), the system can auto-propose the down payment for clearing. The open item in `BSIK` with Special GL = 'A' reduces as each recovery is posted. The AP team should run `FBL1N` with Special GL filter to monitor the outstanding advance balance per contractor.

7

## Retention Money Management

Retention money is a contractual mechanism that protects Casagrand against construction defects discovered after work completion. A percentage of each RA Bill is withheld ("retained") and released only after the contractor completes the **Defect Liability Period (DLP)** — typically 12-24 months after project handover.

### 7.1 Retention Mechanics

Retention Calculation per RA Bill

Retention Amount = Gross RA Bill Value × RETPC (from EKPO)

Example: ₹1,25,00,000 × 5% = ₹6,25,000 retained this period

Over project lifecycle (₹15 Cr contract):

Maximum Retention = ₹15,00,00,000 × 5% = ₹75,00,000

### 7.2 Retention Release Schedule

Release Milestone

% Released

Trigger

SAP Transaction

Condition

Phase 1 — On Completion

50%

Completion Certificate issued

`MRBR`

Final SES accepted, no outstanding defects

Phase 2 — Post DLP

50%

DLP expiry (12-24 months)

`MRBR`

Defect inspection passed, no open snag list

### 7.3 Retention Tracking — Per Contractor

RA Bill

Gross Value (₹)

Retention 5% (₹)

Cumulative Retained (₹)

Status

RA-001

75,00,000

3,75,000

3,75,000

Held

RA-002

90,00,000

4,50,000

8,25,000

Held

RA-003

1,10,00,000

5,50,000

13,75,000

Held

RA-004

1,05,00,000

5,25,000

19,00,000

Held

RA-005

95,00,000

4,75,000

23,75,000

Held

RA-006

1,00,00,000

5,00,000

28,75,000

Held

RA-007

1,25,00,000

6,25,000

35,00,000

Held

⚙️

**SAP Retention Configuration** Retention in S/4HANA is configured through: `EKPO-RETPC` (retention percentage per PO item), `EKKO-RETTP` (retention type — per invoice or at final). The retained amount creates a blocked invoice line that is released via `MRBR`. For financial reporting, the retention liability appears in the vendor sub-ledger as a separate open item.

8

## Escalation & De-escalation

Long-duration construction contracts (typically > 12 months) include **price variation clauses** that protect both parties against material and labour cost fluctuations. Escalation (price increase) benefits the contractor; de-escalation (price decrease) benefits Casagrand. The adjustment is calculated using government-published indices — typically the CPWD (Central Public Works Department) formula or custom index baskets.

### 8.1 CPWD Price Adjustment Formula

CPWD Price Variation Formula

Price Adjustment = V × 0.85 × Σ \[ Wi × (Ii − Io) / Io \]

V = Value of work done at base prices in the billing period

Wi = Weight factor for each material/labour component

Ii = Published index at the time of execution

Io = Base index (at time of tender opening)

0.85 = Variable component (15% deemed as fixed/non-escalable)

### 8.2 Index Component Tracking

Component

Weight (Wi)

Base Index (Io)

Current Index (Ii)

Variation %

Direction

Cement

0.12

320

345

+7.81%

↑ Escalation

Steel (TMT bars)

0.30

1,250

1,180

−5.60%

↓ De-escalation

Labour (unskilled)

0.25

780

835

+7.05%

↑ Escalation

Fuel / Energy

0.10

410

445

+8.54%

↑ Escalation

Plant & Equipment

0.08

290

305

+5.17%

↑ Escalation

Other Materials

0.15

520

530

+1.92%

↑ Marginal

### 8.3 Worked Example — RA Bill #7

V (Work value this period at base prices)   = ₹1,25,00,000
Variable component factor                    = 0.85
Effective base for escalation                = ₹1,06,25,000

Weighted index variation:
  Cement:     0.12 × (+7.81%)  = +0.937%
  Steel:      0.30 × (−5.60%)  = −1.680%
  Labour:     0.25 × (+7.05%)  = +1.763%
  Fuel:       0.10 × (+8.54%)  = +0.854%
  Plant:      0.08 × (+5.17%)  = +0.414%
  Others:     0.15 × (+1.92%)  = +0.288%
  ─────────────────────────────────────
  Net weighted variation          = +2.576%

Price Adjustment = ₹1,06,25,000 × 2.576%
                 = **₹2,73,700** (NET ESCALATION — payable to contractor)

Note: Steel de-escalation partially offsets cement & labour escalation.
      If steel drops further in future, net could turn NEGATIVE = de-escalation.
  

### 8.4 SAP Processing of Escalation / De-escalation

Scenario

SAP Document

Transaction

Accounting Entry

Net Escalation (payable to contractor)

Debit Note to Contractor

`MIRO` (credit memo) or `FB60`

Dr. Project WBS (CG-ZEN-P2.01) / Cr. Vendor

Net De-escalation (recovery from contractor)

Credit Note from Contractor

`MIRO` (credit memo) or `FB65`

Dr. Vendor / Cr. Project WBS (CG-ZEN-P2.01)

Mixed (some items up, some down)

Net amount as single adjustment

Based on net direction

Net posting to vendor account

9

## Approval Workflow & Invoice Verification

The RA Bill approval chain is a multi-level, value-based release strategy that ensures appropriate authorization before payment. The Service Entry Sheet must be fully released before the AP team can create an invoice.

### 9.1 SES Release Strategy

graph LR A\["📋 Contractor Submits  
RA Bill + Documents"\] --> B\["👷 Site Engineer  
Verifies Quantities  
_Release Code: 01_"\] B --> C\["🔬 QA/QC Team  
Quality Certification"\] C --> D\["👔 Project Manager  
Approves SES  
_Release Code: 02_"\] D --> E\["📎 Compliance Check  
PF / ESI / Docs"\] E --> F{"All Docs  
OK?"} F -->|"❌ No"| G\["🔒 Payment Block  
ZAHLS = 'A'"\] F -->|"✅ Yes"| H\["📄 AP Team  
Invoice MIRO"\] H --> I\["🔍 3-Way Match  
PO ↔ SES ↔ Invoice"\] I --> J\["💰 Payment Run  
F110"\] J --> K\["🏦 Bank Transfer  
NEFT/RTGS"\] G --> L\["📧 Contractor  
Submits Missing Docs"\] L --> E style A fill:#6B3FA0,stroke:#6B3FA0,color:#fff style H fill:#059669,stroke:#059669,color:#fff style K fill:#059669,stroke:#059669,color:#fff style G fill:#dc2626,stroke:#dc2626,color:#fff

### 9.2 Value-Based Release Levels

Release Level

Approver

Value Threshold

SAP Release Code

SLA (Days)

Level 1

Site Engineer

All RA Bills

`01`

2

Level 2

Project Manager

\> ₹10 Lakhs

`02`

3

Level 3

VP — Projects

\> ₹50 Lakhs

`03`

3

Level 4

CFO / Director

\> ₹1 Crore

`04`

5

### 9.3 Three-Way Match — Invoice Verification (MIRO)

After the SES is fully released, the AP team creates the invoice in `MIRO`. The system performs a three-way match between the Purchase Order (contracted quantity/value), the Service Entry Sheet (certified quantity/value), and the Invoice (claimed quantity/value). Tolerances are configured in customizing (transaction `OMR6`).

#### Three-Way Match Validation

Element

Source

Value

Tolerance

PO / Work Order

`EKPO-NETWR`

₹15,00,00,000 (total)

—

SES (Certified)

`ESSR-NETWR`

₹1,25,00,000 (this period)

±5% of PO line value

Invoice (Claimed)

`RBKP-RMWWR`

₹1,25,00,000

Must match SES ±₹100

✅ If all three match within tolerance → Invoice posted automatically  
❌ If mismatch → Invoice blocked for manual review (MRBR)

10

## AP Department — Payment Processing

The Accounts Payable department is the financial control center for all contractor payments. After the SES is released and the invoice is posted, the AP team applies all deductions, verifies compliance, and processes the payment through the automatic payment program (F110).

### 10.1 Invoice Deduction Waterfall

The net payable amount is derived by applying a series of deductions to the gross RA Bill value. This "waterfall" ensures Casagrand retains appropriate protections while paying the contractor their earned amount.

RA Bill #7 — Deduction Waterfall

Gross RA Bill Value: ₹1,25,00,000

(−) TDS @ 1% (Sec 194C — Company): ₹ 1,25,000

(−) Retention @ 5% (EKPO-RETPC): ₹ 6,25,000

(−) Advance Recovery @ 10%: ₹ 12,50,000

(−) Material Recovery (Casagrand-supplied): ₹ 9,50,000

(+) Escalation Adjustment (net): ₹ 2,73,700

(−) Penalty / LD (if applicable): ₹ NIL

Net Payable to Contractor: ₹ 98,23,700

### 10.2 Accounting Entries

Step

Description

Debit

Credit

Amount (₹)

1

Invoice posting (MIRO)

Project WBS (CG-ZEN-P2.01)

Vendor Payable (ABC Construction)

1,25,00,000

2

TDS deduction

Vendor Payable

TDS Payable (Sec 194C)

1,25,000

3

Retention withholding

Vendor Payable

Retention Payable (liability)

6,25,000

4

Advance recovery

Vendor Payable

Down Payment Clearing (Spl GL 'A')

12,50,000

5

Material recovery

Vendor Payable

Material Issue Clearing

9,50,000

6

Escalation adjustment

Project WBS

Vendor Payable

2,73,700

7

Payment (F110)

Vendor Payable

Bank Account (NEFT/RTGS)

98,23,700

### 10.3 F110 — Automatic Payment Run Configuration

Parameter

Setting

Purpose

Run Date

5th of each month (or 10th)

Monthly contractor payment cycle

Company Code

1000 (Casagrand)

Paying company

Payment Method

T (NEFT/RTGS)

Electronic fund transfer

Vendor Selection

By due date

Only invoices past payment terms

Payment Block Check

Active

Skips vendors with ZAHLS ≠ blank

Minimum Amount

₹50,000

Consolidate small amounts

11

## % Work Completion vs. % Amount Disbursed — AP Reporting

This is the **single most critical management report** in the RA Bill process. It provides a real-time comparison of physical progress (how much work is done) versus financial progress (how much money has been paid). The variance between these two percentages is the primary indicator of financial health and payment discipline.

### 11.1 Report Logic & Data Sources

Metric

Formula

SAP Data Source

**% Work Complete**

(Cumulative SES Value ÷ Contract Value) × 100

`ESSR-LWERT` ÷ `EKPO-NETWR`

**% Amount Disbursed**

(Cumulative Payments ÷ Contract Value) × 100

Σ `BSAK-WRBTR` ÷ `EKPO-NETWR`

**Variance**

% Work Complete − % Amount Disbursed

Calculated

**Retention Balance**

Σ (RA Bill × RETPC) − Released Retention

`EKPO-RETPC` × cumulative, less MRBR releases

**Advance Balance**

Initial Advance − Σ Recoveries

`BSIK` Spl GL 'A' balance

### 11.2 Illustrative RA Bill Tracker — Contractor Performance Report

RA Bill #

Period

Cumul. Work Value (₹)

% Work Complete

Work Progress

Cumul. Paid (₹)

% Disbursed

Payment Progress

Variance

Health

RA-001

Oct '25

75,00,000

5.0%

56,25,000

3.8%

+1.2%

🟢

RA-002

Nov '25

1,65,00,000

11.0%

1,23,75,000

8.3%

+2.7%

🟢

RA-003

Dec '25

2,75,00,000

18.3%

2,06,25,000

13.8%

+4.5%

🟢

RA-004

Jan '26

3,80,00,000

25.3%

2,85,00,000

19.0%

+6.3%

🟢

RA-005

Feb '26

4,75,00,000

31.7%

3,56,25,000

23.8%

+7.9%

🟢

RA-006

Mar '26

5,75,00,000

38.3%

4,31,25,000

28.8%

+9.5%

🟢

**RA-007**

**Apr '26**

**7,00,00,000**

**46.7%**

**5,29,48,700**

**35.3%**

**+11.4%**

🟢 Healthy

### 11.3 Why % Disbursed Is Always Less Than % Complete

In a healthy construction payment process, the % Disbursed should **always lag behind** % Work Complete. This gap is created by the accumulation of deductions. The wider the gap, the more financial protection Casagrand has. However, an excessively wide gap (beyond 20%) may indicate payment delays that could affect contractor cash flow and project progress.

Deduction Factor

Impact on Gap

Typical Contribution

When Gap Narrows

Retention (5%)

Widens gap permanently until DLP release

~5%

After completion + DLP

Advance Recovery (10%)

Widens gap significantly in early RA Bills

~8-10% (decreasing)

After advance fully recovered

TDS (1-2%)

Widens gap marginally

~1-2%

Never (tax deducted permanently)

Material Recovery

Widens gap when Casagrand supplies materials

Variable

When materials fully consumed

Pending Invoices

Widens gap (work done but not yet paid)

~1 month lag

On next payment run

### 11.4 Variance Health Indicators & Alerts

Variance Range

Status

Indicator

Interpretation

Action Required

+15% to +25%

🟢 Healthy

Normal

Retention + advance recovery working as designed. Contractor has strong work momentum.

No action — monitor monthly

+5% to +15%

🟡 Watch

Monitor

Advance nearly fully recovered — gap will naturally narrow. Expected in later RA Bills.

Confirm advance recovery schedule

0% to +5%

🟠 Caution

Close

Payments are catching up to work value. Could indicate accelerated payments or delayed measurements.

Review SES timing vs. payment dates

0% (Parity)

⚠️ At Risk

Parity

Disbursed equals work done — no financial buffer remaining. All deductions bypassed?

Immediate audit: verify retention & TDS applied

Negative (%Disbursed > %Complete)

🔴 Overpayment

CRITICAL

More money paid than work certified — exposure to financial loss if contractor defaults.

Stop all payments. Forensic audit. Management escalation.

🚨

**Overpayment Alert — Automatic Trigger** If the variance drops below 0% (i.e., `% Disbursed > % Work Complete`), the system must automatically:

1.  Set a payment block on the vendor (`LFB1-ZAHLS = 'Z'` — manual investigation block)
2.  Send an email alert to the CFO and VP-Projects
3.  Generate an exception report with all RA Bills, SES values, and payment history
4.  Trigger a mandatory joint re-measurement at site within 48 hours

### 11.5 Multi-Contractor Dashboard View

At the project level, the PMO monitors all contractors simultaneously. The dashboard aggregates the % Work vs. % Disbursed for every active work order under the project, enabling early detection of contractors where payments are out of alignment.

Contractor

Package

Contract (₹ Cr)

% Work

% Disbursed

Variance

Health

Retention Held (₹)

Advance Bal (₹)

ABC Construction

Civil — Block A

15.00

46.7%

35.3%

+11.4%

🟢

35,00,000

80,00,000

XYZ MEP Services

MEP — Full Project

8.50

32.0%

25.1%

+6.9%

🟢

13,60,000

42,50,000

PQR Interiors

Finishing — Phase 1

4.20

58.0%

54.2%

+3.8%

🟠

12,18,000

NIL

LMN Landscaping

External Development

2.80

15.0%

12.8%

+2.2%

🟡

2,10,000

14,00,000

PROJECT TOTAL

30.50

38.4%

31.9%

+6.5%

🟢

62,88,000

1,36,50,000

12

## SAP Transaction Code Reference

The following table provides a complete reference of all SAP transactions used across the RA Bill lifecycle, organized by process phase and user role.

Phase

Transaction

Description

Used By

Frequency

**Work Order**

`ME21N`

Create Purchase Order (Work Order)

Procurement

Per contract

`ME22N`

Change PO (deviations, extra items)

Procurement

As needed

`ME23N`

Display PO

All stakeholders

Frequent

**Measurement**

`ML81N`

Create Service Entry Sheet (RA Bill)

Site / Billing

Monthly

`ML85`

Mass Release of SES

Project Manager

Monthly

`ML86`

Cancel Service Entry Sheet

Site / Billing

Exception

**Invoice**

`MIRO`

Invoice Verification (3-way match)

AP Team

Monthly

`MRBR`

Release Blocked Invoices / Retention

AP Team

Monthly / DLP

`MIR4`

Display Invoice Document

AP Team

Frequent

`MR11`

GR/IR Account Maintenance

AP Team

Monthly close

**Payment**

`F110`

Automatic Payment Run

Treasury

Weekly / Monthly

`F-47`

Down Payment Request

AP Team

Per advance

`F-48`

Post Down Payment

AP Team

Per advance

**Reporting**

`FBL1N`

Vendor Line Items (open/cleared)

Finance

Daily

`ME2N`

PO List by PO Number

Procurement

Frequent

`CN41N`

Project Structure Overview (PS)

PMO

Weekly

`S_ALR_87012103`

Vendor Balances

Finance

Monthly close

**Project System**

`CJ20N`

Project Builder (create/maintain WBS)

PMO

Project setup

`CJ30`

Project actual cost reports

Finance / PMO

Monthly

13

## Risk, Controls & Audit Framework

The RA Bill process carries inherent financial and operational risks. Casagrand implements a layered control framework to mitigate these risks — spanning preventive controls (before payment), detective controls (during processing), and corrective controls (after exceptions).

### 13.1 Risk Register

Risk ID

Risk Description

Probability

Impact

Residual Risk

Preventive Control

Detective Control

R01

Overpayment — amount paid exceeds work done

Medium

Critical

Medium

Retention + Advance deduction

% Work vs. % Disbursed report

R02

Quantity inflation — contractor overstates measurement

High

High

Medium

Joint measurement; QA/QC sign-off

Random audit measurements

R03

Duplicate RA Bill submission

Low

High

Low

SAP SES → PO → Invoice 3-way match

Duplicate invoice check in MIRO

R04

Statutory non-compliance by contractor

High

Critical

Medium

Payment block (ZAHLS) until docs verified

Monthly compliance tracker

R05

Advance unrecovered — contractor abandons

Medium

Critical

Medium

Bank Guarantee (110% of advance)

Advance balance monitoring report

R06

Rate manipulation in escalation calculation

Low

High

Low

Published govt. index only; QS verification

Annual audit of index calculations

R07

Retention not released on time (DLP expired)

Medium

Medium

Medium

DLP calendar alerts in SAP

Retention aging report (MRBR)

### 13.2 Segregation of Duties (SoD) Matrix

Activity

Site Engineer

Project Manager

Billing Team

AP Team

Treasury

Physical Measurement

✅ Execute

❌

❌

❌

❌

Create SES (ML81N)

❌

❌

✅ Execute

❌

❌

Release SES (Approve)

✅ Level 1

✅ Level 2-3

❌

❌

❌

Compliance Verification

❌

❌

❌

✅ Execute

❌

Invoice Posting (MIRO)

❌

❌

❌

✅ Execute

❌

Payment Run (F110)

❌

❌

❌

❌

✅ Execute

Retention Release

❌

✅ Approve

❌

✅ Execute

❌

14

## End-to-End Process Flow

The following comprehensive flow diagram depicts the complete RA Bill lifecycle — from tender award through recurring monthly billing to final bill settlement and retention release. Each swim lane represents a different organizational function.

### 14.1 Complete RA Bill Lifecycle

graph TB subgraph PRE\["📋 PRE-BILLING SETUP (One-time)"\] A1\["Tender Award &  
Contract Signing"\] --> A2\["Create Work Order PO  
(ME21N)"\] A2 --> A3\["Define BOQ /  
Service Lines (ESLL)"\] A3 --> A4\["Disburse Mobilization  
Advance (F-47/F-48)"\] A4 --> A5\["Contractor provides  
Performance BG"\] end subgraph MONTHLY\["🔄 MONTHLY RA BILL CYCLE (Repeating)"\] B1\["Joint Physical  
Measurement"\] --> B2\["Measurement Book  
Recording"\] B2 --> B3\["QA/QC Quality  
Certification"\] B3 --> B4\["Create SES  
(ML81N)"\] B4 --> B5\["Contractor Submits  
Compliance Docs"\] B5 --> B6{"Compliance  
OK?"} B6 -->|"Yes ✅"| B7\["SES Release  
(Approval Chain)"\] B6 -->|"No ❌"| B8\["Payment Block  
(ZAHLS = A)"\] B8 --> B9\["Contractor  
Rectifies"\] --> B5 B7 --> B10\["Invoice Posting  
(MIRO)"\] B10 --> B11\["Apply Deductions:  
TDS, Retention, Advance,  
Material, Escalation"\] B11 --> B12\["Payment Run  
(F110)"\] B12 --> B13\["Bank Transfer  
(NEFT/RTGS)"\] B13 --> B14\["Update RA Bill  
Tracking Report"\] end subgraph FINAL\["✅ FINAL SETTLEMENT"\] C1\["Last SES  
(FINAL = X)"\] --> C2\["Work Completion  
Certificate"\] C2 --> C3\["No Claim  
Certificate"\] C3 --> C4\["Release 50%  
Retention (MRBR)"\] C4 --> C5\["DLP Period  
(12-24 months)"\] C5 --> C6\["Release 50%  
Retention (MRBR)"\] C6 --> C7\["Release  
Performance BG"\] end A5 --> B1 B14 --> B1 B14 --> C1 style PRE fill:#f8f6ff,stroke:#6B3FA0 style MONTHLY fill:#f0fdf4,stroke:#059669 style FINAL fill:#fffbeb,stroke:#d97706

### 14.2 Timeline — Standard RA Bill Processing SLA

Day 1-3: Physical Measurement

Site Engineer + Contractor conduct joint measurement at site

Day 4-5: QA/QC Verification

Quality team validates test reports and certifies work quality

Day 6-7: SES Creation (ML81N)

Billing team enters the measurement book data into SAP as a Service Entry Sheet

Day 8-12: SES Approval Chain

Site Engineer → PM → VP → CFO releases based on value thresholds

Day 13-15: Compliance Verification

AP team verifies all statutory documents (PF/ESI/GST/Labour/Insurance)

Day 16-18: Invoice Posting (MIRO)

AP team creates invoice, 3-way match validated, deductions applied

Day 19-25: Payment Run (F110)

Treasury runs automatic payment program based on due dates

Day 26-30: Bank Transfer

NEFT/RTGS transfer to contractor's registered bank account

📊

**Target SLA** Complete RA Bill processing from measurement to payment within **30 calendar days**. The RERA Act requires developers to maintain timely contractor payments to avoid project delays. Persistent payment delays beyond 45 days may trigger contractor escalation clauses.

15

## Implementation Roadmap

The following Gantt chart outlines the phased implementation roadmap for digitalizing and automating the RA Bill process within Casagrand's S/4HANA landscape. The roadmap spans 16 weeks across 4 phases, from configuration through go-live.

gantt title Casagrand RA Bill Process — Implementation Roadmap dateFormat YYYY-MM-DD axisFormat %b %d section Phase 1: Foundation Master Data Setup (Vendor, WBS, Service) :p1a, 2026-06-01, 14d PO Configuration (Service PO, Retention) :p1b, 2026-06-01, 10d Release Strategy Config (SES Approval) :p1c, after p1b, 7d Payment Method & Terms Config :p1d, after p1b, 5d section Phase 2: Core Process SES Creation & Release Testing :p2a, 2026-06-22, 10d MIRO 3-Way Match Testing :p2b, after p2a, 7d Advance & Retention Config :p2c, 2026-06-22, 12d Escalation Calculation Setup :p2d, after p2c, 7d F110 Payment Run Testing :p2e, after p2b, 5d section Phase 3: Reporting RA Bill Tracker Report Development :p3a, 2026-07-13, 14d % Work vs % Disbursed Dashboard :p3b, after p3a, 10d Retention & Advance Monitoring Reports :p3c, 2026-07-13, 10d Compliance Document Tracking :p3d, after p3c, 7d section Phase 4: Go-Live User Acceptance Testing (UAT) :p4a, 2026-08-03, 10d User Training (Site + AP + PMO) :p4b, after p4a, 7d Parallel Run (1 month) :p4c, after p4b, 21d Go-Live & Hypercare :p4d, after p4c, 14d

### Phase Deliverables

Phase

Duration

Key Deliverables

Success Criteria

**Phase 1** — Foundation

3 weeks

Master data loaded; PO types configured; SES release strategy active; Payment methods assigned

100% vendor master completeness for active contractors

**Phase 2** — Core Process

4 weeks

End-to-end SES → Invoice → Payment tested; Retention auto-calculated; Advance recovery functional

Zero errors in 3-way match for test RA Bills

**Phase 3** — Reporting

4 weeks

RA Bill tracker report operational; % Work vs. % Disbursed dashboard live; Compliance tracker deployed

All KPIs auto-calculated from SAP data

**Phase 4** — Go-Live

5 weeks

UAT signed off; 50+ users trained; 1 month parallel run passed; Production go-live

Zero overpayment exceptions; 30-day SLA achieved

16

## Appendices & SAP Data Dictionary

### Appendix A — SAP Table Reference for RA Bill Process

Table

Description

Key Fields for RA Bill

Usage in Process

`EKKO`

PO Header

`EBELN`, `BSART`, `LIFNR`, `RETTP`

Work Order identification & retention type

`EKPO`

PO Item

`EBELP`, `PSTYP`, `KNTTP`, `RETPC`, `NETWR`

Service item, retention %, contract value

`EKKN`

PO Account Assignment

`PS_PSP_PNR`, `KOSTL`

Links PO to WBS element for cost posting

`ESLH`

Service Header

`PACKNO`, `EBELN`, `EBELP`

Links BOQ service package to PO item

`ESLL`

Service Line Items

`SRVPOS`, `MENGE`, `NETWR`, `MEINS`

Individual BOQ lines with quantities and values

`ESSR`

SES Header

`LBLNI`, `EBELN`, `NETWR`, `LWERT`, `FRGZU`

RA Bill recording — gross value, cumulative, approval

`RBKP`

Invoice Header

`BELNR`, `GJAHR`, `LIFNR`, `RMWWR`

Invoice document from MIRO

`RSEG`

Invoice Item

`EBELN`, `EBELP`, `WRBTR`

Invoice line linked to PO

`PROJ`

Project Definition

`PSPID`, `POST1`, `WERKS`

Project master — Casagrand township

`PRPS`

WBS Elements

`POSID`, `POST1`, `STUFE`

Work breakdown for cost allocation

`LFA1`

Vendor General

`LIFNR`, `NAME1`, `STCEG`, `SPERR`

Contractor identification & GST

`LFB1`

Vendor Company Code

`ZTERM`, `ZWELS`, `ZAHLS`

Payment terms, method, block

`LFBK`

Vendor Bank Details

`BANKL`, `BANKN`, `BANKS`

EFT payment routing

`BSIK`

Vendor Open Items

`LIFNR`, `BELNR`, `WRBTR`, `ZFBDT`

Outstanding invoices & advance balance

`BSAK`

Vendor Cleared Items

`LIFNR`, `AUGDT`, `WRBTR`

Payment history & cleared amounts

`REGUH`

Payment Run Data

`LAUFD`, `LIFNR`, `RBETR`

F110 payment program results

`KONV`

PO Conditions

`KNUMV`, `KSCHL`, `KBETR`

Pricing conditions for escalation

### Appendix B — Glossary of Terms

Term

Full Form

Definition

RA Bill

Running Account Bill

Periodic interim payment claim by contractor for work done to date

BOQ

Bill of Quantities

Itemized list of work activities with quantities and unit rates

SES

Service Entry Sheet

SAP document recording measured service quantities against PO

MB

Measurement Book

Physical record of joint measurements at construction site

DLP

Defect Liability Period

Post-completion period during which contractor is liable for defects

WBS

Work Breakdown Structure

Hierarchical decomposition of project scope for cost management

TDS

Tax Deducted at Source

Income tax deducted by payer under Section 194C (contractors)

PF

Provident Fund

Employee retirement fund contribution mandated by EPF Act

ESI

Employees' State Insurance

Social security health insurance for employees below wage threshold

RERA

Real Estate Regulatory Authority

Regulatory body for real estate projects in India

CPWD

Central Public Works Department

Government body publishing construction material price indices

LD

Liquidated Damages

Pre-determined penalty for project delay

EOT

Extension of Time

Formal extension of project completion timeline

CAR

Contractor's All Risk

Insurance covering construction-related risks

BG

Bank Guarantee

Financial instrument ensuring contractor performance

### Appendix C — Document Checklist Template

The following checklist is used by the AP team to verify completeness before processing each RA Bill. All mandatory items must be checked before the payment block can be removed.

#

Document

Category

Mandatory?

Received

Verified

Remarks

1

GST Tax Invoice

Statutory

✅ Yes

☐

☐

2

Measurement Book Abstract

Technical

✅ Yes

☐

☐

3

Joint Measurement Sheet

Technical

✅ Yes

☐

☐

4

Quality Test Reports

Technical

✅ Yes

☐

☐

5

PF Challan (prev month)

Statutory

✅ Yes

☐

☐

6

ESI Challan (prev month)

Statutory

✅ Yes

☐

☐

7

Labour License (valid)

Statutory

✅ Yes

☐

☐

8

Wage Register / Muster Roll

Statutory

✅ Yes

☐

☐

9

Insurance — CAR + WC

Contractual

✅ Yes

☐

☐

10

Safety Compliance Cert.

Statutory

✅ Yes

☐

☐

11

Progress Photographs

Technical

✅ Yes

☐

☐

12

Deviation Approval (if any)

Contractual

Conditional

☐

☐

#### Document Details

Document ID: CG-PROC-RABILL-2026-001

Version: 2.0

Status: Final — Approved for Distribution

Classification: Confidential — Internal

#### SAP Reference System

System: S/4HANA 758 (HANA DB)

Modules: MM-SRV, PS, FI-AP

Tables Referenced: EKKO, EKPO, EKKN, ESLH, ESLL, ESSR, RBKP, RSEG, PROJ, PRPS, LFA1, LFB1, LFBK, BSIK, BSAK, REGUH, KONV

Note: Backend HANA used as architectural reference only — not Casagrand production data

#### Prepared By

C16 — Enterprise SAP Process Intelligence

Generated: 06 May 2026

Next Review: November 2026

© 2026 Casagrand Group — All rights reserved. This document contains proprietary business process information.  
Prepared with C16 Enterprise Process Intelligence Platform | SAP S/4HANA 758 Reference Architecture

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._