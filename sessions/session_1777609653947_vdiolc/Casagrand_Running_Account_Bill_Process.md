  Casagrand — Running Account Bill (RA Bill) Process | Enterprise Blueprint mermaid.initialize({startOnLoad:true, theme:'base', themeVariables:{primaryColor:'#6B3FA0', primaryTextColor:'#fff', primaryBorderColor:'#B5567A', lineColor:'#8B5CF6', secondaryColor:'#f3e8ff', tertiaryColor:'#fdf2f8'}}); @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800;900&display=swap'); \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Inter', 'Segoe UI', Arial, sans-serif; color: #1e293b; background: #f1f5f9; line-height: 1.7; font-size: 14.5px; } .doc-wrapper { max-width: 1200px; margin: 0 auto; background: #fff; box-shadow: 0 4px 40px rgba(107,63,160,0.10); } /\* ── HEADER BANNER ── \*/ .header-banner { background: linear-gradient(135deg, #1a0533 0%, #2d1054 20%, #6B3FA0 50%, #B5567A 80%, #C97B3A 100%); color: #fff; padding: 50px 60px; position: relative; overflow: hidden; } .header-banner::before { content: ''; position: absolute; top: -40%; right: -10%; width: 500px; height: 500px; background: radial-gradient(circle, rgba(255,255,255,0.06) 0%, transparent 70%); border-radius: 50%; } .header-banner::after { content: ''; position: absolute; bottom: -30%; left: -5%; width: 400px; height: 400px; background: radial-gradient(circle, rgba(201,123,58,0.12) 0%, transparent 70%); border-radius: 50%; } .header-banner h1 { font-size: 2.4em; font-weight: 800; letter-spacing: -0.5px; margin-bottom: 6px; position: relative; z-index: 1; } .header-banner .subtitle { font-size: 1.15em; font-weight: 300; opacity: 0.92; margin-bottom: 18px; position: relative; z-index: 1; } .header-meta { display: flex; gap: 20px; flex-wrap: wrap; position: relative; z-index: 1; } .header-meta .badge { background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.25); padding: 5px 16px; border-radius: 20px; font-size: 0.82em; font-weight: 500; backdrop-filter: blur(8px); } .logo-row { display: flex; align-items: center; gap: 16px; margin-bottom: 14px; position: relative; z-index: 1; } .logo-row img { height: 50px; border-radius: 8px; } /\* ── DOCUMENT META BAR ── \*/ .meta-bar { background: linear-gradient(90deg, #f8f6ff 0%, #fdf2f8 100%); padding: 18px 60px; border-bottom: 2px solid #e8d5f5; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 14px; } .meta-item { font-size: 0.82em; } .meta-item .label { color: #6B3FA0; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; font-size: 0.75em; } .meta-item .value { color: #334155; font-weight: 500; margin-top: 2px; } /\* ── CONTENT AREA ── \*/ .content { padding: 40px 60px; } /\* ── TABLE OF CONTENTS ── \*/ .toc { background: linear-gradient(135deg, #f8f6ff 0%, #fef3f2 100%); border: 1px solid #e8d5f5; border-radius: 12px; padding: 28px 34px; margin-bottom: 40px; } .toc h2 { color: #6B3FA0; font-size: 1.15em; margin-bottom: 14px; font-weight: 700; } .toc-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6px 30px; } .toc a { color: #475569; text-decoration: none; font-size: 0.88em; padding: 4px 0; border-bottom: 1px dotted #cbd5e1; display: block; transition: all 0.2s; } .toc a:hover { color: #6B3FA0; border-bottom-color: #6B3FA0; } .toc a .num { color: #B5567A; font-weight: 700; margin-right: 6px; } /\* ── SECTION TITLES ── \*/ .section-title { font-size: 1.5em; font-weight: 800; color: #1e293b; margin: 48px 0 20px 0; padding-bottom: 12px; border-bottom: 3px solid #B5567A; display: flex; align-items: center; gap: 14px; } .section-title .num { background: linear-gradient(135deg, #6B3FA0, #B5567A); color: #fff; width: 38px; height: 38px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.55em; font-weight: 800; flex-shrink: 0; } h3 { color: #6B3FA0; font-size: 1.12em; font-weight: 700; margin: 28px 0 12px 0; } h4 { color: #B5567A; font-size: 0.98em; font-weight: 700; margin: 20px 0 10px 0; } /\* ── TABLES ── \*/ table { width: 100%; border-collapse: collapse; margin: 16px 0 24px 0; font-size: 0.88em; } thead th { background: linear-gradient(135deg, #6B3FA0, #7c4fb5); color: #fff; padding: 11px 14px; text-align: left; font-weight: 600; font-size: 0.85em; text-transform: uppercase; letter-spacing: 0.3px; } tbody td { padding: 10px 14px; border-bottom: 1px solid #e8ecf1; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } /\* ── CALLOUT BOXES ── \*/ .callout { border-radius: 10px; padding: 20px 24px; margin: 18px 0; border-left: 5px solid; } .callout-purple { background: #f8f6ff; border-color: #6B3FA0; } .callout-green { background: #f0fdf4; border-color: #16a34a; } .callout-amber { background: #fffbeb; border-color: #d97706; } .callout-red { background: #fef2f2; border-color: #dc2626; } .callout-blue { background: #eff6ff; border-color: #2563eb; } .callout h4 { margin-top: 0; } /\* ── KPI CARDS ── \*/ .kpi-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px; margin: 20px 0; } .kpi-card { background: linear-gradient(135deg, #f8f6ff 0%, #fdf2f8 100%); border: 1px solid #e8d5f5; border-radius: 12px; padding: 20px; text-align: center; } .kpi-card .kpi-value { font-size: 2em; font-weight: 800; color: #6B3FA0; } .kpi-card .kpi-label { font-size: 0.78em; color: #64748b; margin-top: 4px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; } /\* ── STATUS BADGES ── \*/ .badge-mandatory { background: #dc2626; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.78em; font-weight: 600; } .badge-recommended { background: #2563eb; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.78em; font-weight: 600; } .badge-optional { background: #64748b; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.78em; font-weight: 600; } .badge-critical { background: #7f1d1d; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.78em; font-weight: 600; } .badge-auto { background: #16a34a; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.78em; font-weight: 600; } .badge-phase { background: linear-gradient(135deg, #6B3FA0, #B5567A); color: #fff; padding: 3px 14px; border-radius: 14px; font-size: 0.82em; font-weight: 700; } /\* ── PROCESS STEPS ── \*/ .step-card { background: #fff; border: 1px solid #e2e8f0; border-radius: 10px; padding: 20px 24px; margin: 12px 0; border-left: 4px solid #6B3FA0; transition: box-shadow 0.2s; } .step-card:hover { box-shadow: 0 4px 16px rgba(107,63,160,0.1); } .step-card .step-num { background: #6B3FA0; color: #fff; width: 28px; height: 28px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; font-size: 0.78em; font-weight: 800; margin-right: 10px; } .step-card h4 { display: inline; color: #1e293b; margin: 0; } /\* ── FORMULA BOXES ── \*/ .formula-box { background: linear-gradient(135deg, #1e293b 0%, #334155 100%); color: #e2e8f0; border-radius: 10px; padding: 20px 28px; margin: 16px 0; font-family: 'Courier New', monospace; font-size: 0.95em; line-height: 2; } .formula-box .formula-label { color: #B5567A; font-weight: 700; font-family: 'Inter', sans-serif; font-size: 0.82em; text-transform: uppercase; letter-spacing: 0.5px; } .formula-box .formula-expr { color: #a5f3fc; font-size: 1.1em; font-weight: 600; } /\* ── APPROVAL MATRIX ── \*/ .approval-tier { display: grid; grid-template-columns: 60px 1fr 140px 160px; gap: 10px; align-items: center; padding: 14px 20px; border-bottom: 1px solid #e8ecf1; font-size: 0.9em; } .approval-tier:nth-child(even) { background: #f8fafc; } .tier-icon { width: 42px; height: 42px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 800; color: #fff; font-size: 0.82em; } /\* ── FOOTER ── \*/ .doc-footer { background: linear-gradient(135deg, #1a0533 0%, #2d1054 50%, #6B3FA0 100%); color: rgba(255,255,255,0.7); padding: 30px 60px; font-size: 0.78em; line-height: 1.8; } .doc-footer strong { color: #fff; } /\* ── PRINT ── \*/ @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; max-width: 100%; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .toc { page-break-after: always; } .section-title { page-break-before: always; } table { page-break-inside: avoid; } } /\* ── CHECKLIST ── \*/ .checklist { list-style: none; padding: 0; } .checklist li { padding: 8px 0 8px 28px; position: relative; border-bottom: 1px dotted #e2e8f0; } .checklist li::before { content: '☐'; position: absolute; left: 0; color: #6B3FA0; font-weight: 700; } .checklist li.checked::before { content: '☑'; color: #16a34a; } /\* ── RETENTION TABLE ── \*/ .retention-bar { height: 24px; border-radius: 4px; display: flex; overflow: hidden; margin: 4px 0; } .retention-bar .fill { height: 100%; } .fill-released { background: #16a34a; } .fill-held { background: #dc2626; } .fill-pending { background: #d97706; } /\* ── RESPONSIVE ── \*/ @media (max-width: 768px) { .content { padding: 20px; } .header-banner { padding: 30px 20px; } .meta-bar { padding: 14px 20px; } .toc-grid { grid-template-columns: 1fr; } .kpi-row { grid-template-columns: 1fr 1fr; } } /\* ── SAP REFERENCE TAG ── \*/ .sap-ref { background: #f1f5f9; border: 1px dashed #94a3b8; border-radius: 6px; padding: 10px 16px; margin: 8px 0; font-size: 0.82em; color: #475569; } .sap-ref code { background: #e2e8f0; padding: 1px 6px; border-radius: 3px; font-size: 0.92em; color: #6B3FA0; } :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,) Enterprise Process Blueprint

# Running Account Bill (RA Bill) Process

Contractor Payment Lifecycle against Work Orders — Casagrand Construction & Real Estate

Document Version: 3.0 Effective Date: 01 May 2026 Classification: CONFIDENTIAL Module: RE-FX / PS / MM / FI-AP SAP Reference System: S/4HANA (Hana PEOL)

Process Owner

VP — Projects & Commercial

Process Scope

All Casagrand Projects Pan-India

SAP Modules

PS, MM-SRV, FI-AP, RE-FX, CO-CCA

Currency

INR (Indian Rupee)

Reference Backend

S/4HANA Hana PEOL (Demo Reference)

Prepared By

C16 — Enterprise Process Intelligence

## 📋 Table of Contents

[01 Executive Summary & Process Overview](#s1) [02 Contractor Onboarding & Work Order Placement](#s2) [03 Running Account Bill — End-to-End Lifecycle](#s3) [04 Services — RA Bill for Service Contracts](#s4) [05 Materials — RA Bill for Material Supply](#s5) [06 Project Execution & Milestone Tracking](#s6) [07 Compliance Documents — Contractor Submissions](#s7) [08 Adjustment of Advances](#s8) [09 Money Retentions — Holdback & Release](#s9) [10 Escalations & De-Escalations](#s10) [11 Supporting Documents for Accounts Payable](#s11) [12 Reporting — % Work Completion vs % Amount Disbursed](#s12) [13 Approval Workflow & Authority Matrix](#s13) [14 SAP System Mapping & Technical Design](#s14) [15 Annexures, Templates & Checklists](#s15)

## 01 Executive Summary & Process Overview

The **Running Account Bill (RA Bill)** is the cornerstone financial instrument in construction project management. It represents a periodic claim submitted by a contractor against the value of work executed under a Work Order (WO), adjusted for mobilisation advances, material supplies, retention holdbacks, and contractual escalation/de-escalation clauses. For Casagrand — one of India's leading real estate developers operating across 25+ cities — the RA Bill process governs the disbursement of hundreds of crores annually to civil, MEP, interior, and specialty contractors.

This document establishes the **enterprise-grade standard operating procedure (SOP)** for the entire RA Bill lifecycle — from contractor onboarding through final bill settlement — covering services, materials, project execution monitoring, compliance, financial adjustments, and executive reporting. The process is designed to be SAP-enabled, with the reference backend (S/4HANA Hana PEOL) providing the architectural blueprint for table structures, document flows, and integration points.

8

Process Phases

24

Compliance Checkpoints

5-Tier

Approval Hierarchy

100%

Audit Trail Coverage

#### 🏗️ Process Scope — Casagrand RA Bill

This SOP covers: **(a)** Civil contractors, **(b)** MEP (Mechanical, Electrical, Plumbing) contractors, **(c)** Interior fit-out contractors, **(d)** Specialty subcontractors (waterproofing, façade, landscaping), **(e)** Material-only supply contracts, and **(f)** Labour-only contracts. The process applies uniformly across all Casagrand residential, commercial, and plotted development projects.

### 1.1 Process Flow — High-Level Architecture

flowchart LR A\[Contractor  
Onboarding\] --> B\[Work Order  
Placement\] B --> C\[Site Work  
Execution\] C --> D\[Physical  
Measurement\] D --> E\[RA Bill  
Submission\] E --> F\[Technical  
Verification\] F --> G\[Commercial  
Checking\] G --> H\[Compliance  
Validation\] H --> I\[Advance  
Adjustment\] I --> J\[Retention  
Deduction\] J --> K\[Escalation /  
De-Escalation\] K --> L\[Approval  
Workflow\] L --> M\[AP Posting  
& Payment\] M --> N\[MIS  
Reporting\] style A fill:#6B3FA0,color:#fff style E fill:#B5567A,color:#fff style H fill:#d97706,color:#fff style L fill:#16a34a,color:#fff style M fill:#2563eb,color:#fff

### 1.2 Key Terminology

Term

Full Form

Definition

**RA Bill**

Running Account Bill

Periodic interim payment claim by contractor for work completed up to measurement date

**WO**

Work Order

Formal purchase order/contract issued to the contractor specifying scope, rates, terms

**BOQ**

Bill of Quantities

Itemised list of works with quantities, rates, and units of measurement

**MB**

Measurement Book

Physical record of work measured on-site by the project engineer

**SES**

Service Entry Sheet

SAP document recording the acceptance of services (equivalent to Goods Receipt for services)

**GRN**

Goods Receipt Note

Confirmation of material delivery at project site (MIGO posting in SAP)

**Retention**

Retention Money

Percentage of bill value withheld as security, released after Defect Liability Period (DLP)

**DLP**

Defect Liability Period

Post-completion warranty period (typically 12-24 months) during which contractor must rectify defects

**RERA**

Real Estate Regulatory Authority

Statutory body governing project registration, timelines, and financial disclosures

**Mobilisation Advance**

—

Upfront payment to contractor (typically 5-10% of WO value) adjusted proportionally in subsequent RA Bills

**🔗 SAP Reference System:** Vendor master data structure referenced from `LFA1` (10 vendors, India geography), company code configuration from `T001`, payment terms from `T052` (10 configurations), and document types from `T161` (15 procurement document types including NB, FO, RV). These structures serve as the architectural blueprint for Casagrand's SAP landscape design.

## 02 Contractor Onboarding & Work Order Placement

### 2.1 Contractor Registration & Vendor Master

Every contractor engaged by Casagrand must be registered as a vendor in the SAP system before any Work Order can be issued. The vendor master record serves as the single source of truth for contractor identity, banking details, compliance status, and commercial terms. The registration process is a mandatory pre-requisite — no RA Bill can be processed without an active, unblocked vendor master record.

Data Category

SAP Table

Key Fields

Casagrand Requirement

Status

General Data

`LFA1`

LIFNR, NAME1, LAND1, ORT01, STRAS

Full legal name, registered address, PAN, Aadhaar (proprietor), GST registration

Mandatory

Company Code Data

`LFB1`

BUKRS, ZTERM, ZWELS, ZAHLS, AKONT

Payment terms per project company, reconciliation account (trade creditors), payment method

Mandatory

Bank Details

`LFBK`

BANKS, BANKL, BANKN, BKONT

Active bank account with IFSC, verified via cancelled cheque/bank letter

Mandatory

Purchase Organisation

`LFM1`

LIFNR, EKORG, ZTERM

Assigned to Casagrand's central/regional procurement organisation

Mandatory

Withholding Tax

`LFBW`

WITHT, WT\_WITHCD, WT\_AGENT

TDS under Section 194C (Contractors), 194J (Professional services) — auto-calculated at payment

Mandatory

#### Contractor Classification — Casagrand Categories

Category Code

Contractor Type

Typical WO Range (₹ Cr)

Payment Terms

Retention %

Advance %

CAT-A

Prime Civil Contractor

50 – 300+

Net 30 from RA Bill approval

5%

10%

CAT-B

MEP Contractor (Electrical / Plumbing / HVAC)

10 – 80

Net 30 from RA Bill approval

7.5%

5%

CAT-C

Interior & Fit-out Contractor

5 – 40

Net 21 from RA Bill approval

10%

5%

CAT-D

Specialty Subcontractor

1 – 15

Net 15 from RA Bill approval

10%

Nil

CAT-E

Material-Only Supplier

0.5 – 50

As per material type (refer PO)

Nil

Nil

CAT-F

Labour-Only Contractor

0.5 – 10

Weekly / Fortnightly

5%

Nil

### 2.2 Work Order (WO) Placement — SAP Purchase Order

A Work Order in Casagrand's context is an SAP Purchase Order (PO) of a specific document type mapped to the project. The WO contains the complete Bill of Quantities (BOQ), milestone schedule, commercial terms, retention clauses, escalation formula, and compliance requirements. Every RA Bill is processed _against_ a specific WO number — the WO is the parent reference document throughout the payment lifecycle.

**🔗 SAP Reference:** The backend supports document type `NB` (Standard PO, BSTYP=B) and `FO` (Framework/Blanket PO, BSTYP=B) per `T161` configuration. For Casagrand, recommend custom document types: `ZWO` (Work Order — Lumpsum), `ZWR` (Work Order — Rate Contract), `ZWM` (Work Order — Material Supply).

#### Work Order Structure — BOQ Line Items

WO Component

SAP Object

Description

Example

WO Header

EKKO

Overall WO value, contractor, project, validity dates, payment terms

WO-4500017890, Contractor: L&T Construction, Project: Casagrand Zenith, Validity: 24 months

BOQ Line Item

EKPO

Individual work item with quantity, unit, rate per unit, total value

Item 10: RCC M30 Grade Concrete — 5,000 Cum @ ₹6,200/Cum = ₹3,10,00,000

Schedule Line

EKET

Delivery/execution schedule per line item

Month 1-3: 1,200 Cum | Month 4-6: 1,800 Cum | Month 7-9: 2,000 Cum

Account Assignment

EKKN

Cost assignment to WBS element / cost center / profit center

WBS: PROJ-ZENITH-STRUCT-001 | Cost Center: CC-CHN-PROJ-ZENITH

Conditions

KONV / KONP

Pricing conditions — base price, escalation, retention, TDS, GST

ZRET: -5% retention | ZESC: +3.5% escalation | JIOC: 18% GST

Text

STXL

Long text with detailed scope description per item

"Excavation in all types of soil including disposal within 500m lead..."

#### ⚠️ Critical Rule — WO as Ceiling

The cumulative value of all RA Bills against a Work Order **shall never exceed** the total WO value (including approved variations/change orders). Any bill that would breach the WO ceiling must trigger a **Change Order (CO)** approval before processing. The SAP system must enforce this through PO tolerance checks.

## 03 Running Account Bill — End-to-End Lifecycle

### 3.1 RA Bill Lifecycle — Detailed Process Flow

flowchart TD subgraph PHASE1\["Phase 1: Field Measurement"\] A1\[Site Engineer conducts  
physical measurement\] --> A2\[Record in Measurement  
Book MB/MBook\] A2 --> A3\[Joint Measurement with  
Contractor Representative\] A3 --> A4\[Measurement Book signed  
by both parties\] end subgraph PHASE2\["Phase 2: RA Bill Preparation"\] B1\[Contractor prepares RA Bill  
based on MB quantities\] --> B2\[Attach: MB abstract, photos,  
test reports, compliance docs\] B2 --> B3\[Submit RA Bill to  
Project Site Office\] end subgraph PHASE3\["Phase 3: Technical Check"\] C1\[Site Engineer verifies  
quantities vs MB\] --> C2\[QA/QC verifies  
quality parameters\] C2 --> C3\[Project Manager reviews  
& certifies bill\] end subgraph PHASE4\["Phase 4: Commercial Processing"\] D1\[Commercial / QS team  
verifies rates vs BOQ\] --> D2\[Apply Escalation /  
De-Escalation formula\] D2 --> D3\[Deduct: Advance Recovery,  
Retention, Penalties\] D3 --> D4\[Calculate Net Payable  
after all adjustments\] end subgraph PHASE5\["Phase 5: Compliance & Approval"\] E1\[Verify GST, TDS,  
Labour Cess compliance\] --> E2\[Validate insurance,  
safety, RERA docs\] E2 --> E3\[Multi-tier approval  
per authority matrix\] end subgraph PHASE6\["Phase 6: AP & Payment"\] F1\[Create Service Entry Sheet  
SES in SAP\] --> F2\[Post Invoice in  
FI-AP MIRO\] F2 --> F3\[Payment Run F110  
via NEFT/RTGS\] F3 --> F4\[Payment Advice &  
Reconciliation\] end PHASE1 --> PHASE2 --> PHASE3 --> PHASE4 --> PHASE5 --> PHASE6 style PHASE1 fill:#f8f6ff,stroke:#6B3FA0 style PHASE2 fill:#fdf2f8,stroke:#B5567A style PHASE3 fill:#eff6ff,stroke:#2563eb style PHASE4 fill:#fffbeb,stroke:#d97706 style PHASE5 fill:#fef2f2,stroke:#dc2626 style PHASE6 fill:#f0fdf4,stroke:#16a34a

### 3.2 RA Bill Numbering Convention

Every RA Bill follows a structured numbering system that ensures traceability across projects, contractors, and billing cycles:

RA Bill Number Format

RA / {PROJECT\_CODE} / {CONTRACTOR\_CODE} / {TRADE} / {SEQUENCE}

  

Example

RA / CG-ZEN-CHN / LC001 / CIVIL / 007

→ 7th RA Bill for Civil work by Contractor LC001 at Casagrand Zenith, Chennai

### 3.3 RA Bill Components — Gross to Net Calculation

The RA Bill is not a simple invoice — it is a complex financial computation that starts with gross work value and arrives at net payable after multiple layers of adjustments. Understanding each component is critical for both the contractor and the Casagrand commercial team.

#

Component

Type

Calculation Basis

Typical %

SAP Condition Type

A

Gross Work Value (Current Bill)

Base

MB quantity × BOQ rate

100%

PBXX / ZP00

B

Cumulative Work Value (Up to Date)

Reference

Sum of all RA Bills to date

—

—

C

Escalation / De-Escalation

\+ / −

Per contractual formula (WPI / CPI index)

±2-8%

ZESC / ZDSC

D

Subtotal (A ± C)

Subtotal

—

—

—

E

Less: Retention Money

−

% of Subtotal D

5-10%

ZRET

F

Less: Mobilisation Advance Recovery

−

Pro-rata recovery from each RA Bill

10% of D

ZADV

G

Less: Material Advance Recovery

−

Against materials supplied by Casagrand

Variable

ZMAT

H

Less: Penalty / LD (Liquidated Damages)

−

Per contractual penalty clause

0-5%

ZPNL

I

Less: Previous RA Bill Payments

−

Sum of all prior net payments

—

—

J

Net Value Before Tax

Subtotal

D − E − F − G − H − I

—

—

K

Add: GST (CGST + SGST / IGST)

+

% of Net Value J

18%

JIOC / JICG + JISG

L

Less: TDS (u/s 194C / 194J)

−

% of Net Value J

1-2%

ZTDS

M

Less: Labour Cess

−

% of Gross Value A

1%

ZLCS

**N**

**Net Payable to Contractor**

**Final**

**J + K − L − M**

—

—

Net Payable Formula

Net Payable = (Gross\_Value ± Escalation − Retention − Advance\_Recovery − Material\_Recovery − Penalties − Previous\_Payments) + GST − TDS − Labour\_Cess

### 3.4 RA Bill — Cumulative vs Current Period

A critical concept in RA Bill processing is the **cumulative method**. Each RA Bill shows the total work done from inception to date, and the "current bill amount" is the difference between the current cumulative and the previous cumulative. This prevents double-counting and ensures the payment trail is always reconcilable.

#### 📐 Cumulative Method — Illustration

Bill #

Cumulative Work Value

Previous Cumulative

Current Bill Value

RA-001

₹ 1,20,00,000

₹ 0

₹ 1,20,00,000

RA-002

₹ 3,50,00,000

₹ 1,20,00,000

₹ 2,30,00,000

RA-003

₹ 5,80,00,000

₹ 3,50,00,000

₹ 2,30,00,000

RA-004

₹ 8,40,00,000

₹ 5,80,00,000

₹ 2,60,00,000

RA-005

₹ 10,00,00,000

₹ 8,40,00,000

₹ 1,60,00,000

WO Total Value = ₹ 10,00,00,000. RA-005 is the final bill. Cumulative = WO Value = 100% completion.

## 04 Services — RA Bill for Service Contracts

Service contracts form the bulk of Casagrand's contractor engagements. Unlike material procurement (which follows a GRN-based process), service contracts require a **Service Entry Sheet (SES)** as the acceptance document. The SES is the SAP equivalent of a "certified measurement" and is the trigger for invoice posting in Accounts Payable.

### 4.1 Service Categories in Casagrand Projects

Service Code

Service Category

Measurement Unit

Billing Frequency

SES Approver

SRV-CIV

Civil & Structural Works

Cum / Sqm / Rmt / Kg / No.

Monthly (based on MB)

Project Manager + QS

SRV-ELE

Electrical Installation

Point / Rmt / Nos / Lot

Milestone-based

MEP Manager

SRV-PLB

Plumbing & Fire Fighting

Point / Rmt / Nos

Milestone-based

MEP Manager

SRV-HVC

HVAC Installation

TR / Sqft / Nos

Per project phase

MEP Manager + PMC

SRV-INT

Interior Fit-out

Sqft / Nos / Lot

Flat-wise / Block-wise

Interior Project Lead

SRV-WPF

Waterproofing

Sqm / Rmt

Per activity completion

QA/QC Head

SRV-ELV

Elevator Installation

Nos / Lot

Milestone (shaft → install → commission)

MEP Manager + OEM

SRV-LND

Landscaping & External Development

Sqm / Nos / Lot

Phase completion

Landscape Architect

SRV-LAB

Labour Supply (Manpower)

Man-days / Nos

Weekly / Fortnightly

Site In-Charge

SRV-SAF

Safety & Barricading

Rmt / Sqm / Lot

Monthly

Safety Officer

### 4.2 Service Entry Sheet (SES) — SAP Process

1

#### Create SES (ML81N)

The Quantity Surveyor (QS) creates a Service Entry Sheet against the Work Order PO in SAP transaction **ML81N**. Each SES line maps to a BOQ item with measured quantity. The SES can have multiple service lines covering different BOQ items within the same bill period.

2

#### Technical Acceptance

The SES triggers a workflow notification to the Project Manager (PM) for technical review. The PM verifies: (a) quantities match the signed Measurement Book, (b) quality parameters are met (cube test results, inspection reports), (c) no defective work is being claimed.

3

#### Release SES

After PM certification, the SES is released (accepted). Released SES status = "Accepted" in SAP. This is equivalent to Goods Receipt for services and creates the accrual posting in FI.

4

#### Invoice Verification (MIRO)

The contractor's RA Bill invoice is posted in **MIRO** with reference to the released SES. The system auto-populates quantities and values from the SES. Commercial adjustments (escalation, retention, advance recovery) are applied as pricing conditions.

#### ✅ Three-Way Match for Services

**Work Order (PO) ↔ Service Entry Sheet (SES) ↔ Invoice (MIRO)**. All three must agree on quantities and values within tolerance limits. Any variance beyond ±2% triggers automatic hold for QS review.

## 05 Materials — RA Bill for Material Supply

While the majority of RA Bills are for services (labour + materials combined), certain contracts are structured as **material-only supply** — where the contractor supplies specific building materials (ready-mix concrete, steel reinforcement, façade panels, tiles, sanitary ware) and Casagrand's own workforce or another contractor installs them. Additionally, some service contracts have a **material component** where Casagrand free-issues materials to the contractor and recovers the cost from RA Bills.

### 5.1 Material Supply — Two Scenarios

Scenario

Description

SAP Process

RA Bill Impact

**A: Contractor Supplies Material**

Contractor procures and delivers material to site as part of WO scope. Material cost is embedded in BOQ rates.

GRN via MIGO → Invoice via MIRO → Payment via F110. Standard 3-way match: PO ↔ GRN ↔ Invoice

RA Bill includes material value in gross work value. No separate material recovery.

**B: Casagrand Free-Issues Material**

Casagrand procures materials directly (cement, steel, bricks) and issues to contractor. Contractor's WO covers labour-only or reduced-rate items.

Casagrand PO to material supplier → GRN at site → Issue to contractor via 261 movement type (consumption posting). Recovery from contractor's RA Bill.

Material cost recovered from contractor's RA Bill as "Material Advance Recovery" (Component G). Pro-rata or at actual consumption.

### 5.2 Material Reconciliation in RA Bills

For Scenario B (free-issue materials), a rigorous reconciliation is required with every RA Bill submission:

#

Reconciliation Item

Source Document

Verified By

Frequency

1

Material issued to contractor (cumulative)

SAP Material Document (261 posting)

Stores In-Charge

Every RA Bill

2

Material consumed in measured work

Theoretical consumption from BOQ norms

QS / Project Engineer

Every RA Bill

3

Material in contractor's custody (unconsumed)

Physical stock count at contractor's staging area

Stores + Site Engineer

Every RA Bill

4

Wastage / excess consumption

Variance = Issued − Consumed − In Custody

Project Manager

Every RA Bill

5

Recovery value

Issued quantity × issue rate from Casagrand PO

Commercial / QS

Every RA Bill

#### 🚨 Material Wastage Rule

If material wastage exceeds the **permissible limit** (typically 2-5% depending on material type as per IS codes), the excess wastage value is **deducted as penalty** from the contractor's RA Bill. The permissible wastage limits must be defined in the Work Order terms. Example: Cement wastage > 3% → contractor bears excess cost at ₹ 420/bag (current market rate).

### 5.3 Key Materials Tracked in Casagrand Projects

Material Group

Materials

UOM

Wastage Limit

Recovery Method

Cement

OPC 53 Grade, PPC, Slag Cement

MT / Bags

3%

Per bag at issue rate

Steel

TMT Bars (Fe500D), MS Plates, Structural Steel

MT / Kg

2%

Per Kg at issue rate

Aggregates

20mm, 12mm Coarse Aggregate, M-Sand, River Sand

Cum / MT

5%

Per Cum at issue rate

Concrete

RMC M20 to M60 Grade

Cum

2.5%

Per Cum at batch plant rate

Bricks / Blocks

AAC Blocks, Fly Ash Bricks, Red Bricks

Nos / Sqm

3%

Per unit at issue rate

Tiles / Stone

Vitrified, Ceramic, Granite, Marble

Sqm / Sqft

5%

Per Sqft at issue rate

Pipes / Fittings

PVC, CPVC, GI, DI, HDPE Pipes

Rmt / Nos

3%

Per Rmt at issue rate

Electrical

Cables, Conduits, MCBs, Switches, Panels

Rmt / Nos / Lot

2%

Per unit at issue rate

## 06 Project Execution & Milestone Tracking

The RA Bill process is intrinsically linked to project execution progress. Every RA Bill must demonstrate measurable progress against the project schedule. Casagrand uses a **WBS (Work Breakdown Structure)** in SAP PS (Project System) to track physical and financial progress at multiple levels — from overall project down to individual tower, floor, and trade.

### 6.1 WBS Structure — Casagrand Standard Template

graph TD P\[Project: CG-ZEN-CHN  
Casagrand Zenith Chennai\] --> T1\[Tower A  
CG-ZEN-CHN-TA\] P --> T2\[Tower B  
CG-ZEN-CHN-TB\] P --> T3\[Common Areas  
CG-ZEN-CHN-CA\] P --> T4\[External Dev  
CG-ZEN-CHN-ED\] T1 --> T1A\[Substructure  
CG-ZEN-TA-SUB\] T1 --> T1B\[Superstructure  
CG-ZEN-TA-SUP\] T1 --> T1C\[MEP  
CG-ZEN-TA-MEP\] T1 --> T1D\[Finishing  
CG-ZEN-TA-FIN\] T1B --> F1\[Floors G to 5  
CG-ZEN-TA-SUP-01\] T1B --> F2\[Floors 6 to 10  
CG-ZEN-TA-SUP-02\] T1B --> F3\[Floors 11 to 15  
CG-ZEN-TA-SUP-03\] T1B --> F4\[Terrace  
CG-ZEN-TA-SUP-04\] style P fill:#6B3FA0,color:#fff style T1 fill:#B5567A,color:#fff style T2 fill:#B5567A,color:#fff style T3 fill:#B5567A,color:#fff style T4 fill:#B5567A,color:#fff

### 6.2 Milestone-Based Billing — Standard Milestones

For certain contract types (elevator, DG sets, HVAC systems, prefab elements), billing follows predefined milestones rather than running measurements. Each milestone triggers a specific percentage of the WO value:

Milestone #

Milestone Description

% of WO Value

Cumulative %

Verification Required

MS-1

Contract Award / Mobilisation

10%

10%

WO signed, advance bank guarantee received

MS-2

Material Delivery at Site

30%

40%

GRN, material inspection report, test certificates

MS-3

Installation / Erection Complete

25%

65%

Installation certificate, inspection by OEM

MS-4

Testing & Commissioning

20%

85%

Commissioning report, performance test certificate

MS-5

Handover & Documentation

10%

95%

O&M manual, warranty certificate, as-built drawings

MS-6

DLP Completion (12 months)

5%

100%

DLP completion certificate, final inspection

### 6.3 Physical Progress Tracking — % Completion Method

The cornerstone metric for Casagrand's project monitoring is the relationship between **Physical % Completion** and **Financial % Disbursement**. This dual-axis tracking prevents over-billing and ensures the contractor's payment is proportional to actual work delivered. The calculation methodology is:

Physical Completion %

Physical % = (Σ Measured\_Qty × Weightage\_Factor) / (Σ BOQ\_Qty × Weightage\_Factor) × 100

  

Financial Disbursement %

Financial % = (Cumulative\_Net\_Payments) / (Total\_WO\_Value) × 100

  

Health Ratio

Health Ratio = Physical % / Financial %

→ Ratio > 1.0 = Under-billed (healthy) | Ratio < 1.0 = Over-billed (risk) | Ratio = 1.0 = On track

## 07 Compliance Documents — Contractor Submissions

Casagrand mandates a comprehensive set of compliance documents from every contractor as a **pre-condition for RA Bill processing**. No RA Bill shall be approved or paid without all mandatory compliance documents being current and valid. This is not just a contractual requirement — it protects Casagrand from statutory, legal, and safety liabilities under the Building and Other Construction Workers Act, GST Act, Income Tax Act, and RERA regulations.

### 7.1 Compliance Document Checklist

#

Document

Statutory Basis

Validity

Submission

Consequence of Non-Compliance

1

GST Registration Certificate (GSTIN)

GST Act, 2017

Active

Mandatory — at onboarding

WO cannot be issued; vendor blocked

2

GST Monthly Return Filing (GSTR-3B)

GST Act, 2017

Monthly

Mandatory — every RA Bill

RA Bill held until filed; input credit at risk

3

PAN Card (Company / Proprietor)

Income Tax Act

Perpetual

Mandatory — at onboarding

TDS at higher rate (20%) u/s 206AA

4

TDS Compliance — 26AS / AIS Verification

Income Tax Act

Annual

Recommended

Mismatch in TDS credit claims

5

Labour License (CLRA)

CLRA Act, 1970

12 months

Mandatory — every RA Bill

RA Bill held; Casagrand exposed to principal employer liability

6

PF (Provident Fund) Challan & ECR

EPF Act, 1952

Monthly

Mandatory — every RA Bill

RA Bill held; Casagrand liable as principal employer

7

ESI Challan

ESI Act, 1948

Monthly

Mandatory — every RA Bill

RA Bill held; statutory penalty risk

8

Labour Cess Payment Receipt

Building Workers' Cess Act, 1996

Per project

Mandatory — per project registration

Project completion certificate at risk

9

Workmen Compensation Insurance

Workmen's Compensation Act, 1923

Annual

Mandatory — every RA Bill

RA Bill held; Casagrand exposed to compensation claims

10

All-Risk Insurance (CAR / EAR Policy)

Contract Clause

Project duration

Mandatory — at mobilisation

Advance release blocked

11

Third-Party Liability Insurance

Contract Clause

Project duration

Recommended

Risk retained by Casagrand

12

BOCW Registration (Workers)

BOCW Act, 1996

Monthly roster

Mandatory — monthly

RA Bill held; statutory penalty

13

Safety Training Records

BOCW Act / ISO 45001

Monthly

Mandatory — every RA Bill

Safety hold on payments

14

Material Test Certificates

IS Code / Contract Spec

Per batch / lot

Mandatory — per RA Bill

Work not measured until tests pass

15

Quality Inspection Reports

Contract Spec / ISO 9001

Per activity

Mandatory — per RA Bill

Defective work not measured

16

Bank Guarantee (Performance / Advance)

Contract Clause

WO validity + DLP

Mandatory — at mobilisation

Advance not released; payments capped

17

MSME Registration (Udyam)

MSMED Act, 2006

Perpetual

Recommended

Payment within 45 days mandatory for MSME

18

e-Way Bill (for material movements)

GST Act, 2017

Per consignment

Mandatory — material deliveries > ₹50,000

Material rejected at gate

### 7.2 Compliance Verification Matrix — RA Bill Gateway

flowchart LR RA\[RA Bill  
Submitted\] --> CG{Compliance  
Gateway} CG -->|All Clear| AP\[Proceed to  
Approval\] CG -->|GST Non-Filing| H1\[HOLD — GST  
Block Code: ZG\] CG -->|PF/ESI Missing| H2\[HOLD — Labour  
Block Code: ZL\] CG -->|Insurance Expired| H3\[HOLD — Safety  
Block Code: ZS\] CG -->|Test Reports Missing| H4\[HOLD — Quality  
Block Code: ZQ\] H1 --> R1\[Contractor notified  
7-day cure period\] H2 --> R2\[Contractor notified  
7-day cure period\] H3 --> R3\[Contractor notified  
Immediate stoppage\] H4 --> R4\[Re-inspection  
scheduled\] style CG fill:#d97706,color:#fff style AP fill:#16a34a,color:#fff style H1 fill:#dc2626,color:#fff style H2 fill:#dc2626,color:#fff style H3 fill:#dc2626,color:#fff style H4 fill:#dc2626,color:#fff

## 08 Adjustment of Advances

Advances are a critical financial mechanism in construction contracting. They provide the contractor with working capital to mobilise resources, procure materials, and set up site infrastructure. However, advances represent a **financial risk to Casagrand** — every rupee advanced must be systematically recovered through RA Bills and secured through bank guarantees. This section defines the types of advances, their limits, recovery mechanisms, and SAP posting methodology.

### 8.1 Types of Advances

Advance Type

Purpose

Limit

Security Required

Recovery Method

SAP Document Type

**Mobilisation Advance**

Initial setup — labour camps, equipment mobilisation, site office

5-10% of WO Value

Bank Guarantee (110% of advance)

Pro-rata recovery from each RA Bill at 10% until fully recovered

Down Payment — F-48 (Special GL Indicator: A)

**Material Advance**

Bulk material procurement by contractor (steel, cement, pipes)

Up to 75% of material PO value

Material at site (physical custody)

Adjusted against material delivery GRNs or proportional to consumption

Down Payment — F-48 (Special GL Indicator: M)

**Secured Advance**

Against materials brought to site but not yet incorporated in work

75% of material value at site

Physical stock verification certificate

Adjusted as material is consumed and measured in RA Bill

Down Payment — F-48 (Special GL Indicator: S)

**Machinery Advance**

For heavy equipment procurement / leasing (tower cranes, batching plants)

Case-by-case (Board approval)

Equipment hypothecation / insurance assignment

Monthly EMI deduction from RA Bill

Down Payment — F-48 (Special GL Indicator: E)

### 8.2 Advance Recovery Calculation

Mobilisation Advance Recovery — Per RA Bill

Recovery\_Amount = Gross\_RA\_Bill\_Value × Recovery\_Rate%

  

Where Recovery Rate

Recovery\_Rate% = (Advance\_Amount / WO\_Value) × (100% / (100% − Advance%))

  

Example — ₹10 Cr WO, 10% Advance (₹1 Cr)

Recovery per RA Bill = Gross Value × 11.11% (so that advance is fully recovered by ~90% completion)

The 11.11% rate ensures the ₹1 Cr advance is fully recovered by the time 90% of the work (₹9 Cr) is billed. Remaining 10% WO value (₹1 Cr) has no recovery deduction.

### 8.3 Advance Recovery — Illustrative Schedule

RA Bill #

Gross Value (₹ Lakhs)

Recovery @ 11.11%

Cumulative Recovery

Advance Balance

Advance Paid

—

—

₹ 0

₹ 100.00

RA-001

₹ 120.00

₹ 13.33

₹ 13.33

₹ 86.67

RA-002

₹ 230.00

₹ 25.55

₹ 38.88

₹ 61.12

RA-003

₹ 230.00

₹ 25.55

₹ 64.43

₹ 35.57

RA-004

₹ 260.00

₹ 28.89

₹ 93.32

₹ 6.68

RA-005

₹ 160.00

₹ 6.68 (balance)

₹ 100.00

₹ 0.00 ✅

#### ⚠️ Advance Recovery — Safeguards

*   If cumulative recovery exceeds advance amount (rounding), **cap recovery at advance balance** — never over-recover.
*   If contractor is terminated mid-contract, **unrecovered advance** is claimed against the bank guarantee.
*   Bank guarantee must remain valid until advance is fully recovered + 3 months buffer.
*   If BG expires before full recovery, **all payments halted** until BG is renewed.

### 8.4 SAP Posting — Advance & Recovery

Transaction

SAP Tcode

Debit

Credit

Document Type

Advance Payment

F-48

Vendor Advance (Special GL — Advances Paid)

Bank Account

KA (Vendor Down Payment)

RA Bill Invoice Posting

MIRO

WBS Element / Cost Center (Project Cost)

Vendor Payable (Reconciliation A/c)

RE (Invoice Receipt)

Advance Recovery (Clearing)

F-54

Vendor Payable (Reconciliation A/c)

Vendor Advance (Special GL — Advances Paid)

KR (Vendor Down Payment Clearing)

Net Payment to Contractor

F110

Vendor Payable (Reconciliation A/c)

Bank Account

ZP (Payment)

**🔗 SAP Reference:** Reconciliation account `160000` per LFB1 configuration in the reference system. All 10 vendors mapped to account `0000160000` (Trade Creditors). Payment terms range from `0001` (immediate) to `0008` (14/30/45 day terms) per T052 configuration. Special GL indicators (A/M/S/E) manage advance sub-ledger segregation.

## 09 Money Retentions — Holdback & Release

Retention money is a critical risk mitigation tool in construction contracts. It creates a financial buffer that incentivises the contractor to complete the work to specification, rectify defects during the warranty period, and fulfil all contractual obligations through final handover. Casagrand's retention policy follows FIDIC / Indian construction industry norms with project-specific customisation.

### 9.1 Retention Structure

Parameter

CAT-A (Prime Civil)

CAT-B (MEP)

CAT-C (Interior)

CAT-D (Specialty)

Retention %

5%

7.5%

10%

10%

Maximum Retention Cap

5% of WO value

7.5% of WO value

10% of WO value

10% of WO value

Retention Deducted From

Each RA Bill (Gross − Escalation)

Each RA Bill (Gross)

Each RA Bill (Gross)

Each RA Bill (Gross)

First Release (50%)

On Substantial Completion Certificate

On System Commissioning

On Flat Handover

On Activity Completion

Final Release (50%)

DLP Expiry (12 months)

DLP Expiry (18 months)

DLP Expiry (12 months)

DLP Expiry (24 months)

Substitution Allowed

Yes — with BG at 110% of retention

Yes — with BG at 110%

No

No

### 9.2 Retention Lifecycle — Visual

gantt title Retention Money Lifecycle — CAT-A Civil Contractor dateFormat YYYY-MM-DD axisFormat %b %Y section Accumulation RA-001 Retention Deducted :a1, 2026-01-15, 1d RA-002 Retention Deducted :a2, 2026-02-15, 1d RA-003 Retention Deducted :a3, 2026-03-15, 1d RA-004 through RA-012 :a4, 2026-04-15, 270d Retention Cap Reached :milestone, 2027-01-15, 0d section Hold Period Substantial Completion :milestone, 2027-06-01, 0d 50% Release Point :b1, 2027-06-01, 1d DLP Period (12 months) :b2, 2027-06-01, 365d section Release Final 50% Release :c1, 2028-06-01, 1d DLP Expiry :milestone, 2028-06-01, 0d

### 9.3 Retention Release — Approval & SAP Posting

Release Stage

Trigger Event

Documentation Required

Approver

SAP Process

50% Release

Substantial Completion Certificate issued

Completion certificate, snag list closure report, as-built drawings

VP Projects + CFO

Credit note / debit to retention provision → payment via F110

Final 50% Release

DLP expiry + no outstanding defects

DLP inspection report, defect rectification certificate, NOC from Casagrand maintenance

VP Projects + CFO + MD (if > ₹1 Cr)

Reverse retention provision → payment via F110

#### 🚨 Retention Forfeiture Triggers

*   Contractor abandons work mid-contract
*   Persistent quality defects not rectified within cure period (3 notices)
*   Contractor's insolvency / winding up / NCLT proceedings
*   Material safety violations (fatal accident, structural failure attributable to contractor)
*   Fraud / misrepresentation in RA Bill claims (inflated quantities, falsified test reports)

In all forfeiture scenarios, the retention amount is **set off against Casagrand's costs** to complete the pending work / rectify defects through a replacement contractor.

## 10 Escalations & De-Escalations

Construction contracts spanning 12+ months are subject to price fluctuations in key input materials (cement, steel, fuel, labour wages). To share this risk fairly between Casagrand and the contractor, Work Orders include a **Price Variation Clause (PVC)** — also called an Escalation/De-Escalation clause. This clause adjusts the RA Bill value based on movement in published price indices.

### 10.1 Escalation Formula — CPWD / IRC Standard

Price Adjustment Factor (PAF)

PAF = 0.85 × \[(W₁ × L₁/L₀) + (W₂ × C₁/C₀) + (W₃ × S₁/S₀) + (W₄ × F₁/F₀) + (W₅ × B₁/B₀)\] + 0.15

  
Where:  
W₁ to W₅ = Weightage factors for Labour, Cement, Steel, Fuel, Bitumen (sum = 1.0)  
L₁, C₁, S₁, F₁, B₁ = Current period index values  
L₀, C₀, S₀, F₀, B₀ = Base period index values (at WO award date)  
0.15 = Fixed component (15% of WO value is non-escalable — overheads, profit)

### 10.2 Index Sources & Weightage

Component

Index Source

Published By

Frequency

Civil WO Weightage

MEP WO Weightage

**Labour (L)**

Consumer Price Index — Industrial Workers (CPI-IW)

Labour Bureau, Shimla

Monthly

25%

30%

**Cement (C)**

Wholesale Price Index — Cement (WPI Code: 2394)

Office of Economic Adviser, DPIIT

Monthly

20%

5%

**Steel (S)**

WPI — Bars & Rods of Iron/Steel (WPI Code: 2312)

Office of Economic Adviser, DPIIT

Monthly

30%

15%

**Fuel / Energy (F)**

WPI — High Speed Diesel Oil (WPI Code: 2111)

Office of Economic Adviser, DPIIT

Monthly

10%

10%

**Copper / Aluminium (B)**

WPI — Copper (2541) / Aluminium (2531)

Office of Economic Adviser, DPIIT

Monthly

0%

25%

**Other / Bitumen**

WPI — Bitumen (2113) / All Commodities

Office of Economic Adviser / Indian Oil Corp

Monthly

15%

15%

### 10.3 Escalation Calculation — Worked Example

Component

Weightage (W)

Base Index (I₀)

Current Index (I₁)

Ratio (I₁/I₀)

W × Ratio

Labour

0.25

340.2

362.7

1.0661

0.2665

Cement

0.20

155.4

168.9

1.0869

0.2174

Steel

0.30

142.8

156.3

1.0945

0.3284

Fuel

0.10

128.6

134.2

1.0435

0.1044

Other

0.15

118.9

121.5

1.0219

0.1533

Σ (W × Ratio)

**1.0700**

PAF Calculation

PAF = 0.85 × 1.0700 + 0.15 = 0.9095 + 0.15 = 1.0595

  

Escalation %

Escalation = (PAF − 1) × 100 = 5.95%

  

If Current RA Bill Gross = ₹ 2,30,00,000

Escalation Amount = ₹ 2,30,00,000 × 5.95% = ₹ 13,68,500

#### 📊 De-Escalation — Symmetrical Application

If indices fall below the base period (PAF < 1.0), the **same formula applies symmetrically** — resulting in a **negative adjustment (de-escalation)** that reduces the RA Bill value. Casagrand's policy mandates that escalation/de-escalation applies only when the PAF deviates by more than **±2%** from 1.0 (dead band). Within the ±2% band, no adjustment is made.

### 10.4 Escalation Governance Rules

#

Rule

Description

1

Cap on Escalation

Maximum escalation payable per RA Bill period shall not exceed **10% of the base BOQ rate**. Any excess escalation requires VP-Projects approval.

2

Index Verification

Indices must be verified from the **official published source** (Labour Bureau / DPIIT website). Contractor's claims based on market surveys are not accepted.

3

Retrospective Application

If the index for a billing period is not yet published, the **last published index** is used provisionally. Adjustment is made in the next RA Bill when actual index is available.

4

Fixed-Price Contracts

For contracts ≤ 12 months duration, escalation clause is **not applicable**. Rates are firm and fixed.

5

Escalation on Delayed Work

If the contractor is behind schedule (as per approved bar chart), escalation is calculated on indices applicable to the **originally scheduled period**, not the actual (delayed) period. The contractor bears the price risk for their own delay.

## 11 Supporting Documents for Accounts Payable

The Accounts Payable (AP) department is the final gatekeeper before payment. AP must verify that every RA Bill is accompanied by the complete set of supporting documents, properly authorised, and arithmetically correct. This section defines the **mandatory document pack** that must accompany every RA Bill submission to AP.

### 11.1 RA Bill Document Pack — Checklist

#

Document

Prepared By

Verified By

Status

AP Action

1

**RA Bill Cover Sheet** (Summary of Gross → Net)

QS / Commercial

Project Manager

Mandatory

Verify arithmetic, match with SES

2

**Measurement Book Abstract** (MB summary with quantities per BOQ item)

Site Engineer

QS

Mandatory

Cross-check quantities vs SES

3

**Joint Measurement Sheet** (Signed by Casagrand + Contractor)

Site Engineer + Contractor

Project Manager

Mandatory

Verify signatures

4

**Contractor's Tax Invoice** (GST-compliant)

Contractor

AP

Mandatory

Verify GSTIN, HSN/SAC, tax rates, invoice format

5

**Service Entry Sheet (SAP printout)**

QS (SAP)

PM

Mandatory

Match SES # with invoice reference

6

**Escalation Calculation Sheet** (if applicable)

QS / Commercial

Head-Commercial

Mandatory (if escalation applies)

Verify indices, formula, arithmetic

7

**Advance Recovery Statement** (cumulative)

Finance / Commercial

AP Lead

Mandatory

Match with advance ledger balance

8

**Retention Statement** (cumulative)

Finance / Commercial

AP Lead

Mandatory

Match with retention provision ledger

9

**Material Reconciliation Statement**

Stores + QS

Project Manager

Mandatory (if free-issue material)

Verify recovery amounts

10

**Penalty / LD Deduction Note**

Project Manager

VP-Projects

Mandatory (if penalty levied)

Verify approval and calculation

11

**Progress Photographs** (date-stamped)

Site Engineer

Project Manager

Recommended

Visual verification

12

**Quality Test Reports** (concrete cubes, soil compaction, etc.)

Third-party lab / OEM

QA/QC Head

Mandatory

Verify test pass certificates

13

**Compliance Documents** (GST return, PF/ESI challan, insurance)

Contractor

Compliance Officer

Mandatory

Verify validity, currency

14

**Approval Signatures** (Authority matrix sign-off)

—

Per authority matrix

Mandatory

Verify all levels signed

15

**Work Order Copy** (for first RA Bill only)

Procurement

Commercial

Mandatory (RA-001 only)

File for reference

### 11.2 AP Processing Timeline — SLA

Activity

Owner

SLA (Working Days)

Cumulative Days

RA Bill received at site office

QS / Site Admin

Day 0

0

Technical verification (quantities + quality)

Site Engineer + QA/QC

5 days

5

Commercial checking (rates + adjustments)

QS / Commercial

3 days

8

Compliance verification

Compliance Officer

2 days

10

Approval workflow (multi-tier)

PM → VP → CFO

5 days

15

SES creation + invoice posting in SAP

AP Team

3 days

18

Payment run (F110)

Treasury

2 days

20

**Total: Submission to Payment**

—

**20 working days**

**20**

#### ✅ MSME Contractor Payment Rule

For contractors registered under the **Micro, Small and Medium Enterprises Development (MSMED) Act, 2006**, payment must be made within **45 days of acceptance** of goods/services. Delay beyond 45 days attracts **compound interest at 3× RBI bank rate** (Section 16 of MSMED Act). Casagrand's AP team must flag MSME vendors for priority processing in the payment run.

## 12 Reporting — % Work Completion vs % Amount Disbursed

The most critical management report in the RA Bill process is the **% Completion vs % Disbursement Analysis**. This dual-axis report is the primary tool for Casagrand's leadership to detect over-billing, under-billing, cash flow imbalances, and project financial health — in real time. The report operates at multiple levels: overall project, per-tower, per-trade, and per-contractor.

### 12.1 Report Structure — Executive Dashboard

Physical %  
65.4%

Weighted Avg Completion

Financial %  
58.2%

Cumulative Disbursement

Health Ratio  
1.12

Physical / Financial (>1 = Healthy)

Variance  
+7.2%

Under-billed (Favourable)

### 12.2 Contractor-Level Comparison

Contractor

Trade

WO Value (₹ Cr)

Physical %

Financial %

Health Ratio

Status

M/s ABC Constructions

Civil

85.00

72.3%

65.1%

1.11

✅ Healthy

M/s XYZ Electricals

Electrical

18.50

55.0%

58.4%

0.94

⚠️ Monitor

M/s PQR Plumbing

Plumbing

12.20

68.5%

62.0%

1.10

✅ Healthy

M/s LMN Interiors

Interior

22.00

40.2%

48.7%

0.83

🚨 Over-Billed

M/s RST Elevators

Elevator

8.50

65.0%

65.0%

1.00

✅ On Track

M/s UVW Waterproofing

Waterproofing

3.80

90.0%

78.5%

1.15

✅ Healthy

### 12.3 Alert Thresholds & Automated Triggers

Health Ratio Range

Status

Colour Code

Automated Action

\> 1.10

Under-Billed (Healthy)

🟢 Green

None — standard processing

1.00 – 1.10

On Track

🟢 Green

None

0.90 – 1.00

Marginal Over-Bill

🟡 Amber

Auto-alert to QS Head for review

0.80 – 0.90

Over-Billed (Risk)

🔴 Red

Auto-hold next RA Bill for VP review. Detailed measurement audit triggered.

< 0.80

Critically Over-Billed

⚫ Black

All payments frozen. Independent measurement audit. Potential fraud investigation.

### 12.4 Report Types & Frequency

Report Name

Level

Frequency

Audience

SAP Source

Project Dashboard — Completion vs Disbursement

Overall Project

Weekly

MD, VP-Projects, CFO

PS Network + FI-AP Open Items

Contractor Wise RA Bill Tracker

Per Contractor

Monthly

Project Manager, QS Head

EKKO/EKPO + SES + BSIK/BSAK

Trade-Wise Progress Report

Per Trade (Civil/MEP/Interior)

Monthly

VP-Projects, PMC

PS WBS + CJ30 reports

Advance & Retention Statement

Per Contractor

Monthly

CFO, AP Lead, Internal Audit

Special GL Balances (FBL1N)

Over-Billing Risk Report

All Contractors (filtered by Health Ratio)

Monthly

CFO, VP-Projects, Internal Audit

Custom Z-Report / Fiori Tile

MSME Payment Compliance

MSME Contractors

Monthly

CFO, Company Secretary

FBL1N with MSME flag + aging

RERA Financial Progress

Per RERA-registered Project

Quarterly

RERA Compliance Officer

PS Budget vs Actual (CJ30/CJI3)

Escalation / De-Escalation Summary

Per Contractor

Quarterly

VP-Projects, Head-Commercial

Pricing Condition Analysis (ME2M variants)

### 12.5 Graphical Report — S-Curve Analysis

The S-Curve is the industry-standard visualisation for comparing planned vs actual progress (physical and financial) over time. Casagrand generates monthly S-Curves at the project level and per major contractor.

xychart-beta title "Project S-Curve: Physical % vs Financial % (Monthly)" x-axis \["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"\] y-axis "Cumulative %" 0 --> 100 line "Physical %" \[5, 12, 22, 30, 38, 48, 55, 62, 70, 78, 88, 100\] line "Financial %" \[3, 9, 18, 26, 34, 42, 50, 58, 64, 72, 82, 95\] line "Planned %" \[8, 16, 24, 32, 40, 48, 56, 64, 72, 80, 90, 100\]

## 13 Approval Workflow & Authority Matrix

### 13.1 Delegation of Authority — RA Bill Approval

Level

Approver

RA Bill Value Limit

Cumulative WO Value Limit

Escalation Authority

L1

Project Manager

Up to ₹ 25 Lakhs

Up to ₹ 2 Cr

No escalation approval

L2

Regional Head / VP-Projects

Up to ₹ 1 Cr

Up to ₹ 25 Cr

Up to ₹ 10 Lakhs

L3

Chief Financial Officer (CFO)

Up to ₹ 5 Cr

Up to ₹ 100 Cr

Up to ₹ 50 Lakhs

L4

Managing Director (MD)

Above ₹ 5 Cr

Above ₹ 100 Cr

Above ₹ 50 Lakhs

L5

Board of Directors / Investment Committee

— (Policy exceptions only)

New WO > ₹ 200 Cr

Policy-level escalation formula changes

### 13.2 Workflow in SAP

flowchart LR A\[QS Creates SES  
ML81N\] --> B{Bill Value  
≤ ₹25L?} B -->|Yes| C\[L1: PM Approves\] B -->|No| D{Bill Value  
≤ ₹1Cr?} D -->|Yes| E\[L1: PM → L2: VP\] D -->|No| F{Bill Value  
≤ ₹5Cr?} F -->|Yes| G\[L1 → L2 → L3: CFO\] F -->|No| H\[L1 → L2 → L3 → L4: MD\] C --> I\[AP Posts Invoice  
MIRO\] E --> I G --> I H --> I I --> J\[Payment Run  
F110\] style A fill:#6B3FA0,color:#fff style I fill:#2563eb,color:#fff style J fill:#16a34a,color:#fff

### 13.3 Special Approval Scenarios

Scenario

Additional Approval Required

Documentation

RA Bill with Escalation > 5%

VP-Projects + Head-Commercial joint approval

Index verification sheet, CPWD comparison

Health Ratio < 0.90 (Over-billing alert)

VP-Projects must review and certify before AP processing

Re-measurement certificate, photo evidence

Deviation from BOQ rates

VP-Projects + CFO + Procurement Head

Deviation note with justification, market rate comparison

First RA Bill against new WO

Normal hierarchy + Procurement Head (rate verification)

WO copy, BOQ, rate analysis

Final Bill (100% completion)

Normal hierarchy + QA/QC Head + Maintenance Head

Completion certificate, snag closure, handover checklist

RA Bill after Termination

VP-Projects + CFO + Legal

Termination letter, site inspection, settlement calculation

## 14 SAP System Mapping & Technical Design

This section maps the entire RA Bill process to SAP objects, transactions, and data structures. The reference system (S/4HANA Hana PEOL) provides the foundational architecture. For Casagrand's production environment, custom objects (Z-tables, Z-programs, custom condition types) will be built on top of this standard framework.

### 14.1 SAP Transaction Flow

Process Step

SAP Transaction

SAP Module

Key Tables

Document Generated

1\. Create Work Order (PO)

ME21N

MM-PUR

EKKO, EKPO, EKET, EKKN, KONV

Purchase Order (4500xxxxxx)

2\. Create Service Entry Sheet

ML81N

MM-SRV

ESSR, ESLL, ESLH

Service Entry Sheet (1000xxxxxx)

3\. Release Service Entry Sheet

ML81N

MM-SRV

ESSR (status update)

Released SES

4\. Post Invoice (RA Bill)

MIRO

FI-AP / MM-IV

RBKP, RSEG, BSEG, BKPF

Invoice Document (51xxxxxxxx)

5\. Post Down Payment (Advance)

F-48

FI-AP

BSEG, BKPF (Special GL)

Advance Payment Document

6\. Clear Down Payment (Recovery)

F-54

FI-AP

BSEG, BKPF

Clearing Document

7\. Automatic Payment Run

F110

FI-AP

REGUH, REGUP, PAYR

Payment Document + NEFT file

8\. Withholding Tax Certificate

J1INCERT

FI-AP (India)

WITH\_ITEM, J\_1IEWT\_SUMM

TDS Certificate (Form 16A)

9\. GR for Materials

MIGO

MM-IM

MKPF, MSEG

Material Document (49xxxxxxxx)

10\. Material Issue to Contractor

MIGO (261)

MM-IM

MKPF, MSEG

Goods Issue Document

### 14.2 Custom Objects Required (Casagrand-Specific)

Object Type

Proposed Name

Purpose

Key Fields

Z-Table

ZCGD\_RA\_HEADER

RA Bill header — project, contractor, period, cumulative tracking

RA\_BILL\_NO, PROJECT\_ID, LIFNR, EBELN, PERIOD\_FROM, PERIOD\_TO, GROSS\_VALUE, NET\_PAYABLE, STATUS

Z-Table

ZCGD\_RA\_ITEM

RA Bill line items — per BOQ item measurement

RA\_BILL\_NO, ITEM\_NO, BOQ\_ITEM, MB\_QTY, RATE, VALUE, CUM\_QTY, CUM\_VALUE

Z-Table

ZCGD\_RA\_ADJUST

Adjustments — escalation, retention, advance, penalty per bill

RA\_BILL\_NO, ADJUST\_TYPE (ESC/RET/ADV/PNL/MAT), AMOUNT, CUMULATIVE

Z-Table

ZCGD\_COMPLIANCE

Compliance document tracker per contractor per period

LIFNR, DOC\_TYPE, VALID\_FROM, VALID\_TO, STATUS, FILE\_REFERENCE

Z-Table

ZCGD\_RETENTION

Retention ledger — accumulation, release, forfeiture tracking

LIFNR, EBELN, RA\_BILL\_NO, RET\_AMOUNT, CUM\_RETAINED, RELEASED\_AMT, BALANCE

Z-Table

ZCGD\_ESCALATION

Escalation calculation records — index values, PAF, amounts

EBELN, PERIOD, INDEX\_TYPE, BASE\_VALUE, CURRENT\_VALUE, PAF, ESC\_AMOUNT

Z-Program

ZCGD\_RA\_BILL\_REPORT

% Completion vs % Disbursement report with S-Curve

—

Z-Program

ZCGD\_OVERBILL\_ALERT

Automated over-billing detection and alert generation

—

Custom Condition

ZRET

Retention deduction in PO pricing

% of base value

Custom Condition

ZESC

Escalation addition in PO pricing

Calculated amount

Custom Condition

ZADV

Advance recovery deduction

Calculated amount

Custom Condition

ZPNL

Penalty / LD deduction

% or fixed amount

Workflow

ZCGD\_WF\_RABILL

Multi-level approval workflow for RA Bill SES release

Approval levels per value slab

### 14.3 Integration Architecture

flowchart TD subgraph SAP\["SAP S/4HANA"\] PS\[Project System  
WBS / Network\] MM\[MM Procurement  
PO / SES / GRN\] FI\[FI-AP  
Invoice / Payment\] CO\[Controlling  
Cost Centers / WBS Budget\] end subgraph EXT\["External Systems"\] DMS\[Document Mgmt  
SharePoint / GDrive\] BIM\[BIM / Primavera  
Progress Tracking\] MOB\[Mobile App  
Site Measurement\] GOV\[GST Portal  
GSTN / Winman\] BNK\[Bank  
NEFT/RTGS File\] end MOB -->|MB Data Upload| MM BIM -->|Physical % Update| PS PS -->|WBS Budget| CO MM -->|SES → Invoice| FI FI -->|Payment File| BNK FI -->|TDS Data| GOV DMS -->|Compliance Docs| MM style SAP fill:#f8f6ff,stroke:#6B3FA0 style EXT fill:#fdf2f8,stroke:#B5567A

## 15 Annexures, Templates & Checklists

### Annexure A — RA Bill Cover Sheet Template

CASAGRAND — RUNNING ACCOUNT BILL COVER SHEET

RA Bill No.

RA / CG-ZEN-CHN / LC001 / CIVIL / 007

Project Name

Casagrand Zenith, OMR, Chennai

Contractor Name

M/s ABC Construction Pvt Ltd

Work Order No.

WO-4500017890

WO Total Value

₹ 85,00,00,000

Bill Period

01-Apr-2026 to 30-Apr-2026

A. Gross Value (This Bill)

₹ 2,30,00,000

B. Cumulative Gross Value

₹ 62,50,00,000

C. Escalation (+)

₹ 13,68,500

D. Subtotal (A + C)

₹ 2,43,68,500

E. Less: Retention (5%)

₹ 12,18,425

F. Less: Advance Recovery (11.11%)

₹ 27,07,140

G. Less: Material Recovery

₹ 8,45,000

H. Less: Penalty / LD

₹ 0

I. Less: Previous Payments

₹ 0 (Current bill only)

J. Net Value Before Tax

₹ 1,95,97,935

K. Add: GST @ 18%

₹ 35,27,628

L. Less: TDS @ 2% (194C)

₹ 3,91,959

M. Less: Labour Cess @ 1%

₹ 2,30,000

N. NET PAYABLE

₹ 2,25,03,604

### Annexure B — Compliance Document Tracker Template

#

Document

Status

Valid Until

Last Submitted

Verified By

1

GST Registration

Valid

Perpetual

01-Jan-2026

Compliance Officer

2

GSTR-3B (Mar 2026)

Filed

20-Apr-2026

21-Apr-2026

AP Team

3

PF Challan (Mar 2026)

Paid

15-Apr-2026

16-Apr-2026

HR Compliance

4

ESI Challan (Mar 2026)

Paid

15-Apr-2026

16-Apr-2026

HR Compliance

5

Labour License (CLRA)

Valid

31-Dec-2026

05-Jan-2026

Legal

6

Workmen Compensation Insurance

Valid

31-Mar-2027

01-Apr-2026

Insurance Coordinator

7

Safety Training Records

Pending

Monthly

—

Safety Officer

8

Material Test Certificates

Submitted

Per batch

28-Apr-2026

QA/QC Head

### Annexure C — RA Bill Processing Checklist (AP Department)

*   RA Bill cover sheet received with all financial computations
*   Measurement Book abstract attached and signed by both parties
*   Service Entry Sheet (SES) number and SAP printout verified
*   Contractor's Tax Invoice verified — GSTIN, HSN/SAC, tax calculation correct
*   Escalation calculation sheet verified (indices, formula, arithmetic)
*   Advance recovery statement matches advance ledger balance in SAP (Special GL)
*   Retention deduction correctly calculated and matches retention ledger
*   Material recovery statement reconciled with stores records
*   All compliance documents current and valid (GST, PF, ESI, Insurance, Labour License)
*   Approval signatures obtained per authority matrix (all levels signed)
*   Three-way match: WO (PO) value ↔ SES quantity ↔ Invoice amount within tolerance
*   Cumulative billing does not exceed WO value (ceiling check)
*   Health Ratio check: Physical % / Financial % ≥ 0.90 (no over-billing flag)
*   MSME vendor flagged for priority payment (if applicable)
*   Invoice posted in MIRO with correct WBS / Cost Center assignment
*   TDS correctly calculated and deducted at applicable rate
*   Labour Cess deducted at 1% of gross value
*   Payment scheduled in next F110 run

### Annexure D — Document Retention Policy

Document Category

Retention Period

Storage

Legal Basis

Work Order / Contract

8 years post-DLP expiry

SAP DMS + Physical archive

Indian Limitation Act (3 years) + Arbitration buffer

RA Bills + Supporting Documents

8 years from payment date

SAP DMS + Scanned copies in SharePoint

Income Tax Act (6 years) + buffer

Measurement Books

8 years from project completion

Physical archive (original) + Scanned copy

Construction industry standard

Tax Invoices (GST)

8 years from filing due date

SAP + Digital archive

GST Act Section 35 (72 months)

TDS Certificates (Form 16A)

8 years from assessment year

SAP + Income Tax portal download

Income Tax Act

Bank Guarantees

2 years post-release/expiry

Treasury vault (original) + Scanned copy

Negotiable Instruments Act

Compliance Documents (PF/ESI/Insurance)

8 years from expiry

SAP DMS + Digital archive

EPF Act / ESI Act

#### 📋 Version Control — Document Amendment History

Version

Date

Author

Changes

1.0

15-Jan-2025

VP-Projects Office

Initial SOP drafted for civil contractors

2.0

01-Jul-2025

CFO Office + Legal

Added MEP/Interior categories, MSME compliance, RERA reporting

3.0

01-May-2026

C16 — Enterprise Process Intelligence

Full SAP mapping, escalation formula, compliance gateway, over-billing alerts, S-Curve reporting

**Casagrand — Running Account Bill Process | Enterprise SOP v3.0**  
Prepared by **C16 — Enterprise Process Intelligence** | Date: 01 May 2026  
SAP Reference System: **S/4HANA (Hana PEOL)** — Client 800 — used for table structure and configuration reference only (not Casagrand production data)  
  
**Reference Data Sources:** Vendor master structure (LFA1 — 10 vendors, IN geography), Company code config (T001 — 10 codes), Payment terms (T052 — 10 configurations), Document types (T161 — 15 procurement types), Vendor company code data (LFB1 — reconciliation account 160000).  
  
**Confidentiality:** This document contains proprietary process information of Casagrand Group. Distribution restricted to authorised personnel. Unauthorised reproduction or distribution is prohibited.  
  
© 2026 Casagrand Group — All Rights Reserved | Process Intelligence powered by C16

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._