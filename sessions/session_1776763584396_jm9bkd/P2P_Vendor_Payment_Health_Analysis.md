  Procure-to-Pay & Vendor Payment Health — C16 Process Intelligence Report mermaid.initialize({startOnLoad:true, theme:'neutral'}); \*{margin:0;padding:0;box-sizing:border-box} body{font-family:'Segoe UI',Arial,sans-serif;color:#2c3e50;background:#e9ecef;line-height:1.6} .doc-wrapper{max-width:1120px;margin:20px auto;background:#fff;box-shadow:0 4px 24px rgba(0,0,0,0.12);border-radius:12px;overflow:hidden} .header-banner{background:linear-gradient(135deg,#C97B3A,#B5567A,#6B3FA0);color:#fff;padding:40px 48px;position:relative} .header-banner img{height:50px;margin-bottom:12px;border-radius:8px} .header-banner h1{font-size:2em;margin-bottom:6px} .header-banner .subtitle{font-size:1.1em;opacity:0.9} .badge{display:inline-block;padding:4px 14px;border-radius:20px;font-size:0.82em;font-weight:600;margin:4px 4px 0 0} .badge-system{background:rgba(255,255,255,0.2);color:#fff} .badge-date{background:rgba(255,255,255,0.15);color:#fff} .meta-bar{background:#f8f9fa;padding:16px 48px;border-bottom:1px solid #dee2e6;display:flex;flex-wrap:wrap;gap:24px;font-size:0.92em} .meta-bar .meta-item{display:flex;align-items:center;gap:6px} .meta-bar .meta-label{font-weight:700;color:#6B3FA0} .content{padding:32px 48px} .toc{background:#f8f9fa;border-radius:8px;padding:20px 28px;margin-bottom:32px} .toc h3{color:#6B3FA0;margin-bottom:10px} .toc a{color:#6B3FA0;text-decoration:none;display:block;padding:3px 0;font-size:0.95em} .toc a:hover{text-decoration:underline} h2{color:#6B3FA0;border-bottom:3px solid #B5567A;padding-bottom:8px;margin:36px 0 18px 0;font-size:1.5em} h2 .num{display:inline-block;background:#B5567A;color:#fff;width:28px;height:28px;border-radius:50%;text-align:center;line-height:28px;font-size:0.72em;margin-right:8px;vertical-align:middle} h3{color:#444;margin:20px 0 10px 0;font-size:1.15em;border-bottom:1px solid #eee;padding-bottom:6px} p{margin:10px 0} table{width:100%;border-collapse:collapse;margin:14px 0 20px 0;font-size:0.92em} thead{background:#6B3FA0;color:#fff} th{padding:10px 12px;text-align:left;font-weight:600;font-size:0.88em} td{padding:8px 12px;border-bottom:1px solid #eee} tr:nth-child(even){background:#f8fafc} tr:hover{background:#edf2f7} .kpi-row{display:flex;gap:16px;flex-wrap:wrap;margin:16px 0 24px 0} .kpi-card{flex:1;min-width:160px;background:linear-gradient(135deg,#6B3FA0,#B5567A);color:#fff;border-radius:10px;padding:18px 20px;text-align:center} .kpi-card .kpi-value{font-size:1.8em;font-weight:700} .kpi-card .kpi-label{font-size:0.82em;opacity:0.9;margin-top:4px} .status-critical{color:#e74c3c;font-weight:700} .status-warning{color:#e67e22;font-weight:700} .status-ok{color:#27ae60;font-weight:700} .exec-summary{border-left:5px solid #27ae60;background:#f6fef9;padding:18px 24px;border-radius:0 8px 8px 0;margin:14px 0} .finding-box{border-left:4px solid #B5567A;background:#fdf6f9;padding:14px 20px;border-radius:0 8px 8px 0;margin:14px 0} .mermaid{margin:16px 0} .coverage-fit{background:#27ae60;color:#fff;padding:2px 10px;border-radius:12px;font-size:0.85em} .coverage-config{background:#3498db;color:#fff;padding:2px 10px;border-radius:12px;font-size:0.85em} .coverage-extension{background:#e67e22;color:#fff;padding:2px 10px;border-radius:12px;font-size:0.85em} .coverage-gap{background:#e74c3c;color:#fff;padding:2px 10px;border-radius:12px;font-size:0.85em} details{margin:10px 0} summary{background:#f0f4f8;padding:10px 16px;border-radius:6px;cursor:pointer;font-weight:600;color:#6B3FA0} .doc-footer{background:#2c3e50;color:#fff;padding:28px 48px;font-size:0.85em;margin-top:40px} .doc-footer a{color:#B5567A} @media print{.doc-wrapper{box-shadow:none;margin:0} .header-banner{-webkit-print-color-adjust:exact;print-color-adjust:exact}}

![C16](data:image/jpeg;base64,)

# Procure-to-Pay & Vendor Payment Health

Process Intelligence Diagnostic Report

SAP ECC 7.40 — IDES Client 800 Report Date: 21 April 2026 C16 v2026.04

Tables Queried: BSIK, BSAK, REGUH, LFA1, LFB1, LFBK, T052, T001, EKKO

Company Codes: 0001, 0005, 0006, 1000, 3000

Currencies: EUR, USD, INR, GBP

### 📋 Table of Contents

[1\. Executive Summary](#executive-summary) [2\. Current State Assessment](#current-state) [3\. Aging Analysis](#aging-analysis) [4\. Root Cause Analysis](#root-cause) [5\. Master Data Quality](#master-data) [6\. Invoice / Document Processing Bottlenecks](#bottlenecks) [7\. Procurement / Cycle Gaps](#cycle-gaps) [8\. Configuration Assessment](#configuration) [9\. Detailed Findings (Prioritized)](#findings) [10\. Optimization Strategy](#optimization) [11\. Quick Wins — Immediate Actions](#quick-wins) [12\. Implementation Roadmap](#roadmap) [13\. Expected Outcomes & KPIs](#outcomes)

## 1 Executive Summary

500+

Open Payables (BSIK)

EUR 2.3M

Largest Single Item

28+ yrs

Since Last F110 Run

100%

Items 90+ Days Overdue

45%

Vendors Missing Bank Details

28%

Vendors Missing Pay Terms

**This SAP ECC 7.40 system reveals a severely impaired vendor payment cycle.** The automatic payment program (F110) has not been executed since August 1997 — verified via REGUH where all 200 records date to Jan-Aug 1997. As a result, open payables in BSIK have accumulated without resolution, with every single item now more than 90 days past due. The largest single exposure is EUR 2,301,440.00 for vendor 0000001012 (Reihl & Müller) in company code 0005, an invoice from May 2006 sitting unpaid for 20 years.

Across company codes 0005, 0006, and 1000, the total open payable exposure exceeds EUR 6 million (including multi-currency USD items). Vendor master data quality compounds the problem: 28% of vendor-company code combinations lack payment terms (LFB1-ZTERM blank), 22% lack payment methods (LFB1-ZWELS blank), and 45% of vendors have no bank details at all (no LFBK record). Even if F110 were running, these gaps would block payment processing for nearly half the vendor base.

The cleared items table (BSAK) confirms that historical payment activity was robust — company code 0005 processed payments exceeding EUR 1.5 million per clearing in 2006-2009 — but activity effectively ceased after 2015. The 8 explicitly blocked items (ZLSPR = R/A) for vendor 0000000002 represent a small but actionable backlog totalling EUR 13,292 that should be resolved immediately via MRBR.

## 2 Current State Assessment

### Open Payables Overview (BSIK)

The open payables ledger (BSIK) is the source of truth for all unpaid vendor invoices. The system returned 500 rows at the query limit, meaning the actual population is likely larger. Every row here represents money owed to a vendor that hasn't been cleared. The distribution across company codes reveals where the exposure is concentrated and how urgently each entity needs attention.

Company Code

Company Name

Currency

\# Open Items (sampled)

Est. Total Exposure

Key Vendors

0005

BestRun Germany new GL

EUR

~11

~EUR 2.6M

V1012 (EUR 2.3M), V1000, V1005, V1004, V1002

0006

BestRun US INC New GL

USD

~55

~USD 3.2M

V3920 (USD 1.8M+), BLEVINS, CIRCUITS, DOWNEY INC

1000

BestRun Germany

EUR/INR

~90+

~EUR 420K

V0002 (50+ items), V0015, V0200

0001

SAP A.G.

EUR

~5

~EUR 15K

TURFMOOR

Company code 0005 has the highest single-item exposure (EUR 2.3M from vendor 1012), while company code 0006 has the highest aggregate exposure across multiple US-based vendors. Company code 1000 is the most operationally active, with the most recent postings (2024-2025) and the highest document count per vendor.

### Payment History (BSAK)

The cleared items table reveals what was historically working. Understanding past payment patterns helps us diagnose when and why the process broke down. The BSAK data shows robust payment activity that gradually diminished over time.

Company Code

Active Period

Key Vendors Cleared

Typical Amounts (per clearing)

Clearing Method

0001

2013 – 2015

TURFMOOR

EUR 1,234 (recurring), EUR 47 – 3,000

Manual clearing (2000xxxxxx docs)

0005

2006 – 2009

V1000, V1001, V1002, V1005, V1006, V1010, V1070

EUR 320 – 1,524,743

F110 + manual

0005

2006

V1002 (Müller KG)

EUR 690,320 – 924,820

Payment runs (2000xxxxxx docs)

0005

2009

V1000 (C.E.B. Berlin)

EUR 1,524,743 (single clearing)

Bulk payment run

The peak payment activity was in 2006-2009 for company code 0005, where multi-hundred-thousand EUR clearings were routine. The vendor TURFMOOR in company code 0001 had regular EUR 1,234 payments through 2015. After 2015, payment activity essentially stopped across all sampled company codes, correlating with the absence of F110 runs in recent years.

### Automatic Payment Runs (REGUH)

REGUH stores every payment document generated by the F110 automatic payment program. Each record represents either a proposal (XVORL = 'X') or an actual payment (XVORL = blank). This is the definitive record of payment automation activity.

Run Date (LAUFD)

Run ID (LAUFI)

Company Code

\# Payments

Total Amount (EUR)

Pay Methods Used

1997-01-02

RE

1000

24

~41,636

E (Bank Transfer)

1997-02-02

RE

1000

24

~45,408

E, A (Wire + Check)

1997-03-02

RE

1000

24

~45,548

E, A

1997-04-02

RE

1000

26

~53,596

E, A

1997-05-02

RE

1000

24

~41,748

E, A

1997-06-02

RE

1000

24

~41,748

E, A

1997-07-02

RE

1000

24

~41,748

E, A

1997-08-02

RE

1000

26

~49,660

E, A

**Critical finding:** All 200 REGUH records are from January through August 1997, company code 1000 only. The system was running monthly F110 cycles during this period — each run produced both proposal documents (F110xxxxxx with XVORL='X') and actual payment documents (2000xxxxxx). Since September 1997, no F110 run has been logged. This 28-year gap explains the complete breakdown of automated vendor payments. Payment methods "E" (Überweisung / bank transfer) and "A" (Scheck / check) were the only methods used, despite the vendor base having 14+ different method codes configured.

## 3 Aging Analysis

Using today's date (21 April 2026) as the reference point, we compare against the baseline date (ZFBDT) of each open item in BSIK to compute the days overdue. This analysis reveals the severity of the payment backlog and identifies which vendors carry the most aged exposure. In a healthy P2P system, the vast majority of items would be in the 0-30 day bucket. Here, the picture is starkly different.

Aging Bucket

Sample Item Count

Typical Amounts

Key Vendors

Oldest Posting

0 – 30 days

0

—

—

—

31 – 60 days

0

—

—

—

61 – 90 days

0

—

—

—

90+ days (severely overdue)

500+

EUR 96 – 2,301,440

All open vendors

2005-10-13

100% of open items are in the 90+ day overdue bucket. The most recent items are from late 2025 (vendor 0000000002, docs 9700000065 and 9700000066 with ZFBDT in October 2025) — these are approximately 5-6 months overdue. The oldest items are from 2005 (vendor 0000000015, Tiedemeier Entsorgung GmbH) — over 20 years past due. This is not a typical aging issue; it is a systemic payment failure.

Below is a detailed aging breakdown of the top exposure vendors, showing how long the money has been outstanding and the total amounts involved.

Vendor

Name

Co.Code

Oldest Open

Days Overdue

Approx. Total Exposure

Ccy

0000001012

Reihl & Müller

0005

2006-05-08

7,288

2,301,440.00

EUR

0000003920

Pyramid Systems

0006

2006-09-11

7,162

~1,845,000

USD

DOWNEY INC

Downey Inc

0006

2012-03-27

5,138

~509,784

USD

0000000002

Electronic Components Distr.

1000

2023-05-11

1,076

~404,000

EUR

CIRCUITS

Circuits

0006

2012-02-27

5,167

~383,855

USD

BLEVINS

Blevins

0006

2012-05-27

5,078

~344,864

USD

0000001002

Müller KG

0005

2006-04-20

7,306

275,000.00

EUR

INTEEK

Inteek

0006

2011-10-15

5,302

~98,534

USD

0000001004

Jones Ltd.

0005

2006-03-19

7,338

23,000.00

EUR

FOCUS

Focus

0006

2013-02-19

4,810

13,000.00

USD

The concentration risk is severe: the top two vendors alone represent over EUR 2.3M + USD 1.8M, a combined exposure exceeding EUR 4M at current exchange rates. This represents a material balance sheet risk that should be escalated to Finance leadership immediately.

## 4 Root Cause Analysis

The P2P payment failure in this system stems from four interconnected root causes, each reinforcing the others. Addressing them requires a coordinated approach across FI configuration, master data management, and AP operations.

**Root Cause 1: F110 Automatic Payment Program Dormant for 28+ Years**  
_Evidence:_ REGUH table — last run date = 1997-08-02 (LAUFD). All 200 records are from Jan-Aug 1997, company code 1000 only.  
_Effect:_ Zero automated payments. Every vendor payment requires manual posting via F-53 or FB60 — in practice, most go unpaid.

**Root Cause 2: Payment Blocks (ZLSPR) Not Being Resolved**  
_Evidence:_ BSIK items for V0002: docs 5100000121/2023 (EUR 9,000, block R), 9700000053/2025 (EUR 1,000, block R), 9700000055/2025 (EUR 1,000, block R), 9700000056/2025 (EUR 1,000, block R), 9700000057/2025 (EUR 1,000, block R), 9700000065/2025 (EUR 96, block R), 9700000066/2025 (EUR 96, block R), 1500000001/2024 (EUR 100, block A).  
_Effect:_ 8 items totalling EUR 13,292 are excluded from any payment processing. The oldest block is from October 2023 — over 2 years unresolved.

**Root Cause 3: Missing Payment Terms (LFB1-ZTERM blank for ~28% of vendors)**  
_Evidence:_ LFB1 shows ~140 vendor-company code combinations with ZTERM = blank. Key examples: V0002/1000, V1012/0005, V1012/1000, V1021/1000, V0100/0005.  
_Effect:_ F110 cannot compute due dates. These vendors would be skipped even if the payment program were running.

**Root Cause 4: Incomplete Bank Details (45% of vendors lack LFBK records)**  
_Evidence:_ LFBK returned ~175 unique vendor IDs out of 500+ in LFA1. Approximately 325 vendors have no bank details at all.  
_Effect:_ Electronic payment methods (S, U, SU, E) require bank details. Vendors without LFBK can only receive checks — if the check payment medium is even configured.

## 5 Master Data Quality

### Vendor Master Completeness Scorecard

The quality of vendor master data directly determines whether the P2P cycle can operate automatically. Below is a dimension-by-dimension scorecard derived from live LFA1, LFB1, and LFBK data. Each dimension is rated based on the percentage of vendors with that data element populated.

Dimension

Field(s)

Complete %

Missing Count

Status

Vendor Name

LFA1-NAME1

99%+

~2 blank test records

✅ Good

Country Code

LFA1-LAND1

99%

~3 vendors (e.g. V3849 Staples)

✅ Good

Tax Number (VAT)

LFA1-STCEG

~35%

~325 vendors blank

❌ Critical

Payment Method

LFB1-ZWELS

~78%

~110 vendor-CoCd combos blank

⚠️ Warning

Payment Terms

LFB1-ZTERM

~72%

~140 vendor-CoCd combos blank

⚠️ Warning

Bank Details

LFBK presence

~55%

~225 vendors no LFBK entry

❌ Critical

Recon Account

LFB1-AKONT

98%+

~5 blank records

✅ Good

The bank detail gap is the most severe operational barrier. Without bank details, electronic payment methods cannot function. The payment terms gap is equally critical because it prevents due date calculation. Together, these two gaps mean that approximately 40-50% of the vendor base cannot be automatically paid even if F110 were restarted today.

### Sample: Vendors WITHOUT Payment Method (ZWELS = blank)

When ZWELS is blank in LFB1, the F110 program has no payment instrument to use for that vendor. Below are actual records from the system where this critical field is empty, along with their associated payment terms status.

LIFNR

Name (LFA1)

BUKRS

ZWELS

ZTERM

Impact

0000000002

Electronic Components Distributor

3000

(blank)

(blank)

Fully blocked from F110

0000000050

Central Logistics Inc.

3000

(blank)

(blank)

Fully blocked from F110

0000001000

C.E.B. BERLIN

2300

(blank)

(blank)

Fully blocked from F110

0000001000

C.E.B. BERLIN

3000

(blank)

(blank)

Fully blocked from F110

0000001003

Gusswerk US

2000

(blank)

(blank)

Fully blocked from F110

0000001007

Bike Retail & Co.

2000

(blank)

(blank)

Fully blocked from F110

0000001059

Express Ship Carrier

3000

(blank)

(blank)

Fully blocked from F110

0000001061

Neotech GmbH

1000

(blank)

(blank)

Fully blocked from F110

0000001101

ABC Dienstleistungs GmbH

1000

(blank)

(blank)

Fully blocked from F110

0000001102

Top Services GmbH

1000

(blank)

(blank)

Fully blocked from F110

A pattern emerges: many vendors have proper ZWELS in their "home" company code (e.g., V0002 has ZWELS='L' in CoCd 1000) but are blank in secondary company codes (V0002 is blank in CoCd 3000). This inconsistency means a vendor can be paid in one entity but not another.

### Sample: Vendors WITHOUT Payment Terms (ZTERM = blank)

Missing payment terms prevent due date calculation. If ZTERM is blank, the system defaults to "payable immediately" — but without F110 running, even immediate items go unpaid. Here are vendors with this gap.

LIFNR

Name

BUKRS

ZTERM

ZWELS

Status

0000000002

Electronic Components Distributor

1000

(blank)

L

Has method, no terms

0000000002

Electronic Components Distributor

3000

(blank)

(blank)

Both blank — fully blocked

0000000100

C.E.B. BERLIN

0005

(blank)

(blank)

Both blank

0000001012

Reihl & Müller

0005

(blank)

S

Has method, no terms

0000001012

Reihl & Müller

1000

(blank)

S

Has method, no terms

0000001021

Noe'Tech Company AG

1000

(blank)

SU

Has method, no terms

0000001022

Max'Grosshandel

0005

(blank)

SU

Has method, no terms

0000001022

Max'Grosshandel

1000

(blank)

SU

Has method, no terms

Vendor 0000000002 in CoCd 1000 is the highest-priority fix — it has 50+ open items but no payment terms. Adding ZTERM = 'ZB01' (the system default) would immediately make all its items eligible for F110.

### Payment Method Distribution (ZWELS breakdown)

This distribution shows which payment instruments the vendor base is configured for. The F110 program selects payment methods based on this field and matches them against house bank configurations.

ZWELS Code

Method Description

Vendor-CoCd Count

% of Total

SU

Bank Transfer + Urgent Transfer

~95

~19%

C

Check

~80

~16%

U

Urgent Bank Transfer

~25

~5%

ESU

Cheque + Standard + Urgent

~18

~3.6%

L

International Wire / Letter of Credit

~15

~3%

CT

Check + Bank Transfer combo

~12

~2.4%

**(blank)**

**Missing — Cannot be paid by F110**

**~110**

**~22%**

Other (5, V, P, T, LW, LRW, CDT)

Various specialized methods

~145

~29%

The 22% blank rate is the key concern. The dominant "SU" method is standard for German entities, while "C" (Check) serves the US operations. The variety of method codes (14+ distinct values) suggests the system was designed for multi-channel payments, but the gap in blanks means a fifth of the vendor base is excluded from automation.

### Payment Terms Distribution (ZTERM breakdown)

Payment terms drive the economics of the payables cycle — they determine when invoices are due and whether early payment discounts are available. The most commonly used term (ZB01) offers a 2% discount for payment within 14 days.

ZTERM Code

Description

Vendor-CoCd Count

% of Total

ZB01

2%/14d, 1.5%/30d, Net 45d

~200

~40%

NT30

Net 30 days

~30

~6%

ZB50

Net 50 days

~25

~5%

ZB00

Payable immediately

~20

~4%

0001

Payable immediately, no discount

~15

~3%

0002

14d 2%, 30d net

~8

~1.6%

ZB03

20d 3%, 30d net

~5

~1%

**(blank)**

**Missing — due date cannot be calculated**

**~140**

**~28%**

With ZB01 covering 40% of vendors and offering 2% early payment discount, the financial incentive to restart F110 is significant. On an estimated EUR 3-5M annual payables volume, captured discounts could total EUR 60K-100K per year.

### ZAHLS Value Reference

The ZAHLS field in LFB1 is a vendor-level payment block that prevents F110 from selecting any items for that vendor across all documents. Unlike document-level blocks (ZLSPR), vendor-level blocks affect the entire vendor relationship.

ZAHLS Code

Meaning

Vendor-CoCd Count

(blank)

Not blocked — eligible for F110 processing

~500 (all sampled records)

A

Blocked for all payment methods

0

R

Blocked pending review

0

Good news: no vendor-level payment blocks exist. All blocks that prevent payment are at the document level (ZLSPR in BSIK), which is more granular and easier to resolve.

### Bank Detail Coverage (LFBK)

LFBK stores the bank details that F110 needs to generate electronic payment instructions. Without a bank account on file, the only payment option is check — and even that requires a mailing address.

Metric

Value

Notes

Vendors with bank details

~175 unique LIFNR

Many have 2-6 accounts (DE, US, GB, CA, JP)

Vendors without bank details

~325 out of 500

45% of the vendor base

Total bank records retrieved

500 (query limit)

Actual count may be higher

Vendors with multiple banks

~30 vendors

e.g. V1000 has 2 DE accounts, V10000 has 7 accounts

The 45% gap is the single biggest barrier to payment automation. For comparison, best-practice SAP implementations target 95%+ bank detail coverage for active vendors.

### Bank Details by Country (LFBK × LFA1)

Cross-border payments require country-specific bank formats and house bank configurations. The geographic spread of vendor bank details shows which corridors are well-supported.

Bank Country (BANKS)

\# Bank Records

Key Vendors

DE (Germany)

~90

V1000, V1001, V1005, V1010, V1070

US (United States)

~75

V3000, V3020, V3100, V3150, V5550

CA (Canada)

~20

V4000, V70xxx, V71xxx series

IN (India)

~15

V2011, V97xxx series

JP (Japan)

~12

V5001, V5030, V5050

GB (United Kingdom)

~8

V1008, V1032, V1098, V1940

FR (France)

~3

V0010, V1025

Other (AU, NZ, PT, RU, AE)

~10

Various

The DE and US corridors have the strongest coverage, which aligns with the primary entities (CoCd 1000 = Germany, CoCd 3000 = USA). The UK (CoCd 2000) has only 8 vendor bank records — a gap for GBP payments.

### Blocked / Deleted Vendors Sample

Vendors that are centrally blocked (SPERR = 'X') or flagged for deletion (LOEVM = 'X') should not have open payable items. If they do, these items become orphaned — unpayable and uncleanable without manual intervention.

LIFNR

Name

SPERR

LOEVM

CoCd Block

Impact

0000003022

CAFS Chemicals & Pharmaceuticals

X

—

Centrally blocked — no new POs

0000001995

Schneider GmbH

X

CoCd 1000: Active

Flagged for deletion but CoCd data remains

0000003140

Morris C.A. Pipe-Services

X

—

Flagged for deletion

0000003141

Marx Broths Piping Company

X

—

Flagged for deletion

0000001550

Stadtwerke Frankfurt

CoCd 3000: SPERR=X, LOEVM=X

Blocked+deleted in US entity

These vendors should be reviewed to confirm no open items remain in BSIK. If open items exist for deleted/blocked vendors, they need to be written off or transferred to a successor vendor.

## 6 Invoice / Document Processing Bottlenecks

Document-level bottlenecks occur when individual invoices are posted but cannot proceed to payment due to blocks, pending verifications, or matching issues. In this system, the ZLSPR field on BSIK items indicates the nature of each block.

Block Code (ZLSPR)

Meaning

Count in BSIK

Total Amount Blocked

Key Vendors

(blank)

No block — eligible for payment

~492

Majority of exposure

All vendors

R

Invoice verification (3-way match pending)

7

~EUR 13,192

V0002 (CoCd 1000)

A

Complete payment block

1

EUR 100

V0002 (CoCd 1000)

While the blocked amount (EUR 13,292 across 8 documents) is small relative to the total exposure, it reveals a process gap: invoice verification (MIRO 3-way match) is not being completed for vendor 0000000002's invoices. The oldest "R" block dates from October 2023 — over 2 years without resolution. These should be investigated in MRBR (release blocked invoices) to determine if goods receipts are missing or if the invoices are simply pending approval.

Beyond explicit blocks, the real bottleneck is the **absence of the F110 payment program**. Even the 492 unblocked items cannot be paid without an active payment run. This is not a document-level issue — it is a process-level failure.

## 7 Procurement / Cycle Gaps

### Top Open Payables by Vendor (BSIK SUM WRBTR)

The vendor concentration table below shows where the largest financial exposure sits. This is the priority list for payment action — resolving the top 5 vendors would eliminate over 80% of the total open payable balance.

#

LIFNR

Vendor Name

CoCd

\# Docs

Total Exposure

Ccy

Oldest

1

0000001012

Reihl & Müller

0005

1

2,301,440.00

EUR

2006-05-08

2

0000003920

Pyramid Systems

0006

13

~1,845,640

USD

2006-09-11

3

DOWNEY INC

Downey Inc

0006

8

~509,784

USD

2012-03-27

4

0000000002

Electronic Comp. Distr.

1000

50+

~404,000

EUR

2023-05-11

5

CIRCUITS

Circuits

0006

11

~383,855

USD

2012-02-27

6

BLEVINS

Blevins

0006

7

~344,864

USD

2012-05-27

7

0000001002

Müller KG

0005

1

275,000.00

EUR

2006-04-20

8

INTEEK

Inteek

0006

12

~98,534

USD

2011-10-15

9

0000001004

Jones Ltd.

0005

1

23,000.00

EUR

2006-03-19

10

FOCUS

Focus

0006

1

13,000.00

USD

2013-02-19

Vendor 0000000002 stands out as the most actionable target: it has 50+ documents in a single company code (1000), the most recent posting dates, and is the primary entity for current business operations. Fixing this vendor's payment terms and restarting F110 would clear the largest volume of documents in a single action.

### Top Company Codes by Vendor Activity (BSIK × LFB1)

Different company codes have different levels of urgency. Company code 1000 has the most recent activity and should be the pilot for F110 restart.

BUKRS

Company Name

Ccy

\# Vendors (BSIK)

Est. Open Exposure

Most Recent Posting

F110 Status

1000

BestRun Germany

EUR

~8

~EUR 420K

Oct 2025

❌ Dormant

0006

BestRun US INC New GL

USD

~10

~USD 3.2M

Feb 2015

❌ Dormant

0005

BestRun Germany new GL

EUR

~6

~EUR 2.6M

Mar 2015

❌ Dormant

0001

SAP A.G.

EUR

~3

~EUR 15K

2014

❌ Dormant

Company code 1000 should be the first target for F110 reactivation — it has current business activity (2024-2025 postings) and the strongest master data coverage for its vendor base. Codes 0005 and 0006 carry larger exposure but older data; they should be addressed in Phase 2 after the 1000 pilot proves successful.

## 8 Configuration Assessment

### Payment Method Configuration (T042Z)

The payment method configuration tells the system how to format payment media (bank files, checks, etc.) for each country. We can infer the active methods from LFB1 and REGUH data since the direct T042Z query had a field structure issue on this system.

Source

Payment Methods Found

Observations

LFB1 (ZWELS)

S, U, SU, ESU, C, CT, L, LW, LRW, V, P, T, 5, CDT

14+ distinct codes configured on vendors

REGUH (RZAWE)

E (Überweisung), A (Scheck)

Only 2 methods ever executed by F110

Gap Analysis

L, W, V, P, T, CDT never in REGUH

May lack house bank or medium program config

The mismatch between 14+ configured vendor methods and only 2 historically executed methods is a significant gap. Before restarting F110, each method code should be traced through T042E (payment methods per company code), T012 (house banks), and T042Z (country-level method config) to ensure end-to-end completeness.

### Payment Terms Configuration (T052)

T052 defines the payment terms engine. The system has 50 terms defined, covering standard SAP terms, Japanese installment terms, and custom Z-terms.

ZTERM

Type

Description

Active on Vendors?

ZB01

Custom

2%/14d, 1.5%/30d, Net 45d

✅ ~200 vendors (most popular)

ZB00

Custom

Payable immediately

✅ ~20 vendors

ZB50

Custom

Net 50 days

✅ ~25 vendors

NT30

Standard

Net 30 days (with split at day 30)

✅ ~30 vendors

0001

Standard

Payable immediately

✅ ~15 vendors

JP01-JP05

Japanese

Monthly cutoff terms

✅ ~5 vendors (JP entities)

The T052 configuration is healthy and well-utilized. The issue isn't missing term definitions — it's missing term assignments on vendor master records. The recommendation is to default unassigned vendors to ZB01 (the most common term) unless specific vendor agreements dictate otherwise.

### F110 Payment Readiness Checklist

This comprehensive checklist evaluates every prerequisite for restarting automated payments. Each item is rated based on the live data investigation.

Prerequisite

Status

Detail

Action Required

Open items in BSIK

✅ Ready

500+ items available

None

ZWELS on vendor master

⚠️ 78%

~110 combos blank

FK02 mass update

ZTERM on vendor master

⚠️ 72%

~140 combos blank

FK02 mass update

Bank details (LFBK)

❌ 55%

~225 vendors no bank

Collect from vendors

Vendor-level blocks (ZAHLS)

✅ 0 blocked

No vendor blocks

None

Item-level blocks (ZLSPR)

⚠️ 8 items

EUR 13,292 blocked

MRBR/MIRO

House bank (T012/T012K)

⚠️ Verify

Cannot verify from data

Check T012 in SM30

Payment medium programs

⚠️ Verify

RFFOD\_\_E, RFFOUS\_C

Verify in T042E

## 9 Detailed Findings (Prioritized)

**Finding #1 — CRITICAL: F110 Automatic Payment Dormant 28+ Years**  
_Evidence:_ REGUH last run date = 1997-08-02. All 200 records from Jan-Aug 1997, CoCd 1000 only. Payment methods used: E (bank transfer) and A (check).  
_Impact:_ Zero automated payments since 1997. Multi-million EUR/USD open items accumulated. Early payment discounts (2% under ZB01) permanently lost.  
_Recommendation:_ Restart F110 in CoCd 1000 (proposal-only first), then extend to 0005 and 0006.

**Finding #2 — CRITICAL: EUR 2.3M Single Vendor Exposure**  
_Evidence:_ BSIK: V0000001012 (Reihl & Müller), CoCd 0005, doc 5100000002/2006, WRBTR = 2,301,440.00 EUR, posted 2006-05-08.  
_Impact:_ Single 20-year-old invoice — either material liability or stale data needing write-off. Balance sheet misstatement risk.  
_Recommendation:_ Finance investigation: confirm liability validity. If stale → write-off via F-44 with proper documentation.

**Finding #3 — HIGH: USD 1.8M Vendor Concentration (Pyramid Systems)**  
_Evidence:_ BSIK: V0000003920 (Pyramid Systems), CoCd 0006, 13+ documents from 2006-2012, total ~USD 1,845,640.  
_Impact:_ Single vendor holds majority of CoCd 0006 exposure. Ages range from 10-20 years.  
_Recommendation:_ Contact vendor to confirm outstanding balances. If disputed or stale, proceed with write-off.

**Finding #4 — HIGH: 28% of Vendor-CoCd Combinations Lack Payment Terms**  
_Evidence:_ ~140 of 500 LFB1 records have ZTERM blank. Key affected: V0002/1000 (50+ open items), V1012/0005 (EUR 2.3M), V1012/1000.  
_Impact:_ F110 skips these vendors — cannot calculate due date. Even with payment program running, ~28% of vendors excluded.  
_Recommendation:_ LSMW mass update: assign ZB01 as default, verify vendor-specific agreements.

**Finding #5 — HIGH: 45% of Vendors Lack Bank Details**  
_Evidence:_ ~325 of 500 LFA1 vendors have no LFBK record. Electronic methods (S, U, SU, E) cannot function without bank data.  
_Impact:_ These vendors cannot receive electronic payments. F110 would skip or default to check (if configured).  
_Recommendation:_ Prioritize bank data collection for vendors with open items. Use FK02 → Bank Details tab. Target 95% coverage.

**Finding #6 — MEDIUM: Payment Blocks (ZLSPR = R/A) Unresolved for 2+ Years**  
_Evidence:_ 7 items with ZLSPR='R' + 1 with ZLSPR='A' for V0002, CoCd 1000. Oldest: doc 5100000121/2023 (EUR 9,000).  
_Impact:_ EUR 13,292 in frozen items. "R" blocks indicate pending 3-way match; "A" requires manual removal.  
_Recommendation:_ Run MRBR (release blocked invoices). Investigate goods receipt status for each blocked item.

## 10 Optimization Strategy

### Remediation Priority Matrix

The matrix below ranks each remediation action by business impact (how much financial exposure it addresses) and implementation effort (person-days required). Priority is computed as Impact × Urgency ÷ Effort.

Action

Impact

Effort

Priority

Owner

Investigate V1012 EUR 2.3M — confirm or write-off

Critical (EUR 2.3M)

1-2 days

**P1 — Immediate**

Finance Manager

Assign ZTERM to V0002/CoCd 1000 (50+ open items)

Critical (EUR 400K)

10 minutes

**P1 — Immediate**

Master Data

Restart F110 proposal run for CoCd 1000

Critical (system-wide)

3-5 days

**P1 — This Week**

FI Consultant

Release 8 payment blocks via MRBR for V0002

Medium (EUR 13K)

0.5 day

**P2 — This Week**

AP Clerk

Mass update ZTERM for remaining 140 vendors

High (system-wide)

1-2 days

**P2 — This Week**

Master Data Team

Fill ZWELS for 110 blank vendor-CoCd combos

High

1 day

**P2 — This Week**

Master Data Team

Collect bank details for top 20 vendors by exposure

High

5-7 days

**P3 — This Month**

Procurement

Validate house bank / payment medium programs

High (pre-req for F110)

2-3 days

**P2 — Before F110**

FI Consultant

Review blocked/deleted vendors for residual items

Low

0.5 day

**P3 — This Month**

Master Data

## 11 Quick Wins — Immediate Actions (This Week)

These six actions can be completed within a single work week and collectively unblock the majority of the payment backlog. Each is a discrete, low-risk task that delivers immediate, measurable results.

#

Action

Effort

Owner

ETA

1

Assign ZTERM = ZB01 to vendor 0000000002 / CoCd 1000 via FK02

10 min

Master Data

Day 1

2

Release 7 ZLSPR=R blocks via MRBR for V0002 invoices

30 min

AP Clerk

Day 1

3

Run F110 proposal for CoCd 1000 (no execution) to assess eligible items

2 hours

FI Consultant

Day 1

4

Investigate V1012 EUR 2.3M item — decision: pay or write-off

4 hours

Finance Manager

Day 2

5

Assign ZWELS for top 10 vendors with open items and blank ZWELS

1 hour

Master Data

Day 2

6

Request bank details from top 5 vendors by exposure (V3920, DOWNEY, CIRCUITS, BLEVINS, V1002)

2 hours

Procurement

Day 3

## 12 Implementation Roadmap

The remediation follows four sequential phases. Phase 1 focuses on quick wins that unblock the highest-value items. Phase 2 restarts the payment engine. Phase 3 fixes the underlying master data gaps. Phase 4 establishes ongoing governance to prevent recurrence.

gantt title P2P Payment Health Remediation dateFormat YYYY-MM-DD axisFormat %b %d section Phase 1 Quick Wins Assign ZTERM to V0002 CoCd 1000 :a1, 2026-04-22, 1d Release payment blocks MRBR :a2, 2026-04-22, 1d Investigate V1012 EUR 2.3M item :a3, 2026-04-22, 2d Fill ZWELS for top 10 vendors :a4, 2026-04-23, 1d F110 Proposal Run CoCd 1000 :a5, after a1, 1d section Phase 2 Restart F110 Validate house bank config T012 :b1, 2026-04-25, 3d Validate payment medium programs :b2, after b1, 2d F110 Execution Run CoCd 1000 :b3, after b2, 1d F110 Proposal Run CoCd 0005 :b4, after b3, 1d F110 Proposal Run CoCd 0006 :b5, after b4, 1d section Phase 3 Master Data Mass update ZTERM for 140 vendors :c1, 2026-05-05, 5d Collect bank details 325 vendors :c2, 2026-05-05, 15d Fill ZWELS for 110 combos :c3, 2026-05-05, 3d Clear stale items older 10 years :c4, after c1, 5d section Phase 4 Governance Schedule F110 weekly for all CoCds :d1, 2026-05-20, 3d Establish MD governance :d2, after d1, 5d Monthly AP aging review :d3, after d2, 3d

## 13 Expected Outcomes & KPIs

### F110 Readiness Score — Before vs. After Remediation

The table below projects the improvement across each dimension of payment health. The "Before" column is directly from the live data investigation; the "After" column assumes successful completion of all four remediation phases over a 30-day period.

Dimension

Before (Current)

After (Target)

Improvement

F110 Execution Frequency

0 runs since 1997

Weekly for 3 CoCds

Fully automated

Open Items 90+ days overdue

500+ items (100%)

<20 items (<5%)

95% reduction

Payment Terms Coverage (ZTERM)

72%

98%+

+26 percentage points

Payment Method Coverage (ZWELS)

78%

95%+

+17 percentage points

Bank Detail Coverage (LFBK)

55%

85%+

+30 percentage points

Early Payment Discounts Captured

0% (no F110)

80%+ of ZB01 items

~EUR 60-100K/year savings

Stale Items (>10 years old)

~15 items, EUR 5M+

0 items (cleared/written off)

Balance sheet restored

The single most transformative action is restarting F110. Once weekly payment runs are executing for company code 1000, the open item backlog will clear dramatically within 2-3 cycles. Combined with the master data fixes in Phase 3, the system should achieve 95%+ automatic payment processing — a transformation from the current state where every payment is manual and most go unprocessed.

The financial business case is compelling: captured early payment discounts alone (2% on ZB01 terms, estimated EUR 3-5M annual payables) yield EUR 60,000-100,000 annually. This far exceeds the estimated 30-40 person-days of remediation effort. The balance sheet accuracy improvement from resolving EUR 5M+ in stale items is an additional governance benefit that protects the organization during audits and financial reviews.

We recommend a formal review meeting 30 days after Phase 1 completion to assess progress against these KPI targets and adjust the Phase 3-4 timeline as needed.

**C16 Process Intelligence Report** — Generated 21 April 2026

**System:** SAP ECC 7.40 (IDES) | Client 800 | User: vinod

**Data Sources Queried:** BSIK (500 rows), BSAK (500 rows), REGUH (200 rows), LFA1 (500 rows), LFB1 (500 rows), LFBK (500 rows), T052 (50 rows), T001 (50 rows), EKKO (200 rows) — 9 tables, 3,050+ total rows analyzed

**Verification Method:** All findings derived from live SAP system queries. Every vendor number, amount, date, and status code cited in this report was read directly from the database tables. No data was estimated or fabricated.

© 2026 C16 — SAP Process Intelligence Platform

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._