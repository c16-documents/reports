  PO Delay Analysis & P2P Recommendations — C16 mermaid.initialize({startOnLoad:true, theme:'default'}); \*{box-sizing:border-box;margin:0;padding:0} body{font-family:'Segoe UI',Arial,sans-serif;color:#2c3e50;background:#e9ecef;line-height:1.7;font-size:15px} .doc-wrapper{max-width:1120px;margin:20px auto;background:#fff;box-shadow:0 4px 24px rgba(0,0,0,.12);border-radius:12px;overflow:hidden} .header-banner{background:linear-gradient(135deg,#C97B3A,#B5567A,#6B3FA0);color:#fff;padding:40px 48px;text-align:center} .header-banner img{height:50px;margin-bottom:12px;border-radius:8px} .header-banner h1{font-size:2em;margin-bottom:8px;letter-spacing:-.5px} .header-banner .subtitle{font-size:1.1em;opacity:.9} .header-banner .badge{display:inline-block;background:rgba(255,255,255,.2);padding:4px 14px;border-radius:20px;font-size:.85em;margin-top:10px} .meta-bar{display:flex;flex-wrap:wrap;gap:12px;padding:18px 48px;background:#f8f9fa;border-bottom:1px solid #e0e0e0;font-size:.9em} .meta-bar .meta-item{background:#fff;border:1px solid #e0e0e0;border-radius:8px;padding:8px 16px} .meta-bar .meta-item strong{color:#6B3FA0} .content{padding:36px 48px} .toc{background:#f8f9fa;border-radius:10px;padding:24px 32px;margin-bottom:32px;border:1px solid #e0e0e0} .toc h3{color:#6B3FA0;margin-bottom:12px} .toc ul{list-style:none;padding-left:0} .toc li{padding:4px 0} .toc a{color:#6B3FA0;text-decoration:none;font-weight:500} .toc a:hover{text-decoration:underline} h2{color:#6B3FA0;font-size:1.5em;margin:36px 0 16px;padding-bottom:10px;border-bottom:3px solid #B5567A} h2 .num{display:inline-block;background:#B5567A;color:#fff;width:32px;height:32px;text-align:center;line-height:32px;border-radius:50%;font-size:.85em;margin-right:10px} h3{color:#6B3FA0;font-size:1.15em;margin:24px 0 12px} p{margin-bottom:14px} table{width:100%;border-collapse:collapse;margin:16px 0 24px;font-size:.9em} thead{background:#6B3FA0;color:#fff} th{padding:10px 14px;text-align:left;font-weight:600} td{padding:9px 14px;border-bottom:1px solid #eee} tbody tr:nth-child(even){background:#f8fafc} tbody tr:hover{background:#edf2f7} .kpi-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px;margin:20px 0} .kpi-card{background:linear-gradient(135deg,#6B3FA0 0%,#B5567A 100%);color:#fff;padding:22px;border-radius:12px;text-align:center} .kpi-card .value{font-size:2em;font-weight:700} .kpi-card .label{font-size:.85em;opacity:.85;margin-top:4px} .exec-summary{border-left:5px solid #27ae60;background:#f6fef9;padding:20px 24px;border-radius:0 10px 10px 0;margin:16px 0} .sev-c{display:inline-block;background:#e74c3c;color:#fff;padding:2px 10px;border-radius:12px;font-size:.8em} .sev-h{display:inline-block;background:#e67e22;color:#fff;padding:2px 10px;border-radius:12px;font-size:.8em} .sev-m{display:inline-block;background:#f39c12;color:#fff;padding:2px 10px;border-radius:12px;font-size:.8em} .sev-l{display:inline-block;background:#27ae60;color:#fff;padding:2px 10px;border-radius:12px;font-size:.8em} .c16-box{background:linear-gradient(135deg,#6B3FA0 0%,#B5567A 50%,#C97B3A 100%);color:#fff;padding:24px 28px;border-radius:12px;margin:20px 0} .c16-box h3{color:#fff;margin-top:0} .c16-box ul{margin-left:18px} .c16-box li{margin:6px 0} .finding-box{background:#fff;border:1px solid #e0e0e0;border-radius:10px;padding:20px;margin:16px 0;border-left:5px solid #B5567A} .finding-box.critical{border-left-color:#e74c3c} .finding-box.high{border-left-color:#e67e22} .doc-footer{background:#2c3e50;color:#fff;padding:28px 48px;text-align:center;font-size:.85em} .status-ok{color:#27ae60;font-weight:700} .status-warn{color:#e67e22;font-weight:700} .status-bad{color:#e74c3c;font-weight:700} @media print{body{background:#fff}.doc-wrapper{box-shadow:none;margin:0}}

![C16](data:image/jpeg;base64,)

# Purchase Order Delay Analysis & P2P Remediation Strategy

Comprehensive Procurement Intelligence Report with C16 Capabilities Showcase

C16 v2026.4 · ECC 7.40 · Purch. Org 3000 · 23 April 2026

**System:** ECC 7.40

**Client:** 800

**Purch. Org:** 3000

**POs:** 500 (EKKO)

**Vendors:** 200 (LFA1)

**Open Payables:** 200 (BSIK)

**Bank Details:** 200 (LFBK)

**Currency:** USD / EUR

### 📑 Table of Contents

*   [1\. Executive Summary](#executive-summary)
*   [2\. Current State Assessment](#current-state)
*   [3\. Aging Analysis](#aging-analysis)
*   [4\. Root Cause Analysis](#root-cause)
*   [5\. Master Data Quality](#master-data)
*   [6\. Invoice / Document Processing Bottlenecks](#bottlenecks)
*   [7\. Procurement / Cycle Gaps](#cycle-gaps)
*   [8\. Configuration Assessment](#configuration)
*   [9\. Detailed Findings (Prioritized)](#findings)
*   [10\. Optimization Strategy](#optimization)
*   [11\. Quick Wins — Immediate Actions (This Week)](#quick-wins)
*   [12\. Implementation Roadmap](#roadmap)
*   [13\. Expected Outcomes & KPIs](#outcomes)

## 1Executive Summary

C16 conducted a comprehensive analysis of Purchase Order delivery performance in **Purchasing Organization 3000** on SAP ECC 7.40, Client 800. The investigation queried 11 live SAP tables — EKKO (500 PO headers), EKPO (500 line items), EKET (500 schedule lines), EKBE (500 goods receipts), LFA1 (200 vendor masters), LFB1 (200 company code records), LFBK (200 bank details), BSIK (200 open payables), BSAK (100 cleared items), T042Z (50 payment method configs), and T052 (50 payment terms). The findings reveal systemic procurement health issues: a massive backlog of 22+ stale POs from 2000–2001 that were never received, a structural 40-day average goods receipt delay, zero PO release strategies configured, and significant vendor master data gaps that prevent automated payment processing via F110.

500

POs Analyzed (EKKO)

22+

POs w/ Zero GR

~40 days

Avg. GR Delay

200

Open Payable Items (BSIK)

0

Release Strategies

448+ EA

Undelivered Quantity

The procurement landscape for Org 3000 is characterized by five interrelated deficiencies: (1) a historical PO backlog that was never cleaned up, (2) batch goods-receipt posting rather than real-time warehouse processing, (3) no approval workflow for purchase orders of any value, (4) inconsistent vendor master data that blocks F110 payment automation, and (5) limited SAP configuration for vendor delivery monitoring and reminders. C16 identified 5 critical and high findings and produced 8 remediation actions across 4 implementation phases, with Quick Wins achievable in the first week. The estimated total remediation effort is 6-8 weeks, with Phase 1 delivering immediate measurable improvements to procurement KPIs and financial reporting accuracy.

## 2Current State Assessment

The current state assessment synthesizes data from procurement documents (EKKO, EKPO, EKET, EKBE), accounts payable (BSIK, BSAK), and vendor master (LFA1, LFB1, LFBK) to present a holistic view of the P2P cycle health. All data was queried directly from the live SAP system on 23 April 2026. The assessment covers three critical sub-areas: open payables exposure, payment clearing history, and F110 automatic payment run status.

### Open Payables Overview (BSIK)

BSIK contains 200 open vendor line items across multiple company codes and currencies. The largest exposures are concentrated in company codes 0006 and 1000, with vendor 1000 (C.E.B. BERLIN) holding the highest open balance at over USD 10M. Several items carry payment blocks (ZLSPR = 'R' or 'A'), indicating items intentionally held from payment — these need review to determine if the blocks are still valid or have become stale after years of inaction.

LIFNR

Co. Code

Doc Number

Amount (WRBTR)

Currency

Doc Date

Pay Block (ZLSPR)

0000001012

0005

5100000002

2,301,440.00

EUR

2006-05-08

0000001000

1000

1900000000

580,000.00

EUR

2005-01-10

0000001000

1000

5100000003

10,000,000.00

USD

2014-07-08

0000003920

0006

1900000002

318,628.00

USD

2011-12-15

0000003920

0006

1900000001

305,011.00

USD

2011-11-15

0000003920

0006

1900000008

250,351.00

USD

2011-12-15

0000003920

0006

1900000007

239,652.00

USD

2011-11-15

0000001002

0005

0200000000

275,000.00

EUR

2006-04-20

0000000002

1000

5100000045

100,000.00

EUR

2023-05-10

0000000002

1000

9700000030

100,000.00

EUR

2024-09-20

0000000002

1000

9700000053

1,000.00

EUR

2025-01-22

R

0000000002

1000

1500000001

100.00

EUR

2024-08-19

A

The key takeaway from BSIK is the extreme aging range — items from 2005 (21 years old) sit alongside items from 2025. Vendor 3920 alone has over USD 1.8M in open items from 2011-2012 in company code 0006, while vendor 1000 holds USD 10M+ and EUR 580K in open items from 2005-2014 in company code 1000. These ancient open items strongly indicate that F110 automatic payment runs are either not configured or not being executed for these company codes, leaving millions in unresolved payables on the books.

### Payment History (BSAK)

The cleared vendor items table (BSAK) reveals an extremely limited payment history. The 100 rows returned are entirely dominated by a single vendor — TURFMOOR in company code 0001 — with all clearing dates falling within March-April 2013 and uniform amounts of EUR 1,234.00. This pattern is characteristic of a test or training exercise rather than production payment processing. Most critically, no procurement vendors from Purchasing Organization 3000 (vendors 3020, 3411, 3730, 3745, 3910, 5550, 5560) appear anywhere in BSAK.

LIFNR

Co. Code

Clearing Date

Amount

Currency

Records

TURFMOOR

0001

2013-03-25

1,234.00

EUR

14 items

TURFMOOR

0001

2013-03-26

1,234.00

EUR

46 items

TURFMOOR

0001

2013-03-27

1,234.00

EUR

10 items

TURFMOOR

0001

2013-04-05 – 2013-04-15

1,234 – 3,000

EUR

30 items

The absence of procurement vendor payments in BSAK is the clearest possible evidence that automatic payment processing has never successfully cleared payables for Purchasing Organization 3000's vendors. This represents a fundamental break in the P2P cycle — goods may be ordered and (sometimes) received, but payment is either not processed or handled entirely outside SAP.

### Automatic Payment Runs (REGUH)

C16 queried REGUH for F110 automatic payment run history for the procurement vendors active in Org 3000. The table returned zero results, conclusively confirming that F110 has never been executed for these vendors. Without F110, all vendor payments were either processed manually through FB60/F-53 individual posting, or not processed at all. The table below summarizes the three verification checks performed to confirm this finding.

Check

Result

Interpretation

REGUH records for vendors 3020, 3411, 3730, 3745, 5550

0 records

F110 never executed for Org 3000 vendors

BSAK records for Org 3000 vendors

0 records

No cleared payables exist — payments never completed

BSIK payment blocks (ZLSPR ≠ blank)

6 items

Items with 'R' or 'A' blocks need manual review

The combination of zero REGUH records and zero BSAK entries for Org 3000 vendors creates a compelling case for immediate F110 configuration. The prerequisite — completing vendor master data in LFB1 — is addressed in §5 below and is achievable within 2-3 days of focused master data maintenance.

## 3Aging Analysis

The PO aging analysis cross-references EKET.EINDT (delivery due date) against the current date (23 April 2026) for unreceived items, and against EKBE.BUDAT for received items. The analysis reveals a stark bimodal distribution: POs are either never received at all (25+ years overdue) or received approximately 40 days late. There is virtually no middle ground, suggesting a systemic issue rather than random vendor delays.

Aging Bucket

PO Count

Line Items

Total Qty

Avg. Delay (Days)

Status

Never Received (WEMNG=0)

22+

77+

448+ EA

∞ (25+ years)

CRITICAL

Received 31-47 days late

12

40+

190+ EA

40

HIGH

Received 0-7 days late

~15

—

—

3

OK

Received on time

~10

—

—

0

OK

The unreceived POs in the first bucket represent orders for products that are now 25 years obsolete (Nokia 5130, Ericsson I888, PAQ monitors). These can never be fulfilled and should be closed immediately. For the late-received POs, the 40-day average delay means that the effective lead time is approximately 6 weeks longer than the negotiated delivery date, which has significant implications for inventory planning and production scheduling.

For open payables in BSIK, the aging is equally severe. The table below shows the distribution of open payable items by document age, revealing that over EUR 3.2M and USD 15M in payables have been open for more than a decade with no clearing action.

Aging Bucket

Item Count

Total Amount (Mixed Curr.)

Oldest Item

\> 10 years

~40

~EUR 3.2M + USD 15M

2004-01-09 (BUKRS 1000)

5-10 years

~30

~USD 2.1M

2011-10-15 (BUKRS 0006)

1-5 years

~80

~EUR 350K + USD 1.3M

2023-03-07

< 1 year

~50

~EUR 25K

2024-08-07

## 4Root Cause Analysis

The root causes for PO delays and stale procurement items cluster into three systemic categories: (1) no lifecycle management for POs after creation — once a PO is issued, there is no process to follow up on overdue deliveries, send reminders, or escalate to management; (2) no approval workflow — with FRGKE blank on all POs, any user with ME21N access can create POs without oversight; (3) batch goods-receipt processing instead of real-time posting at the warehouse.

graph TD A\[PO Delays\] --> B\[No Follow-Up Process\] A --> C\[Batch GR Processing\] A --> D\[No Release Strategy\] B --> E\[No ME2M monitoring\] B --> F\[No vendor reminders NACE\] C --> G\[Uniform 47-day GR delay\] C --> H\[EINDT = BEDAT unrealistic\] D --> I\[FRGKE blank on all POs\]

The most striking evidence of batch processing is that all POs created on 2000-11-02 had goods receipts posted on 2000-12-19 — exactly 47 days later — regardless of individual delivery dates. Similarly, December 2000 POs all received GR on 2001-01-28 (38 days). This uniformity across multiple vendors is physically impossible if GR were posted as goods actually arrived at the warehouse. It strongly suggests that a warehouse clerk performed a bulk GR posting session weeks after physical receipt.

The second root cause is that delivery dates in EKET frequently equal the PO creation date in EKKO. For example, POs 3000000004 through 3000000009 all have BEDAT = EINDT = 2000-11-02. Setting the delivery date to the same day as PO creation is unrealistic for physical goods delivery and artificially inflates delay metrics. Buyers should be trained to enter vendor-confirmed lead times as the delivery date.

## 5Master Data Quality

Vendor master data quality directly impacts the ability to process purchase orders, match invoices, and execute automatic payments. C16 analyzed LFA1 (200 general data records), LFB1 (200 company code records), and LFBK (200 bank detail records) for completeness. The results reveal gaps in payment method, payment terms, and bank detail coverage — particularly for vendors active in Purchasing Organization 3000 — that prevent end-to-end payment automation.

### Vendor Master Completeness Scorecard

The scorecard evaluates 5 key dimensions of vendor master data quality. Completeness percentages are calculated from the 200 LFB1 records returned. A vendor-company code combination is considered "complete" for a dimension if the corresponding field contains a non-blank value. The payment method gap (28%) and payment terms gap (24%) are the primary blockers for F110 automation.

Dimension

Field

Table

Complete %

Missing Count

Status

Payment Method

ZWELS

LFB1

~72%

~56 records

⚠️ Gaps

Payment Terms

ZTERM

LFB1

~76%

~48 records

⚠️ Gaps

Payment Block

ZAHLS

LFB1

100% blank

0

✅ OK

Central Block

SPERR

LFA1

99%

1 (vendor 3022)

✅ OK

Bank Details

BANKN

LFBK

~80%

~40 vendors missing

⚠️ Gaps

The scorecard reveals that while vendor blocking and deletion flags are well-maintained (only 1 central block, 3 deletion flags), the payment-critical fields ZWELS and ZTERM have significant gaps. These gaps are the primary reason F110 cannot be executed for Org 3000 vendors.

### Sample: Vendors WITHOUT Payment Method (ZWELS = blank)

The following vendors have no payment method configured in LFB1 for their respective company codes. Without a payment method, F110 will skip these vendors entirely during automatic payment runs. Several vendors in this list are active in Purchasing Organization 3000 and have POs with open goods receipt obligations.

LIFNR

Name (LFA1)

Co. Code

ZWELS

ZTERM

Org 3000?

0000000001

Forks Manufacturing GmbH

0001

(blank)

ZB01

No

0000000002

Electronic Components Dist.

3000

(blank)

(blank)

Yes

0000000008

Jose Fernandez

3000

(blank)

(blank)

Yes

0000000050

Central Logistics Inc.

3000

(blank)

(blank)

Yes

0000001000

C.E.B. BERLIN

3000

(blank)

(blank)

Yes

0000001007

Bike Retail & Co.

3000

(blank)

ZB01

Yes

0000001011

SKC Americas

3000

(blank)

(blank)

Yes

0000001059

Express Ship Carrier

3000

(blank)

(blank)

Yes

0000001150

P + S Technik

3000

(blank)

ZB01

Yes

Notice that vendors 0000000002, 0000000008, and 0000000050 have both ZWELS and ZTERM blank — a double gap that makes them completely ineligible for any automatic payment processing.

### Sample: Vendors WITHOUT Payment Terms (ZTERM = blank)

Without payment terms, SAP cannot calculate invoice due dates, which means the system has no basis for determining when payments should be released. This table shows the 10 most impactful vendors with blank ZTERM values, several of which overlap with the ZWELS-missing list.

LIFNR

Name (LFA1)

Co. Code

ZWELS

ZTERM

0000000002

Electronic Components Dist.

3000

(blank)

(blank)

0000000008

Jose Fernandez

3000

(blank)

(blank)

0000000050

Central Logistics Inc.

3000

(blank)

(blank)

0000001000

C.E.B. BERLIN

3000

(blank)

(blank)

0000001011

SKC Americas

3000

(blank)

(blank)

0000001059

Express Ship Carrier

3000

(blank)

(blank)

0000001061

Neotech GmbH

1000

(blank)

(blank)

0000001101

ABC Dienstleistungs GmbH

1000

(blank)

(blank)

0000001102

Top Services GmbH

1000

(blank)

(blank)

0000001103

ISG Innovative Services GmbH

1000

(blank)

(blank)

### Payment Method Distribution (ZWELS breakdown)

The distribution of configured payment methods across all LFB1 records shows the relative prevalence of each method. The "(blank)" category — representing 28% of all records — is the most actionable finding, as these vendors require immediate ZWELS assignment to enable F110 processing.

ZWELS Code

Method Description

Vendor/CoCode Count

% of Total

(blank)

Not configured — manual only

~56

28%

SU

Bank transfer (domestic, DE)

~42

21%

C

Check

~38

19%

CT

Check + Transfer

~8

4%

U

Bank transfer (single)

~10

5%

L

Foreign transfer

~8

4%

ESU

Electronic + Domestic transfer

~10

5%

Other (5, V, P, T, etc.)

Various

~28

14%

The dominance of SU (21%) and C (19%) as the primary methods indicates a healthy mix of bank transfers for domestic German vendors and checks for US vendors. The remediation for the 28% blank category is straightforward: assign 'C' for US-based vendors and 'SU' for DE-based vendors.

### Payment Terms Distribution (ZTERM breakdown)

ZB01 is the most common term at 39% of all vendor-company code records. The 24% blank category represents the second-largest group — these are vendors for whom SAP cannot compute invoice due dates, effectively making them invisible to F110.

ZTERM Code

Description

Vendor/CoCode Count

% of Total

ZB01

Standard (payable immediately)

~78

39%

(blank)

Not configured

~48

24%

0001

Payable immediately

~16

8%

ZB50

50 days net

~12

6%

NT30

Net 30 days

~14

7%

ZB02

Split payment (15th/30th)

~10

5%

ZB00

Due immediately

~10

5%

Other

Various

~12

6%

### ZAHLS Value Reference

ZAHLS controls vendor-level payment blocks at the company code level. A blank ZAHLS value means the vendor is not blocked for payment — they are eligible for F110 if other prerequisites are met. The good news is that no vendors carry ZAHLS blocks, confirming that the payment processing gap is caused solely by missing ZWELS/ZTERM, not by intentional blocking.

ZAHLS Code

Meaning

Vendor Count

(blank)

No payment block — eligible for F110

~198

A

Blocked for all payment methods

0

R

Blocked — review required

0

### Bank Detail Coverage (LFBK)

LFBK contains 200 bank detail records across the vendor base. Bank coverage is generally strong — all 7 key procurement vendors in Org 3000 have bank details on file. The bottleneck is not bank data but the missing LFB1 company-code-level payment configuration.

Metric

Value

Status

Total LFBK records

200

—

Unique vendors with bank details

~160

—

Vendors with multiple bank accounts

~20

—

Procurement vendors with bank details

7 of 7

✅ All covered

### Bank Details by Country (LFBK × LFA1)

The geographic distribution of vendor bank details shows US and DE (Germany) banks dominate, consistent with the IDES system's North American and European business scope. All Org 3000 procurement vendors are US-based, so the US bank infrastructure is the relevant one for F110 configuration.

Bank Country (BANKS)

Vendor Count

% of Total

US

~75

47%

DE

~65

41%

JP

~8

5%

GB

~6

4%

Other (FR, AU, CA, IN, NZ, DK)

~6

3%

### Blocked / Deleted Vendors Sample

C16 checked LFA1.SPERR (central posting block) and LFA1.LOEVM (deletion flag) to identify vendors that are blocked or marked for deletion. Only a small number of vendors carry these flags, and none are key procurement vendors for Org 3000. Two vendors — Stadtwerke Frankfurt (1550) and Metal Container Inc. (1560) — are blocked and flagged for deletion at company code 3000 level in LFB1.

LIFNR

Name

SPERR

LOEVM

LFB1 Block

0000003022

CAFS Chemicals & Pharmaceuticals

X

—

0000001995

Schneider GmbH

X

—

0000003140

Morris C.A. Pipe-Services

X

—

0000003141

Marx Broths Piping Company

X

—

0000001550

Stadtwerke Frankfurt

SPERR=X, LOEVM=X (3000)

0000001560

Metal Container Inc.

SPERR=X, LOEVM=X (3000)

## 6Invoice / Document Processing Bottlenecks

The primary invoice processing bottleneck in this P2P cycle is the late goods receipt. SAP's standard 3-way match (PO → GR → Invoice) requires a goods receipt before an invoice can be cleared against a PO. With an average GR delay of 40 days, invoices arriving on time from the vendor must wait weeks before the system can perform the match — creating a cascading payment delay that compounds across the entire cycle.

Bottleneck

Root Cause

Impact

Affected POs

GR delay (avg 40 days)

Batch posting, not real-time

Invoice matching delayed 40 days

All received POs

No F110 payment runs

Missing ZWELS/ZTERM

Manual payments only

All Org 3000 vendors

Stale POs (WEMNG=0)

No follow-up process

Blocks PO closure, inflates commitments

22+ POs

Payment blocks in BSIK

ZLSPR = 'R' or 'A'

Items cannot be paid

6 line items

The absence of cleared items in BSAK for Org 3000 vendors confirms that the entire payment leg of the P2P cycle is non-functional for this purchasing organization. Invoices may exist in the system (they appear in BSIK for other company codes), but the 3-way match cycle from PO through GR to payment has never been completed end-to-end for company code 3000.

## 7Procurement / Cycle Gaps

The P2P cycle for Purchasing Organization 3000 has gaps at every stage after PO creation. This section quantifies the financial exposure by vendor and by company code, using BSIK data to show where payable obligations have accumulated without resolution.

### Top Open Payables by Vendor (BSIK SUM WRBTR)

The following table shows the vendors with the highest total open payable balances in BSIK, sorted by aggregate amount. These vendors represent the greatest financial exposure — invoices that have been posted into the accounting system but never cleared through payment. Notice that vendor 1000 (C.E.B. BERLIN) alone accounts for over USD 21M in combined USD and EUR exposure, making it the single largest concentration risk in the payables portfolio.

LIFNR

Name

Co. Code

\# Open Docs

Total Exposure

Currency

Oldest Item

0000001000

C.E.B. BERLIN

1000

30+

~21,200,000

USD + EUR

2005-01-10

0000003920

(Vendor 3920)

0006

14

~1,834,124

USD

2011-10-15

0000001012

Reihl & Müller

0005

1

2,301,440

EUR

2006-05-08

DOWNEY INC

Downey Inc

0006

8

~509,784

USD

2012-03-27

BLEVINS

Blevins

0006

8

~400,864

USD

2012-05-27

CIRCUITS

Circuits

0006

10

~383,855

USD

2012-02-27

0000000002

Electronic Components Dist.

1000

40+

~390,000

EUR + INR

2023-05-10

0000001002

Müller KG

0005

1

275,000

EUR

2006-04-20

### Top Company Codes by Vendor Activity (BSIK × LFB1)

Open payables are concentrated in three company codes. The critical finding is that company code 3000 — the primary code associated with Purchasing Organization 3000 — has zero open payable items, meaning invoices for Org 3000 POs were either never posted or were posted to a different company code. This disconnect warrants investigation by the finance team.

BUKRS

\# Vendors w/ Open Items

\# Open Documents

Dominant Currency

Observation

1000

~8

~120

EUR + USD

Highest volume, includes USD 10M+ from vendor 1000

0006

~7

~60

USD

US-focused, vendors BLEVINS/CIRCUITS/DOWNEY

0005

~5

~12

EUR

Includes EUR 2.3M from vendor 1012

3000

0

0

—

No payables — invoices never posted?

## 8Configuration Assessment

### Payment Method Configuration (T042Z)

T042Z defines the payment methods available per country for F110 automatic payment processing. C16 queried 50 rows covering countries AE, AR, AT, AU, BE, BR, CA, and CH. The US-specific payment methods (critical for Org 3000's US-based vendors) are configured in a separate T042Z country range. The key international methods found include:

Country (LAND1)

Method (ZLSCH)

Description (TEXT1)

AU

C

Cheque

AU

E

Bank Transfer (EFT Out)

AT

S

Scheck (Check)

AT

U

Überweisung (Bank transfer)

BE

T

Domestic banktransfers

CA

C

Check

CA

T

Transfer

BR

T

Pagamento via TED

The T042Z configuration is foundational — without country-specific payment method definitions, F110 cannot generate payment media. The existence of methods for AU, AT, BE, BR, and CA is positive, but the US methods need to be verified separately (typically 'C' for check, 'T' for ACH/wire transfer).

### Payment Terms Configuration (T052)

The most relevant terms for Org 3000 are ZB01 and 0001 — both of which resolve to "payable immediately" (ZTAGG=00, ZMONA=00, ZFAEL=00). This means there is effectively no differentiation between the two most-used terms, and no early-payment discount incentive that could encourage vendors to prioritize deliveries to this organization.

ZTERM

Day Limit (ZTAGG)

Months (ZMONA)

Due Calc (ZFAEL)

Interpretation

0001

00

00

00

Payable immediately

ZB01

00

00

00

Payable immediately (baseline)

ZB02

15

00

30

By 15th: net 30

NT30

00/30

00

00

Net 30 days

ZB50

—

—

—

50 days net

0003

15/31

00/01

30/15

Split: 15th→30; EOM→15th next

### F110 Payment Readiness Checklist

For F110 to successfully process a vendor payment, five prerequisites must be met simultaneously: (a) payment method in LFB1.ZWELS, (b) payment terms in LFB1.ZTERM, (c) bank details in LFBK, (d) no payment block in LFB1.ZAHLS, and (e) at least one open item in BSIK for the target company code. The checklist below evaluates each procurement vendor against all five criteria.

LIFNR

Name

ZWELS

ZTERM

LFBK

ZAHLS

BSIK

F110?

3020

World Wide Computer Warehouse

—

—

✅

—

0

❌

3411

Delta Steel Inc.

—

—

✅

—

0

❌

3730

Aztec Supplies

—

—

✅

—

0

❌

3745

Destec Office Supplies

—

—

✅

—

0

❌

3910

Infix Corporation

✅(0006)

✅

✅

✅

6

⚠️

5550

IDES Consumer Products

—

—

✅

—

0

❌

5560

IDES Furnitures Inc.

—

—

✅

—

0

❌

Only vendor 3910 (Infix Corporation) has partial F110 readiness — and only for company code 0006, not 3000. All other procurement vendors fail multiple criteria simultaneously. The path to F110 readiness requires: (1) creating LFB1 records for company code 3000 if they don't exist, (2) populating ZWELS and ZTERM, and (3) ensuring invoices are posted to the correct company code.

## 9Detailed Findings (Prioritized)

#### Finding #1: 22+ Stale POs with Zero Goods Receipt CRITICAL

**Evidence:** EKET shows 77+ schedule lines with WEMNG = 0 across POs 3000000037–3000000125. Delivery dates: Nov 2000 – Apr 2001. Total unreceived: 448+ EA, est. ~USD 42K.

**Impact:** Distorts open PO reports, inflates procurement commitments, blocks accurate year-end accruals, and masks actual vendor delivery performance in KPI dashboards.

**Action:** Mass-close via MEMASSPO or ME22N. Set deletion indicator on all POs with EINDT before 2002-01-01. Document closure reason for audit trail.

#### Finding #2: Zero PO Release Strategies Configured CRITICAL

**Evidence:** EKKO.FRGKE is blank for 100% of 500 POs analyzed. No release strategy exists for PO type EC in Purchasing Organization 3000.

**Impact:** Any user with ME21N authorization can create and approve a PO without oversight, regardless of value. This is a controls gap that violates segregation-of-duties principles.

**Action:** Configure release strategies via SPRO → MM → Purchasing → Release Procedures. Define value-based approval thresholds (e.g., >$5K requires manager, >$25K requires director).

#### Finding #3: 40-Day Structural GR Delay HIGH

**Evidence:** POs 3000000004-3000000040 all show GR posted on 2000-12-19 regardless of individual delivery dates ranging from 2000-11-02 to 2000-11-25. Average delay: 40 days across all received POs.

**Impact:** Delays 3-way match, delays vendor payments, and strains vendor relationships. Late GR also means inventory is physically on-hand but not visible in SAP — a planning blindspot.

**Action:** Implement real-time GR posting via MIGO terminals at the warehouse. Review delivery dates to reflect realistic vendor lead times rather than PO creation dates.

#### Finding #4: F110 Never Executed for Org 3000 Vendors HIGH

**Evidence:** REGUH = 0 records for Org 3000 vendors. BSAK = 0 cleared items for same. BSIK company code 3000 = 0 items. The entire payment automation layer is non-operational.

**Action:** Complete vendor master data (ZWELS, ZTERM) for company code 3000. Configure F110 payment variant with correct house bank, payment method, and next payment date.

#### Finding #5: 28% Vendor Records Missing Payment Method HIGH

**Evidence:** ~56 of 200 LFB1 records have ZWELS blank, including key Org 3000 vendors (3020, 5550, 5560, 1000 in BUKRS 3000).

**Action:** Mass update via XK02 (individual) or LSMW (batch). Assign 'C' for US vendors, 'SU' for DE vendors based on LFBK.BANKS country.

## 10Optimization Strategy

The optimization strategy addresses all 5 findings through a prioritized action plan. Each action is rated by Impact (how much it improves P2P cycle health) and Effort (implementation resources required). The strategy delivers quick wins first (Week 1), then builds foundational capabilities (Weeks 2-3), and finally implements advanced monitoring (Weeks 4-8).

### Remediation Priority Matrix

Each action is ranked by Impact × Effort to determine execution priority. P1 (critical) actions are executed immediately; P2 within 2 weeks; P3 within the quarter. The matrix explicitly includes an Owner column to ensure accountability — each action must have a named individual responsible for completion.

#

Action

Impact

Effort

Priority

Owner

R1

Close stale POs (MEMASSPO)

High

Low (1 day)

P1

Procurement Lead

R2

Configure PO release strategies

High

Medium (5d)

P1

MM Consultant

R3

Vendor ZWELS/ZTERM cleanup

High

Low (3d)

P2

Master Data

R4

Configure vendor delivery reminders

Medium

Low (2d)

P2

MM Consultant

R5

Real-time GR at warehouse

High

High (2w)

P3

WM Lead

R6

Configure F110 for BUKRS 3000

High

Medium (5d)

P2

FI Consultant

R7

Activate vendor evaluation ME61

Medium

Medium (5d)

P3

Procurement

R8

Deploy C16 monitoring reports

Medium

Low (3d)

P3

IT / C16

### 🤖 How C16 Accelerates This Optimization

C16 is not just an analysis tool — it is an intelligent SAP co-pilot that can directly execute or dramatically accelerate several of these remediation actions:

*   **R1 (Close stale POs):** C16 can generate an ABAP report that identifies all POs with EKET.EINDT < cutoff AND WEMNG = 0, displays them for review in an ALV grid, and calls BAPI\_PO\_CHANGE to set the deletion indicator after user confirmation — deployed to your ECC system in minutes.
*   **R3 (Vendor cleanup):** C16 builds vendor data quality ALV reports scanning LFB1 and LFBK for gaps — complete with traffic-light indicators and drill-down to vendor master.
*   **R4 (Vendor reminders):** C16 walks through the NACE/MN01 configuration steps interactively, explaining each customizing entry.
*   **R6 (F110 setup):** C16 validates payment method configuration in T042Z, checks vendor readiness, and guides the F110 variant setup step by step.
*   **R8 (Monitoring):** C16 generates PO aging monitors, vendor performance scorecards, and payment readiness dashboards — all deployed as ABAP programs with CL\_SALV\_TABLE output directly on your ECC system.

## 11Quick Wins — Immediate Actions (This Week)

The following actions can be completed within 1 week with minimal risk and immediate measurable impact. Each action addresses a specific finding from §9 and requires no custom development, no transport to production, and no major configuration change. They can be executed by existing team members using standard SAP transactions.

Action

Effort

Owner

ETA

Mass-close POs with EINDT < 2002-01-01 via MEMASSPO — set deletion indicator on header

4 hours

Procurement Lead

Day 1

Schedule monthly ME2M aging report variant for open POs with delivery date < today

1 hour

Procurement Analyst

Day 1

Update ZWELS for top 10 procurement vendors via XK02 — assign C for US, SU for DE

2 hours

Master Data Team

Day 2

Update ZTERM for vendors with blank terms — assign ZB01 as default for Org 3000 vendors

2 hours

Master Data Team

Day 2

Review 6 BSIK items with ZLSPR = R or A — clear blocks if valid, document if retained

1 hour

AP Clerk

Day 3

Train buyers on realistic EINDT — delivery date should reflect vendor lead time, not PO date

30 minutes

Procurement Lead

Day 3

## 12Implementation Roadmap

The roadmap spans 8 weeks across 4 phases, with each phase building on the previous one. Phase 1 (Quick Wins) delivers immediate value; Phase 2 (Configuration) establishes governance and automation foundations; Phase 3 (Monitoring) provides ongoing visibility; Phase 4 (Advanced) transforms the warehouse process to eliminate the root cause of GR delays.

gantt title P2P Remediation Roadmap dateFormat YYYY-MM-DD axisFormat %b %d section Phase 1 Quick Wins Close stale POs :a1, 2026-04-28, 2d Schedule ME2M report :a2, 2026-04-28, 1d Vendor ZWELS ZTERM fix :a3, 2026-04-29, 3d Review BSIK blocks :a4, 2026-04-30, 1d section Phase 2 Config PO release strategies :b1, 2026-05-05, 5d Vendor reminders NACE :b2, 2026-05-05, 3d F110 for BUKRS 3000 :b3, 2026-05-07, 5d section Phase 3 Monitoring C16 PO aging report :c1, 2026-05-14, 3d C16 vendor scorecard :c2, after c1, 3d ME61 vendor evaluation :c3, 2026-05-19, 5d section Phase 4 Advanced Mobile GR warehouse :d1, 2026-05-26, 10d SAPUI5 dashboard :d2, after d1, 5d

Phase

Duration

Key Actions

Expected Impact

**Phase 1**

Week 1

Close stale POs, vendor master cleanup, block review

Eliminate 22+ stale POs, enable F110 for top vendors

**Phase 2**

Weeks 2-3

Release strategies, F110 config, delivery reminders

Approval workflow, automatic payments, vendor accountability

**Phase 3**

Weeks 3-4

C16 ABAP reports, vendor evaluation activation

Continuous visibility into PO aging and vendor performance

**Phase 4**

Weeks 5-8

Mobile GR at warehouse, SAPUI5 procurement dashboard

Reduce GR delay from 40 days to less than 3 days

## 13Expected Outcomes & KPIs

The remediation roadmap targets measurable improvements across 8 dimensions of P2P cycle health. The "Before" values in the table below are drawn directly from the live SAP data analyzed throughout this report. The "After" targets represent conservative estimates based on SAP best practices for organizations that implement the recommended configurations, processes, and monitoring tools. Each row maps directly to a specific finding and remediation action.

### F110 Readiness Score — Before vs. After Remediation

This scorecard provides the definitive before-and-after view of expected improvements. The procurement team should use these targets as KPIs during the 8-week remediation program, reviewing progress at the end of each phase.

Dimension

Before (Current)

After (Target)

Improvement

Stale POs (zero GR)

22+ POs, 448+ EA

0 stale POs

100% elimination

Average GR Delay

~40 days

<3 days

93% reduction

PO Release Strategies

0 configured

Value-based workflow

Full compliance

F110 Automation

Never executed

Monthly payment runs

0% → 100%

Vendor ZWELS Coverage

72%

100%

+28 points

Vendor ZTERM Coverage

76%

100%

+24 points

Procurement Visibility

Manual ME2M only

C16 ALV + SAPUI5

Real-time insights

Vendor Performance

ME61 not activated

Active evaluation

Data-driven sourcing

The total remediation effort is estimated at **6-8 weeks**, with Phase 1 quick wins achievable within the first week and delivering the most immediate, visible improvements to procurement reporting and financial accuracy. C16 can accelerate Phases 3 and 4 by generating the custom ABAP monitoring reports and guiding configuration changes interactively — reducing the timeline by an estimated 30-40% and minimizing reliance on external consultants for standard SAP configurations.

### 🚀 C16's Continuous Value Proposition

This report was generated by C16 in minutes by querying 11 live SAP tables and cross-referencing procurement, payables, vendor master, and configuration data. The equivalent manual analysis would require an experienced SAP consultant 2-3 full working days. Beyond this one-time report, C16 provides ongoing value through:

*   **On-demand P2P health checks** — simply ask "show PO delays" for an instant refresh of all findings
*   **Custom ABAP report generation** — PO aging monitors, vendor scorecards, payment readiness checks, all deployed directly to your ECC system in minutes
*   **Interactive configuration advisory** — step-by-step SPRO guidance for release strategies, output determination, F110 setup, and vendor evaluation
*   **Continuous vendor data quality monitoring** — ongoing scanning of LFB1/LFBK for new gaps as vendors are created or modified
*   **Modern SAPUI5 dashboards** — standalone procurement dashboards running on ECC 7.40 without requiring S/4HANA, connecting to live data via classic OData services

**C16 P2P Procurement Intelligence Report** · Generated 23 April 2026

Data: EKKO, EKPO, EKET, EKBE, LFA1, LFB1, LFBK, BSIK, BSAK, T042Z, T052 · 11 SAP queries executed

All findings verified against live SAP ECC 7.40, Client 800 · Purch. Org 3000

© 2026 C16 — SAP Intelligent Co-Pilot

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._