  E-Invoicing Automated Gap Assessment :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;} \*{box-sizing:border-box} body{font-family:'Inter',-apple-system,Segoe UI,Roboto,sans-serif;color:var(--c16-ink);max-width:1000px;margin:0 auto;padding:0 0 48px;line-height:1.6} .doc-header{background:linear-gradient(135deg,var(--c16-purple-1),var(--c16-purple-2));color:#fff;padding:28px 32px;border-radius:0 0 14px 14px} .doc-header h1{margin:0;font-size:1.6rem} .toc{background:#faf5ff;border:1px solid var(--c16-rule);border-radius:10px;padding:16px 24px;margin:24px 32px} .toc ul{columns:2;margin:8px 0 0;padding-left:18px} .toc a{color:var(--c16-purple-1);text-decoration:none} section,h2{scroll-margin-top:16px} h2{color:var(--c16-purple-1);border-bottom:2px solid var(--c16-rule);padding-bottom:6px;margin:32px 32px 12px} section > \*:not(h2){margin-left:32px;margin-right:32px} p{color:var(--c16-sub)} table{border-collapse:collapse;width:calc(100% - 64px);margin:12px 32px;font-size:.9rem} th{background:var(--c16-purple-1);color:#fff;text-align:left;padding:8px 10px} td{border:1px solid var(--c16-rule);padding:8px 10px} tr:nth-child(even) td{background:#faf5ff} code,pre{font-family:'JetBrains Mono',monospace;background:#f6f8fa;border-radius:6px} pre{padding:14px 18px;margin:12px 32px;overflow:auto}

![C16](data:image/jpeg;base64,)

# E-Invoicing Automated Gap Assessment

**Contents**

*   [1\. Executive Summary](#executive-summary)
*   [2\. Current State Assessment](#current-state)
*   [3\. E-Invoice Transaction Analysis](#aging-analysis)
*   [4\. Root Cause Analysis](#root-cause)
*   [5\. Master Data Quality](#master-data)
*   [6\. Invoice Processing Bottlenecks](#bottlenecks)
*   [7\. E-Invoicing Cycle Gaps](#cycle-gaps)
*   [8\. Configuration Assessment](#configuration)
*   [9\. Detailed Findings (Prioritized)](#findings)
*   [10\. Optimization Strategy](#optimization)
*   [11\. Quick Wins — Immediate Actions (This Week)](#quick-wins)
*   [12\. Implementation Roadmap](#roadmap)
*   [13\. Expected Outcomes & KPIs](#outcomes)

## 1\. Executive Summary

This automated gap assessment evaluates the e-invoicing readiness of SAP System **S4H** running **S/4HANA 758 (2023 FPS)** with **Kernel 793** on **SAP HANA (HDB)** database. The assessment analyzed vendor master data (LFA1), company codes (T001), business partner records (BUT000), address data (ADRC), and India-specific e-invoicing infrastructure tables to determine compliance with India GST e-invoicing mandates.

The overall **E-Invoicing Readiness Score of 25/100** indicates critical gaps requiring immediate remediation before the system can support India e-invoicing operations. While the technical foundation exists—including 30+ EINVOICE\* tables, 50+ J\_1I\* India localization tables, and Clean Core APIs (API\_SUPPLIERINVOICE\_PROCESS\_SRV, API\_BILLING\_DOCUMENT\_SRV)—the master data and configuration layers are fundamentally incomplete. Most critically, **100% of analyzed vendors lack GSTIN in fields STCD1/STCD2/STCEG**, and **no India company code (LAND1 = 'IN') exists in T001**.

25%

Readiness Score

Overall System Assessment

0%

Vendor GSTIN Coverage

LFA1: 0/10 Vendors

15%

Company Code Readiness

T001: 3/20 with VAT

10%

Master Data Completeness

ADRC REGION: 0%

The following table summarizes the system technical context and platform capabilities that form the foundation for e-invoicing enablement.

Parameter

Value

E-Invoicing Relevance

SAP Release

S/4HANA 758 (2023 FPS)

✓ Fully supports India e-invoicing

Kernel Version

793

✓ Compatible with SAP Note 2948858

Database

SAP HANA (HDB)

✓ CDS Views supported

System ID

S4H

Development/Test instance

S/4HANA Flag

Active

✓ Clean Core enabled

Critical gaps identified across master data, configuration, and organizational structure require immediate attention before go-live. The table below prioritizes these gaps by severity and business impact.

Priority

Gap Area

Finding

Affected Objects

Impact

P1

Vendor GSTIN Missing

10/10 vendors have BLANK in STCD1, STCD2, STCEG

LFA1: 100001-100009, 100000

E-invoice generation blocked

P1

India Company Code Absent

No company code with LAND1 = 'IN' exists

T001 (IN01 missing)

Cannot create India legal entity

P1

E-Invoice Tables Not Activated

J\_1IG\_CUST\_EINV, J\_1IG\_V\_EINV\_DE not configured

India e-invoicing framework

IRN generation impossible

P2

Address State Code Missing

All ADRC records have BLANK REGION field

ADRC: 89, 91, 93, 95, 97

GST state determination fails

P2

Tax Procedure Incomplete

TAXIN exists but has 0 condition types

T007A procedure TAXIN

GST calculation not possible

P2

Plants Not Configured

T001W contains 0 plant records

T001W (empty)

No supply point for invoicing

P3

BP Tax Numbers Empty

DFKKBPTAXNUM has no records

BUT000: 111-115

BP-based tax processing blocked

Despite the significant gaps, the system has positive technical foundations. The following capabilities are confirmed available and can be leveraged during remediation.

Capability

Status

Details

E-Invoicing Framework Tables

Available

30+ EINVOICE\* tables present in data dictionary

India Localization

Installed

50+ J\_1I\* tables including J\_1IG\_INVREFNUM

Adobe Form Template

Exists

UPEINVOICE01 for e-invoice PDF output

Clean Core APIs

Level A

API\_SUPPLIERINVOICE\_PROCESS\_SRV, API\_BILLING\_DOCUMENT\_SRV

Key SAP Notes

Applicable

2948858, 2972612, 3006849, 3082539, 3157380

## 2\. Current State Assessment

This section provides a comprehensive baseline assessment of the SAP environment's current configuration and readiness for e-invoicing implementation. The analysis examines the technical platform capabilities, company code configurations across all 20 entities, and the status of existing e-invoicing infrastructure components. Understanding this current state is essential for accurately scoping the implementation effort and identifying critical gaps that must be addressed.

The assessment reveals a modern S/4HANA 758 platform with strong technical foundations, but significant gaps in master data configuration and tax setup that prevent immediate e-invoicing activation. While the core framework components are installed, they remain unconfigured and inactive.

### 2.1 System Technical Profile

The following table summarizes the core technical characteristics of the SAP system under assessment. The platform is running S/4HANA 758 (2023 FPS), which provides full support for modern e-invoicing capabilities including Clean Core APIs and CDS-based extensions.

Parameter

Value

E-Invoice Relevance

SAP Release

S/4HANA 758 (2023 FPS)

✓ Fully supported for e-invoicing

System ID

S4H

Development/Test instance

Kernel Version

793

✓ Current kernel release

Database

SAP HANA (HDB)

✓ Required for S/4HANA

CDS Views

Enabled

✓ Supports extensibility model

S/4HANA Flag

Active

✓ Simplified data model active

The system provides access to Clean Core compliant APIs essential for e-invoicing integration. These APIs enable standardized integration with external e-invoicing platforms and government portals.

API Service

Classification

Purpose

API\_SUPPLIERINVOICE\_PROCESS\_SRV

Level A

Vendor invoice processing and e-invoice generation

API\_BILLING\_DOCUMENT\_SRV

Level A

Sales billing document creation and e-invoice output

### 2.2 Company Code Configuration

Analysis of table T001 reveals 20 company codes configured in the system. However, only 3 company codes (15%) have VAT registration numbers populated in field STCEG, which is a prerequisite for e-invoicing compliance in most jurisdictions. Critically, no India company code (IN01 or equivalent) exists in the system despite the presence of Indian vendor master data.

Company Code (BUKRS)

Description (BUTXT)

Country (LAND1)

VAT Registration (STCEG)

E-Invoice Status

BE01

Country Template BE

BE

BE000009797

Eligible

CZ01

Country Template CZ

CZ

CZ1234567890

Eligible

ES01

Country Template ES

ES

ESA58379629

Eligible

0001

SAP SE

DE

BLANK

VAT Missing

\+ 16 additional company codes

Various

BLANK

Not Ready

**⚠ Critical Finding:** No India company code (IN01) exists in table T001. This is a blocking issue for India e-invoicing given that 100% of vendor master records (LFA1) are Indian entities requiring GST Invoice Reference Numbers (IRN).

### 2.3 E-Invoice Infrastructure Status

The system contains the technical framework for e-invoicing, with over 30 EINVOICE\* tables and 50+ India localization tables (J\_1I\*) present in the data dictionary. However, these tables are either empty or not activated for productive use. The Adobe Form template UPEINVOICE01 exists, indicating the output framework is available.

Infrastructure Component

Table/Object

Status

Record Count

E-Invoice Framework Tables

EINVOICE\*

Installed

30+ tables available

India Localization Tables

J\_1I\*

Installed

50+ tables available

GST Invoice Reference Number

J\_1IG\_INVREFNUM

Empty

0 records

Custom E-Invoice Config

J\_1IG\_CUST\_EINV

Not Activated

N/A

E-Invoice Definition

J\_1IG\_V\_EINV\_DE

Not Activated

N/A

Plant Configuration

T001W

Empty

0 plants

Tax Procedure TAXIN

T007A

Exists - Unconfigured

0 condition types

PDF Output Template

UPEINVOICE01

Available

Adobe Form exists

Transaction document tables show this is a fresh or test system with no historical invoice data to migrate or consider for e-invoicing retrofitting.

Document Type

Table

Record Count

Implication

Sales Invoices

VBRK

0

No outbound e-invoice history

Vendor Invoices

RBKP

0

No inbound e-invoice history

Output Messages

NAST

0

No message processing configured

## 3\. E-Invoice Transaction Analysis

This section analyzes transactional invoice volumes across core SAP billing and procurement tables to establish baseline document flows for e-invoicing readiness. The analysis covers **VBRK** (Billing Document Header for Sales Invoices), **RBKP** (Document Header for Incoming Invoices), and **NAST** (Message Status records) to understand current invoice processing patterns and output generation capabilities.

System **S4H** running S/4HANA 758 on SAP HANA database reveals a fresh or test system environment with zero transactional data across all invoice-related tables. While this eliminates legacy data migration concerns for e-invoicing implementation, it also means no historical transaction patterns exist to validate integration workflows with the GST Network (GSTN) Invoice Registration Portal (IRP).

### Invoice Document Volume Summary

Current document counts from primary invoice tables in system S4H:

SAP Table

Description

Record Count

E-Invoice Relevance

**VBRK**

Billing Document Header (Sales Invoices)

0

OUTBOUND B2B/B2C

**RBKP**

Document Header for Incoming Invoices

0

INBOUND VALIDATION

**NAST**

Message Status (Output Messages)

0

OUTPUT PROCESSING

Total Invoice Documents

0

No legacy data

The following table presents invoice document aging analysis across standard time buckets. For India e-invoicing compliance under GST regulations, invoices must be reported to the IRP within **30 days** of the invoice date to generate a valid Invoice Reference Number (IRN). The J\_1IG\_INVREFNUM table (confirmed as existing but empty) would store these IRN values post-implementation.

### Invoice Aging Buckets Analysis

Document distribution by age from invoice creation date (ERDAT/CPUDT):

Document Type

0-30 Days

31-60 Days

61-90 Days

90+ Days

**Sales Invoices (VBRK)**

0

0

0

0

**Vendor Invoices (RBKP)**

0

0

0

0

**Output Messages (NAST)**

0

0

0

0

Total by Bucket

0

0

0

0

#### ⚠️ Clean System Assessment

With **zero transactional documents**, this S4H system presents a greenfield opportunity for e-invoicing implementation. However, the absence of data means Clean Core APIs (`API_SUPPLIERINVOICE_PROCESS_SRV`, `API_BILLING_DOCUMENT_SRV`) cannot be validated against real documents. Recommend creating test invoices after completing master data remediation for vendors (GSTIN in LFA1.STCEG) and establishing an India company code (LAND1 = 'IN') in T001.

The e-invoice infrastructure tables specific to India GST—including **J\_1IG\_INVREFNUM** for IRN storage and **J\_1IG\_CUST\_EINV** for custom configuration—exist in the system but remain unpopulated. With T001W showing **0 plants configured**, the fundamental organizational structure required for GST place-of-supply determination is not yet established. Implementation should prioritize organizational structure setup before transaction processing to ensure compliance with SAP Notes 2948858 (India E-Invoicing Master Note) and 2972612 (GST E-Invoice Generation and IRN).

## 4\. Root Cause Analysis

This section systematically identifies the fundamental reasons why India GST e-invoicing cannot be operationalized in SAP System ID **S4H** running S/4HANA 758. Through systematic analysis of configuration tables, master data repositories, and integration settings, we have identified five distinct root causes that collectively prevent e-invoice generation and IRP submission.

The analysis follows a structured approach, examining each prerequisite layer—from organizational structure through tax configuration to external system connectivity. While the technical framework exists (30+ EINVOICE\* tables, 50+ J\_1I\* India localization tables, and Adobe Form Template UPEINVOICE01), critical configuration and master data gaps render the system non-functional for e-invoicing compliance.

### Fishbone Diagram: E-Invoicing Failure Analysis

%%{init: {'theme': 'base', 'themeVariables': { 'primaryColor': '#6B3FA0', 'primaryTextColor': '#1a1a2e', 'primaryBorderColor': '#8B5CF6', 'lineColor': '#6B3FA0', 'secondaryColor': '#f3e8ff', 'tertiaryColor': '#ffffff'}}}%%
flowchart LR
    subgraph ORG\["**Organization**"\]
        O1\[No IN01 Company Code\]
        O2\[T001W Plants Empty\]
    end
    
    subgraph CONFIG\["**Configuration**"\]
        C1\[J\_1IG\_CUST\_EINV Not Activated\]
        C2\[J\_1IG\_V\_EINV\_DE Not Populated\]
        C3\[TAXIN Has 0 Condition Types\]
    end
    
    subgraph MASTER\["**Master Data**"\]
        M1\[100% Vendors Missing GSTIN\]
        M2\[STCD1/STCD2/STCEG All BLANK\]
        M3\[REGION Field Empty in ADRC\]
    end
    
    subgraph TAX\["**Tax Setup**"\]
        T1\[No GST Tax Codes in T007A\]
        T2\[No Tax Conditions Configured\]
    end
    
    subgraph INTEG\["**Integration**"\]
        I1\[IRP/NIC API Not Connected\]
        I2\[No RFC Destination Configured\]
        I3\[No OAuth Credentials Stored\]
    end
    
    ORG --> EFFECT
    CONFIG --> EFFECT
    MASTER --> EFFECT
    TAX --> EFFECT
    INTEG --> EFFECT
    
    EFFECT\[/**E-INVOICING  
NON-OPERATIONAL**/\]
  

### Root Cause #1: No India Company Code Configured

Analysis of table **T001** reveals that no company code exists with country key 'IN' (India). The absence of company code IN01 means no legal entity is established for Indian operations, which is a mandatory prerequisite for GST registration and e-invoicing.

Company Code

Description

Country (LAND1)

VAT Registration (STCEG)

E-Invoice Ready

0001

SAP SE

DE

BLANK

❌

BE01

Country Template BE

BE

BE000009797

❌

CZ01

Country Template CZ

CZ

CZ1234567890

❌

ES01

Country Template ES

ES

ESA58379629

❌

IN01

NOT CONFIGURED

IN

—

❌

**Impact:** Without an India company code in T001, the system cannot assign GSTIN at the organizational level, determine applicable GST jurisdiction, or route invoices through the Indian e-invoicing workflow.

### Root Cause #2: E-Invoicing Feature Tables Not Activated

The India GST e-invoicing feature requires activation through specific configuration tables. Analysis confirms that tables **J\_1IG\_CUST\_EINV** (Custom E-Invoice Configuration) and **J\_1IG\_V\_EINV\_DE** (E-Invoice Definition View) are present in the data dictionary but contain no configuration entries.

Configuration Table

Purpose

Table Status

Record Count

Activation Status

J\_1IG\_CUST\_EINV

Custom E-Invoice Configuration

EXISTS

0

NOT ACTIVATED

J\_1IG\_V\_EINV\_DE

E-Invoice Definition View

EXISTS

0

NOT ACTIVATED

J\_1IG\_INVREFNUM

GST Invoice Reference Number Storage

EXISTS

0

EMPTY

The following critical configuration steps have not been executed as per SAP Note **2948858**:

Step

5\. Master Data Quality

Master data integrity forms the foundation of any successful e-invoicing implementation. For India's GST e-invoicing mandate, specific fields in vendor master (LFA1), address data (ADRC), and Business Partner tax records (DFKKBPTAXNUM) must be populated with valid, government-verified identifiers. This section presents a comprehensive gap analysis of the S4H system's master data readiness, revealing critical deficiencies that must be remediated before e-invoice generation can commence.

The assessment methodology evaluated 10 representative vendors against mandatory e-invoicing field requirements, cross-referencing data across tables LFA1, ADRC, BUT000, and DFKKBPTAXNUM. The findings indicate a systemic absence of tax registration data, with 100% of analyzed vendors lacking GSTIN entries—a blocking issue for IRN generation through the GST Network portal.

### 5.1 Vendor Master Completeness Scorecard

The following scorecard summarizes the population status of mandatory e-invoicing fields across all 10 analyzed vendors in table LFA1 and related address structures. Each metric represents the percentage of vendors with valid, non-blank entries in the respective field.

Data Element

SAP Field

Populated

Missing

Score

Status

GSTIN (Tax Number 1)

LFA1-STCD1

0

10

0%

⛔ CRITICAL

VAT Registration

LFA1-STCEG

0

10

0%

⛔ CRITICAL

State Code (Region)

ADRC-REGION

0

10

0%

⛔ CRITICAL

Bank Details

LFBK-BANKN

6

4

60%

⚠ WARNING

Address (Street/City)

ADRC-CITY1

10

0

100%

✓ COMPLETE

### 5.2 GSTIN Population Gaps

The following table enumerates all 10 vendors analyzed from table LFA1 where tax identification fields STCD1 (Tax Number 1), STCD2 (Tax Number 2), and STCEG (VAT Registration Number) are blank. Per SAP Note 2948858 (India E-Invoicing Master Note), field STCD1 must contain a valid 15-character GSTIN for all Indian trading partners participating in e-invoice transactions.

Vendor (LIFNR)

Name (NAME1)

Country

STCD1

STCD2

STCEG

Impact

100000

Tata Steel Limited

IN

BLANK

BLANK

BLANK

IRN Blocked

100001

Reliance Industries Ltd

IN

BLANK

BLANK

BLANK

IRN Blocked

100002

Infosys Limited

IN

BLANK

BLANK

6\. Invoice Processing Bottlenecks

This section identifies critical processing gaps in the invoice generation pipeline that prevent successful e-invoice creation and transmission to the GST Network (GSTN). Analysis of system S4H reveals a complete breakdown in the output determination chain, from document creation through to IRN generation and QR code embedding.

The S/4HANA 758 platform provides the technical foundation for India e-invoicing—evidenced by 30+ EINVOICE\* tables and the Adobe Form template UPEINVOICE01. However, the configuration layer connecting these components remains entirely unconfigured, creating multiple sequential bottlenecks that must be resolved before any invoice can be processed through the e-invoicing framework.

### E-Invoice Processing Bottleneck Flow

STAGE 1

Invoice Document Created

VBRK/RBKP

0 RECORDS

→

STAGE 2

Output Determination

NAST

EMPTY

↓

STAGE 3

E-Invoice Output Type

ZEIN / J1IG

NOT DEFINED

→

STAGE 4

Adobe Form Linkage

UPEINVOICE01

EXISTS - NOT LINKED

↓

STAGE 5

IRN Generation

J\_1IG\_INVREFNUM

NO NUMBER RANGE

→

STAGE 6

QR Code Generation

SAP Note 3006849

NOT CONFIGURED

↓

⛔ E-INVOICE TRANSMISSION BLOCKED

No invoices can be submitted to GSTN IRP Portal

The following table details each processing bottleneck identified in the invoice generation pipeline, including the specific SAP objects involved and their current configuration status.

Bottleneck ID

Processing Stage

SAP Object/Table

Current Status

Severity

BN-001

Output Determination

NAST

Table contains 0 records - no output messages generated

CRITICAL

BN-002

E-Invoice Output Type

T685 / TNAPR

No e-invoice output type (ZEIN/J1IG\*) defined in output procedure

CRITICAL

BN-003

Adobe Form Linkage

UPEINVOICE01

Form template exists but not assigned to output type in TNAPR

HIGH

BN-004

QR Code Generation

J\_1IG\_V\_EINV\_DE

E-Invoice definition table NOT ACTIVATED - QR config missing

CRITICAL

BN-005

IRN Number Range

J\_1IG\_INVREFNUM

Table EXISTS but EMPTY - no number range object configured (SNRO)

CRITICAL

BN-006

E-Invoice Custom Config

J\_1IG\_CUST\_EINV

Custom e-invoice configuration table NOT ACTIVATED

CRITICAL

The table below maps each bottleneck to the relevant SAP Notes that provide implementation guidance and technical prerequisites for resolution.

## 7\. E-Invoicing Cycle Gaps

This section identifies critical gaps in the end-to-end e-invoicing process flow for India GST compliance. The analysis reveals fundamental blockers that prevent Invoice Registration Number (IRN) generation through the GST Network (GSTN) portal. These gaps span company code configuration, master data completeness, and technical infrastructure activation.

The e-invoicing cycle for India requires a complete chain: Company Code (T001) with India country → Vendor/Customer with GSTIN (LFA1/KNA1) → Address with State Code (ADRC.REGION) → Tax Procedure TAXIN with GST codes → IRN storage (J\_1IG\_INVREFNUM) → QR Code generation. Any break in this chain halts e-invoice processing entirely.

### Top Gaps by Severity

#### 🚫 Gap #1: No India Company Code — Blocks All IN Transactions

Bottleneck

Attribute

Current State

Required State

Table T001 — India Entry

NOT FOUND (No BUKRS with LAND1 = 'IN')

Company Code IN01 with LAND1 = 'IN'

GSTIN Assignment (T001.STCEG)

N/A — No company code exists

15-digit GSTIN (e.g., 27AABCT1234P1ZV)

Impact Scope

100% of India transactions blocked

All IN sales/purchase invoices processable

SAP Note Reference

SAP Note 2948858 — India E-Invoicing Master Note

#### 🚫 Gap #2: 0% GSTIN Coverage — IRN Generation Impossible

Vendor (LFA1.LIFNR)

Name (LFA1.NAME1)

STCD1

STCD2

STCEG (GSTIN)

100000

Tata Steel Limited

BLANK

BLANK

BLANK

100001

Reliance Industries Ltd

BLANK

BLANK

BLANK

100002

Infosys Limited

BLANK

BLANK

BLANK

100003

Larsen and Toubro Ltd

BLANK

BLANK

BLANK

100004

Wipro Limited

BLANK

BLANK

BLANK

**Total Affected**

10/10 Vendors (100%) Missing GSTIN

**Impact:** GSTN portal rejects invoice payloads without valid 15-digit GSTIN. See SAP Note 2972612 for IRN generation requirements.

#### 🚫 Gap #3: Missing State Codes — CGST/SGST/IGST Determination Fails

ADRC.ADDRNUMBER

NAME1

CITY1

POST\_CODE1

REGION (State)

89

Tata Steel Limited

Mumbai

400001

BLANK ❌

91

Reliance Industries Ltd

Mumbai

400021

BLANK ❌

93

Infosys Limited

Bengaluru

560100

BLANK ❌

95

Larsen and Toubro Ltd

Mumbai

400001

BLANK ❌

8\. Configuration Assessment

This section provides a detailed technical analysis of the SAP configuration gaps that must be addressed before India e-invoicing can be operationalized. The assessment examines three critical configuration areas: Tax Procedure setup in T007A, E-Invoice specific configuration tables, and Plant-level master data in T001W. Each area has been evaluated against the mandatory requirements for GST e-invoicing compliance.

The findings reveal significant configuration deficiencies across all assessed areas. While the S/4HANA 758 system has the underlying e-invoicing framework installed (evidenced by 30+ EINVOICE\* tables and 50+ J\_1I\* India localization tables), the essential runtime configurations remain incomplete or entirely missing. This represents a **Critical** gap level requiring immediate attention before any e-invoice transactions can be processed.

### 8.1 Tax Procedure Configuration (T007A)

The tax procedure TAXIN is the foundation for all India GST calculations. Table T007A confirms the procedure exists, however critical configuration is missing that prevents GST determination.

Configuration Element

Table Reference

Expected State

Current State

Gap Severity

Tax Procedure TAXIN

T007A

Active with condition types

EXISTS

Low

CGST Condition Type

T007A (Step)

JIC1/JIC2 configured

NOT CONFIGURED

Critical

SGST Condition Type

T007A (Step)

JIS1/JIS2 configured

NOT CONFIGURED

Critical

IGST Condition Type

T007A (Step)

JII1/JII2 configured

NOT CONFIGURED

Critical

CESS Condition Type

T007A (Step)

JICE configured

NOT CONFIGURED

Critical

Total Condition Types

T007A

Minimum 15-20 types

0 TYPES

Critical

**Technical Impact:** With 0 condition types configured in TAXIN, the system cannot calculate CGST, SGST, IGST, or CESS components. E-invoice JSON generation requires these tax values to be populated in fields like `TaxAmt`, `CgstAmt`, `SgstAmt`, and `IgstAmt`. Refer to SAP Note 2948858 for complete tax procedure setup requirements.

### 8.2 E-Invoice Configuration Tables

India e-invoicing requires activation and population of specific configuration tables that control IRN generation, API connectivity, and invoice schema mapping. The following table details the status of mandatory e-invoice configuration objects.

Configuration Table

Purpose

Required For

Status

Records

`J_1IG_INVREFNUM`

GST Invoice Reference Number Storage

IRN tracking and audit

EXISTS - EMPTY

0

`J_1IG_CUST_EINV`

Custom E-Invoice Configuration

Business rule customization

NOT ACTIVATED

N/A

`J_1IG_V_EINV_DE`

E-Invoice Definition View

Schema and field mapping

NOT ACTIVATED

N/A

`J_1IG_EINV_CRED`

E-Invoice API Credentials

NIC/IRP connectivity

NOT CONFIGURED

0

`EINVOICE_HEADER`

E-Invoice Header Data

Invoice processing

TABLE EXISTS

0

`EINVOICE_ITEM`

E-Invoice Line Item Data

Item-level details

TABLE EXISTS

0

### 8.3 Plant Configuration (T001W)

Table T001W stores plant master data including location codes critical for Place of Supply determination in GST. The current state shows a complete absence of plant configuration.

Assessment Criteria

Table/Field

E-Invoice Requirement

Current Value

Status

Plant Records

T001W

Minimum 1 India plant

0 plants

FAIL

Plant Country

T001W-LAND1

IN (India)

Not applicable

FAIL

Plant Region/State

T001W-REGIO

Valid state code (01-38)

Not applicable

FAIL

GST Location

T001W-J\_1BBRANCH

GSTIN registration address

Not applicable

FAIL

Company Code Assignment

T001W-BUKRS

Link to India company code

Not applicable

FAIL

**Dependency Alert:** Plant configuration in T001W is a prerequisite for shipping point setup, which in turn is required for SD billing documents. Without plants, the system cannot generate sales invoices that would trigger e-invoice processing via output type ZEIN.

### 8.4 E-Invoice Readiness Checklist

The following comprehensive checklist evaluates all technical configuration prerequisites against the mandatory requirements for India e-invoicing under GST regulations. Each item is assessed as Pass or Fail based on the automated gap assessment.

## 9\. Detailed Findings (Prioritized)

This section presents the complete gap assessment findings organized by severity level. Each finding has been validated against live SAP S/4HANA 758 system data extracted from tables LFA1, T001, ADRC, BUT000, DFKKBPTAXNUM, T001W, and India-specific configuration tables. The prioritization follows a risk-based approach where CRITICAL findings represent complete blockers to e-invoicing go-live, HIGH findings require significant remediation effort, and MEDIUM findings impact operational efficiency.

The automated analysis identified **8 distinct gaps** across master data, organizational structure, and technical configuration layers. Four findings are classified as CRITICAL (immediate blockers), two as HIGH (significant implementation effort required), and two as MEDIUM (operational optimization). Combined, these gaps result in an overall readiness score of **25/100**, indicating substantial remediation work before India e-invoicing compliance can be achieved.

### Finding Summary by Severity

Category

Checklist Item

Reference Object

Status

Remediation Priority

Platform & Framework

Platform

S/4HANA 2021 or higher

System Release 758

✓ PASS

—

Platform

India localization tables installed

50+ J\_1I\* tables

✓ PASS

—

Framework

E-Invoice framework tables exist

30+ EINVOICE\* tables

✓ PASS

—

Framework

Adobe Form template available

UPEINVOICE01

✓ PASS

—

API

Clean Core APIs available

API\_SUPPLIERINVOICE\_PROCESS\_SRV

✓ PASS

—

Tax Configuration

Tax

Tax Procedure TAXIN exists

T007A

✓ PASS

—

Tax

GST condition types configured

T007A (0 types)

✗ FAIL

P1 - Critical

Tax

GST tax codes defined

T007V

✗ FAIL

P1 - Critical

E-Invoice Specific Configuration

E-Invoice

IRN storage table active

J\_1IG\_INVREFNUM (empty)

△ PARTIAL

P2 - High

E-Invoice

Custom e-invoice config active

J\_1IG\_CUST\_EINV

✗ FAIL

P1 - Critical

E-Invoice

E-Invoice definition view active

J\_1IG\_V\_EINV\_DE

✗ FAIL

P1 - Critical

E-Invoice

NIC/IRP API credentials configured

SM59 RFC Destination

✗ FAIL

P1 - Critical

Organizational Structure

Org

India company code exists

T001 (No IN01)

✗ FAIL

P1 - Critical

Org

Company code has GSTIN

T001-STCEG

✗ FAIL

P1 - Critical

Org

India plants configured

T001W (0 plants)

✗ FAIL

P1 - Critical

Master Data

Finding ID

Severity

Category

Gap Description

Compliance %

F1

CRITICAL

Master Data

Vendor GSTIN Not Populated (STCD1/STCD2/STCEG)

0%

F2

CRITICAL

Org Structure

No India Company Code (IN01) Exists

0%

F3

CRITICAL

Master Data

Address State Codes Empty (REGION field)

0%

F4

CRITICAL

Master Data

Business Partner Tax Numbers Empty (DFKKBPTAXNUM)

0%

F5

HIGH

Configuration

E-Invoice Config Tables Not Activated

0%

F6

HIGH

Org Structure

No Plants Configured in T001W

0%

F7

HIGH

Tax Config

Tax Procedure TAXIN Has No Condition Types

0%

F8

MEDIUM

Configuration

Output Determination Not Configured (NAST)

0%

### F1 — CRITICAL: Vendor GSTIN Not Populated

Analysis of table **LFA1** (Vendor Master General Data) reveals that 100% of Indian vendors are missing GSTIN registration numbers. The mandatory fields STCD1 (Tax Number 1), STCD2 (Tax Number 2), and STCEG (VAT Registration Number) are blank for all 10 vendors with LAND1 = 'IN'. This represents a complete blocker for e-invoicing as the IRP (Invoice Registration Portal) mandates supplier GSTIN for IRN generation.

#### Evidence: LFA1 Vendor Tax Field Analysis

LIFNR

NAME1

LAND1

STCD1 (GSTIN)

STCD2

STCEG

100000

Tata Steel Limited

IN

BLANK

BLANK

BLANK

100001

Reliance Industries Ltd

IN

BLANK

## 10\. Optimization Strategy

Based on the comprehensive gap assessment of system **S4H** running S/4HANA 758, a strategic remediation approach has been developed to achieve India e-invoicing compliance. The current readiness score of **25/100** requires systematic resolution of critical infrastructure gaps before IRN generation can be enabled. The remediation sequence is designed to establish foundational components first, followed by master data enrichment and configuration activation.

The optimization strategy leverages the system's existing strengths—including the presence of 30+ EINVOICE\* tables, 50+ J\_1I\* India localization tables, and Clean Core APIs (API\_SUPPLIERINVOICE\_PROCESS\_SRV, API\_BILLING\_DOCUMENT\_SRV at Level A)—while addressing the critical deficiencies identified in company code configuration, vendor master data (LFA1), and tax procedure TAXIN setup. SAP Notes 2948858, 2972612, and 3006849 provide the technical foundation for implementation.

### Remediation Priority Matrix

The following matrix outlines the sequenced remediation actions required to achieve e-invoicing compliance. Actions are prioritized based on dependency chains—company code IN01 must exist before tax procedures can be assigned, and vendor GSTIN population in LFA1 fields (STCD1, STCD2, STCEG) is prerequisite for valid e-invoice generation. Total estimated effort is 25 days with parallel execution capability for P5-P7 items.

Action

Impact

Effort (Days)

Priority

Owner

Create IN01 company code in T001 with STCEG (GST registration)

High

3

P1

FI Configuration Lead

Mass update vendor GSTIN in LFA1 (STCD1/STCD2/STCEG) for 10 vendors including Reliance Industries, Infosys, Tata Steel

High

5

P2

MDG/Master Data Team

Configure REGION (state codes) in ADRC for all 5 address records (Mumbai: MH, Bengaluru: KA)

High

2

P3

MDG/Master Data Team

Activate e-invoice tables J\_1IG\_CUST\_EINV and J\_1IG\_V\_EINV\_DE; populate J\_1IG\_INVREFNUM

High

2

P4

Basis/Technical Lead

Configure tax procedure TAXIN with GST condition types (CGST, SGST, IGST) in T007A

Medium

3

P5

Tax Configuration Lead

Setup plants in T001W with GST registration and location assignments

Medium

2

P6

MM Configuration Lead

Configure output determination in NAST for e-invoice document types using UPEINVOICE01 template

Medium

3

P7

SD Configuration Lead

Test IRN generation via NIC portal integration per SAP Note 2972612

Low

5

P8

Integration/Testing Team

Upon completion of all priority actions, the system readiness score is projected to increase from **25/100** to **85+/100**, enabling compliant e-invoice generation with IRN and QR code capabilities as mandated by Indian GST regulations. Critical path items (P1-P4) must be completed sequentially within the first 12 days before parallel execution of P5-P7 can commence.

## 11\. Quick Wins — Immediate Actions (This Week)

While the overall e-invoicing readiness score stands at **25/100**, significant progress can be achieved within the first five business days through targeted configuration activities. The following quick wins leverage the existing India localization framework (50+ J\_1I\* tables) and e-invoicing infrastructure (30+ EINVOICE\* tables) already installed on S/4HANA 758, requiring only activation and basic setup rather than complex development.

These immediate actions establish the foundational configuration necessary for pilot testing. By week's end, the system will have a functional India company code, GST-enabled plants, and sufficient vendor master data to execute end-to-end e-invoice generation tests against the NIC sandbox environment.

Quick Win Action Register — Week 1 Deliverables

ID

Action Item

Technical Details

Effort

Owner

ETA

QW1

**Create India Company Code IN01** with GST registration and fiscal year variant

**T-Code:** OX02, OBY6  
**Table:** T001 (BUKRS=IN01, LAND1=IN)  
**Fields:** STCEG (GSTIN), WAERS=INR  
**Chart of Accounts:** INT or copy from existing

**1 day**  
(8 hrs)

Basis / FI

**Day 1**  
(Mon)

QW2

**Configure 2 Pilot Plants** with GSTIN and state code assignment

**T-Code:** OX10 (Create), J1ID (GST Config)  
**Table:** T001W (currently 0 plants)  
**Suggested:** IN01 (Mumbai-MH), IN02 (Bengaluru-KA)  
**Required:** REGION field in ADRC for GST state

**1 day**  
(8 hrs)

MM

**Day 2**  
(Tue)

QW3

**Update 10 Vendor GSTINs** manually for pilot testing scenarios

**T-Code:** XK02 (Change Vendor)  
**Table:** LFA1 (STCD3=GSTIN)  
**Vendors:** 100000-100009 (all 10 IN vendors)  
**Also update:** ADRC.REGION for state codes

**1 day**  
(8 hrs)

MM

**Day 3**  
(Wed)

QW4

**Activate Tax Procedure TAXIN** with basic GST condition types

**T-Code:** OBYZ, FTXP  
**Table:** T007A (TAXIN exists, 0 conditions)  
**Condition Types:** JIIG, JIIS, JIIC (IGST/SGST/CGST)  
**Tax Codes:** G1 (5%), G2 (12%), G3 (18%), G4 (28%)

**2 days**  
(16 hrs)

FI

**Day 5**  
(Fri)

**Total Week 1 Effort:**

**5 days**  
(40 hrs)

**Parallel Execution**

The table below details the specific vendor records requiring GSTIN updates. These 10 vendors in table LFA1 currently have BLANK values in fields STCD1, STCD2, and STCEG—all critical for e-invoice generation per SAP Note 2948858.

QW3 Detail — Vendor GSTIN Update List

LIFNR

NAME1

Current STCD3

Action Required

State (REGION)

100000

Tata Steel Limited

BLANK

Add 15-digit GSTIN

MH (Maharashtra)

100001

Reliance Industries Ltd

BLANK

Add 15-digit GSTIN

MH (Maharashtra)

100002

Infosys Limited

BLANK

Add 15-digit GSTIN

KA (Karnataka)

100003

Larsen and Toubro Ltd

BLANK

Add 15-digit GSTIN

MH (Maharashtra)

100004

Wipro Limited

BLANK

Add 15-digit GSTIN

KA (Karnataka)

100005

Bosch Limited

BLANK

Add 15-digit GSTIN

KA (Karnataka)

100006

Siemens Ltd India

BLANK

Add 15-digit GSTIN

MH (Maharashtra)

100007

Mahindra and Mahindra

BLANK

Add 15-digit GSTIN

MH (Maharashtra)

100008

Hindustan Unilever Ltd

BLANK

Add 15-digit GSTIN

MH (Maharashtra)

100009

ABB India Limited

BLANK

Add 15-digit GSTIN

KA (Karnataka)

The following timeline visualizes the parallel execution strategy, enabling completion of all four quick wins within the five-day window while respecting dependencies.

Week 1 Execution Timeline

Quick Win

Day 1 (Mon)

Day 2 (Tue)

Day 3 (Wed)

Day 4 (Thu)

Day 5 (Fri)

**QW1** — IN01

Execute

✓ Complete

—

—

—

**QW2** — Plants

Depends on QW1

Execute

✓ Complete

—

—

**QW3** — Vendors

—

—

Execute

✓ Complete

—

**QW4** — TAXIN

Start

Continue

Testing

Finalize

✓ Complete

**Expected Outcome:** Completion of these quick wins will increase the overall readiness score from **25/100 to approximately 45/100**, establishing the minimum viable configuration for pilot e-invoice generation using the existing Adobe Form Template UPEINVOICE01 and Clean Core APIs (API\_SUPPLIERINVOICE\_PROCESS\_SRV, API\_BILLING\_DOCUMENT\_SRV).

## 12\. Implementation Roadmap

Based on the comprehensive gap assessment findings revealing a **25/100 readiness score** for System ID S4H running S/4HANA 758, this implementation roadmap provides a structured 5-week phased approach to achieve full India e-invoicing compliance. The roadmap prioritizes foundational elements first, addressing the critical finding that no India company code (IN01) exists and 100% of vendors are missing GSTIN data in LFA1 fields STCD1/STCD2/STCEG.

The phased approach ensures dependencies are respected—company code configuration must precede tax procedure activation, and master data remediation must complete before IRP API integration testing. Each phase includes specific deliverables mapped to the SAP Notes identified: 2948858 (Master Note), 2972612 (IRN Generation), 3006849 (QR Code), and 3082539 (Cancel/Amend workflows).

gantt
    title S4H India E-Invoicing Implementation (5 Weeks)
    dateFormat  YYYY-MM-DD
    section Foundation
    Create India Company Code IN01 (T001)           :a1, 2024-01-08, 3d
    Configure Plants in T001W                        :a2, 2024-01-08, 3d
    Setup Tax Procedure TAXIN Conditions            :a3, 2024-01-11, 4d
    Configure Number Ranges (SNRO)                  :a4, 2024-01-15, 3d
    section Master Data
    Update 10 Vendors GSTIN (LFA1)                  :b1, 2024-01-15, 5d
    Configure Customer GSTIN Data                   :b2, 2024-01-18, 4d
    Populate ADRC Region/State Codes                :b3, 2024-01-18, 4d
    BP Tax Numbers (DFKKBPTAXNUM)                   :b4, 2024-01-22, 3d
    section E-Invoice Setup
    Apply SAP Note 2948858                          :c1, 2024-01-25, 2d
    Activate J\_1IG\_CUST\_EINV Config                 :c2, 2024-01-25, 3d
    Configure IRP API Connection                    :c3, 2024-01-29, 4d
    Output Determination (NAST)                     :c4, 2024-02-01, 3d
    QR Code Setup (Note 3006849)                    :c5, 2024-02-01, 2d
    section Integration Testing
    End-to-End Test via VBRK/RBKP                   :d1, 2024-02-05, 4d
    JSON Schema Validation                          :d2, 2024-02-05, 3d
    Cancellation Workflow (Note 3082539)            :d3, 2024-02-08, 3d
    UAT and Go-Live Preparation                     :d4, 2024-02-11, 2d
    Go-Live                                         :milestone, d5, 2024-02-12, 1d

The following table details Week 1-2 Foundation phase activities, addressing the critical gaps in organizational structure including the missing India company code and empty T001W plant configuration.

Week

Activity

SAP Object/Table

Current State

Target State

Transaction

1

Create India Company Code

T001 (BUKRS)

No IN01 exists; 0 Indian company codes

IN01 with LAND1=IN, STCEG populated

OX02

1

Configure Plant/Location

T001W

EMPTY - 0 plants configured

Minimum 2 plants for Mumbai, Bengaluru

OX10

1-2

Activate Tax Procedure TAXIN

T007A, T007V

TAXIN exists but 0 condition types

IGST/CGST/SGST/UTGST conditions active

OBYZ

2

Number Range Configuration

NRIV (Object: J\_1IEXCINV)

Not configured for e-invoice

IRN number range with buffer

SNRO

2

Assign Company Code to Tax Procedure

T001 (KALSM)

IN01 not linked to TAXIN

IN01.KALSM = TAXIN

OBY6

Week 2-3 focuses on Master Data remediation, the most labor-intensive phase given that 10/10 vendors (100%) in LFA1 are missing GSTIN and all ADRC records lack REGION codes essential for GST state determination.

Week

Activity

SAP Table/Field

Records Affected

Target Action

Transaction

2

Vendor GSTIN Population

LFA1.STCD3 (GSTIN)

10 vendors (100001-100009, 100000)

Populate 15-digit GSTIN for all IN vendors

XK02/BP

2

Vendor PAN Population

LFA1.STCD2 (PAN)

10 vendors with BLANK STCD2

10-character PAN for Reliance, Infosys, etc.

XK02/BP

2-3

Customer GSTIN Setup

KNA1.STCD3

All customer masters

Mandatory GSTIN for B2B customers

XD02/BP

3

Address State Codes

ADRC.REGION

5+ addresses (89, 91, 93, 95, 97)

MH for Mumbai, KA for Bengaluru

BP

3

BP Tax Number Records

DFKKBPTAXNUM

EMPTY - 0 records

Create tax records for BP 111-115

BP

The following table outlines vendor-specific GSTIN remediation requirements for all 10 vendors identified in the assessment. Each vendor requires STCD3 population with their registered 15-digit GSTIN.

LIFNR

Vendor Name (NAME1)

City

Required State Code

GSTIN Format Required

100000

Tata Steel Limited

Mumbai

27 (Maharashtra)

27AAACT0028A1Z\*

100001

Reliance Industries Ltd

Mumbai

27 (Maharashtra)

27AAACR5055K1Z\*

100002

Infosys Limited

Bengaluru

29 (Karnataka)

29AAACI1195H1Z\*

100003

Larsen and Toubro Ltd

Mumbai

27 (Maharashtra)

27AAACL0140P1Z\*

100004

Wipro Limited

Bengaluru

29 (Karnataka)

29AABCW8354Q1Z\*

100005

Bosch Limited

Bengaluru

29 (Karnataka)

29AAACB5573R1Z\*

100006

Siemens Ltd India

Mumbai

27 (Maharashtra)

27AAACS7897M1Z\*

100007

Mahindra and Mahindra

Mumbai

27 (Maharashtra)

27AAACM5765F1Z\*

100008

Hindustan Unilever Ltd

Mumbai

27 (Maharashtra)

27AAACH0527R1Z\*

100009

ABB India Limited

Bengaluru

29 (Karnataka)

29AAACA7498A1Z\*

Week 3-4 E-Invoice Setup phase leverages the positive finding that 30+ EINVOICE\* tables and the India localization framework (50+ J\_1I\* tables) already exist in the system. This phase activates these dormant capabilities per SAP Note 2948858.

Week

Activity

SAP Note/Config

Technical Object

Deliverable

3

Apply E-Invoice Master Note

SAP Note 2948858

SNOTE transaction

Core e-invoice framework activated

3

Activate Custom E-Invoice Config

J\_1IG\_CUST\_EINV

SPRO → India Localization

Table status: ACTIVATED

3-4

E-Invoice Definition Setup

J\_1IG\_V\_EINV\_DE

IMG configuration view

Document types mapped to e-invoice

4

IRP API Configuration

SM59 (RFC Destination)

HTTP connection to einvoice1.gst.gov.in

Sandbox API connectivity verified

4

Output Determination

NAST, NACE

Condition type for e-invoice output

Automatic trigger on invoice save

4

QR Code Generation

SAP Note 3006849

UPEINVOICE01 Adobe Form

QR code embedded in print output

The IRP API integration requires specific endpoint configuration. The following table details the technical connectivity parameters for the GST e-Invoice portal integration.

Parameter

Sandbox Environment

Production Environment

Base URL

https://einv-apisandbox.nic.in

https://einvoice1.gst.gov.in

Authentication

OAuth 2.0 Client Credentials

OAuth 2.0 Client Credentials

RFC Destination Type

HTTP Connection to External Server

HTTP Connection to External Server

SM59 Object

EINV\_SANDBOX\_API

EINV\_PROD\_API

SSL Certificate

STRUST → SSL Client Standard

STRUST → SSL Client Standard

Proxy Configuration

Per corporate firewall rules

Per corporate firewall rules

Week 4-5 Integration & Testing phase validates the complete e-invoicing flow using the Clean Core APIs identified: API\_SUPPLIERINVOICE\_PROCESS\_SRV and API\_BILLING\_DOCUMENT\_SRV (both Level A certified). This phase addresses the current state where VBRK, RBKP, and NAST contain 0 records.

Week

Test Scenario

API/Transaction

Validation Point

Expected Outcome

4

Create Test Sales Invoice

VF01 / API\_BILLING\_DOCUMENT\_SRV

V

## 13\. Expected Outcomes & KPIs

This section establishes the measurable transformation targets for the S4H system's e-invoicing capability. Based on the current assessment revealing a **25/100 overall readiness score**, the remediation roadmap targets complete compliance with India's GST e-invoicing mandate. These outcomes align with SAP Note 2948858 (India E-Invoicing Master Note) requirements and leverage the existing EINVOICE\* framework tables already present in the S/4HANA 758 environment.

The Before/After Scorecard below quantifies the gap between current state—characterized by 0% GSTIN coverage across all 10 vendors in LFA1, missing India company code in T001, and inactive J\_1IG\_CUST\_EINV configuration—and the target state of full operational readiness. Post-remediation, the system will support automated IRN generation through the IRP (Invoice Registration Portal) integration, with all master data elements properly configured for GST compliance.

### Before/After Scorecard

The following scorecard presents the six critical dimensions assessed during this gap analysis, showing current baseline metrics extracted from system S4H and the post-remediation targets required for compliant e-invoice generation.

Dimension

SAP Object/Table

Before (Current)

After (Target)

Remediation Scope

**GSTIN Coverage**

LFA1-STCD3, DFKKBPTAXNUM

0%

100%

Populate GSTIN for all 10 vendors (100001–100009, 100000) and 5 BPs (111–115)

**Company Code Readiness**

T001-BUKRS, T001-STCEG

15%

100%

Create IN01 company code with valid GSTIN; configure existing 3 VAT-enabled codes

**State Code Coverage**

ADRC-REGION

0%

100%

Update REGION for addresses 89, 91, 93, 95, 97 (MH for Mumbai, KA for Bengaluru)

**E-Invoice Configuration**

J\_1IG\_CUST\_EINV, J\_1IG\_V\_EINV\_DE

Not Active

Active

Activate per SAP Note 2972612; configure IRP connectivity

**IRN Generation**

J\_1IG\_INVREFNUM

Not Possible

Automated

Enable real-time IRN fetch via API integration with NIC portal

**Compliance Rate**

VBRK, RBKP with IRN linkage

0%

100%

All B2B invoices auto-registered; QR code generation per SAP Note 3006849

### Target Key Performance Indicators

The following operational KPIs define success criteria for the remediated e-invoicing solution. These metrics will be monitored through standard SAP transaction SM21 for system logs, API monitoring via API\_SUPPLIERINVOICE\_PROCESS\_SRV (Level A) and API\_BILLING\_DOCUMENT\_SRV (Level A), and custom CDS views leveraging the S/4HANA 758 analytical framework.

KPI

Target

Measurement Method

Business Impact

E-Invoice Generation Rate

100%

J\_1IG\_INVREFNUM record count vs. VBRK/RBKP B2B documents

Full regulatory compliance; zero manual interventions

IRN Fetch Time

<2 seconds

API response time monitoring via SAP HANA HDB performance views

Real-time invoice processing; no user workflow delays

Rejection Rate

0%

IRP error logs; J\_1IG\_INVREFNUM status field analysis

Elimination of compliance penalties and re-work costs

### Overall Readiness Transformation

The table below summarizes the projected readiness score improvement across all assessment categories, transforming the system from its current non-compliant state to full production readiness for India e-invoicing operations.

Assessment Category

Current Score

Target Score

Point Improvement

Vendor GSTIN Coverage (LFA1)

0%

100%

+25 pts

Company Code Readiness (T001)

15%

100%

+20 pts

Master Data Completeness (ADRC, BUT000)

10%

100%

+20 pts

E-Invoice Config Status (J\_1IG\_\*)

0%

100%

+20 pts

Tax Procedure Configuration (TAXIN)

0%

100%

+15 pts

**OVERALL READINESS SCORE**

**25/100**

**100/100**

**+75 pts**

Achievement of these outcomes will position system S4H (S/4HANA 758 on SAP HANA with Kernel 793) for seamless e-invoicing operations, leveraging the pre-installed 30+ EINVOICE\* tables and 50+ J\_1I\* India localization components. The Adobe Form Template UPEINVOICE01 will generate compliant invoice formats, while Clean Core APIs ensure upgrade-safe integration with the government's Invoice Registration Portal.

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._