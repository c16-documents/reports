/\* ──────────────────────────────────────────────────────────────── \* C16 v2 EXECUTIVE style — extracted from C16\_Document\_Style\_Prompt\_v2.html \* ──────────────────────────────────────────────────────────────── \* Visual fingerprint: \* - Top header: purple gradient + logo + navy 5pt accent stripe \* - Body: Georgia serif, 11pt, color #1a1a1a, line-height 1.6 \* - Headings: 'Helvetica Neue', Arial, sans-serif, bold, navy #003366 \* - Tables: 1px #DDD border, header bg #F1F1F1, bold cells \* - Code: Courier New 9.5pt, navy #003366 1pt border \* - Footer: purple gradient + white text, 0.8em, rounded corners \* Print-oriented sizing (pt units throughout, not px). \* ──────────────────────────────────────────────────────────────── \*/ :root { --c16-purple-1: #6B3FA0; --c16-purple-2: #8B5CF6; --c16-ink: #1f2937; --c16-sub: #4b5563; --c16-rule: #e5e7eb; --c16-accent: #faf5ff; --c16-v2-navy: #003366; --c16-v2-navy-dark: #002244; --c16-v2-body: #1a1a1a; --c16-v2-rule: #DDD; --c16-v2-table-head: #F1F1F1; --c16-v2-subtle: #F9F9F9; --c16-v2-meta: #666666; --c16-v2-caption: #999999; } body { font-family: Georgia, serif; font-size: 11pt; color: var(--c16-v2-body); line-height: 1.6; background: #fff; margin: 0; padding: 0; } /\* Wrapper: the navy 5pt accent stripe at the top of the document body \*/ .c16-v2-document { border-top: 5px solid var(--c16-v2-navy); padding: 30pt 40pt; background: #fff; } h1, h2, h3, h4, h5, h6 { font-family: 'Helvetica Neue', Arial, sans-serif; font-weight: bold; color: var(--c16-v2-navy); margin: 16pt 0 8pt 0; } h1 { font-size: 18pt; } h2 { font-size: 16pt; } h3 { font-size: 13pt; } h4, h5, h6 { font-size: 11pt; } /\* Subtitles and meta lines (e.g. "Prepared for: …") \*/ .subtitle, .meta { font-family: 'Helvetica Neue', Arial, sans-serif; font-size: 11pt; color: var(--c16-v2-meta); margin: 0 0 20pt 0; } /\* Tiny captions / footnotes \*/ .caption, small, figcaption { font-family: Georgia, serif; font-size: 9pt; color: var(--c16-v2-caption); } a { color: var(--c16-v2-navy); text-decoration: underline; } /\* Horizontal rules — thin gray separators \*/ hr { border: none; border-top: 1px solid var(--c16-v2-rule); margin: 20pt 0; } /\* Lists \*/ ul, ol { margin: 8pt 0; padding-left: 24pt; } li { margin: 4pt 0; } /\* Paragraphs \*/ p { margin: 0 0 8pt 0; } /\* Tables \*/ table { border-collapse: collapse; width: 100%; font-family: 'Helvetica Neue', Arial, sans-serif; font-size: 10.5pt; margin: 12pt 0; } th, td { border: 1px solid var(--c16-v2-rule); padding: 8pt; text-align: left; vertical-align: top; } th { background-color: var(--c16-v2-table-head); color: var(--c16-v2-body); font-weight: bold; } tr:nth-child(even) td { background-color: var(--c16-v2-subtle); } /\* Code blocks and inline code \*/ code { font-family: 'Courier New', Consolas, monospace; font-size: 10pt; border: 1px solid var(--c16-v2-navy); padding: 1pt 4pt; background: #fff; color: var(--c16-v2-body); } pre { font-family: 'Courier New', Consolas, monospace; font-size: 9.5pt; color: var(--c16-v2-body); line-height: 1.5; white-space: pre-wrap; word-wrap: break-word; margin: 8pt 0; padding: 10pt; border: 1px solid var(--c16-v2-navy); background: #fff; } pre code { border: none; padding: 0; } /\* Blockquotes — sub-tle navy left border \*/ blockquote { border-left: 4px solid var(--c16-v2-navy); background: var(--c16-v2-subtle); margin: 12pt 0; padding: 8pt 14pt; font-style: italic; } /\* "Executive summary" or similar callout blocks \*/ .callout, .executive-summary { border-top: 5px solid var(--c16-v2-navy); background: var(--c16-v2-subtle); padding: 12pt 16pt; margin: 16pt 0; } /\* KPI grids — bordered cells with navy accent \*/ .kpi-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(160pt, 1fr)); gap: 8pt; margin: 12pt 0; } .kpi-card { border: 1px solid var(--c16-v2-rule); border-top: 3px solid var(--c16-v2-navy); padding: 10pt 12pt; background: #fff; } .kpi-card .label { font-family: 'Helvetica Neue', Arial, sans-serif; font-size: 9pt; color: var(--c16-v2-meta); text-transform: uppercase; letter-spacing: 0.05em; } .kpi-card .value { font-family: 'Helvetica Neue', Arial, sans-serif; font-size: 18pt; color: var(--c16-v2-navy); font-weight: bold; margin-top: 2pt; } /\* Page wrapper if HTML lacks explicit container \*/ @media print { body { background: #fff; } .c16-v2-document { border-top: 5px solid var(--c16-v2-navy); padding: 30pt 40pt; } }

![C16](data:image/jpeg;base64,)

## 1\. Executive Summary

This Vendor Master Data Quality Report presents a comprehensive assessment of the vendor ecosystem across the BestRun Group SAP ECC 7.40 system (client 800). The analysis examines 500+ vendor records across general master data (LFA1), company code assignments (LFB1), bank details (LFBK), open payables (BSIK), cleared payables (BSAK), payment terms configuration (T052), and company code structures (T001). The investigation reveals a vendor landscape spanning 50+ company codes across 20+ countries with significant data quality gaps that directly impair automatic payment processing readiness.

Total Vendors (LFA1 sample)

500+

Open Payable Items (BSIK)

500+

Cleared Items (BSAK)

500+

Payment Terms Configured

69

Company Codes Active

50+

Blocked / Deleted Vendors

28+

Key critical findings: (1) Vendor 1012 in company code 0005 carries a single open item of EUR 2,301,440 — the largest individual exposure in the system. (2) Vendor 1000 (C.E.B. BERLIN) in company code 1000 has open items totaling over USD 30,000,000 across multiple invoices from 2014 — these represent massive aged exposure with no automatic clearing. (3) The REGUH table (automatic payment run history) is unreadable via RFC — suggesting either no F110 payment runs have been executed, or the table structure has field compatibility issues — both are critical indicators of manual payment processing. (4) Approximately 40% of company code assignments have no payment method (ZWELS = blank), blocking them from automatic payment. (5) Multiple vendor-name vendors (BLEVINS, CIRCUITS, DOWNEY INC, FOCUS, INTEEK, TURFMOOR) exist alongside numeric vendors — indicating inconsistent vendor creation standards.

## 2\. Current State Assessment

### Open Payables Overview (BSIK)

The open payables ledger reveals a deeply concerning picture — 500+ open items spanning from the year 2001 to 2025, many with massive individual values. The concentration risk is extreme: a handful of vendors account for tens of millions in open exposure. This aging profile indicates systemic failures in the payment clearing cycle.

Vendor

Company Code

Doc Number

Doc Date

Amount

Currency

Baseline Date

Pay Block

0000001012

0005

5100000002

2006-05-08

2,301,440.00

EUR

2006-05-08

0000001000

1000

5100000003

2014-07-08

10,000,000.00

USD

2014-07-08

0000001000

1000

5100000000

2014-07-08

5,000,000.00

USD

2014-07-08

0000001000

1000

5100000004

2014-07-08

10,000,000.00

USD

2014-07-08

R

0000001000

1000

5100000005

2014-09-23

3,600,000.00

USD

2014-09-23

0000001000

1000

5100000051

2023-05-15

960,000.00

USD

2023-05-15

0000001000

1000

1900000000

2005-01-10

580,000.00

EUR

2005-01-10

0000003920

0006

1900000002

2011-12-15

318,628.00

USD

2011-12-15

0000003920

0006

1900000001

2011-11-15

305,011.00

USD

2011-11-15

0000001002

0005

0200000000

2006-04-20

275,000.00

EUR

2006-04-20

The table above shows the 10 largest individual open items. Vendor 1000 alone accounts for over USD 30 million in open exposure from 2014 invoices — these have been sitting unpaid for over 11 years. The oldest open item in the sample dates back to January 2001 (vendor 1000, doc 5100000673, EUR 696.00 with payment block "R").

### Payment History (BSAK)

The cleared items table shows payment activity exists but is heavily concentrated on specific vendors and company codes. Company code 0001 shows extensive clearing activity for vendor TURFMOOR (a text-ID vendor), with 100+ items cleared in March–April 2013, all at EUR 1,234.00. This suggests automated test data or mass-clearing exercises rather than genuine business payment cycles.

Vendor

Company Code

Last Clear Date

Sample Amount

Currency

Pattern

TURFMOOR

0001

2015-03-02

1,234.00

EUR

Mass clearings — 100+ items at identical amounts

0000001000

0005

2009-11-30

1,524,743.00

EUR

Large batch clearing with payment doc 2000000000

0000001002

0005

2006-05-08

924,820.00

EUR

Cleared via payment run in 2006

0000001070

0005

2006-05-08

205,085.00

EUR

Cleared via payment run in 2006

0000000002

1000

2025-10-27

97.92

EUR

Recent clearing activity — small amounts only

0000001005

0005

2009-11-30

437,737.00

EUR

Cleared in Nov 2009 batch

0000001010

0005

2009-11-30

130.00

EUR

Small clearing in 2009

INTEEK

0006

2012-08-24

50.00

USD

Single small clearing

A stark contrast emerges: company code 0005 had significant payment runs in 2006 and 2009 (clearing EUR 1.5M+ per batch), but this activity stopped after 2009. Company code 1000 shows recent small-value clearings (EUR 96–1,344) for vendor 0000000002 through 2024–2025, but the massive USD 30M+ exposure for vendor 1000 remains completely untouched.

### Automatic Payment Runs (REGUH)

The query against REGUH (automatic payment run header) returned a field validation error, meaning the table either has an incompatible structure or is empty. This is a critical finding — in a healthy AP environment, REGUH should contain records from regular F110 payment program executions.

Check

Result

Implication

REGUH table accessible via RFC

❌ FIELD\_NOT\_VALID error

Payment run table may use non-standard fields or be empty

F110 payment run evidence

❌ No evidence found

Payments likely processed manually (F-53/F-58) or not at all

Clearing docs in BSAK

✅ Found — but sporadic

Some manual clearings exist but no systematic payment cycle

The absence of accessible F110 payment run data is the single most important process gap. Without automatic payment runs, every invoice must be paid manually — creating risk of late payments, missed discounts, and unbounded AP aging.

## 3\. Aging Analysis

The open payables aging analysis reveals extreme exposure concentration. Using the baseline date (ZFBDT) from BSIK and today's date (2026-05-11), virtually all open items are severely overdue. The system carries payables dating back to 2001 — a 25-year aging tail that no healthy AP function should exhibit.

Aging Bucket

Item Count (est.)

EUR Equivalent (est.)

Key Vendors

0–30 days

0

0

No recent invoices in sample

31–90 days

2

~192 EUR

Vendor 2 (small amounts from Oct 2025)

91–365 days

~15

~15,000 EUR

Vendor 2 (recent postings in 2025)

1–3 years

~60

~650,000 EUR + multi-million USD

Vendor 2 (CoCd 1000), Vendor 1000 (2023 invoices)

3–10 years

~80

~3,000,000+ EUR/USD

Vendor 3920 (CoCd 0006), BLEVINS, CIRCUITS, DOWNEY INC

10+ years

~200+

~35,000,000+ EUR/USD

Vendor 1000 (CoCd 1000), Vendor 1005 (CoCd 1000)

The 10+ year bucket dominates the exposure. Vendor 1000 in company code 1000 alone carries USD 30M+ from 2014 invoices. These are almost certainly not legitimate payable obligations — they are likely test data, migration artifacts, or posting errors that should be written off or reversed. However, their presence contaminates every AP report, aging analysis, and cash flow forecast in the system.

## 4\. Root Cause Analysis

The vendor master data quality issues stem from three interconnected root causes that have compounded over the system's lifetime. First, the system was originally set up as an IDES training/demo system (evidenced by company names like "BestRun Germany", "BestRun USA"), which means much of the data is synthetic. Second, real training exercises and proof-of-concept activities have layered additional data without cleanup. Third, the absence of F110 automatic payment runs means invoices posted through training never got cleared — they just accumulated.

Root Cause

Evidence

Impact

IDES/Demo system origin

Company names (BestRun), vendor names (TURFMOOR, BLEVINS, CIRCUITS), 50+ company codes

Synthetic data mixed with real test data — impossible to distinguish without business knowledge

No F110 payment automation

REGUH inaccessible/empty, BSAK shows only sporadic manual clearings

Open items accumulate indefinitely — 25-year aging tail

Inconsistent vendor creation standards

Text-ID vendors (TURFMOOR, BLEVINS) alongside numeric-ID vendors; mixed account groups (0001, 0005, 0099, LIEF, Z002)

No standardized naming, duplicate vendor risk, no clear categorization

Missing master data fields at scale

40%+ of LFB1 records missing ZWELS; many missing ZTERM

Vendors ineligible for automatic payment even if F110 were configured

## 5\. Master Data Quality

### Vendor Master Completeness Scorecard

The completeness scorecard below evaluates vendor master data across 7 critical dimensions. Each dimension is scored based on the percentage of vendor records that have the field populated with a non-blank value. The thresholds are: Green (≥90%), Yellow (70–89%), Red (<70%).

Dimension

Table.Field

Complete %

Missing Count

Status

Vendor Name

LFA1.NAME1

99%

~2

🟢 Good

Country

LFA1.LAND1

100%

0

🟢 Good

City (Address)

LFA1.ORT01

~88%

~60

🟡 Needs Review

Street (Address)

LFA1.STRAS

~82%

~90

🟡 Needs Review

Payment Method

LFB1.ZWELS

~60%

~200

🔴 Critical

Payment Terms

LFB1.ZTERM

~65%

~175

🔴 Critical

Bank Details

LFBK (has record)

~45%

~275

🔴 Critical

The three red-flagged dimensions — payment method, payment terms, and bank details — together form the "F110 readiness triad". A vendor must have all three populated to be eligible for automatic payment. With only ~45% bank coverage and ~60% payment method coverage, fewer than 40% of vendors would qualify for F110 if it were configured.

### Sample: Vendors WITHOUT Payment Method (ZWELS = blank)

The following vendors have company code assignments with no payment method configured. Without a payment method, F110 cannot determine HOW to pay these vendors (check, wire transfer, ACH, etc.).

LIFNR

NAME1

BUKRS

ZWELS

ZTERM

Recon Account

0000000002

Electronic Components Distributor

3000

(blank)

(blank)

160000

0000000050

Central Logistics Inc.

3000

(blank)

(blank)

160000

0000000100

C.E.B. BERLIN

0005

(blank)

(blank)

160000

0000001000

C.E.B. BERLIN

2300

(blank)

(blank)

161000

0000001000

C.E.B. BERLIN

3000

(blank)

(blank)

160000

0000001002

Müller KG

2300

(blank)

(blank)

161000

0000001003

Gusswerk US

2000

(blank)

(blank)

160000

0000001003

Gusswerk US

2300

(blank)

(blank)

161000

0000001007

Bike Retail & Co.

2000

(blank)

(blank)

160000

0000001011

SKC Americas

2300

(blank)

(blank)

161000

0000001011

SKC Americas

3000

(blank)

(blank)

160000

0000001059

Express Ship Carrier

3000

(blank)

(blank)

160000

Notice that vendor 1000 (C.E.B. BERLIN) has payment methods configured for some company codes (1000: "U", 0005: "SU", 2820: "C") but missing for others (2300, 3000). This inconsistency means the same vendor would be paid automatically in one company code but blocked in another — a common source of late-payment complaints in multi-company environments.

### Sample: Vendors WITHOUT Payment Terms (ZTERM = blank)

Vendors without payment terms will default to immediate payment (or the system default), losing early-payment discounts and creating unpredictable cash outflow. The following sample shows actual vendor records with blank ZTERM.

LIFNR

NAME1

BUKRS

ZTERM

ZWELS

Status

0000000002

Electronic Components Distributor

3000

(blank)

(blank)

Double gap: no method + no terms

0000000008

Jose Fernandez

3000

(blank)

(blank)

Double gap

0000000050

Central Logistics Inc.

3000

(blank)

(blank)

Double gap

0000001000

C.E.B. BERLIN

2300

(blank)

(blank)

Double gap

0000001000

C.E.B. BERLIN

2800

(blank)

U

Has method, missing terms

0000001000

C.E.B. BERLIN

3000

(blank)

(blank)

Double gap

0000001012

Reihl & Müller

0005

(blank)

S

Has method, missing terms

0000001012

Reihl & Müller

1000

(blank)

S

Has method, missing terms

0000001021

Noe´Tech Company AG

1000

(blank)

SU

Has method, missing terms

0000001022

Max´Grosshandel

0005

(blank)

SU

Has method, missing terms

0000001022

Max´Grosshandel

1000

(blank)

SU

Has method, missing terms

0000001061

Neotech GmbH

1000

(blank)

(blank)

Double gap

The "Double gap" pattern (both ZWELS and ZTERM blank) is particularly concerning — these vendors are completely ineligible for any form of automated processing. The "Has method, missing terms" pattern is more insidious: F110 would attempt to pay but wouldn't know when, potentially triggering immediate payment on every invoice.

### Payment Method Distribution (ZWELS breakdown)

The payment method field (ZWELS) in LFB1 tells F110 which payment instruments to use. The distribution below shows the most common payment method codes across all company code assignments.

ZWELS Code

Method Description

Vendor-CoCd Count (approx)

% of Total

SU

Check (S) + Bank Transfer (U)

~90

~18%

C

Check

~75

~15%

U

Bank Transfer

~25

~5%

CT

Check (C) + Wire Transfer (T)

~15

~3%

L

Letter of Credit

~12

~2%

ESU

EFT (E) + Check (S) + Transfer (U)

~10

~2%

(blank)

NOT CONFIGURED

~200

~40%

Other (P, T, V, W, 5, etc.)

Various / non-standard

~73

~15%

The 40% blank rate is the headline number. For the configured vendors, "SU" (check + bank transfer) is the most common pattern, followed by "C" (check only). The "ESU" pattern (EFT + check + bank transfer) gives F110 the most flexibility. Some unusual codes like "5", "5SU", and "CSTW" appear — these may be valid country-specific methods or configuration errors that should be validated against T042Z.

### Payment Terms Distribution (ZTERM breakdown)

Payment terms control when invoices become due and what early-payment discounts are available. The distribution below shows the most frequently used terms across all vendor company code assignments.

ZTERM Code

Description

Vendor-CoCd Count

% of Total

ZB01

3% disc 14 days, net 45 days

~150

~30%

ZB50

2% disc 10 days, net 30 days

~30

~6%

NT30

Net 30 days, no discount

~25

~5%

ZB00

Due immediately, no discount

~15

~3%

0001

Due immediately

~12

~2%

0002

3% disc 14d, net 45d

~8

~2%

ZB02

2% disc 14d, net 45d (end-of-month)

~6

~1%

ZB03

2% disc 20d, net 30d

~5

~1%

(blank)

NOT CONFIGURED

~175

~35%

Other

Various (ZB04-ZB15, JP\*, etc.)

~74

~15%

ZB01 is the dominant payment term at 30% — offering 3% discount within 14 days, which represents a significant cash optimization opportunity (3% for paying 31 days early = ~35% annualized return). However, without F110 automation, these discounts are almost certainly being forfeited. The 35% blank ZTERM rate means over a third of vendor assignments have no defined due date logic.

### ZAHLS Value Reference

The ZAHLS field in LFB1 controls the payment block at the company code level. A blank value means "not blocked" — the vendor is eligible for payment. Non-blank values prevent F110 from processing the vendor entirely.

ZAHLS Code

Meaning

Vendor-CoCd Count

(blank)

Not blocked — eligible for payment

~495

A

Blocked for all payment methods

0 (in sample)

R

Blocked for specific reason

0 (in sample)

Good news: no vendor company code assignments in the LFB1 sample carry a ZAHLS payment block. This means the payment eligibility issue is purely a master data completeness problem (missing ZWELS/ZTERM), not a deliberate blocking decision.

### Bank Detail Coverage (LFBK)

Bank details are required for electronic payment methods (bank transfer, wire, ACH). Without bank details, F110 can only issue checks — and only if the check payment method is configured. The LFBK table returned 500 records, but coverage varies significantly by vendor population.

Metric

Value

Total LFBK records (sample)

500

Unique vendors with bank details

~220

Vendors with multiple bank accounts

~40 (e.g., vendor 10000 has 7 bank records across DE/US)

Estimated bank coverage rate

~45% of vendors have at least one bank record

The 45% coverage rate means that over half the vendor population cannot receive electronic payments. For the 40% of vendors with ZWELS containing "U" (bank transfer) or "E" (EFT), missing bank details would cause F110 to skip them or fall back to check printing — negating the intended payment method.

### Bank Details by Country (LFBK × LFA1)

The bank detail distribution by country shows where the coverage gaps are most severe.

Country (BANKS)

Bank Records

Unique Vendors

Notes

DE (Germany)

~160

~90

Best coverage — most German vendors have bank details

US (United States)

~150

~80

Good coverage — US routing numbers present

IN (India)

~30

~25

Many with identical bank key "B 111" — likely template data

CA (Canada)

~25

~15

Adequate coverage for Canadian vendors

JP (Japan)

~15

~10

Includes BKONT (account type) field populated

GB (Great Britain)

~10

~8

Sort codes present (6-digit BANKL)

FR (France)

~5

~3

French bank keys (10-digit BANKL) present

Other (AU, MX, NZ, PT, RU, etc.)

~25

~20

Scattered coverage across smaller country populations

The Indian vendor bank data is suspicious — approximately 20 vendors (97044–97068 series) all share the identical bank key "B 111" and account number "8888777788787". This is clearly template/copy data and would fail validation in any real payment run.

### Blocked / Deleted Vendors Sample

Vendors flagged for central blocking (SPERR=X) or deletion (LOEVM=X) in LFA1 should be reviewed — they may still have open POs or payables that need resolution before the vendor can be properly retired.

LIFNR

NAME1

Country

SPERR

LOEVM

Account Group

0000003022

CAFS Chemicals & Pharmaceuticals

US

X

LIEF

0000001995

Schneider GmbH

DE

X

0001

0000003140

Morris C.A. Pipe-Services

US

X

LIEF

0000003141

Marx Broths Piping Company

US

X

LIEF

Additionally, at the company code level (LFB1), vendors 1550 and 1560 are both blocked (SPERR=X) AND flagged for deletion (LOEVM=X) in company code 3000, while remaining active in company code 1000. This inconsistency should be resolved — either block/delete across all company codes or remove the flag.

## 6\. Invoice / Document Processing Bottlenecks

The primary bottleneck is not in invoice processing but in payment execution. The BSIK data shows invoices are being posted (500+ open items exist), but they are not being cleared through payment runs. The document types in BSIK include 5100\* (logistics invoices from MIRO), 9700\* (FI invoices), 9500\* (FI invoices), 1900\* (various postings), and 0100\* (general documents) — indicating a healthy mix of posting sources. The bottleneck occurs after posting: no systematic F110 execution to clear these items.

Bottleneck

Evidence

Impact

No F110 automation

REGUH inaccessible; BSAK shows only sporadic manual clearings

500+ open items aging indefinitely

Payment-blocked items

BSIK shows ZLSPR = "R" on ~8 items, ZLSPR = "A" on 1 item

These items explicitly blocked — need review before any clearing

Multi-currency complexity

Open items in EUR, USD, INR, DEM across same vendor

F110 needs currency-specific house banks configured per company code

Stale test data blocking reports

USD 30M+ from 2014, EUR 2.3M from 2006 — clearly not real obligations

Contaminates AP aging, cash forecasting, vendor balance reports

## 7\. Procurement / Cycle Gaps

### Top Open Payables by Vendor (BSIK SUM WRBTR)

The top exposure table below reveals extreme concentration risk. The top 3 vendors account for virtually all the open payable value in the system. Note that amounts are shown in their transaction currency — a full exposure view would require conversion to a single reporting currency.

LIFNR

NAME1

Primary CoCd

\# Open Docs

Total Exposure

Currency Mix

Oldest Item

0000001000

C.E.B. BERLIN

1000

~40

~30,000,000+

EUR, USD

2001-01-23

0000001012

Reihl & Müller

0005

1

2,301,440.00

EUR

2006-05-08

0000003920

(Vendor 3920)

0006

~14

~1,800,000+

USD

2006-09-11

0000001005

PAQ Deutschland GmbH

1000

~12

~730,000+

EUR

2004-01-07

0000000002

Electronic Components Dist.

1000

~50

~520,000+

EUR, INR

2023-05-10

DOWNEY INC

Downey Inc

0006

~8

~509,784.00

USD

2012-03-27

BLEVINS

Blevins

0006

~8

~399,864.00

USD

2012-05-27

CIRCUITS

Circuits

0006

~10

~383,855.00

USD

2012-02-27

0000001002

Müller KG

0005

1

275,000.00

EUR

2006-04-20

0000001010

Sunny Electronics GmbH

1000

~3

~162,000+

EUR

2004-01-08

The presence of text-ID vendors (BLEVINS, CIRCUITS, DOWNEY INC) with combined USD 1.3M+ exposure is a vendor master data governance failure. These should have been created with numeric IDs following the standard LIEF or 0001 account group pattern.

### Top Company Codes by Vendor Activity (BSIK × LFB1)

The distribution of open payables across company codes reveals which entities carry the most AP risk.

BUKRS

Company Name

Currency

\# Open Items (est.)

Key Vendors

1000

BestRun Germany

EUR

~250

Vendor 1000, 0002, 1001, 1003, 1005, 1006, 1010, 1011

0006

BestRun US INC New GL

USD

~80

Vendor 3920, BLEVINS, CIRCUITS, DOWNEY INC, INTEEK, 3910, 500

0005

BestRun Germany new GL

EUR

~15

Vendor 1012, 1000, 1002, 1004, 1005

3000

BestRun USA

USD

~5

Scattered small amounts

Company code 1000 (BestRun Germany) dominates with ~250 open items, followed by 0006 (BestRun US) with ~80 items. The US entity (0006) has the text-ID vendor concentration risk, while Germany (1000) has the massive value concentration in vendor 1000.

## 8\. Configuration Assessment

### Payment Method Configuration (T042Z)

The REGUH query failure prevented direct verification of payment method configuration. However, the payment method codes found in LFB1 (S, U, C, T, L, E, P, V, W) are standard SAP payment method identifiers. The T042Z table would need to be queried with specific country codes to verify each is properly configured with bank determination, form names, and check number ranges.

Payment Method Code

Expected Configuration

Status

S — Check

Check form, number range, bank assignment

⚠️ Cannot verify — needs T042Z check with country DE/US

U — Bank Transfer

DME format, house bank, payment medium program

⚠️ Cannot verify

C — Check (US)

US check form, ABA routing

⚠️ Cannot verify

T — Wire Transfer

SWIFT/BIC configuration, correspondent bank

⚠️ Cannot verify

### Payment Terms Configuration (T052)

The T052 table returned 69 payment term records — a healthy configuration set. The most important terms are well-defined with discount percentages and due date calculations.

ZTERM

Discount %

Disc. Days 1

Disc. Days 2

Net Days

Notes

0001

0%

0

0

0

Due immediately

0002

3%

14

30

45

Standard 3/14 net 45

ZB01

3%

14

30

45

Custom — same as 0002 (most-used term)

ZB50

2%

10

30

—

2/10 net 30

NT30

0%

30

—

—

Net 30, no discount

NT60

0%

60

—

—

Net 60, no discount

ZB00

0%

0

0

0

Due immediately, no discount

ZB03

2%

20

30

—

2/20 net 30

The payment terms configuration is adequate. ZB01 and 0002 are nearly identical (3% discount if paid within 14 days) — this duplication could be consolidated. The 69 total terms include country-specific variants (JP01–JP10 for Japan, T301 for 10th of month) which are properly configured.

### F110 Payment Readiness Checklist

This checklist summarizes the prerequisites for enabling F110 automatic payment processing.

Prerequisite

Status

Action Needed

Payment terms in T052

✅ 69 terms configured

None — adequate

Vendor ZWELS populated

🔴 ~40% missing

Mass-update ~200 LFB1 records

Vendor ZTERM populated

🔴 ~35% missing

Mass-update ~175 LFB1 records

Bank details in LFBK

🔴 ~55% missing

Collect bank details for electronic payment vendors

House banks configured (T012)

⚠️ Not verified

Verify per company code

Payment method forms (T042E)

⚠️ Not verified

Verify print forms configured

Bank determination (T042Z)

⚠️ Not verified

Verify per country/payment method

Stale data cleanup

🔴 Needed

Write off/reverse USD 30M+ of 2014 test data

## 9\. Detailed Findings (Prioritized)

The findings below are ranked by business impact severity — from the issues that would cause the largest financial or operational disruption to those that are quality-of-life improvements.

#

Finding

Severity

Evidence

Impact

F1

Massive stale payables contaminating AP reports

🔴 Critical

Vendor 1000 CoCd 1000: USD 30M+ from 2014; Vendor 1012 CoCd 0005: EUR 2.3M from 2006

AP aging, cash forecasts, vendor balances all unreliable

F2

No F110 automatic payment processing

🔴 Critical

REGUH inaccessible; no systematic clearings in BSAK pattern

All payments manual — lost discounts, late payments, staff overhead

F3

40% of vendor-company code assignments missing payment method

🔴 Critical

~200 LFB1 records with blank ZWELS

Even if F110 enabled, 40% of vendors cannot be paid automatically

F4

35% of vendor-company code assignments missing payment terms

🟠 High

~175 LFB1 records with blank ZTERM

Default to immediate payment — no discount optimization possible

F5

55% of vendors lack bank details

🟠 High

~275 vendors without LFBK records

Electronic payment impossible — check-only or manual

F6

Text-ID vendors (BLEVINS, CIRCUITS, etc.) with USD 1.3M+ exposure

🟠 High

6 text-ID vendors in BSIK CoCd 0006 with open items

Non-standard naming — cannot be matched to standard vendor reports

F7

Indian vendor bank details are template copies

🟡 Medium

20+ vendors (97xxx) with identical BANKL="B 111" BANKN="8888777788787"

Payment would fail on first F110 run — bank validation error

F8

Inconsistent blocking across company codes

🟡 Medium

Vendors 1550, 1560: blocked in CoCd 3000 but active in 1000

Vendor could receive payments in one entity but not another

F9

Payment block flags on open items

🟡 Medium

~8 BSIK items with ZLSPR="R", 1 with "A"

These items intentionally held — need review before clearing

F10

Missing city/street for 12–18% of vendors

🟡 Medium

~60 missing ORT01, ~90 missing STRAS in LFA1

Correspondence, tax reporting, 1099 forms impacted

## 10\. Optimization Strategy

### Remediation Priority Matrix

The remediation matrix below maps each action to its expected business impact and implementation effort, producing a prioritized action list. Items are sorted by priority score (Impact × 1/Effort).

Action

Impact

Effort

Priority

Owner

Clean up stale payables (write-off/reverse USD 30M+ test data)

Very High

Medium

P1 — Immediate

FI Lead + Controller

Mass-update ZWELS for 200 vendor-CoCd records

High

Low

P1 — Immediate

AP / Vendor Master Team

Mass-update ZTERM for 175 vendor-CoCd records

High

Low

P1 — Immediate

AP / Vendor Master Team

Configure and test F110 for CoCd 1000

Very High

Medium

P2 — This Month

FI Consultant

Collect real bank details for electronic-pay vendors

High

High

P2 — This Month

AP + Procurement

Standardize text-ID vendors to numeric IDs

Medium

Medium

P3 — This Quarter

Master Data Governance

Fix Indian vendor template bank data

Medium

Low

P3 — This Quarter

AP Team

Address missing city/street in LFA1

Low

Low

P4 — Ongoing

Master Data Team

## 11\. Quick Wins — Immediate Actions (This Week)

These actions require no configuration changes, no development, and no approval cycles — they can be executed by an AP team member with XK02 (change vendor) and FB08 (reverse document) authorization within this week.

Action

Transaction

Effort

Owner

ETA

Reverse or write-off vendor 1000 test invoices from 2014 (USD 30M+)

FB08 / F-44

2 hours

FI Lead

Day 1

Reverse vendor 1012 open item from 2006 (EUR 2.3M)

FB08

30 min

FI Lead

Day 1

Update ZWELS for top 50 vendors by open payable value

XK02 or LSMW

4 hours

AP Lead

Day 2–3

Update ZTERM for top 50 vendors by open payable value

XK02 or LSMW

4 hours

AP Lead

Day 2–3

Review and release 9 payment-blocked items (ZLSPR = R/A)

FBL1N → F-44

1 hour

AP Analyst

Day 3

Resolve inconsistent vendor 1550/1560 blocking (CoCd 3000 vs 1000)

XK02

30 min

Master Data

Day 4

## 12\. Implementation Roadmap

The roadmap below outlines a 12-week program to achieve full F110 payment automation readiness, starting with quick wins in week 1 and building toward a pilot payment run by week 8.

Phase

Weeks

Activities

Deliverable

Phase 1: Data Cleanup

1–2

Reverse stale payables; mass-update ZWELS/ZTERM for top vendors; fix Indian bank data

Clean AP aging report; 80%+ ZWELS/ZTERM coverage

Phase 2: Configuration

3–5

Configure F110 for CoCd 1000 and 0006; set up house banks; configure payment medium programs; test payment forms

F110 config documentation; test payment run in sandbox

Phase 3: Bank Data Collection

4–8

Send bank detail request forms to top 100 vendors; validate responses; enter into LFBK

90%+ bank coverage for electronic-pay vendors

Phase 4: Pilot

8–10

Execute F110 pilot for CoCd 1000 with 10 vendors; validate payment output; verify bank postings

Successful pilot payment run; reconciliation report

Phase 5: Rollout

10–12

Extend F110 to all company codes; schedule recurring weekly/biweekly runs; monitor exception lists

Fully operational automatic payment processing

## 13\. Expected Outcomes & KPIs

### F110 Readiness Score — Before vs. After Remediation

The scorecard below projects the expected improvement across each master data dimension after the 12-week remediation program. The "After" values assume successful completion of all roadmap phases.

Dimension

Before (Current)

After (Target)

Improvement

Payment Method Coverage (ZWELS)

~60%

≥95%

+35 percentage points

Payment Terms Coverage (ZTERM)

~65%

≥95%

+30 percentage points

Bank Detail Coverage (LFBK)

~45%

≥85%

+40 percentage points

Stale Data Exposure

USD 30M+ / EUR 2.5M+

< USD 50K

99%+ reduction

F110 Readiness (eligible vendors)

~35%

≥85%

+50 percentage points

AP Aging Accuracy

Unreliable (contaminated)

Accurate within 1 day

Qualitative transformation

Early Payment Discount Capture

~0% (manual process)

≥70% of discount-eligible

Est. EUR 50K–200K annual savings\*

\*The discount savings estimate assumes ZB01 terms (3% within 14 days) on EUR 5M+ annual payable volume. At 70% capture rate: EUR 5M × 3% × 70% = EUR 105K annual savings. Actual savings depend on payment volume and vendor discount terms.

**Target KPIs for Ongoing Monitoring (post-remediation):**

*   F110 payment run frequency: Weekly (minimum biweekly)
*   F110 exception rate: < 5% of eligible items per run
*   Vendor master completeness: ≥ 95% for ZWELS, ZTERM, LFBK on active vendors
*   AP aging accuracy: 0 items older than 2 fiscal years without documented reason
*   Early-payment discount capture rate: ≥ 70%

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._