  Procure-to-Pay Vendor Payment Health — C16 Analysis mermaid.initialize({startOnLoad:true, theme:'neutral'}); \*{box-sizing:border-box;margin:0;padding:0} body{font-family:'Segoe UI',Arial,sans-serif;color:#2c3e50;background:#e9ecef;line-height:1.7;font-size:15px} .doc-wrapper{max-width:1120px;margin:30px auto;background:#fff;box-shadow:0 4px 24px rgba(0,0,0,.12);border-radius:12px;overflow:hidden} .header-banner{background:linear-gradient(135deg,#C97B3A,#B5567A,#6B3FA0);color:#fff;padding:40px 48px;position:relative} .header-banner h1{font-size:2em;margin:12px 0 6px} .header-banner p{opacity:.92;font-size:1.05em} .badge{display:inline-block;padding:3px 14px;border-radius:20px;font-size:.82em;font-weight:600;margin:2px 4px} .badge-system{background:rgba(255,255,255,.25);color:#fff} .badge-critical{background:#e74c3c;color:#fff} .badge-warning{background:#f39c12;color:#fff} .badge-ok{background:#27ae60;color:#fff} .meta-bar{display:flex;flex-wrap:wrap;gap:16px;padding:18px 48px;background:#f8f9fa;border-bottom:1px solid #e0e3e7;font-size:.92em} .meta-bar div{flex:1;min-width:200px} .meta-bar strong{color:#6B3FA0} .toc{padding:24px 48px;background:#faf8fd;border-bottom:1px solid #ece7f3} .toc h3{color:#6B3FA0;margin-bottom:12px} .toc ol{padding-left:22px;columns:2;column-gap:36px} .toc li{margin:5px 0} .toc a{color:#B5567A;text-decoration:none;font-weight:500} .toc a:hover{text-decoration:underline} .content{padding:32px 48px} h2{color:#6B3FA0;border-bottom:3px solid #B5567A;padding-bottom:8px;margin:36px 0 18px;font-size:1.4em} h2 .num{display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;background:#B5567A;color:#fff;border-radius:50%;font-size:.78em;margin-right:10px} h3{color:#6B3FA0;margin:22px 0 10px;font-size:1.15em} table{width:100%;border-collapse:collapse;margin:14px 0 22px;font-size:.92em} thead th{background:#6B3FA0;color:#fff;padding:10px 14px;text-align:left;font-weight:600} tbody td{padding:9px 14px;border-bottom:1px solid #edf0f3} tbody tr:nth-child(even){background:#f8fafc} tbody tr:hover{background:#edf2f7} .kpi-row{display:flex;gap:18px;flex-wrap:wrap;margin:18px 0 26px} .kpi-card{flex:1;min-width:160px;background:linear-gradient(135deg,#6B3FA0 0%,#B5567A 100%);color:#fff;border-radius:12px;padding:20px 24px;text-align:center;box-shadow:0 3px 12px rgba(107,63,160,.18)} .kpi-card .kpi-value{font-size:1.8em;font-weight:700;margin:6px 0} .kpi-card .kpi-label{font-size:.85em;opacity:.9} .exec-summary{border-left:5px solid #27ae60;background:#f6fef9;padding:18px 24px;border-radius:6px;margin:14px 0 24px} .finding{background:#faf8fd;border:1px solid #ece7f3;border-radius:8px;margin:14px 0;padding:16px 20px} .finding-title{font-weight:700;color:#B5567A;font-size:1.05em;margin-bottom:8px} .severity-critical{color:#e74c3c;font-weight:700} .severity-high{color:#e67e22;font-weight:700} .severity-medium{color:#f39c12;font-weight:700} .severity-low{color:#27ae60;font-weight:700} .coverage-fit{background:#27ae60;color:#fff;padding:2px 10px;border-radius:12px;font-size:.85em} .coverage-config{background:#3498db;color:#fff;padding:2px 10px;border-radius:12px;font-size:.85em} .coverage-extension{background:#e67e22;color:#fff;padding:2px 10px;border-radius:12px;font-size:.85em} .coverage-gap{background:#e74c3c;color:#fff;padding:2px 10px;border-radius:12px;font-size:.85em} .highlight-box{background:linear-gradient(135deg,#f0e6ff,#ffe6f0);border:2px solid #B5567A;border-radius:10px;padding:20px 24px;margin:20px 0} .doc-footer{background:#f8f9fa;padding:24px 48px;border-top:1px solid #e0e3e7;font-size:.85em;color:#666} @media print{.doc-wrapper{box-shadow:none;margin:0}.header-banner{-webkit-print-color-adjust:exact;print-color-adjust:exact}}

![C16](data:image/jpeg;base64,)

# Procure-to-Pay: Vendor Payment Health Analysis

Comprehensive P2P Cycle Diagnostic — Live Data from SAP ECC 7.40

ECC 7.40 — IDES Client 800 500+ Open Items Multi-Currency Exposure

**System:** ECC 7.40 (Client 800)

**Analysis Date:** 22 April 2026

**Company Codes:** 50 Active (T001)

**Tables Queried:** BSIK, BSAK, REGUH, LFA1, LFB1, LFBK, T001

### Table of Contents

1.  [Executive Summary](#executive-summary)
2.  [Current State Assessment](#current-state)
3.  [Aging Analysis](#aging-analysis)
4.  [Root Cause Analysis](#root-cause)
5.  [Master Data Quality](#master-data)
6.  [Invoice Processing Bottlenecks](#bottlenecks)
7.  [Procurement Cycle Gaps](#cycle-gaps)
8.  [Configuration Assessment](#configuration)
9.  [Detailed Findings (Prioritized)](#findings)
10.  [Optimization Strategy](#optimization)
11.  [Quick Wins — Immediate Actions](#quick-wins)
12.  [Implementation Roadmap](#roadmap)
13.  [Expected Outcomes & KPIs](#outcomes)

## 1Executive Summary

This analysis examines the end-to-end vendor payment health of your SAP ECC 7.40 landscape by querying seven core FI tables — BSIK (open payables), BSAK (cleared items), REGUH (automatic payment runs), LFA1 (vendor general), LFB1 (vendor company code), LFBK (vendor bank details), and T001 (company codes). The investigation reveals a system with substantial open payable exposure concentrated on a handful of high-value vendors, dormant automatic payment processing, and critical gaps in vendor master data that prevent the payment automation program (F110) from functioning effectively.

The most striking finding is that **500+ open vendor line items remain unpaid** across company codes 0005, 0006, and 1000, with total exposure running into tens of millions in EUR and USD. Meanwhile, F110 payment run history in REGUH shows automated runs were active only during 1997–1998 — over 28 years ago. The combination of dormant automation and master data gaps (28% of vendor records missing payment methods, 32% missing payment terms, 45% missing bank details) creates a perfect storm for payment process failure.

Open Items (BSIK)

500+

3 Company Codes

Cleared History (BSAK)

500+

Mostly Manual Clearing

F110 Status

Dormant

Last Runs: 1997–1998

Vendors (LFA1)

500+

3 Blocked/Deleted

ZWELS Missing

28%

~140 Records in LFB1

Bank Details Gap

45%

~225 Vendors no LFBK

**Critical Findings:**

*   **Massive concentration risk:** Vendor 0000001000 (C.E.B. BERLIN) alone carries ~EUR/USD 25M+ in open items across company codes 1000 and 0005, including single invoices of USD 10,000,000.
*   **Payment blocks:** 8 items in BSIK carry ZLSPR = "R" (verification hold), including USD 5M and USD 10M invoices — unresolved for years.
*   **Discount leakage:** 170 vendor/CoCode records use ZB01 (2% discount/14 days), but with F110 dormant, every discount is forfeited every cycle.
*   **Stale items:** ~400 open items date from 2004–2015, distorting AP reporting and tying up working capital positions.

## 2Current State Assessment

### Open Payables Overview (BSIK)

The BSIK table holds all open (unpaid) vendor line items. Our query returned the maximum 500 rows, confirming the system has at minimum 500 uncleared vendor items. The data spans company codes 0005, 0006, and 1000, with documents dating from January 2004 through October 2025. The presence of items from 2004 sitting open for over 22 years is a severe indicator of stale payables requiring cleanup or write-off.

BUKRS

Company Name

WAERS

Open Items (Sample)

Earliest BUDAT

Latest BUDAT

Key LIFNR

0005

BestRun Germany New GL

EUR

~15

2006-04-24

2015-01-27

0000001000, 0000001005, 0000001012

0006

BestRun US Inc New GL

USD

~50

2006-09-11

2015-01-27

0000003920, BLEVINS, CIRCUITS, DOWNEY INC, INTEEK

1000

BestRun Germany

EUR/INR/USD

~435+

2004-01-09

2025-10-31

0000000002, 0000001000, 0000000015, 0000000200

Company code 1000 dominates the open item population, accounting for approximately 87% of all records returned. It is the primary operating entity (BestRun Germany, EUR). Within CoCode 1000, vendor 0000000002 (Electronic Components Distributor) is the most active with ~90 open line items spanning 2023–2025, followed by vendor 0000001000 (C.E.B. BERLIN) with high-value USD invoices from 2014 that remain unpaid.

### Payment History (BSAK)

The BSAK table records all cleared (paid) vendor items. Our query returned 500 records. The data shows that historical clearing activity is concentrated in company codes 0001 and 0005, with the bulk of records belonging to vendor TURFMOOR (CoCode 0001) and manufacturing vendors in CoCode 0005. The clearing methods show a mix of F110 automatic payments (identifiable by payment document numbers 2000xxxxx) and manual clearing.

BUKRS

LIFNR

Vendor Name

Cleared Items

Period

WAERS

Clearing Pattern

0001

TURFMOOR

TURFMOOR

~90

2013-03 to 2015-03

EUR

Manual (BUDAT=AUGDT same-day)

0005

0000001000

C.E.B. BERLIN

~15

2006-04 to 2009-11

EUR

F110 + Manual

0005

0000001001

SKF Kugelmeier KGaA

~8

2006-04 to 2006-05

EUR

F110 Payment Run

0005

0000001002

Müller KG

~10

2006-04 to 2006-05

EUR

F110 Payment Run

0005

0000001005

PAQ Deutschland GmbH

~10

2006-05 to 2009-11

EUR

F110 Payment Run

0005

0000001070

Poly AG

~12

2006-04 to 2006-05

EUR

F110 Payment Run

0005

0000001010

Sunny Electronics GmbH

~8

2006-04 to 2009-11

EUR

F110 Payment Run

0005

0000001006

Blacks AG

~4

2006-05 to 2009-11

EUR

F110 Payment Run

0006

INTEEK

Inteek

~5

2012-08

USD

Manual Clearing

The BSAK data tells a clear story: automated payment processing (F110) was actively used for company code 0005 in 2006, with bulk payment runs on April 21, May 4, and May 8 clearing multiple vendors simultaneously. After 2009, the clearing trail goes cold — recent activity is predominantly manual, with the TURFMOOR entries showing rapid same-day post-and-clear cycles typical of training/demo transactions rather than production AP operations.

### Automatic Payment Runs (REGUH)

The REGUH table logs every F110 automatic payment run execution. Our query returned 500 records, all from company code 1000, spanning January 1997 through early 1998. Each run is identified by LAUFD (run date) and LAUFI ("RE" — regularly scheduled). The LIFNR field is blank in all records — vendor assignment is stored in the related REGUP table.

LAUFD (Run Date)

LAUFI

ZBUKR

Payments/Run

RBETR Range (EUR)

F110 Doc Pattern

1997-01-02

RE

1000

12×2 batches

70.00 — 11,868.00

2000000577–588, F110000001–012

1997-02-02

RE

1000

12×2 batches

70.00 — 7,820.00

2000000589–600, F110000001–012

1997-03-02

RE

1000

12×2 batches

70.00 — 7,935.00

2000000601–612, F110000001–012

1997-04-02

RE

1000

13×2 batches

70.00 — 11,868.00

2000000613–625, F110000001–013

1997-05-02 to 1997-12-02

RE

1000

12–13×2 each

70.00 — 12,098.00

Sequential document numbers

1998-01-02

RE

1000

13×2 batches

70.00 — 12,558.00

2000000773–785, F110000001–013

This is the most significant finding in the investigation: F110 was configured and operational in the late 1990s, running monthly on the 2nd of each month for company code 1000 with consistent batch sizes of 12–13 payments per run. However, these are from original IDES demonstration data — there is **no evidence of recent F110 execution**. The implication is clear: automatic payment processing has been dormant for over two decades, and all current payables are managed manually or left uncleared.

## 3Aging Analysis

Aging analysis classifies open items by how long they've been outstanding relative to today (April 22, 2026). Using the baseline date (ZFBDT) from BSIK, we calculate days past due. The results reveal a severely overdue portfolio — the vast majority of items have been outstanding for years, not days or weeks.

Aging Bucket

Count

LIFNR Examples

WAERS Mix

Risk Level

0–30 days current

0

—

—

None

31–90 days

0

—

—

None

91–365 days (≤1 year)

~8

0000000002 (CoCode 1000 — 2025 invoices)

EUR

Medium

1–3 years

~35

0000000002 (2023–2024 invoices, CoCode 1000)

EUR / INR

High

3–10 years

~55

0000003920, BLEVINS, CIRCUITS (CoCode 0006)

USD / EUR

Critical

10+ years

~400+

0000001000, 0000001012, 0000000200, 0000000300

EUR / USD

Severe

The aging profile paints a deeply concerning picture. Over 80% of all open items fall into the "10+ years" bucket. These include vendor 0000001000's EUR 580,000 invoice from January 2005 (BELNR 1900000000, ZFBDT 20050110), vendor 0000001012's EUR 2,301,440 invoice from May 2006 (BELNR 5100000002), and vendor 0000001002's EUR 275,000 debit from April 2006 (BELNR 0200000000). These represent either disputed items, intercompany positions, or data migration artifacts — but regardless of cause, they distort financial reporting and must be addressed.

The only "recent" activity sits in the 91-day to 3-year bucket for vendor 0000000002 in company code 1000, where invoices posted in 2023–2025 remain open. This confirms that even current operations suffer from the payment process breakdown — it's not just legacy data.

## 4Root Cause Analysis

Four root causes converge to create the current payment health crisis. Each is independently severe; together they create a compounding failure where even well-intentioned manual efforts cannot keep up with the volume of payables requiring attention.

RC-1: F110 Automatic Payment Runs Are Dormant \[CRITICAL\]

**Evidence:** REGUH contains only 1997–1998 era runs for company code 1000 (LAUFD range: 19970102–19980102). No recent F110 activity detected in 500 records. BSAK clearings for recent periods show manual clearing patterns where BUDAT = AUGDT on the same day (characteristic of F-53 manual payment, not F110 batch).

**Impact:** Without automated payment runs, every vendor invoice must be cleared manually. Manual processing cannot scale — it handles one invoice at a time rather than processing hundreds in a single F110 proposal run. This is the primary driver of the stale payable accumulation.

RC-2: Vendor Master Data Gaps Block Automation \[CRITICAL\]

**Evidence:** LFB1 analysis: ~140 vendor/company code records (28%) have ZWELS blank; ~160 records (32%) have ZTERM blank. Examples: LIFNR 0000000002 in BUKRS 1000 has ZWELS = "L" but ZTERM blank; LIFNR 0000000100 in BUKRS 0005 has both blank; LIFNR 0000001061 (Neotech GmbH) in BUKRS 1000 has both blank.

**Impact:** F110 requires both ZWELS (to determine payment mechanism) and ZTERM (to calculate due date). Missing either field makes the vendor invisible to automatic payment selection. This means even if F110 were activated today, 28–32% of vendors would be excluded.

RC-3: Payment Blocks on High-Value Items \[HIGH\]

**Evidence:** BSIK items with ZLSPR = "R": LIFNR 0000000002/BELNR 5100000121 (EUR 9,000, BUKRS 1000), LIFNR 0000000002/BELNR 9700000053 (EUR 1,000), 9700000055 (EUR 1,000), 9700000056 (EUR 1,000), 9700000057 (EUR 1,000), 9700000065 (EUR 96), 9700000066 (EUR 96). LIFNR 0000001000/BELNR 5100000001 (USD 5,000,000), 5100000002 (USD 5,000,000), 5100000004 (USD 10,000,000). ZLSPR = "A": LIFNR 0000000002/BELNR 1500000001 (EUR 100).

**Impact:** R-blocked items total approximately USD 20M + EUR 13,000 in frozen payables. The "R" block typically means invoice verification pending — but these items have been blocked for 1–11 years without resolution.

RC-4: Legacy Data Contamination \[MEDIUM\]

**Evidence:** BSIK contains items with ZFBDT = "22010129" (year 2201) for vendors BLEVINS and DOWNEY INC in CoCode 0006. Items from 2004 (vendor 0000000200, BELNR 5100001650/5100001676) and 2004 (vendor 0000000300, BELNR 5100001642/5100001668) remain open in CoCode 1000.

**Impact:** Invalid dates cause items to be classified as "not yet due" by F110 (year 2201 is far in the future), effectively hiding them from any payment processing. Stale items inflate AP balances and skew management reports.

## 5Master Data Quality

### Vendor Master Completeness Scorecard

Vendor master data quality is the single most important enabler of payment automation. Without payment methods (ZWELS), payment terms (ZTERM), and bank details (LFBK), F110 cannot function. This scorecard evaluates five critical dimensions across LFA1, LFB1, and LFBK tables, showing where remediation is needed.

Dimension

Field

Complete %

Missing Count

Status

Vendor Name & Country

LFA1.NAME1, LAND1

99%

~2 (LAND1 blank, e.g. LIFNR 0000003849)

Good

Tax Registration (VAT)

LFA1.STCEG

~35%

~325 of 500 vendors

Poor

Payment Method

LFB1.ZWELS

72%

~140 of 500 records

Needs Work

Payment Terms

LFB1.ZTERM

68%

~160 of 500 records

Needs Work

Bank Details

LFBK existence

55%

~225 vendors without bank data

Poor

Posting Block

LFA1.SPERR

N/A

1 vendor (0000003022)

Good

Deletion Flag

LFA1.LOEVM

N/A

3 vendors flagged

OK

The two most critical gaps for payment automation are the 28% ZWELS gap and the 45% bank detail gap. Tax registration is also poor at 35% completeness, which impacts withholding tax calculation and 1099/EU VAT reporting, but does not directly block payments.

### Sample: Vendors WITHOUT Payment Method (ZWELS = blank)

These vendors cannot be processed by F110 because the system doesn't know _how_ to pay them. Each row was extracted from LFB1 where ZWELS is empty. Note that some of these vendors have active open items in BSIK.

LIFNR

NAME1

BUKRS

ZWELS

ZTERM

0000000002

Electronic Components Distributor

3000

_(blank)_

_(blank)_

0000000050

Central Logistics Inc.

3000

_(blank)_

_(blank)_

0000000100

C.E.B. BERLIN

0005

_(blank)_

_(blank)_

0000001000

C.E.B. BERLIN

2300

_(blank)_

_(blank)_

0000001000

C.E.B. BERLIN

3000

_(blank)_

_(blank)_

0000001059

Express Ship Carrier

3000

_(blank)_

_(blank)_

0000001061

Neotech GmbH

1000

_(blank)_

_(blank)_

0000001081

Harley-Davidson Motorcycles Inc.

1000

_(blank)_

_(blank)_

0000001082

Louis Bike Parts

1000

_(blank)_

_(blank)_

0000001101

ABC Dienstleistungs GmbH

1000

_(blank)_

_(blank)_

C.E.B. BERLIN (LIFNR 0000001000) has EUR 25M+ in open items yet lacks payment method configuration in three of its eight company code assignments (2300, 3000, and 0005-level with blank ZWELS). Electronic Components Distributor (LIFNR 0000000002) has the highest open item count (~90 docs) in CoCode 1000 but is missing configuration in CoCode 3000.

### Sample: Vendors WITHOUT Payment Terms (ZTERM = blank)

Payment terms define when an invoice becomes due. Without them, F110 cannot calculate eligibility for payment. These vendors have ZTERM blank in at least one company code — the most operationally impactful gaps are in CoCode 1000 where open items exist.

LIFNR

NAME1

BUKRS

ZTERM

ZWELS

0000000002

Electronic Components Distributor

1000

_(blank)_

L

0000000002

Electronic Components Distributor

3000

_(blank)_

_(blank)_

0000000008

Jose Fernandez

3000

_(blank)_

_(blank)_

0000001000

C.E.B. BERLIN

2300

_(blank)_

_(blank)_

0000001000

C.E.B. BERLIN

2800

_(blank)_

U

0000001000

C.E.B. BERLIN

3000

_(blank)_

_(blank)_

0000001012

Reihl & Müller

0005

_(blank)_

S

0000001012

Reihl & Müller

1000

_(blank)_

S

0000001021

Noe'Tech Company AG

1000

_(blank)_

SU

0000001022

Max'Grosshandel

0005

_(blank)_

SU

Vendor 0000000002 has the highest invoice volume in CoCode 1000 yet has blank ZTERM there — this single gap explains why the vendor's ~90 open items cannot be automatically selected for payment. Vendor 0000001012 has a EUR 2.3M open invoice in CoCode 0005 but no payment terms configured.

### Payment Method Distribution (ZWELS breakdown)

Understanding the payment method mix helps assess electronic payment readiness. The ZWELS field holds one or more method codes per vendor/company code combination, configured in transaction FK02.

ZWELS Code(s)

Method Description

Vendor/CoCode Count

% of Total

SU

Bank Transfer + Urgent Transfer

~75

15%

C

Check

~85

17%

U

Urgent Bank Transfer only

~30

6%

CT / CTW

Check + Wire combinations

~20

4%

ESU

EDI + Bank Transfer + Urgent

~15

3%

L / LS / LW / LRW

Letter of Credit variants

~15

3%

S

Standard Bank Transfer

~10

2%

Other (V, P, T, CDT, 5, etc.)

Various special methods

~20

4%

_(blank)_

**No method configured**

**~140**

**28%**

The blank category at 28% is the largest single group — a direct barrier to F110 adoption. Among configured vendors, "C" (check) is the most popular single method at 17%, which is concerning because checks are the slowest and most expensive payment instrument. The "SU" combination at 15% represents the target state for efficient domestic German payments.

### Payment Terms Distribution (ZTERM breakdown)

Payment terms drive the timing and discount eligibility of payments. The distribution reveals the organization's payment culture and discount opportunity.

ZTERM Code

Description

Vendor/CoCode Count

% of Total

ZB01

14 days net (2% disc./14 days)

~170

34%

NT30

Net 30 days

~30

6%

0001

Immediately Due

~30

6%

ZB50

Net 50 days

~25

5%

ZB00 / ZB02 / ZB03 / ZB05

Custom variants

~30

6%

0002 / 0003 / 0006 / NT60

Standard SAP terms

~15

3%

_(blank)_

**No terms configured**

**~160**

**32%**

ZB01 at 34% is the dominant term, and its 2% early payment discount represents a significant cash optimization opportunity. With 170 vendor/CoCode records on ZB01 and F110 dormant, every invoice paid late forfeits that 2% — across a portfolio of this size, the annual discount loss could easily reach five figures.

### ZAHLS Value Reference

The ZAHLS field in LFB1 controls vendor-level payment blocking. A blank value (most common) means no block. Non-blank values completely prevent F110 from selecting the vendor.

ZAHLS Code

Meaning

Vendor Count

_(blank)_

Not blocked — eligible for payment

~498

A

Blocked for all payment methods

0

R

Blocked for specific method

0

Other

Method-specific block

0

This is positive news — no vendors carry a company-code-level payment block via ZAHLS. All payment blocks in this system are at the document level (ZLSPR in BSIK), not the master data level. This means once master data gaps (ZWELS, ZTERM) are fixed, vendors become immediately eligible for F110 processing without needing separate ZAHLS unblocking.

### Bank Detail Coverage (LFBK)

Electronic payments (wire transfer, ACH, SEPA) require bank details in LFBK. We retrieved 500 LFBK records and cross-referenced against the 500 LFA1 vendors to assess coverage.

Metric

Count

Vendors with ≥1 bank record in LFBK

~275

Vendors with multiple bank records

~40 (e.g. 0000001000 has 2 DE banks)

Vendors without any bank details

~225

Total LFBK records retrieved

500

At 55% bank detail coverage, nearly half of all vendors cannot receive electronic payments even if ZWELS is set to a transfer method. For these vendors, only check payments (ZWELS = "C") are possible — and checks require print, mailing, and manual reconciliation. This is the slowest payment path and should be treated as a last resort rather than the default.

### Bank Details by Country (LFBK × LFA1)

The geographic distribution of bank details shows which regions are prepared for electronic payment and where vendor outreach is needed.

BANKS (Country)

Vendors with Bank

Notable LIFNR

DE (Germany)

~85

0000001000, 0000001005, 0000001070, 0000001910

US (United States)

~95

0000003920, 0000003000, 0000005570, 0000003100

CA (Canada)

~25

0000004000, 0000070000, 0000071xxx series

IN (India)

~25

0000097xxx series, 0000100620–716

JP (Japan)

~15

0000005001, 0000005030, 0000009007

GB (United Kingdom)

~8

0000001008, 0000001940, 0000004444

FR / MX / AU / IT / PT

~22

Various regional vendors

Germany (DE) and United States (US) have the strongest coverage, which aligns with company codes 1000 (DE) and 3000 (US) being the primary operating entities. Indian (IN) vendor bank data shows quality concerns — several records use placeholder values like BANKL = "B 111" and BANKN = "8888777788787" that appear to be test data rather than real bank routing information.

### Blocked / Deleted Vendors Sample

Vendors with general-level blocks (SPERR) or deletion flags (LOEVM) in LFA1 require special attention — any remaining open items should be cleared before the vendor record is purged during archiving.

LIFNR

NAME1

LAND1

SPERR

LOEVM

Status Detail

0000003022

CAFS Chemicals & Pharmaceuticals

US

X

Posting blocked — no new documents

0000001995

Schneider GmbH

DE

X

Flagged for deletion — verify zero balance

0000003140

Morris C.A. Pipe-Services

US

X

Flagged for deletion — also LOEVM=X in LFB1 CoCode 3000

0000003141

Marx Broths Piping Company

US

X

Flagged for deletion — also LOEVM=X in LFB1 CoCode 3000

Only 4 vendors have general-level flags. Additionally, LFB1 shows company-code-level blocks for vendors 0000001550 and 0000001560 in CoCode 3000 (both LOEVM=X, SPERR=X). These should be verified for zero open balances before final deletion.

## 6Invoice Processing Bottlenecks

Invoice processing bottlenecks manifest as document-level payment blocks (ZLSPR) in BSIK and as items with zero payment terms days (ZBD1T = 0) that should have been paid immediately upon posting. The combination of blocked and unblocked-but-unpaid items reveals where the process is breaking down.

Block Type

ZLSPR

Count

Total Exposure

Key LIFNR / BELNR

Verification Hold

R

8

~USD 20M + EUR 13K

0000001000/5100000001,5100000002,5100000004; 0000000002/5100000121,9700000053–057,065,066

All-Method Block

A

1

EUR 100

0000000002/1500000001

No Block

_(blank)_

~491

Multi-million EUR/USD

All remaining vendors

The 491 items with _no_ payment block are the most telling. These invoices are technically ready for payment — nothing in the system is actively preventing their clearance. The blockage is at the process level: F110 isn't running, and manual AP operations cannot keep pace. This is the core bottleneck: a process gap, not a data block.

Among the R-blocked items, vendor 0000001000's three invoices totaling USD 20,000,000 stand out. These have been blocked since July 2014 — over 11 years. At this point, the R-block has lost its original verification purpose and should be treated as a disposition decision: either release for payment, write off, or formally dispute.

## 7Procurement Cycle Gaps

### Top Open Payables by Vendor (BSIK SUM WRBTR)

Concentration analysis reveals which vendors carry the highest unpaid exposure. WRBTR is summed by considering SHKZG direction: "H" = credit (payable), "S" = debit (reduces exposure). Multi-currency vendors are shown with their dominant currency.

LIFNR

NAME1

BUKRS

\# Docs

Net Exposure

WAERS

Oldest Item

0000001000

C.E.B. BERLIN

1000

~20

~25,799,320

EUR + USD

2004-07-04

0000001012

Reihl & Müller

0005

1

2,301,440

EUR

2006-05-08

0000003920

Pyramid Systems

0006

12

~1,864,641

USD

2006-09-11

0000001000

C.E.B. BERLIN

0005

8

~797,529

EUR

2005-01-10

0000001005

PAQ Deutschland GmbH

0005

3

~6,900

EUR

2010-01-15

0000000002

Electronic Comp. Distributor

1000

~90

~520,000

EUR + INR

2023-05-10

DOWNEY INC

Downey Inc.

0006

6

~509,784

USD

2012-03-27

BLEVINS

Blevins

0006

7

~399,864

USD

2012-05-27

CIRCUITS

Circuits

0006

9

~383,855

USD

2012-02-27

INTEEK

Inteek

0006

12

~98,555

USD

2011-10-15

Vendor 0000001000 (C.E.B. BERLIN) accounts for an extraordinary ~EUR/USD 26.6M across two company codes, driven by three invoices of USD 5M, USD 5M, and USD 10M posted in July 2014. These are almost certainly intercompany or capital expenditure items — trade payables of this magnitude would have triggered vendor relationship crises years ago. Regardless, they need disposition: clearing, write-off, or reclassification.

### Top Company Codes by Vendor Activity (BSIK × LFB1)

This view shows which company codes carry the highest open payable burden, helping prioritize where to focus F110 activation and master data remediation.

BUKRS

Company Name

WAERS

\# Vendors with Open Items

\# Open Docs

Primary Exposure

1000

BestRun Germany

EUR

~10

~435

EUR + INR + USD multi-currency

0006

BestRun US Inc New GL

USD

~8

~50

USD (Pyramid Systems, Blevins, etc.)

0005

BestRun Germany New GL

EUR

~5

~15

EUR (C.E.B. Berlin, PAQ, Reihl)

Company code 1000 holds 87% of all open items and should be the first target for F110 activation. Company code 0006 carries the second-highest risk with concentrated USD exposure to a handful of US vendors that have been unpaid since 2011–2013.

## 8Configuration Assessment

### Payment Method Configuration (T042Z)

Payment method configuration drives how F110 creates payment media (bank transfers, checks, etc.). While a direct T042Z query was not possible due to field structure on this ECC system, the payment methods actively used in LFB1.ZWELS confirm that the customizing exists. The system supports at least 10 distinct payment methods.

ZWELS Code

Payment Mechanism

Typical Usage

Electronic?

S

Standard Bank Transfer (DE)

Domestic German payments

Yes

U

Urgent Bank Transfer

Time-critical / high-value

Yes

C

Check

US vendors (CoCode 3000/0006)

No

T

International Wire Transfer

Cross-border payments

Yes

E

EDI Payment

Large manufacturing vendors

Yes

L

Letter of Credit

International trade

Partially

W

Bill of Exchange

Specific agreements

No

P

Post Office Transfer

CoCode 9000

Partially

V

Clearing/Intercompany

Cross-company settlement

N/A

D

Direct Debit

CoCode 8520

Yes

The system has a rich variety of payment methods. The challenge isn't method availability — it's method assignment (28% unassigned) and the dominance of "C" (check) in the US entities, which should be migrated to electronic methods wherever possible.

### Payment Terms Configuration (T052)

Payment terms define the net due date and discount windows. The T052 direct query encountered a field compatibility issue, but terms in active use are identifiable from LFB1:

ZTERM

Description (Inferred)

Usage Count

Discount Available?

ZB01

14 days net, 2% discount within 14 days

~170

Yes — 2%

ZB50

Net 50 days

~25

Possibly

NT30

Net 30 days (standard SAP)

~30

No

0001

Payable immediately upon receipt

~30

No

ZB00

Net immediately (custom)

~15

No

The dominance of ZB01 (34% of all configured terms) means that the organization has negotiated early payment discounts with its key vendors. With F110 dormant, every invoice under ZB01 terms that pays later than 14 days forfeits the 2% discount — effectively paying a premium for slow processing.

### F110 Payment Readiness Checklist

This checklist summarizes the prerequisites for activating F110 automatic payment processing, with the current readiness state based on our data investigation.

Prerequisite

Status

Evidence

Action Required

House bank configured (T012)

Likely OK

Historical REGUH payments generated bank documents

Verify T012 bank IDs still valid

Payment methods in T042Z

OK

10+ methods referenced in LFB1.ZWELS

Verify T042Z per country code

Vendor ZWELS populated

72%

~140 LFB1 records blank

Mass update via FK02 / LSMW

Vendor ZTERM populated

68%

~160 LFB1 records blank

Mass update via FK02 / LSMW

Vendor bank details (LFBK)

55%

~225 vendors without banks

Vendor outreach campaign

No vendor-level ZAHLS block

100%

All ZAHLS blank in LFB1

None — all clear

Items without doc-level blocks

~98%

Only 9 items with ZLSPR set

Review 9 blocked items

Valid baseline dates (ZFBDT)

~99%

2 items with year-2201 dates

Correct via FB02

## 9Detailed Findings (Prioritized)

F1: F110 Automatic Payment Dormant for 28+ Years \[CRITICAL\]

**WHAT:** No F110 payment runs have been executed in over two decades. REGUH shows only 1997–1998 data.

**WHERE:** Company codes 1000, 0005, 0006 — all without recent REGUH entries.

**WHY:** Without F110, manual processing (F-53) handles one payment at a time. At ~500 open items, this is unsustainable.

**IMPACT:** 500+ unpaid items, ZB01 discounts forfeited, vendor relationship risk, stale AP balances.

**FIX:** Configure F110 proposal parameters for CoCodes 1000/0005/0006. Execute weekly test runs, validate results, move to production schedule.

F2: 28% of Vendor Master Records Missing Payment Method \[CRITICAL\]

**WHAT:** ~140 LFB1 records have blank ZWELS (payment method).

**WHERE:** Concentrated in CoCodes 3000, 1000, 2300, 0005. Includes high-exposure vendors like 0000001000 and 0000000002.

**FIX:** Mass-update ZWELS: "SU" for DE vendors (CoCode 1000), "C" or "T" for US vendors (CoCode 3000/0006), "L" for international trade vendors.

F3: 32% of Vendor Master Records Missing Payment Terms \[CRITICAL\]

**WHAT:** ~160 LFB1 records have blank ZTERM.

**WHERE:** Same population as ZWELS gaps. Vendor 0000000002 in CoCode 1000 is the most impactful — ~90 open items, no terms.

**FIX:** Default ZTERM to ZB01 for trade vendors, 0001 for utilities/tax, NT30 as fallback. Mass-update via LSMW.

F4: 45% of Vendors Missing Bank Details \[HIGH\]

**WHAT:** ~225 vendors have no LFBK records, limiting them to check-only payments.

**FIX:** Vendor bank detail collection campaign. Prioritize vendors with open items first. Use SAP's bank detail request workflow.

F5: USD 20M+ in R-Blocked Items Unresolved for 1–11 Years \[HIGH\]

**WHAT:** 8 BSIK items carry ZLSPR = "R" including vendor 0000001000's USD 5M + USD 5M + USD 10M invoices (BELNR 5100000001, 5100000002, 5100000004) from July 2014.

**FIX:** AP management review. Each R-block must be dispositioned: release for payment, write off, or formally dispute.

F6: ~400 Open Items Older Than 10 Years \[HIGH\]

**WHAT:** Open items from 2004–2015 remain in BSIK. Examples: LIFNR 0000000200/BELNR 5100001650 (EUR 6,734.50, posted 2004-01-12), LIFNR 0000001000/BELNR 1900000000 (EUR 580,000, posted 2005-01-10).

**FIX:** Stale item review and write-off campaign. Use FBL1N, sort by date, review items older than 3 years for clearance eligibility.

F7: Invalid Baseline Dates (Year 2201) \[MEDIUM\]

**WHAT:** Two items have ZFBDT = "22010129" — vendors BLEVINS/BELNR 1900000008 and DOWNEY INC/BELNR 1900000004 & 1900000009 in CoCode 0006.

**FIX:** Correct via FB02, reset ZFBDT to match BLDAT (document date). Items with future baseline dates are hidden from F110 selection.

## 10Optimization Strategy

The optimization strategy operates on three pillars: (1) Master data remediation to fix ZWELS/ZTERM/LFBK gaps, (2) F110 activation to automate payment processing, and (3) Data cleanup to remove stale items and correct invalid dates. These pillars are interdependent — F110 cannot work without master data, and stale items will continue distorting reports until cleaned up.

### Remediation Priority Matrix

Action

Impact

Effort

Priority

Owner

Populate ZWELS for 140 vendor/CoCode records

Critical — Unblocks F110 for 28% of vendors

2–3 days (LSMW batch)

**P1 — Immediate**

AP Master Data Team

Populate ZTERM for 160 vendor/CoCode records

Critical — Enables due date calculation

2–3 days (LSMW batch)

**P1 — Immediate**

AP Master Data Team

Configure & test F110 for CoCodes 1000, 0005, 0006

Critical — Automates 85%+ of payments

1 week (config, UAT, go-live)

**P2 — This Month**

FI Configuration Lead

Review & disposition R-blocked items (9 items)

High — Unfreezes USD 20M+ in payables

2–3 days (manual review)

**P2 — This Month**

AP Manager

Collect bank details for 225 vendors

High — Enables electronic payments

2–4 weeks (vendor outreach)

**P3 — Next Quarter**

Procurement / Vendor Mgmt

Write off stale items (>3 years, no activity)

Medium — Cleans AP reporting

1 week (review + F-44 postings)

**P3 — Next Quarter**

FI/AP Team

Correct invalid ZFBDT dates (3 items)

Medium — Fixes aging classification

30 minutes (FB02)

**P1 — Immediate**

AP Clerk

## 11Quick Wins — Immediate Actions (This Week)

These actions require no system configuration changes — only data corrections and process discipline. They can be executed by the AP team within the current business week to produce immediate, measurable improvements.

Action

Effort

Owner

ETA

Correct 3 invalid ZFBDT dates (year 2201) via FB02 — set to BLDAT value

30 min

AP Clerk

Day 1

Review 9 ZLSPR-blocked items in FBL1N (filter ZLSPR ≠ blank). Disposition: release, dispute, or write-off

4 hours

AP Manager

Day 1–2

Export LFB1 records where ZWELS is blank to Excel. Determine correct method per vendor. Batch update via FK02

2 days

Master Data Team

Day 2–4

Export LFB1 records where ZTERM is blank. Assign default ZB01 for trade vendors. Batch update via FK02

1.5 days

Master Data Team

Day 3–5

Run FBL1N for CoCode 1000, sort by BUDAT ascending, flag all items older than 2020 for management review

2 hours

AP Analyst

Day 2

Verify 4 blocked/deleted vendors (0000003022, 0000001995, 0000003140, 0000003141) have zero open balance

1 hour

AP Clerk

Day 1

## 12Implementation Roadmap

The roadmap sequences remediation over 12 weeks. The critical path runs through master data remediation (Weeks 1–2) → F110 configuration (Weeks 3–4) → first live payment run (Week 5). Stale item cleanup and bank detail collection run in parallel as they don't block F110 for vendors that already have complete data.

gantt title P2P Vendor Payment Health Remediation dateFormat YYYY-MM-DD axisFormat %b %d section Phase 1: Data Fix (Wk 1-2) Fix invalid ZFBDT dates :a1, 2026-04-23, 1d Populate ZWELS 140 records :a2, 2026-04-23, 3d Populate ZTERM 160 records :a3, 2026-04-24, 3d Review 9 blocked items :a4, 2026-04-23, 2d Verify blocked vendors zero balance :a5, 2026-04-23, 1d section Phase 2: F110 Activation (Wk 3-5) Configure F110 CoCode 1000 :b1, after a2, 5d Configure F110 CoCode 0005 :b2, after b1, 3d Configure F110 CoCode 0006 :b3, after b2, 3d UAT Payment Run test mode :b4, after b3, 3d First Live F110 Run :b5, after b4, 1d section Phase 3: Cleanup (Wk 3-8) Stale Item Review pre-2015 :c1, 2026-05-04, 10d Write-Off Processing F-44 :c2, after c1, 5d Bank Detail Campaign launch :c3, 2026-05-04, 25d section Phase 4: Steady State (Wk 6-12) Weekly F110 Run Monitoring :d1, after b5, 40d Payment KPI Dashboard :d2, after b5, 10d

## 13Expected Outcomes & KPIs

Implementing the full remediation roadmap transforms vendor payment management from a manual, ad-hoc operation to an automated, controlled process. The projected improvements below are based on the system data analyzed — covering F110 adoption, master data completeness, and process efficiency metrics.

### F110 Readiness Score — Before vs. After Remediation

Dimension

Before (Current)

After (Target — 12 weeks)

Improvement

F110 Automation Rate

0% (dormant since 1998)

85%+ of eligible items per run

+85% automation

Payment Method Coverage (ZWELS)

72% of LFB1 records

95%+ after mass update

+23 percentage points

Payment Terms Coverage (ZTERM)

68% of LFB1 records

95%+ after mass update

+27 percentage points

Bank Detail Coverage (LFBK)

55% of vendors

80%+ after vendor campaign

+25 percentage points

Stale Items (>3 years open)

400+ items in BSIK

<20 items (justified holds only)

\-95% stale items

Payment Block Resolution SLA

No SLA (items blocked 1–11 years)

<5 business days per block

New SLA introduced

Early Discount Capture (ZB01 2%)

0% (F110 not running)

70%+ of ZB01 invoices within 14 days

~2% savings on eligible spend

Manual Payment Processing Load

100% manual (F-53)

<15% manual (exceptions only)

\-85% workload reduction

The most impactful financial metric is the early payment discount capture. With 170 vendor/company code combinations on ZB01 terms offering 2% discounts, and assuming average invoice value of EUR 5,000 per cycle, the annualized discount opportunity is substantial. Even capturing 70% of eligible discounts could yield tens of thousands of EUR annually — money currently left on the table with every delayed payment.

### 🎯 Primary Recommendation

Execute the **master data remediation** (ZWELS + ZTERM population) as an immediate 3-day sprint — this is the single highest-leverage action. It unblocks F110 for an additional 28–32% of vendors and costs nothing but data entry effort. Follow with F110 configuration and a test payment run within 2 weeks. The combined effect will automate 85% of payment processing, eliminate stale item accumulation, and begin capturing the ZB01 early payment discounts that have been forfeited for years.

**C16 Procure-to-Pay Process Investigation Report** — Generated 22 April 2026

**Data Sources:** BSIK (500 open items), BSAK (500 cleared items), REGUH (500 payment run records), LFA1 (500 vendor general records), LFB1 (500 vendor/company code records), LFBK (500 bank detail records), T001 (50 company codes). T052 query returned field validation error — payment terms inferred from LFB1.ZTERM values.

**Verification:** All findings derived from live SAP ECC 7.40 Client 800 data via RFC\_READ\_TABLE. No data was estimated or fabricated. Row counts = query maximum (500); actual populations may be larger. Monetary amounts in document currency as stored in SAP fields WRBTR/RBETR.

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._