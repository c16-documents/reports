  ZBDC\_XK01 — Clean Core Analysis Report mermaid.initialize({startOnLoad:true, theme:'neutral', gantt:{fontSize:12}}); body { font-family: 'Segoe UI', Arial, sans-serif; color: #2c3e50; background: #e9ecef; margin: 0; padding: 20px; } .doc-wrapper { max-width: 1120px; margin: 0 auto; background: #fff; box-shadow: 0 4px 24px rgba(0,0,0,0.10); border-radius: 12px; overflow: hidden; } .header-banner { background: linear-gradient(135deg, #C97B3A, #B5567A, #6B3FA0); color: #fff; padding: 40px 48px; } .header-banner img { height: 50px; margin-bottom: 12px; border-radius: 8px; } .header-banner h1 { margin: 0 0 8px 0; font-size: 2.2em; font-weight: 700; } .header-banner .subtitle { font-size: 1.15em; opacity: 0.92; } .header-banner .badge { display: inline-block; padding: 4px 14px; border-radius: 20px; font-size: 0.85em; font-weight: 600; margin-top: 10px; } .badge-s4 { background: rgba(255,255,255,0.25); color: #fff; } .badge-score { background: #e74c3c; color: #fff; } .badge-level-c { background: #e67e22; color: #fff; padding: 4px 14px; border-radius: 20px; font-size: 0.85em; font-weight: 600; } .badge-level-b { background: #3498db; color: #fff; padding: 4px 14px; border-radius: 20px; font-size: 0.85em; font-weight: 600; } .badge-level-a { background: #27ae60; color: #fff; padding: 4px 14px; border-radius: 20px; font-size: 0.85em; font-weight: 600; } .badge-level-d { background: #e74c3c; color: #fff; padding: 4px 14px; border-radius: 20px; font-size: 0.85em; font-weight: 600; } .meta-bar { display: flex; flex-wrap: wrap; gap: 16px; background: #f8f9fa; padding: 18px 48px; border-bottom: 1px solid #e0e0e0; font-size: 0.95em; } .meta-item { display: flex; align-items: center; gap: 6px; } .meta-label { font-weight: 600; color: #6B3FA0; } .content { padding: 32px 48px 48px 48px; } .toc { background: #f8f9fc; border: 1px solid #e0e0e0; border-radius: 8px; padding: 24px 32px; margin-bottom: 32px; } .toc h3 { margin-top: 0; color: #6B3FA0; } .toc ul { list-style: none; padding-left: 0; } .toc li { padding: 4px 0; } .toc a { color: #6B3FA0; text-decoration: none; font-weight: 500; } .toc a:hover { text-decoration: underline; } h2.section-title { color: #6B3FA0; border-bottom: 3px solid #B5567A; padding-bottom: 8px; margin-top: 40px; } h2.section-title .num { display: inline-flex; align-items: center; justify-content: center; width: 30px; height: 30px; background: #B5567A; color: #fff; border-radius: 50%; font-size: 0.7em; margin-right: 10px; vertical-align: middle; } h3 { color: #4a2d7a; margin-top: 28px; } table { width: 100%; border-collapse: collapse; margin: 16px 0; font-size: 0.92em; } thead { background: #6B3FA0; color: #fff; } th { padding: 10px 14px; text-align: left; font-weight: 600; } td { padding: 9px 14px; border-bottom: 1px solid #e8e8e8; } tr:nth-child(even) { background: #f8fafc; } tr:hover { background: #edf2f7; } .exec-summary { border-left: 5px solid #27ae60; background: #f6fef9; padding: 20px 24px; border-radius: 0 8px 8px 0; margin: 16px 0; } .critical-box { border-left: 5px solid #e74c3c; background: #fef6f6; padding: 20px 24px; border-radius: 0 8px 8px 0; margin: 16px 0; } .warning-box { border-left: 5px solid #e67e22; background: #fef9f0; padding: 20px 24px; border-radius: 0 8px 8px 0; margin: 16px 0; } .info-box { border-left: 5px solid #3498db; background: #f0f7fe; padding: 20px 24px; border-radius: 0 8px 8px 0; margin: 16px 0; } .recommendation-box { border-left: 5px solid #6B3FA0; background: #f5f0ff; padding: 20px 24px; border-radius: 0 8px 8px 0; margin: 16px 0; } .kpi-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 18px; margin: 20px 0; } .kpi-card { background: linear-gradient(135deg, #6B3FA0, #8B5CF6); color: #fff; padding: 22px; border-radius: 12px; text-align: center; } .kpi-card .kpi-value { font-size: 2.2em; font-weight: 700; } .kpi-card .kpi-label { font-size: 0.88em; opacity: 0.9; margin-top: 4px; } .kpi-card.red { background: linear-gradient(135deg, #c0392b, #e74c3c); } .kpi-card.orange { background: linear-gradient(135deg, #d35400, #e67e22); } .kpi-card.green { background: linear-gradient(135deg, #1e8449, #27ae60); } .kpi-card.blue { background: linear-gradient(135deg, #2471a3, #3498db); } details { margin: 8px 0; } summary { background: #f0f4f8; padding: 10px 16px; border-radius: 6px; cursor: pointer; font-weight: 600; color: #2c3e50; } summary:hover { background: #e4e9f0; } pre { background: #f5f5f5; border-left: 4px solid #B5567A; padding: 16px; overflow-x: auto; font-size: 0.88em; border-radius: 0 6px 6px 0; } code { font-family: 'Consolas', 'Courier New', monospace; } .coverage-fit { background: #27ae60; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .coverage-config { background: #3498db; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .coverage-extension { background: #e67e22; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .coverage-gap { background: #e74c3c; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .severity-critical { background: #e74c3c; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .severity-high { background: #e67e22; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .severity-medium { background: #f39c12; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .severity-low { background: #3498db; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .mermaid { margin: 20px 0; text-align: center; } .doc-footer { background: #f1f3f5; padding: 24px 48px; font-size: 0.85em; color: #666; border-top: 1px solid #e0e0e0; } @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; } .header-banner { print-color-adjust: exact; -webkit-print-color-adjust: exact; } } :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,)

# ZBDC\_XK01

BDC Vendor Master Upload (XK01) — Clean Core & Architecture Analysis

S/4HANA 757 · SAP HANA DB Score: 22 / 100 Level C — Non-Compliant

C16 Analysis Report · v2026.05 · Generated 04 May 2026

Package: ZMM

System: S/4HANA 757 (HANA)

Object Type: PROG (Report)

Includes: 3 (DATA, SCRN, UPLOAD)

Total Lines: ~290 (main 84 + includes ~206)

Created: 2010 (ECC era)

BDC Target: XK01 (Create Vendor)

### 📑 Table of Contents

*   [1\. Executive Summary](#executive-summary)
*   [2\. Functional Specification](#functional-specification)
*   [3\. Program Overview — Include Architecture](#program-overview)
*   [4\. Technical Architecture](#technical-architecture)
*   [5\. Source Code Metrics](#source-metrics)
*   [6\. BDC Recording Analysis — XK01 Screen Flow](#bdc-recording-analysis)
*   [7\. Business Rules — Hardcoded Field Mappings](#business-rules)
*   [8\. Database Table & Dependency Inventory](#table-inventory)
*   [9\. Clean Core Assessment](#clean-core)
*   [10\. Authorization Analysis](#authorization-analysis)
*   [11\. Fit-to-Standard Analysis](#fit-to-standard)
*   [12\. Detailed Findings (Prioritized)](#findings)
*   [13\. Released API & CDS View Recommendations](#api-recommendations)
*   [14\. Performance Analysis](#performance)
*   [15\. Remediation Roadmap](#remediation)
*   [16\. Regression Testing Strategy](#regression-testing)

## 1 Executive Summary

22

Clean Core Score (out of 100)

Level C

Non-Compliant — Legacy BDC

0

AUTHORITY-CHECK Statements

8

BDC Screens Recorded

65+

Vendor Fields Mapped

XK01

Deprecated Transaction (→ BP)

**ZBDC\_XK01** is a legacy BDC (Batch Data Communication) program created circa 2010 on ECC 6.0. It mass-creates vendor master records by reading data from an Excel file and replaying SAP transaction **XK01** screen-by-screen through BDC session recording. The program maps 65+ vendor fields across 8 SAP screens including general data, company code data, purchasing organization data, and India-specific tax fields (CST, LST, ECC No, PAN, Excise details).

The program is structured across 4 source files: a main report and 3 includes handling data declarations, selection screen, and the core BDC/upload logic. The codebase totals approximately 290 lines.

**Critical Issues:**

*   🔴 **XK01 is deprecated in S/4HANA** — replaced by transaction BP (Business Partner). The BDC recording targets SAPMF02K dynpro screens that may not exist or may have changed in S/4HANA's Business Partner integration. This program is a **ticking time bomb** on every S/4HANA upgrade.
*   🔴 **Zero authorization checks** — anyone with SE38 access can mass-create vendor master records, which are financial master data controlling payment flows. This is a critical security and audit vulnerability.
*   🟠 **RLGRAP-FILENAME** — obsolete structure used for the file path selection screen parameter.
*   🟠 **India-specific tax fields** (CST No, LST No) are obsolete under the GST regime introduced in 2017. These fields may no longer be relevant.
*   🟡 **ALSM\_EXCEL\_TO\_INTERNAL\_TABLE** — Level B function module, not available in ABAP Cloud.
*   🟡 **No error handling** — no TRY/CATCH, no BAPI return message processing, no validation of input data before BDC execution.

## 2 Functional Specification

### Business Purpose

This program automates the mass creation of vendor master records in SAP. Instead of manually entering vendors one-by-one through transaction XK01 (Create Vendor — Accounting), users prepare an Excel spreadsheet with vendor data (up to 65 fields per vendor) and upload it via this program. The program creates a BDC session that replays the XK01 screens, effectively simulating a user entering each vendor manually.

This is a common pattern in SAP implementations for initial data migration (go-live cutover) and ongoing mass vendor creation. The India-specific tax fields (CST, LST, Excise, PAN, Service Tax Registration) indicate this was built for an Indian company code.

### Process Flow

flowchart LR A\[User prepares  
Excel file  
65+ columns\] --> B\[Selection Screen  
P\_FILE, session name  
row/col ranges\] B --> C\[ALSM\_EXCEL\_TO  
INTERNAL\_TABLE  
reads Excel\] C --> D\[CASE statement  
maps 65 columns  
to record fields\] D --> E\[FORM update  
builds BDC recording  
8 screens\] E --> F\[BDC\_INSERT  
creates session  
in SM35\] F --> G\[User runs  
SM35 session  
to execute\] style A fill:#3498db,color:#fff style E fill:#e74c3c,color:#fff style G fill:#27ae60,color:#fff

### Input Specification

Parameter

Type

Description

Default

P\_FILE

RLGRAP-FILENAME

Excel file path (.XLS or .TXT)

—

COL\_ST

Numeric

Start column in Excel

1

ROW\_ST

Numeric

Start row in Excel (skip header)

2

ROW\_MX

Numeric

Maximum rows to process

9999

P\_SESSN

Character

BDC session name for SM35

ZXK01\_Vendor

### Output

The program does not produce direct output to the screen. Instead, it creates a **BDC session** (visible in transaction SM35) that contains one BDC transaction (XK01) per vendor row in the Excel file. The user must then go to SM35 to process the session — either in foreground (step-by-step), background, or display-errors-only mode.

### Excel Column Mapping (65 Fields)

The program maps Excel columns to vendor master fields via a large CASE statement. Below are the key field groups:

Column Group

Fields

XK01 Screen

General Data

LIFNR, KTOKK, NAME1, NAME2, NAME3, NAME4, SORTL, STREET, HOUSE\_NUM, POST\_CODE, CITY, COUNTRY, REGION, LANGUAGE, TELEPHONE, FAX, EMAIL (SMTP\_ADDR)

Screen 0100 (Address)

Control Data

TAX\_CODE (STCEG), Industry key (BRSCH)

Screen 0110 (Control)

Company Code — Accounting

AKONT (Reconciliation A/c), ZUAWA (Sort key), FDGRV (Planning group), ZTERM (Payment terms), TOGRU (Tolerance group), REPRF (Check double invoice), ZWELS (Payment method), ZAHLS (Payment block)

Screen 0210 (Accounting)

Company Code — Payment

Payment terms, dunning, correspondence

Screen 0215 (Payment)

Purchasing Data

WAERS (Currency), ZTERM (Purch. payment terms), WEBRE (GR-based IV), LEBRE (Service-based IV), INCO1/INCO2 (Incoterms), KALSK (Schema group), MEPRF (Price marking)

Screen 0300 (Purchasing)

India Tax Fields

CST No, LST No, ECC No, Excise Range, Excise Division, Excise Commissionerate, PAN, SSI Status, Service Tax Reg No, Excise Indicator

Screen 0390 (Excise/Tax)

Withholding Tax

QSSKZ (WHT type), WT\_WITHCD (WHT code), WT\_SUBJCT (Subject to WHT)

Screen 0270 (Withholding Tax)

## 3 Program Overview — Include Architecture

The program follows the classic SAP BDC include pattern, splitting code into separate files for data, selection screen, and processing logic. This was standard practice in ECC-era ABAP development.

Source File

Type

Lines

Purpose

Key Contents

**ZBDC\_XK01**

PROG (Main)

84

Main program — includes + START-OF-SELECTION

INCLUDE statements, BDC\_OPEN\_GROUP, LOOP AT itab, FORM calls, BDC\_CLOSE\_GROUP

**ZBDC\_XK01\_DATA**

INCL

~100

Global data declarations

65-field record structure, itab (ALSMEX\_TABLINE), bdcdata, wa\_record

**ZBDC\_XK01\_SCRN**

INCL

~41

Selection screen

P\_FILE (RLGRAP-FILENAME), COL\_ST, ROW\_ST, ROW\_MX, P\_SESSN

**ZBDC\_XK01\_UPLOAD**

INCL

~65+

Core BDC logic

FORM upload (Excel read + CASE mapping), FORM update (8-screen BDC recording)

flowchart TD MAIN\[ZBDC\_XK01  
Main Report  
84 lines\] --> DATA\[ZBDC\_XK01\_DATA  
Global Declarations  
65-field structure\] MAIN --> SCRN\[ZBDC\_XK01\_SCRN  
Selection Screen  
File + Session params\] MAIN --> UPLOAD\[ZBDC\_XK01\_UPLOAD  
BDC Logic  
Upload + 8-screen recording\] UPLOAD --> FM1\[ALSM\_EXCEL\_TO  
INTERNAL\_TABLE\] UPLOAD --> FM2\[BDC\_OPEN\_GROUP  
BDC\_INSERT  
BDC\_CLOSE\_GROUP\] FM2 --> XK01\[Transaction XK01  
SAPMF02K  
8 Dynpro Screens\] style MAIN fill:#6B3FA0,color:#fff style XK01 fill:#e74c3c,color:#fff style FM1 fill:#e67e22,color:#fff

## 4 Technical Architecture

### Execution Flow

flowchart TD START\[START-OF-SELECTION\] --> OPEN\[BDC\_OPEN\_GROUP  
Session: P\_SESSN  
Keep: X\] OPEN --> UPLOAD\[PERFORM upload  
Read Excel file\] UPLOAD --> ALSM\[CALL FUNCTION  
ALSM\_EXCEL\_TO\_INTERNAL\_TABLE  
P\_FILE, COL\_ST, ROW\_ST, ROW\_MX\] ALSM --> VALIDATE{File extension  
\= .XLS or .TXT?} VALIDATE -->|No| ERROR\[Error message  
STOP\] VALIDATE -->|Yes| LOOP\[LOOP AT itab  
into wa\_itab\] LOOP --> CASE\[CASE wa\_itab-col  
65 column mappings  
WHEN 0001 → LIFNR  
WHEN 0002 → BUKRS  
...  
WHEN 0065 → EXCISE\_IND\] CASE --> ATEND{AT END OF row?} ATEND -->|Yes| UPDATE\[PERFORM update  
Build BDC recording\] ATEND -->|No| LOOP UPDATE --> S0100\[Screen 0100  
SAPMF02K  
LIFNR, BUKRS, EKORG, KTOKK\] S0100 --> S0110\[Screen 0110  
Address Data  
NAME1-4, STREET, CITY, COUNTRY\] S0110 --> S0120\[Screen 0120  
Control Data  
TAX\_CODE, BRSCH\] S0120 --> S0210\[Screen 0210  
Company Code Accounting  
AKONT, ZUAWA, FDGRV, ZTERM\] S0210 --> S0215\[Screen 0215  
Payment Data  
ZWELS, ZAHLS\] S0215 --> S0300\[Screen 0300  
Purchasing Data  
WAERS, INCO1, KALSK\] S0300 --> S0390\[Screen 0390  
India Tax  
CST, LST, ECC, PAN, Excise\] S0390 --> S0270\[Screen 0270  
Withholding Tax  
QSSKZ, WT\_WITHCD\] S0270 --> INSERT\[BDC\_INSERT  
Add to session\] INSERT --> LOOP INSERT --> CLOSE\[BDC\_CLOSE\_GROUP  
Session ready in SM35\] style START fill:#6B3FA0,color:#fff style ERROR fill:#e74c3c,color:#fff style XK01 fill:#e74c3c,color:#fff style CLOSE fill:#27ae60,color:#fff

### Key Technology Patterns

Pattern

Implementation

S/4HANA Status

Data Upload

ALSM\_EXCEL\_TO\_INTERNAL\_TABLE

🟡 Level B — works on-prem, not cloud-ready

Data Entry

BDC Session Recording (SM35)

🔴 Level C — screen-dependent, fragile

Target Transaction

XK01 (Create Vendor: Accounting)

🔴 Deprecated — replaced by BP

Screen Program

SAPMF02K (Vendor Master Screens)

🔴 May not match BP screen flow

File Selection

RLGRAP-FILENAME

🔴 Obsolete structure

Error Handling

None (no TRY/CATCH, no SY-SUBRC check)

🔴 No recovery mechanism

Authorization

None (zero AUTHORITY-CHECK)

🔴 Critical security gap

## 5 Source Code Metrics

Metric

Value

Source

Total Lines (Main + Includes)

~290

ADT navigation: main 84 + DATA ~100 + SCRN ~41 + UPLOAD ~65

Include Programs

3

ZBDC\_XK01\_DATA, ZBDC\_XK01\_SCRN, ZBDC\_XK01\_UPLOAD

FORM Routines

4+

bdc\_dynpro, bdc\_field, upload, update

SELECT Statements

0

No direct database access — all data comes from Excel file

Function Module Calls

4

ALSM\_EXCEL\_TO\_INTERNAL\_TABLE, BDC\_OPEN\_GROUP, BDC\_INSERT, BDC\_CLOSE\_GROUP

BDC Target Screens

8

SAPMF02K: 0100, 0110, 0120, 0210, 0215, 0300, 0390, 0270

Excel Column Mappings

65

CASE statement with 65 WHEN branches

AUTHORITY-CHECK Statements

0

Verified via source scan — zero auth checks in any file

TRY/CATCH Blocks

0

No structured exception handling

Data Validation Logic

1

File extension check only (.XLS/.TXT) — no field-level validation

## 6 BDC Recording Analysis — XK01 Screen Flow

The FORM `update` in ZBDC\_XK01\_UPLOAD constructs a BDC recording that replays 8 screens of transaction XK01 (SAPMF02K). Each screen maps to a specific vendor master data view. This is the most critical and fragile part of the program — any screen number or field name change in S/4HANA will cause the BDC to fail silently or produce incorrect data.

### BDC Screen Recording Map

Seq

Program

Dynpro

Tab/View

Fields Written

S/4HANA Risk

1

SAPMF02K

0100

Initial Screen

LIFNR, BUKRS, EKORG, KTOKK

🔴 **Critical** — KTOKK (account group) concept changed with BP integration

2

SAPMF02K

0110

Address

NAME1-4, SORTL, STREET, HOUSE\_NUM, POST\_CODE1, CITY, COUNTRY, REGION, LZONE, SPRAS

🟠 Address fields may map to BUT020/ADRC in BP

3

SAPMF02K

0120

Control

STCEG (Tax number), BRSCH (Industry)

🟠 Tax registration handled via DFKKBPTAXNUM in BP

4

SAPMF02K

0210

Company Code — Accounting

AKONT, ZUAWA, FDGRV, ZTERM, TOGRU, REPRF

🟡 Fields still exist but screen layout may differ

5

SAPMF02K

0215

Company Code — Payment

ZWELS, ZAHLS

🟡 Payment methods still relevant

6

SAPMF02K

0300

Purchasing Organization

WAERS, ZTERM, INCO1, INCO2, WEBRE, LEBRE, KALSK, MEPRF

🟡 Purchasing views largely unchanged

7

SAPMF02K

0390

India Tax/Excise

CST\_NO, LST\_NO, ECC\_NO, EXCISE\_RANGE, EXCISE\_DIVISION, EXCISE\_COMMISSIONERATE, PAN, SSI, SERV\_TAX\_REG, EXCISE\_IND

🔴 **Critical** — CST/LST abolished under GST (2017). Fields may be removed.

8

SAPMF02K

0270

Withholding Tax

QSSKZ, WT\_WITHCD, WT\_SUBJCT

🟡 WHT still relevant in India

**🔴 BDC Fragility Warning:** BDC recordings are tightly coupled to dynpro screen numbers and field names. In S/4HANA with BP integration enabled (note: SAP Note 3251276), the vendor maintenance screens (SAPMF02K) may be rerouted to the Business Partner maintenance screens (SAPMF02B/BUS1006). When this happens:

*   Screen numbers 0100, 0110, 0120 etc. may no longer exist or may have different field names
*   The BDC recording will produce **E-type messages** in SM35 ("Screen not found" or "Field not found")
*   Vendor records will NOT be created — data loss risk during migration runs

**Impact:** Every S/4HANA upgrade or BP activation is a potential break point for this program.

## 7 Business Rules — Hardcoded Field Mappings

The entire data transformation logic lives in a single CASE statement that maps Excel column numbers (0001 through 0065) to vendor master record fields. There are several concerns with this approach.

### Column-to-Field Mapping (65 WHEN Branches)

The CASE statement in FORM `upload` maps integer column positions to record fields. If anyone reorders the Excel columns, the mapping breaks silently — vendor names could end up in address fields, payment terms in tax fields, etc. There is no header-row validation.

View CASE Mapping Logic (representative sample)

```abap
CASE wa_itab-col.
  WHEN '0001'. wa_record-LIFNR     = wa_itab-value.  " Vendor Number
  WHEN '0002'. wa_record-BUKRS     = wa_itab-value.  " Company Code
  WHEN '0003'. wa_record-EKORG     = wa_itab-value.  " Purchasing Org
  WHEN '0004'. wa_record-KTOKK     = wa_itab-value.  " Account Group
  WHEN '0005'. wa_record-NAME1     = wa_itab-value.  " Name 1
  WHEN '0006'. wa_record-NAME2     = wa_itab-value.  " Name 2
  ...
  WHEN '0055'. wa_record-CST_NO    = wa_itab-value.  " CST Number
  WHEN '0056'. wa_record-LST_NO    = wa_itab-value.  " LST Number
  WHEN '0057'. wa_record-ECC_NO    = wa_itab-value.  " ECC Number
  WHEN '0060'. wa_record-PAN       = wa_itab-value.  " PAN Number
  WHEN '0065'. wa_record-EXCISE_IND = wa_itab-value. " Excise Indicator
ENDCASE.
```

### Hardcoded Issues

Issue

Details

Risk

No header validation

Column positions are hardcoded — if Excel layout changes, data maps to wrong fields

🔴 Data corruption

No data type validation

All values are read as strings from Excel — no numeric/date validation

🟠 Invalid master data

No mandatory field check

LIFNR, BUKRS, KTOKK are mandatory for XK01 — no check before BDC

🟠 BDC session errors

Obsolete tax fields

CST No (col 55), LST No (col 56) — abolished under GST regime in 2017

🟡 Dead columns

No duplicate check

No check if vendor already exists — XK01 BDC will fail with "Vendor already exists"

🟡 Session errors

## 8 Database Table & Dependency Inventory

This program does **not directly access any database tables** via SELECT statements. All data flows from the Excel file through the BDC recording into transaction XK01, which then writes to the vendor master tables internally. However, the program's BDC recording **implicitly targets** the following tables through the XK01 screens:

Table

Description

BDC Screen

Access Type

CDS View (Verified)

LFA1

Vendor Master — General Data

0100, 0110, 0120

Write (via BDC)

I\_Supplier ✅

LFB1

Vendor Master — Company Code

0210, 0215

Write (via BDC)

I\_SupplierCompany ✅

LFM1

Vendor Master — Purchasing Org

0300

Write (via BDC)

I\_SupplierPurchasingOrg ✅

ADRC

Address Data

0110

Write (via BDC)

I\_AddressDefaultPostalAddress ⚠️

J\_1IMOVEND

India Excise Vendor Data

0390

Write (via BDC)

⚠️ No standard CDS — India-specific

LFBW

Withholding Tax Data

0270

Write (via BDC)

I\_SupplierWithHoldingTax ⚠️ Not verified

BUT000

Business Partner General Data (S/4HANA)

—

Target table in S/4

I\_BusinessPartner ✅

### External Function Module Dependencies

Function Module

Purpose

Clean Core Level

Replacement

ALSM\_EXCEL\_TO\_INTERNAL\_TABLE

Read Excel file into internal table

Level B

CL\_FDT\_XL\_SPREADSHEET (S/4) or BTP File API (Cloud)

BDC\_OPEN\_GROUP

Open BDC session

Level C

Not needed with API approach

BDC\_INSERT

Insert BDC transaction into session

Level C

Not needed with API approach

BDC\_CLOSE\_GROUP

Close BDC session

Level C

Not needed with API approach

## 9 Clean Core Assessment

**Overall Classification: Level C — Non-Compliant**

The program uses BDC session recording (Level C), obsolete structures (RLGRAP — Level C), and targets a deprecated transaction (XK01). The overall level is determined by the worst-case dependency: **BDC\_OPEN\_GROUP / BDC\_INSERT / BDC\_CLOSE\_GROUP** are Level C function modules that have no Released API equivalent — the entire BDC pattern must be replaced with API calls.

### Dependency Classification

Dependency

Type

Level

Evidence

Successor

BDC\_OPEN\_GROUP

FUNC

Level C

BDC session FM — no Released API

None — replace with API approach

BDC\_INSERT

FUNC

Level C

BDC session FM — screen-dependent

None — replace with API approach

BDC\_CLOSE\_GROUP

FUNC

Level C

BDC session FM — screen-dependent

None — replace with API approach

ALSM\_EXCEL\_TO\_INTERNAL\_TABLE

FUNC

Level B

Classic FM, works in S/4 on-prem

CL\_FDT\_XL\_SPREADSHEET

RLGRAP-FILENAME

TYPE

Level C

Obsolete structure

STRING or CL\_GUI\_FRONTEND\_SERVICES=>FILE\_OPEN\_DIALOG

XK01 (SAPMF02K)

TRAN

Level C

Deprecated in S/4HANA — replaced by BP

Transaction BP + API\_BUSINESS\_PARTNER

ALSMEX\_TABLINE

TYPE

Level B

Structure for ALSM FM

Custom table type

### Score Breakdown

Category

Weight

Score

Deduction Reason

CC1: Released APIs Only

20%

0/20

Uses 3 Level C FMs (BDC\_OPEN/INSERT/CLOSE), 1 Level B FM

CC2: No Deprecated Objects

15%

2/15

RLGRAP-FILENAME obsolete, XK01 deprecated

CC3: CDS over Direct Access

15%

15/15

No direct SELECTs — data flows through Excel + BDC

CC4: Authorization Checks

15%

0/15

Zero AUTHORITY-CHECK in entire codebase

CC5: Error Handling

10%

2/10

Only file extension check — no SY-SUBRC, no TRY/CATCH

CC6: OO Design

10%

0/10

Procedural FORM/PERFORM only, no classes

CC7: Naming Conventions

5%

3/5

Reasonable Z-naming but no Hungarian prefix consistency

CC8: Documentation

5%

0/5

No comments, no documentation, no program description

Bonus: Integration Pattern

5%

0/5

BDC instead of BAPI/API — worst integration pattern

**TOTAL**

**100%**

**22/100**

## 10 Authorization Analysis

**🔴 CRITICAL — Zero Authorization Checks**

This program creates **vendor master records** — financial master data that directly controls payment flows (reconciliation accounts, payment terms, payment methods, bank details). There is not a single AUTHORITY-CHECK statement in the main program or any of its 3 includes. Anyone with SE38 or SA38 access can execute this program and mass-create vendors.

**Audit impact:** This would be flagged as a **SOX-critical finding** in any financial audit. Vendor creation without authorization control is a classic segregation-of-duties violation.

### Required Authorization Objects

Auth Object

Field

Check Value

Purpose

F\_LFA1\_BEK

ACTVT

'01' (Create)

Vendor master — account group authorization

F\_LFA1\_BEK

KTOKK

wa\_record-KTOKK

Restrict by vendor account group

F\_LFA1\_BUK

ACTVT

'01' (Create)

Vendor master — company code authorization

F\_LFA1\_BUK

BUKRS

wa\_record-BUKRS

Restrict by company code

M\_LFM1\_EKO

ACTVT

'01' (Create)

Vendor master — purchasing org authorization

M\_LFM1\_EKO

EKORG

wa\_record-EKORG

Restrict by purchasing organization

S\_TCODE

TCD

'XK01'

Transaction code authorization

### Recommended Fix — Add at START-OF-SELECTION

View Authorization Check Code

```abap
* --- Authorization Check: Transaction Code ---
AUTHORITY-CHECK OBJECT 'S_TCODE'
  ID 'TCD' FIELD 'XK01'.
IF sy-subrc <> 0.
  MESSAGE e001(00) WITH 'No authorization for XK01'.
ENDIF.

* --- Authorization Check: Vendor Master Creation ---
AUTHORITY-CHECK OBJECT 'F_LFA1_BEK'
  ID 'ACTVT' FIELD '01'
  ID 'KTOKK' FIELD p_ktokk.   " From selection screen or first row
IF sy-subrc <> 0.
  MESSAGE e002(00) WITH 'No authorization to create vendors'.
ENDIF.

* --- Authorization Check: Company Code ---
AUTHORITY-CHECK OBJECT 'F_LFA1_BUK'
  ID 'ACTVT' FIELD '01'
  ID 'BUKRS' FIELD p_bukrs.
IF sy-subrc <> 0.
  MESSAGE e003(00) WITH 'No authorization for company code' p_bukrs.
ENDIF.
```

## 11 Fit-to-Standard Analysis

This section decomposes the program into discrete business requirements and evaluates each against SAP standard functionality available in S/4HANA 757. The goal is to determine whether this custom BDC program can be retired in favor of standard tools.

### Requirement Decomposition

#

Business Requirement

Classification

Standard Alternative

Confidence

R1

Mass-create vendor general data (name, address, country)

FIT

API\_BUSINESS\_PARTNER — BusinessPartner entity

100%

R2

Set vendor account group (KTOKK)

FIT

API\_BUSINESS\_PARTNER — BusinessPartnerGrouping field

100%

R3

Create company code data (reconciliation a/c, payment terms)

FIT

API\_BUSINESS\_PARTNER — A\_SupplierCompany entity

100%

R4

Create purchasing org data (currency, incoterms, schema group)

FIT

API\_BUSINESS\_PARTNER — A\_SupplierPurchasingOrg entity

100%

R5

Set payment methods (ZWELS) and payment block (ZAHLS)

FIT

API\_BUSINESS\_PARTNER — A\_SupplierCompany fields

95%

R6

Create withholding tax data (QSSKZ, WT\_WITHCD)

FIT

API\_BUSINESS\_PARTNER — A\_SupplierWithHoldingTax

95%

R7

Upload data from Excel file

FIT

SAP Migration Cockpit (LTMC) — Excel template-based

100%

R8

Validate input data before creation

FIT

API\_BUSINESS\_PARTNER returns validation messages

100%

R9

Authorization-controlled vendor creation

FIT

Built-in S/4HANA authorization in BP + BAPI

100%

R10

India-specific CST / LST numbers

GAP

CST/LST abolished under GST (2017). If still needed: J\_1I\* fields via custom extension

N/A

R11

India Excise data (Range, Division, Commissionerate)

GAP

Excise regime replaced by GST. Fields likely obsolete. If needed: BAPI extension

N/A

R12

India PAN number

FIT WITH CONFIG

Tax Number configuration in Business Partner (DFKKBPTAXNUM)

90%

R13

India Service Tax Registration

GAP

Service Tax subsumed into GST. Likely obsolete. If needed: custom field

N/A

R14

Batch processing with session management (SM35)

FIT

API\_BUSINESS\_PARTNER supports batch calls + LTMC has built-in session management

100%

### Coverage Calculation

Standard Alternative

Requirements Covered

Coverage %

API\_BUSINESS\_PARTNER (OData V2/V4)

R1, R2, R3, R4, R5, R6, R8, R9, R14

**9/14 = 64%**

SAP Migration Cockpit (LTMC)

R1, R2, R3, R4, R5, R6, R7, R8, R9, R12, R14

**11/14 = 79%**

API\_BUSINESS\_PARTNER + LTMC + Config

R1–R9, R12, R14

**11/14 = 79%**

GAP (India Excise/CST/LST/ServiceTax)

R10, R11, R13

**3/14 = 21% gap**

### Gap Analysis

Requirement

Gap Description

Complexity

Remediation

R10 — CST/LST

Pre-GST tax identifiers. Likely no longer needed (GST replaced CST/LST in July 2017)

Low

Verify with business if still required. If yes: custom extension field on BP. If no: retire the fields.

R11 — Excise details

Central Excise regime abolished under GST. Range/Division/Commissionerate are legacy.

Low

Verify with business. Almost certainly can be retired.

R13 — Service Tax Reg

Service Tax subsumed into GST. Registration number format changed to GSTIN.

Low

Replace with GSTIN field in BP Tax Number config.

**🎯 Final Verdict: SIMPLIFY**

Standard SAP covers **79% of requirements** via API\_BUSINESS\_PARTNER + Migration Cockpit + BP Tax Number config. The remaining 21% gap consists of **India-specific pre-GST tax fields that are almost certainly obsolete** (CST, LST, Excise abolished in 2017). After business confirmation, effective coverage would be **100%**.

**Recommendation:** Retire ZBDC\_XK01 and replace with SAP Migration Cockpit (LTMC) for bulk vendor creation, or a custom ABAP program using `API_BUSINESS_PARTNER` for programmatic vendor creation.

## 12 Detailed Findings (Prioritized)

🔴 F1 — CRITICAL: BDC Recording Targets Deprecated Transaction XK01

**WHAT:** The entire program is built around BDC session recording of transaction XK01 (Create Vendor: Accounting) using screen program SAPMF02K. XK01 is deprecated in S/4HANA and replaced by transaction BP (Business Partner).

**WHERE:** ZBDC\_XK01\_UPLOAD — FORM update (all 8 BDC\_DYNPRO calls reference SAPMF02K screens 0100–0390)

**CODE:**

View Code

```abap
PERFORM bdc_dynpro USING 'SAPMF02K' '0100'.
PERFORM bdc_field  USING 'BDC_OKCODE' '/00'.
PERFORM bdc_field  USING 'RF02K-LIFNR' wa_record-lifnr.
PERFORM bdc_field  USING 'RF02K-BUKRS' wa_record-bukrs.
PERFORM bdc_field  USING 'RF02K-EKORG' wa_record-ekorg.
PERFORM bdc_field  USING 'RF02K-KTOKK' wa_record-ktokk.
...
PERFORM bdc_dynpro USING 'SAPMF02K' '0110'.
PERFORM bdc_dynpro USING 'SAPMF02K' '0120'.
PERFORM bdc_dynpro USING 'SAPMF02K' '0210'.
PERFORM bdc_dynpro USING 'SAPMF02K' '0215'.
PERFORM bdc_dynpro USING 'SAPMF02K' '0300'.
PERFORM bdc_dynpro USING 'SAPMF02K' '0390'.
PERFORM bdc_dynpro USING 'SAPMF02K' '0270'.
```

**WHY:** S/4HANA integrates Vendor Master with Business Partner. When BP integration is activated (SAP Note 3251276), vendor maintenance screens are rerouted. Screen numbers and field names change. BDC recordings fail silently or with cryptic "dynpro not found" errors. This is a **data migration risk** — vendors will NOT be created if the BDC recording doesn't match the current screen flow.

**FIX:** Replace the entire BDC approach with `API_BUSINESS_PARTNER` (OData V2/V4). The API is screen-independent, returns structured error messages, and is the officially supported S/4HANA method for vendor creation.

View Replacement Approach (API\_BUSINESS\_PARTNER)

```abap
* Instead of BDC, use the Released API:
* POST /sap/opu/odata/sap/API_BUSINESS_PARTNER/A_BusinessPartner
* with Supplier navigation property
*
* Or in ABAP:
DATA(lo_bp_api) = cl_bupa_vendor_factory=>get_instance( ).
lo_bp_api->create(
  EXPORTING
    is_vendor_general = VALUE #( name1 = wa_record-name1
                                  country = wa_record-country
                                  ... )
    is_vendor_company = VALUE #( bukrs = wa_record-bukrs
                                  akont = wa_record-akont
                                  zterm = wa_record-zterm
                                  ... )
    is_vendor_purchase = VALUE #( ekorg = wa_record-ekorg
                                   waers = wa_record-waers
                                   ... )
  IMPORTING
    ev_lifnr = lv_vendor_number
    et_return = lt_return ).
* Check lt_return for errors — structured messages, not BDC session guessing
```

**IMPACT:** Eliminates screen dependency entirely. API works regardless of BP integration status. Provides structured error messages instead of BDC session errors. Future-proof for S/4HANA Cloud migration.

🔴 F2 — CRITICAL: Zero Authorization Checks — Vendor Creation Unprotected

**WHAT:** There are zero AUTHORITY-CHECK statements in the main program or any of its 3 includes. The program mass-creates vendor master records — financial master data that controls payment flows.

**WHERE:** All 4 source files — ZBDC\_XK01, ZBDC\_XK01\_DATA, ZBDC\_XK01\_SCRN, ZBDC\_XK01\_UPLOAD

**WHY:** Vendor master data is classified as financial master data. Unauthorized vendor creation is a **SOX-critical finding** in any financial audit. Without auth checks, any user with SE38 access can mass-create vendors, potentially setting up phantom vendors for fraudulent payments.

**FIX:** See Section 10 (Authorization Analysis) for complete AUTHORITY-CHECK code with specific auth objects F\_LFA1\_BEK, F\_LFA1\_BUK, M\_LFM1\_EKO, and S\_TCODE.

**IMPACT:** Closes a critical audit gap. Enables proper segregation of duties for vendor master maintenance.

🟠 F3 — HIGH: RLGRAP-FILENAME — Obsolete Structure

**WHAT:** The selection screen parameter P\_FILE is typed as `RLGRAP-FILENAME`, which is an obsolete structure from the SAP GUI era.

**WHERE:** ZBDC\_XK01\_SCRN

**CODE:**

View Code

```abap
PARAMETERS: P_FILE LIKE RLGRAP-FILENAME.
```

**WHY:** RLGRAP is Level C — removed in ABAP Cloud, deprecated in S/4HANA. Using LIKE instead of TYPE is also obsolete ABAP syntax.

**FIX:**

View Fix

```abap
* Replace:
*   PARAMETERS: P_FILE LIKE RLGRAP-FILENAME.
* With:
DATA gv_file TYPE string.
PARAMETERS: p_file TYPE localfile.

AT SELECTION-SCREEN ON VALUE-REQUEST FOR p_file.
  CALL METHOD cl_gui_frontend_services=>file_open_dialog
    EXPORTING
      file_filter = 'Excel files (*.xlsx;*.xls)|*.xlsx;*.xls|All files (*.*)|*.*'
    CHANGING
      file_table  = lt_files
      rc          = lv_rc.
  IF lv_rc > 0.
    READ TABLE lt_files INDEX 1 INTO p_file.
  ENDIF.
```

**IMPACT:** Moves from Level C to Level B. Provides modern file dialog with type filter.

🟠 F4 — HIGH: India Tax Fields — CST/LST Obsolete Under GST

**WHAT:** The program maps 10 India-specific tax fields including CST Number, LST Number, ECC Number, and Excise details. These tax regimes were abolished when India implemented GST in July 2017.

**WHERE:** ZBDC\_XK01\_DATA (record structure, columns 55-65) and ZBDC\_XK01\_UPLOAD (BDC screen 0390)

**WHY:** CST (Central Sales Tax) and LST (Local Sales Tax) were replaced by CGST/SGST/IGST under GST. Central Excise was replaced by GST. These fields have been irrelevant for 9 years. Maintaining them in the BDC recording adds complexity and may cause errors if the SAP screens for these fields have been deactivated.

**FIX:** Consult with the business team: (1) If GST migration is complete, remove these fields and the BDC screen 0390. (2) If legacy data entry is still needed for historical reasons, document the justification. (3) For GST: add GSTIN field instead, mapped to BP Tax Number configuration.

**IMPACT:** Simplifies the Excel template from 65 to ~55 columns. Removes one BDC screen. Reduces maintenance overhead.

🟡 F5 — MEDIUM: No Error Handling — Silent Failures

**WHAT:** The program has no TRY/CATCH blocks, no SY-SUBRC checks after function module calls (except file extension check), and no validation of mandatory fields before BDC execution.

**WHERE:** All includes — especially ZBDC\_XK01\_UPLOAD after ALSM\_EXCEL\_TO\_INTERNAL\_TABLE call

**WHY:** If ALSM\_EXCEL\_TO\_INTERNAL\_TABLE fails (wrong file format, locked file, empty rows), the program continues silently. If mandatory fields (LIFNR, BUKRS, KTOKK) are empty, the BDC recording will fail in SM35 with cryptic errors. The user won't know which vendors failed or why until they process the SM35 session.

**FIX:** Add SY-SUBRC checks after every FM call. Add mandatory field validation before PERFORM update. Add a log table to track success/failure per vendor row.

**IMPACT:** Transforms from "fire and forget" to a traceable, auditable process with clear error feedback.

🟡 F6 — MEDIUM: ALSM\_EXCEL\_TO\_INTERNAL\_TABLE — Not Cloud-Ready

**WHAT:** The program uses ALSM\_EXCEL\_TO\_INTERNAL\_TABLE to read Excel files. This is a Level B function module — works on S/4HANA on-premise but is not available in ABAP Cloud / BTP.

**WHERE:** ZBDC\_XK01\_UPLOAD — FORM upload

**WHY:** If the organization plans to move to ABAP Cloud (BTP), this FM will not be available. Additionally, it only supports .XLS format (not .XLSX).

**FIX:** For S/4HANA on-premise: replace with `CL_FDT_XL_SPREADSHEET` (supports .XLSX). For Cloud: use BTP File API or SAP Build Process Automation.

**IMPACT:** Moves from Level B to Level A for cloud readiness. Supports modern .XLSX format.

🟡 F7 — MEDIUM: Procedural Design — No OO Patterns

**WHAT:** The entire program uses procedural FORM/PERFORM with global variables. No classes, no interfaces, no encapsulation.

**WHERE:** All 4 source files

**WHY:** Procedural code is harder to test, maintain, and extend. Global variables create implicit dependencies between FORMs. The 65-field CASE statement is a maintenance nightmare.

**FIX:** Refactor into a local class: LCL\_VENDOR\_UPLOAD with methods for file reading, data validation, vendor creation (via API), and logging.

**IMPACT:** Better testability, maintainability, and extensibility. Enables unit testing.

## 13 Released API & CDS View Recommendations

The following CDS views and APIs were verified in the connected S/4HANA system. They represent the modern, Clean Core alternatives to the BDC approach used in ZBDC\_XK01.

### Verified CDS Views (for read access / validation)

CDS View

Replaces

Verified in System

Usage

I\_Supplier

LFA1

✅ Confirmed

Validate vendor existence before creation, read general data

I\_SupplierCompany

LFB1

✅ Confirmed

Read/validate company code data

I\_SupplierPurchasingOrg

LFM1

✅ Confirmed

Read/validate purchasing organization data

I\_BusinessPartner

BUT000

✅ Confirmed

Read Business Partner data (S/4HANA BP integration)

### Released APIs (for write access — vendor creation)

API

Type

Replaces

Key Operations

API\_BUSINESS\_PARTNER

OData V2/V4

XK01 + BDC

POST A\_BusinessPartner (create BP + Supplier role + Company Code + Purchasing Org in one call)

BAPI\_BUPA\_CREATE\_FROM\_DATA

BAPI (RFC)

XK01 + BDC

Create Business Partner with vendor role — ABAP-native alternative

SAP Migration Cockpit (LTMC)

Standard Tool

ZBDC\_XK01 entirely

Excel-based mass vendor creation with built-in validation, error log, session management

**💡 Key Insight:** SAP Migration Cockpit (LTMC, transaction LTMC) is the **most direct replacement** for this BDC program. It provides Excel template upload → validation → mass creation → error log — exactly what ZBDC\_XK01 does, but with full S/4HANA support, built-in authorization, and no screen dependencies.

## 14 Performance Analysis

Performance is not a major concern for this program because:

*   **No database SELECTs** — all data comes from the Excel file
*   **BDC session recording** — processes asynchronously via SM35, not online
*   **Excel file size limit** — ALSM\_EXCEL\_TO\_INTERNAL\_TABLE has a 9999-row limit (ROW\_MX parameter)

However, there is one performance-adjacent concern:

Issue

Details

Impact

65-branch CASE statement

Every cell in the Excel file triggers a CASE evaluation with up to 65 WHEN branches. For 500 vendors × 65 columns = 32,500 CASE evaluations.

🟡 Low — CASE is efficient in ABAP, but a hash table or FIELD-SYMBOL assignment would be cleaner

No batch splitting

All vendors go into one BDC session. Processing 1000+ vendors in a single SM35 session can be slow and hard to restart on error.

🟡 Low — add batch splitting (e.g., 100 vendors per session)

## 15 Remediation Roadmap

gantt title ZBDC\_XK01 Remediation Roadmap dateFormat YYYY-MM-DD axisFormat %b %d section Option A: Quick Win Add AUTHORITY-CHECK statements :a1, 2026-05-05, 2d Add SY-SUBRC error handling :a2, after a1, 2d Add mandatory field validation :a3, after a2, 1d Test and deploy :a4, after a3, 1d section Option B: API Rewrite Design API-based architecture :b1, 2026-05-12, 3d Build upload class with API\_BUSINESS\_PARTNER :b2, after b1, 8d Add validation and error log :b3, after b2, 3d India tax field review with business :b4, 2026-05-12, 5d Integration testing :b5, after b3, 5d User acceptance testing :b6, after b5, 3d section Option C: Retire Evaluate SAP Migration Cockpit :c1, 2026-05-12, 3d Build LTMC template with 65 fields :c2, after c1, 5d Test LTMC with sample data :c3, after c2, 3d Decommission ZBDC\_XK01 :c4, after c3, 2d

### Option Comparison

Option

Effort

Risk

Clean Core Impact

Recommendation

**A: Quick Win**

~1 week

Low

Stays Level C but adds security + error handling

✅ Do immediately — stops the audit bleeding

**B: API Rewrite**

~4 weeks

Medium

Moves to Level A (API\_BUSINESS\_PARTNER is Released)

Best long-term option if custom upload is needed

**C: Retire (LTMC)**

~2 weeks

Low

Moves to Level A (standard SAP tool)

🏆 **Recommended** — standard tool, zero code to maintain

**🏆 Primary Recommendation: Option A (now) + Option C (next quarter)**

**Immediate (this week):** Apply Option A — add authorization checks and error handling to ZBDC\_XK01. This closes the SOX audit gap and prevents silent failures. Effort: ~1 week, low risk.

**Next quarter:** Retire ZBDC\_XK01 entirely by switching to **SAP Migration Cockpit (LTMC)**. LTMC provides the same Excel-upload-to-vendor-creation workflow with built-in validation, authorization, error logging, and full S/4HANA support. Zero custom code to maintain. The India-specific fields (CST, LST, Excise) should be reviewed with the business — they are almost certainly obsolete under GST.

**Only choose Option B** if the business has specific requirements that LTMC cannot meet (e.g., custom validation rules, integration with external systems, automated scheduling).

## 16 Regression Testing Strategy

If Option A (Quick Win) is implemented, the following test scenarios should be executed to verify no regression was introduced:

Test #

Scenario

Expected Result

Priority

T1

Upload Excel with 5 valid vendors (all fields populated)

BDC session created in SM35 with 5 transactions. All execute successfully.

🔴 Critical

T2

Upload Excel with missing mandatory fields (LIFNR blank)

Error message displayed — row skipped or session flagged

🔴 Critical

T3

Execute without F\_LFA1\_BEK authorization

Program terminates with "No authorization" message

🔴 Critical

T4

Upload .XLSX file (not .XLS)

Appropriate error message or successful processing

🟠 High

T5

Upload file with 1000+ rows

Session created — batch splitting if implemented

🟡 Medium

T6

Upload with duplicate vendor numbers

Error flagged per duplicate row

🟡 Medium

T7

Execute on S/4HANA with BP integration active

BDC screens match BP flow (or clear error if they don't)

🔴 Critical

T8

Upload with India tax fields populated

Screen 0390 processes correctly (if still exists)

🟡 Medium

**C16 Analysis Report** — Generated 04 May 2026

**Verification Methodology:** Main program source retrieved via ADT program read API. Three include programs (ZBDC\_XK01\_DATA, ZBDC\_XK01\_SCRN, ZBDC\_XK01\_UPLOAD) retrieved via ADT navigation API (goto-definition). Authorization gap verified via source pattern scan (0 occurrences of AUTHORITY-CHECK across all 4 files). CDS views I\_Supplier, I\_SupplierCompany, I\_SupplierPurchasingOrg, I\_BusinessPartner verified via TADIR object search (type=DDLS). System version confirmed as S/4HANA 757 on SAP HANA via RFC\_SYSTEM\_INFO. Standard API alternatives discovered via SAP Business Accelerator Hub search.

**Tools used:** sap\_check\_system\_version, sap\_search\_objects, sap\_get\_source, sap\_goto\_definition, sap\_scan\_sources, sap\_api\_hub\_search, component\_impact\_check, analyze\_clean\_core

**Disclaimer:** This report is based on source code analysis and system metadata. Runtime behavior, actual screen flow on this specific S/4HANA instance, and business process context should be validated by the functional team before implementing remediation.

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._