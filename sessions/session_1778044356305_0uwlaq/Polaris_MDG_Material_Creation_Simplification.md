  Material Creation Process Simplification — Polaris MDG :root { --c16-primary: #6B3FA0; --c16-secondary: #8B5CF6; --c16-accent: #B5567A; --c16-copper: #C97B3A; --c16-dark: #1e1e2e; --c16-text: #2c3e50; --c16-bg: #f4f5f7; --success: #27ae60; --warning: #f39c12; --danger: #e74c3c; --info: #3498db; } \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, Arial, sans-serif; color: var(--c16-text); background: var(--c16-bg); line-height: 1.7; font-size: 15px; } .doc-wrapper { max-width: 1200px; margin: 0 auto; background: #fff; box-shadow: 0 4px 30px rgba(107,63,160,0.10); border-radius: 0 0 16px 16px; } /\* ─── HEADER BANNER ─── \*/ .header-banner { background: linear-gradient(135deg, var(--c16-copper), var(--c16-accent), var(--c16-primary)); color: #fff; padding: 48px 56px 40px; position: relative; overflow: hidden; } .header-banner::after { content: ''; position: absolute; top: -60px; right: -60px; width: 260px; height: 260px; background: rgba(255,255,255,0.04); border-radius: 50%; } .header-banner img.logo { height: 52px; margin-bottom: 14px; border-radius: 8px; } .header-banner h1 { font-size: 2.2em; font-weight: 700; margin-bottom: 6px; letter-spacing: -0.5px; } .header-banner .subtitle { font-size: 1.15em; opacity: 0.92; font-weight: 400; margin-bottom: 18px; } .header-meta { display: flex; gap: 20px; flex-wrap: wrap; margin-top: 12px; } .header-meta .tag { background: rgba(255,255,255,0.18); padding: 5px 16px; border-radius: 20px; font-size: 0.88em; backdrop-filter: blur(4px); border: 1px solid rgba(255,255,255,0.2); } /\* ─── DOCUMENT INFO BAR ─── \*/ .doc-info-bar { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 0; border-bottom: 2px solid #eee; background: #fafbfc; } .doc-info-bar .info-item { padding: 14px 24px; border-right: 1px solid #eee; font-size: 0.88em; } .doc-info-bar .info-item:last-child { border-right: none; } .doc-info-bar .info-label { color: #888; font-size: 0.78em; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 2px; } .doc-info-bar .info-value { font-weight: 600; color: var(--c16-primary); } /\* ─── CONTENT AREA ─── \*/ .content { padding: 40px 56px 56px; } /\* ─── TABLE OF CONTENTS ─── \*/ .toc { background: linear-gradient(135deg, #f8f6fc, #f0ecf8); border: 1px solid #e0d6f0; border-radius: 12px; padding: 28px 36px; margin-bottom: 40px; } .toc h2 { font-size: 1.15em; color: var(--c16-primary); margin-bottom: 14px; padding-bottom: 8px; border-bottom: 2px solid var(--c16-accent); } .toc-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6px 40px; } .toc a { color: var(--c16-text); text-decoration: none; font-size: 0.92em; padding: 4px 0; display: block; transition: color 0.2s; } .toc a:hover { color: var(--c16-primary); } .toc a .toc-num { display: inline-block; width: 28px; height: 28px; line-height: 28px; text-align: center; border-radius: 50%; background: var(--c16-primary); color: #fff; font-size: 0.75em; font-weight: 700; margin-right: 8px; vertical-align: middle; } /\* ─── SECTION TITLES ─── \*/ .section-title { font-size: 1.55em; color: var(--c16-primary); margin: 48px 0 20px; padding-bottom: 10px; border-bottom: 3px solid var(--c16-accent); display: flex; align-items: center; gap: 14px; } .section-title .num { display: inline-flex; align-items: center; justify-content: center; width: 38px; height: 38px; border-radius: 50%; background: var(--c16-accent); color: #fff; font-size: 0.65em; font-weight: 700; flex-shrink: 0; } h3 { font-size: 1.18em; color: var(--c16-primary); margin: 28px 0 12px; padding-left: 14px; border-left: 4px solid var(--c16-accent); } h4 { font-size: 1.02em; color: #555; margin: 20px 0 10px; font-weight: 600; } p { margin-bottom: 14px; } /\* ─── EXECUTIVE SUMMARY BOX ─── \*/ .exec-summary { background: linear-gradient(135deg, #f0faf4, #e8f5ee); border-left: 5px solid var(--success); border-radius: 0 12px 12px 0; padding: 24px 30px; margin: 20px 0 28px; } /\* ─── KPI CARDS ─── \*/ .kpi-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 18px; margin: 24px 0 32px; } .kpi-card { background: #fff; border-radius: 12px; padding: 22px 24px; border: 1px solid #e8e8f0; box-shadow: 0 2px 8px rgba(107,63,160,0.06); text-align: center; transition: transform 0.2s; } .kpi-card:hover { transform: translateY(-2px); } .kpi-card .kpi-value { font-size: 2.1em; font-weight: 800; margin-bottom: 4px; } .kpi-card .kpi-label { font-size: 0.82em; color: #777; text-transform: uppercase; letter-spacing: 0.5px; } .kpi-card .kpi-sub { font-size: 0.78em; color: #999; margin-top: 4px; } .kpi-green .kpi-value { color: var(--success); } .kpi-purple .kpi-value { color: var(--c16-primary); } .kpi-orange .kpi-value { color: var(--warning); } .kpi-red .kpi-value { color: var(--danger); } .kpi-blue .kpi-value { color: var(--info); } /\* ─── TABLES ─── \*/ table { width: 100%; border-collapse: collapse; margin: 16px 0 24px; font-size: 0.9em; } thead th { background: var(--c16-primary); color: #fff; padding: 12px 16px; text-align: left; font-weight: 600; font-size: 0.88em; text-transform: uppercase; letter-spacing: 0.3px; } thead th:first-child { border-radius: 8px 0 0 0; } thead th:last-child { border-radius: 0 8px 0 0; } tbody td { padding: 11px 16px; border-bottom: 1px solid #eef0f4; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } /\* ─── BADGES ─── \*/ .badge { display: inline-block; padding: 3px 12px; border-radius: 12px; font-size: 0.8em; font-weight: 600; color: #fff; } .badge-critical { background: var(--danger); } .badge-high { background: #e67e22; } .badge-medium { background: var(--warning); } .badge-low { background: var(--success); } .badge-info { background: var(--info); } .badge-purple { background: var(--c16-primary); } .coverage-fit { background: #27ae60; color: #fff; padding: 3px 12px; border-radius: 12px; font-size: 0.85em; font-weight: 600; } .coverage-config { background: #3498db; color: #fff; padding: 3px 12px; border-radius: 12px; font-size: 0.85em; font-weight: 600; } .coverage-extension { background: #e67e22; color: #fff; padding: 3px 12px; border-radius: 12px; font-size: 0.85em; font-weight: 600; } .coverage-gap { background: #e74c3c; color: #fff; padding: 3px 12px; border-radius: 12px; font-size: 0.85em; font-weight: 600; } /\* ─── CALLOUT BOXES ─── \*/ .callout { border-radius: 10px; padding: 20px 24px; margin: 20px 0; font-size: 0.94em; } .callout-info { background: #eef6ff; border-left: 5px solid var(--info); } .callout-warning { background: #fff8ed; border-left: 5px solid var(--warning); } .callout-success { background: #f0faf4; border-left: 5px solid var(--success); } .callout-danger { background: #fdf0f0; border-left: 5px solid var(--danger); } .callout-purple { background: #f5f0ff; border-left: 5px solid var(--c16-primary); } .callout strong { display: block; margin-bottom: 6px; font-size: 1.02em; } /\* ─── PHASE BOXES ─── \*/ .phase-header { background: linear-gradient(135deg, var(--c16-primary), var(--c16-secondary)); color: #fff; padding: 18px 28px; border-radius: 12px 12px 0 0; margin-top: 40px; display: flex; justify-content: space-between; align-items: center; } .phase-header h3 { color: #fff; border: none; padding: 0; margin: 0; font-size: 1.2em; } .phase-header .phase-timing { background: rgba(255,255,255,0.2); padding: 4px 16px; border-radius: 20px; font-size: 0.85em; } .phase-body { border: 2px solid #e0d6f0; border-top: none; border-radius: 0 0 12px 12px; padding: 24px 28px; margin-bottom: 8px; background: #fcfbfe; } /\* ─── PROCESS FLOW DIAGRAMS ─── \*/ .diagram-container { background: #fafbfe; border: 1px solid #e8e8f0; border-radius: 12px; padding: 24px; margin: 20px 0; overflow-x: auto; } .diagram-container .mermaid { display: flex; justify-content: center; } /\* ─── OPTION CARDS ─── \*/ .option-grid { display: grid; grid-template-columns: 1fr; gap: 20px; margin: 20px 0; } .option-card { border: 2px solid #e0d6f0; border-radius: 12px; overflow: hidden; } .option-card.recommended { border-color: var(--success); box-shadow: 0 0 0 3px rgba(39,174,96,0.12); } .option-header { padding: 16px 24px; display: flex; justify-content: space-between; align-items: center; } .option-header.opt-a { background: linear-gradient(135deg, #e8f5ee, #d4edda); } .option-header.opt-b { background: linear-gradient(135deg, #eef6ff, #d6eaff); } .option-header.opt-c { background: linear-gradient(135deg, #fff8ed, #ffecd2); } .option-header.opt-rec { background: linear-gradient(135deg, #e8f5ee, #c3e6cb); } .option-body { padding: 20px 24px; } /\* ─── TIMELINE ─── \*/ .timeline { position: relative; padding-left: 40px; margin: 24px 0; } .timeline::before { content: ''; position: absolute; left: 14px; top: 0; bottom: 0; width: 3px; background: linear-gradient(180deg, var(--c16-primary), var(--c16-accent)); border-radius: 3px; } .timeline-item { position: relative; margin-bottom: 24px; padding: 16px 20px; background: #fafbfe; border-radius: 10px; border: 1px solid #e8e8f0; } .timeline-item::before { content: ''; position: absolute; left: -33px; top: 20px; width: 14px; height: 14px; background: var(--c16-primary); border: 3px solid #fff; border-radius: 50%; box-shadow: 0 0 0 2px var(--c16-primary); } .timeline-item .tl-title { font-weight: 700; color: var(--c16-primary); margin-bottom: 4px; } .timeline-item .tl-timing { font-size: 0.82em; color: #888; margin-bottom: 8px; } /\* ─── RISK MATRIX ─── \*/ .risk-matrix { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin: 20px 0; } .risk-item { padding: 16px 20px; border-radius: 10px; border: 1px solid #e8e8f0; background: #fff; } .risk-item.risk-critical { border-left: 5px solid var(--danger); } .risk-item.risk-high { border-left: 5px solid #e67e22; } .risk-item.risk-medium { border-left: 5px solid var(--warning); } .risk-item.risk-low { border-left: 5px solid var(--success); } /\* ─── COMPARISON ARROWS ─── \*/ .comparison-box { display: grid; grid-template-columns: 1fr auto 1fr; gap: 20px; align-items: center; margin: 24px 0; } .compare-before, .compare-after { padding: 20px 24px; border-radius: 12px; } .compare-before { background: #fdf0f0; border: 2px solid #f5c6cb; } .compare-after { background: #e8f5ee; border: 2px solid #c3e6cb; } .compare-arrow { font-size: 2em; color: var(--c16-primary); text-align: center; } /\* ─── FOOTER ─── \*/ .doc-footer { background: var(--c16-dark); color: #aaa; padding: 32px 56px; font-size: 0.82em; border-radius: 0 0 16px 16px; } .doc-footer a { color: var(--c16-secondary); } /\* ─── PRINT ─── \*/ @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; max-width: 100%; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .phase-header { -webkit-print-color-adjust: exact; print-color-adjust: exact; } thead th { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .section-title { page-break-after: avoid; } table { page-break-inside: avoid; } } @media (max-width: 768px) { .content { padding: 20px 24px; } .header-banner { padding: 28px; } .kpi-grid { grid-template-columns: 1fr 1fr; } .toc-grid { grid-template-columns: 1fr; } .comparison-box { grid-template-columns: 1fr; } .risk-matrix { grid-template-columns: 1fr; } } :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,)

# Material Creation Process Simplification

Polaris MDG — Process Optimization & Acceleration Program

📋 Strategic Initiative 🏭 Polaris MDG Landscape 🎯 Target: < 14 Days Cycle Time 🔒 GxP Compliant 📅 May 2026

Document Type

Process Optimization Plan

System Landscape

FlowG → GMID → Polaris MDG → S/4HANA

Project Duration

17 Weeks (4 Phases)

Classification

Confidential — Internal

Version

1.0 — 06 May 2026

## 📑 Table of Contents

[1 Executive Summary](#executive-summary) [2 Current State Analysis](#current-state) [3 Process Flow Architecture](#process-flow) [4 Bottleneck & Root Cause Analysis](#bottleneck-analysis) [5 Simplification Options](#simplification-options) [6 Recommended Solution Design](#recommended-solution) [7 Target State Architecture](#target-state) [8 GxP Compliance Preservation](#gxp-compliance) [9 Technical Feasibility Assessment](#technical-feasibility) [10 Implementation Roadmap](#implementation-roadmap) [11 Risk Assessment & Mitigation](#risk-assessment) [12 Resource & Effort Plan](#resource-plan) [13 Change Management Plan](#change-management) [14 Success Metrics & KPIs](#success-metrics) [15 Quick Wins — Immediate Actions](#quick-wins) [16 Appendix — CR Type Reference](#appendix)

## 1 Executive Summary

**🎯 Strategic Objective**

Reduce the material creation cycle time in the Polaris MDG landscape from the current **~90 days (3 months)** to a target of **< 14 days (2 weeks)**, representing a **minimum 60% reduction**, while maintaining **100% GxP compliance** and preserving the business requirement for partial-data material availability.

The current sequential Change Request (CR) chain — spanning FlowG initiation through GMID basic data creation, Polaris MDG data enrichment across 6 CR types, and IDOC distribution to downstream S/4HANA systems — creates extended lead times, multiple user dependencies, workflow queue bottlenecks, and delayed material availability for procurement and sales processes. This program proposes a combined strategy of **parallel CR execution** and **intelligent automation** to collapse the critical path while preserving every regulatory control and audit trail requirement.

~90 Days

Current Cycle Time

End-to-end material creation

< 14 Days

Target Cycle Time

84% reduction goal

6

CR Types in Chain

Sequential dependencies

100%

GxP Compliance

Non-negotiable preservation

17 Weeks

Implementation Duration

4 phases, 6 workstreams

This document provides a comprehensive plan covering current state analysis, bottleneck identification, three simplification options evaluated against feasibility and compliance, a detailed implementation roadmap with sprint-level task breakdowns, risk assessment with mitigations, resource planning, and a change management strategy. The recommended approach — **Parallel CR Execution combined with Smart Automation (Options B+C)** — delivers the target cycle time reduction with medium implementation risk, minimal user disruption, and complete GxP control preservation.

## 2 Current State Analysis

### 2.1 System Landscape Overview

The material creation process traverses four interconnected systems, each serving a distinct role in the master data lifecycle. Understanding the integration architecture is critical to identifying where dependencies create bottlenecks and where parallel processing can be safely introduced.

System

Role

Integration Method

Trigger

User Interaction

**FlowG**

Workflow initiation & orchestration

API / Workflow Events

Business user request

Requester submits material request

**GMID**

Master data creation (basic data views)

IDOC to Polaris MDG

FlowG approval

GMID data steward creates basic data

**Polaris MDG**

Data enrichment & governance (multi-CR workflow)

IDOC inbound (from GMID), IDOC outbound (to S/4HANA)

GMID IDOC arrival

Multiple steward personas across CR types

**S/4HANA & Downstream**

Operational ERP (procurement, production, sales)

IDOC distribution from Polaris

CR activation in Polaris

None (automated distribution)

### 2.2 Current CR Chain — Sequential Dependencies

The heart of the bottleneck lies in the sequential nature of six Change Request types within Polaris MDG. Each CR must be created, processed by a specific user persona, approved, and activated before the next can begin. The table below maps each CR type, its dependencies, and the nature of user interaction required.

#

CR Type

Description

Trigger

Mode

Dependency

User Persona

1

**ZMAT01G**

Create Material Basic Data (GMID)

IDOC from GMID

🤖 Automated

None (initiator)

System (exception: MDG Admin on errors)

2

**ZMAT02GB**

Change Material Basic Data (GMID enrichment)

Auto-created post ZMAT01G activation

👤 Manual

ZMAT01G activated

Basic Data Steward

3a

**ZMAT02GP**

Global Plant Data Extension

Auto-created via ZMD\_T\_GMID\_CR mapping

👤 Manual

ZMAT02GB activated

Plant Data Steward

3b

**ZMAT02GPS**

Plant & Sales Data Extension

Auto-created via ZMD\_T\_GMID\_CR mapping

👤 Manual

ZMAT02GB activated

Sales Data Steward

3c

**ZMAT02GE**

Industrial View Data (S/4HANA production sites)

Auto-created via ZMD\_T\_GMID\_CR mapping

👤 Manual

ZMAT02GB activated

Industrial Data Steward

4

**ZMAT02GX**

GxP Approval (digital signature/compliance)

Auto-created post ZMAT02GE activation

👤 Manual (signature)

ZMAT02GE activated

GxP Quality Approver

**📌 Key Observation — Stages 3a/3b/3c** The plant and sales extension CRs (ZMAT02GP, ZMAT02GPS, ZMAT02GE) are inherently parallel — they address different data domains for different site types. However, all three currently depend on ZMAT02GB completion, creating an artificial serialization point. Furthermore, multiple instances of each type can be created for multi-site materials, compounding the wait time.

### 2.3 The ZMD\_T\_GMID\_CR Configuration Table

The `ZMD_T_GMID_CR` table is the brain of CR auto-creation — it maps material type, plant, sales org, and other attributes to the appropriate CR types that should be triggered. When this table has gaps or missing mappings, the system cannot auto-create the required CRs, forcing manual intervention by an MDG administrator. These manual interventions are a hidden source of delay that doesn't appear in normal workflow metrics.

Configuration Element

Purpose

Failure Impact

Material Type → CR Type Mapping

Determines which CR types are triggered for a given material type

Missing mapping → no auto-creation → manual admin intervention → 2-5 day delay

Plant → Site Category Mapping

Identifies if plant is Athena-integrated, market/pivot, or S/4HANA production

Wrong categorization → wrong CR type created → rework cycle

Sales Org Relevance Flags

Determines if ZMAT02GPS is needed for sales-relevant plants

Missing flag → ZMAT02GPS not created → sales data gap downstream

GxP Flag

Determines if ZMAT02GX is required post-ZMAT02GE

Missing flag → compliance step skipped → regulatory risk

### 2.4 Estimated Current Cycle Time Breakdown

Based on the described process architecture, the following represents the estimated distribution of the ~90 day end-to-end cycle time. The actual values must be confirmed during Phase 1 (Discovery & Measurement) through data extraction from the MDG change log tables. The proportions below are calibrated against industry benchmarks for multi-stage MDG workflows in pharmaceutical companies.

Time Component

Estimated Duration

% of Total

Category

Addressable?

ZMAT01G processing (IDOC + activation)

1–2 days

~2%

System Time

Limited (already automated)

ZMAT01G error resolution (when needed)

3–7 days

~5%

Human Wait

✅ Streamline error handling

**ZMAT02GB queue wait** (sitting in steward inbox)

**15–25 days**

**~25%**

Human Wait

✅ SLA escalation + pre-population

ZMAT02GB data entry + approval

2–5 days

~4%

Human Processing

✅ Pre-population from GMID

**ZMAT02GP/GPS/GE dependency wait** (waiting for 02GB)

**0 days (blocked until 02GB done)**

—

Dependency Wait

✅✅ Parallelization removes this

**ZMAT02GP/GPS/GE queue wait** (sitting in steward inboxes)

**20–30 days**

**~30%**

Human Wait

✅ SLA escalation + auto-approve

ZMAT02GP/GPS/GE data entry + approval

3–8 days

~6%

Human Processing

✅ Bulk processing + pre-population

ZMAT02GX dependency wait (waiting for 02GE)

0 days (blocked)

—

Dependency Wait

Cannot remove (GxP boundary)

**ZMAT02GX approval queue wait**

**10–15 days**

**~14%**

Human Wait

✅ SLA escalation + reviewer pool

ZMAT02GX digital signature processing

1–2 days

~2%

Human Processing

Limited (compliance step)

ZMD\_T\_GMID\_CR mapping gaps (manual interventions)

5–10 days (when occurs)

~8%

Exception Handling

✅✅ Table data cleansing

IDOC distribution to downstream systems

< 1 day

~1%

System Time

Not needed (fast)

**🔴 Critical Finding: ~70% of the 90-day cycle is queue wait time (Human Wait)** The materials are sitting idle in user inboxes — not being actively processed. Only ~12% of the total cycle time is actual human data entry or system processing. The remaining ~18% is dependency waiting and exception handling. This means the primary lever for acceleration is _reducing idle time_, not speeding up data entry.

## 3 Process Flow Architecture

### 3.1 Current State — Sequential CR Chain

The following diagram illustrates the full end-to-end material creation flow as it operates today. Each node represents a CR type or system action, and the edges represent dependencies — the target CR cannot be created until the source CR is fully activated. The critical path (longest duration path) runs through ZMAT01G → ZMAT02GB → ZMAT02GE → ZMAT02GX, representing the full industrial/GxP pathway.

graph LR A\[🌐 FlowG  
Workflow Initiation\] -->|API/Event| B\[📦 GMID  
Basic Data Creation\] B -->|IDOC| C\[🤖 ZMAT01G  
Create Material Basic Data  
_Automated_\] C -->|Activation Triggers| D\[👤 ZMAT02GB  
Enrich Basic Data  
_Manual — 15-25 day queue_\] D -->|Activation Triggers| E\[👤 ZMAT02GP  
Global Plant Data  
_Athena Sites_\] D -->|Activation Triggers| F\[👤 ZMAT02GPS  
Plant + Sales Data  
_Market/Pivot Sites_\] D -->|Activation Triggers| G\[👤 ZMAT02GE  
Industrial View Data  
_S/4HANA Prod Sites_\] G -->|Activation Triggers| H\[👤 ZMAT02GX  
GxP Approval  
_Digital Signature_\] E -->|IDOC| I\[📡 Downstream  
S/4HANA Distribution\] F -->|IDOC| I H -->|IDOC| I style C fill:#d4edda,stroke:#28a745,color:#000 style D fill:#fff3cd,stroke:#ffc107,color:#000 style E fill:#cce5ff,stroke:#004085,color:#000 style F fill:#cce5ff,stroke:#004085,color:#000 style G fill:#cce5ff,stroke:#004085,color:#000 style H fill:#f8d7da,stroke:#721c24,color:#000

### 3.2 Critical Path Analysis

Not all materials traverse every CR type. The table below shows the three primary pathways through the CR chain, depending on the material's site assignments. The GxP pathway is the longest and represents the worst-case scenario for industrial pharmaceutical materials requiring regulatory compliance approval.

Pathway

CR Sequence

Total Stages

Estimated Duration

Material Profile

**🔴 GxP Industrial**

ZMAT01G → ZMAT02GB → ZMAT02GE → ZMAT02GX

4 sequential

~90 days

Industrial materials at S/4HANA production sites with GxP requirement

**🟡 Non-GxP Plant/Sales**

ZMAT01G → ZMAT02GB → ZMAT02GP / ZMAT02GPS

3 sequential

~60 days

Standard materials at Athena or market/pivot sites without GxP

**🟢 Basic Only**

ZMAT01G → ZMAT02GB

2 sequential

~30 days

Materials with basic data only, no plant/sales extension needed

### 3.3 Multi-Site Complexity Multiplier

For materials assigned to multiple sites, the system creates **one CR instance per site** for each relevant extension type. A material assigned to 5 Athena plants, 3 market sites, and 2 S/4HANA production plants would generate: 5× ZMAT02GP + 3× ZMAT02GPS + 2× ZMAT02GE + 2× ZMAT02GX = **12 parallel CRs** in Stage 3+. Each requires individual user processing, creating a fan-out that dramatically increases the total human effort per material.

## 4 Bottleneck & Root Cause Analysis

### 4.1 Delay Contribution Heat Map

The following diagram quantifies the estimated contribution of each delay source to the overall 90-day cycle time. The human queue wait (idle time in user inboxes) dominates all other factors combined, confirming that the primary optimization opportunity is reducing wait-to-action time, not speeding up the actions themselves.

pie title Delay Contribution Breakdown (~90 Days) "ZMAT02GB Queue Wait (25%)" : 25 "ZMAT02GP/GPS/GE Queue Wait (30%)" : 30 "ZMAT02GX Approval Queue (14%)" : 14 "ZMD\_T\_GMID\_CR Exceptions (8%)" : 8 "Actual Data Entry (12%)" : 12 "ZMAT01G Errors (5%)" : 5 "System Processing (3%)" : 3 "IDOC Distribution (1%)" : 1 "Other (2%)" : 2

### 4.2 Root Cause — Five Core Bottlenecks

Through architectural analysis of the CR chain, five root causes emerge. Each represents an independent optimization target, and the recommended solution addresses all five simultaneously.

#

Bottleneck

Root Cause

Estimated Impact

Addressable?

B1

**Sequential dependency: ZMAT02GB → Plant CRs**

Plant/Sales CRs wait for basic data enrichment to complete, but they don't technically need the enriched fields — only the base material record created by ZMAT01G

~20-25 days added to critical path

✅ Parallelization

B2

**Queue wait: CRs idle in user inboxes**

No SLA enforcement, no escalation, no visibility. Users may have 50+ CRs pending with no prioritization mechanism. Materials compete with change requests for the same stewards' attention.

~50-60 days of total wait

✅ SLA + escalation + auto-approve

B3

**Manual data re-entry across CR types**

Data available from GMID (via IDOC) is not auto-propagated to ZMAT02GB fields. Stewards must manually look up and re-enter data that the system already has.

~30 min per material (× volume = significant FTE)

✅ Auto-propagation

B4

**ZMD\_T\_GMID\_CR mapping gaps**

New material types, plants, or sales orgs added to the landscape without updating the configuration table. Gaps require MDG admin to manually create CRs, introducing 2-5 day delays and single-point-of-failure risk.

~5-10 days per occurrence (affects ~15-20% of materials)

✅ Table data cleansing + validation

B5

**GxP approval bottleneck: limited reviewer pool**

ZMAT02GX requires qualified GxP reviewers with digital signature authority. The reviewer pool is small (often 2-3 people), creating capacity constraints during peak periods.

~10-15 days wait per GxP material

Partial — expand reviewer pool + SLA

### 4.3 Dependency Analysis — What Really Needs to Be Sequential?

A critical question for simplification: which dependencies in the CR chain are _technically mandatory_ (data integrity requires sequencing) versus _historically assumed_ (designed this way but not technically required)? The following analysis reveals that only two of the four inter-CR dependencies are truly mandatory.

Dependency

Current

Technically Mandatory?

Rationale

Simplification Opportunity

ZMAT01G → ZMAT02GB

Sequential

YES

ZMAT02GB enriches the basic material record created by ZMAT01G. Without the base record, there is nothing to enrich.

None — must remain sequential

**ZMAT02GB → ZMAT02GP/GPS/GE**

**Sequential**

NO

Plant/sales extensions need the base material record (from ZMAT01G) but do _not_ depend on the enriched basic data fields added by ZMAT02GB. Each extension operates on a separate set of views (plant data, sales data, industrial data).

**✅ Can be parallelized with ZMAT02GB**

ZMAT02GE → ZMAT02GX

Sequential

YES

GxP approval (ZMAT02GX) requires the complete industrial view data from ZMAT02GE. The digital signature certifies the full data set — it cannot precede data entry.

None — must remain sequential (GxP boundary)

ZMAT02GP/GPS → downstream

Sequential

YES

IDOC distribution must happen post-activation to ensure data integrity in downstream systems.

None — must remain sequential

**✅ Critical Finding: The ZMAT02GB → Plant/Sales CR dependency is removable** This single finding unlocks the most significant cycle time reduction. By triggering ZMAT02GP, ZMAT02GPS, and ZMAT02GE immediately after ZMAT01G activation (in parallel with ZMAT02GB), one full sequential stage is removed from the critical path. For the GxP pathway, this reduces the chain from 4 sequential stages to 3 — saving an estimated 20-25 days.

## 5 Simplification Options

Three simplification options have been evaluated against the dimensions of cycle time impact, implementation effort, GxP compliance risk, user disruption, and technical complexity. Each option addresses different bottlenecks with different risk profiles.

**Option A: Unified CR Model**

Merge all CR types into a single multi-step CR

Aggressive

**Concept:** Consolidate ZMAT01G + ZMAT02GB + ZMAT02GP/GPS/GE into a single unified CR type (`ZMAT01G_UNIFIED`) with multiple data entry steps within one CR. ZMAT02GX remains separate as a compliance gate.

Dimension

Assessment

Cycle Time Reduction

**70-80%** — eliminates all inter-CR dependency waits

Implementation Effort

High — 14-18 weeks development, new CR type design, complete FlowG/GMID interface changes

GxP Risk

Medium-High — new audit trail model, requires full GxP re-validation

User Disruption

High — all stewards must learn new process, new screens, new workflow

Rollback Capability

Difficult — structural change hard to reverse

**Pros:** Maximum simplification; single CR per material; cleanest long-term architecture.  
**Cons:** Highest risk; longest implementation; requires extensive GxP re-validation; difficult to phase the rollout.

**Option B: Parallel CR Execution**

Remove non-mandatory sequential dependencies

Moderate

**Concept:** Keep all existing CR types but remove the ZMAT02GB → plant/sales CR dependency. Trigger ZMAT02GP/GPS/GE immediately after ZMAT01G activation, running in parallel with ZMAT02GB. ZMAT02GX remains sequential after ZMAT02GE (GxP boundary preserved).

Dimension

Assessment

Cycle Time Reduction

**40-50%** — removes one full sequential stage from critical path

Implementation Effort

Medium — 6-8 weeks development, changes limited to CR trigger logic and validation

GxP Risk

Low — ZMAT02GX unchanged, audit trail structure preserved

User Disruption

Low — same screens, same CR types, CRs just arrive earlier in inbox

Rollback Capability

Easy — revert trigger timing in ZMD\_T\_GMID\_CR handler

**Pros:** Low risk; easy rollback; preserves existing CR types and user familiarity; limited code changes.  
**Cons:** Doesn't address queue wait or manual data entry — needs Option C additions to reach target.

**Option C: Smart Pre-Population & SLA Automation**

Reduce human processing time within existing process

Quick Win

**Concept:** Keep existing sequential process but dramatically reduce human time through five automation levers: reference material cloning, GMID data auto-propagation, SLA-based escalation, bulk processing UI, and auto-approval for low-risk materials.

Automation Lever

Mechanism

Impact

Reference Material Cloning

Select similar existing material → clone all field values to new CR

Data entry: 30 min → 5 min per CR

GMID Auto-Propagation

Map IDOC segments to ZMAT02GB fields → populate on CR creation

Eliminates manual re-entry of GMID data

SLA Escalation Engine

CR idle > 48 hrs → notify → idle > 72 hrs → escalate to backup

Reduces average queue wait from 20 days to 3-5 days

Bulk Processing UI

Multi-select CRs → batch data entry → batch submit

5x throughput for high-volume stewards

Auto-Approval (non-GxP)

Standard classification materials → skip approval step

~60% of materials auto-approved

Dimension

Assessment

Cycle Time Reduction

**30-40%** — reduces human time and queue wait within existing structure

Implementation Effort

Low-Medium — 4-6 weeks development, no structural changes

GxP Risk

None — auto-approval explicitly excludes GxP materials

User Disruption

Low — same process, better tooling

Rollback Capability

Trivial — disable automation features individually

**Pros:** Lowest risk; fastest to implement; each lever can be rolled out independently; immediate value.  
**Cons:** Alone, doesn't achieve the 60% reduction target — sequential dependencies still exist.

### 5.1 Option Comparison Matrix

Dimension

Option A  
Unified CR

Option B  
Parallel CRs

Option C  
Smart Automation

Option B+C  
Combined ✅

**Cycle Time Reduction**

70-80%

40-50%

30-40%

**60-75%**

**Implementation Effort**

14-18 weeks

6-8 weeks

4-6 weeks

**10-14 weeks**

**GxP Compliance Risk**

Medium-High

Low

None

**Low**

**User Disruption**

High

Low

Low

**Low**

**Rollback Ease**

Difficult

Easy

Trivial

**Easy**

**Reaches <14 Day Target?**

✅ Yes

❌ Alone: ~45 days

❌ Alone: ~55 days

**✅ Yes: ~10-14 days**

**Overall Risk Rating**

High

Medium

Low

**Medium**

## 6 Recommended Solution Design

**✅ Recommendation: Option B + Option C Combined** Implement parallel CR execution (Option B) to remove the structural bottleneck, combined with smart automation (Option C) to reduce human processing time and queue wait. Together, these achieve the **<14 day target** with medium implementation risk, low user disruption, and complete GxP compliance preservation. The combined approach allows phased rollout — Option C quick wins deliver immediate value in weeks 1-4 while Option B structural changes are developed in weeks 5-10.

### 6.1 Solution Architecture — Seven Design Pillars

#

Design Pillar

Source

Bottleneck Addressed

Expected Impact

**P1**

Parallel CR Triggering

Option B

B1 — Sequential dependency

Removes 20-25 days from critical path

**P2**

SLA Escalation Engine

Option C

B2 — Queue wait

Reduces queue wait from 20 days to 3-5 days

**P3**

GMID Data Auto-Propagation

Option C

B3 — Manual re-entry

Eliminates 30 min manual data entry per material

**P4**

Reference Material Cloning

Option C

B3 — Manual re-entry

Data entry: 30 min → 5 min per plant/sales CR

**P5**

Auto-Approval for Non-GxP Materials

Option C

B2 — Queue wait

~60% of materials skip approval step entirely

**P6**

ZMD\_T\_GMID\_CR Data Cleansing & Validation

Option C

B4 — Mapping gaps

Eliminates 5-10 day exceptions for ~15-20% of materials

**P7**

Bulk Processing UI

Option C

B2, B3 — Queue & throughput

5x throughput for stewards with high CR volumes

### 6.2 Modified CR Trigger Logic

The core technical change is modifying the event handler that fires after ZMAT01G activation. Today, this handler creates only ZMAT02GB and waits for its activation before creating plant/sales CRs. The modified logic creates all applicable CRs simultaneously.

Event

Current Behavior

Proposed Behavior

Code Change Location

ZMAT01G Activated

Creates ZMAT02GB only

Creates ZMAT02GB **\+ ZMAT02GP + ZMAT02GPS + ZMAT02GE** (where applicable per ZMD\_T\_GMID\_CR)

CR creation event handler class

ZMAT02GB Activated

Creates ZMAT02GP/GPS/GE

**No action** — plant/sales CRs already created at ZMAT01G stage

Remove/bypass post-02GB trigger

ZMAT02GE Activated

Creates ZMAT02GX

**Unchanged** — ZMAT02GX still requires complete industrial data

No change

### 6.3 Data Readiness Validation (Safeguard)

When plant/sales CRs run in parallel with ZMAT02GB, there is a scenario where the enriched basic data (from 02GB) might be needed by a plant CR. To handle this safely, a validation check is added to the plant/sales CRs that verifies base data completeness before allowing activation — not before allowing data entry. This means stewards can begin working on the CR immediately, but activation is gated on base data readiness.

Validation Rule

Applied To

Trigger

Behavior on Failure

Base material record exists and is active

All plant/sales CRs

On CR data entry start

Block — should not happen (ZMAT01G creates base)

Required basic data fields populated

ZMAT02GP/GPS/GE at activation

On Submit for Approval

Warning: "Basic data enrichment (ZMAT02GB) not yet complete. Activation will proceed once basic data is finalized."

GxP flag correctly set

ZMAT02GE at activation

On Submit

Info: ZMAT02GX will be auto-triggered after activation

## 7 Target State Architecture

### 7.1 Target State — Parallel Process Flow

The target state architecture fundamentally changes the shape of the CR chain from a long sequential pipeline to a fan-out/fan-in pattern. After ZMAT01G activates, all enrichment CRs are created simultaneously and can be processed in parallel by their respective steward personas. Only the ZMAT02GX compliance gate remains strictly sequential after ZMAT02GE.

graph LR A\[🌐 FlowG\] -->|API| B\[📦 GMID\] B -->|IDOC| C\[🤖 ZMAT01G  
Auto-Create Basic Data\] C -->|Parallel Trigger| D\[👤 ZMAT02GB  
Basic Data Enrichment\] C -->|Parallel Trigger| E\[👤 ZMAT02GP  
Global Plant Data\] C -->|Parallel Trigger| F\[👤 ZMAT02GPS  
Plant + Sales Data\] C -->|Parallel Trigger| G\[👤 ZMAT02GE  
Industrial View Data\] G -->|Sequential  
GxP Gate| H\[🔒 ZMAT02GX  
GxP Digital Signature\] D -->|IDOC| I\[📡 S/4HANA &  
Downstream\] E -->|IDOC| I F -->|IDOC| I H -->|IDOC| I style C fill:#d4edda,stroke:#28a745,color:#000 style D fill:#cce5ff,stroke:#004085,color:#000 style E fill:#cce5ff,stroke:#004085,color:#000 style F fill:#cce5ff,stroke:#004085,color:#000 style G fill:#cce5ff,stroke:#004085,color:#000 style H fill:#f8d7da,stroke:#721c24,color:#000

### 7.2 Before vs. After Comparison

#### ❌ Current State (Sequential)

**Critical Path:** 4 sequential stages

**Total Duration:** ~90 days

**Queue Waits:** 3-4 separate queue periods

**User Handoffs:** 4-5 sequential handoffs

**Dependency Waits:** ~20-25 days idle

`01G → 02GB → 02GP/GPS/GE → 02GX`

➡️

#### ✅ Target State (Parallel + Automated)

**Critical Path:** 2 sequential stages + 1 GxP gate

**Total Duration:** ~10-14 days

**Queue Waits:** 1 parallel queue period (all CRs together)

**User Handoffs:** 1 parallel handoff + 1 GxP gate

**Dependency Waits:** ~0 days (parallelized)

`01G → [02GB ∥ 02GP ∥ 02GPS ∥ 02GE] → 02GX`

### 7.3 Target Cycle Time Calculation

The following table derives the target cycle time by applying each design pillar's impact to the current state estimates. The calculation shows that the combined Option B+C approach achieves the <14 day target for the GxP pathway and well under 7 days for non-GxP pathways.

Stage

Current Duration

Pillar Applied

Target Duration

Reduction

ZMAT01G (automated)

1-2 days

P6 — Error handling improvement

1 day

~0.5 day

ZMAT02GB queue + processing

17-30 days

P2 (SLA) + P3 (auto-propagation)

3-5 days

~20 days

ZMAT02GB → Plant CR dependency

0 + 20-30 day chain

**P1 — Parallelization**

**0 days (parallel)**

**~25 days**

ZMAT02GP/GPS/GE queue + processing

23-38 days

P2 (SLA) + P4 (cloning) + P5 (auto-approve) + P7 (bulk)

2-4 days

~28 days

ZMAT02GX queue + approval

11-17 days

P2 (SLA) + expanded reviewer pool

2-3 days

~12 days

IDOC distribution

< 1 day

None needed

< 1 day

—

TOTAL (GxP pathway)

~90 days

~10-14 days

~76-80 days (84-89%)

TOTAL (Non-GxP pathway)

~60 days

~5-7 days

~53-55 days (88-92%)

## 8 GxP Compliance Preservation

GxP compliance is a non-negotiable constraint for pharmaceutical operations. The following analysis demonstrates that the recommended solution (B+C) preserves every existing regulatory control, and in several areas enhances the compliance posture through better audit trail granularity and faster anomaly detection.

### 8.1 GxP Control Mapping — Current vs. Target

GxP Control

Regulatory Basis

Current Implementation

Target Implementation

Impact

**Digital Signature**

21 CFR Part 11, EU Annex 11

ZMAT02GX CR with e-signature step

**Unchanged** — ZMAT02GX remains a separate CR with identical signature workflow

No Change

**Four-Eyes Principle**

GxP Data Integrity (ALCOA+)

Separate data steward (maker) + approver (checker) per CR

**Unchanged** — each CR retains separate maker/checker roles

No Change

**Audit Trail**

21 CFR Part 11 §11.10(e)

MDG change document per CR records field-level changes with user ID and timestamp

**Enhanced** — parallel CRs create independent, non-overlapping audit trails per data domain. Cross-CR correlation via material ID is preserved.

Enhanced ⬆️

**Data Integrity at Distribution**

GxP Data Integrity

IDOC sent post-CR activation with complete record

**Unchanged** — each CR activation triggers its own IDOC with the completed data domain

No Change

**Access Control**

21 CFR Part 11 §11.10(d)

Role-based access per CR type

**Unchanged** — same CR types, same roles, same authorization checks

No Change

**Auto-Approval Exclusion**

N/A (new control)

N/A

**New safeguard** — auto-approval (Pillar P5) explicitly excludes any material/plant combination flagged for GxP. Hard-coded exclusion rule, not configurable.

New Safeguard ⬆️

**🔒 GxP Compliance Verdict: FULL PRESERVATION** The recommended solution does not alter, weaken, or bypass any existing GxP control. ZMAT02GX (the compliance CR) is architecturally untouched — it remains a separate CR type, sequential after ZMAT02GE, with its own digital signature workflow and audit trail. The auto-approval automation (P5) includes a hard-coded exclusion for GxP-flagged materials, ensuring that no regulated material bypasses the manual approval and signature steps.

### 8.2 Validation Approach

Any change to a GxP-regulated system requires formal validation per the organization's Computer System Validation (CSV) framework. The following validation activities are included in the implementation plan:

Validation Activity

Deliverable

Phase

Owner

Impact Assessment

GxP Impact Assessment Document

Phase 2 (Design)

GxP/Quality SME

Risk Assessment

FMEA for changed process flows

Phase 2 (Design)

GxP/Quality SME + MDG Lead

Test Specification

IQ/OQ test scripts for ZMAT02GX workflow

Phase 3 (Build)

QA Tester

Test Execution

Executed test protocols with evidence

Phase 3 (Sprint 4)

QA Tester + GxP Approver

Validation Summary Report

Formal validation closure document

Phase 4 (Pre-cutover)

GxP/Quality SME

## 9 Technical Feasibility Assessment

### 9.1 IDOC Architecture Constraints

The IDOC-based integration between GMID → Polaris MDG and Polaris MDG → S/4HANA downstream systems is the backbone of the material distribution architecture. The proposed changes do not alter the IDOC structure or content — only the _timing_ of when CRs are created within Polaris MDG. IDOC distribution remains triggered by CR activation, which is architecturally unchanged.

Integration Point

IDOC Type

Change Required?

Rationale

GMID → Polaris MDG (inbound)

Material master IDOC

None

ZMAT01G processing unchanged — same IDOC, same handler

Polaris MDG → S/4HANA (outbound, per CR)

Material distribution IDOC

None

Each CR activation still triggers its own IDOC — timing changes but content does not

FlowG → GMID (initiation)

API/Event

None

Upstream flow completely unchanged

### 9.2 ZMD\_T\_GMID\_CR Table Automation Opportunities

Opportunity

Current Gap

Proposed Enhancement

Effort

Missing mapping validation

No validation on table maintenance — gaps go undetected until material creation fails

Add consistency check: for every active plant/material type combination, verify a valid CR mapping exists. Run as scheduled job + on-save validation.

3 days

Automated mapping for new plants

When new plant is added to system, ZMD\_T\_GMID\_CR must be manually updated

Event listener on plant master creation → auto-generate ZMD\_T\_GMID\_CR entries based on plant category (Athena/market/S4HANA)

5 days

Parallel trigger flag

Table doesn't distinguish parallel-eligible vs. sequential CRs

Add PARALLEL\_FLAG column: 'X' = trigger at ZMAT01G activation; ' ' = trigger at ZMAT02GB activation (backward-compatible)

2 days

Auto-approval flag

No mechanism to mark CR types eligible for auto-approval

Add AUTO\_APPROVE column: 'X' = auto-approve if material is non-GxP; ' ' = always manual approval

2 days

### 9.3 Downstream System Impact Analysis

A critical technical concern is whether downstream S/4HANA systems can handle the changed distribution pattern. In the current state, IDOCs arrive in a predictable sequence (basic data → plant data → sales data → GxP clearance). In the target state, plant/sales IDOCs may arrive before or simultaneously with the enriched basic data IDOC.

Concern

Analysis

Mitigation

Plant IDOC arrives before basic data enrichment IDOC

S/4HANA should handle this via standard IDOC sequencing — plant extension can arrive after base material exists (ZMAT01G creates the base record). Enrichment data arrives later via separate IDOC.

Verify IDOC processing order in downstream middleware. Add reprocessing job for sequence-dependent failures.

Multiple parallel IDOCs for same material

Locking conflicts possible if two IDOCs try to update the same material record simultaneously

Configure IDOC serialization group per material number in downstream middleware. Plant and sales IDOCs update different views — locking should not conflict.

Partial material data visible to users

This is the _existing design intent_ — materials are made available with partial data. The target state doesn't change this behavior.

No change needed — this is by design.

## 10 Implementation Roadmap

### 10.1 Four-Phase Gantt Chart

gantt title Polaris MDG — Material Creation Simplification dateFormat YYYY-MM-DD axisFormat %b %d section Phase 1: Discovery CR lifecycle data extraction :p1a, 2026-05-12, 5d Cycle time analysis per CR type :p1b, after p1a, 4d User capacity & queue analysis :p1c, 2026-05-12, 6d ZMD\_T\_GMID\_CR gap analysis :p1d, after p1a, 3d Baseline metrics report :milestone, p1m, after p1d, 0d section Phase 2: Solution Design Parallel CR trigger design :p2a, 2026-05-30, 5d Auto-propagation mapping :p2b, 2026-05-30, 4d SLA escalation rules design :p2c, after p2b, 3d Auto-approval rules engine :p2d, after p2a, 4d GxP impact assessment :p2e, 2026-05-30, 8d Design review & approval :milestone, p2m, after p2e, 0d section Phase 3: Build & Test Sprint 1 — Foundation :p3a, 2026-06-19, 10d Sprint 2 — Automation :p3b, after p3a, 10d Sprint 3 — Integration & UI :p3c, after p3b, 10d Sprint 4 — Testing & Validation :p3d, after p3c, 10d UAT sign-off :milestone, p3m, after p3d, 0d section Phase 4: Cutover User training :p4a, 2026-08-21, 5d Transport migration :p4b, after p4a, 3d Go-live :milestone, p4c, after p4b, 0d Hypercare :p4d, after p4c, 10d Post-go-live measurement :milestone, p4m, after p4d, 0d

### 10.2 Phase 1 — Discovery & Measurement (Weeks 1–3)

### Phase 1: Discovery & Measurement

Weeks 1–3 | 12 May – 30 May 2026

**Objective:** Establish a data-backed baseline of current cycle times per CR type, quantify the split between system processing time and human wait time, identify `ZMD_T_GMID_CR` mapping gaps, and measure user capacity constraints.

#

Task

Method

Owner

Duration

Output

1.1

Extract CR lifecycle data from MDG change log tables

Query USMD\_CREQUEST, USMD\_CREQUESTT, USMD\_CREQ\_STEP

MDG Technical Lead

5 days

Raw event log per CR instance

1.2

Map timestamps per CR stage: Created → In Process → Approved → Activated

MDG workflow status tables + FlowG audit trail

Process Analyst

4 days

Stage-level cycle times per CR type

1.3

Calculate average dwell time per CR type (median, P75, P95)

Aggregation on extracted data

Process Analyst

2 days

Bottleneck heat map

1.4

Separate system time from human time

Compare CR creation timestamp vs. first user action timestamp

MDG Technical Lead

2 days

Human vs. system time split

1.5

Analyze ZMD\_T\_GMID\_CR table completeness

Cross-reference active plant/material combos vs. table entries

MDG Config Lead

3 days

Automation gap report with specific missing entries

1.6

Map user personas and queue depths

Interview + MDG inbox analysis

Change Manager

6 days

RACI matrix + capacity analysis

1.7

Survey top 10 power users on pain points

30-minute structured interviews

Change Manager

5 days

Qualitative findings report

### 10.3 Phase 2 — Solution Design (Weeks 4–6)

### Phase 2: Solution Design

Weeks 4–6 | 1 Jun – 18 Jun 2026

**Objective:** Produce detailed technical specifications for all seven design pillars, complete GxP impact assessment, and obtain stakeholder approval for the simplified process design.

#

Task

Owner

Duration

Output

2.1

Design parallel CR trigger logic (Pillar P1)

MDG Technical Lead

5 days

Technical specification: event handler modifications, ZMD\_T\_GMID\_CR schema changes

2.2

Design GMID auto-propagation mapping (Pillar P3)

MDG Functional Consultant

4 days

Field mapping document: IDOC segment → ZMAT02GB field

2.3

Design SLA escalation rules (Pillar P2)

MDG Functional Consultant

3 days

SLA configuration: thresholds, escalation paths, notification templates

2.4

Design auto-approval rules engine (Pillar P5)

MDG Technical Lead

4 days

Decision matrix: material type × plant type × GxP flag → approve/manual

2.5

Design reference material cloning (Pillar P4)

MDG Functional Consultant

3 days

Cloning field map + UI mock-up for reference material selection

2.6

Design bulk processing UI (Pillar P7)

MDG Technical Lead

3 days

UI wireframes + batch processing specification

2.7

GxP impact assessment (Section 8)

GxP/Quality SME

8 days

GxP Impact Assessment Document + FMEA

2.8

Design review with stakeholders

All

2 days

Approved design document

### 10.4 Phase 3 — Build & Test (Weeks 7–14)

### Phase 3: Build & Test

Weeks 7–14 | 19 Jun – 14 Aug 2026

#### Sprint 1: Foundation (Weeks 7–8)

#

Task

Details

Owner

3.1

Modify ZMD\_T\_GMID\_CR schema

Add PARALLEL\_FLAG and AUTO\_APPROVE columns with migration of existing entries

ABAP Developer 1

3.2

Modify CR creation event handler

Implement parallel trigger logic: read PARALLEL\_FLAG from ZMD\_T\_GMID\_CR, create applicable CRs at ZMAT01G activation

ABAP Developer 1

3.3

Build data-readiness validation

Activation-time checks on parallel CRs: verify base material exists, warn if basic enrichment incomplete

ABAP Developer 2

3.4

ZMD\_T\_GMID\_CR data cleansing

Fill identified gaps from Phase 1; validate all active plant/material combinations have entries

MDG Config Lead

#### Sprint 2: Automation (Weeks 9–10)

#

Task

Details

Owner

3.5

Build GMID data auto-propagation

Map IDOC segments to ZMAT02GB fields; auto-populate on CR creation; mark pre-populated fields as "system-filled"

ABAP Developer 1

3.6

Build reference material cloning engine

UI: select reference material → clone field values → allow steward override; includes field-level audit trail for cloned values

ABAP Developer 2

3.7

Build SLA escalation engine

Background job: scan CR age per type → configurable thresholds → email/Teams notification → reassign to backup pool after escalation period

ABAP Developer 1

3.8

Build auto-approval rules engine

Config-table driven: read AUTO\_APPROVE flag + material GxP flag → auto-approve or route to manual. Hard-coded GxP exclusion (not configurable).

ABAP Developer 2

#### Sprint 3: Integration & UI (Weeks 11–12)

#

Task

Details

Owner

3.9

Build bulk processing UI

MDG UI enhancement: multi-select CRs in worklist → open side-by-side data entry → batch submit with validation

ABAP Developer 1 + 2

3.10

Build monitoring dashboard

Real-time view: CR count by type × status × age × steward. SLA compliance indicators. Drill-down to individual CRs.

ABAP Developer 2

3.11

Downstream IDOC validation

End-to-end test: trigger parallel CR creation → activate all → verify IDOC delivery sequence to S/4HANA test system

MDG Technical Lead + Interface Specialist

3.12

ZMD\_T\_GMID\_CR maintenance validation

On-save validation for table maintenance: check for orphan entries, missing plant mappings, GxP flag consistency

ABAP Developer 1

#### Sprint 4: Testing & Validation (Weeks 13–14)

#

Test Type

Scope

Owner

Pass Criteria

3.13

Unit Testing

Each modified class/function module individually

ABAP Developers

All unit tests pass with code coverage ≥80%

3.14

Integration Testing

FlowG → GMID → Polaris MDG → S/4HANA (end-to-end)

MDG Technical Lead

Material created E2E in <14 days simulated time

3.15

Regression Testing

All existing CR types still work for edge cases (manual workflow, error handling, single-plant materials)

QA Tester

Zero regression in existing functionality

3.16

GxP Validation Testing

ZMAT02GX digital signatures, audit trail completeness, four-eyes enforcement, GxP exclusion from auto-approve

QA + GxP SME

100% GxP controls verified per IQ/OQ protocol

3.17

Performance Testing

Simulate 50+ materials with parallel CR creation simultaneously

MDG Technical Lead

No timeout, no lock conflict, all CRs created within 60 seconds

3.18

User Acceptance Testing

10 power users execute real material creation scenarios in QA environment

Change Manager + Users

All scenarios completed; user satisfaction survey >4/5

### 10.5 Phase 4 — Cutover & Hypercare (Weeks 15–17)

### Phase 4: Cutover & Hypercare

Weeks 15–17 | 21 Aug – 12 Sep 2026

#

Task

Timing

Owner

Details

4.1

User training sessions

Week 15

Change Manager

2 sessions: (1) Basic data stewards — auto-propagation + SLA process; (2) Plant/sales stewards — parallel CRs, bulk processing, reference cloning

4.2

Quick reference guides distribution

Week 15

Change Manager

1-page per persona showing new process flow, key differences, and FAQ

4.3

Transport migration: DEV → QA → PRD

Week 16

Basis/Transport Team

Standard transport chain with go/no-go gate at each stage

4.4

Go-live execution

Week 16 (weekend)

MDG Technical Lead

Activate new CR trigger logic, enable SLA engine, deploy monitoring dashboard

4.5

Hypercare support

Weeks 16–17

Full team

Dedicated Teams channel; daily 15-min standup; monitoring dashboard reviewed hourly for first 3 days

4.6

Post-go-live measurement

Week 17

Process Analyst

Compare actual cycle times vs. Phase 1 baseline; publish results to stakeholders

## 11 Risk Assessment & Mitigation

The risk register below covers technical, operational, compliance, and organizational risks identified through the architectural analysis. Each risk is rated on likelihood and impact, with specific mitigation actions assigned to named owners.

#

Risk

Category

Likelihood

Impact

Overall

Mitigation

Owner

**R1**

Parallel CRs cause data conflicts — two users editing overlapping fields

Technical

Medium

High

High

Implement field-level locking per CR step; define clear data ownership boundaries between CR types (plant views vs. sales views are already non-overlapping)

MDG Tech Lead

**R2**

GxP audit trail gaps in parallel process

Compliance

Low

Critical

High

Dedicated GxP validation sprint (3.16); formal IQ/OQ protocols; independent audit trail per CR preserved; regulatory review gate before go-live

GxP/Quality SME

**R3**

Downstream IDOC receivers not handling parallel distribution

Technical

Medium

High

High

Early engagement with S/4HANA team (Phase 2); configure IDOC serialization groups; add reprocessing job for failed IDOCs; test with 50+ parallel materials in Phase 3

Interface Specialist

**R4**

User resistance to new process flow

Organizational

Medium

Medium

Medium

Early involvement in UAT; champion network (1 champion per steward team); fallback: manual CR creation still supported; user satisfaction survey at go-live

Change Manager

**R5**

ZMD\_T\_GMID\_CR data quality issues causing wrong CR triggers

Data Quality

High

Medium

High

Phase 1 data cleansing sprint; on-save validation rules on table maintenance; scheduled consistency check job (weekly); monitoring dashboard alerts for manual interventions

MDG Config Lead

**R6**

FlowG/GMID interface changes delayed by external vendor

Dependency

Medium

High

High

Architecture designed to not require FlowG/GMID changes. All modifications are within Polaris MDG (CR trigger logic). FlowG/GMID remain untouched. Zero vendor dependency.

MDG Tech Lead

**R7**

Auto-approval incorrectly applied to GxP material

Compliance

Low

Critical

Medium

Hard-coded GxP exclusion (not configurable); unit test specifically validating GxP materials are never auto-approved; GxP flag validation at multiple checkpoints

GxP/Quality SME

**R8**

SLA escalation overloads backup processor pool

Operational

Medium

Low

Low

Configurable escalation thresholds per CR type; backup pool size ≥3 per CR type; escalation rate monitoring in dashboard

MDG Functional

### 11.1 Risk Heat Map

quadrantChart title Risk Heat Map — Likelihood vs Impact x-axis Low Likelihood --> High Likelihood y-axis Low Impact --> High Impact quadrant-1 Monitor quadrant-2 Mitigate Actively quadrant-3 Accept quadrant-4 Watch R1 - Data Conflicts: \[0.5, 0.8\] R2 - GxP Audit Trail: \[0.2, 0.95\] R3 - IDOC Distribution: \[0.5, 0.8\] R4 - User Resistance: \[0.5, 0.5\] R5 - ZMD\_T Data Quality: \[0.8, 0.5\] R6 - Vendor Delay: \[0.5, 0.8\] R7 - GxP Auto-Approve: \[0.15, 0.95\] R8 - Escalation Overload: \[0.5, 0.25\]

## 12 Resource & Effort Plan

### 12.1 Team Composition

Role

FTE

Duration

Phase Involvement

Key Responsibilities

**MDG Technical Lead**

1.0

17 weeks (full)

P1 ▪ P2 ▪ P3 ▪ P4

Architecture, CR trigger design, event handler specs, integration testing lead, go-live execution

**MDG Functional Consultant**

0.5

14 weeks (P1–P3)

P1 ▪ P2 ▪ P3

Field mapping, SLA design, auto-approval rules, reference cloning specification, UAT support

**ABAP Developer 1**

1.0

8 weeks (P3 only)

P3

CR trigger handler, SLA engine, ZMD\_T\_GMID\_CR schema changes, bulk processing UI

**ABAP Developer 2**

1.0

8 weeks (P3 only)

P3

Data validation framework, auto-propagation, reference cloning engine, monitoring dashboard

**Process Analyst**

0.5

6 weeks (P1–P2)

P1 ▪ P2

Cycle time analysis, bottleneck quantification, baseline metrics, post-go-live measurement

**GxP/Quality SME**

0.25

6 weeks (P2 + P3 testing)

P2 ▪ P3

Impact assessment, FMEA, IQ/OQ protocol design, validation testing, compliance sign-off

**Change Manager**

0.25

17 weeks (full)

P1 ▪ P2 ▪ P3 ▪ P4

User interviews, champion network, training content, communication, UAT coordination

**FlowG/GMID Interface Specialist**

0.5

4 weeks (P3 Sprints 3–4)

P3

IDOC validation, downstream system testing, serialization configuration

**MDG Config Lead**

0.25

6 weeks (P1 + P3)

P1 ▪ P3

ZMD\_T\_GMID\_CR gap analysis and data cleansing, parallel flag population, validation rules

### 12.2 Effort Summary

Phase

Duration (Weeks)

Total Person-Days

Key Cost Drivers

Phase 1: Discovery

3

~35

Data extraction, analysis, interviews

Phase 2: Design

3

~40

Technical specs, GxP assessment, design reviews

Phase 3: Build & Test

8

~130

ABAP development (2 FTE), testing (GxP + functional), UAT

Phase 4: Cutover

3

~25

Training, transport, go-live execution, hypercare

TOTAL

17

~230

## 13 Change Management Plan

The change management strategy is designed around the principle of _minimal disruption with maximum enablement_. Because the recommended solution (B+C) preserves existing CR types and user screens, the change for end users is primarily about **timing** (CRs arrive earlier), **tooling** (new automation features), and **behavior** (SLA compliance). No fundamental retraining is needed.

### 13.1 Stakeholder Impact Matrix

Stakeholder Group

Population

Impact Level

What Changes for Them

Engagement Strategy

Basic Data Stewards

~5-10

Medium

ZMAT02GB fields pre-populated from GMID; less manual data entry; SLA notifications

Hands-on training (1 session); quick reference guide; early UAT involvement

Plant Data Stewards

~10-20

Medium

CRs arrive earlier (parallel with 02GB); bulk processing available; reference cloning; auto-approve for eligible materials

Hands-on training (1 session); bulk processing demo; champion per team

Sales Data Stewards

~5-10

Low

CRs arrive earlier; auto-approve for eligible materials

Email communication + quick reference guide

GxP Quality Approvers

~3-5

Low

ZMAT02GX process unchanged; CRs may arrive faster (reduced upstream delays); SLA notifications

Briefing meeting; confirmation that ZMAT02GX is unchanged

MDG Administrators

~2-3

High

ZMD\_T\_GMID\_CR maintenance includes new columns; monitoring dashboard; SLA configuration

Detailed technical training; admin guide; Phase 1 involvement

Business Process Owners

~3-5

Low

Dashboard visibility into cycle times; faster material availability for procurement/sales

Executive briefing; dashboard walkthrough; go-live communication

### 13.2 Communication Plan

Timing

Audience

Channel

Message

Owner

Week 1 (Kickoff)

All stakeholders

Email + Town Hall

"We're launching a project to simplify material creation — your input matters. Interviews scheduled."

Change Manager

Week 3 (Baseline)

All stakeholders

Email + Dashboard

"Here's our current state: \[baseline metrics\]. We've identified X bottlenecks. Design phase starting."

Change Manager

Week 6 (Design Approved)

All stakeholders

Email + Teams

"The simplified process is designed. Key change: parallel CRs + automation. Build phase starting."

Change Manager

Week 12 (Pre-UAT)

UAT participants

Meeting + Guide

"UAT starts next week. Here's your test scenario package. Your feedback is critical."

Change Manager

Week 14 (UAT Complete)

All stakeholders

Email

"UAT complete — \[results\]. Training sessions next week. Go-live on \[date\]."

Change Manager

Week 15 (Pre-Go-Live)

All stewards

Training + Guide

"Here's what changes for you. Quick reference guide attached. Support channel: \[link\]."

Change Manager

Week 16 (Go-Live)

All stakeholders

Email + Teams

"Go-live successful. New process is active. Support channel monitored 24/7 this week."

MDG Tech Lead

Week 17 (Results)

All stakeholders

Email + Dashboard

"First week results: cycle time reduced from X to Y. \[Specific improvements\]. Thank you!"

Process Analyst

## 14 Success Metrics & KPIs

### 14.1 Primary KPIs

KPI

Baseline (Current)

Target

Measurement Method

Measurement Frequency

**End-to-end material creation time**

~90 days

**< 14 days** (GxP) / **< 7 days** (non-GxP)

Median time from ZMAT01G creation to last CR activation per material

Weekly (dashboard)

**Queue wait time per CR**

TBD (Phase 1)

**< 48 hours**

Time from CR creation to first user action

Daily (dashboard)

**Human processing time per material**

TBD (Phase 1)

**< 2 hours total**

Sum of active user time across all CRs per material

Weekly (dashboard)

**Auto-approval rate**

0%

**\> 50%** (non-GxP materials)

Count of auto-approved CRs / total non-GxP CRs

Weekly (dashboard)

**ZMD\_T\_GMID\_CR exception rate**

~15-20% (estimated)

**< 2%**

Count of materials requiring manual CR creation / total materials

Weekly (dashboard)

**GxP compliance score**

100%

**100%**

Audit trail completeness + signature verification per GxP material

Per material (automated check)

**SLA compliance rate**

N/A (no SLA today)

**\> 90%**

% of CRs processed within SLA threshold (48 hours)

Daily (dashboard)

**User satisfaction (NPS)**

TBD (Phase 1 survey)

**+20 points improvement**

Post-go-live survey (same questions as baseline)

Go-live + 30 days, + 90 days

### 14.2 Before vs. After Scorecard

Dimension

Before

After (Target)

Improvement

End-to-end cycle (GxP)

~90 days

< 14 days

**84% reduction**

End-to-end cycle (non-GxP)

~60 days

< 7 days

**88% reduction**

Sequential CR stages

4 stages

2 parallel + 1 GxP gate

**50% fewer sequential stages**

Manual data entry per material

~2.5 hours

< 30 minutes (with pre-population + cloning)

**80% reduction in manual effort**

Exception handling (manual CRs)

~15-20% of materials

< 2%

**90% reduction in exceptions**

Queue visibility

None — no SLA, no dashboard

Real-time dashboard + SLA alerts

**Full operational visibility**

GxP compliance

100%

100%

**Maintained**

## 15 Quick Wins — Immediate Actions

These actions can be executed **in parallel with Phase 1** (Discovery) and deliver immediate, measurable improvement without any structural process change. They require no new development — only configuration, data cleansing, and enabling features already available in the MDG platform.

#

Quick Win

Effort

Impact

Timeline

Owner

No-Go Criteria

**QW1**

**SLA Visibility Dashboard** — Real-time view of CR age per steward with traffic light indicators

3 days

Medium — Peer pressure + management visibility reduces idle time by 20-30%

Week 1–2

MDG Technical Lead

None — read-only dashboard, zero process risk

**QW2**

**Idle CR Notifications** — Email/Teams alert when any CR is idle > 48 hours

2 days

Medium — Reduces "forgotten" CRs, eliminates 10-15 day silent waits

Week 1

MDG Technical Lead

None — notification only, no process change

**QW3**

**ZMD\_T\_GMID\_CR Data Cleansing** — Fill all identified mapping gaps for active plant/material combinations

5 days

High — Eliminates manual CR creation for ~15-20% of materials, saving 5-10 days per occurrence

Week 2–3

MDG Config Lead

Validate each new mapping with plant/material owner before activation

**QW4**

**Backup Processor Pool** — Assign backup stewards for each CR type to handle escalations and absences

2 days

Medium — Eliminates single-point-of-failure delays (vacation, sick leave, overload)

Week 2

Change Manager

Backup processors must have correct roles/authorization

**💡 Quick Win ROI** These four quick wins cost approximately **12 person-days** and can deliver an estimated **15-25% reduction in average cycle time** before the main project delivers its structural changes. They also provide immediate data for Phase 1 measurement (the dashboard enables real-time baseline tracking). Implementing QW1 and QW2 in Week 1 means Phase 1 cycle time data can be collected automatically from Day 1.

## 16 Appendix — CR Type Reference

### 16.1 CR Type Detailed Reference

CR Type

Full Name

Data Domain

Key MDG Views

Auto-Create Trigger

Site Applicability

GxP Relevant

**ZMAT01G**

Create Material Basic Data GMID

Basic material record: material number, description, base UOM, material type, material group

Basic Data 1, Basic Data 2

IDOC from GMID

All

No

**ZMAT02GB**

Change Material Basic Data GMID

Enriched basic data: classification, extended descriptions, cross-plant data, global trade data

Basic Data 1, Basic Data 2, Classification

ZMAT01G activation

All

No

**ZMAT02GP**

Global Plant Data

Plant-level MRP, purchasing, storage, quality management views

MRP, Purchasing, Storage, Quality

ZMD\_T\_GMID\_CR mapping (post ZMAT02GB)

Athena-integrated industrial sites

No

**ZMAT02GPS**

Plant & Sales Data

Plant views + sales org/distribution channel/division views

MRP, Purchasing, Sales Org 1, Sales Org 2

ZMD\_T\_GMID\_CR mapping (post ZMAT02GB)

Market/Pivot sites

No

**ZMAT02GE**

Industrial View Data

Production-specific views: work scheduling, production resources, costing

Work Scheduling, Costing, Production Resources

ZMD\_T\_GMID\_CR mapping (post ZMAT02GB)

S/4HANA production sites

Precursor to GxP

**ZMAT02GX**

Material GxP Approval

Compliance review of complete material data with digital signature

All views (read-only review) + digital signature

ZMAT02GE activation

Industrial sites with GxP requirement

**Yes — Primary GxP Gate**

### 16.2 Glossary

Term

Definition

**CR**

Change Request — an MDG governance object that encapsulates a set of data changes requiring approval before activation

**MDG**

Master Data Governance — SAP's framework for managing master data quality through governed workflows

**GxP**

Good Practice regulations (GMP, GDP, GLP) — pharmaceutical quality and compliance standards

**GMID**

Global Material Identifier — the master data creation system that generates the initial material record

**FlowG**

The workflow initiation system where business users submit material creation requests

**IDOC**

Intermediate Document — SAP's standard data exchange format for asynchronous system integration

**SLA**

Service Level Agreement — time-bound commitment for CR processing (proposed: 48 hours)

**Four-Eyes Principle**

Compliance requirement that data entry (maker) and approval (checker) must be performed by different individuals

**ALCOA+**

Attributable, Legible, Contemporaneous, Original, Accurate + Complete, Consistent, Enduring, Available — data integrity framework for GxP

**21 CFR Part 11**

US FDA regulation governing electronic records and electronic signatures

**EU Annex 11**

European regulatory guideline for computerized systems in GxP environments

**CSV**

Computer System Validation — formal process to verify that a computerized system meets its intended use and regulatory requirements

**IQ/OQ**

Installation Qualification / Operational Qualification — GxP validation testing protocols

**FMEA**

Failure Mode and Effects Analysis — risk assessment methodology for identifying potential failure points

**C16 — SAP Intelligence Platform**  
Material Creation Process Simplification — Polaris MDG  
Document generated: 06 May 2026 | Version 1.0

**Classification:** Confidential — Internal  
**Methodology:** Process architecture analysis based on CR chain specification, dependency mapping, and industry benchmarks for pharmaceutical MDG workflows.  
**Validation:** Estimated cycle times to be confirmed during Phase 1 Discovery via MDG system data extraction.

mermaid.initialize({ startOnLoad: true, theme: 'base', themeVariables: { primaryColor: '#6B3FA0', primaryTextColor: '#fff', primaryBorderColor: '#5a2d91', lineColor: '#8B5CF6', secondaryColor: '#f0ecf8', tertiaryColor: '#f8f6fc', pieStrokeColor: '#fff', pieStrokeWidth: '2px' }, gantt: { titleTopMargin: 25, barHeight: 24, barGap: 6, topPadding: 50, sidePadding: 50 } });

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._