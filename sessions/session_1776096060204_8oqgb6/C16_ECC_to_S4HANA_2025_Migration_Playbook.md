  C16 | ECC → S/4HANA 2025 Migration Playbook :root { --copper: #C97B3A; --rose: #B5567A; --purple: #6B3FA0; --dark-purple: #4A2D6F; --light-bg: #e9ecef; --white: #ffffff; --text: #2c3e50; --text-light: #5a6a7a; --green: #27ae60; --blue: #3498db; --orange: #e67e22; --red: #e74c3c; --teal: #1abc9c; --light-gray: #f8fafc; --border-gray: #e2e8f0; } \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, Arial, sans-serif; color: var(--text); background: var(--light-bg); line-height: 1.7; font-size: 15px; } .doc-wrapper { max-width: 1140px; margin: 30px auto; background: var(--white); border-radius: 16px; box-shadow: 0 8px 40px rgba(0,0,0,0.10); overflow: hidden; } /\* ===== HEADER BANNER ===== \*/ .header-banner { background: linear-gradient(135deg, var(--copper), var(--rose), var(--purple)); color: white; padding: 48px 56px; position: relative; overflow: hidden; } .header-banner::after { content: ''; position: absolute; top: -50%; right: -20%; width: 500px; height: 500px; background: radial-gradient(circle, rgba(255,255,255,0.08) 0%, transparent 70%); border-radius: 50%; } .header-banner img { height: 50px; margin-bottom: 14px; border-radius: 8px; display: block; } .header-banner h1 { font-size: 2.1em; font-weight: 700; margin-bottom: 8px; position: relative; z-index: 1; } .header-banner .subtitle { font-size: 1.15em; opacity: 0.92; margin-bottom: 20px; position: relative; z-index: 1; } .header-banner .version-badge { display: inline-block; background: rgba(255,255,255,0.2); padding: 5px 16px; border-radius: 20px; font-size: 0.85em; backdrop-filter: blur(4px); position: relative; z-index: 1; } .header-stats { display: flex; gap: 24px; margin-top: 24px; flex-wrap: wrap; position: relative; z-index: 1; } .header-stat { background: rgba(255,255,255,0.15); backdrop-filter: blur(6px); border-radius: 12px; padding: 14px 24px; text-align: center; min-width: 120px; } .header-stat .stat-number { font-size: 1.8em; font-weight: 700; display: block; } .header-stat .stat-label { font-size: 0.8em; opacity: 0.88; text-transform: uppercase; letter-spacing: 0.5px; } /\* ===== CONTENT AREA ===== \*/ .content { padding: 40px 56px; } /\* ===== META BAR ===== \*/ .meta-bar { display: flex; flex-wrap: wrap; gap: 12px; padding: 16px 24px; background: var(--light-gray); border-radius: 10px; margin-bottom: 32px; border: 1px solid var(--border-gray); font-size: 0.9em; } .meta-item { display: flex; align-items: center; gap: 6px; } .meta-item .label { color: var(--text-light); font-weight: 500; } .meta-item .value { color: var(--text); font-weight: 600; } /\* ===== TABLE OF CONTENTS ===== \*/ .toc { background: linear-gradient(135deg, #f8f6ff, #fdf2f8); border: 1px solid #e8dff5; border-radius: 12px; padding: 28px 32px; margin-bottom: 36px; } .toc h2 { color: var(--purple); font-size: 1.2em; margin-bottom: 16px; } .toc-columns { display: grid; grid-template-columns: 1fr 1fr; gap: 6px 32px; } .toc a { color: var(--purple); text-decoration: none; font-size: 0.92em; padding: 3px 0; display: block; transition: color 0.2s; } .toc a:hover { color: var(--rose); text-decoration: underline; } .toc a .toc-num { font-weight: 700; margin-right: 6px; color: var(--rose); } /\* ===== SECTION TITLES ===== \*/ .section-title { color: var(--purple); font-size: 1.55em; font-weight: 700; margin: 48px 0 20px 0; padding-bottom: 12px; border-bottom: 3px solid var(--rose); display: flex; align-items: center; gap: 14px; } .section-title .num { background: var(--rose); color: white; width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.65em; font-weight: 700; flex-shrink: 0; } h3 { color: var(--dark-purple); font-size: 1.15em; margin: 24px 0 12px 0; } h4 { color: var(--purple); font-size: 1.0em; margin: 18px 0 10px 0; } p { margin-bottom: 14px; } /\* ===== TABLES ===== \*/ table { width: 100%; border-collapse: collapse; margin: 16px 0 24px 0; font-size: 0.9em; } thead th { background: var(--purple); color: white; padding: 12px 14px; text-align: left; font-weight: 600; font-size: 0.92em; } tbody td { padding: 10px 14px; border-bottom: 1px solid var(--border-gray); vertical-align: top; } tbody tr:nth-child(even) { background: var(--light-gray); } tbody tr:hover { background: #edf2f7; } /\* ===== EXPANDABLE DETAILS ===== \*/ details { margin: 10px 0; border: 1px solid var(--border-gray); border-radius: 10px; overflow: hidden; } summary { background: #f0f4f8; padding: 14px 20px; cursor: pointer; font-weight: 600; color: var(--dark-purple); font-size: 0.95em; user-select: none; list-style: none; display: flex; align-items: center; gap: 10px; transition: background 0.2s; } summary:hover { background: #e4e9f0; } summary::before { content: '▶'; font-size: 0.7em; transition: transform 0.2s; flex-shrink: 0; } details\[open\] > summary::before { transform: rotate(90deg); } summary::-webkit-details-marker { display: none; } .details-content { padding: 20px 24px; } /\* Nested details \*/ details details { margin: 8px 0; border-color: #d4dce8; } details details summary { background: #f8fafc; font-size: 0.9em; padding: 10px 16px; } details details .details-content { padding: 14px 18px; } /\* ===== EXPAND ALL TOGGLE ===== \*/ .expand-controls { display: flex; gap: 10px; margin-bottom: 24px; } .expand-btn { background: var(--purple); color: white; border: none; padding: 8px 20px; border-radius: 8px; font-size: 0.85em; font-weight: 600; cursor: pointer; transition: background 0.2s; } .expand-btn:hover { background: var(--dark-purple); } .expand-btn.secondary { background: var(--white); color: var(--purple); border: 2px solid var(--purple); } .expand-btn.secondary:hover { background: #f8f6ff; } /\* ===== BADGES ===== \*/ .badge { display: inline-block; padding: 3px 12px; border-radius: 14px; font-size: 0.8em; font-weight: 600; letter-spacing: 0.3px; } .badge-a { background: var(--green); color: white; } .badge-b { background: var(--blue); color: white; } .badge-c { background: var(--orange); color: white; } .badge-d { background: var(--red); color: white; } .badge-fit { background: #27ae60; color: #fff; } .badge-config { background: #3498db; color: #fff; } .badge-extend { background: #e67e22; color: #fff; } .badge-gap { background: #e74c3c; color: #fff; } .badge-model-a { background: #2ecc71; color: #fff; } .badge-model-b { background: #3498db; color: #fff; } .badge-model-c { background: #9b59b6; color: #fff; } .badge-phase { background: var(--rose); color: white; } .badge-gate { background: #e74c3c; color: white; font-weight: 700; } .badge-play { background: var(--purple); color: white; } .badge-new { background: #1abc9c; color: white; } /\* ===== CALLOUT BOXES ===== \*/ .callout { border-radius: 10px; padding: 20px 24px; margin: 18px 0; } .callout-green { background: #f0fdf4; border-left: 5px solid var(--green); } .callout-blue { background: #eff6ff; border-left: 5px solid var(--blue); } .callout-orange { background: #fff7ed; border-left: 5px solid var(--orange); } .callout-red { background: #fef2f2; border-left: 5px solid var(--red); } .callout-purple { background: #faf5ff; border-left: 5px solid var(--purple); } .callout-teal { background: #f0fdfa; border-left: 5px solid var(--teal); } .callout h4 { margin-top: 0; } /\* ===== CARD GRID ===== \*/ .card-grid { display: grid; gap: 16px; margin: 18px 0; } .card-grid-2 { grid-template-columns: 1fr 1fr; } .card-grid-3 { grid-template-columns: 1fr 1fr 1fr; } .card-grid-4 { grid-template-columns: 1fr 1fr 1fr 1fr; } .card { background: var(--white); border: 1px solid var(--border-gray); border-radius: 12px; padding: 20px; transition: transform 0.2s, box-shadow 0.2s; } .card:hover { transform: translateY(-2px); box-shadow: 0 4px 16px rgba(0,0,0,0.08); } .card .card-icon { font-size: 1.8em; margin-bottom: 8px; display: block; } .card .card-title { font-weight: 700; color: var(--dark-purple); margin-bottom: 6px; font-size: 1.0em; } .card .card-desc { font-size: 0.85em; color: var(--text-light); line-height: 1.5; } .card .card-badge { margin-top: 10px; } /\* ===== MODEL INDICATOR ===== \*/ .model-tabs { display: flex; gap: 0; margin: 12px 0; border-radius: 8px; overflow: hidden; border: 1px solid var(--border-gray); } .model-tab { flex: 1; padding: 10px 14px; text-align: center; font-size: 0.85em; font-weight: 600; } .model-tab.ma { background: #e8f8f0; color: #1a7a4c; border-right: 1px solid var(--border-gray); } .model-tab.mb { background: #e8f0ff; color: #1a5aa0; border-right: 1px solid var(--border-gray); } .model-tab.mc { background: #f3e8ff; color: #6b3fa0; } /\* ===== CHECKLIST ===== \*/ .checklist { list-style: none; padding: 0; } .checklist li { padding: 8px 0 8px 28px; position: relative; border-bottom: 1px solid #f0f0f0; font-size: 0.92em; } .checklist li::before { content: '☐'; position: absolute; left: 0; color: var(--purple); font-size: 1.1em; } /\* ===== FLOW ARROWS ===== \*/ .flow-container { display: flex; align-items: center; justify-content: center; gap: 0; margin: 24px 0; flex-wrap: wrap; } .flow-step { background: linear-gradient(135deg, var(--purple), var(--rose)); color: white; padding: 16px 28px; border-radius: 10px; text-align: center; font-weight: 600; font-size: 0.95em; min-width: 140px; } .flow-step .flow-label { font-size: 0.75em; opacity: 0.85; display: block; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 1px; } .flow-arrow { font-size: 1.6em; color: var(--rose); padding: 0 8px; } /\* ===== KPI CARDS (horizontal) ===== \*/ .kpi-row { display: flex; gap: 16px; margin: 20px 0; flex-wrap: wrap; } .kpi-card { flex: 1; min-width: 140px; background: linear-gradient(135deg, #f8f6ff, #fdf2f8); border: 1px solid #e8dff5; border-radius: 12px; padding: 18px; text-align: center; } .kpi-card .kpi-num { font-size: 2em; font-weight: 700; color: var(--purple); display: block; } .kpi-card .kpi-label { font-size: 0.78em; color: var(--text-light); text-transform: uppercase; letter-spacing: 0.5px; margin-top: 4px; } /\* ===== GATE BOX ===== \*/ .gate-box { background: linear-gradient(135deg, #fef2f2, #fff5f5); border: 2px solid var(--red); border-radius: 12px; padding: 24px; margin: 24px 0; } .gate-box h4 { color: var(--red); margin-top: 0; font-size: 1.1em; } /\* ===== PHASE BOX ===== \*/ .phase-box { background: linear-gradient(135deg, #faf5ff, #fdf2f8); border: 2px solid var(--purple); border-radius: 12px; padding: 24px; margin: 28px 0; } .phase-box h3 { color: var(--purple); margin-top: 0; } /\* ===== CODE BLOCKS ===== \*/ pre { background: #f5f5f5; border-left: 4px solid var(--rose); padding: 16px 20px; border-radius: 6px; overflow-x: auto; font-family: 'Consolas', 'Fira Code', monospace; font-size: 0.85em; line-height: 1.6; margin: 12px 0; } code { font-family: 'Consolas', 'Fira Code', monospace; font-size: 0.92em; } p code, li code, td code { background: #f0f0f0; padding: 1px 6px; border-radius: 4px; color: var(--dark-purple); } /\* ===== SEVERITY/RISK INDICATORS ===== \*/ .risk-high { color: var(--red); font-weight: 700; } .risk-medium { color: var(--orange); font-weight: 700; } .risk-low { color: var(--green); font-weight: 700; } /\* ===== FOOTER ===== \*/ .doc-footer { background: #f8f9fa; border-top: 3px solid var(--purple); padding: 28px 56px; font-size: 0.85em; color: var(--text-light); } .doc-footer .footer-brand { color: var(--purple); font-weight: 700; font-size: 1.1em; } /\* ===== PRINT ===== \*/ @media print { body { background: white; } .doc-wrapper { box-shadow: none; margin: 0; border-radius: 0; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } details { break-inside: avoid; } details\[open\] > .details-content { display: block !important; } .expand-controls { display: none; } thead th { -webkit-print-color-adjust: exact; print-color-adjust: exact; } } /\* ===== RESPONSIVE ===== \*/ @media (max-width: 800px) { .content { padding: 20px 24px; } .header-banner { padding: 28px 24px; } .header-banner h1 { font-size: 1.5em; } .toc-columns { grid-template-columns: 1fr; } .card-grid-2, .card-grid-3, .card-grid-4 { grid-template-columns: 1fr; } .header-stats { flex-direction: column; gap: 10px; } .kpi-row { flex-direction: column; } .flow-container { flex-direction: column; } .flow-arrow { transform: rotate(90deg); } .doc-footer { padding: 20px 24px; } .meta-bar { flex-direction: column; } .model-tabs { flex-direction: column; } } /\* ===== SEPARATOR ===== \*/ .section-sep { height: 1px; background: linear-gradient(to right, transparent, var(--border-gray), transparent); margin: 40px 0; }

![C16](data:image/jpeg;base64,)

# ECC → S/4HANA 2025 Migration Playbook

Universal Framework for Business Process Mapping, Fit-to-Standard Assessment, Clean Core Classification & Migration Execution

C16 Playbook v2.0 — April 2026

5Phases

21Plays

4Decision Gates

3Engagement Models

~13Months

Scope:ECC (any release) → S/4HANA 2025

Approach:Fit-to-Standard First, Clean Core Mandatory

Applies To:Greenfield · Brownfield · Bluefield

Generated:14 April 2026

📂 Expand All Sections 📁 Collapse All

## 📋 Table of Contents

[1 Engagement Models (A / B / C)](#engagement-models) [2 Phase Overview — The Migration Journey](#phase-overview) [3 Governance & RACI](#governance) [4 Phase 0 — Prerequisites & Extraction](#phase0) [5 Phase 1 — DISCOVER (Current State)](#phase1) [6 Business Process Mapping (ECC → S/4)](#process-mapping) [7 Simplification Impact (SAP Note 2313884)](#simplification) [8 Phase 2 — ASSESS (Fit-to-Standard)](#phase2) [9 Clean Core Classification Framework](#clean-core)

[10 The 5 Rs — Custom Code Disposition](#five-rs) [11 Phase 3 — REMEDIATE (Close Gaps)](#phase3) [12 Business Partner Migration Deep Dive](#bp-migration) [13 Phase 4 — MIGRATE (Build, Test, Go-Live)](#phase4) [14 8-Layer Testing Strategy](#testing) [15 Migration Timeline (Relative)](#timeline) [16 Risk Register](#risks) [17 C16 Automation Capabilities](#c16-accelerator) [18 ECC → S/4HANA Table Migration Reference](#table-reference) [19 Transaction → Fiori App Mapping](#appendix-fiori) [20 Clean Core Code Patterns (Before / After)](#appendix-code) [21 Getting Started — Immediate Next Steps](#getting-started)

1 Engagement Models

This playbook supports **three engagement models** depending on how C16 connects to the customer's landscape. Every phase in this document branches by model — the customer picks their path, and C16 adapts.

🔗

Model A — C16 Connected to ECC

C16 has direct RFC/ADT access to the customer's live ECC system. Maximum automation — C16 auto-scans custom code, reads configs, classifies dependencies, and generates migration reports in hours.

Fastest — Days

🔄

Model B — C16 Connected to S/4HANA

Customer already has an S/4HANA system (sandbox, conversion, or fresh). C16 connects to S/4HANA to validate what migrated — checks Clean Core compliance, finds deprecated API usage, dead code, and performance regressions.

Post-Migration Validation

📤

Model C — Offline / Manual

No direct system access. Customer extracts artifacts (TADIR, ST03N, Readiness Check, source code files) and provides them to C16. C16 analyzes what's provided. Slower but works for any customer.

Customer Extracts — Weeks

🟢 Model A — Auto-scan ECC

🔵 Model B — Validate S/4HANA

🟣 Model C — Manual Extracts

Throughout this playbook, each play includes **execution steps for all three models** inside expandable panels. The customer picks their model once, then follows only that path.

2 Phase Overview — The Migration Journey

Phase 0Prerequisites

→

Phase 1DISCOVER

→

Phase 2ASSESS

→

Phase 3REMEDIATE

→

Phase 4MIGRATE

Phase

Objective

Duration (Relative)

Key Deliverable

Phase 0

Establish governance, team, tools. Customer extracts prerequisites (Model C) or C16 connects (Model A/B).

Month 0 (prep)

Engagement Charter, System Access / Extracts

Phase 1

Inventory everything — processes, custom code, interfaces, data volumes, enhancements, forms.

Month 1–2

Current State Inventory

Phase 2

Map every ECC process to S/4HANA standard. Classify custom objects. Identify gaps.

Month 2–4

Fit-to-Standard Matrix, Clean Core Classification, Gap Register

Phase 3

Close gaps: retire, replace, refit, rewrite. Migrate BP. Remediate enhancements. Convert interfaces & forms.

Month 4–10

Remediated Codebase, BP Migration Complete

Phase 4

Build S/4HANA system, migrate data, execute 8-layer testing, train users, go-live.

Month 8–13

Production Go-Live

#### ⚡ Key Principle: Phases Overlap

Phase 3 remediation sprints can begin on early-classified modules while Phase 2 assessment continues for remaining modules. Phase 4 system build starts while Phase 3 completes. This parallelism keeps the timeline to ~13 months instead of 18+.

3 Governance & RACI

📋 View Governance Structure & RACI Matrix

### Steering Committee

Meets bi-weekly. Approves gate decisions, resolves escalations, controls budget and timeline. Members: Project Sponsor, Program Manager, IT Director, Business Process Owners (1 per module), C16 Lead.

### Workstream RACI

**R** = Responsible (does the work) · **A** = Accountable (owns the outcome) · **C** = Consulted · **I** = Informed

Workstream

C16

Basis / Tech

Functional Leads

Process Owners

Program Manager

Custom Code Analysis

R

C

C

I

A

Business Process Mapping

C

I

R

A

I

Fit-to-Standard Workshops

R

C

R

A

I

Clean Core Classification

R

C

I

I

A

Code Remediation

R

C

C

I

A

Data Migration

C

R

C

A

I

Testing

C

R

R

A

I

Training & Change Mgmt

I

I

C

R

A

4 Phase 0 — Prerequisites & Extraction

#### 🎯 Purpose

Before any analysis begins, the engagement model must be determined and prerequisites satisfied. For **Model A/B**, this means establishing system connectivity. For **Model C**, this means the customer extracts 9 critical artifacts from their ECC system.

Model A C16 Connected to ECC — Setup Steps

*   Customer provides RFC connection details (host, system number, client, service port)
*   Customer creates a technical user with RFC + ADT access (roles: S\_RFC, S\_DEVELOP, read access to TADIR/TRDIR/DD03L/TFDIR)
*   Firewall rules opened for C16 → ECC connectivity (port 443 for ADT HTTPS or 33xx for RFC)
*   C16 validates connection: `sap_test_connection` + `sap_list_programs`
*   No extraction needed — C16 reads everything directly

**Time:** 1–3 days (mostly customer-side network/security approval)

Model B C16 Connected to S/4HANA — Setup Steps

*   Customer provides ADT HTTPS endpoint for S/4HANA (typically port 443 with path /sap/bc/adt/)
*   Customer creates a developer user with ADT + OData access
*   C16 validates connection and confirms S/4HANA release (≥ 2020)
*   C16 scans all custom Z/Y objects that were migrated or created
*   Customer provides ECC baseline documentation (if available) for comparison

**Time:** 1–3 days

Model C Offline — 9 Artifacts the Customer Must Extract

Without system access, the customer must extract and provide these artifacts. Each includes the SAP transaction to run and the expected output format.

#

Artifact

How to Extract

Format

Why C16 Needs It

**1**

Custom Code Inventory

`SE16` on table `TADIR` filtered by `OBJ_NAME LIKE 'Z%' OR 'Y%'`. Alternative: run program `/SDF/CDM_ANALYZE` or SAP Readiness Check.

CSV / Excel

Full list of custom reports, classes, function modules, enhancements, BAdI implementations

**2**

Transaction Usage Statistics

`ST03N` → Expert Mode → Top Transactions (last 12 months). Or `STAD` logs.

CSV / Excel

Identifies which custom and standard transactions are actively used — avoids migrating dead code

**3**

Enhancement & Modification Registry

`SMOD`/`CMOD` (Customer Exits), `SE18`/`SE19` (BAdIs), `SPAU`/`SPDD` history

CSV / Excel

Lists all enhancement implementations and SAP standard modifications

**4**

SAP Readiness Check Results

SAP Launchpad → SAP Readiness Check for S/4HANA 2025 (free tool, no installation needed)

PDF / HTML

Automated analysis: Simplification Items, custom code impact, add-on compatibility, business function status

**5**

Custom Code Migration Worklist

Transaction `SYCM` or ATC with check variant `ABAP_CLOUD_READINESS`

CSV / Excel

Identifies custom code using deprecated/changed APIs, tables, or functions

**6**

Active Business Process List

Solution Manager (SOLMAN) process documentation, or manual list of active IMG configurations and transaction usage per module

Word / Excel

Maps current ECC processes to S/4HANA equivalents

**7**

Interface Inventory

List of all RFCs, IDocs, BAPIs, OData services, middleware connections (PI/PO, CPI). Check `WE20` for partner profiles, `SM59` for RFC destinations.

Excel

Interfaces often break during migration — full inventory needed

**8**

Output / Form Inventory

`SE71` (SAPscript), `SMARTFORMS`, `SFP` (Adobe Forms) — list all Z/Y forms

Excel

Many output technologies are deprecated in S/4HANA

**9**

Add-on & ISV Inventory

Transaction `SAINT` — list all installed add-ons and their versions

Excel / Screenshot

Add-on compatibility with S/4HANA must be verified with each ISV vendor

#### ⚠️ Critical: SAP Readiness Check is the #1 Priority

If the customer can only provide ONE artifact, it should be the **SAP Readiness Check** (Artifact #4). It's free, automated, and provides Simplification Item impact + add-on compatibility + custom code analysis in a single report. Available at `launchpad.support.sap.com`.

**Time:** 1–2 weeks (depends on customer's internal processes)

5 Phase 1 — DISCOVER (Current State Inventory)

#### 🎯 Objective

Build a complete picture of the ECC landscape — processes, customizations, integrations, data volumes, enhancements, and forms. This is the foundation for everything that follows.

4Plays

Month 1–2Duration

Gate 1Exit Gate

Play 1 Business Process Catalogue

**Goal:** Document every active business process across all SAP modules. This becomes the input to Fit-to-Standard workshops in Phase 2.

Model A C16 → ECC

*   C16 queries `ST03N` transaction statistics via RFC to identify top-used transactions
*   C16 runs `sap_discover_programs` by business context (P2P, O2C, R2R, etc.)
*   C16 reads active IMG configurations to detect enabled business functions
*   Cross-reference with process owner interviews (functional leads confirm/correct)

Model B C16 → S/4HANA

*   C16 scans all Z/Y transactions on S/4HANA to identify what migrated
*   C16 checks Fiori Launchpad catalog for active standard + custom Fiori apps
*   Compare migrated transaction list against SAP Best Practices Explorer for gaps
*   Identify processes that migrated "as-is" but should have been simplified

Model C Offline

*   Customer provides Artifact #6 (Active Business Process List) from SOLMAN or manual documentation
*   Customer provides Artifact #2 (ST03N transaction usage) — sorted by usage count
*   C16 team reviews and organizes by module (FI/CO, SD, MM, PP, PM, QM, HR, WM)
*   Validate with process owner interviews (remote sessions)

#### Output Template

#

Module

Process

ECC Transaction(s)

Usage/Month

Process Owner

Custom?

1

MM

Create Purchase Order

ME21N

_from ST03N_

_name_

Yes — ZMM\_PO\_ENH BAdI

2

SD

Create Sales Order

VA01

_from ST03N_

_name_

No

— Populate from discovery —

Play 2 Custom Code & Enhancement Inventory

**Goal:** Complete registry of every Z/Y object, customer exit, BAdI implementation, and SAP standard modification.

Model A C16 → ECC

*   C16 runs `sap_list_programs` — returns all Z/Y programs, classes, function groups, interfaces, tables
*   C16 queries `TADIR` for full custom object catalogue with package assignment
*   C16 scans `MODSAP` + `SXS_ATTR` for active customer exits and BAdI implementations
*   C16 checks `SPAU/SPDD` tables for SAP standard modifications
*   C16 batch-classifies all objects against Clean Core levels (A/B/C/D)

**Time:** 2–4 hours for up to 30,000 objects

Model B C16 → S/4HANA

*   C16 scans all Z/Y objects on S/4HANA — identifies what migrated
*   C16 runs ATC with `ABAP_CLOUD_READINESS` variant on all custom objects
*   C16 identifies dead code (migrated but zero usage — check via `CROSS` cross-reference tables)
*   C16 checks for deprecated API usage that "works today but breaks on next upgrade"
*   Compare against ECC baseline (if provided) to identify objects that were lost/forgotten in migration

Model C Offline

*   Customer provides Artifact #1 (Custom Code Inventory from TADIR)
*   Customer provides Artifact #3 (Enhancement/Modification Registry)
*   Customer provides Artifact #5 (SYCM Custom Code Migration Worklist)
*   C16 team imports and categorizes by object type (PROG, CLAS, FUGR, ENHS, etc.)
*   If customer provides source code files (.abap / .txt), C16 can analyze individual programs

**Time:** 1–2 weeks (depends on customer responsiveness)

#### Enhancement Migration Matrix

ECC Enhancement Type

S/4HANA 2025 Equivalent

Migration Path

User Exits (`CMOD`/`SMOD`)

Released BAdIs (ES\_\* enhancement spots)

Rewrite exit logic as BAdI implementation using released interfaces

Classic BAdIs (`SE18`/`SE19`)

Released Cloud BAdIs (if available)

Check cloudification catalog for released successor; rewrite if internal

SAP Modifications (`SPAU`/`SPDD`)

Key User Extensibility or Released BAdIs

Eliminate ALL modifications — use BAdIs or Key User tools

Append Structures (CI\_\*)

Key User Custom Fields or Extension Includes (E\_\*)

Migrate append fields to Key User extensibility or E\_\* extension includes

`CALL CUSTOMER-FUNCTION`

Released BAdIs

Find released BAdI successor in enhancement spot documentation

Business Transaction Events (BTE)

Released BAdIs or BRF+

Map each BTE to its BAdI successor — most FI BTEs have BAdI replacements

Play 3 Interface & Integration Inventory

**Goal:** Catalogue every inbound and outbound interface — IDocs, RFCs, SOAP/REST services, flat files, middleware connections.

Model A C16 → ECC

*   C16 queries `SM59` (RFC destinations) via RFC\_READ\_TABLE
*   C16 reads `WE20` (partner profiles) for IDoc interfaces
*   C16 lists OData services via Gateway catalog
*   C16 scans custom code for `CALL FUNCTION ... DESTINATION` patterns

Model B C16 → S/4HANA

*   C16 lists all OData services on S/4HANA — identifies custom vs standard
*   C16 checks RFC destinations for orphaned/broken connections
*   C16 scans custom code for hardcoded RFC destinations that may have changed
*   Verify IDoc message types still supported in S/4HANA

Model C Offline

*   Customer provides Artifact #7 (Interface Inventory) — SM59 + WE20 + middleware config
*   C16 team reviews and classifies by interface type and direction (inbound/outbound)
*   Flag interfaces using deprecated BAPIs or removed IDoc message types

#### Interface Migration Paths

ECC Technology

S/4HANA Equivalent

Effort

RFC (synchronous)

Released SOAP/OData APIs or keep stable BAPIs

Low–Medium

IDocs (asynchronous)

IDocs still supported; consider Event Mesh for new patterns

Low

BAPIs (create/change)

Released BAPIs or new OData V4 APIs

Low

SAP PI/PO (middleware)

SAP Integration Suite (CPI) or keep PI/PO short-term

Medium–High

Flat file uploads

Fiori Upload Apps or Released APIs

Medium

Custom OData V2

Migrate to OData V4 (RAP-based)

Medium

Play 4 Data Volume & Forms Assessment

**Goal:** Assess data volumes for migration sizing, archiving strategy, and inventorize all output forms.

#### Critical Data Areas

Data Category

ECC Tables

S/4HANA Change

Action

Financial Documents

BKPF, BSEG, BSIK, BSAK, BSID, BSAD

ACDOCA replaces index tables for reporting

Size ACDOCA, plan archiving

Material Documents

MKPF, MSEG

MATDOC (single table)

Size MATDOC, review custom reports

CO-PA

CE1\*, CE2\* (costing-based)

Account-based CO-PA via ACDOCA (mandatory)

Redesign CO-PA reporting

Vendor/Customer Master

LFA1, LFB1, KNA1, KNB1

Business Partner (BUT000, BUT020, etc.)

BP Migration (Play 15)

Stock/Inventory

MARD, MARC, MCHB

MARD simplified, batch stock restructured

Validate stock management reports

#### Forms Migration Paths

ECC Form Type

S/4HANA Path

Clean Core Level

SAPscript (`SE71`)

Convert to Adobe Forms (SFP) — SAPscript is **deprecated**

Level D

SmartForms (`SMARTFORMS`)

Convert to Adobe Forms — SmartForms are **deprecated**

Level D

Adobe Forms (`SFP`)

Continue using — supported in S/4HANA

Level B

Output Determination (NACE)

Migrate to BRF+ / Output Management Framework

Level C

#### 💡 C16 SmartForm Converter

C16 includes an automated SmartForm → Adobe Form conversion pipeline. It downloads the SmartForm XML, parses the structure, generates a rendering-safe XDP template, and produces deployment ABAP — all automated. Available in Models A and B.

#### 🚧 GATE 1 — Discovery Completion

Phase 1 is complete when ALL of the following are satisfied:

*   Business Process Catalogue complete (all modules covered, process owners assigned)
*   Custom Code Inventory complete (all Z/Y objects catalogued with package and object type)
*   Enhancement/Modification Registry complete (all exits, BAdIs, modifications listed)
*   Interface Inventory complete (all RFC destinations, IDocs, OData services listed)
*   Data Volume Assessment complete (row counts for top 20 tables, archiving strategy drafted)
*   Forms Inventory complete (all SAPscript, SmartForms, Adobe Forms listed)
*   Add-on Inventory complete (all SAINT add-ons listed with S/4HANA compatibility status)
*   SAP Readiness Check results reviewed and imported

**Gate Decision:** Steering Committee reviews inventory completeness. If any artifact is missing or incomplete, Phase 2 workshops will produce unreliable results. Do not pass Gate 1 with gaps.

6 Business Process Mapping — ECC → S/4HANA 2025

This section provides the **universal mapping** between ECC transactions/tables and their S/4HANA 2025 standard equivalents. This mapping applies to every customer — specific customizations are overlaid during Phase 2 workshops.

📦 MM — Materials Management

ECC Process

ECC Transaction

S/4HANA Transaction

Fiori App

Key Data Model Change

Create Purchase Requisition

ME51N

ME51N

F1140 — Manage Purchase Requisitions

Minor — same tables

Create Purchase Order

ME21N

ME21N

F0842 — Create Purchase Order

Minor — KONV → PRCD\_ELEMENTS

Goods Receipt

MIGO

MIGO

F2521 — Post Goods Receipt

**Major:** MKPF/MSEG → MATDOC

Invoice Verification

MIRO

MIRO

F0862 — Create Supplier Invoice

Minor

Material Master

MM01/MM02/MM03

MM01/MM02/MM03

F1732 — Manage Product Master Data

MARA/MARC/MARD restructured

Vendor Master

XK01/XK02/XK03

**BP**

F1739 — Manage Supplier

**Critical:** LFA1/LFB1 → BP (BUT000/BUT020)

Stock Overview

MMBE / MB52

MMBE

F0842A — Stock Overview

MARD simplified

Inventory Management

MI01/MI04/MI07

MI01/MI04/MI07

F2209 — Physical Inventory

MKPF/MSEG → MATDOC

Source List / Info Record

ME01 / ME11

ME01 / ME11

F2084 — Manage Purchasing Info Records

Minor

MRP

MD01/MD02/MD04

MD01N (MRP Live)

F1434 — Monitor Material Coverage

**Major:** MRP Live replaces classic MRP

📦 SD — Sales & Distribution

ECC Process

ECC Transaction

S/4HANA Transaction

Fiori App

Key Data Model Change

Create Sales Order

VA01

VA01

F6589 — Create Sales Orders

Minor — KONV → PRCD\_ELEMENTS

Delivery

VL01N

VL01N

F1907 — Create Outbound Deliveries

Minor

Billing

VF01

VF01

F2325 — Create Billing Documents

Minor

Customer Master

XD01/XD02/XD03

**BP**

F1735 — Manage Business Partner

**Critical:** KNA1/KNB1 → BP

Pricing

VK11/VK12

VK11/VK12

F2735 — Manage Sales Pricing

KONV → PRCD\_ELEMENTS

Credit Management

FD32 / UKM

UKM\_\*

F0710 — Manage Credit

**Major:** Classic → SAP Credit Management (FSCM)

Available-to-Promise

CO09

CO09 / aATP

F3846 — Schedule Sales Orders

aATP (Advanced ATP) available

Output Determination

NACE / VV31

NACE (legacy) or BRF+

—

Output Management Framework recommended

Returns

VA01 (RE order type)

VA01

F6589

Advanced Returns Management available

📦 FI/CO — Finance & Controlling

ECC Process

ECC Transaction

S/4HANA Transaction

Fiori App

Key Data Model Change

GL Posting

FB50 / F-02

FB50

F0717 — Post General Journal Entries

**Critical:** BSEG reporting → ACDOCA

Vendor Invoice

FB60 / MIRO

FB60 / MIRO

F0859 — Post Supplier Invoices

Minor

Payment Run

F110

F110

F0711 — Schedule Payment Runs

Minor

Bank Reconciliation

FF67 / FEBA

FEBA / FEBAN

F2682 — Manage Bank Statements

Advanced bank statement processing

Asset Accounting

AS01/AS02

AS01/AS02

F2598 — Manage Fixed Assets

**Major:** New Asset Accounting (mandatory migration)

Closing Cockpit

—

CCLM

F2723 — Closing Cockpit

New in S/4

CO-PA (Profitability)

KE21N / KE30

KE21N

F2653 — Profitability Analysis

**Critical:** Account-based CO-PA via ACDOCA (costing-based removed)

Cost Center Accounting

KS01/KS02/KSB1

KS01/KS02

F2046 — Manage Cost Centers

Reporting via ACDOCA

Revenue Recognition

Manual / VF44

IFRS 15 / RAR

F3292 — Revenue Accounting

**Major:** IFRS 15 compliance module available

📦 PP / PM / QM — Production, Plant Maintenance, Quality

ECC Process

ECC Transaction

S/4HANA Transaction

Fiori App

Key Change

Create Production Order

CO01

CO01

F2498 — Manage Production Orders

MRP Live integration

MRP Run

MD01 / MD02

MD01N (MRP Live)

F1434 — Monitor Material Coverage

**Major:** MRP Live on HANA — 10x faster

BOM Management

CS01/CS02

CS01/CS02

F2276 — Manage Bills of Material

Minor

Routing

CA01/CA02

CA01/CA02

F2288 — Manage Routings

Minor

Create Maintenance Order

IW31/IW32

IW31/IW32

F1648 — Manage Maintenance Orders

Minor

Maintenance Notifications

IW21/IW22

IW21/IW22

F1579 — Manage Maintenance Notifications

Minor

Quality Inspection

QA01/QA02

QA01/QA02

F3047 — Manage Quality Inspections

Minor

Quality Notifications

QM01/QM02

QM01/QM02

F2983 — Manage Quality Notifications

Minor

📦 WM / EWM — Warehouse Management

#### ⚠️ Critical Change: Classic WM is Removed in S/4HANA 2025

SAP Warehouse Management (LE-WM) using transactions `LT01`/`LT0*` is **not available** in S/4HANA 2025. Customers must migrate to **Embedded EWM** (included with S/4HANA license) or **Decentralized EWM** (separate system). This is one of the highest-effort migration items for warehouse-intensive customers.

ECC WM Process

S/4HANA EWM Equivalent

Effort

Transfer Order (LT01)

Warehouse Task / Warehouse Order

High — new concept

Storage Type/Bin (LS01/LS11)

Activity Area / Storage Bin

Medium — config mapping

Goods Receipt to WM (LT06)

Inbound Delivery Processing

Medium

Picking (LT03)

Outbound Delivery Processing + Wave Mgmt

Medium

Inventory Management in WM

EWM Physical Inventory

Medium

RF Transactions

EWM RF Framework (SAP ITSmobile or Fiori)

High — UI rewrite

7 Simplification Impact — SAP Note 2313884

SAP Note 2313884 contains the **Simplification List for S/4HANA 2025** — every table, function module, transaction, and feature that has been changed, replaced, or removed. Below are the 12 highest-impact items that affect the most customers.

📋 View All 12 High-Impact Simplification Items

#

Area

What Changed

Impact

Action Required

1

Business Partner

KNA1, LFA1 → BUT000, BUT020, BP\* tables

ALL customer/vendor master programs

Mandatory BP migration using CVI tools. All custom reports must use I\_Supplier / I\_Customer CDS views.

2

Material Documents

MKPF + MSEG → MATDOC (single table)

ALL inventory/GR/GI reports

Rewrite SELECTs on MKPF/MSEG to use MATDOC or CDS view I\_MaterialDocumentItem.

3

Universal Journal

BSEG-based reporting → ACDOCA

ALL FI reports, reconciliation

Rewrite FI reports to use ACDOCA or CDS views (I\_JournalEntry, I\_GLAccountLineItem).

4

CO-PA

Costing-based CO-PA removed; account-based CO-PA via ACDOCA mandatory

ALL profitability reports

Redesign CO-PA reporting to use account-based approach.

5

New Asset Accounting

Classic Asset Accounting replaced by New Asset Accounting

ALL asset reports

Mandatory migration to New Asset Accounting. Custom asset reports need rework.

6

Pricing (KONV)

KONV → PRCD\_ELEMENTS

Custom pricing reports, SD/MM conditions

Replace KONV references with PRCD\_ELEMENTS or CDS views.

7

Credit Management

Classic credit (FD32) → SAP Credit Management (FSCM/UKM)

Credit check programs, SD exits

Migrate credit rules to UKM. Custom credit checks need rewrite.

8

MRP Live

Classic MRP → MRP Live (ppMRP) — HANA-optimized

Custom MRP enhancements, exits

Test MRP enhancements with MRP Live. Some user exits don't fire in MRP Live.

9

Warehouse Management

LE-WM completely removed → Embedded EWM

ALL WM programs, RF transactions

Full WM → EWM migration (high effort). See WM section above.

10

Output Management

NACE-based output → Output Management Framework / BRF+

Custom output programs, form triggers

Migrate output determination to new framework.

11

Material Ledger

Material Ledger activation mandatory

Costing, inventory valuation reports

Ensure ML is active and configured. Custom costing reports may need rework.

12

Batch Management

Batch stock tables restructured

Custom batch reports on MCHB

Validate batch management reports work with new data model.

8 Phase 2 — ASSESS (Fit-to-Standard & Gap Analysis)

#### 🎯 Objective

For every business process and custom object identified in Phase 1, determine whether S/4HANA 2025 standard covers it — and document every gap with a closure strategy.

### Fit-to-Standard Classification Framework

Every business requirement extracted from custom programs is classified into one of four categories:

FIT

S/4HANA standard covers this requirement exactly. No customization needed. Adopt the standard process.

FIT + CONFIG

Standard covers it, but requires configuration (IMG settings, BRF+ rules, workflow setup). No ABAP code needed.

FIT + EXTENSION

Standard covers ~70-80%. Remaining gap needs a Clean Core–compliant extension (released BAdI, Key User, or RAP side-by-side).

GAP

No standard equivalent exists. Must be justified, documented, and built as a Clean Core custom extension on S/4HANA or BTP.

### Fit-to-Standard Workshop Template

📋 Workshop Execution Guide (Per Module)

Run one workshop per module. Duration: 4–8 hours per module depending on complexity. Participants: C16 analyst, Functional Lead, Process Owner, Key Users.

#### Step 1 — Requirement Decomposition

Break each custom program/process into discrete business requirements. Example:

#

Business Requirement

Source (Custom Object)

Classification

Standard Alternative

Confidence

R1

Display vendor general data

Z\_VENDOR\_REPORT

FIT

F1739 — Manage Supplier + I\_Supplier CDS

100%

R2

Show vendor payment history

Z\_VENDOR\_REPORT

FIT+CONFIG

FBL1N / F0718 — Supplier Line Items

90%

R3

Custom MSME classification

Z\_VENDOR\_REPORT (Z-table)

GAP

No standard — needs E\_Supplier extension

N/A

— Populate per program —

#### Step 2 — Coverage Calculation

For each standard alternative, calculate how many requirements it covers:

Standard Alternative

Requirements Covered

Coverage %

F1739 — Manage Supplier

R1, R4, R6, R7, R8

5/10 = 50%

BP Transaction

R1, R2, R4, R5, R6, R7

6/10 = 60%

— Populate per analysis —

#### Step 3 — Final Verdict

Overall Coverage

Verdict

Action

≥ 90% FIT

**ADOPT**

Use standard as-is. Retire custom program.

≥ 70% FIT + CONFIG

**SIMPLIFY**

Use standard with configuration. Minor setup.

50–70% FIT + EXTENSION

**MODERNIZE**

Use standard + build Clean Core extensions for gaps.

< 50%

**KEEP**

Significant gaps — rebuild as Clean Core custom app.

#### 🚧 GATE 2 — Assessment Completion

*   Fit-to-Standard workshop completed for every active module
*   Every custom object classified (5 Rs — see Section 10)
*   Clean Core level assigned to every custom object (A/B/C/D)
*   Gap Register complete — every GAP has a documented closure strategy
*   Simplification Item impact assessed for all 12 critical items
*   Enhancement migration path documented for every active exit/BAdI
*   Migration approach decided: Greenfield / Brownfield / Bluefield

9 Clean Core Classification Framework

Every custom object is classified against SAP's **Clean Core** principles — determining whether it uses only released, stable APIs or relies on internal/deprecated interfaces that will break in future upgrades.

Level A Cloud-Ready

Uses ONLY released APIs (CDS views, RAP BOs, released BAPIs with C1 contract). Deployable to S/4HANA Cloud. Future-proof.

Level B Classic Stable

Uses stable classic APIs (BAPIs, REUSE\_ALV, CL\_SALV\_TABLE). Works on S/4HANA on-premise but NOT cloud-deployable.

Level C Non-Compliant

Direct SELECT on standard tables without CDS alternative, unreleased FMs, internal interfaces. Needs modernization.

Level D Critical

Deprecated patterns (SAPscript, removed tables), SAP standard modifications, EXEC SQL. Will break — mandatory rewrite.

📋 Classification Rules — How Objects Are Scored

Pattern Detected

Level

Rationale

Uses only released APIs (C1 contract)

A

Cloud-compatible

Uses CL\_SALV\_TABLE, REUSE\_ALV\_GRID\_DISPLAY

B

Stable on S/4HANA on-prem, not cloud-released

Uses stable BAPIs (BAPI\_PO\_CREATE1, etc.)

B

Classic APIs — work but not cloud-released

Direct SELECT on standard tables (LFA1, EKKO, VBAK)

C

CDS view alternatives exist — should modernize

Uses unreleased function modules

C

May change without notice in future releases

MODIFY/UPDATE/INSERT/DELETE on standard tables

D

Zero cloud compatibility — data integrity risk

SAP standard modifications (SPAU/SPDD)

D

Blocks upgrades — must eliminate

SAPscript / SmartForms

D

Deprecated — convert to Adobe Forms

EXEC SQL / Native SQL

D

DB-specific — incompatible with HANA optimization

BDC / CALL TRANSACTION (screen scraping)

C

Fragile — screen changes break BDC. Use APIs instead.

**Overall Level Rule:** The overall level of a program = **MAX (worst)** of all its dependencies. If a program uses one Level D pattern, the entire program is Level D.

📋 C16 Automation — How Classification Happens Per Model

Model A C16 → ECC

C16 reads every custom program's source code, resolves all dependencies (function modules, classes, tables, BAdIs), and classifies each dependency against SAP's cloudification catalog. The process:

1.  Fetch source via ADT
2.  Parse all external references (SELECT tables, CALL FUNCTION, class method calls, includes)
3.  Look up each reference in the cloudification catalog (Level A = released, Level B = classic, Level C = unreleased)
4.  Compute overall level = MAX(all dependencies)
5.  Generate per-program HTML report with score, findings, and remediation code

**Speed:** ~2 minutes per program. Batch mode: 50-100 programs per hour.

Model B C16 → S/4HANA

Same classification process, but on S/4HANA. Additional checks:

1.  Run ATC with `ABAP_CLOUD_READINESS` variant — SAP's own in-system classification
2.  Cross-reference with C16's cloudification catalog for accuracy
3.  Flag objects that passed migration but still use deprecated patterns (BSEG direct access, KONV, etc.)
4.  Identify "time bombs" — code that works today but will break on next S/4HANA upgrade

Model C Offline

Customer provides ABAP source files (.abap / .txt). C16 analyzes each file:

1.  Parse source code for table references, FM calls, class usage
2.  Classify against cloudification catalog (same rules)
3.  Generate reports — but without live system verification, some CDS view recommendations are marked "requires verification"

Also accepts SAP Readiness Check output for bulk classification.

10 The 5 Rs — Custom Code Disposition

Every custom object gets exactly ONE disposition. This determines its migration path.

🗑️

RETIRE

**Criteria:** Zero usage in ST03N, or redundant (another program does the same thing).  
**Action:** Delete. Do not migrate. Document for audit trail.

🔄

REPLACE

**Criteria:** Standard S/4HANA Fiori app or transaction covers 90%+ of requirements.  
**Action:** Adopt standard. Train users. Decommission custom.

🔧

REFIT

**Criteria:** Still needed but uses deprecated APIs or tables. Logic is valid, technology is not.  
**Action:** Remediate code — replace deprecated calls with released APIs, CDS views, RAP.

🏗️

REWRITE

**Criteria:** Fundamentally incompatible — classic ALV reports, BDC-heavy programs, screen-based transactions.  
**Action:** Rebuild using RAP + Fiori Elements + Clean Core patterns. High effort but future-proof.

✅

RETAIN

**Criteria:** Still needed, no deprecated elements, already Clean Core Level A or B. Unique business IP.  
**Action:** Migrate unchanged. Validate on S/4HANA. Add to regression test suite.

📋 Decision Tree — How to Classify Each Object

flowchart TD A\[Custom Object\] --> B{Used in last 12 months?} B -->|No| C\[🗑️ RETIRE\] B -->|Yes| D{Standard S/4HANA  
covers 90%+?} D -->|Yes| E\[🔄 REPLACE\] D -->|No| F{Uses deprecated  
APIs/tables?} F -->|No| G{Clean Core  
Level A or B?} G -->|Yes| H\[✅ RETAIN\] G -->|No| I\[🔧 REFIT\] F -->|Yes| J{Core logic  
fundamentally  
incompatible?} J -->|No| K\[🔧 REFIT\] J -->|Yes| L\[🏗️ REWRITE\]

11 Phase 3 — REMEDIATE (Close Gaps, Fix Code)

#### 🎯 Objective

Execute on the Phase 2 assessment: retire dead code, replace with standard, refit deprecated APIs, rewrite fundamentally incompatible programs, and retain what's already clean. This is the heaviest phase — 12–20 weeks of sprint-based execution.

6Plays

Month 4–10Duration

Gate 3Exit Gate

Play 5 Quick Wins — RETIRE & REPLACE

Start with the easiest wins: retire dead code and replace programs that have standard equivalents.

#### RETIRE Checklist

*   Export list of zero-usage programs from ST03N / C16 scan
*   Confirm with process owners that each program is truly unused
*   Document each retirement decision (program name, last used date, reason)
*   Do NOT delete from ECC — mark as "Do Not Migrate" in the migration worklist

#### REPLACE Checklist

*   For each REPLACE-classified program, identify the specific standard Fiori app / transaction
*   Configure the standard app (may require IMG settings, authorization roles)
*   Train affected users on the new standard process
*   Run parallel testing: old custom vs. new standard (2-week window)
*   Decommission custom program after successful parallel run

**Expected volume:** Typically 30-50% of custom objects can be retired or replaced. This is the highest ROI activity.

Play 6 Code Remediation Sprints (REFIT & REWRITE)

For objects classified as REFIT or REWRITE, execute remediation in 2-week sprints.

#### Sprint Structure (2-Week Template)

Day

Activity

Deliverable

1

Sprint Planning — select 5-10 objects from backlog, prioritized by risk

Sprint backlog

2-3

C16 deep analysis of each object — Clean Core report, dependency map, Fit-to-Standard

Per-object analysis reports

4-8

Remediation development — C16 generates code, developers review and adjust

Remediated source code

9

Unit testing — syntax check, ATC, ABAP Unit

Test results

10

Sprint Review — demo to process owner, approve or iterate

Approved remediations

#### Common Remediation Patterns

Pattern 1: Direct Table Access → CDS View

**Before (Level C):**

SELECT matnr mtart maktx FROM mara
  INNER JOIN makt ON makt~matnr = mara~matnr
  INTO TABLE @DATA(lt\_materials)
  WHERE mara~mtart = @p\_mtart.

**After (Level A):**

SELECT product, producttype, productname
  FROM i\_product
  WHERE producttype = @p\_mtart
  INTO TABLE @DATA(lt\_materials).

Pattern 2: BDC / CALL TRANSACTION → Released API

**Before (Level C):**

CALL TRANSACTION 'XK01' USING lt\_bdcdata
  MODE 'N' UPDATE 'S'.

**After (Level B → A):**

CALL FUNCTION 'BAPI\_BUPA\_CREATE\_FROM\_DATA'
  EXPORTING
    businesspartnerextern = lv\_bp\_number
    partnercategory       = '2'  " Vendor
  IMPORTING
    businesspartner       = lv\_bp\_created
  TABLES
    return                = lt\_return.

Pattern 3: REUSE\_ALV → CL\_SALV\_TABLE (or RAP Fiori)

**Before (Level B):**

CALL FUNCTION 'REUSE\_ALV\_GRID\_DISPLAY'
  EXPORTING
    i\_callback\_program = sy-repid
    is\_layout          = ls\_layout
    it\_fieldcat        = lt\_fieldcat
  TABLES
    t\_outtab           = lt\_output.

**After — Option A: CL\_SALV\_TABLE (Level B, simpler):**

TRY.
    cl\_salv\_table=>factory(
      IMPORTING r\_salv\_table = DATA(lo\_alv)
      CHANGING  t\_table      = lt\_output ).
    lo\_alv->display( ).
  CATCH cx\_salv\_msg INTO DATA(lx).
    MESSAGE lx->get\_text( ) TYPE 'E'.
ENDTRY.

**After — Option B: RAP Fiori Elements (Level A, S/4HANA only):**

@UI.lineItem: \[{ position: 10 }\]
key MaterialNumber : abap.char(40);
@UI.lineItem: \[{ position: 20 }\]
MaterialType : abap.char(4);
@UI.lineItem: \[{ position: 30 }\]
MaterialDescription : abap.char(40);

Pattern 4: Customer Exit → Released BAdI

**Before (Level D):**

\* Inside CMOD exit implementation — INCLUDE ZXM06U01
DATA: lv\_custom\_check TYPE abap\_bool.
" Custom PO validation logic
IF ekko-bsart = 'ZNB'.
  " ... custom logic
ENDIF.

**After (Level A/B):**

CLASS zcl\_badi\_po\_check DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
  PUBLIC SECTION.
    INTERFACES if\_badi\_me\_check.
ENDCLASS.

CLASS zcl\_badi\_po\_check IMPLEMENTATION.
  METHOD if\_badi\_me\_check~check.
    " Same logic but via released BAdI interface
    IF im\_header-bsart = 'ZNB'.
      " ... custom logic via released interface
    ENDIF.
  ENDMETHOD.
ENDCLASS.

Play 7 Enhancement & Exit Migration

Migrate every active customer exit, BAdI implementation, and modification to its S/4HANA equivalent.

*   For each active exit: find the released BAdI successor using SAP's enhancement spot documentation
*   For SAP modifications: eliminate 100% — replace with BAdIs or Key User extensibility
*   For append structures: migrate to Key User custom fields or E\_\* extension includes
*   Test each migrated enhancement independently before integration testing
*   Verify BAdIs are NOT internal (check `SXS_ATTR.INTERNAL` flag — internal BAdIs are SAP-reserved)

Play 8 Interface & Integration Migration

*   Test all IDocs on S/4HANA sandbox — most work unchanged
*   Replace deprecated BAPIs with released OData APIs where available
*   Update RFC destinations (hostnames, ports may change)
*   If migrating PI/PO → Integration Suite: plan separately (this is a project in itself)
*   Flat file interfaces: consider replacing with Fiori Upload apps or OData batch

Play 9 Forms Conversion (SAPscript / SmartForms → Adobe)

*   Inventory all SAPscript and SmartForms — prioritize by usage frequency
*   Use C16's automated SmartForm → Adobe Form converter (Model A/B) or convert manually (Model C)
*   Test all converted forms with real data — fonts, page breaks, barcodes need verification
*   Migrate output determination from NACE to Output Management Framework (if applicable)

Play 10 Add-on & ISV Compatibility Resolution

*   For each add-on from SAINT inventory: check ISV vendor's S/4HANA compatibility statement
*   Contact ISV vendors for S/4HANA-compatible versions
*   If no S/4HANA version available: plan replacement or custom build
*   Test add-on compatibility on S/4HANA sandbox before production migration

#### 🚧 GATE 3 — Remediation Completion

*   All RETIRE objects documented and excluded from migration scope
*   All REPLACE objects: standard alternative configured, users trained, parallel test passed
*   All REFIT objects: deprecated APIs replaced, ATC clean, unit tests pass
*   All REWRITE objects: rebuilt in Clean Core, functional testing complete
*   All RETAIN objects: validated on S/4HANA sandbox, regression tests pass
*   All enhancements migrated to released BAdIs — zero SPAU/SPDD modifications
*   All SAPscript/SmartForms converted to Adobe Forms
*   All interfaces tested on S/4HANA sandbox
*   Add-on compatibility confirmed for all active add-ons
*   BP Migration complete (see Section 12)

12 Business Partner Migration Deep Dive

#### ⚠️ #1 Risk Item for Every ECC → S/4HANA Migration

The migration from separate Vendor (LFA1) and Customer (KNA1) masters to the unified **Business Partner** model is mandatory, complex, and affects virtually every module. It deserves its own dedicated workstream.

📋 BP Migration — Full Step-by-Step Guide

#### Architecture Change

flowchart LR subgraph ECC\[ECC - Separate Masters\] V\[Vendor  
LFA1/LFB1/LFM1\] C\[Customer  
KNA1/KNB1/KNVV\] end subgraph S4\[S/4HANA - Unified BP\] BP\[Business Partner  
BUT000/BUT020  
BUT0BK/BUT100\] BPS\[BP Supplier Role  
CVI Mapping\] BPC\[BP Customer Role  
CVI Mapping\] end V --> BPS C --> BPC BPS --> BP BPC --> BP

#### 6-Step Migration Process

Step

Activity

Transactions / Tools

Duration

**1**

Pre-Check: Activate CVI (Customer Vendor Integration) synchronization in ECC

`BUPA_SYSTEM_SWITCH`, `MDS_LOAD_COCKPIT`

1 week

**2**

Data Cleansing: Fix duplicate addresses, invalid tax numbers, incomplete bank details

`SE16` on LFA1/KNA1, data quality tools

2-4 weeks

**3**

Number Range Alignment: Decide BP numbering (same as vendor/customer or new range)

`BUCF`, number range config

1 week

**4**

Synchronization Run: Execute CVI sync to create BP records for all vendors/customers

`MDS_LOAD_COCKPIT`

1-2 weeks

**5**

Validation: Verify all vendors/customers have corresponding BP records with correct roles

`BP` transaction, validation reports

1 week

**6**

Custom Code Update: All programs reading LFA1/KNA1 must use BP tables or CDS views

C16 analysis + remediation

2-6 weeks

#### CDS View Replacements for BP

ECC Table

S/4HANA CDS View

Purpose

LFA1 (Vendor General)

`I_Supplier`

Supplier master — general data

LFB1 (Vendor Company)

`I_SupplierCompany`

Supplier company code data

LFM1 (Vendor Purchasing)

`I_SupplierPurchasingOrg`

Supplier purchasing org data

KNA1 (Customer General)

`I_Customer`

Customer master — general data

KNB1 (Customer Company)

`I_CustomerCompany`

Customer company code data

KNVV (Customer Sales)

`I_CustomerSalesArea`

Customer sales area data

#### Custom Code Impact

Every program that reads/writes to these tables needs remediation:

*   `SELECT FROM LFA1` → `SELECT FROM I_Supplier`
*   `SELECT FROM KNA1` → `SELECT FROM I_Customer`
*   Custom reports joining LFA1 + ADRC for address → use I\_Supplier (includes address via association)
*   BDC on XK01/XK02 → Use BP BAPI (BAPI\_BUPA\_CREATE\_FROM\_DATA) or I\_BusinessPartner RAP BO
*   Z-table appends on LFA1/KNA1 → Migrate to E\_Supplier / E\_Customer extension includes

13 Phase 4 — MIGRATE (Build, Test, Go-Live)

#### 🎯 Objective

Build the S/4HANA system, migrate data, execute comprehensive testing, train users, and go-live. This phase overlaps with the tail end of Phase 3 remediation.

Play 11 System Build & Configuration

*   **Greenfield:** Install S/4HANA 2025, activate SAP Best Practices, configure from scratch using Phase 2 Fit-to-Standard decisions
*   **Brownfield:** Run SUM (Software Update Manager) to convert ECC → S/4HANA in-place. Apply Simplification Item fixes post-conversion.
*   **Bluefield:** New S/4HANA install + selective data transfer using tools like SNP CrystalBridge, SAP DMLT, or Syniti
*   Deploy all remediated custom code (REFIT, REWRITE, RETAIN objects)
*   Configure migrated enhancements (BAdI implementations, Key User extensibility)
*   Set up Fiori Launchpad: assign standard Fiori apps, configure tile catalogs and groups

Play 12 Data Migration

Data Area

Tool / Approach

Key Consideration

Business Partner

CVI Sync (if Brownfield) or LSMW/DMIG (Greenfield)

Must complete before any SD/MM transactional data

Material Master

Migration Cockpit (LTMC) or LSMW

Material Ledger must be active first

Financial Data

SAP DMIG or custom migration programs

ACDOCA data model — ensure balances reconcile

Open Items (AP/AR)

Migration Cockpit

Match open items with BP records

Inventory / Stock

Migration Cockpit

Cutover: freeze stock, migrate, unfreeze

Historical Data

Archive or data lake

Decision: migrate history to S/4 or keep in ECC/archive?

Play 13 Cutover Planning

#### Cutover Runbook (Weekend Template)

Time

Activity

Owner

Checkpoint

Friday 18:00

Freeze ECC — no new transactions

Basis

All users locked

Friday 20:00

Final data extract from ECC

Data Team

Extract files verified

Friday 22:00

Load data into S/4HANA

Data Team

Load complete, row counts match

Saturday 06:00

Reconciliation — verify balances, open items, stock

Functional

All reconciliation reports green

Saturday 12:00

Smoke testing — 20 critical transactions

Key Users

All smoke tests pass

Saturday 18:00

Go/No-Go decision

Steering Committee

Formal sign-off

Sunday 06:00

Open S/4HANA for users

Basis

Users unlocked, Fiori accessible

Monday 08:00

Hypercare begins — dedicated support team

All

Incident queue monitored

#### 🚧 GATE 4 — Go-Live Readiness

*   All 8 testing layers passed (see Section 14)
*   Data migration validated — balances reconcile, master data complete
*   All critical Fiori apps accessible and functional
*   All interfaces tested end-to-end with external partners
*   User training complete — at least 90% of key users trained
*   Cutover rehearsal completed (at least 2 dry runs)
*   Rollback plan documented and tested
*   Hypercare team staffed and ready (2-4 weeks post go-live)

14 8-Layer Testing Strategy

📋 View Complete Testing Framework

Layer

Test Type

What's Tested

Who

Tool

**1**

Syntax & Compilation

Every custom object compiles without errors

C16 / Developers

ATC, Syntax Check

**2**

ABAP Unit Tests

Business logic correctness at method level

Developers

ABAP Unit, C16

**3**

Functional Testing

Each process works end-to-end (create PO, post GR, etc.)

Functional Leads

Manual / Test Scripts

**4**

Integration Testing

Cross-module flows (PO → GR → Invoice → Payment)

Functional Leads

Test Scripts

**5**

Interface Testing

All IDocs, RFCs, OData services work with external systems

Integration Team

End-to-end traces

**6**

Performance Testing

Response times, MRP run duration, month-end close time

Basis / Performance

SAP Early Watch, custom benchmarks

**7**

User Acceptance Testing (UAT)

Real users execute real business scenarios

Key Users

Test scripts + sign-off forms

**8**

Cutover Rehearsal

Full dry run of go-live weekend (data migration + smoke tests)

All Teams

Production-like sandbox

#### 💡 C16 Automates Layers 1-2

When connected (Model A/B), C16 automatically runs syntax checks, ATC quality checks, and ABAP Unit tests after every code deployment. This catches 80% of defects before functional testers even see the code.

15 Migration Timeline (Relative)

All durations are relative (Month 1, Week 1) — adjust start date based on engagement kickoff.

📋 View Full Timeline with Milestones

gantt title ECC to S4HANA 2025 Migration Timeline dateFormat YYYY-MM-DD axisFormat %b section Phase 0 - Prerequisites Governance Setup :p0a, 2026-05-01, 10d System Access or Extracts :p0b, 2026-05-01, 14d section Phase 1 - DISCOVER Process Catalogue :p1a, 2026-05-15, 21d Custom Code Inventory :p1b, 2026-05-15, 14d Interface and Forms :p1c, 2026-05-29, 14d Data Volume Assessment :p1d, 2026-06-05, 10d Gate 1 Review :milestone, g1, 2026-06-19, 1d section Phase 2 - ASSESS FtS Workshops MM SD :p2a, 2026-06-20, 21d FtS Workshops FI CO PP :p2b, 2026-07-04, 21d Clean Core Classification :p2c, 2026-06-20, 28d Gap Register :p2d, 2026-07-18, 10d Gate 2 Review :milestone, g2, 2026-08-01, 1d section Phase 3 - REMEDIATE Quick Wins Retire Replace :p3a, 2026-08-02, 28d BP Migration :p3b, 2026-08-02, 56d Code Sprints Wave 1 :p3c, 2026-08-16, 42d Code Sprints Wave 2 :p3d, 2026-09-27, 42d Enhancement Migration :p3e, 2026-09-01, 35d Forms Conversion :p3f, 2026-10-01, 28d Interface Migration :p3g, 2026-10-15, 21d Gate 3 Review :milestone, g3, 2026-11-30, 1d section Phase 4 - MIGRATE System Build :p4a, 2026-10-01, 42d Data Migration Dev :p4b, 2026-10-15, 35d Integration Testing :p4c, 2026-11-15, 28d UAT :p4d, 2026-12-15, 28d Performance Testing :p4e, 2027-01-05, 14d Cutover Rehearsal 1 :p4f, 2027-01-19, 7d Cutover Rehearsal 2 :p4g, 2027-02-01, 7d Training :p4h, 2027-02-01, 21d Gate 4 Review :milestone, g4, 2027-02-20, 1d Production Cutover :crit, p4i, 2027-02-21, 3d Hypercare :p4j, 2027-02-24, 28d

#### Key Milestones

Milestone

Target (Relative)

Criteria

Gate 1

End of Month 2

Discovery complete — all inventories done

Gate 2

End of Month 4

Assessment complete — all objects classified, gaps documented

Gate 3

End of Month 8

Remediation complete — all code fixed, BP migrated

Gate 4

End of Month 11

Go-Live ready — testing complete, users trained

🚀 **Go-Live**

Month 12-13

Production cutover + 4 weeks hypercare

16 Risk Register

📋 View All 8 Risks with Mitigations

#

Risk

Probability

Impact

Severity

Mitigation

1

**BP Migration Complexity** — data quality issues, duplicate records, missing bank details

High

Critical

🔴 CRITICAL

Start data cleansing in Phase 1. Run CVI pre-checks early. Dedicate a full workstream.

2

**Custom Code Volume** — more Z-objects than estimated, extends remediation timeline

High

High

🔴 HIGH

Use C16 batch analysis for rapid classification. Prioritize by usage. Aggressively RETIRE unused code.

3

**WM → EWM Migration** — underestimated complexity for warehouse-intensive customers

Medium

Critical

🔴 HIGH

Start WM assessment in Phase 1. Consider parallel run. Budget extra 3 months if WM is heavy.

4

**Add-on Incompatibility** — ISV vendor doesn't have S/4HANA version ready

Medium

High

🟠 MEDIUM

Contact all ISV vendors in Phase 0. Have contingency plan (custom build or alternative product).

5

**Integration Breaks** — external partner interfaces fail post-migration

Medium

High

🟠 MEDIUM

Full interface inventory in Phase 1. End-to-end testing with all partners in Phase 4.

6

**User Resistance** — Fiori UX change causes productivity drop

High

Medium

🟠 MEDIUM

Early user involvement, training 4+ weeks before go-live, change management program.

7

**Data Migration Reconciliation** — balances don't match after migration

Medium

Critical

🔴 HIGH

Multiple cutover rehearsals. Automated reconciliation reports. Contingency: extend cutover weekend.

8

**Timeline Slippage** — remediation takes longer than planned

Medium

Medium

🟠 MEDIUM

Buffer 20% on each phase. Parallelize where possible. Use C16 automation to accelerate.

17 C16 Automation Capabilities

C16 accelerates the migration across all phases. Here's what C16 automates vs. manual effort:

📋 View C16 Capabilities by Model

Capability

Manual Effort

With C16

Model A

Model B

Model C

Custom Code Inventory

1–2 weeks

2–4 hours

✅ Auto

✅ Auto

⚡ From extract

Clean Core Classification (per program)

2–4 hours

2 minutes

✅ Auto

✅ Auto

⚡ From source

Mass Classification (1000+ objects)

4–8 weeks

1–2 days

✅ Auto

✅ Auto

Partial

Fit-to-Standard per Program

1–2 days

15 minutes

✅ Auto

✅ Auto

⚡ From source

Remediation Code Generation

1–4 weeks/program

1–4 hours

✅ + Deploy

✅ + Deploy

✅ Generate

SmartForm → Adobe Conversion

2–5 days/form

30 minutes

✅ Auto

✅ Auto

From XML

Post-Deploy Testing (Syntax/ATC/Unit)

30 min/object

Automatic

✅ Auto

✅ Auto

N/A

Dependency Analysis

2–4 hours/program

30 seconds

✅ Auto

✅ Auto

From source

Dead Code Detection

Manual review

Automated scan

✅ Auto

✅ Auto

From ST03N

Performance Anti-Pattern Detection

Expert review

Automated (P1-P5)

✅ Auto

✅ Auto

From source

#### 💡 ROI Estimate

For a landscape with 5,000 custom objects, C16 reduces the assessment phase from **~12 weeks manual** to **~2 weeks automated**. Remediation code generation saves an additional **40-60%** of developer effort compared to manual rewriting.

18 ECC → S/4HANA Table Migration Reference

📋 View Complete Table Migration Map (20+ Tables)

ECC Table

Purpose

S/4HANA Status

Replacement Table/View

CDS View

LFA1

Vendor General

Compatibility view (read only)

BUT000 + CVI

`I_Supplier`

LFB1

Vendor Company

Compatibility view

BUT100 + CVI

`I_SupplierCompany`

KNA1

Customer General

Compatibility view

BUT000 + CVI

`I_Customer`

KNB1

Customer Company

Compatibility view

BUT100 + CVI

`I_CustomerCompany`

BSEG

FI Line Items

Exists but not for reporting

ACDOCA

`I_JournalEntry`

BSIK/BSAK

Vendor Open/Cleared Items

Removed for reporting

ACDOCA

`I_OperationalAcctgDocItem`

BSID/BSAD

Customer Open/Cleared Items

Removed for reporting

ACDOCA

`I_OperationalAcctgDocItem`

MKPF

Material Doc Header

Removed

MATDOC

`I_MaterialDocumentHeader`

MSEG

Material Doc Item

Removed

MATDOC

`I_MaterialDocumentItem`

KONV

Pricing Conditions

Removed

PRCD\_ELEMENTS

`I_PricingElement`

MBEW

Material Valuation

Restructured

MBEW (simplified)

`I_ProductValuation`

CE1xxxx/CE2xxxx

CO-PA Segment Tables

Removed (costing-based)

ACDOCA

Analytics CDS views

MARD

Storage Location Stock

Simplified

MARD (fewer fields)

`I_MaterialStock`

MCHB

Batch Stock

Restructured

MCHB (new structure)

`I_MaterialStockByBatch`

GLT0

GL Totals

Removed

ACDOCA (totals via aggregation)

`I_GLAccountBalance`

COEP

CO Line Items

Removed for reporting

ACDOCA

`I_JournalEntry`

ANLA

Asset Master

New Asset Accounting

ANLA (restructured)

`I_FixedAsset`

LQUA

WM Quant

Removed (no WM)

EWM tables

EWM CDS views

EKPO

PO Items

Exists

EKPO

`I_PurchaseOrderItem`

VBAP

Sales Order Items

Exists

VBAP

`I_SalesOrderItem`

19 Transaction → Fiori App Mapping

📋 View 30+ Transaction Mappings

ECC Transaction

Description

Fiori App ID

Fiori App Name

ME21N

Create Purchase Order

F0842

Create Purchase Order

ME23N

Display Purchase Order

F2229

Manage Purchase Orders

ME51N

Create Purchase Requisition

F1140

Manage Purchase Requisitions

MIGO

Goods Movement

F2521

Post Goods Receipt for PO

MIRO

Invoice Verification

F0862

Create Supplier Invoice

MM01/02/03

Material Master

F1732

Manage Product Master Data

VA01

Create Sales Order

F6589

Create Sales Orders

VA03

Display Sales Order

F3893

Manage Sales Orders

VL01N

Create Delivery

F1907

Create Outbound Deliveries

VF01

Create Billing Document

F2325

Create Billing Documents

XK01/XK03

Vendor Master

F1739

Manage Supplier

XD01/XD03

Customer Master

F1735

Manage Business Partner

FB50

GL Posting

F0717

Post General Journal Entries

FB60

Vendor Invoice

F0859

Post Supplier Invoices

F110

Payment Run

F0711

Schedule Payment Runs

FBL1N

Vendor Line Items

F0718

Supplier Line Items

FBL5N

Customer Line Items

F0710

Customer Line Items

MB52

Warehouse Stocks

F0842A

Stock — Multiple Materials

MMBE

Stock Overview

F1727

Stock — Single Material

ME2M

POs by Material

F2229

Manage Purchase Orders (filtered)

CO01

Create Production Order

F2498

Manage Production Orders

MD04

Stock/Reqmts List

F1434

Monitor Material Coverage

IW31

Create Maintenance Order

F1648

Manage Maintenance Orders

QA01

Create Inspection Lot

F3047

Manage Quality Inspections

AS01/AS02

Asset Master

F2598

Manage Fixed Assets

KS01/KS02

Cost Centers

F2046

Manage Cost Centers

KE21N

CO-PA Posting

F2653

Profitability Analysis

BP

Business Partner

F1735

Manage Business Partner

NACE

Output Determination

—

BRF+ / Output Management

SE38

ABAP Editor

—

ADT Eclipse (not Fiori)

_Source: SAP Fiori Apps Library — [fioriappslibrary.hana.ondemand.com](https://fioriappslibrary.hana.ondemand.com)_

20 Clean Core Code Patterns — Before & After

📋 View All Code Migration Patterns

### Pattern 1: Vendor Master Access

Before (ECC — Level C)

After (S/4HANA — Level A)

SELECT lifnr name1 stras ort01
  FROM lfa1
  INTO TABLE @DATA(lt\_vendors)
  WHERE lifnr IN @s\_lifnr.

SELECT supplier, suppliername,
       streetname, cityname
  FROM i\_supplier
  INTO TABLE @DATA(lt\_vendors)
  WHERE supplier IN @s\_lifnr.

### Pattern 2: Financial Document Reporting

Before (ECC — Level C)

After (S/4HANA — Level A)

SELECT bukrs belnr gjahr buzei
       hkont dmbtr
  FROM bseg
  INTO TABLE @DATA(lt\_items)
  WHERE bukrs = @p\_bukrs
    AND gjahr = @p\_gjahr.

SELECT companycode,
       accountingdocument,
       fiscalyear,
       accountingdocumentitem,
       glaccount,
       amountincompanycodecurrency
  FROM i\_journalentryitem
  INTO TABLE @DATA(lt\_items)
  WHERE companycode = @p\_bukrs
    AND fiscalyear  = @p\_gjahr.

### Pattern 3: Material Document Access

Before (ECC — Level D on S/4)

After (S/4HANA — Level A)

SELECT mkpf~mblnr mkpf~budat
       mseg~zeile mseg~matnr
       mseg~menge mseg~bwart
  FROM mkpf
  INNER JOIN mseg
    ON mseg~mblnr = mkpf~mblnr
   AND mseg~mjahr = mkpf~mjahr
  INTO TABLE @DATA(lt\_matdocs)
  WHERE mkpf~budat IN @s\_budat.

SELECT materialdocument,
       postingdate,
       materialdocumentitem,
       material,
       quantityinbaseunit,
       goodsmovementtype
  FROM i\_materialdocumentitem
  INTO TABLE @DATA(lt\_matdocs)
  WHERE postingdate IN @s\_budat.

### Pattern 4: Condition/Pricing Access

Before (ECC — Level D on S/4)

After (S/4HANA — Level A)

SELECT knumv kposn kschl
       kbetr kwert
  FROM konv
  INTO TABLE @DATA(lt\_conds)
  WHERE knumv = @lv\_knumv.

SELECT conditionrecord,
       conditionsequentialnumber,
       conditiontype,
       conditionratevalue,
       conditionamount
  FROM i\_pricingelement
  INTO TABLE @DATA(lt\_conds)
  WHERE pricingdocument = @lv\_knumv.

### Pattern 5: BDC Screen Scraping → API Call

Before (Level C)

After (Level A/B)

\* BDC for ME21N — fragile!
PERFORM bdc\_field USING 'BDC\_OKCODE' '/00'.
PERFORM bdc\_field USING 'EKKO-LIFNR' lv\_vendor.
PERFORM bdc\_field USING 'RM06E-BSART' 'NB'.
CALL TRANSACTION 'ME21N'
  USING lt\_bdcdata
  MODE 'N' UPDATE 'S'.

\* Released BAPI — stable interface
CALL FUNCTION 'BAPI\_PO\_CREATE1'
  EXPORTING
    poheader     = ls\_poheader
    poaccount    = lt\_poaccount
  IMPORTING
    purchaseorder = lv\_po\_number
  TABLES
    poitem        = lt\_poitem
    return        = lt\_return.

IF lt\_return IS INITIAL.
  CALL FUNCTION 'BAPI\_TRANSACTION\_COMMIT'
    EXPORTING wait = 'X'.
ENDIF.

21 Getting Started — Immediate Next Steps

#### 🚀 Week 1 Actions (Regardless of Model)

Step 1 — Run SAP Readiness Check

Free, automated, immediate value. Go to `launchpad.support.sap.com` → SAP Readiness Check for S/4HANA 2025. This single action provides 60% of the information needed for Phase 1.

Step 2 — Choose Engagement Model

Decide: Can C16 connect to ECC (Model A)? To S/4HANA sandbox (Model B)? Or will you extract and provide artifacts (Model C)? This shapes the entire engagement.

Step 3 — Nominate Process Owners

Identify one process owner per module (FI, CO, SD, MM, PP, PM, QM, WM, HR). They'll be required for Fit-to-Standard workshops in Phase 2.

Step 4 — Export Transaction Usage (ST03N)

Run ST03N in ECC and export the top 500 transactions by user count and dialog steps. This instantly reveals which processes are critical and which custom code is actually used.

Step 5 — Decide Migration Approach

**Greenfield** (fresh install, re-implement) vs **Brownfield** (in-place conversion) vs **Bluefield** (selective data transition). This decision impacts every subsequent phase. If Fit-to-Standard and Clean Core are top priorities, **Greenfield or Bluefield** is recommended.

Step 6 — Schedule Kickoff

Bring together the Steering Committee for the engagement kickoff. Review this playbook, confirm scope, assign RACI, and set the timeline start date.

#### 💡 The Single Most Impactful Action

If there is only ONE thing done in Week 1, it should be: **Connect C16 to the system.** Whether it's the ECC (Model A) or an S/4HANA sandbox (Model B), live system access transforms this playbook from a strategic framework into a precision execution plan backed by real data — custom code classified in hours, not weeks; gaps identified with evidence, not estimates; remediation code generated, not hand-crafted.

C16 — ECC → S/4HANA 2025 Migration Playbook

Version 2.0 · April 2026 · Universal Framework (Customer-Agnostic)

This playbook applies to any ECC → S/4HANA 2025 migration regardless of customer size, industry, or module scope. It supports three engagement models: C16 connected to ECC (Model A), C16 connected to S/4HANA (Model B), and offline/manual extraction (Model C). All process mappings, table migrations, and Fiori app references are based on SAP's published documentation and Simplification List (SAP Note 2313884).

Generated by C16 · [SAP Fiori Apps Library](https://fioriappslibrary.hana.ondemand.com) · [SAP S/4HANA Documentation](https://help.sap.com/docs/SAP_S4HANA_ON-PREMISE)

mermaid.initialize({ startOnLoad: true, theme: 'base', themeVariables: { primaryColor: '#6B3FA0', primaryTextColor: '#fff', primaryBorderColor: '#B5567A', lineColor: '#B5567A', secondaryColor: '#f8f6ff', tertiaryColor: '#fdf2f8', fontFamily: 'Segoe UI, Arial, sans-serif' }, flowchart: { curve: 'basis', useMaxWidth: true }, gantt: { useMaxWidth: true, fontSize: 11 } });