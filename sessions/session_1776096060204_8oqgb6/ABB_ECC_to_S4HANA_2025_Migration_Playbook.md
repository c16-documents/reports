  ABB ECC → S/4HANA 2025 Migration Playbook :root { --primary: #6B3FA0; --secondary: #B5567A; --accent: #C97B3A; --success: #27ae60; --warning: #e67e22; --danger: #e74c3c; --info: #3498db; --dark: #2c3e50; --light-bg: #f8fafc; --card-shadow: 0 2px 12px rgba(0,0,0,0.08); } \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: var(--dark); background: #e9ecef; line-height: 1.7; } .doc-wrapper { max-width: 1200px; margin: 0 auto; background: #fff; box-shadow: 0 4px 24px rgba(0,0,0,0.12); } /\* Header Banner \*/ .header-banner { background: linear-gradient(135deg, #C97B3A, #B5567A, #6B3FA0); color: #fff; padding: 50px 60px; position: relative; overflow: hidden; } .header-banner::before { content: ''; position: absolute; top: -50%; right: -10%; width: 400px; height: 400px; background: radial-gradient(circle, rgba(255,255,255,0.08) 0%, transparent 70%); border-radius: 50%; } .header-banner::after { content: ''; position: absolute; bottom: -30%; left: 20%; width: 300px; height: 300px; background: radial-gradient(circle, rgba(255,255,255,0.05) 0%, transparent 70%); border-radius: 50%; } .header-banner h1 { font-size: 2.4em; margin-bottom: 8px; position: relative; z-index: 1; } .header-banner .subtitle { font-size: 1.3em; opacity: 0.92; position: relative; z-index: 1; } .header-banner .meta-badges { margin-top: 18px; position: relative; z-index: 1; display: flex; gap: 10px; flex-wrap: wrap; } .header-badge { background: rgba(255,255,255,0.2); padding: 6px 16px; border-radius: 20px; font-size: 0.85em; backdrop-filter: blur(4px); border: 1px solid rgba(255,255,255,0.25); } .header-logo { position: relative; z-index: 1; margin-bottom: 15px; } .header-logo img { height: 50px; border-radius: 8px; } /\* Meta Bar \*/ .meta-bar { background: linear-gradient(135deg, #f8f9fa, #e9ecef); padding: 20px 60px; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; border-bottom: 3px solid var(--primary); } .meta-item { display: flex; flex-direction: column; } .meta-label { font-size: 0.75em; text-transform: uppercase; color: #888; letter-spacing: 1px; font-weight: 600; } .meta-value { font-size: 0.95em; font-weight: 600; color: var(--dark); } /\* Content \*/ .content { padding: 40px 60px; } /\* Section Title \*/ .section-title { color: var(--primary); font-size: 1.6em; margin: 45px 0 20px 0; padding-bottom: 12px; border-bottom: 3px solid var(--secondary); display: flex; align-items: center; gap: 12px; } .section-title .num { background: var(--secondary); color: #fff; width: 36px; height: 36px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; font-size: 0.7em; font-weight: 700; flex-shrink: 0; } /\* Sub-section \*/ .subsection-title { color: var(--secondary); font-size: 1.25em; margin: 30px 0 12px 0; font-weight: 600; } .sub3-title { color: var(--accent); font-size: 1.05em; margin: 20px 0 10px 0; font-weight: 600; } /\* Table of Contents \*/ .toc { background: linear-gradient(135deg, #f0f4f8, #e8ecf0); padding: 30px 40px; border-radius: 12px; margin: 25px 0 35px 0; border-left: 5px solid var(--primary); } .toc h3 { color: var(--primary); margin-bottom: 15px; font-size: 1.2em; } .toc-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6px 30px; } .toc a { color: var(--dark); text-decoration: none; font-size: 0.9em; padding: 3px 0; display: block; } .toc a:hover { color: var(--primary); text-decoration: underline; } .toc .toc-phase { font-weight: 700; color: var(--primary); margin-top: 8px; } /\* Tables \*/ table { width: 100%; border-collapse: collapse; margin: 15px 0 25px 0; font-size: 0.9em; } thead th { background: var(--primary); color: #fff; padding: 12px 14px; text-align: left; font-weight: 600; font-size: 0.85em; text-transform: uppercase; letter-spacing: 0.5px; } tbody td { padding: 10px 14px; border-bottom: 1px solid #e9ecef; } tbody tr:nth-child(even) { background: var(--light-bg); } tbody tr:hover { background: #edf2f7; } /\* Cards \*/ .kpi-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin: 20px 0; } .kpi-card { background: linear-gradient(135deg, #f8f9fa, #fff); padding: 20px; border-radius: 12px; border-left: 4px solid var(--primary); box-shadow: var(--card-shadow); text-align: center; } .kpi-card .kpi-value { font-size: 2em; font-weight: 700; color: var(--primary); } .kpi-card .kpi-label { font-size: 0.8em; color: #888; margin-top: 4px; text-transform: uppercase; letter-spacing: 0.5px; } /\* Phase Cards \*/ .phase-card { background: #fff; border-radius: 12px; padding: 25px 30px; margin: 20px 0; border-left: 5px solid var(--primary); box-shadow: var(--card-shadow); } .phase-card.discover { border-left-color: var(--info); } .phase-card.assess { border-left-color: var(--warning); } .phase-card.remediate { border-left-color: var(--danger); } .phase-card.migrate { border-left-color: var(--success); } .phase-card h4 { font-size: 1.15em; margin-bottom: 8px; } .phase-card .phase-badge { display: inline-block; padding: 3px 12px; border-radius: 12px; font-size: 0.75em; font-weight: 700; color: #fff; margin-bottom: 10px; } .phase-card.discover .phase-badge { background: var(--info); } .phase-card.assess .phase-badge { background: var(--warning); } .phase-card.remediate .phase-badge { background: var(--danger); } .phase-card.migrate .phase-badge { background: var(--success); } /\* Checklist \*/ .checklist { list-style: none; padding: 0; margin: 10px 0; } .checklist li { padding: 8px 0 8px 32px; position: relative; border-bottom: 1px solid #f0f0f0; } .checklist li::before { content: '☐'; position: absolute; left: 5px; top: 8px; font-size: 1.1em; color: var(--primary); } .checklist li.critical::before { content: '🔴'; } .checklist li.warning::before { content: '🟡'; } .checklist li.done::before { content: '✅'; } /\* Decision Gate \*/ .gate-box { background: linear-gradient(135deg, #fff5f5, #ffe8e8); border: 2px solid var(--danger); border-radius: 12px; padding: 20px 25px; margin: 20px 0; } .gate-box h4 { color: var(--danger); margin-bottom: 8px; } .gate-box .gate-criteria { margin: 8px 0; padding-left: 20px; } .gate-box .gate-criteria li { margin: 4px 0; } /\* Info Box \*/ .info-box { background: linear-gradient(135deg, #f0f8ff, #e6f3ff); border-left: 5px solid var(--info); padding: 18px 22px; border-radius: 8px; margin: 15px 0; } .info-box.success { background: linear-gradient(135deg, #f0fff4, #e6ffe6); border-left-color: var(--success); } .info-box.warning { background: linear-gradient(135deg, #fffbf0, #fff3e0); border-left-color: var(--warning); } .info-box.danger { background: linear-gradient(135deg, #fff5f5, #ffe8e8); border-left-color: var(--danger); } .info-box strong { color: var(--primary); } /\* Badges \*/ .badge { display: inline-block; padding: 2px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 600; color: #fff; } .badge-a { background: var(--success); } .badge-b { background: var(--info); } .badge-c { background: var(--warning); } .badge-d { background: var(--danger); } .badge-fit { background: var(--success); } .badge-config { background: var(--info); } .badge-extend { background: var(--warning); } .badge-gap { background: var(--danger); } .badge-phase { background: var(--primary); } .badge-retire { background: #95a5a6; } .badge-replace { background: var(--info); } .badge-refit { background: var(--warning); } .badge-rewrite { background: var(--danger); } .badge-retain { background: var(--success); } /\* Workstream \*/ .workstream-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 16px; margin: 15px 0; } .ws-card { background: #fff; border-radius: 10px; padding: 20px; box-shadow: var(--card-shadow); border-top: 4px solid var(--primary); } .ws-card h5 { color: var(--primary); margin-bottom: 8px; } .ws-card .ws-owner { font-size: 0.8em; color: #888; margin-bottom: 8px; } .ws-card ul { padding-left: 18px; font-size: 0.9em; } .ws-card ul li { margin: 4px 0; } /\* RACI \*/ .raci-r { background: #e74c3c; color: #fff; padding: 2px 8px; border-radius: 4px; font-weight: 700; font-size: 0.8em; } .raci-a { background: #e67e22; color: #fff; padding: 2px 8px; border-radius: 4px; font-weight: 700; font-size: 0.8em; } .raci-c { background: #3498db; color: #fff; padding: 2px 8px; border-radius: 4px; font-weight: 700; font-size: 0.8em; } .raci-i { background: #95a5a6; color: #fff; padding: 2px 8px; border-radius: 4px; font-weight: 700; font-size: 0.8em; } /\* Collapsible \*/ details { margin: 10px 0; } details summary { background: #f0f4f8; padding: 12px 18px; border-radius: 8px; cursor: pointer; font-weight: 600; color: var(--primary); border-left: 4px solid var(--secondary); } details summary:hover { background: #e8ecf0; } details\[open\] summary { border-radius: 8px 8px 0 0; } details .detail-content { padding: 15px 18px; background: #fafbfc; border: 1px solid #e9ecef; border-top: 0; border-radius: 0 0 8px 8px; } /\* Code \*/ pre { background: #f5f5f5; padding: 15px; border-radius: 8px; border-left: 4px solid var(--secondary); overflow-x: auto; font-size: 0.85em; margin: 10px 0; } code { font-family: 'Cascadia Code', 'Fira Code', Consolas, monospace; } .inline-code { background: #f0f0f0; padding: 2px 6px; border-radius: 4px; font-family: monospace; font-size: 0.9em; } /\* Timeline \*/ .timeline { position: relative; padding-left: 40px; margin: 20px 0; } .timeline::before { content: ''; position: absolute; left: 15px; top: 0; bottom: 0; width: 3px; background: linear-gradient(to bottom, var(--info), var(--warning), var(--danger), var(--success)); } .timeline-item { position: relative; margin-bottom: 25px; padding-left: 20px; } .timeline-item::before { content: ''; position: absolute; left: -32px; top: 5px; width: 14px; height: 14px; background: var(--primary); border-radius: 50%; border: 3px solid #fff; box-shadow: 0 0 0 2px var(--primary); } .timeline-item h5 { color: var(--primary); margin-bottom: 4px; } .timeline-item .tl-date { font-size: 0.8em; color: #888; font-weight: 600; } /\* Risk Matrix \*/ .risk-matrix { display: grid; grid-template-columns: repeat(5, 1fr); gap: 2px; margin: 15px 0; } .risk-cell { padding: 8px; text-align: center; font-size: 0.75em; font-weight: 600; border-radius: 4px; } .risk-low { background: #d4edda; color: #155724; } .risk-med { background: #fff3cd; color: #856404; } .risk-high { background: #f8d7da; color: #721c24; } .risk-critical { background: #e74c3c; color: #fff; } /\* Footer \*/ .doc-footer { background: linear-gradient(135deg, #2c3e50, #34495e); color: rgba(255,255,255,0.8); padding: 30px 60px; font-size: 0.85em; } .doc-footer strong { color: #fff; } /\* Print \*/ @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } details\[open\] summary ~ \* { display: block; } .toc { page-break-after: always; } .section-title { page-break-before: always; } } /\* Responsive \*/ @media (max-width: 768px) { .header-banner, .meta-bar, .content, .doc-footer { padding-left: 20px; padding-right: 20px; } .toc-grid { grid-template-columns: 1fr; } .kpi-grid { grid-template-columns: 1fr 1fr; } .workstream-grid { grid-template-columns: 1fr; } } mermaid.initialize({startOnLoad:true, theme:'base', themeVariables:{primaryColor:'#6B3FA0', primaryTextColor:'#fff', primaryBorderColor:'#B5567A', lineColor:'#B5567A', secondaryColor:'#f0f4f8'}});

![C16](data:image/jpeg;base64,)

# 🏗️ ABB ECC → S/4HANA 2025 Migration Playbook

Comprehensive Step-by-Step Execution Guide — Fit-to-Standard & Clean Core

📋 Version 1.0 📅 April 2026 🔄 ECC 6.0 → S/4HANA 2025 🎯 Fit-to-Standard First 🛡️ Clean Core Mandatory 📖 Playbook — 22 Plays

ClientABB

Source SystemECC 6.0 (Not Connected)

Target SystemS/4HANA 2025

MethodologySAP Activate + Clean Core

Estimated Duration13-16 Months

Document TypeExecution Playbook

### 📖 Playbook Table of Contents

FOUNDATIONS

[Play 1 — Playbook Overview & Governance](#play-1) [Play 2 — Team Structure & RACI](#play-2) [Play 3 — Tooling & Environment Setup](#play-3)

PHASE 1: DISCOVER (Weeks 1-6)

[Play 4 — Business Process Catalogue](#play-4) [Play 5 — Custom Code Inventory](#play-5) [Play 6 — Interface & Integration Map](#play-6) [Play 7 — Data Volume & Archiving Assessment](#play-7) [🚦 Decision Gate 1 — Discovery Complete](#gate-1)

PHASE 2: ASSESS (Weeks 5-12)

[Play 8 — Fit-to-Standard Workshops](#play-8) [Play 9 — Clean Core Classification](#play-9) [Play 10 — Gap Analysis & Disposition](#play-10) [Play 11 — Simplification List Impact](#play-11)

[🚦 Decision Gate 2 — Assessment Complete](#gate-2)

PHASE 3: REMEDIATE (Weeks 10-28)

[Play 12 — Quick Wins (Retire & Replace)](#play-12) [Play 13 — Business Partner Migration](#play-13) [Play 14 — Custom Code Remediation Sprints](#play-14) [Play 15 — Interface Modernization](#play-15) [Play 16 — Forms Migration](#play-16) [🚦 Decision Gate 3 — Remediation Complete](#gate-3)

PHASE 4: MIGRATE (Weeks 26-52)

[Play 17 — System Build & Configuration](#play-17) [Play 18 — Data Migration & Cutover](#play-18) [Play 19 — Testing Strategy (8 Layers)](#play-19) [Play 20 — Training & Change Management](#play-20) [Play 21 — Go-Live & Hypercare](#play-21) [🚦 Decision Gate 4 — Go-Live Readiness](#gate-4)

APPENDICES

[Play 22 — Risk Register & Mitigation](#play-22) [Appendix A — Templates & Checklists](#appendix-a) [Appendix B — SAP Transaction → Fiori Mapping](#appendix-b) [Appendix C — Table Migration Reference](#appendix-c)

4

Major Phases

22

Plays

4

Decision Gates

8

Workstreams

~13

Months Duration

## 1 Play 1 — Playbook Overview & Governance

### 1.1 Purpose

This playbook provides ABB's migration team with a **step-by-step, executable guide** for migrating from SAP ECC 6.0 to S/4HANA 2025. It is organized into **22 Plays** across 4 Phases, each with clear inputs, activities, outputs, owners, and decision gates.

**Core Principles:**  
1\. **Fit-to-Standard FIRST** — Always check if SAP standard can replace custom code before modernizing it.  
2\. **Clean Core MANDATORY** — All surviving custom code must use Released APIs only (Level A target).  
3\. **Data-Driven Decisions** — Every disposition (retire/replace/rewrite/retain) must be backed by usage data and business justification.  
4\. **Zero Modifications** — No SAP standard code modifications in the target system. BAdIs, Key User Extensibility, or BTP only.

### 1.2 Migration Approach Decision

Approach

Description

Pros

Cons

ABB Fit

**Greenfield**

Fresh S/4HANA install. Re-implement all processes from scratch.

Maximum simplification. True Fit-to-Standard. Clean start.

Longest timeline. Highest effort. All config must be rebuilt.

POSSIBLE

**Brownfield** (System Conversion)

In-place technical conversion. Carry forward config + data.

Faster. Preserves tested config. Less business disruption.

Carries technical debt. Custom code may break. Limited simplification opportunity.

RISKY

**Bluefield** (Selective Data Transition)

New system + selectively migrate config & data. Best of both.

Simplify while preserving what works. Phased approach.

Requires SDT tooling. More complex project management.

RECOMMENDED

**Recommendation:** Given ABB's stated priorities (Fit-to-Standard + Clean Core), a **Bluefield** approach is recommended. This enables maximum process simplification while selectively migrating proven configurations and historical data.

### 1.3 Master Timeline

gantt title ABB ECC to S/4HANA 2025 Migration dateFormat YYYY-MM-DD axisFormat %b %Y section Phase 1: DISCOVER Business Process Catalogue :d1, 2026-05-01, 21d Custom Code Inventory :d2, 2026-05-01, 28d Interface Mapping :d3, 2026-05-15, 21d Data Volume Assessment :d4, 2026-05-22, 14d Gate 1: Discovery Complete :milestone, g1, 2026-06-12, 0d section Phase 2: ASSESS Fit-to-Standard Workshops :a1, 2026-06-01, 35d Clean Core Classification :a2, 2026-06-15, 28d Gap Analysis and Disposition :a3, 2026-07-01, 28d Simplification List Review :a4, 2026-07-01, 21d Gate 2: Assessment Complete :milestone, g2, 2026-08-07, 0d section Phase 3: REMEDIATE Quick Wins Retire and Replace :r1, 2026-07-15, 28d BP Migration Prep :r2, 2026-08-01, 56d Code Remediation Sprints :r3, 2026-08-15, 84d Interface Modernization :r4, 2026-09-01, 56d Forms Migration :r5, 2026-09-15, 42d Gate 3: Remediation Complete :milestone, g3, 2026-11-20, 0d section Phase 4: MIGRATE System Build and Config :m1, 2026-11-01, 56d Data Migration Dry Runs :m2, 2026-12-15, 42d Integration Testing :m3, 2027-01-15, 42d UAT :m4, 2027-02-15, 35d Cutover Rehearsal :m5, 2027-03-15, 14d Go-Live :milestone, gl, 2027-04-01, 0d Hypercare :m6, 2027-04-01, 56d

## 2 Play 2 — Team Structure & RACI

### 2.1 Workstream Organization

##### 🎯 WS1: Program Management

Owner: Program Manager

*   Overall governance & reporting
*   Risk & issue management
*   Stakeholder communication
*   Decision gate facilitation

##### 📊 WS2: Business Process

Owner: Business Process Lead

*   Process catalogue & mapping
*   Fit-to-Standard workshops
*   Gap identification
*   UAT coordination

##### 💻 WS3: Technical / ABAP

Owner: Technical Architect

*   Custom code inventory & classification
*   Clean Core remediation
*   Code conversion & testing
*   Performance optimization

##### 🔗 WS4: Integration

Owner: Integration Lead

*   Interface inventory & mapping
*   API modernization (IDoc → OData)
*   Middleware migration (PI/PO → CPI)
*   End-to-end integration testing

##### 🗄️ WS5: Data Migration

Owner: Data Migration Lead

*   Data volume analysis & archiving
*   Business Partner migration
*   Data cleansing & enrichment
*   Cutover execution

##### 🖥️ WS6: UX / Fiori

Owner: UX Lead

*   Fiori app mapping per process
*   Role-based launchpad design
*   Custom Fiori development (gaps)
*   User acceptance testing

##### ⚙️ WS7: Basis / Infrastructure

Owner: Basis Lead

*   System provisioning (sandbox, dev, QA, prod)
*   HANA sizing & database migration
*   Authorization concept redesign
*   Transport management

##### 👥 WS8: Change Management

Owner: OCM Lead

*   Impact assessment per role
*   Training curriculum design
*   Communication plan
*   Super-user network activation

### 2.2 RACI Matrix (Key Activities)

Activity

Program Mgr

Business Lead

Tech Lead

Integration

Data Lead

UX Lead

Process Catalogue

A

R

C

C

I

C

Custom Code Scan

I

C

R

C

I

I

Fit-to-Standard

A

R

C

C

I

C

Clean Core Classification

I

C

R

C

I

I

Code Remediation

A

C

R

C

I

I

BP Migration

A

C

C

C

R

I

Integration Testing

A

C

R

R

C

C

Go-Live Decision

R

A

C

C

C

C

## 3 Play 3 — Tooling & Environment Setup

### 3.1 Required Tools

Tool

Purpose

When Needed

License

**SAP Readiness Check**

Automated simplification impact analysis against SAP Note 2313884

Phase 1, Week 1

Free (SAP Launchpad)

**SAP Custom Code Migration (CCM)**

ATC + ABAP Cloud Readiness check across all Z/Y code

Phase 1-2

Part of S/4HANA license

**C16 AI Platform**

Automated code analysis, Clean Core classification, remediation code gen

Phase 1-3 (once connected)

C16 license

**SAP Business Scenario Recommendations**

AI-driven process mapping from ECC to S/4HANA best practices

Phase 2

Free (SAP Signavio)

**SAP Fiori Apps Library**

Map transactions to Fiori apps by process

Phase 2

Free (fioriappslibrary.hana.ondemand.com)

**SAP Data Services / SDT**

Data migration, transformation, and selective data transition

Phase 4

Licensed

**SAP Signavio Process Manager**

Process mining from ECC event logs + process documentation

Phase 1-2

Licensed (optional)

**Eclipse ADT**

ABAP development, CDS views, RAP development in S/4HANA

Phase 3-4

Free

### 3.2 System Landscape

graph LR subgraph "Current (ECC)" ECC\_P\["ECC Production"\] ECC\_Q\["ECC Quality"\] ECC\_D\["ECC Development"\] end subgraph "Target (S/4HANA 2025)" S4\_SB\["S/4 Sandbox  
(Week 1)"\] S4\_D\["S/4 Development  
(Week 4)"\] S4\_Q\["S/4 Quality  
(Week 20)"\] S4\_P\["S/4 Production  
(Go-Live)"\] end ECC\_P -->|"Data Extract"| S4\_SB ECC\_D -->|"Code Migration"| S4\_D S4\_D -->|"Transport"| S4\_Q S4\_Q -->|"Transport"| S4\_P ECC\_P -->|"Final Cutover"| S4\_P style S4\_P fill:#27ae60,color:#fff style ECC\_P fill:#e74c3c,color:#fff

**⚡ Critical First Step:** Provision the S/4HANA 2025 **Sandbox** system in Week 1. This is essential for Fit-to-Standard workshops — the business team needs to see and touch the standard Fiori apps to make informed decisions.

## 4 Play 4 — Business Process Catalogue

PHASE 1: DISCOVER

#### Objective: Build a complete inventory of all business processes running on ECC

**Duration:** 3 weeks | **Owner:** Business Process Lead | **Inputs:** Process owners, ST03N data, transaction logs

### 4.1 Activities

*   **Run ST03N (Workload Monitor)** — Extract last 12 months of transaction usage. Sort by execution count + unique users. This gives the definitive list of what ABB actually uses.
*   **Extract custom transaction list** — Query TADIR: SELECT \* FROM TADIR WHERE PGMID='R3TR' AND OBJECT='TRAN' AND OBJ\_NAME LIKE 'Z%'
*   **Interview process owners** — One session per module (MM, SD, FI/CO, PP, PM, QM, HR, WM). Document: process name, trigger event, frequency, SLA, pain points.
*   **Map to SAP Best Practice processes** — Use SAP's L1/L2/L3 process hierarchy (e.g., L1: Procure-to-Pay, L2: Purchase Order Processing, L3: Create Standard PO)
*   **Document process variants** — Custom pricing procedures, approval workflows, non-standard movement types, custom MRP strategies

### 4.2 Output Template — Process Catalogue

#

L1 Process

L2 Process

ECC Transaction

Custom?

Usage/Month

Users

Priority

P001

Procure-to-Pay

Create Purchase Order

ME21N

Standard + BAdI

~5,000

120

Critical

P002

Procure-to-Pay

Vendor Evaluation

ME61 + ZMM001

Custom Report

~200

15

Medium

P003

Order-to-Cash

Create Sales Order

VA01

Standard + User Exit

~8,000

200

Critical

P004

Record-to-Report

GL Posting

FB50

Standard

~12,000

50

Critical

... continue for all ~100-200 business processes ...

## 5 Play 5 — Custom Code Inventory

PHASE 1: DISCOVER

#### Objective: Complete inventory of all Z/Y custom objects with usage and dependency data

**Duration:** 4 weeks | **Owner:** Technical Architect | **Inputs:** TADIR, usage logs, code scanner

### 5.1 Extraction Queries

📋 TADIR Query — Extract All Custom Objects

```
SELECT obj_name, object, devclass, author, created_on
  FROM tadir
  WHERE pgmid = 'R3TR'
    AND obj_name LIKE 'Z%' OR obj_name LIKE 'Y%'
  ORDER BY object, obj_name.
```

**Expected categories:**

Object Type

Description

Typical Count (Large Enterprise)

PROG

Reports / Programs

2,000 - 8,000

CLAS

ABAP Classes

500 - 3,000

FUGR

Function Groups

300 - 1,500

TRAN

Transactions

500 - 2,000

TABL

Custom Tables

500 - 3,000

ENHS

Enhancement Implementations

100 - 500

VIEW

Database/Maintenance Views

200 - 1,000

WDYN

Web Dynpro Components

50 - 200

📋 Usage Data — Identify Active vs Dead Code

```
* Cross-reference with usage logs (ST03N or STAD)
* Mark programs with 0 executions in last 12 months as "CANDIDATE FOR RETIRE"
* Mark programs with < 10 executions/month as "LOW USAGE"
* Mark programs with > 100 executions/month as "BUSINESS CRITICAL"
```

**Typical distribution (Pareto rule):**

*   ~20% of custom objects handle ~80% of business transactions
*   ~30-40% of custom objects have ZERO usage in last 12 months (dead code)
*   ~10-15% are test/temp programs in $TMP that can be deleted

### 5.2 Custom Object Classification Template

Object

Type

Module

Usage/Mo

Clean Core Level

Simplification Hit?

Disposition

ZMM\_VENDOR\_RPT

PROG

MM

500

Level C

Yes — BP Migration

REWRITE

ZSD\_PRICING\_EXIT

FUGR

SD

8,000

Level B

No

REFIT

ZFI\_OLD\_GL\_RPT

PROG

FI

0

Level D

Yes — New GL

RETIRE

ZCL\_BP\_HELPER

CLAS

MM

200

Level A

No

RETAIN

... classify ALL custom objects ...

**💡 C16 Acceleration:** If C16 is connected to ABB's ECC, it can auto-classify **all custom objects** against SAP's cloudification catalog in hours — assigning Level A/B/C/D, identifying simplification hits, detecting deprecated API usage, and calculating Clean Core scores per program. This replaces 4-6 weeks of manual classification.

## 6 Play 6 — Interface & Integration Map

PHASE 1: DISCOVER

#### Objective: Map all inbound/outbound interfaces and assess S/4HANA compatibility

**Duration:** 3 weeks | **Owner:** Integration Lead | **Inputs:** PI/PO monitoring, IDoc logs (WE05), RFC destinations (SM59)

### 6.1 Interface Inventory Template

#

Interface Name

Direction

Technology

Frequency

Source/Target

S/4 Impact

Modernization Path

IF001

Vendor Master Sync

Outbound

IDoc (CREMAS)

Real-time

ECC → MDG

🔴 BP change

API\_BUSINESS\_PARTNER OData

IF002

PO Creation from SRM

Inbound

BAPI (PO\_CREATE1)

Batch/Daily

SRM → ECC

🟢 Compatible

Keep BAPI (Level B) or migrate to Purchase Order API

IF003

Invoice from Ariba

Inbound

IDoc (INVOIC02)

Real-time

Ariba → ECC

🟢 Compatible

Keep or migrate to Supplier Invoice API

IF004

GL Postings to DWH

Outbound

Custom RFC

Nightly

ECC → BW

🔴 BSEG → ACDOCA

CDS Analytical View extraction

IF005

Material Stock to WMS

Outbound

IDoc (WMMBXY)

Real-time

ECC → WMS

🔴 WM → EWM

EWM integration or API\_MATERIAL\_DOCUMENT

### 6.2 Integration Technology Migration Map

graph LR subgraph "ECC (Current)" IDOC\["IDocs"\] RFC\["Custom RFCs"\] BAPI\["BAPIs"\] PROXY\["XI/PI Proxies"\] FILE\["File Interfaces"\] end subgraph "S/4HANA 2025 (Target)" ODATA\["Released OData APIs  
(Level A)"\] SOAP\_S4\["SOAP APIs  
(Level B)"\] BAPI\_S4\["Stable BAPIs  
(Level B)"\] IDOC\_S4\["IDocs (maintained)"\] CPI\["SAP CPI / BTP Integration"\] end IDOC -->|"Review"| IDOC\_S4 IDOC -->|"Modernize"| ODATA RFC -->|"Replace"| ODATA BAPI -->|"Keep or Upgrade"| BAPI\_S4 BAPI -->|"Modernize"| ODATA PROXY -->|"Migrate"| CPI FILE -->|"Modernize"| CPI style ODATA fill:#27ae60,color:#fff style CPI fill:#3498db,color:#fff

## 7 Play 7 — Data Volume & Archiving Assessment

PHASE 1: DISCOVER

#### Objective: Assess data volumes, identify archiving opportunities, and size the HANA database

**Duration:** 2 weeks | **Owner:** Data Migration Lead | **Inputs:** DB02, table statistics

### 7.1 Critical Table Sizing

Table

Description

S/4 Change

Action

BSEG

Accounting Document Segments

Replaced by **ACDOCA** (Universal Journal)

Archive old data; only migrate open items + last N years

MSEG / MKPF

Material Document Items / Headers

Replaced by **MATDOC** (single table)

Automatic conversion during brownfield; archive pre-migration for bluefield

BSIK / BSAK

Vendor Open/Cleared Items

Replaced by **ACDOCA** views

Archive cleared items older than 3 years

BSID / BSAD

Customer Open/Cleared Items

Replaced by **ACDOCA** views

Archive cleared items older than 3 years

VBAK / VBAP

Sales Order Header/Items

Compatible but index changes

Archive completed orders older than 5 years

EKKO / EKPO

PO Header/Items

Compatible but field changes (BSTYP logic)

Archive completed POs older than 5 years

LFA1 / KNA1

Vendor / Customer Master

Replaced by **BP (BUT000, BUT020)**

CVI conversion; cleanse duplicates before migration

CDHDR / CDPOS

Change Documents

Compatible but massive volume

Archive aggressively — often 40%+ of total DB size

**🎯 HANA Sizing Rule of Thumb:** HANA in-memory footprint ≈ 30-50% of ECC database size after compression. Pre-migration archiving can reduce this by 40-60%. **Archive BEFORE migrating** — it's far cheaper to archive in ECC than in HANA.

#### 🚦 DECISION GATE 1 — Discovery Complete

**Week 6 | Decision Owner: Program Manager + Business Sponsor**

All criteria must be MET before proceeding to Phase 2:

1.  ✅ Business Process Catalogue complete with usage data (≥95% coverage)
2.  ✅ Custom code inventory extracted with object counts per type and module
3.  ✅ Interface map complete with all inbound/outbound interfaces catalogued
4.  ✅ Data volume report with archiving recommendations
5.  ✅ S/4HANA Sandbox system provisioned and accessible
6.  ✅ SAP Readiness Check report generated and reviewed
7.  ✅ Process owners identified and available for Fit-to-Standard workshops

**If NOT met:** Extend Phase 1 by 1-2 weeks. Do NOT start Fit-to-Standard workshops with incomplete data.

## 8 Play 8 — Fit-to-Standard Workshops

PHASE 2: ASSESS

#### Objective: For every business process, determine if S/4HANA 2025 standard covers it

**Duration:** 5 weeks | **Owner:** Business Process Lead | **Inputs:** Process Catalogue, S/4HANA Sandbox, Fiori Apps Library

### 8.1 Workshop Structure (Per Module)

Day

Activity

Participants

Output

**Day 1**

Current State Walk-through: Present each L2 process from catalogue. Business explains what they do and why.

Process Owners, Functional Consultants

Validated process list with business rules

**Day 2**

Standard Demo: Show S/4HANA standard Fiori apps on Sandbox for each process. Let business "test drive."

Process Owners, SAP Solution Architect

Initial reaction — FIT or GAP per process

**Day 3**

Requirement Decomposition: Break each process into R1-Rn discrete requirements. Classify each as FIT / FIT+CONFIG / FIT+EXTEND / GAP.

All

Requirement decomposition matrix

**Day 4**

Gap Deep Dive: For each GAP, explore: Can we change the process? Is there a BAdI? Can BTP handle it?

Process Owners, Technical Architect

Gap register with closure options

**Day 5**

Disposition Decision: For each custom object linked to these processes, assign RETIRE / REPLACE / REFIT / REWRITE / RETAIN.

All + Program Manager

Signed-off disposition per object

### 8.2 Fit-to-Standard Classification Guide

Classification

Definition

Action

Example

FIT

S/4HANA standard does this **exactly as-is**. No config or extension needed.

Adopt standard. Decommission custom.

Standard PO creation (ME21N / F0842)

FIT + CONFIG

Standard covers it, but **configuration** is needed (no code).

Configure in IMG. Decommission custom.

Custom approval workflow → Standard release strategy with BRF+

FIT + EXTENSION

Standard covers ~70-80%. A **compliant extension** (BAdI, Key User, RAP) fills the gap.

Adopt standard + develop Clean Core extension.

Standard vendor master + custom MSME classification field via E\_Supplier

GAP

No standard equivalent. Custom logic provides **unique business value**.

Document the gap. Build using Released APIs (RAP/CDS) or Side-by-Side on BTP.

Custom vendor blacklist check against government sanctions database

### 8.3 Module-by-Module Workshop Schedule

Week

Module

Key Processes

Expected Complexity

Week 1

**MM** (Materials Management)

Procurement, Vendor Master, Inventory, MRP

🔴 High — BP migration, WM→EWM, MSEG→MATDOC

Week 2

**SD** (Sales & Distribution)

Order Mgmt, Pricing, Delivery, Billing, Credit

🟡 Medium — Output mgmt, Credit mgmt changes

Week 3

**FI/CO** (Finance & Controlling)

GL, AP, AR, AA, CO-PA, Cost Center

🔴 High — BSEG→ACDOCA, New GL mandatory, CO-PA on ACDOCA

Week 4

**PP/PM/QM**

Production, Maintenance, Quality

🟢 Low-Medium — MRP Live, minor changes

Week 5

**WM/HR/Cross-Module**

Warehouse, HR, Workflow, Output, Custom Analytics

🔴 High — WM→EWM is major, HR→SuccessFactors potential

## 9 Play 9 — Clean Core Classification

PHASE 2: ASSESS

#### Objective: Classify every custom object by Clean Core compliance level

**Duration:** 4 weeks | **Owner:** Technical Architect | **Inputs:** Custom code inventory, ATC results, C16 analysis

### 9.1 Clean Core Level Definitions

Level

Meaning

What It Uses

Where It Runs

Target State

Level A

**Cloud-Ready**

Only Released APIs (CDS views, RAP BOs, released BAPIs)

S/4HANA Cloud, Public Edition, Private Edition, On-Premise

✅ Keep as-is — ideal state

Level B

**Classic API**

Stable BAPIs, REUSE\_ALV, CL\_SALV\_TABLE, classic FMs

S/4HANA On-Premise, Private Edition (NOT Cloud Public)

⚠️ Acceptable for on-prem; modernize for cloud path

Level C

**Non-Compliant**

Direct SELECT on standard tables, unreleased APIs

May work on S/4HANA on-prem but risky — no stability guarantee

🔴 Must remediate: replace direct DB access with CDS/API

Level D

**Critical**

Deprecated/removed objects, standard code modifications, EXEC SQL

WILL BREAK on S/4HANA

🚨 Mandatory rewrite or retire — blocking items

### 9.2 Classification Process

flowchart TD A\["Custom Object  
(Z/Y)"\] --> B{"Run ATC with  
ABAP\_CLOUD\_READINESS"} B -->|"0 findings"| C\["Level A  
Cloud-Ready ✅"\] B -->|"Findings found"| D{"Check each  
dependency"} D -->|"All Level B"| E\["Level B  
Classic ⚠️"\] D -->|"Any Level C"| F\["Level C  
Non-Compliant 🔴"\] D -->|"Deprecated/Removed"| G\["Level D  
Critical 🚨"\] F --> H{"Check SAP  
Simplification List"} G --> H H -->|"Object removed  
in S/4HANA"| I\["BLOCKER —  
Must fix before migration"\] H -->|"Object changed  
but available"| J\["WARNING —  
Adapt during migration"\] style C fill:#27ae60,color:#fff style E fill:#3498db,color:#fff style F fill:#e67e22,color:#fff style G fill:#e74c3c,color:#fff style I fill:#e74c3c,color:#fff

### 9.3 Mass Classification — How To

Method

Tool

Speed

Accuracy

Best For

**Manual ATC**

SE80 → ATC → ABAP\_CLOUD\_READINESS

~5 min/object

High

Small volumes (<100 objects)

**SAP Custom Code Migration**

SAP Readiness Check + CCM Worklist

Batch — hours

High

Full landscape scan

**C16 Automated Analysis**

C16 Platform (once connected)

~2 min/object fully analyzed

Very High (3-layer engine)

Deep analysis with remediation code

**🎯 Target Distribution (After Remediation):**  
Level A: ≥60% of surviving custom code  
Level B: ≤30% (acceptable for S/4HANA on-prem)  
Level C: ≤10% (justified exceptions with documented risk acceptance)  
Level D: **0%** (all must be remediated or retired)

## 10 Play 10 — Gap Analysis & Disposition

PHASE 2: ASSESS

#### Objective: Document all gaps and assign a disposition to every custom object

**Duration:** 4 weeks | **Owner:** Business Process Lead + Technical Architect | **Inputs:** FtS workshop output, Clean Core classification

### 10.1 The 5 Rs — Disposition Framework

Disposition

When to Apply

Effort

Clean Core Impact

Typical %

RETIRE

Standard S/4HANA replaces it entirely, OR zero usage in 12 months

Low — decommission + user training

Eliminates technical debt

**30-40%**

REPLACE

Standard covers ≥90% with minor configuration

Low-Medium — configure + test

Moves to Level A (all standard)

**15-25%**

REFIT

Standard covers 70-89%, needs BAdI/Key User extension

Medium — develop compliant extension

Level A extension on Level A standard

**15-20%**

REWRITE

Logic must stay custom but code is Level C/D

High — full rewrite using Released APIs

Moves from C/D → A

**10-15%**

RETAIN

Already Level A/B, no simplification impact, business critical

Low — validate + regression test

Already compliant

**10-15%**

### 10.2 Gap Register Template

#

Gap Description

Module

Business Impact

Complexity

Closure Strategy

Owner

G001

MSME vendor classification (custom Z-field on LFA1)

MM

Regulatory compliance

Medium

Extension field via E\_Supplier CDS view + custom RAP BO

Tech Lead

G002

Government blacklist check for vendors

MM

Compliance critical

High

Side-by-Side BTP app calling external API + BAdI in S/4

Integration Lead

G003

Custom PO approval based on material group

MM

Procurement control

Low

BRF+ decision table (standard S/4HANA feature)

Business Lead

G004

Custom credit scoring formula

SD

Revenue protection

Medium

SAP Credit Management (FSCM) + BAdI for formula

SD Lead

G005

Consolidated vendor report (30+ data sources)

MM

Operational dashboard

High

CDS Analytical View + Fiori Analytical List Page

Tech Lead

G006

Custom WM bin strategy

WM

Warehouse efficiency

High

Embedded EWM with custom storage type determination BAdI

WM Lead

## 11 Play 11 — Simplification List Impact Assessment

PHASE 2: ASSESS

#### Objective: Cross-reference all custom code against SAP Note 2313884 (Simplification List)

**Duration:** 3 weeks | **Owner:** Technical Architect | **Inputs:** SAP Readiness Check report, custom code inventory

### 11.1 Top 15 Simplification Items Impacting ABB

#

Simplification Item

Impact

Affected Area

Action Required

S1

**Business Partner Mandatory**  
Vendor (LFA1) & Customer (KNA1) → BP (BUT000)

🔴 Critical

All vendor/customer programs

All custom code referencing LFA1/KNA1 must use BP CDS views (I\_Supplier, I\_Customer) or BP BAPIs

S2

**Material Ledger Mandatory**  
Actual costing always active

🟡 Medium

FI/CO reporting

Review material valuation reports; MBEW structure changes

S3

**Universal Journal (ACDOCA)**  
BSEG/BSIS/BSAS/BSIK/BSAK/BSID/BSAD → ACDOCA

🔴 Critical

All FI reports reading BSEG

Replace direct BSEG reads with ACDOCA or CDS views (I\_JournalEntry)

S4

**Material Document (MATDOC)**  
MSEG/MKPF → MATDOC

🔴 Critical

Inventory reports, movement type analysis

Replace MSEG reads with MATDOC or CDS view (I\_MaterialDocumentItem)

S5

**New Asset Accounting**  
Classic AA not available

🟡 Medium

FI-AA custom reports

Review ANLP/ANLC access; use New AA CDS views

S6

**Output Management**  
NACE condition-based → BRF+ / Output Management

🟡 Medium

SD/MM output programs

Review custom output types; migrate to new OM or keep NACE (still works)

S7

**Credit Management**  
FD32/SAP Credit → FSCM Credit Management

🟡 Medium

Custom credit checks

Migrate custom credit logic to FSCM + BAdI

S8

**WM → EWM**  
LE-WM decommissioned

🔴 Critical

All WM custom code

Full migration to Embedded EWM

S9

**CO-PA on ACDOCA**  
Costing-based CO-PA → Account-based on ACDOCA

🟡 Medium

CO-PA custom reports, allocations

Review CE1xxxx/CE2xxxx tables; migrate to ACDOCA analytics

S10

**MRP Live (ppMRP)**  
Classic MRP → MRP Live (HANA-optimized)

🟢 Low

Custom MRP enhancements

Validate custom MRP exits work with ppMRP; performance test

S11

**Batch Management Rework**  
Minor batch table changes

🟢 Low

Custom batch programs

Verify custom batch logic; minor field changes

S12

**Profit Center Accounting**  
Integrated into Universal Journal

🟡 Medium

Custom PCA reports

Replace GLPCA/FAGLFLEXT access with ACDOCA CDS views

S13

**Product Hierarchy Length**  
PRDHA field length change (18→40)

🟢 Low

MM custom programs using PRDHA

Check type declarations; may need length adjustment

S14

**Message Control Changes**  
Some message IDs renumbered

🟢 Low

Custom error handling

Run code scan for affected message IDs

S15

**CDS View Deprecations**  
Some early CDS views deprecated in favor of new ones

🟢 Low

Custom code using CDS views

Check @VDM.lifecycle.status annotation; replace deprecated views

#### 🚦 DECISION GATE 2 — Assessment Complete

**Week 12 | Decision Owner: Program Manager + Technical Architect + Business Sponsor**

1.  ✅ Fit-to-Standard workshops completed for ALL modules (signed off by process owners)
2.  ✅ Every custom object has a disposition: RETIRE / REPLACE / REFIT / REWRITE / RETAIN
3.  ✅ Clean Core classification complete (Level A/B/C/D per object)
4.  ✅ Gap register complete with closure strategy for each gap
5.  ✅ Simplification impact assessed — all Level D (BLOCKER) items have remediation plans
6.  ✅ Migration approach confirmed (Greenfield / Brownfield / Bluefield)
7.  ✅ Remediation effort estimated and resource plan approved

**Key Decision:** Approve remediation budget and resource allocation. If gaps are too large, consider narrowing scope (phased module rollout).

## 12 Play 12 — Quick Wins (Retire & Replace)

PHASE 3: REMEDIATE

#### Objective: Eliminate 30-40% of custom objects through retirement and standard replacement

**Duration:** 4 weeks | **Owner:** Technical Architect + Business Process Lead | **Inputs:** Disposition register

### 12.1 Retirement Checklist

*   **Zero-usage programs (RETIRE)** — Programs with 0 executions in 12+ months. Confirm with process owner → mark as retired → document in decommission log.
*   **Test/Temp programs** — Objects in $TMP package, names containing TEST, TEMP, OLD, BACKUP, COPY. Delete directly.
*   **Superseded programs** — Where a newer Z-program already replaces an older version (e.g., ZMM\_VENDOR\_RPT\_V2 supersedes ZMM\_VENDOR\_RPT). Retire the older version.
*   **Standard-replaceable** — Where a Fiori app or standard transaction provides identical functionality. Train users on standard → retire custom.

### 12.2 Standard Replacement Map

Custom Object Type

S/4HANA Standard Replacement

Fiori App

Action

Custom vendor list report

Manage Supplier master data

F1739

Train users on F1739 → Retire custom

Custom PO status report

Monitor Purchase Orders

F0842 / F2217

Train users → Retire custom

Custom sales order report

Manage Sales Orders

F1814

Train users → Retire custom

Custom GL line item report

Display Line Items in General Ledger

F0717

Train users → Retire custom

Custom material stock overview

Stock - Single Material

F0842A

Train users → Retire custom

Custom open items AP

Manage Supplier Line Items

F0708

Train users → Retire custom

Custom open items AR

Manage Customer Line Items

F0711

Train users → Retire custom

Custom maintenance order list

Manage Maintenance Orders

F1570

Train users → Retire custom

## 13 Play 13 — Business Partner Migration

PHASE 3: REMEDIATE

#### Objective: Migrate all vendor/customer master data to Business Partner model

**Duration:** 8 weeks | **Owner:** Data Migration Lead | **Inputs:** LFA1/KNA1 data, CVI config, custom field register

**🚨 #1 Risk Item:** Business Partner migration is the single most impactful change in S/4HANA. It affects EVERY module that touches vendors or customers — MM, SD, FI, PM, QM. Start this work EARLY.

### 13.1 BP Migration Architecture

flowchart LR subgraph "ECC Tables" LFA1\["LFA1  
Vendor General"\] LFB1\["LFB1  
Vendor Company"\] LFM1\["LFM1  
Vendor Purchasing"\] KNA1\["KNA1  
Customer General"\] KNB1\["KNB1  
Customer Company"\] KNVV\["KNVV  
Customer Sales"\] end subgraph "CVI Sync" CVI\["Customer-Vendor  
Integration"\] end subgraph "S/4HANA BP Tables" BUT000\["BUT000  
BP General"\] BUT020\["BUT020  
BP Addresses"\] BUT100\["BUT100  
BP Roles"\] BUPLA\["BP Links to  
Supplier/Customer"\] end subgraph "CDS Views (Clean Core)" I\_SUP\["I\_Supplier"\] I\_CUS\["I\_Customer"\] I\_BP\["I\_BusinessPartner"\] end LFA1 --> CVI LFB1 --> CVI KNA1 --> CVI KNB1 --> CVI CVI --> BUT000 CVI --> BUT020 CVI --> BUT100 BUT000 --> I\_BP BUT000 --> I\_SUP BUT000 --> I\_CUS style CVI fill:#e67e22,color:#fff style I\_SUP fill:#27ae60,color:#fff style I\_CUS fill:#27ae60,color:#fff style I\_BP fill:#27ae60,color:#fff

### 13.2 BP Migration Steps

Step 1 — Weeks 1-2

##### Data Cleansing

Identify and resolve: duplicate vendors/customers, inactive records, missing mandatory fields (name, country, tax number). Clean in ECC BEFORE migration.

Step 2 — Week 3

##### CVI Configuration

Configure Customer-Vendor Integration: number range mapping, BP role assignment (Vendor = FLVN01, Customer = FLCU01), grouping categories.

Step 3 — Weeks 3-4

##### Custom Field Migration

Map ZZ/YY append fields from LFA1/KNA1 to BP extension fields. Options: Key User field extensibility, E\_Supplier/E\_Customer CDS extensions, custom RAP BO.

Step 4 — Weeks 4-6

##### Code Remediation

Update all custom programs that read LFA1/KNA1:  
❌ SELECT \* FROM lfa1 WHERE lifnr = @lv\_vendor  
✅ SELECT \* FROM I\_Supplier WHERE Supplier = @lv\_vendor

Step 5 — Weeks 6-7

##### Dry Run & Validation

Run BP migration in sandbox. Validate: all vendors/customers migrated, no data loss, custom fields populated correctly, all dependent programs work.

Step 6 — Week 8

##### Production Cutover Plan

Document exact cutover steps: freeze master data → run CVI sync → validate counts → activate BP → test critical processes.

## 14 Play 14 — Custom Code Remediation Sprints

PHASE 3: REMEDIATE

#### Objective: Rewrite/Refit all Level C/D custom objects to Level A/B using Released APIs

**Duration:** 12 weeks (6 two-week sprints) | **Owner:** Technical Architect | **Inputs:** Disposition register, ATC findings

### 14.1 Sprint Planning — Prioritization Matrix

Priority

Criteria

Sprint

Examples

🔴 P1 — Blockers

Level D objects (will break), simplification blockers

Sprint 1-2

Programs using removed tables, deprecated BAPIs, standard code modifications

🟠 P2 — Critical Business

Level C objects used >500 times/month

Sprint 2-3

High-usage vendor/customer reports, PO enhancement, pricing exits

🟡 P3 — Standard Remediation

Level C objects, medium usage

Sprint 3-4

Custom reports, data extracts, periodic batch programs

🟢 P4 — Optimization

Level B objects targeted for Level A upgrade

Sprint 5-6

REUSE\_ALV → CL\_SALV\_TABLE → RAP Fiori, classic OData → Released API

### 14.2 Common Remediation Patterns

Pattern 1: Direct Table SELECT → CDS View

**Before (Level C):**

```
SELECT matnr, mtart, matkl, mbrsh
  FROM mara
  INTO TABLE @DATA(lt_materials)
  WHERE mtart = 'FERT'.
```

**After (Level A):**

```
SELECT Material, MaterialType, MaterialGroup, IndustrySector
  FROM I_Product
  INTO TABLE @DATA(lt_materials)
  WHERE MaterialType = 'FERT'.
```

**Key CDS View Replacements:**

ECC Table

S/4 CDS View

Notes

LFA1

I\_Supplier

BP-based; includes general + address data

KNA1

I\_Customer

BP-based

EKKO

I\_PurchaseOrder

Header-level with associations

EKPO

I\_PurchaseOrderItem

Line-item level

VBAK

I\_SalesOrder

Header-level

MARA

I\_Product

Material master general

MARC

I\_ProductPlant

Plant-level material data

BKPF/BSEG

I\_JournalEntry

Universal Journal (ACDOCA-based)

MSEG

I\_MaterialDocumentItem

MATDOC-based

Pattern 2: BDC / CALL TRANSACTION → Released BAPI / API

**Before (Level D):**

```
CALL TRANSACTION 'XK01' USING lt_bdcdata MODE 'N' UPDATE 'A'.
```

**After (Level A):**

```
* Use API_BUSINESS_PARTNER OData service or BAPI_BUPA_CREATE_FROM_DATA
DATA(lo_bp) = cl_bp_create_api=>create(
  iv_category  = '2'    "Organization
  iv_grouping  = 'FLVN01'
  is_data      = ls_bp_data
).
```

**Key BDC → API Replacements:**

BDC Transaction

Released API

XK01 (Create Vendor)

API\_BUSINESS\_PARTNER / BAPI\_BUPA\_CREATE\_FROM\_DATA

ME21N (Create PO)

BAPI\_PO\_CREATE1 (Level B) / API\_PURCHASEORDER\_PROCESS\_SRV (Level A)

VA01 (Create SO)

BAPI\_SALESORDER\_CREATEFROMDAT2 (Level B) / API\_SALES\_ORDER\_SRV (Level A)

MIGO (Goods Receipt)

BAPI\_GOODSMVT\_CREATE (Level B) / API\_MATERIAL\_DOCUMENT\_SRV (Level A)

FB50 (GL Posting)

BAPI\_ACC\_DOCUMENT\_POST (Level B) / API\_JOURNALENTRY (Level A)

Pattern 3: REUSE\_ALV → Fiori Elements (List Report)

**Before (Level B):**

```
CALL FUNCTION 'REUSE_ALV_GRID_DISPLAY'
  EXPORTING
    i_callback_program = sy-repid
    it_fieldcat        = lt_fcat
  TABLES
    t_outtab           = lt_output.
```

**After (Level A):**

Replace with a **RAP-based CDS View** + **Fiori Elements List Report**:

1.  Create Interface CDS View (ZI\_\*) with @UI annotations
2.  Create Consumption CDS View (ZC\_\*) with projections
3.  Create Service Definition + Service Binding
4.  Fiori Elements auto-generates the UI from annotations — zero frontend code

Pattern 4: Custom Enhancement (User Exit) → Released BAdI

**Before (Level C/D):** CMOD user exits, classic enhancement points, standard code modifications

**After (Level A):** Find the Released BAdI for the same process area:

Area

Legacy Exit

Released BAdI

Purchase Order

EXIT\_SAPMM06E\_\*

ME\_PROCESS\_PO\_CUST (check INTERNAL flag first!)

Sales Order

USEREXIT\_\* in MV45A\*

SD\_SALES\_DOCUMENT\_PROCESS

Goods Receipt

MB\_MIGO\_BADI

Check release status via ATC

Invoice Verification

EXIT\_SAPLMRMP\_\*

INVOICE\_UPDATE (check release)

**⚠️ CRITICAL:** Before implementing ANY BAdI, check SXS\_ATTR.INTERNAL flag. If INTERNAL = 'X', it is SAP Internal and must NOT be used. Only use BAdIs where INTERNAL is empty/blank.

### 14.3 Sprint Execution Template

Sprint Activity

Day 1-2

Day 3-7

Day 8-9

Day 10

**Focus**

Analysis & Design

Development

Testing

Review & Deploy

**Activities**

C16 analysis of target programs, identify CDS/API replacements, design solution

Rewrite code using Released APIs, develop Clean Core extensions, unit test

Integration test, regression test against ECC baseline, performance test

Code review, deploy to QA, sprint retrospective

**Output**

Technical design per program

Remediated code in Dev

Test results, defect log

Sprint completion report

## 15 Play 15 — Interface Modernization

PHASE 3: REMEDIATE

#### Objective: Modernize all interfaces to use Released APIs and Clean Core patterns

**Duration:** 8 weeks | **Owner:** Integration Lead

### 15.1 Interface Modernization Decision Tree

flowchart TD A\["Existing Interface"\] --> B{"Interface Type?"} B -->|"IDoc"| C{"Still needed?"} B -->|"Custom RFC"| D{"Business critical?"} B -->|"BAPI"| E{"Released in S/4?"} B -->|"File-based"| F\["Migrate to SAP CPI  
or BTP Integration"\] C -->|"Yes"| G{"BP/MATDOC  
impacted?"} C -->|"No"| H\["RETIRE"\] G -->|"Yes"| I\["Update IDoc segments  
for S/4 structure"\] G -->|"No"| J\["RETAIN as-is"\] D -->|"Yes"| K\["Replace with  
Released OData API"\] D -->|"No"| H E -->|"Yes"| L\["RETAIN  
(Level B OK for on-prem)"\] E -->|"No"| K style H fill:#95a5a6,color:#fff style J fill:#27ae60,color:#fff style K fill:#3498db,color:#fff style L fill:#27ae60,color:#fff

### 15.2 Key API Replacements

ECC Interface Pattern

S/4HANA Released API (Level A)

OData Service

Vendor master IDoc (CREMAS)

Business Partner API

API\_BUSINESS\_PARTNER

Customer master IDoc (DEBMAS)

Business Partner API

API\_BUSINESS\_PARTNER

Purchase Order BAPI

Purchase Order API

API\_PURCHASEORDER\_PROCESS\_SRV

Sales Order BAPI

Sales Order API

API\_SALES\_ORDER\_SRV

Material Document BAPI

Material Document API

API\_MATERIAL\_DOCUMENT\_SRV

Journal Entry BAPI

Journal Entry API

API\_JOURNALENTRYITEMBASIC\_SRV

Supplier Invoice BAPI

Supplier Invoice API

API\_SUPPLIERINVOICE\_PROCESS\_SRV

Production Order RFC

Production Order API

API\_PRODUCTION\_ORDER\_2\_SRV

## 16 Play 16 — Forms Migration

PHASE 3: REMEDIATE

#### Objective: Migrate all output forms to supported technologies

**Duration:** 6 weeks | **Owner:** Technical Architect

Current Technology

Clean Core Level

Target Technology

Migration Path

**SAPscript**

Level D

Adobe Forms (SFP)

Manual redesign or automated conversion (C16 has SmartForm→Adobe pipeline)

**SmartForms**

Level D

Adobe Forms (SFP)

C16 automated conversion: download XML → parse → generate XDP → deploy

**Adobe Forms (SFP)**

Level A

Keep as-is

Already on target technology

**Custom WRITE/WRITE AT reports**

Level C

CL\_SALV\_TABLE or Fiori

Rewrite using OO ALV or RAP List Report

#### 🚦 DECISION GATE 3 — Remediation Complete

**Week 28 | Decision Owner: Technical Architect + Program Manager**

1.  ✅ All Level D objects either RETIRED or REWRITTEN (zero Level D remaining)
2.  ✅ All Level C objects remediated to Level B or better (or documented exception)
3.  ✅ BP migration dry run successful — all vendors/customers converted, custom fields mapped
4.  ✅ All P1 (blocker) simplification items resolved
5.  ✅ All critical interfaces tested with S/4 APIs
6.  ✅ Forms migration complete — all SAPscript/SmartForms converted
7.  ✅ Regression test suite passes at ≥95% for remediated code
8.  ✅ Clean Core distribution: ≥60% Level A, ≤30% Level B, ≤10% Level C, 0% Level D

## 17 Play 17 — System Build & Configuration

PHASE 4: MIGRATE

#### Objective: Build the S/4HANA 2025 target system with all configuration and remediated code

**Duration:** 8 weeks | **Owner:** Basis Lead + All Module Leads

### 17.1 System Build Sequence

Week 1-2

##### System Installation & Foundation

Install S/4HANA 2025 with HANA 2.0. Apply latest SPS. Configure: company codes, plants, purchasing orgs, sales orgs, fiscal year variants, currency settings.

Week 2-3

##### Master Data Configuration

BP configuration (groupings, roles, number ranges). Material type mapping. Account determination. Tax configuration.

Week 3-5

##### Module Configuration

MM: Pricing procedures, release strategies, output management.  
SD: Pricing, availability check, delivery scheduling, billing.  
FI/CO: Chart of accounts, cost elements, cost centers, profit centers.  
PP/PM: Work centers, routings, maintenance plans.

Week 5-6

##### Custom Code Deployment

Deploy all REWRITE/REFIT/RETAIN objects. Transport remediated code from Dev → QA. Activate CDS views, RAP BOs, Fiori apps. Publish service bindings.

Week 6-7

##### Fiori Launchpad Setup

Configure role-based Fiori Launchpad. Assign tiles to business roles. Set up spaces & pages. Configure SAP Start.

Week 7-8

##### Authorization Redesign

Redesign role concept for Fiori. Create composite roles. Map old PFCG roles to new Fiori tile assignments. Test critical authorizations.

## 18 Play 18 — Data Migration & Cutover

PHASE 4: MIGRATE

#### Objective: Migrate all master and transactional data to S/4HANA 2025

**Duration:** 6 weeks (3 dry runs + 1 final) | **Owner:** Data Migration Lead

### 18.1 Data Migration Scope

Data Category

Source

Target

Tool

Complexity

**Vendor Master**

LFA1/LFB1/LFM1

BUT000/BUT020 (BP)

CVI Sync + Migration Cockpit

🔴 High

**Customer Master**

KNA1/KNB1/KNVV

BUT000/BUT020 (BP)

CVI Sync + Migration Cockpit

🔴 High

**Material Master**

MARA/MARC/MARD

MARA/MARC/MARD (compatible)

Migration Cockpit (LTMC)

🟡 Medium

**GL Balances**

BSEG/GLT0/FAGLFLEXT

ACDOCA

Migration Cockpit

🔴 High

**Open Items (AP/AR)**

BSIK/BSID

ACDOCA

Migration Cockpit

🟡 Medium

**Purchase Orders (open)**

EKKO/EKPO

EKKO/EKPO (compatible)

Migration Cockpit

🟢 Low

**Sales Orders (open)**

VBAK/VBAP

VBAK/VBAP (compatible)

Migration Cockpit

🟢 Low

**Asset Master**

ANLA/ANLP

ANLA/ACDOCA (New AA)

Migration Cockpit

🟡 Medium

**Inventory**

MARD (stock)/MSEG

MARD/MATDOC

Migration Cockpit + Physical Inventory

🟡 Medium

**Custom Z-Tables**

Z\*/Y\* tables

Z\*/Y\* tables (if RETAINED)

RFC\_READ\_TABLE + insert or SDT

🟢 Low

### 18.2 Cutover Runbook Template

Step

Activity

Duration

Owner

Validation

1

Freeze ECC production (stop user access)

T+0

Basis

No new postings in BKPF

2

Final ECC data backup

T+2h

Basis

Backup log clean

3

Extract master data delta (since last dry run)

T+4h

Data Lead

Record counts match

4

Extract open transactional documents

T+6h

Data Lead

Open PO/SO/Invoice counts match

5

Load master data to S/4HANA

T+10h

Data Lead

BP count = LFA1 + KNA1 count

6

Load GL balances + open items

T+14h

FI Lead

Trial balance matches ECC

7

Load open documents

T+18h

Module Leads

Open PO/SO counts match

8

Run reconciliation reports

T+20h

All

Zero variances on all checks

9

Smoke test — 10 critical processes

T+22h

Business

All 10 pass

10

GO / NO-GO decision

T+24h

Sponsor

—

11

Open S/4HANA to users

T+24h

Basis

First user login confirmed

## 19 Play 19 — Testing Strategy (8 Layers)

PHASE 4: MIGRATE

#### Objective: Comprehensive testing to ensure zero business disruption at go-live

**Duration:** 10 weeks (overlapping) | **Owner:** Test Manager

Layer

Test Type

Scope

Owner

Environment

Pass Criteria

L1

**Unit Testing**

Individual programs, classes, CDS views

Developers

Dev

100% syntax-free, ATC clean

L2

**ABAP Unit / API Testing**

Automated test classes for remediated code

Developers

Dev

100% test pass rate

L3

**Integration Testing**

End-to-end process flows (e.g., PR→PO→GR→Invoice→Payment)

Functional

QA

All process flows complete without error

L4

**Interface Testing**

All inbound/outbound interfaces with external systems

Integration

QA

All interfaces send/receive correct data

L5

**Data Migration Testing**

Reconcile migrated data vs ECC source

Data Lead

QA

Zero variance on all reconciliation checks

L6

**User Acceptance Testing**

Business scenarios executed by actual end users

Business

QA

Business sign-off on all critical scenarios

L7

**Performance Testing**

Load testing on HANA: MRP Live, month-end close, mass postings

Basis/Tech

Performance

All critical transactions <3 second response

L8

**Cutover Rehearsal**

Full dry run of the cutover runbook with production-like data

All

Pre-Prod

Cutover completes within window; all checks pass

## 20 Play 20 — Training & Change Management

PHASE 4: MIGRATE

#### Objective: Prepare ABB's workforce for the new S/4HANA experience

**Duration:** 8 weeks (parallel to testing) | **Owner:** OCM Lead

### 20.1 Training Curriculum

Audience

Training Content

Format

Duration

**End Users (General)**

Fiori Launchpad navigation, My Inbox, search, personalization, SAP Start

E-learning + Live demo

2 hours

**MM Users**

New PO Fiori apps (F0842), Manage Suppliers (F1739), BP for vendors, Inventory apps

Instructor-led + Hands-on

1 day

**SD Users**

Sales Order apps (F1814), Delivery apps, Billing, Credit Management changes

Instructor-led + Hands-on

1 day

**FI/CO Users**

Universal Journal (ACDOCA), New GL, Fiori Finance apps, CO-PA changes

Instructor-led + Hands-on

1 day

**WM Users**

Embedded EWM processes, new warehouse Fiori apps, RF framework changes

Instructor-led + Hands-on

2 days

**Super Users**

Advanced Fiori, Key User Extensibility, custom field creation, analytical apps, BRF+

Workshop

2 days

**IT / ABAP Team**

RAP development, CDS views, Clean Core practices, ADT Eclipse, Fiori Elements

Technical workshop

5 days

**Basis Team**

HANA administration, Fiori Launchpad config, CDS transport, authorization redesign

Technical workshop

3 days

## 21 Play 21 — Go-Live & Hypercare

PHASE 4: MIGRATE

#### Objective: Smooth go-live with rapid issue resolution during hypercare

**Duration:** 2 weeks hypercare | **Owner:** Program Manager

### 21.1 Go-Live Readiness Checklist

*   **Data migration complete** — Reconciliation reports all show zero variance
*   **All critical interfaces active** — Confirmed data flowing to/from external systems
*   **UAT sign-off** — All process owners have signed acceptance
*   **Cutover rehearsal successful** — Completed within time window
*   **Training complete** — ≥90% of target users completed relevant training
*   **Support team briefed** — Hypercare war room staffed, escalation paths defined
*   **Rollback plan documented** — Clear steps to revert to ECC if catastrophic failure
*   **Communication sent** — All users notified of go-live date, new login procedures, support contacts

### 21.2 Hypercare Support Model

Severity

Definition

Response Time

Resolution Target

Escalation

🔴 **P1 — Critical**

Business stopped. Cannot process orders, post invoices, or run production.

15 minutes

4 hours

War room, all leads

🟠 **P2 — High**

Major function impaired. Workaround available but painful.

1 hour

8 hours

Module lead + Tech lead

🟡 **P3 — Medium**

Minor function impaired. Business can continue with workaround.

4 hours

2 business days

Assigned developer

🟢 **P4 — Low**

Cosmetic issue, enhancement request, minor inconvenience.

Next business day

Next sprint

Backlog

#### 🚦 DECISION GATE 4 — Go-Live Readiness

**Go-Live minus 3 days | Decision Owner: Business Sponsor + Program Manager**

1.  ✅ Cutover rehearsal completed within planned window
2.  ✅ Data reconciliation: zero variance on GL balances, open items, master data counts
3.  ✅ All P1/P2 defects from UAT resolved
4.  ✅ All critical interfaces tested end-to-end in pre-production
5.  ✅ Training completion rate ≥90%
6.  ✅ Hypercare support team confirmed and briefed
7.  ✅ Rollback plan reviewed and approved
8.  ✅ Business sponsor formal GO/NO-GO decision documented

**GO = proceed with production cutover**

**NO-GO = postpone. Identify blocking items. Re-schedule within 2-4 weeks.**

## 22 Play 22 — Risk Register & Mitigation

#

Risk

Probability

Impact

Severity

Mitigation

Owner

R1

**BP migration data quality** — Duplicate vendors/customers cause merge conflicts

High

Critical

🔴 Critical

Start data cleansing in ECC 3 months before migration. Run MDS deduplication.

Data Lead

R2

**Custom code volume underestimated** — More Level D objects than expected

Medium

High

🟠 High

Connect C16 early for automated classification. Scale remediation team if needed.

Tech Lead

R3

**WM → EWM scope creep** — Warehouse migration more complex than planned

High

High

🔴 Critical

Consider phased approach: go-live without EWM, migrate WM later. Or engage SAP EWM expert.

WM Lead

R4

**Change resistance** — Users reject Fiori UX, demand SAP GUI back

Medium

Medium

🟡 Medium

Early Fiori demos in Phase 1. Super-user champions. Side-by-side SAP GUI for transition.

OCM Lead

R5

**Integration failures at cutover** — External systems don't connect to S/4

Medium

Critical

🟠 High

Full interface testing in pre-production. Engage external system teams early.

Integration Lead

R6

**HANA performance issues** — Custom code not optimized for HANA

Low

High

🟡 Medium

Run performance analysis (P1-P5 anti-patterns) during remediation. Use CDS pushdown.

Tech Lead

R7

**Timeline slippage** — Remediation takes longer than planned

Medium

Medium

🟡 Medium

Buffer 20% in each phase. Use C16 for automated remediation code generation.

Program Mgr

R8

**Key resource unavailability** — Process owners or developers leave mid-project

Low

High

🟡 Medium

Cross-train 2 resources per workstream. Document all decisions in this playbook.

Program Mgr

## A Appendix A — Templates & Checklists

### A.1 Per-Program Analysis Template (for C16 or manual analysis)

Field

Value

**Program Name**

\[Z\_PROGRAM\_NAME\]

**Module**

\[MM/SD/FI/CO/PP/PM/QM/WM/HR\]

**Lines of Code**

\[exact count\]

**Usage / Month**

\[from ST03N\]

**Active Users**

\[unique user count\]

**Clean Core Level**

\[A / B / C / D\]

**Simplification Hits**

\[list affected SAP Note items\]

**Tables Accessed**

\[list from source code\]

**BAPIs/APIs Used**

\[list\]

**Enhancements Used**

\[BAdI/User Exit names\]

**Fit-to-Standard**

\[FIT / FIT+CONFIG / FIT+EXTEND / GAP\]

**Standard Alternative**

\[Fiori App ID / Transaction / API name\]

**Disposition**

\[RETIRE / REPLACE / REFIT / REWRITE / RETAIN\]

**Remediation Effort**

\[Low / Medium / High\]

**Business Owner**

\[name + role\]

### A.2 Weekly Status Report Template

Section

Content

**Overall Status**

🟢 Green / 🟡 Yellow / 🔴 Red

**Phase**

\[Current phase and week number\]

**Completed This Week**

\[3-5 bullet points\]

**Planned Next Week**

\[3-5 bullet points\]

**Risks & Issues**

\[Top 3 active risks/issues with mitigation status\]

**Decisions Needed**

\[Any pending decisions from leadership\]

**Key Metrics**

Objects classified: X/Y | Objects remediated: X/Y | Tests passed: X/Y

## B Appendix B — SAP Transaction → Fiori App Mapping

Module

ECC Transaction

Description

Fiori App

App ID

MM

ME21N

Create Purchase Order

Create Purchase Order - Advanced

F0842

MM

ME23N

Display Purchase Order

Manage Purchase Orders

F2217

MM

ME51N

Create Purchase Requisition

Create Purchase Requisition

F1138

MM

ME53N

Display Purchase Requisition

Manage Purchase Requisitions

F1132

MM

MIGO

Goods Receipt/Issue

Post Goods Receipt for PO

F0857

MM

MIRO

Enter Incoming Invoice

Create Supplier Invoice

F0859

MM

XK01/BP

Create Vendor

Manage Supplier

F1739

MM

MM60

MRP Evaluation

Manage Stock

F1076

SD

VA01

Create Sales Order

Create Sales Orders

F1814

SD

VA03

Display Sales Order

Manage Sales Orders

F1815

SD

VL01N

Create Delivery

Create Outbound Delivery

F5765

SD

VF01

Create Billing Document

Create Billing Documents

F0861

SD

XD01/BP

Create Customer

Manage Customer

F2735

FI

FB50

Enter G/L Account Document

Post General Journal Entries

F0717

FI

FB60

Enter Vendor Invoice

Create Supplier Invoice

F0859

FI

FB70

Enter Customer Invoice

Create Customer Invoice

F3731

FI

F-28

Incoming Payment

Post Incoming Payments

F0742

FI

F-53

Outgoing Payment

Post Outgoing Payments

F0749

FI

FBL1N

Vendor Line Items

Manage Supplier Line Items

F0708

FI

FBL5N

Customer Line Items

Manage Customer Line Items

F0711

CO

KS01

Create Cost Center

Manage Cost Centers

F1607

PP

CO01

Create Production Order

Manage Production Orders

F2436

PM

IW31

Create Maintenance Order

Create Maintenance Order

F1570

QM

QA01

Create Inspection Lot

Manage Inspection Lots

F3266

## C Appendix C — Table Migration Reference

ECC Table

S/4HANA Table

CDS View (Level A)

Change Type

LFA1 (Vendor General)

BUT000 (BP General)

I\_Supplier

🔴 Replaced (BP)

LFB1 (Vendor Company)

BUT100 (BP Roles)

I\_SupplierCompany

🔴 Replaced (BP)

KNA1 (Customer General)

BUT000 (BP General)

I\_Customer

🔴 Replaced (BP)

KNB1 (Customer Company)

BUT100 (BP Roles)

I\_CustomerCompany

🔴 Replaced (BP)

BSEG (Accounting Segments)

ACDOCA (Universal Journal)

I\_JournalEntry

🔴 Replaced

BSIS/BSAS (GL Open/Clear)

ACDOCA

I\_GLAccountLineItem

🔴 Replaced

BSIK/BSAK (AP Open/Clear)

ACDOCA

I\_OperationalAcctgDocItem

🔴 Replaced

BSID/BSAD (AR Open/Clear)

ACDOCA

I\_OperationalAcctgDocItem

🔴 Replaced

GLT0 (GL Totals)

ACDOCA (aggregated)

I\_GLAccountBalance

🔴 Replaced

MSEG (Mat Doc Items)

MATDOC

I\_MaterialDocumentItem

🔴 Replaced

MKPF (Mat Doc Header)

MATDOC

I\_MaterialDocumentHeader

🔴 Replaced

EKKO (PO Header)

EKKO (compatible)

I\_PurchaseOrder

🟢 Compatible (minor field changes)

EKPO (PO Items)

EKPO (compatible)

I\_PurchaseOrderItem

🟢 Compatible

VBAK (SO Header)

VBAK (compatible)

I\_SalesOrder

🟢 Compatible

MARA (Material General)

MARA (compatible)

I\_Product

🟢 Compatible

MARC (Material Plant)

MARC (compatible)

I\_ProductPlant

🟢 Compatible (minor changes)

ANLP (Asset Periodic)

ACDOCA + ANLA

I\_FixedAsset

🟡 Changed (New AA)

CE1xxxx (CO-PA Actual)

ACDOCA

I\_ProfitabilitySegment

🔴 Replaced (ACDOCA)

GLPCA (Profit Center)

ACDOCA

I\_GLAccountLineItem

🔴 Replaced (ACDOCA)

**ABB ECC → S/4HANA 2025 Migration Playbook** | Version 1.0 | April 2026  
  
**Prepared by:** C16 AI Platform  
**Methodology:** SAP Activate, Clean Core, Fit-to-Standard  
**Status:** Strategic framework — ABB's ECC is not connected. All specific object counts, Clean Core levels, and simplification impacts should be verified once system access is provided.  
  
**Next Steps:**  
1\. Provision S/4HANA 2025 Sandbox  
2\. Connect C16 to ABB's ECC (RFC + ADT access)  
3\. Run SAP Readiness Check  
4\. Begin Phase 1: DISCOVER (Week 1)  
  
_This playbook is a living document. Update after each Decision Gate with actual findings and adjusted timelines._  
  
© 2026 C16 Platform. All data sourced from SAP standard documentation. Fiori App IDs sourced from SAP Fiori Apps Library. Simplification items sourced from SAP Note 2313884.