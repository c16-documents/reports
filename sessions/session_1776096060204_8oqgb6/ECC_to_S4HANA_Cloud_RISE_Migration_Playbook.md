  ECC → S/4HANA Cloud (RISE) Migration Playbook — C16 :root { --copper: #C97B3A; --rose: #B5567A; --purple: #6B3FA0; --dark-purple: #4A2D6E; --green: #27ae60; --blue: #3498db; --orange: #e67e22; --red: #e74c3c; --dark-red: #c0392b; --teal: #00897B; --body-bg: #e9ecef; --card-bg: #ffffff; --text: #2c3e50; --text-light: #5a6c7d; --border: #dee2e6; } \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, Arial, sans-serif; background: var(--body-bg); color: var(--text); line-height: 1.7; font-size: 15px; } .doc-wrapper { max-width: 1140px; margin: 30px auto; background: var(--card-bg); border-radius: 16px; box-shadow: 0 8px 40px rgba(0,0,0,0.10); overflow: hidden; } /\* ─── Header Banner ─── \*/ .header-banner { background: linear-gradient(135deg, var(--copper), var(--rose), var(--purple)); color: #fff; padding: 48px 48px 40px; position: relative; overflow: hidden; } .header-banner::before { content: ''; position: absolute; top: -50%; left: -50%; width: 200%; height: 200%; background: radial-gradient(circle at 30% 50%, rgba(255,255,255,0.06) 0%, transparent 60%); } .header-banner img { height: 50px; margin-bottom: 14px; border-radius: 8px; position: relative; } .header-banner h1 { font-size: 2.1em; font-weight: 700; margin-bottom: 6px; position: relative; } .header-banner .subtitle { font-size: 1.15em; opacity: 0.92; margin-bottom: 20px; position: relative; } .header-banner .badge-row { display: flex; gap: 10px; flex-wrap: wrap; position: relative; } .header-badge { background: rgba(255,255,255,0.18); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.25); padding: 6px 16px; border-radius: 20px; font-size: 0.88em; font-weight: 600; } /\* ─── KPI Cards ─── \*/ .kpi-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 16px; margin: 24px 0; } .kpi-card { background: linear-gradient(135deg, #f8f9fa, #fff); border: 1px solid var(--border); border-radius: 12px; padding: 20px; text-align: center; border-top: 3px solid var(--purple); } .kpi-card .kpi-value { font-size: 2em; font-weight: 800; color: var(--purple); } .kpi-card .kpi-label { font-size: 0.85em; color: var(--text-light); margin-top: 4px; } /\* ─── Content Area ─── \*/ .content { padding: 40px 48px; } @media (max-width: 768px) { .content { padding: 20px 16px; } .header-banner { padding: 30px 16px; } } /\* ─── Section Titles ─── \*/ .section-title { font-size: 1.55em; font-weight: 700; color: var(--purple); margin: 48px 0 20px; padding-bottom: 10px; border-bottom: 3px solid var(--rose); display: flex; align-items: center; gap: 14px; } .section-title:first-child { margin-top: 0; } .section-num { background: var(--rose); color: #fff; width: 34px; height: 34px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.8em; font-weight: 700; flex-shrink: 0; } /\* ─── Tables ─── \*/ table { width: 100%; border-collapse: collapse; margin: 16px 0 24px; font-size: 0.92em; } thead th { background: var(--purple); color: #fff; padding: 12px 14px; text-align: left; font-weight: 600; font-size: 0.9em; } tbody td { padding: 10px 14px; border-bottom: 1px solid #edf0f2; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } /\* ─── Callout Boxes ─── \*/ .callout { padding: 20px 24px; border-radius: 10px; margin: 20px 0; font-size: 0.95em; } .callout-green { background: #f0fdf4; border-left: 5px solid var(--green); } .callout-blue { background: #eff6ff; border-left: 5px solid var(--blue); } .callout-orange { background: #fffbeb; border-left: 5px solid var(--orange); } .callout-red { background: #fef2f2; border-left: 5px solid var(--red); } .callout-purple { background: #faf5ff; border-left: 5px solid var(--purple); } .callout-teal { background: #f0fdfa; border-left: 5px solid var(--teal); } .callout h4 { margin-bottom: 8px; } /\* ─── Badges ─── \*/ .badge { display: inline-block; padding: 3px 12px; border-radius: 12px; font-size: 0.82em; font-weight: 600; color: #fff; } .badge-a { background: var(--green); } .badge-b { background: var(--blue); } .badge-c { background: var(--orange); } .badge-d { background: var(--red); } .badge-purple { background: var(--purple); } .badge-rose { background: var(--rose); } .badge-teal { background: var(--teal); } .badge-copper { background: var(--copper); } .coverage-fit { background: var(--green); color: #fff; padding: 3px 12px; border-radius: 12px; font-size: 0.82em; font-weight: 600; display: inline-block; } .coverage-config { background: var(--blue); color: #fff; padding: 3px 12px; border-radius: 12px; font-size: 0.82em; font-weight: 600; display: inline-block; } .coverage-extension { background: var(--orange); color: #fff; padding: 3px 12px; border-radius: 12px; font-size: 0.82em; font-weight: 600; display: inline-block; } .coverage-gap { background: var(--red); color: #fff; padding: 3px 12px; border-radius: 12px; font-size: 0.82em; font-weight: 600; display: inline-block; } /\* ─── Model Cards ─── \*/ .model-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(230px, 1fr)); gap: 18px; margin: 20px 0; } .model-card { border-radius: 14px; padding: 24px; color: #fff; position: relative; overflow: hidden; } .model-card::before { content: ''; position: absolute; top: -50%; right: -50%; width: 100%; height: 100%; background: radial-gradient(circle, rgba(255,255,255,0.08) 0%, transparent 70%); } .model-card h4 { font-size: 1.05em; margin-bottom: 8px; position: relative; } .model-card p { font-size: 0.88em; opacity: 0.92; position: relative; } .model-card .model-icon { font-size: 1.8em; margin-bottom: 10px; position: relative; } .model-a-bg { background: linear-gradient(135deg, #6B3FA0, #8B5CF6); } .model-b-bg { background: linear-gradient(135deg, #B5567A, #E879A8); } .model-c-bg { background: linear-gradient(135deg, #C97B3A, #F5A623); } .model-d-bg { background: linear-gradient(135deg, #00897B, #26C6B0); } /\* ─── Phase Cards ─── \*/ .phase-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin: 20px 0; } .phase-card { border: 2px solid var(--border); border-radius: 14px; padding: 22px; text-align: center; transition: border-color 0.3s; background: #fff; } .phase-card:hover { border-color: var(--purple); } .phase-card .phase-num { font-size: 2em; font-weight: 800; color: var(--purple); } .phase-card .phase-name { font-size: 1em; font-weight: 700; color: var(--text); margin: 6px 0; } .phase-card .phase-dur { font-size: 0.82em; color: var(--text-light); } /\* ─── Expandable Details ─── \*/ details { margin: 12px 0; border: 1px solid var(--border); border-radius: 10px; overflow: hidden; } details summary { padding: 14px 20px; background: #f5f7fa; cursor: pointer; font-weight: 600; font-size: 0.95em; list-style: none; display: flex; align-items: center; gap: 10px; transition: background 0.2s; } details summary:hover { background: #edf0f5; } details summary::before { content: '▶'; font-size: 0.7em; color: var(--purple); transition: transform 0.2s; } details\[open\] summary::before { transform: rotate(90deg); } details summary::-webkit-details-marker { display: none; } details .detail-content { padding: 20px 24px; background: #fff; } /\* nested details \*/ details details { margin: 8px 0; border-color: #e8e8e8; } details details summary { background: #fafbfc; font-size: 0.9em; padding: 10px 16px; } details details .detail-content { padding: 16px 20px; } /\* ─── Expand/Collapse All ─── \*/ .toggle-all-bar { display: flex; gap: 10px; margin: 20px 0; padding: 14px 20px; background: #f5f7fa; border-radius: 10px; border: 1px solid var(--border); } .toggle-btn { padding: 6px 18px; border-radius: 8px; border: 1px solid var(--purple); background: #fff; color: var(--purple); font-weight: 600; font-size: 0.85em; cursor: pointer; transition: all 0.2s; } .toggle-btn:hover { background: var(--purple); color: #fff; } /\* ─── TOC ─── \*/ .toc { background: #f8f9fb; border: 1px solid var(--border); border-radius: 12px; padding: 24px 28px; margin: 24px 0; } .toc h3 { color: var(--purple); margin-bottom: 14px; font-size: 1.15em; } .toc ol { padding-left: 20px; } .toc li { margin: 6px 0; } .toc a { color: var(--purple); text-decoration: none; font-weight: 500; } .toc a:hover { text-decoration: underline; } /\* ─── Gate Box ─── \*/ .gate-box { background: linear-gradient(135deg, #faf5ff, #f0e6ff); border: 2px solid var(--purple); border-radius: 12px; padding: 24px; margin: 24px 0; } .gate-box h4 { color: var(--purple); margin-bottom: 10px; font-size: 1.1em; } .gate-box ul { padding-left: 20px; } .gate-box li { margin: 4px 0; font-size: 0.93em; } /\* ─── Flow Steps ─── \*/ .flow-steps { display: flex; align-items: center; gap: 0; margin: 24px 0; flex-wrap: wrap; justify-content: center; } .flow-step { background: linear-gradient(135deg, var(--purple), var(--rose)); color: #fff; padding: 14px 24px; border-radius: 10px; font-weight: 700; font-size: 0.9em; text-align: center; min-width: 140px; } .flow-arrow { font-size: 1.6em; color: var(--purple); padding: 0 6px; } /\* ─── Tier Cards ─── \*/ .tier-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin: 20px 0; } @media (max-width: 768px) { .tier-grid { grid-template-columns: 1fr; } } .tier-card { border-radius: 14px; padding: 24px; border: 2px solid var(--border); text-align: center; background: #fff; } .tier-card h4 { margin: 10px 0 6px; } .tier-card .tier-icon { font-size: 2em; } /\* ─── Rs Cards ─── \*/ .rs-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(190px, 1fr)); gap: 14px; margin: 20px 0; } .rs-card { border-radius: 12px; padding: 20px; color: #fff; text-align: center; } .rs-card h4 { font-size: 1.05em; margin-bottom: 6px; } .rs-card p { font-size: 0.83em; opacity: 0.92; } /\* ─── Comparison Table ─── \*/ .vs-row { display: grid; grid-template-columns: 1fr 1fr; gap: 18px; margin: 20px 0; } @media (max-width: 768px) { .vs-row { grid-template-columns: 1fr; } } .vs-card { border-radius: 12px; padding: 24px; border: 2px solid var(--border); } .vs-card h4 { margin-bottom: 12px; } /\* ─── Code blocks ─── \*/ pre { background: #f5f5f5; border-left: 4px solid var(--rose); padding: 16px 20px; border-radius: 6px; overflow-x: auto; font-size: 0.88em; line-height: 1.5; margin: 12px 0; } code { font-family: 'Cascadia Code', 'Fira Code', 'Consolas', monospace; } /\* ─── Mermaid ─── \*/ .mermaid { margin: 20px 0; text-align: center; } /\* ─── Footer ─── \*/ .doc-footer { background: #f8f9fb; border-top: 1px solid var(--border); padding: 28px 48px; font-size: 0.85em; color: var(--text-light); } .doc-footer strong { color: var(--text); } /\* ─── Print ─── \*/ @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; margin: 0; border-radius: 0; } details { break-inside: avoid; } details\[open\] summary ~ \* { display: block !important; } .toggle-all-bar { display: none; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } thead th { -webkit-print-color-adjust: exact; print-color-adjust: exact; } } document.addEventListener('DOMContentLoaded', () => mermaid.initialize({ startOnLoad: true, theme: 'base', themeVariables: { primaryColor: '#6B3FA0', primaryTextColor: '#fff', lineColor: '#B5567A', secondaryColor: '#f5f0ff' }}));

![C16](data:image/jpeg;base64,)

# ECC → S/4HANA Cloud (RISE) Migration Playbook

Universal Framework for RISE with SAP — Clean Core Mandatory, Level A Only

📕 Playbook 2 of 2 ☁️ RISE with SAP — PCE & PUE 🔒 Clean Core = Mandatory 🏗️ 5 Phases • 23 Plays • 5 Gates 📅 ~14-18 Months C16 v2026.04

5

Migration Phases

23

Executable Plays

5

Quality Gates

A

Clean Core Target Level

3

Extensibility Tiers

📂 Expand All Sections 📁 Collapse All Sections Click any section header to expand/collapse individually

#### 🚨 Key Difference from On-Premise Playbook

**On-Premise S/4HANA:** Clean Core is _recommended_. Level B (classic BAPIs, REUSE\_ALV) is acceptable. Custom Z-programs deploy on-stack.

**RISE with SAP (this playbook):** Clean Core is **mandatory**. Level A (Released APIs only) is the **only acceptable level**. Level B code must be remediated. PUE allows **zero** custom on-stack code — extensions live on BTP.

### 📋 Table of Contents

1.  [RISE with SAP — Executive Context](#section-1)
2.  [PCE vs PUE Decision Framework](#section-2)
3.  [Engagement Models (A / B / C / D)](#section-3)
4.  [RISE Architecture Overview](#section-4)
5.  [Phase Overview — The 5-Phase Journey](#section-5)
6.  [Governance & RACI Matrix](#section-6)
7.  [Phase 0 — Prerequisites & Extraction](#section-7)
8.  [Phase 1 — DISCOVER (Current State Inventory)](#section-8)
9.  [Business Process Mapping (ECC → S/4HANA Cloud)](#section-9)
10.  [Simplification Impact — SAP Note 2313884](#section-10)
11.  [Phase 2 — ASSESS (Fit-to-Standard + Clean Core)](#section-11)
12.  [Clean Core Classification — Level A Mandatory](#section-12)
13.  [3-Tier Extensibility Model](#section-13)
14.  [The 5 Rs — Cloud Edition](#section-14)
15.  [Phase 3 — REMEDIATE (Close Gaps, Build Extensions)](#section-15)
16.  [BP Migration Deep Dive](#section-16)
17.  [Integration Suite Migration](#section-17)
18.  [Phase 4 — MIGRATE (Build, Test, Go-Live)](#section-18)
19.  [8-Layer Testing Strategy](#section-19)
20.  [Timeline — 14-18 Month Roadmap](#section-20)
21.  [Risk Register](#section-21)
22.  [C16 Accelerator for RISE](#section-22)
23.  [Appendices — Table, Transaction & Code Pattern Reference](#section-23)

## 1 RISE with SAP — Executive Context

RISE with SAP is SAP's strategic offering that bundles S/4HANA Cloud with infrastructure, tools, and services as a single subscription. It represents a fundamental shift from traditional on-premise ECC — not just a technical upgrade but an **operating model transformation**.

#### What RISE Changes

*   **From perpetual license to subscription** — ongoing investment vs one-time capex
*   **From customer-managed to SAP-managed infrastructure** — hyperscaler hosting (AWS, Azure, GCP)
*   **From unlimited customization to governed extensibility** — Clean Core is mandatory, not optional
*   **From manual upgrades to continuous innovation** — quarterly updates (PUE) or managed upgrades (PCE)
*   **From on-stack everything to side-by-side architecture** — BTP for extensions, Integration Suite for interfaces

This playbook provides a **universal, repeatable framework** for migrating any customer from ECC to S/4HANA Cloud under RISE. It covers both Private Cloud Edition (PCE) and Public Cloud Edition (PUE), with clear branching at every decision point.

#### 📘 Companion Document

This is **Playbook 2 of 2**. Playbook 1 covers **ECC → S/4HANA On-Premise** migration. The key difference: On-premise allows Level B (classic APIs) and in-stack Z-programs. This playbook demands Level A (Released APIs) and BTP-based extensions.

## 2 PCE vs PUE Decision Framework

RISE with SAP offers two cloud deployment models. The choice fundamentally shapes the migration approach, extensibility strategy, and level of customization possible.

#### ☁️ Private Cloud Edition (PCE)

*   **Hosting:** Dedicated tenant on hyperscaler (AWS/Azure/GCP)
*   **Customization:** Developer Extensibility — restricted ABAP via Embedded Steampunk
*   **Custom ABAP:** ✅ Allowed (with restrictions — Released APIs + ABAP for Cloud language version)
*   **Upgrades:** Customer-controlled timing (within SAP's maintenance windows)
*   **Data residency:** Customer-chosen region
*   **Integration:** Released APIs + Integration Suite recommended
*   **Best for:** Customers with complex, industry-specific processes that need in-stack extensions
*   **Clean Core level:** Level A mandatory for new code, Level B tolerated temporarily for migrated code

#### 🌐 Public Cloud Edition (PUE)

*   **Hosting:** Multi-tenant, SAP-managed
*   **Customization:** Key User Extensibility (no-code/low-code) ONLY on-stack
*   **Custom ABAP:** ❌ Not allowed on-stack — all custom code on BTP (side-by-side)
*   **Upgrades:** Mandatory quarterly — SAP pushes automatically
*   **Data residency:** SAP-managed regions
*   **Integration:** Released APIs + Event Mesh + Integration Suite — mandatory
*   **Best for:** Customers ready to adopt SAP Best Practices with minimal customization
*   **Clean Core level:** Level A only — zero exceptions. Level B = does not compile.

📊 PCE vs PUE Decision Matrix — Expand for Details

Decision Criterion

Points Toward PCE

Points Toward PUE

**Custom code volume**

5,000+ Z-objects, complex industry logic

<500 Z-objects, mostly reports

**Enhancement complexity**

Deep BAdI implementations, custom pricing, complex ATP

Minor field extensions, simple validations

**Upgrade appetite**

Want to control upgrade timing

Welcome automatic quarterly updates

**Integration landscape**

Complex RFC/IDoc interfaces with 3rd parties

Mostly standard API-based integrations

**Industry requirements**

Regulated industries (pharma, defense, banking)

Standard industries (retail, services)

**Data sovereignty**

Strict data residency / sovereignty requirements

Flexible on data location

**Time-to-value**

Willing to invest 14-18 months

Want fastest path (10-14 months)

**BTP skills**

Limited — want to extend in-stack where possible

Strong BTP/CAP/Node.js skills available

**Process standardization readiness**

Need to keep some unique processes

Ready to adopt SAP Best Practices fully

**Budget model**

Higher TCO but more flexibility

Lower TCO, more constraints

#### Recommendation

Most large enterprises migrating from ECC start with **PCE** — it provides a managed cloud experience while preserving the ability to run restricted custom ABAP. Once Clean Core compliance is achieved and BTP skills mature, a future move to PUE becomes feasible. PUE is ideal for **greenfield implementations** or subsidiaries with simple processes.

## 3 Engagement Models

C16's involvement adapts to the customer's system access situation. Four models cover all scenarios:

🔌

#### Model A — C16 → ECC

C16 connects directly to customer's ECC system via RFC/ADT. Auto-scans all custom objects, classifies dependencies, generates migration reports.

**Speed:** Fastest — hours not weeks

☁️

#### Model B — C16 → S/4HANA Cloud

Customer already has S/4HANA Cloud (sandbox/dev). C16 connects to validate what migrated, check Clean Core compliance, find broken or suboptimal code.

**Speed:** Fast — hours for validation

📦

#### Model C — Offline

No system access. Customer extracts artifacts (TADIR, ST03N, Readiness Check, source code files) and provides to C16 team for analysis.

**Speed:** Moderate — depends on customer

🔗

#### Model D — C16 → BTP

For post-migration validation: C16 connects to BTP subaccount to audit side-by-side extensions, CAP services, Integration Suite flows, and Fiori apps.

**Speed:** Fast — cloud API based

📋 Model Selection Guide — Which Model When?

Engagement Stage

Best Model

Why

Pre-migration assessment (ECC still live)

Model A or Model C

Need to scan ECC custom code — direct connection (A) is fastest, offline (C) if no access

During migration (S/4 sandbox exists)

Model B

Validate converted code against Clean Core standards in the target system

Post-migration (S/4 Cloud live + BTP)

Model B + Model D

Audit both on-stack code (B) and BTP extensions (D)

Continuous compliance (ongoing)

Model B + Model D

Periodic scans to catch Clean Core regression after quarterly updates

## 4 RISE Architecture Overview

RISE with SAP introduces a multi-layer architecture that separates the core S/4HANA system from extensions, integrations, and UX. Understanding this architecture is critical for making the right extensibility decisions.

graph TB subgraph "RISE with SAP" direction TB subgraph "S/4HANA Cloud (Clean Core)" CC\["🏛️ S/4HANA Cloud  
PCE or PUE"\] KU\["👤 Key User Extensibility  
Custom Fields, Custom Logic"\] DE\["💻 Developer Extensibility  
PCE Only — Restricted ABAP"\] end subgraph "SAP BTP (Side-by-Side)" CAP\["📦 CAP / RAP Extensions  
Custom Business Logic"\] FIORI\["🖥️ Fiori / SAP Build Apps  
Custom UX"\] WF\["⚙️ SAP Build Process Automation  
Workflows"\] end subgraph "SAP Integration Suite" CPI\["🔄 Cloud Integration (CPI)  
Replaces PI/PO"\] API\["🌐 API Management  
Governed API Layer"\] EM\["📡 Event Mesh  
Event-Driven Architecture"\] end subgraph "Operations" CALM\["📊 Cloud ALM  
Replaces Solution Manager"\] RCA\["🔍 SAP Readiness Check  
\+ Custom Code Migration"\] end end CC --> KU CC --> DE CC -->|Released APIs| CAP CC -->|Events| EM CC -->|OData / SOAP| CPI CAP --> FIORI CPI --> API EM --> CAP CALM --> CC RCA --> CC

🏗️ Architecture Component Details

Component

Role

Replaces (ECC)

Edition

**S/4HANA Cloud Core**

ERP backbone — Finance, Logistics, Manufacturing, HR

ECC Central Instance

PCE + PUE

**Key User Extensibility**

No-code: Custom Fields, Custom Logic (BRF+), Custom CDS Views, Custom Analytical Queries

SE11 appends, user exits, Z-reports

PCE + PUE

**Developer Extensibility**

Restricted ABAP (Embedded Steampunk): RAP, CDS, Released APIs only

Z-programs, Z-classes, Z-function modules

PCE only

**SAP BTP**

Side-by-side extensions: CAP apps, custom microservices, Fiori apps, workflows

No equivalent — new capability

PCE + PUE

**SAP Build Apps**

Low-code application development

WebDynpro, BSP custom apps

PCE + PUE

**SAP Build Process Automation**

Workflow automation + RPA bots

SAP Business Workflow, SWDD

PCE + PUE

**Cloud Integration (CPI)**

A2A and B2B integration flows

PI/PO

PCE + PUE

**API Management**

API governance, rate limiting, analytics

No equivalent

PCE + PUE

**Event Mesh**

Event-driven integration between S/4 and BTP/3rd party

No equivalent (was polling-based)

PCE + PUE

**Cloud ALM**

Application lifecycle management, testing, monitoring

Solution Manager (SOLMAN)

PCE + PUE

## 5 Phase Overview — The 5-Phase Journey

Phase 0  
PREPARE

→

Phase 1  
DISCOVER

→

Phase 2  
ASSESS

→

Phase 3  
REMEDIATE

→

Phase 4  
MIGRATE

0

PREPARE

Weeks 1–2

Governance, tooling, team, RISE contract review, BTP provisioning

1

DISCOVER

Weeks 1–6

Process catalogue, custom code inventory, interface map, data volumes, add-on registry

2

ASSESS

Weeks 5–14

Fit-to-Standard, Clean Core classification, extensibility decisions, gap analysis

3

REMEDIATE

Weeks 12–32

Quick wins, BTP extensions, BP migration, integration suite, forms, add-on replacement

4

MIGRATE

Weeks 30–56

System build, data migration, 8-layer testing, training, cutover, hypercare

## 6 Governance & RACI Matrix

RISE migrations require expanded governance compared to on-premise. New roles include **BTP Architect**, **Integration Suite Lead**, and **Cloud ALM Administrator** — these did not exist in the on-premise playbook.

📊 Full RACI Matrix — 10 Workstreams × 10 Roles

Workstream

Project Lead

Functional Lead

ABAP Dev Lead

BTP Architect

Integration Lead

Data Migration Lead

Test Manager

Cloud ALM Admin

Security Lead

Change Manager

**1\. Process Mapping**

A

R

C

C

C

I

I

I

I

I

**2\. Custom Code Analysis**

A

C

R

C

I

I

I

I

I

I

**3\. Fit-to-Standard**

A

R

C

C

C

I

I

I

I

C

**4\. Clean Core Remediation**

A

C

R

R

I

I

I

I

C

I

**5\. BTP Extension Dev**

A

C

C

R

C

I

I

I

C

I

**6\. Integration Migration**

A

C

C

C

R

I

I

I

C

I

**7\. Data Migration**

A

C

C

I

I

R

C

I

C

I

**8\. Testing**

A

C

C

C

C

C

R

C

C

I

**9\. Cloud ALM Setup**

A

I

I

C

I

I

C

R

C

I

**10\. Change Management**

A

C

I

I

I

I

I

I

I

R

**R** = Responsible (does the work) | **A** = Accountable (owns the outcome) | **C** = Consulted | **I** = Informed

#### 🆕 New Roles vs On-Premise Playbook

**BTP Architect** — owns side-by-side extension design, CAP projects, BTP service consumption. **Integration Lead** — owns PI/PO → CPI migration, API Management, Event Mesh setup. **Cloud ALM Admin** — owns application lifecycle, test management, and monitoring in SAP Cloud ALM (replaces Solution Manager).

## 7 Phase 0 — Prerequisites & Extraction

Before any analysis or migration activity begins, the customer must provide key artifacts from their ECC system. The extraction method depends on the engagement model.

📋 Play 0.1 — Artifact Extraction Checklist (12 Items)

#

Artifact

How to Extract

Model A  
(C16→ECC)

Model B  
(C16→S/4)

Model C  
(Offline)

1

**Custom Code Inventory**  
All Z/Y programs, classes, FMs, tables

SE16 → TADIR WHERE obj\_name LIKE 'Z%' OR 'Y%'  
Or: `/SDF/CDM_ANALYZE`  
Or: SAP Readiness Check

C16 auto-scans

C16 auto-scans

Customer exports CSV

2

**Transaction Usage Statistics**  
Top transactions by execution count

ST03N → Expert Mode → Top Transactions

C16 reads

N/A (new system)

Customer exports

3

**Enhancement/Modification Registry**  
Customer exits, BAdIs, modifications

SMOD/CMOD (exits), SE18/SE19 (BAdIs), SPAU/SPDD history

C16 scans

C16 scans

Customer exports

4

**SAP Readiness Check Results**  
Simplification items, custom code impact

[SAP Launchpad](https://launchpad.support.sap.com) → SAP Readiness Check for S/4HANA 2025

Customer runs

Customer runs

Customer runs

5

**Custom Code Migration Worklist**  
Deprecated API usage list

Transaction SYCM or ATC with ABAP\_CLOUD\_READINESS variant

C16 runs ATC

C16 runs ATC

Customer exports

6

**Active Business Process List**  
Processes per module with owners

Solution Manager (SOLMAN) process docs, or manual from IMG + business

Customer provides

Customer provides

Customer provides

7

**Interface Inventory**  
All RFCs, IDocs, BAPIs, OData, middleware

SM59 (RFC destinations), WE20 (IDoc partners), SOAMANAGER, /IWFND/MAINT\_SERVICE

C16 reads

C16 reads

Customer exports

8

**Output/Form Inventory**  
SAPscript, SmartForms, Adobe Forms

SE71 list, SMARTFORMS list, SFP list

C16 scans

C16 scans

Customer exports

9

**Add-on & ISV List**  
Installed add-ons with versions

Transaction SAINT — installed components

C16 reads

N/A

Customer exports

10

**🆕 RISE Contract Scope**  
PCE or PUE, included BTP services

RISE contract documentation from SAP account executive

Customer provides

Customer provides

Customer provides

11

**🆕 BTP Subaccount Structure**  
Existing BTP services, subaccounts, spaces

BTP Cockpit → Global Account → Subaccounts

N/A (pre-migration)

C16 audits

Customer provides

12

**🆕 Integration Suite Config**  
Existing CPI flows, API Management policies

Integration Suite cockpit → Design → Artifacts

N/A (pre-migration)

C16 audits

Customer provides

#### ⚠️ Items 10-12 are NEW vs the On-Premise Playbook

On-premise migrations don't need RISE contract scope, BTP subaccount info, or Integration Suite config. RISE migrations **must** have these — they determine which extensibility tier is available and what integration patterns are possible.

🔧 Play 0.2 — Environment Setup & BTP Provisioning

Task

Owner

Tools

Output

Provision S/4HANA Cloud sandbox (PCE/PUE)

SAP / Basis Lead

SAP Provisioning Wizard

S/4 sandbox URL + admin credentials

Provision BTP Global Account + Subaccounts

BTP Architect

BTP Cockpit

Dev/QA/Prod subaccounts

Enable Integration Suite

Integration Lead

BTP Cockpit → Subscriptions

CPI tenant URL

Enable Cloud ALM

Cloud ALM Admin

BTP Cockpit → Subscriptions

Cloud ALM tenant URL

Set up SAP Build Work Zone (Launchpad)

BTP Architect

BTP Cockpit

Central Fiori Launchpad URL

Configure Event Mesh

Integration Lead

BTP Cockpit

Event Mesh service key

Install C16 connectivity (if Model A/B)

Basis Lead

RFC + ADT endpoints

C16 connected to ECC/S/4

#### 🚦 Gate 0 — Preparation Complete

*   ☐ RISE edition decided (PCE or PUE)
*   ☐ S/4HANA Cloud sandbox provisioned
*   ☐ BTP Global Account + Dev subaccount provisioned
*   ☐ Cloud ALM tenant active
*   ☐ Artifact extraction started (items 1-12 above)
*   ☐ SAP Readiness Check scheduled or completed
*   ☐ Project team assembled (RACI confirmed)
*   ☐ C16 connectivity established (Model A or B) OR extraction underway (Model C)

## 8 Phase 1 — DISCOVER (Current State Inventory)

**Objective:** Build a complete, quantified picture of the customer's ECC landscape — processes, customizations, integrations, forms, add-ons, and data volumes.

📋 Play 1 — Business Process Catalogue

**Goal:** Document every active business process by module, with transaction codes, owners, and frequency.

Model A (C16 → ECC): Auto-Discovery

1.  C16 queries ST03N usage data via RFC to rank transactions by execution count
2.  C16 groups transactions by module (MM, SD, FI/CO, PP, PM, QM, WM, HR)
3.  C16 identifies custom transactions (Z\*) and maps to their programs
4.  Business stakeholders validate and add process names/owners

**Time: 2-3 days** (1 day C16 scan + 1-2 days stakeholder validation)

Model B (C16 → S/4HANA Cloud): Post-Migration Validation

1.  C16 scans the S/4HANA system for active custom transactions and objects
2.  C16 compares against SAP Best Practices process list to identify gaps
3.  C16 flags processes that migrated but aren't being used (dead processes)
4.  C16 flags processes running on non-compliant custom code

**Time: 1-2 days**

Model C (Offline): Manual Extraction

1.  Customer exports ST03N → Top 200 transactions by execution count
2.  Customer exports TSTC table → all Z/Y custom transaction codes
3.  Customer provides Solution Manager process documentation (if available)
4.  Functional consultants conduct interviews with process owners per module

**Time: 1-2 weeks** (depends on customer responsiveness)

📋 Play 2 — Custom Code Inventory

**Goal:** Quantify and categorize every custom Z/Y object in the ECC system.

Object Type

TADIR Filter

Typical Count (Large Enterprise)

Programs (Reports)

OBJECT = 'PROG', OBJ\_NAME LIKE 'Z%'

2,000 – 10,000

Classes

OBJECT = 'CLAS', OBJ\_NAME LIKE 'Z%'

500 – 3,000

Function Groups/Modules

OBJECT = 'FUGR', OBJ\_NAME LIKE 'Z%'

200 – 1,500

Custom Tables

OBJECT = 'TABL', OBJ\_NAME LIKE 'Z%'

500 – 3,000

Transactions

OBJECT = 'TRAN', OBJ\_NAME LIKE 'Z%'

100 – 1,000

Enhancements (BAdI impl)

OBJECT = 'ENHI'

50 – 500

Enhancement Spots

OBJECT = 'ENHS'

10 – 100

CDS Views

OBJECT = 'DDLS', OBJ\_NAME LIKE 'Z%'

0 – 500 (depends on maturity)

WebDynpro Components

OBJECT = 'WDYN', OBJ\_NAME LIKE 'Z%'

10 – 200

BSP Applications (Fiori)

OBJECT = 'WAPA', OBJ\_NAME LIKE 'Z%'

5 – 100

Model A: C16 auto-scans TADIR + classifies all objects

C16 queries TADIR for all Z/Y objects, then classifies each against SAP's cloudification catalog. Every object gets a Clean Core level (A/B/C/D) and a disposition recommendation. **Time: 2-4 hours for 10,000+ objects.**

Model B: C16 scans S/4HANA for migrated custom objects

C16 scans all Z/Y objects in the S/4HANA system, then runs ATC with ABAP\_CLOUD\_READINESS variant on each. Objects that fail = need remediation before next quarterly update. **Time: 2-4 hours.**

Model C: Customer exports TADIR data + source code files

Customer exports: (1) TADIR table filtered by Z/Y objects, (2) Source code of top 50-100 most-used programs (from SE38/SE80), (3) SAP Readiness Check custom code analysis results. C16 team analyzes the exports. **Time: 1-2 weeks.**

📋 Play 3 — Interface & Integration Inventory

**Goal:** Map every inbound/outbound interface — RFCs, IDocs, BAPIs, OData, middleware, flat files.

Interface Type

ECC Source

S/4HANA Cloud Target

Migration Complexity

RFC (Synchronous)

SM59 destinations

Released APIs via API Management

🟡 Medium

BAPI (RFC-wrapped)

BAPI Explorer

Released APIs (many BAPIs still available in PCE)

🟢 Low (if released)

IDoc (Async)

WE20 partner profiles

IDoc runtime (still supported) or Event Mesh

🟢 Low to 🟡 Medium

PI/PO (Middleware)

ESR/ID objects

Cloud Integration (CPI)

🔴 High — full redesign

OData V2

/IWFND/MAINT\_SERVICE

OData V4 (Released APIs)

🟡 Medium

Flat Files (FTP/SFTP)

AL11 file system

Cloud Integration + SFTP adapter

🟡 Medium

Direct DB (Open SQL)

Custom ABAP programs

Released APIs — no direct DB access in PUE

🔴 High

📋 Play 4 — Add-on & ISV Compatibility Check

**Goal:** Verify every installed add-on is compatible with S/4HANA Cloud edition.

Check

How

Outcome

List installed add-ons

Transaction SAINT

Add-on name, version, vendor

Check S/4HANA Cloud compatibility

SAP Maintenance Planner + vendor confirmation

Compatible / Not Compatible / Replacement Available

Check cloud-qualified status

SAP Partner Finder for cloud-qualified solutions

Listed = OK. Not listed = risk

#### 🚨 PUE Critical: Incompatible add-ons CANNOT be installed on PUE.

For Public Cloud, every add-on must be SAP-certified for S/4HANA Cloud PUE. If not certified, the add-on functionality must be replaced by: (1) standard S/4HANA features, (2) a cloud-qualified alternative from SAP Store, or (3) a custom BTP extension. For PCE, most on-premise add-ons work but must be verified.

#### 🚦 Gate 1 — Discovery Complete

*   ☐ Business process catalogue complete — all modules covered with process owners
*   ☐ Custom code inventory complete — total object count by type, usage statistics attached
*   ☐ Interface inventory complete — all RFC/IDoc/BAPI/OData/file interfaces catalogued
*   ☐ Add-on compatibility check complete — incompatible add-ons flagged
*   ☐ Data volume assessment complete — table sizes for migration planning
*   ☐ SAP Readiness Check results received and reviewed
*   ☐ Findings presented to steering committee

## 9 Business Process Mapping (ECC → S/4HANA Cloud)

These mappings are **universal** — they apply to every ECC → S/4HANA Cloud migration regardless of customer. The table and data model changes are mandated by SAP's Simplification List.

📦 Materials Management (MM)

ECC Process

ECC Transaction

S/4HANA Cloud Standard

Fiori App

Key Data Model Change

Create Purchase Requisition

ME51N

Create Purchase Requisition

F1559

—

Create Purchase Order

ME21N

Create Purchase Order — Advanced

F0842

—

Display Purchase Order

ME23N

Manage Purchase Orders

F2229

—

Goods Receipt

MIGO

Post Goods Receipt for PO

F4699

MKPF/MSEG → **MATDOC**

Invoice Verification

MIRO

Create Supplier Invoice

F0859

—

Stock Overview

MMBE

Stock — Single Material

F0842A

MARD → **MATDOC aggregate**

Material Master

MM01/MM02/MM03

Manage Product Master Data

F1602

—

Vendor Master

XK01/XK02/XK03

Manage Business Partner

F1739

LFA1/LFB1 → **BP (BUT000/BUT020)**

Purchasing Info Records

ME11/ME12/ME13

Manage Purchasing Info Records

F2984

—

Source List

ME01/ME03

Manage Source of Supply

F3735

—

📦 Sales & Distribution (SD)

ECC Process

ECC Transaction

S/4HANA Cloud Standard

Fiori App

Key Data Model Change

Create Sales Order

VA01

Create Sales Order

F5370

—

Display Sales Order

VA03

Manage Sales Orders

F1814

—

Create Delivery

VL01N

Create Outbound Delivery

F6829

—

Post Goods Issue

VL02N

Post Goods Issue for Outbound Delivery

F2557

MKPF/MSEG → **MATDOC**

Create Billing Document

VF01

Create Billing Documents

F2583

—

Customer Master

XD01/XD02/XD03

Manage Business Partner

F1739

KNA1/KNB1 → **BP (BUT000/BUT020)**

Pricing

VK11/VK12

Manage Sales Pricing

F3893

KONV → **PRCD\_ELEMENTS**

Credit Management

FD32/FD33

SAP Credit Management (FSCM)

F3738

Full architectural change

Output Determination

NACE

BRF+ / Output Management

—

Full architectural change

Availability Check

CO09

Advanced ATP (aATP)

F5899

Enhanced engine

📦 Finance (FI/CO)

ECC Process

ECC Transaction

S/4HANA Cloud Standard

Fiori App

Key Data Model Change

Post GL Document

FB01/FB50

Post General Journal Entry

F0717

BKPF/BSEG → **ACDOCA (Universal Journal)**

Display GL Line Items

FBL3N

Display Line Items — GL

F0710

BSEG/BSIS/BSAS → **ACDOCA**

Vendor Invoice

FB60

Create Supplier Invoice — Non-PO

F0862

—

Vendor Payment

F110

Schedule Payment Proposals

F0742

—

Customer Invoice

FB70

Create Customer Invoice

F0717B

—

Bank Reconciliation

FF67/FEBAN

Manage Bank Statements

F1564

—

Asset Accounting

AS01/AS02

Manage Fixed Assets

F2370

New Asset Accounting (no parallel ledger)

Cost Center Planning

KP06

Manage Cost Center Budgets

F2725

—

Profitability Analysis

KE21N/KE24

CO-PA integrated in ACDOCA

F4595

CE1\*/CE2\* → **ACDOCA**

Material Ledger

CKM3/CKMLCP

Material Ledger (mandatory)

F3166

MBEW/CKMLPP → **ACDOCA + MATDOC**

📦 Production Planning / Plant Maintenance / Quality (PP/PM/QM)

ECC Process

ECC Transaction

S/4HANA Cloud Standard

Fiori App

Key Change

Create Production Order

CO01

Manage Production Orders

F3888

—

MRP Run

MD01/MD02

MRP Live (ppMRP)

F2737

Architecture: MRP Live on HANA

BOM Management

CS01/CS02

Manage Bills of Material

F3643

—

Routing

CA01/CA02

Manage Routings

F3700

—

Create Maintenance Order

IW31

Create Maintenance Order

F2456

—

Maintenance Notification

IW21

Create Maintenance Notification

F2407

—

Inspection Lot

QA01

Manage Quality Inspection

F3620

—

Results Recording

QA32

Record Quality Results

F3621

—

📦 Warehouse Management (WM → EWM)

#### 🚨 Major Architectural Change

Classic WM (LE-WM) is **removed** in S/4HANA Cloud. Customers using classic WM must migrate to **Embedded EWM** (Extended Warehouse Management). This is one of the highest-effort migration items.

ECC (Classic WM)

S/4HANA Cloud (Embedded EWM)

Impact

LT01 — Create Transfer Order

Warehouse Tasks (WT) — automated

Conceptual change: TO → WT

LT0E — Create TO for Delivery

PPF / Output Management triggers WT creation

Process redesign needed

LI01/LI02 — Physical Inventory

/SCWM/PI — EWM Physical Inventory

UI + process change

LS01-LS09 — Storage Bins

/SCWM/BIN — EWM Bin Management

Master data migration

Custom WM reports

New EWM Fiori apps or custom BTP reports

Full rewrite required

## 10 Simplification Impact — SAP Note 2313884

SAP Note 2313884 is the master list of all changes between ECC and S/4HANA. The following are the **highest-impact items** that affect virtually every ECC customer:

#

Simplification Item

ECC Object

S/4HANA Replacement

Impact

1

**Business Partner**

KNA1, KNB1, KNVV (Customer)  
LFA1, LFB1, LFBK (Vendor)

BUT000, BUT020, BP0\*  
CDS: I\_Supplier, I\_Customer, I\_BusinessPartner

CRITICAL

2

**Universal Journal**

BSEG, BSIK, BSAK, BSID, BSAD, BSIS, BSAS  
FAGLFLEXA, GLT0, COEP

ACDOCA (single table)  
CDS: I\_JournalEntry, I\_GLAccountLineItem

CRITICAL

3

**Material Document**

MKPF, MSEG

MATDOC (single table)  
CDS: I\_MaterialDocumentItem

CRITICAL

4

**Material Ledger**

Optional in ECC

Mandatory in S/4HANA

HIGH

5

**CO-PA (Profitability Analysis)**

CE1\*/CE2\* (Costing-based CO-PA)

Account-based CO-PA via ACDOCA

HIGH

6

**New Asset Accounting**

ANLP, ANLC (classic depreciation)

ACDOCA-based asset accounting (no parallel ledger)

HIGH

7

**Credit Management**

FD32 (Classic)

SAP Credit Management (FSCM / UKM)

HIGH

8

**Output Management**

NACE (Classic output determination)

BRF+ / Output Management

MEDIUM

9

**MRP Live**

MD01/MD02 (Classic MRP)

ppMRP / MRP Live (HANA-optimized)

MEDIUM

10

**Pricing (KONV)**

KONV (Condition records)

PRCD\_ELEMENTS

MEDIUM

11

**Classic WM Removal**

LE-WM (LT01, LS01, etc.)

Embedded EWM

CRITICAL (if WM used)

12

**Classic GL Removed**

GLT0 (classic GL totals)

FAGLFLEXT / ACDOCA

HIGH

#### ⚠️ Cloud-Specific Impact

On-premise S/4HANA provides **compatibility views** that let legacy code read from old table names (BSEG → compatibility view on ACDOCA). In **PUE**, these compatibility views are **not guaranteed** — custom code must use the new CDS views directly. In **PCE**, compatibility views are available but deprecated — new code must use CDS views.

## 11 Phase 2 — ASSESS (Fit-to-Standard + Clean Core)

**Objective:** For every business process and custom object, determine: (1) Does S/4HANA Cloud standard cover it? (2) What's the gap? (3) What extensibility tier closes the gap?

📋 Play 5 — Fit-to-Standard Workshops

Run structured workshops per module. For each business process, classify against the S/4HANA Cloud standard:

Classification

Definition

Action

Cloud-Specific Rule

FIT

Standard S/4HANA Cloud covers this exactly

Adopt as-is

Same for PCE and PUE

FIT + CONFIG

Standard covers it with configuration changes

Configure in SAP Best Practices

PUE: configuration via self-service. PCE: via IMG

FIT + EXTEND

Standard covers 70-80%, needs extension for rest

Key User Ext (PUE) or Developer Ext (PCE) or BTP

⚠️ Extension must be Level A only

GAP

No standard equivalent

BTP side-by-side (PUE) or Developer Ext (PCE)

🔴 PUE: MUST be on BTP. PCE: Can be in-stack (restricted)

#### 🚨 Key Difference from On-Premise

**On-premise:** GAP = "keep your Z-program." **RISE Cloud:** GAP = "change the business process to fit standard, OR build a Clean Core extension on BTP/Developer Extensibility." There is **no option to keep non-compliant custom code on-stack** in PUE, and restricted options in PCE.

#### Workshop Template (Per Module)

#

Business Requirement

Classification

Standard Alternative

Gap Description

Extension Tier

R1

_(Populated per workshop)_

FIT

_Fiori App / Transaction_

—

—

R2

_(Populated per workshop)_

GAP

_No standard_

_Gap details_

_Key User / Dev Ext / BTP_

R3

_…_

Template — populate during each module workshop. Minimum 10 requirements per module.

📋 Play 6 — Custom Code Clean Core Classification

Every custom Z/Y object must be classified against SAP's cloudification catalog. The classification determines the migration path.

**See Section 12 (Clean Core Classification) for the full framework and rules.**

📋 Play 7 — Enhancement Migration Mapping

Every ECC enhancement (user exit, BAdI, modification) must be mapped to its S/4HANA Cloud equivalent:

ECC Enhancement Type

S/4HANA Cloud Equivalent

PCE

PUE

User Exits (CMOD/SMOD)

Released Cloud BAdIs

✅ Use released BAdI

✅ Use released BAdI (Key User logic) or BTP

Classic BAdIs (SE18/SE19)

Released Cloud BAdIs (check INTERNAL flag)

✅ If released

✅ If released (Key User) or ❌ If internal → BTP

Enhancement Spots (implicit/explicit)

Released Cloud BAdIs

✅ If released

❌ Not available → BTP

Modifications (SPAU/SPDD)

Key User Extensibility or BAdIs

❌ No modifications

❌ No modifications

Append Structures (SE11)

Key User Custom Fields

✅ Key User or extension includes

✅ Key User Custom Fields only

CALL CUSTOMER-FUNCTION

Released Cloud BAdIs

✅ If released

✅ Key User or ❌ BTP

Screen Exits (PBO/PAI)

Fiori Adaptation (Key User) or custom Fiori (BTP)

⚠️ Limited

❌ Not available → BTP Fiori app

Menu Exits

Fiori navigation + tiles

✅ Fiori Launchpad config

✅ Fiori Launchpad config

#### BAdI Validation Rule

Before recommending any BAdI as a migration target, **always verify**: (1) The BAdI exists in S/4HANA Cloud, (2) It is NOT marked as `INTERNAL = 'X'` in SXS\_ATTR, (3) It is released for cloud (check C1 release status). Internal BAdIs are for SAP use only — never recommend them.

#### 🚦 Gate 2 — Assessment Complete

*   ☐ Fit-to-Standard workshops complete for all modules
*   ☐ Every process classified: FIT / FIT+CONFIG / FIT+EXTEND / GAP
*   ☐ Every custom object classified: Level A / B / C / D
*   ☐ Every enhancement mapped to S/4HANA Cloud equivalent
*   ☐ GAP list finalized with extensibility tier assigned (Key User / Dev Ext / BTP)
*   ☐ PCE vs PUE decision confirmed
*   ☐ Effort estimates for remediation sprints calculated
*   ☐ Steering committee sign-off on scope

## 12 Clean Core Classification — Level A Mandatory

#### 🔴 RISE Rule: Level A is the ONLY acceptable level for new and remediated code.

On-premise allows Level B (classic BAPIs, REUSE\_ALV). **RISE does not.** Every custom object must either reach Level A or be rebuilt as a BTP extension. Level B code is tolerated _temporarily_ in PCE during migration, but must be remediated within a defined timeline.

Level

Definition

On-Premise Rule

RISE PCE Rule

RISE PUE Rule

Level A

Uses only Released APIs — cloud-ready, upgrade-stable

✅ Best practice

✅ Required for new code

✅ The ONLY option

Level B

Uses classic stable APIs (BAPIs, REUSE\_ALV, classic FMs)

✅ Acceptable

⚠️ Temporarily tolerated — must remediate

❌ Does not compile

Level C

Direct DB access on standard tables, unreleased APIs

🔴 Needs remediation

❌ Does not compile in Embedded Steampunk

❌ Does not compile

Level D

Deprecated/removed objects, modifications, SAPscript

🚨 Will break

❌ Will not deploy

❌ Will not deploy

📊 Classification Examples — Common ECC Patterns

ECC Pattern

Level

RISE Action

`SELECT * FROM LFA1 WHERE lifnr = lv_vendor`

C

Replace with CDS: `I_Supplier` or Released API: `API_BUSINESS_PARTNER`

`CALL FUNCTION 'REUSE_ALV_GRID_DISPLAY'`

B

PCE: `CL_SALV_TABLE` (Level A). PUE: Fiori Elements on BTP

`SELECT * FROM BSEG WHERE bukrs = '1000'`

D

Replace with CDS: `I_JournalEntry` or `I_GLAccountLineItem`

`CALL FUNCTION 'BAPI_PO_CREATE1'`

B

Replace with Released API: `API_PURCHASEORDER_PROCESS_SRV`

`SELECT * FROM MKPF/MSEG`

D

Replace with CDS: `I_MaterialDocumentItem` (reads from MATDOC)

`CALL TRANSACTION 'ME21N' USING lt_bdc`

D

Replace with Released API: Purchase Order OData V4

`MODIFY ZCUST_TABLE` (custom table)

A

Custom tables are allowed — customer owns them

RAP BO with CDS + BDEF using Released APIs

A

✅ Cloud-ready — no changes needed

## 13 3-Tier Extensibility Model

RISE with SAP introduces a **3-tier extensibility pyramid**. The rule is: always start at the lowest tier that meets the requirement. Escalate only when the lower tier can't deliver.

👤

#### Tier 1: Key User Extensibility

**No-code / Low-code**

Custom Fields, Custom Logic (BRF+), Custom CDS Views, Custom Analytical Queries, Custom Business Objects

**Available:** PCE ✅ + PUE ✅

**Upgrade-safe:** ✅ Fully

💻

#### Tier 2: Developer Extensibility

**Restricted ABAP (Embedded Steampunk)**

RAP Business Objects, CDS Views, Released BAdI Implementations, ABAP Cloud language version — all using Released APIs only

**Available:** PCE ✅ only | PUE ❌

**Upgrade-safe:** ✅ If Level A

☁️

#### Tier 3: Side-by-Side (BTP)

**Full development freedom**

CAP applications, SAP Build Apps, Fiori apps, Node.js/Java microservices, Workflows, connected via Released APIs + Event Mesh

**Available:** PCE ✅ + PUE ✅

**Upgrade-safe:** ✅ If using stable APIs

📊 Tier Selection Decision Tree

graph TD A\["Extension Requirement"\] --> B{"Can Key User  
Extensibility handle it?"} B -->|Yes| C\["✅ Tier 1: Key User  
Custom Fields / BRF+ Logic"\] B -->|No| D{"Is this PCE?"} D -->|Yes| E{"Can Restricted ABAP  
handle it?  
(Released APIs sufficient?)"} E -->|Yes| F\["✅ Tier 2: Developer Ext  
RAP / CDS / BAdI"\] E -->|No| G\["✅ Tier 3: BTP Side-by-Side  
CAP / SAP Build / Fiori"\] D -->|No - PUE| G style C fill:#27ae60,color:#fff style F fill:#3498db,color:#fff style G fill:#6B3FA0,color:#fff

📋 Tier Mapping Examples — Common ECC Custom Objects

ECC Custom Object

Tier 1 (Key User)

Tier 2 (Dev Ext — PCE)

Tier 3 (BTP)

Custom field on vendor master (ZZ\_MSME\_CODE)

✅ Custom Field app

—

—

Validation on PO save (check custom table)

✅ BRF+ Custom Logic

✅ Released BAdI implementation

—

Complex vendor scoring report with 20+ tables

❌ Too complex

✅ RAP + CDS + Fiori Elements

✅ CAP + Fiori app

Custom pricing procedure with external callout

❌

⚠️ If released BAdI exists

✅ CAP microservice + API

Integration with 3rd party logistics system

❌

❌

✅ Integration Suite + CPI flow

Custom approval workflow with email notifications

⚠️ Simple workflows only

—

✅ SAP Build Process Automation

Mobile app for warehouse scanning

❌

❌

✅ SAP Build Apps / Mobile Dev Kit

Custom analytical dashboard

✅ Custom Analytical Query

✅ CDS Analytical view + Fiori

✅ SAP Analytics Cloud

## 14 The 5 Rs — Cloud Edition

Every custom object gets ONE disposition. The cloud edition applies **stricter rules** than the on-premise version:

#### 🗑️ RETIRE

Not used or replaced by standard. Delete — don't migrate.

#### 🔄 REPLACE

Standard S/4HANA Cloud covers it. Adopt standard + configure.

#### 🔧 REFIT

Standard covers 70%+. Close gap with Key User or Dev Ext (Level A).

#### 🏗️ REWRITE

Must stay custom. Rewrite using RAP/CDS (PCE) or CAP (BTP) — Level A only.

#### ✅ RETAIN

Already Level A, cloud-ready. Migrate unchanged.

📊 5 Rs Decision Matrix — On-Premise vs RISE

Disposition

On-Premise Rule

RISE PCE Rule

RISE PUE Rule

**RETIRE**

If ST03N shows 0 usage

Same

Same

**REPLACE**

Standard Fiori app covers it

Same — adopt SAP Best Practice

Same — adopt SAP Best Practice

**REFIT**

Released BAdI + Z-code

Released BAdI (Level A) + Developer Ext

Key User Ext only (no Z-code on-stack)

**REWRITE**

Z-program with CDS + RAP

RAP/CDS in Developer Ext (Level A)

CAP on BTP + Fiori (side-by-side only)

**RETAIN**

Level A or B — keep as-is

Level A only — Level B must remediate

❌ No custom code on-stack — Level A on BTP only

#### ⚠️ The RETAIN Trap

On-premise, RETAIN is the easy option — "it works, don't touch it." In RISE, RETAIN is the **most restrictive** option: the code must be Level A (Released APIs only). If it's Level B (classic BAPIs), it **cannot be retained** — it must be REFIT or REWRITE. This catches many teams off guard.

📋 Disposition Template — Populate Per Object

Object Name

Type

Usage Count

Clean Core Level

Disposition

Target Tier

Effort

_(From C16 scan or customer extract)_

PROG

_From ST03N_

C

REWRITE

Tier 2 (Dev Ext)

5-10 days

_…_

CLAS

A

RETAIN

—

0 days

_…_

ENHI

D

REFIT

Tier 1 (Key User)

2-3 days

Template — populate from C16 analysis (Models A/B) or customer extract (Model C). Minimum fields: object name, type, usage, Clean Core level, disposition, target tier, effort estimate.

## 15 Phase 3 — REMEDIATE (Close Gaps, Build Extensions)

📋 Play 8 — Quick Wins: RETIRE + REPLACE

**Goal:** Eliminate as many custom objects as possible before writing a single line of new code.

Action

Criteria

Effort

**RETIRE — Delete unused objects**

ST03N shows 0 executions in 12+ months

~1 hour per object (verify + delete)

**RETIRE — Delete redundant objects**

Standard S/4HANA Fiori app covers 100% of the functionality

~1-2 days per object (user training + decommission)

**REPLACE — Adopt standard**

Standard covers with configuration, no extension needed

~2-5 days per process (configure + test + train)

#### Target: Retire 30-50% of custom objects in this phase

Industry benchmarks show that 30-50% of custom Z-programs in ECC are unused, redundant, or fully replaced by S/4HANA standard Fiori apps. Eliminating them early reduces the remediation scope dramatically.

📋 Play 9 — Key User Extensibility (Tier 1)

**Goal:** Convert simple customizations to no-code Key User extensions — available in both PCE and PUE.

ECC Customization

Key User Alternative

App to Use

Append structure on standard table (ZZ fields)

Custom Fields

Custom Fields and Logic app

Simple validation on save (check field value)

Custom Logic (BRF+)

Custom Logic app

Custom report showing standard data

Custom Analytical Query

Custom Analytical Queries app

Custom CDS view for reporting

Custom CDS View

Custom CDS Views app

Custom business object (simple CRUD)

Custom Business Object

Custom Business Objects app

UI field arrangement change

Fiori Adaptation

Adapt UI (Key User mode)

📋 Play 10 — Developer Extensibility Sprints (Tier 2 — PCE Only)

**Goal:** Rewrite Level C/D objects using restricted ABAP (Embedded Steampunk) — RAP, CDS, Released APIs only.

#### Sprint Structure (2-week sprints)

Day

Activity

Output

Day 1

Sprint planning — pick 3-5 objects from disposition list

Sprint backlog

Day 2-3

C16 analysis — fetch source, classify dependencies, generate migration report per object

Per-object migration spec

Day 4-7

Development — rewrite using RAP/CDS/Released APIs

New Level A code

Day 8-9

ATC validation — run ABAP\_CLOUD\_READINESS check on all new objects

ATC pass = Level A confirmed

Day 10

Sprint review — demo, deploy to QA, retrospective

Objects deployed + validated

#### C16 Acceleration

C16 can generate Clean Core code (RAP Business Objects, CDS views, Fiori Elements apps) directly from the analysis of the original ECC program. This reduces Day 4-7 (development) from 4 days to 4-8 hours for many objects.

📋 Play 11 — BTP Side-by-Side Extensions (Tier 3)

**Goal:** Build extensions that can't fit in Tier 1 or 2 on BTP — using CAP, SAP Build Apps, or custom microservices.

Extension Type

BTP Technology

When to Use

Complex business logic with external data

CAP (Cloud Application Programming Model)

Logic exceeds what Released BAdIs can handle

Custom Fiori app with unique UX

SAP Build Apps or Freestyle Fiori on BTP

Standard Fiori Elements insufficient

Workflow with multi-step approval

SAP Build Process Automation

Replaces SAP Business Workflow (SWDD)

External API integration

Integration Suite + Destination Service

3rd party connectivity

Event-driven processing

Event Mesh + CAP event handlers

Asynchronous reactions to S/4 business events

Mobile app

Mobile Development Kit or SAP Build Apps

Field service, warehouse scanning

Machine learning / AI

SAP AI Core + CAP

Predictive analytics, document extraction

📋 Play 12 — Forms Migration

ECC Form Technology

S/4HANA Cloud Target

Migration Path

SAPscript (SE71)

Adobe Forms (SFP) or Output Management

🔴 Full redesign — SAPscript is Level D

SmartForms (SMARTFORMS)

Adobe Forms (SFP) or Output Management

🟡 C16 auto-converts SmartForm → Adobe Form

Adobe Forms (SFP) — existing

Adobe Forms (SFP)

🟢 Usually compatible — verify interface

Custom print programs

Output Management + BRF+ rules

🔴 Redesign with Output Management

📋 Play 13 — Add-on Replacement

Scenario

Action

Add-on is cloud-certified for S/4HANA Cloud

✅ Install on S/4HANA Cloud (PCE) or consume as cloud service (PUE)

Add-on vendor provides cloud-qualified upgrade

✅ Upgrade to cloud version

Add-on is not cloud-certified but functionality is simple

Replace with Key User Extensibility or BTP extension

Add-on is not cloud-certified and functionality is complex

Engage vendor for roadmap, or build custom BTP replacement

Add-on is no longer maintained by vendor

🔴 Must replace — evaluate standard S/4HANA features first

#### 🚦 Gate 3 — Remediation Complete

*   ☐ All RETIRE objects decommissioned
*   ☐ All REPLACE objects configured and tested in standard
*   ☐ All REFIT objects extended via Tier 1/2 and ATC-validated
*   ☐ All REWRITE objects rebuilt and ATC-validated (Level A confirmed)
*   ☐ All RETAIN objects verified as Level A
*   ☐ BTP extensions deployed and connectivity tested
*   ☐ Integration Suite flows migrated and tested
*   ☐ Forms migrated and output validated
*   ☐ Incompatible add-ons replaced or decommissioned

## 16 BP Migration Deep Dive

Business Partner migration is the **#1 risk item** in every ECC → S/4HANA migration. In RISE, it's even more critical because the BP model is the **only** master data model — there are no compatibility views for KNA1/LFA1 writes.

🏗️ Architecture: ECC vs S/4HANA Business Partner

graph LR subgraph "ECC Architecture" LFA1\["LFA1  
Vendor General"\] LFB1\["LFB1  
Vendor Company"\] LFBK\["LFB1  
Vendor Bank"\] KNA1\["KNA1  
Customer General"\] KNB1\["KNB1  
Customer Company"\] KNVV\["KNVV  
Customer Sales"\] end subgraph "S/4HANA Cloud Architecture" BUT000\["BUT000  
BP General"\] BUT020\["BUT020  
BP Addresses"\] BUT100\["BUT100  
BP Roles"\] CVI\["CVI Mapping  
BP ↔ Vendor/Customer"\] ICDS\["I\_Supplier  
I\_Customer  
I\_BusinessPartner  
CDS Views"\] end LFA1 -->|Migrated to| BUT000 KNA1 -->|Migrated to| BUT000 LFB1 -->|Migrated to| BUT100 KNB1 -->|Migrated to| BUT100 BUT000 --> ICDS BUT020 --> ICDS CVI --> ICDS

📋 BP Migration Steps

Step

Activity

Tool / Approach

Effort

1

Data quality analysis — find duplicates, inconsistencies, missing fields in KNA1/LFA1

Custom ABAP report or Data Quality Management

1-2 weeks

2

Activate CVI (Customer Vendor Integration) in ECC if not already active

Transaction BUCF — run CVI synchronization

2-3 days

3

Number range alignment — BP number = Vendor/Customer number (recommended)

Transaction BUCF — same number assignment

1 day

4

Custom field migration — map ZZ appends on KNA1/LFA1 to BP extension fields

Key User Custom Fields or E\_Supplier extension includes

1-2 weeks per custom field set

5

Custom code remediation — replace all LFA1/KNA1 SELECTs with I\_Supplier/I\_Customer CDS views or API\_BUSINESS\_PARTNER

C16 analysis + code generation

3-5 days per program

6

Integration update — replace BAPI\_VENDOR\_CREATE/BAPI\_CUSTOMER\_CREATE with BP APIs

API\_BUSINESS\_PARTNER OData

2-3 days per interface

## 17 Integration Suite Migration

RISE with SAP includes SAP Integration Suite as the **mandatory integration platform** — replacing SAP PI/PO for new integrations. This section covers the migration path for each integration pattern.

🔄 PI/PO → Cloud Integration (CPI) Migration

PI/PO Object

CPI Equivalent

Migration Tool

Complexity

Integration Flow (ICO)

Integration Flow (iFlow)

SAP Migration Assessment Tool

🟡 Medium

Mapping (Message Mapping)

Message Mapping / XSLT / Groovy Script

Semi-automatic + manual review

🟡 Medium

Adapter (RFC, IDoc, SOAP, File)

CPI Adapters (RFC, IDoc, SOAP, SFTP, HTTP)

Reconfigure per adapter

🟢 Low

ccBPM (Business Process)

Integration Flow with sequential/parallel routing

Manual redesign

🔴 High

Alerts & Monitoring

CPI Monitoring + Alert Notification Service

Reconfigure

🟢 Low

📡 New Integration Patterns (Event Mesh + API Management)

ECC Pattern

RISE Pattern

When to Use

Polling (scheduled job reads table)

**Event Mesh** — S/4 publishes event, BTP/3rd party subscribes

Any "check for new data" pattern → replace with real-time events

Direct RFC call to external system

**API Management** → governed API gateway with rate limiting + analytics

Any outbound API call

Tight coupling (synchronous RFC)

**Event-driven decoupling** — publish event, consume asynchronously

When real-time response isn't needed

File-based integration (AL11)

**CPI + SFTP adapter** or **Document Management Service**

Legacy file drops

## 18 Phase 4 — MIGRATE (Build, Test, Go-Live)

📋 Play 15 — System Build & Configuration

Activity

PCE Approach

PUE Approach

System provisioning

SAP provisions dedicated tenant on hyperscaler

SAP activates tenant in multi-tenant environment

Base configuration

IMG-based (classic) + SAP Best Practices activation

SAP Best Practices self-service configuration

Org structure setup

Manual via IMG (company codes, plants, sales orgs)

Self-service setup wizard

Custom code deployment

Transport from Dev → QA → Prod (gCTS or classic TMS)

N/A — no custom on-stack code (BTP deployment separate)

BTP extension deployment

cf deploy from BTP subaccount

cf deploy from BTP subaccount

Integration Suite deployment

Deploy iFlows from Design to Runtime

Deploy iFlows from Design to Runtime

Fiori Launchpad setup

SAP Build Work Zone, Standard Edition

SAP Build Work Zone, Standard Edition

📋 Play 16 — Data Migration

Data Category

Migration Tool

Key Consideration

Master Data (BP, Material, GL Accounts)

SAP S/4HANA Migration Cockpit

BP migration = highest risk. Run data quality checks first.

Open Items (FI documents, open POs, open SOs)

SAP S/4HANA Migration Cockpit

Only open/active documents — closed docs optionally archived.

Historical Data (FI/CO, MM, SD)

SAP Data Management & Landscape Transformation (DMLT) or archiving

PUE: limited history loading. Consider SAP Data Warehouse Cloud for analytics on historical data.

Custom Table Data (Z-tables)

Custom migration programs or Migration Cockpit custom objects

Custom tables must be recreated in S/4 (PCE) or BTP (PUE)

Attachments / Documents

SAP Document Management or Content Server migration

Volume can be large — plan bandwidth

📋 Play 17 — Cutover Runbook (Hour-by-Hour)

Time

Activity

Owner

Rollback Point

T-24h

Final ECC backup, lock ECC for postings

Basis Lead

Restore ECC backup

T-20h

Run final data extraction from ECC

Data Migration Lead

—

T-16h

Load master data into S/4 Cloud

Data Migration Lead

Reload from extraction

T-12h

Load transactional data (open items)

Data Migration Lead

Reload from extraction

T-8h

Verify data loads — reconciliation checks

Functional Leads

Fix & reload

T-6h

Deploy BTP extensions (final version)

BTP Architect

Rollback cf deploy

T-4h

Deploy Integration Suite flows (final version)

Integration Lead

Rollback iFlow version

T-3h

Smoke test — critical process per module

Test Manager + Functional Leads

🚨 Go/No-Go decision

T-2h

Configure Fiori Launchpad roles/groups

Security Lead

—

T-1h

Final reconciliation + go-live checklist

Project Lead

🚨 Final Go/No-Go

T=0

🚀 Go-Live — open system for users

Project Lead

—

T+1h

Monitor system health (Cloud ALM)

Cloud ALM Admin

—

T+24h

Day 1 close — verify all postings succeeded

FI Lead

—

#### 🚦 Gate 4 — Migration Complete / Go-Live Authorization

*   ☐ All data loaded and reconciled (master + transactional)
*   ☐ All BTP extensions deployed and tested
*   ☐ All Integration Suite flows active and tested
*   ☐ Smoke test passed for every module
*   ☐ Fiori Launchpad configured — all roles/tiles assigned
*   ☐ Cloud ALM monitoring active
*   ☐ Hypercare team briefed and on standby
*   ☐ Rollback plan documented and tested
*   ☐ Steering committee Go-Live authorization signed

## 19 8-Layer Testing Strategy

Layer

What's Tested

Tool

Owner

Cloud-Specific Addition

**1\. Syntax & ATC**

Code compiles, no deprecated APIs

ATC with ABAP\_CLOUD\_READINESS

ABAP Dev Lead

🆕 ABAP\_CLOUD\_READINESS is a **hard gate** — failures block deployment

**2\. Unit Tests**

Individual methods/classes

ABAP Unit / Jest (BTP)

Developers

🆕 BTP extensions need Node.js/Java unit tests too

**3\. Functional Tests**

End-to-end process per module

Cloud ALM Test Management

Functional Leads

🆕 Use Cloud ALM instead of Solution Manager

**4\. Integration Tests**

Cross-module processes (P2P, O2C, R2R)

Cloud ALM + CPI monitoring

Test Manager

🆕 Include BTP ↔ S/4 integration testing

**5\. Interface Tests**

All inbound/outbound interfaces

CPI test mode + API test tools

Integration Lead

🆕 Test Event Mesh subscriptions + CPI iFlows

**6\. Performance Tests**

Response times, throughput under load

Cloud ALM Performance Monitoring

Basis Lead

🆕 Include BTP service response times

**7\. UAT (User Acceptance)**

Business users validate processes

Cloud ALM Test Plans + manual testing

Business Process Owners

🆕 Include Fiori Launchpad UX validation

**8\. Cutover Rehearsal**

Full dry-run of go-live procedure

S/4 sandbox clone + BTP staging

Project Lead

🆕 Include BTP extension deployment + CPI flow activation

#### 🆕 Quarterly Update Testing (Ongoing — PUE Only)

PUE receives mandatory quarterly updates from SAP. After each update, you must: (1) Verify all Key User extensions still work, (2) Run regression tests on all BTP extensions that consume S/4 APIs, (3) Check CPI iFlows for compatibility. Build an **automated regression test suite** in Cloud ALM to handle this efficiently.

## 20 Timeline — 14-18 Month Roadmap

gantt title ECC to S/4HANA Cloud (RISE) Migration Roadmap dateFormat YYYY-MM-DD axisFormat %b %Y section Phase 0: Prepare Governance and Team Setup :p0a, 2026-05-01, 14d BTP Provisioning :p0b, 2026-05-01, 14d RISE Contract Finalization :p0c, 2026-05-01, 14d section Phase 1: Discover Process Catalogue :p1a, 2026-05-15, 21d Custom Code Inventory :p1b, 2026-05-15, 28d Interface and Forms Inventory :p1c, 2026-06-01, 21d Add-on Compatibility Check :p1d, 2026-06-01, 14d SAP Readiness Check :p1e, 2026-05-15, 7d GATE 1 :milestone, g1, 2026-06-30, 0d section Phase 2: Assess Fit-to-Standard Workshops :p2a, 2026-06-15, 42d Clean Core Classification :p2b, 2026-06-15, 28d Enhancement Migration Mapping :p2c, 2026-07-01, 28d 5 Rs Disposition for All Objects :p2d, 2026-07-15, 21d GATE 2 :milestone, g2, 2026-08-31, 0d section Phase 3: Remediate Quick Wins - RETIRE and REPLACE :p3a, 2026-08-01, 42d Key User Extensibility :p3b, 2026-09-01, 56d Developer Ext Sprints - PCE :p3c, 2026-09-15, 84d BTP Extensions Build :p3d, 2026-10-01, 84d BP Migration Prep :p3e, 2026-09-01, 56d Integration Suite Migration :p3f, 2026-10-01, 56d Forms Migration :p3g, 2026-10-15, 42d GATE 3 :milestone, g3, 2027-01-31, 0d section Phase 4: Migrate System Build and Config :p4a, 2027-01-15, 42d Data Migration Prep and Test :p4b, 2027-02-01, 56d Integration Testing :p4c, 2027-03-01, 42d UAT :p4d, 2027-04-01, 42d Performance Testing :p4e, 2027-04-15, 21d Cutover Rehearsal :p4f, 2027-05-01, 14d Training and Change Management :p4g, 2027-04-01, 42d GATE 4 :milestone, g4, 2027-05-31, 0d GO-LIVE :milestone, gl, 2027-06-15, 0d section Hypercare Hypercare Support :p5a, 2027-06-15, 42d

#### Timeline Notes

*   **PCE typical:** 14-18 months (includes Developer Extensibility sprints)
*   **PUE typical:** 10-14 months (no in-stack code, faster configuration)
*   **C16 acceleration:** Phases 1-2 can be compressed by 40-60% when C16 is connected (auto-scan, auto-classify)
*   **Parallel tracks:** BTP development starts in Phase 3 but can begin prototyping earlier
*   **Critical path:** BP Migration → Data Migration → UAT → Go-Live. BP readiness is the single biggest schedule risk.

## 21 Risk Register

#

Risk

Probability

Impact

Severity

Mitigation

R1

**BP Migration Data Quality** — Duplicate/incomplete vendor/customer records cause migration failures

High

Critical

CRITICAL

Run data quality analysis in Phase 1. Cleanse in ECC before migration. Assign dedicated BP migration team.

R2

**Clean Core Gap Wider Than Expected** — More Level C/D objects than estimated, remediation scope explodes

High

High

CRITICAL

Run SAP Readiness Check + C16 analysis early. Prioritize by usage. Accept phased remediation for PCE.

R3

**BTP Skill Gap** — Team lacks CAP/Node.js/Integration Suite skills for Tier 3 extensions

Medium

High

HIGH

Start BTP training in Phase 0. Engage SAP partner for initial BTP development. Use SAP Build (low-code) where possible.

R4

**PI/PO Migration Complexity** — Complex ccBPM flows don't map cleanly to CPI

Medium

High

HIGH

Inventory all ccBPM flows early. Run SAP Migration Assessment Tool. Budget extra time for complex flows.

R5

**Add-on Incompatibility** — Critical ISV add-on not certified for S/4HANA Cloud

Medium

High

HIGH

Check SAINT list in Phase 1. Engage vendor immediately. Plan BTP replacement as fallback.

R6

**Quarterly Update Breaks Extensions (PUE)** — SAP quarterly update invalidates Key User extensions or BTP API calls

Medium

Medium

MEDIUM

Build automated regression test suite. Subscribe to SAP release notes. Use only stable, versioned APIs.

R7

**Change Resistance** — Users reject Fiori UX, demand SAP GUI equivalents

Medium

Medium

MEDIUM

Start Fiori training in Phase 3. Involve key users in UAT. Highlight productivity gains. PUE: no SAP GUI option.

R8

**WM → EWM Migration Effort** — Classic WM to Embedded EWM requires full process redesign

High (if WM used)

Critical

CRITICAL

If WM is used: start EWM assessment in Phase 1. Assign dedicated WM→EWM workstream. Budget 3-4 months extra.

R9

**Network Latency for BTP Extensions** — Side-by-side BTP apps experience latency calling back to S/4

Low

Medium

MEDIUM

Design for async where possible (Event Mesh). Use BTP Private Link for low-latency connectivity. Cache reference data.

R10

**RISE Subscription Cost Overrun** — BTP consumption, Integration Suite, additional users exceed budget

Medium

Medium

MEDIUM

Monitor BTP consumption dashboard from Day 1. Right-size service plans. Use free-tier where possible during dev.

## 22 C16 Accelerator for RISE

C16 provides automated acceleration across all 4 engagement models. The table below shows what C16 delivers in each model versus manual effort:

Capability

Manual Effort

C16 Effort

Model A

Model B

Model C

Model D

**Custom Code Inventory + Classification**

2-4 weeks (10K objects)

2-4 hours

✅

✅

⚠️ From exports

—

**Per-Program Clean Core Analysis**

2-3 days per program

5-10 minutes

✅

✅

⚠️ From source files

—

**Fit-to-Standard Mapping**

1-2 days per module

30-60 minutes

✅

✅

✅

—

**Dependency Classification**

Manual cross-reference

Automatic (cloudification catalog)

✅

✅

—

—

**Clean Core Code Generation**

3-10 days per object

1-4 hours

✅

✅

—

—

**BTP Extension Audit**

2-3 days

1-2 hours

—

—

—

✅

**SmartForm → Adobe Conversion**

2-3 days per form

15-30 minutes

✅

✅

⚠️ From XML

—

**ATC ABAP\_CLOUD\_READINESS Scan**

Manual per object

Batch automated

✅

✅

—

—

**Post-Migration Compliance Audit**

2-4 weeks

1-2 days

—

✅

—

✅

**Quarterly Update Impact Analysis**

1-2 weeks per update

2-4 hours

—

✅

—

✅

#### ROI Summary

For a customer with 5,000 custom objects, C16 reduces Phase 1 (Discovery) from **4-6 weeks to 3-5 days**, and Phase 2 (Assessment) from **6-8 weeks to 2-3 weeks**. Total acceleration: **40-60% reduction in Phases 1-2 timeline and cost**.

## 23 Appendices

📋 Appendix A — ECC → S/4HANA Table Migration Reference

ECC Table

S/4HANA Table

CDS View Replacement

Impact

LFA1 (Vendor General)

BUT000 (BP General)

I\_Supplier

CRITICAL

LFB1 (Vendor Company)

BUT100 (BP Roles) + LFB1 (compatibility)

I\_SupplierCompany

CRITICAL

KNA1 (Customer General)

BUT000 (BP General)

I\_Customer

CRITICAL

KNB1 (Customer Company)

BUT100 (BP Roles) + KNB1 (compatibility)

I\_CustomerCompany

CRITICAL

BSEG (FI Line Items)

ACDOCA (Universal Journal)

I\_JournalEntry, I\_GLAccountLineItem

CRITICAL

BSIK/BSAK (Vendor Items)

ACDOCA

I\_OperationalAcctgDocItem

HIGH

BSID/BSAD (Customer Items)

ACDOCA

I\_OperationalAcctgDocItem

HIGH

BSIS/BSAS (GL Items)

ACDOCA

I\_GLAccountLineItem

HIGH

GLT0 (GL Totals)

FAGLFLEXT / ACDOCA aggregate

I\_GLAccountBalance

HIGH

MKPF/MSEG (Material Docs)

MATDOC

I\_MaterialDocumentItem

CRITICAL

KONV (Pricing Conditions)

PRCD\_ELEMENTS

I\_PricingElement

MEDIUM

CE1\*/CE2\* (CO-PA)

ACDOCA

I\_ActualPlanJrnlEntryItem

HIGH

ANLP (Asset Depreciation)

ACDOCA-based

I\_FixedAssetLineItem

HIGH

COEP (CO Line Items)

ACDOCA

I\_CostCenterActualLineItem

MEDIUM

MBEW (Material Valuation)

CKMLCR / MATDOC aggregate

I\_ProductValuation

MEDIUM

LIPS (Delivery Items)

LIPS (unchanged)

I\_DeliveryDocumentItem

LOW

EKKO/EKPO (PO Header/Item)

EKKO/EKPO (unchanged)

I\_PurchaseOrder, I\_PurchaseOrderItem

LOW

VBAK/VBAP (SO Header/Item)

VBAK/VBAP (unchanged)

I\_SalesOrder, I\_SalesOrderItem

LOW

📋 Appendix B — ECC Transaction → S/4HANA Fiori App Mapping (30+ Transactions)

Module

ECC Transaction

Description

S/4HANA Fiori App

App ID

MM

ME21N

Create Purchase Order

Create Purchase Order — Advanced

F0842

MM

ME23N

Display Purchase Order

Manage Purchase Orders

F2229

MM

ME51N

Create Purchase Requisition

Create Purchase Requisition

F1559

MM

MIGO

Goods Movement

Post Goods Receipt for PO

F4699

MM

MIRO

Invoice Verification

Create Supplier Invoice

F0859

MM

MM01/02/03

Material Master

Manage Product Master Data

F1602

MM

XK01/02/03

Vendor Master

Manage Business Partner

F1739

SD

VA01

Create Sales Order

Create Sales Order

F5370

SD

VA03

Display Sales Order

Manage Sales Orders

F1814

SD

VL01N

Create Outbound Delivery

Create Outbound Delivery

F6829

SD

VF01

Create Billing Document

Create Billing Documents

F2583

SD

XD01/02/03

Customer Master

Manage Business Partner

F1739

FI

FB01/FB50

Post GL Document

Post General Journal Entry

F0717

FI

FBL3N

GL Line Items

Display Line Items — GL

F0710

FI

FB60

Enter Vendor Invoice

Create Supplier Invoice (Non-PO)

F0862

FI

F110

Payment Run

Schedule Payment Proposals

F0742

FI

FF67/FEBAN

Bank Reconciliation

Manage Bank Statements

F1564

FI

AS01/02

Create/Change Fixed Asset

Manage Fixed Assets

F2370

CO

KP06

Cost Center Planning

Manage Cost Center Budgets

F2725

CO

KE21N

CO-PA Posting

Integrated in ACDOCA

F4595

PP

CO01

Create Production Order

Manage Production Orders

F3888

PP

MD01/02

MRP Run

MRP Live

F2737

PP

CS01/02

Create/Change BOM

Manage Bills of Material

F3643

PM

IW31

Create Maintenance Order

Create Maintenance Order

F2456

PM

IW21

Create Notification

Create Maintenance Notification

F2407

QM

QA01

Create Inspection Lot

Manage Quality Inspection

F3620

QM

QA32

Results Recording

Record Quality Results

F3621

BP

BP

Business Partner Maintenance

Manage Business Partner

F1739

WM

LT01

Create Transfer Order

EWM Warehouse Tasks

—

GR

SE16

Table Display

No Fiori equivalent (use CDS views)

—

📋 Appendix C — Clean Core Code Patterns (Before / After)

#### Pattern 1: Vendor Master Data Access

**Before (ECC — Level C):**

```
SELECT lifnr name1 land1 FROM lfa1
  INTO TABLE @DATA(lt_vendors)
  WHERE lifnr IN @s_lifnr.
```

**After (RISE — Level A):**

```
SELECT Supplier, SupplierName, Country
  FROM I_Supplier
  WHERE Supplier IN @s_lifnr
  INTO TABLE @DATA(lt_vendors).
```

#### Pattern 2: Financial Line Items

**Before (ECC — Level D):**

```
SELECT bukrs belnr gjahr buzei FROM bseg
  INTO TABLE @DATA(lt_items)
  WHERE bukrs = @p_bukrs
    AND gjahr = @p_gjahr.
```

**After (RISE — Level A):**

```
SELECT CompanyCode, AccountingDocument, FiscalYear, AccountingDocumentItem
  FROM I_JournalEntry
  WHERE CompanyCode = @p_bukrs
    AND FiscalYear = @p_gjahr
  INTO TABLE @DATA(lt_items).
```

#### Pattern 3: Material Document Access

**Before (ECC — Level D):**

```
SELECT m~mblnr m~mjahr m~budat s~matnr s~werks s~menge
  FROM mkpf AS m
  INNER JOIN mseg AS s ON m~mblnr = s~mblnr AND m~mjahr = s~mjahr
  INTO TABLE @DATA(lt_matdocs)
  WHERE m~budat IN @s_budat.
```

**After (RISE — Level A):**

```
SELECT MaterialDocument, MaterialDocumentYear, PostingDate,
       Material, Plant, QuantityInBaseUnit
  FROM I_MaterialDocumentItem
  WHERE PostingDate IN @s_budat
  INTO TABLE @DATA(lt_matdocs).
```

#### Pattern 4: ALV Report Output

**Before (ECC — Level B):**

```
CALL FUNCTION 'REUSE_ALV_GRID_DISPLAY'
  EXPORTING
    i_callback_program = sy-repid
    it_fieldcat        = lt_fcat
  TABLES
    t_outtab           = lt_output.
```

**After (RISE PCE — Level A):**

```
TRY.
    cl_salv_table=>factory(
      IMPORTING r_salv_table = DATA(lo_alv)
      CHANGING  t_table      = lt_output ).
    lo_alv->get_functions( )->set_all( ).
    lo_alv->display( ).
  CATCH cx_salv_msg INTO DATA(lx_msg).
    MESSAGE lx_msg->get_text( ) TYPE 'S' DISPLAY LIKE 'E'.
ENDTRY.
```

**After (RISE PUE — Level A, on BTP):**

```
// CAP Service Handler (Node.js)
module.exports = cds.service.impl(async function() {
  this.on('READ', 'VendorReport', async (req) => {
    const { I_Supplier } = this.entities
    return SELECT.from(I_Supplier)
      .where(req.query.SELECT.where)
  })
})
```

#### Pattern 5: BDC Transaction Call

**Before (ECC — Level D):**

```
CALL TRANSACTION 'XK01' USING lt_bdcdata
  MODE 'N' UPDATE 'S'
  MESSAGES INTO lt_messages.
```

**After (RISE — Level A):**

```
* Use Released API: API_BUSINESS_PARTNER
DATA(lo_http) = cl_web_http_client_manager=>create_by_http_destination(
  i_destination = cl_http_destination_provider=>create_by_comm_arrangement(
    comm_scenario  = 'Z_COMM_BP'
    service_id     = 'Z_API_BP_SRV' ) ).
DATA(lo_request) = lo_http->get_http_request( ).
lo_request->set_header_field( i_name = 'Content-Type' i_value = 'application/json' ).
lo_request->set_text( lv_json_payload ).
DATA(lo_response) = lo_http->execute( if_web_http_client=>post ).
DATA(lv_status) = lo_response->get_status( )-code.
```

📋 Appendix D — Quarterly Update Regression Checklist (PUE)

Run this checklist after every SAP quarterly update on PUE:

#

Check

Tool

Pass Criteria

1

All Key User custom fields still visible

Manual spot check in Fiori apps

Fields appear, data intact

2

All Key User custom logic (BRF+) still active

Custom Logic app → review rules

Rules active, no errors

3

All BTP extensions respond correctly

Automated API tests (Jest/Mocha)

All endpoints return 200

4

All CPI iFlows process without errors

CPI Monitoring dashboard

No failed messages

5

Event Mesh subscriptions active

Event Mesh cockpit

Events flowing

6

Fiori Launchpad tiles load correctly

Manual + Cloud ALM synthetic monitoring

All tiles active

7

Critical business process end-to-end

Cloud ALM test plans (automated)

All test steps pass

8

Performance baseline comparison

Cloud ALM performance monitoring

No degradation >10%

📋 Appendix E — Getting Started — Week 1 Action Plan

Day

Action

Owner

Deliverable

Day 1

Confirm RISE edition (PCE vs PUE)

Project Sponsor

Edition decision documented

Day 1

Assign project team (per RACI matrix in Section 6)

Project Lead

Team roster with roles

Day 2

Run SAP Readiness Check on ECC

Basis Lead

Readiness Check report

Day 2

Provision BTP Global Account (if not existing)

BTP Architect

BTP cockpit access

Day 3

Start artifact extraction (Phase 0 checklist)

Functional Leads + Basis

Extraction underway

Day 3

Establish C16 connectivity (Model A or B)

Basis Lead

C16 connected or Model C confirmed

Day 4

Kickoff meeting — present this playbook to full team

Project Lead

Team alignment

Day 5

Begin Phase 1 Discovery (Play 1-4)

All workstream leads

Discovery sprints launched

**ECC → S/4HANA Cloud (RISE) Migration Playbook** — Generated by C16 on 14 April 2026

**Playbook 2 of 2** — Companion to the ECC → S/4HANA On-Premise Migration Playbook

This is a universal, customer-agnostic framework. Adapt to specific customer context during engagement.

**Document Version:** 1.0 | **Classification:** Internal Use | **C16 Platform Version:** 2026.04

**Methodology:** Content derived from SAP S/4HANA 2025 documentation, SAP Note 2313884 (Simplification List), SAP Fiori Apps Library, SAP BTP documentation, SAP Clean Core guidelines, and C16's cloudification catalog. Business process mappings, table migrations, and CDS view references are based on SAP's published standard. Customer-specific data is populated during engagement via C16's automated scanning (Models A/B) or manual extraction (Model C).