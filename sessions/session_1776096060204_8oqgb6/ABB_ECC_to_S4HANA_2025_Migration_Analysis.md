  ABB ECC → S/4HANA 2025 Migration Analysis :root { --primary: #6B3FA0; --secondary: #B5567A; --accent: #C97B3A; --green: #27ae60; --blue: #3498db; --orange: #e67e22; --red: #e74c3c; --dark: #2c3e50; --light-bg: #e9ecef; --white: #ffffff; } \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', Arial, sans-serif; color: var(--dark); background: var(--light-bg); line-height: 1.7; } .doc-wrapper { max-width: 1120px; margin: 30px auto; background: var(--white); box-shadow: 0 4px 24px rgba(0,0,0,0.10); border-radius: 12px; overflow: hidden; } .header-banner { background: linear-gradient(135deg, #C97B3A, #B5567A, #6B3FA0); color: var(--white); padding: 40px 48px; } .header-banner img { height: 50px; margin-bottom: 12px; border-radius: 8px; } .header-banner h1 { font-size: 2.2em; margin-bottom: 8px; font-weight: 700; } .header-banner .subtitle { font-size: 1.2em; opacity: 0.92; margin-bottom: 12px; } .header-banner .badge-row { display: flex; gap: 12px; flex-wrap: wrap; margin-top: 12px; } .header-banner .badge { background: rgba(255,255,255,0.22); padding: 4px 16px; border-radius: 20px; font-size: 0.92em; } .meta-bar { background: #f4f0fa; padding: 18px 48px; display: flex; flex-wrap: wrap; gap: 28px; border-bottom: 2px solid #ece4f6; font-size: 0.97em; } .meta-bar .meta-item { display: flex; align-items: center; gap: 6px; } .meta-bar .meta-label { font-weight: 700; color: var(--primary); } .content { padding: 36px 48px; } .toc { background: #f8f6fb; border: 1px solid #ece4f6; border-radius: 10px; padding: 24px 32px; margin-bottom: 36px; } .toc h2 { color: var(--primary); font-size: 1.2em; margin-bottom: 12px; } .toc ol { padding-left: 20px; } .toc li { margin-bottom: 6px; } .toc a { color: var(--primary); text-decoration: none; } .toc a:hover { text-decoration: underline; color: var(--secondary); } .section-title { color: var(--primary); font-size: 1.5em; margin: 40px 0 18px 0; padding-bottom: 10px; border-bottom: 3px solid var(--secondary); display: flex; align-items: center; gap: 12px; } .section-title .num { background: var(--secondary); color: #fff; width: 34px; height: 34px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.85em; font-weight: 700; flex-shrink: 0; } .exec-summary { border-left: 5px solid var(--green); background: #f6fef9; padding: 20px 28px; border-radius: 0 8px 8px 0; margin-bottom: 24px; } .exec-summary h3 { color: var(--green); margin-bottom: 10px; } .kpi-row { display: flex; gap: 18px; flex-wrap: wrap; margin: 20px 0; } .kpi-card { flex: 1; min-width: 180px; background: linear-gradient(135deg, var(--primary), var(--secondary)); color: #fff; padding: 18px; border-radius: 10px; text-align: center; } .kpi-card .kpi-value { font-size: 2em; font-weight: 700; } .kpi-card .kpi-label { font-size: 0.88em; opacity: 0.9; margin-top: 4px; } table { width: 100%; border-collapse: collapse; margin: 18px 0; font-size: 0.95em; } thead th { background: var(--primary); color: #fff; padding: 12px 14px; text-align: left; font-weight: 600; } tbody td { padding: 10px 14px; border-bottom: 1px solid #eee; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } .coverage-fit { background: #27ae60; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; white-space: nowrap; } .coverage-config { background: #3498db; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; white-space: nowrap; } .coverage-extension { background: #e67e22; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; white-space: nowrap; } .coverage-gap { background: #e74c3c; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; white-space: nowrap; } .badge-level-a { background: #27ae60; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .badge-level-b { background: #3498db; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .badge-level-c { background: #e67e22; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .badge-level-d { background: #e74c3c; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .risk-critical { background: #e74c3c; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .risk-high { background: #e67e22; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .risk-medium { background: #f39c12; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .risk-low { background: #27ae60; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .callout { border-left: 5px solid var(--primary); background: #f4f0fa; padding: 18px 24px; border-radius: 0 8px 8px 0; margin: 18px 0; } .callout-red { border-left: 5px solid var(--red); background: #fdf0f0; padding: 18px 24px; border-radius: 0 8px 8px 0; margin: 18px 0; } .callout-orange { border-left: 5px solid var(--orange); background: #fef9f0; padding: 18px 24px; border-radius: 0 8px 8px 0; margin: 18px 0; } .callout-green { border-left: 5px solid var(--green); background: #f0fef4; padding: 18px 24px; border-radius: 0 8px 8px 0; margin: 18px 0; } .callout-blue { border-left: 5px solid var(--blue); background: #f0f7fe; padding: 18px 24px; border-radius: 0 8px 8px 0; margin: 18px 0; } details { margin: 10px 0; } details summary { background: #f0f4f8; padding: 10px 16px; border-radius: 6px; cursor: pointer; font-weight: 600; color: var(--primary); } details summary:hover { background: #e4eaf0; } pre { background: #f5f5f5; border-left: 4px solid var(--secondary); padding: 14px; border-radius: 4px; overflow-x: auto; font-size: 0.92em; line-height: 1.5; } code { font-family: 'Cascadia Code', 'Fira Code', monospace; } .mermaid { margin: 20px 0; text-align: center; } .verdict-box { border: 3px solid var(--primary); border-radius: 12px; padding: 24px; margin: 24px 0; background: linear-gradient(135deg, #f4f0fa, #fdf0f8); } .verdict-box h3 { color: var(--primary); margin-bottom: 10px; } .phase-card { border: 2px solid var(--primary); border-radius: 10px; padding: 20px; margin: 14px 0; background: #faf8fd; } .phase-card h4 { color: var(--primary); margin-bottom: 8px; } .doc-footer { background: #2c3e50; color: #bbb; padding: 28px 48px; font-size: 0.88em; } .doc-footer a { color: #B5567A; } .grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 18px; margin: 18px 0; } .grid-3 { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 18px; margin: 18px 0; } @media (max-width: 768px) { .grid-2, .grid-3 { grid-template-columns: 1fr; } .content { padding: 20px; } .header-banner { padding: 24px; } } @media print { .doc-wrapper { box-shadow: none; margin: 0; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } } .process-group { margin: 24px 0; } .process-group h4 { color: var(--secondary); border-bottom: 2px solid #ece4f6; padding-bottom: 6px; margin-bottom: 12px; } .timeline-item { display: flex; gap: 16px; margin: 12px 0; } .timeline-dot { width: 16px; height: 16px; border-radius: 50%; background: var(--primary); flex-shrink: 0; margin-top: 4px; } .timeline-content { flex: 1; }

![C16](data:image/jpeg;base64,)

# ABB ECC → S/4HANA 2025 Migration Analysis

Comprehensive Business Process Mapping, Fit-to-Standard Assessment, Gap Analysis & Clean Core Migration Strategy

📋 Strategic Assessment 🏢 Client: ABB 📦 Source: ECC (Not Connected) 🎯 Target: S/4HANA 2025 📅 April 2026 🔖 C16 v2026.04

Migration Type: ECC → S/4HANA 2025 (On-Premise)

Priorities: Fit-to-Standard · Clean Core

Known Custom Programs: ZMM\_PO\_ENHANCE, Z\_VENDOR\_SCORE, ZMM001, ZMM\_VENDOR\_REPORT, ZMM\_VENDOR\_MSME

Assessment Mode: Strategic Framework (Live system not connected)

## 📑 Table of Contents

1.  [Executive Summary](#executive-summary)
2.  [Migration Context & Objectives](#migration-context)
3.  [Business Process Mapping (ECC → S/4HANA 2025)](#process-mapping)
4.  [S/4HANA Simplification Impact Analysis](#simplification-impact)
5.  [Fit-to-Standard Assessment Framework](#fit-to-standard)
6.  [Module Deep Dive — MM (Materials Management)](#fit-mm)
7.  [Module Deep Dive — SD (Sales & Distribution)](#fit-sd)
8.  [Module Deep Dive — FI/CO (Finance & Controlling)](#fit-fi)
9.  [Module Deep Dive — PP/PM/QM (Production, Plant Maintenance, Quality)](#fit-pp)
10.  [Module Deep Dive — WM/EWM (Warehouse Management)](#fit-wm)
11.  [Business Partner Migration Strategy](#bp-migration)
12.  [Clean Core Compliance Strategy](#clean-core)
13.  [Custom Code Disposition Analysis](#custom-code)
14.  [Gap Analysis & Closure Strategies](#gap-analysis)
15.  [Interface & Integration Migration](#interface-migration)
16.  [Forms & Output Management Migration](#forms-output)
17.  [Data Migration Strategy](#data-migration)
18.  [Testing & Validation Strategy](#testing-strategy)
19.  [Migration Roadmap & Timeline](#migration-roadmap)
20.  [Risk Assessment & Mitigation](#risk-assessment)
21.  [Strategic Recommendations](#recommendations)
22.  [Immediate Next Steps](#next-steps)

## 1 Executive Summary

### Strategic Assessment Overview

This document provides a comprehensive migration analysis for ABB's planned transition from SAP ECC to S/4HANA 2025. The analysis establishes a structured framework for business process mapping, Fit-to-Standard evaluation, Clean Core compliance, and gap remediation — with SAP's Fit-to-Standard methodology and Clean Core principles as the governing priorities.

ABB's ECC system is **not connected to C16**, which means this analysis operates as a **strategic framework and methodology guide** rather than a live-data assessment. All process mappings are based on standard SAP ECC-to-S/4HANA change patterns, known custom programs (ZMM\_PO\_ENHANCE, Z\_VENDOR\_SCORE, ZMM001, ZMM\_VENDOR\_REPORT, ZMM\_VENDOR\_MSME), and SAP's published Simplification List (SAP Note 2313884). Once C16 is connected to ABB's ECC, every assessment in this document can be verified and quantified against live data.

12+

SAP Modules Impacted

5

Known Custom Programs

38+

Major Simplification Items

4

Migration Phases

~13 mo

Estimated Timeline

**⚠️ Critical Migration Risks Identified:**

*   **Business Partner Migration (KNA1/LFA1 → BP)** — Mandatory in S/4HANA. Every custom vendor/customer report, interface, and enhancement must be remapped. This is consistently the #1 risk area across all S/4HANA migrations.
*   **Material Ledger Activation** — Mandatory in S/4HANA. All CO-PA, material valuation, and costing reports must be reviewed.
*   **MATDOC Consolidation** — MKPF/MSEG merge into MATDOC. All custom inventory reports must be rewritten.
*   **New GL / ACDOCA** — BSEG replaced by ACDOCA for reporting. All custom FI line-item reports impacted.
*   **Custom ZMM\_VENDOR\_MSME** — Custom MSME classification logic with Z-tables has no standard S/4HANA equivalent (confirmed GAP).

## 2 Migration Context & Objectives

### 2.1 Current Landscape

Attribute

Current State (ECC)

Target State (S/4HANA 2025)

Platform

SAP ECC 6.0 (EHP 7/8)

SAP S/4HANA 2025 On-Premise

Database

Oracle / DB2 / SQL Server / HANA

SAP HANA (mandatory)

UI Layer

SAP GUI, limited Fiori

SAP Fiori Launchpad (primary)

Programming Model

Classic ABAP (Reports, BDC, BAPI, ALV)

RESTful ABAP Programming (RAP), CDS, Fiori Elements

Extension Model

Customer Exits, BAdIs, Modifications

Clean Core: Released APIs, Key User, BTP Side-by-Side

Master Data Model

KNA1/LFA1 (Customer/Vendor)

Business Partner (BUT000, BUT020, etc.)

Inventory Model

MKPF + MSEG (separate header/item)

MATDOC (unified single table)

Finance Model

BSEG (line items), BSIS/BSAS (GL), separate ledgers

ACDOCA (Universal Journal), merged ledgers

Custom Objects

Estimated 5,000-30,000 (large enterprise)

Target: Level A/B Clean Core compliance

### 2.2 Migration Objectives (Priority Ordered)

**🥇 Priority 1: Fit-to-Standard**

Maximize adoption of SAP standard functionality. Retire custom code wherever standard S/4HANA 2025 processes exist. Every custom object must be justified against the standard.

**🥈 Priority 2: Clean Core**

All retained or new custom code must use only SAP Released APIs (Level A). No direct standard table access, no unreleased APIs, no modifications. Cloud-extensibility ready.

**🥉 Priority 3: Process Simplification**

Reduce process variants. Harmonize across plants/company codes. Eliminate redundant custom solutions that evolved organically on ECC.

**4️⃣ Priority 4: UX Modernization**

Migrate from SAP GUI to Fiori Launchpad. Leverage Fiori Elements for new apps. Role-based Fiori spaces for end users.

## 3 Business Process Mapping (ECC → S/4HANA 2025)

The following maps every major ECC business process to its S/4HANA 2025 equivalent, noting changes, Fiori app replacements, and migration complexity. Processes are organized by SAP module.

graph TB subgraph "ECC Landscape" A\[MM - Procurement\] B\[SD - Sales\] C\[FI/CO - Finance\] D\[PP - Production\] E\[PM - Maintenance\] F\[QM - Quality\] G\[WM - Warehouse\] H\[HR/HCM\] end subgraph "S/4HANA 2025 Target" A2\[Sourcing & Procurement\] B2\[Order to Cash\] C2\[Finance & Risk\] D2\[Digital Manufacturing\] E2\[Asset Management\] F2\[Quality Management\] G2\[Embedded EWM\] H2\[SuccessFactors / HCM\] end A --> A2 B --> B2 C --> C2 D --> D2 E --> E2 F --> F2 G --> G2 H --> H2 style A fill:#e74c3c,color:#fff style B fill:#e67e22,color:#fff style C fill:#3498db,color:#fff style D fill:#27ae60,color:#fff style E fill:#9b59b6,color:#fff style F fill:#1abc9c,color:#fff style G fill:#e74c3c,color:#fff style H fill:#34495e,color:#fff style A2 fill:#6B3FA0,color:#fff style B2 fill:#6B3FA0,color:#fff style C2 fill:#6B3FA0,color:#fff style D2 fill:#6B3FA0,color:#fff style E2 fill:#6B3FA0,color:#fff style F2 fill:#6B3FA0,color:#fff style G2 fill:#6B3FA0,color:#fff style H2 fill:#6B3FA0,color:#fff

#### 3.1 Materials Management (MM) — Procure-to-Pay

#

ECC Process

ECC T-Code

S/4HANA 2025 Process

Fiori App

Change Impact

1

Create Purchase Requisition

ME51N

Same + Simplified

F1510 — Manage Purchase Requisitions

Low

2

Create Purchase Order

ME21N

Same + Fiori

F0842 — Create Purchase Order

Low

3

Release Purchase Order

ME28/ME29N

Same + Workflow

F0843 — Approve Purchase Orders

Low

4

Goods Receipt

MIGO

Same (MATDOC backend)

F1083 — Post Goods Receipt for PO

Medium — MATDOC replaces MSEG

5

Invoice Verification

MIRO

Same

F0862 — Create Supplier Invoice

Low

6

Vendor Master

XK01/XK02/XK03

Business Partner

F1739 — Manage Supplier

Critical — BP Migration mandatory

7

Source List / Quota

ME01/MEQ1

Same + enhanced sourcing

F2211 — Manage Source of Supply

Low

8

Material Master

MM01/MM02/MM03

Same (simplified views)

F1602 — Manage Product Master Data

Medium — Some deprecated fields

9

Vendor Evaluation

ME61/ME62/ME63

Same

F2669 — Supplier Evaluation

Low

10

MRP Run

MD01/MD02

MRP Live (ppMRP)

F2732 — Monitor Material Coverage

High — New MRP engine

11

Inventory Management

MMBE/MB52

Same (MATDOC backend)

F0842A — Stock Overview

Medium — MATDOC backend

12

Contract Management

ME31K/ME32K

Central Contracts

F4580 — Manage Purchase Contracts

Medium

#### 3.2 Sales & Distribution (SD) — Order-to-Cash

#

ECC Process

ECC T-Code

S/4HANA 2025 Process

Fiori App

Change Impact

1

Create Sales Order

VA01

Same + Compatibility Views

F5370 — Manage Sales Orders

Low

2

Sales Order Processing

VA02/VA03

Same

F5370

Low

3

Delivery Processing

VL01N/VL02N

Same

F1814 — Create Outbound Deliveries

Low

4

Billing

VF01/VF02

Same

F2093 — Manage Billing Documents

Low

5

Customer Master

XD01/XD02/XD03

Business Partner

F1736 — Manage Business Partner

Critical — BP Migration mandatory

6

Credit Management

FD32/FD33

FSCM Credit Management

F1498 — Manage Credit Limit

High — New credit engine

7

Pricing

VK11/VK12

Same + BRF+ integration

F5549 — Manage Sales Conditions

Medium

8

Available to Promise

CO09

aATP (Advanced ATP)

F4135 — Product Availability

High — If custom ATP exists

9

Output Determination

NACE/VV31

Output Management (BRF+)

Configuration based

High — New output framework

10

Returns Processing

VA01 (RE)

Advanced Returns Management

F5898

Medium

#### 3.3 Finance & Controlling (FI/CO)

#

ECC Process

ECC T-Code

S/4HANA 2025 Process

Fiori App

Change Impact

1

GL Postings

FB50/FB01

Same (ACDOCA backend)

F0717 — Post General Journal Entries

Medium — ACDOCA replaces BSEG for reporting

2

Accounts Payable

FB60/F-43

Same

F0859 — Post Supplier Invoices

Low

3

Accounts Receivable

FB70/F-22

Same

F0718 — Post Customer Invoices

Low

4

Payments

F110

Same + enhanced

F0730 — Schedule Payment Proposals

Low

5

Bank Reconciliation

FF67/FEBA

Advanced Bank Reconciliation

F2782 — Manage Bank Statements

Medium

6

Asset Accounting

AS01/AS02

New Asset Accounting (migration)

F2485 — Manage Fixed Assets

High — New AA engine mandatory

7

Cost Center Accounting

KS01/KSB1

Same (ACDOCA)

F1028 — Cost Centers - Plan/Actual

Medium

8

Profit Center Accounting

KE51/KE5Z

Merged into Universal Journal

F2214 — Manage Profit Centers

High — Merged architecture

9

CO-PA (Profitability Analysis)

KE21N/KE24

Account-based CO-PA (ACDOCA)

F2305 — Profitability Analysis

Critical — Costing-based CO-PA deprecated

10

Period-End Close

Various (CJCO, MR21, etc.)

Simplified with Fiori

F1596 — Manage Closing Tasks

Medium

11

Revenue Recognition

Manual/Custom

IFRS 15 / RAR

F4958 — Revenue Accounting

High — If applicable

#### 3.4 Production, Maintenance, Quality (PP/PM/QM)

#

ECC Process

S/4HANA 2025 Process

Change Impact

PP-1

Production Order (CO01/CO02)

Same + Fiori (F2723)

Low

PP-2

Production Scheduling (CM01)

Same + enhanced with PP/DS

Medium

PP-3

MRP (MD01/MD02)

MRP Live (ppMRP on HANA)

High

PP-4

BOM Management (CS01)

Same + Fiori (F4439)

Low

PM-1

Maintenance Order (IW31)

Same + Fiori (F2680)

Low

PM-2

Maintenance Notification (IW21)

Same + Fiori (F2416)

Low

QM-1

Quality Inspection (QA01)

Same + Fiori (F1895)

Low

QM-2

Quality Certificate (QC21)

Same + enhanced

Low

## 4 S/4HANA Simplification Impact Analysis

SAP Note **2313884** (S/4HANA Simplification List) defines all table, transaction, and function changes between ECC and S/4HANA. The following are the **highest-impact simplification items** for a manufacturing company like ABB:

Simpl. Item

Area

Change Description

Impact on ABB

Severity

S4TWL-001

BP Migration

KNA1/KNB1/KNVV, LFA1/LFB1/LFBK consolidated into Business Partner (BUT000, BUT020, CVI mapping)

All custom vendor/customer master programs must migrate

Critical

S4TWL-002

Material Document

MKPF + MSEG replaced by MATDOC (single table)

Custom inventory/GR reports on MSEG must be rewritten

High

S4TWL-003

Universal Journal

BSEG → ACDOCA for reporting; BSIS/BSAS/BSIK/BSAK/BSID/BSAD become compatibility views

Custom FI reports must use ACDOCA or compatibility views

High

S4TWL-004

Material Ledger

Material Ledger mandatory for all materials

Impacts material valuation, costing, CO-PA

High

S4TWL-005

CO-PA

Costing-based CO-PA deprecated; Account-based CO-PA in ACDOCA

Custom profitability reports need redesign

Critical

S4TWL-006

New Asset Accounting

Classic AA (ANLP, ANLC) → New AA (ACDOCA integration)

Asset reports, depreciation runs, year-end close impacted

High

S4TWL-007

Credit Management

FD32 → SAP Credit Management (FSCM/UKM)

Custom credit check logic must be redesigned

High

S4TWL-008

Output Management

NACE condition-based output → Output Management (BRF+)

Custom output programs/routines must be reviewed

Medium

S4TWL-009

MRP

Classic MRP → MRP Live (ppMRP on HANA)

Custom MRP enhancements/user exits need review

High

S4TWL-010

Warehouse Mgmt

LE-WM removed → Embedded EWM or Stock Room Mgmt

If ABB uses classic WM, full WM→EWM migration needed

Critical

S4TWL-011

Batch Management

New Batch Management (classification-based)

Custom batch logic may need adjustment

Medium

S4TWL-012

SD Billing

Billing document persistence changes

Custom billing reports may need adjustment

Medium

**🚨 Breaking Changes (Will Not Work Without Remediation):**

*   **Direct SELECT on MSEG** — MSEG still exists as a compatibility view but is NOT recommended for new code. MATDOC is the single source of truth.
*   **Direct INSERT/UPDATE on KNA1/LFA1** — These tables are read-only in S/4HANA. Must use BP APIs (BAPI\_BUPA\_CREATE, I\_BusinessPartnerTP RAP BO).
*   **BSEG for reporting** — BSEG exists but must NOT be used for reporting. ACDOCA is the correct source.
*   **Classic WM (LE-WM)** — Completely removed. No compatibility mode.
*   **Costing-based CO-PA** — Deprecated. Only account-based CO-PA on ACDOCA.

## 5 Fit-to-Standard Assessment Framework

Every custom process and Z/Y object at ABB must pass through this Fit-to-Standard evaluation. The methodology decomposes each custom solution into discrete business requirements, then maps each requirement against S/4HANA 2025 standard capabilities.

### 5.1 Classification Framework

Classification

Definition

Action

Example

FIT

S/4HANA standard covers this requirement as-is, no changes needed

ADOPT — use standard

Create PO → ME21N / F0842

FIT WITH CONFIG

Standard covers it but requires configuration (customizing, BRF+ rules, Key User extensibility)

SIMPLIFY — configure, no code

Custom approval workflow → Standard Flexible Workflow with config

FIT WITH EXTENSION

Standard covers 70-80%, needs a compliant extension (BAdI, RAP, BTP side-by-side) for the remainder

MODERNIZE — standard + Clean Core extension

Vendor report with custom MSME fields → I\_Supplier CDS + E\_Supplier extension

GAP

No standard equivalent exists. Must be custom-built using Clean Core principles.

BUILD — new Clean Core custom app

Custom MSME certification tracking → New RAP BO + Fiori app

### 5.2 Per-Requirement Assessment Template

For each custom object, decompose into requirements and classify each:

**Example: ZMM\_VENDOR\_REPORT (Custom Vendor Consolidated Report)**

#

Business Requirement

Classification

Standard Alternative

Confidence

R1

Display vendor general data (name, address, contact)

FIT

F1739 — Manage Supplier (I\_Supplier CDS)

100%

R2

Display vendor company code data (payment terms, recon account)

FIT

F1739 — Manage Supplier (I\_SupplierCompany CDS)

100%

R3

Display vendor purchasing data (incoterms, purch. org)

FIT

F1739 — Manage Supplier (I\_SupplierPurchasingOrg CDS)

100%

R4

Show vendor bank details

FIT

F1739 — I\_SupplierBank CDS

100%

R5

Show vendor email addresses

FIT

I\_AddressEmailAddress CDS

95%

R6

Show vendor evaluation score

FIT WITH CONFIG

F2669 — Supplier Evaluation

85%

R7

Show vendor block/unblock status

FIT

I\_Supplier — PurchasingIsBlocked, PaymentIsBlockedForSup

100%

R8

Show open PO count per vendor

FIT WITH CONFIG

I\_PurchaseOrderAPI01 CDS + aggregation

80%

R9

Show MSME classification

GAP

No standard — custom Z-table (ZMM\_VENDOR\_MSME)

N/A

R10

Show BEE (Broad Based Black Economic Empowerment) status

GAP

No standard — custom Z-table (ZMM\_VENDOR\_BEE)

N/A

R11

Multi-company code / plant filter

FIT

Standard Fiori filter bar with value helps

100%

R12

ALV grid output with export to Excel

FIT

Fiori List Report with built-in export

100%

R13

Consolidated single-screen view

FIT WITH EXTENSION

Fiori Object Page with sections — needs custom section for MSME/BEE

70%

R14

Vendor blacklist status

GAP

No standard — custom Z-table (ZVENDOR\_BLOCK)

N/A

**Coverage Summary:**

Standard Alternative

Requirements Covered

Coverage %

F1739 — Manage Supplier

R1, R2, R3, R4, R5, R7, R11, R12

57% (8/14)

F2669 — Supplier Evaluation

R6

7% (1/14)

Custom Extension (E\_Supplier + RAP)

R8, R13

14% (2/14)

**GAP (no standard)**

R9, R10, R14

**21% GAP**

### Verdict: MODERNIZE

Standard S/4HANA covers ~79% of ZMM\_VENDOR\_REPORT requirements. The remaining 21% (MSME classification, BEE status, vendor blacklist) are ABB-specific master data enrichments with no standard equivalent. **Recommendation:** Adopt F1739 as the base, extend with E\_Supplier custom fields for MSME/BEE/Blacklist data using Clean Core extension views, and build a supplementary Fiori tile for the consolidated view.

## 6 Module Deep Dive — MM (Materials Management)

### 6.1 Known Custom Objects Analysis

Custom Object

Type

Business Purpose

Disposition

Effort

**ZMM\_PO\_ENHANCE**

BAdI Implementation

Enhances PO creation with custom validation logic, additional checks, possibly custom fields on PO header/item

REFIT — Review BAdI against S/4HANA released BAdIs (MM\_PUR\_S4\_PO\_\*). Rewrite implementation using Released APIs only.

Medium

**Z\_VENDOR\_SCORE**

Program (Report)

Custom vendor scoring/evaluation report — likely reads LFA1, EKKO/EKPO, quality data to compute vendor performance scores

REPLACE — S/4HANA has enhanced Supplier Evaluation (ME61 + F2669) with configurable criteria. If standard criteria cover ABB's scoring model, retire. If not, extend with BAdI.

Medium

**ZMM001**

Transaction

Custom transaction for vendor management workflow — likely wraps a custom report or dialog program

REPLACE / RETIRE — Map to standard Fiori app. F1739 (Manage Supplier) + F0842 (Create PO) likely cover the use case.

Low

**ZMM\_VENDOR\_REPORT**

Program (Report)

Consolidated vendor report — reads LFA1, LFB1, LFBK, ADRC, ADR6 + custom Z-tables for MSME/BEE/Blacklist

MODERNIZE — See Fit-to-Standard assessment above. F1739 covers 79%, gaps need Clean Core extension.

High

**ZMM\_VENDOR\_MSME**

Z-Table + Maintenance

Stores MSME certification data for vendors — vendor number + MSME category + certificate date + validity

BUILD — No standard equivalent. Must be rebuilt as a Clean Core custom RAP BO with Released APIs on S/4HANA.

High

### 6.2 MM Table Migration Map

ECC Table

Purpose

S/4HANA Status

Replacement

Custom Code Impact

LFA1

Vendor General

Read-only compatibility view

BUT000 + CVI, I\_Supplier CDS

Critical — All vendor SELECTs must migrate

LFB1

Vendor Company Code

Read-only compatibility view

BUT0BK, I\_SupplierCompany CDS

Critical

LFBK

Vendor Bank

Read-only compatibility view

BUT0BK, I\_SupplierBank CDS

High

EKKO

PO Header

Compatible

Same + I\_PurchaseOrderAPI01 CDS

Low

EKPO

PO Item

Compatible

Same + I\_PurchaseOrderItemAPI01 CDS

Low

MKPF

Material Doc Header

Compatibility view

MATDOC, I\_MaterialDocumentHeader\_2 CDS

High

MSEG

Material Doc Item

Compatibility view

MATDOC, I\_MaterialDocumentItem\_2 CDS

High

MARA

Material General

Compatible (some fields removed)

Same + I\_Product CDS

Medium

MARC

Material Plant

Compatible

Same + I\_ProductPlant CDS

Low

EBAN

Purchase Requisition

Compatible

Same + I\_PurchaseRequisitionAPI01 CDS

Low

## 7 Module Deep Dive — SD (Sales & Distribution)

### 7.1 Key Process Changes

Process Area

ECC Approach

S/4HANA 2025 Approach

Migration Effort

Customer Master

KNA1/KNB1/KNVV (XD01-XD03)

Business Partner (BP) — mandatory CVI migration

Critical

Credit Management

FD32 + custom checks

SAP Credit Management (FSCM/UKM) — new credit segment on BP

High

Output Determination

NACE/NAST condition technique

S/4HANA Output Management (BRF+) — or keep NACE for transition

High

Pricing

VK11 condition records

Same foundation — plus BRF+ for complex pricing rules

Medium

Availability Check

CO09 / custom ATP

aATP (Advanced ATP) on HANA — if ABB uses custom ATP

High

Sales Order

VA01/VA02

Same + Fiori (F5370)

Low

Billing

VF01/VF02

Same + Fiori (F2093)

Low

Returns

VA01 (order type RE)

Advanced Returns Management

Medium

Revenue Recognition

Manual / Custom

IFRS 15 / Revenue Accounting & Reporting (RAR)

High (if applicable)

### 7.2 SD Table Migration Map

ECC Table

S/4HANA Status

CDS View Replacement

KNA1 (Customer General)

Read-only view → BP

I\_Customer, I\_BusinessPartner

KNB1 (Customer Co.Code)

Read-only view → BP

I\_CustomerCompany

KNVV (Customer Sales)

Read-only view → BP

I\_CustomerSalesArea

VBAK (Sales Order Header)

Compatible

I\_SalesOrder

VBAP (Sales Order Item)

Compatible

I\_SalesOrderItem

LIKP (Delivery Header)

Compatible

I\_DeliveryDocument

VBRK (Billing Header)

Compatible

I\_BillingDocument

VBFA (Doc Flow)

Compatible

I\_SalesDocumentFlow

## 8 Module Deep Dive — FI/CO (Finance & Controlling)

### 8.1 The Universal Journal (ACDOCA) — The Biggest FI Change

graph LR subgraph "ECC Architecture" BKPF\["BKPF (Doc Header)"\] BSEG\["BSEG (Line Items)"\] BSIS\["BSIS (GL Open)"\] BSAS\["BSAS (GL Cleared)"\] BSIK\["BSIK (AP Open)"\] BSAK\["BSAK (AP Cleared)"\] BSID\["BSID (AR Open)"\] BSAD\["BSAD (AR Cleared)"\] ANLP\["ANLP (Asset Periodic)"\] COEP\["COEP (CO Line Items)"\] CE1\["CE1xxxx (CO-PA)"\] end subgraph "S/4HANA Architecture" ACDOCA\["ACDOCA  
(Universal Journal)"\] BKPF2\["BKPF (Header - unchanged)"\] end BSEG -->|merged into| ACDOCA BSIS -->|merged into| ACDOCA BSAS -->|merged into| ACDOCA BSIK -->|merged into| ACDOCA BSAK -->|merged into| ACDOCA BSID -->|merged into| ACDOCA BSAD -->|merged into| ACDOCA ANLP -->|merged into| ACDOCA COEP -->|merged into| ACDOCA CE1 -->|merged into| ACDOCA BKPF --> BKPF2 style ACDOCA fill:#6B3FA0,color:#fff,stroke-width:3px style BKPF2 fill:#27ae60,color:#fff

**⚠️ Impact on ABB:** Every custom FI report that reads BSEG, BSIS/BSAS, BSIK/BSAK, BSID/BSAD, ANLP, COEP, or CE1xxxx tables must be redesigned. These tables still exist as **compatibility views** in S/4HANA (so SELECT statements won't fail), but they have significant **performance implications** and **functional limitations**:

*   Compatibility views reconstruct data from ACDOCA — much slower than direct ACDOCA access
*   Some fields are empty in compatibility views
*   New journal entry types (e.g., ML entries, extension ledger) only visible in ACDOCA
*   **Recommendation:** Rewrite all FI reports to use ACDOCA or released CDS views (I\_JournalEntry, I\_GLAccountLineItem)

### 8.2 FI Table Migration Map

ECC Table

Purpose

S/4HANA Status

Released CDS View

BSEG

FI Line Items

Compatibility view (from ACDOCA)

I\_JournalEntry, I\_JournalEntryItem

BSIS/BSAS

GL Open/Cleared

Compatibility views

I\_GLAccountLineItem

BSIK/BSAK

AP Open/Cleared

Compatibility views

I\_OperationalAcctgDocItem

BSID/BSAD

AR Open/Cleared

Compatibility views

I\_OperationalAcctgDocItem

ANLP/ANLC

Asset Periodic Values

Removed — New AA in ACDOCA

I\_FixedAssetBalance\_2

COEP

CO Line Items

Compatibility view

I\_JournalEntryItem (with CO fields)

CE1xxxx

CO-PA Line Items

Removed (costing-based)

I\_JournalEntryItem (account-based CO-PA)

ACDOCA

Universal Journal

NEW — primary table

I\_JournalEntry, I\_GLAccountLineItem

## 9 Module Deep Dive — PP/PM/QM

### 9.1 Production Planning (PP)

PP processes in S/4HANA 2025 remain largely compatible with ECC. The major change is **MRP Live (ppMRP)** — a HANA-optimized MRP engine that replaces classic MRP. Key impacts:

Area

ECC

S/4HANA 2025

Impact

MRP Run

MD01/MD02 (classic MDPS)

MRP Live (ppMRP on HANA) — F2732

High — Custom MRP exits need review

Production Orders

CO01/CO02

Same + Fiori (F2723)

Low

BOM

CS01/CS02

Same + Fiori (F4439)

Low

Routing

CA01/CA02

Same + enhanced Fiori

Low

Demand Mgmt

MD61/MD62

Same + Demand-Driven MRP option

Medium

Capacity Planning

CM01/CM25

Same + PP/DS integration

Medium

### 9.2 Plant Maintenance (PM) & Quality Management (QM)

PM and QM are **low impact** areas. Most processes carry forward unchanged. Key enhancement: Fiori apps for mobile maintenance (F2680 — Manage Maintenance Orders) and quality inspection (F1895).

## 10 Module Deep Dive — WM/EWM (Warehouse Management)

**🚨 CRITICAL DECISION POINT: Classic WM (LE-WM) is REMOVED in S/4HANA.**

If ABB uses classic Warehouse Management (LE-WM) with transactions like LT01, LT02, LT03, LT21, LT22, etc., this is a **mandatory migration** with zero backward compatibility. ABB must choose:

Option

Description

Complexity

Recommended For

**Option A: Embedded EWM**

Full Extended Warehouse Management embedded in S/4HANA. Rich functionality: wave management, task interleaving, resource management, labor management.

Very High

Complex warehouse operations, RF-based picking, multi-level storage

**Option B: Stock Room Management**

Simplified inventory management using Inventory Management (MM-IM) functions only. No bin-level tracking.

Low

Simple storage without bin-level management needs

**Option C: Decentralized EWM**

Standalone EWM system connected to S/4HANA via RFC/APIs. Separate system for warehouse.

High

Very complex warehouses, multi-system landscapes

**📋 WM Assessment Needed:** Before deciding, ABB must inventory: number of warehouses, storage types, movement types with WM relevance, custom WM user exits (MWMIDI01-04), RF transaction usage, WM-TRM (Task & Resource Management) usage. This determines Option A vs B vs C.

## 11 Business Partner Migration Strategy

The migration from separate Customer (KNA1) and Vendor (LFA1) masters to the unified **Business Partner (BP)** model is the single most impactful change in S/4HANA. It affects every module.

graph TB subgraph "ECC Master Data" KNA1\["KNA1  
(Customer General)"\] KNB1\["KNB1  
(Customer Co.Code)"\] KNVV\["KNVV  
(Customer Sales)"\] LFA1\["LFA1  
(Vendor General)"\] LFB1\["LFB1  
(Vendor Co.Code)"\] LFBK\["LFBK  
(Vendor Bank)"\] end subgraph "S/4HANA Business Partner" BUT000\["BUT000  
(BP General)"\] BUT020\["BUT020  
(BP Addresses)"\] BUT0BK\["BUT0BK  
(BP Bank Details)"\] CVI\["CVI Mapping  
(KNA1↔BP↔LFA1)"\] BPEXT\["Custom Extensions  
(E\_Supplier, E\_Customer)"\] end KNA1 -->|CVI sync| BUT000 KNB1 -->|CVI sync| BUT000 KNVV -->|CVI sync| BUT000 LFA1 -->|CVI sync| BUT000 LFB1 -->|CVI sync| BUT000 LFBK -->|CVI sync| BUT0BK BUT000 --> CVI CVI --> BPEXT style BUT000 fill:#6B3FA0,color:#fff,stroke-width:3px style CVI fill:#B5567A,color:#fff style BPEXT fill:#C97B3A,color:#fff

### 11.1 BP Migration Phases

Phase

Activity

Tools/Approach

Effort

**Phase 1: Prepare**

Activate CVI (Customer Vendor Integration) in ECC pre-migration. Run BP synchronization in ECC to create BP records alongside existing vendor/customer masters.

Transaction BUPA\_BRAND\_GET, CVI\_ERP\_SETUP

1-2 weeks

**Phase 2: Synchronize**

Run CVI synchronization for all existing vendors/customers. Resolve duplicates (same natural person is both vendor and customer → single BP).

MDS\_LOAD\_COCKPIT, BP sync reports

2-4 weeks

**Phase 3: Custom Code**

Remap all custom code from KNA1/LFA1 direct access to I\_Supplier/I\_Customer CDS views or BP APIs. Update BDC programs, interfaces, reports.

Code scanning, C16 analysis

4-8 weeks

**Phase 4: Custom Fields**

Migrate Z-appends on KNA1/LFA1 to BP extension fields. Custom tables (ZMM\_VENDOR\_MSME, ZVENDOR\_BLOCK) need new access layer via BP number.

Key User Extensibility or custom RAP BO

2-4 weeks

**Phase 5: Validate**

Run BP consistency checks. Verify all vendors/customers are accessible via BP. Test all custom reports/interfaces with BP data.

BP consistency reports, test automation

2-3 weeks

### 11.2 Impact on ABB's Known Custom Objects

Object

Current Table Access

Required Change

ZMM\_VENDOR\_REPORT

SELECT FROM lfa1, lfb1, lfbk, adrc, adr6

Replace with I\_Supplier, I\_SupplierCompany, I\_SupplierBank CDS views or BP API

ZMM\_VENDOR\_MSME

Custom Z-table keyed on LIFNR (vendor number)

Must support access via BP number. Add BP↔Vendor mapping or re-key on BP number.

Z\_VENDOR\_SCORE

Likely reads LFA1 for vendor master data

Replace with I\_Supplier CDS view

ZMM\_PO\_ENHANCE

May reference vendor data in PO creation

Verify BAdI uses BP-compatible APIs for vendor lookup

ZMM001

Custom transaction likely wrapping vendor maintenance

Retire if F1739 covers the use case; otherwise rewire to BP

## 12 Clean Core Compliance Strategy

SAP's Clean Core framework requires all custom extensions to use only **Released APIs** — no direct standard table access, no unreleased function modules, no kernel-level APIs, no modifications. This is the foundation for S/4HANA upgrade stability and cloud extensibility.

### 12.1 Clean Core Levels

Level

Definition

S/4HANA Impact

Target for ABB

Level A

**Cloud-Ready.** Uses only Released APIs (C1 contract). CDS views with @VDM.lifecycle.contract.type:#PUBLIC\_REMOTE\_API. RAP BOs. Released BAdIs.

Fully upgrade-safe. BTP-extensible. Future-proof.

✅ Target for ALL new code

Level B

**Classic API.** Stable BAPIs (BAPI\_PO\_CREATE1), CL\_SALV\_TABLE, REUSE\_ALV. Works on S/4HANA on-prem but not cloud-released.

Works on S/4HANA on-premise. Not BTP-extensible.

⚠️ Acceptable for on-prem, modernize when feasible

Level C

**Non-Compliant.** Direct SELECT on standard tables (LFA1, EKKO, BSEG), unreleased FMs, internal APIs.

Works today but may break on future upgrades. No cloud path.

🔴 Must remediate

Level D

**Critical.** Deprecated/removed objects (MSEG for write, MODIFY LFA1), kernel modifications, user exits on removed functionality.

Will BREAK on S/4HANA.

🚨 Mandatory rewrite

### 12.2 Clean Core Classification Approach

graph LR A\["Scan Custom Code  
(TADIR + Source)"\] --> B\["Classify Dependencies  
(Released API check)"\] B --> C{"Level?"} C -->|A| D\["✅ Keep as-is"\] C -->|B| E\["⚠️ Modernize if practical"\] C -->|C| F\["🔴 Rewrite with CDS/RAP"\] C -->|D| G\["🚨 Mandatory rewrite"\] style A fill:#6B3FA0,color:#fff style D fill:#27ae60,color:#fff style E fill:#3498db,color:#fff style F fill:#e67e22,color:#fff style G fill:#e74c3c,color:#fff

### 12.3 ABB's Estimated Clean Core Distribution

Based on typical large manufacturing ECC systems, the expected distribution is:

~5-10%

Level A (Cloud-Ready)

~15-25%

Level B (Classic API)

~50-60%

Level C (Non-Compliant)

~10-20%

Level D (Critical/Breaking)

**💡 Once C16 is connected to ABB's ECC:** C16 can auto-classify ALL custom objects in hours. For each object, it runs the 3-layer classification pipeline (cloudification catalog + ATC ABAP\_CLOUD\_READINESS + dependency tree analysis) to produce an exact Level A/B/C/D rating with specific remediation steps per finding.

### 12.4 Clean Core Remediation Patterns

ECC Pattern (Non-Compliant)

Clean Core Replacement

Example

SELECT \* FROM lfa1 WHERE lifnr = ...

SELECT FROM I\_Supplier WHERE Supplier = ...

Released CDS view with stable contract

CALL FUNCTION 'BAPI\_PO\_CREATE1'

Modify entity I\_PurchaseOrderTP (RAP API)

Released RAP BO for transactional PO creation

SELECT FROM mseg WHERE mblnr = ...

SELECT FROM I\_MaterialDocumentItem\_2

Released CDS view reading from MATDOC

SELECT FROM bseg WHERE bukrs = ...

SELECT FROM I\_JournalEntryItem

Released CDS view reading from ACDOCA

REUSE\_ALV\_GRID\_DISPLAY

CL\_SALV\_TABLE (Level B) or Fiori Elements List Report (Level A)

Level B is acceptable on-prem; Level A for cloud

CALL TRANSACTION 'XK01' USING bdc\_tab

BP RAP API or BAPI\_BUPA\_CREATE\_FROM\_DATA

No more BDC recordings against removed transactions

MODIFY lfa1 FROM wa\_lfa1

BP API (MODIFY ENTITY I\_BusinessPartnerTP)

Direct writes to standard tables = Level D in S/4HANA

User Exit (CMOD/SMOD)

Released BAdI implementation

Check SXS\_ATTR for released BAdIs in the same area

## 13 Custom Code Disposition Analysis

### 13.1 The 5 Rs Framework

Every custom object gets exactly ONE disposition:

Disposition

Definition

Effort

When to Apply

RETIRE

Standard S/4HANA replaces the custom solution entirely. Decommission.

Low

Standard Fiori app / transaction covers 100% of requirements

REPLACE

Standard covers 90%+. Minor config needed, no custom code.

Low-Med

Standard app covers requirements with customizing only

REFIT

Standard covers 70-89%. Needs Clean Core extension (BAdI/RAP) for the rest.

Medium

BAdI or Key User extensibility fills the gap

REWRITE

Logic must stay custom but code is Level C/D. Full rewrite with Released APIs.

High

Unique business logic, but current implementation uses deprecated patterns

RETAIN

Already Level A/B, unique IP. Validate on S/4HANA and keep.

Low

Already uses Released APIs, no standard equivalent

### 13.2 Disposition for ABB's Known Custom Objects

Object

Type

Disposition

Rationale

Target Solution

**ZMM\_PO\_ENHANCE**

BAdI

REFIT

PO creation BAdI — need to verify if the ECC BAdI still exists in S/4HANA or if a new released BAdI replaces it (MM\_PUR\_S4\_PO\_\*)

Reimplement under released S/4HANA BAdI using Released APIs

**Z\_VENDOR\_SCORE**

Report

REPLACE

S/4HANA Supplier Evaluation (ME61 + F2669) likely covers scoring. Verify scoring criteria match.

Configure standard Supplier Evaluation; retire custom report

**ZMM001**

Transaction

RETIRE

Custom transaction likely wrapping vendor master workflow — standard Fiori apps (F1739, F0842) cover this with modern UX

Retire; map users to Fiori Launchpad apps

**ZMM\_VENDOR\_REPORT**

Report

REFIT / MODERNIZE

79% coverage by standard (F1739). 21% GAP (MSME/BEE/Blacklist). Cannot retire entirely.

F1739 + Fiori Object Page extension + E\_Supplier custom CDS for MSME/BEE data

**ZMM\_VENDOR\_MSME**

Z-Table

REWRITE

Custom table for MSME data — no standard equivalent. Must be re-implemented as Clean Core RAP BO.

New RAP BO (ZR\_VENDOR\_MSME) with Fiori Elements maintenance UI, Released API access

### 13.3 Expected Disposition Distribution (Full Landscape)

Based on typical large-enterprise ECC landscapes:

Disposition

Expected %

Typical Count (for 10,000 objects)

Effort Profile

RETIRE

25-35%

~2,500-3,500

Low — decommission + user training

REPLACE

15-20%

~1,500-2,000

Low-Medium — configuration only

REFIT

20-25%

~2,000-2,500

Medium — BAdI/extension rework

REWRITE

10-15%

~1,000-1,500

High — full Clean Core rewrite

RETAIN

10-15%

~1,000-1,500

Low — validate + test

## 14 Gap Analysis & Closure Strategies

### 14.1 Identified Gaps (From Known Custom Objects)

#

Gap

Description

Complexity

Closure Strategy

G1

**MSME Classification**

Custom Z-table (ZMM\_VENDOR\_MSME) stores MSME certification category, certificate number, validity dates per vendor. No standard S/4HANA equivalent.

High

Build new RAP BO:  
1\. Custom table ZMSME\_CERT (Clean Core, with BP number as key)  
2\. Interface CDS: ZI\_MSME\_CERT  
3\. Consumption CDS: ZC\_MSME\_CERT  
4\. BDEF with managed scenario + draft  
5\. Fiori Elements maintenance app  
6\. Association from E\_Supplier extension to ZI\_MSME\_CERT for display in Manage Supplier

G2

**BEE Status**

Broad-Based Black Economic Empowerment status tracking per vendor (relevant for South African operations). Custom Z-table.

High

Similar to G1 — new RAP BO (ZI\_VENDOR\_BEE) with Fiori maintenance. Could be combined with G1 into a single "Vendor Compliance" RAP BO.

G3

**Vendor Blacklist**

Custom table (ZVENDOR\_BLOCK) tracking vendor blacklist/denylist status with reason codes and dates.

Medium

Option A: Use standard BP Central Block (transaction BP → Status tab) — may cover the requirement  
Option B: If more detail needed, build as part of Vendor Compliance RAP BO

G4

**Custom PO Validation Logic**

ZMM\_PO\_ENHANCE BAdI — specific validation rules during PO creation (details unknown without source code review)

Medium

Review ECC BAdI implementation. Re-implement under S/4HANA released BAdI (e.g., MM\_PUR\_S4\_PO\_CUST for PO custom checks). Use only Released APIs in implementation.

G5

**Custom Vendor Scoring Algorithm**

Z\_VENDOR\_SCORE — custom scoring criteria beyond standard ME61 supplier evaluation

Medium

Compare custom criteria vs. standard ME61 criteria. If gap exists, use Supplier Evaluation BAdI to add custom criteria. If fundamentally different, build as analytics CDS view.

G6

**Consolidated Vendor View**

ZMM\_VENDOR\_REPORT provides a single-screen consolidated view of all vendor data. No single standard app does this across ALL data points.

Medium

Build Fiori Object Page extension on F1739 (Manage Supplier) with custom sections for MSME, BEE, Blacklist, open POs, evaluation score. All via CDS view extensions and annotation-driven UI.

### 14.2 Gap Closure Architecture (Vendor Compliance Example)

graph TB subgraph "Clean Core Architecture" ZTAB\["Custom Tables  
ZMSME\_CERT, ZVENDOR\_BEE,  
ZVENDOR\_BLOCK"\] ICDS\["Interface CDS Views  
ZI\_MSME\_CERT  
ZI\_VENDOR\_BEE  
ZI\_VENDOR\_BLOCK"\] CCDS\["Consumption CDS Views  
ZC\_VENDOR\_COMPLIANCE"\] BDEF\["Behavior Definition  
(Managed + Draft)"\] SRVD\["Service Definition  
ZSD\_VENDOR\_COMPLIANCE"\] SRVB\["Service Binding (V4)  
ZSB\_VENDOR\_COMPLIANCE"\] FIORI\["Fiori Elements  
List Report + Object Page"\] end subgraph "Standard S/4HANA" IS\["I\_Supplier  
(Released CDS)"\] ESUP\["E\_Supplier  
(Extension Include)"\] F1739\["F1739  
Manage Supplier"\] end ZTAB --> ICDS ICDS --> CCDS CCDS --> BDEF BDEF --> SRVD SRVD --> SRVB SRVB --> FIORI ICDS -->|association| IS CCDS -->|extension| ESUP ESUP -->|appears in| F1739 style ZTAB fill:#C97B3A,color:#fff style ICDS fill:#6B3FA0,color:#fff style CCDS fill:#6B3FA0,color:#fff style BDEF fill:#B5567A,color:#fff style SRVD fill:#B5567A,color:#fff style SRVB fill:#B5567A,color:#fff style FIORI fill:#27ae60,color:#fff style IS fill:#3498db,color:#fff style ESUP fill:#3498db,color:#fff style F1739 fill:#3498db,color:#fff

## 15 Interface & Integration Migration

Interface Type

ECC Technology

S/4HANA Target

Migration Approach

Inbound Master Data

IDoc (CREMAS, DEBMAS), BAPI (BAPI\_VENDOR\_CREATE)

BP APIs (I\_BusinessPartnerTP RAP BO) or OData V4

Remap IDoc receivers to BP APIs. Test CVI sync.

Outbound Master Data

IDoc (CREMAS), RFC

Event-driven via RAP Events or Enterprise Messaging

Evaluate SAP Integration Suite for event-based architecture

PO Integration

IDoc (ORDERS), BAPI\_PO\_CREATE1

BAPI\_PO\_CREATE1 (still Level B) or I\_PurchaseOrderTP (Level A)

BAPI works; for Clean Core, migrate to RAP API

Invoice Integration

IDoc (INVOIC), BAPI\_INCOMINGINVOICE\_CREATE

Same BAPI or OData V4 API

BAPI still works; OData V4 for new integrations

Financial Postings

BAPI\_ACC\_DOCUMENT\_POST, BDC on FB01

Same BAPI (Level B) or I\_JournalEntryTP (Level A)

For Clean Core: RAP API. For quick migration: keep BAPI.

Middleware (PI/PO)

SAP PI/PO

SAP Integration Suite (Cloud Integration) or keep PI/PO

Evaluate cloud integration roadmap

File Interfaces

Flat files (ALSM\_EXCEL, WS\_UPLOAD)

Same approach or S/4HANA File Processing (DRF)

Modernize with OData batch or SAP Data Services

RFC Connections

Direct RFC calls between systems

Same (if systems remain on-prem)

Review RFC destinations; test connectivity post-migration

## 16 Forms & Output Management Migration

ECC Technology

S/4HANA Status

Recommendation

Effort

**SAPscript** (SE71)

Deprecated — Not actively maintained

Convert to Adobe Forms (SFP). C16 can automate conversion.

Medium per form

**SmartForms** (SMARTFORMS)

Supported but no innovation

Convert to Adobe Forms for long-term. SmartForms still work on S/4HANA.

Medium per form

**Adobe Forms** (SFP)

Strategic

Keep. Adobe Forms is SAP's strategic form technology.

Low (validation only)

**Output Determination** (NACE)

Compatible but new Output Management available

Keep NACE for initial migration. Migrate to Output Management (BRF+) in optimization phase.

High (if migrating)

## 17 Data Migration Strategy

### 17.1 Data Migration Scope

Data Category

Key Tables

Migration Approach

Complexity

**Master Data — Vendor/Customer**

KNA1, LFA1, KNB1, LFB1, KNVV, LFBK

CVI synchronization → BP creation. Run pre-migration in ECC.

Critical

**Master Data — Material**

MARA, MARC, MARD, MARM, MBEW

Direct migration (tables compatible). Check deprecated fields.

Medium

**Master Data — GL Accounts**

SKA1, SKB1

Direct migration. Verify chart of accounts structure.

Low

**Transactional — PO/SO/Delivery**

EKKO/EKPO, VBAK/VBAP, LIKP/LIPS

Open documents migrate. Historical depends on approach (Greenfield vs Brownfield).

High

**Transactional — Finance**

BKPF/BSEG → ACDOCA

ACDOCA migration cockpit (SAP standard tool). Largest data volume.

Critical

**Transactional — Inventory**

MKPF/MSEG → MATDOC

MATDOC migration (SAP standard). Stock balances + open documents.

High

**Custom Tables (Z/Y)**

ZMM\_VENDOR\_MSME, ZVENDOR\_BLOCK, ZMM\_VENDOR\_BEE

Direct migration if table structure retained. Re-key on BP if needed.

Medium

**Configuration**

All customizing tables

Brownfield: carries forward. Greenfield: re-implement.

High

### 17.2 Data Volume Considerations

**📊 Archiving Before Migration:** Before migrating, ABB should consider archiving old data to reduce migration volume and S/4HANA database size:

*   Financial documents older than 7 years (statutory retention met)
*   Closed purchase orders older than 3 years
*   Completed sales orders older than 3 years
*   Material documents older than 5 years
*   Change documents (CDHDR/CDPOS) older than 5 years

This can reduce ACDOCA migration volume by 30-50% and significantly shorten downtime.

## 18 Testing & Validation Strategy

Test Phase

Scope

Tools/Approach

Duration

**Unit Testing**

Individual custom objects (programs, classes, BAdIs)

ABAP Unit Tests (CL\_ABAP\_UNIT), manual SE38 testing

Continuous during remediation

**Integration Testing**

End-to-end process flows (P2P, O2C, R2R)

Process test scripts, SAP Solution Manager

4-6 weeks

**BP Migration Testing**

Verify all vendors/customers accessible via BP

BP consistency checks, custom report validation

2-3 weeks

**Performance Testing**

MRP Live, ACDOCA reporting, high-volume transactions

Load testing tools, SAP Early Watch

2-3 weeks

**Interface Testing**

All inbound/outbound interfaces

End-to-end with partner systems

3-4 weeks

**Regression Testing**

All standard transactions + custom processes

Test automation (Tricentis, SAP CBTA)

4-6 weeks

**User Acceptance (UAT)**

Business users validate processes

Business user scripts, Fiori UX validation

4-6 weeks

**Cutover Rehearsal**

Full migration simulation including downtime

Dress rehearsal (2-3 iterations recommended)

2-3 weekends

## 19 Migration Roadmap & Timeline

gantt title ABB ECC to S/4HANA 2025 Migration Roadmap dateFormat YYYY-MM-DD axisFormat %b %Y section Phase 1 - DISCOVER System Connect and Data Export :a1, 2026-05-01, 15d Business Process Inventory :a2, 2026-05-01, 30d Custom Code Scan - Full Landscape :a3, after a1, 20d Clean Core Classification :a4, after a3, 15d Transaction Usage Analysis :a5, after a1, 10d Discovery Report Delivery :a6, after a4, 5d section Phase 2 - MAP and ASSESS Module Workshops - MM :b1, after a6, 10d Module Workshops - SD :b2, after a6, 10d Module Workshops - FICO :b3, after b1, 10d Module Workshops - PP PM QM :b4, after b2, 10d Module Workshops - WM EWM :b5, after b3, 10d Fit to Standard Assessment :b6, after b1, 30d Gap Analysis Finalization :b7, after b6, 10d Custom Code Disposition Decisions :b8, after b7, 10d section Phase 3 - REMEDIATE Quick Wins - Retire and Replace :c1, after b8, 20d BP Migration Prep in ECC :c2, after b8, 30d REFIT Sprint 1 - BAdIs and Exits :c3, after c1, 25d REFIT Sprint 2 - Reports and Forms :c4, after c3, 25d REWRITE Sprint 1 - Gap Closure RAP BOs :c5, after c1, 30d REWRITE Sprint 2 - Interface Modernize :c6, after c5, 20d Clean Core Validation :c7, after c4, 10d section Phase 4 - MIGRATE System Build and Configuration :d1, after c2, 30d Data Migration - Master Data :d2, after d1, 15d Data Migration - Transactional :d3, after d2, 20d Integration Testing :d4, after d3, 30d UAT :d5, after d4, 30d Cutover Rehearsal 1 :d6, after d4, 5d Cutover Rehearsal 2 :d7, after d5, 5d Go Live :milestone, d8, after d7, 1d Hypercare :d9, after d8, 30d

### 19.1 Phase Timeline Summary

Phase

Duration

Start

End

Key Deliverables

**Phase 1: DISCOVER**

~6 weeks

May 2026

Jun 2026

Custom code inventory, Clean Core classification, process catalogue

**Phase 2: MAP & ASSESS**

~8 weeks

Jul 2026

Aug 2026

Fit-to-Standard mapping, gap analysis, disposition decisions

**Phase 3: REMEDIATE**

~16 weeks

Sep 2026

Dec 2026

Retired objects, rewritten code, new RAP BOs, BP prep

**Phase 4: MIGRATE**

~20 weeks

Jan 2027

May 2027

System build, data migration, testing, go-live

**Total: ~50 weeks (~13 months)**

May 2026

Jun 2027

## 20 Risk Assessment & Mitigation

#

Risk

Probability

Impact

Mitigation

R1

**BP Migration Data Quality** — Duplicate vendors/customers, inconsistent addresses, missing bank details cause BP sync failures

High

Critical

Run BP pre-checks in ECC 6 months before cutover. Cleanse master data. Run CVI sync in ECC as early as possible.

R2

**Custom Code Volume** — More custom objects than expected (>15,000). Remediation effort exceeds plan.

High

High

Run SAP Readiness Check + C16 scan early in Phase 1. Prioritize by usage (ST03N). Apply the 80/20 rule: 80% of usage is 20% of objects.

R3

**WM to EWM Migration** — If ABB uses classic WM extensively, EWM migration adds 3-6 months

Medium

Critical

Assess WM usage in Phase 1. If complex WM, consider parallel EWM project or Stock Room Management as interim.

R4

**ACDOCA Data Volume** — Financial data migration to ACDOCA takes longer than planned due to volume

Medium

High

Archive old financial data before migration. Run volume sizing early. Plan multiple cutover rehearsals.

R5

**Interface Failures** — External systems not ready for BP number changes, new APIs

Medium

High

Engage interface partners early. Use CVI backward compatibility (vendor/customer numbers still work via mapping).

R6

**User Adoption** — Resistance to Fiori / new UX from SAP GUI power users

High

Medium

Role-based Fiori spaces. SAP GUI still available for power users. Training program with hands-on workshops.

R7

**CO-PA Redesign** — If ABB uses costing-based CO-PA heavily, migration to account-based is a major project

Medium

Critical

Assess CO-PA usage in Phase 1. If costing-based, plan dedicated CO-PA migration workstream.

R8

**Scope Creep** — Business requests additional enhancements during migration

High

Medium

Strict change control. "Like-for-like" migration first, then optimization phase post-go-live.

## 21 Strategic Recommendations

### 🎯 Primary Recommendation: Bluefield (Selective Data Transition)

Given ABB's emphasis on **Fit-to-Standard** and **Clean Core**, a Bluefield approach is recommended. This provides a fresh S/4HANA 2025 system with selectively migrated data, forcing Fit-to-Standard discipline while preserving critical historical data.

A Brownfield conversion would carry forward too much technical debt. A pure Greenfield would lose valuable historical data and configuration. Bluefield balances both.

### 21.1 Top 10 Recommendations

#

Recommendation

Priority

Phase

1

**Connect C16 to ABB's ECC immediately** — Automates 60-70% of Phase 1 discovery and Clean Core classification. Without it, manual scanning of thousands of objects takes weeks instead of hours.

Critical

Phase 1

2

**Run SAP Readiness Check** — Free tool via SAP Launchpad. First-cut compatibility report against Simplification List.

Critical

Phase 1

3

**Start BP Migration Prep in ECC NOW** — Activate CVI, run BP sync, cleanse vendor/customer master data. This can (and should) start before the main migration project.

Critical

Pre-Phase 1

4

**Assess WM usage immediately** — If classic WM is used, the WM→EWM migration adds significant scope and timeline. Decision needed early.

High

Phase 1

5

**Archive before migrating** — Reduce data volume for ACDOCA migration. Archive old financial docs, closed POs/SOs, old material documents.

High

Pre-Phase 4

6

**Establish Clean Core governance** — Set standards now: all new/rewritten code must use Released APIs only. Enforce via ABAP Cloud language version in new packages.

High

Phase 2

7

**Consolidate vendor compliance (MSME/BEE/Blacklist)** — Build as a single Clean Core RAP BO rather than three separate solutions. Attach to I\_Supplier via extension.

Medium

Phase 3

8

**Prioritize by usage** — Run ST03N to identify top 100 transactions by user count. Focus remediation on these first. Many Z-programs may be unused.

High

Phase 1

9

**Plan 2-3 cutover rehearsals** — Especially for ACDOCA migration and BP sync. Each rehearsal refines the runbook and reduces actual downtime.

High

Phase 4

10

**Invest in Fiori adoption** — Build role-based Fiori spaces. Map every SAP GUI transaction to its Fiori equivalent. Train power users early.

Medium

Phase 3-4

## 22 Immediate Next Steps

#### 📌 Week 1-2: Quick Wins

1.  **Run SAP Readiness Check** on ABB's ECC (free, no C16 needed)
2.  **Extract ST03N usage data** — identify top 100 transactions by user/execution count
3.  **Inventory classic WM usage** — determine if WM→EWM migration is in scope
4.  **Assess CO-PA type** — costing-based or account-based (determines FI migration complexity)

#### 📌 Week 3-4: Connect & Scan

1.  **Provide C16 RFC/ADT access** to ABB's ECC system
2.  **C16 auto-scans** all Z/Y objects and classifies Clean Core levels (A/B/C/D)
3.  **C16 generates per-program analysis** for the top 50 most-used custom programs
4.  **Deliver Discovery Report** with exact object counts, disposition recommendations, and effort estimates

#### 📌 Month 2: Module Workshops

1.  Schedule **Fit-to-Standard workshops** per module (MM, SD, FI/CO, PP, PM, QM, WM)
2.  Each workshop: process owners + C16 analysis data + standard Fiori app demos
3.  **Decision gates:** RETIRE / REPLACE / REFIT / REWRITE / RETAIN per custom object
4.  **Gap documentation** — each GAP gets a closure strategy and effort estimate

#### 📌 Month 2 (Parallel): BP Prep

1.  **Activate CVI in ECC** (Customer Vendor Integration)
2.  **Run BP synchronization** — create BP records for all vendors/customers
3.  **Cleanse master data** — fix duplicate addresses, missing bank details, incomplete tax codes
4.  **Validate BP consistency** — all vendors/customers accessible via BP transaction

**🚀 C16 Automation Advantage:** Once connected to ABB's ECC, C16 can deliver Phase 1 results in **days instead of weeks**:

*   **Full custom code inventory** with Clean Core classification per object — automated scan
*   **Per-program deep analysis** — source architecture, table dependencies, BDC patterns, auth gaps, performance anti-patterns, duplication, Fit-to-Standard mapping
*   **Batch analysis** — 50-100 programs per session with full HTML reports
*   **Remediation code generation** — Clean Core ABAP code (RAP, CDS, Fiori Elements) generated and deployed directly
*   **SmartForm → Adobe conversion** — automated pipeline for form migration

The sooner C16 is connected, the sooner ABB gets hard data instead of estimates.

**C16 Migration Analysis** — Generated April 13, 2026

This analysis is based on SAP's published S/4HANA 2025 Simplification List (SAP Note 2313884), standard ECC-to-S/4HANA migration patterns, and the 5 known custom programs identified by ABB. All process mappings, Fiori app references, and CDS view names are based on SAP standard documentation.

**⚠️ Caveat:** ABB's ECC system is not connected to C16. All estimates (custom object counts, Clean Core distribution, effort) are based on industry benchmarks for large manufacturing enterprises. Once C16 is connected, these estimates will be replaced with exact, verified numbers.

**Methodology:** Fit-to-Standard assessment per SAP Activate methodology. Clean Core classification per SAP Cloudification Catalog (3-layer: Released API check + ATC ABAP\_CLOUD\_READINESS + dependency tree analysis). Gap analysis per SAP Simplification List items. Fiori app IDs from SAP Fiori Apps Library (fioriappslibrary.hana.ondemand.com).

© 2026 C16 — SAP Intelligent Migration Platform

mermaid.initialize({ startOnLoad: true, theme: 'base', themeVariables: { primaryColor: '#6B3FA0', primaryTextColor: '#fff', primaryBorderColor: '#B5567A', lineColor: '#B5567A', secondaryColor: '#f4f0fa', tertiaryColor: '#fdf0f8' }, gantt: { titleTopMargin: 25, barHeight: 20, barGap: 4, topPadding: 50, sidePadding: 75 } });