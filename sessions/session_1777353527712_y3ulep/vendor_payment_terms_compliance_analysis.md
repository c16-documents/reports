 Vendor Payment Terms Compliance — Early Payment Diagnostic body{font-family:'Segoe UI',Arial,sans-serif;color:#2c3e50;background:#e9ecef;margin:0;padding:20px} .doc-wrapper{max-width:1120px;margin:0 auto;background:#fff;box-shadow:0 2px 20px rgba(0,0,0,.12);border-radius:12px;overflow:hidden} .header-banner{background:linear-gradient(135deg,#C97B3A,#B5567A,#6B3FA0);color:#fff;padding:40px;text-align:center} .header-banner img{height:50px;margin-bottom:12px;border-radius:8px} .header-banner h1{font-size:2em;margin:10px 0 5px} .header-banner p{opacity:.9;font-size:1.1em} .badge{display:inline-block;padding:4px 14px;border-radius:20px;font-size:.85em;font-weight:600;margin:4px} .badge-critical{background:#e74c3c;color:#fff} .badge-warning{background:#e67e22;color:#fff} .badge-info{background:#3498db;color:#fff} .badge-ok{background:#27ae60;color:#fff} .meta-bar{background:#f8f9fa;padding:16px 40px;display:flex;flex-wrap:wrap;gap:20px;border-bottom:2px solid #eee;font-size:.92em} .meta-bar span{color:#6B3FA0;font-weight:600} .content{padding:30px 40px} .section-title{color:#6B3FA0;border-bottom:3px solid #B5567A;padding-bottom:10px;margin-top:40px;font-size:1.4em} .exec-summary{border-left:5px solid #27ae60;background:#f6fef9;padding:20px;border-radius:8px;margin:20px 0} .kpi-row{display:flex;flex-wrap:wrap;gap:16px;margin:20px 0} .kpi-card{flex:1;min-width:140px;background:linear-gradient(135deg,#6B3FA0,#8B5CF6);color:#fff;padding:20px;border-radius:12px;text-align:center} .kpi-card .num{font-size:2.2em;font-weight:700} .kpi-card .label{font-size:.85em;opacity:.9;margin-top:4px} table{width:100%;border-collapse:collapse;margin:16px 0;font-size:.92em} thead{background:#6B3FA0;color:#fff} th{padding:10px 12px;text-align:left;font-weight:600} td{padding:8px 12px;border-bottom:1px solid #eee} tr:nth-child(even){background:#f8fafc} tr:hover{background:#edf2f7} .finding-box{background:#fef9e7;border-left:4px solid #e67e22;padding:16px;margin:16px 0;border-radius:6px} .critical-box{background:#fdedec;border-left:4px solid #e74c3c;padding:16px;margin:16px 0;border-radius:6px} .ok-box{background:#eafaf1;border-left:4px solid #27ae60;padding:16px;margin:16px 0;border-radius:6px} .priority-high{color:#e74c3c;font-weight:700} .priority-med{color:#e67e22;font-weight:700} .priority-low{color:#27ae60;font-weight:700} .doc-footer{background:#f8f9fa;padding:20px 40px;border-top:2px solid #eee;font-size:.82em;color:#888;text-align:center;margin-top:40px} h3{color:#6B3FA0;margin-top:25px} @media print{body{background:#fff}.doc-wrapper{box-shadow:none}} :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,)

# Vendor Payment Terms Compliance Analysis

Early Payment Detection — Master Data vs. Actual Payment Behavior

Payment Terms Mismatch Detected ECC 7.40 | Client 800 April 2026

System: ECC 7.40

Tables Queried: LFB1, BSAK, BSIK, LFA1, LFBK, REGUH, T052U

Vendor Master Records: 500

Cleared Items (BSAK): 2,000+

Open Items (BSIK): 500

Bank Details (LFBK): 500

F110 Runs (REGUH): 200

## §1 — Executive Summary

This investigation cross-references vendor master payment terms (LFB1.ZTERM) against actual cleared payment documents (BSAK) and currently open items (BSIK) to identify vendors where payments were made before the payment term due date. The analysis covers 500 vendor-company code master records, 2,000+ cleared items, 500 open items, and 500 bank detail records across the entire ECC landscape.

The findings reveal a systemic pattern of **early payment** across multiple company codes. Vendors assigned the most common term — **ZB01 (14 days 3%, 30 days 2%, 45 days net)** — are consistently receiving payment on **Day 0**, meaning the full 45-day credit period is forfeited. In the most extreme case, AluCast (vendor 300) had DEM 31.2 million cleared on the same day as the baseline date. Additionally, approximately **26% of vendor-company code records have no payment term assigned at all**, which defaults to immediate payment and removes all cash flow control.

*   **CRITICAL:** Vendors with 45-day net terms (ZB01) are paid on Day 0 — forfeiting 45 days of cash retention
*   **HIGH:** 130 vendor-company code pairs (~26%) have blank ZTERM in LFB1 — no payment scheduling possible
*   **HIGH:** Many clearing documents show blank ZTERM on the payment side — indicates manual postings bypassing F110
*   **MEDIUM:** 55% of vendor records lack tolerance group (TOGRU) — no payment amount variance control
*   **MEDIUM:** Several vendors have inconsistent terms across company codes (e.g., immediate in CoCd 1000 but 45-day in CoCd 3000)
*   **INFO:** F110 automatic payment runs exist in REGUH for company code 1000 (monthly, identifier "RE") dating from 1997 — but the LIFNR field is blank, suggesting batch header records only

500

Vendor-CoCd Master Records (LFB1)

2,000+

Cleared Items Analyzed (BSAK)

500

Open Payable Items (BSIK)

ZB01

Most Common Term (45 net)

~26%

Vendors with Blank ZTERM

200

F110 Payment Run Records (REGUH)

## §2 — Payment Term Configuration (T052U)

The following table shows all payment terms configured in the system that are actively used by vendors. Understanding these terms is critical because they define the **due date calculation**: the system adds the ZBD3T (net days) to ZFBDT (baseline date) to determine when payment is due. Early payment occurs when BUDAT (posting date) is before ZFBDT + ZBD3T.

Term Code

Description

Discount 1 (Days / %)

Discount 2 (Days / %)

Net Days

Vendor Count

**ZB01**

14 days 3%, 30/2%, 45 net

14 / 3%

30 / 2%

45

~155

**ZB02**

14 days 2%, 30/1.5%, 45 net

14 / 2%

30 / 1.5%

45

~10

**ZB03**

20 days 2%, 30 days net

20 / 2%

—

30

~4

**ZB50**

2% 10 Days, Net 30

10 / 2%

—

30

~28

**NT30**

Net 30

—

—

30

~30

**NT60**

Net 60

—

—

60

~2

**0001**

Payable immediately

—

—

0

~35

**0002**

Within 30 days

—

—

30

~8

**ZB00**

Payable immediately (custom)

—

—

0

~18

**ZB10**

10 days 3%

10 / 3%

—

10

~5

_(blank)_

No term assigned

—

—

Defaults to 0

~130

The key insight is that ZB01 — the dominant term — provides a 3% cash discount if paid within 14 days, 2% within 30 days, and full payment due at 45 days. Paying on Day 0 (as observed in many cases) gains no additional benefit over paying on Day 14, yet forfeits 14 days of cash retention. Optimal treasury management would target Day 14 exactly to capture the 3% discount while maximizing cash retention.

## §3 — Current Open Payables Overview (BSIK)

The BSIK table contains currently open (unpaid) vendor items. These represent the organization's outstanding payable obligations. Understanding the current open position is essential context for evaluating payment term compliance — it shows what is still waiting to be paid and under what terms.

### 3.1 — Open Items by Vendor (Top 10 by Value)

The following table shows the vendors with the largest open payable balances. These are the vendors where payment term compliance has the highest cash flow impact — early payment on these invoices wastes the most working capital.

#

Vendor

Name

CoCd

Open Items

Sample Amount

Currency

ZTERM on Doc

Payment Block

1

0000001012

Reihl & Müller

0005

1

2,301,440.00

EUR

0002

—

2

0000001002

Müller KG

0005

1

275,000.00

EUR

(blank)

—

3

0000001000

C.E.B. BERLIN

1000

5

580,000.00

EUR

ZB01

—

4

0000003920

Pyramid Systems

0006

12

318,628.00

USD

ZB01

—

5

BLEVINS

Blevins

0006

7

100,000.00

USD

ZB10

—

6

CIRCUITS

Circuits

0006

10

120,000.00

USD

ZB10

—

7

DOWNEY INC

Downey Inc

0006

8

134,000.00

USD

ZB10

—

8

0000000002

Electronic Comp. Distributor

1000

50+

100,000.00

EUR

ZB03

R (partial)

9

0000001004

Jones Ltd.

0005

1

23,000.00

EUR

ZB03

—

10

0000001005

PAQ Deutschland GmbH

0005

3

2,300.00 each

EUR

ZB01

—

Notably, vendor 0000000002 (Electronic Components Distributor) has **50+ open items** in company code 1000, with some carrying a **payment block "R"** (invoice verification hold). This vendor's master record has **no payment term (blank ZTERM)** in LFB1 for CoCd 1000, but the document-level term is ZB03 (20d/2%, 30 net). This discrepancy between master and document terms is precisely the pattern this analysis targets.

### 3.2 — Open Items by Company Code

This breakdown shows the distribution of open payables across company codes, highlighting where the largest payment volumes exist.

Company Code

Open Item Count (sample)

Dominant ZTERM

Key Vendors

0005

~10

ZB01, ZB03

C.E.B. BERLIN, Müller KG, PAQ Deutschland

0006

~40

ZB01, ZB10, 0001

Pyramid Systems, BLEVINS, CIRCUITS, DOWNEY INC

1000

~100+

ZB01, ZB03, ZB00

Electronic Comp. Distributor, C.E.B. BERLIN, SMP

## §4 — Cleared Payment History (BSAK) — Early Payment Detection

The BSAK table contains all cleared (paid) vendor items. By comparing the payment posting date (BUDAT) against the calculated due date (ZFBDT + ZBD3T), we can identify every instance where payment was made before the due date. The "days early" metric shows the gap between when payment was made and when it was actually required.

### 4.1 — Vendors with Extended Terms Paid on Day 0 (Category A — Critical)

**🔴 Critical Finding:** The following vendors have ZB01 (45-day net) in their master data, but payment documents show clearing on the **same day as baseline date** (BUDAT = ZFBDT), meaning payments were made on Day 0 — **45 days before they were due**. This is the most severe cash flow waste pattern.

Each row below represents an actual payment document from BSAK where the payment was made at or near the baseline date despite the vendor having 45-day net terms. The "Days Early" column shows how many days before the net due date the payment was made.

Vendor

Name

CoCd

Master ZTERM

Net Days

Doc#

Amount

Ccy

Payment Date

Baseline

Days Early

0000000300

AluCast

1000

ZB01

45

5100004463

31,250,189.70

DEM

1997-08-07

1997-08-07

**45**

0000000100

C.E.B. BERLIN

1000

ZB01

45

0100003420

14,950,000.00

DEM

1996-07-05

1996-07-05

**45**

0000000300

AluCast

1000

ZB01

45

5100004520

7,985,348.15

DEM

1997-09-08

1997-09-08

**45**

0000000300

AluCast

1000

ZB01

45

5100004439

4,024,039.75

DEM

1997-07-10

1997-07-10

**45**

0000000300

AluCast

1000

ZB01

45

5100004462

1,649,882.00

DEM

1997-08-07

1997-08-07

**45**

0000000100

C.E.B. BERLIN

1000

ZB01

45

5100000132

1,503,750.00

DEM

1996-02-09

1996-02-09

**45**

0000001005

PAQ Deutschland GmbH

0005

ZB01

45

1900000001

575,000.00

EUR

2006-04-19

2006-04-19

**45**

0000001002

Müller KG

0005

ZB01

45

1900000042

522,000.00

EUR

2006-04-21

2006-04-18

**42**

0000001000

C.E.B. BERLIN

0005

ZB01

45

5100000001

464,580.00

EUR

2006-05-08

2006-05-08

**45**

0000001002

Müller KG

0005

ZB01

45

1900000041

402,500.00

EUR

2006-03-04

2006-03-04

**45**

0000001002

Müller KG

0005

ZB01

45

1900000038

345,000.00

EUR

2006-02-01

2006-02-01

**45**

0000001010

Sunny Electronics

0005

ZB01

45

1900000039

287,500.00

EUR

2006-03-15

2006-03-15

**45**

0000001000

C.E.B. BERLIN

0005

ZB01

45

1900000000 (2008)

210,919.00

EUR

2008-04-02

2008-04-02

**45**

0000001005

PAQ Deutschland GmbH

0005

ZB01

45

1900000003

172,500.00

EUR

2006-04-19

2006-04-19

**45**

0000001000

C.E.B. BERLIN

0005

ZB01

45

1900000032

145,000.00

EUR

2006-04-20

2006-04-18

**43**

The data is unambiguous: high-value invoices running into the millions are being paid immediately despite a 45-day credit window. The cumulative cash flow impact for AluCast alone exceeds **DEM 46 million** in same-day payments. At a typical short-term borrowing rate of 4% p.a., retaining DEM 46M for 45 additional days would save approximately DEM 226,000 in financing costs (46M × 4% × 45/365).

### 4.2 — Document-Level Term vs. Master Term Mismatch (Category B)

A second pattern emerges where the payment term on the clearing document differs from the vendor master. This indicates either manual override during posting or inherited terms from the purchase order rather than the vendor master. The table below shows the TURFMOOR vendor as the most prominent example.

Vendor

CoCd

Master ZTERM

Doc ZTERM

ZBD1T/ZBD2T/ZBD3T on Doc

Doc Count

Total Amount

TURFMOOR

0001

0001 (immediate)

0001 or blank

0 / 0 / 0

100+

EUR ~130,000+

TURFMOOR (clearing)

0001

0001

(blank)

0 / 0 / 0

50+

EUR ~65,000+

0000000002

1000

(blank)

ZB03

20 / 30 / 0

50+

EUR ~200,000+

0000000015

1000

0002 (30 net)

(blank)

0 / 0 / 0

3

EUR ~1,772

0000000200

1000

ZB00 (immediate)

ZB00

0 / 0 / 0

2

EUR ~13,735

0000000300

1000

ZB01 (45 net)

ZB00

0 / 0 / 0

2

EUR ~594

0000001000

1000

ZB01 (45 net)

0001

0 / 0 / 0

1

EUR 5,000

0000000100

0005

ZB01 (45 net)

(blank)

0 / 0 / 0

1

EUR 460.84

The most concerning pattern is vendor 0000000002 (Electronic Components Distributor): the master has **blank ZTERM** in CoCd 1000, but documents consistently carry **ZB03**. This means the payment term is being set at the PO or invoice level — creating a disconnect where the vendor master provides no payment scheduling guidance to the F110 program. Additionally, vendor 0000001000 in CoCd 1000 has one document with term **0001 (immediate)** despite the master saying ZB01 — someone manually overrode the term.

## §5 — Automatic Payment Run Analysis (REGUH)

The REGUH table records the output of the F110 automatic payment program. Each row represents a payment document generated by F110. The presence or absence of F110 records tells us whether the organization is using automated payments (which respect due dates) or manual postings (which bypass term logic).

Run Date

Run ID

Company Code

Doc Number Pattern

Sample Amount

Records per Run

1997-01-02

RE

1000

F110000001–F110000012

DEM 103.50 – 11,868.00

24 (12 proposals + 12 payments)

1997-02-02

RE

1000

F110000001–F110000012

DEM 103.50 – 7,820.00

24

1997-03-02

RE

1000

F110000001–F110000012

DEM 103.50 – 7,820.00

24

1997-04-02

RE

1000

F110000001–F110000013

DEM 103.50 – 11,868.00

26

1997-05-02

RE

1000

F110000001–F110000012

DEM 103.50 – 7,820.00

24

1997-06-02

RE

1000

F110000001–F110000012

DEM 103.50 – 7,820.00

24

1997-07-02

RE

1000

F110000001–F110000012

DEM 103.50 – 7,820.00

24

1997-08-02

RE

1000

F110000001–F110000013

DEM 103.50 – 7,912.00

26

The F110 runs in REGUH are **monthly (every 1st or 2nd of the month)** in company code 1000 only, all with identifier "RE". Each run processes approximately 12 vendors with recurring amounts (DEM 3,772 / 6,785 / 7,935 / 3,404 / 2,990 / 103.50 / 11,868 / 2,438 / 1,510 / 70 / 400 / 360). These look like **recurring expense payments** (rent, utilities, insurance) rather than procurement invoices. The LIFNR field is blank in all records, meaning the vendor linkage is in the REGUP detail table which was not queried.

**⚠️ Key Insight:** F110 is configured and running for company code 1000, but only for a small set of recurring payments (~12 vendors). The large procurement invoices (AluCast, C.E.B. BERLIN, Müller KG, etc.) are NOT being processed through F110 — they are being paid manually, which explains why they are cleared on Day 0 regardless of payment terms. **No REGUH records exist for company codes 0005 or 0006**, confirming that those entities have no automated payment processing at all.

## §6 — Vendor Master Data Completeness

A complete vendor master record is the foundation for automated payment processing. Missing fields prevent F110 from selecting vendors, calculating due dates, or executing payments via the correct method. The scorecard below assesses the quality of vendor master data across five critical dimensions.

### 6.1 — Master Data Completeness Scorecard

Dimension

Field (LFB1)

Complete %

Missing Count

Impact

Status

Payment Terms

ZTERM

~74%

~130 blank

No due date calculation; defaults to immediate

⚠️ Needs Attention

Payment Method

ZWELS

~62%

~190 blank

F110 cannot select payment method automatically

⚠️ Needs Attention

Payment Block

ZAHLS

100% (blank = no block)

0 blocked

No vendors are payment-blocked

✅ OK

Tolerance Group

TOGRU

~45%

~275 blank

No payment amount variance control

❌ Critical Gap

Recon Account

AKONT

~98%

~10 blank

Cannot post to vendor subledger

✅ OK

### 6.2 — Vendors WITHOUT Payment Terms (ZTERM = blank)

The following sample shows vendors in company code 1000 that have no payment term assigned. These vendors are at highest risk of uncontrolled early payment because the system has no due date to enforce.

Vendor

Name

CoCd

ZTERM

ZWELS

AKONT

0000000002

Electronic Components Distributor

1000

_(blank)_

L

161000

0000001021

Noe'Tech Company AG

1000

_(blank)_

SU

160000

0000001022

Max'Grosshandel

1000

_(blank)_

SU

160000

0000001061

Neotech GmbH

1000

_(blank)_

—

160000

0000001101

ABC Dienstleistungs GmbH

1000

_(blank)_

—

160000

0000001102

Top Services GmbH

1000

_(blank)_

—

160000

0000001103

ISG Innovative Services GmbH

1000

_(blank)_

—

160000

0000001106

SPEDITRANS Transport GmbH

1000

_(blank)_

—

160000

0000001012

Reihl & Müller

0005

_(blank)_

S

160000

0000001012

Reihl & Müller

1000

_(blank)_

S

160000

### 6.3 — Vendors WITHOUT Payment Method (ZWELS = blank)

Without a payment method, F110 cannot determine how to pay the vendor (check, bank transfer, wire, etc.). These vendors must be paid manually. The following sample shows vendors without ZWELS in active company codes.

Vendor

Name

CoCd

ZTERM

ZWELS

AKONT

0000001014

Herrmann & Riemer

1000

ZB01

_(blank)_

160000

0000001081

Harley-Davidson Motorcycles

1000

ZB01

_(blank)_

160000

0000001082

Louis Bike Parts

1000

ZB01

_(blank)_

160000

0000001104

Hoch und Tiefbau GmbH

1000

ZB01

_(blank)_

160000

0000001105

Harvey Building & Construction

1000

ZB01

_(blank)_

160000

0000001105

Harvey Building & Construction

3000

ZB01

_(blank)_

160000

0000001151

Media AG Trier

1000

ZB01

_(blank)_

160000

0000001151

Media AG Trier

3000

ZB01

_(blank)_

160000

### 6.4 — Payment Method Distribution (ZWELS Breakdown)

The ZWELS field indicates which payment methods are allowed for a vendor. The most common methods in this system are S (check), U (bank transfer), C (check-US), and combinations thereof. Understanding the distribution helps assess F110 readiness.

Payment Method(s)

Description

Vendor-CoCd Count

% of Total

**SU**

Check + Bank Transfer

~80

16%

**C**

Check (US)

~70

14%

_(blank)_

No method assigned

~190

38%

**U**

Bank Transfer only

~30

6%

**ESU**

EDI + Check + Transfer

~20

4%

**L**

Letter of Credit

~15

3%

**CT**

Check + Wire Transfer

~15

3%

**T**

Wire Transfer

~12

2.4%

**P**

Post Office

~10

2%

Other

V, S, LW, LRW, CDT, etc.

~58

11.6%

### 6.5 — Payment Terms Distribution (ZTERM Breakdown)

This shows how payment terms are distributed across the vendor master, revealing the concentration of terms and the blank-term gap.

Payment Term

Description

Count

%

**ZB01**

14d/3%, 30d/2%, 45 net

~155

31%

_(blank)_

No term assigned

~130

26%

**0001**

Payable immediately

~35

7%

**NT30**

Net 30

~30

6%

**ZB50**

2% 10 Days, Net 30

~28

5.6%

**ZB00**

Payable immediately

~18

3.6%

**ZB02**

14d/2%, 30d/1.5%, 45 net

~10

2%

**0002**

Within 30 days

~8

1.6%

**ZB03**

20d/2%, 30 net

~4

0.8%

Other

ZB05, ZB10, various

~82

16.4%

### 6.6 — Bank Detail Coverage (LFBK)

Bank details are essential for automated electronic payments (transfer, wire). Vendors without bank details can only be paid by check or manually. The LFBK table shows 500 bank detail records across the vendor base.

Country

Vendor Count with Bank Details

Sample Vendors

**DE** (Germany)

~120

C.E.B. BERLIN, KBB Schwarze Pumpe, Müller KG, PAQ Deutschland

**US** (United States)

~85

AluCast, Phoenix Supplies, Pyramid Systems, Zebra Supplies

**IN** (India)

~45

Various Z-vendors (97xxx series)

**CA** (Canada)

~25

Iron Power, Electric Engines, D. & P. Forbes

**JP** (Japan)

~15

上吉産業, 鈴木原料, P.S.A. SYSTEMS

**GB** (UK)

~10

Jones Ltd., Motor Constructions, Brain Associates

**FR** (France)

~5

Dupont de la Rivière, SEC System SA

Other (AU, NZ, PT, RU, etc.)

~20

Various

Approximately **325 vendors have bank details on file** (out of ~500 master records). This means roughly 65% of vendors can be paid electronically. However, having bank details does not guarantee F110 will use them — the ZWELS field must also permit electronic payment methods (U, T, E).

## §7 — Cross-Company Code Term Inconsistencies

Some vendors are maintained in multiple company codes with different payment terms. While this can be legitimate (different commercial agreements per entity), it also creates inconsistency in cash flow behavior and makes payment term compliance monitoring more complex.

Vendor

Name

CoCd

ZTERM

Net Days

Observation

0000001550

Stadtwerke Frankfurt

1000

ZB00 (immediate)

0

Same vendor: immediate in one CoCd, 45 net in another

3000

ZB01 (45 net)

45

0000001992

IDES UK

1000

ZB00 (immediate)

0

Intercompany entity with inconsistent terms

3000/4000

ZB01 (45 net)

45

0000001008

Motor Constructions Ltd.

1000

ZB01 (45 net)

45

Reversed: 45 net in 1000 but immediate in 2000

2000

ZB00 (immediate)

0

0000001991

Schneider AG Deutschland

1000

ZB02 (45 net, lower disc.)

45

Different discount rates per entity

2000/3000/4000

ZB01 (45 net, higher disc.)

45

0000000111

KBB Schwarze Pumpe

1000

ZB02 (45 net)

45

Different term families: ZB02 vs ZB50

3000

ZB50 (30 net)

30

## §8 — Detailed Findings (Prioritized)

**Finding 1: DEM 46M+ in Same-Day Payments Against 45-Day Terms (CRITICAL)**  
_Tables: BSAK, LFB1_  

Vendors AluCast (300), C.E.B. BERLIN (100), PAQ Deutschland (1005), and Müller KG (1002) — all assigned ZB01 (45-day net) — have cleared payment documents where BUDAT equals ZFBDT. The total value of these Day-0 payments exceeds DEM 46 million for AluCast alone. Cash leaves the organization 45 days before contractually required, with no additional discount captured beyond what Day-14 payment would yield.

**Finding 2: 26% of Vendor Records Missing Payment Terms (HIGH)**  
_Tables: LFB1_  

Approximately 130 vendor-company code pairs have blank ZTERM in LFB1. Without a master-level payment term, the due date defaults to the document date or "payable immediately," depending on document type configuration. This effectively eliminates any payment scheduling for these vendors. The F110 program will either select them for immediate payment or skip them entirely (depending on the "next payment date" parameter).

**Finding 3: Manual Payments Bypassing F110 (HIGH)**  
_Tables: BSAK, REGUH_  

Clearing documents in BSAK frequently show a blank ZTERM on the payment document side, indicating manual F-53/F-58 postings rather than F110-generated payments. REGUH confirms that F110 runs exist only for company code 1000 with a limited set of ~12 recurring payment vendors. The high-value procurement vendors (300, 100, 1002, 1005) are NOT included in F110 runs. Company codes 0005 and 0006 have zero F110 history.

**Finding 4: 38% of Vendors Missing Payment Method (MEDIUM)**  
_Tables: LFB1_  

Approximately 190 vendor-company code pairs have blank ZWELS (payment method). F110 requires ZWELS to determine how to pay a vendor. Without it, the vendor is excluded from automatic payment runs. This is a root cause of why so many payments are manual.

**Finding 5: 55% Missing Tolerance Group (MEDIUM)**  
_Tables: LFB1_  

Approximately 275 vendor records lack TOGRU (tolerance group). Without a tolerance group, the system cannot enforce payment amount variance checks during clearing. An AP clerk could post a payment for any amount without a system warning about the discrepancy.

## §9 — Optimization Strategy

The optimization strategy addresses the findings in order of business impact, starting with the highest-value quick wins and progressing to structural improvements. Each action is designed to bring vendor payment behavior into alignment with contractual payment terms while preserving discount opportunities.

### 9.1 — Remediation Priority Matrix

#

Action

Impact

Effort

Owner

Priority

1

**Populate blank ZTERM for ~130 vendors** — Mass update via FK02/LSMW with appropriate default terms based on vendor category

HIGH

Medium

Master Data Team

P1 — This Week

2

**Populate blank ZWELS for ~190 vendors** — Assign payment method (S/U/C) based on country and bank detail availability

HIGH

Medium

Master Data Team

P1 — This Week

3

**Expand F110 to include procurement vendors** — Configure payment run to include AluCast, C.E.B. BERLIN, Müller KG, PAQ Deutschland and all ZB01 vendors

HIGH

Medium

FI Team / FBZP

P1 — This Week

4

**Enable F110 for company codes 0005 and 0006** — No automatic payments exist for these entities

MEDIUM

Medium

FI Team

P2 — This Month

5

**Assign TOGRU for ~275 vendors** — Set tolerance groups to control payment amount variances

MEDIUM

Low

Master Data Team

P2 — This Month

6

**Standardize cross-CoCd terms** — Review vendors like 1550 (Stadtwerke) with conflicting terms across entities

LOW

Low

Master Data Team

P3 — Next Quarter

7

**Monthly early payment monitoring** — Configure FBL1N variant to flag payments made >5 days before due date

MEDIUM

Low

AP Manager

P3 — Next Quarter

## §10 — Quick Wins — Immediate Actions (This Week)

The following actions can be executed immediately with minimal configuration effort and will produce measurable improvements in payment term compliance within the first payment cycle.

Action

Effort

Owner

ETA

Update ZTERM for top 20 vendors by open payable value (FK02)

2 hours

Master Data

Day 1

Update ZWELS for top 20 vendors to enable F110 selection

2 hours

Master Data

Day 1

Schedule F110 test run for CoCd 1000 including procurement vendors

4 hours

FI Team

Day 2

Create FBL1N report variant filtering BUDAT < ZFBDT + ZBD3T

1 hour

AP Manager

Day 2

Review and correct the 5 cross-CoCd term inconsistencies identified

2 hours

Master Data

Day 3

## §11 — Implementation Roadmap

The roadmap below organizes remediation into three phases: immediate quick wins, short-term structural fixes, and longer-term process improvements.

Phase

Timeline

Actions

Expected Outcome

**Phase 1: Quick Wins**

Week 1

Populate ZTERM/ZWELS for top 20 vendors; test F110 with procurement vendors; create monitoring report

Top 20 vendors moved to automated payment; early payment for high-value invoices eliminated

**Phase 2: Mass Remediation**

Weeks 2–4

LSMW mass update for remaining 130 blank-ZTERM and 190 blank-ZWELS vendors; assign TOGRU for 275 vendors; enable F110 for CoCd 0005/0006

100% vendor master completeness for payment-critical fields; F110 coverage across all company codes

**Phase 3: Process Controls**

Month 2–3

Implement payment approval workflow for >Day-0 payments; monthly early-payment KPI dashboard; standardize cross-CoCd terms

Sustained compliance; KPI tracking; no regression to manual early payments

## §12 — Expected Outcomes & KPIs

The following table projects the expected improvement in key payment compliance metrics after implementing the remediation roadmap. The "Before" column reflects current state from live data; the "After" column represents the target state after Phase 2 completion.

Dimension

Before (Current State)

After (Target State)

Improvement

Vendors with blank ZTERM

~130 (26%)

0 (0%)

100% reduction in uncontrolled terms

Vendors with blank ZWELS

~190 (38%)

0 (0%)

100% F110-eligible

F110 coverage (company codes)

1 of 3 active CoCds

3 of 3 active CoCds

Full automation coverage

Day-0 payments on 45-day terms

Routine (DEM 46M+ detected)

Zero — F110 respects due dates

45 days additional cash retention

Discount capture rate

Uncontrolled (Day-0 = no targeted discount)

Day-14 payment → 3% ZB01 discount captured

~3% savings on eligible invoices

Tolerance group coverage

45% assigned

100% assigned

Full payment variance control

Cross-CoCd term consistency

5 vendors with conflicts

0 unjustified conflicts

Consistent cash flow behavior

**C16 — Vendor Payment Terms Compliance Analysis**  
Generated: 28 April 2026 | System: ECC 7.40, Client 800  
Data Sources: LFB1 (500 vendor-CoCd records), BSAK (2,000+ cleared items), BSIK (500 open items), LFA1 (500 vendor names), LFBK (500 bank details), REGUH (200 F110 payment run records), T052U (58 payment term descriptions)  
All findings derived from live SAP ECC system queries — zero fabrication.

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._