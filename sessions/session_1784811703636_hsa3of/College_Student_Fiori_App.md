...
## 1 College Student Management — Fiori App Package

**Deployment Status:** Your RAP backend is deployed to SAP (ZSB\_STUDENT\_V4 activated in $TMP). The V4 service binding requires manual publish via ADT Service Binding editor (transaction: right-click → Publish in Eclipse ADT).

### What Was Deployed to SAP
...
✅ Activated — needs publish

## 2 Next Step: Publish Service Binding in ADT

The V4 service binding is activated but needs to be **published** to expose the OData endpoint:

1.  Open **Eclipse ADT** connected to your SAP system
2.  Navigate to `$TMP → Service Bindings → ZSB_STUDENT_V4`
3.  Double-click to open the Service Binding editor
4.  Click the **Publish** button (or right-click → Publish Local Service Endpoint)
5.  Wait for the green checkmark confirming the service is published

After publishing, your OData V4 endpoint will be available at:

```
https://<your-sap-host>/sap/opu/odata4/sap/zsb_student_v4/srvd_a2x/sap/zsd_student/0001/
```

## 3 Testing the Published Service

Once published, test with these URLs in your browser (append to your SAP host):
...
`/sap/opu/odata4/sap/zsb_student_v4/srvd_a2x/sap/zsd_student/0001/`

Metadata

`/sap/opu/odata4/sap/zsb_student_v4/srvd_a2x/sap/zsd_student/0001/$metadata`

Student Data

`/sap/opu/odata4/sap/zsb_student_v4/srvd_a2x/sap/zsd_student/0001/ZC_STUDENT`

## 4 Generate Fiori Elements App in ADT

After the service is published, generate a Fiori Elements app:

1.  In Eclipse ADT, right-click on `ZSB_STUDENT_V4`
2.  Select **New → Fiori Application** (or use the SAP Fiori Tools extension in VS Code)
3.  Choose template: **List Report Object Page**
4.  Select entity: **ZC\_STUDENT**
5.  Configure columns and filters as needed
6.  Generate and preview

## 5 Alternative: VS Code Fiori Tools

You can also generate the Fiori app using VS Code:

1.  Install the **SAP Fiori tools - Application Generator** extension
2.  Open Command Palette (Ctrl+Shift+P) → **Fiori: Open Application Generator**
3.  Select **SAP Fiori elements** → **List Report Page V4**
4.  Data source: **Connect to a System**
5.  Enter your SAP system URL and credentials
6.  Select service: **ZSD\_STUDENT**
7.  Main entity: **ZC\_STUDENT**
8.  Generate and run with `npm start`

## 6 CDS View Structure Reference

Your consumption view (ZC\_STUDENT) exposes these fields for the Fiori UI:
...
**Tip:** Add UI annotations to the DDLX (ZC\_STUDENT metadata extension) to customize the Fiori Elements display — column order, filter fields, criticality coloring for Status, header info, and facets.

**C16 — AI for SAP operational efficiency**  |  Document version 1.0  |  23 July 2026
