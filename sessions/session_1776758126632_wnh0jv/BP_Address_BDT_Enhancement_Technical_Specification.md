  Technical Specification — BP Address Enhancement via BDT :root { --primary: #6B3FA0; --secondary: #B5567A; --accent: #C97B3A; --success: #27ae60; --warning: #e67e22; --danger: #e74c3c; --info: #3498db; --text: #2c3e50; --bg: #e9ecef; --card-bg: #ffffff; } \* { box-sizing: border-box; margin: 0; padding: 0; } body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: var(--text); background: var(--bg); line-height: 1.7; } .doc-wrapper { max-width: 1120px; margin: 0 auto; background: var(--card-bg); box-shadow: 0 4px 24px rgba(0,0,0,0.12); border-radius: 0; } .header-banner { background: linear-gradient(135deg, #C97B3A, #B5567A, #6B3FA0); color: #fff; padding: 40px 48px; } .header-banner img { height: 50px; margin-bottom: 12px; border-radius: 8px; } .header-banner h1 { font-size: 2em; font-weight: 700; margin-bottom: 6px; } .header-banner .subtitle { font-size: 1.1em; opacity: 0.92; } .header-banner .badges { margin-top: 12px; display: flex; gap: 10px; flex-wrap: wrap; } .header-banner .badge { background: rgba(255,255,255,0.2); padding: 4px 14px; border-radius: 20px; font-size: 0.85em; backdrop-filter: blur(4px); } .meta-bar { background: #f8f9fa; border-bottom: 2px solid #eee; padding: 16px 48px; display: flex; flex-wrap: wrap; gap: 24px; font-size: 0.92em; } .meta-bar .meta-item { display: flex; align-items: center; gap: 6px; } .meta-bar .meta-label { font-weight: 600; color: var(--primary); } .toc { padding: 28px 48px; background: #fafbfc; border-bottom: 1px solid #eee; } .toc h2 { font-size: 1.1em; color: var(--primary); margin-bottom: 12px; } .toc ol { columns: 2; column-gap: 40px; padding-left: 20px; } .toc li { font-size: 0.9em; margin-bottom: 4px; } .toc a { color: var(--primary); text-decoration: none; } .toc a:hover { text-decoration: underline; } .content { padding: 36px 48px; } .section { margin-bottom: 44px; } .section-title { color: var(--primary); font-size: 1.35em; font-weight: 700; border-bottom: 3px solid var(--secondary); padding-bottom: 10px; margin-bottom: 20px; display: flex; align-items: center; gap: 12px; } .section-title .num { background: var(--secondary); color: #fff; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.75em; font-weight: 700; flex-shrink: 0; } .exec-summary { background: #f6fef9; border-left: 5px solid var(--success); padding: 20px 24px; margin-bottom: 20px; border-radius: 0 8px 8px 0; } .exec-summary h3 { color: var(--success); margin-bottom: 10px; } .kpi-row { display: flex; gap: 16px; flex-wrap: wrap; margin: 20px 0; } .kpi-card { flex: 1; min-width: 180px; background: linear-gradient(135deg, var(--primary), var(--secondary)); color: #fff; border-radius: 12px; padding: 18px 20px; text-align: center; } .kpi-card .kpi-value { font-size: 2em; font-weight: 800; } .kpi-card .kpi-label { font-size: 0.82em; opacity: 0.9; margin-top: 4px; } table { width: 100%; border-collapse: collapse; margin: 16px 0; font-size: 0.92em; } thead { background: var(--primary); color: #fff; } th { padding: 10px 14px; text-align: left; font-weight: 600; } td { padding: 10px 14px; border-bottom: 1px solid #eef; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } .info-box { background: #eef6ff; border: 1px solid #b8daff; border-left: 5px solid var(--info); padding: 16px 20px; border-radius: 0 8px 8px 0; margin: 16px 0; } .info-box h4 { color: var(--info); margin-bottom: 6px; } .warn-box { background: #fff8e1; border: 1px solid #ffe082; border-left: 5px solid var(--warning); padding: 16px 20px; border-radius: 0 8px 8px 0; margin: 16px 0; } .warn-box h4 { color: var(--warning); margin-bottom: 6px; } .critical-box { background: #ffeef0; border: 1px solid #ffc1c7; border-left: 5px solid var(--danger); padding: 16px 20px; border-radius: 0 8px 8px 0; margin: 16px 0; } .critical-box h4 { color: var(--danger); margin-bottom: 6px; } .success-box { background: #f0faf0; border: 1px solid #a5d6a7; border-left: 5px solid var(--success); padding: 16px 20px; border-radius: 0 8px 8px 0; margin: 16px 0; } .success-box h4 { color: var(--success); margin-bottom: 6px; } pre { background: #f5f5f5; border-left: 4px solid var(--secondary); padding: 16px; overflow-x: auto; font-family: 'Cascadia Code', 'Fira Code', monospace; font-size: 0.88em; line-height: 1.5; margin: 12px 0; border-radius: 0 6px 6px 0; } code { font-family: 'Cascadia Code', 'Fira Code', monospace; } details { margin: 12px 0; } details summary { background: #f0f4f8; padding: 10px 16px; border-radius: 6px; cursor: pointer; font-weight: 600; color: var(--primary); } details summary:hover { background: #e4e9f0; } .phase-card { background: #f8f9fa; border: 1px solid #e0e5ec; border-radius: 10px; padding: 20px; margin: 16px 0; } .phase-card h3 { color: var(--primary); margin-bottom: 10px; display: flex; align-items: center; gap: 8px; } .phase-card .phase-num { background: var(--primary); color: #fff; padding: 3px 10px; border-radius: 14px; font-size: 0.82em; } .obj-table td:first-child { font-family: 'Cascadia Code', monospace; font-weight: 600; color: var(--primary); } .step-list { counter-reset: step; list-style: none; padding: 0; } .step-list li { counter-increment: step; padding: 12px 0 12px 48px; position: relative; border-left: 2px solid #ddd; margin-left: 16px; } .step-list li::before { content: counter(step); position: absolute; left: -14px; width: 28px; height: 28px; background: var(--primary); color: #fff; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.8em; font-weight: 700; } .step-list li:last-child { border-left: none; } .arch-diagram { background: #f8f9fa; border: 1px solid #e0e5ec; border-radius: 10px; padding: 24px; margin: 16px 0; text-align: center; } .doc-footer { background: #2c3e50; color: #fff; padding: 28px 48px; font-size: 0.85em; } .doc-footer a { color: #7fb3d6; } h3 { color: var(--primary); margin: 20px 0 10px; } h4 { color: #555; margin: 14px 0 8px; } ul, ol { margin: 8px 0 8px 20px; } li { margin-bottom: 4px; } p { margin-bottom: 12px; } .badge-ecc { background: var(--info); color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-bdt { background: var(--success); color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-new { background: var(--warning); color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-existing { background: var(--primary); color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; } .toc { page-break-after: always; } .section { page-break-inside: avoid; } } mermaid.initialize({startOnLoad:true, theme:'neutral'});

![C16](data:image/jpeg;base64,)

![C16](data:image/jpeg;base64,)

# BP Address Enhancement via BDT

Technical Specification — Custom Address Tab in Transaction BP

📋 C16 Technical Specification v1.0 🏢 SAP ECC 7.40 🔧 BDT (Business Data Toolset) 📅 21 April 2026

System: ECC 7.40 • Client 800

Transaction: BP (Business Partner)

Framework: BDT (Business Data Toolset)

Existing BPs: 28,558

Address Records: 9,071

Reference Pattern: ZBUPA (18 FMs)

## 📑 Table of Contents

1.  [Executive Summary](#executive-summary)
2.  [Business Objective](#business-objective)
3.  [Current System Landscape](#system-landscape)
4.  [Data Model Design](#data-model)
5.  [BDT Architecture & Configuration](#bdt-architecture)
6.  [Object Inventory](#object-inventory)
7.  [Function Group Specification](#function-group)
8.  [Screen Design (Dynpro 0100)](#screen-design)
9.  [Function Module Specifications](#function-modules)
10.  [BDT Customizing Configuration](#bdt-customizing)
11.  [BAdI Integration (Optional)](#badi-integration)
12.  [Authorization & Security](#authorization)
13.  [Testing Strategy](#testing-strategy)
14.  [S/4HANA Migration Considerations](#migration-considerations)
15.  [Implementation Roadmap](#implementation-roadmap)

1 Executive Summary

### Overview

This specification defines the design and implementation of a **custom address extension tab** within SAP Transaction BP (Business Partner), using the **Business Data Toolset (BDT)** framework. The enhancement adds custom address-level fields (GPS coordinates, address verification status, custom classification, and comments) to each BP address record, fully integrated into the standard BP user interface without modifying SAP standard screens.

28,558

Total Business Partners

9,071

Address Records (BUT020)

2

Existing BDT Implementations

13

Objects to Create

### Key Design Decisions

*   **BDT over BAdI-only:** BDT provides both UI integration (custom tab) and data persistence lifecycle management — BAdIs alone cannot add screens to the BP transaction.
*   **Separate Z-table over CI include:** Custom fields are stored in `ZBUT020_EXT` keyed on PARTNER + ADDRNUMBER, rather than appending to BUT020 or ADRC. This avoids impacting standard table performance and simplifies S/4HANA migration.
*   **Address-level attachment:** The extension is linked to the address object part (ADDR), meaning the custom tab appears within the address detail view — not as a standalone BP-level tab. Each address gets its own extension data independently.
*   **Reference pattern reuse:** The implementation follows the exact naming conventions and FM patterns established by the existing `ZBUPA` function group (18 FMs in package `ZBUPA`), ensuring consistency with prior BDT work in this system.

2 Business Objective

The standard SAP BP address data model (tables `BUT020` + `ADRC`) provides comprehensive address fields including street, city, postal code, country, region, time zone, and transport zone. However, business requirements exist for **additional address-level attributes** not covered by the standard model:

Business Requirement

Standard Coverage

Gap

GPS coordinates for logistics routing

ADRC has no latitude/longitude fields

GAP — Custom Field

Address verification status tracking

No verification workflow in standard

GAP — Custom Field

Custom address classification/type codes

BUT021 address usages cover standard types only

GAP — Custom Field

Free-text address comments/notes

No long-text field on address level

GAP — Custom Field

Address change audit trail

Standard change documents exist but no custom field audit

GAP — Custom Field

#### 💡 Target User Experience

When a user opens Transaction BP, navigates to the **Addresses** section, and clicks on a specific address, they will see a new tab titled **"Custom Address Data"** alongside the standard address detail tabs. This tab displays the custom fields in a clean form layout, respecting the current BP activity mode (Display/Change/Create). Data is saved automatically when the user clicks the standard BP Save button.

3 Current System Landscape

### 3.1 BP Data Volumes

Entity

Table

Record Count

Notes

Business Partners (Total)

BUT000

**28,558**

Persons (Type 1) + Organizations (Type 2)

Address Assignments

BUT020

**9,071**

Some BPs have multiple addresses

Central Addresses

ADRC

—

Linked via ADDRNUMBER

### 3.2 Existing BDT Implementations (Reference Patterns)

Two BDT implementations already exist in this system, both in package `ZBUPA`. These serve as proven reference patterns:

Function Group

Program

FMs

Screens

Custom Table

Pattern

ZBUPA

SAPLZBUPA

18

ZCUS10, ZCUS20

ZBUTHOBBY (CLIENT + PARTNER + HOBBY)

BP-level custom tab

ZBUPAXX

SAPLZBUPAXX

18

ZCXX10, ZCXX20

ZBUTHOBBY (shared)

Second BP-level custom tab

#### Established FM Naming Convention

Both function groups follow the same 18-FM pattern. For our implementation, we adopt the prefix `Z_ZADR_BUPA_`:

FM Category

Existing Pattern (ZBUPA)

New Pattern (ZBUPA\_ADDR)

Purpose

PBO (Screen Display)

Z\_ZCUS\_BUPA\_PBO\_ZCUS10

Z\_ZADR\_BUPA\_PBO\_ZADR10

Return screen number to BDT

PAI (Screen Input)

Z\_ZCUS\_BUPA\_PAI\_ZCUS10

Z\_ZADR\_BUPA\_PAI\_ZADR10

Validate input, return errors

PBO (Subscreen)

Z\_ZCUS\_BUPA\_PBC\_ZCUS20

Z\_ZADR\_BUPA\_PBC\_ZADR20

Subscreen PBO (if needed)

PAI (BUP300)

Z\_ZCUS\_BUPA\_PAI\_BUP300

Z\_ZADR\_BUPA\_PAI\_BUP300

Main screen PAI integration

ISDAT (Read Data)

Z\_ZCUS\_BUPA\_EVENT\_ISDAT

Z\_ZADR\_BUPA\_EVENT\_ISDAT

Load custom data from DB

ISSTA (Set Activity)

Z\_ZCUS\_BUPA\_EVENT\_ISSTA

Z\_ZADR\_BUPA\_EVENT\_ISSTA

Set Display/Change/Create mode

DSAVE (Save)

Z\_ZCUS\_BUPA\_EVENT\_DSAVE

Z\_ZADR\_BUPA\_EVENT\_DSAVE

Write custom data to DB

DSAVB (Pre-Save)

Z\_ZCUS\_BUPA\_EVENT\_DSAVB

Z\_ZADR\_BUPA\_EVENT\_DSAVB

Pre-commit validation

DSAVC (After Save)

Z\_ZCUS\_BUPA\_EVENT\_DSAVC

Z\_ZADR\_BUPA\_EVENT\_DSAVC

Post-commit actions

XCHNG (Changed?)

Z\_ZCUS\_BUPA\_EVENT\_XCHNG

Z\_ZADR\_BUPA\_EVENT\_XCHNG

Tell BDT if data changed

FCODE (Function Code)

Z\_ZCUS\_BUPA\_EVENT\_FCODE

Z\_ZADR\_BUPA\_EVENT\_FCODE

Custom function codes

DCUAC (Update Activity)

Z\_ZCUS\_BUPA\_EVENT\_DCUAC

Z\_ZADR\_BUPA\_EVENT\_DCUAC

Activity update handling

DTAKE (Take Over)

Z\_ZCUS\_BUPA\_EVENT\_DTAKE

Z\_ZADR\_BUPA\_EVENT\_DTAKE

Data take-over from other source

DINP2 (Input Check)

Z\_ZCUS\_BUPA\_EVENT\_DINP2

Z\_ZADR\_BUPA\_EVENT\_DINP2

Secondary input validation

DLVE1 (Leave Event 1)

Z\_ZCUS\_BUPA\_EVENT\_DLVE1

Z\_ZADR\_BUPA\_EVENT\_DLVE1

Leave screen event

DLVE2 (Leave Event 2)

Z\_ZCUS\_BUPA\_EVENT\_DLVE2

Z\_ZADR\_BUPA\_EVENT\_DLVE2

Leave screen event 2

UPDATE (DB Update)

Z\_ZCUS\_BUPA\_UPDATE

Z\_ZADR\_BUPA\_UPDATE

Update function module

GET (Read Helper)

Z\_ZCUS\_BUPA\_ZBUTHOBBY\_GET

Z\_ZADR\_BUPA\_ZBUT020EXT\_GET

Utility read function

### 3.3 Available BAdIs (Address-Level)

These customer-facing BAdIs can optionally complement the BDT implementation:

BAdI

Purpose

When to Use

`BUPA_ADDR_CHECK`

Address validation before save

Cross-field validation beyond PAI

`BUPA_ADDR_UPDATE`

Post-address-save logic

Trigger downstream sync after custom data save

`BUPA_ADDR_IMPORT`

Enrich address on read

Populate custom fields from external sources

`BUPA_ADDR_EXPORT`

Transform address on output

Mask/transform for outbound interfaces

4 Data Model Design

### 4.1 Entity Relationship Diagram

erDiagram BUT000 ||--o{ BUT020 : "has addresses" BUT020 ||--|| ADRC : "links to central address" BUT020 ||--o| ZBUT020\_EXT : "has custom extension" ADRC ||--o{ ADR6 : "has email addresses" BUT020 ||--o{ BUT021 : "has address usages" BUT000 { char10 PARTNER PK char1 TYPE char40 NAME\_ORG1 char40 NAME\_FIRST char40 NAME\_LAST } BUT020 { char10 PARTNER PK char10 ADDRNUMBER PK dec15 ADDR\_VALID\_FROM dec15 ADDR\_VALID\_TO raw16 ADDRESS\_GUID } ADRC { char10 ADDRNUMBER PK char40 NAME1 char60 STREET char10 HOUSE\_NUM1 char10 POST\_CODE1 char40 CITY1 char3 REGION char3 COUNTRY } ZBUT020\_EXT { char10 PARTNER PK char10 ADDRNUMBER PK dec11\_7 ZZ\_GPS\_LAT dec12\_7 ZZ\_GPS\_LON char4 ZZ\_ADDR\_TYPE char1 ZZ\_VERIFIED dats ZZ\_VERIF\_DATE char255 ZZ\_COMMENTS char12 ZZ\_CHANGED\_BY dats ZZ\_CHANGED\_ON }

### 4.2 Custom Table: ZBUT020\_EXT

Field

Key

Data Element

Data Type

Length

Description

`CLIENT`

✅

MANDT

CLNT

3

Client (automatic)

`PARTNER`

✅

BU\_PARTNER

CHAR

10

Business Partner Number

`ADDRNUMBER`

✅

AD\_ADDRNUM

CHAR

10

Address Number (from BUT020)

`ZZ_GPS_LAT`

ZBU\_GPS\_LAT (new)

DEC

11,7

GPS Latitude (-90.0000000 to +90.0000000)

`ZZ_GPS_LON`

ZBU\_GPS\_LON (new)

DEC

12,7

GPS Longitude (-180.0000000 to +180.0000000)

`ZZ_ADDR_TYPE`

ZBU\_ADDR\_TYPE (new)

CHAR

4

Custom Address Type Code

`ZZ_VERIFIED`

ZBU\_VERIFIED (new)

CHAR

1

Address Verified Flag (X = verified)

`ZZ_VERIF_DATE`

ZBU\_VERIF\_DATE (new)

DATS

8

Last Verification Date

`ZZ_COMMENTS`

ZBU\_COMMENTS (new)

CHAR

255

Address Comments / Notes

`ZZ_CHANGED_BY`

AENAM

CHAR

12

Last Changed By (system field)

`ZZ_CHANGED_ON`

AEDAT

DATS

8

Last Changed On (system field)

#### 🔑 Key Design: PARTNER + ADDRNUMBER

The table is keyed on `CLIENT + PARTNER + ADDRNUMBER`, mirroring the BUT020 key structure. This ensures each address of each business partner gets its own independent extension record. When a user views address `0000037157` of partner `0000000001`, the BDT framework reads exactly one ZBUT020\_EXT row.

### 4.3 Custom Data Elements (New)

Data Element

Domain / Built-in

Length

Short Text

Heading

`ZBU_GPS_LAT`

New domain: ZBU\_D\_GPS\_LAT (DEC 11,7)

11,7

GPS Latitude

Latitude

`ZBU_GPS_LON`

New domain: ZBU\_D\_GPS\_LON (DEC 12,7)

12,7

GPS Longitude

Longitude

`ZBU_ADDR_TYPE`

New domain: ZBU\_D\_ADDR\_TYPE (CHAR 4) with fixed values

4

Custom Address Type

Addr Type

`ZBU_VERIFIED`

XFELD (standard checkbox domain)

1

Address Verified

Verified

`ZBU_VERIF_DATE`

DATUM (standard date domain)

8

Verification Date

Verif. Date

`ZBU_COMMENTS`

New domain: ZBU\_D\_COMMENTS (CHAR 255)

255

Address Comments

Comments

#### Domain Fixed Values for ZBU\_D\_ADDR\_TYPE:

Value

Description

MAIN

Main / Primary Address

DLVR

Delivery Address

BILL

Billing Address

WARE

Warehouse / Storage Address

TEMP

Temporary Address

5 BDT Architecture & Configuration

### 5.1 High-Level Architecture

flowchart TB subgraph BP\["Transaction BP"\] UI\["BP UI Framework"\] ADDR\["Address Detail View"\] CUSTOM\["🆕 Custom Address Data Tab"\] end subgraph BDT\["BDT Framework (BUPT)"\] FG\["Field Group: ZADR1"\] SC\["Screen: ZADR10"\] SEC\["Section: ZADR\_SEC"\] EVT\["Events: ISDAT, ISSTA, DSAVE, XCHNG..."\] end subgraph FUGR\["Function Group: ZBUPA\_ADDR"\] PBO\["PBO FM: Z\_ZADR\_BUPA\_PBO\_ZADR10"\] PAI\["PAI FM: Z\_ZADR\_BUPA\_PAI\_ZADR10"\] READ\["ISDAT FM: Z\_ZADR\_BUPA\_EVENT\_ISDAT"\] SAVE\["DSAVE FM: Z\_ZADR\_BUPA\_EVENT\_DSAVE"\] CHK\["XCHNG FM: Z\_ZADR\_BUPA\_EVENT\_XCHNG"\] DYN\["Dynpro 0100 (Screen)"\] end subgraph DB\["Database"\] TAB\["ZBUT020\_EXT"\] end UI --> ADDR ADDR --> CUSTOM CUSTOM --> BDT BDT --> FUGR READ -->|"SELECT"| TAB SAVE -->|"MODIFY"| TAB PBO --> DYN PAI --> DYN style CUSTOM fill:#e8f5e9,stroke:#27ae60,stroke-width:2px style TAB fill:#fff3e0,stroke:#e67e22,stroke-width:2px

### 5.2 Event Flow Sequence

sequenceDiagram participant U as User participant BP as BP Transaction participant BDT as BDT Framework participant FM as ZBUPA\_ADDR FMs participant DB as ZBUT020\_EXT U->>BP: Open BP, click Address BP->>BDT: Navigate to Address Detail BDT->>FM: ISSTA (Set Activity = Display/Change) BDT->>FM: ISDAT (Load Data for PARTNER + ADDRNUMBER) FM->>DB: SELECT SINGLE \* WHERE PARTNER + ADDRNUMBER DB-->>FM: Return custom data (or empty) FM-->>BDT: Data loaded into global variables BDT->>FM: PBO (Display Screen 0100) FM-->>U: Custom tab rendered with data Note over U,DB: User modifies fields... U->>BP: Click Save BP->>BDT: Trigger Save Events BDT->>FM: XCHNG (Data changed?) FM-->>BDT: E\_CHANGED = 'X' BDT->>FM: PAI (Validate input) FM-->>BDT: E\_ERROR = '' (no errors) BDT->>FM: DSAVB (Pre-commit check) FM-->>BDT: OK BDT->>FM: DSAVE (Write to DB) FM->>DB: MODIFY ZBUT020\_EXT BDT->>FM: DSAVC (Post-commit) BDT-->>U: Save confirmed ✅

6 Object Inventory

#

Object Name

Type

Description

Status

1

ZBU\_D\_GPS\_LAT

DOMA

Domain: GPS Latitude (DEC 11,7)

New

2

ZBU\_D\_GPS\_LON

DOMA

Domain: GPS Longitude (DEC 12,7)

New

3

ZBU\_D\_ADDR\_TYPE

DOMA

Domain: Address Type (CHAR 4, fixed values)

New

4

ZBU\_D\_COMMENTS

DOMA

Domain: Comments (CHAR 255)

New

5

ZBU\_GPS\_LAT

DTEL

Data Element: GPS Latitude

New

6

ZBU\_GPS\_LON

DTEL

Data Element: GPS Longitude

New

7

ZBU\_ADDR\_TYPE

DTEL

Data Element: Address Type Code

New

8

ZBU\_COMMENTS

DTEL

Data Element: Address Comments

New

9

ZBUT020\_EXT

TABL

Transparent Table: BP Address Extension

New

10

ZBUPA\_ADDR

FUGR

Function Group: BDT Address Enhancement

New

11

Screen 0100

DYNP

Dynpro: Custom Address Tab (in ZBUPA\_ADDR)

New

12

Z\_ZADR\_BUPA\_\* (18 FMs)

FUNC

Function Modules: BDT event handlers

New

13

ZBUPA\_ADDR (msg class)

MSAG

Message Class: Validation messages

New

#### 📦 Package Recommendation

All objects should be created in package `ZBUPA` (same as existing BDT implementations) to maintain organizational consistency. If a separate package is preferred, create `ZBUPA_ADDR` under the same transport layer.

7 Function Group Specification

### 7.1 TOP Include — Global Data (LZBUPA\_ADDRTOP)

View ABAP Source — TOP Include

```abap
FUNCTION-POOL ZBUPA_ADDR.

*----------------------------------------------------------------------*
* Global Data Declarations for BDT Address Enhancement
*----------------------------------------------------------------------*

TABLES: ZBUT020_EXT.

* Working areas
DATA: gs_addr_ext      TYPE zbut020_ext,      " Current state
      gs_addr_ext_old  TYPE zbut020_ext,      " State at load (for change detection)
      gv_partner       TYPE bu_partner,        " Current BP number
      gv_addrnumber    TYPE ad_addrnum,        " Current address number
      gv_changed       TYPE abap_bool,         " Data changed flag
      gv_is_new        TYPE abap_bool.         " New record (no DB entry exists)

* BDT control
DATA: gv_activity      TYPE c LENGTH 1.       " H=Display, V=Change, A=Create

* Constants
CONSTANTS: gc_display TYPE c LENGTH 1 VALUE 'H',
           gc_change  TYPE c LENGTH 1 VALUE 'V',
           gc_create  TYPE c LENGTH 1 VALUE 'A'.
```

### 7.2 Screen Flow Logic (Dynpro 0100)

View Screen Flow Logic

```abap
*--- PBO ---*
PROCESS BEFORE OUTPUT.
  MODULE pbo_0100.

*--- PAI ---*
PROCESS AFTER INPUT.
  MODULE pai_0100.
```

View PBO/PAI Module Implementations

```abap
*----------------------------------------------------------------------*
* MODULE pbo_0100 OUTPUT
*----------------------------------------------------------------------*
MODULE pbo_0100 OUTPUT.
  " Control field input/display based on BDT activity
  LOOP AT SCREEN.
    IF gv_activity = gc_display.
      screen-input = 0.           " All fields read-only in display mode
    ENDIF.
    " Audit fields always display-only
    IF screen-name CS 'ZZ_CHANGED_BY' OR screen-name CS 'ZZ_CHANGED_ON'.
      screen-input = 0.
    ENDIF.
    MODIFY SCREEN.
  ENDLOOP.

  " Push data to screen
  MOVE-CORRESPONDING gs_addr_ext TO zbut020_ext.
ENDMODULE.

*----------------------------------------------------------------------*
* MODULE pai_0100 INPUT
*----------------------------------------------------------------------*
MODULE pai_0100 INPUT.
  " Pull data from screen
  MOVE-CORRESPONDING zbut020_ext TO gs_addr_ext.
  
  " Detect changes
  IF gs_addr_ext <> gs_addr_ext_old.
    gv_changed = abap_true.
  ELSE.
    gv_changed = abap_false.
  ENDIF.
ENDMODULE.
```

8 Screen Design (Dynpro 0100)

### 8.1 Screen Layout

┌────────────────────────────────────────────────────────────────────────┐
│  📍 Custom Address Data                                              │
│                                                                      │
│  ┌─── Location ─────────────────────────────────────────────────┐    │
│  │                                                               │    │
│  │  GPS Latitude:   \[\_\_\_\_\_\_\_\_\_\_\_\_\_\_ \]    (DEC 11,7)             │    │
│  │  GPS Longitude:  \[\_\_\_\_\_\_\_\_\_\_\_\_\_\_ \]    (DEC 12,7)             │    │
│  │                                                               │    │
│  └───────────────────────────────────────────────────────────────┘    │
│                                                                      │
│  ┌─── Classification ──────────────────────────────────────────┐     │
│  │                                                              │     │
│  │  Address Type:   \[\_\_\_\_▼\]  (Dropdown: MAIN/DLVR/BILL/WARE)  │     │
│  │  Verified:       \[☐\]     (Checkbox)                          │     │
│  │  Verified Date:  \[\_\_\_\_\_\_\_\_\_\_\]  (Date picker, dd.mm.yyyy)    │     │
│  │                                                              │     │
│  └──────────────────────────────────────────────────────────────┘     │
│                                                                      │
│  ┌─── Notes ───────────────────────────────────────────────────┐     │
│  │                                                              │     │
│  │  Comments:  \[\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_│     │
│  │             |                                                 │     │
│  │             |\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\]│     │
│  │                                                              │     │
│  └──────────────────────────────────────────────────────────────┘     │
│                                                                      │
│  ┌─── Audit ───────────────────────────────────────────────────┐     │
│  │                                                              │     │
│  │  Changed By: \[\_\_\_\_\_\_\_\_\_\_\_\_\]  Changed On: \[\_\_\_\_\_\_\_\_\_\_\]        │     │
│  │  (Display only — auto-populated on save)                     │     │
│  │                                                              │     │
│  └──────────────────────────────────────────────────────────────┘     │
└────────────────────────────────────────────────────────────────────────┘

### 8.2 Screen Element Properties

Element

Screen Name

Dict Ref

Type

Input

Notes

GPS Latitude

ZBUT020\_EXT-ZZ\_GPS\_LAT

ZBU\_GPS\_LAT

I/O Field

Change mode

7 decimal places

GPS Longitude

ZBUT020\_EXT-ZZ\_GPS\_LON

ZBU\_GPS\_LON

I/O Field

Change mode

7 decimal places

Address Type

ZBUT020\_EXT-ZZ\_ADDR\_TYPE

ZBU\_ADDR\_TYPE

Dropdown (F4)

Change mode

Fixed value help from domain

Verified

ZBUT020\_EXT-ZZ\_VERIFIED

ZBU\_VERIFIED

Checkbox

Change mode

—

Verification Date

ZBUT020\_EXT-ZZ\_VERIF\_DATE

ZBU\_VERIF\_DATE

Date Field

Change mode

Calendar popup

Comments

ZBUT020\_EXT-ZZ\_COMMENTS

ZBU\_COMMENTS

I/O Field

Change mode

255 chars, multi-line

Changed By

ZBUT020\_EXT-ZZ\_CHANGED\_BY

AENAM

I/O Field

Display only

Always read-only

Changed On

ZBUT020\_EXT-ZZ\_CHANGED\_ON

AEDAT

Date Field

Display only

Always read-only

9 Function Module Specifications

Below are the **7 core function modules** that drive the BDT integration. The remaining 11 FMs follow the established pattern from `ZBUPA` and are stub implementations (empty bodies) unless specific logic is required.

### 9.1 Z\_ZADR\_BUPA\_EVENT\_ISDAT — Read Custom Data

View Full ABAP Source

```abap
FUNCTION z_zadr_bupa_event_isdat.
*"----------------------------------------------------------------------
*" IMPORTING
*"     VALUE(I_PARTNER)    TYPE BU_PARTNER
*"     VALUE(I_ADDRNUMBER) TYPE AD_ADDRNUM OPTIONAL
*"----------------------------------------------------------------------
  CLEAR: gs_addr_ext, gs_addr_ext_old, gv_changed, gv_is_new.
  gv_partner    = i_partner.
  gv_addrnumber = i_addrnumber.

  " Read existing custom address data
  IF i_addrnumber IS NOT INITIAL.
    SELECT SINGLE * FROM zbut020_ext
      INTO gs_addr_ext
      WHERE client     = sy-mandt
        AND partner    = i_partner
        AND addrnumber = i_addrnumber.

    IF sy-subrc = 0.
      gs_addr_ext_old = gs_addr_ext.
      gv_is_new = abap_false.
    ELSE.
      " Initialize for new record
      gs_addr_ext-client     = sy-mandt.
      gs_addr_ext-partner    = i_partner.
      gs_addr_ext-addrnumber = i_addrnumber.
      gv_is_new = abap_true.
    ENDIF.
  ENDIF.
ENDFUNCTION.
```

### 9.2 Z\_ZADR\_BUPA\_EVENT\_ISSTA — Set Activity Mode

View Full ABAP Source

```abap
FUNCTION z_zadr_bupa_event_issta.
*"----------------------------------------------------------------------
*" IMPORTING
*"     VALUE(I_ACTIVITY) TYPE C
*"----------------------------------------------------------------------
  gv_activity = i_activity.
ENDFUNCTION.
```

### 9.3 Z\_ZADR\_BUPA\_PBO\_ZADR10 — Screen PBO

View Full ABAP Source

```abap
FUNCTION z_zadr_bupa_pbo_zadr10.
*"----------------------------------------------------------------------
*" IMPORTING
*"     VALUE(I_PARTNER) TYPE BU_PARTNER OPTIONAL
*" EXPORTING
*"     VALUE(E_SCREENR) TYPE SYDYNNR
*"----------------------------------------------------------------------
  e_screenr = '0100'.
ENDFUNCTION.
```

### 9.4 Z\_ZADR\_BUPA\_PAI\_ZADR10 — Screen PAI (Validation)

View Full ABAP Source

```abap
FUNCTION z_zadr_bupa_pai_zadr10.
*"----------------------------------------------------------------------
*" IMPORTING
*"     VALUE(I_PARTNER) TYPE BU_PARTNER OPTIONAL
*" EXPORTING
*"     VALUE(E_ERROR)   TYPE C
*"----------------------------------------------------------------------
  CLEAR e_error.

  " BR-01: If Verified = X, verification date is mandatory
  IF gs_addr_ext-zz_verified = 'X' AND gs_addr_ext-zz_verif_date IS INITIAL.
    MESSAGE e001(zbupa_addr) WITH 'Verification date is required when address is verified'.
    e_error = 'X'.
    RETURN.
  ENDIF.

  " BR-02: GPS latitude and longitude must both be filled or both empty
  IF ( gs_addr_ext-zz_gps_lat IS NOT INITIAL AND gs_addr_ext-zz_gps_lon IS INITIAL )
  OR ( gs_addr_ext-zz_gps_lat IS INITIAL AND gs_addr_ext-zz_gps_lon IS NOT INITIAL ).
    MESSAGE e002(zbupa_addr) WITH 'GPS Latitude and Longitude must both be provided'.
    e_error = 'X'.
    RETURN.
  ENDIF.

  " BR-03: Latitude range check
  IF gs_addr_ext-zz_gps_lat IS NOT INITIAL.
    IF gs_addr_ext-zz_gps_lat < -90 OR gs_addr_ext-zz_gps_lat > 90.
      MESSAGE e003(zbupa_addr) WITH 'GPS Latitude must be between -90 and +90'.
      e_error = 'X'.
      RETURN.
    ENDIF.
  ENDIF.

  " BR-04: Longitude range check
  IF gs_addr_ext-zz_gps_lon IS NOT INITIAL.
    IF gs_addr_ext-zz_gps_lon < -180 OR gs_addr_ext-zz_gps_lon > 180.
      MESSAGE e004(zbupa_addr) WITH 'GPS Longitude must be between -180 and +180'.
      e_error = 'X'.
      RETURN.
    ENDIF.
  ENDIF.

  " BR-05: Verification date cannot be in the future
  IF gs_addr_ext-zz_verif_date IS NOT INITIAL AND gs_addr_ext-zz_verif_date > sy-datum.
    MESSAGE e005(zbupa_addr) WITH 'Verification date cannot be in the future'.
    e_error = 'X'.
    RETURN.
  ENDIF.
ENDFUNCTION.
```

### 9.5 Z\_ZADR\_BUPA\_EVENT\_DSAVE — Save to Database

View Full ABAP Source

```abap
FUNCTION z_zadr_bupa_event_dsave.
*"----------------------------------------------------------------------
*" IMPORTING
*"     VALUE(I_PARTNER) TYPE BU_PARTNER OPTIONAL
*"----------------------------------------------------------------------
  CHECK gv_changed = abap_true.

  " Set audit fields
  gs_addr_ext-zz_changed_by = sy-uname.
  gs_addr_ext-zz_changed_on = sy-datum.

  " Ensure key fields are set
  gs_addr_ext-client     = sy-mandt.
  gs_addr_ext-partner    = gv_partner.
  gs_addr_ext-addrnumber = gv_addrnumber.

  " Persist to database (MODIFY = INSERT or UPDATE)
  MODIFY zbut020_ext FROM gs_addr_ext.

  IF sy-subrc = 0.
    " Reset change tracking
    gs_addr_ext_old = gs_addr_ext.
    CLEAR gv_changed.
  ENDIF.
ENDFUNCTION.
```

### 9.6 Z\_ZADR\_BUPA\_EVENT\_XCHNG — Check if Data Changed

View Full ABAP Source

```abap
FUNCTION z_zadr_bupa_event_xchng.
*"----------------------------------------------------------------------
*" EXPORTING
*"     VALUE(E_CHANGED) TYPE C
*"----------------------------------------------------------------------
  IF gv_changed = abap_true.
    e_changed = 'X'.
  ELSE.
    CLEAR e_changed.
  ENDIF.
ENDFUNCTION.
```

### 9.7 Z\_ZADR\_BUPA\_EVENT\_DSAVB — Pre-Commit Validation

View Full ABAP Source

```abap
FUNCTION z_zadr_bupa_event_dsavb.
*"----------------------------------------------------------------------
*" IMPORTING
*"     VALUE(I_PARTNER) TYPE BU_PARTNER OPTIONAL
*" EXPORTING
*"     VALUE(E_ERROR)   TYPE C
*"----------------------------------------------------------------------
  " Final validation before commit
  " Return E_ERROR = 'X' to abort the save
  CLEAR e_error.

  " (Add any cross-check validations here, e.g., verify partner still exists)
ENDFUNCTION.
```

10 BDT Customizing Configuration

All BDT configuration is performed via transaction **BUPT** (BDT Customizing) or **BUS7** (Field Groups/Screens). The configuration connects the function group to the BP transaction framework.

### Step A Field Group Definition

**Path:** BUPT → Field Groups → Define

Setting

Value

Application

`BUP`

Field Group

`ZADR1`

Description

Custom Address Extension Data

### Step B Screen Definition

**Path:** BUPT → Screens → Define

Setting

Value

Application

`BUP`

Screen

`ZADR10`

Description

Custom Address Data Screen

Program

`SAPLZBUPA_ADDR`

Screen Number

`0100`

Field Group

`ZADR1`

PBO Function Module

`Z_ZADR_BUPA_PBO_ZADR10`

PAI Function Module

`Z_ZADR_BUPA_PAI_ZADR10`

### Step C Section Definition

**Path:** BUPT → Sections → Define

Setting

Value

Application

`BUP`

Section

`ZADR_SEC`

Description

Custom Address Data

Screen

`ZADR10`

### Step D View Assignment (Address Detail)

**Path:** BUPT → Views → Assign Sections to Views

Setting

Value

View

Address Detail view (assign section `ZADR_SEC`)

Object Part

`ADDR` (Address)

Position

After standard address tabs

#### ⚠️ Critical Configuration Point

The **Object Part** must be set to `ADDR` to ensure the tab appears within the address detail view (when a specific address is selected). If set to the BP root level, the tab would appear as a standalone BP tab without address context.

### Step E Event Registration

**Path:** BUPT → Events → Assign Function Modules

Event

Code

Function Module

Application

Initial Screen Data

ISDAT

`Z_ZADR_BUPA_EVENT_ISDAT`

BUP

Set Activity Status

ISSTA

`Z_ZADR_BUPA_EVENT_ISSTA`

BUP

Save Data

DSAVE

`Z_ZADR_BUPA_EVENT_DSAVE`

BUP

Save Before Commit

DSAVB

`Z_ZADR_BUPA_EVENT_DSAVB`

BUP

Save After Commit

DSAVC

`Z_ZADR_BUPA_EVENT_DSAVC`

BUP

Check Changed

XCHNG

`Z_ZADR_BUPA_EVENT_XCHNG`

BUP

Function Code

FCODE

`Z_ZADR_BUPA_EVENT_FCODE`

BUP

Update Activity

DCUAC

`Z_ZADR_BUPA_EVENT_DCUAC`

BUP

Take Over Data

DTAKE

`Z_ZADR_BUPA_EVENT_DTAKE`

BUP

Secondary Input Check

DINP2

`Z_ZADR_BUPA_EVENT_DINP2`

BUP

Leave Screen 1

DLVE1

`Z_ZADR_BUPA_EVENT_DLVE1`

BUP

Leave Screen 2

DLVE2

`Z_ZADR_BUPA_EVENT_DLVE2`

BUP

### Step F BP Category Assignment

**Path:** BUPT → Object Parts → Assign to BP Categories

BP Category

Type

Assigned?

1 — Person

Natural Person

✅ Yes

2 — Organization

Legal Entity

✅ Yes

3 — Group

BP Group

Optional (configure based on need)

11 BAdI Integration (Optional Enhancements)

While BDT handles the primary UI and persistence, the following BAdIs can be implemented to extend functionality beyond the screen interaction:

BAdI

Use Case

Implementation

Priority

`BUPA_ADDR_CHECK`

Cross-validate GPS coordinates against country (e.g., ensure coordinates are within the country's bounding box)

Read ZBUT020\_EXT in the check method, compare with ADRC-COUNTRY

Phase 2

`BUPA_ADDR_UPDATE`

Sync custom address data to external systems (e.g., push GPS coordinates to a GIS system after save)

Read ZBUT020\_EXT, call RFC/web service to external system

Phase 3

`BUPA_ADDR_IMPORT`

Auto-populate GPS coordinates from an external geocoding service when address is loaded

On read, check if ZBUT020\_EXT has no GPS → call geocoding API → populate

Phase 3

#### 💡 Recommendation

Implement BAdIs in **Phase 2/3** after the core BDT tab is working and tested. The BDT implementation (Phase 1) is self-contained and does not require BAdIs to function correctly.

12 Authorization & Security

### 12.1 Authorization Objects

The BDT framework inherits the standard BP authorization model. Users who can access/edit addresses via BP can also access/edit the custom tab. No additional authorization objects are required for basic functionality.

Auth Object

Field

Purpose

Source

`B_BUPA_GRP`

BGRP

BP Authorization Group

Standard BP auth

`B_BUPA_RLT`

RLTYP

BP Relationship Type

Standard BP auth

`B_BUPA_ATT`

ATTRIB

BP Attributes

Standard BP auth

### 12.2 Optional: Custom Authorization

If fine-grained access control is needed (e.g., only certain users can modify GPS coordinates), create a custom authorization object:

Auth Object

Field

Values

Purpose

`ZBU_ADEXT` (optional)

ACTVT

01=Create, 02=Change, 03=Display

Control create/change/display for custom address extension

#### 🔒 Security Note

If custom authorization is implemented, add an `AUTHORITY-CHECK` statement at the beginning of the PBO FM (`Z_ZADR_BUPA_PBO_ZADR10`) and the DSAVE FM to enforce access control. The activity code mapping is: Display (H) → ACTVT 03, Change (V) → ACTVT 02, Create (A) → ACTVT 01.

13 Testing Strategy

### 13.1 Test Scenarios

#

Test Case

Steps

Expected Result

Type

T01

Custom tab visible in BP

Open BP → select a partner (e.g., 0000000001) → navigate to Addresses → click an address

Custom Address Data tab is visible alongside standard tabs

✅ Positive

T02

Display mode — fields read-only

Open BP in Display mode → navigate to custom tab

All fields are greyed out / not editable

✅ Positive

T03

Change mode — enter and save data

Open BP in Change mode → enter GPS lat/lon, type, check Verified, enter date, add comment → Save

Data saved successfully, no errors. Re-open and verify data persisted.

✅ Positive

T04

Validation: Verified without date

Check Verified checkbox, leave Verification Date blank → Save

Error message: "Verification date is required when address is verified"

❌ Negative

T05

Validation: GPS lat without lon

Enter GPS Latitude only, leave Longitude blank → Save

Error message: "GPS Latitude and Longitude must both be provided"

❌ Negative

T06

Validation: GPS latitude out of range

Enter GPS Latitude = 95.0 → Save

Error message: "GPS Latitude must be between -90 and +90"

❌ Negative

T07

Validation: Future verification date

Enter Verification Date = tomorrow → Save

Error message: "Verification date cannot be in the future"

❌ Negative

T08

Audit fields auto-populated

Save custom data → check Changed By / Changed On

Changed By = current user (SY-UNAME), Changed On = today (SY-DATUM)

✅ Positive

T09

Multiple addresses — independent data

Partner with 2 addresses (e.g., 0000000012) → enter different custom data for each address

Each address retains its own custom data independently

✅ Positive

T10

No data modification — no DB write

Open in Change mode, view custom tab but don't change anything → Save

No MODIFY statement executed (verify via SQL trace or breakpoint on DSAVE)

✅ Positive

T11

Person BP (Type 1) — tab visible

Open a Person BP (e.g., Lars Mueller, 0000000001) → check custom tab

Tab visible and functional

✅ Positive

T12

Organization BP (Type 2) — tab visible

Open an Organization BP (e.g., Global Business Properties, 0000000011) → check custom tab

Tab visible and functional

✅ Positive

### 13.2 Test Data

Partner

Name

Type

ADDRNUMBER

Use For

0000000001

Lars Mueller

Person

0000037157

Person BP with single address

0000000011

Global Business Properties

Organization

0000037175

Organization BP

0000000012

Food for You

Organization

0000037178, 0000037773

Multi-address BP (2 addresses)

0000000013

Andrea Lindsay Att. at Law

Organization

0000037774

Organization BP

14 S/4HANA Migration Considerations

#### ⚠️ Important: BDT is replaced by Key User Extensibility in S/4HANA

In S/4HANA, SAP recommends using the **Custom Fields & Logic** (Key User Extensibility) framework instead of BDT for adding fields to Business Partner. BDT still works on S/4HANA on-premise but is not the strategic direction.

### Migration Path

ECC Component

S/4HANA Equivalent

Migration Effort

ZBUT020\_EXT table

Carry forward as-is (Z-tables are compatible)

Low

BDT Function Group

Replace with Key User Extensibility or RAP extension

Medium

BDT Customizing (BUPT)

Recreate as Fiori extension (Custom Fields & Logic app)

Medium

BAdI implementations

Carry forward (BAdIs still supported in S/4HANA)

Low

Custom authorization

Migrate to IAM / Fiori launchpad role model

Low

### Recommendations for Future-Proofing

*   **Keep data model clean:** Using a separate Z-table (ZBUT020\_EXT) rather than CI includes makes migration simpler — the data model can be reused regardless of the UI framework.
*   **Document all BDT config:** Maintain a complete list of BUPT entries in transport documentation, as these will need manual recreation in S/4HANA if switching to extensibility framework.
*   **Isolate business logic from screen logic:** Keep validations in separate methods (not inline in PAI) so they can be reused in a RAP validation or Fiori handler post-migration.

15 Implementation Roadmap

gantt title BP Address BDT Enhancement — Implementation Roadmap dateFormat YYYY-MM-DD axisFormat %b %d section Phase 1: Foundation Create Domains (4) :a1, 2026-04-22, 1d Create Data Elements (4+2) :a2, after a1, 1d Create Table ZBUT020\_EXT :a3, after a2, 1d Create Message Class :a4, after a2, 1d section Phase 2: Function Group Create FUGR ZBUPA\_ADDR :b1, after a3, 1d Build Dynpro 0100 (Screen) :b2, after b1, 2d Implement 7 Core FMs :b3, after b2, 3d Implement 11 Stub FMs :b4, after b3, 1d section Phase 3: BDT Configuration Field Group + Screen config :c1, after b4, 1d Section + View assignment :c2, after c1, 1d Event registration :c3, after c2, 1d BP Category assignment :c4, after c3, 1d section Phase 4: Testing Unit Testing (T01-T12) :d1, after c4, 3d UAT with Business Users :d2, after d1, 3d Bug fixes + adjustments :d3, after d2, 2d section Phase 5: Optional BAdIs BUPA\_ADDR\_CHECK implementation :e1, after d3, 2d BUPA\_ADDR\_UPDATE implementation :e2, after e1, 2d

### Phase Summary

Phase

Duration

Objects Created

Dependencies

**Phase 1:** Foundation (DDIC)

~3 days

4 domains, 6 data elements, 1 table, 1 message class

None

**Phase 2:** Function Group

~7 days

1 function group, 1 dynpro, 18 function modules

Phase 1 complete

**Phase 3:** BDT Configuration

~4 days

Customizing entries in BUPT

Phase 2 complete

**Phase 4:** Testing

~8 days

Test scripts, bug fixes

Phase 3 complete

**Phase 5:** Optional BAdIs

~4 days

BAdI implementations

Phase 4 complete

#### ✅ Total Estimated Effort: ~22 working days (Phase 1-4) + 4 days optional (Phase 5)

This estimate is based on the existing BDT implementation pattern in the system (ZBUPA function group), which provides proven reference code. The implementation follows established conventions, significantly reducing development and testing time.

**C16 Technical Specification** — Generated 21 April 2026  
BP Address Enhancement via BDT • SAP ECC 7.40 • Client 800

**Data Sources Verified:**  
BUT000 (28,558 BPs) • BUT020 (9,071 addresses) • ADRC (12 key fields verified) • ZBUTHOBBY (reference table) • TFDIR (36 existing BDT FMs verified)

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._