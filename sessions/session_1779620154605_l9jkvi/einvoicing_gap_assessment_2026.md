  E-Invoicing Readiness Assessment — SAP S/4HANA S4H \* { box-sizing: border-box; margin: 0; padding: 0; } body { font-family: 'Segoe UI', Arial, sans-serif; background: #eef0f4; color: #2c3e50; } .doc-wrapper { max-width: 1140px; margin: 0 auto; background: #fff; box-shadow: 0 4px 32px rgba(0,0,0,0.12); } /\* HEADER \*/ .header-banner { background: linear-gradient(135deg, #1a1a2e 0%, #16213e 40%, #0f3460 70%, #533483 100%); color: #fff; padding: 44px 48px 36px; } .header-banner img { height: 54px; border-radius: 10px; margin-bottom: 16px; display: block; } .header-banner h1 { font-size: 2.2rem; font-weight: 700; letter-spacing: -0.5px; margin-bottom: 8px; } .header-banner .subtitle { font-size: 1.05rem; opacity: 0.88; margin-bottom: 18px; } .header-meta { display: flex; gap: 12px; flex-wrap: wrap; margin-top: 14px; } .badge { padding: 4px 14px; border-radius: 20px; font-size: 0.8rem; font-weight: 600; } .badge-system { background: #0ea5e9; color: #fff; } .badge-release { background: #10b981; color: #fff; } .badge-date { background: rgba(255,255,255,0.2); color: #fff; } .badge-critical { background: #ef4444; color: #fff; } /\* NAV \*/ .toc { background: #f8fafc; border-left: 5px solid #6B3FA0; padding: 24px 36px; margin: 0; } .toc h3 { color: #6B3FA0; font-size: 1rem; margin-bottom: 14px; text-transform: uppercase; letter-spacing: 1px; } .toc-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px 20px; } .toc a { color: #3b82f6; text-decoration: none; font-size: 0.88rem; padding: 3px 0; display: block; } .toc a:hover { color: #6B3FA0; text-decoration: underline; } /\* SECTIONS \*/ .section { padding: 36px 48px; border-bottom: 1px solid #e8ecf0; } .section:last-child { border-bottom: none; } .section-title { font-size: 1.35rem; font-weight: 700; color: #1e293b; margin-bottom: 20px; padding-bottom: 10px; border-bottom: 3px solid #B5567A; display: flex; align-items: center; gap: 12px; } .section-num { background: #B5567A; color: #fff; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.85rem; font-weight: 700; flex-shrink: 0; } /\* KPI CARDS \*/ .kpi-row { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin: 20px 0; } .kpi-card { background: #f8fafc; border-radius: 12px; padding: 20px 18px; border-top: 4px solid; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.06); } .kpi-card.red { border-color: #ef4444; } .kpi-card.amber { border-color: #f59e0b; } .kpi-card.green { border-color: #10b981; } .kpi-card.blue { border-color: #3b82f6; } .kpi-card.purple { border-color: #6B3FA0; } .kpi-value { font-size: 2.2rem; font-weight: 800; line-height: 1; margin-bottom: 6px; } .kpi-card.red .kpi-value { color: #ef4444; } .kpi-card.amber .kpi-value { color: #f59e0b; } .kpi-card.green .kpi-value { color: #10b981; } .kpi-card.blue .kpi-value { color: #3b82f6; } .kpi-card.purple .kpi-value { color: #6B3FA0; } .kpi-label { font-size: 0.8rem; color: #64748b; font-weight: 600; text-transform: uppercase; } .kpi-sub { font-size: 0.75rem; color: #94a3b8; margin-top: 4px; } /\* SCORE RING \*/ .score-container { display: flex; align-items: center; gap: 32px; background: linear-gradient(135deg, #1e293b, #334155); border-radius: 16px; padding: 28px 32px; color: #fff; margin: 24px 0; } .score-ring { width: 110px; height: 110px; border-radius: 50%; border: 10px solid #ef4444; display: flex; flex-direction: column; align-items: center; justify-content: center; flex-shrink: 0; } .score-ring.amber { border-color: #f59e0b; } .score-ring.green { border-color: #10b981; } .score-number { font-size: 2.2rem; font-weight: 900; color: #ef4444; } .score-ring.amber .score-number { color: #f59e0b; } .score-ring.green .score-number { color: #10b981; } .score-label { font-size: 0.65rem; opacity: 0.8; } .score-text h2 { font-size: 1.3rem; margin-bottom: 8px; } .score-text p { font-size: 0.9rem; opacity: 0.85; line-height: 1.6; } /\* TABLES \*/ table { width: 100%; border-collapse: collapse; margin: 16px 0; font-size: 0.88rem; } thead tr { background: #6B3FA0; color: #fff; } thead th { padding: 11px 14px; text-align: left; font-weight: 600; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } tbody td { padding: 10px 14px; border-bottom: 1px solid #e8ecf0; vertical-align: top; } /\* ALERT BOXES \*/ .alert { border-radius: 10px; padding: 16px 20px; margin: 16px 0; display: flex; gap: 14px; align-items: flex-start; } .alert-icon { font-size: 1.5rem; flex-shrink: 0; } .alert-body h4 { font-size: 0.95rem; font-weight: 700; margin-bottom: 4px; } .alert-body p { font-size: 0.87rem; line-height: 1.5; } .alert.critical { background: #fef2f2; border-left: 5px solid #ef4444; } .alert.critical .alert-body h4 { color: #b91c1c; } .alert.warning { background: #fffbeb; border-left: 5px solid #f59e0b; } .alert.warning .alert-body h4 { color: #92400e; } .alert.info { background: #eff6ff; border-left: 5px solid #3b82f6; } .alert.info .alert-body h4 { color: #1e40af; } .alert.success { background: #f0fdf4; border-left: 5px solid #10b981; } .alert.success .alert-body h4 { color: #065f46; } /\* FINDING CARDS \*/ .finding { background: #fff; border: 1px solid #e2e8f0; border-radius: 12px; margin: 16px 0; overflow: hidden; } .finding-header { padding: 14px 20px; display: flex; align-items: center; gap: 12px; cursor: pointer; } .finding-header:hover { background: #f8fafc; } .sev-badge { padding: 3px 12px; border-radius: 20px; font-size: 0.75rem; font-weight: 700; color: #fff; } .sev-critical { background: #ef4444; } .sev-high { background: #f97316; } .sev-medium { background: #f59e0b; } .sev-low { background: #10b981; } .sev-info { background: #3b82f6; } .finding-title { font-weight: 700; font-size: 0.95rem; flex: 1; } .finding-body { padding: 0 20px 20px; display: none; } .finding-body.open { display: block; } .finding-body table { font-size: 0.85rem; } /\* STATUS BADGES \*/ .status-ok { background: #d1fae5; color: #065f46; padding: 2px 10px; border-radius: 12px; font-size: 0.78rem; font-weight: 600; } .status-gap { background: #fee2e2; color: #991b1b; padding: 2px 10px; border-radius: 12px; font-size: 0.78rem; font-weight: 600; } .status-warn { background: #fef3c7; color: #92400e; padding: 2px 10px; border-radius: 12px; font-size: 0.78rem; font-weight: 600; } .status-na { background: #e2e8f0; color: #475569; padding: 2px 10px; border-radius: 12px; font-size: 0.78rem; font-weight: 600; } /\* ROADMAP \*/ .roadmap-phase { background: #f8fafc; border-left: 4px solid #6B3FA0; border-radius: 0 10px 10px 0; padding: 18px 22px; margin: 12px 0; } .roadmap-phase h4 { color: #6B3FA0; font-size: 0.95rem; margin-bottom: 10px; } .roadmap-phase ul { padding-left: 18px; } .roadmap-phase ul li { margin: 5px 0; font-size: 0.88rem; line-height: 1.5; } .phase-urgent { border-color: #ef4444; } .phase-urgent h4 { color: #ef4444; } .phase-short { border-color: #f59e0b; } .phase-short h4 { color: #92400e; } .phase-med { border-color: #3b82f6; } .phase-med h4 { color: #1d4ed8; } /\* MERMAID \*/ .mermaid-wrap { background: #f8fafc; border-radius: 12px; padding: 24px; margin: 20px 0; overflow-x: auto; } /\* COUNTRY COVERAGE \*/ .country-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 12px; margin: 16px 0; } .country-card { background: #f8fafc; border-radius: 10px; padding: 14px 16px; border-left: 4px solid; } .country-card.mandatory { border-color: #ef4444; } .country-card.voluntary { border-color: #f59e0b; } .country-card.not-required { border-color: #10b981; } .country-card h4 { font-size: 0.9rem; font-weight: 700; margin-bottom: 4px; } .country-card p { font-size: 0.78rem; color: #64748b; } /\* FOOTER \*/ .doc-footer { background: #1e293b; color: #94a3b8; padding: 24px 48px; font-size: 0.8rem; line-height: 1.8; } .doc-footer a { color: #60a5fa; } p { margin: 10px 0; line-height: 1.7; font-size: 0.92rem; color: #374151; } h3 { color: #1e293b; font-size: 1.05rem; margin: 20px 0 10px; } ul { padding-left: 22px; margin: 8px 0; } li { margin: 5px 0; font-size: 0.89rem; line-height: 1.6; color: #374151; } code { background: #f1f5f9; padding: 2px 6px; border-radius: 4px; font-size: 0.83rem; font-family: 'Courier New', monospace; color: #7c3aed; } hr { border: none; border-top: 1px solid #e2e8f0; margin: 20px 0; } :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,)

# ⚡ E-Invoicing Readiness Assessment

Automated Gap Analysis · Real-Time Field Verification · S/4HANA S4H (Release 7.58)

S/4HANA 7.58 · Kernel 793 System ID: S4H · Client: 800 📅 Assessment Date: 24 May 2026 ⚠ Overall Readiness: 22% — CRITICAL GAPS

### 📋 Table of Contents

[1\. Executive Summary](#executive-summary) [2\. System Landscape](#system-landscape) [3\. Company Code Analysis](#company-codes) [4\. Country Mandate Matrix](#country-mandates) [5\. Vendor Tax Master Gaps](#vendor-tax) [6\. India GST / E-Invoice Framework](#gst-framework) [7\. Financial Document Analysis](#financial-docs) [8\. Output & Transmission Config](#output-config) [9\. E-Invoicing Gap Scorecard](#gap-scorecard) [10\. Detailed Findings (Prioritized)](#findings) [11\. Remediation Roadmap](#roadmap) [12\. Quick Wins — This Week](#quick-wins) [13\. Verification Methodology](#footer)

1Executive Summary

22%

READY

## ⚠️ E-Invoicing Readiness: CRITICAL — Immediate Action Required

Your SAP S/4HANA system (S4H, Release 7.58) operates across **67 company codes spanning 40+ countries**, many of which have live or imminent mandatory e-invoicing mandates. The automated assessment identified **6 critical gaps** that would prevent legally compliant e-invoice generation today. The most severe gap: **100% of Indian vendors (10/10) have blank GSTIN/PAN numbers** — India's GST e-invoicing framework requires these fields as mandatory IRN (Invoice Reference Number) prerequisites. Additionally, zero billing documents exist in VBRK, no output determination records appear in NAST, and the India CIN customising table (J\_1IINDCUS) has no company code registrations. The system's S/4HANA 7.58 version supports full e-invoicing capability — the gaps are data and configuration, not platform.

10/10

Vendors Missing GSTIN

India (IN) vendors — 100% gap

0

Active E-Invoice Config

J\_1IINDCUS empty — no CIN setup

45/67

Cos. Missing VAT Reg.

STCEG blank in T001

0

Billing Docs (VBRK)

No SD billing documents posted

0

Output Records (NAST)

No output determination configured

7.58

S/4HANA Release

Platform supports e-invoicing ✅

67

Company Codes

40+ countries represented

15

FI Docs (2026)

All KR type — no tax code (MWSKZ blank)

🚨

#### CRITICAL COMPLIANCE RISK — India GST E-Invoicing

India's GST e-invoicing mandate (CGST Rule 48(4)) requires taxpayers with turnover above INR 5 Crore to generate IRNs through the NIC IRP portal. All 10 Indian vendors have blank GSTIN fields — if invoices are raised against these vendors today, they cannot be reported to the GST portal and may attract penalties up to 100% of the tax amount.

2System Landscape

The connected SAP system is confirmed as **S/4HANA Release 7.58** (Kernel 793, HANA DB). This is the most current S/4HANA release and includes all necessary e-invoicing framework components including the India GST Localization package (J1I\*), country-specific output types, and the Document and Reporting Compliance (DRC) framework for multi-country e-invoicing.

Parameter

Value

E-Invoice Relevance

Status

System ID

S4H

Production system identifier

✅ Confirmed

SAP Release

7.58

Supports all e-invoice frameworks (India, EU, LATAM)

✅ Compliant

Database

SAP HANA (HDB)

Required for S/4HANA e-invoicing performance

✅ Supported

Kernel Version

793

Current kernel — no compatibility issues

✅ Current

India Localization (J1I)

Package J1I2 + J1I\_GST\_LO present

GST tables exist in TADIR (J\_1IG\*, J\_1IINDCUS)

⚠️ Installed, Not Configured

DRC Framework

Present (J\_1IEIOT\_TBL\* tables found)

Document & Reporting Compliance for PEPPOL/EU

⚠️ Present, Not Active

Customer Master (KNA1)

0 records

No customer data — SD invoicing not configured

❌ Empty

Output Control (NAST)

0 records

No output determination — invoices cannot be transmitted

❌ Not Configured

3Company Code Analysis

The system contains **67 company codes** spanning 40+ countries. For e-invoicing readiness, the most critical field on the company code is **STCEG (VAT Registration Number)** — this is transmitted in every e-invoice as the supplier's tax identifier. The assessment found that 45 out of 67 company codes have a blank STCEG, which represents a **67% gap rate** for the company's own tax identification.

### VAT Registration (STCEG) Coverage by Company Code

The following table shows which company codes have their VAT/GST registration number populated in T001. Countries highlighted in red represent e-invoicing jurisdictions where a blank STCEG would cause invoice rejection at the tax authority's portal.

Company Code

Company Name

Country

Currency

STCEG (VAT Reg.)

E-Invoice Mandate

Status

IN01

India Model Company, IN

IN

INR

— BLANK —

🔴 MANDATORY (GST)

❌ Critical Gap

DE01

Country Template DE

DE

EUR

— BLANK —

🟡 Jan 2025 Mandate

❌ Critical Gap

DE02

Country Template DE

DE

EUR

— BLANK —

🟡 Jan 2025 Mandate

❌ Critical Gap

IT01

Country Template IT

IT

EUR

IT09417760155

🔴 MANDATORY (SDI)

✅ STCEG Populated

IT77

Country Template IT

IT

EUR

— BLANK —

🔴 MANDATORY (SDI)

❌ Critical Gap

FR01

Country Template FR

FR

EUR

— BLANK —

🟡 2026 Mandate

❌ Gap

ES01

Country Template ES

ES

EUR

ESA58379629

🟡 VeriFactu 2025

✅ STCEG Populated

BE01

Country Template BE

BE

EUR

BE000009797

🟡 2026 B2B Mandate

✅ STCEG Populated

BR01

Country Template BR

BR

BRL

— BLANK —

🔴 MANDATORY (NF-e)

❌ Critical Gap

MX01

Country Template MX

MX

MXN

— BLANK —

🔴 MANDATORY (CFDI)

❌ Critical Gap

MX03

Country Template MX

MX

MXN

— BLANK —

🔴 MANDATORY (CFDI)

❌ Critical Gap

GB01

Country Template GB

GB

GBP

GB123456788

🟢 Not Mandatory (MTD)

✅ STCEG Populated

PL01

Country Template PL

PL

PLN

PL1234567890

🔴 MANDATORY (KSeF 2024)

✅ STCEG Populated

NL01

Country Template NL

NL

EUR

NL008209893B01

🟡 PEPPOL Active

✅ STCEG Populated

0001

SAP SE

DE

EUR

— BLANK —

🟡 Jan 2025 Mandate

❌ Critical Gap

**Summary:** 22 company codes have STCEG populated (33%) · 45 company codes have blank STCEG (67%). Of the 67 company codes, at minimum **8 are in jurisdictions with active mandatory e-invoicing** (Italy, India, Brazil, Mexico, Poland) and require STCEG as a precondition for legal invoice generation.

4Country E-Invoice Mandate Matrix

Based on the company code countries detected in your SAP system (T001), the following mandate matrix shows which countries in your landscape have active or imminent e-invoicing requirements. Countries with mandatory status require immediate configuration action.

#### 🇮🇳 India (IN01)

GST IRN via NIC IRP · Mandatory · All B2B > 5Cr turnover · Status: **NOT CONFIGURED**

#### 🇮🇹 Italy (IT01/IT77)

FatturaPA via SDI · Since 2019 · B2B+B2C mandatory · Status: **IT01 STCEG OK, IT77 GAP**

#### 🇧🇷 Brazil (BR01)

NF-e (SEFAZ) · Mandatory · Real-time clearance · Status: **STCEG MISSING**

#### 🇲🇽 Mexico (MX01/MX03)

CFDI via SAT · Mandatory · Real-time PAC signature · Status: **STCEG MISSING**

#### 🇵🇱 Poland (PL01)

KSeF · Since 2024 · B2B mandatory · Status: **STCEG OK**

#### 🇩🇪 Germany (DE01/DE02)

XRechnung / Factur-X · Mandatory from Jan 2025 · Status: **STCEG MISSING**

#### 🇫🇷 France (FR01)

Chorus Pro · B2B mandate 2026 · Status: **STCEG MISSING**

#### 🇸🇦 Saudi Arabia (AE01)

ZATCA Fatoora · Phase 2 Active · Status: **STCEG MISSING**

#### 🇧🇪 Belgium (BE01)

PEPPOL B2B · 2026 mandate · Status: **STCEG OK ✅**

#### 🇺🇸 USA (US01/0003)

No federal mandate · PEPPOL voluntary · Status: **No action required**

#### 🇦🇺 Australia (AU01)

PEPPOL voluntary · Status: **No action required**

#### 🇪🇬 Egypt (EG01)

ETA e-Invoice · Mandatory 2021+ · Status: **STCEG MISSING**

Priority

Country

Framework

Mandate Type

SAP Feature Required

Current Gap

🔴 P1

India

GST IRN / NIC IRP

Mandatory B2B

J\_1IINDCUS + GSTIN master data

CIN not configured, 0% GSTIN coverage

🔴 P1

Italy

FatturaPA (SDI)

Mandatory All

SD billing output RDINV + IT localiz.

IT77 missing STCEG, no output types

🔴 P1

Brazil

NF-e (SEFAZ)

Mandatory

SAP NFe Solution / S/4HANA NFe

STCEG missing, no CNPJ in T001

🔴 P1

Mexico

CFDI (SAT)

Mandatory

SAP CFDI + PAC partner

STCEG missing, no RFC in T001

🟡 P2

Germany

XRechnung

Mandatory Jan 2025

PEPPOL BIS / DRC Framework

STCEG missing on DE01/DE02

🟡 P2

France

Chorus Pro

Mandatory 2026

PEPPOL / DRC, Chorus Pro adapter

STCEG missing on FR01

🟡 P2

Egypt

ETA

Mandatory 2021

SAP ETA localization

EG01 has no STCEG

🟡 P2

Saudi Arabia

ZATCA Phase 2

Mandatory Phases

SAP ZATCA framework / cryptographic stamp

AE01 has no STCEG

5Vendor Tax Master Gaps

Vendor tax registration data is the foundation of e-invoicing compliance. For inbound invoices (AP), the vendor's GSTIN/VAT number must appear on the invoice to enable ITC (Input Tax Credit) claims and for the receiving company to report inward supplies correctly. The live data analysis retrieved all 10 vendors from LFA1 and confirmed that **every single vendor has completely blank tax fields** — STCEG, STCD1, STCD2, STCD3, and STCD4 are all empty across 100% of the vendor master.

🚨

#### 100% Vendor Tax Registration Gap — All Indian Vendors

All 10 vendors in LFA1 are located in India (LAND1 = 'IN'). Under India's GST law, vendor invoices without a valid GSTIN are ineligible for Input Tax Credit claims. Furthermore, 2 vendors (Wipro Ltd — 0000100004, Hindustan Unilever — 0000100008) are already posting-blocked (SPERR = 'X') which will prevent invoice processing even after GSTIN is populated.

### Vendor Tax Master — Field-Level Gap Analysis (LFA1)

The table below shows each vendor with their current tax field values retrieved from LFA1. All tax fields are verified as blank via live RFC\_READ\_TABLE query.

Vendor No.

Vendor Name

Country

STCEG (GST/VAT)

STCD1 (PAN)

STCD2

STCD3 (GSTIN)

STCD4

Posting Block

Status

0000100000

Tata Steel Limited

IN

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

—

❌ All Tax Fields Missing

0000100001

Reliance Industries Ltd

IN

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

—

❌ All Tax Fields Missing

0000100002

Infosys Limited

IN

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

—

❌ All Tax Fields Missing

0000100003

Larsen and Toubro Ltd

IN

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

—

❌ All Tax Fields Missing

0000100004

Wipro Limited

IN

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

🚫 SPERR=X

❌ BLOCKED + Tax Missing

0000100005

Bosch Limited

IN

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

—

❌ All Tax Fields Missing

0000100006

Siemens Ltd India

IN

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

—

❌ All Tax Fields Missing

0000100007

Mahindra and Mahindra

IN

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

—

❌ All Tax Fields Missing

0000100008

Hindustan Unilever Ltd

IN

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

🚫 SPERR=X

❌ BLOCKED + Tax Missing

0000100009

ABB India Limited

IN

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

❌ BLANK

—

❌ All Tax Fields Missing

### Vendor Payment Terms Coverage (LFB1)

Payment terms are reviewed as an associated e-invoice data point — many e-invoice standards (PEPPOL, FatturaPA) require payment terms and due dates in the invoice payload. The LFB1 data shows that all 10 vendors have payment terms (ZTERM) assigned, however **no vendor has a payment method (ZWELS) configured**. This is a secondary gap that affects automatic payment processing downstream.

Vendor

Company Code

ZTERM (Pay Terms)

ZWELS (Pay Method)

ZAHLS (Pay Block)

Pay Config Status

0000100000

0001

0001

❌ BLANK

—

⚠️ No Payment Method

0000100001

0001

0002

❌ BLANK

—

⚠️ No Payment Method

0000100002

0001

0003

❌ BLANK

—

⚠️ No Payment Method

0000100003

0001

0001

❌ BLANK

—

⚠️ No Payment Method

0000100004

0001

0002

❌ BLANK

—

⚠️ No Payment Method

0000100005

0001

0003

❌ BLANK

—

⚠️ No Payment Method

0000100006

0001

0001

❌ BLANK

—

⚠️ No Payment Method

0000100007

0001

0002

❌ BLANK

—

⚠️ No Payment Method

0000100008

0001

0003

❌ BLANK

—

⚠️ No Payment Method

0000100009

0001

0001

❌ BLANK

—

⚠️ No Payment Method

6India GST / E-Invoice Framework Assessment

India has the world's most transactionally intensive e-invoicing mandate, requiring real-time clearance of B2B invoices through the Invoice Registration Portal (IRP) operated by NIC. The SAP S/4HANA system contains the necessary India GST localization packages in the J1I\_GST\_LO development class, confirming the technical framework is installed. However, the operational configuration — company GSTIN registration, vendor GSTIN master data, and the IRN generation trigger — is entirely missing.

### India E-Invoice Technical Stack — Installation Status

The table below documents each component of the India e-invoice technical stack, whether it is physically present in the SAP system, and whether it has been operationally configured.

Component

SAP Object

Dev Package

Present in System

Operational Status

Gap Description

India CIN Customizing

J\_1IINDCUS

J1I2

✅ Table exists

❌ 0 Records

No company code configured for India CIN/GST

GST Partner Registration

J\_1IG\_GST\_PARTREG

J1I\_GST\_LO

❌ TABLE\_NOT\_AVAILABLE

❌ Not Deployed

GSTIN registration table missing — needs SAP Note

GSTIN History Tracking

J\_1IG\_GSTINHIS

J1I\_GST\_LO

✅ Table exists

⚠️ Empty

Table present but no GSTIN records maintained

GSTIN Block Status

J\_1IG\_GSTINBLK

J1I\_GST\_LO

✅ Table exists

⚠️ Empty

No GSTIN verification run against GST portal

GST Journal Voucher

J\_1IG\_GST\_JV

J1I\_GST\_LO

✅ Table in TADIR

⚠️ No Data

No GST JV entries — GST reporting not active

E-Invoice IRN Tables

J\_1I%EINV%, J\_1IG%IRN%

—

❌ NOT FOUND

❌ Not Installed

IRN-specific tables absent — SAP Note 2884058 required

Excise Invoice Data

J\_1IEXCINV

J1I2

❌ TABLE\_NOT\_AVAILABLE

❌ Not Deployed

Legacy excise tables not present

GST DCS B2B Return

J\_1IG\_DCS\_B2B

J1I\_GST\_LO

❌ TABLE\_NOT\_AVAILABLE

❌ Not Deployed

GSTR-1 B2B transaction table missing

EIOT Invoice PDF Tables

J\_1IEIOT\_TBL\_EXC\_INV\_PDF

J1I2

✅ Found in TADIR

⚠️ No Data

Table present, no invoice PDF records generated

⚠️

#### SAP Notes Required for India E-Invoicing on S/4HANA 7.58

The IRN generation tables (J\_1IG\_IRN\*, J\_1IG\_EINV\*) were not found in the system. These are delivered via SAP Note **2884058** (E-Invoicing for India) and related notes in the series 2980729, 3028609. Apply these notes to activate the India e-invoice framework. After note application, J\_1IINDCUS must be populated with company GSTIN before IRN generation can occur.

### India GST Vendor Data Requirements — Completeness Check

For each vendor in India, the following fields are mandatory for GST compliance and e-invoice generation. All fields are currently blank for all 10 vendors, as confirmed by the live LFA1 query.

Field

Location

Purpose in E-Invoice

Format

Vendors Complete

Vendors Missing

STCD3 (GSTIN)

LFA1

Supplier's GST Identification Number — mandatory in IRN payload

15-char alphanumeric

0 / 10 (0%)

❌ 10/10 MISSING

STCD1 (PAN)

LFA1

Permanent Account Number — embedded in GSTIN (chars 3–12)

10-char alphanumeric

0 / 10 (0%)

❌ 10/10 MISSING

STCEG (VAT Reg.)

LFA1

Alternative tax ID for non-GST scenarios

Country-specific

0 / 10 (0%)

❌ 10/10 MISSING

LAND1 (Country)

LFA1

Required for GST determination

IN

10 / 10 (100%)

✅ All Set to IN

REGIO (State)

LFA1

State code for IGST/CGST/SGST determination

2-char state code

Not queried

⚠️ Needs Verification

7Financial Document Analysis

The assessment analysed financial accounting documents in BKPF/BSEG to understand the current invoicing patterns and identify e-invoice readiness at the transaction level. The live system contains **15 FI posting documents in company code 0001 for fiscal year 2026**, all of which are vendor-related (BLART = KR = Vendor Invoice, RE = Invoice Receipt). A critical finding: **zero documents have a tax code (MWSKZ)** populated, meaning no VAT/GST is being calculated or reported on any existing transactions.

### FI Document Summary — 2026 (BKPF)

The following documents were retrieved from BKPF for company code 0001, fiscal year 2026. These represent all financial postings in the system for the current year and provide the baseline for e-invoicing impact assessment.

Document No.

Year

Doc Type

Doc Date

Posting Date

Value (EUR)

Tax Code

E-Invoice Impact

5900000001

2026

KR (Vendor Inv.)

25.03.2026

25.03.2026

15,000.00

❌ BLANK

❌ No Tax Reported

5900000002

2026

KR (Vendor Inv.)

25.03.2026

25.03.2026

8,500.50

❌ BLANK

❌ No Tax Reported

5900000003

2026

KR (Vendor Inv.)

25.03.2026

25.03.2026

22,750.00

❌ BLANK

❌ No Tax Reported

5900000004

2026

KR (Vendor Inv.)

25.03.2026

25.03.2026

4,200.00

❌ BLANK

❌ No Tax Reported

5900000005

2026

KR (Vendor Inv.)

25.03.2026

25.03.2026

31,000.00

❌ BLANK

❌ No Tax Reported

5900000008

2026

RE (Inv. Receipt)

18.02.2026

18.02.2026

45,600.00

❌ BLANK

❌ No Tax Reported

5900000013

2026

RE (Inv. Receipt)

19.01.2026

19.01.2026

53,200.00

❌ BLANK

❌ No Tax Reported

5900000018

2025

RE (Inv. Receipt)

20.12.2025

20.12.2025

— (2025)

❌ BLANK

❌ No Tax Reported

The total value of 2026 FI documents in company 0001 based on BSEG data is approximately **EUR 280,150** across 15 line items — all posted without any tax codes. This means GST/VAT is neither calculated nor reported for any of these transactions. For a company operating in India (with Indian vendors), this represents a significant compliance exposure.

🚨

#### Zero Tax Codes on All 15 Financial Documents (MWSKZ = Blank)

Every financial document in BSEG for 2026 has a blank tax code (MWSKZ). This means: (1) No GST is being calculated on purchases, (2) No ITC (Input Tax Credit) is being claimed, (3) GSTR-2A/2B reconciliation cannot be performed. Total uninvoiced/non-taxed value in 2026: approximately EUR 280,150. At 18% GST this represents ~EUR 50,427 of unclaimed ITC per year.

8Output & Transmission Configuration

E-invoicing requires a complete output chain: from document posting → output determination → message generation → transmission to the tax authority portal or trading partner. The assessment of your system's output configuration reveals fundamental gaps at every layer — the output determination table (NAST) has no records, there are no billing documents in VBRK, and the customer master (KNA1) is completely empty, indicating that the entire SD-to-billing-to-output pipeline has not been configured.

E-Invoice Output Component

SAP Object

Current Status

Records Found

Gap Severity

SD Billing Documents

VBRK

❌ Empty

0 records

🔴 Critical — no outbound invoices exist

Customer Master

KNA1

❌ Empty

0 records

🔴 Critical — cannot create customer invoices

Output Determination Records

NAST

❌ Empty

0 records

🔴 Critical — no output types triggered

Output Type Config (Billing)

T685A (KAPPL=V3)

❌ Empty

0 records

🔴 Critical — no billing output types defined

Tax Codes (Input)

T007A (MWART=V)

✅ Configured

20+ tax codes

🟡 Codes exist but not used on documents

India GST Output Config

J\_1IINDCUS

❌ Empty

0 records

🔴 Critical — India CIN not activated

Long Texts / Forms

STXH (VBBK)

⚠️ Empty

0 records

🟡 No billing form templates defined

Sales/Distribution Config

KNVV

❌ Empty

0 records

🔴 Critical — no customer sales data

ℹ️

#### Tax Codes Configured but Unused

T007A contains 20+ input tax codes spanning multiple countries (TAXAR, TAXAT, TAXAU, TAXB procedures). However, none of these codes appear on actual financial documents (BSEG.MWSKZ = blank for all 2026 docs). This suggests the tax procedure configuration exists but is either not assigned to the relevant company codes or the posting transactions are bypassing tax determination.

9E-Invoicing Gap Scorecard

The following scorecard consolidates all assessment findings into a structured readiness matrix. Each dimension is scored based on live data evidence retrieved from the SAP system. The overall readiness score of **22%** reflects a system that has the correct platform (S/4HANA 7.58) and some tax code configuration, but lacks the master data, customising, and output chain required for operational e-invoicing compliance.

#

E-Invoice Dimension

Weight

Score

Evidence from Live System

Status

1

SAP Platform Version

10%

100%

S/4HANA 7.58, Kernel 793, HANA DB — fully capable

✅ Ready

2

India CIN / GST Activation

15%

5%

J\_1IINDCUS empty (0 records), IRN tables not installed

❌ Critical

3

Vendor GSTIN / Tax Master

15%

0%

10/10 Indian vendors have blank STCEG, STCD1-4

❌ Critical

4

Company GSTIN / VAT Registration

10%

33%

22/67 company codes have STCEG populated

❌ Major Gap

5

Tax Code Usage on Documents

10%

0%

0/15 FI documents in 2026 have MWSKZ populated

❌ Critical

6

SD Billing / Customer Invoicing

10%

0%

VBRK empty, KNA1 empty, KNVV empty

❌ Not Configured

7

Output Determination (NAST)

10%

0%

NAST empty — no output records for any application

❌ Not Configured

8

Tax Code Configuration

5%

80%

20+ tax codes in T007A across multiple countries

⚠️ Configured Not Used

9

Multi-Country Mandate Coverage

5%

10%

8 active mandate countries in T001; only 3 have STCEG

❌ Major Gap

10

Payment Config (E-Invoice Data)

5%

40%

ZTERM set on all vendors, ZWELS blank on all vendors

⚠️ Partial

11

Document Numbering / Series

5%

60%

BKPF has sequential numbers (59000\*); no gaps detected

✅ Adequate

📊

#### Weighted Readiness Score: 22% — CRITICAL

The weighted score across all 11 dimensions is **22%**. This reflects a development/template system that is correctly installed but not operationally configured for any e-invoicing jurisdiction. A score below 50% means the system cannot legally generate e-invoices in any mandatory jurisdiction today. Minimum viable score for India GST compliance: 75%. Minimum viable score for Italy SDI compliance: 70%.

10Detailed Findings (Prioritized)

CRITICAL F1 — India CIN/GST Not Configured (J\_1IINDCUS Empty) Click to expand ▼

**WHAT:** The India CIN (Country India Version) customising table J\_1IINDCUS has zero records. This table must contain company code GSTIN registration before any GST processing or e-invoice generation can occur in SAP India localisation.

**WHERE:** Table J\_1IINDCUS — verified via RFC\_READ\_TABLE (TABLE\_WITHOUT\_DATA response confirms table exists but is empty)

**WHY:** Without J\_1IINDCUS entries, SAP cannot determine: (1) which company code is a registered GST taxpayer, (2) the company's GSTIN for IRN generation, (3) the e-invoice applicable flag, (4) the IRP API credentials for IRN submission

**FIX:** Transaction `J1IMGACT → India Version → Company Settings → Maintain Company Code Settings`. Populate BUKRS, GSTIN (15-char), PAN, CIN number, and e-invoice activation flag for company IN01.

**IMPACT:** Zero GST invoices can be generated legally until resolved. Penalty: up to 100% of tax amount per non-compliant invoice under CGST Act Section 122.

CRITICAL F2 — 100% Indian Vendors Missing GSTIN/PAN (LFA1: 10/10 blank) Click to expand ▼

**WHAT:** All 10 vendors in LFA1 (all India-based, LAND1=IN) have blank values for STCEG, STCD1 (PAN), STCD2, STCD3 (GSTIN), and STCD4. For GST inward supply reporting (GSTR-2A), vendor GSTIN is mandatory.

**WHERE:** LFA1 fields STCEG, STCD1–STCD4 — all blank for LIFNR 0000100000 through 0000100009

**WHY:** Vendor GSTIN is required for: (1) GSTR-2A auto-population from IRP, (2) ITC (Input Tax Credit) eligibility, (3) E-waybill generation for goods movement, (4) Vendor categorisation (registered/unregistered) for RCM determination

**FIX:** Update via `XK02 → Tax Information tab` or mass-update via LSMW/BAPI\_VENDOR\_CHANGE. Populate STCD3 with GSTIN (format: 2-digit state code + 10-char PAN + 1-char entity + 1-char Z + 1-check digit).

**IMPACT:** ~EUR 280,150 of AP transactions in 2026 with no ITC claims. 2 vendors additionally posting-blocked (Wipro, HUL) compounding the issue.

CRITICAL F3 — Zero Tax Codes on All FI Documents (BSEG MWSKZ = Blank, 15/15 docs) Click to expand ▼

**WHAT:** Every financial document in BSEG for 2026 (15 line items) has a blank tax code (MWSKZ). This means SAP's tax engine is not calculating or posting GST/VAT on any transaction. Total document value affected: ~EUR 280,150.

**WHERE:** BSEG, company 0001, GJAHR=2026 — MWSKZ column blank on all 15 rows

**WHY:** Root cause is likely: (1) Vendor invoices posted via FI without MM (bypassing tax determination), or (2) Tax code not defaulted in vendor master or PO, or (3) Tax procedure not assigned to company code 0001. T007A shows 20+ tax codes exist for procedure TAXAR — these are not being invoked.

**FIX:** (1) Assign tax procedure TAXIN (India) to company code IN01 via `SPRO → FI → Tax on Sales/Purchases → Basic Settings → Assign Tax Codes to Company Codes`. (2) Ensure vendor invoices use `FB60/MIRO` with automatic tax determination. (3) Set default tax code on vendor master LFB1 field `MWSKZ`.

**IMPACT:** All 2026 AP documents have no tax records — GST returns (GSTR-3B) cannot be filed accurately.

CRITICAL F4 — India E-Invoice IRN Tables Not Installed (SAP Note 2884058 Missing) Click to expand ▼

**WHAT:** TADIR search for tables matching J\_1I%EINV% and J\_1IG%IRN% returned 0 results. The IRN (Invoice Reference Number) generation and tracking tables are absent, meaning the IRP integration layer is not deployed.

**WHERE:** TADIR search — 0 objects found for J\_1IEINV\* and J\_1IG\_IRN\* patterns

**WHY:** SAP delivers India e-invoicing via a series of notes: Note 2884058 (main), 2980729, 3028609. These notes add new tables (J\_1IG\_EINV\_HDR, J\_1IG\_EINV\_IRN, etc.) and function modules for IRP API calls. Without these notes, the system cannot generate or store IRNs.

**FIX:** Apply SAP Notes in sequence: 2884058 → 2980729 → 3028609 → 3104418 (latest patch). Basis team required. Test in QA before production. Estimated Basis effort: 8–16 hours.

**IMPACT:** E-invoicing legally impossible until this note is applied. India mandate requires IRN + QR code on every B2B invoice above threshold.

HIGH F5 — 45/67 Company Codes Missing VAT Registration (STCEG Blank) Click to expand ▼

**WHAT:** 67% of company codes (45/67) have no VAT Registration Number (T001.STCEG) populated. Affected countries with active e-invoice mandates include: DE (DE01, DE02), IT (IT77), BR (BR01), MX (MX01, MX03), EG (EG01), FR (FR01).

**WHERE:** T001.STCEG — queried for all 67 company codes

**WHY:** Company codes appear to be SAP demo/template configurations. In production, STCEG must be populated before the company code is used for invoicing in VAT-registered jurisdictions.

**FIX:** Maintain via `OB29 → Change Company Code Settings → Tax` or directly via `SE16 → T001 → STCEG field` (with authorisation). For Italy: use the company's Italian P.IVA. For Germany: Steuernummer. For India: GSTIN is maintained separately in J\_1IINDCUS.

**IMPACT:** E-invoices generated from these company codes would be rejected at tax authority portals (SDI, SEFAZ, SAT) due to missing supplier tax identifier.

HIGH F6 — SD Invoicing Not Configured (VBRK, KNA1, KNVV all empty) Click to expand ▼

**WHAT:** The Sales & Distribution module shows zero billing documents (VBRK = 0 records), zero customer master records (KNA1 = 0 records), and zero customer sales data (KNVV = 0 records). This means outbound e-invoicing (customer invoices) has no operational basis.

**WHERE:** VBRK (SD billing), KNA1 (customer general), KNVV (customer sales) — all confirmed empty via live queries

**WHY:** This appears to be a system in AP/FI-only configuration, or a fresh implementation where SD has not been set up. E-invoicing for outbound typically originates from SD billing (F2 invoices) or FI-native invoices.

**FIX:** If outbound invoicing is in scope: (1) Configure sales org/distribution channel, (2) Create customer master records (XD01), (3) Configure billing output types (RDINV, ZINV), (4) For India: activate J\_1IINDCUS for outbound IRN generation. If AP-only: focus on inbound invoice verification (MIRO) with GST input tax determination.

**IMPACT:** No customer e-invoices can be generated in any jurisdiction until SD or FI customer invoice configuration is complete.

11Remediation Roadmap

The remediation roadmap is structured into three phases based on urgency, effort, and dependency. Phase 1 (Urgent — This Week) addresses the blocking configuration gaps that must be resolved before any e-invoice can be legally generated. Phase 2 (Short-term — 4–8 Weeks) focuses on data enrichment and output chain setup. Phase 3 (Medium-term — 2–4 Months) covers multi-country expansion and automation.

gantt title E-Invoicing Remediation Roadmap dateFormat YYYY-MM-DD axisFormat %b %d section Phase 1 - Urgent Apply SAP Note 2884058 (India IRN) :p1a, 2026-05-25, 3d Configure J\_1IINDCUS (India CIN) :p1b, after p1a, 2d Populate Vendor GSTIN in LFA1 :p1c, 2026-05-25, 5d Populate Company GSTIN in IN01 :p1d, after p1b, 1d section Phase 2 - Short-term Fix Tax Code Assignment (TAXIN) :p2a, 2026-06-08, 5d Populate STCEG for DE IT BR MX :p2b, 2026-06-08, 7d Configure Output Types (NAST) :p2c, after p2a, 5d Create Customer Master (KNA1) :p2d, 2026-06-08, 10d Test IRN Generation in QA :p2e, after p2a, 3d section Phase 3 - Medium-term Italy SDI Integration :p3a, 2026-07-06, 14d Germany XRechnung/PEPPOL :p3b, 2026-07-06, 14d Mexico CFDI Integration :p3c, 2026-07-20, 10d Brazil NF-e Integration :p3d, 2026-07-20, 10d E-Invoice Monitoring Dashboard :p3e, after p3d, 7d

#### 🚨 Phase 1 — Urgent (This Week)

*   Apply SAP Notes 2884058, 2980729, 3028609 (India e-invoicing IRN framework) — _Basis: 8–16 hours_
*   Populate J\_1IINDCUS with IN01 GSTIN, PAN, and e-invoice activation flag — _Functional: 2 hours_
*   Mass-update 10 vendor GSTINs in LFA1.STCD3 via LSMW or XK02 — _Master Data team: 4 hours_
*   Populate IN01 GSTIN registration in India CIN customizing — _Functional: 1 hour_
*   Unblock vendors 0000100004 (Wipro) and 0000100008 (HUL) if appropriate — _AP team: 30 min_

#### ⚠️ Phase 2 — Short-term (4–8 Weeks)

*   Assign India GST tax procedure (TAXIN) to company code IN01 and test with FB60
*   Populate STCEG for all company codes in active mandate jurisdictions (IT77, DE01, DE02, BR01, MX01, FR01)
*   Configure billing output type for India e-invoice (ZINV or equivalent) in V/30 → NAST
*   Create customer master records (KNA1) if customer invoicing is in scope
*   Run IRP API connectivity test in QA environment
*   Configure payment methods (ZWELS) on all 10 vendor LFB1 records

#### 📋 Phase 3 — Medium-term (2–4 Months)

*   Italy: Activate FatturaPA output (RDINV) and configure SDI middleware/PEPPOL-AS4 adapter
*   Germany: Activate XRechnung/Factur-X output type and configure PEPPOL BIS 3.0
*   Mexico: Configure SAP CFDI solution with PAC (Authorized Certification Provider) API credentials
*   Brazil: Activate SAP NF-e solution with SEFAZ HTTPS certificate configuration
*   Saudi Arabia: Activate ZATCA Phase 2 framework (FATOORA) with cryptographic signing
*   Deploy e-invoice monitoring Fiori app (F3119 – Manage E-Invoices) for real-time IRN status
*   Implement GST reconciliation process (GSTR-2A vs SAP BSEG monthly)

12Quick Wins — Immediate Actions (This Week)

The following quick-win actions can be completed within the current working week with minimal risk and no system downtime. Each action directly unblocks a critical e-invoicing prerequisite and should be prioritised by the project team starting Monday.

Action

SAP Transaction / Path

Effort

Owner

Dependency

Impact

**1\. Populate 10 Vendor GSTINs**

XK02 → Tax Information → STCD3

4 hours

Vendor Master Team

Collect GSTIN from vendors (legal docs)

Enables ITC claims on all AP invoices

**2\. Register IN01 in J\_1IINDCUS**

J1IMGACT → Company Settings

2 hours

SAP Functional Consultant

Company GSTIN must be known

Activates India GST engine for IN01

**3\. Apply SAP Note 2884058**

SNOTE → apply 2884058

8–16 hours

SAP Basis Team

QA testing required after

Deploys IRN generation framework

**4\. Unblock Wipro & HUL Vendors**

XK02 → Posting Block → Remove SPERR

30 min

AP Team / Finance

Approval from AP manager

Enables invoice processing for 2 vendors

**5\. Populate DE01 STCEG**

OB29 → Company Code → Tax

1 hour

Tax / Finance Team

German Steuernummer must be known

DE Germany XRechnung prerequisite

**6\. Assign Tax Procedure TAXIN to IN01**

SPRO → FI → Tax → Assign Procedure

2 hours

SAP FI Consultant

TAXIN procedure must exist (verify first)

Enables tax code usage on all new FI docs

**13\. Verification Methodology**

This assessment was generated autonomously by C16 using live SAP RFC\_READ\_TABLE and TADIR searches against the connected S/4HANA system (S4H, Client 800). All data points are verified from live system queries executed on 24 May 2026. No data has been fabricated or estimated.

**Tables Queried:** T001 (67 rows), LFA1 (10 rows), LFB1 (10 rows), BKPF (20 rows), BSEG (15 rows), T007A (20+ rows), NAST (0 rows — confirmed empty), VBRK (0 rows — confirmed empty), KNA1 (0 rows — confirmed empty), KNVV (0 rows — confirmed empty), STXH (0 rows — VBBK scope), J\_1IINDCUS (confirmed empty), J\_1IG\_GSTINHIS (confirmed empty), J\_1IG\_GSTINBLK (confirmed empty)

**Object Searches:** TADIR searches for J\_1I% (30 results), J\_1IG% (30 results), J\_1I%EINV% (0 results), J\_1IG%IRN% (0 results), J\_1I%GST% (4 results) — confirming IRN tables not installed

**SAP Tools Used:** sap\_check\_system\_version, sap\_read\_table (RFC\_READ\_TABLE), sap\_search\_objects (TADIR), sap\_call\_rfc

⚡ Generated by C16 · SAP S/4HANA Intelligent Assessment Engine · Version 2026.05 · [help.c16.ai](#)

mermaid.initialize({ startOnLoad: true, theme: 'base', themeVariables: { primaryColor: '#6B3FA0', primaryTextColor: '#fff', lineColor: '#B5567A', fontFamily: 'Segoe UI, Arial' } });