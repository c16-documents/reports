  C16 Enterprise 6-Layer S/4HANA Migration Framework v2.0 mermaid.initialize({startOnLoad:true, theme:'base', themeVariables:{primaryColor:'#6B3FA0', primaryTextColor:'#fff', primaryBorderColor:'#B5567A', lineColor:'#6B3FA0', secondaryColor:'#f0e6ff', tertiaryColor:'#fdf2f8'}}); :root{--c16-purple:#6B3FA0;--c16-pink:#B5567A;--c16-gold:#C97B3A;--c16-dark:#1a1a2e;--c16-light:#f8f9fc;--green:#27ae60;--blue:#3498db;--orange:#e67e22;--red:#e74c3c;--amber:#f39c12} \*{margin:0;padding:0;box-sizing:border-box} body{font-family:'Segoe UI',system-ui,-apple-system,sans-serif;background:#e9ecef;color:#2c3e50;line-height:1.7} .doc-wrapper{max-width:1200px;margin:0 auto;background:#fff;box-shadow:0 4px 24px rgba(0,0,0,.12)} /\* Header \*/ .header-banner{background:linear-gradient(135deg,var(--c16-gold),var(--c16-pink),var(--c16-purple));color:#fff;padding:50px 60px;position:relative;overflow:hidden} .header-banner::after{content:'';position:absolute;top:-50%;right:-20%;width:400px;height:400px;border-radius:50%;background:rgba(255,255,255,.04)} .header-banner h1{font-size:2.4em;font-weight:700;margin:12px 0 8px;letter-spacing:-.5px} .header-banner .subtitle{font-size:1.2em;opacity:.92;margin-bottom:4px} .version-badge{display:inline-block;background:rgba(255,255,255,.2);padding:4px 16px;border-radius:20px;font-size:.85em;margin-top:10px} .correction-badge{display:inline-block;background:rgba(39,174,96,.3);padding:4px 16px;border-radius:20px;font-size:.85em;margin-top:6px;margin-left:8px;border:1px solid rgba(255,255,255,.3)} /\* Navigation \*/ .toc{background:linear-gradient(135deg,#f8f5ff,#fdf2f8);padding:35px 60px;border-bottom:3px solid var(--c16-purple)} .toc h2{color:var(--c16-purple);font-size:1.3em;margin-bottom:16px} .toc-grid{display:grid;grid-template-columns:1fr 1fr;gap:6px 40px} .toc a{color:#555;text-decoration:none;font-size:.95em;padding:4px 0;display:block;border-bottom:1px solid #f0f0f0;transition:color .2s} .toc a:hover{color:var(--c16-purple)} .toc a .num{color:var(--c16-purple);font-weight:700;margin-right:6px} /\* Sections \*/ .section{padding:40px 60px} .section:nth-child(even){background:#fafbfe} .section-title{color:var(--c16-purple);font-size:1.6em;font-weight:700;border-bottom:3px solid var(--c16-pink);padding-bottom:12px;margin-bottom:24px;display:flex;align-items:center;gap:12px} .section-num{background:var(--c16-pink);color:#fff;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.9em;font-weight:700;flex-shrink:0} .section h3{color:var(--c16-purple);font-size:1.15em;margin:24px 0 12px;padding-bottom:6px;border-bottom:2px solid #f0e6ff} .section h4{color:var(--c16-pink);font-size:1.05em;margin:18px 0 10px} /\* Tables \*/ table{width:100%;border-collapse:collapse;margin:16px 0;font-size:.92em} thead th{background:var(--c16-purple);color:#fff;padding:12px 16px;text-align:left;font-weight:600} tbody td{padding:10px 16px;border-bottom:1px solid #eee} tbody tr:nth-child(even){background:#f8fafc} tbody tr:hover{background:#edf2f7} /\* Callout boxes \*/ .callout{border-radius:8px;padding:20px 24px;margin:20px 0} .callout-insight{background:linear-gradient(135deg,#f0e6ff,#fdf2f8);border-left:5px solid var(--c16-purple)} .callout-warning{background:#fff8e6;border-left:5px solid var(--amber)} .callout-success{background:#f0fdf4;border-left:5px solid var(--green)} .callout-error{background:#fef2f2;border-left:5px solid var(--red)} .callout-correction{background:linear-gradient(135deg,#f0fdf4,#e6fff0);border-left:5px solid var(--green);border:2px solid rgba(39,174,96,.2)} .callout h4{margin-top:0} .callout-label{font-weight:700;font-size:.9em;text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px} /\* KPI Cards \*/ .kpi-row{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:16px;margin:20px 0} .kpi-card{background:linear-gradient(135deg,#f8f5ff,#fdf2f8);border-radius:10px;padding:20px;text-align:center;border:1px solid #e8d5f5} .kpi-card .number{font-size:2em;font-weight:800;color:var(--c16-purple)} .kpi-card .label{font-size:.85em;color:#666;margin-top:4px} /\* Code blocks \*/ pre{background:#1a1a2e;color:#e2e8f0;padding:20px;border-radius:8px;overflow-x:auto;font-size:.88em;line-height:1.6;margin:12px 0;border-left:4px solid var(--c16-pink)} code{font-family:'Cascadia Code','Fira Code',monospace} .inline-code{background:#f0e6ff;color:var(--c16-purple);padding:2px 8px;border-radius:4px;font-size:.9em} /\* Badges \*/ .badge{display:inline-block;padding:3px 12px;border-radius:12px;font-size:.82em;font-weight:600;color:#fff} .badge-a{background:var(--green)} .badge-b{background:var(--blue)} .badge-c{background:var(--orange)} .badge-d{background:var(--red)} .badge-retire{background:#95a5a6} .badge-auto{background:var(--green)} .badge-guided{background:var(--amber)} .badge-advisory{background:#95a5a6} .badge-phase{padding:4px 14px;border-radius:6px;font-size:.78em;text-transform:uppercase;letter-spacing:.5px} .phase-discover{background:#e8f5e9;color:#2e7d32;border:1px solid #a5d6a7} .phase-analyse{background:#e3f2fd;color:#1565c0;border:1px solid #90caf9} .phase-build{background:#fff3e0;color:#e65100;border:1px solid #ffcc80} .phase-prove{background:#f3e5f5;color:#6a1b9a;border:1px solid #ce93d8} /\* Collapsible \*/ details{margin:12px 0;border:1px solid #e8e8e8;border-radius:8px;overflow:hidden} details\[open\]{border-color:var(--c16-purple)} summary{padding:14px 20px;background:#f8f9fc;cursor:pointer;font-weight:600;color:#333;transition:background .2s;list-style:none} summary::-webkit-details-marker{display:none} summary::before{content:'▶ ';color:var(--c16-purple);font-size:.8em} details\[open\] summary::before{content:'▼ '} details\[open\] summary{background:linear-gradient(135deg,#f0e6ff,#fdf2f8);color:var(--c16-purple)} details .detail-content{padding:20px} /\* Stream diagram \*/ .stream-row{display:grid;grid-template-columns:140px repeat(6,1fr);gap:2px;margin:4px 0} .stream-cell{padding:10px 8px;text-align:center;font-size:.82em;border-radius:4px;font-weight:600} .stream-header{background:var(--c16-purple);color:#fff} .stream-code{background:#e8f5e9;color:#2e7d32} .stream-config{background:#e3f2fd;color:#1565c0} .stream-org{background:#fff3e0;color:#e65100} .stream-integ{background:#f3e5f5;color:#6a1b9a} .stream-data{background:#fce4ec;color:#c62828} .stream-arrow{background:none;color:var(--c16-purple);font-weight:900;display:flex;align-items:center;justify-content:center} /\* Distribution bars \*/ .dist-bar{display:flex;height:36px;border-radius:6px;overflow:hidden;margin:10px 0;font-size:.8em;font-weight:600;color:#fff} .dist-bar div{display:flex;align-items:center;justify-content:center;min-width:40px} /\* Effort cards \*/ .effort-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin:16px 0} .effort-card{border-radius:8px;padding:16px;text-align:center;border:2px solid #eee} .effort-card .effort-level{font-size:1.4em;font-weight:800;margin-bottom:6px} .effort-card .effort-time{font-size:.85em;color:#666} .effort-card .effort-desc{font-size:.82em;color:#888;margin-top:4px} /\* Journey timeline \*/ .journey-track{display:flex;align-items:center;gap:0;margin:20px 0;position:relative} .journey-node{flex:1;text-align:center;position:relative;padding:16px 8px} .journey-node::after{content:'';position:absolute;top:50%;right:0;width:100%;height:3px;background:linear-gradient(90deg,var(--c16-purple),var(--c16-pink));z-index:0} .journey-node:last-child::after{display:none} .journey-dot{width:20px;height:20px;border-radius:50%;background:var(--c16-purple);margin:0 auto 8px;position:relative;z-index:1} .journey-label{font-size:.82em;font-weight:600;color:#333} .journey-sub{font-size:.75em;color:#888} /\* Lane cards \*/ .lane-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin:16px 0} .lane-card{border-radius:10px;padding:20px;text-align:center} .lane-retire{background:#f0fdf4;border:2px solid var(--green)} .lane-remed{background:#fff8e6;border:2px solid var(--amber)} .lane-clean{background:#f0e6ff;border:2px solid var(--c16-purple)} .lane-card h4{margin-top:0} /\* Footer \*/ .doc-footer{background:var(--c16-dark);color:#aaa;padding:30px 60px;font-size:.85em} .doc-footer a{color:var(--c16-pink)} /\* Print \*/ @media print{.doc-wrapper{box-shadow:none}details{break-inside:avoid}details\[open\] .detail-content{break-inside:avoid}.header-banner{-webkit-print-color-adjust:exact;print-color-adjust:exact}} @media(max-width:768px){.section{padding:24px 20px}.header-banner{padding:30px 20px}.toc{padding:24px 20px}.toc-grid{grid-template-columns:1fr}.kpi-row{grid-template-columns:1fr 1fr}.effort-grid{grid-template-columns:repeat(3,1fr)}.lane-grid{grid-template-columns:1fr}.stream-row{overflow-x:auto}}

![C16](data:image/jpeg;base64,)

Enterprise S/4HANA Transformation Framework

# The 6-Layer Migration Methodology

A process-agnostic, triage-based approach to S/4HANA migration that covers custom code, configuration, org structure, integrations, and data — through all six layers.

Version 2.0 — April 2026 ✅ Corrected: CC Levels, 5 Streams, Triage Economics

## Contents

[01 The Problem This Solves](#problem) [02 Key Corrections (v2.0)](#corrections) [03 Design Principles](#principles) [04 Clean Core Levels — The Real Picture](#cc-levels) [05 The 5 Effort Levels](#effort-spectrum) [06 Triage Decision Engine](#triage) [07 Five Parallel Streams](#five-streams) [08 The 6-Layer Framework](#framework)

[09 Layer 1: Complete AS-IS](#layer1) [10 Layer 2: S/4HANA Standard Delta](#layer2) [11 Layer 3: Fit-to-Standard Analysis](#layer3) [12 Layer 4: Remediation & Clean Core Build](#layer4) [13 Layer 5: Certification & Validation](#layer5) [14 Layer 6: Handoff & Go-Live Readiness](#layer6) [15 The Progressive Journey](#journey) [16 C16 Capability Matrix](#capability) [17 Execution Timeline](#timeline) [18 Cross-Process Examples](#cross-process) [19 Value Proposition](#value)

## 1 The Problem This Solves

Every S/4HANA migration faces the same dilemma: **speed vs quality**. The two extremes both fail:

❌ Approach A: Remediate Everything

**"Make it work on S/4, fix quality later."**

*   Fixes Level D → C (removes blockers)
*   Level C stays C (direct table access unchanged)
*   Level B stays B (BAPIs untouched)
*   Go-live: fast
*   **Result: Everything works, but mostly Level C. No cloud path. Technical debt grows. "Later" never comes.**

❌ Approach B: Clean Core Everything

**"Rewrite everything to Level A before go-live."**

*   1,000+ objects × 15 days average = years of effort
*   Every changed object needs full regression testing
*   Business continuity risk at every step
*   **Result: Timeline explodes. Budget triples. Business says "no."**

💡 The Real Answer: Triage

**Not every object needs the same treatment.** Some should be retired. Some need a one-line API swap (which IS Clean Core at almost zero extra cost). Some need heavy rewrites that should be deferred post-go-live. The 6-Layer Framework builds this triage into the methodology — with 5 parallel workstreams covering code, config, org structure, integrations, and data.

### What Most Frameworks Miss

Gap

Consequence

This Framework's Answer

Focus only on custom code

Config changes break processes at go-live

5 parallel streams — config, org, integrations, data are first-class

Classify customs before understanding S/4 standard

Mapping is done against the wrong target

Layer 2 (Standard Delta) comes BEFORE Layer 3 (Fit-to-Standard)

Blanket "remediate" or "Clean Core" mandate

Either too slow or too risky

Per-object triage with effort-level economics

Clean Core levels stated as B/C

Misleading — ECC code is actually C/D

Honest level distribution with strategic guidance per path

No validation layer

"Trust us, it works" → go-live fails

Layer 5 exists specifically to PROVE everything works

Discovery items without full lifecycle

Org, integration, data discovered but never acted on

Every stream has arrows through all 6 layers

## 2 Key Corrections (v2.0)

This version corrects several important inaccuracies from v1.0:

✅ Correction 1: ECC Code Is Level C/D, Not B/C

v1.0 stated remediated ECC code would be "stuck at Level B/C." This is wrong. Most ECC code is Level C (direct table access) or Level D (deprecated patterns). After basic remediation, D moves to C, but C stays C. Very little reaches Level B without deliberate Clean Core effort. The levels, distribution, and implications are fully corrected in Section 4.

✅ Correction 2: 5 Parallel Streams, Not 2

v1.0 had full lifecycle paths only for custom code and configuration. Org structure, integrations, and data profile were "discovered for context" but had no arrows through Layers 2-6. This version gives all 5 streams their own path through every layer. Section 7 shows the complete 5×6 matrix.

✅ Correction 3: Level B Is Fine On-Prem — Problem Is Strategic

v1.0 implied Level B is always bad. In reality, Level B works perfectly on S/4HANA on-premise. The issue is strategic: Level B blocks cloud migration (RISE Public Cloud, BTP). Whether that matters depends entirely on the customer's cloud roadmap. Section 4 now includes the strategic decision framework.

✅ Correction 4: The Crossover Economics

v1.0 assumed Clean Core always costs significantly more than remediation. In reality, for ~60% of objects (Effort Level 1-2), Clean Core costs almost the same as remediation — sometimes just 30 minutes more. The triage engine now uses this insight: cheap Clean Core objects should be done right the first time, not deferred.

## 3 Design Principles

#### P1: Understand Both Sides Before Mapping

Don't classify any custom object until you've completed BOTH Layer 1 (what you have today) AND Layer 2 (what S/4 standard offers). Mapping against an unknown target is guesswork.

#### P2: Configuration Is First-Class

Config changes break processes just as badly as code changes. Pricing procedures, output determination, partner schemas, credit settings — all get the same 6-layer treatment as custom code.

#### P3: One Comprehensive Discovery

Layer 1 discovers EVERYTHING in one pass — code, config, org structure, integrations, and data profile. Not separate workstreams by separate teams at separate times.

#### P4: Never Write Throwaway Code

If you're touching an object and Clean Core effort is low (Level 1-2 swap), do it right the first time. Only defer the heavy rewrites. Never remediate to Level C then rewrite to Level A — that's paying twice.

#### P5: Triage, Don't Mandate

No blanket policies. Each object gets triaged into Retire, Clean Core Now, or Remediate & Defer — based on effort level, strategic importance, and the crossover economics.

#### P6: Every Stream Has a Full Path

Everything discovered in Layer 1 must have arrows through all 6 layers. Discovering something you never act on is wasted effort. Code, config, org, integrations, data — all 5 streams flow through all 6 layers.

#### P7: Prove It Works

Layer 5 exists specifically to prove everything works — ATC cloud readiness, before/after scorecard, E2E testing, data reconciliation. No handoff without validation evidence.

## 4 Clean Core Levels — The Real Picture

### The Four Levels Explained

Level

What It Means

Analogy

Example Code

Level A

**Cloud-Ready.** Uses only Released APIs, CDS views, Released BAdIs. Can run on S/4 Public Cloud and BTP.

Driving on a highway — smooth, supported, maintained

SELECT FROM I\_Customer

Level B

**Classic Stable.** Uses BAPIs, CL\_SALV\_TABLE, stable interfaces. Works on S/4 on-prem. NOT cloud-compatible.

Driving on a paved road — works fine, just can't access the highway

CALL FUNCTION 'BAPI\_SALESORDER\_CREATE'

Level C

**Non-Compliant.** Direct access to standard SAP tables without CDS. Unreleased function modules. No cloud compatibility.

Driving on a dirt road — works today, potholes appear each upgrade

SELECT \* FROM KNA1

Level D

**Critical/Deprecated.** Obsolete FMs, SAPscript, kernel-level access, core modifications. May break on S/4.

Driving off-road — could hit a cliff at any turn

CALL FUNCTION 'WS\_UPLOAD'

### Where ECC Code Actually Sits Today

Most people assume their ECC code is Level B. **It's not.** The vast majority is Level C or D:

5% A

20% B

50% C

25% D

Why is most ECC code Level C/D?

Pattern

Level

Why It's So Common

SELECT \* FROM VBAK / EKKO / MARA

C

Every report, every interface. Direct table access was the standard way to code for 25 years.

FORM userexit\_\* in MV45AFZZ etc.

D

User exits were the ONLY extension point before BAdIs. Thousands exist in every ECC system.

CALL FUNCTION 'WS\_UPLOAD/DOWNLOAD'

D

Obsolete since 2005 but never removed from working programs.

SAPscript (OPEN\_FORM/WRITE\_FORM)

D

Pre-SmartForm output technology. Many customers still use it for invoices, POs, delivery notes.

BDC / CALL TRANSACTION recordings

D

Screen-level automation. Fundamentally incompatible with Fiori.

BAPI\_SALESORDER\_CREATEFROMDAT2

B

The good developers who used BAPIs. Already at Level B without effort.

### What Remediation Actually Achieves

"Basic remediation" means: make it RUN on S/4, minimum changes. Here's what actually moves:

ECC State

Remediation Action

Level After

Movement

D WS\_UPLOAD

Swap to GUI\_UPLOAD

C

D → C

D SAPscript

Convert to SmartForm

D

Still D (SmartForms deprecated too)

D User exit MV45AFZZ

Fix syntax only

D

Still D (can't change the container)

C SELECT FROM KNA1

Change to BUT000 (BP)

C

Still C (different table, same pattern)

C SELECT FROM VBAK

No change needed

C

Still C

B BAPI call

No change needed

B

Still B

**After remediation:**

5% A

25% B

60% C

10% D

D reduced, C absorbs — but nothing reaches Level A. The bulk stays at C.

### Is Level B a Problem? It Depends on Your Path

⚠️ This Is a Strategic Decision, Not a Technical One

Level B code works perfectly on S/4 on-prem. There is no technical issue. The question is whether your future roadmap requires cloud capabilities.

Your Future Path

Level B Impact

Level C Impact

Action

**Stay S/4 On-Premise**

✅ No issue

⚠️ Upgrade friction

B is fine. Progressively move C→B.

**RISE Private Cloud**

⚠️ Some restrictions

❌ Many restrictions

Evaluate per object. Plan for A.

**RISE Public Cloud**

❌ Will not work

❌ Will not work

Must reach Level A.

**BTP Side-by-Side**

❌ For BTP code

❌ For BTP code

BTP code must be A.

**5+ year horizon unknown**

⚠️ Growing risk

⚠️ Higher risk

Triage approach — keep options open.

#### 🔑 The Time Dimension

Level B is fine **today**. But SAP's direction is clear — they're progressively moving everything toward Level A (Released APIs). Each annual S/4 upgrade adds more Released APIs and deprecates more classic ones. Level B is a safe harbour today, but the harbour is slowly shrinking. Plan accordingly.

## 5 The 5 Effort Levels

Clean Core is NOT always a rewrite. Here's the actual spectrum of effort:

L1

30 min

One-Line Swap

L2

2-4 hrs

Small Block Replace

L3

1-3 days

BAdI / Container Swap

L4

1-2 weeks

Significant Rewrite

L5

2-4 weeks

Full Rebuild

Level 1: One-Line Swap — Table Name Change (30 minutes)

**Before (Level C):**

```
SELECT SINGLE * FROM KNA1 WHERE KUNNR = lv_customer.
```

**After (Level A):**

```
SELECT SINGLE * FROM I_Customer WHERE Customer = lv_customer.
```

**What changed:** One line. Table name → CDS view name. The rest of the program is identical. I\_Customer is a Released API that reads the same data.

Level 2: Small Block Replace — API Swap (2-4 hours)

**Before (Level C):**

```
CALL FUNCTION 'SD_SALES_ORDER_CREATE'
  EXPORTING order_header = ls_header
  TABLES    order_items  = lt_items
  EXCEPTIONS error = 1.
```

**After (Level A):**

```
CALL FUNCTION 'BAPI_SALESORDER_CREATEFROMDAT2'
  EXPORTING order_header_in = ls_header_bapi
  TABLES    order_items_in  = lt_items_bapi
            return          = lt_return.
```

**What changed:** FM name + parameter mapping. 10-15 lines. The business logic that BUILDS the header and items? **Completely untouched.**

Level 3: BAdI / Container Swap (1-3 days)

**Before (Level D) — Classic User Exit:**

```
* Inside MV45AFZZ include (NOT a Released extension point)
FORM USEREXIT_SAVE_DOCUMENT_PREPARE.
  SELECT SINGLE * FROM KNB1 WHERE KUNNR = ...
  IF lv_credit > lv_limit.
    sy-subrc = 4.  " block the order
  ENDIF.
ENDFORM.
```

**After (Level A) — Released BAdI:**

```
CLASS zcl_credit_check DEFINITION PUBLIC FINAL.
  PUBLIC SECTION.
    INTERFACES if_sd_sales_doc_check.   " Released BAdI interface
ENDCLASS.

CLASS zcl_credit_check IMPLEMENTATION.
  METHOD if_sd_sales_doc_check~check.
    " SAME business logic — just in a Released container
    SELECT SINGLE * FROM I_CustomerCompany WHERE ...
    IF lv_credit > lv_limit.
      " Block via BAdI return structure
    ENDIF.
  ENDMETHOD.
ENDCLASS.
```

**What changed:** The CONTAINER changed (user exit → Released BAdI). The LOGIC inside is 90% the same. You change the wrapper, fix a few table references, keep the business rules.

Level 4: Significant Rewrite (1-2 weeks)

**When:** The entire approach is obsolete — BDC screen recordings, custom pricing engines, deep kernel integration.

```
Before: BDC program that plays VA01 screen-by-screen
        500+ lines of BDCDATA, CALL TRANSACTION
        Fundamentally incompatible with Fiori
        
After:  Direct API call to API_SALES_ORDER_SRV
        50 lines of clean code
        Works with any frontend (Fiori, portal, integration)
```

The logic is similar but the architecture is fundamentally different.

Level 5: Full Rebuild (2-4 weeks)

**When:** Custom framework that deeply penetrates SAP internals — custom pricing engines that bypass condition technique, custom inventory engines that bypass MARD, custom workflow engines that bypass SAP BC.

```
Before: Custom pricing engine
        Reads V-tables directly, maintains own price memory
        2,000+ lines across 5 includes
        
After:  Must redesign using standard pricing + Released BAdI
        Completely different architecture
        This is RARE — typically 5-8% of objects
```

### The Real Distribution

L1: 35%

L2: 25%

L3: 20%

L4: 12%

L5: 8%

#### 🔑 The Crossover Insight — This Changes the Strategy

For **Level 1-2 objects (60% of total)**, Clean Core costs almost the same as basic remediation — sometimes just 30 minutes more. You're swapping a table name or an API call, not rewriting the program.

**This means:** For 60% of your custom objects, the choice between "remediate to Level C" and "Clean Core to Level A" is a false dilemma. The cost is nearly identical. Just do it right.

Effort Level

Clean Core Effort

Remediation Effort

Delta

Level 1 (table swap)

30 minutes

30 minutes

**Zero**

Level 2 (API swap)

2-4 hours

1-2 hours

**+2 hours**

Level 3 (BAdI swap)

1-3 days

0.5-1 day

+1-2 days

Level 4 (rewrite)

1-2 weeks

2-3 days

+1 week

Level 5 (rebuild)

2-4 weeks

1 week

+2 weeks

## 6 Triage Decision Engine

Every object gets triaged individually — no blanket mandates. The decision tree:

graph TD A\[Custom Object\] --> B{S/4 standard  
replaces it?} B -->|Yes| C\[🟢 LANE 1: RETIRE\] B -->|No| D{Clean Core effort  
level?} D -->|Level 1-2| E\[🟣 LANE 3: CLEAN CORE NOW  
Costs same as remediation\] D -->|Level 3| F{Strategic object?} D -->|Level 4-5| G\[🟡 LANE 2: REMEDIATE & DEFER  
Too risky before go-live\] F -->|Yes / Cloud planned| E F -->|No / Urgent go-live| G style C fill:#27ae60,color:#fff,stroke:#27ae60 style E fill:#6B3FA0,color:#fff,stroke:#6B3FA0 style G fill:#f39c12,color:#fff,stroke:#f39c12

#### 🟢 Lane 1: Retire

**~30-40% of objects**

S/4 standard replaces it. No code work — document the retirement and switch to standard.

#### 🟣 Lane 3: Clean Core Now

**~30-40% of objects**

Effort Level 1-2 (costs same as remediation) OR Level 3 strategic objects. Write Level A code from day one.

#### 🟡 Lane 2: Remediate & Defer

**~15-25% of objects**

Effort Level 4-5. Make it work on S/4 (Level B/C). Schedule Clean Core conversion for post-go-live waves.

### The Updated Triage Rule

**Old thinking:** "Remediate everything to go live fast, Clean Core later."

**New thinking:** "If Clean Core effort is within 1 day of remediation effort, just do Clean Core now. Only defer the heavy rewrites."

### Example Triage — Sales Order Process (50 objects)

Custom Object

What It Does

Effort Level

Lane

Rationale

Z\_SD\_ORDER\_UPLOAD

BDC mass upload via VA01

\-

RETIRE

API\_SALES\_ORDER\_SRV replaces it

Custom ALV report

Open SO report

\-

RETIRE

Fiori F1814 has built-in analytics

SELECT FROM KNA1

Customer lookup in validation

L1

CLEAN NOW

Swap to I\_Customer — 30 min

Pricing condition read

Custom condition lookup

L2

CLEAN NOW

Swap to Released API — 2 hrs

USEREXIT\_SAVE\_DOCUMENT

Credit check logic

L3

CLEAN NOW

Released BAdI exists — strategic

Custom ATP BAdI

Availability override

L3

REMEDIATE

Complex, not strategic — defer

Custom SmartForm

Order confirmation

L4

REMEDIATE

Adobe conversion post-go-live

Custom pricing engine

Bypasses condition technique

L5

REMEDIATE

Full redesign — scheduled Year 2

**Result at go-live:** 15 retired + 20 Level A + 8 decision point + 5 Level B/C + 2 deferred = **potentially 70-80% Level A at go-live** with almost the same timeline as pure remediation.

## 7 Five Parallel Streams Through All Six Layers

Every stream discovered in Layer 1 has a **complete path** through all 6 layers. Nothing is "discovered for context" then abandoned.

### The 5×6 Matrix

Stream

L1: Discover

L2: S/4 Delta

L3: Fit-to-Std

L4: Build

L5: Certify

L6: Handoff

**📦 Custom Code**

Inventory all Z-programs, exits, BAdIs, appends

Field changes, deprecated FMs, removed transactions

Triage: Retire / Clean Core / Remediate

Write Level A code or remediate

ATC cloud readiness, Level A scorecard

Code migration package

**⚙️ Configuration**

Snapshot: pricing, output, credit, ATP, partners, routes

Config items that change: NACE→Output Mgmt, FD32→FSCM, condition tables

Per config: Same / Changed / Removed / New

Config migration steps (SPRO paths)

Config consistency tests

SPRO migration runbook

**🏢 Org Structure**

Map: sales orgs, plants, channels, divisions, shipping points, credit areas

Org changes: BP location mapping, FSCM credit area, simplified structures

Per org unit: Same / Changed / Needs Remap

Org remap checklist, SPRO delta

Org consistency verification (SO→plant→ship point→billing)

Org migration guide

**🔌 Integrations**

Inventory: EDI/IDoc, RFC, OData, files, middleware connections

IDoc segment changes, deprecated RFCs, V2→V4, Event Mesh option

Per interface: Works / Fix Mapping / Rebuild / Retire

Fix IDoc mappings, replace RFCs, redesign strategic on Event Mesh

E2E interface tests per partner

Interface runbook per partner system

**📊 Data Profile**

Volumes, order types used, doc flow completeness, data quality baseline

BP migration (KNA1/LFA1→BUT000), MATNR 18→40, mandatory field changes

Per data domain: Migrate / Transform / Cleanse First / Archive

Cleansing rules, BP mapping, MATNR conversion, archive decisions

Dry run: load, reconcile, validate counts + relationships

Data migration plan + cleansing report

### Visual: All 5 Streams, All 6 Layers

graph LR subgraph "Layer 1  
DISCOVER" A1\[Code\] A2\[Config\] A3\[Org\] A4\[Integration\] A5\[Data\] end subgraph "Layer 2  
S/4 DELTA" B1\[Code Delta\] B2\[Config Delta\] B3\[Org Delta\] B4\[Interface Delta\] B5\[Data Delta\] end subgraph "Layer 3  
FIT-TO-STD" C1\[Triage\] C2\[Same/Changed\] C3\[Same/Remap\] C4\[Works/Fix/Rebuild\] C5\[Migrate/Transform\] end subgraph "Layer 4  
BUILD" D1\[Code Build\] D2\[Config Steps\] D3\[Org Remap\] D4\[Fix Interfaces\] D5\[Cleanse Data\] end subgraph "Layer 5  
CERTIFY" E1\[ATC/Score\] E2\[Config Test\] E3\[Org Verify\] E4\[E2E Test\] E5\[Dry Run\] end subgraph "Layer 6  
HANDOFF" F1\[Code Package\] F2\[SPRO Runbook\] F3\[Org Guide\] F4\[Interface Runbook\] F5\[Data Plan\] end A1-->B1-->C1-->D1-->E1-->F1 A2-->B2-->C2-->D2-->E2-->F2 A3-->B3-->C3-->D3-->E3-->F3 A4-->B4-->C4-->D4-->E4-->F4 A5-->B5-->C5-->D5-->E5-->F5 style A1 fill:#e8f5e9,stroke:#2e7d32,color:#000 style A2 fill:#e3f2fd,stroke:#1565c0,color:#000 style A3 fill:#fff3e0,stroke:#e65100,color:#000 style A4 fill:#f3e5f5,stroke:#6a1b9a,color:#000 style A5 fill:#fce4ec,stroke:#c62828,color:#000

### What C16 Automates Per Stream

Stream

L1

L2

L3

L4

L5

L6

**Custom Code**

Auto

Auto

Auto

Auto

Auto

Auto

**Configuration**

Auto

Auto

Auto

Guided

Auto

Guided

**Org Structure**

Auto

Auto

Guided

Guided

Guided

Guided

**Integrations**

Auto

Auto

Auto

Auto

Auto

Guided

**Data Profile**

Auto

Auto

Auto

Guided

Guided

Guided

Auto = C16 executes without human intervention  |  Guided = C16 produces the plan/analysis, customer team executes

## 8 The 6-Layer Framework — Overview

graph TB subgraph "PHASE 1: DISCOVER" L1\["**Layer 1: Complete AS-IS**  
What do we have?"\] end subgraph "PHASE 2: ANALYSE" L2\["**Layer 2: S/4HANA Standard Delta**  
What changes regardless?"\] L3\["**Layer 3: Fit-to-Standard + Triage**  
What fits? What lane per object?"\] end subgraph "PHASE 3: BUILD" L4\["**Layer 4: Remediation & Clean Core Build**  
Do the work — right the first time"\] end subgraph "PHASE 4: PROVE" L5\["**Layer 5: Certification & Validation**  
Prove everything works"\] L6\["**Layer 6: Handoff & Go-Live Readiness**  
Package for implementation"\] end L1-->L2-->L3-->L4-->L5-->L6 style L1 fill:#e8f5e9,stroke:#2e7d32,color:#000 style L2 fill:#e3f2fd,stroke:#1565c0,color:#000 style L3 fill:#e3f2fd,stroke:#1565c0,color:#000 style L4 fill:#fff3e0,stroke:#e65100,color:#000 style L5 fill:#f3e5f5,stroke:#6a1b9a,color:#000 style L6 fill:#f3e5f5,stroke:#6a1b9a,color:#000

6

Sequential Layers

5

Parallel Streams

4

Phases

30

Cells in 5×6 Matrix

### Data Flow Contracts

Each layer's output is explicitly defined as the next layer's input:

Layer

Input (from previous layer)

Output (to next layer)

**Layer 1**

SAP system connection

Complete inventory: code + config + org + integrations + data profile

**Layer 2**

Layer 1 inventory

Delta map: what changes per stream, change register, new capabilities

**Layer 3**

Layer 1 inventory + Layer 2 delta

Classification per object: FIT/CONFIG/EXTENSION/GAP + Lane assignment (Retire/Clean/Remediate) + effort level

**Layer 4**

Layer 3 triage decisions

Retirement docs + remediated code + Clean Core code + config migration steps + cleansing rules

**Layer 5**

Layer 4 deliverables

Certification evidence: ATC results, test outcomes, before/after scorecard, dry run reconciliation

**Layer 6**

Layer 5 certification

Go-live package: all 5 stream runbooks, cutover plan, training needs, final scorecard

## 9 Layer 1: Complete AS-IS Discovery

PHASE 1: DISCOVER

**Purpose:** Build a complete inventory of EVERYTHING — not just custom code. One scan, one baseline, all 5 streams.

📦 Stream 1: Custom Code Discovery

What

How

Output

Z-Programs, Classes, Function Groups

Scan TADIR for all Z/Y objects

Object list with type, package, creation date

User Exits per transaction

Scan MODSAP, find\_user\_exits per Tcode

Exit-to-transaction map

BAdI Implementations

Scan SXS\_ATTR, filter INTERNAL flag

BAdI list with Released status

Custom Appends (ZZ/YY fields)

Read DD03L for ZZ\* fields on standard tables

Append field list per table

Source code reading

Read full source of every custom object

Business requirement extraction per object

Custom forms

Scan SmartForms, SAPscript, Adobe Forms

Form inventory with output type

⚙️ Stream 2: Configuration Discovery

Config Area

Tables Read

What We Capture

Pricing procedures

T683S, T685A, KONH/KONP

Condition types, access sequences, scales

Output determination

NACE, NACH, T685B

Output types, programs, forms, conditions

Credit management

T014, KNKK, T691F

Credit areas, limits, rule sets

Availability check

T436A, TVEP, T438A

Checking groups, scope of check, replenishment

Partner determination

TBE1, TPAR, TPAUM

Partner types, schemas, determination procedures

Document types

TVAK (SO), TVLK (DL), TVFK (BL)

Active types, number ranges, copying control

🏢 Stream 3: Org Structure Discovery

Org Element

Table

What We Capture

Company codes

T001

Code, name, currency, country, fiscal year variant

Sales organizations

TVKO

Org, company code assignment, description

Distribution channels

TVKOV

Channel-to-sales-org assignments

Plants

T001W

Plant, name, country, valuation area

Shipping points

TVSTZ

Shipping point to plant assignment

Credit control areas

T014

Area, currency, update group

🔌 Stream 4: Integration Discovery

Integration Type

How Discovered

What We Capture

IDoc interfaces

Scan EDIMSG, partner profiles (WE20)

Message types, segments, partner systems

RFC destinations

Read RFCDES

Destinations, target systems, connection type

OData services

Gateway catalog

Service names, entities, consumers

File interfaces

Scan source for FILE/OPEN DATASET

File paths, frequency, direction

Middleware

Scan for PI/PO, CPI, middleware RFCs

Middleware technology, interfaces

📊 Stream 5: Data Profile Discovery

Data Domain

Tables

What We Capture

Sales order volumes

VBAK count + date range

Total orders, order types used, date range, growth trend

Customer master

KNA1, KNB1

Total customers, active/blocked, address completeness

Material master

MARA, MARC

Total materials, types, MATNR length analysis

Document flow

VBFA

Completeness: orders with delivery %, orders with billing %

Data quality

Various

Missing mandatory fields, duplicate detection, orphan records

## 10 Layer 2: S/4HANA Standard Delta

PHASE 2: ANALYSE

**Purpose:** Understand what changes in S/4HANA REGARDLESS of custom code. This is the baseline every migration needs.

#### Why Layer 2 Comes Before Fit-to-Standard

You cannot map your customs to "S/4 standard" if you don't know what S/4 standard looks like. Layer 2 establishes the target. Layer 3 maps against it.

📦 Code Delta: What Changes in Standard SAP

Change Category

Example

Impact

Transactions replaced by Fiori

VA01 → F1814, VL01N → F1703, VF01 → F0861

Screen exits, dynpro modifications obsolete

Fields removed/added

KNA1 fields → BP fields, VBAK structural changes

Custom SELECTs may reference missing fields

FMs deprecated

Check TFDIR for obsolete flag on each FM used

Custom code calling deprecated FMs will fail

Simplification items

SAP Note 2313884 — per SD/MM/FI component

Mandatory changes documented by SAP

New Released APIs

API\_SALES\_ORDER\_SRV, I\_SalesOrder CDS

New Level A alternatives exist

⚙️ Config Delta: What Configuration Changes

Config Area

ECC

S/4HANA 2025

Migration Impact

Output Management

NACE + SmartForms

Output Management 2.0 + BTP Forms

Output config must be migrated to new framework

Credit Management

FD32 / VKM1-3

FSCM Credit Management (UKM\_\*)

Credit config migrates to new engine

Condition Tables

Some tables restructured

Simplified condition tables

Custom condition tables may need remapping

Material Ledger

Optional

Mandatory in S/4

ML activation required, valuation changes

🏢 Org Delta: What Org Structure Changes

Org Element

S/4 Change

Migration Action

Credit Control Areas

FSCM integration changes mapping

Reassign credit areas under new FSCM config

BP Location mapping

Customer/Vendor locations → BP addresses

Address mapping during BP migration

Simplified org structures

Some redundant levels removed

Review and simplify if applicable

🔌 Integration Delta: What Interface Changes

Interface Type

ECC

S/4 2025

Action

IDoc segments

ORDERS05 fields

Some segments have new/removed fields

Check segment mapping per message type

RFC functions

Some deprecated

Released API successors available

Map deprecated → released per FM

OData version

V2 predominant

V4 preferred, V2 still supported

Plan V4 migration for new development

Event-driven

Not standard

Event Mesh / RAP events available

Strategic interfaces → Event Mesh

📊 Data Delta: What Data Model Changes

Data Change

ECC

S/4

Impact

Business Partner

KNA1 (customer), LFA1 (vendor)

BUT000 (unified BP)

All customer/vendor master must migrate to BP

Material Number

18 characters (CHAR18)

40 characters (CHAR40)

All custom fields/tables referencing MATNR need length check

New mandatory fields

Some optional

Some now mandatory

Data cleansing before migration

Table simplifications

BSEG, BSIK, BSID etc.

Compatibility views exist but underlying tables changed

Custom SELECTs still work but performance may differ

## 11 Layer 3: Fit-to-Standard Analysis + Triage

PHASE 2: ANALYSE

**Purpose:** Map every item from Layer 1 against the S/4 delta from Layer 2. Classify each item and assign a lane.

### Layer 3 Output Per Object

Every object gets three classifications:

Classification

Options

Meaning

**Fit-to-Standard**

FIT / FIT WITH CONFIG / FIT WITH EXTENSION / GAP

How well does S/4 standard cover this requirement?

**Effort Level**

L1 / L2 / L3 / L4 / L5

How much work to make it Clean Core?

**Lane Assignment**

RETIRE / CLEAN CORE NOW / REMEDIATE & DEFER

Which triage lane? (Uses the triage decision tree from Section 6)

### Per-Stream Fit-to-Standard Classifications

📦 Custom Code Classification

Classification

Meaning

Lane

FIT

S/4 standard replaces this completely

RETIRE

FIT WITH CONFIG

Standard covers via configuration

RETIRE + CONFIG

FIT WITH EXTENSION

Standard covers ~70-80%, needs Released BAdI

CLEAN CORE or REMEDIATE (per effort level)

GAP

No standard equivalent

CLEAN CORE or REMEDIATE (per effort level)

⚙️ Configuration Classification

Classification

Meaning

Action

**SAME**

Config works identically in S/4

No action — verify in Layer 5

**CHANGED**

Config exists but path/values changed

Document new SPRO path, migrate settings

**REMOVED**

Config no longer exists in S/4

Map to replacement or redesign

**NEW**

New S/4 config available (not in ECC)

Evaluate and adopt if beneficial

🏢 Org Structure Classification

Classification

Meaning

Action

**SAME**

Org element unchanged

No action

**CHANGED**

Assignment rules differ

Update assignments in SPRO

**NEEDS REMAP**

Org structure must be redesigned

Org design workshop required

🔌 Integration Classification

Classification

Meaning

Action

**WORKS AS-IS**

Interface unchanged in S/4

Verify in Layer 5

**FIX MAPPING**

Fields/segments changed

Update mapping, test E2E

**REBUILD**

Technology deprecated

Redesign (RFC → API, IDoc → Event Mesh)

**RETIRE**

No longer needed (standard API covers it)

Document retirement

📊 Data Classification

Classification

Meaning

Action

**MIGRATE AS-IS**

Data structure unchanged

Standard migration tools

**TRANSFORM**

Data model changed (KNA1 → BP)

Transformation mapping + conversion program

**CLEANSE FIRST**

Data quality issues must be fixed pre-migration

Cleansing rules + execution

**ARCHIVE**

Historical data not needed in S/4

Archive before migration

## 12 Layer 4: Remediation & Clean Core Build

PHASE 3: BUILD

**Purpose:** Execute the triage decisions from Layer 3. Three sub-lanes plus config/org/integration/data work.

### Three Sub-Lanes for Custom Code

#### 4A: Retire

Document the retirement. Validate standard covers the requirement. Update user documentation. Remove from transport scope.

Typical: ~30-40% of objects

#### 4C: Clean Core Build

Write new code directly at Level A. Use Released BAdIs, Released APIs, CDS views. No throwaway code — this is the target state.

Typical: ~30-40% of objects (L1-L2 effort + strategic L3)

#### 4B: Remediate

Minimum changes for S/4 compatibility. Fix BP refs, deprecated FMs, field changes. Level B/C acceptable. MARK for post-go-live Clean Core wave.

Typical: ~15-25% of objects (L4-L5 effort)

### Other Streams in Layer 4

⚙️ Configuration Migration

For each CHANGED/REMOVED config item from Layer 3:

*   Document the new SPRO path in S/4
*   Provide the specific IMG activity and values
*   For REMOVED config: document the replacement approach
*   For NEW config: evaluate adoption (e.g., Output Management 2.0, FSCM Credit)

**Deliverable:** SPRO migration runbook with step-by-step instructions per config area.

🏢 Org Structure Remap

For each CHANGED/NEEDS REMAP org element from Layer 3:

*   Document current vs target assignment
*   For NEEDS REMAP: facilitate org design decision with customer
*   Produce org migration checklist

**Deliverable:** Org migration guide with current/target mapping per org element.

🔌 Integration Fixes

For each FIX MAPPING/REBUILD interface from Layer 3:

*   FIX MAPPING: Update field/segment mappings, generate test data
*   REBUILD: Design new interface using Released APIs or Event Mesh
*   Generate integration test scripts per partner system

**Deliverable:** Updated interface specifications + test scripts.

📊 Data Cleansing & Migration Prep

For each TRANSFORM/CLEANSE FIRST data domain from Layer 3:

*   TRANSFORM: Generate BP migration mapping (KNA1 → BUT000), MATNR conversion rules
*   CLEANSE FIRST: Identify specific records with quality issues, generate cleansing rules
*   ARCHIVE: Define archiving criteria and timeline

**Deliverable:** Data migration plan + cleansing report with specific record lists.

## 13 Layer 5: Certification & Validation

PHASE 4: PROVE

**Purpose:** Prove everything from Layer 4 works. No handoff without evidence.

### Validation Per Stream

Stream

Validation Method

Evidence Produced

**Custom Code**

ATC with ABAP\_CLOUD\_READINESS variant. Syntax check. Unit tests. Clean Core score per object.

Before/after Level A progression scorecard

**Configuration**

Config consistency check — create test SO, test delivery, test billing in S/4 sandbox

Config migration sign-off per area

**Org Structure**

Verify org chain: SO in sales org → correct plant → correct shipping point → correct billing

Org consistency test results

**Integrations**

E2E interface test per partner: send test message → verify processing → verify response

Interface test report per partner

**Data**

Dry run: load subset → reconcile record counts → verify relationships → validate BP conversion

Data reconciliation report

### The Hero Metric: Before/After Scorecard

Metric

Before (ECC)

After Layer 4

Target (12 months post-go-live)

Level A (Cloud-Ready)

5%

50-60%

85-90%

Level B (Classic Stable)

20%

15-20%

8-10%

Level C (Non-Compliant)

50%

15-20%

2-5%

Level D (Critical)

25%

0%

0%

Retired

0%

30-40%

30-40%

## 14 Layer 6: Handoff & Go-Live Readiness

PHASE 4: PROVE

**Purpose:** Package everything for the implementation team. Each stream produces its own runbook.

### Deliverables Per Stream

Stream

Deliverable

Audience

**Custom Code**

Code migration package: retired list, remediated code, Clean Core code, deployment sequence

ABAP development team

**Configuration**

SPRO migration runbook: step-by-step per config area with screenshots/values

Functional consultants

**Org Structure**

Org migration guide: current/target mapping, remap checklist

Functional + basis team

**Integrations**

Interface runbook per partner: updated specs, test scripts, contact info

Integration team + partners

**Data**

Data migration plan: cleansing rules, BP mapping, archiving criteria, dry run results

Data migration team

### Cross-Stream Deliverables

Deliverable

Content

**Cutover dependency map**

Which streams depend on each other? (e.g., BP migration must complete before code testing customer data)

**Training needs assessment**

Who needs training on what? (Fiori for end users, Released BAdIs for developers, new config for functional)

**Post-go-live roadmap**

Quarterly waves to convert remaining Level B/C objects to Level A

**Final transformation scorecard**

All KPIs in one dashboard: coverage %, Level A %, test pass rate, data reconciliation

## 15 The Progressive Journey

Clean Core is a journey, not a gate. You don't need 100% Level A to go live.

gantt title S/4HANA Migration — Progressive Clean Core Journey dateFormat YYYY-MM-DD axisFormat %b %Y section Layer 1-3 Analysis Complete AS-IS Discovery :a1, 2026-06-01, 10d S/4 Standard Delta :a2, after a1, 8d Fit-to-Standard + Triage :a3, after a2, 12d section Layer 4 Build Lane 1 Retire :b1, after a3, 5d Lane 3 Clean Core (L1-L2) :b2, after a3, 20d Lane 2 Remediate (L4-L5) :b3, after a3, 25d Config Migration :b4, after a3, 20d Integration Fixes :b5, after a3, 15d Data Prep :b6, after a3, 20d section Layer 5-6 Prove Certification & Validation :c1, after b3, 15d Handoff & Go-Live Package :c2, after c1, 10d section Post Go-Live Wave 1 Clean Core :d1, 2026-12-01, 60d Wave 2 Clean Core :d2, 2027-03-01, 60d Wave 3 Clean Core :d3, 2027-06-01, 60d

### Level A Progression Over Time

Milestone

Level A %

Level B %

Level C/D %

Retired %

**ECC Today**

5%

20%

75%

0%

**After Layer 4 (Go-Live)**

50-60%

15%

5-10%

30-40%

**+6 months (Wave 1)**

70%

8%

2%

30-40%

**+12 months (Wave 2-3)**

85-90%

3%

0%

30-40%

## 16 C16 Capability Matrix

Capability

Automation

Layer

Stream

Custom object discovery (TADIR scan)

Full Auto

L1

Code

Source code reading & requirement extraction

Full Auto

L1

Code

User exit & BAdI discovery

Full Auto

L1

Code

Configuration table reading

Full Auto

L1

Config

Org structure mapping

Full Auto

L1

Org

Integration interface discovery

Full Auto

L1

Integration

Data profiling & volume analysis

Full Auto

L1

Data

Simplification list mapping

Full Auto

L2

Code

Fiori app / Released API mapping

Full Auto

L2-L3

Code

Clean Core scoring (Level A/B/C/D)

Full Auto

L3

Code

Effort level classification (L1-L5)

Full Auto

L3

Code

Triage lane assignment

Full Auto

L3

Code

Fit-to-Standard per requirement

Full Auto

L3

Code

Remediated code generation

Full Auto

L4

Code

Clean Core code generation

Full Auto

L4

Code

SmartForm → Adobe conversion

Full Auto

L4

Code

ATC cloud readiness check

Full Auto

L5

Code

Interface E2E testing

Full Auto

L5

Integration

Config migration steps

Guided

L4

Config

Org remap decisions

Guided

L4

Org

Data cleansing execution

Guided

L4

Data

Data migration dry run

Guided

L5

Data

Business process redesign decisions

Advisory

L3

All

Go-live cutover execution

Advisory

L6

All

Full Auto C16 executes end-to-end without human intervention  |  Guided C16 produces the plan, customer team executes  |  Advisory C16 provides recommendations, customer makes decisions

## 17 Execution Timeline

Per process area (e.g., Sales Order, Procurement, Finance):

Phase

Layers

Duration

Activities

DISCOVER

Layer 1

2-3 days

Full system scan — all 5 streams

ANALYSE

Layers 2-3

3-5 days

Delta mapping, Fit-to-Standard, triage decisions

BUILD

Layer 4

2-4 weeks

Retire + Clean Core + Remediate + Config + Data

PROVE

Layers 5-6

1-2 weeks

Certification, testing, handoff package

#### Total per process area: 4-8 weeks

Traditional approach: 12-16 weeks with 2-3 consultants per process area.  
C16 approach: 4-8 weeks with 1 C16 instance + 1 functional consultant for review gates.

### For a Full Enterprise (20-30 processes)

Processes can be run in parallel (C16 handles multiple connections). With 3-4 parallel streams:

Scenario

Processes

Parallel Streams

Total Duration

Small enterprise

10 processes

2 parallel

~5-6 months

Medium enterprise

20 processes

3 parallel

~7-9 months

Large enterprise

30+ processes

4 parallel

~9-12 months

## 18 Cross-Process Examples

This framework is process-agnostic. Here's how it applies to 5 common enterprise processes:

🛒 Order-to-Cash (O2C) — Inquiry → Payment Received

Step

ECC Transaction

S/4 Fiori App

Typical Custom Objects

Inquiry/Quotation

VA11/VA21

F1805

Pricing exits, availability BAdIs

Sales Order

VA01

F1814

User exits MV45AFZZ, BDC uploads, custom fields

Delivery

VL01N

F1703

Packing logic, route determination, RFID

Billing

VF01

F0861

Custom invoice forms, tax routines

Payment

F-28/F110

F0711

Lockbox, payment allocation, dunning

🏭 Procure-to-Pay (P2P) — Requisition → Payment

Step

ECC Transaction

S/4 Fiori App

Typical Custom Objects

Purchase Requisition

ME51N

F1138

Approval workflows, auto-source BAdIs

Purchase Order

ME21N

F0842

BDC uploads, pricing exits, release strategy BAdIs

Goods Receipt

MIGO

F0843

Quality check, auto-GR, tolerance BAdIs

Invoice Verification

MIRO

F0859

3-way match, tolerance, parking logic

Payment

F110

F0740

Payment method selection, bank communication

📊 Record-to-Report (R2R) — Journal Entry → Financial Close

Step

ECC Transaction

S/4 Fiori App

Typical Custom Objects

Journal Entry

FB01/F-02

F0717

Substitution/validation rules, auto-posting

Period Close

FAGL\_FC\_VALUATION

F2217

Custom close cockpit, reconciliation programs

Consolidation

Various

Group Reporting

Intercompany elimination, currency translation

Reporting

FBL3N, S\_ALR\_\*

Embedded Analytics

Custom ALV reports, custom data extracts

🔧 Make-to-Order (MTO) — SO → Production → Delivery

Step

ECC Transaction

S/4 Fiori App

Typical Custom Objects

Sales Order (MTO)

VA01 (type TAK)

F1814

BOM explosion BAdIs, project ID fields

BOM Explosion

CS01/CS61

F3834

Custom BOM utilities, variant config

Production Order

CO01

F3201

Auto-creation logic, capacity BAdIs

Shop Floor

CO11N

F3013

Confirmation RFCs, yield tracking

Goods Issue

MIGO (261)

F0843

Backflush logic, serial number handling

👤 Hire-to-Retire (HR) — Recruitment → Separation

Step

ECC Transaction

S/4 / SuccessFactors

Typical Custom Objects

Recruitment

PA40

SuccessFactors RCM

Custom hiring workflows

Org Management

PPOME

SuccessFactors EC

Custom org reports, structure validation

Time Management

PA61/PT01

SuccessFactors Time

Custom time schemas, overtime rules

Payroll

PC00\_M\*

SuccessFactors Payroll / ECP

Custom payroll schemas, wage types

Separation

PA40 (action 10)

SuccessFactors EC

Custom offboarding workflows

## 19 Value Proposition

75%

Faster than traditional Fit-to-Standard

100%

Code coverage (every line read)

5

Parallel streams (not just code)

6

Layers — complete lifecycle

### Traditional vs 6-Layer Framework

Dimension

Traditional Approach

6-Layer Framework

**Discovery method**

Interviews + spreadsheets

Automated system scan — every object, every line

**Config coverage**

Often missed until go-live

First-class stream through all 6 layers

**Triage method**

Blanket "remediate all" or "Clean Core all"

Per-object triage with effort-level economics

**Clean Core economics**

"Clean Core costs 5x more"

"60% of objects cost the same — just do it right"

**Hidden enhancements**

Found at go-live when something breaks

Found in Layer 1 — every user exit, every BAdI, every append

**Hardcoded business rules**

Nobody knows they exist until they fail

Extracted from source code, documented, mapped to BRF+ or config

**Validation**

"Trust us" → go-live surprises

Layer 5 proves it works — ATC, E2E tests, dry runs, scorecards

**Post-go-live plan**

Vague "we'll improve later"

Quarterly waves with specific objects scheduled for Level A conversion

#### The Bottom Line

**Layer 1** tells you what you have.  
**Layer 2** tells you what S/4 changes regardless.  
**Layer 3** tells you what fits and what doesn't — and which lane each object takes.  
**Layer 4** does the work — retire, remediate, or build Clean Core — right the first time.  
**Layer 5** proves everything works.  
**Layer 6** packages it for go-live.  
  
Five streams. Six layers. No gaps. No surprises.

**C16 Enterprise 6-Layer S/4HANA Migration Framework** — Version 2.0

Generated by C16 — April 2026

This framework is process-agnostic and customer-agnostic. It can be applied to any SAP ECC or S/4HANA system, any business process, and any industry. The methodology, triage rules, and effort-level classifications are based on analysis of real enterprise migrations and SAP's cloudification catalog.

Corrections in v2.0: Clean Core level distribution (C/D not B/C), 5 parallel streams through all 6 layers, crossover economics for Level 1-2 effort, strategic guidance for Level B decisions, complete data flow contracts per stream.