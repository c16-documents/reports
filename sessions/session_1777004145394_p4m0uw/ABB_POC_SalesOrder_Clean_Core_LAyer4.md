  ABB — Layer 4: Clean Core Prompt Guide | Sales Order :root { --c16-purple: #6B3FA0; --c16-pink: #B5567A; --c16-copper: #C97B3A; --clean-core-gold: #F59E0B; --level-a: #10B981; --level-b: #3B82F6; --level-c: #F59E0B; --level-d: #EF4444; --released-badi: #8B5CF6; --key-user: #06B6D4; --brfplus: #EC4899; --btp: #14B8A6; --event-mesh: #F97316; } \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, Arial, sans-serif; background: #0F172A; color: #E2E8F0; line-height: 1.7; } .doc-wrapper { max-width: 1100px; margin: 0 auto; padding: 0 20px 60px; } /\* Header \*/ .header-banner { background: linear-gradient(135deg, #C97B3A 0%, #B5567A 40%, #6B3FA0 100%); padding: 50px 40px; margin: 0 -20px 40px; position: relative; overflow: hidden; } .header-banner::before { content: ''; position: absolute; top: -50%; right: -10%; width: 400px; height: 400px; background: radial-gradient(circle, rgba(255,255,255,0.08) 0%, transparent 70%); border-radius: 50%; } .header-banner::after { content: '✨'; position: absolute; top: 20px; right: 40px; font-size: 80px; opacity: 0.15; } .header-banner img { height: 45px; margin-bottom: 12px; border-radius: 8px; } .header-banner h1 { font-size: 2.2em; font-weight: 800; color: #fff; margin-bottom: 6px; letter-spacing: -0.5px; } .header-banner .subtitle { font-size: 1.15em; color: rgba(255,255,255,0.9); font-weight: 400; } .header-meta { display: flex; gap: 20px; margin-top: 20px; flex-wrap: wrap; } .header-meta .tag { background: rgba(255,255,255,0.15); color: #fff; padding: 4px 14px; border-radius: 20px; font-size: 0.85em; backdrop-filter: blur(4px); border: 1px solid rgba(255,255,255,0.2); } /\* Layer Progress \*/ .layer-progress { background: #1E293B; border: 1px solid #334155; border-radius: 16px; padding: 30px; margin-bottom: 40px; } .layer-progress h3 { color: #94A3B8; font-size: 0.85em; text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 20px; } .progress-track { display: flex; align-items: center; gap: 0; } .progress-step { flex: 1; text-align: center; position: relative; } .progress-step .circle { width: 48px; height: 48px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 8px; font-weight: 700; font-size: 1.1em; position: relative; z-index: 2; } .progress-step.done .circle { background: var(--level-a); color: #fff; } .progress-step.current .circle { background: linear-gradient(135deg, var(--clean-core-gold), #FBBF24); color: #1E293B; box-shadow: 0 0 20px rgba(245, 158, 11, 0.4); animation: pulse-gold 2s infinite; } .progress-step.pending .circle { background: #334155; color: #64748B; } @keyframes pulse-gold { 0%, 100% { box-shadow: 0 0 20px rgba(245, 158, 11, 0.4); } 50% { box-shadow: 0 0 35px rgba(245, 158, 11, 0.6); } } .progress-step .label { font-size: 0.8em; color: #94A3B8; } .progress-step.current .label { color: var(--clean-core-gold); font-weight: 600; } .progress-connector { height: 3px; flex: 0.5; background: #334155; margin-bottom: 28px; } .progress-connector.done { background: var(--level-a); } .progress-connector.active { background: linear-gradient(90deg, var(--level-a), var(--clean-core-gold)); } /\* Section Styles \*/ .section { margin-bottom: 45px; } .section-header { display: flex; align-items: center; gap: 14px; margin-bottom: 20px; } .section-number { width: 36px; height: 36px; background: var(--c16-purple); color: #fff; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 0.95em; flex-shrink: 0; } .section-header h2 { font-size: 1.5em; color: #F1F5F9; font-weight: 700; } .section-desc { color: #94A3B8; margin-bottom: 20px; font-size: 0.95em; padding-left: 50px; } /\* Target State Cards \*/ .target-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px; margin-bottom: 35px; } .target-card { background: #1E293B; border: 1px solid #334155; border-radius: 12px; padding: 24px; text-align: center; transition: all 0.3s; position: relative; overflow: hidden; } .target-card:hover { transform: translateY(-3px); border-color: #475569; } .target-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px; } .target-card.released-badi::before { background: var(--released-badi); } .target-card.key-user::before { background: var(--key-user); } .target-card.brfplus::before { background: var(--brfplus); } .target-card.btp::before { background: var(--btp); } .target-card.event-mesh::before { background: var(--event-mesh); } .target-card .icon { font-size: 2em; margin-bottom: 10px; } .target-card h4 { color: #F1F5F9; font-size: 1em; margin-bottom: 6px; } .target-card p { color: #94A3B8; font-size: 0.82em; line-height: 1.5; } /\* Prompt Boxes \*/ .prompt-box { background: #1E293B; border: 1px solid #334155; border-radius: 12px; margin-bottom: 30px; overflow: hidden; } .prompt-header { display: flex; align-items: center; gap: 12px; padding: 16px 24px; background: linear-gradient(135deg, rgba(107,63,160,0.15), rgba(181,86,122,0.1)); border-bottom: 1px solid #334155; } .prompt-id { background: var(--c16-purple); color: #fff; padding: 3px 12px; border-radius: 20px; font-size: 0.8em; font-weight: 700; white-space: nowrap; } .prompt-title { color: #F1F5F9; font-weight: 600; font-size: 1.05em; } .prompt-purpose { padding: 14px 24px; background: rgba(107,63,160,0.08); border-bottom: 1px solid #334155; } .prompt-purpose .label { color: var(--c16-pink); font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.2px; font-weight: 600; } .prompt-purpose p { color: #CBD5E1; font-size: 0.9em; margin-top: 4px; } .prompt-text { padding: 20px 24px; } .prompt-text pre { background: #0F172A; border: 1px solid #1E293B; border-left: 4px solid var(--c16-purple); border-radius: 8px; padding: 20px; font-family: 'Cascadia Code', 'Fira Code', 'Consolas', monospace; font-size: 0.88em; color: #E2E8F0; line-height: 1.8; white-space: pre-wrap; word-wrap: break-word; overflow-x: auto; } .prompt-note { padding: 12px 24px 16px; border-top: 1px solid #334155; background: rgba(245, 158, 11, 0.05); } .prompt-note .note-label { color: var(--clean-core-gold); font-size: 0.75em; text-transform: uppercase; letter-spacing: 1px; font-weight: 600; } .prompt-note p { color: #94A3B8; font-size: 0.85em; margin-top: 3px; } /\* Repeat Note \*/ .repeat-note { background: rgba(59, 130, 246, 0.08); border: 1px solid rgba(59, 130, 246, 0.2); border-radius: 10px; padding: 16px 20px; margin-bottom: 30px; } .repeat-note .icon { color: var(--level-b); font-weight: 600; } .repeat-note p { color: #94A3B8; font-size: 0.9em; margin-top: 4px; } /\* Warning Callout \*/ .warning-callout { background: rgba(245, 158, 11, 0.08); border: 1px solid rgba(245, 158, 11, 0.25); border-left: 4px solid var(--clean-core-gold); border-radius: 10px; padding: 18px 22px; margin: 25px 0; } .warning-callout .warning-title { color: var(--clean-core-gold); font-weight: 700; font-size: 0.95em; margin-bottom: 6px; } .warning-callout p { color: #CBD5E1; font-size: 0.9em; } /\* Critical Callout \*/ .critical-callout { background: rgba(239, 68, 68, 0.08); border: 1px solid rgba(239, 68, 68, 0.25); border-left: 4px solid var(--level-d); border-radius: 10px; padding: 18px 22px; margin: 25px 0; } .critical-callout .critical-title { color: var(--level-d); font-weight: 700; font-size: 0.95em; margin-bottom: 6px; } .critical-callout p { color: #CBD5E1; font-size: 0.9em; } /\* Success Callout \*/ .success-callout { background: rgba(16, 185, 129, 0.08); border: 1px solid rgba(16, 185, 129, 0.25); border-left: 4px solid var(--level-a); border-radius: 10px; padding: 18px 22px; margin: 25px 0; } .success-callout .success-title { color: var(--level-a); font-weight: 700; font-size: 0.95em; margin-bottom: 6px; } .success-callout p { color: #CBD5E1; font-size: 0.9em; } /\* Review Gate \*/ .review-gate { background: linear-gradient(135deg, rgba(139, 92, 246, 0.1), rgba(236, 72, 153, 0.08)); border: 1px dashed #8B5CF6; border-radius: 12px; padding: 20px 24px; margin: 25px 0; text-align: center; } .review-gate .gate-icon { font-size: 1.6em; margin-bottom: 6px; } .review-gate h4 { color: #C4B5FD; font-size: 0.95em; margin-bottom: 6px; } .review-gate p { color: #94A3B8; font-size: 0.85em; } /\* Tables \*/ table { width: 100%; border-collapse: collapse; margin: 16px 0; font-size: 0.9em; } thead th { background: var(--c16-purple); color: #fff; padding: 12px 16px; text-align: left; font-weight: 600; font-size: 0.85em; text-transform: uppercase; letter-spacing: 0.5px; } thead th:first-child { border-radius: 8px 0 0 0; } thead th:last-child { border-radius: 0 8px 0 0; } td { padding: 11px 16px; border-bottom: 1px solid #1E293B; color: #CBD5E1; } tr:nth-child(even) { background: rgba(30, 41, 59, 0.4); } tr:hover { background: rgba(107, 63, 160, 0.08); } /\* Level Badges \*/ .badge-a { background: var(--level-a); color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 600; } .badge-b { background: var(--level-b); color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 600; } .badge-c { background: var(--level-c); color: #1E293B; padding: 2px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 600; } .badge-d { background: var(--level-d); color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 600; } /\* Pillar Badges \*/ .badge-badi { background: var(--released-badi); color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 600; } .badge-keyuser { background: var(--key-user); color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 600; } .badge-brfplus { background: var(--brfplus); color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 600; } .badge-btp { background: var(--btp); color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 600; } .badge-evmesh { background: var(--event-mesh); color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 600; } /\* Target Architecture Diagram \*/ .arch-diagram { background: #1E293B; border: 1px solid #334155; border-radius: 12px; padding: 30px; margin: 25px 0; text-align: center; } .arch-diagram pre { font-family: 'Cascadia Code', 'Fira Code', monospace; font-size: 0.82em; color: #CBD5E1; line-height: 1.6; text-align: left; display: inline-block; } /\* Clean Core Scorecard \*/ .scorecard { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin: 25px 0; } .score-card { background: #1E293B; border: 1px solid #334155; border-radius: 12px; padding: 20px; text-align: center; } .score-card .score-label { color: #64748B; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 6px; } .score-card .score-before { font-size: 1.8em; font-weight: 800; margin-bottom: 4px; } .score-card .score-arrow { color: var(--level-a); font-size: 0.9em; margin-bottom: 4px; } .score-card .score-after { font-size: 1.8em; font-weight: 800; } .score-card.level-a .score-after { color: var(--level-a); } .score-card.level-b .score-after { color: var(--level-b); } /\* Five Pillars Visual \*/ .pillars-visual { display: grid; grid-template-columns: repeat(5, 1fr); gap: 12px; margin: 25px 0; } .pillar { background: #1E293B; border: 1px solid #334155; border-radius: 10px; padding: 18px 12px; text-align: center; position: relative; } .pillar::after { content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 3px; border-radius: 0 0 10px 10px; } .pillar:nth-child(1)::after { background: var(--released-badi); } .pillar:nth-child(2)::after { background: var(--key-user); } .pillar:nth-child(3)::after { background: var(--brfplus); } .pillar:nth-child(4)::after { background: var(--btp); } .pillar:nth-child(5)::after { background: var(--event-mesh); } .pillar .p-icon { font-size: 1.8em; margin-bottom: 8px; } .pillar .p-name { color: #F1F5F9; font-weight: 600; font-size: 0.85em; margin-bottom: 4px; } .pillar .p-desc { color: #64748B; font-size: 0.72em; line-height: 1.4; } /\* Flow Diagram \*/ .flow-section { background: #1E293B; border: 1px solid #334155; border-radius: 12px; padding: 30px; margin: 25px 0; } .flow-section h4 { color: #F1F5F9; margin-bottom: 16px; font-size: 1em; } /\* Follow-up section \*/ .followup-box { background: #1E293B; border: 1px solid #334155; border-radius: 12px; margin-bottom: 20px; overflow: hidden; } .followup-header { display: flex; align-items: center; gap: 12px; padding: 14px 20px; background: rgba(20, 184, 166, 0.08); border-bottom: 1px solid #334155; } .followup-id { background: var(--btp); color: #fff; padding: 3px 10px; border-radius: 20px; font-size: 0.75em; font-weight: 700; } .followup-title { color: #F1F5F9; font-weight: 600; font-size: 0.95em; } .followup-text { padding: 16px 20px; } .followup-text pre { background: #0F172A; border: 1px solid #1E293B; border-left: 4px solid var(--btp); border-radius: 8px; padding: 16px; font-family: 'Cascadia Code', 'Fira Code', monospace; font-size: 0.85em; color: #E2E8F0; line-height: 1.7; white-space: pre-wrap; word-wrap: break-word; } /\* Tips Table \*/ .tips-table td:first-child { color: var(--level-a); font-weight: 600; width: 45%; } .tips-table td:last-child { color: var(--level-d); opacity: 0.8; } /\* Footer \*/ .doc-footer { margin-top: 50px; padding: 30px; background: #1E293B; border: 1px solid #334155; border-radius: 12px; text-align: center; } .doc-footer p { color: #64748B; font-size: 0.85em; margin-bottom: 6px; } .doc-footer .brand { color: var(--c16-purple); font-weight: 700; } /\* Responsive \*/ @media (max-width: 768px) { .header-banner { padding: 30px 20px; } .header-banner h1 { font-size: 1.5em; } .target-grid { grid-template-columns: 1fr 1fr; } .pillars-visual { grid-template-columns: 1fr 1fr; } .scorecard { grid-template-columns: 1fr 1fr; } .progress-track { flex-wrap: wrap; } } @media print { body { background: #fff; color: #1E293B; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .prompt-text pre { border-color: #ccc; background: #f8f8f8; color: #1E293B; } }
...
C16 Prompt Guide · Sales Order Process · ABB POC

Layer 4 of 4 Target: Level A — Cloud Ready 5 Pillars 12 Prompts + 6 Follow-Ups April 2026
...
## What Layer 4 Achieves

Layer 3 made the code **S/4-compatible**. Layer 4 makes it **cloud-ready**. The difference: S/4-compatible code runs on S/4HANA but still uses classic APIs (Level B/C). Cloud-ready code uses only Released APIs (Level A) — it survives every future S/4 upgrade without touch.
...
Each prompt targets one or more of these pillars. Every custom object in the Sales Order process gets mapped to the right pillar.

🔌

Released BAdIs

Replace user exits & classic BAdIs with SAP-released extension points

👤

Key User Extensibility

Replace ZZ appends with no-code custom fields & logic

📐

BRF+

Replace hardcoded IF/ELSE with business-maintainable rules

☁️

BTP Extensions

Side-by-side extensions for logic that can't live in S/4 core

🔗

Event Mesh & Integration Suite

Replace custom RFC/IDoc with event-driven, decoupled integration
...
│                     SAP BTP                                  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │  CAP Side-by │  │  Integration │  │  Event Mesh  │      │
│  │  -Side Apps  │  │  Suite       │  │              │      │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘      │
│         │                 │                  │               │
└─────────┼─────────────────┼──────────────────┼───────────────┘
...
│         ▼                 ▼                  ▼               │
│  ┌──────────────────────────────────────────────────────┐   │
│  │              S/4HANA 2025 Clean Core                  │   │
│  │                                                       │   │
│  │  ┌────────────┐ ┌────────────┐ ┌────────────┐       │   │
│  │  │ Released   │ │ Key User   │ │   BRF+     │       │   │
│  │  │ BAdIs      │ │ Custom     │ │   Rules    │       │   │
│  │  │            │ │ Fields     │ │            │       │   │
│  │  └────────────┘ └────────────┘ └────────────┘       │   │
│  │                                                       │   │
│  │  ┌────────────────────────────────────────────────┐  │   │
│  │  │  Standard Process: Manage Sales Orders (F1814) │  │   │
│  │  │  + Released APIs (API\_SALES\_ORDER\_SRV)         │  │   │
│  │  │  + CDS Views (I\_SalesOrder, I\_SalesOrderItem)  │  │   │
│  │  └────────────────────────────────────────────────┘  │   │
│  └──────────────────────────────────────────────────────┘   │
│                     S/4HANA System                            │
└──────────────────────────────────────────────────────────────┘
...
**Layers 1-3 must be completed first.** Layer 4 prompts reference the inventory from Layer 1, the FIT/GAP classifications from Layer 2, and the remediated code from Layer 3. If you skip layers, C16 will lack the context to produce accurate Clean Core recommendations.

P

## Core Prompts (P1 — P12)

P1 Clean Core Baseline — Score Everything

Purpose

Before you can improve, you need to measure. This prompt scores every custom object found in Layers 1-3 against SAP's Clean Core classification.

Take every custom object discovered in the Sales Order process 
(from our Layer 1 process map and Layer 3 remediation).

For each object, classify its Clean Core level:
- Level A: Uses only Released APIs — cloud-ready
- Level B: Uses classic stable APIs (BAPIs, REUSE\_ALV) — works on S/4 on-prem, not cloud
- Level C: Direct standard table access, unreleased FMs — non-compliant
- Level D: Deprecated patterns, user exits, modifications — critical

Show the results as:
1. Summary scorecard: count and % at each level (A/B/C/D)
2. Full inventory table: Object | Type | Process Step | Current Level | Reason | Target Level
3. The "progression KPI": what % is at Level A today vs target

Don't generate a document yet — show me the scorecard first.

What C16 Does

Runs 3-layer classification engine: SAP cloudification catalog + ATC ABAP\_CLOUD\_READINESS + Simplification List. Returns per-object Level A/B/C/D with specific evidence.

🔍

#### Review Gate — Baseline Validation

Review the scorecard. Confirm the current-state numbers before proceeding. This is the "before" snapshot for your board-level KPI.

P2 Pillar 1 — Released BAdI Mapping

Purpose

User exits and classic BAdIs are the #1 reason code stays at Level C/D. This prompt finds the Released BAdI replacement for each one.

For every user exit and classic BAdI implementation in the Sales 
Order process, find the Released BAdI replacement in S/4HANA 2025.

For each one show:
1. Current enhancement: name, type (user exit / classic BAdI / enh. impl)
2. Business purpose: what it does (from the source code you already read)
3. Released BAdI replacement: exact BAdI name, enhancement spot
4. Verification: confirm the BAdI is NOT marked INTERNAL in SXS\_ATTR
5. Migration complexity: Low (direct port) / Medium (logic restructure) / High (redesign)
6. What changes in the code: key differences between old and new interface

Group by process step. Table format.

Important: only recommend BAdIs where INTERNAL flag = space (not 'X'). 
If a user exit has NO released BAdI equivalent, flag it as 
"Requires BTP side-by-side extension" — don't force-fit.

Don't generate a document yet.

What C16 Does

Searches SXS\_ATTR for SD BAdIs, filters out INTERNAL='X', maps each user exit to its Released successor. Verifies the BAdI interface is compatible with the custom logic.

P3 Pillar 1 — Generate Released BAdI Implementation Code

Purpose

Don't just map — produce the actual code. For each Released BAdI identified in P2, generate the implementation class.

For each Released BAdI mapping from the previous step where 
complexity is Low or Medium:

Generate the complete BAdI implementation class:
1. CLASS definition implementing the BAdI interface
2. Method implementation porting the business logic from the old 
   user exit / classic BAdI
3. Use ONLY Released APIs inside the implementation 
   (no direct table SELECT on standard tables — use CDS views)
4. Include proper error handling (cx\_\* exception classes)
5. Add inline comments explaining what was ported from where

For High complexity items: don't generate code yet — just document 
the design approach and what needs manual architect review.

Show me the code for each implementation. Don't generate a document yet.

⚠️ 80/20 Rule on BAdI Migration

Not every user exit needs a Released BAdI. Some user exits contain logic that should simply be **retired** because S/4 standard handles it now (identified in Layer 2 as FIT/FIT WITH CONFIG). Only migrate exits where the business logic is genuinely custom and has no standard equivalent. C16 will flag which exits to retire vs which to migrate.

P4 Pillar 2 — Key User Extensibility for Custom Fields

Purpose

ZZ/YY append fields on VBAK, VBAP etc. are Level D modifications. Key User Extensibility replaces them with no-code custom fields that are upgrade-safe.

For every custom append field (ZZ\*/YY\*) found on Sales Order 
tables (VBAK, VBAP, VBEP, VBKD, LIKP, LIPS, VBRK, VBRP):

Map each field to S/4HANA Key User Extensibility:
1. Current field: table, field name, data type, description
2. Where it's used: which programs/enhancements read or write this field
3. Key User replacement: 
   - Can it be a Key User custom field? (Yes for simple fields)
   - Does it need a custom CDS extension view? (Yes for complex logic)
   - Does it need a Released BAdI for population logic?
4. Data migration: how does the field value move from ZZ append 
   to Key User field during migration
5. Downstream impact: which reports, interfaces, forms use this field

Table format grouped by table. Don't generate a document yet.

Key Insight

Key User custom fields are maintained in Fiori app "Custom Fields and Logic" (F1481). They automatically appear in standard Fiori apps, CDS views, OData APIs, and analytics — no developer needed after setup.

🔍

#### Review Gate — Custom Fields Sign-Off

This is a business decision. Some ZZ fields may no longer be needed in S/4. ABB's functional team must confirm: keep, migrate to Key User, or retire each field.

P5 Pillar 3 — BRF+ Rule Extraction

Purpose

Hardcoded IF/ELSE business rules in ABAP are fragile and developer-dependent. BRF+ puts rules in the hands of business users. But not every IF/ELSE is a business rule — some are program flow control.

From the source code analysis in Layers 1-3, you identified 
hardcoded business rules in the Sales Order enhancements.

For each hardcoded rule, classify it:

Category A — BRF+ Candidate (business-owned rule that changes):
  Examples: credit thresholds per company code, pricing tier logic,
  approval limits, plant determination rules, customer-specific terms

Category B — Standard Config (should be customizing, not BRF+):
  Examples: tax determination, output condition, partner determination,
  pricing procedure assignment — these have standard config tables

Category C — Program Logic (not a business rule — leave as code):
  Examples: error handling, data validation format checks, 
  loop control, null checks

For each Category A rule:
1. Current code: show the actual IF/ELSE block from source
2. Business rule description: what decision is being made
3. BRF+ design: decision table structure (conditions → result)
4. Who owns it: which business role should maintain this rule
5. Frequency of change: how often does this rule change

Table format. Don't generate a document yet.

🚫 Not Every IF/ELSE Needs BRF+

Over-classifying rules as BRF+ candidates is a common consulting mistake. It creates unnecessary complexity and BRF+ maintenance burden. C16 explicitly separates Category A (real business rules that change) from Category B (standard config) and Category C (program logic). Only Category A goes to BRF+.

P6 Pillar 3 — Generate BRF+ Decision Table Design

Purpose

For each Category A rule from P5, design the BRF+ decision table structure that replaces the hardcoded ABAP.

For each Category A (BRF+ Candidate) rule identified:

Design the BRF+ decision table:
1. Function name and description
2. Context parameters (input): field names, types, source
3. Result parameters (output): what the rule returns
4. Decision table columns: condition 1, condition 2... → result
5. Sample rows: 3-5 example rows showing how current IF/ELSE 
   maps to decision table entries
6. ABAP integration: the calling code that invokes BRF+ 
   (FDT\_FUNCTION\_PROCESS) replacing the old IF/ELSE block

Show the before (current ABAP IF/ELSE) and after (BRF+ call + 
decision table) side by side for each rule.

Don't generate a document yet.

What C16 Does

Extracts the actual IF/ELSE conditions from source code, maps them to decision table columns, generates the ABAP calling code. BRF+ table configuration itself is done in transaction BRF+ (BRFPLUS) by business users — C16 designs the structure, not the runtime config.

🔍

#### Review Gate — BRF+ Ownership

Critical question: who in ABB will OWN these BRF+ rules going forward? Business analyst? Product manager? Plant controller? This must be decided before implementation — BRF+ only delivers value if business users actually maintain it.

P7 Pillar 4 — BTP Side-by-Side Extension Design

Purpose

Some custom logic genuinely cannot live inside S/4's clean core — complex integrations, external API orchestration, ML-driven logic. These become BTP side-by-side extensions.

From the Layer 2 GAP items and P2 "High complexity" items that 
cannot be handled by Released BAdIs or Key User Extensibility:

For each one, design the BTP extension:
1. What logic moves to BTP (and why it can't stay in S/4 core)
2. Extension type: CAP application / Integration Suite flow / 
   Workflow Management / custom Fiori app
3. Communication: how does it connect to S/4?
   - API: which Released API does it call
   - Events: which S/4 business events does it subscribe to
   - Protocol: OData V4 / SOAP / REST / Event Mesh
4. Data flow: what data goes out to BTP, what comes back
5. User experience: embedded in S/4 Fiori Launchpad or 
   standalone BTP app
6. Rough architecture diagram (describe in text)

Important: BTP is the LAST resort, not the first. If a Released 
BAdI or Key User Extensibility can handle it, it should NOT go 
to BTP. Only genuinely cross-system or externally-facing logic 
belongs here.

Don't generate a document yet.

⚠️ The 80/20 Rule on BTP

In a typical Sales Order process, **80-90% of custom logic** should stay in S/4 core (Released BAdIs + Key User + BRF+). Only 10-20% should go to BTP. If C16's analysis shows more than 20% heading to BTP, challenge whether Released BAdIs were properly explored. Over-classifying objects as "needs BTP" is the most expensive mistake in Clean Core projects.

P8 Pillar 5 — Integration Modernisation

Purpose

Custom RFC wrappers, IDoc extensions, and file-based interfaces are Level C/D. Event Mesh and Integration Suite replace them with decoupled, event-driven patterns.

For every integration point in the Sales Order process 
(identified in Layer 1, Prompt 5):

Map each to the Clean Core integration pattern:

1. Current interface: name, direction (in/out), protocol 
   (RFC/IDoc/file/OData), partner system
2. Current Clean Core level: C or D (explain why)
3. Target pattern — choose one:
   a) S/4 Released API (direct call) — for simple read/write
   b) Event Mesh (pub/sub) — for event-driven, fire-and-forget
   c) Integration Suite (orchestration) — for complex mapping/routing
   d) CPI-DS / Datasphere — for analytics/reporting feeds
4. S/4 business event: which standard event replaces the custom trigger
   (e.g., SalesOrder.Created, SalesOrder.Changed, DeliveryCreated)
5. Migration approach: big bang vs parallel run vs strangler pattern
6. Risk: what breaks if this interface fails

Table format grouped by direction (inbound / outbound / cross-module).
Don't generate a document yet.

P9 Output Management — Forms Migration

Purpose

Custom SmartForms and SAPscript are Level D. S/4HANA 2025 uses Output Management with Adobe Forms or BTP Forms Service.

For every custom output form in the Sales Order process 
(order confirmation, delivery note, invoice, pro forma):

Map each to the S/4HANA 2025 output management target:
1. Current form: name, type (SmartForm/SAPscript/Adobe), 
   output type (NACE condition), trigger
2. Target option:
   a) Standard S/4 form (if SAP delivers one for this doc type)
   b) Custom Adobe Form via Output Management (F3809)
   c) BTP Forms Service (for complex/branded documents)
3. Can C16 auto-convert the SmartForm to Adobe? (Yes/No)
4. Custom elements: logos, ABB-specific layout, special calculations
5. Integration: email, EDI, print, archive (which channels)

For SmartForms that C16 can auto-convert: note this as a 
Layer 3 quick win that feeds into Layer 4 compliance.

Don't generate a document yet.

P10 Revised Clean Core Scorecard — After State

Purpose

Now recalculate. With all 5 pillars applied, what's the new Clean Core score? This is the KPI ABB's board cares about.

Based on all the mappings from P2-P9, recalculate the Clean Core 
scorecard for every custom object in the Sales Order process.

Show:
1. BEFORE scorecard (from P1): count and % at Level A/B/C/D
2. AFTER scorecard: count and % at Level A/B/C/D after all 
   pillar transformations are applied
3. Per-object movement table: Object | Before Level | After Level | 
   What Changed (Released BAdI / Key User / BRF+ / BTP / Retired)
4. Objects that REMAIN at Level B or C: why, and is that acceptable
5. The progression KPI: "Level A coverage moved from X% to Y%"

This is the hero metric for the Clean Core transformation.
Don't generate a document yet.

📊

#### Review Gate — Before/After Validation

This is the moment of truth. Review the progression KPI. If the "after" score isn't where ABB needs it, discuss which Level B/C objects need further investment — or whether staying at Level B is acceptable for on-prem.

P11 C16 vs Manual — Boundary Definition

Purpose

Transparency. ABB needs to know exactly what C16 delivers automatically vs what requires manual work by their team or partners.

For the complete Layer 4 Clean Core transformation, create an 
honest "C16 vs Manual" boundary table:

For each activity show:
1. Activity description
2. C16 delivers: what C16 produces automatically 
   (code, design, analysis, document)
3. ABB team does: what requires human decision, configuration, 
   or manual implementation
4. Partner/SAP does: what needs SAP support, Basis, or 
   implementation partner

Categories to cover:
- Released BAdI implementation code
- Key User custom field creation
- BRF+ decision table configuration  
- BTP extension deployment
- Event Mesh / Integration Suite setup
- Output Management configuration
- Data migration for custom fields
- Testing and validation
- Go-live cutover

Be honest. Don't overclaim what C16 automates.
Don't generate a document yet.

P12 Generate the Layer 4 Clean Core Document

Purpose

Compile everything from P1-P11 into the final Layer 4 deliverable. This is the Clean Core transformation blueprint.

Now generate the Layer 4 Clean Core document for the Sales Order process.

Structure:
1. Executive Summary — progression KPI (before/after Level A %), 
   key decisions, scope
2. Clean Core Baseline — current Level A/B/C/D scorecard with 
   full inventory table
3. Target Architecture — diagram showing S/4 core + Released BAdIs 
   + Key User + BRF+ + BTP + Event Mesh
4. Pillar 1: Released BAdIs — mapping table + implementation code 
   for each migration
5. Pillar 2: Key User Extensibility — custom field migration plan 
   with data migration approach
6. Pillar 3: BRF+ — rule extraction with decision table designs 
   and before/after code comparison
7. Pillar 4: BTP Extensions — architecture for side-by-side 
   extensions with API/event specifications
8. Pillar 5: Integration Modernisation — interface-by-interface 
   migration to Event Mesh / Integration Suite
9. Output Management — forms migration with auto-conversion status
10. Revised Scorecard — before/after Level A/B/C/D with per-object 
    movement table
11. C16 vs Manual Boundary — honest capability matrix
12. Implementation Roadmap — phased Gantt with dependencies across 
    all 5 pillars
13. Risk Register — what could go wrong per pillar, mitigation

Use Mermaid diagrams for architecture and roadmap.
Include the progression KPI prominently in the executive summary.
Include C16 header with system details.

F

## Follow-Up Prompts (F1 — F6)

Use these after the core sequence to drill deeper or expand scope.

F1 Generate the Released BAdI Implementation — Deploy-Ready

Pick the top 3 highest-impact Released BAdI migrations from P2 
and generate COMPLETE, deployable ABAP classes for each one.

Deploy them to the system so we can demonstrate live working 
Clean Core code — not just a document.

F2 Cross-Layer Summary — One Object, Four Layers

Pick one significant custom object (e.g., the main Sales Order 
user exit or the custom pricing routine) and show how it flows 
through all four layers:

Layer 1: Where it sits in the process map
Layer 2: Its FIT/GAP classification against S/4 standard
Layer 3: The S/4 remediation applied (syntax, BP, obsolete FMs)
Layer 4: The Clean Core transformation (Released BAdI, BRF+, etc.)

Show the complete journey of this one object — from ECC as-is 
to S/4 Clean Core target.

F3 CAP Extension Prototype

For the highest-priority BTP side-by-side extension identified 
in P7, build a working CAP prototype:

1. CDS data model consuming S/4 Sales Order API
2. Service layer with custom business logic
3. Fiori Elements UI
4. Event handler subscribing to S/4 business event

Run it locally so we can demonstrate the BTP extension 
pattern live.

F4 Extend to Full O2C

Extend the Layer 4 Clean Core analysis to cover the full 
Order-to-Cash cycle:

Add: Customer Master (BP), Contracts (VA41), Scheduling 
Agreements (VA31), Returns, Credit Memos, Dunning, 
Revenue Recognition.

Same 5-pillar analysis per process step. Show cumulative 
progression KPI across all O2C steps.

F5 Board-Level Executive Summary

Generate a one-page executive summary for ABB leadership:

1. One KPI: "Level A coverage: X% → Y%"
2. One sentence per pillar: what was done, what it means
3. Investment vs value: what this saves in future upgrade cycles
4. Risk: what happens if we DON'T do Clean Core
5. Timeline: when each pillar completes

No technical jargon. No ABAP. No BAdI names. 
Pure business language.

F6 smartShift Integration — Bulk vs Intelligent

Define the boundary between C16 and smartShift for Layer 4:

- What objects are best handled by smartShift bulk conversion?
- What objects need C16's intelligent analysis (BRF+ extraction, 
  BAdI mapping, BTP design)?
- Where do they overlap? How do outputs feed each other?
- Recommended workflow: which tool runs first, handoff points

This helps ABB's tooling strategy — not either/or, but both.

E

## Execution Sequence

#### Recommended Order With Review Gates
...
**P4** — Key User Custom Fields

15-20 min

🔍 Business sign-off on field decisions

5

**P5** — BRF+ Rule Extraction
...
**P6** — BRF+ Decision Table Design

20-30 min

🔍 BRF+ ownership decision

7

**P7** — BTP Extension Design
...
**P10** — Revised Scorecard

15-20 min

📊 Before/After KPI review

11

**P11** — C16 vs Manual Boundary
...
**P12** — Generate Document

15-20 min

Final deliverable

**Total estimated time:** 3.5-5 hours of interactive prompting across 1-2 sessions.
...
Review each prompt output before proceeding

Paste all 12 prompts at once

Challenge BTP classifications — "does this really need BTP?"

Accept every BTP recommendation without questioning

Validate BRF+ candidates — "does this rule actually change?"

Move every IF/ELSE to BRF+

Confirm custom field decisions with business owners

Let IT decide which ZZ fields to keep or retire

Ask C16 to verify Released BAdI INTERNAL flag

Assume all BAdIs are available for customer use

Use "show me the code" for high-priority items

Accept design-only output when you need implementation

Run F2 (cross-layer trace) to validate the full journey

Trust each layer independently without tracing through

Keep Level B as acceptable for on-prem S/4 objects

Force everything to Level A when on-prem allows Level B
...
Level A/B/C/D scorecard + full inventory

% at Level A (before)

**P2** BAdI Map

User exit → Released BAdI mapping table

Count of migrations needed

**P3** BAdI Code

Complete implementation classes

Lines of Clean Core code generated

**P4** Key User

Custom field migration plan

Count of fields to migrate/retire

**P5** BRF+ Extract

Categorised rule list (A/B/C)

Count of genuine BRF+ candidates

**P6** BRF+ Design

Decision table structures + before/after code

Rules externalised

**P7** BTP Design

Extension architecture per side-by-side app

Count of BTP extensions (should be small)

**P8** Integration

Interface migration table

Count of interfaces modernised

**P9** Forms

Form migration plan with auto-convert status

Forms converted vs manual

**P10** Revised Score

Before/after scorecard + movement table

% at Level A (after) — the hero KPI

**P11** Boundary

C16 vs Manual vs Partner matrix

% automated by C16

**P12** Document

Complete HTML report, 10,000-15,000 words

The deliverable

→

## What Comes After Layer 4

✅ Layer 4 Completes the Transformation Blueprint

After Layer 4, ABB has a complete, code-level transformation plan for the Sales Order process — from ECC as-is to S/4HANA 2025 Clean Core. The four deliverables together form the **implementation specification**:
...
Process Map — what exists today

Migration scoping & estimation

Layer 2

Fit-to-Standard — what S/4 replaces

Retirement list + gap register

Layer 3

Remediated Code — S/4 compatible

Technical migration execution

Layer 4

Clean Core Blueprint — cloud ready
...
Validate Layer 4 findings with business process owners
...
Decide on Level B acceptance for on-prem objects

ABB Architecture Board

P10 scorecard

3

Assign BRF+ rule ownership to business roles

ABB Business Analysts

P6 design

4

Provision BTP subaccount for extension development

ABB Basis / Cloud Team

P7 architecture

5

Begin implementation — Released BAdIs first (highest ROI)
...
Replicate this 4-layer analysis for next process area

ABB + C16

—

C16 — Layer 4: Clean Core Prompt Guide

Sales Order Process · ABB POC · April 2026

This guide defines the prompt sequence for achieving SAP Clean Core compliance on S/4HANA 2025. Layers 1-3 are prerequisites. Each prompt builds on the previous — execute in sequence with review gates.
