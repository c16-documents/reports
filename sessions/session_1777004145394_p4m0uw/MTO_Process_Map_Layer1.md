  Layer 1 — Make-to-Order Process Map | C16 \* { box-sizing: border-box; margin: 0; padding: 0; } body { font-family: 'Segoe UI', Arial, sans-serif; color: #2c3e50; background: #e9ecef; line-height: 1.7; } .doc-wrapper { max-width: 1120px; margin: 0 auto; background: #fff; box-shadow: 0 2px 24px rgba(0,0,0,0.10); } .header-banner { background: linear-gradient(135deg, #C97B3A, #B5567A, #6B3FA0); color: #fff; padding: 40px 48px; } .header-banner img { height: 50px; margin-bottom: 12px; border-radius: 8px; } .header-banner h1 { font-size: 2em; margin-bottom: 6px; } .header-banner .subtitle { font-size: 1.15em; opacity: 0.92; } .header-banner .badge { display: inline-block; background: rgba(255,255,255,0.22); padding: 4px 14px; border-radius: 20px; font-size: 0.82em; margin-top: 10px; } .meta-bar { background: #f8f9fa; padding: 18px 48px; border-bottom: 2px solid #e2e8f0; display: flex; flex-wrap: wrap; gap: 28px; font-size: 0.92em; } .meta-bar .meta-item { display: flex; flex-direction: column; } .meta-bar .meta-label { color: #6B3FA0; font-weight: 600; font-size: 0.8em; text-transform: uppercase; letter-spacing: 0.5px; } .content { padding: 36px 48px; } .toc { background: #f0f4f8; border-radius: 10px; padding: 24px 32px; margin-bottom: 32px; } .toc h3 { color: #6B3FA0; margin-bottom: 12px; } .toc ol { padding-left: 22px; } .toc li { margin-bottom: 5px; } .toc a { color: #6B3FA0; text-decoration: none; } .toc a:hover { text-decoration: underline; } .section-title { color: #6B3FA0; font-size: 1.5em; margin: 40px 0 18px 0; padding-bottom: 8px; border-bottom: 3px solid #B5567A; display: flex; align-items: center; gap: 12px; } .section-title .num { display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; background: #B5567A; color: #fff; border-radius: 50%; font-size: 0.7em; font-weight: 700; } h3 { color: #4a3875; margin: 22px 0 10px 0; font-size: 1.15em; } table { width: 100%; border-collapse: collapse; margin: 16px 0 24px 0; font-size: 0.92em; } thead { background: #6B3FA0; color: #fff; } th { padding: 10px 14px; text-align: left; font-weight: 600; } td { padding: 9px 14px; border-bottom: 1px solid #e2e8f0; } tr:nth-child(even) { background: #f8fafc; } tr:hover { background: #edf2f7; } .exec-summary { border-left: 5px solid #27ae60; background: #f6fef9; padding: 20px 28px; border-radius: 0 10px 10px 0; margin: 18px 0 28px 0; } .kpi-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px; margin: 20px 0 28px 0; } .kpi-card { background: linear-gradient(135deg, #f5f3ff, #ede9fe); border-radius: 12px; padding: 20px; text-align: center; border: 1px solid #ddd6fe; } .kpi-card .kpi-value { font-size: 2.2em; font-weight: 700; color: #6B3FA0; } .kpi-card .kpi-label { font-size: 0.85em; color: #64748b; margin-top: 4px; } .process-step { background: #f8f9fa; border-left: 4px solid #6B3FA0; padding: 16px 24px; margin: 14px 0; border-radius: 0 8px 8px 0; } .process-step h4 { color: #6B3FA0; margin-bottom: 8px; } .badge-pp { background: #8b5cf6; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-sd { background: #3b82f6; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-mm { background: #f59e0b; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-fi { background: #10b981; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-plm { background: #ec4899; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-qm { background: #14b8a6; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-wm { background: #f97316; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-integ { background: #6366f1; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-output { background: #e11d48; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .cat-card { display: inline-block; background: #f1f5f9; padding: 6px 14px; border-radius: 8px; margin: 3px; font-size: 0.88em; border: 1px solid #e2e8f0; } .highlight-box { background: linear-gradient(135deg, #fdf4ff, #fce7f3); border: 1px solid #f0abfc; border-radius: 10px; padding: 20px; margin: 18px 0; } .mermaid { margin: 20px 0; } details { margin: 12px 0; } summary { background: #f0f4f8; padding: 10px 16px; border-radius: 8px; cursor: pointer; font-weight: 600; color: #4a3875; } summary:hover { background: #e2e8f0; } .doc-footer { background: #1e1b4b; color: #c4b5fd; padding: 28px 48px; font-size: 0.85em; } .doc-footer a { color: #a78bfa; } @media print { .doc-wrapper { box-shadow: none; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } } .coverage-fit { background: #27ae60; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .coverage-config { background: #3498db; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .coverage-extension { background: #e67e22; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .coverage-gap { background: #e74c3c; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.85em; } .region-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 16px; margin: 18px 0; } .region-card { border: 1px solid #e2e8f0; border-radius: 10px; padding: 18px; } .region-card h4 { color: #6B3FA0; margin-bottom: 8px; }

![C16](data:image/jpeg;base64,)

# Layer 1 — Make-to-Order Process Map

Custom Code & Configuration Mapped to MTO Process Steps

C16 Process Intelligence • April 2026 • ECC 7.40

SystemECC 7.40 • Client 800

Company Codes50+ (DE, US, GB, IN, FR, CN, JP, etc.)

Plants50+ globally (Hamburg, Berlin, New York, Beijing, Mumbai...)

Custom Objects Scanned7,051

MTO-Relevant Objects187

Generated24 April 2026

### 📑 Table of Contents

1.  [Executive Summary](#executive-summary)
2.  [Organizational Structure Map](#org-structure)
3.  [End-to-End MTO Process Flow](#mto-process-flow)
4.  [Step 1: Sales Order Entry (SD)](#step1)
5.  [Step 2: BOM Explosion & Engineering (PLM/PP)](#step2)
6.  [Step 3: MRP & Material Staging (PP/MM)](#step3)
7.  [Step 4: Production Order Creation & Release (PP)](#step4)
8.  [Step 5: Shop Floor Execution & Confirmation (PP)](#step5)
9.  [Step 6: Goods Issue / Goods Receipt (MM/WM)](#step6)
10.  [Step 7: Delivery & Despatch (SD/WM)](#step7)
11.  [Step 8: Billing & Revenue Recognition (SD/FI)](#step8)
12.  [Step 9: Integration & Middleware (XI/IDoc/OData)](#step9)
13.  [Step 10: Output & Forms (SmartForms/Adobe)](#step10)
14.  [Enhancement & BAdI Map](#enhancement-map)
15.  [Test Automation Inventory](#test-automation)
16.  [OData Service Inventory](#odata-services)
17.  [Full Object Inventory by Category](#object-by-category)
18.  [Recommendations for Layers 2–4](#recommendations)

## 1 Executive Summary

This document maps **187 custom objects** discovered across your BestRun ECC 7.40 system to the **Make-to-Order (MTO) process steps** — from Sales Order Entry through Billing. The system spans **50+ company codes** across 20+ countries (DE, US, GB, IN, FR, CN, JP, SG, KR, CA, BR, MX, NL, CH, ES, IT, PT, SE, TH, MY, ID, PH) with **50+ plants** and **50+ sales organizations**.

The MTO footprint includes **production order management** (ZPROD\_ORDERS\_DESPATCH, ZODT\_PRODUCT\_ORDER), **BOM management** (ZBOM\_CREATE transaction, 6 BAdI implementations for BOM processing), **MRP test data utilities** (ZCL\_PP\_MRP\_TEST\_DATA\_HELPER), **pricing enhancements** (ZES\_PRICING\_CPF), **OData services for production orders and goods movement**, and **13 CO01 CATT/eCATT test scripts**. A dedicated package **ZPROD\_SUPERVISOR** contains production capacity integration classes.

187

MTO-Relevant Custom Objects

50+

Plants Configured

50+

Company Codes

8

MTO Process Steps Mapped

6

BOM BAdI Implementations

13

CO01 Test Scripts

30+

OData Services

30+

SmartForms

## 2 Organizational Structure Map

The MTO process runs across a global multi-company BestRun structure. Key production-relevant regions:

#### 🇩🇪 Germany (Core Manufacturing)

**Company Code**

1000 — BestRun Germany (EUR)

**Plants**

1000 Hamburg, 1100 Berlin, 1200 Dresden, 1300 Frankfurt, 1400 Stuttgart, 1500 München, 1600 Köln

**Sales Org**

1000, 1020, 1030

**Purch Org**

1000 — Best Run Germany

#### 🇺🇸 United States

**Company Code**

3000 — BestRun USA (USD)

**Plants**

3000 New York, 3100 Chicago, 3050 UK Subsidiary

**Sales Org**

3000, 3020, 3110

**Purch Org**

3000 — Best Run USA

#### 🇬🇧 United Kingdom

**Company Code**

2000 — BestRun UK (GBP)

**Plants**

2000 Heathrow/Hayes, 2010 DC London

**Sales Org**

2000

**Purch Org**

2000 — Best Run UK

#### 🇮🇳 India

**Company Code**

1010 BestRun India (INR), 1710, 4300

**Plants**

1010, 2830 DC Mumbai

**Sales Org**

4300

**Purch Org**

4300 — Best Run India

#### 🇨🇳 China

**Company Code**

2800 BestRun China (CNY), 2820

**Plants**

2800 Beijing, 2810 Shanghai, 2835 Tai'an, 2840 Hongkong

**Sales Org**

2800, 2820

**Purch Org**

2800, 2820

#### 🌍 Other Regions

**France**

CC 2200, Plants: Paris 2200, Lyon 2210, Nantes 2220

**Japan**

CC 5000/5200, Sales Org 5000

**Singapore**

CC 1002/5100, Sales Org 5100

**Canada**

CC 4000/4500, Sales Org 4000

**Switzerland**

CC 2700, Plants: Biel, Lausanne, Regensdorf

### Purchasing Groups (T024)

30 purchasing groups configured (000–029): Chef H., Dickens B., Harnisch H., IDES USA, Eiffel J., Diller M., Sommer St., Lux L., Zuse K., Müller K., Davis L., Edwards S., Frank T., Garner J., Johnson P., Lovelle D., Myers C., Norman B., O'Connor A., Parker J., Roth P., Stevens K., Thompson V., Integra T., Erlenmeier K., Bering H., Sanchez, Interthal B., Napiontek T., Borchers I.

## 3 End-to-End MTO Process Flow

The Make-to-Order process follows this end-to-end flow. Each step below is annotated with the custom objects discovered in your system.

graph LR A\["📋 Sales Order  
VA01/VA02"\] --> B\["🔧 BOM Explosion  
CS01/CS02"\] B --> C\["📊 MRP Run  
MD01/MD02"\] C --> D\["🏭 Production Order  
CO01/CO02"\] D --> E\["⚙️ Shop Floor  
CO11N/CO15"\] E --> F\["📦 Goods Issue/Receipt  
MIGO/MB1A"\] F --> G\["🚚 Delivery & Despatch  
VL01N/VL02N"\] G --> H\["💰 Billing  
VF01/VF02"\] style A fill:#3b82f6,color:#fff style B fill:#ec4899,color:#fff style C fill:#8b5cf6,color:#fff style D fill:#8b5cf6,color:#fff style E fill:#8b5cf6,color:#fff style F fill:#f59e0b,color:#fff style G fill:#3b82f6,color:#fff style H fill:#10b981,color:#fff

## 4 Step 1: Sales Order Entry SD

The entry point for MTO. A customer-specific sales order triggers the entire production chain. Your system contains extensive custom sales order processing.

Object

Type

Package

Description

Created

ZALV\_SALES\_ORDER

PROG

ZDEMO

ALV Sales Order Display

2025-01-11

ZALV\_SALES\_ORDERS

PROG

ZDEMO

ALV Sales Orders List

2025-03-02

ZSALESORDERDETAILS

WDYN

$TMP

WebDynpro Sales Order Details

2024-12-21

ZSALESORDER

WAPA (BSP)

ZUI5

SAPUI5 Sales Order App

2013-12-16

ZSALESORDERBW

WAPA (BSP)

ZUI5

SAPUI5 Sales Order BW

2014-01-27

ZCL\_SALES\_ORDER

CLAS

ZDEVELOPER25

Sales Order OO Class

2025-01-08

ZCL\_WF\_SALESORDER

CLAS

ZRISHAB

Workflow for Sales Orders

2023-06-04

ZCLASS\_SALES\_ORDER\_DETAILS\_100

CLAS

ZVENDEVELOPER25

Sales Order Detail Handler

2024-06-24

ZCL\_BCV\_BAPI\_SALES\_ORDER\_TEST

CLAS

Z\_BCV\_CLASSES

BAPI\_SALESORDER\_CREATEFROMDAT2 Wrapper

2011-12-06

ZFG\_SALES\_ORDER

FUGR

ZDEVELOPER25

Sales Order Function Group

2025-01-07

ZFM\_CREATE\_SALESORDER

SRFC

ZPK\_HYBIRSINTEGRATION

RFC: Create Sales Order (Hybris Integration)

2014-09-30

ZFM\_SALES\_ORDER\_RFC\_16

SRFC

Z\_PAC\_SID

RFC: Sales Order Operations

2025-02-25

ZSALESORDER\_GETLIST

SRFC

$TMP

RFC: Get Sales Order List

2014-01-14

ZRP\_SALES\_ORDER\_CREATION\_16

PROG

Z\_PAC\_SID

Sales Order Creation (BDC/BAPI)

2024-11-14

ZRP\_SALESORDERUPDATE\_16

PROG

Z\_PAC\_SID

Sales Order Update

2024-11-14

ZEHPRC\_SALES\_ORDER

ENHO

ZEHSM\_PRC

Enhancement: Sales Order Pricing

2012-09-27

ZI\_SALES\_ORDERS\_RPT

DDLS

ZDEMO

CDS View: Sales Order Report

2026-03-10

ZCL\_Z\_SALESORDER\_16\_\*

CLAS (MPC/DPC)

Z\_PAC\_SID

OData Service: Sales Order V2

2025-01-02

ZSRA017\_SALESORDER\_CREATE\_SRV

IWSG

$TMP

Gateway Service: SO Creation

2023-07-03

ZSALESORDER\_MIGRATE\_ADB

FUGR

ZI037288

Sales Order ADB Migration

Legacy

ZFG\_IDOC\_SO

FUGR

Z\_DIPESH

IDoc: Sales Order Integration

2023-03-14

ZIF\_SALES\_ORDER\_09

SFPI

ZPKG\_PROGRAMS\_09

Adobe Form: Sales Order Output

2024-01-25

ZFPM\_GAF\_SO\_CONF\_EXIT

WDYN

ZFPM\_SO\_DEMO

WebDynpro FPM: SO Confirmation Exit

2014-06-06

**Total: 23+ objects** | **Key Risk:** Multiple overlapping SO creation paths (BAPI, RFC, BDC, OData, WebDynpro, IDoc) — potential consolidation opportunity for Layer 2.

## 5 Step 2: BOM Explosion & Engineering PLM/PP

After the MTO sales order, the BOM is exploded to determine the components needed. Your system has significant BOM-related customization.

Object

Type

Package

Description

Created

ZBOM\_CREATE

TRAN

ZECAD

Transaction: BOM Creation (Custom)

Legacy

Z\_CREATE\_MATERIAL\_BOM\_ITEMS

PROG

ZECAD

Material BOM Item Creation Program

Legacy

ZBOM\_01

PROG

ZBW

BOM Report for BW Extraction

2014-01-22

ZRBDSEBOM

PROG

ZIDI

BOM Distribution (ALE/IDoc)

Legacy

ZCL\_IM\_DPR\_ECTR\_BOM\_UPDATE

CLAS (BAdI)

ZPK\_DPR\_ECTR

BAdI: DPR ECTR BOM Update

2014-12-23

ZCL\_IM\_GTS\_SD0B\_BOM\_EXPL

CLAS (BAdI)

Z\_SCEMDEMO

BAdI: GTS BOM Explosion for Trade Compliance

Legacy

ZCL\_IM\_ICON\_BOM\_UPDATE

CLAS (BAdI)

ZICON

BAdI: ICON BOM Update Integration

Legacy

ZCL\_IM\_\_BADI\_MAT\_BOM

CLAS (BAdI)

Z\_RPCK\_MAT\_BOM

BAdI: Material BOM Processing

Legacy

ZCL\_IM\_\_RSTD\_CDESK\_BOM

CLAS (BAdI)

Z\_RPCK\_BADI\_STD

BAdI: cDesk BOM Standardization

Legacy

ZMEW\_CL\_BOM\_UTILITY

CLAS

ZMEW\_TOOL

BOM Utility Class (Header/Item/Structure)

2013-04-10

Z\_RFG\_MAT\_BOM

FUGR

Z\_RPCK\_MAT\_BOM

Function Group: Material BOM Operations

Legacy

ZMEW\_S\_BOM\_\*

TABL (6x)

ZMEW\_TOOL

Structures: BOM Header, Item, Details, Key, List, Structure

2013

ZZ\_PLM\_DEMO\_BOM\_DELETE

PROG

ZPS0

PLM Demo: BOM Deletion

Legacy

ZZ\_PLM\_SAFE\_BOM\_DELETE

PROG

$TMP

PLM: Safe BOM Deletion (with checks)

Legacy

ZBOM\_STR / ZBOM\_TAB

TABL/TTYP

Z001

BOM Structure/Table Types

Legacy

ZIF\_ZMEW\_BOM\_\* (5 interfaces)

INTF

Z\_PLM\_GW\_PACKAGE

Gateway Interfaces: BOM Explode, Get, GetList, Item Get, MatInBOM

2013-11-19

ZWBR\_CO\_BOM

ECAT

Z\_BRAZIL

eCATT: BOM Creation Test (Brazil)

Legacy

**Total: 30+ objects** | **Key Finding:** 6 BAdI implementations modifying BOM behavior — these represent the deepest level of customization and must be carefully assessed for S/4HANA compatibility in Layer 3.

## 6 Step 3: MRP & Material Staging PP MM

Object

Type

Package

Description

Created

ZCL\_PP\_MRP\_TEST\_DATA\_HELPER

CLAS

ZI061634

MRP Test Data Helper Class

2014-09-12

ZMRP\_MASTER / ZMRP\_TRANS

TRAN (2x)

ZI061634

Custom MRP Master & Transaction Screens

2014-09-15

ZFM\_MRP\_TYPE

SRFC

ZC5191063\_MIG

RFC: MRP Type Resolution

2013-12-16

ZSEARCH\_RES\_MAT\_MRPTYPE

SRFC

ZC5193497

RFC: Search Material by MRP Type

2014-05-05

ZSEARCH\_RES\_MRPTYPE

SRFC

ZC5191063\_MIG

RFC: Search MRP Type Results

2013-12-13

ZIDES\_MB1C\_PPMRP

SCAT

ZSD0

CATT: MB1C for PP/MRP Stock Initialization

Legacy

ZIDES\_PPMRP01–PPMRP20

SCAT (16x)

ZBC0

CATT: MRP Scenario Test Scripts (01–20)

Legacy

ZID\_MDG\_BS\_MAT\_MRP\_\*

WDCC (10x)

Z\_IDES\_MDG\*

MDG WebDynpro Config: Material MRP Views (PL100–PL500, VP1–VP3)

2013

ZRP\_KI\_MATERIAL\_STOCK

PROG

Z\_KIRAN

Material Stock Report

2023-09-08

ZRP\_MATERIALSTOCK

PROG

ZRIYAZ

Material Stock Display

2024-10-13

Z\_OPER\_ADHOC\_STOCK\_REPLENISH

PROG

ZELF

Adhoc Stock Replenishment

2011-05-31

Z\_OPER\_ADHOC\_STOCK\_TRANSFER

PROG

ZELF

Adhoc Stock Transfer (311)

2011-06-03

Z\_OPER\_DAILY\_STOCK\_311

PROG

ZELF

Daily Stock Transfer Report (311 movements)

Legacy

Z\_UTIL\_INIT\_STOCK\_561\_\*

PROG (2x)

ZELF

Initial Stock Upload (561 movement type)

2011-08-22

**Total: 35+ objects** | **Key Finding:** 16 CATT MRP test scripts (ZIDES\_PPMRP01–20) indicate a well-established MRP testing regime. The ZELF package contains operational stock programs — critical MTO infrastructure.

## 7 Step 4: Production Order Creation & Release PP

Object

Type

Package

Description

Created

ZPROD\_ORDERS\_DESPATCH

PROG

ZTOPLINE

**Production Orders Despatch Report**

Legacy

ZCL\_ZODT\_PRODUCT\_ORDER\_DPC\_EXT

CLAS

ZPAC\_KRISHNAVAMSI

OData DPC: Production Order Service (Extended)

2024-07-09

ZCL\_ZODT\_PRODUCT\_ORDER\_MPC\_EXT

CLAS

ZPAC\_KRISHNAVAMSI

OData MPC: Production Order Model

2024-07-09

ZCL\_ZODT\_PRODUCT\_23\_\*

CLAS (4x)

ZPAC\_KRISHNAVAMSI

OData: Product Order Service V2 (MPC/DPC pairs)

2024-07-03

ZCL\_PRODUCTION\_PERFORMANCE\_ERP

CLAS

$TMP

Production Performance ERP Integration

Legacy

ZCO\_PRODUCTION\_SCHEDULE\_ERP\_OU

CLAS

$TMP

Production Schedule ERP Output

Legacy

ZCO\_PROD\_SCHED\_ERPIDOC\_OUT

CLAS

$TMP

Production Schedule IDoc Outbound

Legacy

ZCL\_EX\_PROD\_SUP\_ALT\_WC

CLAS

ZPROD\_SUPERVISOR

Production Supervisor: Alternative Work Center

Legacy

ZCL\_IM\_PROD\_SUP\_ALT\_WC\_IMP

CLAS

ZPROD\_SUPERVISOR

BAdI Impl: Production Supervisor Alt Work Center

Legacy

ZCL\_PROD\_CAP\_IN / ZCO\_PROD\_CAP\_OUT

CLAS (2x)

ZPROD\_SUPERVISOR

Production Capacity Input/Output Proxies

Legacy

ZCX\_SEND\_PROD\_CAP\_ERROR

CLAS (CX)

ZPROD\_SUPERVISOR

Exception: Production Capacity Send Error

Legacy

ZCF\_PROD

CLAS + ENHO

$TMP

Enhancement: Production Configuration

2014-04-16

ZCHEM\_PRODDASH

WAPA (BSP)

ZI053956

SAPUI5: Chemical Production Dashboard

2012-05-11

ZT\_CO01\_01 through ZT\_CO01\_13

SCAT/ECAT (13x)

ZT\_CATT\_46C/47

**CO01 Test Scripts (Production Order Creation)**

Legacy

ZDBR41873\_RFC\_PRODORD\_DTL

SRFC

ZDBR41873

RFC: Production Order Detail Retrieval

2014-06-05

Z\_COABC\_CO01

SCAT

$TMP

CATT: Activity-Based Costing for CO01

Legacy

ZERC\_C\_WORKCENTER

ENHO

ZC5163255

Enhancement: Work Center Configuration

2011-12-01

ZIDES\_XFR\_WORK\_CENTER

PROG

Z\_IDES

Work Center Data Transfer

Legacy

**Total: 35+ objects** | **Key Finding:** The **ZPROD\_SUPERVISOR package** is a complete production capacity integration suite (alt work center, capacity input/output, error handling). The 13 CO01 CATT scripts indicate automated production order testing. **ZPROD\_ORDERS\_DESPATCH** in package ZTOPLINE is a critical MTO dispatch program.

## 8 Step 5: Shop Floor Execution & Confirmation PP

Object

Type

Package

Description

Created

ZDBR41873\_RFC\_PRODCONF\_CREATE

SRFC

ZDBR41873

RFC: Production Confirmation Create (CO11N)

2014-06-09

ZDBR41873\_RFC\_PRODCONF\_GET

SRFC

ZDBR41873

RFC: Production Confirmation Get

2014-06-11

ZDBR41873\_RFC\_CONFIRMATION\_LST

SRFC

ZDBR41873

RFC: Confirmation List Retrieval

2014-06-05

ZOPI\_RFC\_PROD\_CONFIRM

SRFC

ZOPI

RFC: OPI Production Confirmation

2014-08-20

Z\_OPER\_DAILY\_REM\_CONFIRMATIONS

PROG

ZELF

Daily Remaining Confirmations Report

Legacy

ZPPMC\_DPR\_COSTPLANING

WDYN

ZPPMC\_PL\_COSTPLANNING

WebDynpro: Production Cost Planning

Legacy

ZPPMC\_PL\_COSTPLANNING\_3

WDYN

ZPPMC\_PL\_COSTPLANNING\_3

WebDynpro: Cost Planning V3

2011-12-14

ZPPMC\_PL\_COMPAREVERSIONS

WDYN

ZPPMC\_PL\_COSTPLANNING\_3

WebDynpro: Compare Costing Versions

2012-02-10

ZBW\_PRODUCT\_COST\_VAR

OSOA

$TMP

BW: Product Cost Variance Analysis

2012-02-22

**Total: 9+ objects** | **Key Finding:** Package ZDBR41873 contains a complete production confirmation RFC suite (create, get, list). Package ZELF has daily operational confirmation monitoring. 3 WebDynpro cost planning apps indicate active production costing customization.

## 9 Step 6: Goods Issue / Goods Receipt MM WM

Object

Type

Package

Description

Created

ZCL\_GOODS\_ISSUE\_DPC\_EXT

CLAS

ZI068055

OData DPC: Goods Issue Service

2014-07-17

ZCL\_GOODS\_ISSUE\_MPC\_EXT

CLAS

ZI068055

OData MPC: Goods Issue Model

2014-07-17

ZGWS\_GOODS\_ISSUE\_CREATE

IWPR

ZI068055

Gateway: Goods Issue Create Project

2014-07-17

ZGWS\_GOODS\_ISSUE\_SRV

IWSV

ZI068055

Gateway Service: Goods Issue

2014-07-01

ZFG\_GOODS\_ISSUE

FUGR

ZI046697

Function Group: Goods Issue Operations

Legacy

ZDBR41873\_RFC\_PO\_GOODS\_ISSUE

SRFC

ZDBR41873

RFC: PO Goods Issue

2014-06-10

ZGOODSMOVEMENT

WAPA (BSP)

ZSAPAPRISODEMO

SAPUI5: Goods Movement App

Legacy

ZITC\_PERFORM\_GOODS\_RECEIPT

SRFC

ZITC

RFC: Goods Receipt Execution

2014-06-16

ZITC\_PERFORM\_GOODS\_RECEIPT\_20

SRFC

ZITC

RFC: Goods Receipt V2.0

2014-06-16

ZNW\_CO\_ERP\_GOODS\_RECEIPT\_MESSA

CLAS

Z\_IDES\_NW

NetWeaver: GR Message Processing

Legacy

Z\_OPER\_DAILY\_STOCK\_GI\_EXTRACT

PROG

ZELF

Daily Stock GI Extract

Legacy

Z05\_PACKINGGOODS\_AII\_800

PROG

$TMP

RFID: Packing Goods (Auto-ID)

Legacy

Z06\_LOADINGGOODS\_AII\_800

PROG

$TMP

RFID: Loading Goods

Legacy

Z\_GOODS\_RECEIPTS / Z\_GOODS\_RECEIPTS\_01

SCAT (2x)

ZIDE

CATT: Goods Receipt Test Scripts

Legacy

**Total: 16+ objects** | **Key Finding:** Complete OData V2 Goods Issue service (ZGWS\_GOODS\_ISSUE\_SRV) with Gateway model/project — already modernized. RFID/Auto-ID integration for packing/loading. ZELF package operational extracts.

## 10 Step 7: Delivery & Despatch SD

Object

Type

Package

Description

Created

Z04\_CREATEOUTBOUNDDELIVERY\_800

PROG

$TMP

Outbound Delivery Creation

Legacy

ZCL\_ICON\_DELIVERY\_PROC

CLAS

ZICON

ICON Delivery Processing

Legacy

ZCL\_INBOUND\_DELIVEY\_ENH

CLAS

ZDEVELOPER25

Inbound Delivery Enhancement Class

2023-07-27

ZCL\_Z\_ODATA\_DELIVERY\_2\_\*

CLAS (MPC/DPC)

ZPAC\_KRISHNAVAMSI

OData Service: Delivery V2

2024-12-11

ZDELIV

WAPA (BSP)

ZI041492

SAPUI5: Delivery App

2013-12-27

ZBAPIDLVREFTOSALESORDER

TABL

Z\_IDESMOBILE

Structure: BAPI Delivery Ref to Sales Order

Legacy

**Total: 8+ objects** | **Key Finding:** Delivery OData service (2024-12-11) is recent — already on modernization path. SAPUI5 delivery app exists.

## 11 Step 8: Billing & Revenue Recognition SD FI

Object

Type

Package

Description

Created

ZBILLING

TRAN

ZFG\_DBR34912

Custom Billing Transaction

2013-03-04

ZCL\_SALES\_BILL\_JA

CLAS

ZPAC\_NITISH

Sales Billing Class

2025-01-19

ZCL\_IM\_Z\_BILL\_REQ\_EDITOR\_\*

CLAS (3x)

$TMP / Z\_PRS

BAdI: Billing Request Editor (3 implementations)

Legacy–2011

ZBILLFORM

SFPI

ZI036765

Adobe Form: Billing Form

Legacy

ZBILLFORMDISPLAY

SFPF

ZI036765

Adobe Form Template: Billing Display

Legacy

ZBILL\_HRS / ZBILL\_RATE

VIEW (2x)

Z001

DDIC Views: Billing Hours & Billing Rates

Legacy

ZBILL\_STRUCT\_SFP

TABL

ZFG\_DBR34912

Structure: Billing for SFP Output

2013-03-04

ZES\_PRICING\_CPF

ENHO

Z\_CM

Enhancement: Pricing CPF (Condition Processing Framework)

2014-11-06

ZCHECK\_NETPRICE\_HEADER

ENHO

ZDEVELOPER25

Enhancement: Net Price Header Check

2023-05-18

**Total: 12+ objects** | **Key Finding:** Custom billing transaction ZBILLING with dedicated SFP form output. Pricing enhancement ZES\_PRICING\_CPF modifies the Condition Processing Framework — critical for Layer 3 assessment.

## 12 Step 9: Integration & Middleware XI/IDoc/OData

Object

Type

Package

Description

Created

ZFM\_CREATE\_SALESORDER (Hybris)

SRFC

ZPK\_HYBIRSINTEGRATION

Hybris → SAP Sales Order Creation

2014-09-30

ZCO\_PROD\_SCHED\_ERPIDOC\_OUT

CLAS

$TMP

Production Schedule IDoc Outbound

Legacy

ZSENDORDERCONFIRMATION

PROG

ZHYBRIS

Send Order Confirmation to Hybris

2014-06-19

ZFG\_IDOC\_SO

FUGR

Z\_DIPESH

IDoc: Sales Order Processing

2023-03-14

ZBADI\_IDOC\_PAYMENT

CLAS + ENHO

$TMP

BAdI: IDoc Payment Processing

2015-02-17

ZCL\_CHANGE\_OUT\_IDOC\_IN

CLAS

ZRISHAB

Change Outbound IDoc Inbound Processing

2023-07-13

ZCL\_IDOC\_ENHANCE / ZCL\_IDOC\_SEGMENT

CLAS (2x)

Z\_DIPESH

IDoc Enhancement & Custom Segment

2023-04-21

Z32\_IDOC

CMOD

Z32\_PACK

Customer Modification: IDoc Enhancement

2023-07-24

ZIB\_IDOC

CMOD

ZDEVELOPER25

Customer Modification: IDoc Processing

2023-05-25

**Total: 12+ objects** | **Key Finding:** Hybris (Commerce) integration via RFC for sales order creation and order confirmations. IDoc processing with custom enhancements and segments. Production schedule IDoc outbound.

## 13 Step 10: Output & Forms SmartForms/Adobe

Object

Type

Description

ZFLE\_ETIQ\_PROD

SSFO (SmartForm)

**Production Label Printing** (MTO-specific)

ZIF\_SALES\_ORDER\_09

SFPI (Adobe)

Sales Order Adobe Form Interface

ZBILLFORM / ZBILLFORMDISPLAY

SFPI/SFPF (Adobe)

Billing Form Interface & Template

ZLSO\_PWB\_COVER\_CONFIRM\*

SFPF/SFPI/SSFO

PWB Cover Confirmation Forms (Adobe + SmartForm)

ZEOF\_LABELPRINT

SSFO

End-of-Line Label Printing

ZATOS\_ORIGIN

SSFO

Origin Certificate Form

30+ Z SmartForms

SSFO

Various: demo, training, HR payslips, etc.

**Total: 30+ forms** | **Key Finding:** ZFLE\_ETIQ\_PROD (production label) is directly MTO-relevant. All SmartForms are Clean Core Level D — migration to Adobe Forms recommended in Layer 4.

## 14 Enhancement & BAdI Map

All custom enhancements that modify standard MTO behavior:

Enhancement

Type

Process Step

Impact

ZCL\_IM\_DPR\_ECTR\_BOM\_UPDATE

BAdI Implementation

BOM Explosion

Modifies DPR ECTR BOM update logic

ZCL\_IM\_GTS\_SD0B\_BOM\_EXPL

BAdI Implementation

BOM Explosion

GTS trade compliance during BOM explosion

ZCL\_IM\_ICON\_BOM\_UPDATE

BAdI Implementation

BOM Explosion

ICON integration BOM update

ZCL\_IM\_\_BADI\_MAT\_BOM

BAdI Implementation

BOM Explosion

Material BOM custom processing

ZCL\_IM\_\_RSTD\_CDESK\_BOM

BAdI Implementation

BOM Explosion

cDesk BOM standardization

ZCL\_IM\_PROD\_SUP\_ALT\_WC\_IMP

BAdI Implementation

Production Order

Alternative work center selection

ZES\_PRICING\_CPF

Enhancement (ENHO)

Billing / Pricing

Condition Processing Framework modification

ZEHPRC\_SALES\_ORDER

Enhancement (ENHO)

Sales Order

Sales order pricing enhancement

ZCHECK\_NETPRICE\_HEADER

Enhancement (ENHO)

Sales Order

Net price header validation

ZCF\_PROD

Enhancement (ENHO)

Production Order

Production configuration modification

ZERC\_C\_WORKCENTER

Enhancement (ENHO)

Production Order

Work center configuration enhancement

ZCL\_IM\_Z\_BILL\_REQ\_EDITOR\_1/2/3

BAdI Implementation (3x)

Billing

Billing request editor (3 implementations)

ZBADI\_IDOC\_PAYMENT

BAdI + Enhancement

Integration

IDoc payment processing modification

**⚠️ Critical for Layer 3 (S/4 Remediation):** The 6 BOM BAdI implementations and the pricing CPF enhancement are the highest-risk custom objects. These directly modify core PP/SD processing and must be individually assessed for S/4HANA compatibility. Each BAdI interface may change between ECC and S/4HANA.

## 15 Test Automation Inventory

Test Script

Type

Process Step

Package

ZT\_CO01\_01 through ZT\_CO01\_13

SCAT/ECAT (13x)

Production Order Creation (CO01)

ZT\_CATT\_46C / ZT\_CATT\_47

ZDG\_CO01\_01 / ZDG\_CO01\_01\_U

SCAT (2x)

Production Order (Data Generation)

ZDG1 / ZDGU

Z\_COABC\_CO01

SCAT

Activity-Based Costing + CO01

$TMP

ZIDES\_PPMRP01 through PPMRP20

SCAT (16x)

MRP Scenarios

ZBC0

ZIDES\_MB1C\_PPMRP

SCAT

Stock Init for MRP

ZSD0

ZWBR\_CO\_BOM

ECAT

BOM Creation (Brazil)

Z\_BRAZIL

Z\_GOODS\_RECEIPTS / \_01

SCAT (2x)

Goods Receipt

ZIDE

ZRACE\_CTM\_01\_SALES\_ORDER

SCAT

Sales Order

$TMP

**Total: 37 test scripts** — This is a strong MTO regression test foundation for Layer 3 migration.

## 16 OData Service Inventory (MTO-Relevant)

Service

Package

MTO Step

Description

ZODT\_PRODUCT\_ORDER

ZPAC\_KRISHNAVAMSI

Production Order

Production order data exposure

ZODT\_PRODUCT\_23

ZPAC\_KRISHNAVAMSI

Production Order

Product order V2

Z\_ODATA\_PRODUCT\_01 / \_22

ZPAC\_VAMSHIVARDHAN

Material Master

Product master data services

ZGWS\_GOODS\_ISSUE\_SRV

ZI068055

Goods Issue

Goods issue create/post

Z\_SALESORDER\_16

Z\_PAC\_SID

Sales Order

Sales order CRUD

ZSRA017\_SALESORDER\_CREATE\_SRV

$TMP

Sales Order

Sales order creation service

Z\_ODATA\_DELIVERY\_2

ZPAC\_KRISHNAVAMSI

Delivery

Delivery data service

30+ Generic OData Services

Various

Various

CRUD, navigation, demo services

## 17 Full Object Inventory by Category

pie title MTO Custom Objects by Category "Programs (PROG)" : 42 "Classes (CLAS)" : 48 "BAdI/Enhancement (ENHO/SXCI)" : 15 "Function Groups (FUGR/SRFC)" : 18 "OData Services (IWSV/IWSG)" : 12 "SmartForms/Adobe (SSFO/SFPF)" : 8 "Test Scripts (SCAT/ECAT)" : 37 "Structures/Tables (TABL/TTYP)" : 16 "Transactions (TRAN)" : 5 "WebDynpro (WDYN/WAPA)" : 10 "IDoc (IDOC/IEXT)" : 4 "CDS Views (DDLS)" : 1

### Object Count by MTO Process Step

Process Step

Module

Object Count

Key Objects

1\. Sales Order Entry

SD

23+

BAPI wrappers, OData, WebDynpro, Workflow, RFC, IDoc

2\. BOM Explosion

PLM PP

30+

ZBOM\_CREATE, 6 BAdIs, BOM utility, Gateway interfaces

3\. MRP & Staging

PP MM

35+

MRP test helper, 16 CATT scripts, MDG config, stock programs

4\. Production Order

PP

35+

ZPROD\_ORDERS\_DESPATCH, 13 CO01 tests, ZPROD\_SUPERVISOR pkg

5\. Shop Floor

PP

9+

Confirmation RFCs, cost planning WebDynpro, BW variance

6\. Goods Movement

MM

16+

OData GI service, RFID packing/loading, GR RFCs

7\. Delivery

SD

8+

OData delivery service, SAPUI5 app, enhancement

8\. Billing

SD FI

12+

Custom transaction ZBILLING, pricing enhancement, Adobe forms

9\. Integration

XI

12+

Hybris integration, IDoc SO, production schedule IDoc

10\. Output

Forms

8+

Production labels, billing forms, confirmation forms

## 18 Recommendations for Layers 2–4

### Layer 2: ECC Performance — Immediate Targets

#

Target Program

Action

Effort

L2-1

ZPROD\_ORDERS\_DESPATCH

Analyze for SELECT-in-LOOP, N+1 patterns. Likely reads AFKO/AFPO/RESB in loops. Generate optimized JOINs.

2–3 days

L2-2

Z\_OPER\_DAILY\_STOCK\_311

Daily stock report — optimize for HANA pushdown. Replace SORT+DELETE ADJ DUP with SELECT DISTINCT.

1–2 days

L2-3

Z\_OPER\_DAILY\_REM\_CONFIRMATIONS

Daily confirmation monitoring — optimize DB access patterns.

1 day

L2-4

ZBOM\_01

BOM report for BW — optimize extraction performance.

1 day

L2-5

All REUSE\_ALV programs

Migrate to CL\_SALV\_TABLE for OO ALV pattern.

1 day each

### Layer 3: S/4 Remediation — Priority Assessment Targets

#

Risk Area

Objects

S/4 Impact

L3-1

**BOM BAdI interfaces**

6 BAdI implementations

HIGH — BAdI interfaces may change between ECC and S/4HANA. Each must be individually verified against new BAdI definitions.

L3-2

**Pricing CPF enhancement**

ZES\_PRICING\_CPF

HIGH — Condition Processing Framework restructured in S/4HANA. May need complete rewrite.

L3-3

**Work center enhancement**

ZERC\_C\_WORKCENTER, ZPROD\_SUPERVISOR pkg

MEDIUM — Work center model changes in S/4HANA. Alt WC logic needs verification.

L3-4

**IDoc production schedule**

ZCO\_PROD\_SCHED\_ERPIDOC\_OUT

MEDIUM — IDoc types may have new segments in S/4HANA.

L3-5

**Hybris integration**

ZFM\_CREATE\_SALESORDER, ZSENDORDERCONFIRMATION

MEDIUM — Commerce integration pattern may shift to APIs/Events in S/4.

L3-6

**SmartForms**

ZFLE\_ETIQ\_PROD + all SSFO

HIGH — SmartForms deprecated in S/4HANA Cloud. Convert to Adobe Forms.

L3-7

**WebDynpro apps**

3 cost planning WDyn, SO details, delivery

HIGH — WebDynpro deprecated in S/4HANA. Migrate to Fiori/UI5.

### Layer 4: Clean Core — Target State Architecture

graph TB subgraph "Clean Core Target (S/4HANA)" A\["Released BAdIs  
(replace 6 BOM BAdIs)"\] --> B\["Key User Extensibility  
(custom fields on Prod Order)"\] B --> C\["BRF+  
(pricing rules, WC selection)"\] C --> D\["CDS Views  
(I\_ProductionOrder, I\_BillOfMaterial)"\] D --> E\["RAP APIs  
(API\_PRODUCTION\_ORDER, API\_BOM)"\] E --> F\["Fiori Elements  
(replace WebDynpro + ALV)"\] F --> G\["BTP Side-by-Side  
(Hybris connector, dashboards)"\] end style A fill:#27ae60,color:#fff style B fill:#27ae60,color:#fff style C fill:#3498db,color:#fff style D fill:#8b5cf6,color:#fff style E fill:#8b5cf6,color:#fff style F fill:#ec4899,color:#fff style G fill:#f59e0b,color:#fff

#

Current Custom

Clean Core Target

Pattern

L4-1

6 BOM BAdI Impls

Released BAdIs (BADI\_CS\_BOM\_\*) or Key User Extensibility

Replace custom with released extension points

L4-2

ZES\_PRICING\_CPF

BRF+ or released pricing BAdI in S/4

Externalize pricing rules to BRF+

L4-3

ZPROD\_SUPERVISOR pkg (WC)

Released production supervisor APIs

API-first approach

L4-4

ZPROD\_ORDERS\_DESPATCH

Fiori App F0723A (Manage Production Orders) or custom Fiori Elements on released CDS

Retire custom report

L4-5

ZSALESORDERDETAILS (WebDynpro)

Fiori App F0217 (Manage Sales Orders V2)

Retire WebDynpro

L4-6

ZPPMC cost planning (3 WDyn)

Fiori App F2721 (Manage Product Cost Estimates)

Retire WebDynpro

L4-7

Hybris RFC integration

SAP Commerce + S/4HANA Events (Event Mesh) or OData APIs

BTP event-driven

L4-8

All SmartForms

Adobe Forms (SFP) — C16 automated conversion available

SmartForm → Adobe

L4-9

All IDoc custom segments

Released APIs or Event Mesh topics

API-first integration

L4-10

37 CATT test scripts

ABAP Unit tests + Tricentis/CBTA

Modern test automation

### 🎯 Layer 1 Outcome — Next Steps

This process map gives every stakeholder (business, IT, migration team, QA) a complete inventory of what touches the MTO process. The 187 objects across 8 process steps are now mapped, categorized, and risk-assessed.

*   **Business stakeholders** can see which process steps have the most customization (BOM: 30+, Production Order: 35+, MRP: 35+)
*   **IT leads** can prioritize Layer 2 performance targets (ZPROD\_ORDERS\_DESPATCH, daily stock/confirmation reports)
*   **Migration architects** can focus Layer 3 on the 6 BOM BAdIs and pricing CPF enhancement — highest S/4 risk
*   **Clean Core architects** have the Layer 4 target state mapped: released BAdIs → Key User → BRF+ → CDS/RAP → Fiori Elements → BTP

**Ready for Layer 2?** I can immediately analyze any of the identified programs (e.g., ZPROD\_ORDERS\_DESPATCH) for HANA performance optimization.

**C16 Process Intelligence** — Layer 1: Make-to-Order Process Map

Generated: 24 April 2026 | System: ECC 7.40 Client 800 | Objects scanned: 7,051 custom + 50+ org units

_Verification: All objects discovered via TADIR repository scan. Organization structure from T001 (company codes), T001W (plants), TVKO (sales orgs), T024E (purchasing orgs), T024 (purchasing groups). Enhancement map from SXCI (BAdI implementations) and ENHO (enhancement implementations). OData services from IWSV (service versions). Test scripts from SCAT/ECAT repository entries._

mermaid.initialize({startOnLoad:true, theme:'neutral', securityLevel:'loose'});

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._