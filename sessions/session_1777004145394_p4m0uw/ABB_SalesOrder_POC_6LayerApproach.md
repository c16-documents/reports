  ABB Sales Order Transformation — 6-Layer Approach :root { --c16-purple: #6B3FA0; --c16-pink: #B5567A; --c16-copper: #C97B3A; --c16-dark: #1a1a2e; --c16-light: #f8f9fa; --green: #27ae60; --blue: #3498db; --orange: #e67e22; --red: #e74c3c; --gold: #f39c12; } \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, Arial, sans-serif; color: #2c3e50; background: #e9ecef; line-height: 1.7; } .doc-wrapper { max-width: 1200px; margin: 0 auto; background: #fff; box-shadow: 0 4px 24px rgba(0,0,0,0.12); } /\* ── HEADER ── \*/ .header-banner { background: linear-gradient(135deg, #C97B3A, #B5567A, #6B3FA0); color: #fff; padding: 50px 60px; position: relative; overflow: hidden; } .header-banner::after { content: ''; position: absolute; top: -50%; right: -10%; width: 400px; height: 400px; background: rgba(255,255,255,0.05); border-radius: 50%; } .header-banner img { height: 50px; margin-bottom: 16px; border-radius: 8px; } .header-banner h1 { font-size: 2.4em; font-weight: 700; margin-bottom: 8px; letter-spacing: -0.5px; } .header-banner .subtitle { font-size: 1.3em; opacity: 0.92; margin-bottom: 20px; } .header-meta { display: flex; gap: 24px; flex-wrap: wrap; margin-top: 16px; } .header-meta span { background: rgba(255,255,255,0.18); padding: 6px 16px; border-radius: 20px; font-size: 0.88em; backdrop-filter: blur(4px); } /\* ── EXECUTIVE BAR ── \*/ .exec-bar { background: linear-gradient(135deg, #1a1a2e, #2d1b4e); color: #fff; padding: 30px 60px; display: grid; grid-template-columns: repeat(4, 1fr); gap: 24px; } .exec-stat { text-align: center; padding: 16px; border-right: 1px solid rgba(255,255,255,0.1); } .exec-stat:last-child { border-right: none; } .exec-stat .number { font-size: 2.2em; font-weight: 700; color: #B5567A; } .exec-stat .label { font-size: 0.85em; opacity: 0.8; margin-top: 4px; } /\* ── CONTENT ── \*/ .content { padding: 50px 60px; } h2 { color: var(--c16-purple); font-size: 1.7em; margin: 50px 0 20px; padding-bottom: 10px; border-bottom: 3px solid var(--c16-pink); display: flex; align-items: center; gap: 12px; } h2 .num { background: var(--c16-pink); color: #fff; width: 36px; height: 36px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; font-size: 0.7em; flex-shrink: 0; } h2:first-child { margin-top: 0; } h3 { color: #4a3070; font-size: 1.25em; margin: 30px 0 14px; padding-left: 14px; border-left: 4px solid var(--c16-copper); } h4 { color: #555; font-size: 1.05em; margin: 20px 0 10px; font-weight: 600; } p { margin-bottom: 16px; color: #444; } /\* ── TABLES ── \*/ table { width: 100%; border-collapse: collapse; margin: 16px 0 28px; font-size: 0.92em; } thead th { background: var(--c16-purple); color: #fff; padding: 12px 16px; text-align: left; font-weight: 600; font-size: 0.88em; text-transform: uppercase; letter-spacing: 0.5px; } tbody td { padding: 10px 16px; border-bottom: 1px solid #eee; vertical-align: top; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } /\* ── LAYER CARDS ── \*/ .layer-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin: 30px 0; } .layer-card { border: 2px solid #e2e8f0; border-radius: 12px; padding: 28px; position: relative; transition: all 0.3s; background: #fff; } .layer-card:hover { border-color: var(--c16-purple); box-shadow: 0 4px 16px rgba(107,63,160,0.12); transform: translateY(-2px); } .layer-card .layer-num { position: absolute; top: -14px; left: 20px; background: var(--c16-purple); color: #fff; padding: 4px 16px; border-radius: 20px; font-size: 0.8em; font-weight: 700; letter-spacing: 1px; } .layer-card .layer-phase { position: absolute; top: -14px; right: 20px; padding: 4px 12px; border-radius: 20px; font-size: 0.75em; font-weight: 600; } .phase-discover { background: #dbeafe; color: #1e40af; } .phase-analyse { background: #fef3c7; color: #92400e; } .phase-build { background: #d1fae5; color: #065f46; } .phase-prove { background: #ede9fe; color: #5b21b6; } .layer-card h3 { border: none; padding-left: 0; margin-top: 8px; font-size: 1.15em; color: var(--c16-purple); } .layer-card .layer-icon { font-size: 2em; margin-bottom: 8px; } .layer-card .layer-question { font-style: italic; color: var(--c16-pink); font-size: 1.05em; margin-bottom: 12px; font-weight: 600; } .layer-card ul { margin: 8px 0 0 18px; color: #555; font-size: 0.92em; } .layer-card ul li { margin-bottom: 6px; } /\* ── FLOW DIAGRAM ── \*/ .flow-container { background: linear-gradient(135deg, #f8f5ff, #fef5f0); border: 2px solid #e8dff5; border-radius: 12px; padding: 40px; margin: 30px 0; text-align: center; } .flow-steps { display: flex; align-items: center; justify-content: center; flex-wrap: wrap; gap: 8px; } .flow-step { padding: 14px 20px; border-radius: 10px; color: #fff; font-weight: 600; font-size: 0.88em; min-width: 130px; text-align: center; } .flow-arrow { font-size: 1.5em; color: #999; font-weight: 300; } .fs-1 { background: linear-gradient(135deg, #3498db, #2980b9); } .fs-2 { background: linear-gradient(135deg, #f39c12, #e67e22); } .fs-3 { background: linear-gradient(135deg, #e74c3c, #c0392b); } .fs-4 { background: linear-gradient(135deg, #27ae60, #229954); } .fs-5 { background: linear-gradient(135deg, #8e44ad, #6B3FA0); } .fs-6 { background: linear-gradient(135deg, #1abc9c, #16a085); } /\* ── CALLOUT BOXES ── \*/ .callout { border-radius: 10px; padding: 20px 24px; margin: 20px 0; font-size: 0.95em; } .callout-principle { background: #f0ebf8; border-left: 5px solid var(--c16-purple); } .callout-warning { background: #fef5e7; border-left: 5px solid var(--orange); } .callout-success { background: #eafaf1; border-left: 5px solid var(--green); } .callout-insight { background: #eaf2f8; border-left: 5px solid var(--blue); } .callout-title { font-weight: 700; margin-bottom: 6px; color: #333; } /\* ── BADGES ── \*/ .badge { display: inline-block; padding: 3px 12px; border-radius: 12px; font-size: 0.82em; font-weight: 600; color: #fff; } .badge-fit { background: #27ae60; } .badge-config { background: #3498db; } .badge-extension { background: #e67e22; } .badge-gap { background: #e74c3c; } .badge-a { background: #27ae60; } .badge-b { background: #3498db; } .badge-c { background: #e67e22; } .badge-d { background: #e74c3c; } .badge-auto { background: #27ae60; } .badge-guided { background: #f39c12; } .badge-advisory { background: #95a5a6; } /\* ── COMPARISON TABLE ── \*/ .vs-table thead th:first-child { background: #e74c3c; } .vs-table thead th:last-child { background: #27ae60; } /\* ── TIMELINE ── \*/ .timeline { position: relative; margin: 30px 0; padding-left: 40px; } .timeline::before { content: ''; position: absolute; left: 16px; top: 0; bottom: 0; width: 3px; background: linear-gradient(to bottom, var(--c16-copper), var(--c16-pink), var(--c16-purple)); } .timeline-item { position: relative; margin-bottom: 24px; padding: 16px 20px; background: #f8f9fa; border-radius: 8px; border-left: 3px solid var(--c16-purple); } .timeline-item::before { content: ''; position: absolute; left: -32px; top: 20px; width: 14px; height: 14px; background: var(--c16-pink); border-radius: 50%; border: 3px solid #fff; } .timeline-item .tl-period { font-weight: 700; color: var(--c16-purple); font-size: 0.95em; } .timeline-item .tl-layer { font-weight: 600; color: #333; margin: 4px 0; } .timeline-item .tl-detail { font-size: 0.88em; color: #666; } /\* ── KPI CARDS ── \*/ .kpi-row { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin: 24px 0; } .kpi-card { background: linear-gradient(135deg, #f8f5ff, #fff); border: 1px solid #e8dff5; border-radius: 10px; padding: 20px; text-align: center; } .kpi-card .kpi-value { font-size: 2em; font-weight: 700; color: var(--c16-purple); } .kpi-card .kpi-label { font-size: 0.85em; color: #777; margin-top: 4px; } /\* ── CONNECTOR DIAGRAM ── \*/ .connector-box { background: #1a1a2e; color: #fff; padding: 30px 40px; border-radius: 12px; margin: 30px 0; font-family: 'Courier New', monospace; font-size: 0.88em; line-height: 1.8; overflow-x: auto; white-space: pre; } /\* ── TOC ── \*/ .toc { background: #f8f5ff; border: 1px solid #e8dff5; border-radius: 10px; padding: 24px 32px; margin: 30px 0; } .toc h3 { border: none; padding: 0; margin: 0 0 12px; color: var(--c16-purple); } .toc ol { margin-left: 18px; } .toc li { margin-bottom: 6px; } .toc a { color: var(--c16-purple); text-decoration: none; font-weight: 500; } .toc a:hover { text-decoration: underline; } /\* ── FULL-WIDTH LAYER CARD ── \*/ .layer-full { border: 2px solid #e2e8f0; border-radius: 12px; margin: 30px 0; overflow: hidden; } .layer-full-header { padding: 20px 28px; color: #fff; display: flex; align-items: center; gap: 16px; } .lf-1 { background: linear-gradient(135deg, #3498db, #2471a3); } .lf-2 { background: linear-gradient(135deg, #f39c12, #d68910); } .lf-3 { background: linear-gradient(135deg, #e74c3c, #c0392b); } .lf-4 { background: linear-gradient(135deg, #27ae60, #1e8449); } .lf-5 { background: linear-gradient(135deg, #8e44ad, #6B3FA0); } .lf-6 { background: linear-gradient(135deg, #1abc9c, #148f77); } .layer-full-header .lf-icon { font-size: 1.8em; } .layer-full-header .lf-title { font-size: 1.3em; font-weight: 700; } .layer-full-header .lf-question { opacity: 0.85; font-style: italic; margin-left: auto; } .layer-full-body { padding: 24px 28px; } /\* ── FOOTER ── \*/ .doc-footer { background: linear-gradient(135deg, #1a1a2e, #2d1b4e); color: rgba(255,255,255,0.7); padding: 30px 60px; font-size: 0.85em; text-align: center; } .doc-footer strong { color: #B5567A; } /\* ── PRINT ── \*/ @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; max-width: 100%; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .layer-full-header { -webkit-print-color-adjust: exact; print-color-adjust: exact; } h2 { page-break-after: avoid; } .layer-full { page-break-inside: avoid; } } @media (max-width: 768px) { .header-banner, .content, .doc-footer { padding-left: 24px; padding-right: 24px; } .exec-bar { grid-template-columns: repeat(2, 1fr); padding: 20px 24px; } .layer-grid { grid-template-columns: 1fr; } .kpi-row { grid-template-columns: 1fr; } .flow-steps { flex-direction: column; } .flow-arrow { transform: rotate(90deg); } }

![C16](data:image/jpeg;base64,)

# Sales Order Transformation

6-Layer Approach — ECC to S/4HANA 2025

A structured, automated methodology to analyse, map, remediate, and certify ABB's Sales Order process for S/4HANA Clean Core — covering custom code, standard process changes, configuration migration, and transformation validation.

📋 Client: ABB 🏭 Process: Order-to-Cash (Sales Order Focus) 🔧 Tool: C16 AI Platform 📅 April 2026

6

Transformation Layers

3

Phases: Discover → Build → Prove

10

Process Steps Covered

100%

Process Coverage (Code + Config + Standard)

### 📑 Contents

1.  [The Problem — Why 4 Layers Wasn't Enough](#problem)
2.  [Design Principles](#design-principles)
3.  [The 6-Layer Framework](#six-layers)
4.  [Layer Sequence & Data Flow](#flow)
5.  [Layer Deep Dive (1–6)](#layer-detail)
6.  [Sales Order Process Scope](#process-scope)
7.  [C16 Capability Matrix](#c16-capability)
8.  [Execution Timeline](#timeline)
9.  [Old vs New Approach](#comparison)
10.  [Value Proposition for ABB](#value)
11.  [Pre-Requisites](#prerequisites)
12.  [Next Steps](#next-steps)

## 1 The Problem — Why 4 Layers Wasn't Enough

The original 4-layer approach focused exclusively on **custom code transformation** — discover custom objects, map them to S/4 standard, remediate the code, and make it Clean Core compliant. This is necessary but insufficient for a real transformation.

A Sales Order process transformation involves three equally important dimensions. Ignoring any one of them creates blind spots that surface during migration testing — when they're expensive to fix.

⚙️

**Custom Code**  
Z-programs, exits, BAdIs, appends, forms, integrations

📐

**Standard Process**  
How S/4 standard itself changes — removed fields, new apps, new capabilities

🔧

**Configuration**  
Pricing, output, credit, ATP, partners — config that must be migrated or changed

⚠️ The Core Flaw in Code-Only Approaches

You cannot do a proper Fit-to-Standard analysis (mapping customs to standard) _before_ understanding what S/4 standard itself changes. That's mapping to a target you haven't defined. The new 6-layer approach fixes this by learning both sides first, then mapping.

## 2 Design Principles

The 6-layer framework is built on five principles that ensure completeness, correctness, and natural sequencing.

#

Principle

What It Means

How It's Enforced

**P1**

**Understand Both Sides Before Mapping**

Learn the AS-IS (Layer 1) AND the S/4 target (Layer 2) before attempting Fit-to-Standard (Layer 3)

Layer 3 requires Layer 1 + Layer 2 outputs as inputs

**P2**

**Config Is First Class**

Configuration is discovered, analysed, and migrated alongside code — not bolted on later

Config appears in Layer 1 (discovery), Layer 2 (delta), Layer 3 (mapping), Layer 4 (migration)

**P3**

**One Discovery Pass**

Everything is discovered once in Layer 1 — code, config, org, data, integrations

Layer 1 is the single inventory. All subsequent layers reference it.

**P4**

**Each Layer Feeds the Next**

No layer operates in isolation. Each layer's output is the next layer's input.

Explicit input/output contracts defined per layer

**P5**

**Prove It Works**

Transformation isn't complete until it's validated. Layer 6 provides the proof.

Test scenarios generated from Layer 1 process steps, validated against Layer 4 remediation

## 3 The 6-Layer Framework

The six layers are grouped into three natural phases: **Discover** (understand where you are and where you're going), **Build** (do the analysis and transformation work), and **Prove** (certify and validate).

Layer Sequence — Each Output Feeds the Next Input

LAYER 1  
Complete AS-IS

→

LAYER 2  
S/4 Standard Delta

→

LAYER 3  
Fit-to-Standard

→

LAYER 4  
Remediation

→

LAYER 5  
Clean Core

→

LAYER 6  
Validation

Phase 1: DISCOVER (L1 + L2)  │  Phase 2: BUILD (L3 + L4)  │  Phase 3: PROVE (L5 + L6)

LAYER 1 DISCOVER

🔍

### Complete AS-IS

"What do we have?"

*   Custom code (Z-programs, exits, BAdIs, appends)
*   Standard configuration (pricing, output, credit, ATP)
*   Org structure (sales orgs, plants, channels)
*   Integrations (EDI, RFC, OData, portals)
*   Data profile (volumes, order types, document flow)

LAYER 2 DISCOVER

🔄

### S/4HANA Standard Delta

"What changes regardless of custom code?"

*   Transactions replaced by Fiori (VA01 → F1814)
*   Fields removed/added on standard tables
*   Simplification items impacting SD
*   New capabilities (Advanced ATP, Output Mgmt 2.0)
*   BP migration (Customer/Vendor → Business Partner)
*   Configuration changes required

LAYER 3 ANALYSE

⚖️

### Fit-to-Standard Analysis

"What fits, what doesn't?"

*   Map each custom object → S/4 standard alternative
*   Map each config item → S/4 config change
*   Classify: FIT / CONFIG / EXTENSION / GAP
*   Coverage % per process step
*   Overall verdict: ADOPT / SIMPLIFY / MODERNIZE / KEEP

LAYER 4 BUILD

🔧

### Remediation & Migration

"Fix what needs fixing"

*   FIT → Retire (document retirement)
*   CONFIG → Generate config migration guide
*   EXTENSION → Generate compliant code
*   GAP → Redesign with Released BAdIs / BTP
*   BP migration approach
*   Form conversion (SmartForm → Adobe)

LAYER 5 BUILD

✨

### Clean Core Certification

"Is it cloud-ready?"

*   Level A/B/C/D scoring per object
*   Released BAdIs replacing user exits
*   Key User replacing custom appends
*   BRF+ replacing hardcoded rules
*   Event Mesh replacing custom interfaces
*   Before/after progression scorecard

LAYER 6 PROVE

✅

### Validation & Handoff

"Does it work?"

*   E2E test scenarios per process step
*   Regression test scripts
*   Data migration readiness assessment
*   Cutover dependency map
*   Final transformation scorecard
*   Handoff package for implementation team

## 4 Layer Sequence & Data Flow

Each layer has an explicit input/output contract. No layer guesses — it consumes verified output from the previous layer. This ensures traceability from initial discovery through to final validation.

PHASE 1: DISCOVER │ ├── Layer 1: Complete AS-IS ─────────────────────────────────────────────┐ │ INPUT: Live ECC system connection │ │ OUTPUT: Custom Object Inventory (by process step) │ │ Config Inventory (pricing, output, credit, partners) │ │ Org Structure Map (sales orgs, plants, channels) │ │ Integration Inventory (EDI, RFC, OData, file) │ │ Data Profile (volumes, order types, document flow) │ │ │ ├── Layer 2: S/4HANA Standard Delta ────────────────────────────────────┐│ │ INPUT: SAP Simplification List + Fiori App Catalog ││ │ Config Inventory from Layer 1 (to know what to compare) ││ │ OUTPUT: Transaction Replacement Map (VA01→F1814, VL01N→F1703...) ││ │ Field Change Register (removed, added, renamed) ││ │ Config Delta (what config must change in S/4) ││ │ New Capability Register (what S/4 offers that ECC didn't) ││ │ BP Migration Scope (customers + vendors affected) ││ │ ││ │ ││ PHASE 2: BUILD ││ │ ▼▼ ├── Layer 3: Fit-to-Standard ───────────────────────────────────────────┐ │ INPUT: Custom Object Inventory (from Layer 1) │ │ Config Inventory (from Layer 1) │ │ S/4 Standard Alternatives (from Layer 2) │ │ Config Delta (from Layer 2) │ │ OUTPUT: Classification per object: FIT/CONFIG/EXTENSION/GAP │ │ Classification per config: SAME/CHANGED/REMOVED/NEW │ │ Coverage % per process step │ │ Gap Register with complexity ratings │ │ Overall Verdict (ADOPT/SIMPLIFY/MODERNIZE/KEEP) │ │ │ ├── Layer 4: Remediation & Migration ───────────────────────────────────┐ │ INPUT: Classified objects + configs (from Layer 3) │ │ Source code of EXTENSION + GAP objects (from Layer 1) │ │ S/4 target APIs + BAdIs (from Layer 2) │ │ OUTPUT: Retirement List (FIT objects → documented for decommission) │ │ Config Migration Guide (step-by-step config changes) │ │ Remediated Code (S/4 compatible ABAP) │ │ Extension Designs (Released BAdI implementations) │ │ Gap Redesigns (new architecture for genuine gaps) │ │ BP Migration Plan (conversion approach + timeline) │ │ Form Conversions (SmartForm → Adobe) │ │ │ ├── Layer 5: Clean Core Certification ──────────────────────────────────┐ │ INPUT: Remediated code + extension designs (from Layer 4) │ │ Clean Core classification catalog (SAP cloudification) │ │ OUTPUT: Per-object Level A/B/C/D score │ │ Released BAdI mapping (exit → released BAdI) │ │ Key User field mapping (ZZ\* append → custom field) │ │ BRF+ candidate list (hardcoded rules → BRF+ decisions) │ │ Event Mesh design (custom IDoc → event-driven) │ │ Before/After Progression Scorecard │ │ │ │ │ PHASE 3: PROVE │ │ ▼ └── Layer 6: Validation & Handoff ────────────────────────────────────── INPUT: Process steps (from Layer 1) Remediated code + config guides (from Layer 4) Clean Core scorecard (from Layer 5) OUTPUT: E2E Test Scenarios per process step Regression Test Scripts (auto-generated) Data Migration Readiness Assessment Cutover Dependency Map Final Transformation Scorecard Implementation Handoff Package

📐 The Golden Rule

Every layer's output is the next layer's input. If you skip a layer, all subsequent layers lose their foundation. The sequence is not arbitrary — it's a dependency chain.

## 5 Layer Deep Dive

🔍 Layer 1 — Complete AS-IS Discovery "What do we have?"

Layer 1 is the foundation. It creates a **single, comprehensive inventory** of everything in ABB's ECC system that touches the Sales Order process. This is not just custom code — it includes standard configuration, org structure, integrations, and data profile.

#### Discovery Dimensions

Dimension

What C16 Scans

SAP Sources

Example Output

**Custom Code**

Z-programs, user exits, BAdI implementations, enhancement implementations, function modules, classes

TADIR, MODSAP, SXS\_ATTR, D010TAB

187 custom objects grouped by process step

**Custom Fields**

ZZ/YY appends on VBAK, VBAP, VBEP, VBKD, LIKP, LIPS, VBRK, VBRP

DD03L (WHERE FIELDNAME LIKE 'ZZ%')

14 custom fields on sales order header/item

**Configuration**

Order types, pricing procedures, output determination, credit config, ATP config, partner schemas

TVAK, T683S, T685A, T014, T163, TBE1, TPAR

8 active order types, 3 pricing procedures, 12 output types

**Org Structure**

Sales orgs, distribution channels, divisions, plants, shipping points, credit control areas

TVKO, TVKOV, TVSPT, T001W, TVSTZ, T014

4 sales orgs, 20+ plants, 3 shipping points

**Integrations**

IDoc types, RFC destinations, OData services, file interfaces, BDC uploads

EDIMSG, RFCDES, TADIR (IWSV), Z-source scan

ORDERS05 inbound, 3 RFC-to-CRM, 2 OData services

**Output Forms**

SmartForms, SAPscript forms, Adobe Forms for SO confirmation, delivery note, invoice

TNAPR, STXFADM, FPLAYOUT

2 SmartForms, 1 SAPscript, 1 Adobe form

**Data Profile**

Sales order volumes, date ranges, most-used order types, document flow completeness

VBAK, VBAP, VBFA, LIKP, VBRK

50K orders/year, 85% OR type, 72% delivery completion rate

💡 Why One Discovery Pass Matters

If you discover custom code in one layer and configuration in another, you miss the connections. A pricing user exit only makes sense when you see WHICH pricing procedure it enhances. A custom append only matters when you see WHICH process step uses it. One pass, complete picture.

#### Layer 1 Deliverable

Interactive HTML document with:

*   Process flow diagram with every custom touchpoint marked (Mermaid)
*   Custom object inventory table grouped by process step
*   Configuration footprint tables
*   Org structure map
*   Integration map (inbound/outbound/cross-module)
*   Risk heat map by process step (customisation density)

🔄 Layer 2 — S/4HANA 2025 Standard Delta "What changes regardless of custom code?"

Layer 2 defines the **target state**. Before mapping customs to standard, you need to know what standard looks like in S/4HANA 2025. This layer identifies every change that impacts the Sales Order process — even if ABB had zero custom code.

#### Delta Dimensions

Dimension

ECC Today

S/4HANA 2025

Impact

**Transaction UX**

VA01/VA02/VA03 (SAP GUI)

F1814 Manage Sales Orders (Fiori)

User training, role redesign, launchpad config

**Business Partner**

KNA1/KNB1 (Customer), LFA1/LFB1 (Vendor) — separate objects

BUT000/BUT0ID (Business Partner) — unified model, BP transaction

Master data migration, all code referencing KNA1/LFA1 needs update

**Material Number**

MATNR = CHAR(18)

MATNR = CHAR(40)

All structures, variables, interfaces handling MATNR need widening

**Credit Management**

VKM1/FD32 (classic SD credit)

FSCM Credit Management (UKM\_\*), Fiori F1498

Credit config migration, new credit master, integration changes

**Output Management**

NACE + Condition Technique + SmartForms

Output Management 2.0 (BRF+ driven), Adobe Forms, Fiori F3809

Output config recreation, form migration, print program rewrite

**ATP**

Classic ATP (CO06, VA01 embedded)

Advanced ATP (aATP) optional, classic ATP still available

If adopting aATP: new config, new integration, new Fiori apps

**Pricing**

Condition technique (V/06, VK11)

Same foundation, but new condition types + Fiori apps (F1893)

Pricing procedures carry over, custom routines need review

**Reporting**

Custom ALV reports, VA05, VL06O

Embedded Analytics (CDS-based), Fiori analytical apps

Custom reports can be replaced by CDS analytical views

**Extension Model**

User exits, classic BAdIs, enhancement spots, modifications

Released BAdIs only, Key User Extensibility, BTP side-by-side

All classic exits need migration to released extension points

**Integration**

Custom IDocs, RFC, BAPI wrappers

Event Mesh, Integration Suite, Released APIs (OData V4)

Interface redesign for cloud-native integration

#### Simplification Items (SAP Note 2313884)

C16 reads the SAP Simplification List and maps every item affecting SD. Examples of critical items:

Simplification Item

Impact

ABB Action Required

Customer/Vendor Integration (CVI)

KNA1/LFA1 → Business Partner

Data migration + code remediation

Material Number Length Extension

MATNR widened to 40 chars

Structure/variable review

Credit Management Migration

Classic → FSCM

Config recreation + testing

Output Management 2.0

NACE → BRF+ driven output

Output config recreation

Billing Document Simplification

Table structure changes

Custom billing code review

#### Layer 2 Deliverable

S/4HANA Standard Delta Report containing:

*   Transaction replacement map (ECC → Fiori, with App IDs)
*   Field change register per table (removed, added, renamed)
*   Simplification item inventory for SD
*   New capability register with business value assessment
*   Config delta checklist (what must change, what carries over)
*   BP migration scope and approach

⚖️ Layer 3 — Fit-to-Standard Analysis "What fits, what doesn't?"

Layer 3 is the decision layer. It takes the complete AS-IS from Layer 1 and the S/4 target from Layer 2, then classifies **every** custom object and configuration item into one of four categories.

#### Classification Model

Classification

Meaning

Action in Layer 4

Example

FIT

S/4 standard covers this requirement exactly

Retire custom object

BDC upload → replaced by API\_SALES\_ORDER\_SRV batch

FIT WITH CONFIG

S/4 standard covers via configuration (no code)

Document config steps

Custom pricing routine → standard condition type BO01

FIT WITH EXTENSION

S/4 covers ~70-80%, needs compliant extension

Design Released BAdI / Key User extension

Custom credit check → FSCM + Released BAdI for threshold

GAP

No standard equivalent exists

Redesign or justify keeping

ABB-specific project costing field on SO line item

#### Dual Classification — Code AND Config

Layer 3 classifies **two** dimensions in parallel:

Dimension

Categories

Example

**Custom Objects**

FIT / CONFIG / EXTENSION / GAP

User exit USEREXIT\_SAVE\_DOCUMENT → EXTENSION (Released BAdI SD\_SLS\_DOC\_PROC available)

**Configuration**

SAME / CHANGED / REMOVED / NEW

Output type ZBA0 via NACE → CHANGED (must migrate to Output Management 2.0)

#### Coverage Calculation

For each process step, C16 calculates:

Coverage % = (FIT + CONFIG + EXTENSION) ÷ Total Requirements × 100 Verdict: ≥ 90% coverage → ADOPT (use standard as-is with minor config) ≥ 70% coverage → SIMPLIFY (standard + configuration) ≥ 50% coverage → MODERNIZE (standard + extensions + BTP) < 50% coverage → KEEP (justified custom — significant gaps)

#### Layer 3 Deliverable

*   Requirement decomposition table (R1-Rn per process step, minimum 10 per step)
*   Per-requirement classification with specific standard alternative (Fiori App ID, API name, CDS view)
*   Coverage calculation per process step and overall
*   Gap register with complexity ratings (Low/Medium/High)
*   Config classification table (SAME/CHANGED/REMOVED/NEW per config item)
*   Overall verdict with supporting evidence

🔧 Layer 4 — Remediation & Migration "Fix what needs fixing"

Layer 4 is where the work gets done. Every classification from Layer 3 gets an action — no object is left without a remediation path. This layer covers **code, configuration, data, and forms** together.

#### Action Matrix — What Happens Per Classification

Layer 3 Classification

Layer 4 Action

C16 Delivers

ABB Team Does

FIT

**Retire**

Retirement documentation, dependency impact analysis, decommission checklist

Sign-off on retirement, update process docs

FIT WITH CONFIG

**Configure**

Step-by-step config migration guide (which IMG path, which settings, which values)

Execute config in S/4 sandbox, test

FIT WITH EXTENSION

**Extend**

Released BAdI implementation code, Key User custom field design, BRF+ decision table design

Review code, deploy, test

GAP

**Redesign**

New architecture using Released APIs + BTP if needed, or justified KEEP with documented rationale

Business decision on approach, then implement

#### Additional Migration Workstreams

Workstream

What C16 Does

Scope

**S/4 Code Compatibility**

Scan for obsolete FMs, TABLES declarations, MATNR length, deprecated syntax — generate remediated code

Every EXTENSION + KEEP object

**Business Partner Migration**

Identify all code referencing KNA1/KNB1, map to BP tables/CDS views, generate conversion approach

All customer/vendor-related objects

**Form Conversion**

Auto-convert SmartForms/SAPscript to Adobe Forms with deployment ABAP

Every output form discovered in Layer 1

**Hardcoded Rules → BRF+**

Extract IF/ELSE business rules from source code, design BRF+ decision tables

Every EXTENSION + KEEP object with hardcoded logic

**Config Delta Execution Guide**

For each CHANGED config item: document exact IMG path, old value, new value, migration steps

Every CHANGED + REMOVED config item from Layer 3

#### Layer 4 Deliverable

*   Retirement list with decommission checklists
*   Config migration guide (step-by-step per config area)
*   Remediated ABAP source code (S/4 compatible)
*   Released BAdI implementation designs with code
*   Key User custom field specifications
*   BRF+ decision table designs
*   BP migration approach document
*   Converted Adobe Forms (from SmartForm/SAPscript)

✨ Layer 5 — Clean Core Certification "Is it cloud-ready?"

Layer 5 takes everything that survived Layer 4 (extensions, redesigns, kept objects) and measures it against SAP's Clean Core standard. The goal is to move as many objects as possible to **Level A (cloud-ready)** — the target state for long-term maintainability and upgrade safety.

#### Clean Core Classification Levels

Level

Definition

What It Means

Target

Level A

Cloud-Ready

Uses only Released APIs. Deployable to S/4HANA Cloud. Upgrade-safe.

✅ Target state

Level B

Classic Stable

Uses stable BAPIs/interfaces. Works in S/4 on-premise. Not cloud-deployable.

Acceptable for on-premise

Level C

Non-Compliant

Direct DB access without CDS alternative, unreleased APIs. Migration risk.

Must remediate

Level D

Critical

Deprecated patterns, modifications, unsupported techniques. Blocks upgrades.

Must eliminate

#### Five Pillars of Clean Core

Pillar

ECC Pattern (Replace)

Clean Core Pattern (Target)

C16 Action

**1\. Released BAdIs**

User exits (MV45AFZZ), classic BAdIs, enhancement spots

Released BAdIs with stability contract

Maps each exit → released BAdI, generates implementation code

**2\. Key User Extensibility**

ZZ/YY append fields on standard tables

Custom fields via Key User app (no modification)

Documents each append → Key User field specification

**3\. BRF+ Business Rules**

Hardcoded IF/ELSE in ABAP (company codes, thresholds, currencies)

BRF+ decision tables (business-maintainable, no code change)

Extracts rules from source code, designs BRF+ tables

**4\. BTP Extensions**

Custom ABAP in the core (tight coupling)

Side-by-side extensions on BTP (loose coupling)

Identifies candidates for BTP, designs CAP service architecture

**5\. Event Mesh / Integration Suite**

Custom IDocs, RFC, direct API calls

Event-driven integration via Event Mesh, managed APIs via Integration Suite

Maps each interface → event/API pattern, designs integration flow

#### The Hero Metric — Before/After Scorecard

This is the KPI that ABB's board cares about. It shows the Clean Core progression across the entire Sales Order process:

Metric

Before (Layer 1 Baseline)

After (Layer 5 Target)

Objects at Level A

~5%

≥60%

Objects at Level B

~20%

~25%

Objects at Level C

~45%

~10%

Objects at Level D

~30%

~5%

**Cloud Readiness (A+B)**

**~25%**

**≥85%**

#### Layer 5 Deliverable

*   Per-object Clean Core Level (A/B/C/D) with evidence
*   Five-pillar mapping (each pillar with specific objects assigned)
*   Before/after progression scorecard
*   Released BAdI implementation designs with code
*   Key User extensibility specifications
*   BRF+ decision table designs
*   BTP extension architecture (for objects that need it)
*   Event Mesh / Integration Suite design

✅ Layer 6 — Validation & Handoff "Does it work?"

Layer 6 is the proof layer. Transformation without validation is just documentation. This layer generates test scenarios from the process steps discovered in Layer 1, validates them against the remediation from Layer 4, and produces a handoff package for ABB's implementation team.

#### Validation Dimensions

Dimension

What Gets Validated

How C16 Helps

ABB Team Executes

**E2E Process Tests**

Full process flow: Inquiry → Quotation → Order → Delivery → Billing → Payment

Generates test scenarios from Layer 1 process steps, with expected results per step

Execute in S/4 sandbox, document results

**Regression Tests**

Every custom object that was remediated, retired, or replaced

Generates regression test scripts from source code analysis

Execute tests, validate no functional loss

**Exception Scenarios**

Returns, intercompany, MTO, partial delivery, EDI — all exception paths

Generates exception-specific test cases from Layer 1 exception inventory

Test each exception path end-to-end

**Integration Tests**

Every interface (EDI, RFC, OData) still works after migration

Maps each interface from Layer 1, defines test data and expected behaviour

Connectivity testing with partner systems

**Data Migration Readiness**

Master data (BP), open orders, pricing conditions

Assesses data quality, identifies records that won't migrate cleanly

Data cleansing, migration execution

#### Final Transformation Scorecard

The summary document that ties all 6 layers together:

Metric

Value

Source Layer

Total custom objects discovered

e.g., 187

Layer 1

S/4 standard changes identified

e.g., 42

Layer 2

Standard coverage %

e.g., 82%

Layer 3

Objects retired

e.g., 65

Layer 4

Objects remediated

e.g., 78

Layer 4

GAPs requiring redesign

e.g., 14

Layer 4

Clean Core Level A %

e.g., 62%

Layer 5

Test scenarios generated

e.g., 120

Layer 6

Overall readiness

e.g., SIMPLIFY

Layer 3 verdict

#### Layer 6 Deliverable

*   E2E test scenario catalogue per process step
*   Regression test scripts (auto-generated from code analysis)
*   Exception scenario test cases
*   Integration test specifications
*   Data migration readiness assessment
*   Cutover dependency map (what must happen in what order)
*   Final transformation scorecard
*   Implementation handoff package (all Layer 1-6 deliverables consolidated)

## 6 Sales Order Process Scope

The 6-layer approach is applied to a defined process scope. ABB chooses the entry and exit points based on their transformation ambition.

### Scope Options

Scope

Entry Point

Exit Point

Process Steps

Recommended For

**Narrow**

VA01 — Sales Order Created

VA03 — Order Confirmed

3 steps (Create, Price, Confirm)

Quick POC, time-constrained demo

**Medium**

VA11 — Inquiry

VF01 — Billing Document Posted

7 steps (Inquiry → Quotation → Order → ATP → Delivery → GI → Billing)

Recommended for ABB POC

**Full O2C**

Customer first contact

F-28 — Payment received & cleared

10 steps (full Order-to-Cash)

Full transformation programme

### The 10 Process Steps (Full O2C)

#

Step

ECC Transaction

S/4 Fiori App

Key Tables

1

Customer Inquiry

VA11

—

VBAK, VBAP

2

Quotation

VA21

F1805

VBAK, VBAP, VBKD

3

Sales Order Creation

VA01/VA02

F1814

VBAK, VBAP, VBEP, VBKD

4

Pricing

VK11, V/06

F1893

KONV, KONH, KONP

5

Availability Check (ATP)

CO06 / embedded

Built into F1814

ATPCS, MDBS

6

Credit Check

VKM1, FD32

F1498

KNKK, S066

7

Delivery

VL01N

F1703

LIKP, LIPS

8

Goods Issue

VL02N (PGI)

F1703 (embedded)

MKPF, MSEG, MARD

9

Billing

VF01

F0861

VBRK, VBRP

10

Payment & Clearing

F-28, F110

F0711

BSID, BSAD

### Exception Scenarios (Cross-Cutting)

Exception

Steps Affected

Typical Custom Code

Returns / Credit Memo

3, 7, 9

Return BAdIs, credit approval workflow, restocking logic

Intercompany

3, 7, 8, 9

Cross-company pricing, intercompany billing, STO auto-creation

Third Party (Drop Ship)

3, 7

Auto PO creation, GR-based IV, vendor selection

Make-to-Order

3, 5, 7, 8

BOM explosion BAdIs, CO01 auto-creation, settlement

Consignment

3, 7, 8, 9

Stock management overrides, billing triggers

EDI Orders

3

IDoc ORDERS05, custom segments, partner profiles, error handling

Partial Delivery / Backorder

7, 8

Delivery split BAdIs, backorder processing

Free of Charge

3, 9

Billing block logic, approval workflows

## 7 C16 Capability Matrix

For each layer, this matrix shows what C16 automates fully, what C16 guides (analysis + design, ABB executes), and what is advisory only (C16 recommends, ABB decides and implements).

Layer

Capability

Automation Level

C16 Delivers

**L1: AS-IS**

Custom object discovery

Full Auto

Scans TADIR, MODSAP, SXS\_ATTR — finds every custom object

Source code analysis

Full Auto

Reads every line, extracts business logic and table patterns

Config inventory

Full Auto

Reads TVAK, T683S, NACE, T014 — maps all SD config

Org structure mapping

Full Auto

Reads TVKO, T001W, TVSTZ — maps complete org

Data profiling

Full Auto

Counts VBAK/LIKP/VBRK — volumes, types, date ranges

**L2: S/4 Delta**

Transaction replacement map

Full Auto

Maps every ECC t-code to Fiori app ID from SAP catalog

Simplification item mapping

Full Auto

Reads SAP Note 2313884 items for SD component

Field change register

Full Auto

Identifies removed/renamed/added fields per table

New capability assessment

Guided

Identifies capabilities — ABB decides which to adopt

**L3: Fit-to-Std**

Requirement decomposition

Full Auto

AI-powered extraction from code + config

FIT/GAP classification

Full Auto

Rules-based + AI per requirement

Coverage calculation

Full Auto

% per step, overall verdict

**L4: Remediation**

Code remediation

Full Auto

Generates S/4-compatible ABAP with verified field names

Config migration guide

Guided

Documents steps — ABB executes in S/4 IMG

Form conversion

Full Auto

SmartForm → Adobe Form (auto-convert + deploy)

BRF+ rule design

Guided

Designs decision tables — ABB configures in BRF+ workbench

BP migration plan

Guided

Assesses scope — ABB executes via CVI migration cockpit

**L5: Clean Core**

Clean Core scoring

Full Auto

3-layer classification engine (catalog + ATC + simplification)

Released BAdI mapping

Full Auto

Maps exits → Released BAdIs with code

BTP extension design

Guided

Designs architecture — ABB deploys on BTP

Event Mesh design

Guided

Designs event-driven patterns — ABB configures Event Mesh

**L6: Validation**

Test scenario generation

Full Auto

Generates from process steps and code analysis

Regression test scripts

Full Auto

Auto-generated from source code

E2E test execution

Advisory

Provides scenarios — ABB's QA team executes

Data migration execution

Advisory

Assesses readiness — ABB's data team executes

## 8 Execution Timeline

The 6 layers execute in sequence over 4-6 weeks for the Sales Order process (medium scope). Each layer has defined review gates where ABB stakeholders validate findings before proceeding.

Week 0 — Setup (1-2 days)

Pre-Requisite: Connectivity

VPN or Cloud Connector setup + RFC service user creation on ABB's ECC. Connectivity test.

Week 1 — DISCOVER

Layer 1: Complete AS-IS + Layer 2: S/4 Standard Delta

Full system scan, inventory generation, S/4 delta mapping. Both discovery layers run in parallel. Review gate: ABB validates inventory completeness.

Week 2 — ANALYSE

Layer 3: Fit-to-Standard Analysis

Requirement decomposition, FIT/GAP classification, coverage calculation, verdict. Review gate: ABB functional team validates FIT/GAP classifications.

Week 3-4 — BUILD

Layer 4: Remediation & Migration

Code remediation, config guides, form conversion, BRF+ design, BP migration plan. Review gate: ABB technical team reviews remediated code.

Week 4-5 — BUILD

Layer 5: Clean Core Certification

Clean Core scoring, Released BAdI mapping, progression scorecard. Review gate: ABB architecture team validates target state design.

Week 5-6 — PROVE

Layer 6: Validation & Handoff

Test scenario generation, regression scripts, final scorecard, handoff package. Review gate: ABB programme lead signs off on transformation package.

⏱️ Compared to Traditional Approach

A consulting firm would quote **16-24 weeks** and **3-5 consultants** for the same scope. The 6-layer approach with C16 completes in **4-6 weeks** with higher accuracy (code is read, not interviewed) and complete traceability (every finding links back to a specific object and line of code).

## 9 Old vs New Approach

Old (4 Layers — Custom Code Only)

Aspect

New (6 Layers — Full Process)

Custom code only

**Coverage**

Custom code + standard process + configuration

Mapped customs BEFORE knowing S/4 target

**Sequence**

Learns both sides first (L1 + L2), then maps (L3)

Config was Layer 6 (afterthought)

**Config Treatment**

Config in every layer: L1 discovers, L2 deltas, L3 classifies, L4 migrates

Two separate discovery passes

**Discovery**

One comprehensive pass in Layer 1

No validation layer

**Proof**

Layer 6 generates test scenarios and validates transformation

Layers 5-6 felt bolted on

**Flow**

Natural input→output chain — each layer feeds the next

Code-centric deliverables

**Deliverables**

Process-centric: retirement lists, config guides, test scenarios alongside code

No standard delta analysis

**S/4 Knowledge**

Layer 2 maps every S/4 change: Fiori apps, simplification items, BP migration, new capabilities

## 10 Value Proposition for ABB

75%

Faster than traditional consulting  
(6 weeks vs 24 weeks)

100%

Code coverage  
(reads every line, not interviews)

6

Complete layers  
(code + config + standard + validation)

### Why This Approach Wins

Advantage

How It Benefits ABB

**Exhaustive Discovery**

C16 reads every line of custom code. Finds enhancements that nobody remembers exist. Nothing is missed because the analysis is code-level, not interview-based.

**Both Sides of the Equation**

Maps ABB's ECC reality (Layer 1) AND S/4 target state (Layer 2) before attempting Fit-to-Standard. No mapping to an undefined target.

**Actionable, Not Just Analytical**

Layer 4 produces actual code, config guides, and form conversions — not just a PowerPoint recommendation. ABB gets working artifacts, not just a report.

**Measurable Progression**

The Clean Core scorecard (Layer 5) provides a clear before/after KPI that ABB can track. "We went from 25% cloud-ready to 85% cloud-ready."

**Validated Transformation**

Layer 6 generates test scenarios so ABB can prove the transformation works before go-live. No surprises in UAT.

**Traceable End-to-End**

Every finding in Layer 6 traces back through Layers 5→4→3→2→1 to a specific custom object, config item, or S/4 change. Complete audit trail.

## 11 Pre-Requisites

#

Pre-Requisite

Owner

Effort

Details

1

**RFC Service User**

ABB Basis

2 hours

Read-only roles: S\_RFC, S\_DEVELOP (display), S\_TABU\_NAM. No SAP\_ALL, no write access.

2

**Network Connectivity**

ABB Network + C16

4-8 hours

Option A: Azure VPN Gateway (site-to-site). Option B: BTP Cloud Connector (lighter).

3

**Process Scope Confirmation**

ABB Programme Lead

1 meeting

Define entry/exit points: Narrow (VA01→VA03), Medium (VA11→VF01), or Full O2C.

4

**Stakeholder Availability**

ABB Functional Lead

Available at gates

Review findings at 4 gates: L1 inventory, L3 FIT/GAP, L4 remediation, L6 handoff.

5

**C16 System Configuration**

C16 Team

30 minutes

Configure ABB system connection in C16 platform, verify connectivity.

## 12 Next Steps

Step

Action

Owner

When

**1**

ABB confirms process scope (Narrow / Medium / Full O2C)

ABB Programme Lead

This week

**2**

ABB creates RFC service user + initiates connectivity setup

ABB Basis + Network

Week 1

**3**

C16 team configures ABB system connection, runs connectivity test

C16 Team

Week 1

**4**

Execute Layer 1 + Layer 2 (Discovery Phase)

C16 (automated)

Week 2

**5**

Review Gate 1: ABB validates AS-IS inventory and S/4 delta

ABB Functional + C16

Week 2 end

**6**

Execute Layers 3-6 per timeline

C16 + ABB review gates

Weeks 3-6

📌 The Bottom Line

Six layers. Three phases. One complete transformation picture.  
  
**Layer 1** — What do we have?  
**Layer 2** — What changes in S/4?  
**Layer 3** — What fits, what doesn't?  
**Layer 4** — Fix what needs fixing.  
**Layer 5** — Make it cloud-ready.  
**Layer 6** — Prove it works.  
  
Each layer's output feeds the next. No guessing. No gaps. No surprises.

![C16](data:image/jpeg;base64,)  
**C16 AI Platform** — Sales Order Transformation: 6-Layer Approach  
Prepared for ABB — April 2026  
  
This document defines the transformation methodology. Prompt guides for each layer are available as separate documents.

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._