  C16 × Sydler Technologies — Strategic Partnership Framework :root { --primary: #6B3FA0; --secondary: #B5567A; --accent: #C97B3A; --dark: #2c3e50; --light: #f8f9fa; --success: #27ae60; --warning: #e67e22; --danger: #e74c3c; --info: #3498db; --sydler-blue: #1a5276; --c16-purple: #6B3FA0; } \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: var(--dark); background: #e9ecef; line-height: 1.7; font-size: 15px; } .doc-wrapper { max-width: 1120px; margin: 30px auto; background: #fff; box-shadow: 0 4px 24px rgba(0,0,0,0.12); border-radius: 12px; overflow: hidden; } /\* Header Banner \*/ .header-banner { background: linear-gradient(135deg, #C97B3A 0%, #B5567A 45%, #6B3FA0 100%); color: #fff; padding: 50px 60px; position: relative; overflow: hidden; } .header-banner::before { content: ''; position: absolute; top: -50%; right: -20%; width: 500px; height: 500px; background: rgba(255,255,255,0.05); border-radius: 50%; } .header-banner .logos { display: flex; align-items: center; gap: 20px; margin-bottom: 20px; } .header-banner .logos img { height: 50px; border-radius: 8px; } .header-banner .partnership-symbol { font-size: 2em; font-weight: 300; opacity: 0.8; } .header-banner h1 { font-size: 2.2em; font-weight: 700; margin-bottom: 8px; letter-spacing: -0.5px; } .header-banner .subtitle { font-size: 1.15em; opacity: 0.92; font-weight: 300; margin-bottom: 20px; } .header-banner .doc-meta { display: flex; gap: 30px; font-size: 0.88em; opacity: 0.85; flex-wrap: wrap; } .header-banner .doc-meta span { background: rgba(255,255,255,0.15); padding: 4px 14px; border-radius: 20px; } .confidential-bar { background: #1a1a2e; color: #e74c3c; text-align: center; padding: 8px; font-size: 0.85em; font-weight: 600; letter-spacing: 2px; text-transform: uppercase; } /\* Main Content \*/ .content { padding: 40px 60px; } /\* Table of Contents \*/ .toc { background: linear-gradient(135deg, #f8f4ff 0%, #fff5f5 100%); border: 1px solid #e8d5f5; border-radius: 10px; padding: 30px 40px; margin-bottom: 40px; } .toc h2 { color: var(--primary); font-size: 1.3em; margin-bottom: 15px; padding-bottom: 8px; border-bottom: 2px solid var(--secondary); } .toc ol { columns: 2; column-gap: 40px; padding-left: 20px; } .toc li { margin-bottom: 8px; break-inside: avoid; } .toc a { color: var(--primary); text-decoration: none; font-weight: 500; } .toc a:hover { color: var(--secondary); text-decoration: underline; } /\* Section Titles \*/ .section-title { color: var(--primary); font-size: 1.6em; font-weight: 700; margin: 50px 0 20px; padding-bottom: 10px; border-bottom: 3px solid var(--secondary); display: flex; align-items: center; gap: 15px; } .section-title .num { background: var(--secondary); color: #fff; width: 38px; height: 38px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; font-size: 0.65em; font-weight: 700; flex-shrink: 0; } h3 { color: var(--dark); font-size: 1.2em; margin: 30px 0 12px; padding-left: 15px; border-left: 4px solid var(--secondary); } h4 { color: var(--primary); font-size: 1.05em; margin: 20px 0 10px; } p { margin-bottom: 15px; text-align: justify; } /\* Tables \*/ table { width: 100%; border-collapse: collapse; margin: 20px 0; font-size: 0.92em; } thead th { background: var(--primary); color: #fff; padding: 12px 16px; text-align: left; font-weight: 600; font-size: 0.92em; } tbody td { padding: 10px 16px; border-bottom: 1px solid #e8e8e8; vertical-align: top; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } /\* Highlight Boxes \*/ .vision-box { background: linear-gradient(135deg, #f0e6ff 0%, #ffe6ee 100%); border-left: 5px solid var(--primary); padding: 25px 30px; border-radius: 0 10px 10px 0; margin: 20px 0; font-size: 1.05em; line-height: 1.8; } .key-insight { background: #f6fef9; border-left: 5px solid var(--success); padding: 20px 25px; border-radius: 0 8px 8px 0; margin: 20px 0; } .key-insight strong { color: var(--success); } .warning-box { background: #fff8f0; border-left: 5px solid var(--warning); padding: 20px 25px; border-radius: 0 8px 8px 0; margin: 20px 0; } .principle-box { background: #f0f4ff; border-left: 5px solid var(--info); padding: 20px 25px; border-radius: 0 8px 8px 0; margin: 20px 0; } /\* Cards Grid \*/ .cards-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin: 20px 0; } .card { background: #fff; border: 1px solid #e8e8e8; border-radius: 10px; padding: 25px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); transition: transform 0.2s; } .card:hover { transform: translateY(-2px); box-shadow: 0 4px 16px rgba(0,0,0,0.1); } .card .card-icon { font-size: 2em; margin-bottom: 10px; } .card h4 { margin: 0 0 8px; color: var(--primary); font-size: 1.05em; } .card p { font-size: 0.9em; color: #666; margin: 0; text-align: left; } /\* Two Column Layout \*/ .two-col { display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin: 20px 0; } .col-box { background: #f8f9fa; border-radius: 10px; padding: 25px; border: 1px solid #e8e8e8; } .col-box h4 { margin-top: 0; padding-bottom: 8px; border-bottom: 2px solid var(--secondary); margin-bottom: 12px; } .col-box ul { padding-left: 20px; } .col-box li { margin-bottom: 6px; } /\* Layer badges \*/ .layer-badge { display: inline-block; padding: 3px 12px; border-radius: 15px; font-size: 0.82em; font-weight: 600; color: #fff; } .layer-1 { background: var(--c16-purple); } .layer-2 { background: var(--secondary); } .layer-3 { background: var(--sydler-blue); } /\* Timeline \*/ .timeline { position: relative; padding: 20px 0; } .timeline::before { content: ''; position: absolute; left: 30px; top: 0; bottom: 0; width: 3px; background: linear-gradient(to bottom, var(--c16-purple), var(--secondary), var(--accent)); } .timeline-item { position: relative; padding-left: 70px; margin-bottom: 30px; } .timeline-item .week-badge { position: absolute; left: 10px; top: 0; width: 42px; height: 42px; background: var(--primary); color: #fff; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.7em; font-weight: 700; z-index: 1; } .timeline-item h4 { margin-top: 0; color: var(--primary); } .timeline-item p { margin-bottom: 5px; font-size: 0.93em; } /\* Architecture Diagram \*/ .arch-diagram { background: #1a1a2e; color: #fff; border-radius: 12px; padding: 35px; margin: 25px 0; font-family: 'Courier New', monospace; font-size: 0.88em; line-height: 1.6; overflow-x: auto; white-space: pre; } /\* Checklist \*/ .checklist { list-style: none; padding: 0; } .checklist li { padding: 8px 0 8px 30px; position: relative; border-bottom: 1px solid #f0f0f0; } .checklist li::before { content: '☐'; position: absolute; left: 0; color: var(--primary); font-size: 1.1em; } /\* Flow Arrows \*/ .flow-container { display: flex; align-items: center; justify-content: center; gap: 10px; margin: 25px 0; flex-wrap: wrap; } .flow-step { background: linear-gradient(135deg, var(--primary), var(--secondary)); color: #fff; padding: 15px 25px; border-radius: 10px; text-align: center; font-weight: 600; font-size: 0.9em; min-width: 160px; } .flow-arrow { font-size: 1.5em; color: var(--secondary); font-weight: 700; } /\* Package Box \*/ .package-box { border: 2px solid var(--primary); border-radius: 12px; overflow: hidden; margin: 25px 0; } .package-header { background: linear-gradient(135deg, var(--primary), var(--secondary)); color: #fff; padding: 15px 25px; } .package-header h4 { color: #fff; margin: 0; font-size: 1.15em; } .package-header .pkg-subtitle { opacity: 0.85; font-size: 0.9em; margin-top: 3px; } .package-body { padding: 25px; } /\* Brand Guidelines \*/ .brand-do-dont { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin: 20px 0; } .brand-do { background: #f0faf0; border: 2px solid var(--success); border-radius: 10px; padding: 20px; } .brand-dont { background: #fef0f0; border: 2px solid var(--danger); border-radius: 10px; padding: 20px; } .brand-do h4 { color: var(--success); margin-top: 0; } .brand-dont h4 { color: var(--danger); margin-top: 0; } /\* Footer \*/ .doc-footer { background: #1a1a2e; color: rgba(255,255,255,0.7); padding: 30px 60px; font-size: 0.85em; text-align: center; } .doc-footer .footer-logos { margin-bottom: 15px; font-size: 1.2em; color: #fff; font-weight: 600; } /\* Mermaid \*/ .mermaid { margin: 20px 0; text-align: center; } /\* Print \*/ @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; margin: 0; max-width: 100%; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .section-title { page-break-after: avoid; } table { page-break-inside: avoid; } .cards-grid { grid-template-columns: repeat(2, 1fr); } } @media (max-width: 768px) { .content { padding: 20px; } .header-banner { padding: 30px 20px; } .cards-grid { grid-template-columns: 1fr; } .two-col { grid-template-columns: 1fr; } .toc ol { columns: 1; } .brand-do-dont { grid-template-columns: 1fr; } } /\* Numbered list styling in sections \*/ .content ol { padding-left: 22px; margin-bottom: 15px; } .content ul { padding-left: 22px; margin-bottom: 15px; } .content li { margin-bottom: 6px; } /\* Emphasis spans \*/ .em-c16 { color: var(--c16-purple); font-weight: 600; } .em-sydler { color: var(--sydler-blue); font-weight: 600; } .em-joint { color: var(--secondary); font-weight: 600; } /\* Stat boxes \*/ .stat-row { display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin: 20px 0; } .stat-box { background: linear-gradient(135deg, #f8f4ff, #fff5f5); border: 1px solid #e8d5f5; border-radius: 10px; padding: 20px; text-align: center; } .stat-box .stat-number { font-size: 1.8em; font-weight: 700; color: var(--primary); display: block; } .stat-box .stat-label { font-size: 0.82em; color: #666; margin-top: 5px; }

![C16](data:image/jpeg;base64,) × SYDLER TECHNOLOGIES

# Strategic Partnership Framework

AI-Augmented SAP Services — Combining Intelligent Automation with Expert Consulting

📄 Version 1.0 📅 April 2026 🔒 Confidential — For Internal Discussion Only 📋 Partnership Blueprint

⚠ CONFIDENTIAL — NOT FOR DISTRIBUTION BEYOND SYDLER AND C16 LEADERSHIP ⚠

## Table of Contents

1.  [Partnership Vision](#vision)
2.  [Market Opportunity](#market)
3.  [Role Definition — Who Does What](#roles)
4.  [Service Architecture — The Three-Layer Model](#architecture)
5.  [Technology Integration & Security](#technology)
6.  [Joint Service Packages](#packages)
7.  [Customer Journey — From Demo to Ongoing](#journey)
8.  [Go-to-Market Model](#gtm)
9.  [Commercial Framework](#commercial)
10.  [Governance & Quality Standards](#governance)
11.  [Pilot Plan — First 3 Customers](#pilot)
12.  [Brand Guidelines](#brand)
13.  [Next Steps & Action Items](#next-steps)

01 Partnership Vision

**Sydler Technologies** brings two decades of SAP domain expertise, trusted customer relationships, and full-lifecycle functional and Basis consulting across manufacturing, pharma, chemicals, and engineering industries.  
  
**C16** brings an AI-powered SAP intelligence platform that connects directly to live SAP systems and can analyse, diagnose, remediate, and deploy ABAP code autonomously — performing in minutes what traditionally takes consultants days or weeks.  
  
**Together,** we create a service offering that neither party can deliver alone: _AI-speed intelligence backed by human-expert judgment._ The result is faster incident resolution, proactive system health monitoring, accelerated S/4HANA migration readiness, and a customer experience that no traditional AMS provider or standalone tool can match.

### The Core Premise

The global SAP market is at an inflection point. With ECC's premium end-of-life approaching in 2027 (extended to 2030), over 37,000 SAP customers face a critical question: _"How do I keep my ECC running reliably while planning my future?"_

Traditional SAP Application Management Services (AMS) are linear — more customers require more consultants, which compresses margins and creates quality variance. The C16 + Sydler partnership breaks this linearity:

5× Faster incident root-cause analysis with C16

80% Of diagnostic work automated by C16

3× More customers per Sydler consultant

100% Consistent quality — 55 analysis rules, every time

**The Strategic Position:** Sydler is the trusted advisor who owns the customer relationship. C16 is the intelligent engine that powers the service. The customer buys expertise from Sydler, augmented by AI from C16. Both brands are visible. Both are essential. Neither is replaceable.

02 Market Opportunity

### 2.1 The ECC Sustaining Market

SAP's ECC customer base represents one of the largest enterprise software installed bases in the world. Despite SAP's push toward S/4HANA, the migration reality is far slower than SAP's marketing suggests:

Market Factor

Data Point

Implication

Global ECC customers still running

~37,000 companies

Massive addressable market for sustaining services

S/4HANA live adoption rate

~25–30% of SAP base (as of mid-2026)

70% are in pre-migration or undecided

Mainstream maintenance PEOL

2027 (extended premium to 2030)

Every ECC customer needs a plan in 12–18 months

Average annual SAP maintenance cost

22% of licence fees ($50K–$500K/year)

Customers paying for maintenance, receiving diminishing value

Average custom code estate

500–3,000 Z-programs per mid-market customer

Nobody fully understands their custom code — knowledge gap is universal

SAP developer talent attrition (India)

20%+ annual turnover in IT services

Institutional knowledge walks out the door every quarter

Third-party support market (Rimini Street alone)

~$430M annual revenue

Proven willingness to pay for alternative SAP support models

### 2.2 The Competitive Landscape

Competitor

What They Offer

Price Range

Gap (Our Opportunity)

**Big 4 SIs** (Deloitte, Accenture, EY, KPMG)

Full AMS + migration consulting

$200K–$1M/year

Expensive, slow, consultant-dependent. No AI acceleration.

**Rimini Street**

Third-party ECC support (SAP Note equivalents)

50% of SAP maintenance

No custom code intelligence. No S/4HANA readiness. Reactive only.

**Panaya**

Static custom code impact analysis

$100K+/year

Reports problems but cannot fix them. No AI remediation.

**Basis Technologies**

Automated code transformation

$200K+

Pattern-based, not AI-driven. Limited understanding of business context.

**Celonis / SAP Signavio**

Process mining

$200K–$500K+

Requires BTP or dedicated infrastructure. 6-month implementation. No custom code coverage.

**Mid-tier Indian SIs**

Traditional AMS with dedicated teams

$5K–$20K/month

Linear cost model. Quality varies by individual consultant. No AI augmentation.

**The Whitespace:** No player in the market offers AI-powered custom code intelligence combined with expert human consulting at a mid-market price point. The Big 4 are too expensive. Rimini Street doesn't cover custom code. Panaya doesn't fix anything. Celonis is too complex. _Sydler + C16 fills this gap precisely._

### 2.3 Geographic Opportunity

Geography

Market Size

Primary Driver

Sydler Presence

Priority

**India**

8,000+ SAP customers, 60%+ on ECC

Manufacturing hub, talent attrition, cost optimisation

Strong — HQ, existing clients

🥇 Primary

**Middle East**

3,000+ SAP installations (KSA, UAE, Qatar)

Vision 2030, ZATCA e-invoicing, digital mandates

Existing presence

🥈 Secondary

**United States**

12,000+ SAP customers

Board pressure, ABAP talent retirement, SOX compliance

Expansion target

🥉 Growth

**Europe**

10,000+ SAP customers

DORA compliance, Mittelstand migration, GDPR

Expansion target

4th — Future

03 Role Definition — Who Does What

The success of this partnership depends on absolute clarity about responsibilities. There must be zero ambiguity about which party owns which capability.

### 3.1 C16's Domain — The Intelligent Engine

#### 🤖 C16 Provides

Capability

What C16 Does

Speed

**Custom Code Analysis**

Reads any Z-program, produces Clean Core compliance score (0–100), identifies violations, classifies Level A/B/C/D, maps dependencies

5 minutes per program

**Knowledge Documentation**

Reverse-engineers programs into readable functional specifications: what it does, what tables it uses, what business rules it encodes

10 minutes per program

**Incident Diagnosis**

Reads runtime short dumps, identifies root cause, traces to specific line of code, suggests fix with complete ABAP code

2–5 minutes per incident

**Code Remediation**

Rewrites non-compliant programs (Level C/D → Level A/B), deploys to SAP, runs syntax and ATC checks

15–30 minutes per program

**Performance Optimisation**

Detects P1–P5 anti-patterns (SELECT in LOOP, SELECT \*, missing JOINs), generates optimised code

5 minutes per program

**S/4HANA Readiness Analysis**

Maps custom code against SAP Simplification List (Note 2313884), identifies Fit-to-Standard opportunities, recommends standard Fiori replacements

Batch: 500 programs in 4 hours

**Process Intelligence**

Mines SAP transactional data (P2P, O2C cycle times, bottlenecks, variant analysis) via direct table reads

10 minutes per process

**Security Posture Scanning**

Identifies missing AUTHORITY-CHECKs, SAP\_ALL users, segregation-of-duty risks, unprotected transactions

Monthly automated scan

**SmartForm Conversion**

Converts SmartForms to Adobe Forms automatically — downloads, parses, generates XDP + deployment ABAP

30 minutes per form

### 3.2 Sydler's Domain — The Expert Hand

#### 👨‍💼 Sydler Provides

Capability

What Sydler Does

Why Human-Only

**Customer Relationship Management**

Primary point of contact, SLA ownership, escalation management, quarterly business reviews

Trust is built between humans, not between humans and AI

**Functional Consulting**

Business process interpretation, configuration decisions, regulatory/tax updates, process redesign

Business decisions require domain judgment and stakeholder alignment

**Basis Administration**

Kernel patches, database administration, system copies, transport route management, performance tuning at OS/DB level

Infrastructure operations require system-level access and operational expertise

**SAP Note Application**

Standard SAP correction notes, support packages, compatibility testing

Requires regression testing judgment and transport sequencing expertise

**Quality Assurance**

Reviews all C16-generated code before production deployment, validates business logic, approves transports

Human sign-off is the quality gate — AI proposes, human disposes

**24/7 Support SLA**

Support desk coverage, P1 incident war rooms, on-call rotation

Customers need a human voice at 2 AM when production is down

**Change Management**

ITIL processes, change advisory board, documentation, training

Organisational change requires human facilitation

**User Training**

End-user training, super-user workshops, go-live support

Teaching requires empathy, patience, and adaptability to learning styles

**Audit Support**

External/internal auditor meetings, compliance documentation, sign-offs

Auditors require human accountability and explanations

### 3.3 The Handoff Zone — Where Both Collaborate

For certain services, C16 performs 80% of the work and Sydler completes the remaining 20%. This is the highest-value zone — it's where the partnership creates the most efficiency gain.

Service

C16 Does (80%)

Sydler Does (20%)

Efficiency Gain

**Incident Resolution**

Reads dump → identifies root cause → writes fix → syntax check

Reviews fix logic → validates business rules → approves transport → deploys to production

Resolution in hours instead of days

**Performance Fix**

Detects slow program → generates optimised rewrite → tests

Validates output matches → UAT with business → manages transport

2 days instead of 2 weeks

**Enhancement Request**

Analyses existing code → generates modified version with new feature

Reviews functional correctness → tests with business user → transports

3 days instead of 2 weeks

**Migration Assessment**

Scans all Z-programs → classifies → maps to simplification items → generates report

Interprets findings in business context → presents to CIO → builds migration roadmap

1 week instead of 3 months

**Configuration Analysis**

Reads config tables → identifies gaps → produces gap report

Interprets gaps → recommends changes → implements config

Same day instead of 1 week

**The Golden Rule:** C16 proposes. Sydler validates and approves. No C16-generated code reaches a production system without Sydler's explicit review and sign-off. This principle is non-negotiable and is the foundation of quality assurance in the partnership.

04 Service Architecture — The Three-Layer Model

All services delivered through this partnership are organised into three layers, defined by the degree of AI automation versus human involvement:

┌─────────────────────────────────────────────────────────────────────────┐ │ CUSTOMER'S SAP SYSTEM │ │ (ECC 6.0 / S/4HANA / Any Release) │ └───────────────────────────────┬─────────────────────────────────────────┘ │ RFC / HTTP-ADT Connection (Read-only initially) │ ┌───────────────────────────────▼─────────────────────────────────────────┐ │ │ │ ┌─────────────────────────────────────────────────────────────────┐ │ │ │ LAYER 1: C16 AUTOMATED INTELLIGENCE │ │ │ │ ───────────────────────────────────────── │ │ │ │ • Monthly Code Health Dashboard │ │ │ │ • Clean Core Portfolio Scoring & Trending │ │ │ │ • Knowledge Vault (auto-documentation) │ │ │ │ • Performance Anti-Pattern Detection │ │ │ │ • Security Posture Scanning │ │ │ │ • S/4HANA Readiness Scoring │ │ │ │ │ │ │ │ \[100% C16 — Sydler monitors dashboard, flags to customer\] │ │ │ └────────────────────────────┬────────────────────────────────────┘ │ │ │ │ │ ┌────────────────────────────▼────────────────────────────────────┐ │ │ │ LAYER 2: JOINT DELIVERY (C16 80% + SYDLER 20%) │ │ │ │ ───────────────────────────────────────── │ │ │ │ • Incident Root-Cause Analysis + Fix │ │ │ │ • Performance Optimisation + Deployment │ │ │ │ • Enhancement Requests (code changes) │ │ │ │ • SmartForm Conversion + Testing │ │ │ │ • Code Remediation (Level C→B) + UAT │ │ │ │ • Configuration Gap Analysis + Recommendations │ │ │ │ • Migration Assessment + Executive Presentation │ │ │ │ │ │ │ │ \[C16 does heavy lifting — Sydler reviews, validates, deploys\] │ │ │ └────────────────────────────┬────────────────────────────────────┘ │ │ │ │ │ ┌────────────────────────────▼────────────────────────────────────┐ │ │ │ LAYER 3: SYDLER HUMAN-ONLY │ │ │ │ ───────────────────────────────────────── │ │ │ │ • 24/7 Support SLA & P1 War Rooms │ │ │ │ • Basis Administration (kernel, DB, transports) │ │ │ │ • SAP Standard Note Application │ │ │ │ • Regulatory & Tax Updates (GST, ZATCA, SOX) │ │ │ │ • Functional Consulting & Process Redesign │ │ │ │ • User Training & Change Management │ │ │ │ • Audit Support & Compliance Sign-Off │ │ │ │ • Data Migration & Cutover │ │ │ │ │ │ │ │ \[100% Sydler — C16 not involved\] │ │ │ └────────────────────────────────────────────────────────────────┘ │ │ │ └─────────────────────────────────────────────────────────────────────────┘

### 4.1 Layer Characteristics

Attribute

Layer 1: C16 Automated

Layer 2: Joint Delivery

Layer 3: Sydler Only

**Automation Level**

100% automated

80% automated, 20% human

0% automated, 100% human

**Delivery Speed**

Minutes to hours

Hours to 2 days

Days to weeks

**Scalability**

Unlimited (AI)

Semi-scalable (human bottleneck at 20%)

Linear (headcount-dependent)

**Human Judgment Required**

Low (monitoring only)

Medium (review + approval)

High (every decision is human)

**Customer Data Sensitivity**

Code only (no business data)

Code + limited config data

Full system access

**Margin Profile**

Highest (near-zero marginal cost)

High (minimal human hours)

Standard AMS (headcount-based)

05 Technology Integration & Security

### 5.1 How C16 Connects to Customer SAP Systems

C16 connects to SAP systems via standard, well-understood protocols that every SAP Basis team is familiar with. No agents, no middleware, no custom infrastructure required.

Connection Method

Protocol

Purpose

SAP Standard?

**RFC over HTTPS**

SOAP/RFC

Read table data, call function modules, read program source

✅ Yes — same as any RFC client (SAP GUI, Eclipse ADT)

**ADT REST API**

HTTPS REST

Read/write ABAP objects, syntax checks, ATC checks, activation

✅ Yes — same as Eclipse ADT IDE

**OData Services**

HTTPS OData V2/V4

Read business data (when authorised for process intelligence)

✅ Yes — same as Fiori apps

### 5.2 Graduated Access Model

Customer trust is built incrementally. C16 access starts minimal and expands only as the customer is comfortable:

Phase

System

Access Level

What C16 Can Do

What C16 Cannot Do

**Phase 1**  
(Week 1–4)

DEV / Sandbox only

Read-only

Read program source code, read table structures (DD03L), read class/FM metadata, syntax check

Cannot modify any object, cannot read business data, cannot access production

**Phase 2**  
(Month 2–3)

DEV system

Read + Deploy to $TMP

All of Phase 1 + deploy remediated code to DEV system ($TMP package), run ATC checks, run unit tests

Cannot deploy to transportable packages, cannot access QA/PROD, cannot read business data

**Phase 3**  
(Month 4+)

DEV + QA (optional)

Read + Deploy + Business Data (read-only)

All of Phase 2 + read transactional data for process intelligence (BSIK, EKKO, VBAK, etc.)

Cannot write business data, cannot access production directly, cannot execute transactions

**⚠️ Production Access:** C16 never connects directly to production systems. All code remediation is deployed to DEV, tested by Sydler, and transported to production through the customer's standard transport path. This is a deliberate design choice — not a limitation.

### 5.3 SAP Service User Configuration

C16 requires a dedicated SAP service user (type "Communication" or "System") with specific, minimal authorisations. The principle of least privilege is enforced:

Authorisation Object

Purpose

Scope

S\_RFC

RFC function module access

Limited to specific function groups (SYST, SDIF, SRFC, SEDT)

S\_DEVELOP

ABAP development operations

OBJTYPE restricted to PROG/CLAS/DDLS only; ACTVT restricted to 02 (Change) and 03 (Display)

S\_TABU\_NAM

Table read access

Restricted to specific technical tables (DD03L, TADIR, TRDIR, TFDIR — NOT business data tables)

S\_ADT\_RES

ADT REST API access

Required for source code read/write via Eclipse-compatible protocol

**Explicitly excluded:** SAP\_ALL, SAP\_NEW, S\_USER\_\* (user administration), S\_BTCH\_ADM (batch administration), S\_TCODE (unrestricted transaction access). The C16 service user cannot log into SAP GUI, cannot run transactions, and cannot modify user master records.

### 5.4 Data Handling & Compliance

Concern

How It's Addressed

**Data at rest**

C16 does not store customer source code permanently. Analysis is performed in-memory. Reports are generated and delivered; source code is not retained after the session ends.

**Data in transit**

All connections use HTTPS (TLS 1.2+). RFC calls are encrypted via SNC or HTTPS tunnelling.

**Business data**

Phase 1 and 2 access NO business data. Phase 3 business data access is read-only and subject to customer-defined authorisation restrictions via SAP's native auth model.

**GDPR / Data Privacy**

C16 reads SAP table structures and code logic — not personal data records. When process intelligence is enabled (Phase 3), SAP's native authorisation restricts which data C16 can read.

**Multi-tenancy**

Each customer connection is isolated. Customer A's data/code is never accessible during Customer B's session.

06 Joint Service Packages

Three service packages are defined, each building on the previous. Customers typically start with Package A or B and expand over time.

#### 📦 Package A: ECC Sustaining Intelligence

"Keep your ECC running smarter while you plan your future"

**Target Customer:** ECC customers with active AMS contracts who want proactive intelligence on top of reactive support.

Service

Delivered By

Frequency

Deliverable

Custom Code Health Dashboard

C16

Monthly

HTML report: Clean Core scores, trending, risk heat map

Knowledge Vault

C16

Quarterly refresh

Searchable documentation of all Z-programs

Performance Anti-Pattern Scan

C16

Monthly

P1–P5 findings with remediation priorities

Security Posture Scan

C16

Monthly

Auth gaps, SAP\_ALL users, SoD risks

Incident Resolution (L2/L3)

Joint

As needed

Root cause analysis + fix deployed to DEV

Basis Administration

Sydler

Ongoing

Kernel, DB, transports, system copies

24/7 SLA (P1: 15 min)

Sydler

Ongoing

Support desk with shift coverage

Monthly Review Call

Sydler

Monthly

Dashboard walkthrough + action items

**Pricing:** _Monthly subscription — to be mutually agreed based on system size and scope._

#### 📦 Package B: S/4HANA Migration Readiness Assessment

"Know exactly what you have, what it costs to migrate, and what to do first"

**Target Customer:** Any ECC customer evaluating S/4HANA migration timeline and investment.

Service

Delivered By

Deliverable

Full Z-Portfolio Scan (all programs, classes, function groups, enhancements)

C16

Complete Clean Core classification: Level A/B/C/D per object

Top 50 Program Deep Analysis (CC1–CC8 scoring)

C16

15-section analysis per program: functional spec, Fit-to-Standard, remediation roadmap

Simplification Item Impact Mapping

C16

SAP Note 2313884 mapping per custom object

SmartForm & SAPscript Inventory

C16

Complete form catalogue with complexity classification

Enhancement & User Exit Inventory

C16

All BAdIs, customer exits, enhancement spots with Clean Core status

Process Efficiency Baseline (P2P + O2C)

Joint

Cycle time analysis, variant detection, bottleneck identification

Business Context Interpretation

Sydler

Map findings to business processes, identify critical-path programs

Executive Presentation + Migration Roadmap

Sydler

Board-ready presentation with timeline, investment estimate, risk assessment

**Pricing:** _Fixed price — to be mutually agreed based on portfolio size (number of Z-objects)._

**Delivery Timeline:** 2–4 weeks depending on portfolio size.

#### 📦 Package C: Custom Code Factory — Continuous Remediation

"Don't just know your problems — fix them. Continuously."

**Target Customer:** Customers who have completed Package B and want to actively remediate their custom code estate in preparation for S/4HANA.

Service

Delivered By

Frequency

Deliverable

Monthly Remediation Sprint (batch of programs)

Joint

Monthly

C16 rewrites Level C/D → Level A/B; Sydler tests + transports

New Code Compliance Gating

C16

Per new program

Every new Z-program scanned before go-live; compliance enforced

SmartForm → Adobe Conversion

Joint

Batch per sprint

Automated conversion + manual print testing

Clean Core Score Tracking

C16

Monthly

Portfolio-level trending: "60% Level B last month → 68% this month"

Functional Regression Testing

Sydler

Per sprint

UAT of remediated programs with business users

Quarterly Executive Review

Sydler

Quarterly

Progress report, updated migration readiness score, next sprint plan

**Pricing:** _Monthly subscription — to be mutually agreed based on remediation volume and complexity._

### 6.1 Package Progression — The Natural Upsell Path

Package B  
Migration Assessment  
(One-time)

→

Package A  
Sustaining Intelligence  
(Monthly)

→

Package C  
Code Factory  
(Monthly Add-on)

→

S/4HANA Migration  
Full Project  
(Sydler-led)

Most customers enter through Package B (assessment), convert to Package A (ongoing monitoring), add Package C (remediation) as they approach migration, and ultimately engage Sydler for the full S/4HANA migration project. This creates a multi-year engagement arc.

07 Customer Journey — From Demo to Ongoing

### Phase 1: The 15-Minute Proof of Value (Free)

The first touchpoint with a prospective customer is a live demonstration on their own SAP system. This is not a slide deck — it's real analysis of their real code.

Step

What Happens

Who

Duration

1

Sydler account manager arranges a demonstration meeting with existing AMS customer

Sydler

—

2

Customer provides DEV system connection details (host, client, service user)

Customer

5 min

3

C16 connects, scans 50 Z-programs, produces Clean Core heat map

C16

15 min

4

Live findings presented: "You have X Level C programs, Y have no authorisation checks, Z can be replaced by standard Fiori apps"

Sydler + C16

15 min

5

Customer sees findings about their own system they've never seen before

—

—

6

Sydler proposes Package B (full assessment) based on findings

Sydler

5 min

**Why this works:** The customer sees intelligence about their own system in 15 minutes. No other vendor can do this. It's not a promise — it's a proof. The assessment sells itself after the demo.

### Phase 2: Discovery Engagement (Package B — Fixed Price)

Week

Activities

Who

Deliverable

**Week 1**

Full Z-portfolio scan. All programs, classes, function groups classified. Clean Core scoring.

C16

Portfolio Health Report (HTML)

**Week 2**

Top 50 programs deep analysis. Functional specs, Fit-to-Standard, remediation roadmaps.

C16

50× Deep Analysis Reports (HTML)

**Week 3**

Sydler reviews all findings. Adds business context. Maps to business processes. Identifies critical path.

Sydler

Business Context Overlay (annotated)

**Week 4**

Executive presentation to customer CIO/IT head. Migration roadmap. Investment estimate.

Sydler + C16

Board-Ready Assessment Deck

### Phase 3: Ongoing Intelligence (Package A — Monthly Subscription)

After the assessment, the customer sees the value of continuous monitoring. Sydler's account manager proposes Package A:

*   Monthly health dashboard delivered automatically by C16
*   Incidents resolved 5× faster with C16 diagnosis
*   Sydler's existing AMS team augmented (not replaced) by AI
*   Quarterly review meetings with trending data

### Phase 4: Active Remediation (Package C — Add-on)

As the customer approaches their S/4HANA migration timeline, they engage Package C to actively fix their custom code:

*   Monthly sprints: C16 rewrites a batch of programs, Sydler tests and transports
*   Clean Core score improves month over month — visible, measurable progress
*   When migration day arrives, 80% of custom code is already compliant
*   This dramatically reduces migration project risk, cost, and timeline

08 Go-to-Market Model

### 8.1 Primary Channel: Sydler's Existing AMS Customers

The fastest path to revenue is Sydler's existing customer base. These customers already trust Sydler, already have SAP systems, and already have pain points that this partnership solves.

Advantage

Detail

**Trust is pre-built**

Customer already works with Sydler. No new vendor evaluation needed.

**Access is easy**

Sydler already has access to customer DEV systems for AMS. Adding C16 is a configuration change, not a procurement cycle.

**Pain is known**

Sydler's consultants already know which customers struggle with custom code complexity, slow incident resolution, or S/4HANA anxiety.

**Upsell vs. new sell**

Adding AI intelligence to an existing AMS contract is an upsell. Upsells close 5× faster than new customer acquisitions.

### 8.2 Sydler Account Manager Talking Points

#### When a Customer Asks About S/4HANA Migration:

_"Before you engage a system integrator for migration, you need to know exactly what you have. We've partnered with C16 — an AI platform that connects to your SAP system and analyses every custom program in your estate. In 2 weeks, we'll give you a complete readiness report: which programs can be retired, which need remediation, which can be replaced by standard. This gives you leverage when negotiating with your migration SI."_

#### When a Customer Complains About Slow Incident Resolution:

_"We've added an AI-powered diagnosis capability to our AMS service. When an incident comes in, C16 reads the error dump and identifies the root cause in minutes — not hours. Your Sydler consultant then validates the fix and deploys it. Same quality assurance, 5× faster resolution."_

#### When a Customer Mentions Knowledge Loss / Developer Attrition:

_"We have a solution for that. C16 can read every custom program in your system and produce complete documentation — what it does, what tables it uses, what business rules it contains. When someone leaves, the knowledge stays. Your new team member can read the documentation and be productive in days, not months."_

### 8.3 Secondary Channel: New Customer Acquisition

For customers not currently in Sydler's AMS portfolio, the entry point is Package B (Migration Assessment). This is a fixed-price, low-commitment engagement that demonstrates value and opens the door to ongoing services.

Stage

Activity

Owner

**Lead Generation**

Industry events (SAP TechEd, ASUG chapters), webinars, LinkedIn thought leadership, partner referrals

Joint — Sydler relationships + C16 content

**Qualification**

Verify: runs ECC, has 200+ Z-programs, S/4HANA on roadmap (or should be)

Sydler

**Demo**

15-minute proof of value on customer's DEV system

Joint — Sydler presents, C16 analyses

**Proposal**

Fixed-price Package B proposal

Sydler (using joint proposal template)

**Delivery**

2–4 week assessment

Joint

**Expansion**

Package A (monitoring) → Package C (remediation) → Full AMS

Sydler

09 Commercial Framework

**Guiding Principle:** The commercial model must be simple, fair, and aligned — both parties should benefit when a customer succeeds. Pricing specifics will be mutually agreed and documented in a separate commercial addendum.

### 9.1 Revenue Share Structure Options

Three models are presented for discussion. The final model will be agreed upon before the pilot phase begins.

Model

How It Works

Best When

Considerations

**Model 1: Platform Licence**

Sydler pays C16 a fixed monthly licence fee per connected SAP system. Sydler sets the end-customer price independently and keeps the margin.

Sydler wants pricing control and maximum flexibility with their customers.

Predictable cost for Sydler. C16 revenue doesn't depend on Sydler's pricing to customers.

**Model 2: Revenue Share**

No upfront licence fee. C16 receives a percentage of the revenue Sydler earns from AI-augmented services. Percentage to be mutually agreed.

Starting phase — low risk for Sydler, aligned incentives.

Requires transparency on Sydler's billing. Both parties earn more as customer base grows.

**Model 3: Consumption-Based**

C16 charges per unit of consumption: per analysis report generated, per remediation deployed, per system scanned. No fixed fee.

Usage varies widely between customers. Pays for what you use.

Most granular. Works well for Package B (assessment) where scope varies.

### 9.2 Recommended Hybrid Approach

A combination typically works best for partnerships at this stage:

*   **Package A (Sustaining):** Model 1 (Platform Licence) — fixed monthly fee per system, predictable for both parties
*   **Package B (Assessment):** Model 3 (Consumption) — per-analysis pricing, scales with assessment scope
*   **Package C (Factory):** Model 3 (Consumption) — per-remediation pricing, aligns cost with value delivered

### 9.3 Commercial Commitments

Term

Framework

**Minimum commitment**

To be agreed — suggest 12-month partnership term with 90-day review cycles

**Invoicing**

Customer receives single invoice from Sydler. Sydler pays C16 per agreed model.

**Payment terms**

To be mutually agreed (suggest Net 30 for C16 → Sydler billing)

**Volume incentives**

Tiered discounts on C16 platform fee as Sydler scales connected systems

**Exclusivity**

To be discussed — geographic and/or industry-specific exclusivity options available

10 Governance & Quality Standards

### 10.1 Joint Governance Structure

Forum

Participants

Frequency

Agenda

**Steering Committee**

Sydler MD/Director + C16 Founder

Monthly (first 6 months), then Quarterly

Partnership health, revenue review, strategic direction, new offering ideas

**Delivery Review**

Sydler Delivery Manager + C16 Technical Lead

Weekly (first 3 months), then Bi-weekly

Active engagements, quality issues, customer feedback, tool enhancements

**Technical Sync**

Sydler Senior ABAP Lead + C16 Platform Team

Weekly

C16 platform updates, new features, integration issues, feedback loop

### 10.2 Quality Gates

Gate

What's Checked

Who Checks

Consequence of Failure

**G1: C16 Analysis Output**

Are findings accurate? Are line numbers correct? Are CDS recommendations verified?

Sydler Senior ABAP Consultant

Report sent back to C16 for correction before customer delivery

**G2: Code Remediation**

Does remediated code compile? Does ATC pass? Does output match original?

Sydler QA Team

Code not transported until all checks pass

**G3: Customer Deliverable**

Is the document professional? Is business context accurate? Are recommendations actionable?

Sydler Delivery Manager

Deliverable revised before customer presentation

**G4: Production Deployment**

Has UAT been completed? Has the business signed off? Is the transport tested in QA?

Sydler + Customer Change Manager

No production transport without formal sign-off

### 10.3 Escalation Matrix

Issue Type

Level 1 (Same Day)

Level 2 (Within 48h)

Level 3 (Within 1 Week)

C16 analysis accuracy issue

C16 Technical Lead

C16 Founder

Joint Steering Committee

Customer complaint about service quality

Sydler Delivery Manager

Sydler Director

Joint Steering Committee

SLA breach on incident resolution

Sydler Support Lead

Sydler Director + C16 Technical Lead

Joint Steering Committee

Commercial dispute

—

Sydler Director + C16 Founder

Joint Steering Committee

### 10.4 Continuous Improvement Loop

C16's AI platform improves with every engagement. A structured feedback mechanism ensures learnings are captured:

1.  **Every C16 report reviewed by Sydler** → corrections fed back to C16 as "learned corrections" (built-in feedback mechanism)
2.  **Every production deployment tracked** → success/failure data improves C16's code generation patterns
3.  **Monthly "lessons learned" review** → patterns across multiple customers identified and productised
4.  **Quarterly platform updates** → new capabilities based on field experience

11 Pilot Plan — First 3 Customers

### 11.1 Customer Selection Criteria

The pilot customers should be selected from Sydler's existing AMS portfolio based on these criteria:

Criterion

Ideal Profile

Why

**System**

ECC 6.0 EhP 5+ (SAP Basis 7.02+)

C16 requires ABAP 7.02+ for full functionality

**Custom code estate**

200–1,000 Z-programs

Large enough to show value, small enough to scan completely in the pilot

**Industry**

Manufacturing or Pharma (Sydler's strongest sectors)

Sydler's functional consultants can add the most business context

**Relationship**

Active AMS contract, good rapport with IT head

Easy access, willing to participate, constructive feedback expected

**S/4HANA intent**

On the roadmap (2–4 year horizon)

Migration urgency makes the value proposition most compelling

**Risk tolerance**

Willing to provide DEV system access for a new tool

Must be open to innovation, not risk-averse to a fault

### 11.2 Pilot Timeline — 8 Weeks

W1

#### Week 1: Setup & Connection

**C16:** Connect to pilot customer's DEV system. Verify RFC connectivity, ADT access, service user permissions.

**Sydler:** Coordinate with customer Basis team for service user creation. Brief customer IT head on what to expect.

**Milestone:** C16 successfully reads program list from customer system.

W2

#### Week 2: Portfolio Scan

**C16:** Full Z-portfolio scan. Clean Core classification of all programs. Risk heat map generated.

**Sydler:** Review scan results for accuracy. Identify top 20 business-critical programs from customer knowledge.

**Milestone:** Portfolio Health Dashboard delivered to Sydler for review.

W3-4

#### Weeks 3–4: Deep Analysis

**C16:** Deep analysis of top 20 programs. Full 15-section reports: functional specification, Fit-to-Standard, Clean Core scoring, remediation roadmap.

**Sydler:** Review each report. Add business context annotations. Validate Fit-to-Standard recommendations against customer's actual processes.

**Milestone:** 20 validated deep analysis reports ready.

W5

#### Week 5: Knowledge Vault Generation

**C16:** Generate documentation for all Z-programs — searchable knowledge base: what each program does, tables used, business rules encoded.

**Sydler:** Review documentation quality. Add notes where business context is missing or nuanced.

**Milestone:** Knowledge Vault delivered as searchable HTML site.

W6

#### Week 6: Proof of Remediation

**C16:** Select 3 Level C programs. Rewrite to Level B. Deploy to DEV ($TMP). Syntax check + ATC pass.

**Sydler:** Functional testing of remediated code. Verify output matches original. Document test results.

**Milestone:** 3 programs remediated, tested, and documented. Proof that C16 can fix — not just find.

W7

#### Week 7: Customer Presentation

**Sydler:** Present all findings to customer CIO/IT Head: portfolio health, deep analysis highlights, knowledge vault demo, remediation proof, migration readiness score.

**C16:** Available for live demo / Q&A if needed.

**Milestone:** Customer sees the full value proposition with their own data.

W8

#### Week 8: Review & Decision

**Joint:** Internal pilot review: what worked, what didn't, customer feedback, quality assessment, commercial viability.

**Decision:** Proceed to Package A (sustaining) for pilot customer? Scale to 2nd and 3rd customer?

**Milestone:** Pilot success criteria evaluated. Go/no-go for scale-up.

### 11.3 Pilot Success Metrics

Metric

Target

How Measured

Portfolio scan accuracy

\>95% of classifications validated correct by Sydler

Sydler spot-checks 20 random programs

Deep analysis quality

\>90% of findings confirmed accurate

Sydler functional review of all 20 reports

Remediation success rate

3/3 programs pass syntax + ATC + functional test

SAP system validation + Sydler testing

Customer satisfaction

Customer agrees to continue with Package A

Verbal/written confirmation from customer IT head

Time efficiency

Total effort < 50% of traditional assessment effort

Compare actual hours to Sydler's estimate for manual equivalent

12 Brand Guidelines

### 12.1 Brand Positioning

**Sydler Technologies** is the service face — the trusted advisor who owns the customer relationship, delivers the expertise, and ensures quality.  
  
**C16** is the technology platform — the intelligent engine that powers the analysis, generates the insights, and accelerates the delivery.  
  
Think: **Accenture (service) + Salesforce (platform)**. Both brands are visible. Both are valued. The customer buys a service from Sydler that is powered by C16.

### 12.2 Brand Usage Rules

#### ✅ DO

*   Use "Sydler Technologies, powered by C16" in customer-facing materials
*   Display both logos on joint deliverables (proposals, reports, presentations)
*   Refer to C16 as "our AI intelligence platform" or "our technology partner"
*   Include C16 branding on all AI-generated reports and dashboards
*   Credit C16 platform capabilities in service descriptions
*   Use "Sydler + C16" in joint marketing materials

#### ❌ DON'T

*   Don't present C16 as a "Sydler internal tool" — it's a named technology partner
*   Don't remove C16 branding from generated reports
*   Don't position C16 as the service provider — Sydler owns the customer relationship
*   Don't use C16 to sell directly to Sydler's existing AMS customers (channel conflict)
*   Don't claim C16 capabilities as Sydler-built IP
*   Don't position this as "AI replacing consultants" — it's "AI augmenting consultants"

### 12.3 Messaging Framework

Audience

Key Message

**Customer CIO**

"Sydler brings you AI-augmented SAP services — faster incident resolution, proactive system health monitoring, and S/4HANA readiness intelligence. Powered by C16, the most advanced SAP code intelligence platform."

**Customer IT Manager**

"Your Sydler support team now has an AI co-pilot. Issues that took 3 days to diagnose are resolved in hours. Every custom program is documented and monitored. You'll never be surprised by your own code again."

**Sydler Consultant**

"C16 handles the grunt work — scanning code, finding problems, writing documentation. You focus on the high-value work: understanding the business, making decisions, presenting to stakeholders."

**Market / Industry**

"Sydler Technologies and C16 have partnered to deliver the first AI-augmented SAP Application Management Service. Combining 20 years of SAP consulting expertise with AI-powered code intelligence for faster, smarter, more proactive SAP support."

13 Next Steps & Action Items

#

Action

Owner

Target Date

Dependency

1

Review and approve this Partnership Framework document

Sydler MD + C16 Founder

Week 1

—

2

Agree on commercial model (Platform Licence / Revenue Share / Hybrid) and document in a Commercial Addendum

Joint

Week 1–2

After #1

3

Select 3 pilot customers from Sydler's AMS portfolio using the criteria in Section 11

Sydler

Week 2

After #1

4

Arrange DEV system access for Pilot Customer #1 (service user + RFC connectivity)

Sydler + Customer Basis team

Week 2–3

After #3

5

Execute 15-minute proof-of-value demo for Pilot Customer #1

Joint (Sydler presents, C16 analyses)

Week 3

After #4

6

Begin 8-week pilot execution (per Section 11.2 timeline)

Joint

Week 4–11

After #5

7

Prepare joint customer-facing proposal template and service brochure (Document 2)

C16 (draft) + Sydler (review)

Week 3

After #1

8

Pilot review and scale-up decision

Joint Steering Committee

Week 12

After #6

**The Bottom Line:** This partnership combines Sydler's two decades of SAP consulting excellence with C16's AI-powered intelligence platform to create a service offering that is faster, smarter, and more proactive than anything available in the market today. Neither party can build this alone. Together, we serve ECC customers better than the Big 4 at a fraction of the cost — and position them for a successful S/4HANA future.

  
  

Prepared for joint review by Sydler Technologies and C16 leadership.  
This document is a framework for discussion — all commercial terms are subject to mutual agreement.

  

For C16

Date: \_\_\_\_\_\_\_\_\_\_\_\_\_

For Sydler Technologies

Date: \_\_\_\_\_\_\_\_\_\_\_\_\_

C16 × Sydler Technologies

Strategic Partnership Framework v1.0 — April 2026

This document is confidential and intended solely for the leadership teams of C16 and Sydler Technologies.

© 2026 C16 & Sydler Technologies. All rights reserved.

mermaid.initialize({startOnLoad:true, theme:'neutral'});