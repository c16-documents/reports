  Material Creation Process Simplification — Polaris MDG mermaid.initialize({startOnLoad:true, theme:'neutral', gantt:{useWidth:900}}); :root { --primary: #6B3FA0; --primary-light: #8B5CF6; --accent: #B5567A; --accent-warm: #C97B3A; --success: #27ae60; --warning: #e67e22; --danger: #e74c3c; --info: #3498db; --bg: #e9ecef; --card-bg: #ffffff; --text: #2c3e50; --text-light: #5a6c7d; --border: #dee2e6; } \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, Arial, sans-serif; color: var(--text); background: var(--bg); line-height: 1.7; font-size: 15px; } .doc-wrapper { max-width: 1140px; margin: 30px auto; background: var(--card-bg); box-shadow: 0 4px 24px rgba(0,0,0,0.10); border-radius: 12px; overflow: hidden; } /\* ── Header Banner ── \*/ .header-banner { background: linear-gradient(135deg, #C97B3A 0%, #B5567A 40%, #6B3FA0 100%); color: #fff; padding: 48px 56px 44px; position: relative; overflow: hidden; } .header-banner::after { content: ''; position: absolute; top: -60px; right: -60px; width: 280px; height: 280px; border-radius: 50%; background: rgba(255,255,255,0.06); } .header-banner .logo { height: 50px; border-radius: 8px; margin-bottom: 14px; } .header-banner h1 { font-size: 2.1em; font-weight: 700; letter-spacing: -0.5px; margin-bottom: 6px; } .header-banner .subtitle { font-size: 1.15em; opacity: 0.92; margin-bottom: 18px; } .header-banner .meta-pills { display: flex; gap: 10px; flex-wrap: wrap; } .header-banner .pill { display: inline-block; background: rgba(255,255,255,0.18); border: 1px solid rgba(255,255,255,0.3); padding: 4px 14px; border-radius: 20px; font-size: 0.82em; font-weight: 500; backdrop-filter: blur(4px); } /\* ── Content Area ── \*/ .content { padding: 40px 56px 56px; } /\* ── Section Titles ── \*/ .section-title { font-size: 1.45em; font-weight: 700; color: var(--primary); border-bottom: 3px solid var(--accent); padding-bottom: 10px; margin: 44px 0 20px; display: flex; align-items: center; gap: 14px; } .section-title .num { display: inline-flex; align-items: center; justify-content: center; width: 34px; height: 34px; background: var(--accent); color: #fff; border-radius: 50%; font-size: 0.72em; font-weight: 700; flex-shrink: 0; } h3 { font-size: 1.12em; color: var(--primary); margin: 28px 0 12px; padding-left: 14px; border-left: 4px solid var(--accent); } h4 { font-size: 1.0em; color: var(--text); margin: 20px 0 10px; font-weight: 600; } /\* ── TOC ── \*/ .toc { background: linear-gradient(135deg, #f8f6fc, #f3f0fa); border: 1px solid #e0d6f0; border-radius: 10px; padding: 24px 32px; margin: 24px 0 36px; } .toc-title { font-size: 1.1em; font-weight: 700; color: var(--primary); margin-bottom: 14px; } .toc-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6px 28px; } .toc a { color: var(--primary); text-decoration: none; font-size: 0.92em; padding: 3px 0; display: block; transition: color 0.2s; } .toc a:hover { color: var(--accent); text-decoration: underline; } /\* ── KPI Cards ── \*/ .kpi-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px; margin: 20px 0 28px; } .kpi-card { background: linear-gradient(135deg, #f8f6fc, #fff); border: 1px solid #e0d6f0; border-radius: 10px; padding: 20px; text-align: center; transition: transform 0.2s; } .kpi-card:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(107,63,160,0.10); } .kpi-card .label { font-size: 0.82em; color: var(--text-light); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px; } .kpi-card .value { font-size: 2.0em; font-weight: 700; color: var(--primary); } .kpi-card .sub { font-size: 0.82em; color: var(--text-light); margin-top: 4px; } .kpi-card.danger .value { color: var(--danger); } .kpi-card.success .value { color: var(--success); } .kpi-card.warning .value { color: var(--warning); } /\* ── Tables ── \*/ table { width: 100%; border-collapse: collapse; margin: 16px 0 24px; font-size: 0.92em; } thead th { background: var(--primary); color: #fff; padding: 11px 14px; text-align: left; font-weight: 600; font-size: 0.9em; text-transform: uppercase; letter-spacing: 0.3px; } thead th:first-child { border-radius: 6px 0 0 0; } thead th:last-child { border-radius: 0 6px 0 0; } tbody td { padding: 10px 14px; border-bottom: 1px solid var(--border); } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } /\* ── Badges ── \*/ .badge { display: inline-block; padding: 2px 12px; border-radius: 12px; font-size: 0.82em; font-weight: 600; color: #fff; } .badge-auto { background: var(--success); } .badge-manual { background: var(--warning); } .badge-critical { background: var(--danger); } .badge-info { background: var(--info); } .badge-gxp { background: #8e44ad; } .badge-phase { background: var(--primary); } .badge-low { background: var(--success); } .badge-medium { background: var(--warning); } .badge-high { background: var(--danger); } .badge-none { background: #95a5a6; } .badge-keep { background: #8e44ad; } .badge-merge { background: var(--info); } .badge-absorb { background: var(--accent); } .badge-unchanged { background: #7f8c8d; } /\* ── Callout Boxes ── \*/ .callout { border-radius: 8px; padding: 18px 22px; margin: 18px 0; border-left: 5px solid; font-size: 0.95em; } .callout-success { background: #f0fdf4; border-color: var(--success); } .callout-warning { background: #fffbeb; border-color: var(--warning); } .callout-danger { background: #fef2f2; border-color: var(--danger); } .callout-info { background: #eff6ff; border-color: var(--info); } .callout-purple { background: #f8f6fc; border-color: var(--primary); } .callout h4 { margin-top: 0; margin-bottom: 8px; } /\* ── Process Flow Cards ── \*/ .flow-grid { display: grid; grid-template-columns: 1fr; gap: 14px; margin: 18px 0; } .flow-card { border: 1px solid var(--border); border-radius: 8px; padding: 18px 22px; background: #fff; position: relative; padding-left: 28px; border-left: 5px solid var(--primary); } .flow-card.auto { border-left-color: var(--success); } .flow-card.manual { border-left-color: var(--warning); } .flow-card.parallel { border-left-color: var(--info); } .flow-card.gxp { border-left-color: #8e44ad; } .flow-card .fc-header { font-weight: 700; font-size: 1.02em; color: var(--text); margin-bottom: 6px; display: flex; align-items: center; gap: 10px; } .flow-card .fc-desc { font-size: 0.92em; color: var(--text-light); } .flow-card .fc-meta { font-size: 0.82em; color: var(--text-light); margin-top: 8px; } /\* ── Comparison ── \*/ .comparison { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin: 20px 0; } .compare-box { border: 2px solid var(--border); border-radius: 10px; padding: 22px; position: relative; } .compare-box.current { border-color: var(--danger); background: #fef8f8; } .compare-box.target { border-color: var(--success); background: #f6fef9; } .compare-box .cb-label { position: absolute; top: -12px; left: 16px; background: #fff; padding: 0 10px; font-size: 0.82em; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; } .compare-box.current .cb-label { color: var(--danger); } .compare-box.target .cb-label { color: var(--success); } /\* ── Phase Banner ── \*/ .phase-banner { background: linear-gradient(135deg, #6B3FA0, #8B5CF6); color: #fff; padding: 18px 24px; border-radius: 10px; margin: 32px 0 20px; display: flex; align-items: center; gap: 16px; } .phase-banner .phase-num { width: 48px; height: 48px; background: rgba(255,255,255,0.2); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.4em; font-weight: 700; flex-shrink: 0; } .phase-banner .phase-info h3 { color: #fff; border: none; padding: 0; margin: 0 0 2px; font-size: 1.15em; } .phase-banner .phase-info p { opacity: 0.88; font-size: 0.9em; margin: 0; } /\* ── Mermaid ── \*/ .mermaid { margin: 20px 0; text-align: center; } .mermaid svg { max-width: 100%; } /\* ── Details/Summary ── \*/ details { margin: 12px 0; } summary { background: #f0f4f8; padding: 10px 16px; border-radius: 6px; cursor: pointer; font-weight: 600; font-size: 0.92em; color: var(--primary); transition: background 0.2s; } summary:hover { background: #e5e9f0; } details pre { background: #f5f5f5; border-left: 4px solid var(--accent); padding: 16px; margin: 8px 0; border-radius: 0 6px 6px 0; overflow-x: auto; font-size: 0.88em; line-height: 1.6; } /\* ── Exec Summary ── \*/ .exec-summary { background: #f6fef9; border-left: 5px solid var(--success); border-radius: 0 8px 8px 0; padding: 22px 26px; margin: 18px 0; } /\* ── Risk Matrix ── \*/ .risk-cell-critical { background: #fecdd3 !important; font-weight: 600; color: #9b1c1c; } .risk-cell-high { background: #fed7aa !important; font-weight: 600; color: #9a3412; } .risk-cell-medium { background: #fef3c7 !important; color: #92400e; } .risk-cell-low { background: #d1fae5 !important; color: #065f46; } /\* ── Timeline ── \*/ .timeline { position: relative; padding-left: 30px; margin: 20px 0; } .timeline::before { content: ''; position: absolute; left: 12px; top: 0; bottom: 0; width: 3px; background: linear-gradient(180deg, var(--primary), var(--accent)); border-radius: 3px; } .timeline-item { position: relative; padding: 12px 0 24px 20px; } .timeline-item::before { content: ''; position: absolute; left: -24px; top: 16px; width: 13px; height: 13px; background: var(--primary); border: 3px solid #fff; border-radius: 50%; box-shadow: 0 0 0 2px var(--primary); } .timeline-item .tl-header { font-weight: 700; color: var(--primary); font-size: 0.95em; } .timeline-item .tl-date { font-size: 0.82em; color: var(--text-light); } .timeline-item .tl-body { font-size: 0.92em; color: var(--text); margin-top: 4px; } /\* ── Footer ── \*/ .doc-footer { background: #f8fafc; border-top: 1px solid var(--border); padding: 28px 56px; text-align: center; font-size: 0.82em; color: var(--text-light); } .doc-footer .footer-brand { font-weight: 700; color: var(--primary); } /\* ── Print ── \*/ @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; margin: 0; max-width: 100%; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .phase-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } thead th { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .kpi-card { break-inside: avoid; } .mermaid { break-inside: avoid; } details { open; } } /\* ── Responsive ── \*/ @media (max-width: 768px) { .content { padding: 24px 20px; } .header-banner { padding: 28px 24px; } .kpi-row { grid-template-columns: 1fr 1fr; } .comparison { grid-template-columns: 1fr; } .toc-grid { grid-template-columns: 1fr; } .doc-footer { padding: 20px; } } :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,)

# Material Creation Process Simplification

Polaris MDG — Enterprise Transformation Plan

📋 Strategic Plan v1.0 🏭 Polaris MDG Platform 🎯 Target: < 2 Weeks Cycle Time 📅 06 May 2026 🔒 GxP Compliant

📑 Table of Contents

[1\. Executive Summary](#executive-summary) [2\. System Landscape & Integration Map](#system-landscape) [3\. Current State Analysis](#current-state) [4\. Process Bottleneck Analysis](#bottleneck-analysis) [5\. Target State Design](#target-state) [6\. CR Consolidation Strategy](#cr-consolidation) [7\. GxP Compliance Preservation](#gxp-compliance) [8\. Intelligent Automation Strategy](#automation-strategy) [9\. Technical Feasibility Assessment](#technical-feasibility) [10\. Implementation Roadmap](#implementation-roadmap) [11\. Risk Management](#risk-management) [12\. Change Management Plan](#change-management) [13\. Success Metrics & KPIs](#success-metrics) [14\. Governance & Decision Framework](#governance) [15\. Recommended Next Steps](#next-steps)

## 1Executive Summary

**The material creation process in Polaris MDG currently requires up to 3 months** from initial request to full material availability across all downstream systems. This extended lead time is driven by a sequential chain of 5–6 distinct Change Request (CR) types, each requiring separate user actions, approvals, and inter-system handoffs across FlowG, GMID, and Polaris MDG. The process was architecturally designed to make materials available with partial data at intermediate stages — a valid business requirement — but the sequential CR dependency chain has created compounding delays that far outweigh the partial-availability benefit.

This document presents a **phased simplification strategy** targeting a reduction to **less than 2 weeks** end-to-end cycle time — a minimum **60% improvement** — while maintaining 100% GxP compliance, preserving the partial-data availability requirement, and minimizing disruption to the current user community.

Current Cycle Time

~90 days

End-to-end material creation

Target Cycle Time

< 10 days

Post Phase 3 implementation

CR Types per Material

4–6

Sequential + parallel CRs

Target CR Types

2–3

Consolidated model

The strategy is organized into **five phases** executed over 22 weeks:

Phase

Focus

Timeline

Expected Impact

Phase 0

Discovery & Baselining

Weeks 1–3

Quantified baseline metrics

Phase 1

Quick Wins — Process Optimization

Weeks 4–8

~40–50% cycle time reduction

Phase 2

CR Consolidation — Architecture Change

Weeks 6–14

Additional 30–40% reduction

Phase 3

Intelligent Automation

Weeks 10–18

Auto-completion, templates

Phase 4

Monitoring & Continuous Improvement

Weeks 16–20

Sustained KPI tracking

#### 🎯 Core Design Principles

*   **GxP compliance is non-negotiable** — digital signature and audit trail integrity are preserved at every step
*   **No IDOC structure changes** — downstream systems (S/4HANA, Athena) receive identical data payloads
*   **Partial-data availability is retained** — materials remain usable at intermediate completion stages
*   **Phased rollout** — each phase delivers measurable value before the next begins

## 2System Landscape & Integration Map

The material creation process spans four distinct systems connected via IDOC-based interfaces. Understanding these integration boundaries is critical for identifying where process simplification is achievable without architectural disruption to downstream consumers.

graph LR subgraph "Initiation Layer" FG\["🔄 FlowG  
_Workflow Initiation_"\] end subgraph "Master Data Layer" GMID\["📦 GMID  
_Basic Data Creation_"\] MDG\["⭐ Polaris MDG  
_Data Enrichment & Governance_"\] end subgraph "Downstream Systems" S4\["🏭 SAP S/4HANA  
_Production Operations_"\] ATH\["🔗 Athena  
_Industrial Integration_"\] OTH\["📊 Other Systems  
_Analytics, Reporting_"\] end FG -->|"Material Request"| GMID GMID -->|"IDOC: Basic Data"| MDG MDG -->|"IDOC: Enriched Material"| S4 MDG -->|"IDOC: Plant Data"| ATH MDG -->|"IDOC: Distribution"| OTH style FG fill:#e8f5e9,stroke:#27ae60,stroke-width:2px style GMID fill:#e3f2fd,stroke:#3498db,stroke-width:2px style MDG fill:#f3e5f5,stroke:#8e44ad,stroke-width:2px style S4 fill:#fff3e0,stroke:#e67e22,stroke-width:2px style ATH fill:#fff3e0,stroke:#e67e22,stroke-width:2px style OTH fill:#fff3e0,stroke:#e67e22,stroke-width:2px

### System Roles & Responsibilities

System

Role

Key Functions

Integration Method

**FlowG**

Workflow Orchestration

Initiates material creation requests, routes to GMID based on material type, manages user task assignments

Direct API / Workflow Engine

**GMID**

Master Data Origination

Creates basic material data (description, material type, UoM, classification), assigns Global Material ID

IDOC outbound to Polaris MDG

**Polaris MDG**

Data Governance & Enrichment

Sequential Change Requests for plant/sales/production data enrichment, GxP approval, quality management, data validation & distribution

IDOC inbound from GMID, IDOC outbound to all downstream

**SAP S/4HANA**

Production Operations

Material master consumption for procurement, production planning, inventory management, quality management

IDOC inbound from Polaris MDG

**Athena**

Industrial Integration

Plant-level data for industrial sites, production scheduling interface

IDOC inbound from Polaris MDG

### Key Integration Table

#### 📋 ZMD\_T\_GMID\_CR — CR Mapping Configuration

This custom configuration table governs which Change Request types are automatically created for each plant/sales organization combination upon material creation. It is the **single most important configuration point** for process automation — mapping gaps in this table are a primary cause of manual intervention and delays.

**Current role:** Determines which of ZMAT02GP / ZMAT02GPS / ZMAT02GE CRs are auto-triggered based on site type and organizational assignment.

## 3Current State Analysis

### 3.1 Sequential CR Chain — Process Flow

The current material creation process follows a strictly sequential chain of Change Requests, where each CR type must be activated before the next can begin. While some CRs at Stage 3 can execute in parallel, the overall dependency chain creates a waterfall effect where any delay at one stage cascades to all subsequent stages.

graph TD A\["🟢 ZMAT01G  
Create Material Basic Data  
**AUTOMATED**  
IDOC from GMID"\] --> B\["🟡 ZMAT02GB  
Change Material Basic Data  
**MANUAL**  
Data Enrichment"\] B --> C1\["🔵 ZMAT02GP  
Global Plant Data  
**MANUAL**  
Athena Sites"\] B --> C2\["🔵 ZMAT02GPS  
Plant & Sales Data  
**MANUAL**  
Market/Pivot Sites"\] B --> C3\["🔵 ZMAT02GE  
Industrial View Data  
**MANUAL**  
S/4HANA Sites"\] C3 --> D\["🟣 ZMAT02GX  
GxP Approval  
**COMPLIANCE**  
Digital Signature"\] C1 --> E\["📤 IDOC Distribution  
All Downstream Systems"\] C2 --> E D --> E style A fill:#d4edda,stroke:#27ae60,stroke-width:2px style B fill:#fff3cd,stroke:#ffc107,stroke-width:2px style C1 fill:#cce5ff,stroke:#3498db,stroke-width:2px style C2 fill:#cce5ff,stroke:#3498db,stroke-width:2px style C3 fill:#cce5ff,stroke:#3498db,stroke-width:2px style D fill:#f3e5f5,stroke:#8e44ad,stroke-width:2px style E fill:#e8eaf6,stroke:#5c6bc0,stroke-width:2px

### 3.2 CR Type Detail Breakdown

AUTO Stage 1 — ZMAT01G: Create Material Basic Data GMID

Fully automated creation triggered by inbound IDOC from the GMID system. Creates the material shell in Polaris MDG with basic classification data (material type, description, unit of measure, material group). Manual intervention required only when activation errors occur (e.g., missing mandatory fields, duplicate material check failures, IDOC mapping errors).

**Trigger:** IDOC from GMID  |  **Typical Duration:** Minutes (auto) / Hours–Days (on error)  |  **User Involvement:** Exception only

MANUAL Stage 2 — ZMAT02GB: Change Material Basic Data GMID

Manual workflow for basic data enrichment — extends the material shell with additional attributes such as cross-plant material status, material descriptions in additional languages, hazardous material indicators, classification characteristics, and regulatory attributes. Auto-created immediately upon ZMAT01G activation.

**Trigger:** ZMAT01G activation  |  **Typical Duration:** 2–4 weeks  |  **User Involvement:** Data Steward (manual entry + approval)

PARALLEL Stage 3 — Plant & Sales Extension CRs

Multiple CRs created in parallel based on ZMD\_T\_GMID\_CR table mapping. Each CR type addresses a different organizational scope:

CR Type

Scope

Target Sites

Key Data Views

**ZMAT02GP**

Global Plant Data

Athena-integrated industrial sites

MRP, Purchasing, Storage, Quality

**ZMAT02GPS**

Plant & Sales Data

Market sites, pivot sites

Sales Org, Distribution Channel, Pricing

**ZMAT02GE**

Industrial View Data

S/4HANA production sites

Production Planning, Work Scheduling, Costing

**Trigger:** ZMAT02GB activation + ZMD\_T\_GMID\_CR mapping  |  **Typical Duration:** 2–6 weeks  |  **User Involvement:** Plant/Sales data stewards per site

GxP Stage 4 — ZMAT02GX: Material GxP Approval

Digital signature and regulatory compliance approval step. Required **only** for industrial sites with S/4HANA production operations (pharmaceutical manufacturing, regulated processes). Implements 21 CFR Part 11 electronic signature requirements. This CR is **architecturally separate** from data entry to ensure audit trail integrity and non-repudiation.

**Trigger:** ZMAT02GE activation  |  **Typical Duration:** 1–2 weeks  |  **User Involvement:** Quality Assurance Manager (digital signature)

### 3.3 Estimated Cycle Time Distribution (Hypothesis)

Based on the described process architecture, the following represents an estimated breakdown of the 90-day average cycle time. Phase 0 (Discovery) will validate these estimates with actual CR lifecycle data from the USMD\_CREQUEST and workflow tables.

Stage

CR Type

Processing Mode

Estimated Duration

Primary Delay Driver

Stage 1

ZMAT01G

AUTO

< 1 day

IDOC processing (exceptions: error resolution)

Stage 2

ZMAT02GB

MANUAL

14–28 days

User availability, data gathering, approval queue

Stage 3a

ZMAT02GP

MANUAL

14–42 days

Multi-site coordination, plant steward availability

Stage 3b

ZMAT02GPS

MANUAL

14–42 days

Sales org data entry, pricing setup

Stage 3c

ZMAT02GE

MANUAL

14–42 days

Production planning data, BOM readiness

Stage 4

ZMAT02GX

GxP

7–14 days

QA manager calendar, compliance review

_Inter-stage dependency waits_

—

7–14 days

System idle time between CR creation triggers

**Total End-to-End**

**~56–90+ days**

**Compounding sequential dependencies**

### 3.4 Root Causes of Extended Lead Time

#### 🔴 Primary Delay Factors

1.  **Sequential dependency chain** — Each CR must wait for its predecessor to fully activate before creation. A single slow CR blocks the entire downstream chain.
2.  **Multiple user handoffs** — Different stewards (basic data, plant data, sales data, QA) each have separate inbox queues with no SLA enforcement. Materials wait in queues while users attend to other priorities.
3.  **Multi-site CR explosion** — A material requiring 5 plant extensions generates 5 parallel CRs, each needing a different plant steward. The slowest site determines the overall completion time.
4.  **No pre-population** — Users manually enter 40+ fields per CR even when 70–80% of values are predictable from material type and reference materials.
5.  **ZMD\_T\_GMID\_CR mapping gaps** — Missing configuration entries cause CRs to not auto-trigger, requiring manual creation requests that add days of administrative overhead.
6.  **Error resolution bottleneck on ZMAT01G** — When the automated IDOC processing fails, manual intervention requires specialized technical knowledge, and the resolution sits in a shared queue.

## 4Process Bottleneck Analysis

### 4.1 Bottleneck Waterfall Model

The following waterfall diagram illustrates where time is consumed in the material creation lifecycle. The horizontal axis represents elapsed business days, and each bar shows the expected duration range per CR stage. The critical path runs through ZMAT01G → ZMAT02GB → ZMAT02GE → ZMAT02GX for GxP-regulated materials.

gantt title Material Creation Cycle Time — Current State (Business Days) dateFormat YYYY-MM-DD axisFormat %b %d section Stage 1 - Auto ZMAT01G IDOC Processing :a1, 2026-01-05, 1d Error Resolution (if needed) :a2, after a1, 3d section Stage 2 - Enrichment Queue Wait (ZMAT02GB) :b1, after a2, 5d Manual Data Entry :b2, after b1, 7d Approval Cycle :b3, after b2, 5d Inter-stage Gap :b4, after b3, 3d section Stage 3 - Extensions ZMAT02GP Plant Data :c1, after b4, 15d ZMAT02GPS Sales Data :c2, after b4, 15d ZMAT02GE Industrial Data :c3, after b4, 20d Inter-stage Gap :c4, after c3, 3d section Stage 4 - Compliance ZMAT02GX GxP Approval :d1, after c4, 10d section Distribution IDOC Outbound :e1, after d1, 1d

### 4.2 Delay Category Analysis

Delay Category

Estimated % of Total

Primary Stages Affected

Addressable?

**User Queue Wait Time**

**40–50%**

ZMAT02GB, ZMAT02GP/GPS/GE

Yes — SLAs + Escalation

**Manual Data Entry**

**20–25%**

ZMAT02GB, ZMAT02GP/GPS/GE

Yes — Pre-population + Templates

**Sequential Dependency Wait**

**15–20%**

Between all stages

Yes — CR Consolidation

**Approval Processing**

**10–15%**

ZMAT02GB, ZMAT02GX

Partial — Parallel Lanes

**System Processing**

**< 5%**

ZMAT01G, IDOC Distribution

Minimal — Already Fast

#### 💡 Key Insight

**85–90% of the cycle time is human wait time, not system processing time.** The technical infrastructure (IDOC processing, MDG activation, data replication) is fast — typically completing in minutes to hours. The bottleneck is almost entirely in user queue wait time and manual data entry. This means process and organizational changes (SLAs, consolidation, automation) will have far greater impact than technical optimizations.

### 4.3 Discovery Data Requirements

Phase 0 of the implementation will validate the above estimates by extracting actual metrics from the following Polaris MDG system tables:

Data Source

Table / Object

Key Fields

Metric Extracted

CR Lifecycle

USMD\_CREQUEST

CREQUEST\_TYPE, CREQUEST\_STATUS, CREATED\_AT, ACTIVATED\_AT

Cycle time per CR type, completion rate

Workflow Steps

USMD\_CREQ\_STEP / SWWWIHEAD

WI\_STAT, WI\_CD, WI\_AD, WI\_AAGENT

Time-in-inbox per user, queue depth

IDOC Status

EDIDS / EDIDC

STATUS, CREDAT, UPDDAT, MESTYP

ZMAT01G error rate, resolution time

CR Mapping

ZMD\_T\_GMID\_CR

Plant, SalesOrg, CR\_Type

Mapping completeness, gap identification

Material Volume

MARA / USMD\_ENTITY

MATNR, ERSDA, MTART

Monthly creation volume, material type distribution

Multi-site Profile

MARC (via MDG staging)

MATNR, WERKS

Plants per material distribution

## 5Target State Design

### 5.1 Current vs. Target Comparison

❌ Current State

*   **5–6 sequential CR types** per material
*   4–6 separate user handoffs
*   ~40 fields manually entered per CR
*   No SLA enforcement
*   No data pre-population
*   ZMD\_T\_GMID\_CR gaps → manual CR creation
*   ~90 days end-to-end

✅ Target State

*   **2–3 consolidated CR types** per material
*   1–2 user touchpoints with parallel lanes
*   ~10 fields manual (rest template-populated)
*   SLA-based escalation (3–5 day limits)
*   Reference material pre-population
*   ZMD\_T\_GMID\_CR 100% coverage + predictive
*   **< 10 days end-to-end**

### 5.2 Target State Process Flow

graph TD A\["🔄 FlowG  
Material Request"\] --> B\["📦 GMID  
Basic Data Creation"\] B -->|"IDOC"| C\["🟢 ZMAT01G\_UNIFIED  
Auto-Create + Basic Enrichment  
**AUTOMATED**  
Template Pre-populated"\] C --> D{"Multi-site  
Material?"} D -->|"Yes"| E\["🔵 ZMAT02GP\_UNIFIED  
Consolidated Plant + Sales + Industrial  
**PARALLEL APPROVAL LANES**  
Tabbed UI"\] D -->|"Single site"| E E --> F{"GxP  
Required?"} F -->|"Yes"| G\["🟣 ZMAT02GX  
GxP Digital Signature  
**UNCHANGED**"\] F -->|"No"| H\["📤 IDOC Distribution"\] G --> H style A fill:#e8f5e9,stroke:#27ae60 style B fill:#e3f2fd,stroke:#3498db style C fill:#d4edda,stroke:#27ae60,stroke-width:3px style D fill:#fff8e1,stroke:#ffc107 style E fill:#bbdefb,stroke:#1976d2,stroke-width:3px style F fill:#fff8e1,stroke:#ffc107 style G fill:#f3e5f5,stroke:#8e44ad,stroke-width:3px style H fill:#e8eaf6,stroke:#5c6bc0

### 5.3 Target State Cycle Time

gantt title Material Creation Cycle Time — Target State (Business Days) dateFormat YYYY-MM-DD axisFormat %b %d section Stage 1 - Auto ZMAT01G Unified (auto + template) :a1, 2026-01-05, 1d section Stage 2 - Consolidated ZMAT02GP Unified (parallel lanes) :b1, after a1, 5d section Stage 3 - Compliance ZMAT02GX GxP (if required) :c1, after b1, 3d section Distribution IDOC Outbound :d1, after c1, 1d

Target Total Duration

~10 days

Down from ~90 days

Cycle Time Reduction

89%

Exceeds 60% target

User Handoffs

1–2

Down from 4–6

Manual Fields

~10

Down from ~40

## 6CR Consolidation Strategy

The CR consolidation is the single highest-impact architectural change in this plan. By merging compatible CR types into a unified structure, we eliminate inter-stage dependency waits and reduce the number of user handoffs from 4–6 to 1–2.

### 6.1 Consolidation Mapping

Current CR Type

Current Purpose

Action

Target CR

Rationale

**ZMAT01G**

Auto-create basic data

KEEP

ZMAT01G (unchanged)

Already automated and fast. No benefit from merging.

**ZMAT02GB**

Basic data enrichment

MERGE

**ZMAT02GP\_UNIFIED**

Basic enrichment can happen simultaneously with plant/sales data in a tabbed UI

**ZMAT02GP**

Global plant data

MERGE

Same material entity — artificial separation creates dependency wait

**ZMAT02GPS**

Plant & sales data

MERGE

Sales views can be entered in parallel with plant views in same CR

**ZMAT02GE**

Industrial view data

ABSORB

Conditional tab in unified CR — only visible when S/4HANA site is in scope

**ZMAT02GX**

GxP digital signature

KEEP SEPARATE

ZMAT02GX (unchanged)

**Regulatory requirement:** digital signature must be a discrete auditable step. Cannot merge with data entry CR.

### 6.2 Unified CR — Tabbed Architecture

The consolidated ZMAT02GP\_UNIFIED CR will present a single workflow item to users with a tabbed data entry interface. Each tab addresses a different data domain, and tabs are dynamically shown/hidden based on the material's organizational scope. Different stewards can work on different tabs simultaneously via parallel approval lanes.

Tab

Data Domain

Visibility Condition

Responsible Role

Former CR

**Tab 1: Basic Enrichment**

Material descriptions, status, classification, hazmat indicators

Always visible

Basic Data Steward

ZMAT02GB

**Tab 2: Plant Data**

MRP views, purchasing, storage, quality management

When plant extension required

Plant Data Steward

ZMAT02GP

**Tab 3: Sales Data**

Sales org, distribution channel, pricing, tax classification

When sales org extension required

Sales Data Steward

ZMAT02GPS

**Tab 4: Industrial / Production**

Production planning, work scheduling, costing, MRP4

When S/4HANA production site in scope

Industrial Data Steward

ZMAT02GE

### 6.3 Parallel Approval Lanes

Within the unified CR, different data domain stewards can approve their respective tabs independently. The CR activates only when **all required lanes are complete** (AND logic), but lanes execute concurrently rather than sequentially.

graph LR subgraph "ZMAT02GP\_UNIFIED — Parallel Approval Lanes" direction TB T1\["Tab 1: Basic Enrichment  
Basic Data Steward"\] --> A1\["✅ Lane 1 Approved"\] T2\["Tab 2: Plant Data  
Plant Data Steward"\] --> A2\["✅ Lane 2 Approved"\] T3\["Tab 3: Sales Data  
Sales Data Steward"\] --> A3\["✅ Lane 3 Approved"\] T4\["Tab 4: Industrial  
Industrial Steward"\] --> A4\["✅ Lane 4 Approved"\] end A1 --> G\["AND Gate"\] A2 --> G A3 --> G A4 --> G G --> ACT\["🟢 CR Activated  
→ IDOC Distribution  
→ ZMAT02GX (if GxP)"\] style T1 fill:#e8f5e9,stroke:#27ae60 style T2 fill:#e3f2fd,stroke:#3498db style T3 fill:#fff3e0,stroke:#e67e22 style T4 fill:#f3e5f5,stroke:#8e44ad style G fill:#fff8e1,stroke:#ffc107,stroke-width:2px style ACT fill:#d4edda,stroke:#27ae60,stroke-width:2px

## 7GxP Compliance Preservation

#### ⚠️ Non-Negotiable Compliance Boundary

GxP compliance requirements — including 21 CFR Part 11 electronic signatures, complete audit trail, and regulatory traceability — are treated as **immutable constraints** throughout this simplification. No process change may weaken, bypass, or defer compliance controls.

### 7.1 Compliance Preservation Matrix

GxP Requirement

Current Implementation

Impact of Simplification

Preservation Strategy

**Digital Signature**  
(21 CFR Part 11)

ZMAT02GX — separate CR with electronic signature workflow

No change

ZMAT02GX remains a **completely separate CR**. It is not consolidated, merged, or modified. The digital signature step executes exactly as it does today.

**Audit Trail**  
(Data Change Logging)

MDG Change Request log (USMD\_CREQUEST) records all field changes with timestamp, user ID, before/after values

Enhanced

Consolidated CR _improves_ audit trail by recording all data changes in a single CR context. Currently, tracing a material's full change history requires joining 4–5 CR logs. Post-consolidation, all enrichment changes are in one CR log.

**Segregation of Duties**  
(Data Entry ≠ Approval)

Different users for data entry and approval within each CR

No change

Parallel approval lanes maintain the same entry/approval segregation. Each lane has its own entry and approval steps with different assigned users.

**Regulatory Traceability**  
(Material → CR → Approval → Signature)

CR chain provides traceability from material creation through GxP sign-off

Simplified

Shorter CR chain (3 steps vs. 6) makes traceability _easier_ — fewer intermediate CRs to trace through. ZMAT01G → ZMAT02GP\_UNIFIED → ZMAT02GX is a cleaner audit path.

**GxP Gating**  
(Approval before distribution)

ZMAT02GX must activate before IDOCs distribute to S/4HANA production sites

No change

Same gating logic: ZMAT02GX is triggered only after ZMAT02GP\_UNIFIED activates (specifically when industrial tab data is complete). IDOC distribution to GxP-regulated sites waits for GxP approval.

### 7.2 Compliance Validation Approach

Before any phase involving GxP-adjacent changes goes live, the following validation steps are mandatory:

#

Validation Activity

Responsible

Exit Criteria

1

QA review of consolidated CR design document

Quality Assurance Manager

Written approval that GxP boundaries are maintained

2

IQ/OQ/PQ testing of ZMAT02GP\_UNIFIED → ZMAT02GX handoff

Validation Team

All test cases pass: correct triggering, correct data inheritance, signature enforced

3

Audit trail verification — compare old and new CR logs

IT + QA

All field changes are logged with identical granularity in new structure

4

Regression test: IDOC distribution to S/4HANA production sites

Integration Team

Same IDOC structure and content pre/post consolidation

5

Mock regulatory audit walk-through

Regulatory Affairs

Auditor can trace material from request → creation → enrichment → GxP sign-off → distribution

## 8Intelligent Automation Strategy

Beyond CR consolidation, intelligent automation addresses the second largest delay driver: manual data entry. By pre-populating predictable field values and enabling template-based creation, we reduce the manual data entry burden from ~40 fields to ~10 exception fields.

### 8.1 Material Template Engine

Template Dimension

Key Fields Pre-Populated

Source Logic

Override Allowed?

**MRP Views**

MRP Type, Lot Size, Reorder Point, Safety Stock, Planning Calendar

Material Type + Material Group → Template Table

Yes — user modifies exceptions

**Purchasing Views**

Purchasing Group, GR Processing Time, Planned Delivery Time, Source List Indicator

Plant + Material Group → Template Table

Yes

**Quality Views**

QM Control Key, Inspection Type, Certificate Type, Quality Info Record

Material Type + GxP Flag → Template Table

Yes — QA approval required for changes

**Sales Views**

Tax Classification, Availability Check Group, Loading Group, Item Category Group

Sales Org + Material Type → Template Table

Yes

**Production Views**

Production Scheduling Profile, In-house Production Time, Setup Time, Costing Lot Size

Plant + Material Type + Production Line → Template Table

Yes — Production Planner approval

### 8.2 Reference Material Copy

When a new material is similar to an existing one (same material group, same production profile), the system can propose copying data from a reference material. The user selects the reference, reviews pre-populated fields, and modifies only the deltas.

#### 📊 Expected Impact

**Before:** User manually enters ~40 fields per CR across 4 data domains.  
**After:** Template pre-populates ~30 fields. User verifies and adjusts ~10 exception fields.  
**Time saving per CR:** ~60–70% reduction in data entry time (from ~2 hours to ~30 minutes).

### 8.3 Predictive Site Assignment

Based on historical patterns (material group × business unit × product category), the system predicts which plants and sales organizations need material extensions and pre-configures the ZMD\_T\_GMID\_CR mapping accordingly.

Feature

Implementation

Benefit

Historical pattern analysis

BRF+ decision table analyzing last 12 months of material-to-site assignments

95%+ prediction accuracy for standard material types

Auto-suggest site list

When ZMAT01G activates, system proposes site list based on material group

Eliminates "forgot to extend to site X" rework

Gap detection

Automated check: if material is in Plant A but BOM exists in Plant B, flag missing extension

Proactive completeness instead of reactive error fixing

### 8.4 SLA-Based Workflow Escalation

CR Type / Lane

Current SLA

Target SLA

Escalation Path

ZMAT02GP\_UNIFIED — Basic Lane

None (unbounded)

3 business days

Day 3: Escalate to team lead → Day 5: Escalate to process owner

ZMAT02GP\_UNIFIED — Plant Lane

None

5 business days

Day 5: Escalate to plant manager → Day 7: Auto-reassign to backup steward

ZMAT02GP\_UNIFIED — Sales Lane

None

5 business days

Day 5: Escalate to sales admin lead → Day 7: Auto-reassign

ZMAT02GP\_UNIFIED — Industrial Lane

None

5 business days

Day 5: Escalate to production planning lead

ZMAT02GX — GxP Approval

None

2 business days

Day 2: Escalate to backup QA manager → Day 3: Escalate to QA Director

### 8.5 ZMAT01G Error Resolution Cockpit

A dedicated monitoring interface for the most common ZMAT01G IDOC processing errors, enabling rapid resolution without requiring deep technical knowledge from the support team.

Error Category

Typical Root Cause

Auto-Resolution?

Cockpit Feature

Missing mandatory field

GMID did not populate a required MDG field

No — needs data correction

Highlight missing fields, provide lookup to GMID source data, one-click reprocess

Duplicate material check

Material already exists with different ID

No — needs manual decision

Show existing material details, allow link/merge/override decision

Connection timeout

Transient network issue

**Yes — auto-retry**

Auto-retry up to 3 times with exponential backoff. Only queue for manual if all retries fail.

Mapping error

IDOC field mapping mismatch (segment/field not recognized)

No — needs config fix

Show IDOC segment with error position, link to mapping configuration

## 9Technical Feasibility Assessment

### 9.1 Component-Level Impact Analysis

Component

Change Required

Complexity

Risk

Notes

**MDG Data Model (USMD)**

New CR type ZMAT02GP\_UNIFIED with combined entity types (MARA + MARC + MVKE + production views)

Medium

Medium

MDG supports multi-entity CR types natively. Requires entity type group configuration.

**ZMD\_T\_GMID\_CR Config Table**

Update mapping: single entry per site combination → ZMAT02GP\_UNIFIED instead of 3-4 separate CR triggers

Low

Low

Configuration change only. Existing auto-trigger logic reused.

**Workflow Template**

New parallel approval workflow with lane-based routing and SLA monitoring

Medium

Medium

SAP Business Workflow supports parallel branches. BRF+ for lane determination. SWETYPV for deadlines.

**UI (Fiori / WebDynpro)**

Unified CR screen with tabbed layout: 4 tabs for different data domains, conditional visibility

High

Medium

Largest development effort. Consider Fiori Elements for new UI if S/4HANA-based, or extend existing WDA component if legacy.

**BRF+ Rules Engine**

Template derivation rules, predictive site assignment, field validation for consolidated entity scope

Medium

Low

BRF+ is MDG's native rule engine. Well-tested extensibility point.

**IDOC Outbound**

**No change**

None

None

IDOC distribution triggers on CR activation regardless of CR type. Same segments, same payload.

**Downstream Systems (S/4HANA, Athena)**

**No change**

None

None

Downstream systems consume IDOCs. Internal MDG CR restructuring is invisible to them.

**FlowG Integration**

Minor — update material request routing to trigger ZMAT01G → ZMAT02GP\_UNIFIED chain

Low

Low

FlowG triggers the initial IDOC. Subsequent CR chain is MDG-internal.

### 9.2 IDOC Architecture Constraint Analysis

#### ✅ No IDOC Changes Required

The CR consolidation is **entirely internal to Polaris MDG**. IDOC message types, segments, and partner profiles remain unchanged because:

*   Inbound IDOC from GMID still triggers ZMAT01G — no change to the trigger mechanism
*   Outbound IDOCs are generated from the activated material master data, not from the CR structure
*   Whether the data was entered via 5 separate CRs or 1 consolidated CR, the resulting material master record is identical
*   IDOC distribution logic is tied to CR _activation events_, not CR _types_

### 9.3 Effort Estimation Summary

Work Package

Estimated Effort

Team

Dependencies

Phase 0: Discovery & Data Extraction

3 weeks / 2 consultants

Process Analyst + MDG Technical

Access to Polaris MDG system & workflow logs

Phase 1: SLA Configuration

2 weeks / 1 developer

MDG Workflow Developer

Phase 0 completion (baseline metrics)

Phase 1: Pre-population BRF+ Rules

3 weeks / 1 developer

MDG BRF+ Developer

Template data collection from business

Phase 1: Error Cockpit Development

3 weeks / 1 developer

ABAP/Fiori Developer

ZMAT01G error pattern analysis

Phase 2: MDG Data Model Extension

4 weeks / 1 MDG architect

MDG Data Model Team

Business sign-off on consolidated CR design

Phase 2: Unified CR UI Development

6 weeks / 2 developers

UI/UX + MDG UI Developer

Data model extension complete

Phase 2: Parallel Workflow Design

3 weeks / 1 developer

MDG Workflow Developer

Data model + UI complete

Phase 3: Template Engine

4 weeks / 1 developer

BRF+ / MDG Developer

Phase 2 complete

Phase 3: Predictive Site Assignment

3 weeks / 1 developer

BRF+ / Analytics Developer

Historical data analysis

Testing (IQ/OQ/PQ for GxP)

4 weeks

Validation Team + QA

All development complete

Change Management & Training

4 weeks

Change Management Lead

Parallel to Phases 1–3

**Total Estimated Effort: ~22 weeks / 4–5 FTE average**

**Critical path: UI Development (Phase 2)**

## 10Implementation Roadmap

### 10.1 Phased Delivery Timeline

gantt title Polaris MDG Material Simplification — Implementation Roadmap dateFormat YYYY-MM-DD axisFormat %b %d section Phase 0: Discovery CR Volume Data Extraction :p0a, 2026-05-12, 5d Cycle Time Analysis :p0b, after p0a, 5d ZMD\_T\_GMID\_CR Audit :p0c, 2026-05-12, 5d User Interview & Survey :p0d, 2026-05-19, 5d Baseline Report :p0e, after p0d, 3d section Phase 1: Quick Wins SLA Deadline Config (Workflow) :p1a, 2026-06-02, 10d BRF+ Pre-population Rules :p1b, 2026-06-02, 15d ZMD\_T\_GMID\_CR Gap Fix :p1c, 2026-06-02, 5d ZMAT01G Error Cockpit Dev :p1d, 2026-06-09, 15d Phase 1 Go-Live :milestone, p1m, 2026-06-30, 0d section Phase 2: CR Consolidation MDG Data Model Extension :p2a, 2026-06-16, 20d Unified CR UI Development :p2b, after p2a, 30d Parallel Workflow Design :p2c, after p2a, 15d Integration Testing :p2d, after p2b, 10d GxP Validation (IQ/OQ/PQ) :p2e, after p2d, 15d Phase 2 Go-Live :milestone, p2m, 2026-09-14, 0d section Phase 3: Automation Material Template Engine :p3a, 2026-08-10, 20d Predictive Site Assignment :p3b, 2026-08-17, 15d Auto-Activation Logic :p3c, after p3a, 10d Phase 3 Go-Live :milestone, p3m, 2026-09-28, 0d section Phase 4: Monitoring Dashboard Development :p4a, 2026-09-14, 10d KPI Tracking Setup :p4b, after p4a, 5d Continuous Improvement Review :p4c, after p4b, 10d section Change Management Pain Point Survey :cma, 2026-05-12, 10d Quick Win Training :cmb, 2026-06-23, 5d Consolidated CR Training :cmc, 2026-09-01, 10d Hypercare Support :cmd, 2026-09-14, 20d

### 10.2 Phase-by-Phase Delivery

0

### Discovery & Baselining

Weeks 1–3  |  Quantify the problem before designing solutions

#

Activity

Method

Deliverable

Owner

0.1

CR Volume & Cycle Time Extraction

Query USMD\_CREQUEST: creation date, activation date, CR type, assigned user

Cycle-time heatmap per CR type

Process Analyst

0.2

Bottleneck Identification

Calculate wait\_time = next\_step\_start − prev\_step\_end for each CR pair

Bottleneck waterfall chart

Process Analyst

0.3

ZMD\_T\_GMID\_CR Mapping Audit

Read table, identify gaps where CRs are created but never completed

CR mapping completeness report

MDG Technical

0.4

User Response Time Analysis

Extract from SWWWIHEAD: time-in-inbox per user role vs. system processing

User vs. system delay split

Workflow Analyst

0.5

ZMAT01G Error Rate

Count IDOC failures (EDIDS status ≠ 53), categorize root causes

Error rate & top failure reasons

Integration Team

0.6

Multi-site Material Profile

Materials × plants distribution analysis

CR explosion factor by material type

Process Analyst

1

### Quick Wins — Process Optimization

Weeks 4–8  |  Reduce cycle time by 40–50% without architecture changes

#

Deliverable

Description

Effort

Expected Impact

1.1

SLA-Based Escalation

Configure deadline monitoring in SAP Business Workflow (SWETYPV / BRF+). 3-day basic, 5-day plant/sales, 2-day GxP.

2 weeks

Eliminates unbounded queue wait — biggest single delay driver

1.2

Reference Material Pre-Population

BRF+ derivation rules to copy data from reference material of same type/group. User confirms and adjusts exceptions.

3 weeks

Reduces manual data entry by 60–70%

1.3

ZMAT01G Error Cockpit

Monitoring interface: IDOC errors listed with root cause, one-click reprocess, auto-retry for transient failures.

3 weeks

Error resolution from hours/days to minutes

1.4

ZMD\_T\_GMID\_CR Gap Closure

Add missing plant/sales org entries to eliminate manual CR creation.

1 week

100% auto-trigger coverage

2

### CR Consolidation — Architecture Change

Weeks 6–14  |  Merge 4 CR types into 1 consolidated type with parallel lanes

#

Deliverable

Description

Effort

Expected Impact

2.1

MDG Data Model Extension

Create ZMAT02GP\_UNIFIED CR type with combined entity types (MARA + MARC + MVKE + production views). Configure entity type group.

4 weeks

Single CR for all data domains

2.2

Unified CR UI

Tabbed interface: 4 tabs (Basic / Plant / Sales / Industrial). Conditional visibility based on site scope.

6 weeks

One screen for all enrichment

2.3

Parallel Approval Workflow

Lane-based workflow: each tab has its own entry/approval pair. CR activates when all required lanes complete.

3 weeks

Concurrent instead of sequential processing

2.4

Integration Testing

End-to-end testing: FlowG → GMID → ZMAT01G → ZMAT02GP\_UNIFIED → ZMAT02GX → IDOC

2 weeks

Confidence in full chain

2.5

GxP Validation (IQ/OQ/PQ)

Formal validation protocol for ZMAT02GP\_UNIFIED → ZMAT02GX handoff. Mock audit walk-through.

3 weeks

Regulatory compliance confirmed

3

### Intelligent Automation

Weeks 10–18  |  Auto-completion, templates, predictive site assignment

#

Deliverable

Description

Effort

Expected Impact

3.1

Material Template Engine

BRF+ template tables per material type × plant × material group. Auto-populate MRP, purchasing, quality, sales, production views.

4 weeks

~30 fields auto-populated

3.2

Predictive Site Assignment

Historical pattern analysis: material group × business unit → predicted plant/sales org list. Auto-suggest during creation.

3 weeks

Eliminates missed extensions

3.3

Auto-Activation with Validation

When all mandatory fields are populated (template + user), auto-submit for approval. BRF+ completeness checking.

2 weeks

Reduces manual submit steps

## 11Risk Management

### 11.1 Risk Register

ID

Risk

Probability

Impact

Severity

Mitigation Strategy

Owner

R1

**GxP compliance concern from auditors** during CR consolidation review

Medium

High

Critical

Keep ZMAT02GX 100% unchanged. Engage QA/Regulatory in Phase 0 design review. Formal IQ/OQ/PQ validation before go-live. Mock audit walk-through.

QA Director

R2

**IDOC payload regression** — downstream systems receive different data after consolidation

Low

High

High

No IDOC structure changes planned. Automated regression test: compare IDOC payloads before/after for 100 test materials. Same material master → same IDOC.

Integration Lead

R3

**User resistance** to consolidated CR — loss of familiar "small task" workflow

High

Medium

High

Early involvement in design (Phase 0 surveys). Side-by-side demos. Phased rollout per site. Hypercare support for 4 weeks post go-live.

Change Mgmt Lead

R4

**ZMD\_T\_GMID\_CR edge cases** — unusual site/org combinations not covered by new mapping

Medium

Medium

Medium

Comprehensive mapping audit in Phase 0. Automated validation rules that flag unmapped combinations. Fallback to manual CR creation for exceptional cases.

MDG Architect

R5

**Template data quality** — wrong default values propagated to materials

Medium

Medium

Medium

Template approval workflow: new templates require key user sign-off. Quarterly template review cycle. User can always override template values.

Master Data Owner

R6

**MDG data model complexity** — combined entity type group causes performance issues

Low

Medium

Low

Performance testing with realistic data volumes during Phase 2 integration testing. MDG natively supports multi-entity CRs — proven pattern.

MDG Architect

R7

**Parallel workflow deadlock** — one approval lane permanently blocked, preventing CR activation

Low

Medium

Low

SLA escalation ensures no lane stays blocked indefinitely. Auto-reassign to backup steward after 7 days. Process owner override capability.

Workflow Admin

### 11.2 Risk Heat Map

quadrantChart title Risk Heat Map — Probability vs Impact x-axis "Low Probability" --> "High Probability" y-axis "Low Impact" --> "High Impact" quadrant-1 "Monitor Closely" quadrant-2 "Critical — Mitigate Immediately" quadrant-3 "Accept / Low Priority" quadrant-4 "Manage Actively" R1-GxP Compliance: \[0.5, 0.85\] R2-IDOC Regression: \[0.2, 0.82\] R3-User Resistance: \[0.75, 0.55\] R4-Mapping Edge Cases: \[0.5, 0.5\] R5-Template Quality: \[0.5, 0.5\] R6-MDG Performance: \[0.25, 0.45\] R7-WF Deadlock: \[0.2, 0.45\]

## 12Change Management Plan

Process simplification is as much an organizational change as a technical one. The change management plan ensures users understand, accept, and adopt the new workflows without productivity disruption.

### 12.1 Stakeholder Impact Map

Stakeholder Group

Current Role

Impact of Change

Key Concern

Engagement Strategy

**Basic Data Stewards**

ZMAT02GB data entry + approval

High — workflow changes to tabbed UI with parallel lanes

Learning new interface, SLA pressure

Early involvement in UI design, pilot testing, training sessions

**Plant Data Stewards**

ZMAT02GP / ZMAT02GE data entry

High — consolidated into ZMAT02GP\_UNIFIED Tab 2/4

Larger scope per CR, template accuracy

Template review sessions, super-user network, hypercare

**Sales Data Stewards**

ZMAT02GPS data entry

Medium — consolidated into Tab 3

Data accuracy, SLA enforcement

Training on tabbed workflow, escalation transparency

**QA / GxP Approvers**

ZMAT02GX digital signature

Low — ZMAT02GX unchanged

Compliance preservation

Formal compliance review in Phase 0, validation sign-off

**MDG Support Team**

ZMAT01G error resolution, manual CR creation

High — new error cockpit, automated CR triggers

Reduced manual workload (positive), new tooling

Cockpit design input, training on new monitoring dashboard

**Process Owners**

Overall governance, escalation point

Medium — new KPI dashboard, SLA oversight

Accurate metrics, decision authority

Dashboard co-design, weekly status during implementation

### 12.2 Communication & Training Plan

Phase 0: Awareness & Input

Weeks 1–3

*   Pain-point survey distributed to all MDG users (online, anonymous)
*   Stakeholder interviews with key stewards and process owners
*   Communication: "We're investigating material creation delays — your input is critical"

Phase 1: Quick Win Rollout

Weeks 5–8

*   Announcement: "SLA-based escalations going live — what to expect"
*   30-minute training session: pre-population features, error cockpit walkthrough
*   Quick reference card distributed to all stewards

Phase 2: Consolidated CR Training

Weeks 12–14

*   1-hour training workshop: new tabbed UI, parallel approval lanes, lane responsibilities
*   Side-by-side comparison: "Old process vs. new process" with live demo
*   Super-user network activated: 1 power user per team as first-line support
*   FAQ document & video recordings published on intranet

Phase 3: Automation Training

Weeks 16–18

*   Template management training for MDG administrators and key users
*   Predictive site assignment: how to review and adjust auto-suggestions
*   Dashboard orientation for process owners

Hypercare Support

Weeks 14–22

*   Dedicated support channel (Teams/Slack) monitored by project team
*   Daily check-in calls during first 2 weeks post go-live
*   Weekly retrospectives for first month
*   Formal transition to BAU support team at Week 22

## 13Success Metrics & KPIs

### 13.1 Progressive Target Matrix

KPI

Current (Baseline)

After Phase 1

After Phase 2

After Phase 3

Measurement Source

**End-to-end cycle time**

~90 days

~50 days

~20 days

< 10 days

USMD\_CREQUEST (created\_at → last activation)

**Number of CR types per material**

4–6

4–6 (unchanged)

2–3

2–3

USMD\_CREQUEST count per material

**Manual data entry fields**

~40

~30 (pre-pop)

~30

~10 (templates)

User survey + field change logs

**User handoffs**

4–6

4–6 (unchanged)

1–2

1–2

Workflow step count per material

**SLA compliance rate**

Not measured

\> 60%

\> 80%

\> 95%

Workflow deadline monitoring

**ZMAT01G error resolution time**

Hours–Days

< 2 hours

< 2 hours

< 30 minutes

Error cockpit resolution log

**ZMD\_T\_GMID\_CR auto-trigger rate**

~80% (est.)

100%

100%

100% + predictive

ZMD\_T\_GMID\_CR coverage audit

**GxP compliance**

100%

100%

100%

100%

QA validation protocol sign-off

### 13.2 Success Criteria Validation

Success Criterion

Validation Method

Target

Measured When

Reduce cycle time by minimum 60%

Compare 3-month rolling average pre/post implementation

90 days → < 36 days (60%) / Target: < 10 days (89%)

4 weeks after Phase 2 go-live

Maintain 100% GxP compliance

Formal IQ/OQ/PQ validation + mock regulatory audit

Zero compliance gaps identified

Before Phase 2 go-live + quarterly audit

Preserve partial-data availability

Materials usable in downstream systems after ZMAT01G (basic) and after ZMAT02GP\_UNIFIED (enriched)

Materials visible in S/4HANA at each CR activation stage

Integration testing (Phase 2)

Minimize user disruption

User satisfaction survey + support ticket volume tracking

Satisfaction ≥ 4/5, support tickets < 2× normal volume during hypercare

4 weeks after each phase go-live

## 14Governance & Decision Framework

### 14.1 Decision Gates

Each phase transition requires formal approval through the following gate reviews. No phase begins without the preceding gate passing.

Gate

Timing

Decision

Approver(s)

Criteria to Pass

**Gate 0**

End of Phase 0

Approve baseline + proceed to Phase 1

Project Sponsor + Process Owner

Baseline report delivered, bottlenecks quantified, budget confirmed

**Gate 1**

End of Phase 1

Approve quick win results + proceed to Phase 2 design

Project Sponsor

Quick wins deployed, measurable improvement visible, no regressions

**Gate 2A**

Phase 2 — Design Complete

Approve consolidated CR design including GxP compliance

QA Director + MDG Architect + Project Sponsor

GxP compliance review passed, UI design approved, data model validated

**Gate 2B**

Phase 2 — Pre Go-Live

Approve Phase 2 go-live

Full Steering Committee

IQ/OQ/PQ validation passed, integration test passed, training completed, hypercare plan ready

**Gate 3**

End of Phase 3

Approve automation go-live + transition to BAU

Project Sponsor + Master Data Owner

Templates validated, predictive accuracy > 90%, user acceptance confirmed

### 14.2 RACI Matrix

Activity

Project Sponsor

MDG Architect

QA Director

Process Owner

Dev Team

Change Mgmt

Phase 0: Discovery

I

R

C

A

R

I

Phase 1: Quick Wins

I

A

I

C

R

C

Phase 2: CR Design

A

R

R

C

R

I

Phase 2: GxP Validation

I

C

A

I

R

I

Phase 2: UI Development

I

A

I

C

R

I

Phase 3: Templates

I

A

C

R

R

I

Training & Rollout

I

C

I

C

C

R/A

Hypercare Support

I

C

I

A

R

R

**Legend:** R = Responsible, A = Accountable, C = Consulted, I = Informed

## 15Recommended Next Steps

To initiate this transformation program, the following immediate actions are recommended:

#

Action

Owner

Timeline

Deliverable

1

**Secure project sponsorship & budget approval**  
Present this plan to leadership. Confirm 4–5 FTE allocation for 22 weeks.

Project Sponsor

Week 0

Signed project charter

2

**Grant system access for Phase 0 discovery**  
Provide read access to Polaris MDG tables (USMD\_CREQUEST, SWWWIHEAD, EDIDS, ZMD\_T\_GMID\_CR) for the analysis team.

MDG System Admin

Week 0

System access confirmed

3

**Engage QA/Regulatory early**  
Brief the QA Director on the planned GxP preservation strategy. Get early alignment before any design begins.

QA Director

Week 1

QA stakeholder alignment memo

4

**Distribute pain-point survey**  
Collect quantitative data on user frustrations and delay perceptions from all MDG stewards.

Change Mgmt Lead

Week 1

Survey results (min. 50% response rate)

5

**Begin Phase 0 data extraction**  
Extract CR lifecycle data, workflow logs, and IDOC error statistics. Build the baseline cycle-time model.

Process Analyst

Week 1–3

Baseline report with quantified bottlenecks

6

**Schedule Gate 0 review**  
Plan the first decision gate for end of Week 3. All Phase 0 deliverables presented to steering committee.

Project Manager

Week 3

Gate 0 agenda & attendee list

#### 🤝 How C16 Can Support the Next Phase

C16 can directly accelerate multiple Phase 0 and Phase 1 activities:

*   **Automated data extraction:** If granted access to the Polaris MDG system, C16 can query USMD\_CREQUEST, SWWWIHEAD, and EDIDS tables to produce the cycle-time analysis and bottleneck report automatically.
*   **ZMAT01G Error Cockpit:** C16 can generate the ABAP/Fiori application for the error resolution cockpit (Phase 1.3) — including IDOC monitoring, one-click reprocess, and auto-retry logic.
*   **ZMD\_T\_GMID\_CR gap analysis:** If the table data is exported, C16 can identify all missing plant/sales org combinations and recommend additions.
*   **BRF+ rule design:** C16 can help architect the derivation rules for pre-population, template selection, and predictive site assignment.
*   **Workflow configuration:** C16 can assist with SLA deadline monitoring setup and parallel approval lane design in SAP Business Workflow.

C16 — Enterprise SAP Intelligence Platform

Material Creation Process Simplification — Polaris MDG  |  Generated 06 May 2026

This document is based on the process architecture described by the stakeholder team. Phase 0 Discovery will validate all estimates with actual system data from USMD\_CREQUEST, SWWWIHEAD, EDIDS, and ZMD\_T\_GMID\_CR tables on the Polaris MDG system.

Document Version: 1.0  |  Classification: Internal — Project Team

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._