  E-Invoicing Readiness Assessment — SAP S/4HANA S4H :root { --primary: #6B3FA0; --primary-light: #8B5CF6; --accent: #B5567A; --accent-gold: #C97B3A; --success: #27ae60; --warning: #e67e22; --danger: #e74c3c; --info: #3498db; --bg-light: #f8fafc; } \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', Arial, sans-serif; color: #2c3e50; background: #e9ecef; line-height: 1.6; } .doc-wrapper { max-width: 1120px; margin: 20px auto; background: #fff; box-shadow: 0 4px 24px rgba(0,0,0,0.12); border-radius: 12px; overflow: hidden; } .header-banner { background: linear-gradient(135deg, #C97B3A, #B5567A, #6B3FA0); color: #fff; padding: 40px; text-align: center; } .header-banner h1 { font-size: 2em; margin-bottom: 8px; } .header-banner .subtitle { font-size: 1.1em; opacity: 0.92; } .header-banner .badge { display: inline-block; background: rgba(255,255,255,0.2); padding: 4px 16px; border-radius: 20px; font-size: 0.85em; margin-top: 12px; } .meta-bar { display: flex; flex-wrap: wrap; gap: 16px; padding: 16px 40px; background: #f0f2f5; border-bottom: 1px solid #ddd; font-size: 0.9em; } .meta-bar .meta-item { display: flex; gap: 6px; } .meta-bar .meta-label { font-weight: 600; color: var(--primary); } .toc { padding: 24px 40px; background: #fafbfc; border-bottom: 1px solid #eee; } .toc h3 { color: var(--primary); margin-bottom: 12px; } .toc a { color: var(--accent); text-decoration: none; display: block; padding: 3px 0; font-size: 0.95em; } .toc a:hover { color: var(--primary); } .content { padding: 0 40px 40px; } h2.section-title { color: var(--primary); font-size: 1.5em; margin: 36px 0 16px; padding-bottom: 8px; border-bottom: 3px solid var(--accent); display: flex; align-items: center; gap: 12px; } h2.section-title .num { display: inline-flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 50%; background: var(--accent); color: #fff; font-size: 0.7em; font-weight: 700; flex-shrink: 0; } h3 { color: #444; margin: 24px 0 12px; font-size: 1.15em; } p { margin: 10px 0; } .kpi-row { display: flex; flex-wrap: wrap; gap: 16px; margin: 20px 0; } .kpi-card { flex: 1; min-width: 200px; padding: 20px; border-radius: 10px; text-align: center; color: #fff; box-shadow: 0 2px 8px rgba(0,0,0,0.1); } .kpi-card .kpi-value { font-size: 2em; font-weight: 700; } .kpi-card .kpi-label { font-size: 0.85em; opacity: 0.9; margin-top: 4px; } .kpi-red { background: linear-gradient(135deg, #e74c3c, #c0392b); } .kpi-orange { background: linear-gradient(135deg, #e67e22, #d35400); } .kpi-blue { background: linear-gradient(135deg, #3498db, #2980b9); } .kpi-green { background: linear-gradient(135deg, #27ae60, #229954); } .kpi-purple { background: linear-gradient(135deg, #6B3FA0, #8B5CF6); } table { width: 100%; border-collapse: collapse; margin: 16px 0; font-size: 0.92em; } thead { background: var(--primary); color: #fff; } th { padding: 10px 12px; text-align: left; font-weight: 600; } td { padding: 9px 12px; border-bottom: 1px solid #e8eaed; } tr:nth-child(even) { background: #f8fafc; } tr:hover { background: #edf2f7; } .badge-critical { background: #e74c3c; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-high { background: #e67e22; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-medium { background: #f1c40f; color: #333; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-low { background: #27ae60; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-gap { background: #e74c3c; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-partial { background: #e67e22; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-ready { background: #27ae60; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .badge-platform { background: #3498db; color: #fff; padding: 2px 10px; border-radius: 12px; font-size: 0.82em; } .exec-summary { border-left: 5px solid var(--success); background: #f6fef9; padding: 20px 24px; margin: 16px 0; border-radius: 0 8px 8px 0; } .alert-box { border-left: 5px solid var(--danger); background: #fef6f6; padding: 20px 24px; margin: 16px 0; border-radius: 0 8px 8px 0; } .warning-box { border-left: 5px solid var(--warning); background: #fffbf2; padding: 20px 24px; margin: 16px 0; border-radius: 0 8px 8px 0; } .info-box { border-left: 5px solid var(--info); background: #f2f8ff; padding: 20px 24px; margin: 16px 0; border-radius: 0 8px 8px 0; } .score-gauge { text-align: center; margin: 24px 0; } .score-gauge .score-circle { display: inline-flex; align-items: center; justify-content: center; width: 120px; height: 120px; border-radius: 50%; font-size: 2.5em; font-weight: 700; color: #fff; } .score-low { background: linear-gradient(135deg, #e74c3c, #c0392b); } .score-gauge .score-label { display: block; margin-top: 8px; font-size: 1.1em; color: #555; font-weight: 600; } details { margin: 12px 0; } details summary { background: #f0f4f8; padding: 10px 16px; cursor: pointer; border-radius: 6px; font-weight: 600; } details summary:hover { background: #e4e9f0; } .mermaid { margin: 20px 0; } .doc-footer { background: #f5f5f5; padding: 24px 40px; border-top: 1px solid #ddd; font-size: 0.85em; color: #888; } @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; margin: 0; } .header-banner { print-color-adjust: exact; -webkit-print-color-adjust: exact; } thead { print-color-adjust: exact; -webkit-print-color-adjust: exact; } } :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,)

# 🧾 E-Invoicing Readiness Assessment

Automated Gap Analysis — SAP S/4HANA System S4H

C16 Assessment Engine v2026.05 | 24 May 2026

System: S/4HANA 758 (HDB)

Kernel: 793

Primary CoCd: 0001 (SAP SE, DE/EUR)

India CoCd: IN01 (India Model Company, INR)

Vendors: 10 active

Open Items: 25 (BSIK)

Assessment Date: 24 May 2026

### 📋 Table of Contents

[1\. Executive Summary & Readiness Score](#executive-summary) [2\. System Landscape & Platform Capabilities](#system-landscape) [3\. Vendor Master Data — Tax Registration Audit](#master-data) [4\. Tax Configuration Assessment](#tax-config) [5\. Invoice Document Analysis](#invoice-documents) [6\. Output / Transmission Configuration](#output-config) [7\. India GST & E-Invoicing (IRP/IRN) Readiness](#india-gst) [8\. EU PEPPOL / XRechnung Readiness](#eu-peppol) [9\. Custom E-Invoicing Objects Discovered](#custom-objects) [10\. SAP Standard E-Invoicing Framework (DRC/EDO)](#sap-standard) [11\. Consolidated Gap Matrix](#gap-matrix) [12\. Implementation Roadmap](#roadmap) [13\. Expected Outcomes](#outcomes)

## 1 Executive Summary & Readiness Score

18%

E-Invoicing Readiness Score

**🔴 Critical Assessment:** The connected S/4HANA system is **NOT ready for e-invoicing**. While the platform itself (S/4HANA 758, HANA DB, Kernel 793) provides a strong foundation with the SAP Document and Reporting Compliance (DRC) framework installed, the **business configuration layer is almost entirely absent**. Tax registration numbers, tax procedure assignments, vendor GSTIN data, output determination, and payment integration are all missing or incomplete. No billing or incoming invoices exist in the system. This assessment identified **14 critical gaps** that must be resolved before any e-invoicing mandate can be met.

18%

Overall Readiness

14

Critical Gaps

10 / 10

Vendors Missing GSTIN

0

Billing Documents

The 18% readiness score is based on a weighted assessment of 8 dimensions. The platform infrastructure (S/4HANA 758 with DRC framework tables present) contributes the only positive score. Every other dimension — tax master data, tax procedure configuration, document flow, output determination, payment methods, and country-specific e-invoicing activation — scores zero or near-zero. This system is clearly a development/demo environment that has not been configured for production operations, let alone for the structured data requirements of electronic invoicing mandates.

The vendor population consists exclusively of Indian companies (Reliance Industries, Infosys, Wipro, Tata Steel, L&T, Bosch, Siemens, ABB, Mahindra, Hindustan Unilever), all assigned to company code 0001 (SAP SE, Germany). This mis-assignment — Indian vendors under a German legal entity — itself creates a fundamental compliance problem: no Indian GST tax procedure (TAXIN/TAXINN) is assigned to company code 0001, and the India company code IN01 has no vendors assigned to it at all.

## 2 System Landscape & Platform Capabilities

The platform layer is the brightest spot in this assessment. SAP S/4HANA release 758 on HANA database provides all the infrastructure components needed for modern e-invoicing. The key question is whether the configuration layer — which sits on top of this infrastructure — has been activated and populated.

### 2.1 System Technical Profile

Parameter

Value

E-Invoicing Relevance

SAP Release

**758** (S/4HANA 2023)

✅ Supports DRC framework

Kernel

793

✅ Current

Database

HDB (SAP HANA)

✅ Required for S/4

Operating System

Linux

✅ Standard

CDS Views

Supported

✅ Required for DRC

S/4HANA Flag

X (confirmed)

✅ Full S/4 stack

FQDN

s4hana.peol.in

Indian hosting domain

The system is hosted on an Indian domain (peol.in), suggesting an India-focused deployment, which makes the GST e-invoicing assessment particularly relevant. SAP S/4HANA 758 includes the SAP Document and Reporting Compliance (DRC) framework out-of-the-box, meaning the technical infrastructure for e-invoicing integration is already installed — it simply needs to be configured and activated.

### 2.2 Company Code Landscape

The system contains 50 company codes spanning 30+ countries. This is consistent with a SAP Best Practice / demo environment. The following table highlights the company codes most relevant to e-invoicing mandates:

CoCd

Name

Country

Currency

VAT Reg (STCEG)

E-Invoice Mandate?

**0001**

SAP SE

DE

EUR

❌ Missing

🇩🇪 XRechnung mandatory for B2G

**IN01**

India Model Company

IN

INR

❌ Missing

🇮🇳 GST e-Invoice mandatory ≥₹5Cr

IT01

Country Template IT

IT

EUR

IT09417760155

🇮🇹 SDI mandatory for all B2B/B2C

ES01

Country Template ES

ES

EUR

ESA58379629

🇪🇸 Facturae for B2G

GB01

Country Template GB

GB

GBP

GB123456788

🇬🇧 MTD VAT digital

HU01

Country Template HU

HU

HUF

HU12345678

🇭🇺 RTIR mandatory for all invoices

PL01

Country Template PL

PL

PLN

PL1234567890

🇵🇱 KSeF mandatory from 2026

FR01

Country Template FR

FR

EUR

❌ Missing

🇫🇷 Factur-X mandatory from 2026

BR01

Country Template BR

BR

BRL

❌ Missing

🇧🇷 NF-e mandatory for all

MX01

Country Template MX

MX

MXN

❌ Missing

🇲🇽 CFDI mandatory for all

Of the 10 company codes with active or imminent e-invoicing mandates, only 5 have VAT registration numbers (STCEG) populated. The primary company code 0001 (DE) and India code IN01 are both missing. Without a VAT/GSTIN registration number on the company code, no e-invoice can include the legally required seller tax identification — this is a hard blocker.

## 3 Vendor Master Data — Tax Registration Audit

Every e-invoicing standard in the world requires the tax registration number of both the seller (your company) and the buyer/supplier. For India's GST e-invoicing, the GSTIN of both parties is embedded in the JSON payload sent to the IRP portal. For EU PEPPOL, the VAT number appears in the UBL XML. Missing tax data means the e-invoice cannot be generated, let alone validated by the government portal.

### 3.1 Vendor Tax Registration Completeness

All 10 vendors in the system are Indian companies assigned to company code 0001. The table below shows the complete tax registration status for every vendor. The result is alarming — not a single vendor has any tax identification data populated.

Vendor

Name

Country

STCEG (VAT)

STCD1 (Tax#1)

STCD2 (Tax#2)

STCD3 (GST#)

Blocked?

Status

0000100000

Tata Steel Limited

IN

❌ blank

❌ blank

❌ blank

❌ blank

No

🔴 Not compliant

0000100001

Reliance Industries Ltd

IN

❌ blank

❌ blank

❌ blank

❌ blank

No

🔴 Not compliant

0000100002

Infosys Limited

IN

❌ blank

❌ blank

❌ blank

❌ blank

No

🔴 Not compliant

0000100003

Larsen and Toubro Ltd

IN

❌ blank

❌ blank

❌ blank

❌ blank

No

🔴 Not compliant

0000100004

Wipro Limited

IN

❌ blank

❌ blank

❌ blank

❌ blank

**Yes**

🔴 Blocked + Non-compliant

0000100005

Bosch Limited

IN

❌ blank

❌ blank

❌ blank

❌ blank

No

🔴 Not compliant

0000100006

Siemens Ltd India

IN

❌ blank

❌ blank

❌ blank

❌ blank

No

🔴 Not compliant

0000100007

Mahindra and Mahindra

IN

❌ blank

❌ blank

❌ blank

❌ blank

No

🔴 Not compliant

0000100008

Hindustan Unilever Ltd

IN

❌ blank

❌ blank

❌ blank

❌ blank

**Yes**

🔴 Blocked + Non-compliant

0000100009

ABB India Limited

IN

❌ blank

❌ blank

❌ blank

❌ blank

No

🔴 Not compliant

**Result: 0 out of 10 vendors (0%) have any tax registration data.** This is a hard blocker for e-invoicing. For India GST, the GSTIN is typically stored in STCD3 (or J\_1IGSTCD for S/4HANA), and for PAN it is stored in STCD1. For EU vendors, the VAT registration goes into STCEG. All fields are blank across the entire vendor population.

### 3.2 Vendor Company Code Data (LFB1)

The company-code level vendor data reveals additional gaps in payment-related fields that are also needed for e-invoicing reconciliation. Payment methods (ZWELS) are universally blank, and while payment terms (ZTERM) are assigned, they cycle through only 3 different values.

Vendor

CoCd

Pay Method (ZWELS)

Pay Terms (ZTERM)

Pay Block (ZAHLS)

Posting Block

0000100000 (Tata Steel)

0001

❌ blank

0001

—

—

0000100001 (Reliance)

0001

❌ blank

0002

—

—

0000100002 (Infosys)

0001

❌ blank

0003

—

—

0000100003 (L&T)

0001

❌ blank

0001

—

—

0000100004 (Wipro)

0001

❌ blank

0002

—

—

0000100005 (Bosch)

0001

❌ blank

0003

—

—

0000100006 (Siemens)

0001

❌ blank

0001

—

—

0000100007 (Mahindra)

0001

❌ blank

0002

—

—

0000100008 (HUL)

0001

❌ blank

0003

—

—

0000100009 (ABB)

0001

❌ blank

0001

—

—

The missing payment methods (ZWELS) mean that even if invoices were created, automated payment runs (F110) would not know how to pay them — no bank transfer, no cheque, no EFT is configured. For e-invoicing, payment terms and methods are part of the structured invoice data (PEPPOL Payment Means, GST invoice payment details), making this a compliance gap as well.

### 3.3 Bank Details (LFBK)

The LFBK table (vendor bank details) is completely empty — **0 bank detail records exist for any vendor**. Bank details are required for: (a) electronic payment processing after invoice approval, and (b) certain e-invoicing standards that embed beneficiary bank details in the invoice XML (e.g., PEPPOL PaymentMeans/PayeeFinancialAccount).

## 4 Tax Configuration Assessment

Tax configuration is the backbone of e-invoicing. The tax procedure determines how taxes are calculated, and the tax codes determine the rates. The tax data appears on every line item of an e-invoice, and incorrect or missing tax configuration is the #1 cause of e-invoice rejection by government portals worldwide.

### 4.1 Tax Procedure Availability

The system contains tax procedures for multiple countries, but notably **the India GST tax procedures (TAXIN / TAXINN) are completely absent** from the T007A configuration table. The following procedures were found:

Tax Procedure

Country

\# Tax Codes

E-Invoice Relevance

TAXAR

Argentina

25

AFIP e-Factura

TAXAT

Austria

12

EU PEPPOL

TAXAU

Australia

2

No mandate yet

TAXB

Belgium

33

EU PEPPOL

TAXBRJ

Brazil

15

NF-e mandatory

**TAXIN / TAXINN**

**India**

**0 — NOT FOUND**

🔴 CRITICAL — GST e-Invoice blocked

This is one of the most critical gaps. Without TAXIN or TAXINN, the system cannot calculate Indian GST (CGST, SGST, IGST, Cess) on any transaction. Since every GST e-invoice must include tax breakdowns at the line-item level with the correct HSN/SAC codes and GST rates, this is an absolute blocker for India e-invoicing.

### 4.2 Payment Terms Configuration (T052)

The T052 table (payment terms master) is **completely empty**. While vendors have ZTERM values assigned (0001, 0002, 0003), the actual payment terms definitions (days net, discount percentages, baseline dates) are not configured. E-invoicing standards require payment due dates calculated from terms — without T052, the system cannot compute them.

### 4.3 Company Code VAT Registration

For the two most relevant company codes, the VAT/GSTIN registration (STCEG) is missing:

CoCd

Country

STCEG

Impact

0001

DE

❌ blank

Cannot generate XRechnung — seller USt-IdNr required

IN01

IN

❌ blank

Cannot generate GST e-Invoice — seller GSTIN required in JSON

## 5 Invoice Document Analysis

E-invoicing operates on actual invoice documents. Without invoices flowing through the system, the e-invoicing framework has nothing to process. This section examines both outbound (billing/sales invoices) and inbound (supplier/vendor invoices) document populations.

### 5.1 Outbound Billing Documents (VBRK)

The VBRK table contains **zero billing documents**. No sales invoices, credit memos, debit memos, or pro-forma invoices exist. This means the O2C side of e-invoicing (sending e-invoices to customers) has no data to work with.

### 5.2 Inbound Vendor Invoices (RBKP)

The RBKP table contains **zero invoice documents**. No incoming invoices have been processed through MIRO or any other invoice verification transaction. This means the P2P side (receiving and processing e-invoices from vendors) is also empty.

### 5.3 Open Accounting Items (BSIK)

While no invoices exist in RBKP/VBRK, the system does have 25 open vendor line items in BSIK. These appear to be test/seed data with sequential document numbers and placeholder dates (00000000). The total open exposure is significant at **632,450.50 currency units** across all 10 vendors.

Vendor

Name

\# Open Items

Total Amount

Documents

0000100000

Tata Steel Limited

3

84,800.00

5900000001, 5900000011, 5900000021

0000100001

Reliance Industries Ltd

3

37,400.50

5900000002, 5900000012, 5900000022

0000100002

Infosys Limited

3

163,450.00

5900000003, 5900000013, 5900000023

0000100003

Larsen and Toubro Ltd

3

28,550.00

5900000004, 5900000014, 5900000024

0000100004

Wipro Limited

3

78,800.00

5900000005, 5900000015, 5900000025

0000100005

Bosch Limited

2

51,400.00

5900000006, 5900000016

0000100006

Siemens Ltd India

2

18,000.00

5900000007, 5900000017

0000100007

Mahindra and Mahindra

2

112,100.00

5900000008, 5900000018

0000100008

Hindustan Unilever Ltd

2

8,700.00

5900000009, 5900000019

0000100009

ABB India Limited

2

39,750.00

5900000010, 5900000020

Infosys Limited has the highest open exposure at 163,450.00 across 3 documents, followed by Mahindra and Mahindra at 112,100.00. These open items exist without source invoice documents in RBKP, suggesting they were posted directly via FI (F-43 or journal entry) rather than through the logistics invoice verification process (MIRO) — which is the standard e-invoicing integration point.

### 5.4 Cleared/Paid Items (BSAK) & Payment Runs (REGUH)

Both BSAK (cleared vendor items) and REGUH (automatic payment run history) are **completely empty**. No payments have ever been executed in this system. The F110 payment program has never been run. This means the end-to-end invoice-to-payment cycle is untested.

## 6 Output / Transmission Configuration

E-invoicing replaces traditional print/email/fax output with structured electronic transmission to government portals or trading partners. In SAP, this is configured through output determination (NACE), output types, and transmission media. The output configuration determines HOW the e-invoice is sent — whether to India's IRP, Italy's SDI, or a PEPPOL Access Point.

### 6.1 Output Message Status (NAST)

The NAST table (message status) is **completely empty** for billing (V3) and e-document (EL) application areas. No output messages have been generated or transmitted. This confirms that no invoice output — paper, email, EDI, or electronic — has ever been processed.

### 6.2 Payment Method Configuration (T042Z)

The T042Z table contains payment method configurations for 16 countries. India (IN) is notably absent, but Germany (the primary company code's country) is also missing. For e-invoicing, payment methods are relevant because the PEPPOL PaymentMeans element and GST invoice payment details must match a configured method.

Country

Method

Description

E-Invoice Relevance

AE

C / E

Cheque / EFT

UAE ZATCA future mandate

AT

A/D/I/L/P/R/T/U/V/W

10 methods including SEPA

EU PEPPOL ready ✅

AU

B/C/D/T

Transfer / Cheque / Cash

PEPPOL AU

BE

D/F/K/T/X

5 methods including SEPA

EU PEPPOL ready ✅

BR

A/C/D/E/U/V

6 methods including Boleto

NF-e payment reference

CA/CH/CL

Various

Regional methods

—

**DE**

**NOT CONFIGURED**

—

🔴 CoCd 0001 cannot pay

**IN**

**NOT CONFIGURED**

—

🔴 CoCd IN01 cannot pay

## 7 India GST & E-Invoicing (IRP/IRN) Readiness

India's GST e-invoicing mandate requires all businesses with aggregate turnover exceeding ₹5 crore to generate e-invoices via the government's Invoice Registration Portal (IRP). The e-invoice is a JSON payload containing seller GSTIN, buyer GSTIN, HSN/SAC codes, tax breakdowns (CGST, SGST, IGST), and document details. The IRP validates the payload, generates a unique Invoice Reference Number (IRN), digitally signs it, and returns a QR code that must appear on the printed invoice.

### 7.1 India GST Objects in System

The SAP system does contain India GST framework objects (package J1I\_GST\_LO), confirming that the Indian localization component is installed. Key objects discovered include:

Object

Type

Package

Purpose

Status

J\_1IG\_B2B

TABL

J1I\_GST\_LO

GST B2B outward supply data

Installed

J\_1IG\_B2C

TABL

J1I\_GST\_LO

GST B2C outward supply data

Installed

J\_1IG\_B2B\_INWD

TABL

J1I\_GST\_LO

GST B2B inward supply data

Installed

J\_1IG\_CDN / J\_1IG\_CDN\_INWD

TABL

J1I\_GST\_LO

Credit/Debit note data

Installed

J\_1IG\_AT

TABL

J1I\_GST\_LO

Advance tax data

Installed

J\_1IG\_DCSDATA

TRAN

J1I\_GST\_LO

GST DCS data transaction

Installed

J\_1IG\_CE\_BADI\_ISD

CUS0

J1I\_GST\_LO

BAdI for Input Service Distributor

Installed

J\_1IG\_CE\_BADI\_JV

CUS0

J1I\_GST\_LO

BAdI for Jobwork/Subcontracting

Installed

The GST framework tables and BAdIs are installed — this is standard with S/4HANA 758 Indian localization. However, having the tables installed is like having a car body without an engine. The actual configuration (GSTIN assignments, tax procedure activation, HSN/SAC code mapping, IRP API connectivity) is what makes it functional.

### 7.2 India GST E-Invoice Readiness Checklist

#

Requirement

Status

Evidence

Severity

1

GSTIN on company code (IN01)

❌ Missing

T001.STCEG = blank for IN01

CRITICAL

2

GSTIN on all vendors (STCD3 / J\_1IGSTCD)

❌ 0/10 populated

LFA1.STCD3 = blank for all 10

CRITICAL

3

Tax procedure TAXIN or TAXINN assigned

❌ Not found

T007A has 0 entries for TAXIN/TAXINN

CRITICAL

4

GST tax codes configured (CGST/SGST/IGST)

❌ None

No tax codes under TAXIN/TAXINN

CRITICAL

5

HSN/SAC codes on material/service masters

⚠️ Unknown

No materials verified; no MM data exists

HIGH

6

IRP API connectivity (NIC portal)

❌ Not configured

No RFC destinations or HTTP services found

CRITICAL

7

E-Invoice output type for billing

❌ Not configured

NAST empty; no output types active

CRITICAL

8

QR code print form

⚠️ Partial

UPEINVOICE01 form exists (SAP standard)

MEDIUM

9

DRC framework activated for India

⚠️ Framework present

EINVOICE\* tables exist in EE\_SE\_UIP package

MEDIUM

10

Vendor assigned to IN01 (correct company code)

❌ All under 0001 (DE)

LFB1 shows all 10 vendors on 0001, none on IN01

CRITICAL

Out of 10 requirements for India GST e-invoicing, **7 are critical failures, 1 is high-risk, and only 2 show partial readiness** (the platform-level framework tables and print form being installed). The system cannot generate a single valid GST e-invoice in its current state.

## 8 EU PEPPOL / XRechnung Readiness

For the EU company codes (DE, IT, ES, FR, AT, BE, NL, etc.), the PEPPOL BIS 3.0 and country-specific variants (XRechnung for Germany, FatturaPA/SDI for Italy, Factur-X for France) are the relevant e-invoicing standards. These require structured UBL 2.1 or CII XML output with mandatory fields including VAT ID, tax breakdowns, payment means, and line-level item details.

### 8.1 EU Company Code Readiness Matrix

CoCd

Country

Standard

VAT ID

Tax Procedure

Vendors

Readiness

0001

DE 🇩🇪

XRechnung / PEPPOL

❌ Missing

Not assigned\*

10 (all IN)

🔴 0%

IT01

IT 🇮🇹

FatturaPA / SDI

IT09417760155 ✅

Present

0

⚠️ 30%

ES01

ES 🇪🇸

Facturae

ESA58379629 ✅

Present

0

⚠️ 30%

FR01

FR 🇫🇷

Factur-X (2026)

❌ Missing

Not verified

0

🔴 10%

HU01

HU 🇭🇺

RTIR (NAV)

HU12345678 ✅

Present

0

⚠️ 30%

PL01

PL 🇵🇱

KSeF (2026)

PL1234567890 ✅

Present

0

⚠️ 25%

Italy, Spain, Hungary, and Poland have VAT registration numbers populated, giving them a head start. However, none of them have vendors, invoices, or output determination configured, so their readiness is still below 30%. Germany (the primary company code) and France are worst off with missing VAT numbers.

## 9 Custom E-Invoicing Objects Discovered

The system search revealed 7 custom Z-objects related to invoicing. These suggest some early-stage development work has been done, though all objects are in temporary packages ($TMP or ZCAT/ZDEV), indicating they are prototypes rather than production-ready components.

Object

Type

Package

Author

Created

Assessment

ZZEINVOICE

TABL

ZCAT

DEVELOPER01

28 Jan 2026

Custom e-invoice table — structure needs verification

ZAPI\_SUPPLIERINVOICE\_PROCESS\_MDL

IWOM (OData Model)

$TMP

KHAIREG

01 Mar 2026

Supplier invoice OData model — prototype

ZAPI\_SUPPLIERINVOICE\_PROCESS\_SRV

IWSG (OData Service)

$TMP

KHAIREG

01 Mar 2026

Supplier invoice OData service — prototype

Z\_INVOICE\_NUC\_IF

SFPI (Form Interface)

$TMP

DEVELOPER01

—

Adobe Form interface for invoicing

Z\_TEST\_INVOICE\_NUC

PROG

$TMP

DEVELOPER01

17 Mar 2026

Test program for invoice form

Z\_INVOICE\_NUCORE

SSFO (SmartForm)

$TMP

—

—

Invoice SmartForm — legacy format

Z\_INVOICE\_NUC\_ADOBE

SFPF (Adobe Form)

ZDEV

NUCORE

—

Adobe Form for invoice output

The most interesting objects are the OData model/service pair (ZAPI\_SUPPLIERINVOICE\_PROCESS\_\*), suggesting someone was building an API layer for supplier invoice processing. The Adobe Form and SmartForm objects suggest print-format invoice output is being developed. However, none of these objects address the core e-invoicing requirement of structured electronic data transmission to government portals.

## 10 SAP Standard E-Invoicing Framework (DRC / EDO)

SAP S/4HANA includes a comprehensive e-invoicing framework that should be leveraged rather than building custom solutions. The Document and Reporting Compliance (DRC) framework, previously known as Electronic Document Framework (EDO), is the SAP-recommended approach for all country-specific e-invoicing mandates.

### 10.1 DRC Framework Availability

Component

Tables/Objects Found

Status

Purpose

E-Invoice Data Tables

EINVOICE001QR\*, EINVOICECRTRC\*, EINVOICECNCRC\*, EINVOICERELRC\*, EINVOICEAFQR\*, etc.

✅ Installed

E-invoice payload staging & response storage

E-Invoice Category

EINVOICECAT, EINVOICECATT

✅ Installed

E-invoice type classification

E-Invoice Print Form

UPEINVOICE01 (SFPI + SFPF + FORM)

✅ Installed

QR code-enabled invoice print form

Argentina E-Invoice

ARGENTINA\_EINVOICE\_EDOCUMENT, ARGENTINA\_EINVOICE\_J\_1ACAE

✅ Installed

Argentine e-Factura integration

India GST Tables

J\_1IG\_B2B, J\_1IG\_B2C, J\_1IG\_CDN, J\_1IG\_AT, J\_1IG\_B2B\_INWD

✅ Installed

GST return data staging (GSTR-1, GSTR-2B)

SAP Released APIs

API\_SUPPLIERINVOICE\_PROCESS\_SRV, API\_BILLING\_DOCUMENT\_SRV

✅ Available

Clean Core Level A APIs for invoice processing

The framework infrastructure is fully installed. SAP's recommendation is to use DRC with country-specific activation, not custom ABAP development. The ZZEINVOICE custom table and Z\_\* form objects found earlier should be evaluated against this standard framework to avoid duplicate, non-standard solutions.

### 10.2 Recommended SAP Standard Approach per Country

Country

SAP Solution

Fiori App

Integration Pattern

🇮🇳 India

SAP DRC + IRP Integration

Manage E-Invoices (F6771)

JSON → IRP API → IRN + QR Code

🇮🇹 Italy

SAP DRC + SDI via Peppol/EDI

Monitor E-Documents (F5765)

FatturaPA XML → SDI

🇩🇪 Germany

SAP DRC + XRechnung

Monitor E-Documents (F5765)

UBL 2.1 XML → Peppol AP or Leitweg-ID

🇫🇷 France

SAP DRC + Factur-X / Chorus

Monitor E-Documents (F5765)

CII/Factur-X → PPF

🇭🇺 Hungary

SAP DRC + NAV RTIR

Monitor E-Documents (F5765)

XML → NAV API real-time

🇵🇱 Poland

SAP DRC + KSeF

Monitor E-Documents (F5765)

FA(2) XML → KSeF API

🇧🇷 Brazil

SAP DRC + NF-e / NFS-e

Monitor E-Documents (F5765)

XML → SEFAZ

🇲🇽 Mexico

SAP DRC + CFDI 4.0

Monitor E-Documents (F5765)

XML → PAC stamping

## 11 Consolidated Gap Matrix

The following matrix consolidates every gap identified across all assessment dimensions, ranked by severity and business impact. Each gap includes the specific SAP table/field evidence, the impact on e-invoicing compliance, and the recommended remediation action.

#

Gap

Severity

Evidence

Impact

Remediation

G1

India GST tax procedure (TAXIN/TAXINN) not configured

CRITICAL

T007A: 0 entries for TAXIN/TAXINN

Cannot calculate GST on any transaction

SPRO → Tax procedure → Assign TAXINN to IN01

G2

All vendors missing GSTIN (STCD3 = blank)

CRITICAL

LFA1: 10/10 vendors have blank STCD1-3

E-invoice JSON requires buyer GSTIN

Mass update via BP/XK02 — populate GSTIN for each vendor

G3

Company codes 0001 & IN01 missing VAT/GSTIN

CRITICAL

T001.STCEG = blank for 0001, IN01

E-invoice requires seller tax ID

OB52 / Company code settings → Populate STCEG

G4

Indian vendors assigned to DE company code (0001), not IN01

CRITICAL

LFB1: All 10 vendors on BUKRS=0001

Wrong legal entity for GST jurisdiction

Create vendor company code data under IN01

G5

No IRP/NIC API connectivity configured

CRITICAL

No RFC/HTTP destinations found for IRP

Cannot transmit e-invoices to government

SM59 → Configure IRP sandbox/production endpoints

G6

Zero billing documents (VBRK empty)

CRITICAL

VBRK: 0 records

No outbound invoices to e-invoice

Requires full SD config (sales orders → deliveries → billing)

G7

Zero vendor invoices (RBKP empty)

CRITICAL

RBKP: 0 records

No inbound invoices to process

Requires MM config (POs → GR → Invoice verification)

G8

No output determination for invoices (NAST empty)

HIGH

NAST: 0 records for V3/EL

No output channel for e-invoice transmission

NACE → Configure output types for billing docs

G9

Payment terms (T052) not configured

HIGH

T052: empty table

E-invoice PaymentDueDate cannot be calculated

OBB8 → Define payment terms

G10

No payment methods for DE/IN (T042Z missing)

HIGH

T042Z: No entries for DE or IN

E-invoice PaymentMeans element empty

FBZP → Configure payment methods per country

G11

Zero vendor bank details (LFBK empty)

HIGH

LFBK: 0 records

PEPPOL PayeeFinancialAccount empty; no auto-payment

FK02 → Maintain bank details per vendor

G12

No payment runs ever executed (REGUH empty)

HIGH

REGUH: 0 records

Invoice-to-payment cycle untested

Requires G9+G10+G11 first, then F110 test run

G13

2 vendors blocked (Wipro, HUL) with no tax data

MEDIUM

LFA1.SPERR = X for 100004, 100008

Blocked vendors excluded from e-invoice batch

Review block reason; unblock or archive

G14

Custom Z\_\* invoice objects in $TMP — not production-grade

MEDIUM

7 objects in $TMP/ZCAT/ZDEV

Risk of custom vs standard divergence

Evaluate against DRC framework; retire if redundant

## 12 Implementation Roadmap

The roadmap is structured in 4 phases, progressing from foundational configuration (without which nothing works) to advanced electronic transmission. Each phase has clear prerequisites — Phase 2 cannot start until Phase 1 is complete, and so on.

gantt title E-Invoicing Implementation Roadmap dateFormat YYYY-MM-DD axisFormat %b %d section Phase 1: Foundation Tax procedure TAXINN activation :a1, 2026-06-01, 5d Company code GSTIN population :a2, 2026-06-01, 2d Vendor GSTIN mass update :a3, after a2, 5d Payment terms configuration :a4, after a1, 3d Payment method setup DE + IN :a5, after a4, 3d section Phase 2: Master Data Vendor bank details population :b1, after a3, 5d Vendor company code reassignment :b2, after a3, 5d HSN/SAC code mapping :b3, after a1, 10d Tax code creation (CGST/SGST/IGST) :b4, after a1, 5d section Phase 3: Document Flow SD config (order to billing) :c1, after b4, 15d MM config (PO to invoice) :c2, after b4, 15d Output determination setup :c3, after c1, 5d Test billing + vendor invoices :c4, after c3, 5d section Phase 4: E-Invoice Activation IRP API connectivity (SM59) :d1, after c4, 5d DRC framework activation (India) :d2, after d1, 5d E-invoice output type config :d3, after d2, 3d End-to-end test (IRN generation) :d4, after d3, 5d EU PEPPOL activation (optional) :d5, after d3, 10d

### 12.1 Phase Priority Matrix

Phase

Duration

Effort

Gaps Addressed

Prerequisite

**Phase 1: Foundation**

~2 weeks

Medium

G1, G3, G9, G10

None — start immediately

**Phase 2: Master Data**

~2 weeks

Medium

G2, G4, G5, G11

Phase 1 complete

**Phase 3: Document Flow**

~3 weeks

High

G6, G7, G8, G12

Phase 2 complete

**Phase 4: E-Invoice Go-Live**

~3 weeks

High

G5, G13, G14

Phase 3 complete

Total estimated timeline: **10-12 weeks** from start to first successful e-invoice generation. This assumes dedicated resources and no scope creep into multi-country rollout (EU PEPPOL would add 4-6 additional weeks per country).

## 13 Expected Outcomes

Once all 4 phases are completed, the system should achieve full e-invoicing compliance for India GST (primary target) with a strong foundation for EU expansion. The table below projects the readiness score improvement across each assessment dimension.

### 13.1 Readiness Score — Before vs. After Remediation

Dimension

Weight

Before

After

Improvement

Platform Infrastructure

15%

✅ 100%

✅ 100%

— (already complete)

Tax Configuration

20%

🔴 0%

🟢 95%

+95% (TAXINN + GST tax codes)

Vendor Tax Registration

15%

🔴 0%

🟢 100%

+100% (all GSTINs populated)

Company Code Tax ID

10%

🔴 0%

🟢 100%

+100% (STCEG populated)

Document Flow (invoices exist)

15%

🔴 0%

🟢 90%

+90% (SD + MM cycles active)

Output Determination

10%

🔴 0%

🟢 95%

+95% (e-invoice output types)

IRP/DRC Connectivity

10%

🔴 0%

🟢 100%

+100% (SM59 + DRC activated)

Payment Integration

5%

🔴 0%

🟢 85%

+85% (bank + payment methods)

**Weighted Total**

**🔴 18%**

**🟢 95%**

**+77 percentage points**

### 13.2 Key Success Metrics (Post-Implementation)

95%

Target Readiness Score

0

Critical Gaps Remaining

100%

Vendor GSTIN Coverage

< 2 min

IRN Generation Time

**💡 Recommendation:** Prioritize India GST e-invoicing (Phases 1-4) as the primary objective, given the Indian vendor population and Indian hosting environment. EU PEPPOL/XRechnung can be addressed as a follow-on project using the same DRC framework. Leverage SAP standard DRC rather than building on the custom ZZEINVOICE table — this ensures upgrade compatibility and Clean Core compliance.

**Assessment Methodology:** This report was generated by C16 using automated SAP system analysis. Data was queried in real-time from 15+ SAP tables including T001 (company codes), LFA1/LFB1/LFBK (vendor master), T007A/T007S (tax codes), T042Z (payment methods), T052 (payment terms), BSIK/BSAK (open/cleared items), RBKP (vendor invoices), VBRK (billing documents), NAST (output messages), REGUH (payment runs), and TADIR (repository objects). Object discovery covered EINVOICE\*, J\_1IG\*, Z%INVOICE%, %GST%, and %EDOCUMENT% patterns. All findings are evidence-based with specific table references.

**C16 E-Invoicing Assessment Engine v2026.05** | Generated: 24 May 2026 | System: S4H (S/4HANA 758, HDB, Kernel 793)

mermaid.initialize({ startOnLoad: true, theme: 'neutral', gantt: { titleTopMargin: 15, barHeight: 20, barGap: 4, topPadding: 50 }});

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._