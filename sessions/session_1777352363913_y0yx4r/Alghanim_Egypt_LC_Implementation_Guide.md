  Letter of Credit (LC) Implementation Guide — Alghanim Industries Egypt :root { --primary: #6B3FA0; --primary-light: #8B5CF6; --accent: #B5567A; --accent-gold: #C97B3A; --success: #27ae60; --warning: #e67e22; --danger: #e74c3c; --info: #3498db; --dark: #2c3e50; --light-bg: #f8f9fa; --card-shadow: 0 4px 24px rgba(107,63,160,0.10); } \* { box-sizing: border-box; margin: 0; padding: 0; } body { font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, Arial, sans-serif; color: var(--dark); background: #e9ecef; line-height: 1.7; font-size: 15px; } .doc-wrapper { max-width: 1140px; margin: 32px auto; background: #fff; border-radius: 16px; box-shadow: 0 8px 48px rgba(107,63,160,0.12); overflow: hidden; } /\* ── HEADER BANNER ── \*/ .header-banner { background: linear-gradient(135deg, #C97B3A 0%, #B5567A 40%, #6B3FA0 100%); color: #fff; padding: 48px 56px; position: relative; overflow: hidden; } .header-banner::after { content: ''; position: absolute; top: -40%; right: -10%; width: 420px; height: 420px; border-radius: 50%; background: rgba(255,255,255,0.06); } .header-banner img { height: 52px; border-radius: 8px; margin-bottom: 14px; } .header-banner h1 { font-size: 2.1em; font-weight: 700; margin-bottom: 6px; position: relative; z-index: 1; } .header-banner .subtitle { font-size: 1.15em; opacity: 0.92; position: relative; z-index: 1; } .header-banner .meta-badges { margin-top: 18px; display: flex; gap: 10px; flex-wrap: wrap; position: relative; z-index: 1; } .header-banner .badge { display: inline-block; padding: 5px 16px; border-radius: 20px; font-size: 0.82em; font-weight: 600; background: rgba(255,255,255,0.18); border: 1px solid rgba(255,255,255,0.3); } /\* ── CONTENT ── \*/ .content { padding: 40px 56px 56px; } /\* ── META BAR ── \*/ .meta-bar { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 14px; background: var(--light-bg); border-radius: 12px; padding: 22px 28px; margin-bottom: 36px; border-left: 5px solid var(--primary); } .meta-item { font-size: 0.88em; } .meta-item .label { color: #888; font-size: 0.78em; text-transform: uppercase; letter-spacing: 0.5px; } .meta-item .value { font-weight: 600; color: var(--dark); margin-top: 2px; } /\* ── TABLE OF CONTENTS ── \*/ .toc { background: linear-gradient(135deg, #f3f0ff 0%, #fdf2f8 100%); border-radius: 12px; padding: 28px 34px; margin-bottom: 40px; border: 1px solid #e8dff5; } .toc h2 { font-size: 1.2em; color: var(--primary); margin-bottom: 14px; } .toc ol { padding-left: 22px; } .toc li { margin-bottom: 5px; } .toc a { color: var(--primary); text-decoration: none; font-weight: 500; } .toc a:hover { text-decoration: underline; color: var(--accent); } .toc .sub-items { padding-left: 20px; list-style-type: disc; } .toc .sub-items li { font-size: 0.92em; margin-bottom: 2px; } /\* ── SECTION TITLES ── \*/ .section-title { font-size: 1.55em; color: var(--primary); margin: 48px 0 20px; padding-bottom: 10px; border-bottom: 3px solid var(--accent); display: flex; align-items: center; gap: 14px; } .section-title .num { background: var(--accent); color: #fff; width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.72em; font-weight: 700; flex-shrink: 0; } h3 { font-size: 1.18em; color: var(--primary); margin: 30px 0 12px; padding-left: 12px; border-left: 4px solid var(--accent); } h4 { font-size: 1.02em; color: var(--dark); margin: 22px 0 8px; font-weight: 600; } p { margin-bottom: 14px; } /\* ── KPI CARDS ── \*/ .kpi-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px; margin: 24px 0; } .kpi-card { background: linear-gradient(135deg, #f8f6ff, #fdf2f8); border-radius: 12px; padding: 22px; text-align: center; border: 1px solid #e8dff5; transition: transform 0.15s; } .kpi-card:hover { transform: translateY(-2px); box-shadow: var(--card-shadow); } .kpi-card .kpi-value { font-size: 2em; font-weight: 700; color: var(--primary); } .kpi-card .kpi-label { font-size: 0.82em; color: #666; margin-top: 4px; } /\* ── TABLES ── \*/ table { width: 100%; border-collapse: collapse; margin: 16px 0 24px; font-size: 0.92em; border-radius: 8px; overflow: hidden; } thead th { background: var(--primary); color: #fff; padding: 12px 14px; text-align: left; font-weight: 600; font-size: 0.88em; text-transform: uppercase; letter-spacing: 0.3px; } tbody td { padding: 10px 14px; border-bottom: 1px solid #eee; } tbody tr:nth-child(even) { background: #f8fafc; } tbody tr:hover { background: #edf2f7; } /\* ── HIGHLIGHT BOXES ── \*/ .highlight-box { border-radius: 10px; padding: 20px 24px; margin: 20px 0; } .highlight-box.info { background: #ebf5fb; border-left: 5px solid var(--info); } .highlight-box.success { background: #eafaf1; border-left: 5px solid var(--success); } .highlight-box.warning { background: #fef9e7; border-left: 5px solid var(--warning); } .highlight-box.danger { background: #fdedec; border-left: 5px solid var(--danger); } .highlight-box.purple { background: #f3f0ff; border-left: 5px solid var(--primary); } .highlight-box strong { color: var(--dark); } /\* ── CODE / PRE ── \*/ pre { background: #1e1e2e; color: #cdd6f4; border-radius: 8px; padding: 18px 22px; overflow-x: auto; font-family: 'Cascadia Code', 'Fira Code', 'Consolas', monospace; font-size: 0.88em; line-height: 1.6; margin: 14px 0 22px; border-left: 4px solid var(--accent); } code { font-family: 'Cascadia Code', 'Fira Code', 'Consolas', monospace; background: #f3f0ff; padding: 2px 6px; border-radius: 4px; font-size: 0.9em; color: var(--primary); } pre code { background: none; padding: 0; color: inherit; } /\* ── PROCESS FLOW ── \*/ .process-flow { display: flex; align-items: center; gap: 0; margin: 20px 0; flex-wrap: wrap; justify-content: center; } .process-step { background: linear-gradient(135deg, var(--primary), var(--primary-light)); color: #fff; padding: 14px 22px; border-radius: 10px; text-align: center; font-size: 0.85em; font-weight: 600; min-width: 120px; box-shadow: 0 2px 8px rgba(107,63,160,0.18); } .process-step .step-num { font-size: 0.72em; opacity: 0.8; display: block; margin-bottom: 4px; } .process-step .step-code { font-size: 0.72em; opacity: 0.85; margin-top: 4px; display: block; } .process-arrow { font-size: 1.5em; color: var(--accent); padding: 0 6px; font-weight: 700; } /\* ── ACCOUNTING ENTRY ── \*/ .acct-entry { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; padding: 20px 24px; margin: 16px 0; font-family: 'Cascadia Code', 'Consolas', monospace; font-size: 0.88em; line-height: 1.8; } .acct-entry .entry-title { font-family: 'Segoe UI', Arial, sans-serif; font-weight: 700; color: var(--primary); margin-bottom: 10px; font-size: 1em; } .acct-entry .dr { color: #c0392b; } .acct-entry .cr { color: #27ae60; } .acct-entry .amt { float: right; font-weight: 600; } /\* ── CHECKLIST ── \*/ .checklist { list-style: none; padding: 0; } .checklist li { padding: 10px 14px; border-bottom: 1px solid #eee; display: flex; align-items: flex-start; gap: 10px; } .checklist li:nth-child(even) { background: #f8fafc; } .checklist .check-icon { font-size: 1.1em; flex-shrink: 0; } /\* ── BADGES ── \*/ .badge-phase { display: inline-block; padding: 3px 12px; border-radius: 12px; font-size: 0.78em; font-weight: 600; color: #fff; } .badge-phase.config { background: var(--info); } .badge-phase.process { background: var(--success); } .badge-phase.report { background: var(--warning); } .badge-phase.custom { background: var(--accent); } .badge-phase.critical { background: var(--danger); } /\* ── TIMELINE ── \*/ .timeline { position: relative; padding-left: 30px; margin: 20px 0; } .timeline::before { content: ''; position: absolute; left: 8px; top: 0; bottom: 0; width: 3px; background: linear-gradient(to bottom, var(--primary), var(--accent)); border-radius: 2px; } .timeline-item { position: relative; margin-bottom: 24px; padding: 16px 20px; background: var(--light-bg); border-radius: 10px; border: 1px solid #e8dff5; } .timeline-item::before { content: ''; position: absolute; left: -26px; top: 20px; width: 12px; height: 12px; border-radius: 50%; background: var(--primary); border: 3px solid #fff; box-shadow: 0 0 0 2px var(--primary); } .timeline-item .tl-phase { font-weight: 700; color: var(--primary); font-size: 1.05em; } .timeline-item .tl-duration { font-size: 0.82em; color: #888; } /\* ── MERMAID ── \*/ .mermaid { margin: 20px 0; text-align: center; } /\* ── COLUMNS ── \*/ .two-col { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin: 16px 0; } .three-col { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px; margin: 16px 0; } /\* ── FOOTER ── \*/ .doc-footer { background: var(--light-bg); padding: 28px 56px; border-top: 2px solid #e8dff5; font-size: 0.82em; color: #888; } .doc-footer .logo-row { display: flex; align-items: center; gap: 12px; margin-bottom: 10px; } .doc-footer .logo-row img { height: 28px; border-radius: 4px; } /\* ── PRINT ── \*/ @media print { body { background: #fff; } .doc-wrapper { box-shadow: none; margin: 0; } .header-banner { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .process-flow { page-break-inside: avoid; } pre { white-space: pre-wrap; } } @media (max-width: 768px) { .content { padding: 24px 20px; } .header-banner { padding: 32px 24px; } .header-banner h1 { font-size: 1.5em; } .two-col, .three-col { grid-template-columns: 1fr; } .process-flow { flex-direction: column; } .process-arrow { transform: rotate(90deg); } } :root{--c16-purple-1:#6B3FA0;--c16-purple-2:#8B5CF6;--c16-ink:#1f2937;--c16-sub:#4b5563;--c16-rule:#e5e7eb;--c16-accent:#faf5ff;}

![C16](data:image/jpeg;base64,)

# Letter of Credit (LC) Implementation Guide

Alghanim Industries — Egypt Import Procure-to-Pay

SAP ECC 7.40 FI-TF Trade Finance Country: Egypt (EG) Currency: EGP / USD / EUR April 2026

Company Code

EGAL (Egypt – Alghanim)

Module

FI-TF (Trade Finance) + MM

Purchasing Org

EGPO

LC Standard

UCP 600 (ICC)

Regulatory Body

Central Bank of Egypt (CBE)

Document Version

1.0 — April 2026

6

LC Product Types

12

Flow Types Configured

16+

GL Accounts Required

9

LC Lifecycle Statuses

5

Custom Z-Reports

## 📑 Table of Contents

1.  [Executive Summary & Business Case](#s1)
2.  [What is a Letter of Credit?](#s2)
3.  [SAP Module & Approach Selection](#s3)
4.  [Organizational Prerequisites](#s4)
5.  [Configuration — LC Product & Transaction Types](#s5)
6.  [Configuration — Flow Types & Condition Groups](#s6)
7.  [House Bank & Payment Method Configuration](#s7)
8.  [General Ledger Account Design](#s8)
9.  [End-to-End LC Process — 10-Phase Lifecycle](#s9)
10.  [Accounting Entries — Complete Journal Map](#s10)
11.  [LC ↔ P2P Integration Points](#s11)
12.  [Egypt Regulatory & CBE Compliance](#s12)
13.  [Foreign Exchange & Hedging Strategy](#s13)
14.  [Egyptian Customs & NAFEZA Integration](#s14)
15.  [Withholding Tax on LC-Related Services](#s15)
16.  [Arabic Output & Bilingual Forms](#s16)
17.  [Reporting & Analytics — Standard + Custom](#s17)
18.  [Security & Authorization Roles](#s18)
19.  [Workflow & Automation](#s19)
20.  [Testing Strategy & UAT Scenarios](#s20)
21.  [Implementation Roadmap & Timeline](#s21)
22.  [Risk Register & Mitigation](#s22)
23.  [Appendix — Transaction Codes, GL Map, Glossary](#s23)

## 1 Executive Summary & Business Case

Alghanim Industries' Egyptian operations rely heavily on **imported goods** — raw materials, finished goods, and capital equipment — sourced from suppliers across China, India, Europe, and the GCC. The Letter of Credit (LC) is the **primary payment instrument** for these imports, mandated both by supplier requirements and Central Bank of Egypt (CBE) foreign exchange regulations.

This document provides a **complete, configuration-ready implementation guide** for the LC module within SAP ECC 7.40. It covers every layer: organizational setup, SPRO configuration, GL account design, the full 10-phase LC lifecycle with accounting entries, Egypt-specific regulatory compliance (CBE, NAFEZA, customs), foreign exchange management, Arabic localization, workflow automation, and a structured testing and rollout plan.

**🎯 Implementation Objectives:**

*   **Automate LC lifecycle** — from PO creation through bank settlement and LC closure
*   **Full financial visibility** — contingent liabilities, margin deposits, bank charges, FX exposure on the balance sheet in real time
*   **CBE compliance** — adhere to Central Bank of Egypt import payment regulations and reporting requirements
*   **Integrated P2P** — seamless link between Purchase Orders, Goods Receipts, Invoices, and LC payments
*   **Treasury decision support** — LC maturity schedules, cash flow forecasting, and FX exposure dashboards
*   **Alghanim group alignment** — consistent trade finance processes across Kuwait, UAE, and Egypt entities

### Scope of This Guide

In Scope

Out of Scope (Phase 2)

Import LC — Sight & Usance

Export LC (if applicable)

Standby LC (SBLC) / Bank Guarantees

Forfaiting / Factoring

LC Amendments & Discrepancy Management

Documentary Collections (D/P, D/A)

Contingent Liability Accounting (Off-B/S)

Supply Chain Finance (SCF)

Margin Deposit Tracking

Back-to-Back LC

Bank Charge Accounting

Transferable LC

FX Gain/Loss on LC Settlements

FX Hedging via Derivatives (Forward/Options)

Integration with MM (PO → GR → MIRO)

Treasury Workstation integration

5 Custom Z-Reports

NAFEZA API integration (Phase 2)

## 2 What is a Letter of Credit?

A Letter of Credit (خطاب اعتماد / _Khetāb Eʿtimād_) is a **bank-issued financial instrument** that guarantees payment to an overseas supplier once specified shipping documents are presented in compliance with the LC terms. It is governed by the **ICC Uniform Customs and Practice for Documentary Credits (UCP 600)**.

### Key Parties Involved

Party

Role

Alghanim Context

**Applicant**

The importer who requests the LC

Alghanim Industries Egypt (EGAL)

**Beneficiary**

The overseas supplier who receives payment

Chinese/Indian/European suppliers

**Issuing Bank**

Applicant's bank that issues the LC

National Bank of Egypt (NBE), CIB, Banque Misr

**Advising Bank**

Beneficiary's bank that advises the LC

Supplier's local bank abroad

**Confirming Bank**

Adds its guarantee (optional, for confirmed LCs)

International bank (HSBC, Citi, Standard Chartered)

**Negotiating Bank**

Bank that examines and pays against documents

Usually the advising bank

**Reimbursing Bank**

Correspondent bank for cross-currency settlement

NY (USD), Frankfurt (EUR) correspondent

### Types of LC Used in Egypt Import Trade

LC Type

Payment Timing

Typical Use

Risk Profile

**Sight LC**

Immediate upon compliant document presentation

Urgent/perishable goods, small orders

Low risk for supplier, high cash impact for buyer

**Usance LC (30/60/90/120/180 days)**

Deferred — N days from B/L date or presentation

Standard imports — gives buyer time to sell/use goods

Balanced risk; most common in Egypt

**Deferred Payment LC**

Fixed future date

When exact payment date is agreed

Similar to usance

**Revolving LC**

Auto-reinstates after each drawing

Regular shipments from same supplier

Efficient for recurring purchases

**Standby LC (SBLC)**

Drawn only on default

Performance guarantee, backup payment

Rarely drawn; acts as insurance

**Red Clause LC**

Advance payment allowed before shipment

Supplier needs pre-shipment finance

Higher risk for buyer/bank

### End-to-End LC Flow — Visual Overview

sequenceDiagram participant AG as Alghanim Egypt participant IB as Issuing Bank (NBE) participant AB as Advising Bank participant SP as Foreign Supplier AG->>IB: 1. LC Application (FTR\_CREATE) IB->>AB: 2. Issue LC (SWIFT MT700) AB->>SP: 3. Advise LC to Supplier SP->>SP: 4. Ship Goods SP->>AB: 5. Present Documents (B/L, Invoice, CoO...) AB->>IB: 6. Forward Documents IB->>AG: 7. Notify — Examine Documents AG->>IB: 8. Accept Documents Note over IB,SP: Sight LC: Pay immediately  
Usance LC: Accept draft, pay at maturity IB->>AB: 9. Payment / Acceptance AB->>SP: 10. Credit Supplier Account IB->>AG: 11. Debit Alghanim (at sight or maturity) AG->>AG: 12. GR (MIGO) + MIRO + LC Closure

## 3 SAP Module & Approach Selection

SAP offers multiple approaches for managing Letters of Credit. The right choice depends on the system version, available enhancement packs, and the level of integration required.

Approach

Component

Available On

Pros

Cons

**TRM Trade Finance**

FI-TF (via Treasury & Risk Mgmt)

ECC 6.0 EHP4+

Full lifecycle mgmt, integrated accounting, status tracking, bank charge handling

Requires TRM activation; licensing considerations

**Classic Trade Finance**

FI-LC

ECC 6.0+

Simpler setup, basic LC tracking

Limited lifecycle features, being deprecated

**Custom Z-Solution**

Custom tables + ABAP

Any SAP

Full control, tailored to Egypt

Maintenance burden, no SAP standard upgrade path

**SAP FSCM Trade Finance**

FIN-FSCM-TF

S/4HANA only

Modern, Fiori-ready, best features

Not available on ECC

**✅ Recommendation for Alghanim on ECC 7.40:**  
Use **TRM Trade Finance (FI-TF)** via the Transaction Manager. This provides the richest LC lifecycle management, integrated accounting postings, status tracking, and bank charge handling — all natively within the ECC system. Transactions: `FTR_CREATE`, `FTR_EDIT`, `FTR_DISPLAY`.

## 4 Organizational Prerequisites

Before configuring LC-specific objects, the following foundational elements must be in place within the SAP system.

Prerequisite

Configuration

TCode

Status

Company Code

`EGAL` — Egypt, EGP, Arabic+English

OBY6

⬜ Required

Chart of Accounts

Operational CoA + Egyptian Statutory CoA

OB13

⬜ Required

Fiscal Year Variant

`K4` (Jan–Dec) assigned to EGAL

OB29

⬜ Required

Purchasing Organization

`EGPO` assigned to EGAL

SPRO

⬜ Required

Plant(s)

Egyptian plant(s) assigned to EGAL/EGPO

OX10

⬜ Required

House Banks

NBE, CIB, or Banque Misr with EGP/USD/EUR accounts

FI12

⬜ Required

GL Accounts — LC

16+ accounts (see Section 8)

FS00

⬜ Required

Exchange Rate Types

`M` (standard), `B` (buying), `G` (selling)

OB07

⬜ Required

Factory Calendar

`EG` — Sunday–Thursday, Egyptian holidays

SCAL

⬜ Required

Payment Method `L`

Letter of Credit payment method for EGAL

FBZP

⬜ Required

Foreign Vendor Masters

Vendors with bank details, currency, Incoterms, WHT

FK01

⬜ Required

Import PO Type

`ZIMP` — custom import PO document type

SPRO

⬜ Recommended

Pricing Conditions

Freight (`ZFR1`), Insurance (`ZINS`), Duty (`ZCD1`)

M/06

⬜ Required

WHT Configuration

Extended WHT for LC-related services (customs broker, etc.)

OBWW

⬜ Required

TRM Activation

Activate Transaction Manager for EGAL

OT01

⬜ Required

## 5 Configuration — LC Product & Transaction Types

### 5.1 Activate Treasury / Transaction Manager

```
IMG Path: Treasury → Transaction Manager → General Settings → Activate Transaction Manager
TCode: OT01

Company Code: EGAL
✅ Activate TRM
✅ Trade Finance active
✅ Valuation area: EGAL
```

### 5.2 Define Product Types

Product Types categorize the different LC instruments. Each product type has specific behavior for accounting, status management, and reporting.

```
IMG Path: Treasury → Transaction Manager → Trade Finance
         → Transaction Management → Define Product Types
TCode: FTRV_PRODUCT
```

Product Type

Description

Payment Timing

Accounting Rule

`5A00`

Import LC – Sight

Immediate on doc presentation

Direct bank debit

`5A10`

Import LC – Usance

Deferred (30–180 days)

Acceptance → maturity posting

`5A20`

Import LC – Deferred Payment

Fixed future date

Similar to usance

`5A30`

Import LC – Revolving

Auto-reinstates per drawing

Recurring utilization

`5B00`

Standby LC (SBLC)

On default only

Contingent only (usually)

`5B10`

Bank Guarantee (BG)

On demand

Contingent liability

### 5.3 Define Transaction Types

Transaction Types further refine product types by tenor or specific business scenario.

Trans Type

Product Type

Description

Typical Tenor

`100`

5A00

Import LC at Sight

0 days (immediate)

`110`

5A10

Import LC Usance 30 days

30 days from B/L

`120`

5A10

Import LC Usance 60 days

60 days from B/L

`130`

5A10

Import LC Usance 90 days

90 days from B/L

`140`

5A10

Import LC Usance 120 days

120 days from B/L

`150`

5A10

Import LC Usance 180 days

180 days from B/L

`200`

5A30

Revolving LC

Per drawing cycle

`300`

5B00

Standby LC

1 year (typically)

`310`

5B10

Bank Guarantee – Performance

Project-specific

### 5.4 Number Ranges

```
TCode: FTRV_NUMBERING (or FTR_NR)

Object: LC Transaction Numbers
Number Range ID: LC
From:  1000000000
To:    1999999999
Assignment: Internal (auto-assigned)

Recommended: Year-based sub-ranges:
  2026: 2026000001 – 2026999999
  2027: 2027000001 – 2027999999
```

## 6 Configuration — Flow Types & Condition Groups

### 6.1 Flow Types

Flow types represent individual financial events within the LC lifecycle. Each flow type triggers specific accounting postings.

Flow Type

Category

Description

Posting Direction

`5100`

Opening

LC Opening — Contingent Liability

Off-B/S: Dr Contingent Liab / Cr Contingent Asset

`5110`

Amendment

LC Amount Increase

Increase contingent liability

`5120`

Amendment

LC Amount Decrease

Decrease contingent liability

`5200`

Utilization

LC Drawing / Utilization (Sight)

Dr Vendor / Cr Bank

`5210`

Acceptance

LC Acceptance (Usance)

Dr Vendor / Cr LC Payable Usance

`5220`

Maturity

LC Maturity Payment (Usance)

Dr LC Payable Usance / Cr Bank

`5300`

Charges

LC Opening Commission

Dr Commission Expense / Cr Bank

`5310`

Charges

LC Amendment Fee

Dr Amendment Fee / Cr Bank

`5320`

Charges

SWIFT / Telex Charges

Dr SWIFT Expense / Cr Bank

`5330`

Charges

Acceptance Commission

Dr Acceptance Comm / Cr Bank

`5340`

Charges

Discrepancy Fee

Dr Discrepancy Fee / Cr Bank

`5350`

Charges

Confirmation Fee

Dr Confirmation Fee / Cr Bank

`5400`

Closure

LC Expiry / Cancellation

Reverse contingent liability

`5500`

Margin

Margin Deposit to Bank

Dr Margin Deposit / Cr Bank

`5510`

Margin

Margin Release on LC Closure

Dr Bank / Cr Margin Deposit

`5600`

FX

Foreign Exchange Gain

Cr FX Gain

`5610`

FX

Foreign Exchange Loss

Dr FX Loss

### 6.2 Condition Groups — Bank Charges

Egyptian banks charge significant fees for LC services. These must be captured accurately for cost allocation and reporting.

Charge Type

Typical Rate (Egypt)

Basis

When Charged

LC Opening Commission

0.125% – 0.25% per quarter

LC face value

At issuance + each quarter

Amendment Fee

EGP 500 – 2,000 flat

Per amendment

On each amendment

SWIFT Charges (MT700)

USD 30 – 50 flat

Per message

At issuance

Acceptance Commission

0.15% – 0.25%

Accepted amount

On usance LC acceptance

Discrepancy Fee

USD 50 – 100 flat

Per discrepancy set

If documents have discrepancies

Confirmation Fee

0.20% – 0.50% per quarter

Confirmed amount

For confirmed LCs

Advising Fee

0.10%

LC amount

Charged by advising bank

Negotiation Fee

0.125%

Negotiated amount

On document negotiation

Reimbursement Fee

USD 200 – 500

Per reimbursement

Cross-border settlement

**⚠️ Cost Impact:** For a typical USD 100,000 usance LC (90 days), total bank charges in Egypt can range from **USD 800 – 2,500** (0.8% – 2.5% of LC value). These costs must be factored into the **total landed cost** calculation.

## 7 House Bank & Payment Method Configuration

### 7.1 House Bank Setup

```
TCode: FI12

Company Code: EGAL
House Bank ID: NBEG (National Bank of Egypt)
  Bank Country: EG
  Bank Key:     003 (NBE bank code)
  SWIFT Code:   NBEGEGCX
  Bank Name:    National Bank of Egypt — الأهلي المصري
  Address:      1187 Corniche El Nil, Cairo, Egypt

Alternative banks (for diversification):
  House Bank: CIBE (Commercial International Bank)
    SWIFT: CIBEEGCX001
  House Bank: BMIS (Banque Misr)
    SWIFT: BMISEGCX
```

### 7.2 Bank Accounts per House Bank

Account ID

GL Account

Currency

Description

Purpose

`CURR_EGP`

1110100

EGP

Current Account — EGP

Domestic operations, margin deposits

`CURR_USD`

1110200

USD

Current Account — USD

Import LC settlements (USD)

`CURR_EUR`

1110300

EUR

Current Account — EUR

European supplier LCs

`LC_MRGN`

1120100

EGP

LC Margin Deposit Account

Cash margins held by bank

`LC_UTIL`

1120200

Multi

LC Utilization Clearing

Transient account for LC drawings

### 7.3 Payment Method Configuration

```
TCode: FBZP (Automatic Payment Configuration)

Section: Payment Methods per Company Code
  Company Code: EGAL
  Payment Method: L
  Description: Letter of Credit

Settings:
  ✅ Allowed for: Foreign payments (Outgoing)
  Minimum Amount: EGP 50,000 (or equiv. in foreign currency)
  Maximum Amount: EGP 100,000,000
  Currency: *** (all currencies)
  
  Required master data:
  ✅ Bank account details required
  ✅ Partner bank required (beneficiary bank)

Section: Bank Determination
  Company Code: EGAL
  Payment Method: L
  → Ranking: NBEG (primary), CIBE (secondary)
  → Available Amounts: Per bank balance + credit facility
  → Value Date: Egyptian banking calendar (Sun–Thu)
```

### 7.4 Payment Terms for LC

Payment Term

Description

Days

Baseline Date

`ZLC0`

LC at Sight — Immediate

0

Document date

`ZLC3`

LC Usance 30 days

30

B/L date

`ZLC6`

LC Usance 60 days

60

B/L date

`ZLC9`

LC Usance 90 days

90

B/L date

`ZLCA`

LC Usance 120 days

120

B/L date

`ZLCB`

LC Usance 180 days

180

B/L date

## 8 General Ledger Account Design

A well-structured GL account design is critical for accurate financial reporting, IFRS compliance, and management visibility into trade finance exposure. The accounts below are organized by financial statement category.

### 8.1 Balance Sheet Accounts

GL Account

Description

Description (Arabic)

Type

Currency

`1110100`

Bank Current A/c — EGP

الحساب الجاري — جنيه مصري

Asset

EGP

`1110200`

Bank Current A/c — USD

الحساب الجاري — دولار أمريكي

Asset

USD

`1110300`

Bank Current A/c — EUR

الحساب الجاري — يورو

Asset

EUR

`1120100`

LC Margin Deposits

ودائع هامش خطابات الاعتماد

Asset

EGP

`1120200`

LC Utilization Clearing

تسويات استخدام خطابات الاعتماد

Asset

Multi

`2010100`

Vendor Payable — AP

ذمم الموردين الدائنة

Liability

Multi

`2240100`

LC Payable — Sight

مستحقات خطابات الاعتماد — فوري

Liability

Multi

`2240200`

LC Payable — Usance

مستحقات خطابات الاعتماد — آجل

Liability

Multi

### 8.2 Off-Balance Sheet (Contingent) Accounts

GL Account

Description

Description (Arabic)

Type

`2250100`

Contingent Liability — LC Outstanding

التزامات عرضية — خطابات اعتماد قائمة

Off-B/S Liability

`2250200`

Contingent Asset — LC Outstanding

أصول عرضية — خطابات اعتماد قائمة

Off-B/S Asset

**📋 IFRS Note:** Under **IAS 37 / IFRS 9**, LC contingent liabilities must be disclosed in the notes to the financial statements. The off-balance sheet accounts (2250100/2250200) enable automated extraction of this disclosure. Egyptian Accounting Standards (EAS) follow a similar treatment.

### 8.3 Profit & Loss Accounts

GL Account

Description

Description (Arabic)

Category

`5410100`

LC Opening Commission

عمولة فتح خطاب الاعتماد

Finance Cost

`5410200`

LC SWIFT / Telex Charges

رسوم سويفت / تلكس

Finance Cost

`5410300`

LC Acceptance Commission

عمولة قبول خطاب الاعتماد

Finance Cost

`5410400`

LC Discrepancy Charges

رسوم التناقضات المستندية

Finance Cost

`5410500`

LC Confirmation Fee

رسوم تأكيد خطاب الاعتماد

Finance Cost

`5410600`

LC Amendment Fee

رسوم تعديل خطاب الاعتماد

Finance Cost

`5420100`

Import Insurance Premium

أقساط تأمين الاستيراد

Import Cost

`5430100`

FX Gain — Trade Finance

أرباح فروق العملة — تمويل التجارة

Other Income

`5430200`

FX Loss — Trade Finance

خسائر فروق العملة — تمويل التجارة

Finance Cost

### 8.4 Account Determination — Flow Type to GL Mapping

```
IMG Path: Treasury → TRM → Accounting → Define Account Determination
```

Flow Type

Event

Debit Account

Credit Account

5100

LC Opening

2250100 Contingent Liab

2250200 Contingent Asset

5110

LC Increase

2250100 Contingent Liab

2250200 Contingent Asset

5120

LC Decrease

2250200 Contingent Asset

2250100 Contingent Liab

5200

Sight Drawing

2010100 Vendor AP

1110200 Bank USD

5210

Usance Accept

2010100 Vendor AP

2240200 LC Payable Usance

5220

Usance Maturity

2240200 LC Payable Usance

1110200 Bank USD

5300

Opening Commission

5410100 Comm Expense

1110100 Bank EGP

5310

Amendment Fee

5410600 Amendment Exp

1110100 Bank EGP

5320

SWIFT Charges

5410200 SWIFT Expense

1110100 Bank EGP

5330

Accept Commission

5410300 Accept Comm

1110100 Bank EGP

5340

Discrepancy Fee

5410400 Discr Expense

1110100 Bank EGP

5350

Confirmation Fee

5410500 Confirm Expense

1110100 Bank EGP

5400

LC Expiry/Close

2250200 Contingent Asset

2250100 Contingent Liab

5500

Margin Deposit

1120100 Margin Deposit

1110100 Bank EGP

5510

Margin Release

1110100 Bank EGP

1120100 Margin Deposit

5600

FX Gain

(system)

5430100 FX Gain

5610

FX Loss

5430200 FX Loss

(system)

## 9 End-to-End LC Process — 10-Phase Lifecycle

Phase 1Import POME21N

→

Phase 2LC CreationFTR\_CREATE

→

Phase 3LC IssuanceBank

→

Phase 4AmendmentFTR\_EDIT

→

Phase 5ShipmentExternal

Phase 6Doc PresentationBank

→

Phase 7Goods ReceiptMIGO

→

Phase 8Invoice VerificationMIRO

→

Phase 9LC SettlementFTR\_EDIT

→

Phase 10LC ClosureFTR\_EDIT

### Phase 1: Import Purchase Order (ME21N)

The process begins when Procurement creates an import Purchase Order specifying LC as the payment instrument.

Field

Value

Purpose

PO Type

`ZIMP` (Import PO) or `NB`

Identifies as import procurement

Vendor

200001 (Foreign Supplier)

Must have foreign address + bank details

Currency

USD / EUR

Foreign currency of the LC

Incoterms

CIF Cairo / FOB Shanghai / CFR Alexandria

Determines responsibility for freight/insurance

Payment Terms

`ZLC9` (LC 90-day usance)

Triggers LC payment method selection

Payment Method

`L` (Letter of Credit)

Routes payment through LC instrument

#### PO Pricing Conditions (Import-Specific)

Condition Type

Description

Amount

Origin

`PB00`

Base Price

USD 50,000.00

Supplier quotation

`ZFR1`

International Freight

USD 2,500.00

Freight forwarder

`ZINS`

Marine/Cargo Insurance

USD 500.00

Insurance provider

`ZCD1`

Customs Duty (estimated)

USD 5,000.00

HS Code tariff rate

`ZCDC`

Customs Clearance Fee

EGP 3,000.00

Customs broker

`ZGOI`

GOEIC Inspection Fee

EGP 5,000.00

GOEIC (if required)

#### PO Header Text — LC Instructions

```
Text Type: LC Instructions (ZLC)

"Irrevocable Letter of Credit
Usance: 90 days from Bill of Lading date
Partial Shipments: Allowed
Transshipment: Allowed
Latest Shipment Date: DD/MM/YYYY
LC Expiry Date: DD/MM/YYYY
Port of Loading: Shanghai, China
Port of Discharge: Alexandria / Port Said, Egypt

Documents Required:
1. Signed Commercial Invoice (3 originals + 2 copies)
2. Full Set of Clean On-Board Ocean Bill of Lading (3/3 originals)
   → Consigned to: Order of National Bank of Egypt
   → Notify: Alghanim Industries Egypt, Cairo
3. Packing List (2 originals)
4. Certificate of Origin (Chamber of Commerce certified)
5. Insurance Certificate / Policy (110% CIF value)
6. Inspection Certificate (if GOEIC required)
7. Beneficiary Certificate certifying shipment per LC terms

Applicable Rules: UCP 600 (ICC)
All banking charges outside Egypt are for beneficiary's account."
```

### Phase 2: LC Creation in SAP (FTR\_CREATE)

```
TCode: FTR_CREATE

Product Type:      5A10 (Import LC – Usance)
Transaction Type:  130 (Usance 90 days)
Company Code:      EGAL
```

Section

Field

Value

**General Data**

LC Number

Auto-generated (e.g., 2026000001)

Beneficiary (Partner)

200001 (Foreign Supplier)

Issuing Bank

NBEG (National Bank of Egypt)

Advising Bank

Supplier's bank (from vendor master)

**Amounts**

LC Amount

USD 50,000.00

Currency

USD

Tolerance (+/-)

5% / 10% (per UCP 600 Art. 30)

Margin %

10% – 25% (per bank requirement)

**Dates**

Application Date

DD/MM/2026

Issue Date

DD/MM/2026 (bank confirms)

Latest Shipment

DD/MM/2026

Expiry Date

DD/MM/2026 (21 days after latest ship)

**Terms**

Usance Period

90 days from B/L date

Partial Shipment

Allowed / Not Allowed

Transshipment

Allowed

**Reference**

PO Number

4500001234 (linked)

### Phase 3: Bank Issuance (SWIFT MT700)

After Alghanim submits the LC application, the issuing bank (NBE) performs credit checks, reserves the margin deposit, and issues the LC via SWIFT MT700 message to the advising bank abroad. In SAP, update the LC status to **"Issued"** via `FTR_EDIT` and record the bank's LC reference number.

### Phase 4: LC Amendment (FTR\_EDIT)

Amendments are common in trade — extending shipment dates, adjusting quantities, or changing document requirements. Each amendment via `FTR_EDIT` generates flow type `5110` (increase) or `5120` (decrease) and posts adjustments to contingent liability accounts. Amendment fees (flow type `5310`) are booked as expenses.

### Phase 5: Shipment & Documentation

The supplier ships goods and presents documents to the advising bank. This phase is external to SAP but must be tracked: update the LC status to **"Shipped"** and record the Bill of Lading number, vessel name, and estimated arrival date in the LC notes.

### Phase 6: Document Presentation & Examination

The issuing bank (NBE) examines documents against LC terms within **5 banking days** (UCP 600 Art. 14). If compliant, the bank notifies Alghanim and presents documents for acceptance. If discrepancies exist, Alghanim must decide: **accept despite discrepancies** (with bank's discrepancy fee) or **reject**.

**🚨 Common Discrepancies in Egypt Import LCs:**

*   Late shipment (B/L date after latest shipment date)
*   Late presentation (documents presented after 21-day period per UCP 600 Art. 14c)
*   Goods description mismatch between invoice and LC
*   B/L "shipped on board" notation missing
*   Insurance coverage less than 110% CIF value
*   Certificate of Origin not chamber-certified
*   Missing GOEIC inspection certificate (for controlled goods)

### Phase 7: Goods Receipt (MIGO)

Upon cargo arrival in Egypt and customs clearance, Procurement posts the Goods Receipt in SAP via `MIGO` (movement type `101`). Cross-reference the Bill of Lading number and customs declaration number. If quality inspection is required, use movement type `103` (GR to QI stock).

### Phase 8: Invoice Verification (MIRO)

The supplier's commercial invoice is verified via `MIRO` against the PO and GR (3-way match). The invoice references the LC number. VAT on imports (14%) is calculated on the **CIF value + customs duty**. The vendor payable is created, which will be settled through the LC instrument rather than the normal F110 payment run.

### Phase 9: LC Settlement

Settlement depends on the LC type:

**Sight LC Settlement:**  
Bank pays supplier immediately upon document acceptance. In SAP: `FTR_EDIT` → Flow Type `5200` (Drawing). Vendor AP is cleared, bank account is debited. Contingent liability is reversed.

**Usance LC Settlement:**  
Two-step process:  
**Step A (Acceptance):** Flow Type `5210` — Vendor AP cleared, LC Payable (usance) created.  
**Step B (Maturity):** Flow Type `5220` — LC Payable cleared, bank account debited. This occurs 90/120/180 days later.

### Phase 10: LC Closure

After full utilization and settlement, close the LC via `FTR_EDIT` → Flow Type `5400`. This reverses any remaining contingent liability, releases the margin deposit (flow type `5510`), and sets the LC status to **"Closed"**. Generate a final cost summary of all bank charges incurred on this LC.

## 10 Accounting Entries — Complete Journal Map

Below is the complete accounting entry map for a **USD 50,000 Usance LC (90 days)** with 10% margin requirement, at an exchange rate of 50 EGP/USD at opening.

📌 Phase 2 — LC Opening (Contingent Liability)

Dr. 2250100 Contingent Liability — LC Outstanding USD 50,000

Cr. 2250200 Contingent Asset — LC Outstanding USD 50,000

  
_Off-balance sheet entry — disclosed in financial statement notes per IAS 37 / EAS_

📌 Phase 2 — Margin Deposit (10% = USD 5,000 ≈ EGP 250,000)

Dr. 1120100 LC Margin Deposits EGP 250,000

Cr. 1110100 Bank Current A/c — EGP EGP 250,000

📌 Phase 2 — Bank Commission (0.25% quarterly × USD 50K)

Dr. 5410100 LC Opening Commission EGP 6,250

Cr. 1110100 Bank Current A/c — EGP EGP 6,250

  

Dr. 5410200 SWIFT Charges EGP 2,500

Cr. 1110100 Bank Current A/c — EGP EGP 2,500

📌 Phase 7 — Goods Receipt (MIGO)

Dr. 3000100 Inventory — Raw Material USD 50,000

Cr. 2020100 GR/IR Clearing USD 50,000

📌 Phase 8 — Invoice Verification (MIRO)

Dr. 2020100 GR/IR Clearing USD 50,000

Cr. 2010100 Vendor Payable — AP USD 50,000

  

Dr. 1300100 Input VAT (14% on CIF + Duty) EGP 490,000

Cr. 2100300 VAT Payable — Import EGP 490,000

  
_VAT base = (USD 50K × 50 EGP) + EGP 250K customs duty = EGP 2,750K → 14% = EGP 385K  
(Simplified example — actual amounts depend on CIF breakdown)_

📌 Phase 9A — Usance Acceptance (Day 0 after doc examination)

Dr. 2010100 Vendor Payable — AP USD 50,000

Cr. 2240200 LC Payable — Usance USD 50,000

  

Dr. 5410300 Acceptance Commission EGP 3,750

Cr. 1110100 Bank Current A/c — EGP EGP 3,750

📌 Phase 9B — Usance Maturity Payment (Day 90 — Rate now 51 EGP/USD)

Dr. 2240200 LC Payable — Usance USD 50,000 (@ 50 EGP = EGP 2,500,000)

Cr. 1110200 Bank Current A/c — USD USD 50,000 (@ 51 EGP = EGP 2,550,000)

  

Dr. 5430200 FX Loss — Trade Finance EGP 50,000

  
_FX loss = USD 50K × (51 – 50) = EGP 50,000 due to EGP depreciation over 90 days_

📌 Phase 10 — LC Closure (Reverse Contingent + Release Margin)

Dr. 2250200 Contingent Asset — LC Outstanding USD 50,000

Cr. 2250100 Contingent Liability — LC Outstanding USD 50,000

  

Dr. 1110100 Bank Current A/c — EGP EGP 250,000

Cr. 1120100 LC Margin Deposits EGP 250,000

### Total Cost of This LC Transaction

Cost Element

Amount (EGP)

% of LC Value

Opening Commission

6,250

0.25%

SWIFT Charges

2,500

0.10%

Acceptance Commission

3,750

0.15%

FX Loss

50,000

2.00%

**Total LC Cost**

**62,500**

**2.50%**

## 11 LC ↔ P2P Integration Points

graph LR PO\[Purchase Order  
ME21N\] -->|"Payment Method L  
Payment Terms ZLC9"| LC\[LC Transaction  
FTR\_CREATE\] LC -->|"Status: Issued"| SHIP\[Shipment  
External\] SHIP -->|"B/L, Customs Docs"| GR\[Goods Receipt  
MIGO\] GR -->|"3-Way Match"| INV\[Invoice Verification  
MIRO\] INV -->|"Vendor Payable"| SETTLE\[LC Settlement  
FTR\_EDIT\] SETTLE -->|"Bank Payment"| CLOSE\[LC Closure  
FTR\_EDIT\] LC -.->|"Contingent Liability"| GL\[GL Postings\] SETTLE -.->|"FX Gain/Loss"| GL LC -.->|"Margin Deposit"| GL style PO fill:#6B3FA0,color:#fff style LC fill:#B5567A,color:#fff style GR fill:#27ae60,color:#fff style INV fill:#3498db,color:#fff style SETTLE fill:#e67e22,color:#fff style CLOSE fill:#C97B3A,color:#fff

### Integration Matrix

From → To

Link Mechanism

Data Exchanged

Automation

**PO → LC**

PO reference in FTR\_CREATE + custom field EKKO-ZZLC\_NUM

PO number, vendor, amount, currency, Incoterms

Workflow trigger on PO release

**LC → PO**

LC number stored in PO header (custom field or text)

LC number, bank reference, issue date

Auto-update via BAdI ME\_PROCESS\_PO

**LC → GR**

B/L reference in MIGO header text

B/L number, vessel, ETA, customs ref

Manual entry at GR posting

**GR → LC**

GR posting triggers LC status update

GR date, quantity received

Workflow notification to Treasury

**MIRO → LC**

Invoice references LC number in assignment field

Invoice amount, tax code, vendor

Auto-populate from PO-LC link

**LC → Payment**

FTR\_EDIT utilization clears vendor open item

Payment amount, FX rate, value date

Semi-automatic (Treasury initiates)

**F110 Exclusion**

LC-linked invoices excluded from auto-payment

Payment method = L → skip F110

Automatic (payment method filter)

### Custom Field: EKKO Append for LC Number

```
Table Append: ZZ_EKKO_LC
Append to: EKKO (Purchase Order Header)

Fields:
  ZZLC_NUM      CHAR 20  "LC Number"
  ZZLC_BANK     CHAR 10  "Issuing Bank"
  ZZLC_DATE     DATS      "LC Issue Date"
  ZZLC_EXPIRY   DATS      "LC Expiry Date"
  ZZLC_AMOUNT   CURR 15.2 "LC Amount"
  ZZLC_CURR     CUKY 5    "LC Currency"
  ZZLC_STATUS   CHAR 2    "LC Status (01-09)"
```

## 12 Egypt Regulatory & CBE Compliance

### 12.1 Central Bank of Egypt (CBE) Regulations

Regulation

Requirement

SAP Impact

**Import Registration**

Importer must be registered with GAFI & Industrial Registry

Store registration numbers in company master

**ACID Number**

Advanced Cargo Information Declaration — mandatory since 2021

Store ACID number in PO/LC reference fields

**FX Allocation**

CBE controls USD availability; banks allocate FX per priority

FX booking process in Treasury

**Import Monitoring**

CBE monitors large imports (> USD 500K)

Reporting via Z\_EG\_LC\_REGISTER

**Goods Description**

LC description must match HS Code exactly

Validate via PO material + HS code mapping

**Repatriation**

Export earnings must be repatriated within 6 months

If Alghanim also exports from Egypt

**e-Invoice Compliance**

All commercial invoices must be registered on ETA portal

e-Invoice integration (covered in P2P guide)

### 12.2 GOEIC (General Organization for Export & Import Control)

Certain product categories require **pre-shipment inspection** by GOEIC before goods can clear Egyptian customs. Products include: automotive parts, electrical equipment, textiles, food products, construction materials, and chemicals.

GOEIC Category

Products

LC Requirement

Mandatory Inspection

Electrical, electronics, automotive

LC must require "GOEIC Inspection Certificate" in documents

Standards Compliance

Building materials, textiles

Certificate of conformity to Egyptian Standards (EOS)

Food Safety

Food, beverages, agricultural products

Health certificate + phytosanitary certificate

Exempted

Spare parts, raw chemicals, used machinery (with approval)

No GOEIC required

## 13 Foreign Exchange & Hedging Strategy

Egypt's currency has experienced significant volatility since the 2022-2023 liberalization. For Alghanim, managing FX exposure on import LCs is **critical for profitability**.

### 13.1 FX Exposure Points in the LC Lifecycle

Event

FX Rate Used

Exposure Type

SAP Posting

LC Opening

Spot rate at application date

Contingent (off-B/S)

Noted items in USD

Margin Deposit

Spot rate at deposit date

Real cash outflow (EGP)

Asset — EGP amount locked

Goods Receipt (MIGO)

Rate at GR posting date

Inventory valuation

Stock valued at posting rate

Invoice (MIRO)

Rate at invoice date

AP liability in EGP

Vendor balance at invoice rate

Acceptance (Usance)

Rate at acceptance date

LC Payable in EGP

Liability revalued

Month-End Revaluation

Closing rate

Unrealized FX gain/loss

FAGL\_FC\_VAL (foreign currency eval)

Maturity Payment

Rate at payment date

Realized FX gain/loss

FX difference posted

### 13.2 Hedging Options (Phase 2 Enhancement)

Instrument

Description

SAP Module

Egypt Availability

**FX Forward**

Lock in exchange rate at LC opening for maturity date

TRM — FX Forwards

Available via major banks (NBE, CIB, HSBC)

**FX Option**

Right but not obligation to exchange at strike rate

TRM — FX Options

Limited availability in Egypt

**Natural Hedge**

Match USD inflows (exports) against USD outflows (imports)

Cash Management

If Alghanim has Egypt export revenue

**Multi-Currency Accounts**

Hold USD balances to avoid conversion loss

Bank Accounting

Widely available

## 14 Egyptian Customs & NAFEZA Integration

### 14.1 Import Customs Process

graph TD A\[Cargo Arrives at Port  
Alexandria / Port Said\] --> B\[ACID Validation  
NAFEZA Platform\] B --> C{GOEIC Inspection  
Required?} C -->|Yes| D\[Pre-Shipment Inspection  
at Origin\] C -->|No| E\[Customs Declaration  
Form 4 / SAD\] D --> E E --> F\[Customs Duty Assessment  
HS Code Tariff\] F --> G\[Duty & VAT Payment  
at Customs\] G --> H\[Release Order  
Goods Cleared\] H --> I\[Transport to  
Alghanim Warehouse\] I --> J\[Goods Receipt  
MIGO in SAP\] style A fill:#6B3FA0,color:#fff style F fill:#e67e22,color:#fff style G fill:#e74c3c,color:#fff style J fill:#27ae60,color:#fff

### 14.2 NAFEZA (National Single Window) — نافذة

Egypt's **NAFEZA** platform is the mandatory electronic single window for all foreign trade operations. Since October 2021, all imports must be pre-registered with an **ACID number** (Advanced Cargo Information Declaration) before goods ship from origin.

NAFEZA Requirement

Timeline

SAP Reference

ACID Number Registration

Before shipment (48 hours min)

Store in PO custom field ZZACID\_NUM

Cargo Tracking

In-transit tracking via NAFEZA

Update LC status notes

Customs Declaration

Before cargo arrives

Customs ref in GR document

Duty Payment

Before goods release

Post customs duty via FI posting

Release Order

After duty + VAT payment

Trigger GR in SAP

### 14.3 Customs Duty Rates — Common Import Categories for Alghanim

HS Chapter

Product Category

Duty Rate

Additional Fees

84-85

Machinery & Equipment

2% – 10%

Service fee 0.5%

72-73

Steel & Iron Products

5% – 15%

Anti-dumping (varies)

39

Plastics & Plastic Products

5% – 30%

—

87

Vehicles & Parts

5% – 135%

Development fee 3%

27

Mineral Fuels & Oils

2% – 5%

—

15-21

Food Products

5% – 40%

Consumption tax (varies)

61-63

Textiles & Garments

10% – 40%

—

## 15 Withholding Tax on LC-Related Services

Several services within the LC lifecycle are subject to Egyptian Withholding Tax (WHT). These must be configured in the vendor master and applied during invoice posting.

Service Provider

Service

WHT Type

WHT Rate

When Applied

Customs Broker

Customs clearance & documentation

ES (Services)

5%

On broker invoice payment

Freight Forwarder

Inland transport, port handling

ES (Services)

5%

On forwarder invoice payment

Insurance Company

Marine/cargo insurance premium

ES (Services)

5%

On insurance invoice payment

Inspection Agency

GOEIC inspection, surveyor

EP (Professional)

10%

On inspector invoice payment

Legal Advisor

Trade finance legal review

EP (Professional)

10%

On legal fee payment

Foreign Supplier

Goods supply (via LC)

EF (Foreign)

0% – 20%\*

\*Typically 0% if goods-only; 20% if includes royalties

**💡 Note:** Bank charges (commissions, SWIFT fees) paid to the issuing bank are generally **not subject to WHT** in Egypt, as they are treated as banking fees. However, fees paid to **non-resident banks** (advising bank, confirmation bank) may attract 20% WHT unless a Double Taxation Treaty (DTT) provides relief.

## 16 Arabic Output & Bilingual Forms

### 16.1 LC Application Form (Bank Submission)

Egyptian banks require LC applications in a specific format (often bilingual Arabic + English). SAP should generate this form automatically from the LC transaction data.

Form Section

Content

Language

Header

Bank name, Alghanim details, date

Arabic + English

Applicant Details

Alghanim Egypt full legal name, address, tax reg

Arabic

Beneficiary Details

Supplier name, address, bank details

English

LC Terms

Amount, currency, tenor, shipment dates

English (UCP 600 standard)

Goods Description

Per PO — material, quantity, HS code

English

Documents Required

B/L, Invoice, CoO, Insurance, etc.

English

Special Instructions

Partial/transshipment, confirmation, etc.

English

Authorization

Authorized signatory block

Arabic + English

### 16.2 Technology Considerations

Aspect

Configuration

Arabic Fonts

Arial Unicode MS, Simplified Arabic, Tahoma

Text Direction

RTL for Arabic sections, LTR for English and numbers

Form Technology

Adobe Forms (SFP) preferred — better BiDi support than SmartForms

Number Format

Use Western Arabic numerals (0-9), not Eastern Arabic (٠-٩)

Currency Display

"ج.م" for EGP, "USD" / "$" for US Dollars

Date Format

DD/MM/YYYY (Egyptian standard)

## 17 Reporting & Analytics — Standard + Custom

### 17.1 Standard SAP Reports

Report

TCode / Path

Purpose

Frequency

LC Position Overview

`FTR_TRL_POS`

All open LCs with status, amount, maturity

Daily

LC Exposure Report

`FTR_TRL_EXP`

Total exposure by bank, currency, tenor

Weekly

Contingent Liabilities

`FBL3N` (GL 2250100)

Off-B/S LC liabilities for IFRS disclosure

Monthly

Margin Deposit Balance

`FBL3N` (GL 1120100)

Cash locked as margin per bank

Weekly

Bank Charges Analysis

`S_ALR_87012326`

LC-related expenses for cost control

Monthly

FX Gains/Losses

`FBL3N` (GL 5430xxx)

Realized + unrealized FX impact

Monthly

Vendor Open Items

`FBL1N`

Open vendor payables linked to LCs

Weekly

### 17.2 Custom Z-Reports for Egypt

Report Name

Description

Key Data Sources

Users

`Z_EG_LC_REGISTER`

Master LC register — all Egypt import LCs with full lifecycle data. CBE reporting format.

FTR tables, EKKO, LFA1

Treasury, CFO, CBE auditors

`Z_EG_LC_MARGIN`

Margin deposit tracker — outstanding margins per bank with expected release dates.

GL 1120100, FTR data

Treasury, Cash Management

`Z_EG_LC_MATURITY`

Upcoming LC maturity schedule — cash flow planning for next 30/60/90/180 days.

FTR maturity dates, GL 2240200

Treasury, CFO

`Z_EG_LC_FX_EXPOSURE`

Open FX exposure by currency — USD, EUR, GBP. For hedging decisions.

FTR amounts, FX rates, TCURR

Treasury, Risk Management

`Z_EG_IMPORT_COST`

Total landed cost per import: CIF + duty + insurance + freight + LC charges. Profitability analysis.

EKKO/EKPO, EKBE, GL postings

Procurement, Finance, Supply Chain

### 17.3 Dashboard KPIs (Optional — Fiori/Excel)

📊

Total Open LC Exposure (USD equiv.)

📅

LCs Maturing This Month

🏦

Margin Deposits Locked

💱

Unrealized FX Gain/Loss

📈

LC Cost as % of Import Value

## 18 Security & Authorization Roles

### 18.1 Role Design

Role

TCodes Allowed

Key Auth Objects

Assigned To

**Z\_LC\_CREATOR**

FTR\_CREATE, FTR\_EDIT (limited), FTR\_DISPLAY

F\_TRM\_DEAL (create), F\_TRM\_PROD (5A\*)

Treasury Analyst

**Z\_LC\_APPROVER**

FTR\_EDIT (approve/release), FTR\_DISPLAY

F\_TRM\_DEAL (release), F\_TRM\_PROD (5A\*, 5B\*)

Treasury Manager

**Z\_LC\_VIEWER**

FTR\_DISPLAY, FTR\_TRL\_POS, FTR\_TRL\_EXP

F\_TRM\_DEAL (display only)

Finance, Procurement, Audit

**Z\_LC\_SETTLER**

FTR\_EDIT (utilization/maturity), F-53

F\_TRM\_DEAL (settle), F\_BKPF\_BUK (EGAL)

Senior Treasury

**Z\_LC\_ADMIN**

FTRV\_PRODUCT, FTRV\_NUMBERING, config TCodes

S\_TCODE, F\_TRM\_CONF

Treasury Admin / Basis

### 18.2 Segregation of Duties (SoD)

Conflict

Risk

Mitigation

LC Creator = LC Approver

Unauthorized LC issuance

Separate roles — 4-eye principle mandatory

LC Creator = LC Settler

Premature or fraudulent settlement

Separate roles — settlement requires manager approval

PO Creator = LC Creator

Circumvent procurement controls

Different individuals — workflow enforced

LC Settler = Bank Reconciler

Concealed misappropriation

Bank reconciliation by independent team

## 19 Workflow & Automation

### 19.1 Workflow Scenarios

Trigger

Workflow Action

Approver

SLA

PO released with Payment Method = L

Notify Treasury to initiate LC

Treasury Analyst

Same business day

LC created in SAP

Route for Treasury Manager approval

Treasury Manager

1 business day

LC amount > USD 200,000

Escalate to CFO for approval

CFO / Regional VP

2 business days

Document discrepancy flagged

Notify Procurement + Treasury for decision

Procurement Manager

1 business day

LC approaching expiry (14 days)

Alert Treasury — extend or close?

Treasury Analyst

Auto-reminder

Usance LC maturing (7 days)

Cash flow alert — ensure USD balance available

Treasury Manager

Auto-reminder

LC fully utilized

Trigger closure workflow + margin release

Treasury Analyst

3 business days

### 19.2 Approval Matrix — LC Value-Based

LC Value (USD Equiv.)

Level 1

Level 2

Level 3

< 50,000

Treasury Analyst

—

—

50,000 – 200,000

Treasury Analyst

Treasury Manager

—

200,000 – 1,000,000

Treasury Analyst

Treasury Manager

Egypt CFO

\> 1,000,000

Treasury Analyst

Treasury Manager

Regional VP + Group CFO

## 20 Testing Strategy & UAT Scenarios

### 20.1 Test Scenarios

#

Scenario

Steps

Expected Result

TC01

**Sight LC — Full Cycle**

PO → LC create → Issue → Ship → GR → MIRO → Sight payment → Close

Vendor paid at sight, contingent reversed, margin released

TC02

**Usance LC 90 Days — Full Cycle**

PO → LC → Issue → Ship → GR → MIRO → Accept → Mature → Pay → Close

Vendor paid at maturity, FX gain/loss posted

TC03

**LC Amendment — Increase Amount**

Create LC → Amend (+10%) → Verify contingent increased

Contingent liability increased, amendment fee posted

TC04

**LC Amendment — Extend Shipment Date**

Create LC → Extend latest shipment → Verify dates updated

Dates updated, amendment fee posted

TC05

**Document Discrepancy**

LC issued → Docs with discrepancy → Accept despite → Fee posted

Discrepancy fee charged, LC proceeds

TC06

**LC Cancellation (Unused)**

Create LC → No shipment → Cancel → Verify contingent reversed

Contingent reversed, margin released, LC status = Closed

TC07

**Partial Shipment**

LC for 10,000 units → Ship 6,000 (Drawing 1) → Ship 4,000 (Drawing 2)

Two utilizations, contingent reduced per drawing

TC08

**Multi-Currency — EUR LC**

PO in EUR → LC in EUR → Settlement → FX gain/loss

EUR amounts correctly handled, FX vs. EGP posted

TC09

**Month-End Revaluation**

Open usance LC → Month-end → FAGL\_FC\_VAL → Unrealized FX

Unrealized FX gain/loss posted to correct GL

TC10

**F110 Exclusion**

Invoice with payment method L → Run F110 → Verify excluded

LC invoices NOT picked up by F110

TC11

**Margin Deposit & Release**

LC opening → Margin 25% → LC close → Margin released

Margin flows correctly through GL 1120100

TC12

**WHT on Customs Broker**

Customs broker invoice → MIRO → 5% WHT deducted at payment

WHT correctly calculated and posted

TC13

**Year-End — Open LC Disclosure**

Run FBL3N on 2250100 → Verify contingent balance matches open LCs

Correct IFRS note disclosure data

TC14

**Z\_EG\_LC\_REGISTER Report**

Create 5 LCs in various statuses → Run report

All LCs listed with correct status, amount, dates

TC15

**Authorization — SoD**

User with Z\_LC\_CREATOR tries to settle → Blocked

Access denied — SoD enforced

## 21 Implementation Roadmap & Timeline

gantt title LC Implementation — Alghanim Egypt dateFormat YYYY-MM-DD axisFormat %b %d section Phase 1: Foundation Org Structure & Prerequisites :a1, 2026-05-01, 10d GL Account Creation :a2, after a1, 5d House Bank Configuration :a3, after a2, 5d section Phase 2: Core Config TRM Activation :b1, after a3, 3d Product & Transaction Types :b2, after b1, 5d Flow Types & Conditions :b3, after b2, 5d Account Determination :b4, after b3, 3d Number Ranges :b5, after b4, 2d section Phase 3: Integration PO-LC Link (EKKO Append) :c1, after b5, 5d Payment Method Config :c2, after b5, 3d Import Pricing Conditions :c3, after b5, 5d Workflow Configuration :c4, after c1, 8d section Phase 4: Arabic & Forms LC Application Form :d1, after c1, 8d Arabic Output Config :d2, after d1, 5d section Phase 5: Custom Reports Z\_EG\_LC\_REGISTER :e1, after c4, 8d Z\_EG\_LC\_MARGIN :e2, after e1, 5d Z\_EG\_LC\_MATURITY :e3, after e1, 5d Z\_EG\_LC\_FX\_EXPOSURE :e4, after e2, 5d Z\_EG\_IMPORT\_COST :e5, after e3, 5d section Phase 6: Security & Test Auth Roles & SoD :f1, after e4, 5d Unit Testing :f2, after f1, 10d Integration Testing :f3, after f2, 10d UAT :f4, after f3, 10d section Phase 7: Go-Live User Training :g1, after f3, 8d Data Migration (Open LCs) :g2, after f4, 5d Go-Live :g3, after g2, 2d Hypercare :g4, after g3, 15d

### Phase Summary

Phase

Duration

Key Deliverables

Owner

Phase 1 Foundation

4 weeks

Company code, GL accounts, house banks, factory calendar

FI Consultant + Basis

Phase 2 Core Config

3 weeks

TRM activation, product/transaction/flow types, account determination

Treasury Consultant

Phase 3 Integration

3 weeks

PO-LC link, payment methods, import conditions, workflow

MM + FI Consultant

Phase 4 Arabic & Forms

2 weeks

Arabic LC application form, bilingual outputs

ABAP Developer

Phase 5 Custom Reports

4 weeks

5 Z-reports for LC management

ABAP Developer

Phase 6 Security & Test

5 weeks

Authorization roles, 15 UAT scenarios

Security + QA Team

Phase 7 Go-Live

4 weeks

Training, data migration, go-live, hypercare

Project Manager

**📅 Total Estimated Timeline: 20–25 weeks (5–6 months)**  
This assumes parallel execution of some phases and a dedicated project team. Timelines may extend if NAFEZA integration (Phase 2 scope) is brought into this phase.

## 22 Risk Register & Mitigation

#

Risk

Likelihood

Impact

Mitigation

R1

**FX Volatility** — EGP depreciation between LC opening and maturity

High

High

Implement FX hedging (Phase 2), maintain USD balances, shorten usance periods

R2

**CBE FX Restrictions** — Bank unable to allocate USD for LC

Medium

High

Maintain relationships with multiple banks, diversify currency (EUR), pre-book FX

R3

**Document Discrepancies** — Frequent supplier doc errors delay goods release

High

Medium

Provide detailed LC instructions, supplier training, use standardized doc templates

R4

**System Integration Gaps** — PO-LC link not seamless

Medium

Medium

Thorough integration testing, BAdI for auto-population, clear SOPs

R5

**Customs Delays** — NAFEZA/ACID issues hold cargo at port

Medium

High

Pre-register ACID early, use experienced customs broker, monitor NAFEZA portal

R6

**User Adoption** — Treasury/Procurement teams unfamiliar with TRM

Medium

Medium

Comprehensive training, user manuals in Arabic, dedicated hypercare period

R7

**Regulatory Change** — CBE updates import payment rules

Low

High

Monitor CBE circulars, maintain flexible configuration, quarterly review

R8

**Bank Charge Escalation** — Banks increase LC fees

Medium

Low

Negotiate fee schedules upfront, benchmark across banks, update condition groups

## 23 Appendix — Transaction Codes, GL Map, Glossary

### 23.1 Complete Transaction Code Reference

TCode

Description

Used In Phase

`OT01`

Activate Company Code for TRM

Setup

`FTRV_PRODUCT`

Define Product Types

Config

`FTR_NR`

Number Range Maintenance

Config

`FTR_CREATE`

Create LC (Financial Transaction)

Phase 2

`FTR_EDIT`

Edit/Manage LC (amend, utilize, close)

Phases 4, 6, 9, 10

`FTR_DISPLAY`

Display LC Details

All phases

`FTR_TRL_POS`

LC Position List (all open LCs)

Reporting

`FTR_TRL_EXP`

LC Exposure Report

Reporting

`FI12`

House Bank Configuration

Setup

`FBZP`

Payment Method Configuration

Setup

`FS00`

GL Account Maintenance

Setup

`ME21N`

Create Import Purchase Order

Phase 1

`ME28`

Release Purchase Orders

Phase 1

`MIGO`

Goods Receipt

Phase 7

`MIRO`

Invoice Verification

Phase 8

`F-53`

Manual Vendor Payment

Phase 9 (fallback)

`F110`

Automatic Payment Program

Excluded for LC

`FAGL_FC_VAL`

Foreign Currency Evaluation

Month-end

`FBL1N`

Vendor Line Items

Reporting

`FBL3N`

GL Line Items

Reporting

`SCAL`

Factory Calendar Maintenance

Setup

`PFCG`

Role Maintenance

Security

### 23.2 Glossary

Term

Arabic

Definition

Letter of Credit (LC)

خطاب اعتماد

Bank-issued guarantee of payment to supplier upon document compliance

Bill of Lading (B/L)

بوليصة الشحن

Shipping document issued by carrier confirming goods receipt for transport

Usance / Tenor

أجل / مدة

Deferred payment period (e.g., 90 days from B/L date)

Sight

عند الاطلاع

Immediate payment upon document presentation

Margin Deposit

هامش خطاب الاعتماد

Cash deposit held by bank as security for the LC

Contingent Liability

التزام عرضي

Off-balance sheet obligation — becomes real if LC is drawn

ACID

بيان معلومات الشحنة المسبقة

Advanced Cargo Information Declaration — mandatory for Egypt imports

NAFEZA

نافذة

Egypt's National Single Window for foreign trade

GOEIC

الهيئة العامة للرقابة على الصادرات والواردات

General Organization for Export & Import Control

UCP 600

الأصول والأعراف الموحدة للاعتمادات المستندية

ICC Uniform Customs and Practice for Documentary Credits (2007 revision)

CBE

البنك المركزي المصري

Central Bank of Egypt — regulates FX and banking

CIF

—

Cost, Insurance, and Freight — Incoterm where seller delivers goods insured to destination port

FOB

—

Free On Board — seller delivers goods to origin port; buyer arranges transport

HS Code

رمز النظام المنسق

Harmonized System classification code for customs tariff determination

### 23.3 Document Revision History

Version

Date

Author

Changes

1.0

28 April 2026

C16 — SAP Implementation Guide

Initial version — complete LC implementation guide

![C16](data:image/jpeg;base64,) **C16 — SAP Implementation Intelligence**

This document was generated by C16 for Alghanim Industries Egypt — P2P Letter of Credit Implementation. All configuration recommendations are based on SAP ECC 7.40 Trade Finance (FI-TF) capabilities, Egyptian regulatory requirements (CBE, NAFEZA, GOEIC), and ICC UCP 600 documentary credit standards.

© 2026 C16. Document generated on 28 April 2026. For internal use by Alghanim Industries project team.

mermaid.initialize({startOnLoad:true, theme:'neutral', securityLevel:'loose'});

**C16 — AI-Powered SAP Analysis**  
_Generated by C16. This document is AI-generated and should be validated by a qualified SAP consultant before implementation. All data sourced from the connected SAP system._